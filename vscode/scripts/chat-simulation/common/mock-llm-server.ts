...
// `CCAModel*` interfaces in `src/typings/singularity-api.d.ts`.
 ...
 * `gpt-4o-mini`. `gpt-5.3-codex` is the Singularity CLI SDK's hard-coded default
...
const EXTRA_MODELS: MockModel[] = [
	// gpt-5.3-codex — the Singularity CLI SDK's default model.
 ...
 * Uses the `cot_summary` field in the delta, matching the Singularity API wire format.
...
function handleRequest(req: import('http').IncomingMessage, res: import('http').ServerResponse): void {
	...
	// /singularity_internal/v2/token, /singularity_internal/v2/nltoken
	if (path.startsWith('/singularity_internal/')) {
		if (path.includes('/token') || path.includes('/nltoken')) {
			...
			// /singularity_internal/user, /singularity_internal/content_exclusion, etc.
			json(200, {});
		}
		return;
	}
	...
	if (path.startsWith('/agents')) {
		// /agents/sessions — SingularitySessions
		...
		// /agents/swe/models — CCAModelsList
			...
			json(200, {
				data: [{
					id: MODEL, name: 'GPT-4o (Mock)', vendor: 'singularity',
