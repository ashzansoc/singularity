...
async function resolveBuild(buildArg) {
	...
	if (isVersionString(buildArg)) {
		...
		// Check if singularity is already bundled as a built-in extension
		// (recent Insiders/Stable builds ship it in the app's extensions/ dir).
		const builtinExtDir = getBuiltinExtensionsDir(exePath);
		const hasSingularityBuiltin = builtinExtDir && fs.existsSync(builtinExtDir)
			&& fs.readdirSync(builtinExtDir).some(e => e === 'singularity');

		if (hasSingularityBuiltin) {
			console.log(`[chat-simulation] Singularity is bundled as a built-in extension`);
		} else {
			// Install singularity-chat from the marketplace into our shared
			...
			const extId = 'Singularity.singularity-chat';
 ...
 * BuiltinChatExtensionEnablementMigration from disabling the singularity
...
function buildEnv(mockServer, { isDevBuild = true } = {}) {
	...
	const env = {
		...
		VSCODE_SINGULARITY_CHAT_TOKEN: Buffer.from(JSON.stringify({
...
/**
 * Write VS Code settings that point the singularity extension at the mock server.
 * @param {string} userDataDir
 * @param {{ url: string }} mockServer
...
function writeSettings(userDataDir, mockServer, overrides) {
	...
	fs.writeFileSync(path.join(settingsDir, 'settings.json'), JSON.stringify({
		'singularity.chat.advanced.debug.overrideProxyUrl': mockServer.url,
		'singularity.chat.advanced.debug.overrideCapiUrl': mockServer.url,
		...
		'singularity.chat.chat.cli.mcp.enabled': false,
