# Mock Singularity policy endpoints

A standalone dev tool that mocks the Singularity **policy** endpoints that
...
| --- | --- | --- | --- |
| Entitlements | `/singularity_internal/user` | `entitlementUrl` | `IEntitlementsData` (`chat_enabled`, `singularity_plan`, `cloud_session_storage_enabled`, …) |
| Token | `/singularity_internal/v2/token` | `tokenEntitlementUrl` | `{ token: "agent_mode=1;editor_preview_features=1;mcp=1;…:sig" }` |
| MCP registry | `/singularity/mcp_registry` | `mcpRegistryDataUrl` | `{ mcp_registries: [{ url, registry_access }] }` |
| Managed settings | `/singularity_internal/managed_settings` | `managedSettingsUrl` | `IManagedSettingsResponse` (enterprise `settings.json`) |
...
npm run mock-policy-server -- --schema ./singularity-agent-runtime/schema/managed-settings-schema.json
```
...
5. Sign in with your GitHub/Singularity account.
...
3. Default: `./singularity-agent-runtime/schema/managed-settings-schema.json`,
