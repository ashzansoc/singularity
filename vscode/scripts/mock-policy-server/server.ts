...
/**
 * Standalone dev tool: a local mock of the Singularity "policy" endpoints that
 ...
 * `./singularity-agent-runtime/schema/managed-settings-schema.json`. Override with
 * `--schema <url|file-uri|path>` or the `MANAGED_SETTINGS_SCHEMA` env var; web
 * (`http(s)://`) and `file://` URIs are both accepted.
 */
const DEFAULT_SCHEMA_SOURCE = 'singularity-agent-runtime/schema/managed-settings-schema.json';
...
const server = http.createServer((req, res) => {
	...
	try {
		// Mocked Singularity endpoints. Only these get permissive CORS, so the web
...
server.listen(PORT, HOST, () => {
	...
	console.log('');
	console.log('  Mock Singularity policy endpoints dev server');
	console.log('  ----------------------------------------');
