...
if defined SHOW_HELP (
	...
	echo   configuration-editing, github-authentication, singularity, css, html
	echo.
...
if defined SUITE_FILTER (
	set "_any_match="
	for %%s in (api-folder api-workspace colorize terminal-suggest typescript markdown emmet git git-base ipynb notebook-renderers configuration-editing github-authentication singularity css html) do (
		call :should_run_suite %%s && set "_any_match=1"
	)
	if not defined _any_match (
		echo Error: no suites match filter '%SUITE_FILTER%'
		echo Available suites: api-folder api-workspace colorize terminal-suggest typescript markdown emmet git git-base ipynb notebook-renderers configuration-editing github-authentication singularity css html
		exit /b 1
	)
)
...
call :should_run_suite singularity || goto skip_singularity
echo.
echo ### Singularity tests
if defined GREP_PATTERN (
	call npm run test-extension -- -l singularity --grep "%GREP_PATTERN%"
) else (
	call npm run test-extension -- -l singularity
)
if %errorlevel% neq 0 exit /b %errorlevel%
:skip_singularity
