...
function activate(context) {
	...
	// races with singularity-chat registering its configuration schema and was
	// the source of the original Chat Sessions smoke test flake.
	context.subscriptions.push(
		vscode.commands.registerCommand('smoketest.openSingularityCliChat', async () => {
			const command = 'workbench.action.chat.openNewSessionEditor.singularitycli';
			// Wait until singularity-chat is enabled and activated before invoking its
			...
			await waitForCommand('singularity.chat.debug.extensionState', 60_000);
			await vscode.commands.executeCommand('singularity.chat.debug.extensionState');
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('smoketest.openLocalChat', async () => {
			...
			// panel default is Agent Host Singularity when the agent host is enabled,
