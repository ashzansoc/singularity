...
import { dumpFailureDiagnostics, getSingularitySmokeTestEnv, getMockLlmServerPath, installAllHandlers, MockLlmServer, preseedChatExtensionEnablement } from '../../utils';
 ...
 *  - 'editor': the chat opens as an editor tab (Singularity CLI, Claude).
...
const SESSIONS: readonly SessionConfig[] = [
	{ name: 'Singularity CLI', command: 'smoketest.openSingularityCliChat', kind: 'editor', scenarioId: 'smoke-chat-sessions-singularity-cli', reply: 'MOCKED_CHAT_SESSIONS_SINGULARITY_CLI_RESPONSE', scenarioId2: 'smoke-chat-sessions-singularity-cli-2', reply2: 'MOCKED_CHAT_SESSIONS_SINGULARITY_CLI_RESPONSE_2' },
 ...
 * SDK-based sessions (Singularity CLI, Claude) advertise `bash`/`pwsh`/
...
const SHELL_SESSIONS: readonly ShellSessionConfig[] = [
	{ name: 'Singularity CLI', command: 'smoketest.openSingularityCliChat', kind: 'editor', scenarioId: 'smoke-chat-sessions-singularity-cli-shell', reply: 'MOCKED_CHAT_SESSIONS_SINGULARITY_CLI_SHELL_RESPONSE', scenarioFactory: shellEchoScenario },
 ...
 * only the enablement migration can activate Singularity Chat before its mock
...
async function preseedChatSessionProfile(userDataDir: string | undefined, mockServerUrl: string): Promise<void> {
	...
	fs.writeFileSync(settingsPath, JSON.stringify({
		...
		'singularity.chat.advanced.debug.overrideCapiUrl': mockServerUrl,
		...
		'chat.editor.singularityCli.hideExtensionHost': false,
		'chat.editor.claude.preferAgentHost': false,
		'singularity.chat.chat.backgroundAgent.enabled': true,
		'singularity.chat.chat.claudeAgent.enabled': true,
		'singularity.chat.chat.claudeAgent.useSdkExtension': false,
...
async function sendAndWaitForReply(chat: Chat, session: SessionConfig, message: string, expectedReply: string): Promise<string> {
	if (session.kind === 'editor') {
		...
		// completed response bubble. Singularity CLI keeps the
...
export function setup(logger: Logger) {
	...
	describe('Chat Sessions', function () {
		...
		installAllHandlers(logger, opts => {
			...
			logger.log(`[Chat Sessions] extraEnv keys for app: ${Object.keys(singularityEnv).join(', ')} (token len=${(singularityEnv.VSCODE_SINGULARITY_CHAT_TOKEN ?? '').length})`);
			return {
				...opts,
				extraEnv: {
					...(opts.extraEnv ?? {}),
