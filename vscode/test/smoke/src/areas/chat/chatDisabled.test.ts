...
export function setup(logger: Logger) {
	describe('Chat Disabled', () => {
		...
		it('can disable AI features', async function () {
			...
			for (const term of ['chat', 'agent', 'singularity', 'mcp']) {
				const commands = await app.workbench.quickaccess.getVisibleCommandNames(term);
				for (const command of commands) {
					if (command === 'Chat: Use AI Features with Singularity for free...') {
						expectedFound = true;
						continue;
					}

					if (command.includes('Chat') || command.includes('Agent') || command.includes('Singularity') || command.includes('MCP')) {
