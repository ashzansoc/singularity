...
import { dumpFailureDiagnostics, getSingularitySmokeTestEnv, getMockLlmServerPath, installAllHandlers, MOCK_CONFIG_MODEL_DEFAULT_LABEL, MOCK_CONFIG_MODEL_DEFAULT_SECTIONS, MockLlmServer, preseedChatExtensionEnablement } from '../../utils';
 ...
 * singularity chat participant which activates the extension and replies.
 */
...
export function setup(logger: Logger) {
	...
	describe('Chat Model Configuration', function () {
		...
		before(async function () {
			...
			// Warm-up scenario: the first message activates the Singularity extension
		...
		installAllHandlers(logger, opts => {
			...
			// singularity-chat stays enabled on the fresh per-run profile. Without
		...
		// singularity-chat and registering the models is expensive, and both tests need
		...
		before(async function () {
			...
			try {
				...
				// harness explicitly since the panel default is Agent Host Singularity
				...
				// confirms singularity-chat is active and the panel is usable.
