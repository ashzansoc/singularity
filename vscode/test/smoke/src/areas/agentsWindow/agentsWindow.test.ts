...
const SESSIONS: readonly SessionConfig[] = [
	{ name: 'Singularity', scenarioId: 'smoke-hello-singularity', reply: 'MOCKED_SINGULARITY_RESPONSE', scenarioId2: 'smoke-hello-singularity-2', reply2: 'MOCKED_SINGULARITY_RESPONSE_2' },
...
const SINGULARITY_SANDBOX_SCENARIO_ID = 'smoke-hello-singularity-sandbox';
const SINGULARITY_SANDBOX_REPLY = 'MOCKED_SINGULARITY_SANDBOX_RESPONSE';
...
// activates singularity-chat in the Agents Window exthost and registers the
...
async function preseedExtensionHostAgentsProfiles(userDataDir: string | undefined, mockServerUrl: string, additionalSettings: Record<string, string | boolean> = {}): Promise<void> {
	...
	const settings = JSON.stringify({
		'singularity.chat.advanced.debug.overrideProxyUrl': mockServerUrl,
		...
		'chat.agents.singularityCli.hideExtensionHost': false,
		'chat.agents.claude.preferAgentHost': false,
		'sessions.chat.localAgent.enabled': true,
		'singularity.chat.chat.cli.sandbox.enabled': 'on',
		'singularity.chat.chat.cli.sessionEventLogging.enabled': true,
		// Keep follow-up turns in the same chat so the test flow is deterministic.
		'sessions.singularity.chat.multiChatSessions': false,
		// Capture enough runtime detail to diagnose CI hangs.
		'chat.agentHost.singularitySdk.logLevel': 'trace',
		...additionalSettings,
	}, null, 2);
...
export function setup(logger: Logger) {
	...
	extensionSuite('Agents Window', function () {
		// Cold start of the Singularity CLI SDK (first turn) routinely takes ~60-90s
		...
		interface ShellSession {
			 ...
			 * The Agents Window suite enables the Singularity CLI sandbox at the
			 * suite level (for the "Test Singularity CLI session (sandbox)"
			 ...
			 * (61389575, 61155944) enabled, which would fail any Singularity
			 * shell-tool test on Windows. Set this for non-sandbox Singularity
		...
		const SHELL_SESSIONS: readonly ShellSession[] = [
			{
				name: 'Singularity',
				sessionType: 'Singularity',
				scenarioId: 'smoke-hello-singularity-shell',
				reply: 'MOCKED_SINGULARITY_SHELL_RESPONSE',
				scenarioFactory: shellEchoScenario,
				disableCliSandbox: true,
				// Confirm the shell tool actually executed by checking the
				// SingularityCLISession diagnostic log. We don't care whether
				// the command was sandboxed for this test.
				extraAssertion: async (app) => {
					const chatLogPath = path.join(app.logsPath, 'window2', 'exthost', 'Singularity.singularity-chat', 'GitHub Singularity Chat.log');
					const chatLog = await fs.promises.readFile(chatLogPath, 'utf8');
					assert.match(
						chatLog,
						/\[SingularityCLISession\] tool\.execution_complete /,
		...
		before(async function () {
			...
			registerScenario(SINGULARITY_SANDBOX_SCENARIO_ID, shellEchoScenario(SINGULARITY_SANDBOX_REPLY));
		...
		installAllHandlers(logger, opts => {
			const singularityEnv = getSingularitySmokeTestEnv(mockServer, { userDataDir: opts.userDataDir });
			logger.log(`[Agents Window] XDG_STATE_HOME=${singularityEnv.XDG_STATE_HOME ?? '<unset>'}`);
			logger.log(`[Agents Window] extraEnv keys for app: ${Object.keys(singularityEnv).join(', ')} (token len=${(singularityEnv.VSCODE_SINGULARITY_CHAT_TOKEN ?? '').length})`);
			return {
				...opts,
				extraEnv: {
					...(opts.extraEnv ?? {}),
			...
			it(`Test ${session.name} session`, async function () {
				...
				try {
					...
					if (!session.skipReply2) {
						// Singularity CLI: after a request completes, the Agents Window
						...
						// For Singularity CLI, pass the expected active label so
						...
						const expectedActiveLabel = session.name === 'Singularity' ? session.reply : undefined;
						const activeRowMatch = session.name === 'Singularity' ? [firstPrompt, session.reply] : undefined;
						...
						const secondTurnTimeout = session.name === 'Singularity' ? 180_000 : 60_000;
					...
					logger.log(`[Agents Window/Singularity] FAILURE: ${error instanceof Error ? error.stack ?? error.message : String(error)}`);
					logger.log(`[Agents Window/Singularity] mock server requestCount at failure: ${mockServer.requestCount()}`);
					await dumpFailureDiagnostics(app, logger, 'Agents Window/Singularity', { sendButtonSelector: AGENTS_SEND_BUTTON_SELECTOR });
					throw error;
				}
			});
		}

		it('Test Singularity CLI session (sandbox)', async function () {
			...
			//   Singularity.singularity-chat/GitHub Singularity Chat.log` — the
			//   SingularityCLISession diagnostic log: `[sandboxSpawn]` lines from
			//   the runtime and `[SingularityCLISession] tool.execution_complete
			//   ... success=… sandboxed=… [error=…] content=…` lines from
			//   `_logSessionEvent` (gated by the
			//   `singularity.chat.chat.cli.sessionEventLogging.enabled`
			//   setting that this suite sets in the `before` hook).
			// - `smoke-tests-electron/<N>_suite_Agents_Window/playwright-screenshot-
			//   *-Test_Singularity_CLI_session*.png` — last-frame screenshot of
			...
			await app.workbench.agentsWindow.startNewSession();
			await app.workbench.agentsWindow.waitForNewSessionView();
			await app.workbench.agentsWindow.selectSessionType('Singularity');

			const requestsBefore = mockServer.requestCount();
			await app.workbench.agentsWindow.submitNewSessionPrompt(`hello world [scenario:${SINGULARITY_SANDBOX_SCENARIO_ID}]`);
			...
			const text = await app.workbench.agentsWindow.waitForAssistantText(shellEchoResponseMatcher(SINGULARITY_SANDBOX_REPLY), 120_000);
			logger.log(`Agents Window (Singularity sandbox) response: ${text}`);

			assert.ok(
				mockServer.requestCount() > requestsBefore,
				'expected the mock LLM server to have received a new request from the Singularity sandbox session'
			);
			...
			assert.match(
				chatLog,
				/\[SingularityCLISession\] tool\.execution_complete .* sandboxed=true/,
			...
			it(`Test ${shellSession.name} session run in terminal`, async function () {
				...
				try {
					if (shellSession.disableCliSandbox) {
						...
						await overrideUserSettingOnDisk(app, 'singularity.chat.chat.cli.sandbox.enabled', 'off');
					}
					await app.workbench.agentsWindow.startNewSession();
					await app.workbench.agentsWindow.waitForNewSessionView();
	...
	extensionSuite('Agents Window (model configuration)', function () {
		// Cold start of the Local session's singularity-chat exthost plus model
		...
		installAllHandlers(logger, opts => {
			...
			'singularity.chat.advanced.debug.overrideCapiUrl': getMockLlmServerUrl(mockServer),
			'chat.mcp.discovery.enabled': false,
			'chat.mcp.enabled': false,
			'singularity.chat.chat.responsesApiContextManagement.enabled': true,
		...
		// the Local session's singularity-chat exthost and registering the models is
		...
		before(async function () {
			...
			try {
				...
				// Warm up: the first Local message activates singularity-chat in the
	...
	describe('Agents Window (local AgentHost)', () => {
		...
		it('Test Singularity CLI session via AgentHost', async function () {
			this.timeout(5 * 60 * 1000);
			...
			try {
				...
				// the renderer-side Singularity Chat extension fallback) by checking
		...
		it('Test Singularity CLI session via AgentHost (sandbox)', async function () {
			// See the Singularity CLI sandbox test above for the rationale on
			...
			// SingularityAgentSession when it auto-approves a sandboxed shell call.
			if (process.platform === 'win32') {
				this.skip();
			}
			...
			try {
				...
				assert.match(
					agentHostLog,
					/\[Singularity:[^\]]+\] Auto-approving sandboxed shell command for tool call /,
	...
	describe('Agents Window (local AgentHost, SDK sandbox)', () => {
		...
		// (mirroring how the Singularity extension configures the CLI sandbox), so
		...
		it('Test Singularity CLI session via AgentHost (SDK sandbox)', async function () {
			// See the Singularity CLI sandbox test above for the rationale on
			// platform gating and where to find logs when debugging CI runs.
			// The AgentHost-side sandbox log we assert on is
			// `<logsPath>/agenthost.log` (the utility-process log), produced by
			...
			try {
				...
				const agentHostLogPath = path.join(agentHost.logsPath, 'agenthost.log');
				const autoApprove = /\[Singularity:[^\]]+\] Auto-approving sandboxed shell command for tool call /;
				const agentHostLog = await waitForLogContent(() => readFileIfExists(agentHostLogPath), autoApprove);
				assert.match(
					agentHostLog,
					/\[Singularity:[^\]]+\] Applied SDK sandboxConfig via session\.options\.update/,
 ...
 * cold-start "No model available" race (github/singularity-agent-runtime#9876):
 ...
 * Agent Host Singularity selected so the caller can submit the real prompt
	...
	before(async function () {
		...
		this.app = createApp(defaultOptions, opts => ({
			...
			extraEnv: {
				...
				SINGULARITY_ENABLE_ALT_PROVIDERS: 'true',
				SINGULARITY_API_URL: getMockLlmServerUrl(mockServer),
				SINGULARITY_DEBUG_GITHUB_API_URL: getMockLlmServerUrl(mockServer),
				GITHUB_SINGULARITY_API_TOKEN: 'smoketest-fake-agent-host-token',
		...
		if (userDataDir) {
			const settings = JSON.stringify({
				'singularity.chat.advanced.debug.overrideProxyUrl': getMockLlmServerUrl(mockServer),
				...
				// Verbose Singularity runtime logging for capturable failure diagnostics.
				'chat.agentHost.singularitySdk.logLevel': 'trace',
				...config.settings,
			}, null, 2);
			for (const settingsPath of [
