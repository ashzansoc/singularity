 ...
 * singularity-chat extension on a fresh per-run profile. Without this, the first
...
export function buildSingularityChatToken(mockUrl: string): string {
	return Buffer.from(JSON.stringify({
		...
		individual: true,
		isNoAuthUser: true,
		singularity_plan: 'free',
		organization_login_list: [],
		endpoints: { api: mockUrl, proxy: mockUrl },
	})).toString('base64');
}

export function getSingularitySmokeTestEnv(mockServer?: MockLlmServer, opts?: { userDataDir?: string }): Readonly<Record<string, string | undefined>> {
	// When `userDataDir` is provided, isolate the Singularity CLI session store
	// from the user's real `~/.singularity/` by pointing `XDG_STATE_HOME` at a
	// sibling of the per-run `userDataDir`. The extension's `getSingularityHome()`
	// / `getSingularityCLISessionStateDir()` (in
	// `extensions/singularity-chat/src/extension/chatSessions/singularitycli/node/cliHelpers.ts`)
	// and the underlying CLI SDK both anchor to `XDG_STATE_HOME/.singularity/`
	// when that env var is set, otherwise to `~/.singularity/`. Pinning it under
	// the per-run `userDataDir` means the smoke-test cleanup (which removes
	// the whole `testDataPath`) also wipes the Singularity state, so repeated
	// local runs don't accumulate sessions that slow down `listSessions`
	// and other startup paths.
	let xdgStateHome: string | undefined;
	let singularityHome: string | undefined;
	if (opts?.userDataDir) {
		xdgStateHome = `${opts.userDataDir}-singularity-state`;
		// Anchor the Singularity runtime's home (`SINGULARITY_HOME`) at the same
		// `.singularity` directory the extension resolves from `XDG_STATE_HOME`,
		// so the runtime's process logs land in a known, per-run location we
		// can attach on failure (see `getSingularityRuntimeLogDir` /
		// `dumpFailureDiagnostics`). The runtime resolves its process-log dir
		// as `${SINGULARITY_HOME}/logs` and is NOT influenced by `XDG_STATE_HOME`,
		// so without this the logs would go to the agent's real `~/.singularity`.
		singularityHome = join(xdgStateHome, '.singularity');
		try {
			fs.mkdirSync(singularityHome, { recursive: true });
		} catch {
	...
	return {
		...
		//   - GITHUB_PAT switches singularityTokenManager into FixedSingularityTokenManager,
		//     skipping the real GitHub OAuth flow.
		//   - IS_SCENARIO_AUTOMATION tells the Singularity extension this is an automation run
		//     so it suppresses sign-in prompts and uses NoAuth paths.
		//   - VSCODE_SINGULARITY_CHAT_TOKEN is a fake token whose endpoints.api/proxy
		//     point at the mock LLM server.
		GITHUB_PAT: 'smoketest-fake-pat',
		IS_SCENARIO_AUTOMATION: '1',
		VSCODE_SINGULARITY_CHAT_TOKEN: mockServer ? buildSingularityChatToken(getMockLlmServerUrl(mockServer)) : undefined,
		XDG_STATE_HOME: xdgStateHome,
		SINGULARITY_HOME: singularityHome,
	};
}

/**
 * The directory that holds the Singularity runtime's `process-*.log` files for a
 * smoke run, or `undefined` when this run did not pin a per-run `SINGULARITY_HOME`.
 *
 * The runtime writes its process logs to `${SINGULARITY_HOME}/logs`, and
 * `getSingularitySmokeTestEnv` pins `SINGULARITY_HOME` for the run. Diagnostics resolve
 * the directory from the *exact* `SINGULARITY_HOME` the app launched with (read back
 ...
 * ambient `~/.singularity/logs` — on a reused CI agent that could surface an
 * unrelated session's trace log (session/model/auth diagnostics), which we must
 * never copy into an uploaded artifact.
 */
export function getSingularityRuntimeLogDir(singularityHome: string | undefined): string | undefined {
	return singularityHome ? join(singularityHome, 'logs') : undefined;
}
...
/**
 * Best-effort failure diagnostics for the Singularity smoke tests.
 *
 ...
 *  - The tail (last 80 lines) of the `GitHub Singularity Chat.log` extension
 *    host log for every `window*` directory under the suite logs that has
 *    an `exthost/Singularity.singularity-chat/` subfolder. This surfaces extension-side
 *    errors directly in the runner log so a CI failure does not require
 *    downloading the per-platform `logs-*-*-1` artifact.
 *  - The Singularity runtime (`@github/copilot` CLI) `process-*.log` files: the
 *    most recent are copied into `<logsPath>/singularity-runtime-logs/` (so they
...
export async function dumpFailureDiagnostics(
	...
	// 3. Tail the Singularity Chat extension log for every window in this suite.
	try {
		...
		for (const w of windowDirs) {
			const chatLogPath = join(logsPath, w, 'exthost', 'Singularity.singularity-chat', 'GitHub Singularity Chat.log');
			try {
				const stat = await fs.promises.stat(chatLogPath);
				const content = await fs.promises.readFile(chatLogPath, 'utf8');
				const lines = content.split(/\r?\n/);
				const tail = lines.slice(-80);
				logger.log(`[${label}] --- BEGIN ${w}/GitHub Singularity Chat.log (size=${stat.size}; last ${tail.length} of ${lines.length} lines) ---`);
				for (const ln of tail) {
					logger.log(`[${label}] | ${ln}`);
				}
				logger.log(`[${label}] --- END ${w}/GitHub Singularity Chat.log ---`);
			} catch {
				// File does not exist for this window (e.g. windows without the
				// Singularity Chat extension activated). That's expected — skip.
			}
		}
	} catch (err) {
	...
	// 4. Capture the Singularity runtime (`@github/copilot` CLI) process logs.
	//    These are the SDK/CLI's OWN diagnostics — the single most useful
	//    signal when a Singularity-runtime session hangs or times out, which the
	//    mocha "Timed out waiting for response" failure on its own does not
	//    explain. The runtime writes `process-*.log` files to `${SINGULARITY_HOME}/logs`;
	//    we resolve that from the exact `SINGULARITY_HOME` the app launched with (see
	//    `getSingularityRuntimeLogDir`).
	//    NOTE: every Singularity-runtime session spawns this runtime, but the detail
	//    differs. Agent Host sessions (Agents Window / local AgentHost) write a
	//    full, verbose account (run at `trace`). The Chat Sessions editor (Singularity
	...
	//    `logService` (the `GitHub Singularity Chat.log` tailed in step 3), so their
	...
	try {
		const logDir = getSingularityRuntimeLogDir(app.extraEnv?.SINGULARITY_HOME);
		if (!logDir) {
			logger.log(`[${label}] Singularity runtime logs unavailable (SINGULARITY_HOME not pinned for this suite)`);
			return;
		}
		...
		try {
			...
			logger.log(`[${label}] no Singularity runtime logs under ${logDir} (no Singularity CLI session spawned the runtime for this suite)`);
			return;
		}
		...
		if (collected.length === 0) {
			logger.log(`[${label}] no Singularity runtime process-*.log found under ${logDir}`);
		} else {
			...
			const runtimeLogsDest = join(logsPath, 'singularity-runtime-logs');
			try {
			...
			for (const { path: logPath } of collected.slice(0, 2)) {
				try {
					...
					logger.log(`[${label}] --- BEGIN singularity runtime ${destName} (${logPath}; last ${tail.length} of ${lines.length} lines) ---`);
					for (const ln of tail) {
						logger.log(`[${label}] # ${ln}`);
					}
					logger.log(`[${label}] --- END singularity runtime ${destName} ---`);
				} catch (readErr) {
		...
		logger.log(`[${label}] failed to capture Singularity runtime logs: ${err instanceof Error ? err.message : String(err)}`);
	}
}
