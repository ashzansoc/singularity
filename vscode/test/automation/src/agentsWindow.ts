...
export class AgentsWindow {
	...
	async switchToAgentsWindow(previousWindowCount: number, timeoutMs: number = 30_000): Promise<void> {
		...
		while (Date.now() < deadline) {
			...
			if (windows.length > previousWindowCount) {
				...
				// sleeping a fixed amount — Singularity review feedback #317545.
	...
	async selectSessionType(label: string): Promise<void> {
		...
		// providers (e.g. the AgentHost-backed Singularity CLI variant) can
		...
		// session type label like "Claude" or "Singularity CLI").
	 ...
	 * (notably Singularity CLI) where the workbench auto-creates a fresh untitled
	 ...
	 * For some providers (e.g. Singularity CLI) the "loading" class can linger
		...
		while (Date.now() < deadline) {
			...
			for (const text of candidates) {
				if (typeof predicate === 'string' ? text.includes(predicate) : predicate.test(text)) {
					...
					// in extensions (notably Singularity CLI) has post-response async
	...
	private async waitForResponseSettled(timeoutMs: number, fallbackQuietMs: number): Promise<void> {
		...
		// Loading class never cleared (e.g. Singularity CLI). Wait a bit longer
