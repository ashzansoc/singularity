...
import { checkSingularityEnginesVersion, checkNoNewJavaScriptFiles, hygiene } from './hygiene.ts';
...
const checkPackageJSONTask = task.define('check-package-json', () => {
	return gulp.src('package.json').pipe(
		es.through(function () {
			...
			const singularityError = checkSingularityEnginesVersion(repoRoot);
			if (singularityError) {
				this.emit('error', singularityError);
			}
