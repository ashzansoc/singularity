...
import { compileNonNativeExtensionsBuildTask, compileNativeExtensionsBuildTask, compileAllExtensionsBuildTask, compileExtensionMediaBuildTask, cleanExtensionsBuildTask, compileSingularityExtensionBuildTask } from './gulpfile.extensions.ts';
import { copyCodiconsTask } from './lib/compilation.ts';
import { ensureSingularityPlatformPackage, ensureSingularityRuntimeNativeAlias, getSingularityExcludeFilter, getSingularityRuntimePrebuildFiles, getSingularityTgrepExcludeFilter, getMxcExcludeFilter, getRipgrepExcludeFilter, prepareBuiltInSingularityRipgrepShim } from './lib/singularity.ts';
...
const vscodeResourceIncludes = [
	...
	'out-build/vs/workbench/contrib/welcomeGettingStarted/browser/media/singularity-logo.gif',
	'out-build/vs/workbench/contrib/welcomeGettingStarted/browser/media/singularity-logo-*.png',
	'out-build/vs/workbench/contrib/welcomeGettingStarted/browser/media/singularity-signin-bg.png',
	'out-build/vs/workbench/contrib/welcomeGettingStarted/browser/media/AwesomeLathusca.woff',
	'out-build/vs/workbench/contrib/welcomeGettingStarted/browser/media/AwesomeLathusca.ttf',
	'out-build/vs/workbench/contrib/welcomeOnboarding/browser/media/*.svg',

	// Brand marks
	'out-build/vs/workbench/browser/media/singularity-logo-*.{png,svg}',
	'out-build/vs/workbench/browser/media/singularity-type-*.png',
	'out-build/vs/workbench/browser/media/singularity-icon.png',
	'out-build/vs/workbench/browser/media/code-icon.svg',
	'out-build/vs/workbench/browser/parts/editor/media/letterpress*.svg',
...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		ensureSingularityPlatformPackage(platform, arch);
		ensureSingularityRuntimeNativeAlias(platform, arch);
		const singularityRuntimePrebuilds = gulp.src(getSingularityRuntimePrebuildFiles(platform, arch), { base: '.', dot: true, allowEmpty: true });
		...
		const deps = es.merge(cleanedDeps, singularityRuntimePrebuilds, osProxyResolverPlatformPackage)
			.pipe(filter(getSingularityExcludeFilter(platform, arch)))
			.pipe(filter(getSingularityTgrepExcludeFilter(platform, arch)))
			...
			.pipe(createAsar(path.join(process.cwd(), 'node_modules'), [
				...
				// Only the platform-specific Singularity CLI packages (`@github/copilot-<os>-<arch>`)
...
function prepareSingularityRipgrepShimTask(platform: string, arch: string, destinationFolderName: string) {
	...
	return async () => {
		...
		const builtInSingularityExtensionDir = path.join(appBase, 'extensions', 'singularity-chat');
		prepareBuiltInSingularityRipgrepShim(platform, arch, builtInSingularityExtensionDir, appNodeModulesDir);
	};
}
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		const packageTasks: task.Task[] = [
			...
			prepareSingularityRipgrepShimTask(platform, arch, destinationFolderName)
		];
		...
		if (useEsbuildTranspile) {
			...
			vscodeTask = task.define(`vscode${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(
				...
				compileNonNativeExtensionsBuildTask,
				compileSingularityExtensionBuildTask,
				compileExtensionMediaBuildTask,
				writeISODate('out-build'),
				esbuildBundleTask,
				vscodeTaskCI
