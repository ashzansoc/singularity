...
async function main() {
	...
	const claudeMdLinkType = ensureAgentHarnessLink(path.join('..', '.github', 'singularity-instructions.md'), claudeMdLink);
	if (claudeMdLinkType !== 'existing') {
		log('.', `Created ${claudeMdLinkType} .claude/CLAUDE.md -> .github/singularity-instructions.md`);
	}
	...
	for (const dir of ['', 'remote']) {
		const sessionFile = path.join(root, dir, 'node_modules', '@github', 'singularity-sdk', 'dist', 'session.js');
