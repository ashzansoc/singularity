...
/**
 * Checks that engines.vscode in extensions/singularity-chat/package.json matches ^{version} from the root package.json.
 * Returns an error message if mismatched, or undefined if OK.
 */
export function checkSingularityEnginesVersion(repoRoot: string): string | undefined {
	const rootPkg = JSON.parse(fs.readFileSync(path.join(repoRoot, 'package.json'), 'utf8'));
	const singularityPkg = JSON.parse(fs.readFileSync(path.join(repoRoot, 'extensions/singularity-chat/package.json'), 'utf8'));
	const expected = `^${rootPkg.version}`;
	const actual = singularityPkg?.engines?.vscode;
	if (actual !== expected) {
		return `engines.vscode in 'extensions/singularity-chat/package.json' must be "${expected}" (the version from the root package.json), but found "${actual ?? '<missing>'}"`;
	}
	return undefined;
}
	...
	if (process.argv.length > 2) {
		...
		cp.exec(
			...
			(err, out) => {
				...
				if (some.length > 0) {
					// Check singularity engines.vscode version if relevant files are staged
					if (some.some(f => f === 'package.json' || f.startsWith('extensions/singularity-chat/'))) {
						const singularityError = checkSingularityEnginesVersion(process.cwd());
						if (singularityError) {
							console.error(singularityError);
							process.exit(1);
						}
					}
