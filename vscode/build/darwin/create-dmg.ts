...
async function main(buildDir?: string, outDir?: string): Promise<void> {
	...
	if (product.applicationName === 'code-oss') {
		title = 'Code OSS';
		switch (quality) {
			case 'stable':
				title = 'VS Code';
	...
	// Gatekeeper helpers for ad-hoc signed builds (no Apple notarization).
	// .command may also be blocked when the DMG is quarantined (e.g. WhatsApp);
	// the README.txt always opens and has the Terminal workaround.
	const installerName = 'Install Singularity.command';
	const installerSrc = path.join(root, '..', 'scripts', installerName);
	const installerPath = path.join(outDir, installerName);
	const readmeName = 'HOW TO OPEN.txt';
	const readmeSrc = path.join(root, '..', 'scripts', readmeName);
	const readmePath = path.join(outDir, readmeName);
	if (!fs.existsSync(installerSrc)) {
		throw new Error(`Missing installer helper: ${installerSrc}`);
	}
	if (!fs.existsSync(readmeSrc)) {
		throw new Error(`Missing DMG readme: ${readmeSrc}`);
	}
	fs.copyFileSync(installerSrc, installerPath);
	fs.chmodSync(installerPath, 0o755);
	fs.copyFileSync(readmeSrc, readmePath);
	...
	settingsContent = settingsContent
		...
		.replace('{{INSTALLER_PATH}}', JSON.stringify(installerPath))
		.replace('{{INSTALLER_NAME}}', JSON.stringify(installerName))
		.replace('{{README_PATH}}', JSON.stringify(readmePath))
		.replace('{{README_NAME}}', JSON.stringify(readmeName));
