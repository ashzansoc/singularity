...
/**
 * Compiles the built-in singularity extension for the build.
 * Used by non-CI local builds where singularity is not downloaded as a VSIX.
 */
export const compileSingularityExtensionBuildTask = task.define('compile-singularity-chat-extension-build', () => ext.packageSingularityExtensionStream(false).pipe(gulp.dest('.build')));
task.task(compileSingularityExtensionBuildTask);
