...
import { cleanExtensionsBuildTask, compileNonNativeExtensionsBuildTask, compileNativeExtensionsBuildTask, compileExtensionMediaBuildTask, compileSingularityExtensionBuildTask } from './gulpfile.extensions.ts';
...
function packageTask(type: string, platform: string, arch: string, sourceFolderName: string, destinationFolderName: string) {
	...
	return () => {
		...
		ensureSingularityPlatformPackage(platform, arch, 'remote/node_modules');
		ensureSingularityRuntimeNativeAlias(platform, arch, 'remote/node_modules');
		const singularityRuntimePrebuilds = gulp.src(getSingularityRuntimePrebuildFiles(platform, arch, 'remote/node_modules'), { base: 'remote', dot: true, allowEmpty: true });
		const deps = es.merge(cleanedDeps, singularityRuntimePrebuilds)
...
function prepareSingularityRipgrepShimTaskREH(platform: string, arch: string, destinationFolderName: string) {
	return async () => {
		const outputDir = path.join(BUILD_ROOT, destinationFolderName);
		const nodeModulesDir = path.join(outputDir, 'node_modules');

		const builtInSingularityExtensionDir = path.join(outputDir, 'extensions', 'singularity-chat');
		prepareBuiltInSingularityRipgrepShim(platform, arch, builtInSingularityExtensionDir, nodeModulesDir);
	};
}
	...
	BUILD_TARGETS.forEach(buildTarget => {
		...
		['', 'min'].forEach(minified => {
			...
			const packageTasks: task.Task[] = [
				...
				prepareSingularityRipgrepShimTaskREH(platform, arch, destinationFolderName)
			];
