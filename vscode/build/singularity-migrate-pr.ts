...
// Migrates a pull request from microsoft/vscode-singularity-chat to microsoft/vscode.
//
// The diff is fetched, file paths are rewritten to prepend `extensions/singularity-chat/`,
// and a new PR is created in microsoft/vscode via the `gh` CLI.
//
// Usage:
//   node build/singularity-migrate-pr.ts <PR_NUMBER> [--dry-run] [--verbose]
//
// Requirements:
...
const SOURCE_REPO = 'microsoft/vscode-singularity-chat';
const TARGET_REPO = 'microsoft/vscode';
const PATH_PREFIX = 'extensions/singularity-chat';
...
function parseArgs(): Options {
	...
	if (!prNumber) {
		console.error('Usage: node build/singularity-migrate-pr.ts <PR_NUMBER> [--dry-run] [--verbose]');
		process.exit(1);
	}
...
function getMigrationBranchName(prNumber: number): string {
	return `vscode-singularity-chat/migrate-${prNumber}`;
}
...
/**
 * Rewrites file paths in a unified diff to prepend `extensions/singularity-chat/`.
 *
 * Handles:
...
async function createBranchAndApplyCommits(
	...
	for (let i = 0; i < commits.length; i++) {
		...
		const tmpPatch = path.join(os.tmpdir(), `singularity-migrate-pr-${prNumber}-${i + 1}.patch`);
