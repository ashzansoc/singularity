...
function fromLocal(extensionPath: string, forWeb: boolean, _disableMangle: boolean): Stream {
	...
	// Fallback: check for .esbuild.mts/.esbuild.ts (used by extensions with their own build system, e.g. singularity)
...
/**
 * Package the built-in singularity extension specifically.
 * This is used by non-CI local builds where singularity is not downloaded as a VSIX
 * but must be compiled from source and included in the build.
 */
export function packageSingularityExtensionStream(disableMangle: boolean): Stream {
	const extensionPath = path.join(root, 'extensions', 'singularity-chat');
	...
	const localExtensionsStream = minifyExtensionResources(
		fromLocal(extensionPath, false, disableMangle)
			.pipe(rename(p => p.dirname = `extensions/singularity-chat/${p.dirname}`))
	);

	const productionDependencies = getProductionDependencies('extensions/singularity-chat');
	const dependenciesSrc = productionDependencies.map(d => path.relative(root, d)).map(d => [`${d}/**`, `!${d}/**/{test,tests}/**`]).flat();
