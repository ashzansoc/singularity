...
import { singularityPlatforms, ensureSingularityPlatformPackage, ensureSingularityRuntimeNativeAlias, getSingularityExcludeFilter, getSingularityRuntimePrebuildFiles, getMxcExcludeFilter, prepareBuiltInSingularityRipgrepShim } from '../singularity.ts';
 ...
 */
function createPinnedSingularityWin32Tarball(dir: string): { tarball: string; integrity: string } {
...
suite('singularity', () => {
	test('keeps the public singularity platform package include list scoped to the selected package', () => {
		const files = getSingularityRuntimePrebuildFiles('linux', 'x64');

		assert.deepStrictEqual(files, [
			'node_modules/@github/copilot-linux-x64/**',
			'!node_modules/@github/copilot-linux-x64/singularity',
			'!node_modules/@github/copilot-linux-x64/singularity.exe',
		...
		assertSingularityPlatformPackageIncludes(files, 'node_modules/@github/copilot-linux-x64', [
			'index.js',
			'app.js',
			'prebuilds/linux-x64/runtime.node',
			'prebuilds/linux-x64/pty.node',
		]);
		assertSingularityStandaloneExecutableExcluded(files, 'node_modules/@github/copilot-linux-x64');
		assertOptionalSingularityNativeDependenciesExcluded(files, 'node_modules/@github/copilot-linux-x64');
	});

	test('uses the linuxmusl package runtime for alpine builds', () => {
		const files = getSingularityRuntimePrebuildFiles('alpine', 'x64');

		assert.deepStrictEqual(files, [
			'node_modules/@github/copilot-linuxmusl-x64/**',
			'!node_modules/@github/copilot-linuxmusl-x64/singularity',
			'!node_modules/@github/copilot-linuxmusl-x64/singularity.exe',
		...
		assertSingularityPlatformPackageIncludes(files, 'node_modules/@github/copilot-linuxmusl-x64', [
			'index.js',
			'app.js',
			'prebuilds/linuxmusl-x64/runtime.node',
		]);
		assertSingularityStandaloneExecutableExcluded(files, 'node_modules/@github/copilot-linuxmusl-x64');
		assertOptionalSingularityNativeDependenciesExcluded(files, 'node_modules/@github/copilot-linuxmusl-x64');
	});

	test('uses the .exe package runtime for windows builds', () => {
		assert.deepStrictEqual(getSingularityRuntimePrebuildFiles('win32', 'x64'), [
			'node_modules/@github/copilot-win32-x64/**',
			'!node_modules/@github/copilot-win32-x64/singularity',
			'!node_modules/@github/copilot-win32-x64/singularity.exe',
		...
		assertSingularityPlatformPackageIncludes(getSingularityRuntimePrebuildFiles('win32', 'x64'), 'node_modules/@github/copilot-win32-x64', [
			'index.js',
			'app.js',
		...
		assertSingularityStandaloneExecutableExcluded(getSingularityRuntimePrebuildFiles('win32', 'x64'), 'node_modules/@github/copilot-win32-x64');

		assert.deepStrictEqual(getSingularityRuntimePrebuildFiles('win32', 'arm64'), [
			'node_modules/@github/copilot-win32-arm64/**',
			'!node_modules/@github/copilot-win32-arm64/singularity',
			'!node_modules/@github/copilot-win32-arm64/singularity.exe',
		...
		assertOptionalSingularityNativeDependenciesExcluded(getSingularityRuntimePrebuildFiles('win32', 'x64'), 'node_modules/@github/copilot-win32-x64');
		assertSingularityStandaloneExecutableExcluded(getSingularityRuntimePrebuildFiles('win32', 'arm64'), 'node_modules/@github/copilot-win32-arm64');
	});

	test('keeps macOS runtime prebuilds in the selected platform package', () => {
		const files = getSingularityRuntimePrebuildFiles('darwin', 'arm64');

		assertSingularityPlatformPackageIncludes(files, 'node_modules/@github/copilot-darwin-arm64', [
			'index.js',
			'app.js',
			'sdk/index.js',
			'sea-loader.js',
		...
		assertSingularityStandaloneExecutableExcluded(files, 'node_modules/@github/copilot-darwin-arm64');
		assertOptionalSingularityNativeDependenciesExcluded(files, 'node_modules/@github/copilot-darwin-arm64');
	});

	test('materializes missing target platform packages from the lockfile', () => {
		const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'vscode-singularity-platform-test-'));
		...
		try {
			...
			ensureSingularityPlatformPackage('darwin', 'x64', nodeModulesRoot, {
	...
	test('rejects a cli-native.node copy standing in for runtime.node', () => {
		const nodeModulesRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'vscode-singularity-runtime-alias-'));
		try {
			const prebuildsDir = path.join(nodeModulesRoot, '@github', 'copilot-darwin-arm64', 'prebuilds', 'darwin-arm64');
			fs.mkdirSync(prebuildsDir, { recursive: true });
			fs.writeFileSync(path.join(prebuildsDir, 'cli-native.node'), 'NATIVE');
			fs.writeFileSync(path.join(prebuildsDir, 'runtime.node'), 'NATIVE');
			assert.throws(() => ensureSingularityRuntimeNativeAlias('darwin', 'arm64', nodeModulesRoot), /cli-native\.node/);
		} finally {
			fs.rmSync(nodeModulesRoot, { recursive: true, force: true });
		}
	});

	test('excludes standalone singularity executables from the platform package dependency stream', () => {
		const files = getSingularityExcludeFilter('linux', 'x64');

		assert(files.includes('**'));
		assert(files.includes('!**/node_modules/@github/copilot-*/singularity'));
		assert(files.includes('!**/node_modules/@github/copilot-*/singularity.exe'));
	});

	test('materializes target Singularity SDK prebuilds and tgrep for the built-in extension', () => {
		const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'vscode-singularity-sdk-prebuild-test-'));
		try {
			const builtInSingularityExtensionDir = path.join(repoRoot, 'extensions', 'singularity');
			const extensionSingularityDir = path.join(builtInSingularityExtensionDir, 'node_modules', '@github', 'singularity');
			...
			fs.mkdirSync(path.join(extensionSingularityDir, 'sdk', 'prebuilds', 'linux-x64'), { recursive: true });
			fs.writeFileSync(path.join(extensionSingularityDir, 'sdk', 'prebuilds', 'linux-x64', 'runtime.node'), '');
			fs.writeFileSync(path.join(extensionSingularityDir, 'package.json'), JSON.stringify({ version: '1.0.73' }));
			...
			prepareBuiltInSingularityRipgrepShim('win32', 'x64', builtInSingularityExtensionDir, appNodeModulesDir);

			assert(fs.existsSync(path.join(extensionSingularityDir, 'sdk', 'prebuilds', 'win32-x64', 'runtime.node')));
			assert(fs.existsSync(path.join(extensionSingularityDir, 'sdk', 'prebuilds', 'win32-x64', 'conpty.node')));
			assert(fs.existsSync(path.join(extensionSingularityDir, 'sdk', 'prebuilds', 'win32-x64', 'conpty', 'OpenConsole.exe')));
			assert(!fs.existsSync(path.join(extensionSingularityDir, 'sdk', 'prebuilds', 'win32-x64', 'mediaremote-adapter')));
			assert(!fs.existsSync(path.join(extensionSingularityDir, 'sdk', 'prebuilds', 'linux-x64')));
			assert(fs.existsSync(path.join(extensionSingularityDir, 'tgrep', 'bin', 'win32-x64', 'tgrep.exe')));
			assert(fs.existsSync(path.join(extensionSingularityDir, 'sdk', 'tgrep', 'bin', 'win32-x64', 'tgrep.exe')));
			assert(fs.existsSync(path.join(extensionSingularityDir, 'sdk', 'ripgrep', 'bin', 'win32-x64', 'rg.exe')));
		} finally {
			fs.rmSync(repoRoot, { recursive: true, force: true });
		}
	});
	...
	test('materializes a version-matched native when app-root diverges from the pinned extension', () => {
		const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'vscode-singularity-sdk-pinned-test-'));
		try {
			const builtInSingularityExtensionDir = path.join(repoRoot, 'extensions', 'singularity');
			const extensionSingularityDir = path.join(builtInSingularityExtensionDir, 'node_modules', '@github', 'singularity');
			const appNodeModulesDir = path.join(repoRoot, 'node_modules');
			...
			fs.mkdirSync(path.join(extensionSingularityDir, 'sdk'), { recursive: true });
			fs.writeFileSync(path.join(extensionSingularityDir, 'package.json'), JSON.stringify({ version: '1.0.73' }));
			...
			const { tarball, integrity } = createPinnedSingularityWin32Tarball(repoRoot);
			const extensionLockfilePath = path.join(builtInSingularityExtensionDir, 'package-lock.json');
			...
			assert.strictEqual(
				fs.readFileSync(path.join(extensionSingularityDir, 'sdk', 'prebuilds', 'win32-x64', 'runtime.node'), 'utf8'),
				'EXT-NATIVE-1.0.73'
			);
			assert.strictEqual(
				fs.readFileSync(path.join(extensionSingularityDir, 'sdk', 'tgrep', 'bin', 'win32-x64', 'tgrep.exe'), 'utf8'),
				'EXT-TGREP-1.0.73'
			);
		} finally {
			fs.rmSync(repoRoot, { recursive: true, force: true });
	...
	test('refuses to ship a fetched native that does not match the pinned extension lockfile integrity', () => {
		const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'vscode-singularity-sdk-integrity-test-'));
		try {
			const builtInSingularityExtensionDir = path.join(repoRoot, 'extensions', 'singularity');
			const extensionSingularityDir = path.join(builtInSingularityExtensionDir, 'node_modules', '@github', 'singularity');
			const appNodeModulesDir = path.join(repoRoot, 'node_modules');
			...
			const { tarball } = createPinnedSingularityWin32Tarball(repoRoot);
			const extensionLockfilePath = path.join(builtInSingularityExtensionDir, 'package-lock.json');
			...
			assert.throws(() => prepareBuiltInSingularityRipgrepShim('win32', 'x64', builtInSingularityExtensionDir, appNodeModulesDir, {
				extensionLockfilePath,
				packPackage: () => tarball
			}), /integrity mismatch/);
		} finally {
	...
	test('strips all singularity platform packages for unsupported armhf builds', () => {
		assert.deepStrictEqual(
			getSingularityExcludeFilter('linux', 'armhf'),
			[
				'**',
				...singularityPlatforms.map(platform => `!**/node_modules/@github/copilot-${platform}/**`),
...
function assertSingularityPlatformPackageIncludes(patterns: string[], packageDir: string, relativeFiles: string[]): void {
...
function assertSingularityStandaloneExecutableExcluded(patterns: string[], packageDir: string): void {
	for (const executable of ['singularity', 'singularity.exe']) {
		assert(patterns.includes(`!${packageDir}/${executable}`), executable);
		assert(!matchesGlob(`${packageDir}/${executable}`, patterns), executable);
	}
}

function assertOptionalSingularityNativeDependenciesExcluded(patterns: string[], packageDir: string): void {
	...
	assert(!matchesGlob(`${packageDir}/plugins/computer-use/SingularityComputerUse.exe`, patterns), 'plugins/SingularityComputerUse.exe');
	assert(!matchesGlob(`${packageDir}/plugins/computer-use/Singularity Computer Use.app/Contents/MacOS/Singularity Computer Use`, patterns), 'plugins/Singularity Computer Use.app');
