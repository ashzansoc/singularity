...
export function createAsar(folderPath: string, unpackGlobs: string[], skipGlobs: string[], duplicateGlobs: string[], destFilename: string): NodeJS.ReadWriteStream {
	...
	// Serialize filesystem.insertFile calls. asar's insertFile awaits integrity
	// hashing before bumping `offset`, so concurrent inserts race and write
	// overlapping/wrong offsets — corrupting package.json and other files in
	// the archive (seen as "Invalid package config" on launch).
