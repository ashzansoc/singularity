...
import { DataChannelForwardingTelemetryService, forwardToChannelIf, isSingularityLikeExtension } from '../../../../../platform/dataChannel/browser/forwardingTelemetryService.js';
...
export class InlineCompletionsSource extends Disposable {
	...
	private _sendInlineCompletionsRequestTelemetry(
		...
		if (!requestResponseInfo.providers.some(p => isSingularityLikeExtension(p.providerId?.extensionId))) {
			return;
		}
		...
		const emptyEndOfLifeEvent: InlineCompletionEndOfLifeEvent = {
			...
			...forwardToChannelIf(requestResponseInfo.providers.some(p => isSingularityLikeExtension(p.providerId?.extensionId))),
