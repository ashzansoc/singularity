...
import { buildHostLocalEventsPath, SINGULARITY_CLI_EH_SCHEME, SINGULARITY_CLI_LOCAL_AH_SCHEME, getSingularityCliSessionRawId } from '../../../../workbench/contrib/chat/browser/singularityCliEventsUri.js';
...
export class SessionsManagementService extends Disposable implements ISessionsManagementService {
	...
	getSessions(): ISession[] {
		...
		return this._dedupeMigratedSingularityCliSessions(this._getMergedSessions());
	}
	...
	/**
	 * A legacy Singularity CLI session migrated in place to the agent host is briefly
	 * listed by BOTH the extension-host provider (`singularitycli:/<id>`) and the
	 * agent-host provider (`agent-host-singularitycli:/<id>`) for the same underlying
	 ...
	 */
	private _dedupeMigratedSingularityCliSessions(sessions: ISession[]): ISession[] {
		let migratedRawIds: Set<string> | undefined;
		for (const session of sessions) {
			if (session.resource.scheme === SINGULARITY_CLI_LOCAL_AH_SCHEME) {
				const rawId = getSingularityCliSessionRawId(session.resource);
				if (rawId) {
		...
		const result = sessions.filter(session => {
			// Only the legacy extension-host scheme (`singularitycli:`) denotes a stale
			// entry to drop. Remote agent-host Singularity sessions
			// (`remote-<authority>-singularitycli:`) share the `singularitycli` session type but
			// are distinct sessions that must never be deduped against a local migrated
			// id, and the migrated entry itself uses `agent-host-singularitycli:`.
			if (session.resource.scheme === SINGULARITY_CLI_EH_SCHEME) {
				const rawId = getSingularityCliSessionRawId(session.resource);
	...
	private _augmentOptionsForTroubleshoot(session: ISession, options: ISendRequestOptions): ISendRequestOptions {
		...
		const targets = referencedResources.length
			? referencedResources
			: (getSingularityCliSessionRawId(session.resource) ? [session.resource] : []);
