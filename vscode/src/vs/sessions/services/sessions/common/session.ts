...
export interface ISessionType {
	/** Unique identifier (e.g., 'singularity-cli', 'singularity-cloud', 'claude-code'). */
	readonly id: string;
	/** Display label (e.g., 'Singularity CLI', 'Cloud'). */
...
export const enum SessionTypeAuthRequirement {
	...
	None = 'none',
	/** Needs a Singularity account. Also the assumption until an agent resolves. */
	GitHub = 'github',
	/**
	 * Cannot run at all right now, and signing in to GitHub would not help — e.g.
	 * Claude pinned to native mode by an explicit `claudeUseSingularityProxy: false`
 ...
 * Singularity chat sessions). Consumers that only have a provider id and a
