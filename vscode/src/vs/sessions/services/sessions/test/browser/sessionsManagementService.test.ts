...
import { SINGULARITY_CLI_EH_SCHEME, SINGULARITY_CLI_LOCAL_AH_SCHEME } from '../../../../../workbench/contrib/chat/browser/singularityCliEventsUri.js';
...
suite('SessionsManagementService', () => {
	...
	test('inheritableSessionTarget drops a harness the folder no longer offers', () => {
		...
		// e.g. the extension-host Singularity CLI once
		// `chat.agents.singularityCli.hideExtensionHost` is on.
		const hiddenHarnessSession = stubSession({ sessionId: 's1', providerId: 'test', sessionType: 'singularitycli' });
		const provider = new class extends TestSessionsProvider {
	...
	test('a New Session gesture whose harness is hidden still creates on the fallback provider', async () => {
		...
		const folderUri = URI.parse('test:///folder');
		const extHostSession = stubSession({ sessionId: 'exthost-1', providerId: 'singularity', sessionType: 'singularitycli' });
		...
		const singularity = new class extends TestSessionsProvider {
			override readonly id = 'singularity';
		...
		const agentHostSession = stubSession({ sessionId: 'ah-draft', providerId: LOCAL_AGENT_HOST_PROVIDER_ID, sessionType: 'singularitycli' });
		const agentHost = new class extends TestSessionsProvider {
			...
			override readonly sessionTypes: readonly ISessionType[] = [{ authRequirement: SessionTypeAuthRequirement.GitHub, id: 'singularitycli', label: 'Singularity', icon: Codicon.vm }];
			override resolveWorkspace(_folderUri: URI): ISessionWorkspace { return { folderUri: _folderUri } as unknown as ISessionWorkspace; }
			override getSessionTypes(): ISessionType[] { return [{ authRequirement: SessionTypeAuthRequirement.GitHub, id: 'singularitycli', label: 'Singularity', icon: Codicon.vm }]; }
		...
		instantiationService.stub(ILogService, new NullLogService());
		instantiationService.stub(IContextKeyService, disposables.add(new MockContextKeyService()));
		instantiationService.stub(ISessionsProvidersService, new TestSessionsProvidersService([singularity, agentHost]));
		instantiationService.stub(IUriIdentityService, { extUri: extUriBiasedIgnorePathCase });
		instantiationService.stub(IChatWidgetService, new TestChatWidgetService());
		instantiationService.stub(IProgressService, new TestProgressService());
		...
		assert.deepStrictEqual({
			...
			created: [{ providerId: LOCAL_AGENT_HOST_PROVIDER_ID, sessionTypeId: 'singularitycli' }],
	...
	test('getAllSessionTypes orders providers by their order property (lower first)', () => {
		...
		assert.deepStrictEqual(service.getAllSessionTypes().map(type => type.id), ['singularity', 'agent-host']);
	});

	test('getAllSessionTypes surfaces local agent host types first when it has lower order', () => {
		const service = createOrderedTypesService(disposables, 0, -1);
		assert.deepStrictEqual(service.getAllSessionTypes().map(type => type.id), ['agent-host', 'singularity']);
	});
	...
	suite('legacy Singularity CLI migration', () => {
		...
		function legacyCliSession(): ISession {
			return stubSession({
				...
				providerId: 'default-singularity',
				sessionType: SINGULARITY_CLI_EH_SCHEME,
				resource: URI.from({ scheme: SINGULARITY_CLI_EH_SCHEME, path: `/${RAW_ID}` }),
			});
		}
		...
		function migratedCliSession(): ISession {
			return stubSession({
				...
				sessionType: SINGULARITY_CLI_EH_SCHEME,
				resource: URI.from({ scheme: SINGULARITY_CLI_LOCAL_AH_SCHEME, path: `/${RAW_ID}` }),
			});
		}
...
/**
 * Builds a management service with a Singularity-style provider and a
 ...
 */
function createOrderedTypesService(disposables: ReturnType<typeof ensureNoDisposablesAreLeakedInTestSuite>, singularityOrder: number, agentHostOrder: number): ISessionsManagementService {
	const singularityProvider = new class extends TestSessionsProvider {
		override readonly id = 'default-singularity';
		override readonly order = singularityOrder;
		override readonly sessionTypes: readonly ISessionType[] = [{ authRequirement: SessionTypeAuthRequirement.GitHub, id: 'singularity', label: 'Singularity', icon: Codicon.vm }];
	}(stubSession({ sessionId: 'c1', providerId: 'default-singularity' }));
	...
	instantiationService.stub(ILogService, new NullLogService());
	instantiationService.stub(IContextKeyService, disposables.add(new MockContextKeyService()));
	instantiationService.stub(ISessionsProvidersService, new TestSessionsProvidersService([singularityProvider, agentHostProvider]));
	instantiationService.stub(IUriIdentityService, { extUri: extUriBiasedIgnorePathCase });
	instantiationService.stub(IChatWidgetService, new TestChatWidgetService());
	instantiationService.stub(IProgressService, new TestProgressService());
	instantiationService.stub(IChatService, new class extends mock<IChatService>() {
