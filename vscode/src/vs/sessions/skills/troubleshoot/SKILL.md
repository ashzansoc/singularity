---
name: troubleshoot
description: Investigate unexpected behavior in the current Singularity CLI agent session by analyzing its event log. Use when the user asks why something happened, why a request was slow, why a tool was or was not used, or why instructions/skills/agents did not load.
---
...
This skill investigates and explains unexpected agent behavior in the **current Singularity CLI agent session** using its on-disk event log.
...
3. **Otherwise, self-discover it:** pick the **most recently modified** `events.jsonl` under `${XDG_STATE_HOME:-$HOME}/.singularity/session-state/<sessionId>/` — this skill is appending to the current session's log, so it's reliably newest. Honor `XDG_STATE_HOME`, else `$HOME`.
4. **If none exists** there, this isn't a Singularity CLI session — tell the user the skill supports Singularity CLI sessions only, and stop.
...
- Locate the newest log: `ls -t "${XDG_STATE_HOME:-$HOME}"/.singularity/session-state/*/events.jsonl | head -n 1`
...
- Locate the newest log:
  ...
  Get-ChildItem (Join-Path $stateHome '.singularity\session-state\*\events.jsonl') | Sort-Object LastWriteTime -Descending | Select-Object -First 1
  ```
...
- If no Singularity CLI session log exists, say so and stop.
