...
└───────────────────────────┬─────────────────────────────────────┘
                ...
                └──────┬──────────┬────────┘
                       │          │
          ┌────────────▼──┐  ┌───▼──────────────────┐
          │  SingularityChat  │  │ AgentHost / Remote    │
...
├── singularityChatSessions/  # Singularity chat sessions provider (wraps ChatSessionsService)
...
`IAgentSessionsService` (`vs/workbench/contrib/chat/browser/agentSessions/agentSessionsService`) is a **Singularity-provider internal** and must be consumed **only** by the Singularity chat sessions provider (`contrib/providers/singularityChatSessions/`). The rest of the Agents window — core, services, and non-provider contribs (e.g. the sessions list, the visible-sessions grid) — must stay **provider-agnostic** and interact with sessions exclusively through `ISession`/`ISessionsManagementService`. Reaching into `IAgentSessionsService` from shared code (for example to call `model.observeSession(...)` for lazy loading) couples the whole window to one provider and is prohibited. If a provider needs to react to provider-agnostic signals (such as a session becoming visible), surface that signal on the shared services and subscribe to it **inside the provider**. This rule is enforced by an ESLint `no-restricted-imports` ban scoped to `src/vs/sessions/**` (with the Singularity provider folder exempted).

> **Temporary exception (tracked by [#320480](https://github.com/microsoft/vscode/issues/320480)):** the sessions list (`contrib/sessions/browser/views/sessionsList.ts`) currently keeps one deliberate `IAgentSessionsService` usage to trigger lazy resolution of expensive session properties for rows scrolling into view. It carries a prominent comment and a localized `eslint-disable-next-line no-restricted-imports`. This must be moved into the Singularity provider; do not add further usages or copy the suppression.

### Provider-Specific Documentation

- [Singularity AI Sessions Provider](contrib/providers/singularityChatSessions/SINGULARITY_CHAT_SESSIONS_PROVIDER.md) — wraps `ChatSessionsService`, metadata contract, workspace derivation
...
config). For the agent host providers this is enabled for the `singularitycli` and
`claude` session types, whose backends (`SingularityAgent` / `ClaudeAgent`)
...
provider (e.g. `singularitycli`), so adding a peer chat to a restored session works
...
turns. `SingularityAgent.createChat` forks the source chat's SDK conversation
...
via the Singularity utility model and replaces the placeholder with a
...
context/restore implementation advertise it (currently Claude and Singularity).
...
message. Claude and Singularity use their SDK fork primitives to inherit source
...
`activeTurn`. Claude and Singularity serialize side-chat creation on the new chat's
...
while the source turn continues. Native Singularity forks do not persist an active turn until it reaches the backing
...
Agent-host approval levels map to the Singularity SDK allow-all modes before each
...
Single-chat providers (`singularityChatSessions`, `localChatSessions`) implement
...
always renameable; `singularityChatSessions` sets it only for the SingularityCLI and Claude
...
`setIsolationMode` and `setBranch` setters before sending. Agent-host Singularity CLI
...
Model-picker-aware chat input notifications also stay input-scoped. Each `NewChatInputWidget` owns an `INewChatModelPickerService`; its model picker registers both an opener and identifier-based selection, and the notification widget delegates semantic model actions to that service. Notification `sessionTypes` are concrete language-model target identifiers: derive them from `getChatSessionType(session.resource)` or `ISessionType.chatSessionType`, never from the logical `ISession.sessionType` (for Agent Host sessions these are `agent-host-singularitycli` and `singularitycli`, respectively). The harness picker exposes that concrete target as an observable and the notification widget subscribes to it; do not pair a pull getter with manual re-render calls, because the trigger and value can drift during asynchronous session recreation. The latest registration owns both picker operations, so phone layouts cannot open one picker while selecting through another. Notification-driven selection calls the scoped model and follows the same canonical storage and provider update path as a manual pick, but emits only `chatInputNotificationAction`; it does not emit picker-close telemetry because no picker was opened.
...
Every model-picker trigger identifies the selected model's vendor with a leading provider icon derived from the model metadata (for example OpenAI, Claude, Gemini, or Singularity), including editor chat, active sessions, new sessions, and phone-layout pickers. Auto is provider-agnostic and always uses the Singularity icon, regardless of provider metadata or generic-icon presentation. Standard editor and Agents-window chat inputs collapse the trigger to that provider icon below 280px while retaining the full accessible model label.
