...
[`playwright-cli`](https://github.com/microsoft/playwright-cli) and Singularity CLI.
...
| Service | Mock | Why |
|---------|------|-----|
| `IChatEntitlementService` | Returns `ChatEntitlement.Free` | No real Singularity account in CI |
...
| Chat agents (`singularitycli`, etc.) | Canned keyword-matched responses with `textEdit` progress items | No real LLM backend |
...
```
Create PR button clicked → singularity.chat.chat.createPullRequestSingularityCLIAgentSession.createPR
  → Mock extension logs and shows info message
```

The PR buttons (Create PR, Open PR, Merge) are contributed via the mock
extension's `package.json` menus, gated by `chatSessionType == singularitycli`.
The `chatSessionType` context key is derived from the session URI scheme
(`getChatSessionType()`), which returns `singularitycli` for mock sessions.
...
3. Sends each natural-language step + snapshot to **Singularity CLI**, which returns
...
├── generate.cjs              # Compiles scenarios → .commands.json via Singularity CLI
...
- Dependencies installed:
  ...
  ```
- Singularity CLI available (for `npm run generate` only):
  ```bash
  singularity --version
  ```
...
Write steps in plain English. The Singularity agent interprets them against the
...
Singularity CLI returns: `click e143`
