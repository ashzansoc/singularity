...
/**
 * Compile .scenario.md files into .commands.json using Singularity CLI.
 *
 ...
 * 3. Sends each step + snapshot to Singularity CLI to get the playwright-cli commands
...
// ---------------------------------------------------------------------------
// Ask Singularity CLI to translate a step
// ---------------------------------------------------------------------------

function askSingularity(step, snapshot) {
	const prompt = `Snapshot:\n\`\`\`\n${snapshot}\n\`\`\`\n\nStep: ${step}\n\nOutput the semantic commands:`;

	const result = cp.spawnSync('singularity', ['-p', `${SYSTEM_PROMPT}\n\n${prompt}`, '--model', 'claude-sonnet-4.6'], {
		cwd: APP_ROOT,
	...
	if (result.status !== 0) {
		throw new Error(`Singularity CLI failed: ${stderr || stdout}`);
	}
...
function compileScenario(scenario) {
	...
	for (const [i, step] of scenario.steps.entries()) {
		...
		try {
			const commands = askSingularity(step, snapshot.stdout);
