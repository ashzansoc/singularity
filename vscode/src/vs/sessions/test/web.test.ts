...
import { IDefaultAccount, IDefaultAccountAuthenticationProvider, ISingularityTokenInfo, IPolicyData } from '../../base/common/defaultAccount.js';
 ...
 * window think the user is signed in with a Free Singularity plan.
 */
class MockChatEntitlementService implements IChatEntitlementService {
	...
	readonly sku = 'free';
	readonly singularityTrackingId = 'mock-tracking-id';
...
class MockDefaultAccountService implements IDefaultAccountService {
	...
	readonly singularityTokenInfo: ISingularityTokenInfo | null = null;
...
class MockChatAgentContribution extends Disposable implements IWorkbenchContribution {
	 ...
	 * ChangesViewPane can render them for background (singularitycli) sessions.
	...
	private addSessionItem(resource: URI, message: string, responseText: string, fileEdits?: MockFileEdit[]): void {
		...
		this._sessionHistory.get(key)!.push(
			{ type: 'request', prompt: message, participant: 'singularity' },
			{ type: 'response', parts: [{ kind: 'markdownContent', content: { value: responseText, isTrusted: false, supportThemeIcons: false, supportHtml: false } }], participant: 'singularity' },
		);
	...
	private registerMockAgents(): void {
		const agentIds = ['singularitycli', 'singularity-cloud-agent'];
		...
		for (const agentId of agentIds) {
			const agentData: IChatAgentData = {
				...
				isDefault: agentId === 'singularitycli',
				metadata: {},
				slashCommands: [],
	...
	private registerMockSessionProvider(): void {
		const schemes = ['singularitycli', 'singularity-cloud-agent'];
		const self = this;
		for (const scheme of schemes) {
			try {
				...
				// Only singularitycli (Background) sessions need real items — the
				// singularity-cloud-agent controller must return an empty array to
				// prevent it from overwriting sessions with the wrong providerType
				// during a full model resolve.
				const controllerItems = scheme === 'singularitycli' ? this._sessionItems : [];
...
export class TestSessionsBrowserMain extends SessionsBrowserMain {
	...
	private preseedFolder(storageService: IStorageService): void {
		...
		const providerId = 'default-singularity';
