...
- No folder selected
- No user customizations created (from this tool or others i.e. Singularity CLI)
...
- Instructions includes root-level files (AGENTS.md, CLAUDE.md, singularity-instructions.md) under "Workspace"
...
- If user-level files exist in `~/.singularity/` or `~/.claude/`, a "User" group appears for applicable sections
...
5. Verify the file path shown in the editor header is `~/.singularity/instructions/<name>.instructions.md`
...
11. Check the "Source Folders (creation targets)" section — verify `[user]` points to `~/.singularity/instructions`

#### Expected Results

- User file is created under `~/.singularity/instructions/` (not the VS Code profile folder)
- The file appears in the "User" group in the list
- Sidebar badge count includes the new user file
- Debug report confirms the user creation target is `~/.singularity/instructions`

#### Notes

- This validates that `AgenticPromptsService.getSourceFolders()` correctly redirects user creation to `~/.singularity/`
...
- JSON skeleton has correct Singularity CLI format: `"version": 1`, `"bash"` field
- All hook events from `SINGULARITY_CLI_HOOK_TYPE_MAP` are present in the skeleton
...
- Hook events are derived from `SINGULARITY_CLI_HOOK_TYPE_MAP` — adding new events to the schema auto-includes them in the skeleton
- Only `"bash"` is used (not `"command"`) to match the Singularity CLI schema
