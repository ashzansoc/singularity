# Singularity Agent Runtime — Customization Surface Spec
...
>
> **Source:** `github/singularity-agent-runtime` codebase as of 2026-02-25.
...
|------------|-------------|-------|
| Singularity | `{repo}/.github/singularity-instructions.md` | Primary repo instructions |
...
| User | `~/.singularity/instructions/**/*.instructions.md` | `readUserSingularityInstructions()` |

### 1.3 User-Level Instructions

| Scope | File Pattern | Code Reference |
|-------|-------------|----------------|
| User global | `~/.singularity/singularity-instructions.md` | `hasHomeSingularityInstructions()` |
...
- `{cwd}/.github/singularity-instructions.md`
...
| Source | Mechanism |
|--------|-----------|
| Env var | `SINGULARITY_CUSTOM_INSTRUCTIONS_DIRS` — comma-separated list of additional directories to scan |
...
1. User global (`~/.singularity/singularity-instructions.md`)
...
|--------|-------------|----------------|
| Repo (Singularity) | `{repo}/.github/skills/*/SKILL.md` | `loader.ts` collectProjectDirs |
...
| User (Singularity) | `~/.singularity/skills/*/SKILL.md` | `loader.ts` personalDirs |
| User (Claude) | `~/.claude/skills/*/SKILL.md` | `loader.ts` personalDirs |
| Env var | Dirs listed in `SINGULARITY_SKILLS_DIRS` (comma-separated) | `loader.ts` |
...
| Source | File Pattern | Code Reference |
|--------|-------------|----------------|
| Repo (Singularity) | `{repo}/.github/agents/*.md`, `*.agent.md` | `useCustomAgents.ts` |
| Repo (Claude) | `{repo}/.claude/agents/*.md`, `*.agent.md` | `useCustomAgents.ts` |
| User (Singularity) | `~/.github/agents/*.md`, `*.agent.md` | `useCustomAgents.ts` |
...
|----------|--------|-------------|----------------|
| 1 (lowest) | User | `~/.singularity/mcp-config.json` | `mcp-config.ts` |
...
All `~/.singularity/` paths respect XDG overrides:

| Type | Default | XDG Override |
|------|---------|-------------|
| Config files | `~/.singularity/` | `$XDG_CONFIG_HOME/.singularity/` |
| State/cache | `~/.singularity/` | `$XDG_STATE_HOME/.singularity/` |

The base directory name is always `.singularity` (`APP_DIRECTORY` in `path-helpers.ts`).
...
Message received
 ...
 │   └─ Env vars (SINGULARITY_CLI_ENABLED_FEATURE_FLAGS, individual)
 │
 ├─ System prompt assembled
 │   ├─ Base agent prompt
 │   ├─ User instructions      ~/.singularity/singularity-instructions.md
 │   ├─ Repo instructions       .github/singularity-instructions.md, AGENTS.md, CLAUDE.md, GEMINI.md
 ...
 ├─ Tools assembled
 │   ├─ Built-in tools
 │   ├─ MCP servers             ~/.singularity/mcp-config.json + .mcp.json + .vscode/mcp.json + plugins
