...
export class CustomizationLinkViewItem extends ActionViewItem {
	...
	private _readCount(reader: Parameters<Parameters<typeof autorun>[0]>[0]): number {
		...
		if (this._config.isTools) {
			const state = this._toolEnablementService.observe(AGENT_HOST_SINGULARITY_CLI_SESSION_TYPE).read(reader);
 ...
 * hosts (e.g. `singularitycli`). Lookup therefore prefers the resource scheme so
