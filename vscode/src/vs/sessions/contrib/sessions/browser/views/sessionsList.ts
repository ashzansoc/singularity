...
// -----------------------------------------------------------------------------
// `IAgentSessionsService` is a Singularity-provider internal and must normally only
// be consumed by the Singularity chat sessions provider — the rest of the Agents
...
// re-implements it the right way (driven from inside the Singularity provider, or
...
class SessionItemRenderer implements ITreeRenderer<SessionListItem, FuzzyScore, ISessionItemTemplate> {
	...
	private renderSession(element: ISession, template: ISessionItemTemplate, matches?: IMatch[]): void {
		...
		// renders. This reaches into a Singularity-provider internal and must be
...
export class SessionsFlatList extends Disposable {
	...
	constructor(
		...
		// TEMPORARY (#320480): the row renderer reaches into a Singularity-provider
