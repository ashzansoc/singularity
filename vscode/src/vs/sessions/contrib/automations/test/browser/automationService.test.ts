...
suite('AutomationService', () => {
	...
	test('recordRunStart inserts a pending run; updateRun applies a patch', async () => {
		...
		const updated = await service.updateRun(run.id, { status: 'completed', sessionResource: 'vscode-chat-session://singularity/sess-1', completedAt: new Date().toISOString() });
		assert.strictEqual(updated?.status, 'completed');
		assert.strictEqual(updated?.sessionResource, 'vscode-chat-session://singularity/sess-1');
	});
	...
	test('round-trips target changes without carrying repository configuration into quick-chat mode', async () => {
		...
		const quickChat = await firstService.updateAutomation(created.id, {
			target: { kind: 'quickChat', providerId: 'local-agent-host', sessionTypeId: 'singularitycli' },
		});
		firstService.dispose();
		...
		assert.deepStrictEqual({
			...
			quickChat: { kind: 'quickChat', providerId: 'local-agent-host', sessionTypeId: 'singularitycli' },
			restored: { kind: 'quickChat', providerId: 'local-agent-host', sessionTypeId: 'singularitycli' },
	...
	test('migrates valid schema v1 records to v3 while dropping malformed targets', async () => {
		...
		const ledger = {
			...
			automations: [
				...
				{ id: 'quick', name: 'Quick', prompt: 'p', schedule: { interval: 'daily', scheduleHour: 9, scheduleMinute: 0, scheduleDay: 0 }, isQuickChat: true, providerId: 'local-agent-host', sessionTypeId: 'singularitycli', enabled: true, createdAt: '2024-01-01T00:00:00Z', updatedAt: '2024-01-01T00:00:00Z' },
			],
			runs: [
	...
	test('migrates schema v2 flat targets to schema v3 target unions', async () => {
		...
		storage.store('chat.automations.ledger', JSON.stringify({
			...
			automations: [
				...
				{ ...common, id: 'quick', name: 'Quick', isQuickChat: true, providerId: 'local-agent-host', sessionTypeId: 'singularitycli' },
			],
			runs: [],
		}), -1, 1);
		...
		assert.deepStrictEqual(service.automations.get().map(automation => automation.target), [
			...
			{ kind: 'quickChat', providerId: 'local-agent-host', sessionTypeId: 'singularitycli' },
		]);
