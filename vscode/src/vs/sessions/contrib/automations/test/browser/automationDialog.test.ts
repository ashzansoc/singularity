...
suite('Automation branch picker', () => {
	...
	function createItem(options?: {
		...
		const repository = upcastPartial<IGitRepository>({
			...
			getRefs: options?.getRefs ?? (async () => [
				...
				{ type: GitRefType.Head, name: 'singularity-worktree-generated' },
			]),
		});
		...
		instantiationService.stub(ISessionsManagementService, upcastPartial<ISessionsManagementService>({
			...
			getSessionTypesForFolder: () => providerAvailable ? [{
				providerId: state.providerId ?? 'default-singularity',
				sessionType: {
					id: state.sessionTypeId ?? 'singularitycli',
					label: 'Singularity',
					icon: Codicon.singularity,
					supportsWorktreeConfiguration: state.sessionTypeId === 'singularitycli',
	...
	test('enables Worktree branches for agent-host Singularity CLI', async () => {
		const { container } = createItem({
			state: createFormState({ providerId: 'local-agent-host', sessionTypeId: 'singularitycli' }),
		});
		await timeout(0);
		const trigger = container.querySelector<HTMLElement>('.automation-form-branch-slot');
		assert.ok(trigger);
	...
	test('resolves a legacy model identifier to the selected concrete target', () => {
		const legacyIdentifier = 'singularitycli/gpt-5.6-sol';
		const concreteIdentifier = 'agent-host-singularitycli:gpt-5.6-sol';
		...
		const models = new Map<string, ILanguageModelChatMetadata>([
			[legacyIdentifier, upcastPartial<ILanguageModelChatMetadata>({ id: 'gpt-5.6-sol', targetChatSessionType: 'singularitycli' })],
			[concreteIdentifier, upcastPartial<ILanguageModelChatMetadata>({ id: 'gpt-5.6-sol', targetChatSessionType: 'agent-host-singularitycli' })],
		...
		const beforeConcreteTargetArrives = resolveAutomationModelIdentifier(languageModelsService, legacyIdentifier, 'singularitycli', 'agent-host-singularitycli');
		modelIds.push(concreteIdentifier);

		assert.deepStrictEqual({
			beforeConcreteTargetArrives,
			afterConcreteTargetArrives: resolveAutomationModelIdentifier(languageModelsService, legacyIdentifier, 'singularitycli', 'agent-host-singularitycli'),
			alreadyConcrete: resolveAutomationModelIdentifier(languageModelsService, concreteIdentifier, 'singularitycli', 'agent-host-singularitycli'),
			unrelated: resolveAutomationModelIdentifier(languageModelsService, unrelatedIdentifier, 'singularitycli', 'agent-host-singularitycli'),
		}, {
