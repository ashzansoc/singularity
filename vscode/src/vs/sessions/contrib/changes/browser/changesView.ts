...
class ChangesMenuWorkbenchButtonBarWidget extends Disposable implements IChangesButtonBarWidget {
	...
	private _getButtonConfiguration(action: IAction, outgoingChanges: number, hasGitOperationInProgress: boolean, runningLabelObs: IObservable<string | IMarkdownString | undefined>): { showIcon: boolean; showLabel: boolean; isSecondary?: boolean; customLabel?: string | IMarkdownString; customLabelObs?: IObservable<string | IMarkdownString | undefined>; customClass?: string } | undefined {
		if (
			action.id === 'singularity.chat.sessions.commit' ||
			action.id === 'singularity.chat.chat.createPullRequestSingularityCLIAgentSession.createPR'
		) {
			if (!hasGitOperationInProgress) {
		...
		if (
			action.id === 'singularity.chat.sessions.sync' ||
			action.id === 'singularity.chat.sessions.commitAndSync'
		) {
		...
		if (
			action.id === 'singularity.chat.claude.sessions.sync' ||
		...
		if (
			...
			action.id === 'singularity.chat.chat.openPullRequestSingularityCLIAgentSession.openPR'
		) {
		...
		if (
			action.id === 'singularity.chat.chat.createPullRequestSingularityCLIAgentSession.createPR' ||
			action.id === 'singularity.chat.chat.mergeSingularityCLIAgentSessionChanges.merge' ||
			action.id === 'singularity.chat.chat.checkoutPullRequestReroute' ||
			action.id === 'pr.checkoutFromChat' ||
			action.id === 'singularity.chat.sessions.initializeRepository' ||
			action.id === 'singularity.chat.claude.sessions.initializeRepository' ||
			action.id === 'singularity.chat.claude.sessions.commit' ||
			action.id === 'singularity.chat.claude.sessions.commitAndSync' ||
...
export class ChangesViewPane extends ViewPane {
	...
	private getTreeRootInfo(items: readonly IChangesFileItem[]): IChangesTreeRootInfo | undefined {
		...
		if (workspaceFolderUri.scheme === GITHUB_REMOTE_FILE_SCHEME) {
			// Cloud session
			resourceTreeRootUri = URI.from({ scheme: Schemas.singularityPr, path: '/' });
			...
			[this.instantiationService.createInstance(ChangesTreeRenderer, resourceLabels, actionRunner,
				() => {
					...
					return folder?.root.scheme === GITHUB_REMOTE_FILE_SCHEME
						? URI.from({ scheme: Schemas.singularityPr, path: '/' })
