...
import { AGENT_HOST_SINGULARITY_CLI_SESSION_TYPE, countEnabledCustomizationTools, IAgentHostToolSetEnablementService } from '../../../../workbench/contrib/chat/browser/agentSessions/agentHost/agentHostToolSetEnablementService.js';
	...
	private async loadCounts(): Promise<void> {
		...
		await Promise.all(sectionPromptTypes.map(async ({ section, type }) => {
			...
			if (type === PromptsType.skill) {
				...
				// For instructions, also count agent instructions (AGENTS.md, singularity-instructions.md, CLAUDE.md, etc.)
		...
		if (toolsSection) {
			this._register(autorun(reader => {
				const state = this.toolEnablementService.observe(AGENT_HOST_SINGULARITY_CLI_SESSION_TYPE).read(reader);
