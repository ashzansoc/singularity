...
suite('SessionsTerminalContribution', () => {
	...
	test('does not hide the active terminal during visibility updates when cwd does not match', async () => {
		const worktreeUri = URI.file('/worktree');
		const otherUri = URI.file('/other');
		const session1 = makeAgentSession({ sessionId: 'test:session-1', worktree: worktreeUri, providerType: AgentSessionProviders.Background });
		activeSessionObs.set(session1, undefined);
		await tick();
		...
		const session2 = makeAgentSession({ sessionId: 'test:session-2', worktree: otherUri, providerType: AgentSessionProviders.Background });
		activeSessionObs.set(session2, undefined);
		await tick();

		assert.strictEqual(moveToBackgroundCalls.length, 0, 'active terminal must not be hidden when switching sessions');
	});
