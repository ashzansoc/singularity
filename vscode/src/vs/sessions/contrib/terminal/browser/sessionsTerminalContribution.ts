...
import { autorun, derived, IReader, waitForState } from '../../../../base/common/observable.js';
...
export class SessionsTerminalContribution extends Disposable implements IWorkbenchContribution {
	...
	private async _ensureTerminal(cwd: URI, focus: boolean, session?: ISession): Promise<ITerminalInstance[]> {
		...
		// The terminal view must be visible before createTerminal can succeed.
		if (focus) {
	...
	private async _updateTerminalVisibility(activeSession: ISession, activeKey: string, forceForegroundTerminalIds: number[]): Promise<void> {
		...
		for (const instance of [...this._terminalService.instances]) {
			...
			if ((belongsToActiveSession || isForceVisible) && !isForeground) {
				...
				if (protectedInstanceId !== undefined && currentInstance.instanceId === protectedInstanceId) {
					this._logService.trace(`[SessionsTerminal] Skipping active terminal ${currentInstance.instanceId} during visibility update`);
					continue;
				}
...
class OpenSessionInTerminalAction extends Action2 {
	...
	override async run(_accessor: ServicesAccessor): Promise<void> {
		...
		const terminalService = _accessor.get(ITerminalService);

		// Toggle off only when the panel is showing a foreground terminal the user
		// can see. An empty terminal view (e.g. after a visibility sweep moved tabs
		// to background) should open a terminal instead of collapsing the panel.
		if (layoutService.isVisible(Parts.PANEL_PART)) {
			if (viewsService.isViewVisible(TERMINAL_VIEW_ID) && terminalService.foregroundInstances.length > 0) {
		...
		if (activeSession?.loading.get()) {
			await waitForState(activeSession.loading, loading => !loading);
		}
