...
class TitleBarAccountWidget extends BaseActionViewItem {
	...
	private readonly singularityDashboardStore = this._register(new MutableDisposable<DisposableStore>());
	...
	private createCombinedPanelContent(panelStore: DisposableStore): HTMLElement {
		...
		if (this.accountName || this.isAccountLoading) {
			const singularityAccount = append(identities, $('section.sessions-account-titlebar-panel-provider-account', {
				'aria-label': localize('singularityAccountSectionLabel', "Singularity account")
			}));
			const singularityIdentity = append(singularityAccount, $('.sessions-account-titlebar-panel-provider-identity'));
			const loadedAvatarUrl = !this.isAccountLoading ? this.loadedAvatarUrl : undefined;
			if (loadedAvatarUrl) {
				const avatar = append(singularityIdentity, $('img.sessions-account-titlebar-panel-provider-avatar', {
				...
				const accountIcon = append(singularityIdentity, $('span.sessions-account-titlebar-panel-provider-icon', { 'aria-hidden': 'true' }));
				accountIcon.classList.add(...ThemeIcon.asClassNameArray(Codicon.github));
			}
			const title = append(singularityIdentity, $('div.sessions-account-titlebar-panel-provider-name'));
			title.textContent = this.getPanelHeaderLabel();
			const singularityActions = append(singularityIdentity, $('.sessions-account-titlebar-panel-provider-actions'));
			const singularityActionBar = panelStore.add(new ActionBar(singularityActions));
			panelStore.add(singularityActionBar.onWillRun(() => {
				this.hoverService.hideHover(true);
				this.clickPanelDisposable.clear();
			}));
			singularityActionBar.push(panelStore.add(new Action(
				'singularity.manageModels',
				localize('manageSingularityModels', "Manage Singularity Models"),
				ThemeIcon.asClassName(Codicon.singularity),
				true,
				() => this.commandService.executeCommand(MANAGE_CHAT_COMMAND_ID, '@provider:"Singularity"'),
			)), { icon: true, label: false });
			singularityActionBar.push(panelStore.add(new Action(
				'singularity.openAgentCustomizations',
				localize('openSingularityAgentCustomizations', "Agent Customizations for Singularity"),
				...
				() => this.commandService.executeCommand(AICustomizationManagementCommands.OpenEditor, {
					sessionType: SessionType.AgentHostSingularity,
					section: AICustomizationManagementSection.Agents,
				}),
			)), { icon: true, label: false });
			if (partitioned.signOut) {
				singularityActionBar.push(partitioned.signOut, { icon: true, label: false });
			}
			this.appendSingularityUsage(singularityAccount, panelStore);
		}
		...
		if (this.shouldShowSingularityDashboardHover()) {
			...
			append(footer, this.createSingularityHoverContent({ compactQuotaLayout: true }));
		}
		...
		if (!this.shouldShowSingularityDashboardHover() && !this.isAccountLoading) {
	...
	private appendSingularityUsage(accountSection: HTMLElement, panelStore: DisposableStore): void {
		...
		append(planRow, $('span.sessions-account-titlebar-panel-provider-plan', undefined, this.getSingularityPlanLabel()));
		if (quota && !quota.unlimited) {
			...
			const percentageLabel = localize('singularityCreditsUsedPercentageValue', "{0}%", usedPercentage);
			const percentageAriaLabel = localize('singularityCreditsUsedPercentage', "{0}% credits used", usedPercentage);
			...
			if (quota.entitlement) {
				...
				const creditsValue = localize('singularityCreditsUsedRatioValue', "{0} / {1}", formatter.value.format(used), formatter.value.format(quota.entitlement));
				const creditsAriaLabel = localize('singularityCreditsUsedRatio', "{0} / {1} credits used", formatter.value.format(used), formatter.value.format(quota.entitlement));
				const showCredits = () => {
			...
			const resetLabel = this.getSingularityResetLabel(quota.resetAt);
			if (resetLabel) {
			...
			append(detailRow, $('span.sessions-account-titlebar-panel-provider-usage-label', undefined, localize('singularityCreditsUsedLabel', "Credits used")));
		}
	}
	...
	private getSingularityResetLabel(resetAt: number | undefined): string | undefined {
		if (resetAt) {
			const resetDate = new Date(resetAt * 1000);
			return localize('singularityCreditsResetAt', "Resets {0} at {1}", accountDateFormatter.value.format(resetDate), accountTimeFormatter.value.format(resetDate));
		}
		...
		return resetDateHasTime
			? localize('singularityCreditsResetAt', "Resets {0} at {1}", accountDateFormatter.value.format(date), accountTimeFormatter.value.format(date))
			: localize('singularityCreditsReset', "Resets {0}", accountDateFormatter.value.format(date));
	}
	...
	private getSingularityPlanLabel(): string {
	...
	private shouldShowSingularityDashboardHover(): boolean {
		return !this.chatEntitlementService.sentiment.hidden && !!this.accountName;
	}

	private createSingularityHoverContent(extraOptions?: Partial<IChatStatusDashboardOptions>): HTMLElement {
		const store = new DisposableStore();
		this.singularityDashboardStore.value = store;
