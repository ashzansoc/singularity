...
suite('Sessions - Account Title Bar State', () => {
	...
	test('shows low token badge for Singularity Free users', () => {
		...
		assert.strictEqual(getAccountTitleBarBadgeKey(state), 'singularity:error:10%');
	});
	...
	test('shows quota reached warning when free quota is exhausted', () => {
		...
		assert.strictEqual(getAccountTitleBarBadgeKey(state), 'singularity:error:');
	});
