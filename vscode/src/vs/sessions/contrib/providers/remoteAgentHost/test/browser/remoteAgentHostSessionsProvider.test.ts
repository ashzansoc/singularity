...
class MockAgentConnection extends mock<IAgentConnection>() {
	...
	private _rootStateValue: RootState = { agents: [{ provider: 'singularitycli', displayName: 'Singularity', description: '', models: [] } as AgentInfo] };
	...
	override async createSession(config?: { session?: URI }): Promise<URI> {
		const uri = config?.session ?? URI.parse('singularitycli:///auto');
...
function createProvider(disposables: DisposableStore, connection: MockAgentConnection, overrides?: { address?: string; preferenceKey?: string; connectionName?: string | undefined; sendRequest?: (resource: URI, message: string, options?: IChatSendRequestOptions) => Promise<ChatSendResult>; openSession?: boolean; storageService?: IStorageService; noConnection?: boolean; isWebPlatform?: boolean; workspaceTrusted?: boolean }): RemoteAgentHostSessionsProvider {
	...
	instantiationService.stub(IChatSessionsService, {
		getChatSessionContribution: () => ({ type: 'remote-test-singularity', name: 'test', displayName: 'Test', description: 'test', icon: undefined }),
...
function fireSessionRemoved(connection: MockAgentConnection, rawId: string, provider = 'singularitycli'): void {
	const sessionUri = AgentSession.uri(provider, rawId);
	connection.fireNotification({
		channel: 'ahp-root://',
...
suite('RemoteAgentHostSessionsProvider', () => {
	...
	test('derives id and label from config, and session types from rootState agents', () => {
		...
		assert.strictEqual(provider.sessionTypes[0].id, SingularityCLISessionType.id);
		assert.strictEqual(provider.sessionTypes[0].label, 'Singularity [My Host]');
	});
	...
	test('session types update when the host advertises additional agents', () => {
		...
		assert.deepStrictEqual(provider.sessionTypes.map(t => ({ id: t.id, label: t.label })), [
			{ id: SingularityCLISessionType.id, label: 'Singularity [My Host]' },
	...
	test('session-type labels omit host suffix on web', () => {
		...
		assert.deepStrictEqual(provider.sessionTypes.map(t => ({ id: t.id, label: t.label })), [
			{ id: SingularityCLISessionType.id, label: 'Singularity' },
	...
	test('session type icons use per-agent codicons', () => {
		...
		assert.deepStrictEqual(
			...
			[
				{ id: SingularityCLISessionType.id, icon: 'singularity' },
	...
	test('session added notifications ingest any advertised agent provider', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		fireSessionAdded(connection, 'cop-1', { provider: 'singularitycli', title: 'Singularity Session' });
		...
		assert.deepStrictEqual(
			...
			[
				{ title: 'Singularity Session', sessionType: SingularityCLISessionType.id },
	...
	test('uses project metadata as workspace group source', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const workingDirectory = URI.parse('vscode-agent-host://localhost__4321/tmp/singularity-worktrees/vscode-feature?_ah%3DeyJzY2hlbWUiOiJmaWxlIn0');
	...
	test('session added converts file project URIs and preserves repository URLs', () => {
		...
		fireSessionAdded(connection, 'file-project', {
			...
			workingDirectory: 'file:///tmp/singularity-worktrees/vscode-feature',
		});
	...
	test('setModel updates existing session model without dispatching session-level model change', () => {
		...
		provider.setModel(session!.sessionId, 'remote-localhost__4321-singularitycli:new-model');

		assert.strictEqual(session!.modelId.get(), 'remote-localhost__4321-singularitycli:new-model');
	...
	test('setModel leaves dispatch log untouched for later message-level selection', () => {
		...
		provider.setModel(session!.sessionId, 'remote-localhost__4321-singularitycli:configured-model');

		assert.strictEqual(session!.modelId.get(), 'remote-localhost__4321-singularitycli:configured-model');
		assert.strictEqual(connection.dispatchedActions.length, 0);
	});
	...
	test('clearConnection clears pending new session config and capabilities', () => {
		connection.setAgents([{ provider: 'singularitycli', displayName: 'Singularity', description: '', models: [], capabilities: { multipleChats: { fork: true } } } as AgentInfo]);
		const provider = createProvider(disposables, connection);
	...
	test('deleteSession calls disposeSession with backend agent URI and removes from cache', async () => {
		...
		// The disposed URI must be a backend agent session URI (singularity://del-sess),
		// not the UI resource (remote-localhost_4321-singularity:///del-sess)
	...
	test('server-echoed ChatTurnStarted model does not update cached session model', () => {
		...
		assert.ok(target);
		provider.setModel(target!.sessionId, 'remote-localhost__4321-singularitycli:old-model');
		...
		assert.strictEqual(target!.modelId.get(), 'remote-localhost__4321-singularitycli:old-model');
		assert.strictEqual(changes.length, 0);
	});
	...
	test('renamed title survives session refresh from listSessions', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		connection.fireAction({
			channel: buildDefaultChatUri(AgentSession.uri('singularitycli', 'persist-sess').toString()),
			action: {
	...
	test('getSessionConfig seeds running config from session state subscription with full schema', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		connection.setSessionState('seed-1', 'singularitycli', fakeState);
	...
	test('replacing the connection disposes all session-state subscriptions', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		provider.getSessionConfig(session!.sessionId);
		const sessionUriStr = AgentSession.uri('singularitycli', 'seed-3').toString();
		assert.strictEqual(connection.sessionSubscribeCounts.get(sessionUriStr), 1);
		assert.strictEqual(connection.sessionUnsubscribeCounts.get(sessionUriStr) ?? 0, 0);
