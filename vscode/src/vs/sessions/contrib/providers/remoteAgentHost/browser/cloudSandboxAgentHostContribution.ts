...
// Surfaces Singularity cloud sandbox (singularity-developer-cli) sessions as native agent-host sessions.
 ...
 * `singularity` agent, so the two schemes name the same session.
 */
	...
	private async _doDiscoverAndSeed(): Promise<void> {
		...
		for (const session of result.sessions) {
			...
			const meta: IAgentSessionMetadata = {
				...
				// Singularity host lists that same id back, so the seed reconciles deterministically with
				// the live `listSessions()` result on connect. See singularity-host session-identity docs.
