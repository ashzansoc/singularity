...
export class RemoteAgentHostContribution extends Disposable implements IWorkbenchContribution {
	...
	private _registerAgent(address: string, connection: IAgentConnection, agent: AgentInfo, configuredName: string | undefined): void {
		...
		// `singularity` + `openai` from the same machine) produces distinct
		// labels instead of collapsing to a single `configuredName`.
		const hostLabel = configuredName || address;
		const rawAgentLabel = agent.displayName?.trim() || agent.provider;
		const agentLabel = /^singularity$/i.test(rawAgentLabel) || rawAgentLabel === 'singularitycli'
			? nls.localize('singularityCLI', "Singularity")
			: rawAgentLabel;
...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	properties: {
		...
		// Off by default: sandbox tasks currently carry the `singularity-developer-cli` slug, which the
		// Singularity extension's cloud provider does not list, so the two do not overlap. That slug is
		// expected to change, at which point both providers would list the same task — see
		// `CLOUD_SANDBOX_AGENT_SLUG`.
		[CloudSandboxEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.cloudSandbox.enabled', "Enable connecting to Singularity cloud sandbox sessions over a live Agent Host Protocol relay. When enabled, opening a Singularity CLI cloud session connects to its sandbox for slash commands and a responsive, steerable experience instead of only polling logs."),
			default: false,
			scope: ConfigurationScope.APPLICATION,
			tags: ['experimental', 'advanced'],
		},
