...
export interface IRemoteAgentHostSessionsProviderConfig {
	 ...
	 * the cloud sandbox host does (sessions are `ahp-session:/<id>` while the agent is `singularity`).
...
export interface ISessionSchemeAlias {
	/** Scheme the UI routes by — the agent provider (e.g. `singularity`). */
	readonly ui: string;
 ...
 * - **sessionType** - the logical session type (e.g. `singularitycli` for singularity
 ...
 *   session URI (`singularity:///abc123`), reconstructed via `AgentSession.uri`.
 */
