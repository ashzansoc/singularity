...
import { GITHUB_DOT_COM_SINGULARITY_API_BASE_URI } from '../../../../../platform/agentHost/common/githubEndpoints.js';
...
import { SINGULARITY_INTEGRATION_ID } from '../../../../../platform/endpoint/common/licenseAgreement.js';
 ...
 *
 * Runs in the renderer so the sandbox path works in VS Code Web, where no Singularity extension host is
 * available.
 */
export class CloudSandboxApiService extends Disposable implements ICloudSandboxApiService {
	...
	async getSessionHistory(taskId: string, token: CancellationToken): Promise<IReplayedTaskHistory | undefined> {
		...
		const context = await this._request(url, 'mc.taskClient.events', 'getTaskEvents', {
			...
			'Singularity-Integration-Id': SINGULARITY_INTEGRATION_ID,
	...
	private async _sendEnvironment(
		...
		const url = `${GITHUB_DOT_COM_SINGULARITY_API_BASE_URI}/agents/environments/${encodeURIComponent(environmentId)}${path}${toQuery(searchParams)}`;
	...
	/**
	 * Mission Control task API base. Uses the Singularity API host: `api.github.com/agents/*` omits
	...
	private _tasksBaseUrl(): string {
		return `${GITHUB_DOT_COM_SINGULARITY_API_BASE_URI}/agents`;
	}
