...
import { ClaudeCodeSessionType, SINGULARITY_PROVIDER_ID, SingularityChatSessionsProvider } from './singularityChatSessionsProvider.js';
...
import { SingularityPermissionPickerDelegate, PermissionPicker } from './permissionPicker.js';
import { SingularityCLISessionType } from '../../agentHost/browser/baseAgentHostSessionsProvider.js';
import { ISessionContext } from '../../../../services/sessions/browser/sessionContext.js';

const IsActiveSessionSingularityCLI = ContextKeyExpr.equals(SessionTypeContext.key, SingularityCLISessionType.id);
const IsActiveSessionLocal = ContextKeyExpr.equals(SessionTypeContext.key, LocalSessionType.id);
const IsActiveSingularityChatSessionProvider = ContextKeyExpr.equals(SessionProviderIdContext.key, SINGULARITY_PROVIDER_ID);
const IsActiveSessionSingularityChatCLI = ContextKeyExpr.and(IsActiveSessionSingularityCLI, IsActiveSingularityChatSessionProvider);
const IsActiveSessionClaudeCode = ContextKeyExpr.equals(SessionTypeContext.key, ClaudeCodeSessionType.id);
const IsActiveSessionSingularityChatClaudeCode = ContextKeyExpr.and(IsActiveSessionClaudeCode, IsActiveSingularityChatSessionProvider);
const IsActiveSessionSingularityChatLocal = ContextKeyExpr.and(IsActiveSessionLocal, IsActiveSingularityChatSessionProvider);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSingularity.branchPicker',
			...
			menu: [{
				...
				group: 'navigation',
				order: 2,
				when: ContextKeyExpr.and(IsNewChatSessionContext, IsActiveSessionSingularityChatCLI, SessionHasGitRepositoryContext),
			}],
		});
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSingularity.modePicker',
			...
			menu: [{
				...
				group: 'navigation',
				order: 0,
				when: ContextKeyExpr.or(IsActiveSessionSingularityChatCLI, IsActiveSessionSingularityChatLocal, IsActiveSessionLocal),
			}],
		});
	}
	override async run(): Promise<void> { /* handled by action view item */ }
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSingularity.permissionPicker',
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSingularity.claudePermissionModePicker',
			...
			menu: [{
				...
				group: 'navigation',
				order: 1,
				when: IsActiveSessionSingularityChatClaudeCode,
			}],
		});
	}
	override async run(): Promise<void> { /* handled by action view item */ }
 ...
 *
 * Exported so the web-only `SingularityPermissionPickerWebContribution`
...
class SingularityPickerActionViewItemContribution extends Disposable implements IWorkbenchContribution {

	static readonly ID = 'workbench.contrib.singularityPickerActionViewItems';
	...
	constructor(
		...
		this._register(autorun(reader => {
			...
			if (session) {
				...
				if (provider instanceof SingularityChatSessionsProvider) {
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionRepositoryConfig, 'sessions.defaultSingularity.branchPicker',
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionConfig, 'sessions.defaultSingularity.modePicker',
			(_action, _options, scopedInstantiationService) => {
				const { session } = scopedInstantiationService.invokeFunction(accessor => accessor.get(ISessionContext));
		...
		// web-only `SingularityPermissionPickerWebContribution` (registered
		...
		if (!isWeb) {
			this._register(actionViewItemService.register(
				Menus.NewSessionControl, 'sessions.defaultSingularity.permissionPicker',
				(_action, _options, scopedInstantiationService) => {
					const { session } = scopedInstantiationService.invokeFunction(accessor => accessor.get(ISessionContext));
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionControl, 'sessions.defaultSingularity.claudePermissionModePicker',
			(_action, _options, scopedInstantiationService) => {
				const { session } = scopedInstantiationService.invokeFunction(accessor => accessor.get(ISessionContext));
...
registerWorkbenchContribution2(SingularityPickerActionViewItemContribution.ID, SingularityPickerActionViewItemContribution, WorkbenchPhase.AfterRestored);
