...
import { SingularityChatSessionsProvider, ISingularityChatSession } from './singularityChatSessionsProvider.js';

/**
 * Singularity-specific adapter that drives the shared BranchPicker with
...
export class BranchPicker extends Disposable {
	...
	constructor(
		...
		this._isolationOptionEnabled = this._configurationService.getValue<boolean>('singularity.chat.chat.cli.isolationOption.enabled') !== false;

		this._register(this._configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('singularity.chat.chat.cli.isolationOption.enabled')) {
				this._isolationOptionEnabled = this._configurationService.getValue<boolean>('singularity.chat.chat.cli.isolationOption.enabled') !== false;
		...
		this._register(autorun(reader => {
			...
			const providerSession = provider instanceof SingularityChatSessionsProvider ? provider.getSession(session!.sessionId) : undefined;
			if (providerSession) {
	...
	private _getSession(): ISingularityChatSession | undefined {
		const session = this._session.get();
		if (!session) {
			return undefined;
		}
		const provider = this.sessionsProvidersService.getProvider(session.providerId);
		return provider instanceof SingularityChatSessionsProvider ? provider.getSession(session.sessionId) : undefined;
	}
