...
import { SingularityCLISessionType } from '../../agentHost/browser/baseAgentHostSessionsProvider.js';
import { createChangesets } from './singularityChatSessionsChangesets.js';
...
export const ClaudeCodeSessionType: ISessionType = {
	...
	icon: Codicon.claude,
	// Extension-contributed (legacy) generation: no native mode, always Singularity-backed.
	authRequirement: SessionTypeAuthRequirement.GitHub,
};

/** Singularity Cloud session type - cloud-hosted agent. */
export const SingularityCloudSessionType: ISessionType = {
	id: 'singularity-cloud-agent',
	label: localize('singularityCloud', "Cloud"),
	icon: Codicon.cloud,
	authRequirement: SessionTypeAuthRequirement.GitHub,
};
...
export interface ISingularityChatSession {
	...
	/** Session type ID (e.g., 'singularity-cli', 'singularity-cloud', 'local'). */
...
const OPEN_REPO_COMMAND = 'singularity.chat.chat.cloudSessions.openRepository';

/** Provider ID for the Singularity AI Sessions provider. */
export const SINGULARITY_PROVIDER_ID = 'default-singularity';

/** Setting key controlling whether the Singularity provider supports multiple chats per session. */
export const SINGULARITY_MULTI_CHAT_SETTING = 'sessions.singularity.chat.multiChatSessions';
...
type NewSession = SingularityCLISession | RemoteNewSession | ClaudeCodeNewSession;

function isNewSession(session: ISingularityChatSession): session is NewSession {
	return session instanceof SingularityCLISession || session instanceof RemoteNewSession || session instanceof ClaudeCodeNewSession;
}

/**
 * Builds an {@link IChat} snapshot from an {@link ISingularityChatSession}. Used to
 ...
 */
function buildChatFromSession(chat: Omit<ISingularityChatSession, 'mainChat'>, resource?: URI): IChat {
	return {
 ...
 * Implements {@link ISingularityChatSession} (session facade) and provides
 * pre-send configuration methods for the new-session flow.
 */
class SingularityCLISession extends Disposable implements ISingularityChatSession {

	static readonly SINGULARITY_WORKTREE_PATTERN = 'singularity-worktree-';

	// -- ISessionData fields --

	readonly sessionId: string;
	readonly providerId: string;
	readonly sessionType: typeof SessionType.SingularityCLI;
	readonly icon: ThemeIcon;
	readonly createdAt: Date;
	...
	constructor(
		...
		this.icon = SingularityCLISessionType.icon;
	...
	private _loadBranches(repo: IGitRepository): void {
		...
		repo.getRefs({ pattern: 'refs/heads' }, cts.token).then(refs => {
			...
			const branches = refs
				...
				.filter(name => !name.includes(SingularityCLISession.SINGULARITY_WORKTREE_PATTERN));
 ...
 * pre-send configuration methods for the new-session flow.
 */
export class RemoteNewSession extends Disposable implements ISingularityChatSession {
	...
	constructor(
		...
		this.icon = SingularityCloudSessionType.icon;
		this.createdAt = new Date();
 ...
 * Implements {@link ISingularityChatSession} (session facade) and provides
 * pre-send configuration methods for the new-session flow.
 * Simpler than {@link SingularityCLISession} because the Claude agent manages
 * its own worktrees and branches at runtime.
 */
class ClaudeCodeNewSession extends Disposable implements ISingularityChatSession {
 ...
 * workspace ({@link SingularityChatSessionsProvider.resolveWorkspace}) and the committed
...
/**
 * Adapts an existing {@link IAgentSession} from the chat layer into the new {@link ISingularityChatSession} facade.
 */
class AgentSessionAdapter implements ISingularityChatSession {
	...
	private _getSessionTypeIcon(session: IAgentSession): ThemeIcon {
		switch (session.providerType) {
			case AgentSessionProviders.Background:
				return SingularityCLISessionType.icon;
			case AgentSessionProviders.Cloud:
				return SingularityCloudSessionType.icon;
...
/**
 * Default sessions provider for Singularity CLI, Cloud, Claude, and Local session types.
 * Wraps the existing session infrastructure into the extensible provider model.
 */
export class SingularityChatSessionsProvider extends Disposable implements ISessionsProvider {

	readonly id = SINGULARITY_PROVIDER_ID;
	readonly label = localize('singularityChatSessionsProvider', "Singularity AI");
	readonly icon = Codicon.singularity;
	readonly order = 0;

	get sessionTypes(): readonly ISessionType[] {
		const types: ISessionType[] = [];
		if (this._isSingularityCliAvailable()) {
			types.push(SingularityCLISessionType);
		}
		types.push(SingularityCloudSessionType);
	...
	private readonly _sessionCache = new Map<string, AgentSessionAdapter | SingularityCLISession | RemoteNewSession | ClaudeCodeNewSession>();
	...
	private _chatByRawSessionIdCache: Map<string, ISingularityChatSession> | undefined;
	...
	/**
	 * Claude is offered by this (Singularity AI sessions) provider only when the
	...
	/**
	 * The Extension Host Singularity CLI is offered by this provider unless the user
	 * has hidden it via `chat.agents.singularityCli.hideExtensionHost`, in which case
	 * the Agents window picker only surfaces the Agent Host Singularity CLI entry.
	 * Hiding it only makes sense when the agent host runtime is available to
	 * surface the Agent Host Singularity CLI in its place.
	 */
	private _isSingularityCliAvailable(): boolean {
		const hideExtensionHost = this.configurationService.getValue<boolean>(ChatConfiguration.SingularityCliHideExtensionHostAgents) ?? false;
	...
	constructor(
		...
		this._multiChatEnabled = this.configurationService.getValue<boolean>(SINGULARITY_MULTI_CHAT_SETTING) ?? true;
		...
		this._register(this.configurationService.onDidChangeConfiguration(e => {
			const affectsSessionTypes = e.affectsConfiguration(CLAUDE_CODE_ENABLED_SETTING)
				...
				|| e.affectsConfiguration(ChatConfiguration.SingularityCliHideExtensionHostAgents);
	...
	getSessionTypes(workspaceUri: URI): ISessionType[] {
		if (workspaceUri.scheme === GITHUB_REMOTE_FILE_SCHEME || workspaceUri.scheme === SessionType.SingularityCloud) {
			return [SingularityCloudSessionType];
		}
		const types: ISessionType[] = [];
		if (this._isSingularityCliAvailable()) {
			types.push(SingularityCLISessionType);
	...
	getSession(sessionId: string): ISingularityChatSession | undefined {
	...
	createNewSession(workspaceUri: URI, sessionTypeId: string): ISession {
		...
		if (workspaceUri.scheme === GITHUB_REMOTE_FILE_SCHEME) {
			if (sessionTypeId !== SingularityCloudSessionType.id) {
				throw new Error('Only Singularity Cloud sessions can be created for GitHub repositories');
			}
		...
		if (sessionTypeId !== SingularityCLISessionType.id) {
		...
		const session = this.instantiationService.createInstance(SingularityCLISession, resource, workspace, this.id);
	...
	createQuickChat(_sessionTypeId: string): ISession {
		...
		throw new Error('SingularityChatSessionsProvider does not support quick chats');
	}
	...
	getModelPickerOptions(sessionId: string): ISessionModelPickerOptions {
		...
		// Singularity Free / Student user), the picker shows a "No models available"
		// state instead of Auto. Harnesses that support Auto (e.g. the Singularity
	...
	setMode(sessionId: string, modeId: string): void {
		const setSessionMode = (session: ISingularityChatSession): void => {
	...
	async renameChat(sessionId: string, chatUri: URI, title: string): Promise<void> {
		...
		if (agentSession?.providerType === SingularityCLISessionType.id) {
			await this.commandService.executeCommand('singularity.chat.cli.sessions.setTitle', { resource: chatUri }, title);
			return;
		}
		if (agentSession?.providerType === AgentSessionProviders.Claude) {
			await this.commandService.executeCommand('singularity.chat.claude.sessions.rename', { resource: chatUri }, title);
			return;
		}
	...
	private async _deleteAgentSessions(agentSessions: IAgentSession[]): Promise<void> {
		...
		if (cliSessionItems.length > 0) {
			await this.commandService.executeCommand('agents.singularity.chat.cli.deleteSessions', cliSessionItems, { skipConfirmation: true });
		}
	}
	...
	async createNewChat(sessionId: string, prompt?: string): Promise<IChat> {
		...
		if (currentNewSession) {
			...
			if (session instanceof ClaudeCodeNewSession) {
				...
				if (!newItem) {
					throw new Error('[SingularityChatSessionsProvider] Failed to create Claude session item');
				}
		...
		if (!this._isMultiChatEnabled()) {
			throw new Error(`[SingularityChatSessionsProvider] Session '${sessionId}' does not support multiple chats`);
		}
	...
	private async _createNewSubsequentChat(sessionId: string): Promise<IChat> {
		...
		if (chat.sessionType !== SingularityCLISessionType.id) {
			throw new Error('Multiple chats per session is only supported for Singularity CLI sessions');
		}
		...
		const resource = URI.from({ scheme: AgentSessionProviders.Background, path: `/untitled-${generateUuid()}` });
		const session = this.instantiationService.createInstance(SingularityCLISession, resource, newWorkspace, this.id);
	...
	async sendRequest(sessionId: string, chatResource: URI, options: ISendRequestOptions): Promise<ISession> {
		...
		if (!chatSession || !(chatSession instanceof SingularityCLISession)) {
	...
	private async _sendFirstChat(session: SingularityCLISession | RemoteNewSession | ClaudeCodeNewSession, chatResource: URI, options: ISendRequestOptions): Promise<ISession> {
		...
		const resourceChangesOnCommit = session instanceof SingularityCLISession || session instanceof RemoteNewSession;
		...
		const sendOptions: IChatSendRequestOptions = {
			...
			attachedContext,
			agentHostSessionConfig: session instanceof SingularityCLISession ? session.getAgentHostSessionConfig() : undefined,
		};

		const ref = await this._updateChatSessionState(chatResource, session, sendOptions.modeInfo?.permissionLevel);
		this.logService.debug(`[SingularityChatSessionsProvider] Sending first chat for session ${session.sessionId} with options:`, {
		...
		try {
			...
			if (result.kind === 'rejected') {
				...
				session.dispose();
				throw new Error(`[DefaultSingularityProvider] sendRequest rejected: ${result.reason}`);
			}
			...
			this.logService.error(`[SingularityChatSessionsProvider] Failed to send first chat for session ${session.sessionId}:`, error);
			throw error;
		} finally {
			if (committedKey) {
	 ...
	 * in the cache.
	 */
	private async _sendExistingChat(sessionId: string, newChatSession: SingularityCLISession, options: ISendRequestOptions): Promise<ISession> {
	...
	private async _waitForSessionInCache(resource: URI, token?: CancellationToken): Promise<AgentSessionAdapter> {
		...
		try {
			...
			// providers (notably Singularity CLI's V1 contribution) scan disk for
	...
	private _ensureGroupingCaches(): void {
		...
		const chatByRawSessionId = new Map<string, ISingularityChatSession>();
		for (const chat of chats) {
			chatByRawSessionId.set(chat.resource.path.slice(1), chat);
		}

		const groupIdByChatId = new Map<string, string>();
		const chatsByGroupId = new Map<string, ISingularityChatSession[]>();

		const resolveGroupId = (chat: ISingularityChatSession): string => {
			const cachedGroupId = groupIdByChatId.get(chat.sessionId);
			if (cachedGroupId) {
				return cachedGroupId;
			}

			const trail: ISingularityChatSession[] = [];
			const seen = new Set<string>();
			let current: ISingularityChatSession = chat;
	...
	private _refreshSessionCache(): void {
		...
		const addedData: ISingularityChatSession[] = [];
		const changedData: ISingularityChatSession[] = [];
		...
		for (const session of this.agentSessionsService.model.sessions) {
			...
			if (existing) {
				...
				// unread. Singularity read state is owned by the agent session model,
		...
		const removedData: ISingularityChatSession[] = [];
		...
		let removedGroupIds: Map<ISingularityChatSession, string> | undefined;
	...
	private _refreshSessionCacheMultiChat(
		addedData: ISingularityChatSession[],
		removedData: ISingularityChatSession[],
		changedData: ISingularityChatSession[],
		removedGroupIds: Map<ISingularityChatSession, string>,
	): void {
		...
		const trulyRemovedSessions: { chat: ISingularityChatSession; groupId: string }[] = [];
		...
		const newSessions: ISingularityChatSession[] = [];
		...
		const deduplicatedChanged: ISingularityChatSession[] = [];
	...
	private _findChatSession(chatId: string): ISingularityChatSession | undefined {
	 ...
	 */
	private _getGroupIdForChat(chat: ISingularityChatSession): string {
	...
	private _getDirectParentRawSessionId(chat: ISingularityChatSession): string | undefined {
	...
	/**
	 * Wraps a primary {@link ISingularityChatSession} and its sibling chats into an {@link ISession}.
	 ...
	 */
	private _chatToSession(chat: ISingularityChatSession): ISession {
		if (!this._isMultiChatEnabled()) {
		...
		const primaryChat: ISingularityChatSession = firstChatId
		...
		const groupChatsObs = derivedOpts<readonly IChat[] | undefined>({
			...
			const resolved: ISingularityChatSession[] = [];
		...
		const session: ISession = {
			...
			capabilities: constObservable({
				supportsMultipleChats: primaryChat.sessionType === SingularityCLISessionType.id && this._isMultiChatEnabled(),
				...
				runsWorktreeCreatedTasks: primaryChat.sessionType === SingularityCloudSessionType.id,
			}),
		};
	...
	private _chatToSingleChatSession(chat: ISingularityChatSession): ISession {
		...
		return {
			...
			capabilities: constObservable({
				...
				runsWorktreeCreatedTasks: chat.sessionType === SingularityCloudSessionType.id,
			}),
		};
	}
	 ...
	 * the SingularityCLI and Claude backends expose a rename command; others throw.
	 */
	private _sessionTypeSupportsRename(sessionType: string): boolean {
		return sessionType === SingularityCLISessionType.id || sessionType === AgentSessionProviders.Claude;
	}

	private _sessionTypeSupportsDelete(sessionType: string): boolean {
		return sessionType === SingularityCLISessionType.id;
	}

	private _toChat(chat: ISingularityChatSession, resource?: URI, interactivity: ChatInteractivity = ChatInteractivity.Full): IChat {
		return {
			resource: resource ?? chat.resource,
			createdAt: chat.createdAt,
			title: chat.title,
