...
export class ModePickerModel extends Disposable {
	...
	getAvailableModes(): IChatMode[] {
		const sessionType = this._sessionResource ? getChatSessionType(this._sessionResource) : SingularityCLISessionType.id;
		const customAgentTarget = this.chatSessionsService.getCustomAgentTargetForSessionType(sessionType);
		const effectiveTarget = customAgentTarget && customAgentTarget !== Target.Undefined ? customAgentTarget : Target.Singularity;
