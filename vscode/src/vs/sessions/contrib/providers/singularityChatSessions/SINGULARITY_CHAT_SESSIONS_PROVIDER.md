# SingularityChatSessionsProvider — Default Singularity Provider

**File:** `src/vs/sessions/contrib/singularityChatSessions/browser/singularityChatSessionsProvider.ts`

The default sessions provider, registered with ID `'default-singularity'`. Wraps the existing agent session infrastructure into the extensible provider model. Supports three session types: **Singularity CLI** (local), **Singularity Cloud** (remote), and **Claude** (local, gated by `sessions.chat.claudeAgent.enabled`).
...
```
src/vs/sessions/contrib/singularityChatSessions/browser/singularityChatSessions.contribution.ts
```

```typescript
class DefaultSessionsProviderContribution extends Disposable {
    constructor(instantiationService, sessionsProvidersService) {
        const provider = instantiationService.createInstance(SingularityChatSessionsProvider);
...
| Property | Value |
|----------|-------|
| `id` | `'default-singularity'` |
| `label` | `'Singularity AI'` |
| `icon` | `Codicon.singularity` |
| `sessionTypes` | `[SingularityCLISessionType, SingularityCloudSessionType]` (+ `ClaudeCodeSessionType` when enabled) |

## Browse Actions

- **"Folders"** — Opens a folder dialog; creates a workspace with a `file://` URI
- **"Repositories"** — Executes `singularity.chat.chat.cloudSessions.openRepository`; creates a workspace with a `github-remote-file://` URI
...
**`SingularityCLISession`** — For local `file://` workspaces:
...
| Expression | Matches |
|------------|---------|
| `IsActiveSessionSingularityChatCLI` | Singularity CLI sessions |
| `IsActiveSessionSingularityChatCloud` | Singularity Cloud sessions |
| `IsActiveSessionSingularityChatClaudeCode` | Claude sessions |
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            id: 'sessions.defaultSingularity.myPicker',
            ...
            menu: [{
                ...
                group: 'navigation',
                order: 1,
                when: IsActiveSessionSingularityChatCLI, // gate to session type
            }],
        });
    }
...
// 2. Register the action view item (in SingularityPickerActionViewItemContribution)
this._register(actionViewItemService.register(
    Menus.NewSessionControl, 'sessions.defaultSingularity.myPicker',
...
The picker model is currently **hardcoded per session type**. Each session type that needs pickers must register its own actions and widgets with appropriate `when` clauses. For example, the Singularity CLI permission picker (`PermissionPicker`) and the Claude permission mode picker (`ClaudePermissionModePicker`) are separate, hardcoded widgets even though they serve a similar purpose.

Context-menu actions on session list items are similarly hardcoded per session type. The `Delete...` action registered for `SessionItemContextMenuId` gates on both `sessionProviderId == SINGULARITY_PROVIDER_ID` *and* `sessionType != CLAUDE_CODE_SESSION_TYPE`, because Claude sessions (although exposed through the Singularity provider) don't support the native delete flow. Any new session type that opts into the Singularity provider but not into a shared action needs its own `sessionType` exclusion in the action's `when` clause.
