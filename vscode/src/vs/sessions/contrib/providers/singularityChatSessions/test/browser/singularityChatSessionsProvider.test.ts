...
import { CLAUDE_CODE_ENABLED_SETTING, SingularityChatSessionsProvider, SINGULARITY_PROVIDER_ID, ClaudeCodeSessionType, SingularityCloudSessionType, ISingularityChatSession } from '../../browser/singularityChatSessionsProvider.js';
...
interface ICreateProviderOptions {
	...
	readonly hideSingularityCli?: boolean;
...
function createProviderWithConfig(
	...
	model: MockAgentSessionsModel,
	opts?: ICreateProviderOptions,
): { provider: SingularityChatSessionsProvider; configService: TestConfigurationService; agentHostEnabled: ISettableObservable<boolean> } {
	const instantiationService = disposables.add(new TestInstantiationService());

	const configService = new TestConfigurationService();
	configService.setUserConfiguration('sessions.singularity.chat.multiChatSessions', opts?.multiChatEnabled ?? true);
	...
	configService.setUserConfiguration(ChatConfiguration.SingularityCliHideExtensionHostAgents, opts?.hideSingularityCli ?? false);
	...
	instantiationService.stub(ICommandService, {
		executeCommand: async (id: string, ...args: unknown[]) => {
			...
			// Simulate 'agents.singularity.chat.cli.deleteSessions' removing sessions
			const items = args[0];
	...
	instantiationService.stub(IChatSessionsService, {
		getChatSessionContribution: () => ({ type: 'test-singularity', name: 'test', displayName: 'Test', description: 'test', icon: undefined }),
	...
	// Stub IInstantiationService so provider can use createInstance for SingularityCLISession
	...
	const provider = disposables.add(instantiationService.createInstance(SingularityChatSessionsProvider));
 ...
 * needed by SingularityCLISession and _sendFirstChat, including IGitService and a
...
function createProviderForSendTests(
	...
	configService.setUserConfiguration('sessions.singularity.chat.multiChatSessions', true);
	configService.setUserConfiguration(CLAUDE_CODE_ENABLED_SETTING, opts?.claudeEnabled ?? true);
	...
	return disposables.add(instantiationService.createInstance(SingularityChatSessionsProvider));
}

suite('SingularityChatSessionsProvider', () => {
	...
	test('has correct id and label', () => {
		...
		assert.strictEqual(provider.id, SINGULARITY_PROVIDER_ID);
	...
	test('sessionTypes excludes Singularity CLI when hideExtensionHost is true', () => {
		// When the user hides the Extension Host Singularity CLI, this provider
		// must drop the entry so the Agents window picker only surfaces the
		// Agent Host Singularity CLI.
		const provider = createProvider(disposables, model, { hideSingularityCli: true });
		assert.ok(!provider.sessionTypes.some(t => t.id === SingularityCLISessionType.id));
	});
	...
	test('onDidChangeSessionTypes fires when hideExtensionHost setting changes', () => {
		...
		// the EXP-backed preference unregisters this provider's Singularity CLI
		...
		configService.setUserConfiguration(ChatConfiguration.SingularityCliHideExtensionHostAgents, true);
		configService.onDidChangeConfigurationEmitter.fire({
			source: ConfigurationTarget.USER,
			affectedKeys: new Set([ChatConfiguration.SingularityCliHideExtensionHostAgents]),
			change: { keys: [ChatConfiguration.SingularityCliHideExtensionHostAgents], overrides: [] },
			affectsConfiguration: (key: string) => key === ChatConfiguration.SingularityCliHideExtensionHostAgents,
		});
	...
	test('hideExtensionHost is not respected when Agent Host is unavailable', () => {
		// Hiding the Extension Host Singularity CLI only makes sense when the agent
		// host is available to surface the Agent Host Singularity CLI in its place.
		// Without an Agent Host runtime the hide setting must be ignored so the entry
		// stays visible.
		const provider = createProvider(disposables, model, { hideSingularityCli: true, agentHostEnabled: false });
		assert.ok(provider.sessionTypes.some(t => t.id === SingularityCLISessionType.id));
	});

	test('Agent Host availability is observed after the provider is created', () => {
		const { provider, agentHostEnabled } = createProviderWithConfig(disposables, model, { hideSingularityCli: true, agentHostEnabled: false });
		let changeCount = 0;
		disposables.add(provider.onDidChangeSessionTypes(() => changeCount++));
		const visibleBeforeAvailability = provider.sessionTypes.some(t => t.id === SingularityCLISessionType.id);

		agentHostEnabled.set(true, undefined);

		assert.deepStrictEqual({
			visibleBeforeAvailability,
			visibleAfterAvailability: provider.sessionTypes.some(t => t.id === SingularityCLISessionType.id),
			changeCount,
		}, {
	...
	// Note: createNewSession tests are limited because SingularityCLISession
	...
	test('cloud models resolve arbitrary restored ids with option groups', () => {
		...
		const session = provider.createNewSession(workspace, SingularityCloudSessionType.id);
	...
	test('Singularity CLI keeps an empty Singularity catalog pending until live models arrive', () => {
		...
		const session = provider.createNewSession(URI.file('/test/project'), SingularityCLISessionType.id);
		const empty = provider.getModelsSnapshot(session.sessionId, 'singularity/remembered');

		models.set('singularity/other', {
			...
			targetChatSessionType: SingularityCLISessionType.id,
		});
		const live = provider.getModelsSnapshot(session.sessionId, 'singularity/remembered');
		...
		assert.deepStrictEqual({
			...
			empty: { resolution: { kind: 'pending', identifier: 'singularity/remembered' }, modelTarget: SingularityCLISessionType.id },
			live: { resolution: { kind: 'unavailable', identifier: 'singularity/remembered' }, modelTarget: SingularityCLISessionType.id },
		});
	});

	test('Singularity CLI session maps workspace selection to Agent Host folder config', async () => {
		const provider = createProviderForSendTests(disposables, model, async () => ({ kind: 'sent' as const, data: {} as IChatSendRequestData }));
		const session = provider.createNewSession(URI.file('/test/project'), SingularityCLISessionType.id);
		const providerSession = provider.getSession(session.sessionId) as ISingularityChatSession & IDisposable & { getAgentHostSessionConfig(): Record<string, unknown> };
	...
	test('Singularity CLI session maps worktree selection to Agent Host config', async () => {
		const provider = createProviderForSendTests(disposables, model, async () => ({ kind: 'sent' as const, data: {} as IChatSendRequestData }));
		const session = provider.createNewSession(URI.file('/test/project'), SingularityCLISessionType.id);
		const providerSession = provider.getSession(session.sessionId)! as ISingularityChatSession & IDisposable & { getAgentHostSessionConfig(): Record<string, unknown> };
	...
	test('Singularity CLI session forwards git.branchPrefix as worktreeBranchPrefix for worktree isolation', async () => {
		const configService = new TestConfigurationService();
	...
	test('Singularity CLI session forwards git.worktreeIncludeFiles for worktree isolation', async () => {
		const configService = new TestConfigurationService();
	...
	test('singularity CLI sessions have supportsMultipleChats capability', () => {
		const resource = URI.from({ scheme: AgentSessionProviders.Background, path: '/session-1' });
	...
	test('singularity cloud sessions do not have supportsMultipleChats capability', () => {
	...
	test('singularity CLI sessions do not have supportsMultipleChats when setting is disabled', () => {
		const resource = URI.from({ scheme: AgentSessionProviders.Background, path: '/session-1' });
		model.addSession(createMockAgentSession(resource));
	...
	test('setModel applies to existing sessions and their new chats', async () => {
		...
		const provider = createProvider(disposables, model);
		const session = provider.getSessions()[0];
		provider.setModel(session.sessionId, 'singularity/gpt-4o');

		assert.strictEqual(session.modelId.get(), 'singularity/gpt-4o');

		const chat = await provider.createNewChat(session.sessionId);
		try {
			assert.strictEqual(chat.modelId.get(), 'singularity/gpt-4o');
		} finally {
	...
	test('deleteSession passes Singularity CLI session label to delete command', async () => {
		const resource = URI.from({ scheme: SingularityCLISessionType.id, path: '/session-1' });
		const commandExecutions: IExecutedCommand[] = [];
		model.addSession(createMockAgentSession(resource, { providerType: SingularityCLISessionType.id, title: 'Fix Build' }));
		...
		assert.deepStrictEqual(commandExecutions.map(command => ({
			...
			id: 'agents.singularity.chat.cli.deleteSessions',
	...
	function makeClaudeInFlightProvider(): { provider: SingularityChatSessionsProvider; cancelRequest: () => void; realResource: URI; commitSession: () => void } {
	...
	function waitForSessionAdded(provider: SingularityChatSessionsProvider): Promise<void> {
	...
	suite('uncommitted temp session cleanup', () => {
		...
		function makeInFlightProvider(): {
			provider: SingularityChatSessionsProvider;
			cancelRequest: () => void;
		} {
			let resolveComplete!: () => void;
			let resolveCreated!: (r: IChatResponseModel) => void;
		...
		test('deleteSession removes a temp session that is awaiting commit', async () => {
			...
			const newSession = provider.createNewSession(workspace, SingularityCLISessionType.id);
	...
	suite('new session default permission level', () => {
		...
		test('CLI session seeds permission level from chat.permissions.default', () => {
			...
			const sessionInfo = provider.createNewSession(workspace, SingularityCLISessionType.id);
