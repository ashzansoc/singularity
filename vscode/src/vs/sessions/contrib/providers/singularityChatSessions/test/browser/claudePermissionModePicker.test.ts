...
import { SingularityChatSessionsProvider, ISingularityChatSession } from '../../browser/singularityChatSessionsProvider.js';
...
function createPicker(
	...
	const mockSession: Partial<ISingularityChatSession> = {
		setOption: setOptionSpy as ISingularityChatSession['setOption'],
	};

	const provider = Object.assign(Object.create(SingularityChatSessionsProvider.prototype), {
...
suite('ClaudePermissionModePicker', () => {
	...
	test('shows bypass permissions option when setting is enabled', () => {
		const { picker, actionWidgetItems } = createPicker(disposables, {
			configValues: { 'singularity.chat.chat.claudeAgent.allowDangerouslySkipPermissions': true },
		});
		const container = document.createElement('div');
		picker.render(container);
		showPicker(container);
