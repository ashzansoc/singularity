...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	private _rootStateValue: RootState | Error | undefined = { agents: [{ provider: 'singularitycli', displayName: 'Singularity', description: '', models: [], capabilities: { multipleChats: { fork: true } } } as AgentInfo] };
	...
	override async createSession(config?: IAgentCreateSessionConfig): Promise<URI> {
		const uri = config?.session ?? URI.parse('singularitycli:///auto-' + this._nextSeq);
...
function createSession(id: string, opts?: { provider?: string; summary?: string; project?: { uri: URI; displayName: string }; workingDirectory?: URI; startTime?: number; modifiedTime?: number; quickChat?: boolean; multiRoot?: { workspaceFile: string } }): IAgentSessionMetadata {
	...
	return {
		session: AgentSession.uri(opts?.provider ?? 'singularitycli', id),
...
function createProvider(disposables: DisposableStore, agentHostService: MockAgentHostService, contributions = [
	{ type: 'agent-host-singularitycli', name: 'singularity', displayName: 'Singularity', description: 'test', icon: undefined },
...
function createTestLanguageModel(id: string): ILanguageModelChatMetadata {
	return {
		...
		id,
		vendor: 'agent-host-singularitycli',
		name: id,
		version: '1.0',
		family: id,
		maxInputTokens: 1,
...
function fireSessionAdded(agentHost: MockAgentHostService, rawId: string, opts?: { provider?: string; title?: string; project?: { uri: string; displayName: string }; workingDirectory?: string; changes?: ChangesSummary; workspaceless?: boolean; createdAt?: string; modifiedAt?: string }): void {
	const provider = opts?.provider ?? 'singularitycli';
...
function fireSessionMetaChanged(agentHost: MockAgentHostService, rawId: string, meta: Record<string, unknown> | undefined, provider = 'singularitycli'): void {
...
function fireSessionRemoved(agentHost: MockAgentHostService, rawId: string, provider = 'singularitycli'): void {
	const sessionUri = AgentSession.uri(provider, rawId);
	agentHost.fireNotification({
		channel: 'ahp-root://',
...
function fireSessionSummaryChanged(agentHost: MockAgentHostService, rawId: string, changes: Partial<SessionSummary>, provider = 'singularitycli'): void {
	const sessionUri = AgentSession.uri(provider, rawId);
	agentHost.fireNotification({
		channel: 'ahp-root://',
...
suite('LocalAgentHostSessionsProvider', () => {
	...
	test('has correct id, label, and sessionType from rootState agents', () => {
		...
		// the same agent (e.g. `singularitycli`) shares one session type across
		// local and remote hosts and the standalone Singularity CLI provider.
		assert.strictEqual(provider.sessionTypes[0].id, 'singularitycli');
		assert.strictEqual(provider.sessionTypes[0].label, 'Singularity');
	});
	...
	test('session types update when the local host advertises additional agents', () => {
		...
		assert.deepStrictEqual(provider.sessionTypes.map(t => ({ id: t.id, label: t.label })), [
			{ id: 'singularitycli', label: 'Singularity' },
		]);
		...
		agentHost.setAgents([
			{ provider: 'singularitycli', displayName: 'Singularity', description: '', models: [] } as AgentInfo,
	...
	test('shares the root-state listener across session adapters', () => {
		agentHost.setAgents([{ provider: 'singularitycli', displayName: 'Singularity', description: '', models: [], capabilities: {} } as AgentInfo]);
		const provider = createProvider(disposables, agentHost);
		...
		agentHost.setAgents([{ provider: 'singularitycli', displayName: 'Singularity', description: '', models: [], capabilities: { multipleChats: { fork: true } } } as AgentInfo]);
	...
	test('rebinds session types when Agent Host starts with a new root subscription', () => {
		...
		assert.deepStrictEqual({
			...
			sessionTypes: [{ id: 'singularitycli', label: 'Singularity' }],
	...
	test('session type icons use per-agent codicons', () => {
		...
		assert.deepStrictEqual(
			...
			[
				{ id: 'singularitycli', icon: 'singularity' },
	...
	// the extension-host Claude (the GitHub Singularity Chat extension's), the local
	...
	// `singularityChatSessionsProvider`.
	...
	test('hides agent-host Claude when the Agents window prefers extension-host Claude', () => {
		...
		assert.deepStrictEqual(provider.sessionTypes.map(t => t.id), ['singularitycli']);
	});
	...
	test('shows agent-host Claude when the Agents window prefers agent-host Claude', () => {
		...
		assert.deepStrictEqual(provider.sessionTypes.map(t => t.id), ['singularitycli', 'claude']);
	});
	...
	test('gates agent-host Codex in the Agents window on the provider enablement setting', () => {
		...
		assert.deepStrictEqual({
			...
			sessionTypes: ['singularitycli', 'codex'],
		});
	});
	...
	test('getSessions hides agent-host Claude sessions when extension-host Claude is preferred', () => {
		...
		configService.setUserConfiguration(ClaudePreferAgentHostAgentsSettingId, false);
		const provider = createProvider(disposables, agentHost, undefined, { configurationService: configService, isSessionsWindow: true });
		fireSessionAdded(agentHost, 'cli-sess', { title: 'CLI', provider: 'singularitycli' });
		fireSessionAdded(agentHost, 'claude-sess', { title: 'Claude', provider: 'claude' });

		assert.deepStrictEqual(provider.getSessions().map(s => s.sessionType), ['singularitycli']);
	});
	...
	test('session icons match the session type icon', () => {
		...
		assert.deepStrictEqual(
			...
			[
				...
				{ sessionType: 'singularitycli', icon: 'singularity' },
	...
	test('a session whose agent reports nothing survives the refresh', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.addSession(createSession('cli-1', { provider: 'singularitycli', summary: 'CLI One' }));
		...
		agentHost.fireAction({
			channel: buildDefaultChatUri(AgentSession.uri('singularitycli', 'cli-1').toString()),
	...
	test('a session missing while its agent still reports others is evicted', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.addSession(createSession('cli-gone', { provider: 'singularitycli', summary: 'Gone' }));
		agentHost.addSession(createSession('cli-kept', { provider: 'singularitycli', summary: 'Kept' }));
		...
		agentHost.fireAction({
			channel: buildDefaultChatUri(AgentSession.uri('singularitycli', 'cli-kept').toString()),
			action: { type: ActionType.ChatTurnComplete },
			serverSeq: 1,
			origin: undefined,
		} as ActionEnvelope);
	...
	test('hydrated quick chat stays workspace-less after reload despite a scratch working directory', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		await persistCachedSessions(disposables, storageService, [
			createSession('quick-cached', {
				summary: 'Quick Chat',
				workingDirectory: URI.file('/tmp/singularity-scratch/quick-cached'),
				quickChat: true,
			}),
		]);
	...
	test('hydrated session preserves multi-root metadata after reload', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		nextHost.fireAction({
			channel: AgentSession.uri('singularitycli', 'multi-root-cached').toString(),
	...
	test('a refresh publishes _meta and summary fields as one atomic update', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.fireAction({
			channel: buildDefaultChatUri(AgentSession.uri('singularitycli', 'atomic-1').toString()),
			action: { type: ActionType.ChatTurnComplete },
			serverSeq: 1,
			origin: undefined,
		} as ActionEnvelope);
	...
	test('uses project metadata as workspace group source', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const workingDirectory = URI.file('/tmp/singularity-worktrees/vscode-feature');
	...
	test('getModels returns only models targeting the session resource scheme', () => {
		const matchingModel = { ...createTestLanguageModel('matching'), targetChatSessionType: 'agent-host-singularitycli' };
		...
		assert.deepStrictEqual({
			...
			models: ['matching'],
			modelTarget: 'agent-host-singularitycli',
		});
	});
	...
	test('getModelsSnapshot canonicalizes a matching logical-session model identifier', () => {
		...
		const logicalIdentifier = `singularitycli/${modelId}`;
		const unrelatedIdentifier = `other/${modelId}`;
		const targetIdentifier = `agent-host-singularitycli:${modelId}`;
		const languageModelIds = [logicalIdentifier, unrelatedIdentifier];
		const languageModels = new Map([
			[logicalIdentifier, { ...createTestLanguageModel(modelId), vendor: 'singularitycli', targetChatSessionType: 'singularitycli' }],
			[unrelatedIdentifier, { ...createTestLanguageModel(modelId), vendor: 'other', targetChatSessionType: 'other' }],
			[targetIdentifier, { ...createTestLanguageModel(modelId), targetChatSessionType: 'agent-host-singularitycli' }],
		]);
		const provider = createProvider(disposables, agentHost, undefined, {
			languageModelIds,
	...
	test('setModel updates existing session model and lets draft debounce persist it', () => {
		...
		provider.setModel(session!.sessionId, 'agent-host-singularitycli:new-model');

		assert.strictEqual(session!.modelId.get(), 'agent-host-singularitycli:new-model');
	...
	test('setModel updates cached selection for later message-level selection', () => {
		...
		provider.setModel(session!.sessionId, 'agent-host-singularitycli:configured-model');

		assert.strictEqual(session!.modelId.get(), 'agent-host-singularitycli:configured-model');
		assert.deepStrictEqual(agentHost.dispatchedActions, []);
	});
	...
	test('restores the selected agent from the default chat draft on resume', () => {
		...
		const defaultChatUri = buildDefaultChatUri(AgentSession.uri('singularitycli', 'resume-agent'));
	...
	test('does not override a live agent selection with the persisted draft agent', () => {
		...
		const defaultChatUri = buildDefaultChatUri(AgentSession.uri('singularitycli', 'resume-nooverride'));
		agentHost.setChatState(defaultChatUri, {
			resource: defaultChatUri,
	...
	test('rebases the selected agent to its worktree twin from the agent list before the working directory flips', () => {
		...
		provider.getSessionConfig(session!.sessionId);
		agentHost.setSessionState('rebase-worktree', 'singularitycli', {
	...
	test('leaves the selected agent untouched when the agent list has no relocated twin', () => {
		...
		provider.getSessionConfig(session!.sessionId);
		agentHost.setSessionState('rebase-none', 'singularitycli', {
			provider: 'singularitycli',
			title: 'Rebase None',
			status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
	...
	test('getCustomAgents collects agents from session customizations, coalesced by URI and sorted by name', async () => {
		...
		provider.getSessionConfig(session!.sessionId);
		agentHost.setSessionState('agents-merge', 'singularitycli', fakeState);
	...
	test('getMcpServers dispatches MCP lifecycle requests', async () => {
		...
		provider.getSessionConfig(session!.sessionId);
		agentHost.setSessionState('mcp-lifecycle', 'singularitycli', fakeState);
	...
	test('getBackendChatResource looks up the host-supplied backend chat URI', () => {
		...
		const backendSession = AgentSession.uri('singularitycli', 'backend-abc').toString();
		...
		provider.getSessionConfig(session.sessionId);
		agentHost.setSessionState('chat-lookup', 'singularitycli', fakeState);
	...
	test('onDidChangeCustomAgents fires on root state and session state changes', async () => {
		...
		provider.getSessionConfig(session!.sessionId);
		agentHost.setSessionState('cust-events', 'singularitycli', {
			provider: 'singularitycli',
			title: 'Cust Events',
			status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
	...
	test('createQuickChat throws when no agents are advertised', () => {
		agentHost.setAgents([]);
		const provider = createProvider(disposables, agentHost);
		assert.throws(() => provider.createQuickChat('singularitycli'));
	});
	...
	test('restores a quick chat from listSessions as workspace-less despite a scratch working directory', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.addSession(createSession('quick-1', {
			summary: 'Quick Chat',
			workingDirectory: URI.file('/tmp/singularity-scratch/quick-1'),
			quickChat: true,
		}));
	...
	test('restored quick chat collapses to a single chat even when state advertises peer chats', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.addSession(createSession('quick-multi', {
			summary: 'Quick Chat',
			workingDirectory: URI.file('/tmp/singularity-scratch/quick-multi'),
			quickChat: true,
		}));
		...
		const sessionUri = AgentSession.uri('singularitycli', 'quick-multi').toString();
		const defaultChat = buildDefaultChatUri(sessionUri);
		agentHost.setSessionState('quick-multi', 'singularitycli', {
			provider: 'singularitycli',
			title: 'Quick Chat',
			status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
	...
	test('promotes an untagged session to a quick chat once state reports it workspace-less, and persists the promotion', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.addSession(createSession('quick-untagged', {
			summary: 'Quick Chat',
			workingDirectory: URI.file('/home/user/.singularity/chats/quick-untagged'),
		}));
		...
		const sessionUri = AgentSession.uri('singularitycli', 'quick-untagged').toString();
		const defaultChat = buildDefaultChatUri(sessionUri);
		agentHost.setSessionState('quick-untagged', 'singularitycli', {
			provider: 'singularitycli',
			title: 'Quick Chat',
			status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
	...
	test('reports a kind-only promotion so the list regroups a session that never had a workspace', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const sessionUri = AgentSession.uri('singularitycli', 'quick-no-cwd').toString();
		const defaultChat = buildDefaultChatUri(sessionUri);
		agentHost.setSessionState('quick-no-cwd', 'singularitycli', {
			provider: 'singularitycli',
			title: 'Quick Chat',
			status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
	...
	test('listing reconcile promotes a cached adapter in place and announces the regroup', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const storageService = disposables.add(new InMemoryStorageService());
		const scratchDir = URI.file('/home/user/.singularity/chats/quick-poisoned');
		await persistCachedSessions(disposables, storageService, [
	...
	test('committed quick chat announced via sessionAdded stays workspace-less despite a scratch working directory', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		fireSessionAdded(agentHost, 'quick-committed', {
			title: 'Quick Chat',
			workingDirectory: URI.file('/tmp/singularity-scratch/quick-committed').toString(),
			workspaceless: true,
		});
	...
	test('joins the active client with customizations when opening an existing session', () => {
		...
		const resource = URI.from({ scheme: 'agent-host-singularitycli', path: '/active-client' });
		activeSession.set({
			providerId: provider.id,
		...
		assert.deepStrictEqual(agentHost.dispatchedActions.filter(dispatch => dispatch.action.type === ActionType.SessionActiveClientSet), [{
			channel: AgentSession.uri('singularitycli', 'active-client').toString(),
			action: {
	...
	test('deleteSession calls disposeSession and removes from cache', async () => {
		...
		assert.strictEqual(AgentSession.provider(disposedUri), 'singularitycli');
	...
	test('renameSession dispatches SessionTitleChanged on the session channel', async () => {
		...
		assert.strictEqual(AgentSession.provider(actionSession), 'singularitycli');
	...
	test('renameChat on the default chat renames the chat tab, not the session', async () => {
		...
		const dispatched = agentHost.dispatchedActions[0];
		assert.strictEqual(dispatched.action.type, ActionType.SessionTitleChanged);
		assert.strictEqual(dispatched.channel.toString(), buildDefaultChatUri(AgentSession.uri('singularitycli', 'rename-default-chat').toString()));
	});
	...
	suite('multi-chat catalog', () => {
		...
		test('default + peer catalog surfaces both chats with the default as mainChat', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-1').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');

			agentHost.setSessionState('multi-1', 'singularitycli', makeState([
		...
		test('equivalent chat catalogs do not notify chat observers', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-stable').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');
			...
			agentHost.setSessionState('multi-stable', 'singularitycli', makeCatalog());
			let updateCount = 0;
			disposables.add(autorun(reader => {
		...
		test('equivalent peer chat values do not notify observers', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-values').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');
			const makeCatalog = () => makeState([
				makeChatSummary(defaultChat, ''),
			...
			agentHost.setSessionState('multi-values', 'singularitycli', makeCatalog());
		...
		test('peer chats map protocol interactivity to the provider-agnostic tri-state', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-ro').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const readOnlyPeer = buildChatUri(sessionUri, 'peer-ro');
			const hiddenPeer = buildChatUri(sessionUri, 'peer-hidden');

			agentHost.setSessionState('multi-ro', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
		...
		test('subagent (tool-origin) chats surface as read-only peers', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-sub').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const subagentChat = buildSubagentChatUri(sessionUri, 'tc-1');

			agentHost.setSessionState('multi-sub', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
		...
		test('the main chat is renameable but never deletable via capabilities', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'main-caps').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');

			agentHost.setSessionState('main-caps', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
		...
		test('a new peer chat is presented as Untitled until its first request is sent', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-new').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');

			(session as AgentHostSessionAdapter).markChatAsNew('peer-1');
			agentHost.setSessionState('multi-new', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
				makeChatSummary(peerChat, 'Peer'),
			], { defaultChat }));
		...
		test('a peer catalog collapsed while capabilities were absent re-expands when they hydrate', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-late-caps').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');

			agentHost.setSessionState('multi-late-caps', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
				makeChatSummary(peerChat, 'Peer'),
			], { defaultChat }));
		...
		test('forkChat forwards the source chat and turn to the host and surfaces a new peer chat', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-fork').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);

			agentHost.setSessionState('multi-fork', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
			], { defaultChat }));
		...
		test('createSideChat forwards the source chat and turn to the host and surfaces a new peer chat', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			agentHost.setAgents([{ provider: 'singularitycli', displayName: 'Singularity', description: '', models: [], capabilities: { multipleChats: { fork: true, sideChat: true } } } as AgentInfo]);
			const provider = createProvider(disposables, agentHost);
			const session = setupMultiChatSession(provider, 'multi-side-chat');
			const sessionUri = AgentSession.uri('singularitycli', 'multi-side-chat').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);

			agentHost.setSessionState('multi-side-chat', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
			], { defaultChat }));
		...
		test('createSideChat inherits model and agent selection from the source peer chat', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-side-chat-peer-selection').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');
			agentHost.setSessionState('multi-side-chat-peer-selection', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
				makeChatSummary(peerChat, 'Peer'),
			], { defaultChat }));
			...
			provider.setModel(session.sessionId, 'agent-host-singularitycli:peer-model');
			...
			assert.deepStrictEqual({
				...
				peerInputSelectedModels: ['agent-host-singularitycli:peer-model'],
		...
		test('createNewChat forwards the selected model to the host and seeds the chat input state', async () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-model').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			agentHost.setSessionState('multi-model', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
			], { defaultChat }));

			provider.setModel(session.sessionId, 'agent-host-singularitycli:selected-model');
			...
			assert.deepStrictEqual({
				...
				peerInputSelectedModels: ['agent-host-singularitycli:selected-model'],
			});
		});
		...
		test('sendRequest keeps a peer chat model loaded while dispatching', async () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-send-peer').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');
			agentHost.setSessionState('multi-send-peer', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
				makeChatSummary(peerChat, 'Peer'),
			], { defaultChat }));
			const peer = session.chats.get().find(c => c.resource.fragment === 'peer-1');
		...
		test('setModel updates the active peer chat model without changing the default chat model', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-active-model').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');
			agentHost.setSessionState('multi-active-model', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
				makeChatSummary(peerChat, 'Peer'),
			], { defaultChat }));
			...
			assert.deepStrictEqual({
				...
				peerModelId: 'agent-host-singularitycli:peer-model',
			});
		});
		...
		test('deleteChat prompts for confirmation and disposes the peer chat when confirmed', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-del').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');

			agentHost.setSessionState('multi-del', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
				makeChatSummary(peerChat, 'Peer'),
			], { defaultChat }));
		...
		test('deleteChat does not dispose the peer chat when the confirmation is cancelled', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-del-cancel').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');

			agentHost.setSessionState('multi-del-cancel', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
				makeChatSummary(peerChat, 'Peer'),
			], { defaultChat }));
		...
		test('single-chat catalog degrades to the default chat only', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-single').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);

			agentHost.setSessionState('multi-single', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
			], { defaultChat }));
		...
		test('removing a peer from the catalog drops it back to the default chat', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-remove').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');

			agentHost.setSessionState('multi-remove', 'singularitycli', makeState([
				makeChatSummary(defaultChat, ''),
				makeChatSummary(peerChat, 'Peer'),
			], { defaultChat }));
		...
		test('default chat title diverges from the session title when renamed in the catalog', () => {
			...
			const sessionUri = AgentSession.uri('singularitycli', 'multi-title').toString();
			const defaultChat = buildDefaultChatUri(sessionUri);
			const peerChat = buildChatUri(sessionUri, 'peer-1');

			agentHost.setSessionState('multi-title', 'singularitycli', makeState([
	...
	test('server-echoed SessionTitleChanged updates cached title', () => {
		...
		agentHost.fireAction({
			channel: AgentSession.uri('singularitycli', 'echo-sess').toString(),
			action: {
	...
	test('server-echoed ChatTurnStarted model does not update cached session model', () => {
		...
		assert.ok(target);
		provider.setModel(target!.sessionId, 'agent-host-singularitycli:old-model');

		const changes: ISessionChangeEvent[] = [];
		disposables.add(provider.onDidChangeSessions(e => changes.push(e)));

		agentHost.fireAction({
			channel: AgentSession.uri('singularitycli', 'model-change').toString(),
			action: {
		...
		assert.strictEqual(target!.modelId.get(), 'agent-host-singularitycli:old-model');
		assert.strictEqual(changes.length, 0);
	});
	...
	test('turnComplete action triggers session refresh', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.fireAction({
			channel: buildDefaultChatUri(AgentSession.uri('singularitycli', 'turn-sess').toString()),
			action: {
	...
	test('sendRequest only commits a session of the same type, ignoring a foreign-type session that appears mid-send', async () => {
		...
		// singularity). When a slow session (e.g. codex cold start) is sent while a
	...
	test('sendRequest waits beyond 30 seconds for the backend session to commit', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.fireAction({
			channel: buildDefaultChatUri(AgentSession.uri('singularitycli', session.sessionId).toString()),
			action: { type: ActionType.ChatTurnComplete },
			serverSeq: 1,
			origin: undefined,
		} as ActionEnvelope);
	...
	test('sendRequest clears chat input draft while preserving selected model and agent', async () => {
		...
		const provider = createProvider(disposables, agentHost, undefined, {
			lookupLanguageModel: modelId => modelId === 'agent-host-singularitycli:selected-model' ? languageModel : undefined,
		...
		assert.deepStrictEqual({
			...
			hasSelectedModelUpdate: inputStates.some(state => state.selectedModel?.identifier === 'agent-host-singularitycli:selected-model'),
	...
	test('getSessionConfig seeds running config from session state subscription with full schema', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			provider: 'singularitycli', title: 'Seeded Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
			config,
		};
		agentHost.setSessionState('seed-1', 'singularitycli', fakeState);
	...
	test('running config state seeding preserves already-resolved schema properties', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fullState: SessionState = {
			provider: 'singularitycli', title: 'Schema Preserve Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
			config: {
		...
		agentHost.setSessionState('seed-schema', 'singularitycli', fullState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.schema.properties['codex.networkAccessEnabled'] !== undefined);

		agentHost.setSessionState('seed-schema', 'singularitycli', {
			...fullState,
			config: {
				schema: {
					type: 'object',
	...
	test('removing a session disposes its session-state subscription', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		provider.getSessionConfig(session!.sessionId);
		const sessionUriStr = AgentSession.uri('singularitycli', 'seed-2').toString();
	...
	test('session-state subscription auto-releases after the idle window', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const sessionUriStr = AgentSession.uri('singularitycli', 'idle-1').toString();
	...
	test('equivalent GitHub info does not notify observers', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		assert.ok(session);
		provider.getSessionConfig(session.sessionId);
		agentHost.setSessionState('github-stable', 'singularitycli', {
			provider: 'singularitycli',
			title: 'PR Session',
			status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
	...
	test.skip('keeps a resolved PR number sticky across gitHubInfo recomputes (no re-lookup / icon flap)', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		provider.getSessionConfig(session!.sessionId);
		agentHost.setSessionState('pr-sticky', 'singularitycli', {
			provider: 'singularitycli', title: 'PR Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
	...
	test('surfaces a default open-PR icon immediately when a PR is detected before the live model loads', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		provider.getSessionConfig(session!.sessionId);
		agentHost.setSessionState('pr-default-icon', 'singularitycli', {
			provider: 'singularitycli', title: 'PR Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
	...
	test('replaceSessionConfig only replaces sessionMutable, non-readOnly values and preserves everything else', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			provider: 'singularitycli', title: 'Replace Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
			config,
		};
		agentHost.setSessionState('rep-1', 'singularitycli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');
		...
		const sessionUri = AgentSession.uri('singularitycli', 'rep-1').toString();
	...
	test('running session config writes clamp autoApprove to default when policy disables global auto-approve', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			provider: 'singularitycli', title: 'Policy Write Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
			config,
		};
		agentHost.setSessionState('policy-write', 'singularitycli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');

		await provider.setSessionConfigValue(session!.sessionId, SessionConfigKey.AutoApprove, 'autopilot');
		const sessionUri = AgentSession.uri('singularitycli', 'policy-write').toString();
	...
	test('running session config write re-resolves schema-dependent properties', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			provider: 'singularitycli', title: 'Schema Write Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
			config,
		};
		agentHost.setSessionState('schema-write', 'singularitycli', fakeState);
		...
		agentHost.setSessionState('schema-write', 'singularitycli', {
			...fakeState,
			config: {
				...config,
	...
	test('replaceSessionConfig is a no-op when nothing editable actually changes', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			provider: 'singularitycli', title: 'No-op Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
			config,
		};
		agentHost.setSessionState('rep-2', 'singularitycli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');
	...
	test('server-echoed SessionConfigChanged merges config values into the running cache by default', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			provider: 'singularitycli', title: 'Merge Session', status: ProtocolSessionStatus.Idle,
			lifecycle: SessionLifecycle.Ready,
			activeClients: [],
			chats: [],
			config: {
		...
		agentHost.setSessionState('cfg-merge', 'singularitycli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');

		agentHost.fireAction({
			channel: AgentSession.uri('singularitycli', 'cfg-merge').toString(),
			action: {
				type: ActionType.SessionConfigChanged,
	...
	test('server-echoed SessionConfigChanged with replace:true overwrites the running cache', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.setSessionState('cfg-replace', 'singularitycli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');

		agentHost.fireAction({
			channel: AgentSession.uri('singularitycli', 'cfg-replace').toString(),
			action: {
				type: ActionType.SessionConfigChanged,
	...
	test('keeps a visible session subscribed so host-spawned subagent chats keep reaching the catalog', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const sessionUri = AgentSession.uri('singularitycli', 'subagent-live').toString();
		const defaultChat = buildDefaultChatUri(sessionUri);
		...
		const stateWith = (chats: ChatSummary[]): SessionState => ({
			provider: 'singularitycli', title: 'Lead', status: ProtocolSessionStatus.Idle,
		...
		agentHost.setSessionState('subagent-live', 'singularitycli', stateWith([defaultSummary, toolChat(subagentOne, 'tc-1', 'Add name to README')]));
		...
		agentHost.setSessionState('subagent-live', 'singularitycli', stateWith([
			defaultSummary,
...
suite.skip('LocalAgentHostSessionsProvider - active-session branch changeset subscription', () => {
	...
	function makeActive(rawId: string, sessionType: string = 'singularitycli', status: SessionStatus = SessionStatus.Completed): IActiveSession {
		return {
			// providerId: 'unused',
			sessionType,
	...
	function branchChangesKeyFor(rawId: string, sessionType: string = 'singularitycli'): string {
	...
	test('does NOT subscribe to uncommitted changes for an untitled active session', () => {
		...
		activeSession.set(makeActive('sess-new', 'singularitycli', SessionStatus.Untitled), undefined);
