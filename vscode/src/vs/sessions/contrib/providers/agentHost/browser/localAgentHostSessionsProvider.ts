...
export class LocalAgentHostSessionsProvider extends BaseAgentHostSessionsProvider {
	 ...
	 * order). Otherwise it sorts after the default providers so Singularity AI
	 * keeps precedence.
	 */
	...
	constructor(
		...
		// settle because the underlying agent (e.g. SingularityAgent) throws
	 ...
	 * (e.g. `singularitycli`) so the same agent shares one session type across
