...
import { AgentSession, AuthenticateParams, AuthenticateResult, IAgentConnection, IAgentSessionMetadata, protectedResourcesRequireSingularitySignIn } from '../../../../../platform/agentHost/common/agentService.js';
...
/** Singularity CLI session type */
export const SingularityCLISessionType: ISessionType = {
	id: 'singularitycli',
	label: localize('singularityCLI', "Singularity"),
 ...
 * advertises. An agent that still requires the Singularity protected
...
export function resolveAgentAuthRequirement(agent: AgentInfo): SessionTypeAuthRequirement {
	if (!agent.protectedResources || protectedResourcesRequireSingularitySignIn(agent.protectedResources)) {
...
export interface IAgentHostAdapterOptions {
	 ...
	 * resolvers. Shared with the Singularity chat sessions provider so all
	 ...
	 * (cloud sandbox: provider `singularity`, sessions `ahp-session:/<id>`). Defaults to the provider.
	 */
...
export class AgentHostSessionAdapter extends Disposable implements ISession {
	 ...
	 * than {@link agentProvider} (cloud sandbox: provider `singularity`, backend `ahp-session:/<id>`).
	 ...
	 * multiple chats (the `singularitycli` case) OR when it is a subagent
...
interface INewSessionConstructionContext {
	 ...
	 * while the agent provider is `singularity`; the eager backend `createSession`/
...
class NewSession extends Disposable {
	...
	constructor(
		...
		// scheme (cloud sandbox: provider `singularity`, scheme `ahp-session`) override it.
...
export abstract class BaseAgentHostSessionsProvider extends Disposable implements IAgentHostSessionsProvider {
	 ...
	 * its agent provider is `singularity`), so that routing, persistence, and content
	 ...
	 * identity (scheme == provider), which holds for every host except the Singularity
	 * host used by cloud sandbox, whose sessions are addressed under
	 * `ahp-session:/<id>` while the agent provider is `singularity`. Subclasses
	 ...
	 * agent provider name itself, so the same agent (e.g. `singularitycli`)
	 * appears under one shared session type across hosts.
	 */
	protected abstract resourceSchemeForProvider(provider: string): string;

	/** Format the human-readable label for a session type entry (e.g. `Singularity`). */
	protected abstract _formatSessionTypeLabel(agentLabel: string): string;

	protected _brandSessionTypeLabel(agentLabel: string): string {
		if (/^singularity$/i.test(agentLabel) || agentLabel === 'singularitycli') {
			return localize('singularityCLI', "Singularity");
		}
		return agentLabel;
	}
	...
	protected _syncSessionTypesFromRootState(rootState: RootState): void {
		...
		const next = rootState.agents
			...
			.map((agent): ISessionType => ({
				id: agent.provider,
				supportsWorktreeConfiguration: agent.provider === SingularityCLISessionType.id,
				...
				label: this._formatSessionTypeLabel(this._brandSessionTypeLabel(agent.displayName?.trim() || agent.provider)),
	...
	private iconForAgentProvider(provider: string): ThemeIcon | undefined {
		if (provider === SingularityCLISessionType.id) {
	...
	private _ensureSessionStateSubscription(sessionId: string): void {
		...
		// A surfaced-but-un-adopted legacy Singularity CLI session must NOT be
	 ...
	 * **every** agent-host session type (codex, claude, singularity, …). A send
