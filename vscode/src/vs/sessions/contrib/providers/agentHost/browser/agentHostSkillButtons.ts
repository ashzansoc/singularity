 ...
 * skill toolbar buttons and to suppress the Singularity CLI extension's own
 ...
 * `agent-host-*` session.
 *
 * They mirror the buttons the Singularity CLI extension contributes for its own
 * `chatSessionType == singularitycli` sessions, sending the same `/<skill-name>`
 ...
 * as the Singularity CLI extension's Sync PR button. Exported so the changes
...
function registerAgentHostSkillButton(spec: IAgentHostSkillButtonSpec): void {
	registerAction2(class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			// `agent-host-singularitycli`), which is the agent id the chat
			// service uses for routing. The `sessionType` field is the
			// logical, user-facing id (e.g. `singularitycli`) that is shared
			// between the Singularity CLI extension and the local/remote
