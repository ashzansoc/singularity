...
suite('LocalChatSessionsProvider', () => {
	...
	test('keeps empty Singularity resolution pending until live Singularity models arrive', () => {
		const store = leaks.add(new DisposableStore());
		const { instantiationService } = createFixture(store);
		...
		const provider = store.add(instantiationService.createInstance(LocalChatSessionsProvider));
		const emptySingularity = provider.getModelsSnapshot('session', 'singularity/remembered');
		...
		const liveSingularity = provider.getModelsSnapshot('session', 'singularity/remembered');

		assert.deepStrictEqual({
			emptySingularity: emptySingularity.desiredModelResolution,
			emptyByok: emptyByok.desiredModelResolution,
			liveSingularity: liveSingularity.desiredModelResolution,
			models: liveSingularity.models.map(model => model.identifier),
			modelTarget: liveSingularity.modelTarget,
		}, {
