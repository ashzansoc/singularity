...
suite('NewChatVoiceTargetService', () => {
	...
	test('bound input resource follows composer > active created session > focus priority', () => {
		...
		const chatResource = URI.parse('agent-host-singularity:/session-1');
		...
		const otherResource = URI.parse('agent-host-singularity:/session-2');
	...
	test('new-session composer does not show another session voice connection as active', () => {
		...
		assert.strictEqual(isNewChatVoiceSessionActive(true, false, URI.parse('agent-host-singularity:/session-1'), false), false);
