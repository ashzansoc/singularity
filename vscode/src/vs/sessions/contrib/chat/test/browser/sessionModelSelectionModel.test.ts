...
suite('SessionModelSelectionModel', () => {
	...
	test('new Codex sessions use the most recently selected provider model', () => {
		...
		const singularityModel = {
			...model('codex:@provider=vscode-proxy:gpt-test'),
			metadata: { ...model('codex:@provider=vscode-proxy:gpt-test').metadata, modelGroup: { id: 'singularity' } },
		};
		const chatGPTModel = {
		...
		provider.models = [singularityModel, chatGPTModel];
		...
		assert.strictEqual(draftSelection.selectModel(singularityModel.identifier), true);
		...
		nextProvider.models = [chatGPTModel, singularityModel];
		...
		assert.deepStrictEqual({ current: nextSelection.state.get().currentModel?.identifier, writes: nextProvider.writes }, {
			current: singularityModel.identifier,
			writes: [singularityModel.identifier],
		});
	});
