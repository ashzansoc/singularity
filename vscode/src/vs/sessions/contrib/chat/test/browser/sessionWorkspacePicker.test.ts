...
// path roots (e.g. /remote, /local, /singularity, /agent-host).
const MOCK_PROVIDER_PATH_PREFIXES: Record<string, string> = {
	'agenthost-remote-1': '/remote',
	'local-1': '/local',
	'default-singularity': '/singularity',
...
suite('WorkspacePicker - Connection Status', () => {
	...
	test('shows manually picked worktree folders but filters them from VS Code recents', async () => {
		...
		const ownSingularityWorktreeUri = URI.file('/tmp/singularity-worktrees/owned-feature');
		...
		const globalSingularityWorktreeUri = URI.file('/tmp/singularity-worktrees/global-feature');
		const globalUppercaseSingularityWorktreeUri = URI.file('/tmp/SINGULARITY-WORKTREES/other-global-feature');
		...
		seedStorage(storage, [
			...
			{ uri: ownSingularityWorktreeUri, providerId: 'provider', checked: false },
		...
		const workspacesService = {
			getRecentlyOpened: async () => ({
				workspaces: [
					...
					{ folderUri: globalSingularityWorktreeUri },
					{ folderUri: globalUppercaseSingularityWorktreeUri },
		...
		assert.deepStrictEqual(
			...
			[ownWorktreeUri, ownSingularityWorktreeUri, ownRegularUri, globalSimilarUri, globalRegularUri].map(uri => uri.toString()),
		);
	});
	...
	test('restore never preselects a worktree folder', async () => {
		...
		for (const excludedUri of [
			...
			URI.file('/local/singularity-worktrees/feature'),
		]) {
			const storage = disposables.add(new TestStorageService());
	...
	test('restore picks the stored workspace when its provider registers after another provider', () => {
		...
		// provider (for example, local-agent-host registering after default-singularity),
		...
		const singularityProvider = createMockProvider('default-singularity');

		const storage = disposables.add(new TestStorageService());
		seedStorage(storage, [
			{ uri: URI.file('/singularity/old-project'), providerId: 'default-singularity', checked: false },
		...
		providersService.setProviders([singularityProvider]);
		const picker = createTestPicker(disposables, providersService, storage);
		...
		providersService.setProviders([singularityProvider, agentHostProvider]);
	...
	test('late-registering provider does not move selection out from under user', () => {
		...
		// User explicitly picks a Singularity workspace.
		picker.setSelectedWorkspace(URI.file('/singularity/picked'), { fireEvent: false });
		assertSelectedProvider(picker, 'default-singularity', 'User pick is honored');

		// Now the late provider for the (still-stored) checked entry arrives.
		const agentHostProvider = createMockProvider('local-agent-host');
		providersService.setProviders([singularityProvider, agentHostProvider]);

		assertSelectedProvider(picker, 'default-singularity', 'User selection is preserved across late provider registration');
	});
});
