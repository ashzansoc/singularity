...
import { getSingularityCliSessionRawId } from '../../../../workbench/contrib/chat/browser/singularityCliEventsUri.js';
 ...
 * Singularity CLI session (active, past, or archived). Accepting an item inserts an
...
export class SessionReferenceCompletionHandler extends Disposable {
	...
	private _collectSessionItems(range: { insert: Range; replace: Range }): CompletionItem[] {
		...
		// Only Singularity CLI sessions have a readable event log; include active,
		// past, and archived. Resolve the id once, then newest first.
		const sessions = this.sessionsManagementService.getSessions()
			.map(session => ({ session, rawId: getSingularityCliSessionRawId(session.resource) }))
			...
			.filter(entry => {
				...
				// resource scheme (local `agent-host-singularitycli` vs remote
				// `remote-<authority>-singularitycli`), so a session whose events.jsonl
