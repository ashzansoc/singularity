...
import { SingularityChatTelemetryWidget } from '../../../../workbench/contrib/chat/browser/widget/input/singularityChatTelemetryWidget.js';
...
const OTEL_STATUS_COMMAND = 'singularity.chat.chat.otel.statusActive';
const OTEL_STATUS_ENTRY_ID = 'singularity.otelStatus';
...
export class NewChatInputWidget extends Disposable implements IHistoryNavigationWidget {
	...
	render(parent: HTMLElement, root: HTMLElement): void {
		...
		const telemetryHost = dom.append(newChatBottomContainer, dom.$('.new-chat-telemetry-host'));
		const telemetryWidget = this._register(this.instantiationService.createInstance(SingularityChatTelemetryWidget));
		telemetryHost.appendChild(telemetryWidget.domNode);
