...
const SINGULARITY_CHAT_VIEW_CONTAINER_ID = 'workbench.view.extension.singularity-chat';
const SINGULARITY_CHAT_VIEW_ID = 'singularity-chat';
...
class RegisterChatDebugViewContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		// The singularity-chat view is contributed by the Singularity AI extension,
		...
		if (!this.tryMoveView(viewContainerRegistry, viewsRegistry)) {
			const listener = viewsRegistry.onViewsRegistered(e => {
				for (const { views } of e) {
					if (views.some(v => v.id === SINGULARITY_CHAT_VIEW_ID)) {
						if (this.tryMoveView(viewContainerRegistry, viewsRegistry)) {
							listener.dispose();
						}
						break;
	...
	private tryMoveView(viewContainerRegistry: IViewContainersRegistry, viewsRegistry: IViewsRegistry): boolean {
		const viewContainer = viewContainerRegistry.get(SINGULARITY_CHAT_VIEW_CONTAINER_ID);
		if (!viewContainer) {
			return false;
		}

		const view = viewsRegistry.getView(SINGULARITY_CHAT_VIEW_ID);
		if (!view) {
			return false;
		}
