...
export type AccountTitleBarStateSource = 'account' | 'singularity';
...
export function getAccountTitleBarState(context: IAccountTitleBarStateContext): IAccountTitleBarState {
	...
	const singularityState = getSingularityPresentation(context.entitlement, context.sentiment, context.quotas, context.usableWithoutGitHub);
	if (singularityState) {
...
function getSingularityPresentation(
	...
	if (sentiment.disabled || sentiment.untrusted) {
		return {
			...
			kind: 'warning',
			icon: Codicon.account,
			label: localize('singularityUnavailable', "Singularity Unavailable"),
			ariaLabel: sentiment.untrusted
				? localize('singularityUnavailableUntrustedAria', "Singularity is unavailable in untrusted workspaces")
				: localize('singularityUnavailableDisabledAria', "Singularity is disabled"),
		};
	}
	...
	if (entitlement === ChatEntitlement.Free && (chatQuotaExceeded || completionsQuotaExceeded)) {
		return {
			...
			kind: 'warning',
			icon: Codicon.account,
			label: localize('singularityQuotaReached', "Quota Reached"),
			dotBadge: 'error',
	...
	if (entitlement === ChatEntitlement.Free && typeof remainingPercent === 'number' && remainingPercent <= 25) {
		return {
			...
			icon: Codicon.account,
			label: localize('singularityTokensRemaining', "Tokens Remaining"),
			badge: `${remainingPercent}%`,
			dotBadge: remainingPercent <= 10 ? 'error' : 'warning',
			ariaLabel: localize('singularityTokensRemainingAria', "{0}% Singularity tokens remaining", remainingPercent),
		};
	}
...
function getQuotaReachedAriaLabel(chatQuotaExceeded: boolean, completionsQuotaExceeded: boolean): string {
	if (chatQuotaExceeded && completionsQuotaExceeded) {
		return localize('singularityAllQuotaReachedAria', "Singularity chat and inline suggestion quota reached");
	}

	if (chatQuotaExceeded) {
		return localize('singularityChatQuotaReachedAria', "Singularity chat quota reached");
	}

	return localize('singularityCompletionsQuotaReachedAria', "Singularity inline suggestion quota reached");
}
