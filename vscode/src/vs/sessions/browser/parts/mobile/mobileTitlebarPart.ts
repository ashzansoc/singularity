 ...
 * account info, singularity status dashboard, and sign-in/sign-out actions.
 */
	...
	private showAccountPanel(): void {
		...
		panelStore.add({
			dispose: () => {
				...
				this.singularityDashboardStore.clear();
				this.renderAccountState();
			}
		});
		...
		// Singularity status dashboard — only when signed in AND entitlements
		// have resolved. When entitlement is Unknown or Available (setup
		// pending), the dashboard shows a "Set up Singularity" prompt that
