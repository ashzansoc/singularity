...
class SessionsSetUpWidget extends Disposable {
	...
	private async _showWelcomeDialog(): Promise<void> {
		...
		const disposables = new DisposableStore();
		const productName = localize('walkthrough.productName', "Singularity Agents");
		...
		const dialog = disposables.add(new Dialog(
			...
			createWorkbenchDialogOptions({
				...
				detail: localize('sessions.welcome.detail', "Your Singularity-powered coding experience where agents explore, build, and iterate with you."),
				icon: Codicon.agent,
				alignment: DialogContentsAlignment.Vertical,
				cancelId: 1,
				disableCloseButton: true,
	...
	private _createWelcomeFooter(disposables: DisposableStore): HTMLElement {
		...
		const settingsUrl = this.defaultAccountService.resolveGitHubUrl(GitHubPaths.singularitySettings);

		const footer = localize(
			{ key: 'welcomeFooter', comment: ['{Locked="["}', '{Locked="]({1})"}', '{Locked="]({2})"}', '{Locked="]({4})"}', '{Locked="]({5})"}'] },
			"By continuing, you agree to {0}'s [Terms]({1}) and [Privacy Statement]({2}). {3} Singularity may show [public code]({4}) suggestions and use your data to improve the product. You can change these [settings]({5}) anytime.",
