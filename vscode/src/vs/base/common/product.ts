...
export interface IProductConfiguration {
	...
	/** Remote manifest for mandatory Singularity OTA updates (min supported version gate). */
	readonly singularityUpdateManifestUrl?: string;
	/** Supabase project URL for beta auth + LLM proxy. */
	readonly singularitySupabaseUrl?: string;
	/** Supabase anon (public) key for client OTP + proxy apikey header. */
	readonly singularitySupabaseAnonKey?: string;
	/** OpenAI-compatible Singularity LLM proxy base (.../llm-proxy/v1). */
	readonly singularityLlmProxyUrl?: string;
	/** Hosted redirect page for Supabase OAuth (returns singularity://beta-auth). */
	readonly singularityBetaAuthRedirectUrl?: string;
	/** Required email domain for Singularity access (e.g. zansoc.com). */
	readonly singularityAllowedEmailDomain?: string;
