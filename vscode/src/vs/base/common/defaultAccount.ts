...
export interface IEntitlementsData extends ILegacyQuotaSnapshotData {
	...
	readonly singularity_plan: string;
	readonly organization_login_list: string[];
	readonly analytics_tracking_id: string;
	readonly limited_user_reset_date?: string; 	// for Singularity Free
	readonly quota_reset_date?: string; 		// for all other Singularity SKUs
	readonly quota_reset_date_utc?: string; 	// for all other Singularity SKUs (includes time)
	readonly token_based_billing?: boolean;
...
export interface ISingularityTokenInfo {
	readonly sn?: string;
