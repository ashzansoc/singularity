		...
		if (!this._apiObject) {
			...
			this._apiObject = Object.freeze<vscode.ChatResponseStream>({
				...
				agentTeam(payload: {
					...
					mission: string;
					expanded?: boolean;
					summary: vscode.ChatAgentTeamSummaryPayload;
					agents: vscode.ChatAgentTeamAgentRowPayload[];
					verificationPhase?: boolean;
				}) {
					throwIfDone(this.agentTeam);
					checkProposedApiEnabled(that._extension, 'chatParticipantAdditions');
					const part = new extHostTypes.ChatResponseAgentTeamPart({
						id: payload.id,
						...
						verificationPhase: payload.verificationPhase,
					});
					const dto = typeConvert.ChatResponseAgentTeamPart.from(part);
					_report(dto);
					return this;
				},
				...
				push(part) {
					...
					if (part instanceof extHostTypes.ChatResponseReferencePart) {
						...
						// Singularity: drop Auto routing explainability parts so the routed model is never shown.
						return;
...
function isBuiltinParticipant(agentId: string): boolean {
	return agentId.startsWith('singularity.chat');
}
