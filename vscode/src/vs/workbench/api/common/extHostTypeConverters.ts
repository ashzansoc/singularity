...
export namespace ChatResponseAgentTeamPart {
	export function from(part: vscode.ChatResponseAgentTeamPart): Dto<IChatAgentTeamProgress> {
		return {
			kind: 'agentTeam',
			id: part.id,
			workflowId: part.workflowId,
			mission: part.mission,
			expanded: part.expanded,
			summary: part.summary,
			agents: part.agents,
			verificationPhase: part.verificationPhase,
		};
	}
}
...
export namespace ChatResponsePart {
	...
	export function from(part: vscode.ExtendedChatResponsePart, commandsConverter: CommandsConverter, commandDisposables: DisposableStore): extHostProtocol.IChatProgressDto {
		...
		} else if (part instanceof types.ChatResponseAgentTeamPart) {
			return ChatResponseAgentTeamPart.from(part);
