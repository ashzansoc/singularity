		...
		try {
			disposables.add(this._chatProviderService.registerLanguageModelProvider(vendor, {
				...
				provideLanguageModelChatInfo: async (options, token) => {
					...
					const singularityExtensionId = this._productService.defaultChatAgent?.chatExtensionId;
					return modelsAndIdentifiers.map(m => {
						...
						// Any contributed model that isn't from the built-in Singularity chat extension is BYOK.
						const isBuiltinSingularity = !!singularityExtensionId && ExtensionIdentifier.equals(m.metadata.extension, singularityExtensionId);
						return { ...m, metadata: { ...m.metadata, isBYOK: !isBuiltinSingularity } };
					});
				},
