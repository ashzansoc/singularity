...
export class MainThreadChatDebug extends Disposable implements MainThreadChatDebugShape {
	...
	private _serializeEvent(event: IChatDebugEvent): IChatDebugEventDto {
		...
		switch (event.kind) {
			...
			case 'modelTurn':
				return { ...base, kind: 'modelTurn', model: event.model, requestName: event.requestName, inputTokens: event.inputTokens, outputTokens: event.outputTokens, cachedTokens: event.cachedTokens, totalTokens: event.totalTokens, singularityUsageNanoAiu: event.singularityUsageNanoAiu, durationInMillis: event.durationInMillis };
			case 'generic':
		...
		switch (dto.kind) {
			...
			case 'modelTurn':
				return {
					...
					singularityUsageNanoAiu: dto.singularityUsageNanoAiu,
					durationInMillis: dto.durationInMillis,
				};
			case 'generic':
				return {
