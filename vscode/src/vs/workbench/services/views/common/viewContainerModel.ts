...
export class ViewContainerModel extends Disposable implements IViewContainerModel {
	...
	add(addedViewDescriptorStates: IAddedViewDescriptorState[]): void {
		...
		for (const addedViewDescriptorState of addedViewDescriptorStates) {
			...
			// A view can only live in one container model; skip duplicates that
			// arrive via stale layout customizations or racing registrations,
			// otherwise every 'added' listener (commands, panes) fires twice.
			if (this.viewDescriptorItems.some(item => item.viewDescriptor.id === viewDescriptor.id)) {
				continue;
			}
