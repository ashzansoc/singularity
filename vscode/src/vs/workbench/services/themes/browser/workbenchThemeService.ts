...
export class WorkbenchThemeService extends Disposable implements IWorkbenchThemeService {
	...
	private migrateIconThemeSettings(): void {
		const themeSettings: [ThemeSettings, (settingsId: string) => string][] = [
			[ThemeSettings.FILE_ICON_THEME, migrateFileIconThemeSettingsId],
			[ThemeSettings.PRODUCT_ICON_THEME, migrateProductIconThemeSettingsId],
		];
		for (const [key, migrate] of themeSettings) {
			const inspection = this.configurationService.inspect<string | null>(key);
			for (const [target, value] of [
				[ConfigurationTarget.USER, inspection.userValue],
				[ConfigurationTarget.USER_REMOTE, inspection.userRemoteValue],
				[ConfigurationTarget.WORKSPACE, inspection.workspaceValue],
