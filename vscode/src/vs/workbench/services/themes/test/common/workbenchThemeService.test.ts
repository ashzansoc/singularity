...
import { migrateThemeSettingsId, migrateFileIconThemeSettingsId, migrateProductIconThemeSettingsId, ThemeSettingDefaults, ThemeSettings } from '../../common/workbenchThemeService.js';
...
suite('WorkbenchThemeService', () => {
	...
	suite('migrateFileIconThemeSettingsId', () => {

		test('migrates Seti to Material Icon Theme', () => {
			assert.strictEqual(migrateFileIconThemeSettingsId('vs-seti'), ThemeSettingDefaults.FILE_ICON_THEME);
		});

		test('returns unknown IDs unchanged', () => {
			assert.strictEqual(migrateFileIconThemeSettingsId('material-icon-theme'), 'material-icon-theme');
		});
	});

	suite('migrateProductIconThemeSettingsId', () => {

		test('migrates Default to Material Product Icons', () => {
			assert.strictEqual(migrateProductIconThemeSettingsId('Default'), ThemeSettingDefaults.PRODUCT_ICON_THEME);
		});

		test('returns unknown IDs unchanged', () => {
			assert.strictEqual(migrateProductIconThemeSettingsId('material-product-icons'), 'material-product-icons');
		});
	});
