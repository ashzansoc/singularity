...
import { SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY, SINGULARITY_ENABLED_PLUGINS_KEY, INativeManagedSettingsService, IFileManagedSettingsService } from '../../../../../platform/policy/common/singularityManagedSettings.js';
...
class DefaultAccountProvider implements IDefaultAccountProvider {
	...
	readonly singularityTokenInfo = null;
	readonly onDidChangeSingularityTokenInfo = Event.None;
...
suite('AccountPolicyService', () => {
		...
		'properties': {
			...
			'setting.F': {
				...
				policy: {
					...
					minimumVersion: '1.0.0',
					localization: { description: { key: '', value: '' } },
					value: policyData => policyData.managedSettings?.[SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY] === 'disable' ? false : undefined,
					managedSettings: {
			...
			'setting.G': {
				...
				policy: {
					...
					minimumVersion: '1.0.0',
					localization: { description: { key: '', value: '' } },
					value: policyData => policyData.managedSettings?.[SINGULARITY_ENABLED_PLUGINS_KEY],
					managedSettings: {
	...
	test('should apply managed-settings policy data from default account', async () => {
		const policyData: IPolicyData = { managedSettings: { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' } };
		defaultAccountService.setDefaultAccountProvider(new DefaultAccountProvider(BASE_DEFAULT_ACCOUNT, policyData));
		await defaultAccountService.refresh();
	...
	test('should apply managed-settings policy data from native managed-settings service', async () => {
		const nativeManagedSettingsService = disposables.add(new FakeNativeManagedSettingsService({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' }));
	...
	test('managed settings: native MDM value wins over server for the same declared key', async () => {
		...
		const policyData: IPolicyData = { managedSettings: { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'enable' } };
		defaultAccountService.setDefaultAccountProvider(new DefaultAccountProvider(BASE_DEFAULT_ACCOUNT, policyData));
		await defaultAccountService.refresh();
	...
	test('managed settings: three-channel precedence native MDM > Server > File', async () => {
		...
		const fileManagedSettingsService = new FakeFileManagedSettingsService({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'file-value' });
		const nativeManagedSettingsService = disposables.add(new FakeNativeManagedSettingsService({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' }));
	...
	test('managed settings: file-based settings apply when server and MDM are empty', async () => {
		...
		const fileManagedSettingsService = new FakeFileManagedSettingsService({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' });
	...
	test('managed settings: per-key precedence merges across channels — different keys win from different channels', async () => {
		...
		const fileManagedSettingsService = new FakeFileManagedSettingsService({ [SINGULARITY_ENABLED_PLUGINS_KEY]: enabledPluginsJson });
		const nativeManagedSettingsService = disposables.add(new FakeNativeManagedSettingsService({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' }));
		policyService = disposables.add(new AccountPolicyService(logService, defaultAccountService, undefined, nativeManagedSettingsService, fileManagedSettingsService));
		const defaultConfiguration = disposables.add(new DefaultConfiguration(new NullLogService()));
		await defaultConfiguration.initialize();
	...
	test('managed settings: an object-typed setting resolves identically from server and native MDM JSON strings', async () => {
		...
		const resolveEnabledPlugins = async (source: { server?: string; mdm?: string }): Promise<unknown> => {
			...
			const nativeManagedSettingsService = disposables.add(new FakeNativeManagedSettingsService(
				source.mdm !== undefined ? { [SINGULARITY_ENABLED_PLUGINS_KEY]: source.mdm } : {},
			));
			...
			const policyData: IPolicyData = source.server !== undefined
				? { managedSettings: { [SINGULARITY_ENABLED_PLUGINS_KEY]: source.server } }
				: {};
