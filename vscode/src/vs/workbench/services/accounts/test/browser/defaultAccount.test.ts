...
import { SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY, INativeManagedSettingsService } from '../../../../../platform/policy/common/singularityManagedSettings.js';
...
suite('DefaultAccountProvider managed settings', () => {
	...
	test('native false suppresses cached server true', async () => {
		...
		const provider = await createProvider({ [SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY]: false }, requestService);
		const cachedPolicy = createCachedPolicy(true);
	...
	test('failed forced fetch retains cached managed settings', async () => {
		...
		const provider = await createProvider({ [SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY]: true }, requestService);
	...
	async function createProvider(nativeManagedSettings: Record<string, boolean>, requestService: TestRequestService): Promise<DefaultAccountProvider> {
		...
		const provider = disposables.add(instantiationService.createInstance(DefaultAccountProvider, {
			...
			authenticationProvider: {
				...
				enterpriseProviderConfig: 'singularity.chat.advanced.authProvider',
			...
			entitlementUrl: '',
			mcpRegistryDataUrl: '',
			managedSettingsUrl: 'https://api.github.com/singularity_internal/managed_settings',
		}));
