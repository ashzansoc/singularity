...
import { ISingularityTokenInfo, IDefaultAccount, IDefaultAccountAuthenticationProvider, IEntitlementsData, IPolicyData } from '../../../../base/common/defaultAccount.js';
...
import { INativeManagedSettingsService, shouldForceRemoteSettingsRefresh } from '../../../../platform/policy/common/singularityManagedSettings.js';
...
export class DefaultAccountService extends Disposable implements IDefaultAccountService {
	...
	get singularityTokenInfo(): ISingularityTokenInfo | null { return this.defaultAccountProvider?.singularityTokenInfo ?? null; }
	...
	private readonly _onDidChangeSingularityTokenInfo = this._register(new Emitter<ISingularityTokenInfo | null>());
	readonly onDidChangeSingularityTokenInfo = this._onDidChangeSingularityTokenInfo.event;
	...
	setDefaultAccountProvider(provider: IDefaultAccountProvider): void {
		...
		// Subscribe before refresh so a session restored from the keychain is not
		// missed — that event can fire during provider init, before refresh() returns.
		...
		this._register(provider.onDidChangeSingularityTokenInfo(tokenInfo => this._onDidChangeSingularityTokenInfo.fire(tokenInfo)));
...
interface ICachedAccountData {
	...
	readonly singularityTokenInfo?: ISingularityTokenInfo;
}
...
interface IDefaultAccountData {
	...
	singularityTokenInfo: ISingularityTokenInfo | null;
}
...
export class DefaultAccountProvider extends Disposable implements IDefaultAccountProvider {
	...
	private _singularityTokenInfo: ISingularityTokenInfo | null = null;
	get singularityTokenInfo(): ISingularityTokenInfo | null { return this._singularityTokenInfo; }
	...
	private _signInInFlight: Promise<IDefaultAccount | null> | undefined;
	...
	constructor(
		...
		this._singularityTokenInfo = cachedAccountData?.singularityTokenInfo ?? null;
	...
	private getCachedAccountData(): ICachedAccountData | null {
		...
		if (cached) {
			try {
				...
				// ICachedAccountData ({ accountPolicyData, singularityTokenInfo }).
				...
				const { accountId, policyData, tokenEntitlementsFetchedAt, mcpRegistryDataFetchedAt, singularityTokenInfo } = parsed;
				if (accountId && policyData) {
					this.logService.debug('[DefaultAccount] Initializing with cached policy data (migrating old format)');
					const result: ICachedAccountData = { accountPolicyData: { accountId, policyData, tokenEntitlementsFetchedAt, mcpRegistryDataFetchedAt }, singularityTokenInfo };
					this.storageService.store(CACHED_POLICY_DATA_KEY, JSON.stringify(result), StorageScope.APPLICATION, StorageTarget.MACHINE);
					return result;
				}

				// New format
				const { accountPolicyData, singularityTokenInfo: wrappedSingularityTokenInfo } = parsed;
				if (accountPolicyData?.accountId && accountPolicyData?.policyData) {
					...
					return { accountPolicyData, singularityTokenInfo: wrappedSingularityTokenInfo };
				}
			} catch (error) {
	...
	private async init(): Promise<void> {
		...
		this._register(this.authenticationService.onDidChangeSessions(e => {
			...
			const removedCurrent = Boolean(this.defaultAccount && e.event.removed?.some(session => session.id === this.defaultAccount?.sessionId));
			const replaced = removedCurrent && (e.event.added?.length ?? 0) > 0;
			if (removedCurrent && !replaced) {
	...
	private setDefaultAccount(account: IDefaultAccountData | null): void {
		...
		if (account) {
			...
			this.setSingularityTokenInfo(account.singularityTokenInfo);
			...
			this.setSingularityTokenInfo(null);
	...
	private setSingularityTokenInfo(singularityTokenInfo: ISingularityTokenInfo | null): void {
		if (equals(this._singularityTokenInfo, singularityTokenInfo)) {
			return;
		}
		this._singularityTokenInfo = singularityTokenInfo;
		this._onDidChangeSingularityTokenInfo.fire(this._singularityTokenInfo);
	}
	...
	private cachePolicyData(accountPolicyData: IAccountPolicyData | null): void {
		if (accountPolicyData) {
			...
			const cachedAccountData: ICachedAccountData = {
				accountPolicyData,
				singularityTokenInfo: this._singularityTokenInfo ?? undefined,
			};
	...
	private async getDefaultAccountFromAuthenticatedSessions(authenticationProvider: IDefaultAccountAuthenticationProvider, sessions: AuthenticationSession[], options?: { forceRefresh?: boolean }): Promise<IDefaultAccountData | null> {
		try {
			...
			return {
				...
				singularityTokenInfo: tokenEntitlementsResult?.data?.singularityTokenInfo ?? null,
			};
		} catch (error) {
	...
	private async getTokenEntitlements(sessions: AuthenticationSession[], accountPolicyData: IAccountPolicyData | undefined, options?: { forceRefresh?: boolean }): Promise<{ data: { policyData: Partial<IPolicyData>; singularityTokenInfo: ISingularityTokenInfo } | undefined; fetchedAt: number }> {
		if (!options?.forceRefresh && accountPolicyData?.tokenEntitlementsFetchedAt && !this.isDataStale(accountPolicyData.tokenEntitlementsFetchedAt)) {
			...
			return { data: { policyData: accountPolicyData.policyData, singularityTokenInfo: this._singularityTokenInfo ?? {} }, fetchedAt: accountPolicyData.tokenEntitlementsFetchedAt };
		}
		const data = await this.requestTokenEntitlements(sessions);
		return { data, fetchedAt: Date.now() };
	}

	private async requestTokenEntitlements(sessions: AuthenticationSession[]): Promise<{ policyData: Partial<IPolicyData>; singularityTokenInfo: ISingularityTokenInfo } | undefined> {
	...
	private async request(url: string, type: 'GET' | 'POST', body: object | undefined, sessions: AuthenticationSession[], token: CancellationToken, callSite: string, requestTimeoutMs?: number): Promise<IRequestContext | undefined> {
		// Rate-limit backoff: when any prior `/singularity_internal/*` request was
	...
	private getEntitlementUrl(): string | undefined {
		if (this.getDefaultAccountAuthenticationProvider().enterprise) {
			try {
				...
				return `${enterpriseUrl.protocol}//api.${enterpriseUrl.hostname}${enterpriseUrl.port ? ':' + enterpriseUrl.port : ''}/singularity_internal/user`;
			} catch (error) {
	...
	private getTokenEntitlementUrl(): string | undefined {
		if (this.getDefaultAccountAuthenticationProvider().enterprise) {
			try {
				...
				return `${enterpriseUrl.protocol}//api.${enterpriseUrl.hostname}${enterpriseUrl.port ? ':' + enterpriseUrl.port : ''}/singularity_internal/v2/token`;
			} catch (error) {
				this.logService.error(error);
			}
		}
	...
	private getMcpRegistryDataUrl(): string | undefined {
		if (this.getDefaultAccountAuthenticationProvider().enterprise) {
			try {
				...
				return `${enterpriseUrl.protocol}//api.${enterpriseUrl.hostname}${enterpriseUrl.port ? ':' + enterpriseUrl.port : ''}/singularity/mcp_registry`;
			} catch (error) {
				this.logService.error(error);
			}
		}
	...
	private getManagedSettingsUrl(): string | undefined {
		if (this.getDefaultAccountAuthenticationProvider().enterprise) {
			try {
				...
				return `${enterpriseUrl.protocol}//api.${enterpriseUrl.hostname}${enterpriseUrl.port ? ':' + enterpriseUrl.port : ''}/singularity_internal/managed_settings`;
			} catch (error) {
				this.logService.error(error);
			}
		}
	...
	async signIn(options?: { additionalScopes?: readonly string[];[key: string]: unknown }): Promise<IDefaultAccount | null> {
		...
		this._signInInFlight = this.doSignIn(options).finally(() => {
			this._signInInFlight = undefined;
		});
		return this._signInInFlight;
	}

	private async doSignIn(options?: { additionalScopes?: readonly string[];[key: string]: unknown }): Promise<IDefaultAccount | null> {
		...
		const existing = await this.findMatchingProviderSession(authProvider.id, [scopes]);
		if (existing?.length) {
			this.logService.info('[DefaultAccount] Reusing existing GitHub session instead of starting another device login');
			for (const preferredExtension of this.defaultAccountConfig.preferredExtensions) {
				this.authenticationExtensionsService.updateAccountPreference(preferredExtension, authProvider.id, existing[0].account);
			}
			await this.updateDefaultAccount({ forceRefresh: true });
			return this.defaultAccount;
		}
