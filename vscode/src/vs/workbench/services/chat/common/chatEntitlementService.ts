...
export namespace ChatEntitlementContextKeys {
	...
	export const clientByokEnabled = new RawContextKey<boolean>('singularity.chat.clientByokEnabled', true, true);

	export const hasByokModels = new RawContextKey<boolean>('singularity.chat.hasByokModels', false, true);
}
 ...
 * through setup) and the model picker (which surfaces a "Sign in to use Singularity"
...
export function chatRequiresSetup(context: IChatSetupRequirement): boolean {
	if (singularityCloudEntitlementsDisabled()) {
...
export interface IChatEntitlementService {
	...
	readonly singularityTrackingId: string | undefined;
 ...
 * @returns The localized full plan name (e.g., "Singularity Pro", "Singularity Free")
 */
export function getChatPlanName(chatEntitlement: ChatEntitlement): string {
	switch (chatEntitlement) {
		case ChatEntitlement.EDU:
			return localize('plan.eduName', 'Singularity Student');
		case ChatEntitlement.Pro:
			return localize('plan.proName', 'Singularity Pro');
		case ChatEntitlement.ProPlus:
			return localize('plan.proPlusName', 'Singularity Pro+');
		case ChatEntitlement.Max:
			return localize('plan.maxName', 'Singularity Max');
		case ChatEntitlement.Business:
			return localize('plan.businessName', 'Singularity Business');
		case ChatEntitlement.Enterprise:
			return localize('plan.enterpriseName', 'Singularity Enterprise');
		default:
			return localize('plan.freeName', 'Singularity Free');
	}
}
...
/** GitHub Singularity CAPI entitlement/signup is unused; chat is billed via Singularity. */
export function singularityCloudEntitlementsDisabled(): boolean {
	return !defaultChatAgent.entitlementUrl && !defaultChatAgent.entitlementSignupLimitedUrl;
}
...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	constructor(
		...
		// Only apply the workbench-side default if no other source (e.g. the Singularity extension)
		...
		if ((isWeb && !environmentService.remoteAuthority && !environmentService.isSessionsWindow)) {
			ChatEntitlementContextKeys.Setup.hidden.bindTo(this.contextKeyService).set(true); // hide singularity UI on web if unsupported
			return;
		}
	...
	get singularityTrackingId(): string | undefined {
		return this.context?.value.state.singularityTrackingId;
	}

	get clientByokEnabled(): boolean {
		return this.contextKeyService.getContextKeyValue<boolean>('singularity.chat.clientByokEnabled') === true;
	}

	get hasByokModels(): boolean {
		return this.contextKeyService.getContextKeyValue<boolean>('singularity.chat.hasByokModels') === true;
	}
	...
	private quotaSingularityTrackingId: string | undefined;
	...
	acceptQuotas(incomingQuotas: IQuotas): void {
		...
		const cachedQuota = this.quotaSingularityTrackingId === this.singularityTrackingId ? oldQuota : {};
		...
		this.quotaSingularityTrackingId = this.singularityTrackingId;
		this._quotas = quotas;
...
interface IEntitlements {
	...
	readonly sku?: string;
	readonly singularityTrackingId?: string;
	readonly quotas?: IQuotas;
}
...
export class ChatEntitlementRequests extends Disposable {
	...
	private async doResolveEntitlement(defaultAccount: IDefaultAccount, token: CancellationToken): Promise<IEntitlements | undefined> {
		...
		if (!entitlementsData) {
			if (singularityCloudEntitlementsDisabled()) {
				this.logService.trace('[chat entitlement]: Singularity CAPI disabled; treating signed-in user as Free');
		...
		if (entitlementsData.access_type_sku === 'free_limited_singularity') {
		...
		} else if (entitlementsData.singularity_plan === 'individual_edu') {
			entitlement = ChatEntitlement.EDU;
		} else if (entitlementsData.singularity_plan === 'individual') {
			entitlement = ChatEntitlement.Pro;
		} else if (entitlementsData.singularity_plan === 'individual_pro') {
			entitlement = ChatEntitlement.ProPlus;
		} else if (entitlementsData.singularity_plan === 'individual_max') {
			entitlement = ChatEntitlement.Max;
		} else if (entitlementsData.singularity_plan === 'business') {
			entitlement = ChatEntitlement.Business;
		} else if (entitlementsData.singularity_plan === 'enterprise') {
		...
		const entitlements: IEntitlements = {
			...
			singularityTrackingId: entitlementsData.analytics_tracking_id
		};
	...
	private update(state: IEntitlements): void {
		...
		this.context.update({ entitlement: this.state.entitlement, organisations: this.state.organisations, sku: this.state.sku, singularityTrackingId: this.state.singularityTrackingId });
	...
	private async onUnknownSignUpError(detail: string, logMessage: string): Promise<boolean> {
		...
		if (!this.lifecycleService.willShutdown) {
			const { confirmed } = await this.dialogService.confirm({
				type: Severity.Error,
				message: localize('unknownSignUpError', "An error occurred while signing up for the Singularity Free plan. Would you like to try again?"),
				detail,
	...
	private onUnprocessableSignUpError(logMessage: string, logDetails: string): void {
		...
		if (!this.lifecycleService.willShutdown) {
			this.dialogService.prompt({
				type: Severity.Error,
				message: localize('unprocessableSignUpError', "An error occurred while signing up for the Singularity Free plan."),
				detail: logDetails,
				buttons: [
					{
	...
	async signIn(options?: { useSocialProvider?: string; additionalScopes?: readonly string[] }): Promise<{ defaultAccount?: IDefaultAccount; entitlements?: IEntitlements }> {
		const defaultAccount = await this.defaultAccountService.signIn({
			...
			extraAuthorizeParameters: { get_started_with: 'singularity-vscode' },
...
export interface IChatEntitlementContextState extends IChatSentiment {
	...
	/**
	 * User's Singularity tracking ID from the entitlement API.
	 */
	singularityTrackingId: string | undefined;
}
...
export class ChatEntitlementContext extends Disposable {
	...
	constructor(
		...
		this._state = this.storageService.getObject<IChatEntitlementContextState>(ChatEntitlementContext.CHAT_ENTITLEMENT_CONTEXT_STORAGE_KEY, StorageScope.PROFILE) ?? {
			...
			sku: undefined,
			singularityTrackingId: undefined
		};

		if (singularityCloudEntitlementsDisabled()) {
			this._state.completed = true;
			if (
				this._state.entitlement === ChatEntitlement.Unknown ||
				this._state.entitlement === ChatEntitlement.Unresolved ||
				this._state.entitlement === ChatEntitlement.Available
			) {
				this._state.entitlement = ChatEntitlement.Free;
			}
		}
	...
	update(context: { entitlement: ChatEntitlement; organisations: string[] | undefined; sku: string | undefined; singularityTrackingId: string | undefined }): Promise<void>;
	async update(context: { completed?: boolean; installed?: boolean; disabled?: boolean; untrusted?: boolean; disabledInWorkspace?: boolean; hidden?: false; later?: boolean; entitlement?: ChatEntitlement; organisations?: string[]; sku?: string; singularityTrackingId?: string }): Promise<void> {
		...
		if (typeof context.entitlement === 'number') {
			...
			this._state.singularityTrackingId = context.singularityTrackingId;
		...
		if (isAnonymous(this.configurationService, this._state.entitlement, this._state)) {
			this._state.sku = 'no_auth_limited_singularity'; // no-auth users have a fixed SKU
		}
	...
	private updateContextSync(): void {
		...
		this.isInternalContextKey.set(Boolean(state.organisations?.some(org => org === 'github' || org === 'microsoft' || org === 'ms-singularity' || org === 'MicrosoftSingularity')));
