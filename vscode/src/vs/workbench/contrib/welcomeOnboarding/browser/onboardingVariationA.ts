 ...
 *
 * Steps:
 * 1. Sign In — sessions-style sign-in hero with Singularity, Google, and Apple options
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to Singularity"));
		...
		if (this.footerLeft) {
			if (this._isLastStep()) {
				...
				if (!this._footerSignInBtn && !this._userSignedIn) {
					...
					this._footerSignInBtn.textContent = localize('onboarding.sessions.signInNudge', "Sign in to use Singularity");
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to Singularity");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "Sign in to use Singularity.");
		...
		// Singularity disclaimer
		const singularityDisclaimer = append(disclaimerCol, $('.onboarding-a-signin-disclaimer'));
		singularityDisclaimer.append(localize('onboarding.signIn.disclaimer.prefix', "By signing in, you agree to {0}'s ", defaultChat.provider.default.name));
		this._createInlineLink(singularityDisclaimer, localize('onboarding.signIn.disclaimer.terms', "Terms"), defaultChat.termsStatementUrl);
		singularityDisclaimer.append(localize('onboarding.signIn.disclaimer.middle', " and "));
		this._createInlineLink(singularityDisclaimer, localize('onboarding.signIn.disclaimer.privacy', "Privacy Statement"), defaultChat.privacyStatementUrl);
		singularityDisclaimer.append(localize('onboarding.signIn.disclaimer.singularityPrefix', ". {0} may show ", defaultChat.provider.default.name));
		this._createInlineLink(singularityDisclaimer, localize('onboarding.signIn.disclaimer.publicCode', "public code"), defaultChat.publicCodeMatchesUrl);
		singularityDisclaimer.append(localize('onboarding.signIn.disclaimer.improveSuffix', " suggestions and use your data to improve the product."));
		singularityDisclaimer.append(' ');
		singularityDisclaimer.append(localize('onboarding.signIn.disclaimer.settingsPrefix', "You can change these "));
		this._createInlineLink(singularityDisclaimer, localize('onboarding.signIn.disclaimer.settings', "settings"), this.defaultAccountService.resolveGitHubUrl(GitHubPaths.singularitySettings));
		singularityDisclaimer.append(localize('onboarding.signIn.disclaimer.suffix', " anytime."));
	}
	...
	private _renderAgentSessionsStep(container: HTMLElement): void {
		...
		this._createFeatureCard(moreGrid, Codicon.rocket,
			...
			localize('onboarding.sessions.runAnywhere.desc', "Run agents locally for interactive work, in the background with Singularity CLI, or in the cloud with cloud agents that open a pull request your team can review."));

		this._createFeatureCard(moreGrid, Codicon.settingsGear,
			localize('onboarding.sessions.customize', "Customize Your Agents"),
			localize('onboarding.sessions.customize.desc', "Tailor Singularity to your project with custom instructions and agents, skills, reusable prompts, and MCP servers that connect to the tools and context you rely on."));
