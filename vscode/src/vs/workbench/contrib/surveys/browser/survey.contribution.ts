...
import { SingularityPMFSurvey } from './surveyQuestions.js';
...
// Programmatic command for extensions to trigger the survey (e.g. from Singularity survey service)
CommandsRegistry.registerCommand('_workbench.action.openSingularitySurvey', (accessor: ServicesAccessor, source?: unknown) => {
...
function openSurveyEditor(accessor: ServicesAccessor, source?: string): Promise<void> {
	...
	const input = instantiationService.createInstance(SurveyEditorInput, SingularityPMFSurvey, source);
