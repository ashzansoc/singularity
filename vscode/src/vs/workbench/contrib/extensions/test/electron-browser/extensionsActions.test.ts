...
suite('EnableAIFeaturesInWorkspaceAction', () => {
	...
	setup(() => {
		...
		instantiationService.stub(IProductService, { defaultChatAgent: { chatExtensionId: 'Singularity.singularity-chat' } } as Partial<IProductService>);
	});
	...
	test('test enable AI in workspace updates workspace setting when AI is disabled globally', async () => {
		...
		const chatExtension = aLocalExtension('singularity-chat', { publisher: 'GitHub' }, { type: ExtensionType.System });
		...
		const singularityChat = extensions.find(e => e.identifier.id === 'singularity.chat-chat');
		assert.ok(singularityChat);
		...
		disposables.add(instantiationService.createInstance(ExtensionContainers, [testObject]));
		testObject.extension = singularityChat;
		assert.ok(testObject.enabled);
