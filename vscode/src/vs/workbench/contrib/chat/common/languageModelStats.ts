...
export class LanguageModelStatsService extends Disposable implements ILanguageModelStatsService {
	...
	async update(model: string, extensionId: ExtensionIdentifier, agent: string | undefined, tokenCount: number | undefined): Promise<void> {
		await this.extensionFeaturesManagementService.getAccess(extensionId, SingularityUsageExtensionFeatureId);
	}

}

export const SingularityUsageExtensionFeatureId = 'singularity';
Registry.as<IExtensionFeaturesRegistry>(Extensions.ExtensionFeaturesRegistry).registerExtensionFeature({
	id: SingularityUsageExtensionFeatureId,
	label: localize('Language Models', "Singularity"),
