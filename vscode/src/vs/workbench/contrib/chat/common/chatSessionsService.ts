...
export interface IChatSessionsExtensionPoint {
	 ...
	 * session can own a custom pool yet still support Auto (e.g. the Singularity
	 * CLI agent host).
	 */
	readonly supportsAutoModel?: boolean;
	/**
	 * Logical Agent Host provider ID for Agent Host-backed chat sessions.
	 * For example, both local `agent-host-singularitycli` and remote
	 * `remote-{authority}-singularitycli` sessions use `singularitycli`.
	 */
	readonly agentHostProviderId?: string;
	/**
	 * Whether this type needs a Singularity account and so is unusable until the user signs in. Set by
	 * Singularity-backed types (Singularity CLI / agent host, cloud agent) where BYOK isn't supported. Defaults to false, so
	 * third-party types that don't depend on Singularity stay usable while signed out.
	 *
	 ...
	 */
	readonly requiresSingularitySignIn?: boolean | (() => boolean);
	/**
	 * Fires when a functional {@link requiresSingularitySignIn} would return a
	 ...
	 */
	readonly onDidChangeRequiresSingularitySignIn?: Event<void>;
	/**
...
export namespace SessionType {
	export const SingularityCLI = 'singularitycli';
	export const SingularityCloud = 'singularity-cloud-agent';
	export const Local = 'local';
	export const ClaudeCode = 'claude-code';
	export const Codex = 'openai-codex';
	export const Growth = 'singularity-growth';
	export const AgentHostSingularity = 'agent-host-singularitycli';
...
export function isLocalAgentHostTarget(target: string): boolean {
	return target === SessionType.AgentHostSingularity ||
...
export interface IChatSessionsService {
	...
	/**
	 * Whether the session type needs a Singularity account and so is unusable until the user signs in (BYOK isn't
	 * supported). A type's {@link IChatSessionsExtensionPoint.requiresSingularitySignIn} may be a static boolean or a
	 ...
	 */
	requiresSingularitySignInForSessionType(chatSessionType: string): boolean;
