...
export class ChatDebugServiceImpl extends Disposable implements IChatDebugService {
	...
	private static readonly _debugEligibleSessionTypes = new Set([
		...
		'singularitycli',				// Singularity CLI background sessions
		'agent-host-singularitycli',		// local Agent Host Singularity CLI sessions
	...
	private _isDebugEligibleSession(sessionResource: URI): boolean {
		...
		return ChatDebugServiceImpl._debugEligibleSessionTypes.has(sessionType)
			// Remote Agent Host Singularity CLI sessions use a dynamic
			// `remote-<authority>-singularitycli` scheme; see singularityCliEventsUri.ts.
			|| (sessionType.startsWith('remote-') && sessionType.endsWith('-singularitycli'))
			|| this._importedSessions.has(sessionResource);
	}

	/**
	 * The in-memory event capacity for a session. Agent host (Singularity CLI)
