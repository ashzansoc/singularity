...
import { AGENT_MD_FILENAME, CLAUDE_CONFIG_FOLDER, CLAUDE_LOCAL_MD_FILENAME, CLAUDE_MD_FILENAME, SINGULARITY_CONFIG_FOLDER, SINGULARITY_CUSTOM_INSTRUCTIONS_FILENAME, DICTATION_INSTRUCTIONS_FILENAME, getCleanPromptName, getSkillFolderName, GITHUB_CONFIG_FOLDER, IResolvedPromptSourceFolder, isInClaudeRulesFolder, VOICE_INSTRUCTIONS_FILENAME } from '../config/promptFileLocations.js';
...
export class PromptsService extends Disposable implements IPromptsService {
	...
	constructor(
		...
		const onDidChangeCustomizationLockdown = Event.filter(this.configurationService.onDidChangeConfiguration,
			e => e.affectsConfiguration(SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_CONFIG) || e.affectsConfiguration(SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG));
		...
		const managedHooksOnly = observableFromEvent(this, onDidChangeCustomizationLockdown,
			() => this.configurationService.getValue<boolean>(SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG) === true);
	...
	public async getSourceFolders(type: PromptsType): Promise<readonly IPromptPath[]> {
		...
		if (type === PromptsType.hook) {
			// For hooks, return the Singularity hooks folder for creating new hooks
		...
		if (type === PromptsType.skill) {
			...
			// ~/.singularity/skills). Use the resolved source folders so each
	...
	private areStandalonePromptFilesBlocked(type: PromptsType): boolean {
		const strictPluginOnly = this.configurationService.getValue<StrictPluginOnlyCustomization>(SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_CONFIG);
		return isPromptTypeBlocked(strictPluginOnly, type)
			|| (type === PromptsType.hook && this.configurationService.getValue<boolean>(SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG) === true);
	}

	private areAgentHooksAllowed(promptPath: IPromptPath): boolean {
		if (this.configurationService.getValue<boolean>(SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG) === true) {
	...
	public async listAgentInstructions(token: CancellationToken, logger: Logger | undefined): Promise<IAgentInstructionFile[]> {
		...
		const useSingularityInstructionsFiles = this.configurationService.getValue(PromptsConfig.USE_SINGULARITY_INSTRUCTION_FILES);
		if (!useSingularityInstructionsFiles) {
			logger?.logInfo('Singularity instructions files are disabled via configuration.');
		} else {
			const singularityInstructionsFile = { fileName: SINGULARITY_CUSTOM_INSTRUCTIONS_FILENAME, type: AgentInstructionFileType.singularityInstructionsMd };
			promises.push(this.fileLocator.findFilesInRoots(rootFolders, GITHUB_CONFIG_FOLDER, [singularityInstructionsFile], token, resolvedAgentFiles)); // singularity-instructions.md in .github folder under workspace root
			promises.push(this.fileLocator.findFilesInRoots([await this.pathService.userHome()], SINGULARITY_CONFIG_FOLDER, [singularityInstructionsFile], token, resolvedAgentFiles)); // singularity-instructions.md in ~/.singularity folder
		}
	...
	private async getSpeechInstructions(fileName: string, kind: 'voice' | 'dictation', token: CancellationToken): Promise<string | undefined> {
		...
		const candidates = [joinPath(userHome, SINGULARITY_CONFIG_FOLDER, fileName)];
	...
	private async computeSkillDiscovery(token: CancellationToken): Promise<IPromptDiscoveryInfo> {
		...
		type AgentSkillsFoundClassification = {
			...
			singularityPersonal: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; isMeasurement: true; comment: 'Number of Singularity personal skills.' };
		...
		this.telemetryService.publicLog2<AgentSkillsFoundEvent, AgentSkillsFoundClassification>('agentSkillsFound', {
			...
			singularityPersonal: skillsBySource.get(PromptFileSource.SingularityPersonal) ?? 0,
	...
	private async computeHooks(token: CancellationToken): Promise<IHookDiscoveryInfo> {
		...
		const fileResults = await Promise.all(hookFiles.map(async (hookFile): Promise<{
			...
			try {
				...
				// Use format-aware parsing that handles Singularity and Claude formats
		...
		const managedHooksOnlyValue = this.configurationService.getValue<boolean>(SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG) === true;
