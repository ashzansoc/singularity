...
export function isTarget(value: unknown): value is Target {
	return value === Target.VSCode || value === Target.Singularity || value === Target.Claude || value === Target.Undefined;
}
...
export const customAgentAttributes: Record<string, IAttributeDefinition> = {
	...
	[PromptHeaderAttributes.target]: {
		type: 'scalar',
		description: localize('promptHeader.agent.target', 'The target to which the header attributes like tools apply to. Possible values are `github-singularity` and `vscode`.'),
