...
const LIST_CONNECTORS_TOOL_ID = 'singularity_list_connectors';

function formatConnectedConnectorsPrompt(items: readonly { label: string; tools: readonly string[] }[]): string | undefined {
	if (!items.length) {
		return undefined;
	}
	const lines = [
...
type InstructionsCollectionClassification = {
	...
	agentInstructionsCount: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; isMeasurement: true; comment: 'Number of agent instructions added (singularity-instructions.md and agents.md).' };
...
export class ComputeAutomaticInstructions {
	...
	public async collect(variables: ChatRequestVariableSet, token: CancellationToken): Promise<void> {
		...
		const connectorsVariable = this._getConnectedConnectorsIndex();
		if (connectorsVariable) {
			variables.add(connectorsVariable);
			telemetryEvent.listedInstructionsCount++;
		}
	...
	private async _addAgentInstructions(variables: ChatRequestVariableSet, telemetryEvent: InstructionsCollectionEvent, debugInfo: InstructionsCollectionDebugInfo, token: CancellationToken): Promise<void> {
		...
		const singularityEntries: ChatRequestVariableSet = new ChatRequestVariableSet();
		...
		for (const { uri, type } of allCandidates) {
			...
			if (type === AgentInstructionFileType.singularityInstructionsMd) {
				singularityEntries.add(varEntry);
			}
		...
		// Process referenced instructions from singularity files (maintaining original behavior)
		if (singularityEntries.length > 0) {
			await this._addReferencedInstructions(singularityEntries, telemetryEvent, debugInfo, token);
			for (const entry of singularityEntries.asArray()) {
				variables.add(entry);
			}
		}
	...
	private _getConnectedConnectorsIndex(): IPromptTextVariableEntry | undefined {
		const byLabel = new Map<string, string[]>();
		for (const tool of this._languageModelToolsService.getAllToolsIncludingDisabled()) {
			if (tool.source.type !== 'mcp' && tool.id !== LIST_CONNECTORS_TOOL_ID) {
				continue;
			}
			const label = tool.source.type === 'mcp' ? tool.source.label : 'Connectors';
			const name = tool.toolReferenceName ?? tool.displayName;
			const tools = byLabel.get(label) ?? [];
			if (tool.id !== LIST_CONNECTORS_TOOL_ID) {
				tools.push(name);
			}
			byLabel.set(label, tools);
		}
		const content = formatConnectedConnectorsPrompt([...byLabel.entries()].map(([label, tools]) => ({ label, tools })))
			?? (this._languageModelToolsService.getTool(LIST_CONNECTORS_TOOL_ID)
				? 'Call list_connectors immediately when asked about Notion or other connectors. Do not search mcp.json, and do not stop after announcing that you will check.'
				: undefined);
		if (!content) {
			return undefined;
		}
		return {
			id: 'singularity.connectors',
			name: 'prompt:connectors',
			value: content,
			kind: 'promptText',
			modelDescription: 'Connected Singularity connectors',
