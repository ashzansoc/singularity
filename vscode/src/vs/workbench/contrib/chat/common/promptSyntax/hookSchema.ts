...
/**
 * JSON Schema for Singularity hook configuration files.
...
/**
 * Hook command schema for the Singularity CLI format.
 * Adds `bash`, `powershell`, and `timeoutSec` fields alongside the standard ones.
 */
const singularityCliHookCommandSchema: IJSONSchema = {
	type: 'object',
	additionalProperties: true,
	required: ['type'],
	anyOf: [
...
const singularityCliHookArraySchema: IJSONSchema = {
	type: 'array',
	items: singularityCliHookCommandSchema
};

/**
 * Hook properties for the Singularity CLI format.
 */
const singularityCliHookProperties: Record<string, IJSONSchema> = buildHookProperties(Target.Singularity, singularityCliHookArraySchema);

export const hookFileSchema: IJSONSchema = {
	$schema: 'http://json-schema.org/draft-07/schema#',
	type: 'object',
	description: nls.localize('hookFile.description', 'Singularity hook configuration file. Hooks enable executing custom shell commands at strategic points in an agent\'s workflow.'),
	additionalProperties: true,
	required: ['hooks'],
	properties: {
		hooks: {
	...
	// whether the file uses the Singularity CLI format (detected by the "version" field).
	if: {
		required: ['version'],
		properties: {
			version: { type: 'number' }
	...
	then: {
		// Singularity CLI format: camelCase hook names, bash/powershell/timeoutSec fields
		properties: {
			version: {
				type: 'number',
			...
			hooks: {
				properties: singularityCliHookProperties
			}
		}
	},
	else: {
 ...
 * Only matches exact enum values. For tool-specific naming conventions (e.g., Claude, Singularity CLI),
 ...
 *
 * This allows Singularity format to handle Claude-style entries if pasted.
 ...
 * This ensures compatibility when Claude-style hooks are pasted into Singularity format.
 */
