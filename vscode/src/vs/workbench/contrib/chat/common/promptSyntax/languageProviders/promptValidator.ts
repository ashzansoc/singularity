...
export class PromptValidator {
	...
	private async validateSkillAttributes(promptAST: ParsedPromptFile, promptType: PromptsType, report: (markers: IMarkerData) => void): Promise<void> {
		...
		if (contextAttribute && contextAttribute.value.type === 'scalar' && contextAttribute.value.value.trim() === 'fork') {
			const skillToolEnabled = this.configurationService.getValue<boolean>('singularity.chat.chat.skillTool.enabled');
			if (!skillToolEnabled) {
				report(toMarker(
					localize('promptValidator.contextForkNotSupported', "The 'context: fork' attribute requires the skill tool to be enabled (singularity.chat.chat.skillTool.enabled)."),
	...
	private async validateHeader(promptAST: ParsedPromptFile, promptType: PromptsType, target: Target, report: (markers: IMarkerData) => void): Promise<void> {
		...
		switch (promptType) {
			...
			case PromptsType.agent: {
				...
				} else if (target === Target.Singularity) {
					this.validateGithubPermissions(attributes, report);
				}
				break;
			}
	...
	private checkForInvalidArguments(attributes: IHeaderAttribute[], promptType: PromptsType, target: Target, report: (markers: IMarkerData) => void): void {
		...
		const validGithubSingularityAttributeNames = new Lazy(() => new Set(getValidAttributeNames(promptType, false, Target.Singularity)));
			...
			if (!validAttributeNames.includes(attribute.key)) {
				...
				switch (promptType) {
					case PromptsType.prompt:
						...
						break;
					case PromptsType.agent:
						if (target === Target.Singularity) {
							report(toMarker(localize('promptValidator.unknownAttribute.github-agent', "Attribute '{0}' is not supported in custom Singularity agent files. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Hint, [MarkerTag.Unnecessary]));
						} else if (target === Target.Claude) {
							// ignore for now as we don't have a full list of supported attributes for claude target
						} else {
							if (validGithubSingularityAttributeNames.value.has(attribute.key)) {
	...
	private validateTools(attributes: IHeaderAttribute[], agentKind: ChatModeKind, target: Target, report: (markers: IMarkerData) => void): undefined {
		...
		if (target === Target.Singularity || target === Target.Claude) {
			// no validation for github-singularity target and claude
		} else {
	...
	private getMissingGithubMcpServerMarker(toolReferenceName: string, range: Range): IMarkerData | undefined {
		...
		return toMarker(
			localize(
				...
				"Tool alias '{0}' requires the GitHub MCP server. Enable the built-in server with setting 'singularity.chat.chat.githubMcpServer.enabled' or install extension 'io.github.github/github-mcp-server' from Extensions (`@mcp github`).",
				toolReferenceName
			),
			range,
			MarkerSeverity.Hint,
	...
	private validateHookCommand(item: IValue, target: Target, report: (markers: IMarkerData) => void): void {
		...
		if (!hasCommandField) {
			if (isSingularityCli) {
				report(toMarker(localize('promptValidator.hookMissingSingularityCommand', "Hook command must specify at least one of 'bash' or 'powershell'."), item.range, MarkerSeverity.Error));
			} else {
	...
	private validateTarget(attributes: IHeaderAttribute[], report: (markers: IMarkerData) => void): undefined {
		...
		const validTargets = ['github-singularity', 'vscode'];
...
const githubSingularityAgentAttributeNames = [PromptHeaderAttributes.name, PromptHeaderAttributes.description, PromptHeaderAttributes.tools, PromptHeaderAttributes.target, GithubPromptHeaderAttributes.mcpServers, GithubPromptHeaderAttributes.github, PromptHeaderAttributes.infer];
...
export function getValidAttributeNames(promptType: PromptsType, includeNonRecommended: boolean, target: Target): string[] {
	if (target === Target.Claude) {
		...
		if (promptType === PromptsType.agent) {
			return githubSingularityAgentAttributeNames;
		}
	}
		...
		case PromptsType.agent:
			switch (attributeName) {
				...
				case PromptHeaderAttributes.target:
					return localize('promptHeader.agent.target', 'The target to which the header attributes like tools apply to. Possible values are `github-singularity` and `vscode`.');
...
// The list of tools known to be used by Singularity custom agents
export const knownGithubSingularityTools = [
	{ name: SpecedToolAliases.execute, description: localize('githubSingularity.execute', 'Execute commands') },
	{ name: SpecedToolAliases.read, description: localize('githubSingularity.read', 'Read files') },
	{ name: SpecedToolAliases.edit, description: localize('githubSingularity.edit', 'Edit files') },
	{ name: SpecedToolAliases.search, description: localize('githubSingularity.search', 'Search files') },
	{ name: SpecedToolAliases.agent, description: localize('githubSingularity.agent', 'Use subagents') },
];
...
export const knownClaudeModels = [
	{ name: 'sonnet', description: localize('claude.sonnet', 'Latest Claude Sonnet'), modelEquivalent: 'Claude Sonnet 4.5 (singularity)' },
	{ name: 'opus', description: localize('claude.opus', 'Latest Claude Opus'), modelEquivalent: 'Claude Opus 4.6 (singularity)' },
	{ name: 'haiku', description: localize('claude.haiku', 'Latest Claude Haiku, fast for simple tasks'), modelEquivalent: 'Claude Haiku 4.5 (singularity)' },
...
export function getTarget(promptType: PromptsType, header: PromptHeader | URI): Target {
	...
	if (promptType === PromptsType.agent) {
		...
		if (!(header instanceof URI)) {
			...
			if (target === Target.Singularity || target === Target.VSCode) {
				return target;
			}
		}
