...
import { resolveSingularityCliHookType } from './hookSingularityCliCompat.js';
...
export function getHookSourceFormat(fileUri: URI): HookSourceFormat {
	...
	// Default to Singularity format
	return HookSourceFormat.Singularity;
}
...
/**
 * Parses hooks from a Singularity hooks .json file (our native format).
 */
export function parseSingularityHooks(
	json: unknown,
	...
	for (const originalId of Object.keys(hooksObj)) {
		const hookType = resolveSingularityCliHookType(originalId) ?? toHookType(originalId);
		if (!hookType) {
			continue;
		}
...
export function parseHooksFromFile(
	...
	switch (format) {
		...
		case HookSourceFormat.Singularity:
		default:
			hooks = parseSingularityHooks(json, workspaceRootUri, userHome);
			break;
	}
 ...
 * - Singularity format: `{ type: 'command', command: '' }`
