...
/**
 * Singularity configuration folder name.
 */
export const SINGULARITY_CONFIG_FOLDER = '.singularity';

/**
 * Singularity custom instructions file name.
 */
export const SINGULARITY_CUSTOM_INSTRUCTIONS_FILENAME = 'singularity-instructions.md';
...
/**
 * Singularity user agents folder.
 */
export const SINGULARITY_USER_AGENTS_SOURCE_FOLDER = '~/.singularity/agents';
...
export interface IResolvedPromptSourceFolder {
	...
	readonly storage: PromptsStorage.local | PromptsStorage.user;
	/**
	 * The original path string before resolution (e.g., '~/.singularity/agents' or '.github/agents').
...
export const DEFAULT_SKILL_SOURCE_FOLDERS: readonly IPromptSourceFolder[] = [
	...
	{ path: '~/.singularity/skills', source: PromptFileSource.SingularityPersonal, storage: PromptsStorage.user },
...
export const DEFAULT_INSTRUCTIONS_SOURCE_FOLDERS: readonly IPromptSourceFolder[] = [
	...
	{ path: '~/.singularity/instructions', source: PromptFileSource.SingularityPersonal, storage: PromptsStorage.user },
...
export const DEFAULT_AGENT_SOURCE_FOLDERS: readonly IPromptSourceFolder[] = [
	...
	{ path: SINGULARITY_USER_AGENTS_SOURCE_FOLDER, source: PromptFileSource.SingularityPersonal, storage: PromptsStorage.user },
...
export const DEFAULT_HOOK_FILE_PATHS: readonly IPromptSourceFolder[] = [
	...
	{ path: '~/.singularity/hooks', source: PromptFileSource.SingularityPersonal, storage: PromptsStorage.user },
...
function isInAgentsFolder(fileUri: URI): boolean {
	...
	return dir.endsWith('/' + AGENTS_SOURCE_FOLDER) || dir.endsWith('/' + CLAUDE_AGENTS_SOURCE_FOLDER) || isInSingularityAgentsFolder(fileUri);
}
...
/**
 * Helper function to check if a file is directly in the ~/.singularity/agents/ folder.
 */
export function isInSingularityAgentsFolder(fileUri: URI): boolean {
	const dir = dirname(fileUri).path;
	return dir.endsWith(SINGULARITY_USER_AGENTS_SOURCE_FOLDER.substring(1));
}
...
export function getPromptFileType(fileUri: URI): PromptsType | undefined {
	...
	if (filename.endsWith(INSTRUCTION_FILE_EXTENSION) || (filename === SINGULARITY_CUSTOM_INSTRUCTIONS_FILENAME)) {
...
export function getCleanPromptName(fileUri: URI): string {
	...
	if (fileName === SINGULARITY_CUSTOM_INSTRUCTIONS_FILENAME) {
