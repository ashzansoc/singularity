...
export interface ICustomAgent {
	...
	/**
	 * Target of the agent: Singularity, VSCode, Claude, or undefined if not specified.
	 */
...
export enum AgentInstructionFileType {
	agentsMd = 'agentsMd',
	claudeMd = 'claudeMd',
	singularityInstructionsMd = 'singularityInstructionsMd',
}
...
export interface IPromptsService extends IDisposable {
	...
	/**
	 * Gets combined list of agent instruction files (AGENTS.md, CLAUDE.md, singularity-instructions.md).
	 * Combines results from listAgentMDs (non-nested), listClaudeMDs, and listSingularityInstructionsMDs.
	 */
	listAgentInstructions(token: CancellationToken, logger?: Logger): Promise<IAgentInstructionFile[]>;

	/**
	 * Gets the combined voice customization from `~/.singularity/voice.md` and each
	 * trusted workspace's `.github/voice.md`.
	 */
	getVoiceInstructions(token: CancellationToken): Promise<string | undefined>;

	/**
	 * Gets the combined dictation customization from `~/.singularity/dictation.md`
