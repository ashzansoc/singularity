...
import { AGENTS_SOURCE_FOLDER, CLAUDE_CONFIG_FOLDER, SINGULARITY_CONFIG_FOLDER, GITHUB_CONFIG_FOLDER, getPromptFileExtension, getPromptFileType, LEGACY_MODE_FILE_EXTENSION, getCleanPromptName, AGENT_FILE_EXTENSION, getPromptFileDefaultLocations, SKILL_FILENAME, IPromptSourceFolder, IResolvedPromptSourceFolder } from '../config/promptFileLocations.js';
...
export class PromptFilesLocator {
	...
	public createAgentInstructionsUpdatedEvent(): { readonly event: Event<void>; dispose: () => void } {
		...
		const updateWatchers = async () => {
			...
			const watchSingularityFolders = this.configService.getValue(PromptsConfig.USE_SINGULARITY_INSTRUCTION_FILES);
			...
			if (watchSingularityFolders) {
				addWatch(joinPath(userHome, SINGULARITY_CONFIG_FOLDER));
			}
		};
		...
		disposables.add(this.configService.onDidChangeConfiguration(e => {
			if (
				...
				e.affectsConfiguration(PromptsConfig.USE_SINGULARITY_INSTRUCTION_FILES) ||
