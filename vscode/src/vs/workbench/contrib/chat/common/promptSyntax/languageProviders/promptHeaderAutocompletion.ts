...
import { getAttributeDefinition, getTarget, getValidAttributeNames, knownClaudeTools, knownGithubSingularityTools, IValueEntry, ClaudeHeaderAttributes, } from './promptFileAttributes.js';
...
export class PromptHeaderAutocompletion implements CompletionItemProvider {
		...
		if (promptType === PromptsType.prompt || promptType === PromptsType.agent) {
			...
			if (attribute.key === PromptHeaderAttributes.tools || attribute.key === ClaudeHeaderAttributes.disallowedTools) {
				...
				if (value.type === 'sequence') {
					// if the position is inside the tools metadata, we provide tool name completions
					const getValues = async () => {
						if (target === Target.Singularity || this.environmentService.isSessionsWindow) {
							// for Singularity targets and the Sessions Window, we only suggest the known set of tools that are supported by Singularity, instead of all tools that the user has defined, because many tools won't work in these contexts and it would be frustrating for users to select a tool that doesn't work
							return knownGithubSingularityTools;
	...
	private provideHookCommandFieldCompletions(
		...
		const validFields = isSingularityCli
