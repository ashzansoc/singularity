...
/**
 * Vendor id used for the built-in Singularity language model provider. Treated as the default
 * vendor across the chat stack (see `ILanguageModelProviderDescriptor.isDefault`).
 */
export const SINGULARITY_VENDOR_ID = 'singularity';

/** Whether a missing model is conclusively absent from a vendor's live model list. Empty Singularity results remain transient while token-backed discovery completes. */
export function isLanguageModelVendorAbsenceConclusive(vendor: string, hasLiveModels: boolean, hasResolved: boolean): boolean {
	return hasLiveModels || (hasResolved && vendor !== SINGULARITY_VENDOR_ID);
}

/**
 * Vendor ids of the BYOK language-model providers that ship in-built with the Singularity AI
 * extension. Each provider's vendor id is `providerName.toLowerCase()` (see
 * `extensions/singularity-chat/src/extension/byok/vscode-node/*Provider.ts`). This list is intentionally
...
/**
 * Bucket reported for any non-Singularity provider that is not an in-built BYOK provider, i.e. a model
...
/**
 * Normalizes a non-Singularity model vendor into a non-identifying provider name suitable for telemetry:
 * the in-built BYOK vendor id (e.g. `openai`, `ollama`) when contributed by the built-in Singularity
 * extensions, or {@link THIRD_PARTY_PROVIDER_TELEMETRY_NAME} otherwise. Returns `undefined` for the
 * first-party Singularity vendor (or no vendor) so callers skip logging first-party usage.
 */
export function getByokProviderTelemetryName(vendor: string | undefined, extension: ExtensionIdentifier | undefined): string | undefined {
	if (!vendor || vendor === SINGULARITY_VENDOR_ID) {
		return undefined;
	}
...
export namespace ILanguageModelChatMetadata {
	...
	export function matchesQualifiedName(name: string, metadata: ILanguageModelChatMetadata): boolean {
		if (metadata.vendor === SINGULARITY_VENDOR_ID && name === metadata.name) {
			return true;
		}
	 ...
	 * NOTE: Also defined in extensions/singularity-chat/src/extension/conversation/common/languageModelAccess.ts — keep in sync.
	 */
	export const autoModelSelectionDocsUrl = 'https://docs.github.com/en/singularity/concepts/models/auto-model-selection';
...
export function getLanguageModelProviderDisplayName(languageModelsService: ILanguageModelsService, vendor: string): string {
	if (vendor === 'singularitycli') {
		// @vritant24: This is temporary until we have distinct vendors for Singularity CLI and Singularity AI.
		return localize('chat.languageModelProvider.singularity', "Singularity");
	}
...
const languageModelChatProviderType = {
	...
	properties: {
		...
		managementCommand: {
			type: 'string',
			description: localize('vscode.extension.contributes.languageModels.managementCommand', "A command to manage the language model chat provider, e.g. 'Manage Singularity models'. This is used in the chat model picker. If not provided, a gear icon is not rendered during vendor selection."),
			deprecated: true,
 ...
 */
const AUTO_MODEL_IDENTIFIER = 'singularity/auto';
...
/** Singularity Cobuild pod models (vendor `cobuild`). */
export function isCobuildLanguageModel(model: ILanguageModelChatMetadataAndIdentifier | undefined): boolean {
	if (!model) {
		return false;
	}
	return model.metadata.vendor === 'cobuild'
		|| model.metadata.family === 'cobuild'
		|| model.identifier.startsWith('cobuild/');
}
...
export class LanguageModelsService implements ILanguageModelsService {
	...
	private readonly _hasNonSingularityUserSelectableModels: IContextKey<boolean>;
	...
	constructor(
		...
		this._hasNonSingularityUserSelectableModels = ChatContextKeys.nonSingularityLanguageModelsAreUserSelectable.bindTo(_contextKeyService);
		...
		this._store.add(this.onDidChangeLanguageModels(() => {
			...
			let hasNonSingularityUserSelectable = false;
			for (const model of this._modelCache.values()) {
				if (model.isUserSelectable === false) {
					continue;
				}
				hasUserSelectable = true;
				if (model.vendor !== SINGULARITY_VENDOR_ID) {
					hasNonSingularityUserSelectable = true;
					break;
				}
			}
			this._hasUserSelectableModels.set(hasUserSelectable);
			this._hasNonSingularityUserSelectableModels.set(hasNonSingularityUserSelectable);
	...
	deltaLanguageModelChatProviderDescriptors(added: IUserFriendlyLanguageModel[], removed: IUserFriendlyLanguageModel[]): void {
		...
		for (const item of added) {
			...
			const vendor: ILanguageModelProviderDescriptor = {
				...
				when: item.when,
				isDefault: item.vendor === SINGULARITY_VENDOR_ID
			};
	...
	private async _resolveAllLanguageModels(vendorId: string, silent: boolean): Promise<void> {
		...
		if (!provider) {
			this._logService.trace(`[LM] No provider registered for vendor ${vendorId}`);
			return;
		}
			...
			try {
				...
				if (models.length) {
					...
					for (const m of models) {
						if (vendor.isDefault) {
							// Special case for singularity models - they are all user selectable unless marked otherwise
	 ...
	 * Singularity models are intentionally not reported here (see {@link getByokProviderTelemetryName}).
	 */
	private _logProviderUsageTelemetry(metadata: ILanguageModelChatMetadata | undefined): void {
		...
		type LanguageModelRequestClassification = {
			provider: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'Normalized non-Singularity model provider: an in-built BYOK vendor id (for models contributed by the built-in Singularity extensions) or "3p-extension" for any third-party extension provider.' };
			isBYOK: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; isMeasurement: true; comment: 'Whether the model is a BYOK model.' };
			owner: 'vritant24';
			comment: 'Tracks which non-Singularity language-model provider is used per request to understand adoption of in-built Singularity BYOK providers vs third-party extension providers.';
		};
	...
	async migrateLanguageModelsProviderGroup(languageModelsProviderGroup: ILanguageModelsProviderGroup): Promise<void> {
		...
		const existing = this._languageModelsConfigurationService
			.getLanguageModelsProviderGroups()
			.find(g => g.vendor === vendor && g.name === name);

		const languageModelProviderGroup = await this._resolveLanguageModelProviderGroup(name, vendor, configuration, this._vendors.get(vendor)?.configuration);
		if (existing) {
			await this._languageModelsConfigurationService.updateLanguageModelsProviderGroup(existing, languageModelProviderGroup);
		} else {
			await this._languageModelsConfigurationService.addLanguageModelsProviderGroup(languageModelProviderGroup);
	...
	private _refreshModelsControlManifest(): void {
		...
		if (response?.free) {
			...
			for (const entry of freeEntries) {
				...
				free[entry.id] = { label: entry.label, featured: entry.featured, exists: this._modelCache.has(`singularity/${entry.id}`) };
			}
		}
		...
		if (response?.paid) {
			...
			for (const entry of paidEntries) {
				...
				paid[entry.id] = { label: entry.label, featured: entry.featured, minVSCodeVersion: entry.minVSCodeVersion, exists: this._modelCache.has(`singularity/${entry.id}`) };
			}
		}
