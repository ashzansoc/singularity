...
const CHAT_PREFERRED_SINGULARITY_HARNESS_STORAGE_KEY = 'chat.preferredSingularityHarness';
...
export function hasPreferredSingularityHarness(storageService: IStorageService): boolean {
	return storageService.getBoolean(CHAT_PREFERRED_SINGULARITY_HARNESS_STORAGE_KEY, StorageScope.PROFILE, false);
}

export function markPreferredSingularityHarness(storageService: IStorageService): void {
	storageService.store(CHAT_PREFERRED_SINGULARITY_HARNESS_STORAGE_KEY, true, StorageScope.PROFILE, StorageTarget.MACHINE);
}
