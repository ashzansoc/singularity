...
/**
 * Path fragment that identifies a Singularity-CLI-installed plugin. Mirrored by
 * `ConfiguredAgentPluginDiscovery._resolveEnterprisePluginId`.
 */
const SINGULARITY_CLI_INSTALL_PATH_FRAGMENT = '/.singularity/installed-plugins/';
...
function getCanonicalPluginIdentity(plugin: IAgentPlugin): string {
	return getMarketplaceCanonicalIdentity(plugin.fromMarketplace)
		?? getSingularityCliInstallCanonicalIdentity(plugin)
...
function getSingularityCliInstallCanonicalIdentity(plugin: IAgentPlugin): string | undefined {
	if (plugin.uri.scheme !== 'file') {
		return undefined;
	}

	const idx = plugin.uri.path.indexOf(SINGULARITY_CLI_INSTALL_PATH_FRAGMENT);
	if (idx === -1) {
		return undefined;
	}

	const segments = plugin.uri.path.slice(idx + SINGULARITY_CLI_INSTALL_PATH_FRAGMENT.length).split('/').filter(s => s.length > 0);
	...
	if (bucket !== '_direct') {
		return `singularity-cli-marketplace:${normalizePluginIdentitySegment(bucket)}|${normalizedName}`;
	}
