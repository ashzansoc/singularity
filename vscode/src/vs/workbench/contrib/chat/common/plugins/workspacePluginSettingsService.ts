...
/** Singularity CLI settings folder inside `.github/`. */
const SINGULARITY_CONFIG_FOLDER = '.github/singularity';
...
export interface IWorkspacePluginSettingsService {
	 ...
	 * settings files (`.claude/settings.json`, `.github/singularity/settings.json`).
	 */
 ...
 * folder (e.g. `.claude/` or `.github/singularity/`) across all workspace
...
export class WorkspacePluginSettingsService extends Disposable implements IWorkspacePluginSettingsService {
	...
	constructor(
		...
		const singularityReader = this._register(new WorkspaceSettingsReader(
			SINGULARITY_CONFIG_FOLDER, '[SingularityPluginSettings]',
			fileService, workspaceContextService, logService,
		));
		...
		this.extraMarketplaces = derived(reader => {
			...
			const singularity = singularityReader.data.read(reader).marketplaces;
			const byCanonicalId = new Map<string, IWorkspaceMarketplaceEntry>();
			for (const entry of [...claude, ...singularity]) {
		...
		this.enabledPlugins = derived(reader => {
			...
			const singularity = singularityReader.data.read(reader).enabledPlugins;
			const merged = new Map<string, boolean>();
			for (const [key, value] of claude) {
				merged.set(key, value);
			}
			for (const [key, value] of singularity) {
				if (!merged.has(key)) {
					merged.set(key, value);
				}
			}
