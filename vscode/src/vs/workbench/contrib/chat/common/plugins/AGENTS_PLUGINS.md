...
1. **Format detection** — recognizes strict Agent Plugins v1 by its root schema, then falls back to the Singularity, Claude, and Open Plugin path conventions.
...
```
<plugin-root>/
├── plugin.json                                    # Agent Plugins v1 or Singularity manifest
...
3. `.github/plugin/marketplace.json` → `MarketplaceType.Singularity`
...
Provider adapters do not delegate strict packages to legacy SDK plugin discovery. Singularity explicitly receives skills and MCP, Codex explicitly receives skills and MCP, and Claude explicitly receives remote MCP. Remote MCP transport selection remains the responsibility of each provider's existing auto-detection. The Claude SDK currently has no external skill-directory API or per-server stdio working-directory option, so strict Claude skills and stdio MCP are skipped rather than reinterpreted.
