...
export class ConfiguredAgentPluginDiscovery extends AbstractAgentPluginDiscovery {
	...
	protected override async _discoverPluginSources(): Promise<readonly IPluginSource[]> {
		...
		// `<plugin>@<marketplace>`, resolved to the Singularity CLI install convention.
	 ...
	 * the Singularity CLI install convention `~/.singularity/installed-plugins/<marketplace>/<plugin>/`.
	...
	private _resolveEnterprisePluginId(id: string, userHome: string): URI | undefined {
		...
		return URI.file(`${userHome}/.singularity/installed-plugins/${marketplace}/${plugin}`);
	}
...
// ---------------------------------------------------------------------------
// Singularity CLI plugin discovery
// ---------------------------------------------------------------------------

/**
 * Directory under the Singularity CLI home where installed plugins are cached.
 * Layout is two levels deep: `<marketplace>/<plugin>/`. Direct (non-marketplace)
 * installs use the reserved marketplace segment `_direct`.
 *
 * See `src/plugins/manager.ts` in the singularity-agent-runtime repo.
 */
const SINGULARITY_CLI_INSTALLED_PLUGINS_DIR = '.singularity/installed-plugins';

/**
 * Discovers plugins installed by the Singularity CLI under
 * `~/.singularity/installed-plugins/<marketplace>/<plugin>/`. Each leaf directory
 ...
 */
export class SingularityCliAgentPluginDiscovery extends AbstractAgentPluginDiscovery {
	...
	public override start(enablementModel: IEnablementModel): void {
		...
		const setupWatchers = async () => {
			...
			// exist, so we need to watch an existing parent (e.g. ~/.singularity or
	...
	private async _getInstalledPluginsDir(): Promise<URI> {
		const userHome = await this._pathService.userHome();
		return joinPath(userHome, SINGULARITY_CLI_INSTALLED_PLUGINS_DIR);
	}
	...
	protected override async _discoverPluginSources(): Promise<readonly IPluginSource[]> {
		...
		try {
			...
			// Directory doesn't exist — Singularity CLI hasn't installed any plugins.
			return [];
		}
	...
	private async _promptRemove(resource: URI): Promise<void> {
		const { confirmed } = await this._dialogService.confirm({
			message: localize('singularityCliPlugin.remove.confirm', "This plugin was installed by the Singularity CLI. Remove it from disk?"),
			detail: localize('singularityCliPlugin.remove.detail', "The plugin directory '{0}' will be moved to the trash. You can reinstall it later via the Singularity CLI.", resource.fsPath),
			primaryButton: localize('singularityCliPlugin.remove.primary', "Remove"),
		});
		if (!confirmed) {
			return;
		}
		...
		try {
			...
			this._logService.error('[SingularityCliAgentPluginDiscovery] Failed to remove plugin', error);
		}
	}
}
