...
export class RunSubagentTool extends Disposable implements IToolImpl {
	...
	getToolData(): IToolData {
		...
		properties.model = {
			type: 'string',
			description: 'Optional model for the subagent. Format: "Model Name (Vendor)", vendor is usually "singularity". Only use to enforce a specific model.',
		};
		...
		try {
			...
			const progressCallback = (parts: IChatProgress[]) => {
				for (const part of parts) {
					...
					if (part.kind === 'usage') {
						if (typeof part.singularityCredits === 'number' && Number.isFinite(part.singularityCredits) && part.singularityCredits >= 0) {
							subagentCredits = Math.max(subagentCredits ?? 0, part.singularityCredits);
						}
						continue;
					}
			...
			modeTools['singularity_askQuestions'] = false;
			...
			if (subagentCredits !== undefined) {
				request.response?.setSubagentSingularityCredits(invocation.callId, subagentCredits);
				if (invocation.toolSpecificData?.kind === 'subagent') {
	...
	private resolveSubagentModel(subagent: ICustomAgent | undefined, mainModelId: string | undefined, explicitModelQualifiedName?: string): { modeModelId: string | undefined; resolvedModelName: string | undefined } {
		...
		if (subagent && !explicitModelResolved) {
			...
			if (modeModelQualifiedNames) {
				// When the main model is BYOK (flagged via `metadata.isBYOK`), skip Singularity/CAPI fallback models
				...
				const skipSingularityFallbacks = mainModelIsByok && isBuiltinAgent(subagent.source, subagent.uri, this.productService);
				...
				for (const qualifiedName of modeModelQualifiedNames) {
					...
					if (lmByQualifiedName?.identifier) {
						if (skipSingularityFallbacks && lmByQualifiedName.metadata.vendor === SINGULARITY_VENDOR_ID) {
							continue;
						}
