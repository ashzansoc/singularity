		...
		items: {
			...
			properties: {
				name: {
					...
					pattern: '^(?!singularity_|vscode_)[\\w-]+$'
				},
				toolReferenceName: {
		...
		languageModelToolsExtensionPoint.setHandler((_extensions, delta) => {
			for (const extension of delta.added) {
				...
				for (const rawTool of extension.value) {
					...
					if ((rawTool.name.startsWith('singularity_') || rawTool.name.startsWith('vscode_')) && !isProposedApiEnabled(extension.description, 'chatParticipantPrivate')) {
						extension.collector.error(`Extension '${extension.description.identifier.value}' CANNOT register tool with name starting with "vscode_" or "singularity_"`);
						continue;
					}

					if (rawTool.tags?.some(tag => tag.startsWith('singularity_') || tag.startsWith('vscode_')) && !isProposedApiEnabled(extension.description, 'chatParticipantPrivate')) {
						extension.collector.error(`Extension '${extension.description.identifier.value}' CANNOT register tool with tags starting with "vscode_" or "singularity_"`);
					}
