...
import { clearUserSelectedSessionType, getRememberedSessionType, hasPreferredSingularityHarness, storeUserSelectedSessionType } from './chatSessionTypePreference.js';
...
export enum ChatConfiguration {
	...
	MigrateLegacySingularityCliSessions = 'chat.agentSessions.migrateLegacySingularityCli',
	...
	/** When true, Visual Critic stop-hook iterates until pass/max. When false, design-spec once then stop. */
	VisualCriticEnabled = 'chat.visualCritic.enabled',
	...
	SingularityCliHideExtensionHostAgents = 'chat.agents.singularityCli.hideExtensionHost',
	EditorPreferSingularityHarness = 'chat.editor.preferSingularityHarness',
	DefaultToSingularityHarness = 'chat.defaultToSingularityHarness',
	EditorLocalAgentEnabled = 'chat.editor.localAgent.enabled',
	SingularityCliHideExtensionHostEditor = 'chat.editor.singularityCli.hideExtensionHost',
	...
	/** Dependency-aware parallel execution engine (Agent Host). */
	ExecutionEnabled = 'singularity.execution.enabled',
	ExecutionMaxConcurrentAgents = 'singularity.execution.maxConcurrentAgents',
	ExecutionAutoPlanThreshold = 'singularity.execution.autoPlanThreshold',
	ExecutionRiskParallelization = 'singularity.execution.riskParallelization',
}
 ...
 * sessions (such as Singularity CLI). All properties are optional — a missing property
 ...
 * when the agent host is enabled and `chat.defaultToSingularityHarness` is opted in,
 * Agent Host Singularity CLI is the default. It falls back to the local harness
...
export function getComputedDefaultSessionType(
	...
	if (agentHostEnabled && configurationService.getValue<boolean>(ChatConfiguration.DefaultToSingularityHarness)) {
		return SessionType.AgentHostSingularity;
	}
...
export interface IResolvedNewChatSessionType {
	...
	/**
	 * True when {@link sessionType} is the one-time `chat.editor.preferSingularityHarness`
	 * swap. The caller must persist the marker (via `markPreferredSingularityHarness`)
	 * only once it has actually applied this session type, so the migration is not
	 * consumed by a caller that discards the result.
	 */
	readonly isPreferSingularityHarnessSwap: boolean;
}
...
export function resolveDefaultNewChatSessionType(
	...
	if (options?.explicitOverride) {
		return { sessionType: options.explicitOverride, isPreferSingularityHarnessSwap: false };
	}

	if (isVirtualWorkspace(workspace)) {
		return { sessionType: localChatSessionType, isPreferSingularityHarnessSwap: false };
	}

	const remembered = getUsableRememberedSessionType(storageService, configurationService, chatSessionsService, workspace);
	if (remembered && remembered !== localChatSessionType) {
		return { sessionType: remembered, isPreferSingularityHarnessSwap: false };
	}

	// One-time migration: when the agent host is enabled and the user has opted
	// in via `chat.editor.preferSingularityHarness`, swap an existing local editor
	// session to Singularity exactly once (guarded by the persisted marker). Never
	// swap when the agent host is disabled, since the Singularity harness would be
	...
	if (options?.currentSessionType === localChatSessionType
		&& agentHostEnabled
		&& configurationService.getValue<boolean>(ChatConfiguration.EditorPreferSingularityHarness)
		&& !hasPreferredSingularityHarness(storageService)) {
		return { sessionType: SessionType.AgentHostSingularity, isPreferSingularityHarnessSwap: true };
	}

	return { sessionType: getDefaultNewChatSessionType(configurationService, chatSessionsService, storageService, workspace, agentHostEnabled, options), isPreferSingularityHarnessSwap: false };
}
...
export function isVisibleEditorChatSessionType(
	...
	if (sessionType === SessionType.SingularityCLI && configurationService.getValue<boolean>(ChatConfiguration.SingularityCliHideExtensionHostEditor)) {
		return false;
	}
