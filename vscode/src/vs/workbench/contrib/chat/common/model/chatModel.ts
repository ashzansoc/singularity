...
export class Response extends AbstractResponse implements IDisposable {
	...
	updateContent(progress: IChatProgressResponseContent | IChatTextEdit | IChatNotebookEdit | IChatTask | IChatExternalToolInvocationUpdate, quiet?: boolean): void {
		...
		if (progress.kind === 'clearToPreviousToolInvocation') {
			...
			this._contentChanged(quiet);
		} else if (progress.kind === 'agentTeam') {
			const idx = this._responseParts.findIndex(p => p.kind === 'agentTeam' && p.id === progress.id);
			if (idx === -1) {
				this._responseParts.push(progress);
			} else {
				this._responseParts[idx] = progress;
...
export class ChatResponseModel extends Disposable implements IChatResponseModel {
	...
	private readonly _subagentSingularityCredits = new Map<string, number>();
	...
	setUsage(usage: IChatUsage): void {
		...
		this._setUsage(this._withSubagentSingularityCredits(usage), true);
	}

	setSubagentSingularityCredits(subagentCallId: string, singularityCredits: number): void {
		const currentCredits = this._subagentSingularityCredits.get(subagentCallId);
		if (!Number.isFinite(singularityCredits) || singularityCredits < 0 || (currentCredits !== undefined && singularityCredits <= currentCredits)) {
			return;
		}
		this._subagentSingularityCredits.set(subagentCallId, singularityCredits);
		const usage = this._parentUsage ?? { kind: 'usage', promptTokens: 0, completionTokens: 0 };
		this._setUsage(this._withSubagentSingularityCredits(usage), false);
	}

	private _withSubagentSingularityCredits(usage: IChatUsage): IChatUsage {
		let subagentSingularityCredits = 0;
		for (const credits of this._subagentSingularityCredits.values()) {
			subagentSingularityCredits += credits;
		}
		return subagentSingularityCredits === 0
			? usage
			: { ...usage, singularityCredits: (usage.singularityCredits ?? 0) + subagentSingularityCredits };
	}
	...
	private isSameUsage(currentUsage: IChatUsage, usage: IChatUsage): boolean {
		return currentUsage.promptTokens === usage.promptTokens
			&& currentUsage.completionTokens === usage.completionTokens
			&& currentUsage.outputBuffer === usage.outputBuffer
			&& currentUsage.singularityCredits === usage.singularityCredits
			&& currentUsage.sessionSingularityCredits === usage.sessionSingularityCredits
	...
	toJSON(): Omit<ISerializableChatResponseData, 'timestamp'> {
		...
		return {
			...
			singularityCredits: this.usage?.singularityCredits,
			modelTotals: this.usage?.modelTotals,
			sessionSingularityCredits: this.usage?.sessionSingularityCredits,
...
export interface IChatModel extends IDisposable {
	...
	/** Total singularity credits consumed across all turns in this session. */
...
interface ISerializableChatResponseData {
	...
	singularityCredits?: number;
	modelTotals?: readonly IChatUsageModelTotal[];
	sessionSingularityCredits?: number;
	elapsedMs?: number;
}
...
export class ChatModel extends Disposable implements IChatModel {
	...
	get sessionCost(): number {
		...
		for (const request of this._requests) {
			...
			if (typeof usage?.singularityCredits === 'number') {
				summedCredits += usage.singularityCredits;
			}
			if (typeof usage?.sessionSingularityCredits === 'number') {
				reportedSessionCredits = Math.max(reportedSessionCredits, usage.sessionSingularityCredits);
			}
		}
	...
	private _deserializeRequest(raw: ISerializableChatRequestData): ChatRequestModel {
		...
		if (raw.response || raw.result || (raw as any).responseErrorDetails) {
			...
			if (typeof raw.completionTokens === 'number' || typeof raw.promptTokens === 'number' || typeof raw.singularityCredits === 'number' || typeof raw.sessionSingularityCredits === 'number') {
				request.response.setUsage({
					...
					singularityCredits: raw.singularityCredits,
					modelTotals: raw.modelTotals,
					sessionSingularityCredits: raw.sessionSingularityCredits,
				});
			}
