...
import { AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING, TROUBLESHOOT_COMMAND_NAME, TROUBLESHOOT_SKILL_PATH, SINGULARITY_SKILL_URI_SCHEME } from '../promptSyntax/promptTypes.js';
...
export class ChatService extends Disposable implements IChatService {
		...
		const sendRequestInternal = async () => {
			...
			// agentDebugLog.enabled is deprecated; only fileLogging.enabled is authoritative.
			{
				...
				if (!fileLoggingEnabled) {
					...
					const hasTroubleshootSkill = options?.attachedContext?.some(v => {
						...
						return uri && (uri.scheme === SINGULARITY_SKILL_URI_SCHEME || uri.path.includes(TROUBLESHOOT_SKILL_PATH));
					});
			...
			try {
				...
				if (agentPart || (defaultAgent && !commandPart)) {
					...
					const userMessage = promptTextResult.message;
					...
					if (model.canUseTools) {
						const autostartResult = new ChatMcpServersStarting(this.mcpService.autostart(token, { message: userMessage }));
						if (!autostartResult.isEmpty) {
							progressCallback([autostartResult]);
						}
					}
					void this.wakeRemoteConnectors(token, userMessage).catch(err => this.logService.error('[ChatService] Error waking remote connectors:', err));
					const connectorContext = this.getConnectedConnectorsContext();
					if (connectorContext) {
						allContext.push(connectorContext);
					}
	...
	private async wakeRemoteConnectors(token: CancellationToken, userMessage?: string): Promise<void> {
		await Promise.all(this.mcpService.servers.get()
			.filter(server => {
	...
	private getConnectedConnectorsContext(): IChatRequestVariableEntry | undefined {
		const lines: string[] = [];
		for (const server of this.mcpService.servers.get()) {
			...
			const state = McpConnectionState.toKindString(server.connectionState.get().state);
			lines.push(`- ${server.definition.label} [${state}]: ${tools.length ? tools.join(', ') : 'connected, tools still loading'}`);
		}
		if (!lines.length) {
			return undefined;
		}
		return {
			...
			value: [
				'Connected Singularity connectors (this is live state — do not search mcp.json, and do not say you will check):',
				...lines,
				'If the user asks whether you can access a connector, answer from this list.',
				'If the user asks you to do work with a connector (import, export, move, create, edit, link), call that connector\'s tools in this turn. Never stop after only announcing a plan.',
			].join('\n'),
			kind: 'promptText',
