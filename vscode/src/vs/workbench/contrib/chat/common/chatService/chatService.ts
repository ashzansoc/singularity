...
export interface IChatUsage {
	...
	/**
	 * The Singularity credit cost of whatever this usage describes — a turn's own
	 ...
	 * lower bound on the session; prefer {@link sessionSingularityCredits} for that.
	 */
	...
	/**
	 * The whole session's Singularity credit cost as reported by the backend, rather
	 * than summed from the individual turns. Unlike {@link singularityCredits} this
...
/**
 * Formats a singularity credit value for display.
 */
export function formatSingularityCredits(credits: number): string {
...
export interface IChatQuestionCarousel {
	...
	/** True when Singularity supplied the answer through automatic reply. */
	autoReply?: boolean;
 ...
 *   agent host (e.g. Singularity CLI).
 */
...
export interface IChatAgentTeamSummary {
	total: number;
	completed: number;
	working: number;
	queued: number;
...
export interface IChatAgentTeamAgentRow {
	agentId: string;
	taskId: string;
	role: string;
	title: string;
...
/** Live multi-agent mission panel embedded in chat. */
export interface IChatAgentTeamProgress {
	kind: 'agentTeam';
	/** Stable id — later updates replace this part in place. */
	id: string;
	workflowId: string;
	mission: string;
	expanded: boolean;
	summary: IChatAgentTeamSummary;
	agents: IChatAgentTeamAgentRow[];
