...
import { SINGULARITY_VENDOR_ID, ILanguageModelChatMetadata, ILanguageModelsService } from './languageModels.js';
 ...
 * `"singularity/gpt-4.1"` from storage, or a short id like `"gpt-4.1"` from
 ...
 * selection surface (e.g. `"singularity/gpt-4.1"` or `"customendpoint/ANT/gpt-4.1"`).
 *
 ...
 *
 * The selected identifier may be a fully-qualified id (e.g. `"singularity/gpt-4.1"`
 ...
 * persisted model identifier (e.g. `"singularity/gpt-4.1"` → `"singularity"`).
 *
...
export function getSelectedModelVendor(
	...
	// (e.g. "singularity/gpt-4.1" or "customendpoint/ANT/claude-sonnet-4-6")
	const modelId = getSelectedModelIdentifier(contextKeyService, storageService);
	if (modelId?.includes('/')) {
 ...
 * other models (built-in Singularity, Singularity/Claude CLI, and agent-host models)
 * are served through the Singularity (CAPI) service and are therefore not BYOK.
 */
...
/**
 * Returns whether the currently selected chat model is a Singularity model
 * (i.e. not BYOK).
 *
 * When the selection resolves to registered metadata this is the inverse of
 * {@link isByokModel}, so agent-host (CAPI-backed) models count as Singularity.
 * When no model is selected yet (widget not initialized) this returns `true`
 * so quota-style surfaces treat the unknown case as Singularity. As a last
 * resort, an unregistered selection is classified by its vendor prefix.
 */
export function isSelectedModelSingularity(
	contextKeyService: IContextKeyService,
	storageService: IStorageService,
	languageModelsService: ILanguageModelsService,
): boolean {
	...
	if (!vendor) {
		return true; // no selection → treat as Singularity
	}
	return vendor === SINGULARITY_VENDOR_ID;
}
