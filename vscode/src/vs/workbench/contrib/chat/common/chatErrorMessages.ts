...
/**
 * Mirror of the Singularity extension's `ChatFetchResponseType` (see
 * `extensions/singularity-chat/src/platform/chat/common/commonTypes.ts`). These string
 * values are forwarded verbatim from the agent host harnesses (Singularity CLI,
...
/**
 * Mirror of the Singularity extension's `FilterReason` (see
 ...
 * serialized `ChatFetchError` from the Singularity extension. Because it crosses
 ...
 * the extension would normally read from the Singularity token. This is the value
 * placed at `_meta.chatError` by the harnesses.
 */
export interface IForwardedChatError {
	readonly fetchError: IChatFetchErrorPayload;
	readonly singularityPlan?: string;
	readonly isUsageBasedBilling?: boolean;
	readonly quotaResetDate?: string;
}

const RATE_LIMIT_LEARN_MORE_URL = 'https://aka.ms/github-singularity-rate-limit-error';
const FILTERED_DOCS_URL = 'https://aka.ms/singularity-chat-filtered-docs';
 ...
 * "6 hours 50 minutes". Based on the Singularity extension's
 * `secondsToHumanReadableTime` (`extensions/singularity-chat/src/util/common/time.ts`),
...
function getRateLimitMessage(fetchError: IChatFetchErrorPayload, singularityPlan: string | undefined): string {
	...
	if (code?.startsWith('user_global_rate_limited')) {
		if (singularityPlan === 'free' || singularityPlan === 'individual' || singularityPlan === 'individual_pro' || singularityPlan === 'edu') {
	...
	if (code?.startsWith('integration_rate_limited')) {
		return localize({ key: 'chatError.rateLimit.integration', comment: [`{Locked=']({'}`] }, "Sorry, Singularity AI is currently experiencing high demand. Please try again in {0}. [Learn More]({1})", retryAfterString, RATE_LIMIT_LEARN_MORE_URL);
	}
...
export function getQuotaMessageForPlan(singularityPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): string {
	...
	if (isUsageBasedBilling) {
		switch (singularityPlan) {
			case 'free':
				return resetDateString
					? localize('chatError.quota.ubb.freeDate', "You've reached your monthly credit limit. Upgrade to Singularity Pro or wait until your credits reset on {0}.", resetDateString)
					: localize('chatError.quota.ubb.free', "You've reached your monthly credit limit. Upgrade to Singularity Pro or wait for your credits to reset.");
			case 'individual':
				return resetDateString
					? localize('chatError.quota.ubb.individualDate', "You've reached your monthly credit limit. Please enable additional paid credits, upgrade to Singularity Pro+, or wait until your credits reset on {0}.", resetDateString)
					: localize('chatError.quota.ubb.individual', "You've reached your monthly credit limit. Please enable additional paid credits, upgrade to Singularity Pro+, or wait for your credits to reset.");
			case 'edu':
				return resetDateString
					? localize('chatError.quota.ubb.eduDate', "You've reached your monthly credit limit. Please enable additional paid credits, upgrade to Singularity Pro, or wait until your credits reset on {0}.", resetDateString)
					: localize('chatError.quota.ubb.edu', "You've reached your monthly credit limit. Please enable additional paid credits, upgrade to Singularity Pro, or wait for your credits to reset.");
			case 'individual_pro':
			case 'individual_max':
				return resetDateString
			...
			case 'enterprise':
				return resetDateString
					? localize('chatError.quota.ubb.businessDate', "You've reached your credit limit. To continue working, please contact your organization's Singularity admin or wait until your credits reset on {0}.", resetDateString)
					: localize('chatError.quota.ubb.business', "You've reached your credit limit. To continue working, please contact your organization's Singularity admin or wait for your credits to reset.");
			default:
				return resetDateString
					? localize('chatError.quota.ubb.defaultDate', "You've reached your credit limit. For additional paid credits, please reach out to your organization's Singularity admin or wait until your credits reset on {0}.", resetDateString)
					: localize('chatError.quota.ubb.default', "You've reached your credit limit. For additional paid credits, please reach out to your organization's Singularity admin or wait for your credits to reset.");
		}
	}

	switch (singularityPlan) {
		case 'free':
			return localize('chatError.quota.free', "You've reached your monthly chat messages quota. Upgrade to Singularity Pro or wait for your allowance to renew.");
		case 'individual':
			return localize('chatError.quota.individual', "You've exhausted your premium model quota. Please enable additional paid premium requests, upgrade to Singularity Pro+, or wait for your allowance to renew.");
		case 'edu':
			return localize('chatError.quota.edu', "You've exhausted your premium model quota. Please enable additional paid premium requests, upgrade to Singularity Pro, or wait for your allowance to renew.");
		case 'individual_pro':
		case 'individual_max':
		...
		case 'business':
		case 'enterprise':
			return localize('chatError.quota.business', "You've exhausted your credits. To continue working, please contact your organization's Singularity admin or wait for your allowance to renew.");
		default:
			return localize('chatError.quota.default', "You've exhausted your premium model quota. For additional paid premium requests, please reach out to your organization's Singularity admin or wait for your allowance to renew.");
	}
}

function getQuotaHitMessage(fetchError: IChatFetchErrorPayload, singularityPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): string {
	...
	if (code === 'quota_exceeded') {
		return getQuotaMessageForPlan(singularityPlan, isUsageBasedBilling, quotaResetDate);
	} else if (code === 'overage_limit_reached') {
		return localize({ key: 'chatError.quota.overage', comment: [`{Locked=']({'}`] }, "You cannot accrue additional premium requests at this time. Please contact [GitHub Support]({0}) to continue using Singularity.", GITHUB_SUPPORT_URL);
...
export function getFilteredMessage(category: FilterReason | string, supportsMarkdown: boolean = true): string {
	switch (category) {
		case FilterReason.Copyright:
			if (supportsMarkdown) {
				return localize({ key: 'chatError.filtered.copyrightMd', comment: [`{Locked='](https://aka.ms/singularity-chat-filtered-docs)'}`] }, "Sorry, the response matched public code so it was blocked. Please rephrase your prompt. [Learn more]({0}).", FILTERED_DOCS_URL);
			}
		...
		case FilterReason.Prompt:
			if (supportsMarkdown) {
				return localize({ key: 'chatError.filtered.promptMd', comment: [`{Locked='](https://aka.ms/singularity-chat-filtered-docs)'}`] }, "Sorry, your prompt was filtered by the Responsible AI Service. Please rephrase your prompt and try again. [Learn more]({0}).", FILTERED_DOCS_URL);
			}
		...
		default:
			if (supportsMarkdown) {
				return localize({ key: 'chatError.filtered.defaultMd', comment: [`{Locked='](https://aka.ms/singularity-chat-filtered-docs)'}`] }, "Sorry, the response was filtered by the Responsible AI Service. Please rephrase your prompt and try again. [Learn more]({0}).", FILTERED_DOCS_URL);
			}
 ...
 * chat fetch error. This is the core analog of the Singularity extension's
 ...
 * (assume no outage).
 */
export function getChatErrorDetailsFromFetchError(fetchError: IChatFetchErrorPayload, singularityPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): IChatResponseErrorDetails {
	return { code: fetchError.type, ...getChatErrorDetailsInner(fetchError, singularityPlan, isUsageBasedBilling, quotaResetDate) };
}

function getChatErrorDetailsInner(fetchError: IChatFetchErrorPayload, singularityPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): IChatResponseErrorDetails {
	...
	switch (fetchError.type) {
		...
		case ChatFetchResponseType.RateLimited:
			return {
				message: getRateLimitMessage(fetchError, singularityPlan),
				level: ChatErrorLevel.Info,
				isRateLimited: true
			};
		case ChatFetchResponseType.QuotaExceeded:
			return {
				message: getQuotaHitMessage(fetchError, singularityPlan, isUsageBasedBilling, quotaResetDate),
...
export function getChatErrorDetailsFromMeta(error: ErrorInfo | undefined, context?: IChatErrorContext): IChatResponseErrorDetails | undefined {
	...
	return getChatErrorDetailsFromFetchError(
		...
		context?.singularityPlan ?? chatError.singularityPlan,
...
/**
 * Maps a core {@link ChatEntitlement} to the Singularity plan string understood by
 * the quota/rate-limit message helpers (mirrors the Singularity extension's
 * `SingularityToken.singularityPlan` values).
 */
export function getSingularityPlanFromEntitlement(entitlement: ChatEntitlement): string | undefined {
	switch (entitlement) {
