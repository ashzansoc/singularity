...
export namespace ChatContextKeys {
	...
	/**
	 * Widget-scoped: logical Agent Host provider ID for this chat widget, e.g. `singularitycli`, `claude`, or `codex`.
	 */
	...
	export const nonSingularityLanguageModelsAreUserSelectable = new RawContextKey<boolean>('chatNonSingularityModelsAreUserSelectable', false, { type: 'boolean', description: localize('chatNonSingularityModelsAreUserSelectable', "True when a user-selectable chat model from a non-Singularity vendor is available.") });
	...
	export const newChatButtonExperimentIcon = new RawContextKey<string>('chatNewChatButtonExperimentIcon', '', { type: 'string', description: localize('chatNewChatButtonExperimentIcon', "The icon variant for the new chat button, controlled by experiment. Values: 'singularity', 'new-session', 'comment', or empty for default.") });
}
