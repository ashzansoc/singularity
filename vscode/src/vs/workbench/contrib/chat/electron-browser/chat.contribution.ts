...
import { SingularityConfigSlashSubmitHandlerContribution } from '../browser/agentSessions/agentHost/singularityConfigSlashSubmitHandler.js';
...
class ChatCommandLineHandler extends Disposable {
	...
	private async prompt(args: typeof this.environmentService.args.chat): Promise<void> {
		...
		const trusted = await this.workspaceTrustRequestService.requestWorkspaceTrust({
			message: localize('singularityWorkspaceTrust', "AI features are currently only supported in trusted workspaces.")
		});
...
registerWorkbenchContribution2(SingularityConfigSlashSubmitHandlerContribution.ID, SingularityConfigSlashSubmitHandlerContribution, WorkbenchPhase.AfterRestored);
...
registerWorkbenchContribution2(AgentHostSingularityCliSettingsContribution.ID, AgentHostSingularityCliSettingsContribution, WorkbenchPhase.AfterRestored);
...
function getSingularityAgentInfo(rootState: RootState | Error | undefined): AgentInfo | undefined {
	if (!rootState || rootState instanceof Error) {
		return undefined;
	}
	return rootState.agents.find(a => a.provider === 'singularitycli');
}
 ...
 * `agent-host-${agent.provider}` (e.g. `agent-host-singularitycli`) only after it
 * surfaces an `AgentInfo` via `rootState`. This is asynchronous, so the static
 * `agent-host-singularity` umbrella commands need to wait for that registration
...
async function resolveAgentHostSessionType(agentHostService: IAgentHostService): Promise<string> {
	const agent = getSingularityAgentInfo(agentHostService.rootState.value);
	if (agent) {
	...
	const waitForAgent = new Promise<AgentInfo | undefined>(res => {
		const sub = agentHostService.rootState.onDidChange(state => {
			const found = getSingularityAgentInfo(state);
			if (found) {
				sub.dispose();
				res(found);
			}
	...
	if (!resolved) {
		throw new Error('Agent host did not register a singularitycli agent within the timeout period. Ensure the agent host is enabled and running.');
	}
...
// The dynamic per-agent commands (e.g. `agent-host-singularity`) are only
...
CommandsRegistry.registerCommand(
	`workbench.action.chat.openNewSessionSidebar.${AgentSessionProviders.AgentHostSingularity}`,
	accessor => openNewAgentHostSession(accessor, ChatSessionPosition.Sidebar)
);
CommandsRegistry.registerCommand(
	`workbench.action.chat.openNewSessionEditor.${AgentSessionProviders.AgentHostSingularity}`,
