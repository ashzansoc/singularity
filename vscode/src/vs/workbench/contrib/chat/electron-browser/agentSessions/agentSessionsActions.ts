 ...
 * the active chat is a first-party agent-host session (Singularity CLI today)
		...
		super({
			...
			menu: [{
				...
				when: ContextKeyExpr.and(
					OPEN_AGENTS_WINDOW_PRECONDITION,
					ContextKeyExpr.or(
						ChatContextKeys.chatSessionType.isEqualTo(SessionType.SingularityCLI),
						ChatContextKeys.chatSessionType.isEqualTo(SessionType.AgentHostSingularity),
					),
				),
			}],
		});
...
export const enum AgentsHandoffTipMode {
	...
	Default = 'default',
	/** Show the tip with the alternate "Free with your Singularity" framing. */
	Custom = 'custom',
}
...
type AgentsHandoffTipActionClassification = {
	...
	sessionType: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'The chat session type / agent harness being handed off (e.g. singularity-cli, agent-host-singularity).' };
	owner: 'justschen';
 ...
 * widget is showing a contributed session (Singularity CLI, Cloud, Claude, etc.)
...
export class AgentsHandoffInputTipContribution extends Disposable implements IWorkbenchContribution {
	...
	private static readonly ELIGIBLE_SESSION_TYPES: ReadonlySet<string> = new Set([SessionType.SingularityCLI, SessionType.AgentHostSingularity]);
	...
	private _update(): void {
		...
		// don't post the tip for non-eligible session kinds (Singularity Cloud,
		...
		const emptyWorkspaceEligible = preconditionMet
			...
			&& widgetSessionType === SessionType.AgentHostSingularity;
		...
		// Empty-workspace + local Singularity CLI: the local agent host can't
		...
		const message = useEmptyWorkspaceCopy
			? localize('chat.agentsHandoff.tip.emptyWorkspace.message', "Singularity CLI [Agent Host] isn't available without an open folder")
		...
		const description = useEmptyWorkspaceCopy
			? localize('chat.agentsHandoff.tip.emptyWorkspace.description', "Open the Agents Window to start a Singularity CLI session.")
			: mode === AgentsHandoffTipMode.Custom
				? localize('chat.agentsHandoff.tip.description.singularity', "Free with your Singularity plan — get a dedicated, multi-pane view alongside your workspace.")
		...
		this._notificationService.setNotification({
			...
			sessionTypes: useEmptyWorkspaceCopy
				? [SessionType.AgentHostSingularity]
