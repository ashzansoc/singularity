...
async function collectNetworkDiagnostics(connectionsService: IAgentHostConnectionsService, nativeHostService: INativeHostService): Promise<string> {
	...
	output += 'Connectivity probes run inside each agent host process (local or remote), so results reflect the environment the Singularity SDK actually connects from.\n\n';
