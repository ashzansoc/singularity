...
export class ChatStatusDashboard extends DomWidget {
	...
	private render(): void {
		...
		if (hasUsageSection && !this.options?.compactQuotaLayout) {
			...
			const header = this.renderHeader(headerHost, this._store, planName, toAction({
				id: 'workbench.action.manageSingularity',
				label: localize('quotaLabel', "Manage Singularity Settings"),
				tooltip: localize('quotaTooltip', "Manage Singularity Settings"),
				class: ThemeIcon.asClassName(Codicon.settings),
				run: () => this.runCommandAndClose(() => this.openerService.open(URI.parse(this.defaultAccountService.resolveGitHubUrl(GitHubPaths.singularitySettings)))),
			}));
	...
	private renderSetupSection(): void {
		...
		// users can still authenticate to unlock the full Singularity experience.
		...
		if (newUser && anonymousUser) {
			descriptionText = new MarkdownString(localize({ key: 'activeDescriptionAnonymous', comment: ['{Locked="]({1})"}', '{Locked="]({2})"}'] }, "By continuing with Singularity, you agree to {0}'s [Terms]({1}) and [Privacy Statement]({2})", defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl), { isTrusted: true });
			descriptionClass = `${descriptionClass}.terms`;
		} else if (newUser) {
			descriptionText = localize('activateDescription', "Set up Singularity to use AI features.");
		} else if (anonymousUser) {
			descriptionText = localize('enableMoreDescription', "Sign in to enable more Singularity AI features.");
		} else if (disabled) {
			descriptionText = localize('enableDescription', "Enable Singularity to use AI features.");
		} else {
			descriptionText = localize('signInDescription', "Sign in to use Singularity AI features.");
		}
		...
		if (newUser) {
			...
			buttonLabel = localize('enableSingularityButton', "Enable AI Features");
		} else {
			buttonLabel = localize('signInToUseAIFeatures', "Sign in to use Singularity");
		}
	...
	private createGlobalQuotaCallout(container: HTMLElement): () => { calloutVisible: boolean; additionalUsageEnabled: boolean } {
		...
		const update = () => {
			...
			// not trigger the "Singularity is paused" message reserved for chat limits.
			...
			if (isEnterpriseUser && isPooledQuotaExhausted) {
				...
				calloutText.textContent = localize('quotaBudgetExceededEnterprise', "Your organization or enterprise has exceeded its Singularity budget. Contact your admin to resume usage.");
			} else if (maxUsedPercentage >= 100 && additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaAdditionalUsageActiveEnterprise', "Singularity has paused because your limits are reached. Please contact your admin to increase your limits.")
					: isUsageBasedBilling
				...
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaAdditionalUsageApproachingEnterprise', "Singularity will pause when your limits are reached. Please contact your admin to increase your limits.")
					: isUsageBasedBilling
				...
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaPausedEnterprise', "Singularity is paused until the limit resets. Contact your administrator for more information.")
					: localize('quotaPaused', "Singularity is paused until the limit resets.");
			} else if (maxUsedPercentage >= 75 && !additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaWarningEnterprise', "Singularity will pause when the limit is reached. Contact your administrator for more information.")
					: localize('quotaWarning', "Singularity will pause when the limit is reached.");
			} else {
				quotaCallout.style.display = 'none';
			}
