...
/**
 * Tracks whether Singularity is currently blocked by a reached quota limit, has
...
/**
 * Pure state transition for the Singularity quota "resumed" indicator:
...
export class ChatStatusBarEntry extends Disposable implements IWorkbenchContribution {
	...
	private getEntryProps(): IStatusbarEntry {
		let text = '$(singularity)';
		let ariaLabel = localize('chatStatusAria', "Singularity status");
		...
		if (isNewUser(this.chatEntitlementService)) {
			...
			if (this.chatEntitlementService.sentiment.disabled || this.chatEntitlementService.sentiment.untrusted) {
				text = '$(singularity-unavailable)';
				ariaLabel = localize('singularityDisabledStatus', "Singularity disabled");
			}

			// Signed out — keep showing Sign-in affordance even when BYOK models are present
			// so air-gapped users can still authenticate to unlock the full Singularity experience.
			...
			// Quota Exceeded (all tracked plans share the premium chat quota)
				...
				text = `$(singularity-warning) ${quotaWarning}`;
				ariaLabel = quotaWarning;
				kind = 'prominent';
			}

			// Singularity Resumed (limit reset after the user was previously blocked)
			else if (this.quotaResumeState === 'resumed') {
				const resumedLabel = localize('chatResumedStatus', "Singularity Resumed");
				text = `$(singularity) ${resumedLabel}`;
	...
	private getSetupEntryProps(): IStatusbarEntry {
		...
		return {
			name: localize('chatStatus', "Singularity Status"),
			text: showSignInLabel ? `$(singularity) ${signInLabel}` : '$(singularity)',
			ariaLabel: showSignInLabel ? signInLabel : localize('chatStatusAria', "Singularity status"),
