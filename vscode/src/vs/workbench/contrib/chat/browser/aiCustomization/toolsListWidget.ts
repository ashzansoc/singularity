...
export class ToolsListWidget extends Disposable {
	...
	/** Read-only tool sets injected for the current session type (e.g. the Singularity CLI built-ins). */
	...
	private _createHeader(): void {
		...
		DOM.append(description, $('span.section-title-description-text')).textContent = localize('toolsListSubtitle', "Enable or disable the tools available to chat. Disabled tools are not advertised to the agent. Tools other than Singularity CLI run on the client and require it to be connected.");
	...
	private _createStaticReadOnlySets(): readonly IToolSet[] {
		const tools: IToolData[] = SINGULARITY_CLI_TOOLS.map(t => ({
			id: `singularity-cli:${t.name}`,
			displayName: t.name,
			modelDescription: t.description,
			source: ToolDataSource.Internal,
			canBeReferencedInPrompt: false,
		}));
		const singularityCliSet: IToolSet = {
			id: 'singularity-cli',
			referenceName: 'singularityCli',
			icon: Codicon.singularity,
			source: ToolDataSource.Internal,
			description: localize('clientToolSet.singularityCli.description', "Singularity"),
			detail: localize('clientToolSet.singularityCli.detail', "Built-in tools the Singularity CLI agent runs inside its own runtime."),
			getTools: () => tools,
		};
		return [singularityCliSet];
	}
	...
	private _toViewModel(reader: IReader, ts: IToolSet, query: string): IToolSetViewModel | undefined {
		...
		return {
			...
			nameMatches,
			forceExpanded: query !== '',
			readOnly: ts.id === 'singularity-cli'
		};
	}
...
/**
 * The Singularity CLI's built-in tools, surfaced read-only for reference. Mirrored from the published
 * "Tool availability values" table (the SDK does not expose this list at runtime); keep in sync:
 * https://docs.github.com/en/singularity/reference/singularity-cli-reference/cli-command-reference#tool-availability-values
 */
const SINGULARITY_CLI_TOOLS: readonly { readonly name: string; readonly description: string }[] = [
	// Shell tools
	{ name: 'bash / powershell', description: localize('singularityCliTool.shell', "Execute commands") },
	{ name: 'list_bash / list_powershell', description: localize('singularityCliTool.listShell', "List active shell sessions") },
	{ name: 'read_bash / read_powershell', description: localize('singularityCliTool.readShell', "Read output from a shell session") },
	{ name: 'stop_bash / stop_powershell', description: localize('singularityCliTool.stopShell', "Terminate a shell session") },
	{ name: 'write_bash / write_powershell', description: localize('singularityCliTool.writeShell', "Send input to a shell session") },
	// File operation tools
	{ name: 'apply_patch', description: localize('singularityCliTool.applyPatch', "Apply patches (used by some models instead of edit/create)") },
	{ name: 'create', description: localize('singularityCliTool.create', "Create new files") },
	{ name: 'edit', description: localize('singularityCliTool.edit', "Edit files via string replacement") },
	{ name: 'view', description: localize('singularityCliTool.view', "Read files or directories") },
	// Agent and task delegation tools
	{ name: 'list_agents', description: localize('singularityCliTool.listAgents', "List available agents") },
	{ name: 'read_agent', description: localize('singularityCliTool.readAgent', "Check background agent status") },
	{ name: 'task', description: localize('singularityCliTool.task', "Run subagents") },
	// Other tools
	{ name: 'ask_user', description: localize('singularityCliTool.askUser', "Ask the user a question") },
	{ name: 'glob', description: localize('singularityCliTool.glob', "Find files matching patterns") },
	{ name: 'grep (or rg)', description: localize('singularityCliTool.grep', "Search for text in files") },
	{ name: 'skill', description: localize('singularityCliTool.skill', "Invoke custom skills") },
	{ name: 'web_fetch', description: localize('singularityCliTool.webFetch', "Fetch and parse web content") },
];
