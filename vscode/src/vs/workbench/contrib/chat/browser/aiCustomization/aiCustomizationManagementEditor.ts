...
import { AGENT_HOST_SINGULARITY_CLI_SESSION_TYPE } from '../agentSessions/agentHost/agentHostToolSetEnablementService.js';
import {
...
export class AICustomizationManagementEditor extends EditorPane {
	...
	private createContent(): void {
		...
		if (hasSections.has(AICustomizationManagementSection.Tools)) {
			...
			// Tools customizations only target the agent host (Singularity CLI), in both windows.
			this.toolsListWidget = this.editorDisposables.add(this.instantiationService.createInstance(ToolsListWidget, AGENT_HOST_SINGULARITY_CLI_SESSION_TYPE));
	...
	private async createNewItemManual(type: PromptsType, target: 'local' | 'user' | 'workspace-root', rootFileName?: string): Promise<void> {
		...
		if (type === PromptsType.hook) {
			if (this.workspaceService.isSessionsWindow) {
				// Sessions: show hooks filtered to Singularity CLI (Singularity) hook types
