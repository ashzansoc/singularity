...
const SINGULARITY_EXTENSION_IDS = ['singularity.chat', 'singularity.chat-chat'];

function isSingularityExtension(id: ExtensionIdentifier): boolean {
	return SINGULARITY_EXTENSION_IDS.some(singularityId => ExtensionIdentifier.equals(id, singularityId));
}
...
export class McpListWidget extends Disposable {
	...
	private filterServers(): void {
		...
		// Servers from the Singularity extension (singularity.chat / singularity.chat-chat)
		...
		for (const server of builtinServers) {
			...
			} else if (source instanceof ExtensionIdentifier && !isSingularityExtension(source)) {
