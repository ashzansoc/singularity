...
import { AgentHostSingularitySdkLogLevelSettingId, AgentHostCustomTerminalToolEnabledSettingId, AgentHostModelCapabilityOverridesSettingId, AgentHostOpus48PromptEnabledSettingId, AgentHostReasoningEffortOverrideSettingId, AgentHostToolSearchDeferThresholdSettingId, AgentHostToolSearchEnabledSettingId, singularitySdkLogLevelSettingValues } from '../../../../platform/agentHost/common/singularityCliConfig.js';
import { AgentHostAutoReplyEnabledConfigKey, AgentHostGlobalAutoApproveEnabledConfigKey, AgentHostMigrateLegacySingularityCliEnabledConfigKey, AgentHostSessionSyncEnabledConfigKey } from '../../../../platform/agentHost/common/agentHostSchema.js';
...
import { SINGULARITY_ALLOWED_MCP_SERVERS_KEY, SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG, SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_KEY, SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_CONFIG, SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_KEY, SINGULARITY_DENIED_MCP_SERVERS_KEY, SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY, SINGULARITY_ENABLED_PLUGINS_KEY, SINGULARITY_EXTRA_MARKETPLACES_KEY, SINGULARITY_MODEL_KEY, SINGULARITY_STRICT_MARKETPLACES_KEY, SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_CONFIG, SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY, managedModelValue, managedSettingValue } from '../../../../platform/policy/common/singularityManagedSettings.js';
...
import { IPlanTodoSyncService, PlanTodoSyncService } from '../common/plan/planTodoSyncService.js';
import { IExecutionEngineService, ExecutionEngineService } from '../common/execution/executionEngineService.js';
import '../browser/execution/executionGraphContribution.js';
import '../browser/execution/executionAgentHostContribution.js';
import '../browser/execution/executionExtensionHostContribution.js';
import './plan/planEditor.contribution.js';
...
import { AgentPluginService, ConfiguredAgentPluginDiscovery, SingularityCliAgentPluginDiscovery, ExtensionAgentPluginDiscovery, MarketplaceAgentPluginDiscovery } from '../common/plugins/agentPluginServiceImpl.js';
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.MigrateLegacySingularityCliSessions]: {
			type: 'boolean',
			markdownDescription: nls.localize('chat.agentSessions.migrateLegacySingularityCli', "Controls whether legacy extension host Singularity CLI chat sessions are migrated in place to the Agent host when opened, so their history becomes editable. When disabled, legacy sessions open as before."),
			default: false,
			tags: ['experimental'],
			experiment: {
				mode: 'startup'
			},
			agentHost: { key: AgentHostMigrateLegacySingularityCliEnabledConfigKey },
		},
		...
		[ChatConfiguration.VisualCriticEnabled]: {
			type: 'boolean',
			default: false,
			markdownDescription: nls.localize('chat.visualCritic.enabled', "When enabled, **Critic** keeps iterating on frontend work (Visual Critic stop-hook, up to 3 passes) until scores pass. When disabled, skill.md still builds `design-spec.json` and the agent implements once, then stops — no refine loop."),
			tags: ['experimental'],
		},
		...
		[ChatConfiguration.DefaultConfiguration]: {
			...
			markdownDescription: nls.localize('chat.defaultConfiguration.settingDescription', "Controls the default configuration for new agent sessions (such as Singularity CLI). You can still change the mode and approval behavior per session, and each session remembers what was used."),
		},
		[ChatConfiguration.DefaultModel]: {
			type: 'string',
			default: '',
			markdownDescription: nls.localize('chat.defaultModel.description', "The default model for new chat conversations. Use \"auto\" to let Singularity pick a model, a model family name (such as \"opus\" or \"gemini\") to use the latest available model in that family, or a full model id. You can still switch the model within a conversation; each new conversation starts at this model."),
			experiment: {
				mode: 'auto'
			},
			policy: {
				...
				managedSettings: {
					[SINGULARITY_MODEL_KEY]: { type: 'string' },
				},
				localization: {
					description: {
		...
		[ChatConfiguration.GlobalAutoApprove]: {
			...
			policy: {
				...
				category: PolicyCategory.InteractiveSession,
				minimumVersion: '1.99',
				value: (policyData) => policyData.managedSettings?.[SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY] === 'disable' || policyData.chat_preview_features_enabled === false ? false : undefined,
				managedSettings: {
					[SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: { type: 'string' },
				},
				localization: {
					description: {
		...
		[ChatConfiguration.SessionSyncEnabled]: {
			default: false,
			markdownDescription: nls.localize('chat.sessionSync.enabled', "Enable session sync to GitHub.com. When enabled, Singularity session data is synced to your GitHub account for cross-device access and richer insights. Requires `#singularity.chat.chat.localIndex.enabled#` to also be enabled."),
			type: 'boolean',
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
			...
			policy: {
				...
				localization: {
					description: {
						...
						value: nls.localize('chat.sessionSync.enabled.policy', "Enable session sync to GitHub.com for cross-device Singularity session history. When disabled by organization policy, session data is kept local only."),
					}
				},
			},
		...
		[ClaudePreferAgentHostAgentsSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agents.claude.preferAgentHost', "When enabled, Claude sessions opened from the Agents Window run inside the agent host process instead of the Singularity AI extension. Only one Claude implementation surfaces per window."),
			default: true,
			tags: ['experimental'],
			experiment: { mode: 'startup' },
		},
		[ClaudePreferAgentHostEditorSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.editor.claude.preferAgentHost', "When enabled, Claude sessions opened from the regular workbench (sidebar chat) run inside the agent host process instead of the Singularity AI extension. Only one Claude implementation surfaces per window."),
			default: true,
			tags: ['experimental'],
			experiment: { mode: 'startup' },
		},
		...
		[mcpAllowedServersConfig]: {
			...
			policy: {
				...
				value: managedSettingValue(SINGULARITY_ALLOWED_MCP_SERVERS_KEY),
				managedSettings: {
					[SINGULARITY_ALLOWED_MCP_SERVERS_KEY]: { type: 'string' },
				},
				localization: {
					description: {
		...
		[mcpDeniedServersConfig]: {
			...
			policy: {
				...
				category: PolicyCategory.InteractiveSession,
				minimumVersion: '1.130',
				value: managedSettingValue(SINGULARITY_DENIED_MCP_SERVERS_KEY),
				managedSettings: {
					[SINGULARITY_DENIED_MCP_SERVERS_KEY]: { type: 'string' },
				},
				localization: {
					description: {
		...
		[SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_CONFIG]: {
			type: 'boolean',
			default: false,
			scope: ConfigurationScope.APPLICATION,
			included: false,
			...
			policy: {
				...
				value: managedSettingValue(SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_KEY),
				managedSettings: {
					[SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_KEY]: { type: 'boolean' },
				},
				localization: {
					description: {
		...
		[ChatConfiguration.EnabledPlugins]: {
			...
			policy: {
				...
				category: PolicyCategory.InteractiveSession,
				minimumVersion: '1.122',
				value: managedSettingValue(SINGULARITY_ENABLED_PLUGINS_KEY),
				managedSettings: {
					[SINGULARITY_ENABLED_PLUGINS_KEY]: { type: 'string' },
				},
				localization: {
					description: {
		...
		[ChatConfiguration.PluginMarketplaces]: {
			...
			default: ['github/singularity-plugins', 'github/awesome-singularity#marketplace'],
			scope: ConfigurationScope.APPLICATION,
			tags: ['experimental'],
		},
		[ChatConfiguration.ExtraMarketplaces]: {
			...
			policy: {
				...
				category: PolicyCategory.InteractiveSession,
				minimumVersion: '1.122',
				value: managedSettingValue(SINGULARITY_EXTRA_MARKETPLACES_KEY),
				managedSettings: {
					[SINGULARITY_EXTRA_MARKETPLACES_KEY]: { type: 'string' },
				},
				localization: {
					description: {
		...
		[ChatConfiguration.StrictMarketplaces]: {
			...
			policy: {
				...
				category: PolicyCategory.InteractiveSession,
				minimumVersion: '1.122',
				value: managedSettingValue(SINGULARITY_STRICT_MARKETPLACES_KEY),
				managedSettings: {
					[SINGULARITY_STRICT_MARKETPLACES_KEY]: { type: 'string' },
				},
				localization: {
					description: {
		...
		[SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_CONFIG]: {
			type: 'boolean',
			default: false,
			scope: ConfigurationScope.APPLICATION,
			included: false,
			...
			policy: {
				...
				category: PolicyCategory.InteractiveSession,
				minimumVersion: '1.132',
				value: managedSettingValue(SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY),
				managedSettings: {
					[SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY]: { type: 'boolean' },
				},
				localization: {
					description: {
		...
		[SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG]: {
			type: 'boolean',
			default: false,
			scope: ConfigurationScope.APPLICATION,
			included: false,
			...
			policy: {
				...
				category: PolicyCategory.InteractiveSession,
				minimumVersion: '1.132',
				value: managedSettingValue(SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_KEY),
				managedSettings: {
					[SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_KEY]: { type: 'boolean' },
				},
				localization: {
					description: {
		...
		[AgentHostCustomTerminalToolEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.customTerminalTool.enabled', "When enabled, Singularity SDK sessions use the Agent Host terminal tool override instead of the SDK's default terminal behavior."),
			default: false,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostSingularitySdkLogLevelSettingId]: {
			type: 'string',
			enum: [...singularitySdkLogLevelSettingValues],
			enumDescriptions: [
				nls.localize('chat.agentHost.singularitySdk.logLevel.info', "Log informational messages. Running VS Code with trace logging still enables all Singularity SDK runtime diagnostics."),
				nls.localize('chat.agentHost.singularitySdk.logLevel.trace', "Log all Singularity SDK runtime diagnostics."),
			],
			markdownDescription: nls.localize('chat.agentHost.singularitySdk.logLevel', "Controls the log level for the Singularity SDK runtime used by the local agent host. Changing this setting restarts the Singularity SDK client; active sessions are reloaded when next used."),
			default: 'info',
			scope: ConfigurationScope.APPLICATION,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostOpus48PromptEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.opus48Prompt.enabled', "When enabled, Singularity SDK sessions running a Claude Opus 4.8 model apply Opus 4.8-tuned system-prompt section overrides on top of the default system message."),
			default: false,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostToolSearchEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.singularity.toolSearch.enabled', "When enabled, Singularity SDK sessions defer MCP and non-core VS Code tools behind a tool-search tool so the model discovers them on demand instead of loading every tool definition up front."),
			default: true,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostToolSearchDeferThresholdSettingId]: {
			type: 'number',
			description: nls.localize('chat.agentHost.singularity.toolSearch.deferThreshold', "Minimum number of tools before MCP and external tools are deferred behind tool search. Set to 0 to always defer external tools. Only effective when tool search is enabled."),
			default: 1,
			minimum: 0,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostReasoningEffortOverrideSettingId]: {
			type: 'string',
			markdownDescription: nls.localize('chat.agentHost.reasoningEffortOverride', "Overrides the reasoning effort for Singularity SDK agent sessions regardless of the per-model picker value. Set it to a level the selected model supports (for example `low`, `medium`, `high`, or `xhigh`) — choosing a level the model does not support may be rejected by the model. A value that isn't a recognized effort level is ignored and the session falls back to the picker value. Applied when a session is created and when its model changes. Only affects Singularity CLI agent sessions.\n\n**Note**: This is an advanced setting for experimentation."),
			default: '',
			tags: ['experimental', 'advanced'],
		},
		[AgentHostModelCapabilityOverridesSettingId]: {
			type: 'object',
			markdownDescription: nls.localize('chat.agentHost.modelCapabilityOverrides', "Per-model capability overrides for Singularity SDK agent sessions, keyed by model id, intended for evaluating preview models against an existing model's profile. For each model id, declare an aliased `family` (for example `claude-opus-4-8`) to route the model to that family's tuned system prompt without a code change; the model id sent to the runtime is unaffected. Only affects Singularity CLI agent sessions.\n\n**Note**: This is an advanced setting for experimentation."),
			additionalProperties: {
				type: 'object',
				properties: {
					family: {
		...
		[AgentHostSdkSandboxEnabledSettingId]: {
			...
			markdownDescription: nls.localize('chat.agentHost.sdkSandbox.enabled', "Sandbox mode for the Singularity SDK's built-in shell tool. Only takes effect when `#chat.agentHost.customTerminalTool.enabled#` is `false`; when the Agent Host's own terminal tool is enabled, the engine sandbox is controlled by `#chat.agent.sandbox.enabled#`. The sandbox applies only to requests that run with default permissions — not when approvals are bypassed — and is not supported on Windows yet."),
		...
		[ChatConfiguration.ToolRiskAssessmentModel]: {
			...
			default: 'singularity-utility-small',
			tags: ['experimental', 'advanced'],
			experiment: {
				mode: 'auto'
			},
		...
		[ChatConfiguration.BYOKUtilityModelDefault]: {
			...
			enumItemLabels: [
				...
				nls.localize('chat.byokUtilityModelDefault.singularity.label', "Singularity"),
			],
			markdownEnumDescriptions: [
				...
				nls.localize('chat.byokUtilityModelDefault.singularity.description', "Use the default Singularity utility models."),
			],
			default: BYOKUtilityModelDefault.Singularity,
		},
		...
		[PromptsConfig.AGENTS_LOCATION_KEY]: {
			...
			default: {
				...
				[SINGULARITY_USER_AGENTS_SOURCE_FOLDER]: true,
			},
			additionalProperties: { type: 'boolean' },
			propertyNames: {
				pattern: VALID_PROMPT_FOLDER_PATTERN,
		...
		[PromptsConfig.HOOKS_LOCATION_KEY]: {
			...
			markdownDescription: nls.localize(
				...
				"Specify paths to hook configuration files that define custom shell commands to execute at strategic points in an agent's workflow. [Learn More]({0}).\n\nRelative paths are resolved from the root folder(s) of your workspace. Supports Singularity hooks (`*.json`) and Claude Code hooks (`settings.json`, `settings.local.json`).\n\nThis setting is only used by the Local agent harness.",
		...
		[ChatAIDisabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.disableAIFeatures', "Disable and hide built-in AI features provided by Singularity, including chat and inline suggestions."),
			default: false,
			scope: ConfigurationScope.WINDOW,
		},
		[ChatConfiguration.TitleBarSignInEnabled]: {
			type: 'boolean',
			description: nls.localize('chat.titleBar.signIn.enabled', "Controls whether the Singularity Sign In button is shown in the title bar when signed out. When disabled, the Sign In affordance falls back to the status bar."),
			default: true,
		},
		...
		[ChatConfiguration.GrowthNotificationEnabled]: {
			type: 'boolean',
			description: nls.localize('chat.growthNotification', "Controls whether to show a growth notification in the agent sessions view to encourage new users to try Singularity."),
			default: false,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
		...
		'chat.extensionUnification.enabled': {
			type: 'boolean',
			description: nls.localize('chat.extensionUnification.enabled', "Enables the unification of Singularity extensions. When enabled, all Singularity functionality is served from the Singularity AI extension. When disabled, the Singularity and Singularity AI extensions operate independently."),
			default: true,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
		...
		[ChatConfiguration.CollectInstructionsInExtension]: {
			type: 'boolean',
			description: nls.localize('chat.experimental.collectInstructionsInExtension', "When enabled, automatic instruction collection (.instructions.md, agent instructions, customizations index) is performed by the Singularity AI extension instead of the core workbench."),
			default: false,
			tags: ['experimental'],
		},
...
Registry.as<IConfigurationMigrationRegistry>(Extensions.ConfigurationMigration).registerConfigurationMigrations([
	...
	{
		key: 'chat.useSingularityModelsForUtilityModels',
		migrateFn: (value: unknown, valueAccessor) => {
			const result: ConfigurationKeyValuePairs = [['chat.useSingularityModelsForUtilityModels', { value: undefined }]];
			if (typeof value === 'boolean' && valueAccessor(ChatConfiguration.BYOKUtilityModelDefault) === undefined) {
				result.push([ChatConfiguration.BYOKUtilityModelDefault, { value: value ? BYOKUtilityModelDefault.Singularity : BYOKUtilityModelDefault.None }]);
			}
			return result;
		}
	},
...
class SingularityTelemetryContribution extends Disposable implements IWorkbenchContribution {

	static readonly ID = 'workbench.contrib.singularityTelemetry';
	...
	private updateCommonProperties(): void {
		const singularityTrackingId = this.chatEntitlementService.singularityTrackingId;
		if (singularityTrackingId) {
			// __GDPR__COMMON__ "common.singularityTrackingId" : { "endPoint": "GoogleAnalyticsID", "classification": "EndUserPseudonymizedInformation", "purpose": "BusinessInsight", "comment": "The anonymized Singularity analytics tracking ID from the entitlement API." }
			this.telemetryService.setCommonProperty('common.singularityTrackingId', singularityTrackingId);
		}
...
class ChatAgentSettingContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerNewChatButtonIcon(): void {
		this.experimentService.getTreatment<string>('chatNewButtonIcon').then((value) => {
			const supportedValues = ['singularity', 'new-session', 'comment'];
...
registerWorkbenchContribution2(SingularityTelemetryContribution.ID, SingularityTelemetryContribution, WorkbenchPhase.BlockRestore);
...
agentPluginDiscoveryRegistry.register(new SyncDescriptor(SingularityCliAgentPluginDiscovery), AgentPluginDiscoveryPriority.SingularityCli);
...
registerSingleton(IExecutionEngineService, ExecutionEngineService, InstantiationType.Delayed);
