...
export class ChatSlashCommandsContribution extends Disposable {
	...
	constructor(
		...
		this._store.add(slashCommandService.registerSlashCommand({
			...
			silent: true,
			locations: [ChatAgentLocation.Chat],
			sessionTypes: [SessionType.Local, SessionType.AgentHostSingularity],
		}, async (_prompt, _progress, _history, _location, sessionResource) => {
			if (getChatSessionType(sessionResource) === SessionType.AgentHostSingularity) {
		...
		if (!this.environmentService.isSessionsWindow) {
			this._store.add(slashCommandService.registerSlashCommand({
				...
				await commandService.executeCommand('singularity.chat.debug.showChatLogView');
			}));
		}
		this._store.add(slashCommandService.registerSlashCommand({
			command: 'agents',
		...
		if (autoApprovePolicyValue !== false) {
			this._store.add(slashCommandService.registerSlashCommand({
				...
				silent: true,
				locations: [ChatAgentLocation.Chat],
				sessionTypes: [SessionType.Local, SessionType.SingularityCLI],
			}, async (_prompt, _progress, _history, _location, sessionResource) => {
