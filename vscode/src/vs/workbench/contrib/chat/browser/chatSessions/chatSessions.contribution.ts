		...
		items: {
			...
			properties: {
				...
				requiresSingularitySignIn: {
					description: localize('chatSessionsExtPoint.requiresSingularitySignIn', 'Whether the chat session relies on a Singularity account and so cannot be used until the user signs in. Defaults to false.'),
					type: 'boolean',
					default: false
				},
...
export class ChatSessionsService extends Disposable implements IChatSessionsService {
	...
	registerChatSessionContribution(contribution: IChatSessionsExtensionPoint): IDisposable {
		...
		// this, types like `agent-host-singularitycli` (registered by the local
		...
		// `requiresSingularitySignIn`) and signal when it changes; re-fire the aggregate
		// availability event so consumers re-evaluate. Generic — no per-provider
		// knowledge lives here.
		if (contribution.onDidChangeRequiresSingularitySignIn) {
			disposables.add(contribution.onDidChangeRequiresSingularitySignIn(() => this._onDidChangeAvailability.fire()));
		}
		this._updateHasCanDelegateProvidersContextKey();
		this._onDidChangeAvailability.fire();
	...
	public requiresSingularitySignInForSessionType(chatSessionType: string): boolean {
		const contribution = this._contributions.get(chatSessionType)?.contribution;
		if (!contribution) {
			return false;
		}
		...
		const requires = contribution.requiresSingularitySignIn;
