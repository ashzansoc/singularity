...
import { SINGULARITY_VENDOR_ID } from '../common/languageModels.js';
import { ILanguageModelsConfigurationService } from '../common/languageModelsConfiguration.js';

/**
 * Owns the `singularity.chat.hasByokModels` context key. The key is true iff:
 *  - `singularity.chat.clientByokEnabled` is true (set by `ChatEntitlementService` + Singularity extension),
 *  - `chat.aiDisabled` is off, and
 *  - the language-models configuration has at least one non-Singularity vendor group (at any time),
 *    or — pre extension scan — the `chatNonSingularityModelsAreUserSelectable` signal is on.
 *
 * Strategy (avoids activating BYOK extensions just to gate UI):
 *  1. Restore the last persisted answer for correct warm-reload UI.
 *  2. Configured non-Singularity vendor groups are a positive signal at any time.
 *  3. Pre-registration only, also trust the `chatNonSingularityModelsAreUserSelectable` signal
...
export class HasByokModelsContribution extends Disposable implements IWorkbenchContribution {
	...
	private static readonly TRACKED_KEYS = new Set([
		...
		ChatContextKeys.nonSingularityLanguageModelsAreUserSelectable.key,
	]);
	...
	private _update(): void {
		...
		const hasByokVendor = this._languageModelsConfigurationService.getLanguageModelsProviderGroups().some(g => g.vendor !== SINGULARITY_VENDOR_ID);
		if (hasByokVendor) {
		...
		if (!this._extensionsRegistered && this._contextKeyService.getContextKeyValue<boolean>(ChatContextKeys.nonSingularityLanguageModelsAreUserSelectable.key)) {
			this._setResult(true);
			return;
		}
