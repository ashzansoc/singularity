...
import { buildLocalSingularityLogsUri, buildRemoteSingularityLogsUri, SINGULARITY_CLI_LOCAL_AH_SCHEME, getSingularityCliSessionRawId, parseRemoteAuthorityFromScheme, resolveEventsUri } from '../singularityCliEventsUri.js';
...
/** Bound the best-effort scan of Singularity SDK process logs. */
export const MAX_SINGULARITY_LOG_SCAN_FILES = 10;
export const MAX_SINGULARITY_LOG_SCAN_FILE_SIZE = 1024 * 1024 * 1024;
const MAX_SINGULARITY_LOG_VIEW_FILE_SIZE = 10 * 1024 * 1024;
...
/**
 * A selected Singularity process log that can be read lazily.
 */
export interface ISingularityLogFile {
...
export const enum AgentHostLogSourceKind {
	/** The Singularity CLI `events.jsonl` model/conversation stream. */
	Events = 'events',
	/** The client-side AHP JSON-RPC wire log (`<logsHome>/ahp/*.jsonl`). */
	WireLog = 'wire',
	/** The Singularity SDK process logs under `<SINGULARITY_HOME>/logs`. */
	CliLog = 'cliLog',
...
export interface IAgentHostLogSource {
	...
	/** Singularity logs directory + session id, for the lazy session-matched CLI log read. */
 ...
 * remote Singularity CLI). Only these sessions have AHP logs and agent-host
...
export function isAgentHostSession(resource: URI | undefined): boolean {
	...
	return resource.scheme === SINGULARITY_CLI_LOCAL_AH_SCHEME || !!parseRemoteAuthorityFromScheme(resource.scheme);
}
...
export async function enumerateAgentHostLogSources(
	...
	const isLocal = sessionResource.scheme === SINGULARITY_CLI_LOCAL_AH_SCHEME;
	...
	// 5. Singularity SDK process logs (<SINGULARITY_HOME>/logs), matched lazily by session id with a newest-log fallback.
	const rawSessionId = getSingularityCliSessionRawId(sessionResource);
	if (rawSessionId) {
		const singularityLogsDir = isLocal
			? buildLocalSingularityLogsUri(userHome)
			: remoteConnection ? buildRemoteSingularityLogsUri(remoteConnection) : undefined;
		if (singularityLogsDir) {
			sources.push({
				id: 'cliLog',
				label: localize('agentHostLogs.cliLog', "Singularity CLI Logs"),
				kind: AgentHostLogSourceKind.CliLog,
				isRemote: !isLocal,
				cliLogs: { dir: singularityLogsDir, rawSessionId },
			});
		}
	}
...
export async function readAgentHostLogSourceContent(
	...
	switch (source.kind) {
		...
		case AgentHostLogSourceKind.CliLog: {
			...
			const files = await readSingularityLogsForSession(source.cliLogs.dir, source.cliLogs.rawSessionId, fileService, logService);
			if (files.length === 0) {
...
/**
 * Reads Singularity process logs selected for a session, or the newest log when
 * none mention the session id. Bounded by
 * {@link MAX_SINGULARITY_LOG_SCAN_FILES} and {@link MAX_SINGULARITY_LOG_SCAN_FILE_SIZE}.
 */
export async function readSingularityLogsForSession(
	logsDir: URI,
	rawSessionId: string,
	fileService: IFileService,
	logService: ILogService,
): Promise<{ path: string; contents: string }[]> {
	const matchingLogs = await findRelevantSingularityLogs(logsDir, rawSessionId, fileService, logService);
	const files: { path: string; contents: string }[] = [];
	for (const log of matchingLogs) {
		try {
			const content = log.size > MAX_SINGULARITY_LOG_VIEW_FILE_SIZE
				? await fileService.readFile(log.resource, { position: log.size - MAX_SINGULARITY_LOG_VIEW_FILE_SIZE, length: MAX_SINGULARITY_LOG_VIEW_FILE_SIZE })
				: await fileService.readFile(log.resource, { limits: { size: MAX_SINGULARITY_LOG_VIEW_FILE_SIZE } });
			files.push({ path: log.path, contents: content.value.toString() });
		} catch (error) {
			logService.warn(`[AgentHostLogSources] Failed to read Singularity log '${log.resource.path}': ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	return files;
}
...
/**
 * Scans a bounded set of Singularity process logs for the session id, falling back
 * to the newest process log when no match can be found.
 */
export async function findRelevantSingularityLogs(
	logsDir: URI,
	rawSessionId: string | undefined,
	fileService: IFileService,
	logService: ILogService,
): Promise<ISingularityLogFile[]> {
	let children: IFileStatWithMetadata[] | undefined;
	try {
	...
	const processLogs = (children ?? [])
		...
		.map(child => ({ path: `singularity-logs/${child.name}`, resource: child.resource, size: child.size }));
	const files: ISingularityLogFile[] = [];
	const candidateLogs = processLogs
		.slice(0, MAX_SINGULARITY_LOG_SCAN_FILES)
		.filter(child => child.size <= MAX_SINGULARITY_LOG_SCAN_FILE_SIZE);
	if (rawSessionId) {
		for (const candidate of candidateLogs) {
			try {
				...
				logService.warn(`[AgentHostLogSources] Failed to scan Singularity log '${candidate.path}': ${error instanceof Error ? error.message : String(error)}`);
			}
		}
	}
...
async function logStreamContains(
	...
	try {
		stream = (await fileService.readFileStream(resource, {
			length: MAX_SINGULARITY_LOG_SCAN_FILE_SIZE,
			limits: { size: MAX_SINGULARITY_LOG_SCAN_FILE_SIZE },
