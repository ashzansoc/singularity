...
export class ChatDebugOverviewView extends Disposable {
	...
	private renderMetricsContent(container: HTMLElement, events: readonly IChatDebugEvent[]): void {
		...
		const totalSingularityUsageNanoAiu = modelTurns.reduce((sum, e) => sum + (e.singularityUsageNanoAiu ?? 0), 0);
		...
		if (totalSingularityUsageNanoAiu > 0) {
			const aic = totalSingularityUsageNanoAiu / NANO_AIU_PER_AIC;
			metrics.push({ label: localize('chatDebug.metric.singularityUsage', "Singularity Usage (AIC)"), value: aicFormatter.value.format(aic) });
		}
