...
import { getSingularityCliSessionRawId } from '../singularityCliEventsUri.js';
 ...
 * folders and never written into the CLI's `~/.singularity` tree.
 */
const USAGE_DIR = 'agentHostUsage';

/**
 * One captured Singularity `ChatUsage` action, i.e. the usage report for a single
...
export interface IAgentHostUsageRecord {
	...
	/** Per-turn cumulative Singularity AIU (nano) at the time of this call. */
...
abstract class AgentHostActionRecorder extends Disposable {
	...
	private _onNotification(notification: INotification): void {
		...
		const rawId = getSingularityCliSessionRawId(URI.parse(notification.session));
		if (!rawId) {
			return; // not a Singularity CLI session
		}
...
/**
 * Captures Singularity `ChatUsage` actions on the client and appends them to a
...
export class AgentHostUsageRecorder extends AgentHostActionRecorder {
	...
	protected _onAction(envelope: ActionEnvelope): void {
		...
		// The backend chat channel encodes a `singularitycli:/<rawId>` session URI;
		// `getSingularityCliSessionRawId` yields the same raw id the reader derives
		// from the client session resource.
		const rawId = getSingularityCliSessionRawId(URI.parse(session));
		if (!rawId) {
			return;
		}
		const record: IAgentHostUsageRecord = {
			...
			totalNanoAiu: meta.singularityUsage?.totalNanoAiu,
 ...
 * the usage sidecar, and never written into the CLI's `~/.singularity` tree.
 */
...
export class AgentHostCustomizationRecorder extends AgentHostActionRecorder {
	...
	protected _onAction(envelope: ActionEnvelope): void {
		...
		const rawId = getSingularityCliSessionRawId(URI.parse(envelope.channel));
		if (!rawId) {
			return; // not a Singularity CLI session channel
		}
