...
import { buildLocalSessionStateUri, SINGULARITY_CLI_EH_SCHEME, SINGULARITY_CLI_LOCAL_AH_SCHEME, getSingularityCliSessionRawId, resolveEventsUri } from '../singularityCliEventsUri.js';
...
/**
 * One record in an Agent Host Singularity CLI `events.jsonl` stream. The CLI
 * writes a line-delimited JSON log of the session under
 * `<SINGULARITY_HOME>/session-state/<id>/events.jsonl`. Every record shares the same
...
/**
 * Feeds Agent Host (Singularity CLI) sessions into the Agent Debug Logs panel by
...
export class AgentHostChatDebugContribution extends Disposable implements IWorkbenchContribution {
	...
	private _ensureLiveRefresh(sessionResource: URI, eventsUri: URI): void {
		...
		// the single `events.jsonl`: the external Singularity CLI process writes
	...
	private _sessionChatSubscription(sessionResource: URI) {
		if (sessionResource.scheme !== SINGULARITY_CLI_LOCAL_AH_SCHEME) {
			return undefined; // live usage only for local Agent Host sessions
		}
		const rawId = getSingularityCliSessionRawId(sessionResource);
		if (!rawId) {
			return undefined;
		}
		const backendSession = URI.from({ scheme: SINGULARITY_CLI_EH_SCHEME, path: `/${rawId}` });
	...
	/**
	 * Reads live Singularity AIU from the AHP session state as a fallback usage
	...
	private async _provideChatDebugLog(sessionResource: URI, token: CancellationToken): Promise<IChatDebugEvent[] | undefined> {
		...
		if (!eventsUri) {
			return undefined; // not an Agent Host Singularity CLI session
		}
		...
		// Singularity AIU from the AHP session state so the usage tile isn't blank.
	 ...
	 *
	 * The Singularity CLI appends to `events.jsonl` line-by-line from a separate
	...
	private async _discoverLocalSessions(token: CancellationToken): Promise<{ uri: URI; title?: string }[]> {
		...
		try {
			...
			return []; // no local Singularity CLI sessions on disk
		}
		if (token.isCancellationRequested) {
			return [];
		}
		...
		const found = await Promise.all(folders.map(async folder => {
			...
			return { uri: URI.from({ scheme: SINGULARITY_CLI_LOCAL_AH_SCHEME, path: `/${folder.name}` }), title };
		}));
 ...
 *
 * The record `parentId` is **not** a logical parent: the Singularity SDK documents
...
export function convertAgentHostEventsToDebugEvents(
	...
	for (const record of records) {
		...
		switch (record.type) {
			case 'session.start': {
				...
				const version = asString(record.data.singularityVersion);
			...
			case 'assistant.message': {
				...
				events.push({
					...
					model, requestName: 'singularitycli', outputTokens, durationInMillis,
				});
				...
				resolved.set(record.id, { kind: 'modelTurn', requestName: 'singularitycli', model, outputTokens, durationInMillis, sections });
				break;
			}
	...
	// input/cache-read tokens and Singularity AIU come from elsewhere:
	...
	const fillTurnsWithTotals = (targets: readonly IModelTurnRef[], totals: ISessionUsageTotals) => {
		...
		for (let i = 0; i < n; i++) {
			...
			const singularityUsageNanoAiu = aiu[i] > 0 ? aiu[i] : undefined;
			events[ref.index] = { ...turn, inputTokens, cachedTokens, totalTokens, singularityUsageNanoAiu };
...
interface IPerTurnUsageCoverage {
	...
	/** Sum of Singularity AIU (nano) assigned from the sidecar. */
...
function applyPerTurnUsage(
	...
	const assign = (ref: typeof modelTurnRefs[number], inputTokens: number | undefined, cachedTokens: number | undefined, singularityUsageNanoAiu: number | undefined) => {
		const turn = events[ref.index] as IChatDebugModelTurnEvent;
		const totalTokens = inputTokens !== undefined ? inputTokens + (ref.outputTokens ?? 0) : undefined;
		events[ref.index] = { ...turn, inputTokens, cachedTokens, totalTokens, singularityUsageNanoAiu };
		const detail = resolved.get(ref.id);
	...
	// Singularity AIU is cumulative per user turn (it resets each turn), so
...
interface ISessionUsageTotals {
	...
	/** Cumulative Singularity AIU (nano). */
...
/**
 * Sums Singularity AIU across a live chat's turns (for in-progress sessions).
 *
 ...
 * only AIU is accumulated per turn (`_turnSingularityUsageTotalNanoAiu`). So the
...
function sumChatStateUsage(chat: ChatState): ISessionUsageTotals | undefined {
	...
	const add = (usage: UsageInfo | undefined) => {
		...
		hasUsage = true;
		totalNanoAiu += readSingularityNanoAiu(usage);
	};
...
/** Reads `_meta.singularityUsage.totalNanoAiu` (per-turn cumulative AIU) from a usage report. */
function readSingularityNanoAiu(usage: UsageInfo): number {
	return readUsageInfoMeta(usage).singularityUsage?.totalNanoAiu ?? 0;
}
...
function fallbackSessionTitle(sessionId: string): string {
	return localize('agentHost.debug.untitledSession', "Singularity CLI Session {0}", sessionId.slice(0, 8));
}
