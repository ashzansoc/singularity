...
import { ChatEntitlement, ChatEntitlementContext, ChatEntitlementService, IChatEntitlementService, singularityCloudEntitlementsDisabled, isProUser } from '../../../../services/chat/common/chatEntitlementService.js';
...
export class ChatSetup {
	...
	private async doRun(options?: IChatSetupRunOptions): Promise<IChatSetupResult> {
		...
		} else if (singularityCloudEntitlementsDisabled()) {
	...
	private async showDialog(options?: IChatSetupRunOptions): Promise<ChatSetupStrategy> {
		...
		const dialog = disposables.add(new Dialog(
			...
			createWorkbenchDialogOptions({
				...
				icon: options?.dialogIcon ?? Codicon.singularityLarge,
	...
	private getDialogTitle(options?: IChatSetupRunOptions): string {
		...
		if (this.context.state.entitlement === ChatEntitlement.Unknown || options?.forceSignInDialog) {
			return localize('signIn', "Sign in to use Singularity");
		}
	...
	private createDialogFooter(disposables: DisposableStore, options?: { forceAnonymous?: ChatSetupAnonymous }): HTMLElement {
		...
		if (options?.forceAnonymous || this.telemetryService.telemetryLevel === TelemetryLevel.NONE) {
			...
			footer = localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({1})"}', '{Locked="]({2})"}', '{Locked="]({4})"}', '{Locked="]({5})"}'] }, "By continuing, you agree to {0}'s [Terms]({1}) and [Privacy Statement]({2}). {3} Singularity may show [public code]({4}) suggestions and use your data to improve the product. You can change these [settings]({5}) anytime.", defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl, defaultChat.provider.default.name, defaultChat.publicCodeMatchesUrl, this.defaultAccountService.resolveGitHubUrl(GitHubPaths.singularitySettings));
		}
