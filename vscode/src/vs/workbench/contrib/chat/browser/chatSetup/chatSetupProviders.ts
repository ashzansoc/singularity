...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatModeKind, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { agent: SetupAgent; disposable: IDisposable } {
		return instantiationService.invokeFunction(accessor => {
			...
			return SetupAgent.doRegisterAgent(instantiationService, chatAgentService, id, `Singularity` /* Must match chatListRenderer SINGULARITY_USERNAME to hide identity */, true, description, location, mode, context, controller);
		});
	}
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpSingularityNeeded', "You need to set up Singularity and be signed in to use Chat."));
	...
	private async forwardRequestToChat(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		try {
			...
			progress({
				kind: 'warning',
				content: new MarkdownString(localize('singularityUnavailableWarning', "Failed to get a response. Please try again."))
			});
		}
	}
	...
	private async doForwardRequestToChatWhenReady(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		...
		if (whenLanguageModelReady instanceof Promise || whenAgentReady instanceof Promise || whenToolsModelReady instanceof Promise) {
			...
			try {
				...
				// Singularity BYOK (TokenRouter): Singularity token failure often surfaces
				// panel "guidance" before gateway models finish seeding. Aborting on
				// that race causes a spurious "Please try again." on the first message
				// after launch; keep waiting for agent/models instead.
				const byokReadyPath = this.chatEntitlementService.hasByokModels
					|| this.chatEntitlementService.clientByokEnabled
					|| this.chatEntitlementService.anonymous;
				const readinessRace: Promise<'timedout' | 'panelGuidance' | PromiseSettledResult<unknown>[]>[] = [
					timeout(this.environmentService.remoteAuthority ? 60000 /* increase for remote scenarios */ : 20000).then(() => 'timedout' as const),
					allReady,
				];
				if (!byokReadyPath) {
					readinessRace.splice(1, 0, this.whenPanelAgentHasGuidance(disposables).then(() => 'panelGuidance' as const));
				}
				const ready = await Promise.race(readinessRace);
					...
					if (defaultChat.outputChannelId && this.outputService.getChannelDescriptor(defaultChat.outputChannelId)) {
						progress({
							...
							command: {
								...
								title: localize('showSingularityChatDetails', "Show Details")
							}
						});
					} else {
	...
	private whenToolsModelReady(languageModelToolsService: ILanguageModelToolsService, requestModel: IChatRequestModel): Promise<unknown> | void {
		...
		for (const tool of languageModelToolsService.getAllToolsIncludingDisabled()) {
			if (tool.id.startsWith('singularity_')) {
...
export class ChatCodeActionsProvider {
	...
	async provideCodeActions(model: ITextModel, range: Range | Selection): Promise<CodeActionList | undefined> {
		...
		if (generateOrModifyTitle && generateOrModifyCommand) {
			actions.push({
				kind: CodeActionKind.RefactorRewrite.append('singularity').value,
				isAI: true,
		...
		if (markers.length > 0) {
			...
			actions.push({
				kind: CodeActionKind.QuickFix.append('singularity').value,
				isAI: true,
				diagnostics: markers,
			...
			actions.push({
				kind: CodeActionKind.QuickFix.append('explain').append('singularity').value,
				isAI: true,
				diagnostics: markers,
