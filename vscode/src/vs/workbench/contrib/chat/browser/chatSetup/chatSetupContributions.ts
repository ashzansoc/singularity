...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerActions(context: ChatEntitlementContext, requests: ChatEntitlementRequests, controller: Lazy<ChatSetupController>): void {
		...
		class ChatSetupTriggerAction extends Action2 {
			...
			static CHAT_SETUP_ACTION_LABEL = localize2('triggerChatSetup', "Use AI Features with Singularity for free...");
		...
		class ChatSetupTriggerForceSignInDialogAction extends Action2 {
			...
			constructor() {
				super({
					...
					title: localize2('forceSignIn', "Sign in to use Singularity")
				});
			}
		...
		class ToggleSignInTitleBarAction extends ToggleTitleBarConfigAction {
			constructor() {
				super(
					...
					localize('toggle.chatSignIn', 'Singularity Sign In'),
					localize('toggle.chatSignInDescription', "Toggle visibility of the Singularity Sign In button in title bar"),
					3,
					ContextKeyExpr.and(
						IsWebContext.negate(),
						ChatContextKeys.Entitlement.signedOut,
		...
		class UpgradePlanAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('managePlan', "Upgrade Singularity"),
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const baseUrl = defaultAccountService.resolveGitHubUrl(GitHubPaths.singularityUpgrade);
		...
		registerGenerateCodeCommand('chat.internal.explain', 'singularity.chat.chat.explain');
		registerGenerateCodeCommand('chat.internal.fix', 'singularity.chat.chat.fix');
		registerGenerateCodeCommand('chat.internal.review', 'singularity.chat.chat.review');
...
class ChatSetupExtensionUrlHandler implements IExtensionUrlHandlerOverride {
	...
	private static readonly UPGRADE_SUCCESS_NOTIFICATION_ID = 'singularity.upgradeSuccess';
