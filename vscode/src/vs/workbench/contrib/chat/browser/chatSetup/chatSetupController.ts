...
import { ChatEntitlement, ChatEntitlementContext, ChatEntitlementRequests, singularityCloudEntitlementsDisabled, isProUser } from '../../../../services/chat/common/chatEntitlementService.js';
...
export class ChatSetupController extends Disposable {
	...
	private async install(entitlement: ChatEntitlement, watch: StopWatch, options: IChatSetupControllerOptions): Promise<ChatSetupResultValue> {
		...
		try {
			if (
				!singularityCloudEntitlementsDisabled() &&
				!options.forceAnonymous &&						// User is not asking for anonymous access
				entitlement !== ChatEntitlement.Free &&			// User is not signed up to Singularity Free
