 ...
 * paragraph breaks that the streaming ASR model omits. Requires Singularity/AI to be
...
const LLM_CLEANUP_MODEL_SELECTOR = { vendor: 'singularity', id: 'singularity-utility-small' };
...
export interface IChatSpeechToTextService {
	...
	/**
	 * Whether speech-to-text is available for the current Singularity entitlement,
	...
	private async _start(window: Window & typeof globalThis, surface: ChatDictationSurface, generation: number): Promise<void> {
		...
		if (!this._isEntitledForBackend(backend)) {
			this._notificationService.warn(backend === 'mai' && this._chatEntitlementService.entitlement === ChatEntitlement.Enterprise
				? localize('chatStt.maiEnterpriseUnavailable', "Cloud speech-to-text is not available for Singularity Enterprise accounts.")
				: localize('chatStt.requiresPaidPlan', "Dictation requires a paid Singularity plan."));
			return;
		}
