...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const mainMessage = localize('chatFailErrorMessage', "Chat failed to load because the installed version of the Singularity AI extension is not compatible with this version of {0}. Please ensure that the Singularity AI extension is up to date.", this.productService.nameLong);
		...
		const versionMessage = `Singularity AI version: ${chatExtension.version}`;
