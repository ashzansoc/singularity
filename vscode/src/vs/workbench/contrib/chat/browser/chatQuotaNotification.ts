...
import { getSelectedModelIdentifier, getSelectedModelMetadata, isSelectedModelSingularity, SELECTED_MODEL_STORAGE_KEY_PREFIX, SELECTED_MODEL_STORAGE_SCOPE } from '../common/chatSelectedModel.js';
...
const QUOTA_NOTIFICATION_ID = 'singularity.quotaStatus';
...
export class ChatQuotaNotificationContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		// Re-evaluate when the selected model changes (e.g. switching between Singularity and BYOK).
	...
	private _update(): void {
		const entitlement = this._chatEntitlementService.entitlement;
		const isSingularity = this._isSingularityModelSelected();
		...
		// time quota runs out. Done before the Singularity/BYOK gate so a recovery is
		...
		// selection hasn't loaded yet — quota only applies to Singularity models.
	...
	/**
	 * Returns `true` only when a Singularity model is actively selected.
	 * Returns `false` if no model is selected yet (widget not initialized)
	 * or if the selected model is from a non-Singularity vendor (BYOK).
	 */
	private _isSingularityModelSelected(): boolean {
		return isSelectedModelSingularity(this._contextKeyService, this._storageService, this._languageModelsService);
	}
	...
	private _refreshActiveQuotaApproachingWarning(): void {
		...
		if (!warning || !this._isSingularityModelSelected()) {
			return;
		}
	...
	private _showManagedPlanBlockedNotification(): void {
		...
		this._setNotification({
			...
			description: localize('quota.blocked.managed', "Your organization or enterprise has exceeded its Singularity budget. Contact your admin to resume usage."),
			actions: [],
			dismissible: true,
			autoDismissOnMessage: true,
		});
