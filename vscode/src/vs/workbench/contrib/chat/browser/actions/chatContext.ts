...
class SessionReferenceContextPickerPick implements IChatContextPickerItem {
	...
	asPicker(widget: IChatWidget): IChatContextPicker {
		...
		const onlyShowAttachableSingularityCliSessions = !!currentSessionResource && isAgentHostTarget(getChatSessionType(currentSessionResource));
		return {
			placeholder: localize('chatContext.sessions.placeholder', 'Select a session'),
			picks: (async () => {
				const picks: IChatContextPickerPickItem[] = [];
				const sessionProviderFilter = [AgentSessionProviders.Local, AgentSessionProviders.Background, AgentSessionProviders.Claude, AgentSessionProviders.AgentHostSingularity];
				for await (const group of this._chatSessionsService.getChatSessionItems(sessionProviderFilter, CancellationToken.None)) {
					...
					for (const item of group.items) {
						...
						if (onlyShowAttachableSingularityCliSessions && !this._canAttachSingularityCliSession(sessionResource)) {
							continue;
						}
	...
	private _canAttachSingularityCliSession(sessionResource: URI): boolean {
		// For now, attachments while in an Agent Host Singularity harness are attachable when backed by Singularity CLI events.jsonl.
