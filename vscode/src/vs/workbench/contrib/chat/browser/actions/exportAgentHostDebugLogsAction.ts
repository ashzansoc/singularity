...
import { findRelevantSingularityLogs, getRemoteConnectionForSession, readRemoteAgentHostLog, sanitizeFilePart } from '../chatDebug/agentHostLogSources.js';
...
const MAX_REMOTE_SINGULARITY_LOG_EXPORT_SIZE = 10 * 1024 * 1024;
 ...
 * Singularity CLI session events file (if available), the window/shared/local
...
export async function collectAgentHostDebugLogs(
	...
	// 5. Singularity SDK process logs under <SINGULARITY_HOME>/logs.
	const rawSessionId = getSingularityCliSessionRawId(activeSession?.resource);
	const singularityLogsDir = activeSession
		? rawSessionId
	...
	if (singularityLogsDir) {
		const singularityLogFiles = await findRelevantSingularityLogs(singularityLogsDir, rawSessionId, fileService, logService);
		for (const file of singularityLogFiles) {
			try {
				files.push(await createDebugLogFile(file.path, file.resource, fileService, file.size, MAX_REMOTE_SINGULARITY_LOG_EXPORT_SIZE));
			} catch (error) {
				logService.warn(`[ExportAgentHostDebugLogs] Failed to read Singularity log '${file.path}': ${error instanceof Error ? error.message : String(error)}`);
			}
		}
	}
 ...
 * find the active Singularity CLI chat session. Suitable for vscode where the
