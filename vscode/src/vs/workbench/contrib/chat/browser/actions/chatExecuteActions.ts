...
/**
 * Singularity Cobuild — multi-user GPU pods (create / join / VRAM pool).
 * Sits directly next to the Auto model picker in the chat input footer.
 */
export class OpenCobuildAction extends Action2 {
	static readonly ID = 'workbench.action.chat.openCobuild';

	constructor() {
		super({
			id: OpenCobuildAction.ID,
			title: localize2('interactive.openCobuild.label', "Cobuild"),
			tooltip: localize('interactive.openCobuild.tooltip', "Singularity Cobuild — pool GPUs with friends"),
			category: CHAT_CATEGORY,
			f1: true,
			precondition: ChatContextKeys.enabled,
			menu: {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService);
		try {
			await commandService.executeCommand('singularity.ai.cobuild');
		} catch (err) {
			const log = accessor.get(ILogService);
			log.error('[Cobuild] singularity.ai.cobuild failed', err);
		}
	}
}

/**
 * Toggle Visual Critic loop in the chat input.
 * Off: skill.md → design-spec.json + one implement pass, then stop.
 * On: Visual Critic stop-hook keeps refining until pass / max iterations.
 */
export class ToggleVisualCriticAction extends Action2 {
	static readonly ID = 'workbench.action.chat.toggleVisualCritic';

	constructor() {
		super({
			id: ToggleVisualCriticAction.ID,
			title: localize2('interactive.visualCritic.label', "Critic"),
			tooltip: localize('interactive.visualCritic.tooltip', "Critic — iterate with Visual Critic until design scores pass"),
			category: CHAT_CATEGORY,
			f1: true,
			precondition: ChatContextKeys.enabled,
			toggled: {
				condition: ContextKeyExpr.equals(`config.${ChatConfiguration.VisualCriticEnabled}`, true),
				tooltip: localize('interactive.visualCritic.onTooltip', "Critic on — refining until scores pass (click to turn off)"),
			},
			menu: {
				id: MenuId.ChatInput,
				order: 3.6,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		const configurationService = accessor.get(IConfigurationService);
		const current = configurationService.getValue<boolean>(ChatConfiguration.VisualCriticEnabled) === true;
		await configurationService.updateValue(ChatConfiguration.VisualCriticEnabled, !current);
	}
}

/**
 * Toggle dependency-aware multi-subagent execution in the chat input.
 * Off: standard single-agent Agent flow (default).
 * On: planner builds a DAG and runs parallel subagents via RunSubagent.
 */
export class ToggleMultiSubagentExecutionAction extends Action2 {
	static readonly ID = 'workbench.action.chat.toggleMultiSubagentExecution';

	constructor() {
		super({
			id: ToggleMultiSubagentExecutionAction.ID,
			title: localize2('interactive.multiSubagent.label', "Multi-agent"),
			tooltip: localize('interactive.multiSubagent.tooltip', "Multi-agent — run dependency-aware parallel subagents for complex tasks"),
			category: CHAT_CATEGORY,
			f1: true,
			precondition: ChatContextKeys.enabled,
			toggled: {
				condition: ContextKeyExpr.equals(`config.${ChatConfiguration.ExecutionEnabled}`, true),
				tooltip: localize('interactive.multiSubagent.onTooltip', "Multi-agent on — parallel subagent execution (click to turn off)"),
			},
			menu: {
				id: MenuId.ChatInput,
				order: 3.65,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		const configurationService = accessor.get(IConfigurationService);
		const current = configurationService.getValue<boolean>(ChatConfiguration.ExecutionEnabled) === true;
		await configurationService.updateValue(ChatConfiguration.ExecutionEnabled, !current);
	}
}
		...
		super({
			...
			menu: [
				...
				},
				{
					...
					// In the Agents window only, hide the worktree/branch pickers for Singularity
...
export function registerChatExecuteActions(): DisposableStore {
	...
	store.add(registerAction2(OpenCobuildAction));
	store.add(registerAction2(ToggleVisualCriticAction));
	store.add(registerAction2(ToggleMultiSubagentExecutionAction));
