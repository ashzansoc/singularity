...
import { resolveEventsUri } from '../singularityCliEventsUri.js';

/**
 * Shared implementation of "Open Singularity CLI State File". Resolves the
 ...
 * this helper after resolving the active Singularity CLI session resource.
 */
export async function openSingularityCliStateFile(
	...
	switch (result.kind) {
		case 'ok':
			...
			return;
		case 'no-session':
			notificationService.info(localize('openSessionEventsFile.noSession', "No Singularity CLI session is active."));
			return;
		case 'unsupported-scheme':
			notificationService.info(localize('openSessionEventsFile.unsupported', "The active chat session is not a Singularity CLI session."));
			return;
 ...
 */
export class OpenSingularityCliStateFileAction extends Action2 {

	static readonly ID = 'workbench.action.chat.openSingularityCliStateFile';

	constructor() {
		super({
			id: OpenSingularityCliStateFileAction.ID,
			title: localize2('openSessionEventsFile', "Open Singularity CLI State File"),
			f1: true,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		await openSingularityCliStateFile(accessor, sessionResource);
	}
}
