...
import { markPreferredSingularityHarness } from '../../common/chatSessionTypePreference.js';
import { ILanguageModelChatSelector, ILanguageModelsService } from '../../common/languageModels.js';
import { SingularityUsageExtensionFeatureId } from '../../common/languageModelStats.js';
...
const SINGULARITY_CLI_AGENT_HOST_PROVIDER_ID = 'singularitycli';
...
export function registerChatActions() {
		...
		constructor() {
			super({
				...
				menu: [{
					...
					group: 'navigation',
					when: ContextKeyExpr.and(ActiveEditorContext.isEqualTo(ChatEditorInput.EditorID), ChatContextKeys.newChatButtonExperimentIcon.notEqualsTo('singularity'), ChatContextKeys.newChatButtonExperimentIcon.notEqualsTo('new-session'), ChatContextKeys.newChatButtonExperimentIcon.notEqualsTo('comment')),
					order: 1
				}],
			});
		}
	...
	registerAction2(class NewChatEditorSingularityIconAction extends Action2 {
		constructor() {
			super({
				id: ACTION_ID_OPEN_CHAT + '.singularityIcon',
				title: localize2('interactiveSession.open', "New Chat Editor"),
				...
				menu: [{
					id: MenuId.EditorTitle,
					group: 'navigation',
					when: ContextKeyExpr.and(ActiveEditorContext.isEqualTo(ChatEditorInput.EditorID), ChatContextKeys.newChatButtonExperimentIcon.isEqualTo('singularity')),
					order: 1
				}],
			});
		}
			...
			super({
				...
				menu: {
					...
					when: ContextKeyExpr.and(
						...
						ChatContextKeys.chatAgentHostProviderId.isEqualTo(SINGULARITY_CLI_AGENT_HOST_PROVIDER_ID)
					),
				},
			});
		}
	...
	const nonEnterpriseSingularityUsers = ContextKeyExpr.and(ChatContextKeys.enabled, ContextKeyExpr.notEquals(`config.${defaultChat.completionsAdvancedSetting}.authProvider`, defaultChat.provider.enterprise.id));
	registerAction2(class extends Action2 {
		constructor() {
			super({
				id: 'workbench.action.chat.manageSettings',
				title: localize2('manageChat', "Manage Singularity Settings"),
				category: CHAT_CATEGORY,
				f1: true,
				precondition: ContextKeyExpr.and(
					ContextKeyExpr.or(
					...
					nonEnterpriseSingularityUsers
				),
				menu: {
					id: MenuId.ChatTitleBarMenu,
					group: 'y_manage',
					order: 1,
					when: nonEnterpriseSingularityUsers
				}
			});
		}
		...
		override async run(accessor: ServicesAccessor): Promise<void> {
			...
			openerService.open(URI.parse(defaultAccountService.resolveGitHubUrl(GitHubPaths.singularitySettings)));
		}
	});

	registerAction2(class ShowExtensionsUsingSingularity extends Action2 {

		constructor() {
			super({
				id: 'workbench.action.chat.showExtensionsUsingSingularity',
				title: localize2('showSingularityUsageExtensions', "Show Extensions using Singularity"),
				f1: true,
		...
		override async run(accessor: ServicesAccessor): Promise<void> {
			...
			extensionsWorkbenchService.openSearch(`@contribute:${SingularityUsageExtensionFeatureId}`);
		}
	});

	registerAction2(class ConfigureSingularityCompletions extends Action2 {
	...
	registerAction2(class ShowQuotaExceededDialogAction extends Action2 {
		...
		constructor() {
			super({
				...
				title: localize('upgradeChat', "Upgrade Singularity Plan")
			});
		}
		...
		override async run(accessor: ServicesAccessor) {
			...
			const upgradeToPro = free ? localize('upgradeToPro', "Upgrade to Singularity Pro for:\n- Unlimited inline suggestions\n- Unlimited chat messages\n- Access to premium models") : undefined;

			await dialogService.prompt({
				type: 'none',
				message: localize('singularityQuotaReached', "Singularity Quota Reached"),
				cancelButton: {
				...
				buttons: [
					{
						label: free ? localize('upgradePro', "Upgrade to Singularity Pro") : localize('upgradePlan', "Upgrade Singularity Plan"),
						run: () => {
				...
				custom: {
					icon: Codicon.singularityWarningLarge,
...
export async function clearChatSessionPreservingType(accessor: ServicesAccessor, widget: IChatWidget, sessionType: string | undefined): Promise<void> {
	...
	const { sessionType: newSessionType, isPreferSingularityHarnessSwap } = resolveDefaultNewChatSessionType(accessor, { explicitOverride: sessionType, currentSessionType });
	if (isIChatViewViewContext(widget.viewContext)) {
		...
		if (newSessionType !== localChatSessionType) {
			...
			if (isPreferSingularityHarnessSwap) {
