...
import { SINGULARITY_CONFIG_FOLDER, DICTATION_INSTRUCTIONS_FILENAME, GITHUB_CONFIG_FOLDER, VOICE_INSTRUCTIONS_FILENAME } from '../../common/promptSyntax/config/promptFileLocations.js';
...
class ConfigureSpeechInstructionsAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const items: IVoiceInstructionsQuickPickItem[] = [{
			...
			description: `~/.singularity/${this.fileName}`,
			detail: localize('userSpeechInstructionsDetail', "Applies across all workspaces."),
			resource: joinPath(userHome, SINGULARITY_CONFIG_FOLDER, this.fileName),
		}];
