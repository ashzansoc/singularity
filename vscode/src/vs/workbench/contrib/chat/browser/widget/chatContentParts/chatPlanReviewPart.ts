...
import { PLAN_EDITOR_ID, parsePlanDocument } from '../../../common/plan/planDocument.js';
import { IPlanTodoSyncService } from '../../../common/plan/planTodoSyncService.js';
import { ChatPlanDocumentRenderer } from './chatPlanDocumentRenderer.js';
import './media/chatPlanReview.css';

const PLAN_EDITOR_OVERRIDE = PLAN_EDITOR_ID;
...
export class ChatPlanReviewPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		@IPlanTodoSyncService private readonly _planTodoSyncService: IPlanTodoSyncService,
	) {
		super();
		...
		if (review.planUri) {
			void this._planTodoSyncService.syncPlanFileToChatTodos(
				context.element.sessionResource,
				URI.revive(review.planUri),
			);
		} else if (review.content) {
			const doc = parsePlanDocument(review.content);
			if (doc.todos.length > 0) {
				this._planTodoSyncService.syncPlanToChatTodos(context.element.sessionResource, doc);
			}
		}
	...
	private renderMarkdown(): void {
		...
		const doc = parsePlanDocument(this.review.content);
		const usePlanChrome = doc.hasFrontmatter || doc.format === 'todoMd' || doc.todos.length > 0;

		if (usePlanChrome) {
			// Prefer structured plan chrome; keep review title bar for approve/feedback.
			if (doc.name && !this.review.title) {
				// no-op: review.title is the widget chrome title
			}
			const titleLabel = this.domNode.querySelector('.chat-plan-review-title-label');
			if (titleLabel && doc.name) {
				titleLabel.textContent = doc.name;
			}
			const renderer = store.add(this._instantiationService.createInstance(ChatPlanDocumentRenderer, doc, {
				interactiveTodos: false,
				scrollableBody: false,
			}));
			store.add(renderer.onDidChangeHeight(() => {
				this._messageScrollable.scanDomNode();
				this._onDidChangeHeight.fire();
			}));
			this._messageEl.append(renderer.domNode);
		} else {
	...
	private async revealInlineComment(item: IPlanReviewFeedbackItem): Promise<void> {
		...
		await this._editorService.openEditor({
			...
			options: {
				pinned: true,
				...(isEqual(item.resource, planUri) ? { override: PLAN_EDITOR_OVERRIDE } : {}),
		...
		if (!this.review.canProvideFeedback || this._isSubmitted) {
			if (this.review.planUri) {
				await this._editorService.openEditor({
					...
					options: { pinned: true, override: PLAN_EDITOR_OVERRIDE },
				});
			}
			return;
		}
