...
export class SessionTypePickerActionItem extends ChatInputPickerActionViewItem {
	...
	constructor(
		...
		this._register(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(ChatConfiguration.EditorPreferSingularityHarness) ||
				e.affectsConfiguration(ChatConfiguration.DefaultToSingularityHarness) ||
				e.affectsConfiguration(ChatConfiguration.EditorLocalAgentEnabled) ||
				e.affectsConfiguration(ChatConfiguration.SingularityCliHideExtensionHostEditor)) {
				this._updateAgentSessionItems();
				if (this.element) {
	 ...
	 * Defaults to Agent Host Singularity when the agent host is enabled, otherwise
