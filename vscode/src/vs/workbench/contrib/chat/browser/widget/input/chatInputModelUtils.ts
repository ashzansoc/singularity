 ...
 * different pool in either direction — e.g. a `singularity/*` model leaking into an agent-host
 ...
 * (e.g. `singularity/claude-sonnet-4.6` → `agent-host-singularitycli:claude-sonnet-4.6`).
 ...
 * - Singularity is the exception: its models are gated on an async token that can resolve slower than fast/local BYOK
 *   providers, so an early empty resolution is transient. Keeping its cache avoids resetting (and persisting) a
 *   restored Singularity selection to a BYOK default, which also preserves the selection across sign-out/in (see #321037).
