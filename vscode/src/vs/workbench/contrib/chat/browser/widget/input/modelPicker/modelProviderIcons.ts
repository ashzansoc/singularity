...
const singularityModelProviderIcon = registerIcon('chat-model-provider-singularity', Codicon.singularityCompact, localize('chatModelProviderSingularityIcon', "Icon for Singularity models."));
...
export function getModelProviderIcon(model: ILanguageModelChatMetadataAndIdentifier): ThemeIcon {
	...
	if (isAutoLanguageModel(model)) {
		return singularityModelProviderIcon;
	}
	...
	if (modelIdentity.includes('singularity')) {
		return singularityModelProviderIcon;
	}
	return genericModelProviderIcon;
}
