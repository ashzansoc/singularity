...
export class ModelPickerWidget extends Disposable {
	...
	show(anchor?: HTMLElement): void {
		...
		const onSelect = (model: ILanguageModelChatMetadataAndIdentifier) => {
			this._telemetryService.publicLog2<ChatModelChangeEvent, ChatModelChangeClassification>('chat.modelChange', {
				fromModel: previousModel?.metadata.vendor === 'singularity' ? new TelemetryTrustedValue(previousModel.identifier) : 'unknown',
				toModel: model.metadata.vendor === 'singularity' ? new TelemetryTrustedValue(model.identifier) : 'unknown',
		...
		const manageSettingsUrl = this._defaultAccountService.resolveGitHubUrl(GitHubPaths.singularitySettings);
	...
	private _renderLabel(): void {
		...
		const ariaLabel = restrictedMode
			...
			: setupRequired
				? localize('chat.modelPicker.ariaLabelSetupRequired', "Models, sign in to use Singularity")
