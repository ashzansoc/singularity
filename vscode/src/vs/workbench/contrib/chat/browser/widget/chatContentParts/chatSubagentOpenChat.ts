...
export function shouldShowSubagentModel(subagentModelName?: string, parentModelId?: string, parentModelName?: string, parentModelMetadataId?: string): boolean {
	if (!subagentModelName) {
		return false;
	}
	const normalize = (value: string | undefined) => value?.trim().toLowerCase();
	const sub = normalize(subagentModelName);
	if (!sub) {
		return false;
	}
	const parents = [parentModelId, parentModelName, parentModelMetadataId]
		.map(normalize)
		.filter((value): value is string => !!value);
	// Show when the subagent model differs from the parent Auto/selected model.
	return !parents.some((parent) => parent === sub || parent.endsWith(`/${sub}`) || sub.endsWith(`/${parent}`) || parent.includes(sub) || sub.includes(parent));
}
