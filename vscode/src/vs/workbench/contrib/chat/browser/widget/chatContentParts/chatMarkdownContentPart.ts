...
export class CollapsedCodeBlock extends ChatEditPillElement {
	...
	render(uri: URI): void {
		...
		this.progressStore.add(autorun(r => {
			if (isStreaming.read(r)) {
				...
				this.setStatus(codicon, localize('chat.codeblock.editing', 'Editing'));
				const entry = editSession.readEntry(uri, r);
		...
		// Render the +/- diff and Cursor-style mini snippet card
		this.progressStore.add(autorunSelfDisposable(r => {
			...
			if (changes && !changes?.identical && !changes?.quitEarly) {
				...
				void this.updateDiffSnippet(changes.originalURI, changes.modifiedURI);
	...
	private async updateDiffSnippet(originalURI: URI, modifiedURI: URI): Promise<void> {
		const [original, modified] = await Promise.all([
			readResourceText(this.textModelService, originalURI),
			readResourceText(this.textModelService, modifiedURI),
		]);
		if (this._store.isDisposed) {
			return;
		}
		if (original === undefined || modified === undefined) {
			this.setDiffSnippet(undefined);
			return;
		}
		const preview = computeDiffSnippetPreview(original, modified);
		this.setDiffSnippet(preview.lines.length ? preview.lines : undefined);
	}
}
