...
import { ChatEditPillElement, isResourceContentEmpty, readResourceText } from './chatEditPillElement.js';
import { computeDiffSnippetPreview } from './chatEditDiffSnippet.js';
...
export class ChatExternalEditContentPart extends ChatEditPillElement implements IChatContentPart {
	...
	constructor(
		...
		// Always try to load a snippet for create/edit/delete when content URIs exist,
		// even if the producer omitted pre-computed +/- counts.
		if (!(edit.diff && (edit.diff.added > 0 || edit.diff.removed > 0))) {
			void this.updateDiffSnippet(edit);
		}
	}
	...
	private async updateDiffSnippet(edit: IChatExternalEdit): Promise<void> {
		if (edit.afterContentUri && (edit.beforeContentUri || edit.editKind === 'create')) {
			const [original, modified] = await Promise.all([
				edit.beforeContentUri
					? readResourceText(this.textModelService, edit.beforeContentUri)
					: Promise.resolve(''),
				readResourceText(this.textModelService, edit.afterContentUri),
			]);
			if (this._store.isDisposed) {
				return;
			}
			...
			const preview = computeDiffSnippetPreview(original, modified);
			if (preview.lines.length) {
				this.setDiffSnippet(preview.lines);
			}
			return;
		}

		if (edit.editKind === 'delete' && edit.beforeContentUri) {
			const original = await readResourceText(this.textModelService, edit.beforeContentUri);
			if (this._store.isDisposed || original === undefined) {
				return;
			}
			const preview = computeDiffSnippetPreview(original, '');
			if (preview.lines.length) {
				this.setDiffSnippet(preview.lines);
			}
		}
