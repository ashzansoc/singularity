...
import { ILanguageModelChatMetadata, ILanguageModelChatMetadataAndIdentifier, ILanguageModelsService, isAutoLanguageModel, isCobuildLanguageModel } from '../../../common/languageModels.js';
...
import { SingularityChatTelemetryWidget } from './singularityChatTelemetryWidget.js';
...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	private singularityTelemetryWidget?: SingularityChatTelemetryWidget;
	...
	constructor(
		...
		const updateAfterModelListChange = (reconcileSelection: boolean) => {
			...
			if (canLog(this.logService.getLevel(), LogLevel.Debug)) {
				...
				if (this.getCurrentSessionType() !== SessionType.SingularityCLI) {
					const delegateSessionType = this.options.sessionTypePickerDelegate?.getActiveSessionProvider?.();
	...
	public switchModelByQualifiedName(qualifiedModelNames: readonly string[]): boolean {
		...
		this.logService.warn(`[chat] Node of the models "${qualifiedModelNames.join(', ')}" not found. Use format "<name> (<vendor>)", e.g. "GPT-4o (singularity)".`);
		return false;
	}
	...
	private _getModelPickerPresentationOptions(): IModelPickerPresentationOptions {
		...
		return {
			...
			// Singularity chat exposes only Auto — hide manage/featured surfaces that list other models.
	...
	*/
	 ...
	 * Singularity Free / Student user. In this state there is no model to send a
	...
	private getModelsForSessionType(sessionType: string | undefined): ILanguageModelChatMetadataAndIdentifier[] {
		...
		// Singularity chat offers Auto plus active Cobuild pod models.
		return sessionFiltered
			.filter(m => !isModelHiddenInPicker(m, id => this.languageModelsService.isModelHidden(id)))
			.filter(m => isAutoLanguageModel(m) || isCobuildLanguageModel(m));
	}
	...
	private preselectModelFromSessionHistory(): void {
		...
		if (!requests || requests.length === 0 || getChatSessionType(sessionResource) !== SessionType.SingularityCLI) {
			return;
		}
	...
	private ensureNotificationWidget(): void {
		if (!this._notificationWidget.value) {
			...
			// (e.g. empty workspace + Singularity CLI [Agent Host] selected). Without
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
			...
			elements = dom.h('.interactive-input-part', [
				...
				dom.h('.interactive-input-and-edit-session', [
					...
					dom.h('.chat-secondary-toolbar@secondaryToolbar', [
						dom.h('.chat-singularity-telemetry@telemetryStrip'),
		...
		this.singularityTelemetryWidget = this._register(this.instantiationService.createInstance(SingularityChatTelemetryWidget));
		elements.telemetryStrip.appendChild(this.singularityTelemetryWidget.domNode);
		...
		toolbarsContainer.classList.add('chat-input-footer');
		...
		store.add(autorun(reader => {
			...
			reader.store.add(scopedInstantiationService.createInstance(MenuWorkbenchButtonBar, actionsContainer, isSessionMenu ? MenuId.ChatEditingSessionChangesToolbar : MenuId.ChatEditingWidgetToolbar, {
				...
				buttonConfigProvider: (action) => {
					...
					if (action.id === 'singularity.chat.chat.cloudSessions.openPullRequestForTask') {
