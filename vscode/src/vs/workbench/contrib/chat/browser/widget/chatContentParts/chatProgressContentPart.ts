...
/** Matches “Context Engine called” and “Context Engine — called” progress lines. */
const ENGINE_CALLED_PROGRESS = /\bEngine(?:\s+[—–-]\s+|\s+)called\b/i;

export function isEngineCalledProgressMessage(content: IMarkdownString | string): boolean {
	const text = typeof content === 'string' ? content : content.value;
	return ENGINE_CALLED_PROGRESS.test(text);
}
...
export class ChatProgressContentPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		this.markEngineCalled(result.element, progress.content);
		if (useShimmer) {
	...
	updateMessage(content: IMarkdownString): void {
		...
		renderFileWidgets(result.element, this.instantiationService, this.chatMarkdownAnchorService, this._fileWidgetStore);
		this.markEngineCalled(result.element, content);
	...
	private markEngineCalled(messageElement: HTMLElement, content: IMarkdownString): void {
		const isEngine = isEngineCalledProgressMessage(content);
		messageElement.classList.toggle('singularity-engine-called', isEngine);
		this.domNode.classList.toggle('singularity-engine-called', isEngine);
	}
}
...
export class ChatWorkingProgressContentPart extends ChatProgressContentPart implements IChatContentPart {
	...
	constructor(
		...
		// Prefer an explicit label, then the latest progressMessage from the response
		// model (e.g. "Runtime Engine — planning…"), then a rotating placeholder.
		const fromResponseProgress = isResponseVM(context.element)
			? context.element.progressMessages.at(-1)?.content
			: undefined;
		const explicitContent = workingProgress.content ?? fromResponseProgress;
