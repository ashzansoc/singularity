...
const builtinDefaultIcon = (mode: IChatMode) => {
	switch (mode.name.get().toLowerCase()) {
		...
		case 'debug': return Codicon.debugAlt;
		case 'review': return Codicon.checklist;
		case 'multitask': return Codicon.layers;
	...
	constructor(
		...
		const actionProvider: IActionWidgetDropdownActionProvider = {
			getActions: () => {
				...
				customBuiltinModeActions.sort((a, b) => compareModeLabels(a.label, b.label));

				const customModeActions = customModes.custom?.map(mode => makeActionFromCustomMode(mode, currentMode)) ?? [];
				customModeActions.sort((a, b) => compareModeLabels(a.label, b.label));
...
/** Preferred Singularity mode order in the chat mode picker (after Agent). */
const MODE_PICKER_ORDER: readonly string[] = [
	'plan',
	'debug',
	'multitask',
	'ask',
...
function compareModeLabels(a: string, b: string): number {
	const ai = MODE_PICKER_ORDER.indexOf(a.toLowerCase());
	const bi = MODE_PICKER_ORDER.indexOf(b.toLowerCase());
	if (ai !== -1 || bi !== -1) {
