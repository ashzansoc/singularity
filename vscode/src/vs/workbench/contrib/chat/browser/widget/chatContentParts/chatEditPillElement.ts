...
import type { DiffSnippetLine } from './chatEditDiffSnippet.js';
 ...
 *
 * When a line-level snippet is available, a Cursor-style card is also shown
 * under the status row: filename header, +N/-N, and red/green preview lines.
 */
export class ChatEditPillElement extends Disposable {
	...
	private readonly _snippetCard: HTMLElement;
	private readonly _snippetHeaderIcon: HTMLElement;
	private readonly _snippetHeaderName: HTMLElement;
	private readonly _snippetHeaderAdded: HTMLElement;
	private readonly _snippetHeaderRemoved: HTMLElement;
	private readonly _snippetBody: HTMLElement;
	...
	private _snippetIconClasses: string[] = [];
	...
	constructor(
		...
		this._snippetCard = $('div.chat-edit-snippet-card');
		this._snippetCard.style.display = 'none';
		this._snippetCard.tabIndex = 0;
		this._snippetCard.role = 'button';
		const snippetHeader = $('div.chat-edit-snippet-header.show-file-icons');
		this._snippetHeaderIcon = $('span.icon');
		this._snippetHeaderName = $('span.chat-edit-snippet-filename', {}, '');
		this._snippetHeaderAdded = $('span.label-added', {}, '');
		this._snippetHeaderRemoved = $('span.label-removed', {}, '');
		snippetHeader.append(this._snippetHeaderIcon, this._snippetHeaderName, this._snippetHeaderAdded, this._snippetHeaderRemoved);
		this._snippetBody = $('div.chat-edit-snippet-body');
		this._snippetCard.append(snippetHeader, this._snippetBody);

		this.element.append(this.statusIndicatorContainer, this.pillElement, this._snippetCard);

		this._register(registerOpenEditorListeners(this.pillElement, opts => this._onDidClick.fire(opts)));
		this._register(registerOpenEditorListeners(this._snippetCard, opts => this._onDidClick.fire(opts)));
		...
		this._register(dom.addDisposableListener(this._snippetCard, dom.EventType.CONTEXT_MENU, e => {
			const event = new StandardMouseEvent(dom.getWindow(e), e);
			dom.EventHelper.stop(e, true);
			this._onDidContextMenu.fire(event);
		}));
	...
	setUri(uri: URI): void {
		...
		this._snippetHeaderName.textContent = iconText;
		const fileKind = uri.path.endsWith('/') ? FileKind.FOLDER : FileKind.FILE;
		this.fileIconEl.classList.remove(...this._pillIconClasses);
		this._snippetHeaderIcon.classList.remove(...this._snippetIconClasses);
		this._pillIconClasses = getIconClasses(this.modelService, this.languageService, uri, fileKind);
		this._snippetIconClasses = [...this._pillIconClasses];
		this.fileIconEl.classList.add(...this._pillIconClasses);
		this._snippetHeaderIcon.classList.add(...this._snippetIconClasses);
	...
	setDiff(diff: { added: number; removed: number } | undefined): void {
		if (!diff) {
			...
			this._snippetHeaderAdded.textContent = '';
			this._snippetHeaderRemoved.textContent = '';
			return;
		}
		...
		this._labelAddedEl.textContent = diff.added > 0 ? `+${diff.added}` : '';
		this._labelRemovedEl.textContent = diff.removed > 0 ? `-${diff.removed}` : '';
		this._snippetHeaderAdded.textContent = diff.added > 0 ? `+${diff.added}` : '';
		this._snippetHeaderRemoved.textContent = diff.removed > 0 ? `-${diff.removed}` : '';
	}

	/**
	 * Shows a Cursor-style mini diff card under the status row. When a snippet
	 * is present the compact pill chip is hidden so the card is the primary
	 * surface (filename + +/- + red/green lines).
	 */
	setDiffSnippet(lines: readonly DiffSnippetLine[] | undefined): void {
		dom.clearNode(this._snippetBody);
		if (!lines?.length) {
			this._snippetCard.style.display = 'none';
			this.element.classList.remove('has-diff-snippet');
			this.pillElement.style.display = '';
			return;
		}

		for (const line of lines) {
			const row = $('div.chat-edit-snippet-line');
			row.classList.add(`is-${line.kind}`);
			const gutter = $('span.chat-edit-snippet-gutter', {}, String(line.lineNumber));
			const marker = $('span.chat-edit-snippet-marker', {}, line.kind === 'added' ? '+' : line.kind === 'removed' ? '-' : ' ');
			const content = $('span.chat-edit-snippet-text', {}, line.text.length ? line.text : ' ');
			row.append(gutter, marker, content);
			this._snippetBody.appendChild(row);
		}

		this._snippetCard.style.display = '';
		this.element.classList.add('has-diff-snippet');
		// Prefer the rich card; keep the compact pill for streaming-only states.
	...
	setAriaLabel(label: string): void {
		...
		this._snippetCard.ariaLabel = label;
	}
...
export async function readResourceText(textModelService: ITextModelService, uri: URI): Promise<string | undefined> {
	try {
		const ref = await textModelService.createModelReference(uri);
		try {
