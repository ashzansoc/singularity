...
import './media/singularity-chat.css';
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private _inputOnTopActive = false;
	...
	get rendersInputOnTop(): boolean {
		if (this.viewOptions.renderInputOnTopWhenEmpty) {
			return this._inputOnTopActive;
		}
	...
	render(parent: HTMLElement): void {
		...
		const renderInputOnTopWhenEmpty = this.viewOptions.renderInputOnTopWhenEmpty ?? false;
		const renderInputOnTop = (this.viewOptions.renderInputOnTop ?? false) || renderInputOnTopWhenEmpty;
		...
		if (renderInputOnTop) {
			this.container.classList.add('input-on-top');
		}
		if (renderInputOnTopWhenEmpty) {
			this._inputOnTopActive = true;
		}
	...
	private updateChatViewVisibility(): void {
		if (this.viewModel) {
			...
			this.updateInputLayoutPosition(numItems === 0);
	...
	private updateInputLayoutPosition(isEmpty: boolean): void {
		if (!this.viewOptions.renderInputOnTopWhenEmpty || !this.inputPartDisposable.value) {
			return;
		}

		const shouldBeOnTop = isEmpty;
		if (shouldBeOnTop === this._inputOnTopActive) {
			return;
		}
		this._inputOnTopActive = shouldBeOnTop;

		const inputElement = this.inputPart.element;
		if (shouldBeOnTop) {
			this.container.classList.add('input-on-top');
			this.container.insertBefore(inputElement, this.listContainer);
		} else {
			this.container.classList.remove('input-on-top');
		...
		try {
			...
			if (!numItems) {
				...
				if (this.chatEntitlementService.anonymous && !this.chatEntitlementService.sentiment.completed) {
					...
					additionalMessage = new MarkdownString(localize({ key: 'settings', comment: ['{Locked="]({1})"}', '{Locked="]({2})"}'] }, "By continuing with Singularity, you agree to {0}'s [Terms]({1}) and [Privacy Statement]({2}).", providers.default.name, product.defaultChatAgent.termsStatementUrl, product.defaultChatAgent.privacyStatementUrl), { isTrusted: true });
				} else {
	...
	/**
	 * Checks if any agent instruction files (.github/singularity-instructions.md or AGENTS.md) exist in the workspace.
	...
	private getWelcomeViewContent(additionalMessage: string | IMarkdownString | undefined): IChatViewWelcomeContent {
		if (this.isLockedToCodingAgent) {
			...
			const message = providerMessage
				...
				: (this._lockedAgent?.prefix === '@singularity '
					? new MarkdownString(localize('singularityCodingAgentMessage', "This chat session will be forwarded to the {0} [coding agent]({1}) where work is completed in the background. ", this._lockedAgent.prefix, 'https://aka.ms/coding-agent-docs') + DISCLAIMER, { isTrusted: true })
		...
		let title: string;
		const modeName = this.input.currentModeObs.get().name.get().toLowerCase();
		if (this.input.currentModeKind === ChatModeKind.Ask) {
			title = localize('chatDescription', "Ask Singularity about your code");
		} else if (this.input.currentModeKind === ChatModeKind.Edit) {
			title = localize('editsTitle', "Edit with Singularity");
		} else if (modeName === 'plan') {
			title = localize('planTitle', "Plan with Singularity");
		} else if (modeName === 'debug') {
			title = localize('debugTitle', "Debug with Singularity");
		} else if (modeName === 'review') {
			title = localize('reviewTitle', "Review with Singularity");
		} else {
			title = localize('agentTitle', "Code with Singularity");
		}
	...
	private _maybeStartGoalSummary(prompt: string): void {
		...
		// harness. Agent-host backed sessions (Singularity CLI, Claude, Codex and the
