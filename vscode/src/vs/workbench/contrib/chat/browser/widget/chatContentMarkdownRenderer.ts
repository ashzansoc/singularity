...
export const allowedChatMarkdownHtmlTags = Object.freeze([
	...
	// TODO@roblourens when we sanitize attributes in markdown source, we can ban these elements at that step. microsoft/vscode-singularity#5091
...
export function getChatMarkdownRenderOptions(options?: MarkdownRenderOptions): MarkdownRenderOptions {
	return {
		...
		sanitizerConfig: {
			...
			allowedLinkSchemes: { augment: [product.urlProtocol, 'singularity-skill', Schemas.vscodeBrowser, AGENT_HOST_SCHEME, AGENT_HOST_SESSION_LINK_SCHEME] },
