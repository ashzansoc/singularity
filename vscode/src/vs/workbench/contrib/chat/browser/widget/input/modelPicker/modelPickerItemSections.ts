...
export function buildUnavailableStateItems(options: IBuildModelPickerItemsOptions): IActionListItem<IActionWidgetDropdownAction>[] | undefined {
	...
	if (setupRequired) {
		...
		return [
			{ kind: ActionListItemKind.Header, label: localize('chat.modelPicker.setupRequired', "Sign in to use Singularity") },
			{
				item: {
					...
					checked: false,
					class: undefined,
					tooltip: localize('chat.modelPicker.setupRequired.signInTooltip', "Sign in to Singularity to choose a model."),
					label: localize('chat.modelPicker.setupRequired.signIn', "Sign in to use Singularity..."),
	...
	hover?.appendMarkdown(localize('chat.modelPicker.upgradeHover', "[Upgrade to Singularity Pro](command:workbench.action.chat.upgradePlan \" \") to use the best models."));
	return [{
		item: {
			id: 'noModels',
			enabled: false,
...
function appendOtherModels(context: IGroupedContext): boolean {
	...
	const sortedGroups = [...groups.values()].sort((left, right) => {
		if (left.vendor === 'singularity' && right.vendor !== 'singularity') { return -1; }
		if (right.vendor === 'singularity' && left.vendor !== 'singularity') { return 1; }
