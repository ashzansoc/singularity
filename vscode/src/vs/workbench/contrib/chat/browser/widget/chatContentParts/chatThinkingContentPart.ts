...
const THINKING_SCROLL_MAX_HEIGHT = 120;
...
export class ChatThinkingContentPart extends ChatCollapsibleContentPart implements IChatContentPart {
	...
	constructor(
		...
		if (!this.streamingCompleted && !this.element.isComplete) {
			...
			// Past turns / already-finished thinking should not leave a leftover header.
			hide(node);
			node.classList.add('chat-thinking-hidden');
		}
	...
	private getThoughtForTitle(): string | undefined {
		if (!this.containsReasoning || !this.reasoningDurationMs || this.containsGroupedItems) {
			return undefined;
		}
		const seconds = Math.max(1, Math.ceil(this.reasoningDurationMs / 1000));
		return seconds === 1
			? localize('chat.thinking.thoughtFor.one', "Thought for 1 second")
			: localize('chat.thinking.thoughtFor', "Thought for {0} seconds", seconds);
	}

	private getFinalizedDisplayTitle(title: string): string {
		return this.getThoughtForTitle() ?? title;
	}
	...
	/**
	 * Hides the thinking stream window once the assistant reply is visible.
	 * Used by fixed-scrolling mode so thinking does not linger as a header.
	 */
	public hideAfterResponse(): void {
		if (this._store.isDisposed) {
			return;
		}
		hide(this.domNode);
		this.domNode.classList.add('chat-thinking-hidden');
		this._onDidChangeHeight.fire();
	}
	...
	public updateThinking(content: IChatThinkingPart): void {
		...
		// Keep the working spinner aligned with real status text (e.g. "Runtime Engine —
		// Planning…") instead of cycling random "Evaluating" while we already know the phase.
		if (this.workingSpinnerLabel) {
			const detailLine = raw.split('\n').slice(1).join(' ').trim();
			const statusLabel = extractedTitle
				? (detailLine ? `${extractedTitle} — ${detailLine}` : extractedTitle)
				: undefined;
			if (statusLabel) {
				this.workingSpinnerLabel.textContent = statusLabel;
			}
		}
	...
	public finalizeTitleIfDefault(): void {
		...
		const thoughtForTitle = this.getThoughtForTitle();
		if (thoughtForTitle) {
			this.currentTitle = thoughtForTitle;
			this.content.generatedTitle = thoughtForTitle;
			this.setGeneratedTitleOnAllParts(thoughtForTitle);
			this.setFinalizedTitle(thoughtForTitle);
			return;
		}
	...
	private async generateTitleViaLLM(): Promise<void> {
		...
		try {
			const models = await this.languageModelsService.selectLanguageModels({ vendor: 'singularity', id: 'singularity-utility-small' });
			if (!models.length) {
				this.setFallbackTitle();
				return;
			}
	...
	public setupThinkingContainer(content: IChatThinkingPart) {
		...
		if (content.value) {
			...
			if (this.workingSpinnerLabel) {
				...
				const extractedTitle = extractTitleFromThinkingContent(raw);
				const detailLine = raw.split('\n').slice(1).join(' ').trim();
				this.workingSpinnerLabel.textContent = extractedTitle
					? (detailLine ? `${extractedTitle} — ${detailLine}` : extractedTitle)
					: this.getRandomWorkingMessage(WorkingMessageCategory.Thinking);
			}
		}
		this.updateDropdownClickability();
	}
