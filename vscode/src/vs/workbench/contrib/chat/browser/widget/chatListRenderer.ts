...
import { formatChatRequestTimestamp, formatChatResponseDetails, formatChatResponseElapsedTime, formatChatResponsePerf } from '../../common/chatProgressFormatting.js';
...
import { ChatAgentTeamContentPart } from './chatContentParts/chatAgentTeamContentPart.js';
...
const SINGULARITY_USERNAME = 'Singularity';
const SINGULARITY_DISPLAY_NAME = 'Singularity';
...
/**
 * Renders the response footer: completion timestamp, delivery perf (time + TPS),
 * and the model/credits stat.
 ...
 */
export function renderChatResponseDetails(container: HTMLElement, details: string | undefined, completedAt: number | undefined, elapsedMs: number | undefined, verbose: boolean, tokenStatsAriaLabel?: string, completionTokens?: number): HTMLElement | undefined {
	...
	const alternate = completion?.isRelative
		...
		: elapsed;
	const perf = formatChatResponsePerf(elapsedMs, completionTokens);
	...
	if (perf) {
		...
		dom.append(container, $('span.chat-response-perf', undefined, perf));
	}
	...
	const accessibleElapsed = elapsed
		...
		: undefined;
	const accessiblePerf = perf
		? localize('chatResponsePerf', "Response performance {0}", perf)
		: undefined;
	container.ariaLabel = [details, accessibleTiming, accessibleElapsed, accessiblePerf, tokenStatsAriaLabel].filter(Boolean).join(', ');
	container.classList.toggle('hidden', !responseDetails && !perf);
	container.tabIndex = (responseDetails || perf) ? 0 : -1;
...
export function shouldHideChatUserIdentity(username: string, sessionResource: URI, isResponse: boolean, isSessionsWindow: boolean, isSystemInitiatedRequest: boolean): boolean {
	...
	// Show Singularity as the response author; still hide session-window / system chrome.
	return (isResponse && isAgentHostSingularitySessionType(sessionType)) ||
		isSessionsWindow ||
		isSystemInitiatedRequest;
}

function isAgentHostSingularitySessionType(sessionType: string): boolean {
	return sessionType === AgentSessionProviders.AgentHostSingularity ||
		parseRemoteAgentHostSessionTypeAuthority(sessionType, SessionType.SingularityCLI) !== undefined;
}
...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	private renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplate): void {
		...
		const updateResponseDetails = () => {
			...
			const completedAtElement = renderChatResponseDetails(
				...
				isResponseVM(element) ? element.model.completionTokenCount : undefined,
			);
		...
		templateData.username.textContent =
			element.username === SINGULARITY_USERNAME || element.username === 'Singularity' || element.username === 'Singularity AI' || element.username === 'GitHub Singularity' || element.username === 'GitHub Singularity Chat' || element.username === 'Singularity'
				? SINGULARITY_DISPLAY_NAME
				: element.username;
	...
	private finalizeCompletedResponseParts(element: IChatResponseViewModel, templateData: IChatListItemTemplate): void {
		...
		if (lastThinking?.domNode && lastThinking.getIsActive()) {
			...
			if (getEffectiveThinkingDisplayMode(this.configService, this.contextKeyService) === ThinkingDisplayMode.FixedScrolling) {
				lastThinking.hideAfterResponse();
			}
		}
	...
	private shouldShowWorkingProgress(element: IChatResponseViewModel, partsToRender: IChatRendererContent[], moreContentAvailable: boolean, templateData: IChatListItemTemplate): IChatWorkingProgress | undefined {
		...
		if (
			...
			// Prefer the latest real progress line over a random "Evaluating" placeholder
			// so users see what is happening while waiting for the next streamed part.
			const latestProgress = isResponseVM(element)
				? element.progressMessages.at(-1)?.content
				: undefined;
			return latestProgress
				? { kind: 'working', content: latestProgress }
				: { kind: 'working' };
		}
	...
	private finalizeCurrentThinkingPart(context: IChatContentPartRenderContext, templateData: IChatListItemTemplate): void {
		...
		lastThinking.markAsInactive();
		// Fixed-scrolling thinking is a temporary stream window — hide once the reply starts.
		if (style === ThinkingDisplayMode.FixedScrolling) {
			lastThinking.hideAfterResponse();
		}
	}
	...
	private renderChatContentPart(content: IChatRendererContent, templateData: IChatListItemTemplate, context: IChatContentPartRenderContext, batchedSubagentParts?: Set<ChatSubagentContentPart>): IChatContentPart | undefined {
		try {
			...
			} else if (content.kind === 'agentTeam') {
				return this.instantiationService.createInstance(ChatAgentTeamContentPart, content, context);
