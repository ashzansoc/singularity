...
export class ChatModifiedFilesConfirmationSubPart extends AbstractToolConfirmationSubPart {
	...
	constructor(
		...
		const customOptions = state.confirmationMessages?.customOptions;
		const buttons = customOptions && customOptions.length > 0
			? this.buildCustomOptionButtons(toolInvocation, customOptions)
			: this.createLegacyOptionButtons(data.options);
	...
	private createLegacyOptionButtons(options: readonly string[]): IChatConfirmationButton<() => void>[] {
		...
		return [
			{
				...
				moreActions: secondaryOptions.length > 0
