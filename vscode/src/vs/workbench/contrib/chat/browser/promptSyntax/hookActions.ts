...
import { getSingularityCliHookTypeName, resolveSingularityCliHookType } from '../../common/promptSyntax/hookSingularityCliCompat.js';
...
/**
 * Detects if existing hooks use Singularity CLI naming convention (camelCase).
 * Returns true if any existing key matches the Singularity CLI format.
 */
function usesSingularityCliNaming(hooksObj: Record<string, unknown>): boolean {
	for (const key of Object.keys(hooksObj)) {
		// Check if any key resolves to a Singularity CLI hook type
		if (resolveSingularityCliHookType(key) !== undefined) {
			return true;
		}
	}
	return false;
 ...
 */
function getHookTypeKeyName(hookTypeId: HookType, useSingularityCliNamingConvention: boolean): string {
	if (useSingularityCliNamingConvention) {
		const singularityCliName = getSingularityCliHookTypeName(hookTypeId);
		if (singularityCliName) {
			return singularityCliName;
		}
	}
...
async function addHookToFile(
	...
	const useSingularityCliNamingConvention = !isClaude && usesSingularityCliNaming(hooksContent.hooks);
	const hookTypeKeyName = isClaude
		? (getClaudeHookTypeName(hookTypeId) ?? hookTypeId)
		: getHookTypeKeyName(hookTypeId, useSingularityCliNamingConvention);
	...
	for (const key of Object.keys(hooksContent.hooks)) {
		const resolvedType = isClaude
			...
			: resolveSingularityCliHookType(key);
			...
			switch (step) {
				case Step.SelectHookType: {
					...
					if (options?.target) {
						...
						// No target: group into Default (shared), VS Code Only, Singularity CLI Only
						const vscodeTypes = new Set(Object.values(HOOKS_BY_TARGET[Target.VSCode]));
						const singularityTypes = new Set(Object.values(HOOKS_BY_TARGET[Target.Singularity]));
						const allEntries = Object.entries(HOOK_METADATA) as [HookType, IHookTypeMeta][];

						const shared = allEntries.filter(([h]) => vscodeTypes.has(h) && singularityTypes.has(h));
						const vscodeOnly = allEntries.filter(([h]) => vscodeTypes.has(h) && !singularityTypes.has(h));
						const singularityOnly = allEntries.filter(([h]) => !vscodeTypes.has(h) && singularityTypes.has(h));

						pickerItems = [];
						if (shared.length > 0) {
							pickerItems.push({ type: 'separator', label: localize('hookSection.default', "Local/Singularity CLI Agents") });
						...
						if (singularityOnly.length > 0) {
							pickerItems.push({ type: 'separator', label: localize('hookSection.singularityCliOnly', "Singularity CLI Agents") });
							pickerItems.push(...singularityOnly.map(makeItem));
						}
					}
				...
				case Step.EnterFilename: {
					...
					const isSingularityCliOnly = !isClaudeNewFile
						&& !new Set(Object.values(HOOKS_BY_TARGET[Target.VSCode])).has(selectedHookType!.hookType)
						&& new Set(Object.values(HOOKS_BY_TARGET[Target.Singularity])).has(selectedHookType!.hookType);
					const hookTypeKey = isClaudeNewFile
						? (getClaudeHookTypeName(selectedHookType!.hookType) ?? selectedHookType!.hookType)
						: isSingularityCliOnly
							? (getSingularityCliHookTypeName(selectedHookType!.hookType) ?? selectedHookType!.hookType)
							: selectedHookType!.hookType;
					const newFileHookEntry = isSingularityCliOnly
						? { type: 'command', [targetOS === OperatingSystem.Windows ? 'powershell' : 'bash']: '' }
						: buildNewHookEntry(newFileFormat);
					const commandFieldKey = isSingularityCliOnly
					...
					const hooksContent: Record<string, unknown> = {
						...(isSingularityCliOnly ? { version: 1 } : {}),
						hooks: {
							[hookTypeKey]: [
								newFileHookEntry
							]
