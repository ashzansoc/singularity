...
// Fetch uses two tools: the model-facing 'singularity_fetchWebPage' and the internal
...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	constructor(
		...
		this._register(this._configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(ChatConfiguration.ExtensionToolsEnabled) || e.affectsConfiguration(ChatConfiguration.AgentEnabled) || e.affectsConfiguration(SingularityChatSettingId.Gpt55ReadFileToolEnabled)) {
				this._onDidChangeToolsScheduler.schedule();
			}
		}));
		...
		this.vscodeToolSet = this._register(this.createToolSet(
			...
			VSCodeToolReference.vscode,
			{
				...
				description: localize('singularity.toolSet.vscode.description', 'Use VS Code features'),
				deprecated: true,
			}
		));
		...
		this.executeToolSet = this._register(this.createToolSet(
			...
			SpecedToolAliases.execute,
			{
				...
				description: localize('singularity.toolSet.execute.description', 'Execute code and applications on your machine'),
				deprecated: true,
			}
		));
		...
		this.readToolSet = this._register(this.createToolSet(
			...
			SpecedToolAliases.read,
			{
				...
				description: localize('singularity.toolSet.read.description', 'Read files in your workspace'),
				deprecated: true,
			}
		));
		...
		this.agentToolSet = this._register(this.createToolSet(
			...
			SpecedToolAliases.agent,
			{
				...
				description: localize('singularity.toolSet.agent.description', 'Delegate tasks to other agents'),
				deprecated: true,
			}
		));
	}
	...
	private isToolEnabledForModel(toolData: IToolData, model: ILanguageModelChatMetadata | undefined): boolean {
		...
		if (toolData.id === SingularityToolId.ReadFile && model?.family.startsWith('gpt-5.5') && this._configurationService.getValue<boolean>(SingularityChatSettingId.Gpt55ReadFileToolEnabled) === false) {
			return false;
		}
	...
	private isPermitted(toolOrToolSet: IToolData | ToolSet, reader?: IReader): boolean {
		...
		// Fetch is implemented with two tools, this one and 'singularity_fetchWebPage'
	...
	private isToolEligibleForAutoApproval(toolData: IToolData): boolean {
		...
		if (toolData.id === 'singularity_fetchWebPage') {
