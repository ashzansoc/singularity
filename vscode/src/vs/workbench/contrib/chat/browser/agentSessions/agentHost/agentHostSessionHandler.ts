...
import { getChatErrorDetailsFromMeta, getSingularityPlanFromEntitlement, IChatErrorContext } from '../../../common/chatErrorMessages.js';
...
import { coercePinnedFlashModelId } from '../../../../../../platform/agentHost/common/singularityCliConfig.js';
...
import { buildHostLocalEventsPath } from '../../singularityCliEventsUri.js';
...
interface IObserveTurnOptions {
	 ...
	 * singularity credits reported by this subagent's turns. Subagent turn
...
export interface IAgentHostSessionHandlerConfig {
	 ...
	 * exact URI — while the UI still routes the session to the `singularity` provider.
	 */
...
export class AgentHostSessionHandler extends Disposable implements IChatSessionContentProvider {
	...
	constructor(
		...
		@IExecutionEngineService private readonly _executionEngineService: IExecutionEngineService,
	) {
		super();
		this._config = config;
	...
	private _chatErrorContext(): IChatErrorContext {
		...
		return {
			singularityPlan: getSingularityPlanFromEntitlement(this._chatEntitlementService.entitlement),
	...
	private async _maybeRunExecutionEngine(
		request: IChatAgentRequest,
		goal: string,
		progress: (parts: IChatProgress[]) => void,
		cancellationToken: CancellationToken,
	): Promise<boolean> {
		if (!this._configurationService.getValue<boolean>(ChatConfiguration.ExecutionEnabled)) {
			return false;
		}
		if (!this._executionEngineService.isEnabled()) {
			return false;
		}
		...
		progress([{
			...
			content: new MarkdownString(localize('agentHost.executionStarted', "Running dependency-aware execution engine…")),
		}]);

		try {
			const parentChatUri = this._getChatURI(request.sessionResource).toString();
			const result = await this._executionEngineService.run({
				goal,
				sessionId: request.sessionResource.toString(),
				workspaceRoot: folder.uri.fsPath,
				parentSessionId: request.sessionResource.toString(),
				parentChatUri,
			});

			this._executionEngineService.updateGraph({
				executionId: result.executionId,
				objective: goal,
				status: result.ok ? 'completed' : 'failed',
				batches: this._executionEngineService.getGraph(result.executionId)?.batches ?? [],
				criticalPathLength: result.criticalPathLength ?? 0,
			});

			progress([{
				kind: 'markdownContent',
				content: new MarkdownString(result.summary || (result.ok
					? localize('agentHost.executionCompleted', "Execution completed.")
					: localize('agentHost.executionFailed', "Execution failed."))),
			}]);

			return true;
		} catch (err) {
			if (!cancellationToken.isCancellationRequested) {
				this._logService.error('[AgentHost] Execution engine failed', err);
			}
			return false;
		}
	}
	...
	private async _handleTurn(
		...
		const executionHandled = await this._maybeRunExecutionEngine(request, goal, progress, cancellationToken);
		if (executionHandled) {
			return undefined;
		}
	...
	private _observeTurn(opts: IObserveTurnOptions): IDisposable {
		...
		if (opts.subAgentInvocationId === undefined) {
			...
			store.add(autorun(reader => {
				...
				if (lastUsage
					...
					&& lastUsage.singularityCredits === usage.singularityCredits
					&& lastUsage.sessionSingularityCredits === usage.sessionSingularityCredits
		...
		// For subagent observers: accumulate singularity credits from child turns
		...
		// own child turns. Some hosts (e.g. singularitycli) instead bundle a
		...
		if (opts.subAgentInvocationId !== undefined && opts.subAgentCreditsAccumulator) {
			...
			store.add(autorun(reader => {
				const rawUsage = usage$.read(reader);
				const credits = usageInfoToChatUsage(rawUsage)?.singularityCredits;
	...
	private _applySubagentUsageToHistoryPart(part: IChatProgress, sessionResource: URI, childState: ISessionWithDefaultChat): void {
		...
		for (const turn of turns) {
			const turnCredits = usageInfoToChatUsage(turn.usage)?.singularityCredits;
	...
	private _createModelSelection(languageModelIdentifier: string | undefined, modelConfiguration: Record<string, unknown> | undefined): ModelSelection | undefined {
		...
		// to the SDK context tier by the agent's `getSingularityContextTier`).
	 ...
	 * E.g. "agent-host-singularity:claude-sonnet-4-20250514" → "claude-sonnet-4-20250514".
	...
	private _extractRawModelId(languageModelIdentifier: string | undefined): string | undefined {
		if (!languageModelIdentifier) {
			return coercePinnedFlashModelId(undefined);
		}
		const prefix = this._config.sessionType + ':';
		let raw = languageModelIdentifier;
		if (languageModelIdentifier.startsWith(prefix)) {
			raw = languageModelIdentifier.substring(prefix.length);
		} else if (languageModelIdentifier.includes('/')) {
			this._logService.warn(`[AgentHost] Dropping foreign model identifier '${languageModelIdentifier}' for session type '${this._config.sessionType}'; pinning to OpenRouter Flash.`);
			return coercePinnedFlashModelId(undefined);
		}
		return coercePinnedFlashModelId(raw);
	}
	 ...
	 * an embedded resource. Singularity CLI and Codex both run as separate processes with only disk
	...
	private _backendInlinesUnsavedEditors(): boolean {
		return this._config.provider === SessionType.SingularityCLI || this._config.provider === CODEX_AGENT_PROVIDER_ID;
	}
	...
	private _convertVariableToAttachment(v: IChatRequestVariableEntry, sessionResource: URI, messageText?: string): MessageAttachment | MessageAttachment[] | undefined {
		...
		// Singularity CLI and Codex can't read unsaved content from disk, so inline the live buffer; drop unreadable schemes.
	...
	private _toSessionReferenceTrajectoryPath(sessionResource: URI): string | undefined {
		// TODO: Support non-Singularity-CLI session references through IChatModel or a first-class AHP attachment path.
