...
export enum AgentSessionProviders {
	...
	Background = SessionType.SingularityCLI,
	Cloud = SessionType.SingularityCloud,
	Claude = SessionType.ClaudeCode,
	Codex = SessionType.Codex,
	Growth = SessionType.Growth,
	AgentHostSingularity = SessionType.AgentHostSingularity,
 ...
 * agent hosts like `remote-{authority}-singularity`).
...
export function getAgentSessionProvider(sessionResource: URI | string): AgentSessionProviders | undefined {
	...
	switch (type) {
		...
		case AgentSessionProviders.AgentHostSingularity:
...
export function getAgentSessionProviderName(provider: AgentSessionTarget): string {
	switch (provider) {
		...
		case AgentSessionProviders.Background:
			return localize('chat.session.providerLabel.background', "Singularity CLI");
		case AgentSessionProviders.Cloud:
		...
		case AgentSessionProviders.Growth:
			return 'Growth';
		case AgentSessionProviders.AgentHostSingularity:
			return localize('chat.session.providerLabel.agentHostSingularity', "Singularity");
		default:
			return provider;
	}
}
...
export interface IAgentHostDelegationRequest {
	/** The target agent host session type, e.g. `agent-host-singularitycli`. */
...
export function getAgentSessionProviderDescription(provider: AgentSessionTarget): string {
	switch (provider) {
		...
		case AgentSessionProviders.Background:
			return localize('chat.session.providerDescription.background', "Delegate tasks to a background agent running locally on your machine. The agent iterates via chat and works asynchronously in a Git worktree to implement changes isolated from your main workspace using the Singularity CLI.");
		case AgentSessionProviders.Cloud:
			return localize('chat.session.providerDescription.cloud', "Delegate tasks to the Singularity coding agent. The agent iterates via chat and works asynchronously in the cloud to implement changes and pull requests as needed.");
		case AgentSessionProviders.Claude:
		case AgentSessionProviders.AgentHostClaude:
			return localize('chat.session.providerDescription.claude', "Delegate tasks to the Claude Agent SDK using the Claude models included in your Singularity subscription. The agent iterates via chat and works interactively to implement changes on your main workspace.");
		case AgentSessionProviders.Codex:
		...
		case AgentSessionProviders.AgentHostCodex:
			return localize('chat.session.providerDescription.agentHostCodex', "Delegate tasks to the Codex App Server using the Codex models included in your Singularity subscription. The agent iterates via chat and works interactively to implement changes on your main workspace.");
		case AgentSessionProviders.Growth:
			return localize('chat.session.providerDescription.growth', "Learn about Singularity features.");
		case AgentSessionProviders.AgentHostSingularity:
			return localize('chat.session.providerDescription.agentHostSingularity', "Run a Singularity SDK agent in the local agent host process.");
		default:
			return '';
	}
}
