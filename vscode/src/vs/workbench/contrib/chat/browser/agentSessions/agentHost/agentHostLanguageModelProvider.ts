 ...
 * fall back to.
 *
 * Today only the Singularity CLI harness exposes an Auto selection and can run
 ...
 *
 * `provider` is the underlying agent provider id (e.g. `'singularitycli'`,
...
export function agentHostProviderSupportsAutoModel(provider: string): boolean {
	return provider === 'singularitycli';
}
...
export class AgentHostLanguageModelProvider extends Disposable implements ILanguageModelChatProvider {
			...
			.map(m => {
				...
				return {
					...
					metadata: {
						...
						// (Singularity CLI, OpenAI, a 3p BYOK provider, …). All of a host's
	 ...
	 * e.g. `singularitycli`). The picker resolves the display name from the vendor registry —
