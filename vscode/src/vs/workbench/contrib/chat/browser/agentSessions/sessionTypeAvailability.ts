...
export enum SessionTypeAvailability {
	...
	Available,
	/** Unusable until the user signs in (the type needs a Singularity account; BYOK is not supported here). */
	SignInRequired,
	/** Unusable, but the user can resolve it by upgrading (Singularity Free / Student). */
	UpgradeRequired,
 ...
 * targeted model. When a type is not usable, Singularity Free / Student (EDU) users
 ...
 * affordance ({@link SessionTypeAvailability.SignInRequired}) for Singularity-backed
 * types ({@link IChatSessionsService.requiresSingularitySignInForSessionType}), unless
 * anonymous access is enabled; types that don't depend on Singularity stay usable
...
export function getSessionTypeAvailability(
	...
	// Singularity-backed types need a Singularity account (BYOK isn't supported here), so a signed-out user can't use them —
	...
	if (entitlement === ChatEntitlement.Unknown && !chatEntitlementService.anonymous && chatSessionsService.requiresSingularitySignInForSessionType(type)) {
		return SessionTypeAvailability.SignInRequired;
	}
	// Signed in: a model targeting the type (e.g. BYOK) or an "Auto" fallback
	// (e.g. the Singularity CLI harness) makes it usable.
...
export function getSessionTypeUnavailableHover(availability: SessionTypeAvailability): IMarkdownString | undefined {
	switch (availability) {
		case SessionTypeAvailability.SignInRequired: {
			...
			hover.appendMarkdown(localize('chat.sessionType.signInHover', "[Sign in to Singularity](command:workbench.action.chat.triggerSetup) to use this agent."));
			return hover;
		}
		case SessionTypeAvailability.UpgradeRequired: {
			const hover = new MarkdownString('', { isTrusted: { enabledCommands: ['workbench.action.chat.upgradePlan'] }, supportThemeIcons: true });
			hover.appendMarkdown(localize('chat.sessionType.upgradeHover', "[Upgrade to Singularity Pro](command:workbench.action.chat.upgradePlan) to use this agent."));
			return hover;
		}
		case SessionTypeAvailability.NoModels:
...
export function getSessionTypeUnavailableLabel(availability: SessionTypeAvailability): string | undefined {
	switch (availability) {
		...
		case SessionTypeAvailability.UpgradeRequired:
			return localize('chat.sessionType.upgradeMobile', "Requires Singularity Pro");
		case SessionTypeAvailability.NoModels:
