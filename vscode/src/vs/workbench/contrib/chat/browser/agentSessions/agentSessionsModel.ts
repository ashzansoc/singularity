...
import { SINGULARITY_CLI_EH_SCHEME, SINGULARITY_CLI_LOCAL_AH_SCHEME, getSingularityCliSessionRawId } from '../singularityCliEventsUri.js';
...
export class AgentSessionsModel extends Disposable implements IAgentSessionsModel {
	...
	get sessions(): IAgentSession[] { return this._dedupeMigratedSingularityCliSessions(Array.from(this._sessions.values())); }
	...
	/**
	 * Hide the extension-host `singularitycli:` row when its agent-host
	 * `agent-host-singularitycli:` twin is present, so the list shows a single entry
	 * per legacy Singularity CLI session — the agent-host one, which migrates on open.
	 * Only display is deduped; {@link getSession} and the cache use the full map so
	 * a hidden row can still resolve.
	 */
	private _dedupeMigratedSingularityCliSessions(sessions: IAgentSession[]): IAgentSession[] {
