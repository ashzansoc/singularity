...
import { affectsAgentHostProviderPreference, IAgentHostService, protectedResourcesRequireSingularitySignIn, shouldSurfaceLocalAgentHostProvider, type AgentProvider } from '../../../../../../platform/agentHost/common/agentService.js';
...
export class AgentHostContribution extends Disposable implements IWorkbenchContribution {
	...
	private _registerAgent(agent: AgentInfo): void {
		...
		store.add(this._chatSessionsService.registerChatSessionContribution({
			...
			customAgentTarget: this._isSessionsWindow ? undefined : Target.Singularity,
			canDelegate: true,
			...
			// Singularity resource `required: false` (Claude in native mode, Codex on
			// OpenAI) is usable without signing in. Falls back to "required" until the
			// agent host resolves. The paired `onDidChangeRequiresSingularitySignIn` lets
			// the sessions service re-evaluate this when the set changes.
			requiresSingularitySignIn: () => {
			...
			onDidChangeRequiresSingularitySignIn: Event.signal(Event.filter(this._protectedResourcesService.onDidChange, provider => provider === agent.provider, store)),
		...
		// `[Agent Host]` suffix disambiguates from the extension-host Singularity CLI harness, which uses the same displayName.
		store.add(this._customizationHarnessService.registerExternalHarness({
			...
			// The Tools section is surfaced for the Singularity CLI agent host only.
			hiddenSections: agent.provider === 'singularitycli' ? [AICustomizationManagementSection.Prompts] : [AICustomizationManagementSection.Tools, AICustomizationManagementSection.Prompts],
