...
/**
 * Formats a turn's response footer: routed model name + optional credits.
 * Returns `undefined` when there is nothing user-visible to show.
 */
export function formatTurnResponseDetails(
	...
	const modelLabel = formatTurnModelName(model, billedModelId);
	const credits = usageInfoToChatUsage(usage)?.singularityCredits;
	const creditLabel = credits !== undefined
		? (formatSingularityCredits(credits) === '1'
			? localize('agentHost.responseDetails.credit', "{0} credit", formatSingularityCredits(credits))
			: localize('agentHost.responseDetails.credits', "{0} credits", formatSingularityCredits(credits)))
		: undefined;
	if (modelLabel && creditLabel) {
		return `${modelLabel} \u2022 ${creditLabel}`;
	}
	return modelLabel ?? creditLabel;
}

function formatTurnModelName(model: ITurnResponseModel, billedModelId: string | undefined): string | undefined {
	const name = (model.name || billedModelId || '').trim();
...
export function usageInfoToAutoModeResolution(usage: UsageInfo | undefined, resolvedModelName: string | undefined): IChatAutoModeResolutionPart | undefined {
	// Permanent model pin: suppress the Auto-mode resolution chip. This path
	// renders the CLI SDK's reported auto-router decision (e.g. claude-haiku-4.5
	// / gpt-5-mini), which is never the model actually used — Singularity is
	// pinned to DeepSeek V4 Flash via OpenRouter. Showing it misleads.
	return undefined;
	// The original implementation (kept for reference / re-enable) is below.
...
export function usageInfoToChatUsage(usage: UsageInfo | undefined, modelDisplayNameResolver?: (rawModelId: string) => string | undefined): IChatUsage | undefined {
	...
	return {
		...
		singularityCredits: getSingularityCredits(usage),
		sessionSingularityCredits: getSessionSingularityCredits(usage),
...
function getSessionSingularityCredits(usage: UsageInfo | undefined): number | undefined {
	const sessionTotalNanoAiu = readUsageInfoMeta(usage).singularityUsage?.sessionTotalNanoAiu;
	return typeof sessionTotalNanoAiu === 'number' && sessionTotalNanoAiu >= 0
		? sessionTotalNanoAiu / 1_000_000_000
		: undefined;
}

function getSingularityCredits(usage: UsageInfo | undefined): number | undefined {
	const meta = readUsageInfoMeta(usage);
	const totalNanoAiu = meta?.singularityUsage?.totalNanoAiu;
 ...
 * Singularity CLI's MCP host and never surfaces back over the AHP), so we
...
export function toolCallStateToInvocation(tc: ToolCallState, subAgentInvocationId: string | undefined, sessionResource: URI, connectionAuthority: string, mcpServerAuthority = sessionResource.authority, options?: IAgentHostToolInvocationOptions): ChatToolInvocation {
	...
	if (tc.status === ToolCallStatus.PendingConfirmation) {
		...
		// Singularity SDK's `task` tool doesn't request permission, and the event
