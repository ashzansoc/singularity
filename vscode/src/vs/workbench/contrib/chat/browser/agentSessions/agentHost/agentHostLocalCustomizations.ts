...
import { AGENT_HOST_SINGULARITY_CLI_SESSION_TYPE } from './agentHostToolSetEnablementService.js';
 ...
 *
 * True only for the local Singularity Agent Host harness, in a multi-root workspace,
...
export function shouldSyncWorkspaceDotMcp(sessionType: string, workspaceFolderCount: number, multiRootSettingEnabled: boolean): boolean {
	return sessionType === AGENT_HOST_SINGULARITY_CLI_SESSION_TYPE
 ...
 * interaction.
 *
 * When {@link includeWorkspaceDotMcp} is `true` (multi-root Singularity Agent Host
...
export async function collectNonPluginMcpServers(mcpService: IMcpService, configurationResolverService: IConfigurationResolverService, includeWorkspaceDotMcp: boolean): Promise<ISyncableMcpServer[]> {
	...
	for (const server of mcpService.servers.get()) {
		...
		// Remote MCP (Notion, Slack, …) is executed in-process by VS Code with
		// vault-backed OAuth. Do not hand the URL to the agent-host CLI — it
		// would start a second, unauthenticated copy and hide the real tools.
