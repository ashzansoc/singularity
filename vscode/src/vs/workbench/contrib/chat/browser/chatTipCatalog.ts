...
export const TIP_CATALOG: readonly ITipDefinition[] = [
	...
	{
		id: 'tip.singularityCli',
		tier: ChatTipTier.Qol,
		buildMessage() {
			return new MarkdownString(
				localize(
					'tip.singularityCli',
					"Run agents in parallel with [Singularity CLI](command:workbench.action.chat.openNewChatSessionInPlace.singularitycli?%5B%22sidebar%22%5D \"Switch to Singularity CLI\")."
				)
			);
		},
		when: ContextKeyExpr.and(
		...
		excludeWhenCommandsExecuted: ['workbench.action.chat.openNewChatSessionInPlace.singularitycli'],
	},
	{
