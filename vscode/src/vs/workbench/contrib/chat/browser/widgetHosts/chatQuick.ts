...
class QuickChat extends Disposable {
	...
	private setupDisclaimer(parent: HTMLElement): void {
		...
		this._register(autorun(reader => {
			...
			if (showDisclaimer) {
				const renderedMarkdown = disposables.add(this.markdownRendererService.render(new MarkdownString(localize({ key: 'termsDisclaimer', comment: ['{Locked="]({1})"}', '{Locked="]({2})"}'] }, "By continuing with Singularity, you agree to {0}'s [Terms]({1}) and [Privacy Statement]({2})", product.defaultChatAgent?.provider?.default?.name ?? '', product.defaultChatAgent?.termsStatementUrl ?? '', product.defaultChatAgent?.privacyStatementUrl ?? ''), { isTrusted: true })));
