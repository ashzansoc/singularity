			...
			for (const ext of extensions) {
				...
				for (const contribution of ext.value) {
					...
					if (!icon && contribution.icon) {
						ext.collector.error(localize('chatContextExtPoint.invalidIcon', "Invalid icon format for chat context contribution '{0}'. Icon must be in the format '{1}' or '{2}', e.g. '{3}'.", contribution.id, '$(iconId)', '$(iconId~spin)', '$(singularity)'));
						continue;
					}
					if (!icon) {
