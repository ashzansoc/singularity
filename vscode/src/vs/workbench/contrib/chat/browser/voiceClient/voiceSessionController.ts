...
export class VoiceSessionController extends Disposable implements IVoiceSessionController {
	 ...
	 * `singularitycli:/<id>`) to the UI agent-host session resource string
	 ...
	 * resident. Singularity/remote session models are disposed as soon as the user
	 ...
	 * summary of the latest Singularity reply per active session. Sent to the
	...
	/**
	 * Max sentences of Singularity's last reply we include per active coding
	...
	async connect(window: Window & typeof globalThis): Promise<void> {
		...
		// Voice Code is tightly coupled to GitHub auth via Singularity — one session
				...
				const autorunDisposable = autorun(reader => {
					...
					for (const s of agentSessions) {
						...
						if (model) {
							...
							// Remote/Singularity sessions don't keep their model resident, so a
	...
	private async _sendTranscriptionToChat(text: string): Promise<void> {
		...
		if (target) {
			...
			this.commandService.executeCommand('workbench.panel.chat.view.singularity.focus').catch(() => { /* ignore */ });
		}
	}
	 ...
	 * active coding session (first ~2 sentences of the latest Singularity
	...
	private _currentNarratable(resource: URI): IVoiceNarratable | undefined {
		...
		if (session?.status === AgentSessionStatus.Completed) {
			...
			// never went resident this turn, e.g. a remote/Singularity reply that
	...
	private _checkSessionStateChanges(): void {
		...
		for (const s of sessions) {
			...
			// Summary-less idle transitions for remote/Singularity sessions: narrate
	...
	private _ensureModelLoaded(resource: URI): void {
		...
		// A surfaced-but-un-adopted legacy Singularity CLI session must NOT be eagerly
	 ...
	 * Singularity sessions don't keep their model loaded, so without this the
	...
	private _formatQuestionnaireNarration(questionnaire: IVisibleVoiceQuestionnaire): string | undefined {
		...
		for (let index = 0; index < questionLimit; index++) {
			...
			if (visibleOptions.length > 0) {
				...
				questionLines.push(localize('voice.questionnaire.freeform', "response: enter a free-form answer in Singularity"));
			}
			...
			const reservedSuffix = remainingAfterCandidate > 0
				? remainingAfterCandidate === 1
					? localize('voice.questionnaire.oneOmitted', "1 more question is open in Singularity.")
					: localize('voice.questionnaire.manyOmitted', "{0} more questions are open in Singularity.", remainingAfterCandidate)
				: localize('voice.questionnaire.open', "The questionnaire is open in Singularity.");
		...
		if (omittedQuestions > 0) {
			lines.push(omittedQuestions === 1
				? localize('voice.questionnaire.oneOmitted', "1 more question is open in Singularity.")
				: localize('voice.questionnaire.manyOmitted', "{0} more questions are open in Singularity.", omittedQuestions));
		} else {
			lines.push(localize('voice.questionnaire.open', "The questionnaire is open in Singularity."));
		}
		return lines.join('\n') || fallback;
	}
	...
	private _formatPlanNarration(plan: IChatPlanReview): string {
		const fallback = localize('voice.plan.fallback', "A plan is open in Singularity and needs your approval.");
		...
		lines.push(localize('voice.plan.open', "The plan is open in Singularity."));
	...
	private _formatElicitationNarration(elicitation: IChatElicitationRequest): string {
		const fallback = localize('voice.elicitation.fallback', "Singularity needs your input in the open request.");
	...
	private _formatConfirmationNarration(confirmation: IChatConfirmation): string {
		const fallback = localize('voice.confirmation.fallback', "Singularity needs your approval to continue.");
	...
	private _formatToolNarration(toolInvocation: IChatToolInvocation): string {
		const fallback = localize('voice.toolConfirmation.fallback', "Singularity needs your approval to continue.");
		const state = toolInvocation.state.get();
		if (state.type !== IChatToolInvocation.StateKind.WaitingForConfirmation && state.type !== IChatToolInvocation.StateKind.WaitingForPostApproval) {
			return fallback;
		}
	...
	private _formatToolAuthenticationNarration(toolInvocation: IChatToolInvocation): string | undefined {
		...
		const fallback = localize('voice.authentication.fallback', "Singularity needs authentication to continue.");
