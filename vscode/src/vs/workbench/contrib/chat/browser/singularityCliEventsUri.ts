...
import { getSingularityHomePath } from '../../../../platform/agentHost/common/singularityHome.js';
...
// Scheme conventions for `singularitycli` chat sessions:
// - Local AH:  `agent-host-singularitycli:/<id>`         (LOCAL_RESOURCE_SCHEME_PREFIX + provider)
// - Remote AH: `remote-<auth>-singularitycli:/<id>`      (remoteAgentHostSessionTypeId, agent.provider = 'singularitycli')
// - EH CLI ext: `singularitycli:/<id>`                   (extension's own session type)
// The singularity agent reports its provider id as `singularitycli` on every host, so
// the local, remote and EH schemes all carry the `singularitycli` token.
const SINGULARITY_CLI_PROVIDER = 'singularitycli';
export const SINGULARITY_CLI_LOCAL_AH_SCHEME = `agent-host-${SINGULARITY_CLI_PROVIDER}`;
export const SINGULARITY_CLI_EH_SCHEME = SINGULARITY_CLI_PROVIDER;

/**
 * Builds the local `events.jsonl` URI under `<SINGULARITY_HOME>/session-state/<rawId>/`.
 *
 * Used for both the local Agent Host Singularity CLI provider and the
 * extension-host Singularity CLI provider, which share the same on-disk layout
 * and the same chat session URI shape (`singularitycli:/<rawId>`).
 */
export function buildLocalEventsUri(userHome: URI, rawSessionId: string, environment: IProcessEnvironment = env): URI {
	return joinPath(buildLocalSingularityHomeUri(userHome, environment), 'session-state', rawSessionId, 'events.jsonl');
}

/**
 * Builds the local `<SINGULARITY_HOME>/logs` directory URI.
 */
export function buildLocalSingularityLogsUri(userHome: URI, environment: IProcessEnvironment = env): URI {
	return joinPath(buildLocalSingularityHomeUri(userHome, environment), 'logs');
}

/**
 * Builds the local `<SINGULARITY_HOME>/session-state` directory URI.
 */
export function buildLocalSessionStateUri(userHome: URI, environment: IProcessEnvironment = env): URI {
	return joinPath(buildLocalSingularityHomeUri(userHome, environment), 'session-state');
}
...
export function buildRemoteEventsUri(connection: IRemoteAgentHostConnectionInfo, rawSessionId: string): URI | undefined {
	...
	const remoteFileUri = URI.from({
		scheme: 'file',
		path: `${trimmed}/.singularity/session-state/${rawSessionId}/events.jsonl`,
	});
...
/**
 * Builds a `vscode-agent-host://` URI for the host's `~/.singularity/logs`
 * directory, using the connection's reported home directory.
 */
export function buildRemoteSingularityLogsUri(connection: IRemoteAgentHostConnectionInfo): URI | undefined {
	const homePath = connection.defaultDirectory;
	if (!homePath) {
		return undefined;
	}
 ...
 * of the form `remote-<authority>-singularitycli`. Returns `undefined` for
 * any other scheme, including the local `singularitycli` scheme.
 */
export function parseRemoteAuthorityFromScheme(scheme: string): string | undefined {
	return parseRemoteAgentHostSessionTypeAuthority(scheme, SINGULARITY_CLI_PROVIDER);
}

/**
 * Extracts the raw Singularity CLI session id from a chat session resource.
 */
export function getSingularityCliSessionRawId(sessionResource: URI | undefined): string | undefined {
	if (!sessionResource) {
		return undefined;
	}
	if (sessionResource.scheme !== SINGULARITY_CLI_LOCAL_AH_SCHEME && sessionResource.scheme !== SINGULARITY_CLI_EH_SCHEME && !parseRemoteAuthorityFromScheme(sessionResource.scheme)) {
		return undefined;
	}
 ...
 * `events.jsonl` URI for the corresponding Singularity CLI session, or
...
export function resolveEventsUri(
	...
	if (sessionResource.scheme === SINGULARITY_CLI_LOCAL_AH_SCHEME || sessionResource.scheme === SINGULARITY_CLI_EH_SCHEME) {
...
/**
 * Resolves the host-local filesystem path string of a Singularity CLI session's
 ...
 * used by the chat debug panel and the "Open Singularity CLI State File" command,
 ...
 *
 * Returns `undefined` when the session is not a Singularity CLI session or the
...
function buildLocalSingularityHomeUri(userHome: URI, environment: IProcessEnvironment): URI {
	return URI.file(getSingularityHomePath(userHome.fsPath, environment));
}
