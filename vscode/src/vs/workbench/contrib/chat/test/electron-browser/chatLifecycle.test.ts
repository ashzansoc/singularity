...
suite('ChatLifecycle', () => {
	...
	test('shouldWarnForSessionShutdown', () => {
		assert.deepStrictEqual([
			...
			{ name: 'local agent host', warnings: warningsByReason(createSession(AgentSessionProviders.AgentHostSingularity)) },
			{ name: 'dynamic local agent host', warnings: warningsByReason(createSession('agent-host-foo')) },
			{ name: 'remote agent host', warnings: warningsByReason(createSession('remote-host-singularitycli')) },
