...
import {
	...
	getSingularityPlanFromEntitlement,
...
suite('ChatErrorMessages', () => {
		...
		test('formats a forwarded rate-limit error', () => {
			const details = getChatErrorDetailsFromMeta(errorInfo({
				chatError: {
					...
					singularityPlan: 'free',
				},
			}));
			assert.deepStrictEqual(details, {
				code: ChatFetchResponseType.RateLimited,
				message: 'You\'ve hit your session rate limit. Please upgrade your plan or wait 60 seconds for your limit to reset. [Learn More](https://aka.ms/github-singularity-rate-limit-error)',
		...
		test('context overrides the forwarded plan (free user)', () => {
			const details = getChatErrorDetailsFromMeta(errorInfo({
				chatError: {
					...
					singularityPlan: 'business',
				},
			}), { singularityPlan: 'free' });
			assert.strictEqual(details?.message, 'You\'ve reached your monthly chat messages quota. Upgrade to Singularity Pro or wait for your allowance to renew.');
		});
	...
	suite('getChatErrorDetailsFromFetchError', () => {
		...
		test('quota exceeded uses plan-specific message and quota code', () => {
			...
			assert.deepStrictEqual(details, {
				code: 'quota_exceeded',
				message: 'You\'ve reached your monthly chat messages quota. Upgrade to Singularity Pro or wait for your allowance to renew.',
		...
		test('filtered response is marked filtered', () => {
			...
			assert.deepStrictEqual(details, {
				...
				message: 'Sorry, the response matched public code so it was blocked. Please rephrase your prompt. [Learn more](https://aka.ms/singularity-chat-filtered-docs).',
		...
		test('rate-limit messages vary by capi code and auto flag', () => {
			...
			assert.deepStrictEqual(messages, [
				'Sorry, you have exceeded the agent mode rate limit. Please switch to ask mode and try again in 30 seconds. [Learn More](https://aka.ms/github-singularity-rate-limit-error)',
				'Sorry, the upstream model provider is currently experiencing high demand. Please try again in 30 seconds. [Learn More](https://aka.ms/github-singularity-rate-limit-error)',
				'Sorry, Singularity AI is currently experiencing high demand. Please try again in 30 seconds. [Learn More](https://aka.ms/github-singularity-rate-limit-error)',
				'Sorry, your request was rate-limited. Please wait 30 seconds before trying again or consider switching to Auto. [Learn More](https://aka.ms/github-singularity-rate-limit-error)',
			]);
		});
		...
		test('quota sub-codes map to specific messages', () => {
			...
			assert.deepStrictEqual(messages, [
				'You cannot accrue additional premium requests at this time. Please contact [GitHub Support](https://support.github.com/contact) to continue using Singularity.',
	...
	suite('getQuotaMessageForPlan', () => {
		...
		test('usage-based billing business plan with reset date', () => {
			...
			assert.ok(message.startsWith('You\'ve reached your credit limit. To continue working, please contact your organization\'s Singularity admin or wait until your credits reset on'));
		});

		test('default plan, no usage-based billing', () => {
			assert.strictEqual(
				getQuotaMessageForPlan(undefined),
				'You\'ve exhausted your premium model quota. For additional paid premium requests, please reach out to your organization\'s Singularity admin or wait for your allowance to renew.',
			);
		});
		...
		test('edu plan with usage-based billing', () => {
			assert.deepStrictEqual(
				...
				[
					`You've reached your monthly credit limit. Please enable additional paid credits, upgrade to Singularity Pro, or wait until your credits reset on ${new Date('2030-01-15T00:00:00.000Z').toLocaleString(undefined, { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: '2-digit' })}.`,
					'You\'ve reached your monthly credit limit. Please enable additional paid credits, upgrade to Singularity Pro, or wait for your credits to reset.',
				],
			);
		});
		...
		test('edu plan without usage-based billing', () => {
			assert.strictEqual(
				...
				'You\'ve exhausted your premium model quota. Please enable additional paid premium requests, upgrade to Singularity Pro, or wait for your allowance to renew.',
			);
		});
	});

	suite('getSingularityPlanFromEntitlement', () => {

		test('maps entitlements to Singularity plan strings', () => {
			const actual = [
			...
			].map(getSingularityPlanFromEntitlement);
