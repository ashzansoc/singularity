...
import { AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING, SINGULARITY_SKILL_URI_SCHEME, TROUBLESHOOT_SKILL_PATH } from '../../../common/promptSyntax/promptTypes.js';
...
suite('ChatService', () => {
	...
	test('sendPendingRequestImmediately re-sends a steering message as a turn on agent host sessions', async () => {
		const sessionType = 'agent-host-singularity';
	...
	test('sendRequest redacts remote session type in provider invoked telemetry', async () => {
		const sessionType = 'remote-test-singularity';
		const sessionResource = URI.from({ scheme: sessionType, path: '/session' });
	...
	test('troubleshoot skill via attachedContext is blocked when fileLogging.enabled is off', async () => {
		...
		const skillUri = URI.from({ scheme: SINGULARITY_SKILL_URI_SCHEME, path: TROUBLESHOOT_SKILL_PATH });
	...
	suite('loadRemoteSession progress streaming', () => {
		...
		test('restored draft uses the session history model, not the persisted (stale) one', async () => {
			...
			const historyMetadata: ILanguageModelChatMetadata = {
				id: historyModelId, name: 'History Model', vendor: 'singularity', version: '1.0', family: 'history',
		...
		test('restored draft keeps the history model while the live catalog is cold', async () => {
			const historyModelId = 'agent-host-singularitycli:gpt-5.6-sol';
			const historyMetadata: ILanguageModelChatMetadata = {
				id: historyModelId, name: 'GPT-5.6 Sol', vendor: 'agent-host-singularitycli', version: '1.0', family: 'gpt-5.6-sol',
				extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 8192, maxOutputTokens: 1024,
...
suite('backfillRestoredPickerState', () => {
	...
	const model = (identifier: string): ISerializableChatModelInputState['selectedModel'] => ({
		identifier,
		metadata: {
			id: identifier, name: identifier, vendor: 'singularity', version: '1.0', family: 'test',
			extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 8192, maxOutputTokens: 1024,
...
suite('backfillTransferredModel', () => {
	...
	test('backfills the history model when the transferred state dropped its model', () => {
		const history = model('agent-host-singularitycli:gpt-5.6-sol');
		const result = backfillTransferredModel(state(undefined), history);
		assert.strictEqual(result?.selectedModel?.identifier, 'agent-host-singularitycli:gpt-5.6-sol');
	});

	test('never overrides a model already present on the transferred state', () => {
		const result = backfillTransferredModel(state(model('agent-host-singularitycli:gpt-5.6-terra')), model('agent-host-singularitycli:gpt-5.6-sol'));
		assert.strictEqual(result?.selectedModel?.identifier, 'agent-host-singularitycli:gpt-5.6-terra');
	});
	...
	test('returns undefined state as-is', () => {
		assert.strictEqual(backfillTransferredModel(undefined, model('agent-host-singularitycli:gpt-5.6-sol')), undefined);
	});
});
