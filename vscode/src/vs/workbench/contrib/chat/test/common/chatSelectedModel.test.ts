...
suite('ChatSelectedModel', () => {
	...
	test('stores shared and targeted selections in profile user storage', () => {
		...
		storeSelectedModel(storage, ChatAgentLocation.Chat, undefined, 'singularity/shared');
		storeSelectedModel(storage, ChatAgentLocation.Chat, 'agent-host-singularitycli', 'singularity/targeted');

		assert.deepStrictEqual({
			sharedKey: getSelectedModelStorageKey(ChatAgentLocation.Chat),
			targetedKey: getSelectedModelStorageKey(ChatAgentLocation.Chat, 'agent-host-singularitycli'),
			shared: getStoredSelectedModel(storage, ChatAgentLocation.Chat),
			targeted: getStoredSelectedModel(storage, ChatAgentLocation.Chat, 'agent-host-singularitycli'),
			...
			targetedKey: 'chat.currentLanguageModel.panel.agent-host-singularitycli',
			shared: 'singularity/shared',
			targeted: 'singularity/targeted',
			profileUserKeys: [
				'chat.currentLanguageModel.panel',
				'chat.currentLanguageModel.panel.agent-host-singularitycli',
			],
			profileMachineKeys: [],
		});
	});
	...
	test('migrates the application-scoped selection to profile user storage', () => {
		const storage = disposables.add(new InMemoryStorageService());
		const key = getSelectedModelStorageKey(ChatAgentLocation.Chat, 'singularitycli');
		const isDefaultKey = `${key}.isDefault`;
		storage.store(key, 'singularity/legacy', StorageScope.APPLICATION, StorageTarget.USER);
		storage.store(isDefaultKey, false, StorageScope.APPLICATION, StorageTarget.USER);

		assert.deepStrictEqual({
			selection: getStoredSelectedModel(storage, ChatAgentLocation.Chat, 'singularitycli'),
			...
			selection: 'singularity/legacy',
			identifier: 'singularity/legacy',
	...
	test('drops a retired automatic profile selection', () => {
		...
		const isDefaultKey = `${key}.isDefault`;
		storage.store(key, 'singularity/selected', StorageScope.PROFILE, StorageTarget.USER);
	...
	test('preserves a retired explicit profile selection', () => {
		...
		assert.deepStrictEqual({
			...
			selection: 'singularity/selected',
			isDefault: undefined,
			profileUserKeys: [key],
		});
	});
