...
suite('promptsServiceUtils', () => {
	...
	suite('isOrganizationPromptFile', () => {
		const CHAT_EXTENSION_ID = 'singularity.chat-chat';
		...
		test('extension ID comparison is case-insensitive', () => {
			...
			const extensionId = new ExtensionIdentifier('GITHUB.SINGULARITY-CHAT');
			const productService = createProductService('singularity.chat-chat');
