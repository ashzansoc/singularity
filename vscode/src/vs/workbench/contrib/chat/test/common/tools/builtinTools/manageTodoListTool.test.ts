...
suite('ManageTodoListTool Schema', () => {
	...
	test('createManageTodoListToolData todoList items have correct required fields', () => {
		...
		assert.ok('content' in properties, 'Schema should have content property');
		assert.ok('status' in properties, 'Schema should have status property');
		assert.deepStrictEqual(required, ['id', 'status'], 'Required fields should be id, status');
	});
	...
	test('createManageTodoListToolData status has correct enum values', () => {
		...
		assert.deepStrictEqual(statusProperty.enum, ['not-started', 'in-progress', 'completed', 'pending', 'in_progress'], 'Status should have correct enum values');
	});
});
