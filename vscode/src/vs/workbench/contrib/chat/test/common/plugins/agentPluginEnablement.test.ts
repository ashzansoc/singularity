...
suite('AgentPlugin enablement', () => {
	...
	test('colliding marketplace and Singularity CLI direct installs use priority-ordered enablement', () => {
		...
		const singularityCliDirectUri = URI.file('/Users/test/.singularity/installed-plugins/_direct/microsoft--vscode-team-kit--model-council');
		const discoveries: IDiscoveredAgentPlugins[] = [
			{
				priority: AgentPluginDiscoveryPriority.SingularityCli,
				order: 3,
				plugins: [makePlugin(singularityCliDirectUri, 'model-council')],
			},
			{
		...
		enablementModel.setEnabled(singularityCliDirectUri.toString(), ContributionEnablementState.EnabledWorkspace);
		...
		assert.deepStrictEqual({
			...
			singularityCliInitiallyEnabled: isContributionEnabled(new AgentPluginCollisionEnablementModel(makeTestEnablementModel(), collisionGroups).readEnabled(singularityCliDirectUri.toString())),
			marketplaceAfterEnablingCli: enablementModel.readEnabled(marketplaceUri.toString()),
			singularityCliAfterEnablingCli: enablementModel.readEnabled(singularityCliDirectUri.toString()),
		}, {
			plugins: [singularityCliDirectUri.toString(), marketplaceUri.toString()],
			marketplaceInitiallyEnabled: true,
			singularityCliInitiallyEnabled: false,
			marketplaceAfterEnablingCli: ContributionEnablementState.DisabledWorkspace,
			singularityCliAfterEnablingCli: ContributionEnablementState.EnabledWorkspace,
		});
	});
	...
	test('policy-blocked duplicate does not suppress allowed duplicate', () => {
		...
		assert.ok(isContributionEnabled(enablementModel.readEnabled(singularityCliDirectUri.toString())));
	});

	test('same-URI duplicates collapse before collision grouping', () => {
		const sharedUri = URI.file('/Users/test/.singularity/installed-plugins/team/model-council');
		const discoveries: IDiscoveredAgentPlugins[] = [
			{
	...
	suite('isAgentPluginBlockedByPolicy', () => {
		...
		test('additive: a plugin the policy never mentions is not blocked', () => {
			...
			assert.strictEqual(isAgentPluginBlockedByPolicy(plugin, { 'workiq@singularity-plugins': true }), false);
		});
