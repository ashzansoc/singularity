...
import { SINGULARITY_VENDOR_ID, ILanguageModelChatMetadata, ILanguageModelChatMetadataAndIdentifier, ILanguageModelsService } from '../../../../common/languageModels.js';
...
suite('RunSubagentTool', () => {
	...
	suite('model fallback behavior', () => {
		const BUILTIN_CHAT_EXTENSION_ID = 'singularity.chat-chat';
		...
		function createMetadata(name: string, multiplierNumeric?: number, vendor: string = 'TestVendor'): ILanguageModelChatMetadata {
			return {
				...
				isBYOK: vendor !== SINGULARITY_VENDOR_ID,
			};
		}
		...
		test('skips Singularity fallback models when main model is BYOK and inherits the main model', async () => {
			const mainMeta = createMetadata('Claude Sonnet BYOK', undefined, 'anthropic');
			const singularityFallback = createMetadata('Singularity Haiku', undefined, SINGULARITY_VENDOR_ID);
			const models = new Map([
				['main-byok-id', mainMeta],
				['singularity-fallback-id', singularityFallback],
			]);
			const qualifiedNameMap = new Map([
				['Singularity Haiku (singularity)', { metadata: singularityFallback, identifier: 'singularity-fallback-id' }],
			]);

			const agent = createBuiltinAgent('ExploreAgent', ['Singularity Haiku (singularity)']);
			const tool = createTool({ models, qualifiedNameMap, customAgents: [agent] });
			...
			assert.ok(result);
			// The Singularity fallback is skipped, so the subagent inherits the BYOK main model.
			assert.deepStrictEqual(result.toolSpecificData, {
				kind: 'subagent',
				description: 'test task',
		...
		test('skips Singularity fallback but uses a non-Singularity fallback when main model is BYOK', async () => {
			const mainMeta = createMetadata('Claude Sonnet BYOK', undefined, 'anthropic');
			const singularityFallback = createMetadata('Singularity Haiku', undefined, SINGULARITY_VENDOR_ID);
			...
			// Singularity fallback is listed first, the BYOK fallback second.
			const agent = createBuiltinAgent('ExploreAgent', ['Singularity Haiku (singularity)', 'Ollama Llama (ollama)']);
			const tool = createTool({ models, qualifiedNameMap, customAgents: [agent] });
		...
		test('uses the Singularity fallback model when the main model is also Singularity', async () => {
			const mainMeta = createMetadata('Singularity GPT-4o', undefined, SINGULARITY_VENDOR_ID);
			const singularityFallback = createMetadata('Singularity Haiku', undefined, SINGULARITY_VENDOR_ID);
			const models = new Map([
				['main-singularity-id', mainMeta],
				['singularity-fallback-id', singularityFallback],
			]);
			const qualifiedNameMap = new Map([
				['Singularity Haiku (singularity)', { metadata: singularityFallback, identifier: 'singularity-fallback-id' }],
			...
			assert.deepStrictEqual(result.toolSpecificData, {
				...
				agentName: 'ExploreAgent',
				prompt: 'test',
				modelName: 'Singularity Haiku',
			});
		});

		test('uses the Singularity fallback model when no main model is set', async () => {
			const singularityFallback = createMetadata('Singularity Haiku', undefined, SINGULARITY_VENDOR_ID);
			const models = new Map([
				['singularity-fallback-id', singularityFallback],
			]);
		...
		test('honors a user-authored agent\'s explicit Singularity model even when main model is BYOK', async () => {
			const mainMeta = createMetadata('Claude Sonnet BYOK', undefined, 'anthropic');
			const singularityPinned = createMetadata('Singularity Sonnet', undefined, SINGULARITY_VENDOR_ID);
			const models = new Map([
				['main-byok-id', mainMeta],
				['singularity-pinned-id', singularityPinned],
			]);
			const qualifiedNameMap = new Map([
				['Singularity Sonnet (singularity)', { metadata: singularityPinned, identifier: 'singularity-pinned-id' }],
			]);

			// A user-authored (local) agent that deliberately pins a Singularity model — must not be skipped.
			const agent = createAgent('MyAgent', ['Singularity Sonnet (singularity)']);
			const tool = createTool({ models, qualifiedNameMap, customAgents: [agent] });
	...
	suite('subagent credits', () => {
		...
		function createCreditTool(usageParts: IChatProgress[], result: IChatAgentResult = {}) {
			...
			const configService = new TestConfigurationService();
			const promptsService = new MockPromptsService();
			const parentCredits: { subagentCallId: string; singularityCredits: number }[] = [];
				...
				getSession() {
					return {
						getRequests: () => [{
							id: 'req-1',
							response: {
								setSubagentSingularityCredits: (subagentCallId: string, singularityCredits: number) => parentCredits.push({ subagentCallId, singularityCredits }),
							},
						}],
						acceptResponseProgress: () => { },
					} as unknown as IChatModel;
		...
		test('writes the running credit total onto the subagent toolSpecificData', async () => {
			...
			const { tool, parentCredits } = createCreditTool([
				{ kind: 'usage', promptTokens: 10, completionTokens: 5, singularityCredits: 2 },
				{ kind: 'usage', promptTokens: 20, completionTokens: 8, singularityCredits: 5 },
				{ kind: 'usage', promptTokens: 20, completionTokens: 8, singularityCredits: 3 },
			]);
			...
			assert.deepStrictEqual({
				...
				toolCredits: 5,
				parentCredits: [{ subagentCallId: invocation.callId, singularityCredits: 5 }],
			});
		});

		test('records credits when the subagent fails after reporting usage', async () => {
			const { tool, parentCredits } = createCreditTool(
				[{ kind: 'usage', promptTokens: 10, completionTokens: 5, singularityCredits: 3 }],
			...
			assert.deepStrictEqual({
				...
				toolCredits: 3,
				parentCredits: [{ subagentCallId: invocation.callId, singularityCredits: 3 }],
			});
		});
