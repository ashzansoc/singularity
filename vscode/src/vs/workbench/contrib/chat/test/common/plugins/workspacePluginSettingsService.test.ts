...
suite('WorkspacePluginSettingsService', () => {
	...
	async function writeSingularitySettings(content: string): Promise<void> {
		const uri = URI.from({ scheme: Schemas.inMemory, path: '/workspace/.github/singularity/settings.json' });
		await fileService.writeFile(uri, VSBuffer.fromString(content));
	}
	...
	test('merges enabledPlugins from Claude and Singularity settings', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		await writeClaudeSettings(JSON.stringify({
			enabledPlugins: { 'from-claude@mp': true }
		}));
		await writeSingularitySettings(JSON.stringify({
			enabledPlugins: { 'from-singularity@mp': true }
		}));
		...
		assert.strictEqual(enabled.get('from-singularity@mp'), true);
	}));

	test('Claude enabledPlugins take precedence over Singularity for same key', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		await writeClaudeSettings(JSON.stringify({
	...
	test('deduplicates marketplaces across Claude and Singularity by canonical ID', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		await writeClaudeSettings(JSON.stringify({
			extraKnownMarketplaces: {
		...
		await writeSingularitySettings(JSON.stringify({
			extraKnownMarketplaces: {
				'singularity-name': { source: 'github', repo: 'owner/repo' }
			}
		}));
