...
import { ChatMessageRole, LanguageModelsService, IChatMessage, IChatResponsePart, ILanguageModelChatMetadata, createModelConfigurationActions, ILanguageModelConfigurationSchema, getByokProviderTelemetryName, THIRD_PARTY_PROVIDER_TELEMETRY_NAME, SINGULARITY_VENDOR_ID, getLanguageModelDisplayNameWithProvider, ILanguageModelChatMetadataAndIdentifier, ILanguageModelsService } from '../../common/languageModels.js';
...
suite('LanguageModels', function () {
	...
	test('BYOK display names use provider and optional configured group paths', function () {
		...
		const bridgedModel: ILanguageModelChatMetadataAndIdentifier = {
			identifier: 'agent-host-singularitycli:openrouter/amazon/nova-micro-v1',
			metadata: {
		...
		const nativeModel: ILanguageModelChatMetadataAndIdentifier = {
			identifier: 'agent-host-singularitycli:claude-sonnet-4.6',
			metadata: {
				...originalModel.metadata,
	...
	test('selectLanguageModels matches by id for singularity vendor models even when isUserSelectable is false', async function () {
		// Mirrors how the singularity extension publishes utility aliases such as
		// `singularity-utility-small`: under the `singularity` (default) vendor, with
		// `isUserSelectable: false`. The workbench's
		// `chatToolRiskAssessmentService` resolves them with
		// `selectLanguageModels({ vendor: 'singularity', id: 'singularity-utility-small' })`
		// and must get a match.
		languageModels.deltaLanguageModelChatProviderDescriptors([
			{ vendor: 'singularity', displayName: 'Singularity', configuration: undefined, managementCommand: undefined, when: undefined }
		], []);

		store.add(languageModels.registerLanguageModelProvider('singularity', {
			onDidChange: Event.None,
			provideLanguageModelChatInfo: async () => {
				const modelMetadata: ILanguageModelChatMetadata[] = [
					...
					},
					{
						...
						name: 'GPT 4o mini',
						vendor: 'singularity',
						family: 'singularity-utility-small',
						version: '2024-07-18',
						id: 'singularity-utility-small',
						maxInputTokens: 100,
						maxOutputTokens: 100,
						isDefaultForLocation: {},
		...
		const result = await languageModels.selectLanguageModels({ vendor: 'singularity', id: 'singularity-utility-small' });
		assert.deepStrictEqual(result, ['singularity/singularity-utility-small']);
	});
	...
	test('model visibility — hiding an agent-host group excludes BYOK model copies', async function () {
		...
		languageModels.deltaLanguageModelChatProviderDescriptors([
			{ vendor: 'agent-host-singularitycli', displayName: 'Singularity', configuration: undefined, managementCommand: undefined, when: undefined }
		], []);
		store.add(languageModels.registerLanguageModelProvider('agent-host-singularitycli', {
			onDidChange: Event.None,
			provideLanguageModelChatInfo: async () => [
				{
					metadata: {
						extension: nullExtensionDescription.identifier,
						name: 'Claude Haiku 4.5', vendor: 'agent-host-singularitycli', family: 'claude-haiku-4.5', version: '1.0',
						id: 'claude-haiku-4.5', maxInputTokens: 100, maxOutputTokens: 100, isDefaultForLocation: {},
						targetChatSessionType: 'agent-host-singularitycli', modelGroup: { id: 'singularitycli' },
					} satisfies ILanguageModelChatMetadata,
					identifier: 'agent-host-singularitycli:claude-haiku-4.5',
				},
				{
					metadata: {
						extension: nullExtensionDescription.identifier,
						name: 'AionLabs: Aion-3.0', vendor: 'agent-host-singularitycli', family: 'openrouter/aion-labs/aion-3.0', version: '1.0',
						id: 'openrouter/aion-labs/aion-3.0', maxInputTokens: 100, maxOutputTokens: 100, isDefaultForLocation: {},
						targetChatSessionType: 'agent-host-singularitycli', modelGroup: { id: 'openrouter' },
						byokModelIdentifier: 'openrouter/OpenRouter 2/aion-labs/aion-3.0',
					} satisfies ILanguageModelChatMetadata,
					identifier: 'agent-host-singularitycli:openrouter/aion-labs/aion-3.0',
				},
			],
			sendChatRequest: async () => { throw new Error(); },
			provideTokenCount: async () => { throw new Error(); },
		}));
		await languageModels.selectLanguageModels({ vendor: 'agent-host-singularitycli' });

		languageModels.setGroupHidden('agent-host-singularitycli', 'Singularity', true);
		...
		assert.deepStrictEqual({
			hiddenModels: languageModels.getHiddenModelIds(),
			groupHidden: languageModels.isGroupHidden('agent-host-singularitycli', 'Singularity'),
			byokCopyHidden: languageModels.isModelHidden('agent-host-singularitycli:openrouter/aion-labs/aion-3.0'),
		}, {
			hiddenModels: ['agent-host-singularitycli:claude-haiku-4.5'],
			groupHidden: true,
...
suite('LanguageModels - provider usage telemetry', function () {
	...
	test('getByokProviderTelemetryName classifies vendors', function () {
		const singularityExtension = new ExtensionIdentifier('singularity.chat-chat');
		const thirdPartyExtension = new ExtensionIdentifier('publisher.third-party');
		assert.deepStrictEqual(
			[
				getByokProviderTelemetryName(undefined, singularityExtension),
				getByokProviderTelemetryName(SINGULARITY_VENDOR_ID, singularityExtension),
				getByokProviderTelemetryName('openai', singularityExtension),
				getByokProviderTelemetryName('ollama', singularityExtension),
	...
	test('sendChatRequest reports an in-built BYOK provider by name', async function () {
		const events = await sendRequestForVendor('openai', new ExtensionIdentifier('singularity.chat-chat'), true);
	...
	test('sendChatRequest does not report first-party Singularity models', async function () {
		const events = await sendRequestForVendor(SINGULARITY_VENDOR_ID, new ExtensionIdentifier('singularity.chat-chat'));
