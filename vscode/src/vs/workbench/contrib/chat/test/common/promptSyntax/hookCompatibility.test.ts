...
import { parseSingularityHooks, parseHooksFromFile, HookSourceFormat } from '../../../common/promptSyntax/hookCompatibility.js';
import { URI } from '../../../../../../base/common/uri.js';

suite('HookCompatibility', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	suite('parseSingularityHooks', () => {
		...
		suite('basic parsing', () => {
			test('parses simple hook with command', () => {
				...
				const result = parseSingularityHooks(json, workspaceRoot, userHome);
		...
		suite('invalid inputs', () => {
			test('returns empty result for null json', () => {
				const result = parseSingularityHooks(null, workspaceRoot, userHome);
				assert.strictEqual(result.size, 0);
			});

			test('returns empty result for undefined json', () => {
				const result = parseSingularityHooks(undefined, workspaceRoot, userHome);
				assert.strictEqual(result.size, 0);
			});

			test('returns empty result for missing hooks property', () => {
				const result = parseSingularityHooks({}, workspaceRoot, userHome);
				assert.strictEqual(result.size, 0);
			});
		});
		...
		suite('Claude-style matcher compatibility', () => {
			test('parses Claude-style nested matcher structure', () => {
				// When Claude format is pasted into Singularity hooks file
				const json = {
					hooks: {
						PreToolUse: [
							{
	...
	suite('parseHooksFromFile', () => {
		...
		test('uses Singularity format for .github/hooks/*.json files', () => {
			...
			assert.strictEqual(result.format, HookSourceFormat.Singularity);
		...
		test('disableAllHooks is ignored for Singularity format', () => {
			...
			// Singularity format does not support disableAllHooks
			assert.strictEqual(result.disabledAllHooks, false);
			assert.strictEqual(result.hooks.size, 1);
		});
