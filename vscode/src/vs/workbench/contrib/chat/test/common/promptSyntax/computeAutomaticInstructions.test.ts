...
suite('ComputeAutomaticInstructions', () => {
	...
	suite('collect', () => {
		test('should collect all types of instructions', async () => {
			...
			]);
			{
				...
				assert.ok(paths.includes(`${rootFolder}/.github/singularity-instructions.md`), 'Should include singularity-instructions');
			...
			{
				testConfigService.setUserConfiguration(PromptsConfig.INCLUDE_APPLYING_INSTRUCTIONS, true);
				testConfigService.setUserConfiguration(PromptsConfig.USE_SINGULARITY_INSTRUCTION_FILES, false);
				testConfigService.setUserConfiguration(PromptsConfig.USE_AGENT_MD, true);
				const contextComputer = instaService.createInstance(ComputeAutomaticInstructions, ChatModeKind.Agent, undefined, undefined, localSessionType);
				const variables = new ChatRequestVariableSet();
				variables.add(toFileVariableEntry(URI.joinPath(rootFolderUri, 'src/file.ts')));
				...
				assert.ok(paths.includes(`${rootFolder}/.github/instructions/typescript.instructions.md`), 'Should include applying instruction');
				assert.ok(!paths.includes(`${rootFolder}/.github/singularity-instructions.md`), 'Should not include singularity-instructions');
				assert.ok(paths.includes(`${rootFolder}/AGENTS.md`), 'Should include AGENTS.md');
			}
			{
				testConfigService.setUserConfiguration(PromptsConfig.INCLUDE_APPLYING_INSTRUCTIONS, true);
	...
	suite('telemetry', () => {
		test('should emit telemetry event with counts', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: `${rootFolder}/.github/singularity-instructions.md`,
					contents: ['Singularity instructions'],
				},
				{
					path: `${rootFolder}/AGENTS.md`,
	...
	suite('instructions list variable', () => {
		...
		test('should include skills from home folder in skills list', async () => {
			...
			assert.equal(xmlContents(skills[0], 'file')[0], getFilePath(`/home/user/.singularity/skills/personal-skill/SKILL.md`));
	...
	test('should collect parent folder singularity-instructions.md and AGENTS.md when includeWorkspaceFolderParents is enabled', async () => {
		...
		await mockFiles(fileService, [
			...
			{
				path: `${parentFolder}/.github/singularity-instructions.md`,
				contents: ['Parent singularity instructions'],
			},
			{
		...
		let paths = disabledParentVariables.asArray()
			.filter(v => isPromptFileVariableEntry(v))
			.map(i => isPromptFileVariableEntry(i) ? i.value.path : undefined);
		assert.ok(!paths.includes(`${parentFolder}/.github/singularity-instructions.md`), 'Should not include parent singularity-instructions.md when parent search is disabled');
		...
		paths = enabledParentVariables.asArray()
			.filter(v => isPromptFileVariableEntry(v))
			.map(i => isPromptFileVariableEntry(i) ? i.value.path : undefined);
		assert.ok(paths.includes(`${parentFolder}/.github/singularity-instructions.md`), 'Should include parent singularity-instructions.md when parent search is enabled');
	...
	test('should collect ~/.singularity/singularity-instructions.md when enabled', async () => {
		const rootFolderName = 'collect-singularity-home-test';
		const rootFolder = `/${rootFolderName}`;
		const rootFolderUri = URI.file(rootFolder);
		...
		await mockFiles(fileService, [
			{
				path: `/home/user/.singularity/singularity-instructions.md`,
				contents: [
					'Singularity guidelines from home',
				]
			},
			{
				path: `${rootFolder}/src/file.ts`,
		...
		let instructionFiles = variables.asArray().filter(v => isPromptFileVariableEntry(v));
		let paths = instructionFiles.map(i => isPromptFileVariableEntry(i) ? i.value.path : undefined);
		assert.ok(paths.includes(`/home/user/.singularity/singularity-instructions.md`), 'Should include ~/.singularity/singularity-instructions.md when enabled');
		...
		instructionFiles = variables2.asArray().filter(v => isPromptFileVariableEntry(v));
		paths = instructionFiles.map(i => isPromptFileVariableEntry(i) ? i.value.path : undefined);
		assert.ok(!paths.includes(`/home/user/.singularity/singularity-instructions.md`), 'Should not include ~/.singularity/singularity-instructions.md when disabled');
	});
	...
	test('should filter symlinks', async () => {
		...
		const singularityUri = URI.joinPath(rootFolderUri, '.github/singularity-instructions.md');
		...
		// AGENTS.md and CLAUDE.md are symlinks to singularity
		fileSystemProvider.setRealPath(agentMdUri, singularityUri);
		fileSystemProvider.setRealPath(claudeMdUri, singularityUri);
		...
		// singularity-instructions.md should be included
		// AGENTS.md should be skipped as link to singularity
		// CLAUDE.md should be skipped as link to singularity
		assert.strictEqual(instructionFiles.length, 1, 'Should include 1 files (singularity)');
		assert.ok(paths.includes(singularityUri.path), 'Should include singularity-instructions.md');
		assert.ok(!paths.includes(agentMdUri.path), 'Should not include AGENTS.md (symlink to singularity)');
		assert.ok(!paths.includes(claudeMdUri.path), 'Should not include CLAUDE.md (symlink to singularity)');
	});
});
...
suite('getFilePath', () => {
	...
	test('should not convert file:// URIs to vscode-local:/ when not connected to a remote', () => {
		const uri = URI.file('/home/user/.singularity/agent-plugins/my-skill/SKILL.md');
