...
suite('AgentPlugin format detection', () => {
	...
	test('falls back to Singularity format when no vendor manifest exists', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		const uri = pluginUri('/plugins/my-singularity-plugin');
		await writeFile('/plugins/my-singularity-plugin/plugin.json', JSON.stringify({ name: 'my-singularity-plugin' }));
		await writeFile('/plugins/my-singularity-plugin/commands/run.md', '# Run');
	...
	test('Singularity Plugin MCP servers expand root aliases and default cwd to plugin root', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		const uri = pluginUri('/plugins/singularity-mcp-root');
		await writeFile('/plugins/singularity-mcp-root/plugin.json', JSON.stringify({ name: 'singularity-mcp-root' }));
		await writeFile('/plugins/singularity-mcp-root/.mcp.json', JSON.stringify({
			mcpServers: {
				'singularity-server': {
					command: '${PLUGIN_ROOT}/bin/server',
		...
		const defaultCwdConfig = servers.get('singularity-server');
