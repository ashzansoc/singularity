...
import { getRememberedSessionType, hasPreferredSingularityHarness, markPreferredSingularityHarness } from '../../common/chatSessionTypePreference.js';
...
suite('ChatConfiguration defaults', () => {
	...
	test('editor default returns local when agent host disabled and local enabled', () => {
		...
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity);
	...
	test('editor default prefers agent host Singularity when the agent host is enabled', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.DefaultToSingularityHarness]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity);
		const storageService = disposables.add(new TestStorageService());
		...
		assert.deepStrictEqual({
			...
			computed: SessionType.AgentHostSingularity,
			rememberedAware: SessionType.AgentHostSingularity,
			localVisible: true,
		});
	});

	test('editor default stays local when the agent host is enabled but the Singularity default is not opted in', () => {
		const configurationService = new TestConfigurationService();
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity);
		const storageService = disposables.add(new TestStorageService());

		// The agent host is enabled but `chat.defaultToSingularityHarness` is off (its
	...
	test('editor default keeps agent host Singularity before contribution registers', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.DefaultToSingularityHarness]: true,
			[ChatConfiguration.EditorLocalAgentEnabled]: false,
		});
		const chatSessionsService = createChatSessionsService(SessionType.SingularityCLI);
		const storageService = disposables.add(new TestStorageService());
	...
	test('editor default skips hidden extension host Singularity CLI', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.EditorLocalAgentEnabled]: false,
			[ChatConfiguration.SingularityCliHideExtensionHostEditor]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.SingularityCLI, SessionType.AgentHostSingularity);
		const storageService = disposables.add(new TestStorageService());

		assert.deepStrictEqual({
			computed: getComputedDefaultSessionType(configurationService, chatSessionsService, localWorkspace, false),
			rememberedAware: getDefaultNewChatSessionType(configurationService, chatSessionsService, storageService, localWorkspace, false),
			extensionHostVisible: isVisibleEditorChatSessionType(SessionType.SingularityCLI, configurationService, chatSessionsService, localWorkspace),
		}, {
			computed: SessionType.AgentHostSingularity,
			rememberedAware: SessionType.AgentHostSingularity,
	...
	test('hidden remembered extension host Singularity CLI falls back for a new chat', async () => {
		const configurationService = new TestConfigurationService();
		const chatSessionsService = createChatSessionsService(SessionType.SingularityCLI, SessionType.AgentHostSingularity);
		const storageService = disposables.add(new TestStorageService());

		recordUserSelectedSessionType(storageService, configurationService, chatSessionsService, localWorkspace, SessionType.SingularityCLI, true);
		await configurationService.setUserConfiguration(ChatConfiguration.SingularityCliHideExtensionHostEditor, true);

		assert.deepStrictEqual({
			remembered: getRememberedSessionType(storageService),
			rememberedUsable: isNewChatSessionTypeUsable(SessionType.SingularityCLI, configurationService, chatSessionsService, localWorkspace),
			newSessionType: getDefaultNewChatSessionType(configurationService, chatSessionsService, storageService, localWorkspace, true),
		}, {
			remembered: SessionType.SingularityCLI,
			rememberedUsable: false,
			newSessionType: localChatSessionType,
		});
	});

	test('hidden current extension host Singularity CLI is not inherited by a new chat', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.SingularityCliHideExtensionHostEditor]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.SingularityCLI, SessionType.AgentHostSingularity);
		...
		assert.deepStrictEqual(
			resolveSessionType(configurationService, chatSessionsService, storageService, localWorkspace, true, { currentSessionType: SessionType.SingularityCLI }),
			{ sessionType: localChatSessionType, isPreferSingularityHarnessSwap: false }
		);
	});

	test('visible current extension host Singularity CLI is inherited by a new chat', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.DefaultToSingularityHarness]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.SingularityCLI, SessionType.AgentHostSingularity);
		...
		assert.deepStrictEqual(
			resolveSessionType(configurationService, chatSessionsService, storageService, localWorkspace, true, { currentSessionType: SessionType.SingularityCLI }),
			{ sessionType: SessionType.SingularityCLI, isPreferSingularityHarnessSwap: false }
		);
	});
	...
	test('remembered non-local selection wins over the agent host default', () => {
		...
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity, SessionType.AgentHostClaude);
		const storageService = disposables.add(new TestStorageService());
		...
		assert.deepStrictEqual({
			...
			rememberedAware: { sessionType: SessionType.AgentHostClaude, isPreferSingularityHarnessSwap: false },
		});
	});
	...
	test('explicit override wins over remembered selection', () => {
		...
		assert.deepStrictEqual({
			remembered: getRememberedSessionType(storageService),
			rememberedAware: getDefaultNewChatSessionType(configurationService, chatSessionsService, storageService, localWorkspace, false, { explicitOverride: SessionType.AgentHostSingularity }),
		}, {
			remembered: SessionType.AgentHostClaude,
			rememberedAware: SessionType.AgentHostSingularity,
		});
	...
	test('current session type is fallback after remembered selection', () => {
		...
		assert.deepStrictEqual({
			withoutRemembered: getDefaultNewChatSessionType(configurationService, chatSessionsService, storageService, localWorkspace, false, { currentSessionType: SessionType.AgentHostSingularity }),
		}, {
			withoutRemembered: SessionType.AgentHostSingularity,
		});

		recordUserSelectedSessionType(storageService, configurationService, chatSessionsService, localWorkspace, SessionType.AgentHostClaude, false);

		assert.deepStrictEqual({
			withRemembered: getDefaultNewChatSessionType(configurationService, chatSessionsService, storageService, localWorkspace, false, { currentSessionType: SessionType.AgentHostSingularity }),
		}, {
			withRemembered: SessionType.AgentHostClaude,
		});
	});

	test('preferSingularityHarness resolves the swap without consuming the marker until applied', () => {
		// DefaultToSingularityHarness stays unset so this proves the one-time swap
		// fires solely because EditorPreferSingularityHarness is enabled, independent
		// of the new default gate.
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.EditorPreferSingularityHarness]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity, SessionType.AgentHostClaude);
		const storageService = disposables.add(new TestStorageService());
		...
		const markerBeforeApply = hasPreferredSingularityHarness(storageService);
		...
		markPreferredSingularityHarness(storageService);
		...
		assert.deepStrictEqual({
			...
			afterApply,
			markerAfterApply: hasPreferredSingularityHarness(storageService),
		}, {
			firstResolve: { sessionType: SessionType.AgentHostSingularity, isPreferSingularityHarnessSwap: true },
			markerBeforeApply: false,
			secondResolveBeforeApply: { sessionType: SessionType.AgentHostSingularity, isPreferSingularityHarnessSwap: true },
			// Once marked, the one-time swap no longer fires; with no remembered
			// selection the current local session type is returned.
			afterApply: { sessionType: localChatSessionType, isPreferSingularityHarnessSwap: false },
			markerAfterApply: true,
		});
	});

	test('one-time Singularity swap is skipped and unmarked when the agent host is disabled', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.EditorPreferSingularityHarness]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity);
		...
		// neither returns an unresolvable Singularity type nor marks the transition.
		...
		assert.deepStrictEqual({
			resolved,
			preferenceApplied: hasPreferredSingularityHarness(storageService),
		}, {
			resolved: { sessionType: localChatSessionType, isPreferSingularityHarnessSwap: false },
	...
	test('selecting computed default clears remembered selection', () => {
		...
		recordUserSelectedSessionType(storageService, configurationService, chatSessionsService, localWorkspace, SessionType.AgentHostClaude, true);
		recordUserSelectedSessionType(storageService, configurationService, chatSessionsService, localWorkspace, SessionType.AgentHostSingularity, true);
	...
	test('selecting local while the agent host default is Singularity remembers local as an opt-out', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.DefaultToSingularityHarness]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity);
		...
		// With the agent host enabled the computed default is Singularity, so picking
	...
	test('one-time Singularity swap overrides a remembered local opt-out and stays redundant when agent host is enabled', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.DefaultToSingularityHarness]: true,
			[ChatConfiguration.EditorPreferSingularityHarness]: true,
		});
		...
		// Remember local (only reachable because the computed default is Singularity).
		recordUserSelectedSessionType(storageService, configurationService, chatSessionsService, localWorkspace, localChatSessionType, true);

		// The `remembered !== local` guard lets the one-time swap replace the
		// remembered local, even though the computed default is already Singularity.
		...
		assert.deepStrictEqual({
			...
			swapped: { sessionType: SessionType.AgentHostSingularity, isPreferSingularityHarnessSwap: true },
			preferenceApplied: false,
		});
	});

	test('new chat from a local session preserves local even when the agent host default is Singularity', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.DefaultToSingularityHarness]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity);
		...
		// session type wins over the Singularity computed default (session preservation).
		assert.deepStrictEqual({
	...
	test('explicit New Local Chat wins over a non-local current session even when the agent host default is Singularity', () => {
		const configurationService = new TestConfigurationService({
			[ChatConfiguration.DefaultToSingularityHarness]: true,
		});
		const chatSessionsService = createChatSessionsService(SessionType.AgentHostSingularity);
		...
		// "New Local Chat" from a Singularity session must resolve to local: the explicit
		...
		assert.deepStrictEqual({
			resolved: resolveSessionType(configurationService, chatSessionsService, storageService, localWorkspace, true, { explicitOverride: localChatSessionType, currentSessionType: SessionType.AgentHostSingularity }),
		}, {
			resolved: { sessionType: localChatSessionType, isPreferSingularityHarnessSwap: false },
		});
	});
	...
	test('default session resource follows the agent host default', () => {
		...
		assert.deepStrictEqual({
			...
			computedWithAgentHost: SessionType.AgentHostSingularity,
			computedWithoutAgentHost: localChatSessionType,
			defaultNewWithAgentHost: SessionType.AgentHostSingularity,
	...
	test('virtual workspace defaults implicit new chats to local', () => {
		...
		assert.deepStrictEqual({
			...
			currentAware: getDefaultNewChatSessionType(configurationService, chatSessionsService, currentStorageService, workspace, true, { currentSessionType: SessionType.AgentHostSingularity }),
			resolvedRemembered: resolveSessionType(configurationService, chatSessionsService, rememberedStorageService, workspace, true, { currentSessionType: SessionType.AgentHostSingularity }),
			resolvedCurrent: resolveSessionType(configurationService, chatSessionsService, currentStorageService, workspace, true, { currentSessionType: SessionType.AgentHostSingularity }),
			...
			resolvedRemembered: { sessionType: localChatSessionType, isPreferSingularityHarnessSwap: false },
			resolvedCurrent: { sessionType: localChatSessionType, isPreferSingularityHarnessSwap: false },
			resolvedPreferMigration: { sessionType: localChatSessionType, isPreferSingularityHarnessSwap: false },
			explicitOverride: { sessionType: SessionType.AgentHostClaude, isPreferSingularityHarnessSwap: false },
			localVisible: true,
	...
	test('remembered agent host is usable before contribution registers', () => {
		...
		assert.deepStrictEqual({
			...
			agentHost: true,
			agentHostCurrent: { sessionType: SessionType.AgentHostClaude, isPreferSingularityHarnessSwap: false },
