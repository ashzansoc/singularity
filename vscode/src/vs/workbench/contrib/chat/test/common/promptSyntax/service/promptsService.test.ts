...
import { SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG, SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_CONFIG } from '../../../../../../../platform/policy/common/singularityManagedSettings.js';
...
suite('PromptsService', () => {
	...
	setup(async () => {
		...
		testConfigService.setUserConfiguration(PromptsConfig.USE_SINGULARITY_INSTRUCTION_FILES, true);
	...
	suite('voice instructions', () => {
		test('combines user and trusted workspace voice.md files', async () => {
			...
			await mockFiles(fileService, [
				{ path: '/home/user/.singularity/voice.md', contents: ['Use short paragraphs.'] },
		...
		test('cancels in-flight voice instruction reads', async () => {
			...
			try {
				...
				await readStarted.p;
				cts.cancel();
				assert.strictEqual(await instructions, undefined);
			} finally {
	...
	suite('dictation instructions', () => {
		test('combines user and trusted workspace dictation.md files separately from voice.md', async () => {
			...
			workspaceContextService.setWorkspace(testWorkspace(rootFolderUri));
			await mockFiles(fileService, [
				{ path: '/home/user/.singularity/dictation.md', contents: ['Use short paragraphs.'] },
				{ path: '/workspace/.github/dictation.md', contents: ['Spell the product name as Contoso DB.'] },
				{ path: '/home/user/.singularity/voice.md', contents: ['Keep spoken responses concise.'] },
			]);
	...
	suite('findInstructionFilesFor', () => {
		...
		test('singularity-instructions and AGENTS.md', async () => {
			const rootFolderName = 'singularity-instructions-and-agents';
			const rootFolder = `/${rootFolderName}`;
			const rootFolderUri = URI.file(rootFolder);
			...
			await mockFiles(fileService, [
				...
				{
					path: `${rootFolder}/.github/singularity-instructions.md`,
					contents: [
			...
			assert.deepStrictEqual(
				...
				[
					URI.joinPath(rootFolderUri, '.github/singularity-instructions.md').path,
			...
			await mockFiles(fileService, [
				{
					...
					contents: [
						'---',
						'description: \'GitHub Singularity specialized agent.\'',
						'target: \'github-singularity\'',
						'tools: [ github-api, code-search ]',
						'---',
						'I am optimized for Singularity workflows.',
					]
				},
				{
			...
			const expected: ICustomAgent[] = [
				{
					...
					name: 'github-agent',
					description: 'Singularity specialized agent.',
					target: Target.Singularity,
					tools: ['github-api', 'code-search'],
					agentInstructions: {
						content: 'I am optimized for Singularity workflows.',
						toolReferences: [],
						metadata: undefined
					},
					handOffs: undefined,
		...
		test('claude agent maps tools and model to vscode equivalents', async () => {
			...
			await mockFiles(fileService, [
				...
				},
				{
					...
					path: `${rootFolder}/.github/agents/singularity-agent.agent.md`,
					contents: [
						'---',
						'description: \'Singularity agent with same tool names.\'',
						'target: \'github-singularity\'',
						'tools: [ Read, Edit ]',
						'model: gpt-4',
						'---',
						'I am a Singularity agent.',
					]
				},
			]);
			...
			const expected: ICustomAgent[] = [
				{
					id: URI.joinPath(rootFolderUri, '.github/agents/singularity-agent.agent.md').toString(),
					...
					model: ['gpt-4'],
					agentInstructions: {
						content: 'I am a Singularity agent.',
						toolReferences: [],
						metadata: undefined
					},
					handOffs: undefined,
					...
					agents: undefined,
					hooks: undefined,
					uri: URI.joinPath(rootFolderUri, '.github/agents/singularity-agent.agent.md'),
					sessionTypes: undefined,
					source: { storage: PromptsStorage.local },
					enabled: true,
				},
				{
					...
					model: ['Claude Opus 4.6 (singularity)'],
					agentInstructions: {
				...
				},
				{
					...
					model: ['Claude Sonnet 4.5 (singularity)'],
					agentInstructions: {
	...
	suite('listPromptFiles - skills ', () => {
		...
		test('should list skill files from user home', async () => {
			...
			await mockFiles(fileService, [
				{
					path: '/home/user/.singularity/skills/personal-skill/SKILL.md',
					contents: [
						'---',
						'name: "Personal Skill"',
			...
			const singularitySkill = personalSkills.find(s => s.uri.path.includes('.singularity'));
			assert.ok(singularitySkill, 'Should find singularity personal skill');
	...
	suite('getSourceFolders - skills', () => {
		test('includes user-level skill source folders', async () => {
			...
			assert.ok(
				userFolders.some(f => f.uri.path === '/home/user/.singularity/skills'),
				'Should include ~/.singularity/skills as a user source folder'
			);
		});
		...
		test('excludes defaults explicitly disabled via configuration', async () => {
			...
			assert.ok(!paths.includes('/home/user/.singularity/skills'), 'Disabled ~/.singularity/skills must not appear');
	...
	suite('listPromptFiles - extensions', () => {
		...
		test('Contributed file sessionTypes metadata is preserved in core prompt models', async () => {
			...
			const sessionTypes = ['singularitycli'];
	...
	suite('findAgentSkills', () => {
		...
		test('should find skills in workspace and user home', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: '/home/user/.singularity/skills/Singularity Skill 1/SKILL.md',
					contents: [
						'---',
						'name: "Singularity Skill 1"',
						'description: "A Singularity skill for testing"',
						'---',
						'This is Singularity skill 1 content',
					],
				},
			]);
			...
			const singularitySkill1 = personalSkills.find(skill => skill.name === 'Singularity Skill 1');
			assert.ok(singularitySkill1, 'Should find Singularity Skill 1');
			assert.strictEqual(singularitySkill1.description, 'A Singularity skill for testing');
			assert.strictEqual(singularitySkill1.uri.path, '/home/user/.singularity/skills/Singularity Skill 1/SKILL.md');
		});
		...
		test('should skip duplicate skill names and keep first by priority', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: '/home/user/.singularity/skills/Duplicate Skill/SKILL.md',
					contents: [
						'---',
						'name: "Duplicate Skill"',
		...
		test('should prioritize skills by source: workspace > user > extension', async () => {
			...
			await mockFiles(fileService, [
				{
					path: '/home/user/.singularity/skills/Priority Skill/SKILL.md',
					contents: [
						'---',
						'name: "Priority Skill"',
	...
	suite('customization lockdown', () => {
		test('policy changes invalidate cached standalone agent locations', async () => {
			...
			testConfigService.setUserConfiguration(SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_CONFIG, true);
			fireConfigChange(testConfigService, SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_CONFIG);
		...
		test('managed-only hooks preserve frontmatter hooks from force-enabled plugin agents', async () => {
			testConfigService.setUserConfiguration(PromptsConfig.USE_CHAT_HOOKS, true);
			testConfigService.setUserConfiguration(SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG, true);
			const pluginUri = URI.file('/home/user/.singularity/installed-plugins/managed-marketplace/managed-plugin');
			...
			testPluginsObservable.set([plugin], undefined);
			fireConfigChange(testConfigService, SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG, ChatConfiguration.EnabledPlugins);
	...
	suite('hooks', () => {
		...
		test('managed-only hooks block standalone and unmanaged plugin hooks', async function () {
			...
			testConfigService.setUserConfiguration(PromptsConfig.HOOKS_LOCATION_KEY, { [HOOKS_SOURCE_FOLDER]: true });
			testConfigService.setUserConfiguration(SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG, true);
			fireConfigChange(testConfigService, SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG);
			await mockFiles(fileService, [{
