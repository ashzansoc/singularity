...
suite('ChatModel', () => {
	...
	test('a refinement of the same model call updates usage without recounting its tokens', () => {
		...
		model.acceptResponseProgress(request, { kind: 'usage', promptTokens: 10, completionTokens: 2, singularityCredits: 1, sessionSingularityCredits: 1 });
		model.acceptResponseProgress(request, { kind: 'usage', promptTokens: 10, completionTokens: 2, singularityCredits: 1, sessionSingularityCredits: 5 });

		assert.deepStrictEqual({
			sessionSingularityCredits: request.response?.usage?.sessionSingularityCredits,
			completionTokenCount: request.response?.completionTokenCount,
		}, {
			sessionSingularityCredits: 5,
	...
	test('subagent credits are folded into parent response usage', () => {
		...
		request.response?.setSubagentSingularityCredits('subagent-1', 5);
		model.acceptResponseProgress(request, { kind: 'usage', promptTokens: 10, completionTokens: 2, singularityCredits: 2 });
		request.response?.setSubagentSingularityCredits('subagent-1', 6);
		request.response?.setSubagentSingularityCredits('subagent-1', 4);
		request.response?.setSubagentSingularityCredits('subagent-2', 3);
		request.response?.setSubagentSingularityCredits('invalid', Number.NaN);
		request.response?.setSubagentSingularityCredits('invalid', -1);
		...
		assert.deepStrictEqual({ usage: request.response?.usage, completionTokenCount: request.response?.completionTokenCount, sessionCost: model.sessionCost }, {
			usage: { kind: 'usage', promptTokens: 10, completionTokens: 2, singularityCredits: 11 },
			completionTokenCount: 2,
			sessionCost: 11,
		});
	...
	test('the session total and the summed turns each provide a floor for session cost', () => {
		...
		model.acceptResponseProgress(first, { kind: 'usage', promptTokens: 10, completionTokens: 2, singularityCredits: 2 });
		...
		model.acceptResponseProgress(second, { kind: 'usage', promptTokens: 10, completionTokens: 2, singularityCredits: 3, sessionSingularityCredits: 9 });
		...
		model.acceptResponseProgress(third, { kind: 'usage', promptTokens: 10, completionTokens: 2, singularityCredits: 6 });
	...
	test('restores legacy top-level modelConfiguration into selectedModel (backwards compat)', async () => {
		...
		const legacyInputState = {
			...
			selectedModel: { identifier: 'singularity/gpt', metadata: { name: 'GPT' } },
	...
	test('MCP tool authentication marks the response as needing input', () => {
		...
		const toolInvocation = {
			...
			state: observableValue<any>('state', {
				...
				server: { id: 'server', name: 'GitHub MCP', resource: 'https://api.githubsingularity.com/mcp' },
				cancel: () => { },
			}),
		} as Partial<IChatToolInvocation> as IChatToolInvocation;
...
suite('serializeSendOptions', () => {
	...
	test('preserves request-scoped options through persist/restore', () => {
		...
		const serialized = serializeSendOptions({
			userSelectedModelId: 'singularity/gpt',
