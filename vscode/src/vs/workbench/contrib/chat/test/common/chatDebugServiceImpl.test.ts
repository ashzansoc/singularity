...
suite('ChatDebugServiceImpl', () => {
	...
	const singularityCliSession = URI.parse('singularitycli:/test-session-id');
	...
	suite('addEvent and getEvents', () => {
		...
		test('should handle different event kinds', () => {
			...
			const modelTurn: IChatDebugModelTurnEvent = {
				...
				outputTokens: 50,
				totalTokens: 150,
				singularityUsageNanoAiu: 5_000_000_000,
			...
			assert.strictEqual((events[1] as IChatDebugModelTurnEvent).singularityUsageNanoAiu, 5_000_000_000);
		});
	});
	...
	suite('log', () => {
		...
		test('should log events for singularitycli sessions', () => {
			const firedEvents: IChatDebugEvent[] = [];
			disposables.add(service.onDidAddEvent(e => firedEvents.push(e)));

			service.log(singularityCliSession, 'cli-event', 'details');

			assert.strictEqual(firedEvents.length, 1);
			assert.strictEqual(service.getEvents(singularityCliSession).length, 1);
		});
	...
	suite('invokeProviders', () => {
		...
		test('should invoke providers for singularitycli sessions', async () => {
			let providerCalled = false;
			...
			const provider: IChatDebugLogProvider = {
				provideChatDebugLog: async () => {
					...
					return [{
						kind: 'generic',
						sessionResource: singularityCliSession,
						created: new Date(),
			...
			disposables.add(service.registerProvider(provider));
			await service.invokeProviders(singularityCliSession);

			assert.strictEqual(providerCalled, true);
			assert.ok(service.getEvents(singularityCliSession).length > 0);
		});
