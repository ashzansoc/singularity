...
suite('ModelSelection', () => {
	...
	test('uses shared vendor readiness for empty and live catalogs', () => {
		const resolvedVendors = new Set(['singularity', 'ollama']);
		...
		const emptySingularity = resolveModelIdentifierFromCatalog([], 'singularity/remembered', vendorResolution);
		const emptyByok = resolveModelIdentifierFromCatalog([], 'ollama/remembered', vendorResolution);
		liveVendors.add('singularity');
		const liveSingularity = resolveModelIdentifierFromCatalog([], 'singularity/remembered', vendorResolution);

		assert.deepStrictEqual({ emptySingularity, emptyByok, liveSingularity }, {
			emptySingularity: { kind: 'pending', identifier: 'singularity/remembered' },
			emptyByok: { kind: 'unavailable', identifier: 'ollama/remembered' },
			liveSingularity: { kind: 'unavailable', identifier: 'singularity/remembered' },
		});
	});
	...
	test('treats a resolved-but-empty agent-host vendor as still loading (pending)', () => {
		...
		// like Singularity — an empty resolution during startup is transient (pending), not conclusive.
		...
		const resolvedVendors = new Set(['agent-host-singularitycli', 'remote-abc-singularitycli']);
		const liveVendors = new Set<string>();
		const vendorResolution = {
			hasLiveModels: (vendor: string) => liveVendors.has(vendor),
			hasResolved: (vendor: string) => resolvedVendors.has(vendor),
		};
		const localDesired = 'agent-host-singularitycli:gpt-5.6-sol';
		const remoteDesired = 'remote-abc-singularitycli:gpt-5.6-sol';
		...
		liveVendors.add('agent-host-singularitycli');
	...
	test('treats an agent-host pool of only bridged BYOK models as still loading (pending)', () => {
		...
		const desired = hostModel('agent-host-singularitycli:gpt-5.6-sol');
		const bridged = hostModel('agent-host-singularitycli:openrouter/ai21/jamba-large-1.7', 'openrouter/OpenRouter/ai21/jamba-large-1.7');
		...
		assert.deepStrictEqual({
			...
			ownModelsPublishedWithout: resolve([bridged, hostModel('agent-host-singularitycli:auto')]),
		}, {
	...
	test('restores, waits for, and repairs existing-session models', () => {
		assert.deepStrictEqual([
			...
			current: undefined, pending: { reference: 'target:missing' }, effect: 'clear', applied: undefined, reason: ModelSelectionReason.SessionRestore, lastPushedChatKey: 'chat:one',
		}, {
	...
	test('uses the same new-conversation policy for configured, remembered, pending, and fallback models', () => {
		assert.deepStrictEqual([
			...
			current: undefined, pending: { reference: second.identifier }, effect: 'none', applied: undefined, reason: undefined, lastPushedChatKey: 'chat:previous',
		}, {
			current: first.identifier, pending: undefined, effect: 'apply', applied: first.identifier, reason: ModelSelectionReason.FirstAvailable, lastPushedChatKey: 'chat:one',
		}]);
	});
