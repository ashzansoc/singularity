...
suite('HookSourceFormat', () => {
	...
	suite('getHookSourceFormat', () => {
		...
		test('returns Singularity format for .github/hooks/hooks.json', () => {
			assert.strictEqual(getHookSourceFormat(URI.file('/workspace/.github/hooks/hooks.json')), HookSourceFormat.Singularity);
		});

		test('returns Singularity format for arbitrary .json file', () => {
			assert.strictEqual(getHookSourceFormat(URI.file('/workspace/.github/hooks/my-hooks.json')), HookSourceFormat.Singularity);
		});

		test('returns Singularity format for settings.json not inside .claude', () => {
			assert.strictEqual(getHookSourceFormat(URI.file('/workspace/.vscode/settings.json')), HookSourceFormat.Singularity);
		});
	});

	suite('buildNewHookEntry', () => {
		test('builds Singularity format entry', () => {
			assert.deepStrictEqual(buildNewHookEntry(HookSourceFormat.Singularity), {
				type: 'command',
				command: ''
			});
		});
		...
		test('Singularity format entry serializes correctly in JSON', () => {
			const entry = buildNewHookEntry(HookSourceFormat.Singularity);
			const hooksContent = {
				hooks: {
					SubagentStart: [entry]
				}
