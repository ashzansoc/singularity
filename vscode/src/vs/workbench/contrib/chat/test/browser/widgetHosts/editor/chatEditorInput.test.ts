...
suite('ChatEditorInput', () => {
	...
	test('new chat replaces a hidden current Singularity CLI harness', async () => {
		...
		chatSessionsService.setContributions([{
			type: SessionType.SingularityCLI,
			name: 'Singularity CLI',
			displayName: 'Singularity CLI',
		...
		const input = store.add(instantiationService.createInstance(
			ChatEditorInput,
			URI.from({ scheme: SessionType.SingularityCLI, path: '/session' }),
			{},
		));
		...
		try {
			...
			assert.deepStrictEqual({
				...
				currentSessionType: SessionType.SingularityCLI,
