...
suite('AgentSessions', () => {
	...
	suite('AgentSessionsViewModel - Provider Icons and Names', () => {
		...
		test('should return correct icon for Background provider', () => {
			...
			assert.strictEqual(icon.id, Codicon.singularity.id);
		});
		...
		test('should return correct icon for AgentHostSingularity provider', () => {
			const icon = getAgentSessionProviderIcon(AgentSessionProviders.AgentHostSingularity);
			assert.strictEqual(icon.id, Codicon.vm.id);
		});

		test('should return simplified AgentHostSingularity name', () => {
			const name = getAgentSessionProviderName(AgentSessionProviders.AgentHostSingularity);
			assert.strictEqual(name, 'Singularity');
		});
		...
		test('should handle Background provider type in model', async () => {
			return runWithFakedTimers({}, async () => {
				...
				assert.strictEqual(session.icon.id, Codicon.singularity.id);
	...
	suite('AgentSessionsViewModel - getAgentCanContinueIn', () => {
		...
		test('should return true for the Singularity agent host provider', () => {
			const result = getAgentCanContinueIn(AgentSessionProviders.AgentHostSingularity);
			assert.strictEqual(result, true);
		});
		...
		test('should return true for dynamically registered agent host session types', () => {
			...
			assert.strictEqual(getAgentCanContinueIn('remote-myauthority-singularity'), true);
		});
