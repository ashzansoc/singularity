...
suite('agentHostMcpServerId', () => {
	...
	test('is stable for the same authority, server name and resource url', () => {
		...
		const a = agentHostMcpServerId('remote-host', 'GitHub', 'https://api.githubsingularity.com/mcp/');
		const b = agentHostMcpServerId('remote-host', 'GitHub', 'https://api.githubsingularity.com/mcp/');
		assert.strictEqual(a, b);
		assert.strictEqual(a, 'agent-host-mcp:remote-host/GitHub/https%3A%2F%2Fapi.githubsingularity.com%2Fmcp%2F');
	});
...
suite('resolveMcpServerAuthentication', () => {
	...
	test('does not eagerly request GitHub MCP supported scopes', async () => {
		...
		const result = await instantiationService.invokeFunction(resolveMcpServerAuthentication, {
			...
			mcpServerUrl: 'https://api.githubsingularity.com/mcp',
			scopes: [],
			authenticate: async () => { },
		});
