...
suite('ChatPlanReviewPart', () => {
	...
	function createWidget(review: IChatPlanReview, dialogService?: TestDialogService, onSubmit?: () => void): ChatPlanReviewPart {
		...
		instantiationService.stub(IPlanTodoSyncService, new class extends mock<IPlanTodoSyncService>() {
			override onDidChangePlanUri = VSCodeEvent.None;
			override bindPlanUri(): void { }
			override getPlanUri(): undefined { return undefined; }
			override syncPlanToChatTodos(): void { }
			override notifyPlanFileChanged(): void { }
			override async syncChatTodosToPlanFile(): Promise<void> { }
			override async syncPlanFileToChatTodos(): Promise<void> { }
		}());
