...
suite('ChatInputNotificationWidget', () => {
	...
	test('reactively applies session type filter when pending delegation target changes', () => {
		...
		currentSessionType.set(SessionType.AgentHostSingularity, undefined);
	...
	test('reactively applies session resource filter when the session changes', () => {
		const firstSession = URI.parse('vscode-chat-session://agent-host-singularitycli/session-1');
		const secondSession = URI.parse('vscode-chat-session://agent-host-singularitycli/session-2');
		const currentSessionType = observableValue<string | undefined>('currentSessionType', SessionType.AgentHostSingularity);
	...
	test('auto-dismiss on message only applies to the sending session', () => {
		...
		notificationService.handleMessageSent({ sessionType: SessionType.AgentHostSingularity, sessionResource: firstSession });
	...
	test('matches Agent Host notifications against the resource scheme', () => {
		const sessionResource = URI.from({ scheme: 'agent-host-singularitycli', path: '/untitled-session' });
		const { notificationService, widget } = createWidget({
		...
		showNotification(notificationService, {
			...
			actions: [],
			sessionTypes: ['agent-host-singularitycli'],
		});
	...
	test('announces only the notification rendered in the current session', () => {
		...
		// A promo scoped to the Singularity harness must not be announced while the
		// input is in the local session.
		notificationService.set({
			id: 'singularity-promo',
			severity: ChatInputNotificationSeverity.Info,
			message: 'Singularity promo',
			description: undefined,
			actions: [],
			dismissible: true,
			autoDismissOnMessage: false,
			sessionTypes: [SessionType.AgentHostSingularity],
		});
		assert.strictEqual(lastAnnounced(), undefined, 'nothing should be announced in a non-matching session');

		currentSessionType.set(SessionType.AgentHostSingularity, undefined);
		assert.strictEqual(lastAnnounced()?.id, 'singularity-promo', 'the promo should be announced once its session is active');
	});
});
