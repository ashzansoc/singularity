...
function createModel(id: string, name: string, vendor = 'singularity'): ILanguageModelChatMetadataAndIdentifier {
	return {
...
function createAutoModel(): ILanguageModelChatMetadataAndIdentifier {
	return createModel('auto', 'Auto', 'singularity');
}
...
function createAgentHostModel(id: string, name: string, modelGroup: { id: string; sourceId?: string }): ILanguageModelChatMetadataAndIdentifier {
	const vendor = 'agent-host-singularitycli';
	return {
		identifier: `${vendor}:${id}`,
		metadata: {
			id,
...
suite('buildModelPickerItems', () => {
	...
	test('accessibility provider includes inline source and right-aligned multiplier', () => {
		...
		assert.strictEqual(provider.getAriaLabel({
			...
			badge: 'Singularity',
			description: '15x',
		} as IActionListItem<IActionWidgetDropdownAction>), 'Claude Opus 4.7, Singularity, 15x');
	});
	...
	test('setupRequired shows an explanatory header and a Sign In action instead of auto', () => {
		...
		const actions = getActionItems(items);
		assert.ok(items.some(i => i.kind === ActionListItemKind.Header && i.label === 'Sign in to use Singularity'));
		assert.strictEqual(actions.length, 1);
	...
	test('model variants sharing metadata ids remain visible across promoted and other sections', () => {
		...
		const items = callBuild([auto, singularitySol, singularityTerra, singularityOther, byokSol, byokTerra, byokOther], {
			recentModelIds: [singularitySol.identifier, byokSol.identifier, singularityTerra.identifier, byokTerra.identifier],
		...
		assert.deepStrictEqual(getActionItems(items)
			...
			{ id: singularitySol.identifier, section: undefined, provider: 'Singularity' },
			{ id: byokSol.identifier, section: undefined, provider: 'OpenAI (Work)' },
			{ id: singularityTerra.identifier, section: undefined, provider: 'Singularity' },
			{ id: singularityOther.identifier, section: 'other', provider: undefined },
	...
	test('available targeted models remain featured while entitlement is signed out', () => {
		const auto = createAgentHostModel('auto', 'Auto', { id: 'singularitycli' });
		const freeFeatured = createAgentHostModel('free-model', 'Free Model', { id: 'singularitycli' });
		const paidFeatured = createAgentHostModel('paid-model', 'Paid Model', { id: 'singularitycli' });
		const other = createAgentHostModel('other-model', 'Other Model', { id: 'singularitycli' });
	...
	test('signed-out control models exclude unavailable and BYOK models', () => {
		...
		const availableTargeted = createAgentHostModel('available-targeted', 'Available Targeted', { id: 'singularitycli' });
	...
	test('Other Models grouped by vendor with separator headers', () => {
		const auto = createAutoModel();
		const modelA = createModel('zebra', 'Zebra', 'singularity');
		const modelB = createModel('alpha', 'Alpha', 'other-vendor');
		const modelC = createModel('beta', 'Beta', 'singularity');
		const items = callBuild([auto, modelA, modelB, modelC]);
		...
		assert.strictEqual(vendorSeparators[0].label, 'Singularity');
	...
	test('single vendor group omits vendor separator header', () => {
		const auto = createAutoModel();
		const modelA = createModel('gpt-4o', 'GPT-4o', 'singularity');
		const modelB = createModel('claude', 'Claude', 'singularity');
		const items = callBuild([auto, modelA, modelB]);
	...
	test('Other Models splits agent-host models into sections by their modelGroup and labels singularitycli as Singularity', () => {
		...
		const auto = createAutoModel();
		const cli = createAgentHostModel('claude-haiku-4.5', 'Claude Haiku 4.5', { id: 'singularitycli' });
		...
		const service = createLanguageModelsServiceStub([
			{ vendor: 'singularitycli', displayName: 'Singularity CLI', groups: [] },
		...
		assert.deepStrictEqual(labelledSeparators.map(s => s.label), ['Singularity', 'Hugging Face', 'OpenAI']);
	});
	...
	test('Other Models resolves a trusted source label without a synthetic vendor descriptor', () => {
		store.add(languageModelSourcePresentationRegistry.register({
			ownerVendor: 'agent-host-singularitycli',
		...
		assert.deepStrictEqual(labelledSeparators.map(s => s.label), ['ChatGPT', 'Singularity']);
	});
	...
	test('Other Models respects the configured BYOK group name for agent-host models', () => {
		...
		assert.deepStrictEqual(labelledSeparators.map(s => s.label), ['Singularity', 'GoogleBYOK']);
	});
	...
	test('Other Models keeps a single section when agent-host models share one modelGroup', () => {
		const auto = createAutoModel();
		const a = createAgentHostModel('claude-haiku-4.5', 'Claude Haiku 4.5', { id: 'singularitycli' });
		const b = createAgentHostModel('gpt-5', 'GPT-5', { id: 'singularitycli' });
		const service = createLanguageModelsServiceStub([{ vendor: 'singularitycli', displayName: 'Singularity CLI', groups: [] }]);
	...
	test('admin unavailable model shows manage settings link in description', () => {
		...
		assert.ok(description.value.includes('https://aka.ms/github-singularity-settings'));
	});
	...
	test('promoted models show inline vendor label when multiple vendors exist across all models', () => {
		...
		assert.strictEqual(gptItem.badge, 'Singularity');
	...
	test('vendor detail is suppressed in Other Models when multiple vendor groups shown', () => {
		const auto = createAutoModel();
		const modelA = createModel('gpt-4o', 'GPT-4o', 'singularity');
		modelA.metadata = { ...modelA.metadata, detail: 'Singularity' } as ILanguageModelChatMetadata;
		const modelB = createModel('claude', 'Claude', 'anthropic');
	...
	test('pinned models are grouped by provider and sorted alphabetically', () => {
		const auto = createAutoModel();
		const singularityAlpha = createAgentHostModel('singularity-alpha', 'Alpha', { id: 'singularitycli' });
		const singularityZeta = createAgentHostModel('singularity-zeta', 'Zeta', { id: 'singularitycli' });
		...
		const items = callBuild([auto, singularityZeta, openRouterZeta, singularityAlpha, openRouterAlpha], {
			pinnedModelIds: [openRouterZeta.identifier, singularityZeta.identifier, openRouterAlpha.identifier, singularityAlpha.identifier],
			languageModelsService,
		});
		...
		assert.deepStrictEqual(pinnedItems.map(item => ({ provider: item.badge, name: item.label })), [
			{ provider: 'Singularity', name: 'Alpha' },
			{ provider: 'Singularity', name: 'Zeta' },
 ...
 * make the model visible. This rule applies uniformly to the singularity vendor
...
suite('chat model picker - languageModelChatProvider visibility regression', () => {
	...
	function createSingularityModel(
		id: string,
		name: string,
	...
	test('regression: third-party model with isUserSelectable: false is hidden from the picker', () => {
		...
		// This applies uniformly to singularity and third-party vendors.
	...
	test('regression: singularity internal model (isUserSelectable: false) is hidden from the picker', () => {
		const internal = createSingularityModel('internal', 'Internal', { isUserSelectable: false });
		const lmService = buildLmService([
			{ vendor: 'singularity', displayName: 'Singularity', modelIdentifiers: [internal.identifier] },
		]);
		...
		assert.deepStrictEqual(
			labels,
			[],
			'Internal singularity models marked isUserSelectable: false must remain hidden from the picker.',
		);
	});

	test('regression: singularity model with omitted isUserSelectable defaults to visible', () => {
		// `isUserSelectable` defaults to `true` for every vendor, so a singularity
		// model that omits the flag is now treated as user-selectable.
		const model = createSingularityModel('public', 'Public', { isUserSelectable: undefined });
		const lmService = buildLmService([
			{ vendor: 'singularity', displayName: 'Singularity', modelIdentifiers: [model.identifier] },
		]);
	...
	test('regression: singularity public model (isUserSelectable: true) is shown in the picker', () => {
		const pub = createSingularityModel('gpt-4o', 'GPT-4o', { isUserSelectable: true });
		const lmService = buildLmService([
			{ vendor: 'singularity', displayName: 'Singularity', modelIdentifiers: [pub.identifier] },
		]);
	...
	test('regression: mixed vendors - only explicit isUserSelectable: false models are hidden', () => {
		const singularityPublic = createSingularityModel('gpt-4o', 'GPT-4o', { isUserSelectable: true });
		const singularityInternal = createSingularityModel('internal', 'Internal', { isUserSelectable: false });
		...
		const lmService = buildLmService([
			{
				...
				modelIdentifiers: [singularityPublic.identifier, singularityInternal.identifier],
			},
			{
				vendor: 'my-vendor',
		...
		const labels = runPickerPipeline(
			[singularityPublic, singularityInternal, tpTrue, tpFalse, tpUndefined],
			lmService,
		).map(i => i.label).sort();
