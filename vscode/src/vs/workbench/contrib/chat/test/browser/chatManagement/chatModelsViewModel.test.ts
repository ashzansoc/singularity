...
class MockLanguageModelsService implements ILanguageModelsService {
	...
	getVendors(): ILanguageModelProviderDescriptor[] {
		return this.vendors.map(v => ({ ...v, isDefault: v.vendor === 'singularity' }));
	}
...
suite('ChatModelsViewModel', () => {
	...
	setup(async () => {
		...
		languageModelsService.addModel('singularity', 'singularity-gpt-4', {
			extension: new ExtensionIdentifier('singularity.chat'),
			id: 'gpt-4',
			name: 'GPT-4',
			family: 'gpt-4',
			version: '1.0',
		...
		languageModelsService.addModel('singularity', 'singularity-gpt-4o', {
			extension: new ExtensionIdentifier('singularity.chat'),
			id: 'gpt-4o',
			name: 'GPT-4o',
			family: 'gpt-4',
	...
	test('should filter by provider name (vendor ID and display name)', () => {
		const resultsBySingularityId = viewModel.filter('@provider:singularity');
		assert.strictEqual(resultsBySingularityId.length, 3);
		assert.strictEqual(resultsBySingularityId[0].type, 'vendor');
		assert.strictEqual(resultsBySingularityId[0].vendorEntry.vendor.vendor, 'singularity');
		assert.strictEqual(resultsBySingularityId[1].type, 'model');
		assert.strictEqual(resultsBySingularityId[1].model.identifier, 'singularity-gpt-4');
		assert.strictEqual(resultsBySingularityId[2].type, 'model');
		assert.strictEqual(resultsBySingularityId[2].model.identifier, 'singularity-gpt-4o');
	...
	test('should filter by multiple providers with OR logic', () => {
		const results = viewModel.filter('@provider:singularity @provider:openai');
	...
	test('should combine provider and capability filters', () => {
		const results = viewModel.filter('@provider:singularity @capability:vision');

		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m =>
			m.model.provider.vendor.vendor === 'singularity' &&
			m.model.metadata.capabilities?.vision === true
		));
	});
	...
	test('should filter by text matching model id', () => {
		...
		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 1);
		assert.strictEqual(models[0].model.identifier, 'singularity-gpt-4o');
	...
	test('should filter by text matching vendor name', () => {
		...
		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m => m.model.provider.group.name === 'Singularity'));
	});
	...
	test('should toggle vendor collapsed state', () => {
		const vendorEntry = viewModel.viewModelEntries.find(r => isLanguageModelProviderEntry(r) && r.vendorEntry.vendor.vendor === 'singularity') as ILanguageModelProviderEntry;
		viewModel.toggleCollapsed(vendorEntry);

		const results = viewModel.filter('');
		const singularityVendor = results.find(r => isLanguageModelProviderEntry(r) && (r as ILanguageModelProviderEntry).vendorEntry.vendor.vendor === 'singularity') as ILanguageModelProviderEntry;
		assert.ok(singularityVendor);
		assert.strictEqual(singularityVendor.collapsed, true);

		// Models should not be shown when vendor is collapsed
		const singularityModelsAfterCollapse = results.filter(r =>
			!isLanguageModelProviderEntry(r) && (r as ILanguageModelEntry).model.provider.vendor.vendor === 'singularity'
		);
		assert.strictEqual(singularityModelsAfterCollapse.length, 0);

		// Toggle back
		viewModel.toggleCollapsed(vendorEntry);
		const resultsAfterExpand = viewModel.filter('');
		const singularityModelsAfterExpand = resultsAfterExpand.filter(r =>
			!isLanguageModelProviderEntry(r) && (r as ILanguageModelEntry).model.provider.vendor.vendor === 'singularity'
		);
		assert.strictEqual(singularityModelsAfterExpand.length, 2);
	});
	...
	test('should remove filter keywords from text search', () => {
		const results = viewModel.filter('@provider:singularity @capability:vision GPT');

		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		// Should only search 'GPT' in model names, not the filter keywords
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m => m.model.provider.vendor.vendor === 'singularity'));
	});
	...
	function createSingleVendorViewModel(includeSecondModel: boolean = true): { service: MockLanguageModelsService; viewModel: ChatModelsViewModel } {
		...
		service.addModel('singularity', 'singularity-gpt-4', {
			extension: new ExtensionIdentifier('singularity.chat'),
			id: 'gpt-4',
			name: 'GPT-4',
			family: 'gpt-4',
		...
		if (includeSecondModel) {
			service.addModel('singularity', 'singularity-gpt-4o', {
				extension: new ExtensionIdentifier('singularity.chat'),
				id: 'gpt-4o',
				name: 'GPT-4o',
				family: 'gpt-4',
	...
	test('should always place singularity vendor at the top when multiple vendors exist', async () => {
		// Test with default setup (singularity and openai)
		let results = viewModel.filter('');
		let vendors = results.filter(isLanguageModelProviderEntry) as ILanguageModelProviderEntry[];
		assert.strictEqual(vendors[0].vendorEntry.vendor.vendor, 'singularity');
	...
	test('should get configured vendors', () => {
		...
		assert.ok(vendors.length > 0);
		assert.ok(vendors.some(v => v.vendor.vendor === 'singularity'));
	...
	test('should filter by case-insensitive provider name', () => {
		const results1 = viewModel.filter('@provider:SINGULARITY');
		const results2 = viewModel.filter('@provider:singularity');
		const results3 = viewModel.filter('@provider:SingularITy');
	...
	test('should filter out agent-host BYOK model copies but keep native agent-host models', async () => {
		// An agent host (e.g. Singularity CLI) surfaces the user's own BYOK models as copies
		...
		const service = new MockLanguageModelsService();
		service.addVendor({ vendor: 'agent-host-singularitycli', displayName: 'Singularity', managementCommand: undefined, when: undefined, configuration: undefined });

		// Native agent-host model — no `byokModelIdentifier`; kept.
		service.addModel('agent-host-singularitycli', 'agent-host-singularitycli:claude-haiku-4.5', {
			...
			maxOutputTokens: 4096,
			isUserSelectable: true,
			targetChatSessionType: 'agent-host-singularitycli',
		...
		service.addModel('agent-host-singularitycli', 'agent-host-singularitycli:openrouter/aion-labs/aion-3.0', {
			extension: new ExtensionIdentifier('vscode.chat'),
