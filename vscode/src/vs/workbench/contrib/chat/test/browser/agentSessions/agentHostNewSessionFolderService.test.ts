...
suite('AgentHostNewSessionFolderService', () => {
	...
	const sessionA = URI.from({ scheme: 'agent-host-singularity', path: '/untitled-a' });
	const sessionB = URI.from({ scheme: 'agent-host-singularity', path: '/untitled-b' });
...
suite('computeWorkingDirectories', () => {
	...
	const advertised = rootStateWith('singularity', true);

	test('derives the ordered set from the chosen primary (capability advertised)', () => {
		assert.deepStrictEqual({
			noPrimary: toStrings(computeWorkingDirectories(undefined, [folderA, folderB, folderC], advertised, 'singularity')),
			singleFolder: toStrings(computeWorkingDirectories(folderA, [folderA], advertised, 'singularity')),
			primaryFirst: toStrings(computeWorkingDirectories(folderA, [folderA, folderB, folderC], advertised, 'singularity')),
			primaryMiddle: toStrings(computeWorkingDirectories(folderB, [folderA, folderB, folderC], advertised, 'singularity')),
			primaryOutsideWorkspace: toStrings(computeWorkingDirectories(folderC, [folderA, folderB], advertised, 'singularity')),
			noWorkspaceFolders: toStrings(computeWorkingDirectories(folderA, [], advertised, 'singularity')),
		}, {
			noPrimary: undefined,
	...
	test('restricts to the primary unless the provider advertises the capability', () => {
		...
		assert.deepStrictEqual({
			...
			advertised: toStrings(computeWorkingDirectories(folderB, [folderA, folderB, folderC], advertised, 'singularity')),
			...
			rootStateUndefined: toStrings(computeWorkingDirectories(folderB, [folderA, folderB, folderC], undefined, 'singularity')),
			// Root state errored → unsupported → primary only.
			rootStateError: toStrings(computeWorkingDirectories(folderB, [folderA, folderB, folderC], new Error('boom'), 'singularity')),
		}, {
