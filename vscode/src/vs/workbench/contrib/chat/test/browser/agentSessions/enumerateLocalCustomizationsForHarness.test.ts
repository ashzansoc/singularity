...
suite('enumerateLocalCustomizationsForHarness', () => {
	...
	test('emits built-in skills with BUILTIN_STORAGE even when no other storage source has files', async () => {
		...
		const result = await enumerateLocalCustomizationsForHarness(promptsService, new FakeSyncProvider(), SessionType.SingularityCLI, CancellationToken.None);
	...
	test('marks built-in skills disabled when the sync provider says so', async () => {
		...
		const result = await enumerateLocalCustomizationsForHarness(promptsService, syncProvider, SessionType.SingularityCLI, CancellationToken.None);
	...
	test('includes all user prompt types only when user storage is enabled', async () => {
		const userAgent = URI.file('/home/user/.singularity/agents/user.agent.md');
		...
		const localResult = await enumerateLocalCustomizationsForHarness(promptsService, new FakeSyncProvider(), SessionType.SingularityCLI, CancellationToken.None);
		const remoteResult = await enumerateLocalCustomizationsForHarness(promptsService, new FakeSyncProvider(), SessionType.SingularityCLI, CancellationToken.None, { includeUserStorage: true });
