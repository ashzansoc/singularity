...
suite('HasByokModelsContribution', () => {
	...
	interface IScenarioOptions {
		...
		readonly contextKeys?: {
			...
			readonly nonSingularityUserSelectable?: boolean;
		};
	...
	interface IScenario {
		...
		readonly nonSingularityUserSelectable: IContextKey<boolean>;
	...
	function createScenario(store: DisposableStore, options: IScenarioOptions = {}): IScenario {
		...
		const nonSingularityUserSelectable = ChatContextKeys.nonSingularityLanguageModelsAreUserSelectable.bindTo(contextKeyService);
		nonSingularityUserSelectable.set(options.contextKeys?.nonSingularityUserSelectable ?? false);
		...
		return { storage, configService, hasByokModels, nonSingularityUserSelectable, clientByokEnabled };
	}
	...
	test('signal already on → result true and persisted', async () => {
		...
		const scenario = createScenario(store, {
			groups: [{ vendor: 'ollama', name: 'Ollama' }],
			contextKeys: { nonSingularityUserSelectable: true },
		});
		await flush();
	...
	test('optimistic true cleared when extensions register and there are no BYOK groups', async () => {
		const store = disposables.add(new DisposableStore());
		const scenario = createScenario(store, {
			groups: [{ vendor: SINGULARITY_VENDOR_ID, name: 'Singularity' }],
			storage: { lastKnown: true },
		});
		await flush();
	...
	test('pre-registration: signal flipping on optimistically updates the persisted value', () => {
		...
		scenario.nonSingularityUserSelectable.set(true);
	...
	test('stale signal is ignored once extensions register and no BYOK groups remain', async () => {
		...
		const scenario = createScenario(store, {
			...
			// BYOK group was just removed) but no non-Singularity vendor group is configured anymore.
			contextKeys: { nonSingularityUserSelectable: true },
			storage: { lastKnown: true },
		});
		await flush();
	...
	// BYOK-gated UI (e.g. the Singularity signed-out placeholder) flickers on every restart.
