...
suite('ChatStatusDashboard', () => {
	...
	test('Enterprise Managed — PRU with credits used (compact): shows plan title, credits and reset', () => {
		...
		assert.strictEqual(indicator?.querySelector('.quota-title')?.textContent, 'Singularity Business');
		assert.strictEqual(credits?.value, '1,284');
		assert.strictEqual(credits?.suffix, 'Credits used');
	...
	test('Business — pooled exhausted (no overages): shows exhausted indicator and callout', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Your organization or enterprise has exceeded its Singularity budget. Contact your admin to resume usage.');
	});
	...
	test('Callout: shows paused when quota exhausted and overage not permitted', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Singularity is paused until the limit resets.');
	});
	...
	test('Callout: shows warning when quota >= 75% used and overage not permitted', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Singularity will pause when the limit is reached.');
	});
	...
	test('Callout: shows paused for enterprise when quota exhausted', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Singularity is paused until the limit resets. Contact your administrator for more information.');
	});
	...
	test('Callout: Enterprise — shows org-specific wording when approaching limit with additional usage', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Singularity will pause when your limits are reached. Please contact your admin to increase your limits.');
	});
	...
	test('Callout: Enterprise — shows org-specific wording when quota exhausted with additional usage', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Singularity has paused because your limits are reached. Please contact your admin to increase your limits.');
	});
