...
suite('ChatTipService', () => {
	...
	function createProductService(hasSingularity: boolean): IProductService {
		return {
			_serviceBrand: undefined,
			defaultChatAgent: hasSingularity ? { chatExtensionId: 'singularity.chat-chat' } : undefined,
		} as IProductService;
	}

	function createService(hasSingularity: boolean = true, tipsEnabled: boolean = true): ChatTipService {
		instantiationService.stub(IProductService, createProductService(hasSingularity));
	...
	test('returns Auto switch tip when current model is persisted and context key is empty', () => {
		storeSelectedModel(storageService, ChatAgentLocation.Chat, undefined, 'singularity/gpt-4.1-2025-04-14');
		const service = createService();
		contextKeyService.createKey(ChatContextKeys.chatModelId.key, '');
	...
	test('returns undefined when Singularity is not enabled', () => {
		const service = createService(/* hasSingularity */ false);

		const tip = service.getWelcomeTip(contextKeyService);
		assert.strictEqual(tip, undefined, 'Should not return a tip when Singularity is not enabled');
	});
	...
	test('returns undefined when tips setting is disabled', () => {
		const service = createService(/* hasSingularity */ true, /* tipsEnabled */ false);
	...
	test('excludes tip.customInstructions when singularity-instructions.md exists in workspace', async () => {
		const tip = createMockTip({
			id: 'tip.customInstructions',
			excludeWhenPromptFilesExist: { promptType: PromptsType.instructions, agentFileType: AgentInstructionFileType.singularityInstructionsMd, excludeUntilChecked: true },
		});
		...
		const tracker = testDisposables.add(new TipEligibilityTracker(
			...
			storageService,
			createMockPromptsService([{ uri: { path: '/.github/singularity-instructions.md' }, realPath: undefined, type: AgentInstructionFileType.singularityInstructionsMd } as IAgentInstructionFile]) as IPromptsService,
			createMockToolsService(),
			new NullLogService(),
		));
		...
		assert.strictEqual(tracker.isExcluded(tip), true, 'Should be excluded when singularity-instructions.md exists');
	});
