...
suite('ChatQuestionCarouselPart', () => {
	...
	suite('Used Carousel Summary', () => {
		...
		test('shows answered message when answeredExternally but no data', () => {
			...
			assert.ok(!summary?.querySelector('.codicon-singularity-compact'), 'Should not present a generic external answer as an automatic reply');
		});

		test('renders a Singularity icon for a structured automatic answer', () => {
			const carousel: IChatQuestionCarousel = {
				kind: 'questionCarousel',
				questions: [
			...
			assert.deepStrictEqual({
				...
				answerIcon: widget.domNode.querySelector('.chat-question-summary-answer-icon')?.classList.contains('codicon-singularity-compact'),
