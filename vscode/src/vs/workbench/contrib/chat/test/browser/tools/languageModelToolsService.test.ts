...
import { SingularityChatSettingId, SingularityToolId } from '../../../common/tools/singularityToolIds.js';
...
suite('LanguageModelToolsService', () => {
	...
	test('autopilot risk gate runs a non-red fetch web page call that has no confirmation', async () => {
		...
		const t = setupRiskGateTool(setup, store, { withConfirmation: false, toolId: 'singularity_fetchWebPage' });
	...
	test('gpt-5.5 readFile setting controls Singularity read tool availability', () => {
		const readTool: IToolData = {
			id: SingularityToolId.ReadFile,
		...
		const gpt55Model = { id: 'gpt-5.5', vendor: 'singularity', family: 'gpt-5.5', version: '1.0' } as ILanguageModelChatMetadata;

		configurationService.setUserConfiguration(SingularityChatSettingId.Gpt55ReadFileToolEnabled, false);

		const disabledTools = Array.from(service.getTools(gpt55Model));
		assert.ok(!disabledTools.some(tool => tool.id === SingularityToolId.ReadFile), 'readFile should not be returned from getTools when disabled for gpt-5.5');
		...
		assert.ok(!Array.from(disabledReadToolSet.getTools()).some(tool => tool.id === SingularityToolId.ReadFile), 'readFile should not be included as a read tool-set member when disabled for gpt-5.5');
		...
		configurationService.setUserConfiguration(SingularityChatSettingId.Gpt55ReadFileToolEnabled, true);

		const enabledTools = Array.from(service.getTools(gpt55Model));
		assert.ok(enabledTools.some(tool => tool.id === SingularityToolId.ReadFile), 'readFile should be returned from getTools when enabled for gpt-5.5');
		...
		assert.ok(Array.from(enabledReadToolSet.getTools()).some(tool => tool.id === SingularityToolId.ReadFile), 'readFile should be included as a read tool-set member when enabled for gpt-5.5');
