...
function createModel(id: string, name: string, vendor = 'singularity', metadata?: Partial<ILanguageModelChatMetadata>): ILanguageModelChatMetadataAndIdentifier {
	return {
...
suite('ModelProviderIcons', () => {
	...
	test('uses provider-specific icons', () => {
		assert.deepStrictEqual([
			...
			getModelProviderIcon(createModel('grok-code-fast-1', 'Grok Code Fast 1', 'agent-host-singularity')).id,
			...
			'chat-model-provider-singularity',
			'chat-model-provider-singularity',
			'chat-model-provider-generic',
			'chat-model-provider-generic',
			'chat-model-provider-generic',
