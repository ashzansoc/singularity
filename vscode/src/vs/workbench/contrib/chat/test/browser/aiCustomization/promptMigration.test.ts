...
suite('promptMigration', () => {
	...
	test('picks the matching storage from provider source folders', () => {
		...
		const skillRoots: ICustomizationSourceFolder[] = [
			...
			{ uri: URI.file('/home/test/.singularity/skills'), label: '~/.singularity/skills', source: PromptsStorage.user },
		];
		...
		assert.strictEqual(pickSkillSourceFolder({ ...promptFile, storage: PromptsStorage.user }, skillRoots)?.uri.toString(), URI.file('/home/test/.singularity/skills').toString());
	});
