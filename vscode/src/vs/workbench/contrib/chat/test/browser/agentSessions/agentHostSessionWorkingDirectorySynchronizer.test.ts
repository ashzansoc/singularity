...
suite('AgentHostSessionWorkingDirectorySynchronizer', () => {
	...
	const session = URI.parse('singularity:/session');
	...
	test('reconciles Singularity sessions through the provider-neutral capability', async () => {
		const synchronizer = createSynchronizer();
		const subscription = createSubscription();
		const connection = new TestConnection(subscription, 'singularitycli');
		disposables.add(synchronizer.register({ session, provider: 'singularitycli', connection, subscription }));
