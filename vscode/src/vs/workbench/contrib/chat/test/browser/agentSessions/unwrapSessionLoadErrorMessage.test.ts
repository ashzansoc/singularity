...
suite('unwrapSessionLoadErrorMessage', () => {
	...
	test('strips the restore wrapper and preserves the underlying cause', () => {
		const actual = {
			...
			wrapped: unwrapSessionLoadErrorMessage(new Error(`Failed to restore session singularitycli:/abc-123: This session couldn't be loaded because its worktree is missing and could not be recreated: git worktree exited with code 128: use 'add -f'`)),
