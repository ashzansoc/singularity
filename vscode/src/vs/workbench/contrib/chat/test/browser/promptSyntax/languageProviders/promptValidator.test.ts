...
suite('PromptValidator', () => {
	...
	setup(async () => {
		...
		const testModels: ILanguageModelChatMetadata[] = [
			...
			{ id: 'mae-4.1', name: 'MAE 4.1', vendor: 'singularity', version: '1.0', family: 'mae', extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 8192, maxOutputTokens: 1024, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: { [ChatAgentLocation.Chat]: true } } satisfies ILanguageModelChatMetadata,
			{ id: 'mae-3.5-turbo', name: 'MAE 3.5 Turbo', vendor: 'singularity', version: '1.0', family: 'mae', extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 8192, maxOutputTokens: 1024, isDefaultForLocation: { [ChatAgentLocation.Chat]: true } } satisfies ILanguageModelChatMetadata
		];
	...
	suite('agents', () => {
		...
		test('github-singularity agent with supported attributes', async () => {
			const content = [
				'---',
				'name: "GitHub_Singularity_Custom_Agent"',
				'description: "Singularity agent"',
			...
			const markers = await validate(content, PromptsType.agent);
			assert.deepStrictEqual(markers, [], 'Expected no validation issues for github-singularity target');
		});

		test('github-singularity agent warns about model and handoffs attributes', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "Singularity agent"',
			...
			assert.deepStrictEqual(messages, [
				'Attribute \'model\' is not supported in custom Singularity agent files. Supported: description, github, infer, mcp-servers, name, target, tools.',
				'Attribute \'handoffs\' is not supported in custom Singularity agent files. Supported: description, github, infer, mcp-servers, name, target, tools.',
			], 'Model and handoffs are not validated for github-singularity target');
		});

		test('github-singularity agent does not validate variable references', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "Singularity agent"',
			...
			const markers = await validate(content, PromptsType.agent);
			// Variable references should not be validated for github-singularity target
			assert.deepStrictEqual(markers, [], 'Variable references are not validated for github-singularity target');
		});

		test('github-singularity agent rejects unsupported attributes', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "Singularity agent"',
		...
		test('github-singularity agent with valid permissions', async () => {
			const content = [
				'---',
				'name: "IssueTriage"',
		...
		test('github-singularity agent with invalid permission scope', async () => {
			const content = [
				'---',
				'name: "TestAgent"',
				'description: "Test"',
		...
		test('github-singularity agent with invalid permission value', async () => {
			const content = [
				'---',
				'name: "TestAgent"',
				'description: "Test"',
		...
		test('github-singularity agent with non-map github attribute', async () => {
			const content = [
				'---',
				'name: "TestAgent"',
				'description: "Test"',
		...
		test('github-singularity agent with unknown github sub-property', async () => {
			const content = [
				'---',
				'name: "TestAgent"',
				'description: "Test"',
		...
		test('github-singularity target requires name attribute', async () => {
			// Missing name with github-singularity target
			{
				const content = [
					'---',
					'description: "Singularity agent"',
			...
			// Valid name with github-singularity target
			{
				const content = [
					'---',
					'name: "GitHubAgent"',
				...
				const markers = await validate(content, PromptsType.agent);
				assert.deepStrictEqual(markers, [], 'Valid github-singularity agent with name should not produce errors');
			}
	...
	suite('body', () => {
		...
		test('body with tool not present in tools list', async () => {
			const content = [
				...
				'---',
				'I need',
				'#tool:ms-azuretools.vscode-azure-github-singularity/azure_recommend_custom_modes',
			...
			assert.deepEqual(actual, [
				{ message: `Unknown extension tool 'ms-azuretools.vscode-azure-github-singularity/azure_recommend_custom_modes'. It is likely to be a missing extension, please ensure it is installed and enabled.`, startColumn: 7, endColumn: 77 },
