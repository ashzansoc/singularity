 ...
 * (e.g. `ollama/deepseek`). Mirrors how the language model registry qualifies non-Singularity models.
 */
...
suite('ChatInputModelUtils', () => {
	...
	suite('findBestMatchingModel', () => {
		...
		test('returns undefined when previous is undefined', () => {
			const pool = [createSessionModel('claude-sonnet-4.6', 'Claude Sonnet 4.6', 'agent-host-singularitycli')];
		...
		test('matches across vendors by raw model id (the issue #319583 case)', () => {
			// Previous selection from the in-extension singularitycli participant,
			// switching to the agent-host pool where the same model exists with
			// a different identifier/vendor.
			const prev = createModel('claude-sonnet-4.6', 'Claude Sonnet 4.6', { vendor: 'singularitycli', family: 'claude-sonnet-4.6' });
			const target = createSessionModel('claude-sonnet-4.6', 'Claude Sonnet 4.6', 'agent-host-singularitycli', { family: 'claude-sonnet-4.6' });
			const other = createSessionModel('gpt-5', 'GPT-5', 'agent-host-singularitycli', { family: 'gpt-5' });
		...
		test('matches by id even when family differs', () => {
			...
			const target = createSessionModel('claude-sonnet-4.6', 'Other Name', 'agent-host-singularitycli', { family: 'other' });
		...
		test('prefers id over family when both could match different pool entries', () => {
			...
			const prev = createModel('claude-sonnet-4.6', 'Claude Sonnet 4.6', { family: 'claude' });
			const familyMatch = createSessionModel('claude-opus-4.7', 'Claude Opus 4.7', 'agent-host-singularitycli', { family: 'claude' });
			const idMatch = createSessionModel('claude-sonnet-4.6', 'Claude Sonnet 4.6', 'agent-host-singularitycli', { family: 'claude-sonnet' });
		...
		test('falls back to name when neither id nor family match', () => {
			...
			const target = createSessionModel('b', 'Claude Sonnet 4.6', 'agent-host-singularitycli', { family: 'fb' });
			assert.strictEqual(findBestMatchingModel(prev, [target])?.identifier, target.identifier);
		});

		test('returns undefined when nothing matches', () => {
			const prev = createModel('gpt-5', 'GPT-5', { family: 'gpt-5' });
			const pool = [createSessionModel('claude', 'Claude', 'agent-host-singularitycli', { family: 'claude' })];
	...
	suite('resolveModelFromSyncState', () => {
		...
		test('returns default when current and state share an identifier but neither belongs to the new session pool', () => {
			...
			// session-targeted pool (`agent-host-singularitycli`) while the picker
			...
			const sessionModel = createSessionModel('claude', 'Claude', 'agent-host-singularitycli');
			const allModels = [generalModel, sessionModel];
			const result = resolveModelFromSyncState(generalModel, generalModel, allModels, 'agent-host-singularitycli');
			assert.strictEqual(result.action, 'default');
		});
	});
	...
	suite('mergeModelsWithCache', () => {
		...
		test('uses live models when available', () => {
			...
			const result = mergeModelsWithCache([liveModel], [cachedModel], new Set(['singularity']));
		...
		test('falls back to cached models when no live models', () => {
			const cachedModel = createModel('cached-gpt', 'Cached GPT');
			const result = mergeModelsWithCache([], [cachedModel], new Set(['singularity']));
			assert.strictEqual(result.length, 1);
		...
		test('merges cached models from vendors not yet resolved', () => {
			...
			const result = mergeModelsWithCache(
				[liveModel],
				[cachedOtherVendor],
				new Set(['singularity', 'other-vendor']),
			);
		...
		test('evicts cached models from vendors no longer contributed', () => {
			...
			const result = mergeModelsWithCache(
				[liveModel],
				[cachedRemovedVendor],
				new Set(['singularity']), // removed-vendor is NOT contributed
			);
			assert.strictEqual(result.length, 1);
			assert.strictEqual(result[0].metadata.id, 'gpt');
		});
		...
		test('does not duplicate models from same vendor', () => {
			...
			// Both are vendor 'singularity', live vendor takes priority
			assert.strictEqual(result.length, 1);
			assert.strictEqual(result[0].metadata.id, 'gpt');
		});
		...
		test('evicts cached agent-host entries when the vendor is resolved with zero live models', () => {
			...
			const liveSingularity = createModel('gpt', 'GPT');
			const staleAgentHost = createVendorModel('agent-host-singularitycli', 'gpt-5.6-sol', 'GPT 5.6 Sol');
			const result = mergeModelsWithCache(
				[liveSingularity],
				[staleAgentHost],
				new Set(['singularity', 'agent-host-singularitycli']),
				new Set(['singularity', 'agent-host-singularitycli']),
			);
			assert.strictEqual(result.length, 1);
			assert.strictEqual(result[0].metadata.vendor, 'singularity');
		});
	});
	...
	suite('onDidChangeLanguageModels race conditions', () => {
		...
		test('model temporarily removed then re-added loses user choice', () => {
			...
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', [gpt, claude]), false);

			// Step 2: Extension reloads, GPT temporarily disappears from model list
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', [claude]), true);
			...
			assert.strictEqual(shouldResetOnModelListChange('singularity/claude', [gpt, claude]), false);
		...
		test('reset when models list is empty', () => {
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', []), true);
		});
		...
		test('cache bridges the gap when live models temporarily unavailable', () => {
			...
			const merged = mergeModelsWithCache([], [cachedGpt, cachedClaude], new Set(['singularity']));
			assert.strictEqual(merged.length, 2);

			// Selected model is still found in the cached list
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', merged), false);
		});
		...
		test('cache evicted for uncontributed vendor once live models arrive', () => {
			...
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', merged), true);
		});
	});
	...
	suite('mode with forced model (mode.model property)', () => {
		...
		test('mode forces model — simulating switchModelByQualifiedName success', () => {
			...
			// Simulate: mode wants "GPT-4o (singularity)"
			const qualifiedName = 'GPT-4o (singularity)';
		...
		test('mode forces model — singularity vendor shorthand works', () => {
			const gpt = createModel('gpt-4o', 'GPT-4o');
			// For singularity vendor, just the name works
		...
		test('mode forces model — nonexistent model gracefully misses', () => {
			const gpt = createModel('gpt-4o', 'GPT-4o');
			const match = [gpt].find(m => ILanguageModelChatMetadata.matchesQualifiedName('NonExistent (singularity)', m.metadata));
	...
	suite('realistic multi-step race simulations', () => {
		...
		test('startup: cached model → live models arrive → user choice preserved', () => {
			...
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', cachedModels), false);
			...
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', liveModels), false);
		});
		...
		test('extension reload: selected model flickers out then back', () => {
			...
			// Step 2: Extension reloads, singularity vendor has no live models
			// But cache bridges the gap
			const duringReload = mergeModelsWithCache([], [gpt, claude], new Set(['singularity']));
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', duringReload), false);

			// Step 3: Extension finishes loading, live models back
			const afterReload = mergeModelsWithCache([gpt, claude], [gpt, claude], new Set(['singularity']));
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', afterReload), false);
		});
		...
		test('extension reload without cache: model lost', () => {
			...
			const duringReload = mergeModelsWithCache([], [], new Set(['singularity']));
			assert.strictEqual(duringReload.length, 0);
			assert.strictEqual(shouldResetOnModelListChange('singularity/gpt', duringReload), true);
			// → Model is lost, reset to default

			// Step 3: Models come back but user's choice is already gone
			const afterReload = mergeModelsWithCache([gpt], [], new Set(['singularity']));
		...
		// Repro for #321037: on first launch the restored Singularity selection is reset to a BYOK model. The Singularity
		// vendor depends on the Singularity token, which round-trips slower than fast/local BYOK providers (Ollama,
		// Cerebras). So the Singularity vendor resolves an EMPTY live list first while the BYOK vendors already have live
		// models. `mergeModelsWithCache` then treats Singularity's empty resolution as authoritative and evicts the cached
		// Singularity models that were used to restore the selection — leaving only BYOK models, which triggers a
		// reset-to-default that clobbers the user's persisted Singularity choice.
		test('startup race #321037: Singularity vendor resolves empty before BYOK, restored selection must survive', () => {
			// The user's persisted choice (a Singularity model) and its siblings, seeded into the cache from the previous
			// session.
			const persistedId = 'singularity/claude-opus-4.6-1m';
			const cachedSingularity = [
			...
			// Singularity contributed a vendor but resolved an EMPTY live list (token not ready yet); the BYOK vendors
			// resolved with models. All three are therefore "resolved".
			const contributedVendors = new Set(['singularity', 'ollama', 'cerebras']);
			const resolvedVendors = new Set(['singularity', 'ollama', 'cerebras']);

			const available = computeAvailableModels(
				liveByok,
				[...cachedSingularity, ...liveByok],
				contributedVendors,
				undefined,
				ChatModeKind.Agent,
				ChatAgentLocation.Chat,
			...
			// DESIRED: the user's restored Singularity model is still selectable during the race, so no reset-to-BYOK
			// happens and the persisted choice is kept. CURRENT (bug): Singularity cache is evicted, only BYOK remains, the
			// model is considered unavailable and gets reset to a BYOK default.
			assert.ok(
				available.some(m => m.identifier === persistedId),
				'restored Singularity model should remain available while its vendor is still activating',
			);
			assert.strictEqual(
				shouldResetOnModelListChange(persistedId, available),
				false,
				'must not reset the restored Singularity selection during the startup race',
			);
	...
	suite('agent-host model restore', () => {
		...
		test('a started contributed session is never a new conversation, even before its requests load', () => {
			const startedAgentHost = URI.parse('agent-host-singularitycli:/933e7602-f84e-431e-8756-c5e85c8f33d0');
			const untitledAgentHost = URI.parse('agent-host-singularitycli:/untitled-933e7602');
	...
	suite('BYOK agent-host visibility (isModelHiddenInPicker / getAgentHostByokManageModelsIdentifier)', () => {
		...
		function createAgentHostByokModel(vendor: string, modelId: string, manageModelsIdentifier: string): ILanguageModelChatMetadataAndIdentifier {
			const sessionType = 'agent-host-singularitycli';
		...
		// A native harness model (e.g. Singularity CLI's own model) carries no
		...
		test('filters out a hidden grouped BYOK model but keeps visible peers', () => {
			...
			assert.deepStrictEqual(result.map(m => m.identifier), ['agent-host-singularitycli:anthropic/claude-sonnet-4']);
		});
	});
});
