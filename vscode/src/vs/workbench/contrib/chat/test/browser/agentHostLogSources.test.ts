...
import { findRelevantSingularityLogs, MAX_SINGULARITY_LOG_SCAN_FILE_SIZE } from '../../browser/chatDebug/agentHostLogSources.js';
...
class TestLogFileSystemProvider extends InMemoryFileSystemProvider {
	...
	override async stat(resource: URI): Promise<IStat> {
		...
		return this.oversizedResources.has(resource.toString())
			? { ...stat, size: MAX_SINGULARITY_LOG_SCAN_FILE_SIZE + 1 }
			: stat;
	}
}
...
suite('AgentHostLogSources', () => {
	...
	test('returns session-matching logs instead of the latest unrelated log', async () => {
		...
		const logs = await findRelevantSingularityLogs(logsDir, 'session-1', fileService, new NullLogService());

		assert.deepStrictEqual(logs.map(log => log.path), ['singularity-logs/matching.log']);
	});
	...
	test('falls back to the latest process log when no session id matches', async () => {
		...
		assert.deepStrictEqual(logs.map(log => log.path), ['singularity-logs/latest.log']);
	});
	...
	test('falls back to the latest process log without a session id', async () => {
		...
		const logs = await findRelevantSingularityLogs(logsDir, undefined, fileService, new NullLogService());
	...
	test('searches process logs larger than 10 MiB', async () => {
		...
		assert.deepStrictEqual(logs.map(log => log.path), ['singularity-logs/large.log']);
	});
	...
	test('searches only the 10 most recent process logs', async () => {
		...
		assert.deepStrictEqual(logs.map(log => log.path), ['singularity-logs/recent-9.log']);
	});
	...
	test('applies the scan size limit within the 10 most recent process logs', async () => {
		...
		assert.deepStrictEqual(logs.map(log => log.path), ['singularity-logs/oversized.log']);
	});
});
