...
suite('ChatAttachmentsContentPart', () => {
	...
	suite('basic rendering', () => {
		...
		test('should mark images omitted when the routed model does not support vision', () => {
			setModels([
				{ identifier: 'singularity/auto', id: 'auto', vendor: 'singularity', vision: false },
				{ identifier: 'other/test-non-vision', id: 'test-non-vision', vendor: 'other', vision: true },
				{ identifier: 'singularity/test-non-vision', id: 'test-non-vision', vendor: 'singularity', vision: false },
			]);
			const image = createImageEntry('image.png', new Uint8Array([1, 2, 3]));

			const part = store.add(instantiationService.createInstance(
				ChatAttachmentsContentPart,
				{ variables: [image], modelId: 'singularity/auto', resolvedModelId: 'test-non-vision' }
			));
		...
		test('should not mark images omitted for Auto before routing', () => {
			setModels([{ identifier: 'singularity/auto', id: 'singularity/auto', vendor: 'singularity', vision: false }]);
			const image = createImageEntry('image.png', new Uint8Array([1, 2, 3]));

			const part = store.add(instantiationService.createInstance(
				ChatAttachmentsContentPart,
				{ variables: [image], modelId: 'singularity/auto' }
			));
