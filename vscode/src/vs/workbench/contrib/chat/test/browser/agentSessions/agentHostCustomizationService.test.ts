...
suite('AbstractAgentHostCustomizationService - MCP server enablement', () => {
	...
	const sessionA1 = URI.from({ scheme: 'agent-host-singularitycli', authority: 'session-a1', path: '/' });
	const sessionA2 = URI.from({ scheme: 'agent-host-singularitycli', authority: 'session-a2', path: '/' });
	...
	const sessionB = URI.from({ scheme: 'remote-hostB-singularitycli', authority: 'session-b', path: '/' });
