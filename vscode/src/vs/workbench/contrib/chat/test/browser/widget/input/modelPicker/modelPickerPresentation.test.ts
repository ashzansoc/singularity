...
suite('ModelPickerPresentation', () => {
	...
	test('uses canonical Auto model detection', () => {
		assert.deepStrictEqual({
			...
			canonicalIdentifier: isAutoModel(model('singularity/auto', 'missing-auto-id')),
			ordinary: isAutoModel(model('singularity/gpt', 'gpt')),
		}, {
			metadataAuto: true,
	...
	test('computes unavailable picker state from trust, setup, and live models', () => {
		const gpt = model('singularity/gpt', 'gpt');
