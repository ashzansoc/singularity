...
suite('AgentHostCustomizationRecorder', () => {
	...
	test('writes the snapshot for a Singularity CLI session channel and ignores other channels/actions', async () => {
		const { fileService, actions } = makeRecorder();

		// Ignored: a non-customization action, and a customization action on a
		// non-Singularity-CLI channel — neither should produce a sidecar file.
		actions.fire({ channel: 'singularitycli:/wrongtype', action: { type: ActionType.ChatUsage, turnId: '0', usage: {} }, serverSeq: 1, origin: undefined } as unknown as ActionEnvelope);
		...
		// `singularitycli:/<rawId>` channel is persisted under that raw id.
		actions.fire({ channel: 'singularitycli:/good', action: { type: ActionType.SessionCustomizationsChanged, customizations: skills }, serverSeq: 3, origin: undefined } as unknown as ActionEnvelope);
	...
	test('does not capture while disabled', async () => {
		...
		actions.fire({ channel: 'singularitycli:/gated', action: { type: ActionType.SessionCustomizationsChanged, customizations: skills }, serverSeq: 1, origin: undefined } as unknown as ActionEnvelope);
		await timeout(2);
		...
		enabled = true;
		actions.fire({ channel: 'singularitycli:/gated', action: { type: ActionType.SessionCustomizationsChanged, customizations: skills }, serverSeq: 2, origin: undefined } as unknown as ActionEnvelope);
	...
	test('deletes the sidecar when its session is removed, even after capture is disabled', async () => {
		...
		actions.fire({ channel: 'singularitycli:/doomed', action: { type: ActionType.SessionCustomizationsChanged, customizations: skills }, serverSeq: 1, origin: undefined } as unknown as ActionEnvelope);
		...
		enabled = false;
		notifications.fire({ type: NotificationType.SessionRemoved, channel: 'ahp-root://', session: 'singularitycli:/doomed' } as unknown as INotification);
	...
	test('ignores unrelated notifications and sessions with no sidecar', async () => {
		...
		actions.fire({ channel: 'singularitycli:/keep', action: { type: ActionType.SessionCustomizationsChanged, customizations: skills }, serverSeq: 1, origin: undefined } as unknown as ActionEnvelope);
		...
		// must not throw.
		notifications.fire({ type: NotificationType.SessionRemoved, channel: 'ahp-root://', session: 'singularitycli:/never-recorded' } as unknown as INotification);
		...
		notifications.fire({ type: NotificationType.SessionAdded, channel: 'ahp-root://', session: 'singularitycli:/keep' } as unknown as INotification);
		await timeout(2);
