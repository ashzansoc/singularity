...
suite('SessionTypePickerActionItem', () => {
	...
	test('creates plain accessible text for an unavailable Codex action', () => {
		...
		assert.deepStrictEqual({
			...
			ariaDescription: 'Sign in. Sign in to Singularity to use this agent.',
			hover: '[Sign in to Singularity](command:workbench.action.chat.triggerSetup) to use this agent.',
		});
	});
});
