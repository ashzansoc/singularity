...
suite('resolveCustomizationRefs - built-in skills', () => {
	...
	test('includes enabled user files only when user storage is enabled', async () => {
		const enabled = URI.file('/home/user/.singularity/instructions/enabled.instructions.md');
...
suite('shouldSyncWorkspaceDotMcp - multi-root gate', () => {
	...
	// Pins the production local Singularity Agent Host session type so a drift in the
	// gate's session-type comparison (the class of bug that would otherwise leave
	// the feature tests green) fails here.
	const LOCAL_SINGULARITY = 'agent-host-singularitycli';

	test('true only for local Singularity + multi-root workspace + setting enabled', () => {
		assert.strictEqual(shouldSyncWorkspaceDotMcp(LOCAL_SINGULARITY, 2, true), true);
	});

	test('false when the multi-root setting is disabled', () => {
		assert.strictEqual(shouldSyncWorkspaceDotMcp(LOCAL_SINGULARITY, 2, false), false);
	});

	test('false for a single-folder workspace', () => {
		assert.strictEqual(shouldSyncWorkspaceDotMcp(LOCAL_SINGULARITY, 1, true), false);
	});

	test('false for an empty workspace', () => {
		assert.strictEqual(shouldSyncWorkspaceDotMcp(LOCAL_SINGULARITY, 0, true), false);
	});

	test('false for a non-Singularity harness (e.g. Claude)', () => {
		assert.strictEqual(shouldSyncWorkspaceDotMcp('agent-host-claude', 2, true), false);
	});

	test('false for the Singularity CLI (extension host) harness', () => {
		assert.strictEqual(shouldSyncWorkspaceDotMcp('singularitycli', 2, true), false);
	});

	test('false for a remote Singularity Agent Host session', () => {
		assert.strictEqual(shouldSyncWorkspaceDotMcp('remote-myauthority-singularitycli', 2, true), false);
	});
});
