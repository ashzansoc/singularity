...
suite('AgentHostLanguageModelProvider', () => {
	...
	function makeModel(id: string, meta?: Record<string, unknown>): SessionModelInfo {
		return { id, provider: 'singularitycli', name: id === 'auto' ? 'Auto' : id, ...(meta && { _meta: meta }) };
	}

	function createProvider(): AgentHostLanguageModelProvider {
		return store.add(new AgentHostLanguageModelProvider('agent-host-singularitycli', 'singularitycli'));
	}
	...
	test('derives the picker group from the model-id prefix, not the harness provider', async () => {
		const provider = createProvider();
		// The agent host reports every model under the harness provider (`singularitycli`);
		// the upstream provider lives in the id prefix. Native models have no prefix.
		provider.updateModels([
			{ id: 'claude-haiku-4.5', provider: 'singularitycli', name: 'Claude Haiku 4.5' },
			{ id: 'openai/gpt-5-nano', provider: 'singularitycli', name: 'GPT-5 nano' },
			{ id: 'huggingface/allenai/Olmo-3-7B-Instruct:cheapest', provider: 'singularitycli', name: 'Olmo 3' },
			{ id: 'acme/model', provider: 'singularitycli', name: 'Acme' },
		]);
		...
		assert.deepStrictEqual(groups, {
			'claude-haiku-4.5': { id: 'singularitycli' },
	...
	test('keeps duplicate Codex model names distinct and provider scoped', async () => {
		...
		provider.updateModels([
			{ id: '@provider=vscode-proxy:gpt-5.6-sol', provider: 'singularity', name: 'GPT-5.6 Sol' },
		...
		assert.deepStrictEqual(infos.map(info => ({
			...
			{ identifier: 'codex:@provider=vscode-proxy:gpt-5.6-sol', name: 'GPT-5.6 Sol', group: { id: 'singularity' } },
