...
import { formatChatRequestTimestamp, formatChatResponseDetails, formatChatResponsePerf, formatElapsedTime } from '../../../common/chatProgressFormatting.js';
...
suite('ChatListRenderer', () => {
	...
	suite('formatChatResponseDetails', () => {
		...
		test('formats per-response time and tokens/sec', () => {
			assert.deepStrictEqual([
				formatChatResponsePerf(2_500, 100),
				formatChatResponsePerf(24_000, 480),
				formatChatResponsePerf(500, undefined),
				formatChatResponsePerf(undefined, 100),
			], [
				'2.5s \u00b7 40 tok/s \u00b7 100 out',
				'24s \u00b7 20 tok/s \u00b7 480 out',
				'0.5s',
				undefined,
			]);
		});
		...
		test('renders completion time with elapsed-time alternate only in verbose mode', () => {
			...
			const compact = {
				...
				perf: container.querySelector('.chat-response-perf')?.textContent,
				tabIndex: container.tabIndex,
			};

			renderChatResponseDetails(container, 'Claude Opus 4.8', completedAt, 24_000, true, undefined, 480);
			assert.deepStrictEqual({
				compact,
	...
	suite('shouldHideChatUserIdentity', () => {
		test('shows Singularity local identity; hides Agent Host Singularity and session chrome', () => {
			assert.deepStrictEqual([
				shouldHideChatUserIdentity('Singularity', URI.from({ scheme: 'vscode-chat-editor' }), true, false, false),
				shouldHideChatUserIdentity('Singularity', URI.from({ scheme: 'agent-host-singularitycli' }), true, false, false),
				shouldHideChatUserIdentity('Singularity', URI.from({ scheme: 'agent-host-singularitycli' }), false, false, false),
				shouldHideChatUserIdentity('Singularity', URI.from({ scheme: 'remote-test-authority-singularitycli' }), true, false, false),
				shouldHideChatUserIdentity('Singularity', URI.from({ scheme: 'remote-test-authority-singularitycli' }), false, false, false),
