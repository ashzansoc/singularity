...
import { GITHUB_SINGULARITY_PROTECTED_RESOURCE, IAgentHostService } from '../../../../../../platform/agentHost/common/agentService.js';
...
suite('AgentHostProtectedResourcesService', () => {
	...
	test('onDidChange emits the changed provider, and only on a real change', () => {
		...
		setAgents([agent('a', [GITHUB_SINGULARITY_PROTECTED_RESOURCE]), agent('b', [GITHUB_SINGULARITY_PROTECTED_RESOURCE])]);

		// Only provider 'a' flips (Singularity required → not-required) → only 'a' is emitted.
		setAgents([agent('a', [{ ...GITHUB_SINGULARITY_PROTECTED_RESOURCE, required: false }]), agent('b', [GITHUB_SINGULARITY_PROTECTED_RESOURCE])]);
