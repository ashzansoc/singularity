...
import { SINGULARITY_CLI_LOCAL_AH_SCHEME } from '../../browser/singularityCliEventsUri.js';
...
suite('AgentHostChatDebugProvider - convertAgentHostEventsToDebugEvents', () => {
	...
	const sessionResource = URI.from({ scheme: SINGULARITY_CLI_LOCAL_AH_SCHEME, path: '/session-1' });
	...
	test('surfaces errors, warnings, model changes, error hooks and routine lifecycle hooks', () => {
		...
		const failing = [
			{ type: 'session.start', id: 's', parentId: null, timestamp: '2026-06-17T00:00:00.000Z', data: { selectedModel: 'echo', singularityVersion: '1.0.69', context: { repository: 'microsoft/vscode', branch: 'main' } } },
	...
	test('back-fills session.shutdown usage onto model turns so tile sums are exact', () => {
		...
		const nanoAiu = sum(t => t.singularityUsageNanoAiu);
	...
	test('live fallback (no session.shutdown) contributes AIU only — input/cache stay blank (F1)', () => {
		...
		assert.deepStrictEqual(
			{ input: sum(t => t.inputTokens), cached: sum(t => t.cachedTokens), output: sum(t => t.outputTokens), aic: sum(t => t.singularityUsageNanoAiu) / 1_000_000_000 },
	...
	test('back-fills per-round usage from the sidecar, correlating positionally across mismatched turn-id namespaces', () => {
		...
		assert.deepStrictEqual(
			{ input: sum(t => t.inputTokens), cached: sum(t => t.cachedTokens), output: sum(t => t.outputTokens), total: sum(t => t.totalTokens), aic: sum(t => t.singularityUsageNanoAiu) / 1_000_000_000 },
	...
	test('a zero-usage session.shutdown takes precedence over the live fallback', () => {
		...
		assert.deepStrictEqual(
			{ aiu: sum(t => t.singularityUsageNanoAiu), input: sum(t => t.inputTokens) },
			{ aiu: 0, input: 0 },
		);
	});
