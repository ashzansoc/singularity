...
suite('buildUpgradeUrlWithRedirect', () => {
	...
	test('stable quality uses vscode.dev host', () => {
		const result = buildUpgradeUrlWithRedirect(
			'https://github.com/github-singularity/upgrade?utm_source=vscode',
			'vscode',
			'stable'
		);
		const { redirectHost, vscodeUri } = parseRedirectUrl(result);
		assert.strictEqual(redirectHost, 'vscode.dev');
		assert.strictEqual(vscodeUri, 'vscode://Singularity.singularity-chat/upgrade-success');
	});
	...
	test('insider quality uses insiders.vscode.dev host', () => {
		...
		assert.strictEqual(vscodeUri, 'vscode-insiders://Singularity.singularity-chat/upgrade-success');
	});
	...
	test('undefined quality defaults to insiders.vscode.dev host', () => {
		...
		const { redirectHost, vscodeUri } = parseRedirectUrl(result);
		assert.strictEqual(redirectHost, 'insiders.vscode.dev');
		assert.strictEqual(vscodeUri, 'code-oss://Singularity.singularity-chat/upgrade-success');
	});
	...
	test('appends with & when base URL already has query params', () => {
		...
		assert.ok(result.startsWith('https://github.com/github-singularity/upgrade?utm_source=vscode&return_to='));
	});

	test('appends with ? when base URL has no query params', () => {
		const result = buildUpgradeUrlWithRedirect(
			'https://github.com/github-singularity/upgrade',
			'vscode',
			'stable'
		);
		assert.ok(result.startsWith('https://github.com/github-singularity/upgrade?return_to='));
	});

	test('GHE URL is handled correctly', () => {
		const result = buildUpgradeUrlWithRedirect(
			'https://github.example.com/github-singularity/upgrade?utm_source=vscode',
			'vscode',
			'stable'
		);
		assert.ok(result.startsWith('https://github.example.com/github-singularity/upgrade?utm_source=vscode&return_to='));
