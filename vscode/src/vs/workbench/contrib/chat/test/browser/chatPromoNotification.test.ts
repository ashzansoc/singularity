...
suite('ChatPromoNotificationContribution', () => {
	...
	test('shows notification for model with promo', () => {
		...
		const { service: lmService } = createMockLanguageModelsService([{
			identifier: 'singularity:gpt-5.5',
		...
		assert.deepStrictEqual(notification.actions, [{
			...
			modelIdentifier: 'singularity:gpt-5.5',
		}]);
	});
	...
	test('renders the server message for a 0% promo', () => {
		const notifService = createMockNotificationService(disposables);
		const { service: lmService } = createMockLanguageModelsService([{
			identifier: 'singularity:zero-discount',
	...
	test('prefers a discounted promo over a 0% one in the same harness', () => {
		const notifService = createMockNotificationService(disposables);
		const { service: lmService } = createMockLanguageModelsService([
			{ identifier: 'singularity:featured', metadata: { name: 'Featured', id: 'featured', promo: { id: 'promo-zero', discountPercent: 0, message: 'Featured model' } } },
			{ identifier: 'singularity:discounted', metadata: { name: 'Discounted', id: 'discounted', promo: { id: 'promo-discount', discountPercent: 20, message: 'Get 20% off' } } },
		], disposables);
		const storageService = disposables.add(new InMemoryStorageService());
	...
	test('does not show notification for negative promo discounts', () => {
		const notifService = createMockNotificationService(disposables);
		const { service: lmService } = createMockLanguageModelsService([{
			identifier: 'singularity:negative-discount',
	...
	test('omits the end date when the promo has none', () => {
		...
		const { service: lmService } = createMockLanguageModelsService([
			...
			{ identifier: 'singularity:bad-end-date', metadata: { name: 'Bad Date', id: 'bad-end-date', targetChatSessionType: 'singularitycli', promo: { id: 'promo-bad-date', discountPercent: 20, endsAt: 'not a date', message: 'Get 20% off' } } },
		], disposables);
		const storageService = disposables.add(new InMemoryStorageService());
	...
	test('skips second promo if first is not dismissed', () => {
		const notifService = createMockNotificationService(disposables);
		const { service: lmService } = createMockLanguageModelsService([
			{ identifier: 'singularity:gpt-5.5', metadata: { name: 'GPT-5.5', id: 'gpt-5.5', promo: { id: 'promo-a', discountPercent: 20, endsAt: '2026-07-20T23:59:59Z', message: 'First promo' } } },
			{ identifier: 'singularity:claude', metadata: { name: 'Claude', id: 'claude', promo: { id: 'promo-b', discountPercent: 10, endsAt: '2026-08-01T00:00:00Z', message: 'Second promo' } } },
		], disposables);
		const storageService = disposables.add(new InMemoryStorageService());
	...
	test('shows a scoped promo per harness', () => {
		...
		const { service: lmService } = createMockLanguageModelsService([
			...
			{ identifier: 'singularity:claude', metadata: { name: 'Claude', id: 'claude', targetChatSessionType: 'singularitycli', promo: { id: 'promo-singularity', discountPercent: 20, endsAt: '2026-07-20T23:59:59Z', message: 'Singularity promo' } } },
		...
		const singularity = notifService.getNotificationForSession('singularitycli');
		assert.ok(singularity, 'Expected a Singularity promo');
		assert.ok(singularity.message.toString().includes('Singularity promo'));
		assert.deepStrictEqual(singularity.actions, [{ label: 'Try Claude', kind: ChatInputNotificationActionKind.SwitchToModel, modelIdentifier: 'singularity:claude' }]);
	...
	test('does not leak a harness promo into a different session type', () => {
		...
		assert.ok(notifService.getNotificationForSession('singularitycli'), 'Promo should show in its own harness');
	...
	test('dismissing a promo in one harness hides the same promo in the others', () => {
		...
		const { service: lmService } = createMockLanguageModelsService([
			{ identifier: 'singularity:claude', metadata: { name: 'Claude', id: 'claude', targetChatSessionType: 'singularitycli', promo: sharedPromo } },
		...
		// Dismiss the Singularity notification.
		const singularity = notifService.getNotificationForSession('singularitycli');
		assert.ok(singularity);
		notifService.dismiss(singularity.id);
