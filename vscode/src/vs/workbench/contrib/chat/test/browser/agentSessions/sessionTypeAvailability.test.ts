...
interface ITypeConfig {
	...
	/** Whether the type relies on a Singularity account (defaults to false). */
	readonly requiresSingularitySignIn?: boolean;
}
...
function createChatSessionsService(config: ITypeConfig): IChatSessionsService {
	return new class extends mock<IChatSessionsService>() {
		...
		override requiresSingularitySignInForSessionType(type: string): boolean {
			return type === TYPE ? !!config.requiresSingularitySignIn : false;
		}
...
suite('getSessionTypeAvailability', () => {
	...
	test('signed-out user must sign in even when the type supports the Auto fallback', () => {
		// Singularity CLI: supportsAutoModel=true. The Auto model needs a Singularity
		// account and BYOK is not supported here, so a signed-out user can't use it.
		const config: ITypeConfig = { registered: true, supportsAutoModel: true, requiresCustomModels: true, requiresSingularitySignIn: true };
	...
	test('signed-out user must sign in for a custom-model agent host type', () => {
		...
		const config: ITypeConfig = { registered: true, supportsAutoModel: false, requiresCustomModels: true, requiresSingularitySignIn: true };
		assert.strictEqual(availability(config, ChatEntitlement.Unknown), SessionTypeAvailability.SignInRequired);
	});

	test('signed-out user must sign in for a delegation type (e.g. Cloud)', () => {
		// The cloud agent delegates to a remote Singularity: supportsAutoModel=false,
		// requiresCustomModels=false. It still needs a Singularity account, so a
		// signed-out user is prompted to sign in rather than offered the type.
		const config: ITypeConfig = { registered: true, supportsAutoModel: false, requiresCustomModels: false, requiresSingularitySignIn: true };
		assert.strictEqual(availability(config, ChatEntitlement.Unknown), SessionTypeAvailability.SignInRequired);
	});

	test('signed-out user can still use a non-Singularity third-party type', () => {
		// A general contributed session type that doesn't rely on Singularity
		// (requiresSingularitySignIn defaults to false) must not be gated behind
	...
	test('anonymous access lets a signed-out user use a Singularity-backed type', () => {
		// With chat.allowAnonymousAccess enabled, a signed-out user is granted
		// access without signing in, so a Singularity-backed type (e.g. the local
	...
	test('a targeting model does NOT override sign-in for a Singularity-backed type', () => {
		// BYOK is not supported in the agent host / Singularity CLI, so a stale/cached
		// Singularity model still targeting the type must not unlock it when signed out.
		const config: ITypeConfig = { registered: true, supportsAutoModel: true, requiresCustomModels: true, requiresSingularitySignIn: true };
