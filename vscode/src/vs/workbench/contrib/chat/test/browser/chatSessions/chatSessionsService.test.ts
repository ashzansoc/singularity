...
import { AgentHostCodexAgentEnabledSettingId, CodexPreferAgentHostEditorSettingId, GITHUB_SINGULARITY_PROTECTED_RESOURCE, GITHUB_REPO_PROTECTED_RESOURCE, protectedResourcesRequireSingularitySignIn } from '../../../../../../platform/agentHost/common/agentService.js';
...
suite('ChatSessionsService - requiresSingularitySignInForSessionType', () => {
	...
	test('evaluates a functional requiresSingularitySignIn, and reads a static flag otherwise', () => {
		// Declarative (extension) types supply a static boolean, read directly.
		register('static-required', { requiresSingularitySignIn: true });
		register('static-not-required', { requiresSingularitySignIn: false });
		...
		// that marks the Singularity resource `required: false` (Claude native, Codex on
		...
		const resourcesByProvider: Record<string, readonly ProtectedResourceMetadata[] | undefined> = {
			proxy: [GITHUB_SINGULARITY_PROTECTED_RESOURCE, GITHUB_REPO_PROTECTED_RESOURCE],
			native: [{ ...GITHUB_SINGULARITY_PROTECTED_RESOURCE, required: false }, GITHUB_REPO_PROTECTED_RESOURCE],
			'codex-openai': [{ ...GITHUB_SINGULARITY_PROTECTED_RESOURCE, required: false }],
			unresolved: undefined,
		};
		const derive = (provider: string) => () => {
			const resources = resourcesByProvider[provider];
			return resources !== undefined ? protectedResourcesRequireSingularitySignIn(resources) : true;
		};
		register('ah-proxy', { agentHostProviderId: 'proxy', requiresSingularitySignIn: derive('proxy') });
		register('ah-native', { agentHostProviderId: 'native', requiresSingularitySignIn: derive('native') });
		register('ah-codex-openai', { agentHostProviderId: 'codex-openai', requiresSingularitySignIn: derive('codex-openai') });
		register('ah-unresolved', { agentHostProviderId: 'unresolved', requiresSingularitySignIn: derive('unresolved') });

		assert.deepStrictEqual({
			staticRequired: service.requiresSingularitySignInForSessionType('static-required'),
			staticNotRequired: service.requiresSingularitySignInForSessionType('static-not-required'),
			ahProxy: service.requiresSingularitySignInForSessionType('ah-proxy'),
			ahNative: service.requiresSingularitySignInForSessionType('ah-native'),
			ahCodexOpenai: service.requiresSingularitySignInForSessionType('ah-codex-openai'),
			ahUnresolved: service.requiresSingularitySignInForSessionType('ah-unresolved'),
			unknownType: service.requiresSingularitySignInForSessionType('never-registered'),
		}, {
			staticRequired: true,
	...
	test('a contribution change event re-fires onDidChangeAvailability until it is unregistered', () => {
		...
		// its onDidChangeRequiresSingularitySignIn is wired generically.
		const registration = store.add(service.registerChatSessionContribution({
			type: 'dyn', name: 'dyn', displayName: 'dyn', description: '',
			requiresSingularitySignIn: () => true,
			onDidChangeRequiresSingularitySignIn: changed.event,
		}));
	...
	suite('toStrValueArray', () => {
		...
		test('should extract .id from IChatSessionProviderOptionItem values', () => {
			const map: ReadonlyChatSessionOptionsMap = new Map([
				['agent', { id: 'singularity', name: 'Singularity' }],
			]);
			assert.deepStrictEqual(ChatSessionOptionsMap.toStrValueArray(map), [
				{ optionId: 'agent', value: 'singularity' },
			]);
		});
