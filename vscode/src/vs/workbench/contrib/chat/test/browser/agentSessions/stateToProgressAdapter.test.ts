...
suite('stateToProgressAdapter', () => {
		...
		test('preserves client-handled link schemes', () => {
			assert.deepStrictEqual(
				[
					...
					rewriteAgentHostLinkTarget('singularity-skill:/plan', 'my-host'),
	...
	suite('turnsToHistory', () => {
					...
					kind: ResponsePartKind.ToolCall, toolCall: createCompletedToolCall({
						...
						_meta: {
							ui: {
								...
								channel: 'mcp://singularity/session/GitHub',
							},
						},
					})
				} as ToolCallResponsePart],
		...
		test('per-turn model id flows from usage.model without exposing display names in details', () => {
			...
			const lookup = makeLookup('agent-host-singularity:', { 'gpt-5': 'GPT-5', 'opus-4.7': 'Claude Opus 4.7' });
			...
			assert.deepStrictEqual(
				...
				[
					{ type: 'request', modelId: 'agent-host-singularity:gpt-5' },
					{ type: 'response', details: undefined },
					{ type: 'request', modelId: 'agent-host-singularity:opus-4.7' },
					{ type: 'response', details: undefined },
				],
			);
		});

		test('does not restore Auto model routing into the shared chat UI part', () => {
			const turn = createTurn({
				usage: {
					model: 'gpt-5.4-mini',
					_meta: {
			...
			const lookup = makeLookup('agent-host-singularity:', { 'gpt-5.4-mini': 'GPT-5.4 mini' });
		...
		test('falls back to session-level model when turn has no usage.model', () => {
			...
			const lookup = makeLookup('agent-host-singularity:', { 'gpt-5': 'GPT-5' }, 'gpt-5');
			const history = turnsToHistory(URI.file('/'), [turn], 'p', lookup);
		...
		test('maps turn usage to chat usage progress for restored history', () => {
			...
			const history = turnsToHistory(URI.file('/'), [turn], 'p', makeLookup('agent-host-singularity:', { 'gpt-5': 'GPT-5' }));
			const response = history[1];
			assert.strictEqual(response.type, 'response');
			if (response.type !== 'response') { return; }
		...
		test('request history includes restored model id', () => {
			...
			const lookup = makeLookup('agent-host-singularity:', {}, 'gpt-5');
			...
			assert.deepStrictEqual(history[0], {
				...
				modelId: 'agent-host-singularity:gpt-5',
		...
		test('subagent tool call in history has correct subagent data', () => {
				...
				responseParts: [{
					kind: ResponsePartKind.ToolCall, toolCall: createCompletedToolCall({
						...
						content: [
							...
							{ type: ToolResultContentType.Subagent, resource: 'singularity://session/subagent/tc-1', title: 'Explore', agentName: 'explore', description: 'Explores the codebase' },
						],
						success: true,
					})
				} as ToolCallResponsePart],
			...
			if (serialized.toolSpecificData.kind === 'subagent') {
				...
				assert.strictEqual(serialized.toolSpecificData.chatResource, 'singularity://session/subagent/tc-1');
			}
		});
	...
	suite('toolCallStateToInvocation', () => {
		...
		test('creates authentication-required invocation for an MCP tool call', () => {
			...
			}, undefined, URI.parse('agent-host-singularity://backend/session'), 'remote', 'frontend');
	...
	suite('streaming tool invocations (#314858)', () => {
		...
		test('toolCallStateToStreamingInvocation preserves subagent metadata before ready', () => {
			const sessionResource = URI.parse('singularitycli:/session-1');
			const invocation = toolCallStateToStreamingInvocation({
		...
		test('requestConfirmation re-arms confirmation from Executing (Singularity Running → PendingConfirmation)', () => {
			// Real Singularity flow: onToolStart readies the tool (Running/Executing)
	...
	suite('finalizeToolInvocation', () => {
		...
		test('preserves subagent credits when finalizing', () => {
			...
			finalizeToolInvocation(invocation, {
				...
				content: [{
					type: ToolResultContentType.Subagent,
					resource: 'singularity://session/subagent/tc-1',
					title: 'Explore',
					agentName: 'explore',
					description: 'Explores the codebase',
				}, {
	...
	suite('activeTurnToProgress', () => {
		...
		test('preserves create metadata and proposed content for pending file confirmations', () => {
			...
			assert.deepStrictEqual(
				state.type === IChatToolInvocation.StateKind.WaitingForConfirmation ? state.confirmationMessages?.customOptions?.map(o => o.id) : undefined,
				['allow-session', 'allow-once', 'skip'],
			);
		});
	...
	suite('terminal content blocks', () => {
		...
		test('uses terminal completion exit code for completed SDK shell tool history', () => {
			const tc = createCompletedToolCall({
				...
				content: [
					...
					{ type: ToolResultContentType.Terminal, resource: 'agenthost-terminal://shell/singularityNonPtyShells/tc-1', title: 'Run Shell Command', isPty: false, result: { exitCode: 127, preview: 'preview only\n', truncated: true } },
				],
				success: true,
			});
		...
		test('preserves an explicitly empty non-PTY retained completion snapshot', () => {
			const tc = createCompletedToolCall({
				...
				toolInput: 'true',
				content: [
					{ type: ToolResultContentType.Terminal, resource: 'agenthost-terminal://shell/singularityNonPtyShells/tc-1', title: 'Run Shell Command', isPty: false, result: { exitCode: 0, preview: '' } },
				],
				success: true,
			});
		...
		test('does not use text content when a terminal block owns the output', () => {
			const tc = createCompletedToolCall({
				...
				content: [
					...
					{ type: ToolResultContentType.Terminal, resource: 'agenthost-terminal://shell/singularityNonPtyShells/tc-1', title: 'Run Shell Command', isPty: false, result: { exitCode: 127 } },
				],
				success: true,
			});
		...
		test('keeps zero terminal completion exit code as success for completed SDK shell tool history', () => {
			const tc = createCompletedToolCall({
				...
				content: [
					{ type: ToolResultContentType.Text, text: '/repo\n' },
					{ type: ToolResultContentType.Terminal, resource: 'agenthost-terminal://shell/singularityNonPtyShells/tc-1', title: 'Run Shell Command', isPty: false, result: { exitCode: 0 } },
				],
				success: true,
			});
		...
		test('does not fall back to tool success when terminal completion has no exit code', () => {
			const tc = createCompletedToolCall({
				...
				content: [
					{ type: ToolResultContentType.Text, text: '/repo\n' },
					{ type: ToolResultContentType.Terminal, resource: 'agenthost-terminal://shell/singularityNonPtyShells/tc-1', title: 'Run Shell Command', isPty: false, result: {} },
				],
				success: true,
			});
		...
		test('uses failed tool state when an output-only terminal has no shell exit', () => {
			const tc = createCompletedToolCall({
				...
				content: [
					...
					{ type: ToolResultContentType.Terminal, resource: 'agenthost-terminal://shell/singularityNonPtyShells/tc-1', title: 'Run Shell Command', isPty: false },
				],
				success: false,
			});
		...
		test('finalize uses terminal completion exit code over SDK tool success', () => {
			...
			finalizeToolInvocation(invocation, {
				...
				content: [
					...
					{ type: ToolResultContentType.Terminal, resource: 'agenthost-terminal://shell/singularityNonPtyShells/tc-1', title: 'Run Shell Command', isPty: false, result: { exitCode: 1 } },
				],
			});
	...
	suite('formatTurnResponseDetails', () => {
		...
		test('shows model name and optional credits in the footer', () => {
			const result = {
			...
			assert.deepStrictEqual(result, {
				resolvedModel: 'Auto',
				withPricing: 'Auto',
				withCredits: 'Auto \u2022 2 credits',
				oneCredit: 'Auto \u2022 1 credit',
				noBilledModel: 'Auto',
			});
		});

		test('surfaces concrete model names in the footer', () => {
	...
	suite('usageInfoToChatUsage', () => {
		test('carries whole-turn per-model token totals and resolves display names', () => {
			...
			assert.deepStrictEqual(usageInfoToChatUsage(
				...
				promptTokens: 30,
				completionTokens: 40,
				singularityCredits: undefined,
				sessionSingularityCredits: undefined,
