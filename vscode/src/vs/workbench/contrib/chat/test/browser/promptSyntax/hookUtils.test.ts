...
suite('hookUtils', () => {
	...
	suite('findHookCommandSelection - singularityCLICompat', () => {
	...
	suite('findHookCommandSelection with buildNewHookEntry', () => {
		...
		test('finds command in Singularity-format generated JSON', () => {
		...
		test('Singularity format JSON has correct structure', () => {
			const entry = buildNewHookEntry(HookSourceFormat.Singularity);
			const content = JSON.stringify({ hooks: { SubagentStart: [entry] } }, null, '\t');
			const parsed = JSON.parse(content);
			assert.deepStrictEqual(parsed, {
