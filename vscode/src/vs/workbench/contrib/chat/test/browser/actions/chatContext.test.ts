...
suite('ChatContext', () => {
	...
	test('shows Open Editors for regular Singularity CLI sessions with eligible editors', () => {
		assert.strictEqual(
			shouldShowOpenEditorsContext(widgetWithSession(URI.parse('singularitycli:/session-1')), true),
			true
		);
	});
	...
	test('hides Open Editors for Agent Host sessions with eligible editors', () => {
		assert.strictEqual(
			shouldShowOpenEditorsContext(widgetWithSession(URI.parse('agent-host-singularitycli:/session-1')), true),
			false
		);
	});
	...
	test('hides Open Editors for locked Agent Host ids without a session resource', () => {
		assert.strictEqual(
			shouldShowOpenEditorsContext(widget({ lockedAgentId: 'agent-host-singularitycli' }), true),
			false
		);
	});
	...
	test('hides Open Editors when there are no eligible editors', () => {
		assert.strictEqual(
			shouldShowOpenEditorsContext(widgetWithSession(URI.parse('singularitycli:/session-1')), false),
			false
		);
	});
});
