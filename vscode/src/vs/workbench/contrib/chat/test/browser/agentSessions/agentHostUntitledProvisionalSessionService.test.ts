...
function untitledChatUri(id: string): URI {
	return URI.from({ scheme: 'agent-host-singularity', path: `/untitled-${id}` });
}
...
suite('AgentHostUntitledProvisionalSessionService', () => {
	...
	test('getOrCreate creates one backend provisional and returns the same URI on repeat calls', async () => {
		...
		const [a, b] = await Promise.all([
			provisional.getOrCreate(ui, 'singularity', undefined),
			provisional.getOrCreate(ui, 'singularity', undefined),
		]);
	...
	test('publishes active-client customizations before the first prompt and keeps them updated', async () => {
		...
		await provisional.getOrCreate(untitledChatUri('customizations'), 'singularity', undefined);
	...
	test('getOrCreate includes Editor multi-root workspace metadata', async () => {
		...
		await provisional.getOrCreate(untitledChatUri('multi-root'), 'singularity', workspaceFolders[0]);
	...
	test('workspace folder changes recreate a multi-root provisional with the latest secondary set', async () => {
		...
		workbenchState = WorkbenchState.WORKSPACE;
		agentHost.rootStateAgents = [agentInfo('singularity', true)];
		const ui = untitledChatUri('multi-root-folder-changes');

		await provisional.getOrCreate(ui, 'singularity', primary);
	...
	test('tryRebind recomputes the latest multi-root folder set without relying on a workspace event', async () => {
		...
		const real = URI.from({ scheme: 'agent-host-singularity', path: '/real-multi-root-rebind' });

		await provisional.getOrCreate(ui, 'singularity', primary);
		workspaceFolders = [secondary, added];
		await provisional.tryRebind(ui, real, 'singularity', primary);
	...
	test('tryRebind promotes a single-folder draft when a second folder appears without a workspace event', async () => {
		...
		const real = URI.from({ scheme: 'agent-host-singularity', path: '/real-single-to-multi-root-rebind' });
	...
	test('getOrCreate omits multi-root metadata without a workspace configuration', async () => {
		...
		await provisional.getOrCreate(untitledChatUri('multi-root-no-config'), 'singularity', workspaceFolders[0]);
	...
	test('getOrCreate omits multi-root metadata in the Agents window', async () => {
		...
		await provisional.getOrCreate(untitledChatUri('agents-window'), 'singularity', workspaceFolders[0]);
	...
	test('getOrCreate does not spawn a backend provisional in an untrusted workspace', async () => {
		...
		const result = await provisional.getOrCreate(ui, 'singularity', undefined);
	...
	test('getOrCreate does not spawn a backend provisional in an untrusted working directory folder', async () => {
		...
		const result = await provisional.getOrCreate(ui, 'singularity', workingDirectory);
		assert.strictEqual(result, undefined);
		assert.strictEqual(agentHost.createCalls.length, 0);
		assert.strictEqual(provisional.get(ui), undefined);
	});
	...
	test('applyConfigChange dispatches SessionConfigChanged before schema re-resolution completes', async () => {
		...
		const promise = provisional.applyConfigChange(ui, 'singularity', undefined, { isolation: 'worktree' });
	...
	test('getResolvedConfig reflects the re-resolved schema/values after applyConfigChange', async () => {
		...
		await provisional.applyConfigChange(ui, 'singularity', undefined, { isolation: 'worktree' });
	...
	test('refreshResolvedConfig stores a schema overlay for running sessions', async () => {
		const ui = URI.from({ scheme: 'agent-host-singularity', path: '/real-j' });
		const resolved: ResolveSessionConfigResult = {
		...
		await provisional.refreshResolvedConfig(ui, 'singularity', undefined, { isolation: 'folder' });
	...
	test('refreshResolvedConfig ignores stale running-session responses', async () => {
		const ui = URI.from({ scheme: 'agent-host-singularity', path: '/real-k' });
		...
		const a = provisional.refreshResolvedConfig(ui, 'singularity', undefined, { isolation: 'worktree' });
		const b = provisional.refreshResolvedConfig(ui, 'singularity', undefined, { isolation: 'folder' });
	...
	test('optimistic merge: overlay.values reflects partial before re-resolve completes', async () => {
		...
		const promise = provisional.applyConfigChange(ui, 'singularity', undefined, { branch: 'feature/x' });
		for (let i = 0; i < 20; i++) {
			await Promise.resolve();
		}
		await timeout(0);
	...
	test('racing applyConfigChange calls: the second one wins (sequencer order)', async () => {
		...
		const a = provisional.applyConfigChange(ui, 'singularity', undefined, { isolation: 'worktree' });
		const b = provisional.applyConfigChange(ui, 'singularity', undefined, { isolation: 'folder' });
	...
	test('tryRebind waits for pending config reconciliation', async () => {
		...
		void provisional.applyConfigChange(ui, 'singularity', undefined, { isolation: 'worktree' });
		...
		const newUi = URI.from({ scheme: 'agent-host-singularity', path: '/real-g' });
		const rebind = provisional.tryRebind(ui, newUi, 'singularity', undefined);
	...
	test('tryRebind retries when config changes during final session creation', async () => {
		...
		const realUi = URI.from({ scheme: 'agent-host-singularity', path: '/real-config-race' });
		await provisional.getOrCreate(ui, 'singularity', undefined);
		...
		const rebind = provisional.tryRebind(ui, realUi, 'singularity', undefined);
		await timeout(0);
		const configChange = provisional.applyConfigChange(ui, 'singularity', undefined, { isolation: 'worktree' });
		gate.complete();
		...
		assert.deepStrictEqual({
			...
			rebound: URI.from({ scheme: 'singularity', path: '/real-config-race' }).toString(),
			current: URI.from({ scheme: 'singularity', path: '/real-config-race' }).toString(),
	...
	test('tryRebind disposes its candidate when the old entry is retired during creation', async () => {
		...
		const realUi = URI.from({ scheme: 'agent-host-singularity', path: '/real-dispose-race' });
		await provisional.getOrCreate(ui, 'singularity', undefined);
		const oldBackend = provisional.get(ui);
		assert.ok(oldBackend);
		const gate = new DeferredPromise<void>();
		...
		assert.deepStrictEqual({
			...
			disposed: [
				oldBackend.toString(),
				URI.from({ scheme: 'singularity', path: '/real-dispose-race' }).toString(),
			].sort(),
		});
	});
	...
	test('tryRebind restores an imported conversation when final creation fails', async () => {
		...
		const realUi = URI.from({ scheme: 'agent-host-singularity', path: '/real-import-failure' });
		await provisional.getOrCreate(ui, 'singularity', undefined);
		...
		const rebound = await provisional.tryRebind(ui, realUi, 'singularity', undefined);
		...
		assert.deepStrictEqual({
			...
			rebound: undefined,
			imported,
			disposed: [URI.from({ scheme: 'singularity', path: '/real-import-failure' }).toString()],
		});
	});
	...
	test('tryRebind blocks deterministic URI reuse until failed disposal is retried', async () => {
		...
		const realUi = URI.from({ scheme: 'agent-host-singularity', path: '/real-dispose-failure' });
		await provisional.getOrCreate(ui, 'singularity', undefined);
		const gate = new DeferredPromise<void>();
		cleanup.add({ dispose: () => gate.cancel() });
		agentHost.createGate = gate;
		...
		await configChange;
		const reboundUri = URI.from({ scheme: 'singularity', path: '/real-dispose-failure' });
		assert.deepStrictEqual({
	...
	test('failed re-resolve preserves the previous overlay', async () => {
		...
		await provisional.applyConfigChange(ui, 'singularity', undefined, { branch: 'feature/x' });
	...
	test('folder change recreates the provisional at the new cwd preserving config', async () => {
		...
		agentHost.resolveQueue = [{ schema: makeSchema(false), values: { isolation: 'worktree' } }];
		await provisional.applyConfigChange(ui, 'singularity', folderA, { isolation: 'worktree' });
	...
	test('folder change listeners can wait for the queued replacement', async () => {
		...
		await provisional.getOrCreate(ui, 'singularity', folderA);
	...
	test('failed folder replacement cleans up its candidate and recreates on retry', async () => {
		...
		const retried = await provisional.getOrCreate(ui, 'singularity', folderB);
		assert.deepStrictEqual({
	...
	test('config changed during creation retires the stale candidate', async () => {
		...
		cleanup.add({ dispose: () => gate.cancel() });
		agentHost.createGate = gate;
		const initialCreate = provisional.getOrCreate(ui, 'singularity', folder);
		await timeout(0);
		agentHost.resolveQueue = [{ schema: makeSchema(false), values: { isolation: 'worktree' } }];

		const configChange = provisional.applyConfigChange(ui, 'singularity', folder, { isolation: 'worktree' });
		gate.complete();
	...
	test('dispose queued behind creation cannot publish or deadlock', async () => {
		...
		cleanup.add({ dispose: () => gate.cancel() });
		agentHost.createGate = gate;
		const creation = provisional.getOrCreate(ui, 'singularity', URI.file('/repo'));
		await timeout(0);
	...
	test('derives the ordered working-directory set from the picked primary', async () => {
		...
		await provisional.getOrCreate(multiRoot, 'singularity', folderB);
		...
		await provisional.getOrCreate(singleRoot, 'singularity', folderA);
	...
	test('sends only the primary when the provider does not advertise multiple working directories', async () => {
		...
		await provisional.getOrCreate(multi, 'singularity', folderB);

		agentHost.rootStateAgents = [agentInfo('singularity', false)];
		const single = untitledChatUri('cap-single');
		await provisional.getOrCreate(single, 'singularity', folderB);
