...
suite('AgentHostGenericConfigChips', () => {
	...
	test('moves its subscription when the provisional generation changes', () => {
		const sessionResource = URI.parse('agent-host-singularity:/untitled-test');
		const firstBackend = URI.parse('singularity:/first-generation');
		const secondBackend = URI.parse('singularity:/second-generation');
