...
const CHAT_SESSION_RESOURCE = URI.from({ scheme: 'agent-host-singularitycli', path: '/abc-123' });
const BACKEND_SESSION = URI.from({ scheme: 'singularitycli', path: '/abc-123' });
...
suite('AgentSessionSettingsFileSystemProvider (editor-window per-session adapter)', () => {
	...
	test('URI routing: encodes and round-trips the backend session URI', () => {
		...
		assert.strictEqual(uri.authority, 'singularitycli');
	...
	suite('schema registration', () => {
		...
		const schemaId = `vscode://schemas/agent-session-settings/singularitycli/abc-123.jsonc`;
...
suite('workbench.action.chat.openAgentSessionSettings', () => {
	...
	test('appears in the local agent-host session context menu, not for remote or non-agent-host sessions', () => {
		...
		assert.strictEqual(evalWhen(item.when, { ...base, [ChatContextKeys.agentSessionType.key]: 'agent-host-singularitycli' }), true);
		assert.strictEqual(evalWhen(item.when, { ...base, [ChatContextKeys.agentSessionType.key]: 'remote-singularitycli' }), false);
		assert.strictEqual(evalWhen(item.when, { ...base, [ChatContextKeys.agentSessionType.key]: 'singularitycli' }), false);
		assert.strictEqual(evalWhen(item.when, { ...base, [ChatContextKeys.enabled.key]: false, [ChatContextKeys.agentSessionType.key]: 'agent-host-singularitycli' }), false);
	});
	...
	test('run() with a marshalled agent-session context routes via context.session, ignoring context.sessions', async () => {
		const session = makeAgentSession(CHAT_SESSION_RESOURCE);
		const otherSession = makeAgentSession(URI.from({ scheme: 'agent-host-singularitycli', path: '/other' }));
