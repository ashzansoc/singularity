...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	public agents = [{ provider: 'singularity' as const, displayName: 'Agent Host - Singularity', description: 'test', requiresAuth: true }];
	...
	override async createSession(config?: IAgentCreateSessionConfig): Promise<URI> {
		...
		const session = config?.session ?? AgentSession.uri('singularity', `sdk-session-${this._nextId++}`);
	...
	private _buildDefaultChatState(sessionUriStr: string, chatUriStr: string): ChatState {
		...
		const sessionSummary: SessionSummary = {
			...
			provider: seeded?.provider ?? 'singularity',
...
function createSessionListController(disposables: DisposableStore, instantiationService: TestInstantiationService, connection: IAgentHostSessionListConnection, sessionType = 'agent-host-singularity', provider = 'singularity', description: string | undefined = undefined): AgentHostSessionListController {
...
function createContribution(disposables: DisposableStore, opts?: { authServiceOverride?: Partial<IAuthenticationService>; workingDirectoryResolver?: { resolve(sessionResource: URI): URI | undefined; isNewSession?: (sessionResource: URI) => boolean }; languageModels?: ReadonlyMap<string, ILanguageModelChatMetadata>; provisionalServiceOverride?: Partial<IAgentHostUntitledProvisionalSessionService>; languageModelToolsServiceOverride?: Partial<ILanguageModelToolsService>; configOverrides?: Record<string, unknown>; provider?: string; chatSessionsServiceOverride?: Partial<IChatSessionsService>; chatDebugServiceOverride?: Partial<IChatDebugService>; remoteAgentHostServiceOverride?: Partial<IRemoteAgentHostService>; customizationServiceOverride?: IAgentHostCustomizationService; agentHostTerminalServiceOverride?: Partial<IAgentHostTerminalService>; languageModelsServiceOverride?: Partial<ILanguageModelsService>; workspaceFolders?: readonly URI[] }) {
	...
	const sessionHandler = disposables.add(instantiationService.createInstance(AgentHostSessionHandler, {
		provider: opts?.provider ?? 'singularity',
		agentId: 'agent-host-singularity',
		sessionType: 'agent-host-singularity',
		fullName: 'Agent Host - Singularity',
...
function createByokLanguageModelTestData(groupName?: string): { languageModels: ReadonlyMap<string, ILanguageModelChatMetadata>; languageModelsServiceOverride: Partial<ILanguageModelsService> } {
	...
	return {
		languageModels: new Map([
			...
			[`agent-host-singularity:${rawModelId}`, bridgedMetadata],
		]),
		languageModelsServiceOverride: {
			getVendors: () => [{
				vendor: providerVendor,
...
function makeRequest(overrides: Partial<{ message: string; sessionResource: URI; variables: IChatAgentRequest['variables']; userSelectedModelId: string; modelConfiguration: Record<string, unknown>; agentHostSessionConfig: Record<string, string>; agentId: string; requestId: string }> = {}): IChatAgentRequest {
	return upcastPartial<IChatAgentRequest>({
		...
		agentId: overrides.agentId ?? 'agent-host-singularity',
...
async function startTurn(
	...
	const agentId = overrides?.agentId ?? 'agent-host-singularity';
...
suite('AgentHostChatContribution', () => {
	...
	suite('registration', () => {
		...
		test('registers agent', () => {
			...
			assert.ok(chatAgentService.registeredAgents.has('agent-host-singularity'));
		});
	});
	...
	suite('request attachments', () => {
		...
		test('sends agent feedback variables as annotations attachments referencing each comment', async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/feedback-send' });
		...
		test('sends agent feedback variables as a simple attachment when no annotations channel is known', async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/feedback-send-fallback' });
			const feedbackFile = URI.file('/workspace/foo.ts');
			const { turnPromise, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, chatAgentService, disposables, {
				message: '/act-on-feedback',
				sessionResource,
		...
		test('restores session reference simple attachments as session reference variable entries', () => {
			const sessionResource = URI.from({ scheme: 'singularitycli', path: '/session-123' });
			const variableData = messageAttachmentsToVariableData([{
				type: MessageAttachmentKind.Simple,
				...
				uri: URI.file('/home/test-user/.singularity/session-state/session-123/events.jsonl').toString(),
				_meta: {
					[AgentHostSessionReferenceAttachmentMetadataKey]: {
						sessionResource: sessionResource.toString(),
						sessionID: sessionResource.toString(),
	...
	suite('draft', () => {
		...
		test('hydrates chat input state from AHP draft', async () => {
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-singularity:opus-4.7', upcastPartial<ILanguageModelChatMetadata>({ id: 'opus-4.7', name: 'Opus 4.7' })],
			]);
			const { sessionHandler, agentHostService } = createContribution(disposables, { languageModels });
			const backendSession = AgentSession.uri('singularity', 'draft-session');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/draft-session' });
			...
			assert.deepStrictEqual({
				...
				inputText: 'draft\ntext',
				model: 'agent-host-singularity:opus-4.7',
		...
		test('hydrates chat input attachments from AHP draft', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'draft-attachments');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/draft-attachments' });
		...
		test('creates an empty draft from the last request selection when none exists', async () => {
			...
			const { sessionHandler, agentHostService } = createContribution(disposables, { languageModels });
			const backendSession = AgentSession.uri('singularity', 'last-selection-session');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/last-selection-session' });
		...
		test('debounces chat input state into AHP draft', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-singularity:opus-4.7', modelMetadata],
			]);
			const { sessionHandler, agentHostService, chatService } = createContribution(disposables, { languageModels });
			const backendSession = AgentSession.uri('singularity', 'draft-sync');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/draft-sync' });
			...
			const inputState = observableValue<IChatModelInputState | undefined>('test.inputState', {
				attachments: [],
				mode: { id: 'agent://reviewer', kind: ChatModeKind.Agent },
				selectedModel: { identifier: 'agent-host-singularity:opus-4.7', metadata: modelMetadata },
		...
		test('flushes pending chat input draft when the session is disposed', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'draft-sync-dispose');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/draft-sync-dispose' });
		...
		test('applies a remote draft to a clean live input', async () => {
			...
			const { sessionHandler, agentHostService, chatService } = createContribution(disposables, { languageModels });
			const backendSession = AgentSession.uri('singularity', 'remote-draft');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/remote-draft' });
		...
		test('does not overwrite a locally edited input', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatService } = createContribution(disposables);
			const backendSession = AgentSession.uri('singularity', 'local-draft');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/local-draft' });
		...
		test('remote clear wipes text and attachments but keeps the model', async () => {
			...
			const { sessionHandler, agentHostService, chatService } = createContribution(disposables, { languageModels });
			const backendSession = AgentSession.uri('singularity', 'remote-clear');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/remote-clear' });
			const attachment = {
				type: MessageAttachmentKind.Simple,
				label: 'Pasted context',
		...
		test('does not echo a locally re-derived draft when the remote model is unavailable', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-singularity:fallback', fallbackModel],
			]);
			const { sessionHandler, agentHostService, chatService } = createContribution(disposables, { languageModels });
			const backendSession = AgentSession.uri('singularity', 'unavailable-remote-model');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/unavailable-remote-model' });
			...
			inputModel.setState({ selectedModel: { identifier: 'agent-host-singularity:fallback', metadata: fallbackModel } });
			await timeout(500);
	...
	suite('disposal', () => {
		...
		test('fires onWillDispose before session is disposed', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/dispose-test' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
		...
		test('disposing one chat does not tear down a sibling peer chat subscription (peer chat never loads after reload)', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/multi' });
			const backendSession = AgentSession.uri('singularity', 'multi');
			const peerResource = URI.from({ scheme: 'agent-host-singularity', path: '/multi', fragment: 'peer-1' });
	...
	suite('session list', () => {
		...
		test('refresh populates items from agent host', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'aaa'), startTime: 1000, modifiedTime: 2000, summary: 'My session' });
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'bbb'), startTime: 3000, modifiedTime: 4000 });
			...
			assert.strictEqual(listController.items[0].resource.scheme, 'agent-host-singularity');
		...
		test('refresh fires onDidChangeChatSessionItems', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'x'), startTime: 1000, modifiedTime: 2000 });
			await listController.refresh(CancellationToken.None);
		...
		test('refresh marks archived sessions as archived items', async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'archived'),
				startTime: 1000,
				modifiedTime: 2000,
		...
		test('leaves isRead unset for a session the host reports no status for', async () => {
			...
			// Neither Singularity's nor Claude's `listSessions` projects a status, so a
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'no-status'),
				startTime: 1000,
				modifiedTime: 2000,
		...
		test('adopts host read state once a summary reports a status', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'late-status');
			agentHostService.addSession({
		...
		test('archive mutations dispatch through AHP and reconcile server summaries', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'archivable');
			...
			const listController = disposables.add(instantiationService.createInstance(AgentHostSessionListController, 'agent-host-singularity', 'singularity', sessionListStore, undefined, 'local'));
			await listController.refresh(CancellationToken.None);
			agentHostService.dispatchedActions.length = 0;
			...
			listController.setChatSessionItemArchived(resource, true);
			const archivedStatus = sessionListStore.getSessions('singularity')[0].summary.status;

			listController.setChatSessionItemArchived(resource, false);
			listController.setChatSessionItemArchived(resource, false);
			const unarchivedStatus = sessionListStore.getSessions('singularity')[0].summary.status;
		...
		test('read mutations dispatch through AHP and reconcile server summaries', async () => {
			const { instantiationService, agentHostService } = createTestServices(disposables);
			const backendSession = AgentSession.uri('singularity', 'readable');
			...
			listController.setChatSessionItemRead(resource, false);
			const unreadStatus = sessionListStore.getSessions('singularity')[0].summary.status;
		...
		test('archive mutation prevents an in-flight stale refresh from overwriting optimistic state', async () => {
			const { instantiationService, agentHostService } = createTestServices(disposables);
			const backendSession = AgentSession.uri('singularity', 'archive-refresh-race');
		...
		test('archive mutation invalidates an in-flight refresh before the session is cached', async () => {
			const { instantiationService, agentHostService } = createTestServices(disposables);
			const backendSession = AgentSession.uri('singularity', 'uncached-archive-refresh-race');
			const metadata: IAgentSessionMetadata = {
				session: backendSession,
				startTime: 1000,
				modifiedTime: 2000,
			...
			const refresh = listController.refresh(CancellationToken.None);
			await timeout(0);
			listController.setChatSessionItemArchived(URI.from({ scheme: 'agent-host-singularity', path: `/${AgentSession.id(backendSession)}` }), true);
			releaseListSessions.fire();
			await refresh;
		...
		test('controllers sharing a connection coalesce their listSessions RPCs', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'aaa'), startTime: 1000, modifiedTime: 2000, summary: 'Singularity session' });
			...
			const sessionListStore = createSessionListStore(disposables, instantiationService, agentHostService);
			const singularityController = disposables.add(instantiationService.createInstance(AgentHostSessionListController, 'agent-host-singularity', 'singularity', sessionListStore, undefined, 'local'));
			...
			const refreshSingularity = singularityController.refresh(CancellationToken.None);
			const refreshOther = otherController.refresh(CancellationToken.None);
			await timeout(0);
			assert.strictEqual(listCalls, 1);

			releaseListSessions.fire();
			await Promise.all([refreshSingularity, refreshOther]);

			assert.deepStrictEqual({
				listCalls,
				singularity: singularityController.items.map(item => item.label),
				other: otherController.items.map(item => item.label),
			}, {
				listCalls: 1,
				singularity: ['Singularity session'],
				other: [],
			});
		});
		...
		test('shared session list store keeps same raw id separate by provider', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'same'), startTime: 1000, modifiedTime: 2000, summary: 'Singularity session' });
			...
			await Promise.all([
				singularityController.refresh(CancellationToken.None),
		...
		test('sessionRemoved during initial refresh does not let stale listSessions response resurrect the session', async () => {
			...
			const staleSession = { session: AgentSession.uri('singularity', 'removed-before-cache'), startTime: 1000, modifiedTime: 2000, summary: 'Stale session' };
			let listCalls = 0;
			const releaseListSessions = disposables.add(new Emitter<void>());
			const released = Event.toPromise(releaseListSessions.event);
			agentHostService.listSessions = async () => {
			...
			const notification: INotification = {
				type: 'root/sessionRemoved',
				channel: 'ahp-root://',
				session: AgentSession.uri('singularity', 'removed-before-cache').toString(),
			};
			agentHostService.fireNotification(notification);
			releaseListSessions.fire();
			await refresh;
		...
		test('shared session list notifications project only to matching provider controllers', () => {
			...
			const singularityController = disposables.add(instantiationService.createInstance(AgentHostSessionListController, 'agent-host-singularity', 'singularity', sessionListStore, undefined, 'local'));
			const otherController = disposables.add(instantiationService.createInstance(AgentHostSessionListController, 'agent-host-other', 'other', sessionListStore, undefined, 'local'));
			const singularityEvents: string[][] = [];
			const otherEvents: string[][] = [];
			disposables.add(singularityController.onDidChangeChatSessionItems(delta => singularityEvents.push((delta.addedOrUpdated ?? []).map(item => item.label))));
			...
			agentHostService.fireNotification({
				...
				channel: ROOT_STATE_URI,
				summary: {
					resource: AgentSession.uri('singularity', 'notify').toString(),
					provider: 'singularity',
					title: 'Singularity notification',
					status: SessionStatus.Idle,
			...
			assert.deepStrictEqual({
				singularityItems: singularityController.items.map(item => item.label),
				otherItems: otherController.items.map(item => item.label),
				singularityEvents,
				otherEvents,
			}, {
				singularityItems: ['Singularity notification'],
				otherItems: ['Other notification'],
				singularityEvents: [['Singularity notification']],
		...
		test('summaryChanged notification emits only the changed item and reflects the update', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'a'), startTime: 1000, modifiedTime: 2000, summary: 'A' });
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'b'), startTime: 1000, modifiedTime: 2000, summary: 'B' });
			...
			agentHostService.fireNotification({
				type: 'root/sessionSummaryChanged',
				channel: 'ahp-root://',
				session: AgentSession.uri('singularity', 'b').toString(),
		...
		test('sessionRemoved notification removes only the matching item', async () => {
			...
			agentHostService.fireNotification({
				type: 'root/sessionRemoved',
				channel: 'ahp-root://',
				session: AgentSession.uri('singularity', 'a').toString(),
			} as INotification);
		...
		test('agent host restart invalidates cache so next refresh re-fetches', async () => {
			...
			agentHostService.setRootState({
				agents: [{ provider: 'singularity' as const, displayName: 'Agent Host - Singularity', description: 'test', models: [] }],
				activeSessions: 0,
			});
			assert.strictEqual(chatSessionItemControllers.length, 1);
			const listController = chatSessionItemControllers[0].controller;

			agentHostService.addSession({ session: AgentSession.uri('singularity', 'aaa'), startTime: 1000, modifiedTime: 2000, summary: 'Before restart' });
		...
		test('agent host restart during refresh does not let stale listSessions response mark cache valid', async () => {
			...
			agentHostService.listSessions = async () => {
				...
				if (listCalls === 1) {
					await released;
					return [{ session: AgentSession.uri('singularity', 'stale-before-restart'), startTime: 1000, modifiedTime: 2000, summary: 'Stale session' }];
				}
				return [{ session: AgentSession.uri('singularity', 'fresh-after-restart'), startTime: 3000, modifiedTime: 4000, summary: 'Fresh session' }];
			};
		...
		test('refresh filters out sessions whose workingDirectory is not in any workspace folder', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'in-ws'), startTime: 1000, modifiedTime: 2000, summary: 'In workspace', workingDirectories: [URI.file('/workspace/root/sub')] });
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'tail-in-ws'), startTime: 1000, modifiedTime: 2000, summary: 'Tail in workspace', workingDirectories: [URI.file('/other/primary'), URI.file('/workspace/root/sub')] });
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'out-ws'), startTime: 1000, modifiedTime: 2000, summary: 'Outside workspace', workingDirectories: [URI.file('/other/place')] });
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'no-wd'), startTime: 1000, modifiedTime: 2000, summary: 'No working directory' });
		...
		test('refresh does not filter when no workspace folders are open', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'a'), startTime: 1000, modifiedTime: 2000, summary: 'A', workingDirectories: [URI.file('/any/path')] });
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'b'), startTime: 1000, modifiedTime: 2000, summary: 'B' });
		...
		test('multi-root workspace filtering uses workspace-file metadata', async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'matching-workspace-file'),
				startTime: 1000,
				modifiedTime: 2000,
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'different-workspace-file'),
				startTime: 1000,
				modifiedTime: 2000,
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'metadata-less'),
				startTime: 1000,
				modifiedTime: 2000,
		...
		test('multi-root session is shown in an empty window', async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'multi-root'),
				startTime: 1000,
				modifiedTime: 2000,
		...
		test('multi-root session is shown in a single-folder window that is one of its roots', async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'contains-folder'),
				startTime: 1000,
				modifiedTime: 2000,
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'other-roots'),
				startTime: 1000,
				modifiedTime: 2000,
				summary: 'Other roots',
		...
		test('sessionAdded notification filters out sessions outside the workspace', async () => {
			...
			agentHostService.fireNotification({
				...
				channel: ROOT_STATE_URI,
				summary: {
					resource: AgentSession.uri('singularity', 'foreign').toString(),
					provider: 'singularity',
			...
			agentHostService.fireNotification({
				...
				channel: ROOT_STATE_URI,
				summary: {
					resource: AgentSession.uri('singularity', 'tail-match').toString(),
					provider: 'singularity',
			...
			agentHostService.fireNotification({
				...
				channel: ROOT_STATE_URI,
				summary: {
					resource: AgentSession.uri('singularity', 'local').toString(),
					provider: 'singularity',
					title: 'Local session',
					status: SessionStatus.Idle,
					createdAt: new Date(1000).toISOString(),
		...
		test('deleteChatSessionItem disposes the corresponding backend session and removes it from the cached items', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'session-to-delete'), startTime: 1000, modifiedTime: 2000, summary: 'Doomed' });
			await listController.refresh(CancellationToken.None);
			assert.strictEqual(listController.items.length, 1);
			...
			assert.deepStrictEqual(agentHostService.disposedSessions.map(s => s.toString()), [AgentSession.uri('singularity', 'session-to-delete').toString()]);
			assert.strictEqual(listController.items.length, 0);
		...
		test('newChatSessionItem creates final-looking resource used for requested backend session', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.ok(item);
			assert.strictEqual(item.resource.scheme, 'agent-host-singularity');
			...
			assert.strictEqual(agentHostService.createSessionCalls[0].session?.toString(), AgentSession.uri('singularity', item.resource.path.substring(1)).toString());
		...
		test('newChatSessionItem rebinds untitled provisional to real resource so chip-selected config survives first send', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const untitledResource = URI.from({ scheme: 'agent-host-singularity', path: '/untitled-abc' });
		...
		test('newChatSessionItem routes the store-selected folder as the working directory in multi-root windows', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const untitledResource = URI.from({ scheme: 'agent-host-singularity', path: '/untitled-route' });
		...
		test('folder picker is visible only in multi-root agent-host editor windows', () => {
			...
			const agentHost = {
				[ChatContextKeys.lockedCodingAgentId.key]: 'agent-host-singularity',
			...
			assert.deepStrictEqual({
				...
				nonAgentHost: evalWhen({ [ChatContextKeys.lockedCodingAgentId.key]: 'singularity', [ChatContextKeys.chatIsAgentHostSession.key]: false, workspaceFolderCount: 2, isSessionsWindow: false }),
			}, {
	...
	suite('session ID resolution', () => {
		...
		test('reuses SDK session for same resource on second message', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const resource = URI.from({ scheme: 'agent-host-singularity', path: '/new-reuse' });
			...
			const registered = chatAgentService.registeredAgents.get('agent-host-singularity')!;
		...
		test('uses sessionId from agent-host scheme resource', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const { turnPromise, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, chatAgentService, disposables, {
				message: 'Hi',
				sessionResource: URI.from({ scheme: 'agent-host-singularity', path: '/existing-session-42' }),
			});
			fire({ type: 'chat/turnComplete', endedAt: '2025-01-01T00:00:00.000Z', session, turnId } as ChatAction);
			await turnPromise;
		...
		test('waits for provisional replacement before resolving first-send backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/untitled-replacement' });
			const oldBackend = AgentSession.uri('singularity', 'old-provisional');
			const newBackend = AgentSession.uri('singularity', 'new-provisional');
		...
		test('recovers from stale failed subscription before first send', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/existing-subscribe-retry' });
			const backendSession = AgentSession.uri('singularity', 'existing-subscribe-retry');
			const { agentHostService, chatAgentService } = createContribution(disposables, {
				provisionalServiceOverride: {
		...
		test('recovers when session subscription errors AFTER being observed pending (repro #5242 hang)', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			// Regression for the agent-host-singularitycli session-subscribe race
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/pending-then-error-race' });
			const backendSession = AgentSession.uri('singularity', 'pending-then-error-race');
			const { agentHostService, chatAgentService } = createContribution(disposables, {
				provisionalServiceOverride: {
					get: resource => resource.toString() === sessionResource.toString() ? backendSession : undefined,
				},
		...
		test('rejects generic contributed-chat untitled resource', async () => {
			...
			await assert.rejects(
				() => sessionHandler.provideChatSessionContent(URI.from({ scheme: 'agent-host-singularity', path: '/untitled-abc123' }), CancellationToken.None),
				/created by the sessions provider/
			);
			assert.strictEqual(agentHostService.createSessionCalls.length, 0);
			assert.strictEqual(chatAgentService.registeredAgents.has('agent-host-singularity'), true);
		});
		test('passes raw model id extracted from language model identifier', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const { turnPromise, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, chatAgentService, disposables, {
				message: 'Hi',
				userSelectedModelId: 'agent-host-singularity:claude-sonnet-4-20250514',
			});
			fire({ type: 'chat/turnComplete', endedAt: '2025-01-01T00:00:00.000Z', session, turnId } as ChatAction);
			await turnPromise;
		...
		test('drops foreign vendor model id (issue #319583)', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			// `singularitycli` participant), then switches to the agent-host mode,
			...
			const { turnPromise, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, chatAgentService, disposables, {
				message: 'Hi',
				userSelectedModelId: 'singularitycli/claude-sonnet-4.6',
			});
			fire({ type: 'chat/turnComplete', endedAt: '2025-01-01T00:00:00.000Z', session, turnId } as ChatAction);
			await turnPromise;
		...
		test('does not create backend session eagerly for untitled sessions', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-deferred' });
	...
	suite('workspace trust', () => {
		...
		test('aborts the turn without creating a session when trust is declined', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/trust-declined' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
	...
	suite('preparing session status', () => {
		...
		test('an early rejection cancels the pending status', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const collected: IChatProgress[][] = [];
			const registered = chatAgentService.registeredAgents.get('agent-host-singularity')!;
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/import-reject' });
			const turnPromise = registered.impl.invoke(
	...
	suite('progress routing', () => {
		...
		test('Auto model routing metadata is not surfaced as a live resolution part', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-singularity:gpt-5.4-mini', upcastPartial<ILanguageModelChatMetadata>({ name: 'GPT-5.4 mini' })],
			]);
		...
		test('live turn returns model credit details from usage', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-singularity:opus-4.7', upcastPartial<ILanguageModelChatMetadata>({ name: 'Opus 4.7', pricing: '15x' })],
			]);
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables, { languageModels });
			...
			assert.strictEqual(result.details, '1.5 credits');
		}));
		...
		test('live BYOK turn includes a configured group between provider and model', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(result.details, '1 credit');
		}));
		...
		test('live turn renders zero-cost usage as 0 credits instead of multiplier', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(result.details, '0 credits');
		}));
		...
		test('slug-normalised billed id resolves without suffix (e.g. claude-sonnet-4-6 → claude-sonnet-4.6)', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-singularity:claude-sonnet-4.6', upcastPartial<ILanguageModelChatMetadata>({ name: 'Claude Sonnet 4.6', pricing: '1x' })],
			]);
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables, { languageModels });
		...
		test('unregistered billed id does not reveal routed model identity', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			// Free accounts only have "Auto" registered; usage.model reports the concrete model.
			// Singularity still hides the routed model name and only surfaces credits.
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-singularity:auto', upcastPartial<ILanguageModelChatMetadata>({ name: 'Auto' })],
			]);
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables, { languageModels });

			const { turnPromise, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, chatAgentService, disposables,
				{ userSelectedModelId: 'agent-host-singularity:auto' });
		...
		test('subagent credits surface on the subagent tool without re-aggregating into parent usage', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(lastUsage!.singularityCredits, 2.0, 'parent session cost is not re-aggregated on the client');
		...
		test('events from other sessions are ignored', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			agentHostService.fireAction({
				channel: AgentSession.uri('singularity', 'other-session').toString(),
		...
		test('plan-review completion from another client clears local plan review without redispatching', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-plan-review-test' });
		...
		test('plan-review cancellation clears docked widget even when review was already dismissed', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/dismissed-plan-review-test' });
			chatWidgetService.setWidgetForSession(sessionResource);
			const { turnPromise, collected, turnId, fire } = await startTurn(sessionHandler, agentHostService, chatAgentService, disposables, { sessionResource });
		...
		test('input request completion from another client clears local question carousel', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-input-request-test' });
			chatWidgetService.setWidgetForSession(sessionResource);
		...
		test('input request completion echo applies authoritative answers after local submit', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-local-input-request-test' });
			chatWidgetService.setWidgetForSession(sessionResource);
		...
		test('input request cancellation does not show draft answers as submitted', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-cancelled-input-request-test' });
			chatWidgetService.setWidgetForSession(sessionResource);
		...
		test('input request accepted without answers is marked answered, not skipped', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-answered-externally-input-request-test' });
			chatWidgetService.setWidgetForSession(sessionResource);
	...
	suite('permission requests', () => {
		...
		test('terminal command needing confirmation renders exactly one card (#314858)', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			// Reproduce the REAL Singularity SDK sequence: `onToolStart` optimistically
		...
		test('re-confirmation (Running → PendingConfirmation) reuses the same card and dispatches again', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			// (Running → PendingConfirmation — the shape every Singularity permission
	...
	suite('history loading', () => {
		...
		test('archived session read-only state follows session status', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'archived-session');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/archived-session' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('non-archived session read-only state follows chat interactivity', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'read-only-chat');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({
					resource: sessionUri.toString(),
					provider: 'singularity',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/read-only-chat' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
			assert.ok(session.isReadOnly);
		...
		test('new session read-only state follows status after materialization', async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-read-only-session' });
			...
			const backendSession = AgentSession.uri('singularity', 'new-read-only-session');
		...
		test('eager-created session read-only state follows status after materialization', async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			const backendSession = AgentSession.uri('singularity', 'new-eager-read-only-session');
			agentHostService.sessionStates.set(backendSession.toString(), {
				...createSessionState({
					resource: backendSession.toString(),
					provider: 'singularity',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-eager-read-only-session' });
			const { chatSession, turnPromise, fire, session, turnId } = await startTurn(
				sessionHandler,
				agentHostService,
				chatAgentService,
		...
		test('loads user and assistant messages into history', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'sess-1');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [{
					id: 'turn-1',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/sess-1' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('restores agent feedback attachments into request history variable data', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/feedback-history' });
			const feedbackFile = URI.file('/workspace/foo.ts');
			const sessionUri = AgentSession.uri('singularity', 'feedback-history');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [{
		...
		test('restores agent feedback annotations attachments as one aggregated request variable', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/feedback-annotations-history' });
			const annotationsResource = 'ahp-session:/feedback-annotations-history/annotations';
			const feedbackFile = URI.file('/workspace/foo.ts');
			const feedbackFile2 = URI.file('/workspace/bar.ts');
			const sessionUri = AgentSession.uri('singularity', 'feedback-annotations-history');
		...
		test('restores agent host completion attachments as hidden request variables', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/completion-history' });
			const sessionUri = AgentSession.uri('singularity', 'completion-history');
			const skillMeta = {
		...
		test('restores chat references to a never-opened cross-session chat as the opaque backend URI', async () => {
			...
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'driven');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/driven' });

			const otherBackend = AgentSession.uri('singularity', 'other-sess').toString();
		...
		test('untitled sessions have empty history', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-xyz' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('history requests get per-turn modelId from usage or message model', async () => {
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-singularity:opus-4.7', upcastPartial<ILanguageModelChatMetadata>({ name: 'Opus 4.7', pricing: '15x' })],
				['agent-host-singularity:sonnet-4.6', upcastPartial<ILanguageModelChatMetadata>({ name: 'Sonnet 4.6', pricing: '2x' })],
			]);
			const { sessionHandler, agentHostService } = createContribution(disposables, { languageModels });

			const sessionUri = AgentSession.uri('singularity', 'sess-models');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({
					resource: sessionUri.toString(), provider: 'singularity', title: 'Test',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/sess-models' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
			...
			assert.deepStrictEqual(
				...
				[
					{ prompt: 'Q1', modelId: 'agent-host-singularity:opus-4.7' },
					{ prompt: 'Q2', modelId: 'agent-host-singularity:sonnet-4.6' },
					{ prompt: 'Q3', modelId: 'agent-host-singularity:sonnet-4.6' },
				],
			);
			...
			assert.deepStrictEqual(
				...
				['1.5 credits', undefined, '1 credit'],
			);
	...
	suite('tool invocation rendering', () => {
		...
		test('terminal metadata is observable before terminal revival completes', async () => {
			...
			assert.deepStrictEqual({
				...
				terminalToolSessionId: JSON.stringify({ terminal: 'agenthost-terminal://bang/live-shell', session: 'singularity:/new-turntest' }),
				revivedTerminalUri: 'agenthost-terminal://bang/live-shell',
				revivedTerminalToolSessionId: JSON.stringify({ terminal: 'agenthost-terminal://bang/live-shell', session: 'singularity:/new-turntest' }),
		...
		test('completed output-only terminal retires its live attachment before finalizing its retained snapshot', async () => {
			...
			assert.deepStrictEqual(attached, {
				...
				terminalToolSessionId: JSON.stringify({ terminal: 'agenthost-terminal://shell/output', session: 'singularity:/new-turntest' }),
			});
			let stateNotifications = 0;
	...
	suite('history with tool events', () => {
		...
		test('tool_start and tool_complete appear as toolInvocationSerialized in history', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'tool-hist');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/tool-hist' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('restores subagent pills from the chat catalog when tool metadata was lost', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'subagent-history');
			...
			const summary = { resource: sessionUri.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() };
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState(summary),
				lifecycle: SessionLifecycle.Ready,
				defaultChat: defaultChatUri,
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/subagent-history' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
			const response = chatSession.history[1];
			assert.strictEqual(response.type, 'response');
		...
		test('opens a subagent editor from its exact chat URI before the chat catalog hydrates', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'direct-subagent');
			const defaultChatUri = buildDefaultChatUri(sessionUri.toString());
			const childChatUri = buildSubagentChatUri(sessionUri.toString(), 'tc-direct');
			const summary = { resource: sessionUri.toString(), provider: 'singularity', title: 'Parent', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() };
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState(summary),
				lifecycle: SessionLifecycle.Ready,
				defaultChat: defaultChatUri,
		...
		test('resolved input requests preserve their stream position and answers in history', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'input-history');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/input-history' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('orphaned tool_start is marked complete in history', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'orphan-tool');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/orphan-tool' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('non-terminal tool_complete sets pastTenseMessage in history', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'generic-tool');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/generic-tool' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('empty session produces empty history', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'empty-sess');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/empty-sess' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
	...
	suite('session list provider filtering', () => {
		...
		test('filters sessions to only the matching provider', async () => {
			...
			// Add sessions from both providers (use a non-singularity scheme to test filtering)
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'cp-1'), startTime: 1000, modifiedTime: 2000 });
			agentHostService.addSession({ session: URI.from({ scheme: 'other-provider', path: '/cl-1' }), startTime: 1000, modifiedTime: 2000 });
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'cp-2'), startTime: 3000, modifiedTime: 4000 });

			await listController.refresh(CancellationToken.None);

			// The list controller is configured for 'singularity', so only singularity sessions
			assert.strictEqual(listController.items.length, 2);
			assert.ok(listController.items.every(item => item.resource.scheme === 'agent-host-singularity'));
		});
	});
	...
	suite('language model provider', () => {
		...
		test('maps models with correct metadata', async () => {
			const provider = disposables.add(new AgentHostLanguageModelProvider('agent-host-singularity', 'agent-host-singularity'));
			provider.updateModels([
				{ provider: 'singularity', id: 'gpt-4o', name: 'GPT-4o', maxContextWindow: 128000, maxPromptTokens: 128000, supportsVision: true, _meta: { multiplierNumeric: 1.5 } },
			]);

			const models = await provider.provideLanguageModelChatInfo({}, CancellationToken.None);

			assert.strictEqual(models.length, 1);
			assert.strictEqual(models[0].identifier, 'agent-host-singularity:gpt-4o');
			...
			assert.strictEqual(models[0].metadata.targetChatSessionType, 'agent-host-singularity');
		});
		...
		test('maps cost metadata from _meta onto model metadata', async () => {
			...
			provider.updateModels([
				{
					provider: 'singularity', id: 'claude-sonnet', name: 'Claude Sonnet', maxContextWindow: 200000, supportsVision: false,
					_meta: {
						multiplierNumeric: 1,
						inputCost: 3,
						cacheCost: 1,
		...
		test('filters out disabled models', async () => {
			const provider = disposables.add(new AgentHostLanguageModelProvider('agent-host-singularity', 'agent-host-singularity'));
			provider.updateModels([
				{ provider: 'singularity', id: 'gpt-4o', name: 'GPT-4o', maxContextWindow: 128000, supportsVision: false, policyState: PolicyState.Enabled },
				{ provider: 'singularity', id: 'gpt-3.5', name: 'GPT-3.5', maxContextWindow: 16000, supportsVision: false, policyState: PolicyState.Disabled },
			]);
	...
	suite('attachment context', () => {
		...
		test('local agent host Singularity CLI session reference becomes simple and trajectory attachments', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			const sessionReference = URI.from({ scheme: 'agent-host-singularitycli', path: '/session-123' });
			const trajectoryUri = URI.file('/home/test-user/.singularity/session-state/session-123/events.jsonl');
		...
		test('chat reference variable entry becomes a chat attachment', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-turntest' });
		...
		test('extension host Singularity CLI session reference becomes simple and trajectory attachments', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			const sessionReference = URI.from({ scheme: 'singularitycli', path: '/session-123' });
			const trajectoryUri = URI.file('/home/test-user/.singularity/session-state/session-123/events.jsonl');
		...
		test('remote agent host Singularity CLI session reference becomes simple and trajectory attachments', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables, {
			...
			const sessionReference = URI.from({ scheme: 'remote-remote.example-singularitycli', path: '/session-123' });
			const trajectoryPath = '/home/remote-user/.singularity/session-state/session-123/events.jsonl';
		...
		test('agent feedback variable becomes a simple attachment when no annotations channel is known', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/feedback-test' });
		...
		test('active editor implicit context is not forwarded when the setting is disabled', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-implicit-disabled' });
		...
		test('active editor implicit context is forwarded as a document attachment for agent sessions', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-implicit-active-editor' });
			const fileUri = URI.file('/workspace/foo.ts');
			chatWidgetService.setWidgetForSession(sessionResource, [
				{ kind: 'implicit', id: 'vscode.implicit.file', name: 'foo.ts', isSelection: false, uri: fileUri, value: fileUri },
			]);
		...
		test('browser implicit context is not forwarded as an attachment', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-implicit-browser' });
		...
		test('active editor implicit context is not duplicated when also attached explicitly', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-implicit-dedup' });
			const fileUri = URI.file('/workspace/foo.ts');
			chatWidgetService.setWidgetForSession(sessionResource, [
				{ kind: 'implicit', id: 'vscode.implicit.file', name: 'foo.ts', isSelection: false, uri: fileUri, value: fileUri },
			]);
		...
		test('active editor implicit selection is forwarded even when the same file is attached as a document', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-implicit-selection-dedup' });
			const fileUri = URI.file('/workspace/foo.ts');
			chatWidgetService.setWidgetForSession(sessionResource, [
		...
		test('active editor implicit context is not forwarded for untitled editors on non-Singularity-CLI backends', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-implicit-untitled' });
		...
		test('active editor implicit context for an untitled editor is inlined for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService, modelService } = createContribution(disposables, { provider: 'singularitycli' });
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/singularity-implicit-untitled' });
			const untitledUri = URI.from({ scheme: 'untitled', path: '/Untitled-1' });
		...
		test('active editor implicit context for a dirty saved file is inlined for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService, modelService, workingCopyService } = createContribution(disposables, { provider: 'singularitycli' });
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/singularity-implicit-dirty' });
			const fileUri = URI.file('/workspace/foo.ts');
		...
		test('active editor implicit selection in an unsaved editor preserves the selection range when inlined for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService, modelService, workingCopyService } = createContribution(disposables, { provider: 'singularitycli' });
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/singularity-implicit-dirty-selection' });
			const fileUri = URI.file('/workspace/foo.ts');
		...
		test('active editor implicit context for a clean saved file is forwarded as a path for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService, modelService } = createContribution(disposables, { provider: 'singularitycli' });
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/singularity-implicit-clean' });
			const fileUri = URI.file('/workspace/foo.ts');
		...
		test('active editor implicit context over the size cap is dropped for an untitled editor on the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService, modelService } = createContribution(disposables, { provider: 'singularitycli' });
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/singularity-implicit-untitled-toolarge' });
			const untitledUri = URI.from({ scheme: 'untitled', path: '/Untitled-1' });
		...
		test('active editor implicit context over the size cap falls back to a path for a dirty saved file on the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService, modelService, workingCopyService } = createContribution(disposables, { provider: 'singularitycli' });
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/singularity-implicit-dirty-toolarge' });
			const fileUri = URI.file('/workspace/foo.ts');
		...
		test('explicitly attached untitled editor is inlined for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, modelService } = createContribution(disposables, { provider: 'singularitycli' });
			const untitledUri = URI.from({ scheme: 'untitled', path: '/Untitled-1' });
			modelService.setModelContent(untitledUri, 'console.log("draft")');
		...
		test('explicitly attached dirty saved file is inlined for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, modelService, workingCopyService } = createContribution(disposables, { provider: 'singularitycli' });
			const fileUri = URI.file('/workspace/foo.ts');
			modelService.setModelContent(fileUri, 'edited but not saved');
			workingCopyService.setDirty(fileUri, true);
		...
		test('explicitly attached clean saved file is forwarded as a path for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, modelService } = createContribution(disposables, { provider: 'singularitycli' });
			const fileUri = URI.file('/workspace/foo.ts');
			modelService.setModelContent(fileUri, 'saved content');
		...
		test('explicitly attached unsaved file is not duplicated by the active editor for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService, modelService, workingCopyService } = createContribution(disposables, { provider: 'singularitycli' });
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/singularity-explicit-implicit-dedup' });
			const fileUri = URI.file('/workspace/foo.ts');
			modelService.setModelContent(fileUri, 'edited but not saved');
			workingCopyService.setDirty(fileUri, true);
			chatWidgetService.setWidgetForSession(sessionResource, [
		...
		test('inlined unsaved attachment preserves _meta and selection for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, modelService, workingCopyService } = createContribution(disposables, { provider: 'singularitycli' });
			const fileUri = URI.file('/workspace/foo.ts');
			modelService.setModelContent(fileUri, 'first line\nsecond line\nthird line\nfourth line content');
			workingCopyService.setDirty(fileUri, true);
		...
		test('dirty non-file resource that cannot be inlined is dropped for the Singularity CLI backend', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, workingCopyService } = createContribution(disposables, { provider: 'singularitycli' });
		...
		test('rebases file/directory/selection attachments under requested working dir onto resolved working dir', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				variables: {
					variables: [
						...
						upcastPartial({
							...
							name: 'Feedback',
							value: 'Feedback text for the model',
							sessionResource: URI.from({ scheme: 'agent-host-singularity', path: '/new-turntest' }),
							feedbackItems: [{
								id: 'feedback-1',
								text: 'Please simplify this.',
			...
			assert.deepStrictEqual(turnAction.message.attachments, [
				...
				},
				{
					...
					_meta: {
						[AgentFeedbackAttachmentMetadataKey]: {
							sessionResource: 'agent-host-singularity:/new-turntest',
							feedbackItems: [{
								id: 'feedback-1',
								text: 'Please simplify this.',
		...
		test('does not rebase attachments owned by a nested secondary working directory', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			agentHostService.setRootState({
				agents: [{
					provider: 'singularity',
					displayName: 'Agent Host - Singularity',
					description: 'test',
					models: [],
	...
	suite('dynamic discovery', () => {
		...
		test('local agent contribution advertises image attachments', () => {
			...
			assert.deepStrictEqual(chatSessionContributions.map(c => ({ type: c.type, supportsImageAttachments: c.capabilities?.supportsImageAttachments })), [
				{ type: 'agent-host-singularity', supportsImageAttachments: true },
			]);
		...
		test('session list contribution registers item controller in editor window', () => {
			...
			assert.deepStrictEqual(chatSessionItemControllers.map(c => c.type), ['agent-host-singularity']);
		});
	...
	suite('IAgentConnection config', () => {
		...
		test('handler uses custom extensionId from config', async () => {
			...
			disposables.add(instantiationService.createInstance(AgentHostSessionHandler, {
				provider: 'singularity' as const,
				agentId: 'remote-test-singularity',
				sessionType: 'remote-test-singularity',
				fullName: 'Remote Singularity',
			...
			const registered = chatAgentService.registeredAgents.get('remote-test-singularity');
			assert.ok(registered);
		...
		test('handler waits for an in-flight provisional before falling back to direct creation', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'new-pending-config');
			const { instantiationService, agentHostService, chatAgentService } = createTestServices(
				disposables,
			...
			agentHostService.sessionStates.set(backendSession.toString(), {
				...createSessionState({ resource: backendSession.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			});
		...
		test('handler cancellation during eager-state hydration does not create a fallback session', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { agentHostService, chatAgentService } = createContribution(disposables);
			const cancellation = disposables.add(new CancellationTokenSource());
			const sessionUri = AgentSession.uri('singularity', 'new-cancel-eager-hydration');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-cancel-eager-hydration' });
		...
		test('handler forwards request session config via SessionConfigChanged on eager-create path', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'eager-config');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/eager-config' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('handler resolves authentication before sending to an eager-created session', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'eager-auth');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/eager-auth' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('handler does not clobber picker-set session config on eager-create path', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'eager-picker');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/eager-picker' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('handler awaits in-flight eager createSession before falling through to a duplicate create (issue #319764)', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'new-race');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-race' });
		...
		test('list controller includes description in items', async () => {
			...
			const controller = createSessionListController(disposables, instantiationService, agentHostService, 'remote-test', 'singularity', 'My Remote Host');

			agentHostService.addSession({ session: AgentSession.uri('singularity', 'sess-1'), startTime: 1000, modifiedTime: 2000, summary: 'Test session' });
		...
		test('list controller omits description when undefined', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('singularity', 'sess-2'), startTime: 1000, modifiedTime: 2000, summary: 'Test' });
			await controller.refresh(CancellationToken.None);
		...
		test('list controller surfaces only working directory in metadata (git state is now per-session state, not summary)', async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri('singularity', 'sess-git'),
				startTime: 1000,
				modifiedTime: 2000,
				summary: 'With git',
	...
	suite('reconnection to active turn', () => {
		...
		test('loads completed turns as history and active turn request/response', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-1');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-1' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('sets isCompleteObs to false and populates progressObs for active turn', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-2');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-2' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('does not duplicate system notification progress when reconnecting', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'reconnect-system-notification');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString(), {
				systemNotification: 'Background command completed',
			}));

			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-system-notification' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('provides interruptActiveResponseCallback when reconnecting', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-3');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-3' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('interrupt callback dispatches turnCancelled action', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-cancel');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-cancel' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('streams new text deltas into progressObs after reconnection', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-stream');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-stream' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('marks session complete when turn finishes', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-complete');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-complete' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('handles active turn with running tool call', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-tool');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-tool' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('adopts and updates an active streaming tool call after reconnect', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('singularity', 'reconnect-streaming-tool');
			const sessionState = makeSessionStateWithActiveTurn(sessionUri.toString());
			sessionState.activeTurn!.responseParts.push({
				kind: ResponsePartKind.ToolCall,
				toolCall: {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-streaming-tool' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
			const progress = session.progressObs?.get() ?? [];
		...
		test('handles active turn with pending tool confirmation', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-perm');
			const sessionState = makeSessionStateWithActiveTurn(sessionUri.toString());
			sessionState.activeTurn!.responseParts.push({
				kind: ResponsePartKind.ToolCall,
				toolCall: {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-perm' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('reconnect during streaming then confirmation renders exactly one card (#314858)', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-stream-confirm');
			const sessionState = makeSessionStateWithActiveTurn(sessionUri.toString());
			sessionState.activeTurn!.responseParts.push({
				kind: ResponsePartKind.ToolCall,
				toolCall: {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-stream-confirm' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('no active turn loads completed history only with isComplete true', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'no-active-turn');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'singularity', title: 'Done', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [{
					id: 'turn-done',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/no-active-turn' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('includes reasoning in initial progress', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'reconnect-reasoning');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString(), {
				streamingText: 'text',
				reasoning: 'Let me think...',
			}));

			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/reconnect-reasoning' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
	...
	suite('server-initiated turns', () => {
		...
		test('syncs queued messages added to restored active sessions idempotently', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'restored-pending-sync');
			agentHostService.sessionStates.set(backendSession.toString(), {
				...createSessionState({
					resource: backendSession.toString(),
					provider: 'singularity',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/restored-pending-sync' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('projects pending messages queued by another client without republishing element attachments', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'remote-queued');
			agentHostService.sessionStates.set(backendSession.toString(), {
				...createSessionState({
					resource: backendSession.toString(),
					provider: 'singularity',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/remote-queued' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('hydrates queued messages from the protocol instead of clearing them when attaching', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'attach-existing-queue');
			agentHostService.sessionStates.set(backendSession.toString(), {
				...createSessionState({
					resource: backendSession.toString(),
					provider: 'singularity',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/attach-existing-queue' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('syncs text updates for existing queued pending messages', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'pending-text-update');
			agentHostService.sessionStates.set(backendSession.toString(), {
				...createSessionState({
					resource: backendSession.toString(),
					provider: 'singularity',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/pending-text-update' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('detects server-initiated turn and fires onDidStartServerRequest', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-server-turn' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('server-initiated turn streams progress through progressObs', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-server-progress' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('disposing chat session does not call disposeSession on connection', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/existing-session-1' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
		...
		test('client-dispatched turns are not treated as server-initiated', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-no-dupe' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('server-initiated turn does not duplicate tool calls on repeated state changes', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-server-tool-dedup' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('server-initiated turn picks up markdown arriving with turnStarted', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-server-md-initial' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('removes consumed queued message from chat model when server-initiated turn appears', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-queue-removal' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('removes steering message from chat model when steering id changes', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/new-steering-removal' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
	...
	suite('customizations', () => {
		...
		test('dispatches activeClientSet when a new session is created', async () => {
			...
			disposables.add(seedActiveClient('agent-host-singularity', { customizations }));
		...
		test('does not dispatch activeClientSet when an existing session is restored and this client is already active', async () => {
			const { instantiationService, agentHostService } = createTestServices(disposables);
			const sessionResource = AgentSession.uri('singularity', 'existing-session');
			const summary: SessionSummary = {
		...
		test('refreshes customizations when the current active client hydrates after open', async () => {
			...
			disposables.add(seedActiveClient('agent-host-singularity', { customizations }));
			const sessionResource = AgentSession.uri('singularity', 'late-active-client');
			const summary: SessionSummary = {
				resource: sessionResource.toString(),
				provider: 'singularity',
				title: 'Test',
	...
	suite('auth dedupe', () => {
		...
		test('propagates interactive authentication errors for eager-created sessions', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'eager-auth-error');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'singularity', title: 'Test', status: SessionStatus.Idle, createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/eager-auth-error' });
			const registered = chatAgentService.registeredAgents.get('agent-host-singularity')!;
			await assert.rejects(
			...
			assert.deepStrictEqual({
				...
				commandCalls: [{
					...
					args: [undefined, {
						...
						dialogTitle: 'Sign in to use Singularity',
	...
	suite('mcp auth prompt', () => {
		...
		test('prepares MCP enablement immediately before dispatching the turn', async () => {
			...
			await runTurn(
				...
				agentHostService,
				chatAgentService,
				URI.from({ scheme: 'agent-host-singularity', path: '/mcp-prepare' }),
				{ v: 1 },
			);
		...
		test('silently authenticates an existing session without an active turn', async () => {
			...
			const backendSession = AgentSession.uri('singularity', 'background-mcp-auth');
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/background-mcp-auth' });
			agentHostService.sessionStates.set(backendSession.toString(), {
				...createSessionState({
					resource: backendSession.toString(),
					provider: 'singularity',
		...
		test('deduplicates concurrent silent authentication across sessions', async () => {
			...
			const sessionResources = ['background-mcp-auth-1', 'background-mcp-auth-2'].map(id => {
				const backendSession = AgentSession.uri('singularity', id);
				agentHostService.sessionStates.set(backendSession.toString(), {
					...createSessionState({
						resource: backendSession.toString(),
						provider: 'singularity',
				...
				return URI.from({ scheme: 'agent-host-singularity', path: `/${id}` });
			});
		...
		test('surfaces an unauthenticated server once, then suppresses it on the next turn', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			const resource = URI.from({ scheme: 'agent-host-singularity', path: '/mcp-auth-1' });
			const seq = { v: 1 };
		...
		test('re-surfaces a server that reaches Ready and then needs auth again', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const customizationService = disposables.add(new TestMcpCustomizationService());
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables, { customizationServiceOverride: customizationService });
			const resource = URI.from({ scheme: 'agent-host-singularity', path: '/mcp-auth-2' });
			const seq = { v: 1 };
		...
		test('keeps a server suppressed when it leaves auth-required without reaching Ready', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const customizationService = disposables.add(new TestMcpCustomizationService());
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables, { customizationServiceOverride: customizationService });
			const resource = URI.from({ scheme: 'agent-host-singularity', path: '/mcp-auth-3' });
			const seq = { v: 1 };
	...
	suite('provideChatInputCompletions', () => {
		...
		test('forwards text/offset to the agent host and maps file attachments back to chat input items', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/abc' });
		...
		test('uses a command attachment label when it differs from the inserted text', async () => {
			...
			const result = await sessionHandler.provideChatInputCompletions(
				URI.from({ scheme: 'agent-host-singularity', path: '/abc' }),
				{ text: '/y', offset: 2 },
				CancellationToken.None,
			);
		...
		test('routes untitled completions to the current opaque provisional backend', async () => {
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/untitled-completions' });
			const backendSession = AgentSession.uri('singularity', 'opaque-provisional');
			const { sessionHandler, agentHostService } = createContribution(disposables, {
				provisionalServiceOverride: {
					get: resource => resource.toString() === sessionResource.toString() ? backendSession : undefined,
		...
		test('waits for a pending provisional before requesting untitled completions', async () => {
			const sessionResource = URI.from({ scheme: 'agent-host-singularity', path: '/untitled-pending-completions' });
			const backendSession = AgentSession.uri('singularity', 'replacement-provisional');
			const pending = new DeferredPromise<void>();
			disposables.add({ dispose: () => pending.cancel() });
		...
		test('returns no untitled completions when there is no current generation', async () => {
			...
			const result = await sessionHandler.provideChatInputCompletions(
				URI.from({ scheme: 'agent-host-singularity', path: '/untitled-no-generation' }),
				{ text: '/', offset: 1 },
				CancellationToken.None,
			);
		...
		test('surfaces a chat completion backend URI verbatim on the completion attachment', async () => {
			...
			// value verbatim.
			const backendChatUri = buildChatUri(AgentSession.uri('singularity', 'abc').toString(), 'peer-1');
			(agentHostService as unknown as { completions: (p: CompletionsParams) => Promise<CompletionsResult> }).completions = async () => ({
				items: [
					{
