...
suite('ChatModelConfigurationStore', () => {
	...
	function createControllableService(): IControllableService {
		...
		return {
			service,
			setConfigCalls,
			fireModelsChanged: () => emitter.fire('singularity'),
	...
	test('hostile model identifiers do not pollute the bucket prototype', () => {
		...
		storage.store(KEY, '{"__proto__":{"polluted":true},"singularity/gpt":{"thinkingEffort":"high"}}', StorageScope.APPLICATION, StorageTarget.USER);
