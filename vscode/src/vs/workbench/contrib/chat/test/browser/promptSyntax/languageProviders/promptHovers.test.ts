...
suite('PromptHoverProvider', () => {
	...
	setup(async () => {
		...
		const testModels: ILanguageModelChatMetadata[] = [
			...
			{ id: 'claude-sonnet-4.5', name: 'Claude Sonnet 4.5', vendor: 'singularity', version: '1.0', family: 'claude', extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 200000, maxOutputTokens: 8192, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: {} } satisfies ILanguageModelChatMetadata,
			{ id: 'claude-opus-4.6', name: 'Claude Opus 4.6', vendor: 'singularity', version: '1.0', family: 'claude', extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 200000, maxOutputTokens: 8192, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: {} } satisfies ILanguageModelChatMetadata,
			{ id: 'claude-haiku-4.5', name: 'Claude Haiku 4.5', vendor: 'singularity', version: '1.0', family: 'claude', extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 200000, maxOutputTokens: 8192, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: {} } satisfies ILanguageModelChatMetadata,
		];
	...
	suite('agent hovers', () => {
		test('hover on target attribute shows description', async () => {
			...
			assert.strictEqual(hover, 'The target to which the header attributes like tools apply to. Possible values are `github-singularity` and `vscode`.');
		});

		test('hover on model attribute with github-singularity target shows note', async () => {
			const content = [
				'---',
				'description: "Test"',
			...
			const expected = [
				...
				'',
				'Note: This attribute is not used when target is github-singularity.'
			].join('\n');
		...
		test('hover on handoffs attribute with github-singularity target shows note', async () => {
			const content = [
				'---',
				'description: "Test"',
				'target: github-singularity',
			...
			const expected = [
				...
				'',
				'Note: This attribute is not used in Singularity or Claude targets.'
			].join('\n');
			assert.strictEqual(hover, expected);
		});
		...
		test('hover on github-singularity tool shows simple description', async () => {
			const content = [
				'---',
				'description: "Test"',
				'target: github-singularity',
		...
		test('hover on github-singularity tool with target undefined', async () => {
			const content = [
				'---',
				'name: "Test"',
				'description: "Test"',
		...
		test('hover on model attribute with vscode target and model array', async () => {
			const content = [
				...
				'description: "Test"',
				'target: vscode',
				`model: ['MAE 4 (olama)', 'MAE 4.1 (singularity)']`,
				'---',
			].join('\n');
			const hover = await getHover(content, 4, 10, PromptsType.agent);
			const expected = [
