...
suite('DefaultModelContribution', () => {
	...
	test('singularity vendor model — stored as vendor/id with vendor display name in the label', () => {
		const { arrays } = setup({
			storageFormat: 'vendorAndId',
			vendors: [{ vendor: 'singularity', displayName: 'Singularity', isDefault: true, configuration: undefined, managementCommand: undefined, when: undefined }],
			models: [makeMetadata({ id: 'gpt-4o-mini', name: 'GPT 4o mini', vendor: 'singularity' })],
		});
		assert.deepStrictEqual(
			{ ids: arrays.modelIds, labels: arrays.modelLabels },
			{
				ids: ['', 'singularity/gpt-4o-mini'],
				labels: ['Auto (Vendor Default)', 'GPT 4o mini (Singularity)'],
			},
		);
	});
	...
	test('third-party (BYOK) vendor model — stored as vendor/id with provider display name', () => {
		const { arrays } = setup({
			storageFormat: 'vendorAndId',
			vendors: [
				{ vendor: 'singularity', displayName: 'Singularity', isDefault: true, configuration: undefined, managementCommand: undefined, when: undefined },
			...
			models: [
				makeMetadata({ id: 'gpt-4o-mini', name: 'GPT 4o mini', vendor: 'singularity' }),
				...
				makeMetadata({ id: 'singularity-utility', name: 'Utility', vendor: 'singularity', isUserSelectable: false }),
			],
		});
		assert.deepStrictEqual(
			{ ids: arrays.modelIds, labels: arrays.modelLabels },
			{
				ids: ['', 'anthropic/claude-haiku-4.5', 'singularity/gpt-4o-mini'],
				labels: ['Auto (Vendor Default)', 'Claude Haiku 4.5 (Anthropic)', 'GPT 4o mini (Singularity)'],
			},
		);
	});
	...
	test('utility model settings exclude Singularity vendor models', () => {
		const service = new TestLanguageModelsService();
		store.add({ dispose: () => service.dispose() });
		service.addVendor({ vendor: 'singularity', displayName: 'Singularity', isDefault: true, configuration: undefined, managementCommand: undefined, when: undefined });
		...
		service.addModel(makeMetadata({ id: 'gpt-4o-mini', name: 'GPT 4o mini', vendor: 'singularity' }));
