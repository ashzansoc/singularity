...
suite('ChatQuotaNotificationContribution', () => {
	...
	function createContribution(
		...
		const vendor = modelOpts?.vendor ?? 'singularity';
		...
		const modelIds = ['singularity/auto', selectedModelId];
		const languageModelsService = {
			...
			lookupLanguageModel: (id: string): ILanguageModelChatMetadata | undefined => {
				if (id === 'singularity/auto') {
					return { id: 'auto', vendor: 'singularity', family: 'auto', isBYOK: false } as ILanguageModelChatMetadata;
				}
				// Agent-host models (e.g. the Singularity CLI harness) are registered
				...
				return { id: modelId, vendor: modelVendor, family: modelId, isBYOK: modelVendor !== 'singularity' } as ILanguageModelChatMetadata;
			},
	...
	suite('exhausted notification descriptions', () => {
		...
		test('managed plan user with hasQuota=false gets budget exceeded message', () => {
			...
			assert.strictEqual(notificationMock.getNotification()!.description, 'Your organization or enterprise has exceeded its Singularity budget. Contact your admin to resume usage.');
			assert.strictEqual(notificationMock.getNotification()!.actions.length, 0);
		});
	...
	suite('quota approaching threshold', () => {
		...
		test('treatment suggests switching to Auto when another model is selected', async () => {
			...
			assert.deepStrictEqual({
				...
				actions: [{
					...
					modelIdentifier: 'singularity/auto',
				}],
			});
		});
		...
		test('does not enroll and suggests managing budget when Auto is already selected', async () => {
			const { assignmentMock, entitlementMock, notificationMock } = createContribution(
				{ quotas: { usageBasedBilling: true, premiumChat: makeQuotaSnapshot(60) } },
				{ selectedModelId: 'singularity/auto', switchToAutoTreatment: true },
			);
		...
		test('recognizes the live short Auto model id before persisted selection updates', async () => {
			const { assignmentMock, entitlementMock, notificationMock } = createContribution(
				{ quotas: { usageBasedBilling: true, premiumChat: makeQuotaSnapshot(60) } },
				{ contextModelId: 'auto', selectedModelId: 'singularity/test-model', switchToAutoTreatment: true },
			);
	...
	suite('BYOK model suppression', () => {
		...
		test('shows notification when Singularity model is selected', () => {
			const { notificationMock } = createContribution(
				{ quotas: { usageBasedBilling: true, premiumChat: makeQuotaSnapshot(0) } },
				{ vendor: 'singularity' },
			);
		...
		test('shows notification when switching from BYOK to Singularity model', () => {
			...
			// Switch to Singularity model via storage — triggers storage listener
			storageService.store('chat.currentLanguageModel.panel', 'singularity/gpt-4.1', StorageScope.PROFILE, StorageTarget.USER);
