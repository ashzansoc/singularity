...
export class ToolTerminalCreator {
	...
	async createTerminal(shellOrProfile: string | ITerminalProfile, os: OperatingSystem, token: CancellationToken): Promise<IToolTerminal> {
		const instance = await this._createSingularityTerminal(shellOrProfile, os);
	...
	private _createSingularityTerminal(shellOrProfile: string | ITerminalProfile, os: OperatingSystem) {
		...
		const env: Record<string, string> = {
			...
			// `AI_AGENT` is the cross-vendor standard; `SINGULARITY_AGENT` is kept
			// for back-compat with CLIs that already adopted it.
			[AiAgentEnvVar]: AiAgentEnvValue,
			SINGULARITY_AGENT: '1',
			// Avoid making `git diff` interactive when called from singularity
			GIT_PAGER: 'cat',
