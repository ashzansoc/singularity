 ...
 *
 * Mirrors singularity-agent-runtime's largeOutputHandler.ts pattern:
...
export class LargeOutputFileWriter extends Disposable {
	...
	private async _writeToTempFile(output: string): Promise<string | undefined> {
		try {
			const fileName = `singularity-terminal-output-${generateUuid()}.txt`;
			const dirUri = URI.joinPath(this._environmentService.cacheHome, 'singularity-terminal-output');
