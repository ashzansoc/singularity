...
export class CreateAndRunTaskTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
		...
		_progress.report({ message: new MarkdownString(localize('singularityChat.fetchingTask', 'Resolving the task')) });
		...
		if (!task) {
			return { content: [{ kind: 'text', value: `Task not found: ${args.task.label}` }], toolResultMessage: new MarkdownString(localize('singularityChat.taskNotFound', 'Task not found: `{0}`', args.task.label)) };
		}
		...
		try {
			...
			_progress.report({ message: new MarkdownString(localize('singularityChat.runningTask', 'Running task `{0}`', args.task.label)) });
			...
			if (!terminals || terminals.length === 0) {
				return { content: [{ kind: 'text', value: `Task started but no terminal was found for: ${args.task.label}` }], toolResultMessage: new MarkdownString(localize('singularityChat.noTerminal', 'Task started but no terminal was found for: `{0}`', args.task.label)) };
			}
		...
		for (const r of terminalResults) {
			this._telemetryService.publicLog2?.<TaskToolEvent, TaskToolClassification>('singularityChat.runTaskTool.createAndRunTask', {
