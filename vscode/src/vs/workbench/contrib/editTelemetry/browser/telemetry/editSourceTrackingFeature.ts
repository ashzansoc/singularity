...
export class EditTrackingFeature extends Disposable {
	...
	constructor(
		...
		const singularityInstalled = getExtensionInfoObs('Singularity.singularity');
		const singularityChatInstalled = getExtensionInfoObs('Singularity.singularity-chat');

		const shouldSendDetails = derived(reader => editSourceDetailsEnabled.read(reader) || !!singularityInstalled.read(reader) || !!singularityChatInstalled.read(reader));
