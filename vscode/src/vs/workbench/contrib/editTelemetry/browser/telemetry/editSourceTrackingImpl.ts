...
export type EditTelemetryCategory = 'nes' | 'inlineCompletionsSingularity' | 'inlineCompletionsNES' | 'inlineCompletionsOther' | 'otherAI' | 'user' | 'ide' | 'external' | 'unknown';
...
export function getEditTelemetryCategory(source: EditSource): EditTelemetryCategory {
	...
	if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'singularity.chat') { return 'inlineCompletionsSingularity'; }
	if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'singularity.chat-chat' && source.providerId === 'nes') { return 'inlineCompletionsNES'; }
	if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'singularity.chat-chat' && source.providerId === 'completions') { return 'inlineCompletionsSingularity'; }
...
class TrackedDocumentInfo extends Disposable {
	...
	async sendTelemetry(mode: EditTelemetryMode, trigger: EditTelemetryTrigger, t: DocumentEditSourceTracker, focusTime: number, actualTime: number) {
		...
		sendEditSourcesStatsTelemetry(this._telemetryService, {
			...
			inlineCompletionsSingularityModifiedCount: data.inlineCompletionsSingularityModifiedCount,
	...
	getTelemetryData(ranges: readonly TrackedEdit[]) {
		...
		return {
			...
			inlineCompletionsSingularityModifiedCount: sums.inlineCompletionsSingularity ?? 0,
