...
suite('Edit Telemetry Source Categories', () => {
	...
	test('maps every edit source category', () => {
		const sources = {
			...
			singularityCompletion: EditSources.inlineCompletionAccept({
				nes: false,
				requestUuid: 'request-1',
				languageId: 'typescript',
				providerId: new ProviderId('singularity.chat', '1.0.0', 'completions'),
				correlationId: undefined,
			}),
			singularityChatCompletion: EditSources.inlineCompletionAccept({
				nes: false,
				requestUuid: 'request-2',
				languageId: 'typescript',
				providerId: new ProviderId('singularity.chat-chat', '1.0.0', 'completions'),
				correlationId: undefined,
			}),
			nes: EditSources.inlineCompletionAccept({
				...
				languageId: 'typescript',
				providerId: new ProviderId('singularity.chat-chat', '1.0.0', 'nes'),
				correlationId: undefined,
			}),
		...
		assert.deepStrictEqual(Object.fromEntries(Object.entries(sources).map(([key, source]) => [
			...
			chat: 'otherAI',
			singularityCompletion: 'inlineCompletionsSingularity',
			singularityChatCompletion: 'inlineCompletionsSingularity',
			nes: 'nes',
