...
/**
 * The sentinel prefix used by singularity shell tools for exit code detection.
 ...
 */
const SINGULARITY_SENTINEL_PREFIX = '<<<SINGULARITY_SENTINEL_';

/** Returns whether a command line is a singularity sentinel echo, not a real user command. */
function isSingularitySentinelCommand(commandLine: string): boolean {
	return commandLine.includes(SINGULARITY_SENTINEL_PREFIX);
}
...
export class AgentHostPty extends BasePty implements ITerminalChildProcess {
	 ...
	 * integration events. When the singularity shell tools fall back to sentinel-
	...
	private _handleAction(envelope: ActionEnvelope): void {
		...
		switch (action.type) {
			...
			case ActionType.TerminalCommandExecuted:
				if (isSingularitySentinelCommand(action.commandLine)) {
	...
	private _replayContent(content: TerminalContentPart[]): void {
		for (const part of content) {
			if (part.type === 'unclassified') {
				...
				if (isSingularitySentinelCommand(part.commandLine)) {
					continue;
				}
