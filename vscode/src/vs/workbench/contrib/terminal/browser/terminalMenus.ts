...
function isAiContributedProfile(profile: IExtensionTerminalProfile): boolean {
	...
	if (extensionIdentifier === 'singularity.chat-chat' || extensionIdentifier === 'anthropic.claude-code') {
		return true;
	}
...
function isAiProfileName(name: string): boolean {
	...
	return lowerCaseName.includes('singularity') || lowerCaseName.includes('claude');
}
