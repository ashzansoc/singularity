...
suite('Workbench - TerminalInstance', () => {
	...
	suite('TerminalLabelComputer', () => {
		...
		test('should use ${sequence} for agent CLI shell types', () => {
			...
			terminalLabelComputer.refreshLabel(createInstance({ capabilities, shellType: GeneralShellType.Singularity, sequence: 'Singularity Agent', processName: 'singularity' }));
			strictEqual(terminalLabelComputer.title, 'Singularity Agent');
		});
		...
		test('should prefer shellLaunchConfig.titleTemplate over agent CLI shell type override', () => {
			const terminalLabelComputer = createLabelComputer({ terminal: { integrated: { tabs: { separator: ' - ', title: '${process}', description: '${cwd}', allowAgentCliTitle: true } } } });
			terminalLabelComputer.refreshLabel(createInstance({ capabilities, shellType: GeneralShellType.Singularity, sequence: 'Singularity Agent', processName: 'singularity', shellLaunchConfig: { titleTemplate: '${process}' } }));
			strictEqual(terminalLabelComputer.title, 'singularity');
		});
