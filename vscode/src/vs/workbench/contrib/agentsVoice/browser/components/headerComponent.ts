...
export interface HeaderProps {
	readonly singularityIconSrc: string;
	readonly showSingularityIcon: boolean;
...
export function createHeader(): HeaderComponent {
	...
	// Singularity icon
	const singularityIcon = document.createElement('img');
	singularityIcon.style.cssText = 'width:18px;height:18px;margin-right:4px;';
	...
	container.append(singularityIcon, micBtn, placeholderText, connIndicator, spacer, popoutBtn, closeBtn, connStyle);
	...
	return {
		...
		update(props: HeaderProps) {
			...
			// Singularity icon
			singularityIcon.style.display = props.showSingularityIcon ? '' : 'none';
			singularityIcon.src = props.singularityIconSrc;
