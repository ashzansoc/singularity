...
export class IssueReporterEditorPane extends EditorPane {
	...
	override async setInput(
		...
		this.inputDisposables.add(this.wizard.onDidRequestGenerateTitle(async (description) => {
			try {
				// Wait for installed extensions to be registered so the Singularity Chat
				// extension has had a chance to contribute its `singularity` language
				// model vendor before we try to resolve a model. (Other call sites
				// like the chat thinking title generator are reached after Singularity
				// has already activated; we're the only place that can be invoked
				// before it has.)
				await this.extensionService.whenInstalledExtensionsRegistered();

				// `singularity-utility-small` matches what other utility callers in the
				// workbench use (chat thinking summaries, tool-risk assessment,
				// chat-edit explanations). The earlier `singularity-fast` id never
				// existed and was the root cause of the empty-result regression.
				const modelIds = await this.languageModelsService.selectLanguageModels({ vendor: 'singularity', id: 'singularity-utility-small' });
