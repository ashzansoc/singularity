...
export class GettingStartedPage extends EditorPane {
	...
	/** Light logo on dark themes; dark logo on light themes. */
	private getSingularityHomeLogoUri(): string {
		const darkTheme = isDark(this.themeService.getColorTheme().type);
		const media = darkTheme
			? 'vs/workbench/browser/media/singularity-logo-light.svg'
			: 'vs/workbench/browser/media/singularity-logo-dark.svg';
		return FileAccess.asBrowserUri(media).toString(true);
	}

	private createSingularityLogoImage(className: string): HTMLImageElement {
		const logoImg = $(`img.${className}`, {
			src: this.getSingularityHomeLogoUri(),
			alt: '',
			draggable: 'false',
		}) as HTMLImageElement;
		this.categoriesSlideDisposables.add(this.themeService.onDidColorThemeChange(() => {
			logoImg.src = this.getSingularityHomeLogoUri();
		}));
		return logoImg;
	}
	...
	private async buildCategoriesSlide(preserveFocus?: boolean) {
		...
		this.container.classList.remove('singularity-home');

		// Cursor-style home for empty windows — first screen on open
		if (this.workspaceContextService.getWorkbenchState() === WorkbenchState.EMPTY
			&& !this.editorInput?.selectedCategory) {
			await this.buildSingularityHomeSlide();
			return;
		}
		...
		const header = $('.header', {},
			$('.header-brand-row', {},
				this.createSingularityLogoImage('singularity-welcome-logo'),
	...
	private async buildSingularityHomeSlide(): Promise<void> {
		this.container.classList.add('singularity-home');
		this.container.classList.add('noWalkthroughs');

		const logoImg = this.createSingularityLogoImage('singularity-home-logo');

		const brandRow = $('.singularity-home-brand-row', {},
			logoImg,
			$('h1.singularity-home-wordmark', {}, (this.productService.nameShort || 'Singularity').toUpperCase()),
		);

		const planLabel = isProUser(this.chatEntitlementService.entitlement)
			? localize('singularityHome.pro', "Pro")
			: localize('singularityHome.free', "Free");
		const settingsBtn = $('button.singularity-home-settings', {
			type: 'button',
		}, localize('singularityHome.settings', "Settings"));
		this.categoriesSlideDisposables.add(addDisposableListener(settingsBtn, 'click', e => {
			e.preventDefault();
			e.stopPropagation();
			this.commandService.executeCommand('workbench.action.openSettings');
		}));
		const meta = $('.singularity-home-meta', {},
			$('span', {}, planLabel),
			$('span.singularity-home-meta-sep', {}, '·'),
			settingsBtn,
		);

		const startEntriesVisible = parsedStartEntries.filter(e => this.contextService.contextMatchesRules(e.when));
		const actions = $('.singularity-home-actions');
		for (const entry of startEntriesVisible.slice(0, 3)) {
			const btn = $('button.singularity-home-action', {
				type: 'button',
		...
		const recentHeader = $('.singularity-home-recent-header', {},
			$('span.singularity-home-recent-title', {}, localize('singularityHome.recentProjects', "Recent projects")),
			$('button.singularity-home-view-all', {
				type: 'button',
				'x-dispatch': 'showMoreRecents',
			}, localize('singularityHome.viewAll', "View all")),
		);

		const recentList = $('ul.singularity-home-recent-list');
		const recentSection = $('.singularity-home-recents', {}, recentHeader, recentList);

		const renderRecents = (entries: RecentEntry[], totalCount: number) => {
			clearNode(recentList);
			const viewAll = recentHeader.querySelector('.singularity-home-view-all');
			if (viewAll) {
				viewAll.textContent = totalCount > 0
					? localize('singularityHome.viewAllCount', "View all ({0})", totalCount)
					: localize('singularityHome.viewAll', "View all");
			}

			if (!entries.length) {
				recentList.appendChild($('li.singularity-home-recent-empty', {},
					localize('singularityHome.noRecents', "No recent projects"),
				));
				return;
			}

			for (const recent of entries.slice(0, 5)) {
				let fullPath: string;
				...
				const li = $('li.singularity-home-recent-item');
				const nameEl = $('button.singularity-home-recent-name', {
					type: 'button',
					title: fullPath,
				}, name);
				...
				li.appendChild($('span.singularity-home-recent-tilde', {}, '~'));
				recentList.appendChild(li);
			}
		};

		this.recentlyOpened.then(({ workspaces }) => {
			const filtered = this.filterRecentlyOpened(workspaces);
			renderRecents(filtered, filtered.length);
			this.registerDispatchListeners();
			this.categoriesPageScrollbar?.scanDomNode();
		}).catch(onUnexpectedError);

		const center = $('.singularity-home-center', {}, brandRow, meta, actions, recentSection);

		const footerChildren: HTMLElement[] = [];
		if (!isWeb) {
			const agentsCta = $('button.singularity-home-agents-cta', {
				type: 'button',
			},
				$('span', {}, localize('singularityHome.agentsCta', "Try a new window for running parallel agents")),
				$('span.codicon.codicon-link-external'),
			);
			this.categoriesSlideDisposables.add(addDisposableListener(agentsCta, 'click', e => {
				e.preventDefault();
				e.stopPropagation();
				this.commandService.executeCommand(OPEN_AGENTS_WINDOW_COMMAND_ID);
			}));
			footerChildren.push(agentsCta);
		}

		const footer = $('.singularity-home-footer', {}, ...footerChildren);
		reset(this.categoriesSlide, $('.singularity-home-root', {}, center, footer));
		this.categoriesPageScrollbar?.scanDomNode();
		this.registerDispatchListeners();
		this.setSlide('categories');
	}
