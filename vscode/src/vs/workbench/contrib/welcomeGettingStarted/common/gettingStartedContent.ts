...
export function singularitySettingsMessage(manageSettingsUrl: string): string {
	return localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({0})"}', '{Locked="]({1})"}'] }, "Singularity may show [public code]({1}) suggestions and use your data to improve the product. You can change these [settings]({2}) anytime.", defaultChat.provider.default.name, defaultChat.publicCodeMatchesUrl, manageSettingsUrl);
}
...
export const startEntries: GettingStartedStartEntryContent = [
	{
		...
		description: localize('gettingStarted.openProject.description', "Open a folder to start working"),
	...
	{
		id: 'topLevelGitClone',
		title: localize('gettingStarted.cloneRepo.title', "Clone repo"),
	...
	},
	{
		...
		title: localize('gettingStarted.connectSsh.title', "Connect via SSH"),
...
const SingularityStepTitle = localize('gettingStarted.singularitySetup.title', "Code with Singularity for free");
const SingularityDescription = localize({ key: 'gettingStarted.singularitySetup.description', comment: ['{Locked="["}', '{Locked="]({0})"}'] }, "You can use [Singularity]({0}) to generate code across multiple files, fix errors, ask questions about your code, and much more using natural language.", defaultChat.documentationUrl ?? '');
const SingularityTermsString = localize({ key: 'gettingStarted.singularitySetup.terms', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with Singularity, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl);
const SingularityAnonymousButton = Button(localize('setupSingularityButton.setup', "Use Singularity"), `command:workbench.action.chat.triggerSetupAnonymousWithoutDialog`);
const SingularitySignedOutButton = Button(localize('setupSingularityButton.setup', "Use Singularity"), `command:workbench.action.chat.triggerSetup`);
const SingularitySignedInButton = Button(localize('setupSingularityButton.setup', "Use Singularity"), `command:workbench.action.chat.triggerSetup`);
const SingularityCompleteButton = Button(localize('setupSingularityButton.chatWithSingularity', "Code with Singularity"), 'command:workbench.action.chat.open');

function createSingularitySetupStep(id: string, button: string, when: string, includeTerms: boolean): BuiltinGettingStartedStep {
	const description = includeTerms ?
		`${SingularityDescription}\n${SingularityTermsString}\n${button}` :
		`${SingularityDescription}\n${button}`;

	return {
		id,
		title: SingularityStepTitle,
		description,
		when: `${when} && !chatSetupHidden && !chatSetupDisabledInWorkspace`,
		media: {
			type: 'svg', altText: 'Singularity multi file edits', path: 'multi-file-edits.svg'
		},
	};
}
...
export const walkthroughs: GettingStartedWalkthroughContent = [
	{
		id: 'Setup',
		title: localize('gettingStarted.setup.title', "Get started with Singularity"),
		description: localize('gettingStarted.setup.description', "Customize your editor, learn the basics, and start coding"),
		isFeatured: true,
		icon: setupIcon,
		when: '!isWeb',
		walkthroughPageTitle: localize('gettingStarted.setup.walkthroughPageTitle', 'Setup Singularity'),
		next: 'Beginner',
		content: {
			type: 'steps',
			steps: [
				createSingularitySetupStep('SingularitySetupAnonymous', SingularityAnonymousButton, 'chatAnonymous && !chatSetupCompleted', true),
				createSingularitySetupStep('SingularitySetupSignedOut', SingularitySignedOutButton, 'chatEntitlementSignedOut && !chatAnonymous && !singularity.chat.hasByokModels', false),
				createSingularitySetupStep('SingularitySetupComplete', SingularityCompleteButton, 'chatSetupCompleted && !chatSetupDisabled && (chatAnonymous || chatPlanPro || chatPlanProPlus || chatPlanMax || chatPlanBusiness || chatPlanEnterprise || chatPlanFree)', false),
				createSingularitySetupStep('SingularitySetupSignedIn', SingularitySignedInButton, '!chatEntitlementSignedOut && (!chatSetupCompleted || chatSetupDisabled || chatPlanCanSignUp)', false),
				{
					id: 'pickColorTheme',
				...
				},
				{
					...
					description: localize('gettingStarted.videoTutorial.description.interpolated', "Watch the first in a series of short & practical video tutorials for Singularity's key features.\n{0}", Button(localize('watch', "Watch Tutorial"), 'https://aka.ms/vscode-getting-started-video')),
					media: { type: 'svg', altText: 'Singularity Settings', path: 'learn.svg' },
				}
			]
		}
	},
	...
	{
		id: 'SetupWeb',
		title: localize('gettingStarted.setupWeb.title', "Get Started with Singularity for the Web"),
		description: localize('gettingStarted.setupWeb.description', "Customize your editor, learn the basics, and start coding"),
		isFeatured: true,
		icon: setupIcon,
		when: 'isWeb',
		next: 'Beginner',
		walkthroughPageTitle: localize('gettingStarted.setupWeb.walkthroughPageTitle', 'Setup Singularity Web'),
		content: {
			type: 'steps',
			steps: [
				{
				...
				},
				{
					...
					description: localize('gettingStarted.extensionsWeb.description.interpolated', "Extensions are Singularity's power-ups. A growing number are becoming available in the web.\n{0}", Button(localize('browsePopularWeb', "Browse Popular Web Extensions"), 'command:workbench.extensions.action.showPopularExtensions')),
					when: 'workspacePlatform == \'webworker\'',
					media: {
						type: 'svg', altText: 'Singularity extension marketplace with featured language extensions', path: 'extensions-web.svg'
					},
				},
				{
				...
				},
				{
					...
					description: localize('gettingStarted.commandPalette.description.interpolated', "Run commands without reaching for your mouse to accomplish any task in Singularity.\n{0}", Button(localize('commandPalette', "Open Command Palette"), 'command:workbench.action.showCommands')),
					media: { type: 'svg', altText: 'Command Palette overlay for searching and executing commands.', path: 'commandPalette.svg' },
				},
				{
					id: 'pickAFolderTask-WebWeb',
					title: localize('gettingStarted.setup.OpenFolder.title', "Open up your code"),
					description: localize('gettingStarted.setup.OpenFolderWeb.description.interpolated', "You're all set to start coding. You can open a local project or a remote repository to get your files into Singularity.\n{0}\n{1}", Button(localize('openFolder', "Open Folder"), 'command:workbench.action.addRootFolder'), Button(localize('openRepository', "Open Repository"), 'command:remoteHub.openRepository')),
	...
	},
	{
		...
		description: localize('gettingStarted.setupAccessibility.description', "Learn the tools and shortcuts that make Singularity accessible. Note that some actions are not actionable from within the context of the walkthrough."),
		isFeatured: true,
		icon: setupIcon,
		when: CONTEXT_ACCESSIBILITY_MODE_ENABLED.key,
		next: 'Setup',
		walkthroughPageTitle: localize('gettingStarted.setupAccessibility.walkthroughPageTitle', 'Setup Singularity Accessibility'),
		content: {
			type: 'steps',
			steps: [
				{
				...
				},
				{
					...
					description: localize('gettingStarted.commandPaletteAccessibility.description.interpolated', "Run commands without reaching for your mouse to accomplish any task in Singularity.\n{0}", Button(localize('commandPalette', "Open Command Palette"), 'command:workbench.action.showCommands')),
			...
			steps: [
				{
					...
					description: localize('gettingStarted.settingsAndSync.description.interpolated', "Customize every aspect of Singularity and [sync](command:workbench.userDataSync.actions.turnOn) customizations across devices.\n{0}", Button(localize('tweakSettings', "Open Settings"), 'command:toSide:workbench.action.openSettings')),
					when: 'workspacePlatform != \'webworker\' && syncStatus != uninitialized',
					completionEvents: ['onEvent:sync-enabled'],
					media: {
						type: 'svg', altText: 'Singularity Settings', path: 'settings.svg'
					},
				},
				{
					id: 'extensions',
					title: localize('gettingStarted.extensions.title', "Code with extensions"),
					description: localize('gettingStarted.extensions.description.interpolated', "Extensions are Singularity's power-ups. They range from handy productivity hacks, expanding out-of-the-box features, to adding completely new capabilities.\n{0}", Button(localize('browsePopular', "Browse Popular Extensions"), 'command:workbench.extensions.action.showPopularExtensions')),
					when: 'workspacePlatform != \'webworker\'',
					media: {
						type: 'svg', altText: 'Singularity extension marketplace with featured language extensions', path: 'extensions.svg'
					},
				},
				{
					id: 'terminal',
