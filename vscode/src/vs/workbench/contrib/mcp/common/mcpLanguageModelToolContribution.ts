...
import { IMcpServer, IMcpService, IMcpTool, IMcpToolResourceLinkContents, McpResourceURI, McpToolResourceLinkMimeType, McpServerTransportType, McpToolVisibility } from './mcpTypes.js';
...
export class McpLanguageModelToolContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		type Rec = { source?: ToolDataSource; isRemote?: boolean } & IDisposable;
		...
		this._register(autorun(reader => {
			...
			for (const server of servers) {
				...
				// Remote/OAuth connectors must be client tools: the agent-host CLI
				// cannot reuse VS Code's secret vault, so advertising them only as
				// native MCP leaves chat with no authenticated Notion/Slack/etc. tools.
				const isRemote = server.readDefinitions().read(reader).server?.launch?.type === McpServerTransportType.HTTP;

				const previousRec = previous.get(server);
				if (previousRec) {
					toDelete.delete(server);
					if (previousRec.isRemote === isRemote && (!previousRec.source || equals(previousRec.source, mcpServerToSourceData(server, reader)))) {
				...
				const rec: Rec = { dispose: () => store.dispose(), isRemote };
