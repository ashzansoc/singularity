...
import { IAutostartResult, IMcpAutostartOptions, IMcpServer, IMcpService, McpCollectionDefinition, McpConnectionState, McpDefinitionReference, McpServerCacheState, McpServerDefinition, McpServerTransportType, McpStartServerInteraction, UserInteractionRequiredError } from './mcpTypes.js';
import { mcpServerMatchesUserMessage } from './mcpKeywordFilter.js';
...
export class McpService extends Disposable implements IMcpService {
	...
	public autostart(_token?: CancellationToken, _options?: IMcpAutostartOptions): IObservable<IAutostartResult> {
		...
		this._autostart(autoStartConfig, state, cts.token, _options?.message)
			.catch(err => {
	...
	private async _autostart(autoStartConfig: McpAutoStartValue, state: ISettableObservable<IAutostartResult>, token: CancellationToken, userMessage?: string) {
		...
		if (autoStartConfig === McpAutoStartValue.OnlyNew) {
			...
			todo = new Set(candidates.filter(s => {
				...
				const launch = s.readDefinitions().get().server?.launch;
				const stoppedRemote = launch?.type === McpServerTransportType.HTTP && s.connectionState.get().state === McpConnectionState.Kind.Stopped;
				return c === McpServerCacheState.Unknown || c === McpServerCacheState.Outdated || stoppedRemote;
			}));
		}

		if (userMessage !== undefined) {
			todo = new Set([...todo].filter(s => mcpServerMatchesUserMessage(s, userMessage)));
		}
