 ...
 * starts it, and waits until its tools are live.
 */
export async function startServerByFilter(mcpService: IMcpService, filter: (s: IMcpServer) => boolean, timeout = 5000) {
	const server = await new Promise<IMcpServer>((resolve, reject) => {
		const store = new DisposableStore();
		store.add(autorun(reader => {
			const found = mcpService.servers.read(reader).find(filter);
			if (found) {
				store.dispose();
				resolve(found);
			}
	...
	const ok = await startServerAndWaitForLiveTools(server, { promptType: 'all-untrusted' });
	if (!ok) {
