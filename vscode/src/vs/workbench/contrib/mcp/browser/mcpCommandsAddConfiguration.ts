...
const enum AddConfigurationSingularityCommand {
	/** Returns whether MCP enhanced setup is enabled. */
	IsSupported = 'singularity.chat.chat.mcp.setup.check',

	/** Takes an npm/pip package name, validates its owner. */
	ValidatePackage = 'singularity.chat.chat.mcp.setup.validatePackage',

	/** Returns the resolved MCP configuration. */
	StartFlow = 'singularity.chat.chat.mcp.setup.flow',
}
...
export class McpAddConfigurationCommand {
	...
	private async getServerType(): Promise<AddConfigurationType | undefined> {
		...
		try {
			aiSupported = await this._commandService.executeCommand<boolean>(AddConfigurationSingularityCommand.IsSupported);
		} catch {
			// ignored
		}
		...
		if (aiSupported) {
			...
			items.push(
				{ type: 'separator', label: localize('mcp.serverType.singularity', "Model-Assisted") },
				...elligableTypes
			);
		}
	...
	private async getAssistedConfig(type: AssistedConfigurationType): Promise<{ name?: string; server: Omit<IMcpStdioServerConfiguration, 'type'>; inputs?: IMcpServerVariable[]; inputValues?: Record<string, string> } | undefined> {
		...
		this._commandService.executeCommand<ValidatePackageResult>(
			AddConfigurationSingularityCommand.ValidatePackage,
			{
				type: packageType,
				name: packageName,
				targetConfig: {
		...
		const config = await this._commandService.executeCommand<AssistedServerConfiguration>(
			AddConfigurationSingularityCommand.StartFlow,
			{
				name: packageName,
				type: packageType
			}
