...
export class InlineCompletionLanguageStatusBarContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._register(autorunWithStore((reader, store) => {
			// Do not show the Singularity icon in the language status when AI features are disabled
			...
			const statusMap: Record<typeof status, { shortLabel: string; label: string; loading: boolean }> = {
				...
				ghostText: { shortLabel: '$(lightbulb)', label: '$(singularity) ' + localize('inlineCompletionAvailable', "Inline completion available"), loading: false, },
				inlineEdit: { shortLabel: '$(lightbulb-sparkle)', label: '$(singularity) ' + localize('inlineEditAvailable', "Inline edit available"), loading: false, },
				noSuggestion: { shortLabel: '$(circle-slash)', label: '$(singularity) ' + localize('noInlineSuggestionAvailable', "No inline suggestion available"), loading: false, },
			};
