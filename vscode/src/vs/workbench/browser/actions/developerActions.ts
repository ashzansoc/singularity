...
import { SINGULARITY_ENABLED_PLUGINS_KEY, SINGULARITY_EXTRA_MARKETPLACES_KEY, SINGULARITY_STRICT_MARKETPLACES_KEY, INativeManagedSettingsService, IFileManagedSettingsService, IManagedSettingResolution, MANAGED_SETTINGS_CHANNELS, ManagedSettingsChannel, ManagedSettingsSource, normalizeManagedSettings, projectManagedSettings, pickManagedSettings } from '../../../platform/policy/common/singularityManagedSettings.js';
...
class PolicyDiagnosticsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		try {
			...
			content += '*Only keys declared here can reach VS Code policy callbacks. Runtime-owned keys may still be enforced by the Singularity runtime even when absent from the projections below.*\n\n';
			...
			content += PROPERTY_VALUE_TABLE_HEADER;
			content += '| Endpoint | `/singularity_internal/managed_settings` |\n';
			...
			for (const key of [SINGULARITY_ENABLED_PLUGINS_KEY, SINGULARITY_STRICT_MARKETPLACES_KEY, SINGULARITY_EXTRA_MARKETPLACES_KEY]) {
