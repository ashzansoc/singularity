...
export class ModalEditorPart {
	...
	async create(options?: IModalEditorPartOptions): Promise<ICreateModalEditorPartResult> {
		...
		const getActiveModalOptions = () => {
			const activeEditor = editorPart.activeGroup.activeEditor;
			return isModalEditorOptionsProvider(activeEditor) ? activeEditor.getModalEditorOptions() : undefined;
		};
		...
		const getDefaultSize = (): Dimension => {
			...
			const ratio = Math.min(1, Math.max(0.2, getActiveModalOptions()?.sizeRatio ?? 0.8));
			const targetWidth = containerDimension.width * ratio;
			const targetHeight = availableHeight * ratio;
		...
		disposables.add(Event.runAndSubscribe(modalEditorService.onDidActiveEditorChange, () => {
			const editorModalOptions = getActiveModalOptions();
			modalElement.classList.toggle('compact-header', !!editorModalOptions?.compactHeader);
			modalElement.classList.toggle('backdrop-blur', !!editorModalOptions?.backdropBlur);
			layoutModal();
		}));
