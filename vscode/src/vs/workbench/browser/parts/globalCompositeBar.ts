...
export interface IGlobalCompositeBarOptions {
...
export class GlobalCompositeBar extends Disposable {
	...
	private readonly profileButton: HTMLElement;
	private readonly profileAvatar: HTMLElement;
	private readonly profileInitials: HTMLElement;
	private readonly profileImage: HTMLImageElement;
	...
	constructor(
		...
		_barOptions: IGlobalCompositeBarOptions = {},
		...
		this.element = $('.singularity-nav-footer');

		this.profileButton = append(this.element, $('button.singularity-nav-profile', { type: 'button' }));
		this.profileAvatar = append(this.profileButton, $('span.singularity-nav-profile-avatar'));
		this.profileInitials = append(this.profileAvatar, $('span.singularity-nav-profile-initials'));
		this.profileImage = append(this.profileAvatar, $('img.singularity-nav-profile-img')) as HTMLImageElement;
		this.profileImage.alt = '';
		this.profileImage.referrerPolicy = 'no-referrer';
		this.profileBadge = append(this.profileButton, $('span.singularity-nav-profile-badge'));
		this._register(addDisposableListener(this.profileButton, EventType.CLICK, () => {
			void this.commandService.executeCommand(OPEN_SINGULARITY_PROFILE_COMMAND_ID);
		}));
		this._register(addDisposableListener(this.profileImage, 'error', () => this.profileImage.classList.add('hidden')));
		this.updateProfile();
		this._register(this.billingService.onDidChange(() => this.updateProfile()));
		...
		this.globalActivityActionBar = this._register(new ActionBar(this.element, {
			actionViewItemProvider: (action, options) => {
				if (action.id === GLOBAL_ACTIVITY_ID) {
					return this.instantiationService.createInstance(GlobalActivityActionViewItem, this.contextMenuActionsProvider, { ...options, icon: true, colors: this.colors, hoverOptions: this.activityHoverOptions }, contextMenuAlignmentOptions);
				}
	...
	private updateProfile(): void {
		...
		this.profileInitials.textContent = snapshot.alias;
		this.profileButton.title = snapshot.githubLogin
			? localize('singularity.nav.profileSignedIn', "Profile · @{0}", snapshot.githubLogin)
			: snapshot.kind === 'restoring'
				? localize('singularity.nav.profileRestoring', "Profile · Restoring GitHub login")
				: localize('singularity.nav.profileSignedOut', "Profile · Sign in with GitHub");
		this.profileButton.setAttribute('aria-label', this.profileButton.title);
		this.profileImage.classList.toggle('hidden', !snapshot.avatarUrl);
		if (snapshot.avatarUrl) {
			this.profileImage.src = snapshot.avatarUrl;
		}
		this.profileButton.classList.toggle('is-signed-out', snapshot.kind === 'signedOut');
		this.profileButton.classList.toggle('is-expired', snapshot.kind === 'expired');
		this.profileButton.classList.toggle('is-subscribed', snapshot.kind === 'subscribed');
		if (snapshot.kind === 'trial') {
			this.profileBadge.textContent = String(snapshot.trialDaysRemaining);
			this.profileBadge.classList.remove('hidden');
		} else if (snapshot.kind === 'subscribed') {
			this.profileBadge.textContent = '';
			this.profileBadge.classList.add('hidden');
		} else {
			this.profileBadge.textContent = snapshot.kind === 'expired' ? '!' : '';
			this.profileBadge.classList.toggle('hidden', snapshot.kind !== 'expired');
		}
	}
