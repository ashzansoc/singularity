...
export class CompositeBarActionViewItem extends BaseActionViewItem {
	...
	private singularityNavLabel: HTMLElement | undefined;
	...
	override render(container: HTMLElement): void {
		...
		// Singularity: text label revealed when the rounded nav expands on hover
		if (this.options.icon) {
			this.singularityNavLabel = append(container, $('span.singularity-nav-label'));
		}
	...
	protected override updateLabel(): void {
		...
		const isSingularityGithub = !!this.singularityNavLabel && this.compositeBarActionItem.id === 'workbench.view.scm';
		if (isSingularityGithub && this.options.icon) {
			this.label.classList.remove('codicon-source-control');
			this.label.classList.add(...ThemeIcon.asClassNameArray(Codicon.github));
		}
		...
		if (this.singularityNavLabel) {
			this.singularityNavLabel.textContent = isSingularityGithub
				? localize('singularity.nav.github', "GitHub")
				: (this.compositeBarActionItem.name || this.action.label);
		}
	}
