...
import { SingularityModeSwitcher } from './singularityModeSwitcher.js';
...
export class BrowserTitlebarPart extends Part implements ITitlebarPart {
	...
	private singularityTitleBrandLogo: HTMLImageElement | undefined;
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		...
		// Singularity wordmark + Preview/Design/Code switcher
		if (!this.isAuxiliary) {
			this.createSingularityTitleBrand();
			this.createSingularityModeSwitcher();
		}
	...
	private getSingularityTypeLogoUri(): string {
		// Light wordmark on dark themes; dark wordmark on light themes.
		const media = isDark(this.themeService.getColorTheme().type)
	...
	private createSingularityTitleBrand(): void {
		const brand = append(this.leftContent, $('.singularity-title-brand'));
		const logo = append(brand, $('img.singularity-title-brand-logo')) as HTMLImageElement;
		logo.alt = '';
		logo.draggable = false;
		logo.src = this.getSingularityTypeLogoUri();

		const name = append(brand, $('span.singularity-title-brand-name'));
		name.textContent = 'Singularity';

		this.singularityTitleBrandLogo = logo;

		this._register(this.themeService.onDidColorThemeChange(() => {
			if (this.singularityTitleBrandLogo) {
				this.singularityTitleBrandLogo.src = this.getSingularityTypeLogoUri();
			}
		}));
	}

	private createSingularityModeSwitcher(): void {
		const switcher = this._register(this.instantiationService.createInstance(SingularityModeSwitcher));
		append(this.leftContent, switcher.element);
	}
