...
/** View containers shown in the Singularity rounded activity nav. SCM is labeled GitHub. */
const SINGULARITY_ACTIVITY_BAR_COMPOSITES = new Set([
	'workbench.view.explorer',
	'workbench.view.extensions',
	'workbench.view.connectors',
	'workbench.view.scm',
	'workbench.view.extension.singularityBrain',
]);
...
export class ActivitybarPart extends Part {
	...
	static readonly ICON_SIZE = 29; // ~20% larger than default 24
	...
	/** Pill width for the Singularity floating nav (must match singularity-activitybar.css). */
	static readonly SINGULARITY_NAV_WIDTH = 56;
	/** Left + right float gutters (12 + 12) around the pill. */
	static readonly SINGULARITY_NAV_GUTTER = 24;

	/** The intrinsic activity bar width (excludes any floating gutter). */
	private get baseWidth(): number {
		return ActivitybarPart.SINGULARITY_NAV_WIDTH;
	}
	...
	/** Extra horizontal space reserved as left/right float gutters around the pill. */
	private get floatingGutter(): number {
		return ActivitybarPart.SINGULARITY_NAV_GUTTER;
	}
	...
	private createCompositeBar(): PaneCompositeBar {
		...
		return this.instantiationService.createInstance(ActivityBarCompositeBar, this.location, {
			...
			compositeSize: 52,
			filterComposite: (id) => SINGULARITY_ACTIVITY_BAR_COMPOSITES.has(id),
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		this.element = parent;
		this.element.classList.add('singularity-activitybar');
		this.content = append(this.element, $('.content.singularity-nav'));
		...
		// Reserve width for floating gutters around the Singularity pill
		this._onDidChange.fire(undefined);
	...
	override updateStyles(): void {
		...
		if (this.element?.classList.contains('singularity-activitybar')) {
			// App black behind the pill — only the pill uses the charcoal grey
			container.style.setProperty('background-color', '#0F0F0F', 'important');
	...
	override layout(width: number, height: number): void {
		...
		// Lay out to the full part size. CSS margins on `.singularity-nav` create the
		// floating left/right gaps; shrinking here made the grey pill fill the column.
		const contentAreaSize = super.layoutContents(width, height).contentSize;

		this.compositeBar.value?.layout(ActivitybarPart.SINGULARITY_NAV_WIDTH, contentAreaSize.height);
	}
...
export class ActivityBarCompositeBar extends PaneCompositeBar {
	...
	constructor(
		...
		if (showGlobalActivities) {
			this.globalCompositeBar = this._register(instantiationService.createInstance(GlobalCompositeBar, () => this.getContextMenuActions(), (theme: IColorTheme) => this.options.colors(theme), this.options.activityHoverOptions, { showAccounts: false }));
		}
