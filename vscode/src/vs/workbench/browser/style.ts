...
import './media/singularity-glass.css';
import './media/singularity-activitybar.css';
import { registerThemingParticipant } from '../../platform/theme/common/themeService.js';
import { TITLE_BAR_ACTIVE_BACKGROUND } from '../common/theme.js';
...
registerThemingParticipant((theme, collector) => {
	...
	// Keep Singularity shell on brand black; chat applies its own glass layer.
	collector.addRule(`.monaco-workbench { background-color: #0F0F0F !important; }`);
	collector.addRule(`html, body { background-color: #0F0F0F !important; }`);
	...
	// Keep iOS PWA home-indicator area translucent like the shell
	if (isIOS && isStandalone()) {
		collector.addRule(`body { background-color: #0F0F0F !important; }`);
	}
});
