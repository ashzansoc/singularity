...
/** Windows registry root for GitHub Singularity policies. */
export const GITHUB_SINGULARITY_WIN32_REGISTRY_PATH = 'SOFTWARE\\Policies\\Singularity';

/** Windows product name passed to the native policy watcher. */
export const GITHUB_SINGULARITY_WIN32_POLICY_NAME = 'Singularity';

/** macOS CFPreferences application ID for GitHub Singularity managed preferences. */
export const GITHUB_SINGULARITY_MACOS_BUNDLE_ID = 'com.singularity.chat';

/** MDM key for the V0 managed setting. */
export const SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY = 'permissions.disableBypassPermissionsMode';

/** Managed-settings key for enterprise plugin enablement (carried as a JSON-encoded `{ [pluginId]: boolean }`). */
export const SINGULARITY_ENABLED_PLUGINS_KEY = 'enabledPlugins';

/** Managed-settings key for enterprise marketplaces (carried as a JSON-encoded `{ [name]: url-or-shorthand }`). */
export const SINGULARITY_EXTRA_MARKETPLACES_KEY = 'extraKnownMarketplaces';
...
export const SINGULARITY_STRICT_MARKETPLACES_KEY = 'strictKnownMarketplaces';
...
export const SINGULARITY_ALLOWED_MCP_SERVERS_KEY = 'allowedMcpServers';
...
export const SINGULARITY_DENIED_MCP_SERVERS_KEY = 'deniedMcpServers';

/** Managed-settings key that blocks standalone user/workspace customizations. */
export const SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY = 'strictPluginOnlyCustomization';

/** Managed-settings key that makes the enterprise MCP allowlist authoritative. */
export const SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_KEY = 'allowManagedMcpServersOnly';

/** Managed-settings key that allows hooks only from managed sources. */
export const SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_KEY = 'allowManagedHooksOnly';

/** Managed-settings transport control that requires a fresh server fetch on startup. */
export const SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY = 'forceRemoteSettingsRefresh';
...
export const MANAGED_SETTINGS_CONTROL_DEFINITIONS: IManagedSettingsPolicyDefinitions = {
	[SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY]: { type: 'boolean' },
};

/** Policy-only configuration delivery slot for {@link SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY}. */
export const SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_CONFIG = 'chat.customizations.strictPluginOnlyCustomization';

/** Policy-only configuration delivery slot for {@link SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_KEY}. */
export const SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_CONFIG = 'chat.mcp.allowManagedServersOnly';

/** Policy-only configuration delivery slot for {@link SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_KEY}. */
export const SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_CONFIG = 'chat.hooks.allowManagedOnly';
 ...
 * (alongside {@link SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY}), so it flattens to the dot-path
 * `permissions.model` in the normalized bag — the key policy `value()` callbacks must read.
 */
export const SINGULARITY_MODEL_KEY = 'permissions.model';
...
export const SINGULARITY_OTEL_ENABLED_KEY = 'telemetry.enabled';

/** Managed-settings key for the enterprise OTLP collector endpoint. */
export const SINGULARITY_OTEL_ENDPOINT_KEY = 'telemetry.endpoint';

/** Managed-settings key for the enterprise OTLP protocol (`http/json`, `http/protobuf`, or `grpc`). */
export const SINGULARITY_OTEL_PROTOCOL_KEY = 'telemetry.protocol';

/** Managed-settings key for enterprise OTel content capture. */
export const SINGULARITY_OTEL_CAPTURE_CONTENT_KEY = 'telemetry.captureContent';

/** Managed-settings key that prevents users from enabling OTel content capture themselves. */
export const SINGULARITY_OTEL_LOCK_CAPTURE_CONTENT_KEY = 'telemetry.lockCaptureContent';

/** Managed-settings key for the OTel `service.name` resource attribute. */
export const SINGULARITY_OTEL_SERVICE_NAME_KEY = 'telemetry.serviceName';

/** Managed-settings key for additional OTel resource attributes (a `{ [k]: string }` map). */
export const SINGULARITY_OTEL_RESOURCE_ATTRIBUTES_KEY = 'telemetry.resourceAttributes';

/** Managed-settings key for extra OTLP exporter headers (a `{ [k]: string }` map). */
export const SINGULARITY_OTEL_HEADERS_KEY = 'telemetry.headers';
...
export function shouldForceRemoteSettingsRefresh(nativeMdm: ManagedSettingsData | undefined, server: ManagedSettingsData | undefined): boolean {
	const nativeValue = nativeMdm?.[SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY];
	if (typeof nativeValue === 'boolean') {
		return nativeValue;
	}
	return server?.[SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY] === true;
}

let managedModelValueCallback: ((policyData: IPolicyData) => ManagedSettingValue | undefined) | undefined;

/**
 * `value` callback for the default-chat-model managed setting ({@link SINGULARITY_MODEL_KEY}). Like
...
export function managedModelValue(): (policyData: IPolicyData) => ManagedSettingValue | undefined {
	if (!managedModelValueCallback) {
		managedModelValueCallback = policyData => {
			const model = policyData.managedSettings?.[SINGULARITY_MODEL_KEY];
 ...
 * key -> definition map. This is the single source of truth for policy-backed Singularity
...
export type ManagedSettingsChannel =
	/** GitHub `/singularity_internal/managed_settings` endpoint (server-delivered). */
	| 'server'
...
export const MANAGED_SETTINGS_MACOS_FILE_PATH = '/Library/Application Support/Singularity/managed-settings.json';

/** Linux well-known path for file-based managed settings. */
export const MANAGED_SETTINGS_LINUX_FILE_PATH = '/etc/github-singularity/managed-settings.json';

/** Windows directory name under %ProgramFiles% for file-based managed settings. */
export const MANAGED_SETTINGS_WINDOWS_DIR = 'Singularity';
...
const STRUCTURED_MANAGED_SETTINGS: readonly IStructuredManagedSetting[] = [
	{
		key: SINGULARITY_ENABLED_PLUGINS_KEY,
		encode: encodeObject,
	},
	{
		key: SINGULARITY_STRICT_MARKETPLACES_KEY,
		encode: encodeArray,
	},
	{
		key: SINGULARITY_ALLOWED_MCP_SERVERS_KEY,
		encode: encodeArray,
	},
	{
		key: SINGULARITY_DENIED_MCP_SERVERS_KEY,
		encode: encodeArray,
	},
	{
		key: SINGULARITY_EXTRA_MARKETPLACES_KEY,
