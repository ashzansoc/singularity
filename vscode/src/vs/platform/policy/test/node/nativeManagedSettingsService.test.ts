...
import { SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY, SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY } from '../../common/singularityManagedSettings.js';
...
suite('NativeManagedSettingsService', () => {
	...
	test('watches managed-settings keys from policy definitions and exposes raw managed settings', async () => {
		...
		const service = disposables.add(new NativeManagedSettingsService(new NullLogService(), 'com.singularity.chat', undefined, watcherFactory));
		...
		onDidChange?.({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' });
		assert.deepStrictEqual(service.managedSettings, { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' });

		onDidChange?.({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'enable' });
		assert.deepStrictEqual(service.managedSettings, { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'enable' });
	});
	...
	test('watches transport controls without a managed-settings policy definition', async () => {
		...
		const watcherFactory: NativePolicyWatcherFactory = (_productName, policies, callback) => {
			...
			callback({ [SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY]: true });
			return Disposable.None;
		};
		...
		assert.deepStrictEqual({
			...
			watchedSettings: { [SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY]: { type: 'boolean' } },
			managedSettings: { [SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY]: true },
		});
	});
	...
	test('keeps raw managed settings while definitions are unchanged', async () => {
		...
		assert.deepStrictEqual({ managedSettings: service.managedSettings, watcherCreateCount }, { managedSettings: { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' }, watcherCreateCount: 1 });
	});
	...
	test('removes raw managed settings whose definitions are no longer watched', async () => {
		...
		onDidChange?.({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable', [otherManagedSettingKey]: true });
		await service.updatePolicyDefinitions({
			[policyName]: {
				type: 'boolean',
				managedSettings: {
	...
	test('channel client keeps newer event state when initial snapshot resolves later', async () => {
		...
		channel.fire({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' });
		channel.resolveInitialSnapshot({ [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'enable' });
		await client.initialize();

		assert.deepStrictEqual(client.managedSettings, { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' });
	});
	...
	test('channel client updatePolicyDefinitions updates cache without firing change event', async () => {
		...
		channel.updatePolicyDefinitionsResult = { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' };
		...
		assert.deepStrictEqual({ result, managedSettings: client.managedSettings, eventCount }, {
			result: { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' },
			managedSettings: { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' },
			eventCount: 0,
		});
	});
	...
	test('channel client retries a failed initial snapshot', async () => {
		...
		assert.deepStrictEqual(await client.initialize(), { [SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY]: true });
...
class RetryManagedSettingsChannel implements IChannel {
	...
	call<T>(command: string): Promise<T> {
		...
		return Promise.resolve({ [SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY]: true } as T);
	}
