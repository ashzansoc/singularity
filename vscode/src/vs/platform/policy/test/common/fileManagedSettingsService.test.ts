...
import { SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_KEY, SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_KEY, SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY, SINGULARITY_ENABLED_PLUGINS_KEY, SINGULARITY_EXTRA_MARKETPLACES_KEY, SINGULARITY_MODEL_KEY, SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY, managedModelValue, normalizeManagedSettings, RawManagedSettingsData } from '../../common/singularityManagedSettings.js';
...
suite('normalizeManagedSettings', () => {
	...
	test('JSON-stringifies structured keys (enabledPlugins)', () => {
		...
		const result = normalizeManagedSettings({
			[SINGULARITY_ENABLED_PLUGINS_KEY]: plugins
		});
		assert.deepStrictEqual(result, {
			[SINGULARITY_ENABLED_PLUGINS_KEY]: JSON.stringify(plugins)
		});
	});

	test('normalizes customization lockdown controls', () => {
		assert.deepStrictEqual(normalizeManagedSettings({
			[SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY]: true,
			[SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_KEY]: true,
			[SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_KEY]: false,
		}), {
			[SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY]: true,
			[SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_KEY]: true,
			[SINGULARITY_ALLOW_MANAGED_HOOKS_ONLY_KEY]: false,
	...
	test('drops a non-boolean strictPluginOnlyCustomization value', () => {
		assert.deepStrictEqual(normalizeManagedSettings({
			[SINGULARITY_STRICT_PLUGIN_ONLY_CUSTOMIZATION_KEY]: ['skills', 'unknown'],
		}), {});
	});

	test('normalizes extraKnownMarketplaces from schema format to config dict', () => {
		const result = normalizeManagedSettings({
			[SINGULARITY_EXTRA_MARKETPLACES_KEY]: {
				...
				'c': { source: { source: 'github', repo: 'github/singularity-plugins' } },
			}
		});
		assert.deepStrictEqual(result, {
			[SINGULARITY_EXTRA_MARKETPLACES_KEY]: '{"a":"{\\"source\\":\\"github/agent-skills\\",\\"autoUpdate\\":true}","b":"{\\"source\\":\\"https://example.com/repo.git#v1\\",\\"autoUpdate\\":false}","c":"github/singularity-plugins"}',
		});
	});
	...
	test('ignores non-boolean marketplace autoUpdate with warning', () => {
		...
		assert.deepStrictEqual(result, {
			[SINGULARITY_EXTRA_MARKETPLACES_KEY]: '{"a":"github/agent-skills"}',
		});
	...
	test('drops malformed marketplace entries with warning', () => {
		...
		assert.deepStrictEqual(result, {
			[SINGULARITY_EXTRA_MARKETPLACES_KEY]: '{"good":"a/b"}',
		});
	...
	test('handles mixed scalar and structured keys', () => {
		const result = normalizeManagedSettings({
			...
			[SINGULARITY_ENABLED_PLUGINS_KEY]: { 'plugin': true },
		});
		assert.deepStrictEqual(result, {
			'permissions.disableBypassPermissionsMode': 'disable',
			'strictKnownMarketplaces': '["github/foo"]',
	...
	test('flattens the model setting nested under permissions', () => {
		...
		assert.strictEqual(SINGULARITY_MODEL_KEY, 'permissions.model');
	...
	test('drops a structured key whose value is not an object', () => {
		const result = normalizeManagedSettings({
			[SINGULARITY_ENABLED_PLUGINS_KEY]: 'already-a-string'
		});
		assert.deepStrictEqual(result, {});
	});
});
...
suite('FileManagedSettingsChannelClient', () => {
	...
	test('keeps newer event state when the initial snapshot resolves later', async () => {
		...
		assert.deepStrictEqual({ raw: client.rawManagedSettings, normalized: client.managedSettings }, {
			...
			normalized: { [SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY]: 'disable' },
		});
	});
});
