...
export function createNativeAboutDialogDetails(productService: IProductService, osProps: IOSProperties): { title: string; details: string; detailsToCopy: string } {
	...
	const getDetails = (useAgo: boolean): string => {
		return localize({ key: 'aboutDetail', comment: ['Electron, Chromium, Node.js, V8 and Singularity are product names that need no translation'] },
			...
			process.versions['v8'],
			productService.singularityVersions?.runtime || 'Unknown',
			productService.singularityVersions?.sdk || 'Unknown',
