...
import { SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_CONFIG } from '../../policy/common/singularityManagedSettings.js';
...
export class AllowedMcpServersService extends Disposable implements IAllowedMcpServersService {
	...
	constructor(
		...
		this._register(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(mcpAccessConfig) || e.affectsConfiguration(mcpAllowedServersConfig) || e.affectsConfiguration(mcpDeniedServersConfig) || e.affectsConfiguration(SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_CONFIG)) {
	...
	isServerAllowed(identity: IMcpServerIdentity): true | IMarkdownString {
		...
		const managedOnly = this.configurationService.getValue<boolean>(SINGULARITY_ALLOW_MANAGED_MCP_SERVERS_ONLY_CONFIG) === true;
