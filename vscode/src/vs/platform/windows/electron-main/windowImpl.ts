...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			...
			// Singularity glass: macOS vibrancy under translucent chrome
			if (isMacintosh) {
				try {
					this._win.setVibrancy('under-window');
				} catch (err) {
					this.logService.warn(`window#ctor: failed to enable vibrancy`, err);
				}
			}
