...
suite('TelemetryService', () => {
	...
	test('setCommonProperty adds property to all subsequent events', function () {
		...
		service.setCommonProperty('common.singularityTrackingId', 'test-tracking-id');
		service.publicLog('eventAfterSet');

		assert.strictEqual(testAppender.events[0].data['common.singularityTrackingId'], undefined);
		assert.strictEqual(testAppender.events[1].data['common.singularityTrackingId'], 'test-tracking-id');
