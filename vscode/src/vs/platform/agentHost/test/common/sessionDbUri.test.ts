...
suite('buildSessionDbUri / parseSessionDbUri', () => {
	...
	test('round-trips a simple URI', () => {
		const uri = buildSessionDbUri('singularity:/abc-123', 'tc-1', '/workspace/file.ts', 'before');
		const parsed = parseSessionDbUri(uri);
		assert.ok(parsed);
		assert.deepStrictEqual(parsed, {
			sessionUri: 'singularity:/abc-123',
			toolCallId: 'tc-1',
	...
	test('round-trips with special characters in filePath', () => {
		const uri = buildSessionDbUri('singularity:/s1', 'tc-2', '/work space/file (1).ts', 'after');
		const parsed = parseSessionDbUri(uri);
		assert.ok(parsed);
	...
	test('round-trips with special characters in toolCallId', () => {
		const uri = buildSessionDbUri('singularity:/s1', 'call_abc=123&x', '/file.ts', 'before');
		const parsed = parseSessionDbUri(uri);
		assert.ok(parsed);
	...
	test('round-trips a backslashed Windows filePath, which the database lookup needs verbatim', () => {
		...
		const parsed = parseSessionDbUri(buildSessionDbUri('singularity:/s1', 'tc-1', filePath, 'before'));
		assert.ok(parsed);
	...
	test('parseSessionDbUri returns undefined for malformed session-db URIs', () => {
		assert.strictEqual(parseSessionDbUri('session-db:singularity:/s1'), undefined);
		assert.strictEqual(parseSessionDbUri('session-db:singularity:/s1?toolCallId=tc-1'), undefined);
		assert.strictEqual(parseSessionDbUri('session-db:singularity:/s1?toolCallId=tc-1&filePath=/f&part=middle'), undefined);
	});
	...
	test('URI path is the file path, so labels show a real path', () => {
		const uri = buildSessionDbUri('singularity:/s1', 'tc-1', '/workspace/src/index.ts', 'before');
	...
	test('URI path is the file path for files with spaces and special chars', () => {
		const uri = buildSessionDbUri('singularity:/s1', 'tc-1', '/work space/file (1).ts', 'after');
		assert.strictEqual(URI.parse(uri).path, '/work space/file (1).ts');
	});

	test('parses the legacy hex-encoded layout', () => {
		const legacy = legacyUri('singularity:/abc-123', 'tc-1', '/workspace/file.ts', 'before', 'file.ts').toString();
...
suite('canonicalizeSessionDbUri', () => {
	...
	test('rewrites a legacy URI into the current layout', () => {
		const legacy = legacyUri('singularity:/abc-123', 'call_1', '/workspace/file.ts', 'before', 'file.ts');
		...
		assert.deepStrictEqual(
			...
			['/workspace/file.ts', { sessionUri: 'singularity:/abc-123', toolCallId: 'call_1', filePath: '/workspace/file.ts', part: 'before' }],
		);
	});

	test('takes the path from the file URI, so a Windows session canonicalizes the same way on any client', () => {
		const legacy = legacyUri('singularity:/abc-123', 'call_1', 'C:\\Code\\repo\\file.ts', 'before', 'file.ts');
	...
	test('leaves canonical, unparseable and foreign URIs untouched', () => {
		const canonical = URI.parse(buildSessionDbUri('singularity:/s1', 'tc-1', '/workspace/file.ts', 'before'));
