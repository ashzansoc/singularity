...
suite('openSessionLink', () => {
	...
	test('builds a link from a backend session URI', () => {
		assert.strictEqual(buildOpenSessionLinkUri('singularitycli:/abc-123'), 'agent-host-session://singularitycli/abc-123');
	});

	test('round-trips backend session URI', () => {
		const backend = 'singularitycli:/abc-123';
	...
	test('carries an optional chat id', () => {
		const link = buildOpenSessionLinkUri('singularitycli:/abc-123', 'chat-9');
		assert.strictEqual(link, 'agent-host-session://singularitycli/abc-123?chat=chat-9');
		assert.strictEqual(parseOpenSessionLinkUri(link)?.toString(), URI.parse('singularitycli:/abc-123').toString());
		assert.strictEqual(parseOpenSessionLinkChatId(link), 'chat-9');
		assert.strictEqual(parseOpenSessionLinkChatId(buildOpenSessionLinkUri('singularitycli:/abc-123')), undefined);
	});

	test('normalizes the default chat id to a session-only link', () => {
		assert.strictEqual(buildOpenSessionLinkUri('singularitycli:/abc-123', 'default'), 'agent-host-session://singularitycli/abc-123');
	});

	test('parseOpenSessionLinkChatId treats chat=default as absent', () => {
		assert.strictEqual(parseOpenSessionLinkChatId('agent-host-session://singularitycli/abc-123?chat=default'), undefined);
		assert.strictEqual(parseOpenSessionLinkChatId('agent-host-session://singularitycli/abc-123?chat=peer1'), 'peer1');
	});

	test('buildOpenSessionLinkForChatResource maps chat resources to session links', () => {
		const session = 'singularitycli:/abc-123';
		assert.deepStrictEqual({
			...
			defaultChat: 'agent-host-session://singularitycli/abc-123',
			peerChat: 'agent-host-session://singularitycli/abc-123?chat=peer1',
			bareSession: 'agent-host-session://singularitycli/abc-123',
		});
	});

	test('returns undefined for non-session-link URIs', () => {
		assert.strictEqual(parseOpenSessionLinkUri('https://example.com/x'), undefined);
		assert.strictEqual(parseOpenSessionLinkUri('singularitycli:/abc'), undefined);
		assert.strictEqual(parseOpenSessionLinkUri('agent-host-session://singularitycli/'), undefined);
	});
});
