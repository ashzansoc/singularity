...
suite('AgentSubscriptionManager', () => {
	...
	test('creating session subscription for singularity: URI', async () => {
		const mgr = createManager();
		const mySessionUri = URI.from({ scheme: 'singularity', path: '/my-session' });
	...
	test('getSubscriptionUnmanaged returns undefined when no subscription exists', () => {
		const mgr = createManager();
		const result = mgr.getSubscriptionUnmanaged<SessionState>(URI.parse('singularity:/nonexistent'));
