...
suite('changesetUri', () => {
	...
	const sessionUri = 'singularity:/abc-123';

	test('builders produce the documented shapes', () => {
		assert.strictEqual(buildSessionChangesetUri(sessionUri), 'singularity:/abc-123/changeset/session');
		assert.strictEqual(buildUncommittedChangesetUri(sessionUri), 'singularity:/abc-123/changeset/uncommitted');
		assert.strictEqual(buildTurnChangesetUri(sessionUri, 't1'), 'singularity:/abc-123/changeset/turn/t1');
		assert.strictEqual(buildTurnChangesetUriTemplate(sessionUri), 'singularity:/abc-123/changeset/turn/{turnId}');
		assert.strictEqual(buildCompareTurnsChangesetUri(sessionUri, 't1', 't2'), 'singularity:/abc-123/changeset/compare/t1/t2');
		assert.strictEqual(buildCompareTurnsChangesetUriTemplate(sessionUri), 'singularity:/abc-123/changeset/compare/{originalTurnId}/{modifiedTurnId}');
