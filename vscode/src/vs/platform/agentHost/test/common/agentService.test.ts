...
import { AgentHostByokModelsEnabledEnvVar, AgentHostCodexAgentEnabledSettingId, AgentSession, AgentHostOTelEnvVars, buildAgentHostOTelEnv, buildAgentSdkEnv, ClaudePreferAgentHostAgentsSettingId, ClaudePreferAgentHostEditorSettingId, CodexPreferAgentHostEditorSettingId, GITHUB_SINGULARITY_PROTECTED_RESOURCE, GITHUB_REPO_PROTECTED_RESOURCE, isAgentEnabled, protectedResourcesRequireSingularitySignIn, readAgentHostOTelPolicySettings, sanitizeAgentHostOTelPolicySettings, shouldSurfaceLocalAgentHostProvider } from '../../common/agentService.js';
...
suite('AgentSession namespace', () => {
	...
	test('uri creates a URI with provider as scheme and id as path', () => {
		const session = AgentSession.uri('singularity', 'abc-123');
		assert.strictEqual(session.scheme, 'singularity');
		assert.strictEqual(session.path, '/abc-123');
	});

	test('id extracts the raw session ID from a session URI', () => {
		const session = URI.from({ scheme: 'singularity', path: '/my-session-42' });
	...
	test('uri and id are inverse operations', () => {
		...
		const session = AgentSession.uri('singularity', rawId);
		assert.strictEqual(AgentSession.id(session), rawId);
	});

	test('provider extracts singularity from a singularity-scheme URI', () => {
		const session = AgentSession.uri('singularity', 'sess-1');
...
suite('shouldSurfaceLocalAgentHostProvider', () => {
	...
	test('uses provider- and window-specific availability settings', () => {
		...
		assert.deepStrictEqual({
			...
			otherProvider: shouldSurfaceLocalAgentHostProvider('singularity', configurationService, true),
		}, {
			agentsClaude: true,
			editorClaude: false,
			agentsCodex: true,
...
suite('protectedResourcesRequireSingularitySignIn', () => {

	ensureNoDisposablesAreLeakedInTestSuite();

	const githubSingularityWithoutRequired: ProtectedResourceMetadata = { resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource };
	const githubSingularityRequiredFalse: ProtectedResourceMetadata = { ...GITHUB_SINGULARITY_PROTECTED_RESOURCE, required: false };
	...
	test('derives the requirement from advertised protected resources', () => {
		const scenarios: Record<string, ProtectedResourceMetadata[]> = {
			// Proxy-mode Singularity / Claude: advertises the resource as required.
			singularityRequired: [GITHUB_SINGULARITY_PROTECTED_RESOURCE],
			// Absent `required` is treated the same as `true`.
			singularityRequiredAbsent: [githubSingularityWithoutRequired],
			...
			singularityRequiredFalse: [githubSingularityRequiredFalse],
			// Only unrelated resources are advertised.
			onlyOtherResource: [otherRequiredResource],
			// Mixed: an optional GitHub Singularity resource alongside a required other one.
			optionalSingularityWithOtherRequired: [githubSingularityRequiredFalse, otherRequiredResource],
		};

		const result = Object.fromEntries(
			Object.entries(scenarios).map(([name, resources]) => [name, protectedResourcesRequireSingularitySignIn(resources)]),
		);

		assert.deepStrictEqual(result, {
			singularityRequired: true,
			singularityRequiredAbsent: true,
			noResourcesAdvertised: false,
			singularityRequiredFalse: false,
			onlyOtherResource: false,
			optionalSingularityWithOtherRequired: false,
		});
	});

	test('the GitHub repo resource alone does not require Singularity sign-in', () => {
		assert.strictEqual(protectedResourcesRequireSingularitySignIn([GITHUB_REPO_PROTECTED_RESOURCE]), false);
	});
});
