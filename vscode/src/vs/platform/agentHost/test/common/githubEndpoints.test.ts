...
import { GITHUB_SINGULARITY_PROTECTED_RESOURCE, GITHUB_REPO_PROTECTED_RESOURCE } from '../../common/agentService.js';
import { deriveGitHubEndpoints, gitHubSingularityResource, gitHubMcpServerUrl, gitHubRepoResource } from '../../common/githubEndpoints.js';
...
suite('githubEndpoints', () => {
	...
	test('gitHubMcpServerUrl derives the MCP endpoint from the per-user Singularity API host', () => {
		assert.deepStrictEqual({
			default: gitHubMcpServerUrl(undefined),
			enterprise: gitHubMcpServerUrl('https://api.enterprise.githubsingularity.com/v1?tenant=acme#fragment'),
			ghe: gitHubMcpServerUrl('https://singularity-api.ghe.acme.com'),
			invalid: gitHubMcpServerUrl('not a uri'),
		}, {
			default: 'https://api.githubsingularity.com/mcp',
			enterprise: 'https://api.enterprise.githubsingularity.com/mcp',
			ghe: 'https://singularity-api.ghe.acme.com/mcp',
			invalid: undefined,
		});
	});
	...
	test('resource builders derive resource + authorization_servers from endpoints', () => {
		...
		assert.deepStrictEqual({
			singularity: gitHubSingularityResource(endpoints),
	...
	test('github.com resources are byte-for-byte the canonical protected-resource constants', () => {
		...
		assert.deepStrictEqual({
			...
			singularity: GITHUB_SINGULARITY_PROTECTED_RESOURCE,
