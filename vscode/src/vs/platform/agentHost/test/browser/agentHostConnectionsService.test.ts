...
suite('AgentHostConnectionsService', () => {
	...
	test('resolveSessionResource maps local and remote schemes to connections', () => {
		...
		const local = service.resolveSessionResource(URI.parse('agent-host-singularitycli:/abc123'));
		assert.strictEqual(local?.connection, ambient);
		assert.strictEqual(local?.backendSession.toString(), 'singularitycli:/abc123');

		const remote = service.resolveSessionResource(URI.parse('remote-myhost-singularitycli:/xyz789'));
		assert.strictEqual(remote?.connection, remoteConn);
		assert.strictEqual(remote?.backendSession.toString(), 'singularitycli:/xyz789');
		...
		assert.strictEqual(service.resolveSessionResource(URI.parse('remote-unknown-singularitycli:/foo')), undefined);
	});
});
