...
suite('AgentHostFileCompletionProvider', () => {
	...
	suite('provideCompletionItems', () => {
		...
		function setup(opts: {
			...
			const sessionUri = URI.from({ scheme: 'singularity', path: '/test' }).toString();
			const stateManager = disposables.add(new AgentHostStateManager(new NullLogService()));
		...
		test('formats duplicate names as root and relative path', async () => {
			const singularityRoot = URI.file('/workspace/singularity-sdk');
			const vscodeRoot = URI.file('/workspace/vscode');
			const singularityFile = URI.joinPath(singularityRoot, 'nodejs/package.json');
			const vscodeFile = URI.joinPath(vscodeRoot, 'test/package.json');
			const { sessionUri, provider } = setup({
				workingDirectories: [singularityRoot, vscodeRoot],
				results: new Map([
					[singularityRoot.path, [singularityFile]],
			...
			assert.deepStrictEqual(completions.map(item => item.attachment.label).sort(), [
				'singularity-sdk \u2022 nodejs/package.json',
