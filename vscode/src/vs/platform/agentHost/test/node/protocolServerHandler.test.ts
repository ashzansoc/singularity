...
class MockAgentService implements IAgentService {
	...
	async createSession(config?: IAgentCreateSessionConfig): Promise<URI> {
		...
		const session = config?.session ?? URI.parse('singularity:///new-session');
		this._stateManager.createSession({
			resource: session.toString(),
			provider: config?.provider ?? 'singularity',
			title: '',
	...
	async executeExecutionTask(): Promise<import('../../common/agentService.js').IAgentHostExecutionTaskResult> {
		return { taskId: 'mock', ok: true };
	}
...
suite('ProtocolServerHandler', () => {
	...
	test('listSessions carries the workspace-less marker on _meta', async () => {
		...
		agentService.listedSessions.push({
			...
			modifiedTime: 2000,
			summary: 'Quick Chat',
			workingDirectories: [URI.file('/home/user/.singularity/chats/session-1')],
	...
	test('createSession forwards request metadata and broadcasts project in sessionAdded summary', async () => {
		...
		const newSession = URI.parse('singularity:///created-session').toString();
	...
	suite('createChat / disposeChat', () => {
		...
		test('createChat for an unknown session fails with SESSION_NOT_FOUND', async () => {
			...
			transport.simulateMessage(request(2, 'createChat', { channel: 'singularity:/missing', chat: buildChatUri('singularity:/missing', 'peer-1') }));
	...
	suite('createSession activeClient', () => {
		...
		test('forwards activeClient to the agent service', async () => {
			const newSession = URI.parse('singularity:///eager-session').toString();
		...
		test('rejects createSession when activeClient.clientId mismatches', async () => {
			const newSession = URI.parse('singularity:///mismatch-session').toString();
