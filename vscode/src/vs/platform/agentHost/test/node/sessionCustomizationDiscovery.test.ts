...
import type { CopilotClient as SingularityClient } from '@github/copilot-sdk';
...
import { DiscoveredType, SessionCustomizationDiscovery } from '../../node/singularity/sessionCustomizationDiscovery.js';
import { SessionPluginBundler } from '../../node/shared/sessionPluginBundler.js';
import { mapToParsedPlugin, toDiscoveredDirectoryCustomizations } from '../../node/singularity/singularityAgent.js';

type AgentsDiscoverRequest = Parameters<SingularityClient['rpc']['agents']['discover']>[0];
...
suite('SessionCustomizationDiscovery', () => {
	...
	test('discovers supported agent instruction files in workspace roots', async () => {
		const wsSingularityInstructions = await seed('/workspace/.github/singularity-instructions.md', 'workspace singularity instructions');
		...
		assert.deepStrictEqual(files, [
			wsSingularityInstructions.toString(),
	...
	test('groups discovered customizations by parent folder', async () => {
		...
		} as unknown as SingularityClient;
	...
	test('discover includes hooks from recursive and fixed hook locations', async () => {
		...
		await seed('/workspace/.github/singularity/settings.json', '{"hooks": {"PreToolUse": []}}');
		...
		assert.deepStrictEqual(hookDirectories, [
			{ uri: '/home/.singularity/hooks', children: [] },
			{ uri: '/workspace/.github/singularity', children: ['/workspace/.github/singularity/settings.json'] },
	...
	test('discover returns working-directory agents, skills, instructions, hooks, and agent instructions', async () => {
		...
		await seed('/workspace/.github/hooks/pre-tool.json', '{"PreToolUse": []}');
		await seed('/workspace/.github/singularity/settings.json', '{"hooks": {"PreToolUse": []}}');
		await seed('/workspace/.github/singularity-instructions.md', 'workspace singularity instructions');
		await seed('/workspace/AGENTS.md', 'workspace agents instructions');
		await seed('/home/.singularity/singularity-instructions.md', 'user singularity instructions');
			...
			rpc: {
				...
				instructions: {
					getDiscoveryPaths: async () => ({
						paths: [
							{ path: '/workspace/.github/instructions', kind: 'directory' },
							{ path: '/workspace/.github/singularity-instructions.md', kind: 'file' },
							{ path: '/workspace/AGENTS.md', kind: 'file' },
							{ path: '/home/.singularity/singularity-instructions.md', kind: 'file' },
						],
					}),
					discover: async () => ({
						sources: [
		...
		assert.deepStrictEqual(directories, [
			{ contents: 'rule', uri: '/home', writable: false, children: ['/home/.singularity/singularity-instructions.md'] },
			{ contents: 'hook', uri: '/home/.singularity/hooks', writable: true, children: [] },
			{ contents: 'rule', uri: '/workspace', writable: false, children: ['/workspace/.github/singularity-instructions.md', '/workspace/AGENTS.md'] },
			{ contents: 'agent', uri: '/workspace/.github/agents', writable: true, children: ['/workspace/.github/agents/foo.agent.md'] },
			{ contents: 'hook', uri: '/workspace/.github/singularity', writable: true, children: ['/workspace/.github/singularity/settings.json'] },
	...
	test('discover groups case-variant instructions and nested skills under their roots', async () => {
		...
		const client = {
			rpc: {
				...
				instructions: {
					getDiscoveryPaths: async () => ({ paths: [{ path: '/home/.singularity/singularity-instructions.md', kind: 'file' }] }),
					discover: async () => ({ sources: [{ id: 'userInstruction', label: 'User instruction', sourcePath: '/home/.singularity/singularity-instructions.md', type: 'home' }] }),
				},
				skills: {
					getDiscoveryPaths: async () => ({
						paths: [
		...
		assert.deepStrictEqual(directories, [
			{ contents: 'rule', uri: '/HOME', children: ['/home/.singularity/singularity-instructions.md'] },
	...
	test('returns directories sorted by type and URI', async () => {
		...
		await seed('/workspace/.github/singularity-instructions.md', 'workspace singularity instructions');
		await seed('/home/.singularity/agents/abc.agent.md', 'user agent abc');
		await seed('/home/.singularity/agents/qux.agent.md', 'user agent');
		await seed('/home/.singularity/skills/alpha/SKILL.md', 'user singularity skill');
	...
	test('does not discover agent instruction files outside supported roots', async () => {
		...
		await seed('/home/singularity-instructions.md', 'unsupported home root');
		...
		assert.deepStrictEqual(files, [
			URI.from({ scheme: Schemas.inMemory, path: '/workspace/.github/singularity-instructions.md' }).toString(),
		]);
	});
	...
	test('installs watchers for roots that contain discovered customizations', async () => {
		...
		await seed('/home/.singularity/agents/user.agent.md', 'user agent');
		await seed('/home/.singularity/skills/singularity-user-skill/SKILL.md', 'user singularity skill');
		await seed('/home/.agents/skills/user-skill/SKILL.md', 'user skill');
		await seed('/home/.singularity/instructions/user.instructions.md', 'user instruction');
		await seed('/home/.singularity/hooks/post-tool.json', '{"PostToolUse": []}');
		await seed('/home/.singularity/singularity-instructions.md', 'user singularity instructions');
		...
		assert.strictEqual(watched.get(URI.joinPath(userHome, '.singularity').toString()), false);
		assert.strictEqual(watched.get(URI.joinPath(userHome, '.singularity', 'agents').toString()), false);
		assert.strictEqual(watched.get(URI.joinPath(userHome, '.singularity', 'skills').toString()), true);
		assert.strictEqual(watched.get(URI.joinPath(userHome, '.agents', 'skills').toString()), true);
		assert.strictEqual(watched.get(URI.joinPath(userHome, '.singularity', 'instructions').toString()), true);
		assert.strictEqual(watched.get(URI.joinPath(userHome, '.singularity', 'hooks').toString()), true);
	});
	...
	test('discovers agents, skills, instructions, and hooks across workspace and home roots', async () => {
		...
		const userAgent = await seed('/home/.singularity/agents/qux.agent.md', 'user agent');
		const userSingularitySkill = await seed('/home/.singularity/skills/singularity-zap/SKILL.md', 'user singularity skill');
		const userSkill = await seed('/home/.agents/skills/zap/SKILL.md', 'user skill');
		const userHook = await seed('/home/.singularity/hooks/post-tool.json', '{"PostToolUse": []}');
		...
		assert.deepStrictEqual([...files].sort((a, b) => a.uri.toString().localeCompare(b.uri.toString())), [
			...
			{ uri: userSingularitySkill, type: DiscoveredType.Skill },
	...
	test('discovers nested .json hook files', async () => {
		...
		const nestedUserHook = await seed('/home/.singularity/hooks/domain/tools/post-tool.json', '{"PostToolUse": []}');
	...
	test('discovers hook settings files from fixed workspace locations', async () => {
		const githubSettings = await seed('/workspace/.github/singularity/settings.json', '{"hooks": {"PreToolUse": []}}');
		const githubLocalSettings = await seed('/workspace/.github/singularity/settings.local.json', '{"hooks": {"PostToolUse": []}}');
		...
		await seed('/workspace/.github/singularity/settings.dev.json', '{"hooks": {"Ignored": []}}');
	...
	test('fires onDidChange when fixed hook settings file is modified', async () => {
		...
		await seed('/workspace/.github/singularity/settings.json', '{"hooks": {"PreToolUse": [{"command": "echo test"}]}}');
		await raceTimeout(fired.p, 500);
	...
	test('includes non-README markdown files inside agent folders', async () => {
		...
		const wsSingularityInstructions = await seed('/workspace/.github/agents/singularity-instructions.md', 'singularity instructions body');
		...
		assert.deepStrictEqual([...files].sort((a, b) => a.uri.toString().localeCompare(b.uri.toString())), [
			{ uri: wsSingularityInstructions, type: DiscoveredType.Agent },
			{ uri: wsAgent, type: DiscoveredType.Agent },
	...
	test('discovers nested .instructions.md files', async () => {
		...
		const nestedUserInstr = await seed('/home/.singularity/instructions/domain/tools/deep.instructions.md', 'user nested instruction');
	...
	test('bundles nested .instructions.md files into rules', async () => {
		...
		await seed('/home/.singularity/instructions/domain/tools/deep.instructions.md', 'user nested instruction');
	...
	test('does not include parsed agent-instruction rules in mapToParsedPlugin output', async () => {
		await seed('/workspace/.github/singularity-instructions.md', 'workspace instructions');
	...
	test('returns undefined from mapToParsedPlugin when all customizations are agent-instruction files', async () => {
		...
		await seed('/workspace/.github/singularity-instructions.md', 'workspace instructions');
		await seed('/home/.singularity/singularity-instructions.md', 'user instructions');
	...
	test('scan discovers agent instruction files across every working directory', async () => {
		...
		const first = await seed('/workspace/.github/singularity-instructions.md', 'first');
		const second = await seed('/workspace2/.github/singularity-instructions.md', 'second');
	...
	test('scan discovers hooks from the primary working directory only', async () => {
		const secondWorkspace = URI.from({ scheme: Schemas.inMemory, path: '/workspace2' });
		// `workspace` is primary (index 0); `secondWorkspace` is a non-primary root.
		...
		const discovery = disposables.add(instantiationService.createInstance(SessionCustomizationDiscovery, [workspace, secondWorkspace], userHome, URI.file));
		const hookFiles = (await discovery.scan(CancellationToken.None))
			.filter(directory => directory.type === DiscoveredType.Hook)
			.flatMap(directory => directory.files.map(file => file.uri.toString()))
			.sort((a, b) => a.localeCompare(b));

		// Only the primary root's hook is discovered; the non-primary root's hook is ignored.
	...
	test('discover resolves relative instructions against their attributed project root and groups per root', async () => {
		const secondWorkspace = URI.from({ scheme: Schemas.inMemory, path: '/workspace2' });
		const firstFile = await seed('/workspace/.github/singularity-instructions.md', 'first');
		const secondFile = await seed('/workspace2/.github/singularity-instructions.md', 'second');
			...
			rpc: {
				...
				instructions: {
					getDiscoveryPaths: async () => ({
						paths: [
							{ path: '/workspace/.github/singularity-instructions.md', kind: 'file' },
							{ path: '/workspace2/.github/singularity-instructions.md', kind: 'file' },
						],
					}),
					discover: async (request: AgentsDiscoverRequest) => {
						...
						return {
							sources: [
								{ id: 'a', label: 'A', sourcePath: '.github/singularity-instructions.md', applyTo: undefined, type: 'repo', projectPath: workspace.fsPath },
								{ id: 'b', label: 'B', sourcePath: '.github/singularity-instructions.md', applyTo: undefined, type: 'repo', projectPath: secondWorkspace.fsPath },
							],
						};
					},
				},
