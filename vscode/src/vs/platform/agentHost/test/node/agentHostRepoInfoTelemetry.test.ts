...
suite('AgentHostRepoInfoTelemetry', () => {
	...
	test('emits structured begin and end snapshots against the branch baseline', async () => {
		...
		await collector.reportBegin(restrictedContext, 'agent-session://singularity/s1', 'turn-1', AgentHostClientType.EditorWindow, root, undefined, () => true);
		await collector.reportEnd(restrictedContext, 'agent-session://singularity/s1', 'turn-1', root, undefined, () => true);
	...
	test('skips Git collection when restricted telemetry is unavailable', async () => {
		...
		await collector.reportBegin({ ...restrictedContext, restrictedTelemetryEnabled: false, isInternal: false }, 'agent-session://singularity/s1', 'turn-1', AgentHostClientType.Unknown, URI.file('/repo'), undefined, () => true);
	...
	test('does not emit end after a begin result that legacy suppresses', async () => {
		...
		await collector.reportBegin(restrictedContext, 'agent-session://singularity/s1', 'turn-1', AgentHostClientType.AgentsWindow, root, undefined, () => true);
		await collector.reportEnd(restrictedContext, 'agent-session://singularity/s1', 'turn-1', root, undefined, () => true);
	...
	test('fails closed when content exclusion is unavailable or no checker is provided', async () => {
		...
		await collector.reportBegin({ ...restrictedContext, singularityIgnoreEnabled: true }, 'agent-session://singularity/s1', 'turn-0', AgentHostClientType.Unknown, root, undefined, () => true, async () => ({ available: false, checks: [] }));
		await collector.reportBegin({ ...restrictedContext, singularityIgnoreEnabled: undefined }, 'agent-session://singularity/s1', 'turn-1', AgentHostClientType.Unknown, root, undefined, () => true);
		await collector.reportBegin({ ...restrictedContext, singularityIgnoreEnabled: true }, 'agent-session://singularity/s1', 'turn-2', AgentHostClientType.Unknown, root, undefined, () => true, async () => { throw new Error('policy unavailable'); });
		await collector.reportBegin({ ...restrictedContext, singularityIgnoreEnabled: true }, 'agent-session://singularity/s1', 'turn-3', AgentHostClientType.Unknown, root, undefined, () => true, async () => ({ available: 'yes', checks: [{ path: '/repo/new.txt', excluded: null }] }) as never);
	...
	test('emits paths and patches only when every path for a change is allowed', async () => {
		...
		await collector.reportBegin({ ...restrictedContext, singularityIgnoreEnabled: true }, 'agent-session://singularity/s1', 'turn-1', AgentHostClientType.EditorWindow, root, undefined, () => true, async paths => {
