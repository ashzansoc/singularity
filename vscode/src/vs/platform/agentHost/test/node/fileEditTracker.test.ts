...
suite('FileEditTracker', () => {
	...
	setup(async () => {
		...
		tracker = instantiationService.createInstance(FileEditTracker, 'singularity:/test-session', db);
	});
	...
	test('tracks edit start and complete for existing file', async () => {
		...
		assert.strictEqual(beforeFields.sessionUri, 'singularity:/test-session');
	...
	test('attaches Agent attribution marker to the file edit result', async () => {
		...
		const instantiationService: IInstantiationService = disposables.add(new InstantiationService(services));
		const localTracker = instantiationService.createInstance(FileEditTracker, 'singularity:/test-session', db);
	...
	test('Write to non-existent file records kind=create with removed=0', async () => {
		...
		const localTracker = inst.createInstance(FileEditTracker, 'singularity:/test-session', db);
