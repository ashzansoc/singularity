...
suite('AgentHostStateManager', () => {
	...
	const sessionUri = URI.from({ scheme: 'singularity', path: '/test-session' }).toString();
	...
	test('getSnapshot returns undefined for unknown session', () => {
		const unknown = URI.from({ scheme: 'singularity', path: '/unknown' }).toString();
	...
	test('activeSessions reflects concurrent turn count across sessions', () => {
		const session2Uri = URI.from({ scheme: 'singularity', path: '/test-session-2' }).toString();
	...
	suite('unused-draft tracking', () => {
		...
		test('reports draft status by origin, addressable by session or chat URI', () => {
			const restoredUri = URI.from({ scheme: 'singularity', path: '/restored-session' }).toString();
			manager.createSession(makeSessionSummary());
			...
			assert.deepStrictEqual({
				...
				unknown: manager.isUnusedDraft(URI.from({ scheme: 'singularity', path: '/nope' }).toString()),
			}, {
				created: true,
	...
	suite('multi-chat catalog', () => {
		...
		test('addChat for an unknown session is a no-op', () => {
			...
			const summary = manager.addChat('singularity:/missing', peerChat);
	...
	suite('catalog characterization (A3)', () => {
		...
		test('registerRestoredChatSummary does not register a peer for an unknown session', async () => {
			const summary = manager.registerRestoredChatSummary('singularity:/missing', peerChat, {
				resolver: async () => ({ turns: [] }),
			});
...
suite('Subagent URI helpers', () => {
	...
	test('buildSubagentSessionUri creates correct URI', () => {
		assert.strictEqual(
			buildSubagentSessionUri('singularity:/session-1', 'tc-1'),
			'singularity:/session-1/subagent/tc-1',
		);
	});

	test('buildSubagentSessionUri preserves parent URI path shape', () => {
		assert.strictEqual(
			buildSubagentSessionUri('singularity:/session-1//nested/../kept', 'tc-1'),
			'singularity:/session-1//nested/../kept/subagent/tc-1',
		);
	});

	test('parseSubagentSessionUri extracts parent and toolCallId', () => {
		const parsed = parseSubagentSessionUri('singularity:/session-1/subagent/tc-1');
		assert.deepStrictEqual(parsed && {
			parentSession: parsed.parentSession.toString(),
			toolCallId: parsed.toolCallId,
		}, {
			parentSession: 'singularity:/session-1',
			toolCallId: 'tc-1',
		});
	});

	test('parseSubagentSessionUri handles nested subagent URIs', () => {
		const parsed = parseSubagentSessionUri('singularity:/session-1/subagent/tc-1/subagent/tc-2');
		assert.deepStrictEqual(parsed && {
			parentSession: parsed.parentSession.toString(),
			toolCallId: parsed.toolCallId,
		}, {
			parentSession: 'singularity:/session-1/subagent/tc-1',
			toolCallId: 'tc-2',
		});
	});

	test('parseSubagentSessionUri returns undefined for non-subagent URIs', () => {
		assert.strictEqual(parseSubagentSessionUri('singularity:/session-1'), undefined);
	});

	test('isSubagentSession identifies subagent URIs', () => {
		assert.strictEqual(isSubagentSession('singularity:/session-1/subagent/tc-1'), true);
		assert.strictEqual(isSubagentSession('singularity:/session-1'), false);
	});

	test('buildSubagentSessionUriPrefix creates state manager prefix', () => {
		assert.strictEqual(
			buildSubagentSessionUriPrefix('singularity:/session-1'),
			'singularity:/session-1/subagent/',
		);
	});

	test('buildSubagentSessionUriPrefix preserves parent URI path shape', () => {
		assert.strictEqual(
			buildSubagentSessionUriPrefix('singularity:/session-1//nested/../kept'),
			'singularity:/session-1//nested/../kept/subagent/',
		);
	});
	...
	suite('mergeSessionWithDefaultChat', () => {
		...
		function makeChatState(workingDirectory?: string): ChatState {
			return {
				resource: 'singularity:/test-session/chat/peer',
				title: 'Peer',
				status: SessionStatus.Idle,
				modifiedAt: new Date().toISOString(),
				workingDirectories: workingDirectory ? [workingDirectory] : undefined,
