...
import { SingularityCliConfigKey, applyModelFamilyAlias, singularityCliConfigSchema } from '../../common/singularityCliConfig.js';
import type { SchemaValues } from '../../common/agentHostSchema.js';
import type { ModelSelection } from '../../common/state/protocol/state.js';
import { AgentHostPromptRegistry, agentHostPromptRegistry, type IAgentHostPromptContext } from '../../node/singularity/prompts/promptRegistry.js';
import { SINGULARITY_AGENT_HOST_FILE_LINK_INSTRUCTIONS, SINGULARITY_AGENT_HOST_WORKSPACELESS_INSTRUCTIONS, SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE } from '../../node/singularity/prompts/systemMessage.js';
...
import '../../node/singularity/prompts/allPrompts.js';

/**
 * Builds a prompt context backed by an in-memory bag of customization settings
 * and an optional set of available tool names.
 */
function context(settings: SchemaValues<typeof singularityCliConfigSchema.definition> = {}, tools: readonly string[] = [], workspaceless = false, toolSearchActive = false): IAgentHostPromptContext {
...
suite('AgentHostPromptRegistry', () => {
	...
	const withFileLinkInstructions = (config: SystemMessageConfig): SystemMessageConfig => ({
		...config,
		content: config.content ? `${config.content}\n\n${SINGULARITY_AGENT_HOST_FILE_LINK_INSTRUCTIONS}` : SINGULARITY_AGENT_HOST_FILE_LINK_INSTRUCTIONS,
	});

	test('falls back to the default system message when no model is provided', () => {
		const registry = new AgentHostPromptRegistry();
		assert.deepStrictEqual(registry.resolveSystemMessageConfig(undefined, context()), withFileLinkInstructions(SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE));
	});

	test('falls back to the default when no contributor matches the model', () => {
		const registry = new AgentHostPromptRegistry();
		assert.deepStrictEqual(registry.resolveSystemMessageConfig({ id: 'unknown-model' }, context()), withFileLinkInstructions(SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE));
	});
	...
	test('treats empty section overrides as no override (falls back to default)', () => {
		...
		assert.deepStrictEqual(
			registry.resolveSystemMessageConfig({ id: 'claude-sonnet' }, context()),
			withFileLinkInstructions(SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE)
		);
	});
	...
	test('contributors gate on the prompt context', () => {
		...
		registry.registerPrompt(class {
			...
			resolveSectionOverrides(_model: ModelSelection, ctx: IAgentHostPromptContext): Partial<Record<SystemMessageSection, SectionOverride>> | undefined {
				return ctx.getSetting(SingularityCliConfigKey.Opus48Prompt) === true ? { tone: { action: 'append', content: 'GATED' } } : undefined;
			}
		});
		assert.deepStrictEqual(
			registry.resolveSystemMessageConfig({ id: 'claude-x' }, context({ [SingularityCliConfigKey.Opus48Prompt]: true })),
	...
	suite('Opus contributor (registered via allPrompts)', () => {
		...
		function resolveOpus(enabled: boolean | undefined) {
			return agentHostPromptRegistry.resolveSystemMessageConfig(opusModel, context(enabled === undefined ? {} : { [SingularityCliConfigKey.Opus48Prompt]: enabled }));
		}

		test('applies customize overrides only when enabled', () => {
			assert.deepStrictEqual(resolveOpus(undefined), withFileLinkInstructions(SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE));
			assert.deepStrictEqual(resolveOpus(false), withFileLinkInstructions(SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE));
	...
	suite('model capability overrides (family alias)', () => {
		...
		// The alias helper's own behavior is covered in singularityCliConfig.test.ts.
		test('an aliased preview model routes to the family contributor', () => {
			...
			const result = agentHostPromptRegistry.resolveSystemMessageConfig(
				...
				context({ [SingularityCliConfigKey.Opus48Prompt]: true })
			);
	...
	suite('workspace-less scratch/repoless wiring', () => {
		test('appends the scratch instructions to the default config for a workspace-less chat', () => {
			...
			assert.deepStrictEqual(
				...
				{
					mode: 'customize',
					sections: SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE.sections,
					content: `${SINGULARITY_AGENT_HOST_WORKSPACELESS_INSTRUCTIONS}\n\n${SINGULARITY_AGENT_HOST_FILE_LINK_INSTRUCTIONS}`,
				}
			);
		});
	...
	suite('universal tool instructions wiring', () => {
		...
		test('is a no-op when the session exposes no matching tools', () => {
			const registry = new AgentHostPromptRegistry();
			assert.deepStrictEqual(registry.resolveSystemMessageConfig({ id: 'm' }, context({}, ['anyTool'])), withFileLinkInstructions(SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE));
		});
		...
		test('layers the browser tool_instructions onto the default config when browser tools are present', () => {
			...
			assert.deepStrictEqual(
				...
				withFileLinkInstructions({
					mode: 'customize',
					sections: {
						identity: SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE.sections.identity,
