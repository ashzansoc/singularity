...
import type { CopilotSession as SingularitySession, CurrentToolMetadata, PermissionAllowAllMode, PermissionRequest, SessionEvent, SessionEventHandler, SessionEventPayload, SessionEventType, Tool, ToolResultObject, TypedSessionEventHandler } from '@github/copilot-sdk';
...
import { buildNonPtyShellTerminalUri } from '../../node/singularity/singularityNonPtyShellTerminals.js';
import { ActiveClientToolSet } from '../../node/activeClientState.js';
import { type SingularitySessionLaunchPlan, type IActiveClientSnapshot, type ISingularitySessionLauncher, type ISingularitySessionRuntime } from '../../node/singularity/singularitySessionLauncher.js';
import { SingularitySessionWrapper } from '../../node/singularity/singularitySessionWrapper.js';
import { AgentHostStateManager, IAgentHostStateManager } from '../../node/agentHostStateManager.js';
import { IAgentHostTerminalManager } from '../../node/agentHostTerminalManager.js';
import { TestAgentHostTerminalManager } from './testAgentHostTerminalManager.js';
import { buildSingularitySystemNotification } from '../../node/singularity/singularitySystemNotification.js';
...
// ---- Mock SingularitySession (SDK level) ----------------------------------------

/**
 * Minimal mock of the SDK's {@link SingularitySession}. Implements `on()` to
 * store typed handlers, and exposes `fire()` so tests can push events
 * through the real {@link SingularitySessionWrapper} event pipeline.
 */
class MockSingularitySession {
	...
	/**
	 * Mirrors the SDK's own usage tracker, which folds the `singularityUsage` billed on
	...
	private _accumulateUsageMetrics(type: SessionEventType, data: unknown): void {
		...
		const billed = type === 'assistant.usage'
			...
			: type === 'session.compaction_complete'
				...
				: undefined;
		const totalNanoAiu = (billed as { singularityUsage?: { totalNanoAiu?: number } } | undefined)?.singularityUsage?.totalNanoAiu;
...
class TestSingularityApiService implements ISingularityApiService {
	...
	async utilityChatCompletion(_githubToken: string, _request: ISingularityUtilityChatCompletionRequest): Promise<string> { throw new Error('not used'); }
 ...
 * {@link ToolResultObject} — which is what {@link SingularityAgentSession}'s
...
type TestSingularitySessionRuntime = Omit<ISingularitySessionRuntime, 'handlePermissionRequest'> & {
	handlePermissionRequest(request: TestPermissionRequest): ReturnType<ISingularitySessionRuntime['handlePermissionRequest']>;
};
...
async function createAgentSession(disposables: DisposableStore, options?: {
	...
	captureRuntime?: { current?: ISingularitySessionRuntime };
	...
	/** Configure the mock session before {@link SingularityAgentSession.initializeSession} runs. */
	configureMockSession?: (session: MockSingularitySession) => void;
	...
	session: SingularityAgentSession;
	runtime: TestSingularitySessionRuntime;
	mockSession: MockSingularitySession;
	signals: AgentSignal[];
	...
	const parentSessionUri = AgentSession.uri('singularity', 'test-session-1');
	...
	const launchPlanBase = {
		client: {
			createSession: async () => mockSession as unknown as SingularitySession,
	...
	const launchPlan: SingularitySessionLaunchPlan = options?.resume
		? {
			...launchPlanBase,
			kind: 'resume',
	...
	let launchedRuntime: ISingularitySessionRuntime | undefined;
	const sessionLauncher: ISingularitySessionLauncher = {
		launch: async (_plan, runtime) => {
			launchedRuntime = runtime;
			if (options?.captureRuntime) {
				options.captureRuntime.current = runtime;
			}
			return new SingularitySessionWrapper(mockSession as unknown as SingularitySession);
		}
	};
	...
	singularityApiService.apiEndpoint = options?.singularityApiEndpoint;
	if (options?.restrictedTelemetryContext) {
		singularityApiService.restrictedTelemetryContext = options.restrictedTelemetryContext;
	}
	singularityApiService.restrictedTelemetryContextError = options?.restrictedTelemetryContextError;
	...
	const runtime: TestSingularitySessionRuntime = {
		...sdkRuntime,
...
suite('SingularityAgentSession', () => {
	...
	suite('SingularitySessionWrapper', () => {
		test('fires unhandled events when no wrapped listener is registered', () => {
			const mockSession = new MockSingularitySession();
			const wrapper = disposables.add(new SingularitySessionWrapper(mockSession as unknown as SingularitySession));
			const events: string[] = [];
	...
	test('logs SDK events without wrapped handlers', async () => {
		...
		assert.deepStrictEqual(
			...
			['[Singularity:test-session-1] Unhandled SDK event: {"type":"session.title_changed","data":{"title":"A new title"},"id":"evt-title","timestamp":"2026-06-24T00:00:00.000Z","parentId":null,"ephemeral":true}']
		);
	});
	...
	test('logs managed settings resolution and enforcement', async () => {
		...
		assert.deepStrictEqual({
			...
			infos: ['[Singularity:test-session-1] Managed settings resolved: source=server, managedKeys=permissions, bypassPermissionsDisabled=true, failClosed=false'],
			warnings: ['[Singularity:test-session-1] Managed settings enforced: action=bypass_permissions_blocked, setting=permissions.disableBypassPermissionsMode, escalation=allow_all, failClosed=false, message=Bypass permissions mode is disabled by enterprise policy.'],
		});
	});

	test('maps internal attachment URIs to Singularity SDK path fields', async () => {
	...
	test('a resumed session does not bill its restored history to the first new turn', async () => {
		...
		mockSession.fire('assistant.usage', {
			...
			inputTokens: 10,
			outputTokens: 20,
			singularityUsage: { totalNanoAiu: 500_000_000 },
		} as unknown as SessionEventPayload<'assistant.usage'>['data']);
		await timeout(0);
		...
		assert.deepStrictEqual(usageActions.at(-1)?.usage._meta?.singularityUsage, {
	...
	test('a failed usage read leaves the turn cost intact', async () => {
		...
		mockSession.fire('assistant.usage', {
			...
			inputTokens: 10,
			outputTokens: 20,
			singularityUsage: { totalNanoAiu: 250_000_000 },
		} as unknown as SessionEventPayload<'assistant.usage'>['data']);
		await timeout(0);
	...
	test('a session total that drops after truncation is adopted rather than treated as stale', async () => {
		...
		mockSession.fire('assistant.usage', {
			...
			inputTokens: 10,
			outputTokens: 20,
			singularityUsage: { totalNanoAiu: 10_000_000_000 },
		} as unknown as SessionEventPayload<'assistant.usage'>['data']);
		await timeout(0);
		...
		mockSession.fire('assistant.usage', {
			...
			inputTokens: 10,
			outputTokens: 20,
			singularityUsage: { totalNanoAiu: 1_000_000_000 },
		} as unknown as SessionEventPayload<'assistant.usage'>['data']);
		await timeout(0);
	...
	test('overlapping usage events issue one metrics read at a time and converge on the newest', async () => {
		...
		const usageActions = getActions(signals).filter(a => a.type === ActionType.ChatUsage) as ChatUsageAction[];
		assert.strictEqual(
			(usageActions.at(-1)?.usage._meta as UsageInfoMeta | undefined)?.singularityUsage?.sessionTotalNanoAiu,
			2_250_000_000,
		);
	});
	...
	test('a turn ending while its usage refresh is in flight still bills that turn', async () => {
		...
		assert.strictEqual((usageActions.at(-1)?.usage._meta as UsageInfoMeta | undefined)?.singularityUsage?.totalNanoAiu, 500_000_000);
	});
	...
	test('`/compact` reports the compaction call credits on the post-compaction usage', async () => {
		...
		mockSession.onCompact = () => mockSession.fire('session.compaction_complete', {
			success: true,
			tokensRemoved: 1200,
			// `singularityUsage` is `asInternal` in the SDK schema, so it is absent from the public type but present at runtime.
			compactionTokensUsed: { model: 'claude-sonnet-4.6', inputTokens: 9000, outputTokens: 400, singularityUsage: { totalNanoAiu: 250_000_000 } },
		...
		assert.deepStrictEqual(usageActions.at(-1), {
			...
			usage: {
				...
				model: undefined,
				_meta: {
					singularityUsage: { totalNanoAiu: 250_000_000, sessionTotalNanoAiu: 250_000_000 },
	...
	test('automatic compaction folds its credits into the turn running total', async () => {
		...
		mockSession.fire('session.compaction_complete', {
			success: true,
			tokensRemoved: 1200,
			compactionTokensUsed: { model: 'claude-sonnet-4.6', singularityUsage: { totalNanoAiu: 250_000_000 } },
		} as unknown as SessionEventPayload<'session.compaction_complete'>['data']);
		await timeout(0);
		...
		assert.deepStrictEqual(usageActions.at(-1)?.usage, {
			...
			_meta: {
				singularityUsage: { totalNanoAiu: 750_000_000, sessionTotalNanoAiu: 750_000_000 },
	...
	test('failed compaction does not report credits', async () => {
		...
		mockSession.fire('session.compaction_complete', {
			success: false,
			error: 'boom',
			compactionTokensUsed: { singularityUsage: { totalNanoAiu: 250_000_000 } },
		} as unknown as SessionEventPayload<'session.compaction_complete'>['data']);
		await timeout(0);
	...
	test('compaction billed outside a turn shows in the session total, not on the next turn', async () => {
		...
		mockSession.fire('session.compaction_complete', {
			success: true,
			compactionTokensUsed: { model: 'claude-opus-4.6', singularityUsage: { totalNanoAiu: 133_468_375_000 } },
		} as unknown as SessionEventPayload<'session.compaction_complete'>['data']);
		await timeout(0);
		assert.deepStrictEqual(getActions(signals).filter(a => a.type === ActionType.ChatUsage), []);
		...
		assert.deepStrictEqual(usageActions.at(-1)?.usage, {
			...
			_meta: {
				...
				singularityUsage: { totalNanoAiu: 500_000_000, sessionTotalNanoAiu: 133_968_375_000 },
	...
	test('a turn that never reports usage does not inherit out-of-turn compaction cost', async () => {
		...
		mockSession.fire('session.compaction_complete', {
			success: true,
			compactionTokensUsed: { singularityUsage: { totalNanoAiu: 2_000_000_000 } },
		} as unknown as SessionEventPayload<'session.compaction_complete'>['data']);
		await timeout(0);
		...
		assert.deepStrictEqual(usageActions.at(-1)?.usage._meta, {
			singularityUsage: { totalNanoAiu: 1_000_000_000, sessionTotalNanoAiu: 3_000_000_000 },
	...
	test('each turn bills only its own calls while the session total accumulates', async () => {
		...
		assert.deepStrictEqual(usageActions.map(a => ({ turnId: a.turnId, singularityUsage: a.usage._meta?.singularityUsage })), [
			{ turnId: 'turn-1', singularityUsage: { totalNanoAiu: 1_000_000_000, sessionTotalNanoAiu: 2_000_000_000 } },
			{ turnId: 'turn-1', singularityUsage: { totalNanoAiu: 1_000_000_000, sessionTotalNanoAiu: 3_000_000_000 } },
			{ turnId: 'turn-2', singularityUsage: { totalNanoAiu: 1_000_000_000, sessionTotalNanoAiu: 3_000_000_000 } },
			{ turnId: 'turn-2', singularityUsage: { totalNanoAiu: 1_000_000_000, sessionTotalNanoAiu: 4_000_000_000 } },
		]);
	});
	...
	test('emits accumulated Singularity usage metadata', async () => {
		const { session, mockSession, signals } = await createAgentSession(disposables);
		...
		mockSession.fire('assistant.usage', {
			...
			cacheReadTokens: 5,
			cost: 2,
			// `singularityUsage` is marked `asInternal` in the SDK schema so it is not on the public type, but is present at runtime.
			singularityUsage: { totalNanoAiu: 500_000_000, tokenDetails: [] },
		} as unknown as SessionEventPayload<'assistant.usage'>['data']);
		await timeout(0);
		mockSession.fire('assistant.usage', {
			model: 'claude-sonnet-4.6',
			...
			outputTokens: 40,
			cost: 2,
			singularityUsage: { totalNanoAiu: 750_000_000, tokenDetails: [] },
		} as unknown as SessionEventPayload<'assistant.usage'>['data']);
		await timeout(0);
		...
		assert.deepStrictEqual(usageActions.at(-1)?.usage, {
			...
			_meta: {
				cost: 2,
				singularityUsage: { totalNanoAiu: 1_250_000_000, sessionTotalNanoAiu: 1_250_000_000 },
	...
	test('reports the parent turn aggregate and additionally the per-subagent component', async () => {
		...
		mockSession.fire('assistant.usage', {
			...
			inputTokens: 5,
			outputTokens: 7,
			singularityUsage: { totalNanoAiu: 200_000_000, tokenDetails: [] },
		} as unknown as SessionEventPayload<'assistant.usage'>['data'], { agentId: 'agent-1' });
		await timeout(0);
		...
		mockSession.fire('assistant.usage', {
			...
			inputTokens: 6,
			outputTokens: 8,
			singularityUsage: { totalNanoAiu: 300_000_000, tokenDetails: [] },
		} as unknown as SessionEventPayload<'assistant.usage'>['data'], { agentId: 'agent-1' });
		await timeout(0);
		...
		const usageSignals = signals.flatMap(signal => {
			...
			return [{
				...
				totalNanoAiu: (signal.action.usage._meta as UsageInfoMeta | undefined)?.singularityUsage?.totalNanoAiu,
			}];
		});
	...
	suite('permission handling', () => {
		...
		test('auto-approves read permission for session-state plan files', async () => {
			...
			process.env['SINGULARITY_HOME'] = '/mock-state-home/.singularity';
			try {
				const { runtime, signals } = await createAgentSession(disposables);
				const result = await runtime.handlePermissionRequest({
					kind: 'read',
					path: join('/mock-state-home', '.singularity', 'session-state', 'test-session-1', 'plan.md'),
		...
		test('resolves native environment through INativeEnvironmentService registration', async () => {
			...
			try {
				...
				const result = await runtime.handlePermissionRequest({
					kind: 'read',
					path: join('/mock-home', '.singularity', 'session-state', 'test-session-1', 'plan.md'),
		...
		test('logs and rethrows permission failures', async () => {
			...
			try {
				...
				assert.strictEqual(entry.args[0], '[Singularity:test-session-1] Failed to handle permission request: kind=read, toolCallId=tc-read-plan-missing-env');
			} finally {
				if (previousSingularityHome === undefined) {
					delete process.env['SINGULARITY_HOME'];
				} else {
		...
		test('auto-approves write permission for session-state plan files', async () => {
			...
			try {
				...
				const result = await runtime.handlePermissionRequest({
					kind: 'write',
					fileName: join('/mock-state-home', '.singularity', 'session-state', 'test-session-1', 'plan.md'),
		...
		test('does not auto-approve session-state files from another session', async () => {
			...
			try {
				...
				const resultPromise = runtime.handlePermissionRequest({
					kind: 'write',
					fileName: join('/mock-state-home', '.singularity', 'session-state', 'different-session', 'plan.md'),
		...
		test('does not auto-approve traversal paths that escape the session-state directory', async () => {
			...
			try {
				const { session, runtime, signals, waitForSignal } = await createAgentSession(disposables);
				const sessionDir = join('/mock-state-home', '.singularity', 'session-state', 'test-session-1');
				const resultPromise = runtime.handlePermissionRequest({
					kind: 'write',
		...
		test('auto-approves read of Singularity SDK large-tool-output temp files', async () => {
			const { runtime, signals } = await createAgentSession(disposables);

			// Layout 1: <timestamp>-singularity-tool-output-<id>.txt
			const result1 = await runtime.handlePermissionRequest({
				kind: 'read',
				path: join('/mock-tmp', '1730000000000-singularity-tool-output-abc123.txt'),
				toolCallId: 'tc-tool-output-1',
			});
			assert.strictEqual(result1.kind, 'approve-once');

			// Layout 2: singularity-tool-output-<timestamp>-<id>.txt
			const result2 = await runtime.handlePermissionRequest({
				kind: 'read',
				path: join('/mock-tmp', 'singularity-tool-output-1730000000000-abc123.txt'),
		...
		test('does not auto-approve tool-output-named files outside tmpdir', async () => {
			...
			const resultPromise = runtime.handlePermissionRequest({
				kind: 'read',
				path: join('/some/other/dir', 'singularity-tool-output-1730000000000-abc123.txt'),
		...
		test('does not auto-approve write to a tool-output temp path', async () => {
			...
			const resultPromise = runtime.handlePermissionRequest({
				kind: 'write',
				fileName: join('/mock-tmp', 'singularity-tool-output-1730000000000-abc123.txt'),
		...
		test('switching from Bypass to Default immediately disables SDK allow-all mode', async () => {
			...
			assert.deepStrictEqual({
				...
				modes: ['on', 'off'],
				logs: [
					'[Singularity:test-session-1] Syncing permission mode: source=turn-start, agentMode=interactive, configuredLevel=autoApprove, sdkMode=on, previousSdkMode=unknown, globalAutoApprove=false',
					'[Singularity:test-session-1] Syncing permission mode: source=config-change, agentMode=interactive, configuredLevel=default, sdkMode=off, previousSdkMode=on, globalAutoApprove=false',
				],
			});
		});
		...
		test('ignores approval changes for other sessions', async () => {
			...
			fireSessionConfigChange({ [SessionConfigKey.AutoApprove]: 'default' }, AgentSession.uri('singularity', 'other-session').toString());
			await timeout(0);
		...
		test('per-request sandbox: left untouched when the custom terminal tool is enabled', async () => {
			const { session, mockSession } = await createAgentSession(disposables, {
				rootValues: {
					[AgentHostSandboxConfigKey.Sandbox]: { [AgentHostSandboxKey.Enabled]: AgentSandboxEnabledValue.On },
					[SingularityCliConfigKey.EnableCustomTerminalTool]: true,
				},
			});
		...
		test('rejects permission requests for unroutable subagent tools', async () => {
			...
			assert.deepStrictEqual({
				...
				errors: ['[Singularity:test-session-1] Rejecting permission request for unroutable subagent tool call: toolCallId=tc-orphaned-subagent-tool, kind=shell'],
			});
		});
	});
	...
	suite('system.notification', () => {
		...
		test('translator handles every notification kind and ignores empty content', () => {
			...
			assert.deepStrictEqual(buildSingularitySystemNotification({
				...base,
				data: {
			...
			const shellNotificationWithoutDescription = buildSingularitySystemNotification({
				...base,
				data: {
					content: 'Shell done',
			...
			const detachedShellNotification = buildSingularitySystemNotification({
				...base,
				data: {
			...
			assert.strictEqual(buildSingularitySystemNotification({
				...base,
				data: {
					content: '   ',
					kind: { type: 'agent_idle', agentId: 'agent-a', agentType: 'task' },
	...
	suite('event mapping', () => {
		...
		test('MCP tool deltas stream before final contributor metadata arrives', async () => {
			...
			assert.deepStrictEqual({
				...
				readyContributor: {
					...
					customizationId: 'mcp-top-level:singularity:test-session-1:docs',
				},
			});
		});
			...
			if (toolStart && isAction(toolStart, ActionType.ChatToolCallStart)) {
				...
				assert.deepStrictEqual({
					...
					meta: {
						...
						ui: {
							resourceUri: 'ui://docs',
							channel: 'mcp://singularity/test-session-1/docs',
						},
					},
				});
			}
		...
		test('non-pty shell terminal URIs are scoped by session and tool call', () => {
			assert.deepStrictEqual([
				buildNonPtyShellTerminalUri(AgentSession.uri('singularity', 'session-1'), 'tool-call-1'),
				buildNonPtyShellTerminalUri(AgentSession.uri('singularity', 'session-1'), 'tool-call-2'),
				buildNonPtyShellTerminalUri(AgentSession.uri('singularity', 'session-2'), 'tool-call-1'),
			], [
		...
		test('emits todo store telemetry for successful built-in Singularity SQL', async () => {
			const telemetryService = new CapturingTelemetryService();
			const { mockSession, waitForSignal } = await createAgentSession(disposables, {
				telemetryService,
				sessionUri: AgentSession.uri('singularitycli', 'test-session-1'),
			});
		...
		test('tool partial results stream into an output-only terminal channel', async () => {
			...
			assert.deepStrictEqual(terminalManager.outputTerminalsCreated, [{
				...
				claim: { kind: TerminalClaimKind.Session, session: AgentSession.uri('singularity', 'test-session-1').toString(), toolCallId: 'tc-stream' },
			}]);
		...
		test('edit hooks resolve relative apply_patch file paths against workingDirectory', async () => {
			const capturedRuntime: { current?: ISingularitySessionRuntime } = {};
		...
		test('error event is forwarded', async () => {
			...
			assert.deepStrictEqual(telemetryService.events.filter(event => event.eventName === 'agentHost.singularitySdkSessionError'), [{
				eventName: 'agentHost.singularitySdkSessionError',
				data: {
					agentSessionId: 'test-session-1',
					chatSessionId: getTelemetryChatSessionId(URI.parse(buildDefaultChatUri(AgentSession.uri('singularity', 'test-session-1')))),
					turnId: 'turn-1',
		...
		test('model call failure emits structured correlation telemetry without the restricted error message', async () => {
			...
			assert.deepStrictEqual(telemetryService.events.filter(event => event.eventName === 'agentHost.singularityModelCallFailure'), [{
				eventName: 'agentHost.singularityModelCallFailure',
				data: {
					agentSessionId: 'test-session-1',
					chatSessionId: getTelemetryChatSessionId(URI.parse(buildDefaultChatUri(AgentSession.uri('singularity', 'test-session-1')))),
					turnId: 'turn-1',
	...
	suite('SDK callback logging', () => {
		...
		test('logs and rethrows user input callback failures', async () => {
			...
			assert.strictEqual(entry.args[0], '[Singularity:test-session-1] Failed to handle user input request: question="Need input"');
		});
		...
		test('logs and rethrows onPreToolUse failures', async () => {
			...
			assert.strictEqual(entry.args[0], '[Singularity:test-session-1] Failed in onPreToolUse: tool=edit');
		});
		...
		test('logs and rethrows onPostToolUse failures', async () => {
			...
			assert.strictEqual(entry.args[0], '[Singularity:test-session-1] Failed in onPostToolUse: tool=edit');
		});
	});
	...
	suite('client tool calls', () => {
		...
		async function createToolSearchSession(autoApprove: boolean) {
			...
			const created = await createAgentSession(disposables, {
				...
				rootValues: { [SingularityCliConfigKey.ToolSearchEnabled]: true },
			});
			if (autoApprove) {
		...
		test('toolSearch is omitted when the flag is off or the model is unsupported', async () => {
			...
			const flagOff = await createAgentSession(disposables, {
				clientSnapshot: toolSearchSnapshot,
				modelId: 'claude-opus-4.8',
				rootValues: { [SingularityCliConfigKey.ToolSearchEnabled]: false },
			});
		...
		test('client tool handler logs and rethrows failures', async () => {
			...
			assert.strictEqual(entry.args[0], '[Singularity:test-session-1] Failed in client tool handler: tool=my_tool, toolCallId=tc-client-error');
		});
	...
	suite('server tools', () => {
		...
		test('advertises the server tools on initialize and exposes them as server SDK tools', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', 'test-session-1').toString();
		...
		test('server tool handler routes to the host and returns a success result', async () => {
			...
			const sessionUri = buildDefaultChatUri(AgentSession.uri('singularity', 'test-session-1'));
	...
	suite('plan mode', () => {
		...
		test('completing the input request with autopilot preserves Ask When Needed and syncs mode=autopilot', async () => {
			...
			assert.deepStrictEqual(sessionConfigUpdates, [
				{ session: 'singularity:/test-session-1', patch: { mode: 'autopilot' } },
			]);
		});
		...
		test('completing the input request with interactive resolves with approved + interactive (no autoApprove) and syncs mode=interactive', async () => {
			...
			assert.deepStrictEqual(sessionConfigUpdates, [
				{ session: 'singularity:/test-session-1', patch: { mode: 'interactive' } },
			]);
		});
		...
		test('session.mode_changed → plan updates the AHP session config', async () => {
			...
			assert.deepStrictEqual(sessionConfigUpdates, [
				{ session: 'singularity:/test-session-1', patch: { mode: 'plan' } },
			]);
		});
		...
		test('session.mode_changed → autopilot maps directly to mode=autopilot', async () => {
			...
			// untouched. The translation is contained in the Singularity agent.
			const { mockSession, sessionConfigUpdates } = await createAgentSession(disposables);
	...
	suite('MCP server inventory', () => {
		...
		test('session MCP desired enablement reconciles runtime drift', async () => {
			...
			const id = 'mcp-top-level:singularity:test-session-1:slack';
		...
		test('sending a message does not mark an enabled server as Starting', async () => {
			const serverName = 'db';
			const id = 'mcp-top-level:singularity:test-session-1:db';
			const { session } = await createAgentSession(disposables, {
				sessionCustomizations: () => [{
					type: CustomizationType.McpServer,
					id,
		...
		test('peer chat MCP desired enablement uses parent session customizations', async () => {
			const parentSessionUri = AgentSession.uri('singularity', 'parent-session');
		...
		test('tool-triggered MCP auth transitions the active MCP tool call until authentication resolves', async () => {
			...
			const authPromise = runtime.handleMcpAuthRequest({
				requestId: 'auth-tool',
				serverName: 'github',
				serverUrl: 'https://api.githubsingularity.com/mcp/',
				reason: 'upscope',
				resourceMetadata: JSON.stringify({
					resource: 'https://api.githubsingularity.com/mcp/',
		...
		test('initial GitHub MCP auth reuses the existing token without requesting the advertised scope catalog', async () => {
			...
			const result = await runtime.handleMcpAuthRequest({
				...
				wwwAuthenticateParams: {
					resourceMetadataUrl: 'https://api.githubsingularity.com/.well-known/oauth-protected-resource/mcp/',
		...
		test('initial GitHub MCP auth reuses the existing token for the per-user enterprise endpoint', async () => {
			const { runtime, signals } = await createAgentSession(disposables, {
				...
				singularityApiEndpoint: 'https://api.enterprise.githubsingularity.com',
			});

			const result = await runtime.handleMcpAuthRequest({
				requestId: 'enterprise-initial-auth',
				serverName: 'github',
				serverUrl: 'https://api.enterprise.githubsingularity.com/mcp/',
				reason: 'initial',
			}, { sessionId: 'test-session-1' });
		...
		test('MCP auth request publishes authRequired state and resolves with authenticate token', async () => {
			...
			assert.deepStrictEqual(action.customization, {
				type: 'mcpServer',
				id: 'mcp-top-level:singularity:test-session-1:github',
				uri: 'mcp-top-level:singularity:test-session-1:github',
				name: 'github',
				enabled: true,
				state: {
			...
			assert.strictEqual(await session.resolveMcpAuthentication({ resource: 'https://api.githubsingularity.com/mcp', scopes: ['repo', 'notifications'], token: 'token-1' }), true);
		...
		test('a failed MCP server logs at error with structured attributes and preserves the failure detail', async () => {
			...
			assert.deepStrictEqual({
				...
				body: record ? String(record.first).replace(/^\[Singularity:[^\]]*\]\s*/, '') : undefined,
	...
	suite('repoInfo telemetry', () => {
		test('captures one begin and end across root SDK rounds', async () => {
			...
			const { session, mockSession } = await createAgentSession(disposables, {
				...
				restrictedTelemetryContext: {
					...
					singularityIgnoreEnabled: false,
				},
			});
		...
		test('skips capture when repository telemetry context resolution fails', async () => {
			...
			assert.deepStrictEqual({
				...
				events: [],
				warnings: ['[Singularity:test-session-1] Failed to resolve repository info telemetry context: context failed'],
			});
		});
	});
	...
	suite('instructionsCollected telemetry', () => {
		...
		function fireUserMessage(mockSession: MockSingularitySession, overrides?: Partial<{ content: string; source: string }>) {
			mockSession.fire('user.message', {
		...
		test('emits with counts derived from source types + AH identifiers', async () => {
			...
			mockSession.getInstructionSourcesResult = {
				sources: [
					{ id: 'a', label: '', sourcePath: '.github/singularity-instructions.md', content: '', type: 'repo', location: 'repository' },
