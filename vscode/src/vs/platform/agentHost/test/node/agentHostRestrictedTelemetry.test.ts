...
/** The enhanced/restricted iKey (`singularity_v0_restricted_singularity_event`). */
...
suite('AgentHostRestrictedTelemetrySender', () => {
	...
	test('context-scoped enhanced telemetry ignores mutable account routing and identity', () => {
		...
		sender.setSingularityTrackingId('current-account-tid');
		...
		}, 'engine.messages', { singularity_trackingId: 'payload-tid' });

		assert.deepStrictEqual({
			posts,
			trackingId: envelopes[0].data.baseData.properties.singularity_trackingId,
		}, {
