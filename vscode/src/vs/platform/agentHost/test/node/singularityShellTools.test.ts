...
import { createShellTools, type IUnsandboxedCommandConfirmationRequest, isMultilineCommand, ShellManager, prefixForHistorySuppression, shellTypeForExecutable } from '../../node/singularity/singularityShellTools.js';
...
suite('SingularityShellTools', () => {
	...
	test('uses session working directory for created shells', async () => {
		...
		const shellManager = disposables.add(instantiationService.createInstance(ShellManager, URI.parse('singularity:/session-1'), URI.file(worktreePath)));
	...
	test('opts every managed shell into shell-history suppression and non-interactive mode', async () => {
		const { instantiationService, terminalManager } = createServices();
		const shellManager = disposables.add(instantiationService.createInstance(ShellManager, URI.parse('singularity:/session-1'), undefined));
	...
	test('primary shell tool normalizes multiline command input', async () => {
		...
		assert.match(terminalManager.writes[1].data, /^echo "<<<SINGULARITY_SENTINEL_[a-f0-9]+_EXIT_\$\?>>>"\r$/);
	});
	...
	test('primary shell tool ignores echoed sentinel command text', async () => {
		...
		const sentinelMatch = terminalManager.writes[1].data.match(/<<<SINGULARITY_SENTINEL_([a-f0-9]+)_EXIT_\$\?>>/);
		...
		const content = [
			...
			`echo "<<<SINGULARITY_SENTINEL_${sentinelId}_EXIT_$?>>>"`,
			`<<<SINGULARITY_SENTINEL_${sentinelId}_EXIT_0>>>`,
		].join('\r\n');
	...
	test('backgrounded shell is not immediately reused', async () => {
		...
		await waitForSentTexts(terminalManager, 1);
		terminalManager.fireClaimChanged({ kind: TerminalClaimKind.Session, session: 'singularity:/session-1', turnId: 'turn-1' });
		const result = await resultPromise;
		assert.strictEqual(result.resultType, 'success');
	...
	test('primary shell tool schema only exposes requestUnsandboxedExecution params when the sandbox is enabled', async () => {
		...
		const enabledShell = disposables.add(enabled.instantiationService.createInstance(ShellManager, URI.parse('singularity:/session-enabled'), undefined));
		...
		const disabledShell = disposables.add(disabled.instantiationService.createInstance(ShellManager, URI.parse('singularity:/session-disabled'), undefined));
	...
	test('primary shell tool writes a sandbox config exposing the working directory as writable', async () => {
		...
		const shellManager = disposables.add(instantiationService.createInstance(ShellManager, URI.parse('singularity:/session-1'), workingDirectory));
		const tools = await createShellTools(shellManager, terminalManager, new NullLogService());
		const bashTool = tools.find(tool => tool.name === 'bash');
		assert.ok(bashTool);
	...
	test('primary shell tool merges configured filesystem allowRead paths into the sandbox config', async () => {
		...
		const shellManager = disposables.add(instantiationService.createInstance(ShellManager, URI.parse('singularity:/session-1'), URI.file('/workspace/test-workspace')));
		const tools = await createShellTools(shellManager, terminalManager, new NullLogService());
		const bashTool = tools.find(tool => tool.name === 'bash');
		assert.ok(bashTool);
