...
import { SingularityGitHubTelemetryForwarder } from '../../node/singularity/singularityGitHubTelemetryForwarder.js';
...
suite('SingularityGitHubTelemetryForwarder', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	test('forwards a standard event to VS Code telemetry', () => {
		const telemetryService = new TestTelemetryService();
		const forwarder = new SingularityGitHubTelemetryForwarder(() => false, telemetryService);
		...
		forwarder.forward({
			...
			event: {
				...
				singularity_tracking_id: 'tracking-id',
				client: {
					cli_version: '1.0.69',
					os_platform: 'win32',
					os_version: '11',
			...
			} as import('@github/copilot-sdk').GitHubTelemetryEvent & { singularity_tracking_id?: string },
		});

		assert.deepStrictEqual(telemetryService.events, [{
			eventName: 'singularityCli/tool_call_executed',
			data: {
				cli_version: '1.0.69',
				os_platform: 'win32',
				os_version: '11',
	...
	test('gates restricted events on the restricted telemetry option', () => {
		...
		const forwarder = new SingularityGitHubTelemetryForwarder(() => restrictedTelemetryEnabled, telemetryService);
		...
		assert.deepStrictEqual(telemetryService.events, [{
			eventName: 'singularityCli/restricted_event',
			data: {
				...
				singularity_tracking_id: undefined,
				kind: 'restricted_event',
				restricted: true,
			},
		}]);
