...
/**
 * Agent Host integration tests using the real Singularity provider and a synthetic local LLM.
 */
...
suite('Agent Host Provider Integration — Singularity with Mock LLM', function () {
	...
	test('returns a hello response via mock LLM', async function () {
		...
		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, 'real-sdk-mock-hello', createdSessions, URI.file(workspaceDir));
...
/**
 * Idle-session release exercised against the real Singularity SDK and a mock LLM.
 ...
 */
suite('Agent Host Provider Integration — Singularity Idle Release', function () {
	...
	test('releases an idle session and resumes it losslessly on re-subscribe (mock LLM)', async function () {
		...
		tempDirs.push(workspaceDir);
		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, 'real-sdk-mock-release', createdSessions, URI.file(workspaceDir));
