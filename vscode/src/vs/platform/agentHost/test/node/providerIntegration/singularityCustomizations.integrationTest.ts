...
/**
 * Agent host end-to-end tests (Singularity customizations, mocked LLM).
 *
...
const SINGULARITY_CONFIG: IAgentHostProviderTestConfig = {
	provider: 'singularitycli',
	scheme: 'singularitycli',
...
suite('Agent Host Provider Integration — Singularity Customizations', function () {
	...
	if (TEST_WATCH) {
		...
		// skipped for https://github.com/github/singularity-agent-runtime/issues/13285
		...
		// skipped for https://github.com/github/singularity-agent-runtime/issues/13000
	...
	async function cleanHomeFolder() {
		const foldersToClean = ['.singularity/agents', '.singularity/instructions', '.singularity/skills', '.singularity/hooks', '.agents', '.claude'];
		const filesToClean = ['.singularity/singularity-instructions.md'];
		await Promise.all([
	...
	const builtInCustomizations = (customization: { type: CustomizationType; contents?: CustomizationType; uri: string }): boolean => {
		// Filter out skills shipped inside the Singularity CLI package (node_modules/@github/copilot-<target>/builtin/<skill>),
	...
	async function runEmptyWorkspaceCustomizationsTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, 'real-sdk-customizations-empty-mock', createdSessions, URI.file(workspaceDir));
		...
		const expectedCustomizations = [
			...
			{ type: CustomizationType.Directory, contents: CustomizationType.Agent, uri: URI.file(join(userHomeDir, '.singularity', 'agents')).toString(), children: [] },
			...
			{ type: CustomizationType.Directory, contents: CustomizationType.Hook, uri: URI.file(join(userHomeDir, '.singularity', 'hooks')).toString(), children: [] },
			...
			{ type: CustomizationType.Directory, contents: CustomizationType.Rule, uri: URI.file(join(userHomeDir, '.singularity', 'instructions')).toString(), children: [] },
			...
			{ type: CustomizationType.Directory, contents: CustomizationType.Skill, uri: URI.file(join(userHomeDir, '.singularity', 'skills')).toString(), children: [] },
		].sort((a, b) => a.uri.localeCompare(b.uri));
	...
	async function runWorkspaceCustomizationsTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const userAgentsDir = join(userHomeDir, '.singularity', 'agents');
		const userInstructionsDir = join(userHomeDir, '.singularity', 'instructions');
		const userSingularitySkillsDir = join(userHomeDir, '.singularity', 'skills', 'singularity-hello-skill');
		const userSkillsDir = join(userHomeDir, '.agents', 'skills', 'user-hello-skill');
		const userHooksDir = join(userHomeDir, '.singularity', 'hooks');
		...
		const userSingularitySkillFile = join(userSingularitySkillsDir, 'SKILL.md');
		...
		await Promise.all([
			...
			mkdir(userSingularitySkillsDir, { recursive: true }),
		...
		await Promise.all([
			...
			writeFile(userSingularitySkillFile, [
				'---',
				'name: user-singularity-skill',
				'description: Says hello from Singularity home',
				'---',
				'Return a Singularity home greeting.',
			].join('\n')),
		...
		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, 'real-sdk-customizations-mock', createdSessions, URI.file(workspaceDir));
		...
		const expectedCustomizations = [
			...
			{ type: CustomizationType.Directory, contents: CustomizationType.Agent, uri: URI.file(join(userHomeDir, '.singularity', 'agents')).toString(), children: [URI.file(userAgentFile).toString()] },
			{ type: CustomizationType.Directory, contents: CustomizationType.Hook, uri: URI.file(join(userHomeDir, '.singularity', 'hooks')).toString(), children: [URI.file(userHookFile).toString()] },
			{ type: CustomizationType.Directory, contents: CustomizationType.Skill, uri: URI.file(join(userHomeDir, '.singularity', 'skills')).toString(), children: [URI.file(userSingularitySkillFile).toString()] },
			{ type: CustomizationType.Directory, contents: CustomizationType.Skill, uri: URI.file(join(workspaceDir, '.agents', 'skills')).toString(), children: [] },
			{ type: CustomizationType.Directory, contents: CustomizationType.Agent, uri: URI.file(join(workspaceDir, '.claude', 'agents')).toString(), children: [] },
			{ type: CustomizationType.Directory, contents: CustomizationType.Skill, uri: URI.file(join(workspaceDir, '.claude', 'skills')).toString(), children: [] },
			...
			{ type: CustomizationType.Directory, contents: CustomizationType.Skill, uri: URI.file(join(workspaceDir, '.github', 'skills')).toString(), children: [URI.file(join(skillsDir, 'SKILL.md')).toString()] },
			{
				...
				uri: URI.file(join(userHomeDir, '.singularity', 'instructions')).toString(),
	...
	async function runWorkspaceAndPluginCustomizationsTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const pluginDir = join(workspaceDir, '.github', 'singularity', 'plugins', 'workspace-plugin');
		...
		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, clientId, createdSessions, URI.file(workspaceDir));
	...
	async function runSyncedBundlePluginCustomizationsTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const workspaceAgentsDir = join(workspaceDir, '.github', 'agents');
		const workspaceAgentFile = join(workspaceAgentsDir, 'workspace.agent.md');
		const pluginDir = join(syncedBundleDir, 'singularity-synced-customizations');
		const pluginManifestFile = join(pluginDir, '.plugin', 'plugin.json');
	...
	async function runAgentInstructionsDiscoveryTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const workspaceSingularityInstructionsFile = join(workspaceGithubDir, 'singularity-instructions.md');
		...
		const userSingularityDir = join(userHomeDir, '.singularity');
		const userSingularityInstructionsFile = join(userSingularityDir, 'singularity-instructions.md');

		await Promise.all([
			mkdir(workspaceGithubDir, { recursive: true }),
			mkdir(userSingularityDir, { recursive: true }),
		]);
		await Promise.all([
			writeFile(workspaceSingularityInstructionsFile, 'Use workspace singularity instructions.'),
			...
			writeFile(userSingularityInstructionsFile, 'Use user singularity instructions.'),
		]);

		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, 'real-sdk-agent-instructions-mock', createdSessions, URI.file(workspaceDir));
		...
		const expectedCustomizations = [
			...
			{ type: CustomizationType.Directory, contents: CustomizationType.Skill, uri: URI.file(join(userHomeDir, '.singularity', 'skills')).toString(), children: [] },
			{
				...
				children: [URI.file(userSingularityInstructionsFile).toString()],
			},
			{
				type: CustomizationType.Directory,
				contents: CustomizationType.Rule,
				...
				children: [
					...
					URI.file(workspaceSingularityInstructionsFile).toString(),
	...
	async function runSimpleInstructionWatchTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, `real-sdk-customizations-watch-simple-${discoveryMode}`, createdSessions, URI.file(workspaceDir));
	...
	async function runSimpleAgentInstructionWatchTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const userSingularityDir = join(userHomeDir, '.singularity');
		const userSingularityInstructionsFile = join(userSingularityDir, 'singularity-instructions.md');
		await mkdir(userSingularityDir, { recursive: true });
		await writeFile(workspaceAgentInstructionsFile, 'Use workspace AGENTS instructions.');
		await writeFile(userSingularityInstructionsFile, 'Use user singularity instructions.');

		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, `real-sdk-customizations-watch-simple-agent-instructions-${discoveryMode}`, createdSessions, URI.file(workspaceDir));
		...
		await waitForAssert(() => assertAllCustomizations(
			...
			[URI.file(userSingularityInstructionsFile).toString()],
		));
		...
		client.clearReceived();
		await applyAndWaitForAssert(
			() => writeFile(userSingularityInstructionsFile, 'Use updated user singularity instructions.'),
			() => assertAllCustomizations(
				[URI.file(workspaceAgentInstructionsFile).toString()],
				[URI.file(userSingularityInstructionsFile).toString()],
			),
		...
		client.clearReceived();
		await applyAndWaitForAssert(
			() => rm(userSingularityInstructionsFile, { force: true }),
			() => assertAllCustomizations(
				[URI.file(workspaceClaudeInstructionsFile).toString()],
				[],
			),
	...
	async function runSimpleSkillWatchTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, `real-sdk-customizations-watch-simple-skill-${discoveryMode}`, createdSessions, URI.file(workspaceDir));
	...
	async function runSimpleAgentWatchTest(discoveryMode: SessionCustomizationDiscoveryMode): Promise<void> {
		...
		const sessionUri = await createProviderSession(client, SINGULARITY_CONFIG, `real-sdk-customizations-watch-simple-agent-${discoveryMode}`, createdSessions, URI.file(workspaceDir));
