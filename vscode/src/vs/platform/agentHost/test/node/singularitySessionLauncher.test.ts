...
import type { CopilotClient as SingularityClient, CopilotSession as SingularitySession, Verbosity } from '@github/copilot-sdk';
...
import { ByokLmProxyService, IByokLmProxyService, type IByokLmProxyHandle } from '../../node/singularity/byokLmProxyService.js';
import { SingularitySessionLauncher, getSingularityReasoningEffort, isSingularityReasoningEffort, resolveByokSessionConfig, type SingularitySessionLaunchPlan, type ISingularitySessionRuntime } from '../../node/singularity/singularitySessionLauncher.js';
import type { ISingularityPluginInfo } from '../../node/singularity/singularityAgent.js';

const testRuntime: ISingularitySessionRuntime = {
 ...
 * {@link SingularitySessionLauncher.disposeByokProxyHandle} (called by the agent
 * after the runtime subprocess stops) releases it so the next launch mints a
 * fresh nonce.
 */
suite('SingularitySessionLauncher BYOK proxy lifecycle', () => {
	...
	function createLauncher(store: DisposableStore, proxy: IByokLmProxyService, registry: IByokLmBridgeRegistry): SingularitySessionLauncher {
		...
		return instantiationService.createInstance(SingularitySessionLauncher);
	}
...
suite('SingularitySessionLauncher shared session config', () => {

	ensureNoDisposablesAreLeakedInTestSuite();

	test('passes Agent Host defaults and exit-plan handler to create and resume', async () => {
		const createConfigs: Parameters<SingularityClient['createSession']>[0][] = [];
		const resumeConfigs: Parameters<SingularityClient['resumeSession']>[1][] = [];
		const session = {
			sessionId: 'session-1',
			on: () => () => { },
			disconnect: async () => { },
		} as unknown as SingularitySession;
		const client = {
			createSession: async (config: Parameters<SingularityClient['createSession']>[0]) => {
				createConfigs.push(config);
				return session;
			},
			resumeSession: async (_sessionId: string, config: Parameters<SingularityClient['resumeSession']>[1]) => {
		...
		const plugin: ISingularityPluginInfo = {
		...
		const createPlan: SingularitySessionLaunchPlan = {
			...basePlan,
			kind: 'create',
			model: undefined,
		};
		const resumePlan: SingularitySessionLaunchPlan = {
			...basePlan,
			kind: 'resume',
...
suite('SingularitySessionLauncher resume fallback', () => {
	...
	function createResumeFailingLaunch(message: string, code = -32603): { readonly launcher: SingularitySessionLauncher; readonly plan: SingularitySessionLaunchPlan; readonly getCreateSessionCalls: () => number } {
	...
	test('does not replace a session with an empty one after a transient network failure', async () => {
		...
		const { launcher, plan, getCreateSessionCalls } = createResumeFailingLaunch('Request session.resume failed with message: network fetch failed: request failed: error sending request for url (https://api.github.com/singularity_internal/user)');
	...
	test('does not replace a corrupted session file with an empty session', async () => {
		const { launcher, plan, getCreateSessionCalls } = createResumeFailingLaunch('Request session.resume failed with message: Session file is corrupted (line 19567: data.compactionTokensUsed.singularityUsage.tokenDetails.0.batchSize: Number must be greater than 0)');
...
suite('SingularitySessionLauncher verbosity', () => {

	ensureNoDisposablesAreLeakedInTestSuite();

	function applyVerbosity(verbosity: Verbosity): Promise<void> {
		const launcher = createTestLauncher() as unknown as {
			_applyVerbosity(session: SingularitySession, verbosity: Verbosity, sessionId: string): Promise<void>;
		};
		const session = {
			rpc: {
				options: {
 ...
 * `SingularityAgent._changeModel`: the host-level override (see
 * `SingularityCliConfigKey.ReasoningEffortOverride`) wins over the model picker's
 * thinking level when valid, and degrades to the picker value otherwise.
 */
suite('getSingularityReasoningEffort', () => {
	...
	test('a valid override wins over the picker value; an invalid or absent override falls back', () => {
		...
		assert.deepStrictEqual(
			[
				getSingularityReasoningEffort(model),
				getSingularityReasoningEffort(model, 'xhigh'),
				getSingularityReasoningEffort(model, 'turbo'),
				getSingularityReasoningEffort(undefined, 'high'),
				getSingularityReasoningEffort(undefined),
			],
			['medium', 'xhigh', 'medium', 'high', undefined]
		);
	});

	// The model picker's options are `supportedReasoningEfforts.filter(isSingularityReasoningEffort)`,
	...
	test('recognizes every canonical reasoning-effort tier so none is dropped from the picker', () => {
		assert.deepStrictEqual({
			accepted: reasoningEffortLevels.filter(isSingularityReasoningEffort),
			rejectsUnknown: isSingularityReasoningEffort('turbo'),
		}, {
