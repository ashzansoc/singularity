...
import { isRestrictedTelemetryEnabled, parseSingularityTokenFields } from '../../node/singularity/singularityTokenFields.js';

suite('singularityTokenFields', () => {

	ensureNoDisposablesAreLeakedInTestSuite();

	suite('parseSingularityTokenFields', () => {
		test('returns empty map for undefined token', () => {
			assert.strictEqual(parseSingularityTokenFields(undefined).size, 0);
		});

		test('returns empty map for empty token', () => {
			assert.strictEqual(parseSingularityTokenFields('').size, 0);
		});

		test('parses fields from the leading colon-delimited segment', () => {
			const fields = parseSingularityTokenFields('tid=abc;exp=123;rt=1:HMACSIGNATURE');
		...
		test('parses fields when no colon separator is present', () => {
			const fields = parseSingularityTokenFields('tid=abc;rt=1');
			assert.strictEqual(fields.get('tid'), 'abc');
			assert.strictEqual(fields.get('rt'), '1');
		});

		test('skips segments without a value separator', () => {
			const fields = parseSingularityTokenFields('tid=abc;rt;exp=123:HMAC');
