...
import { appendSdkToolResultContent, mapSessionEvents } from '../../node/singularity/mapSessionEvents.js';
import { toSessionEvents, type ISessionEvent } from './singularityTestEvents.js';

suite('mapSessionEvents — history replay', () => {

	ensureNoDisposablesAreLeakedInTestSuite();

	const session = AgentSession.uri('singularity', 'test-session');
	...
	test('restores MCP app data for completed tool calls', async () => {
		...
		assert.deepStrictEqual({
			...
			contributor: {
				...
				customizationId: 'mcp-top-level:singularity:test-session:GitHub',
			},
			meta: {
				mcpServerName: 'GitHub',
				mcpToolName: 'get_me',
				ui: {
					resourceUri: 'ui://github-mcp-server/get-me',
					channel: 'mcp://singularity/test-session/GitHub',
				},
			},
		});
	});
...
suite('appendSdkToolResultContent', () => {
	...
	test('folds shell_exit into an existing terminal block instead of adding a second one', () => {
		...
		], { session: AgentSession.uri('singularity', 'test-session'), toolCallId: 'tc-1', title: 'Run Shell Command' });
