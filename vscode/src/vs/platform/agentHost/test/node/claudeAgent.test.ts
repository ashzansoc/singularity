...
import { IAgentChatDataChange, IAgentMaterializeSessionEvent, IAgentSpawnChatEvent, AgentSession, AgentSignal, GITHUB_SINGULARITY_PROTECTED_RESOURCE } from '../../common/agentService.js';
...
import { ISingularityApiService, type ISingularityApiServiceRequestOptions } from '../../node/shared/singularityApiService.js';
...
const SYNTHETIC_AUTO = makeModel({ id: 'auto', name: 'Auto', vendor: 'singularity' });
...
interface ITestContext {
	...
	readonly api: FakeSingularityApiService;
...
function createTestContext(
	...
	const api = new FakeSingularityApiService();
	...
	// Phase 19: seed root config (e.g. `claudeUseSingularityProxy`) BEFORE the agent
...
suite('ClaudeAgent', () => {
	...
	test('getProtectedResources returns the GitHub resource', () => {
		...
		assert.deepStrictEqual(agent.getProtectedResources(), [{
			...
			resource_name: 'GitHub Singularity',
	...
	test('enterprise: getProtectedResources + authenticate use the computed enterprise resource', async () => {
		...
		assert.deepStrictEqual({
			...
			acceptsEnterpriseSingularity: await agent.authenticate('https://ghe.acme.com/api/v3', 'tok'),
			...
			acceptsEnterpriseSingularity: true,
			rejectsDotCom: false,
		});
	});
	...
	test('native transport: getProtectedResources keeps the Singularity resource but marks it not required', () => {
		// Native keeps advertising the Singularity resource with `required: false`
		...
		const { agent } = createTestContext(disposables, { rootConfig: { claudeUseSingularityProxy: false } });
		assert.deepStrictEqual(
	...
	test('signed-in probe flips inferred-native to proxy (allowSignedOutWhenUsable)', async () => {
		...
		// advertises the Singularity resource as not-required so the host can probe. If
		...
		try {
			...
			// Signed out at startup ⇒ native, Singularity advertised as not-required.
			const before = {
	...
	test('native transport: models populate from supportedModels() with no proxy start and no CAPI models() call', async () => {
		...
		await fs.writeFile(join(userHome.fsPath, '.claude', 'settings.json'), JSON.stringify({ env: { ANTHROPIC_API_KEY: 'sk-ant-test-key' } }), 'utf8');
		try {
			const { agent, proxy, api, sdk } = createTestContext(disposables, { rootConfig: { claudeUseSingularityProxy: false }, userHome });
	...
	test('native without a credential publishes an empty catalog instead of the SDK static list', async () => {
		...
		const { agent, sdk } = createTestContext(disposables, { rootConfig: { claudeUseSingularityProxy: false } });
		sdk.supportedModelsResult = [
	...
	test('native model enumeration closes the throwaway query (no leaked subprocess)', async () => {
		...
		await fs.writeFile(join(userHome.fsPath, '.claude', 'settings.json'), JSON.stringify({ env: { ANTHROPIC_API_KEY: 'sk-ant-test-key' } }), 'utf8');
		try {
			const { sdk } = createTestContext(disposables, { rootConfig: { claudeUseSingularityProxy: false }, userHome });
			sdk.supportedModelsResult = [
				{ value: 'claude-sonnet-4-5-20250929', displayName: 'Claude Sonnet 4.5', description: '' },
			];
	...
	test('native transport: authenticate never starts the proxy', async () => {
		const { agent, proxy } = createTestContext(disposables, { rootConfig: { claudeUseSingularityProxy: false } });
	...
	test('unusable native (explicit proxy off, no setup) does not demand GitHub sign-in', async () => {
		// The "unusable" case: explicit `claudeUseSingularityProxy=false` is a hard
		// override to native even with no usable credentials. It must degrade to
		// "no models" (NoModels), NOT a GitHub sign-in prompt — so the Singularity
		...
		assert.deepStrictEqual({
			singularityRequired: agent.getProtectedResources().find(r => r.resource === 'https://api.github.com')?.required,
			createdWithoutAuthPrompt: created.provisional === true,
		}, {
			singularityRequired: false,
			createdWithoutAuthPrompt: true,
		});
	});

	test('transport flip native→proxy with no proxy handle emits auth/required once', () => {
		const { agent, configService } = createTestContext(disposables, { rootConfig: { claudeUseSingularityProxy: false } });
		const events: Omit<AuthRequiredParams, 'channel'>[] = [];
		disposables.add(agent.onDidRequireAuth(e => events.push(e)));

		configService.updateRootConfig({ claudeUseSingularityProxy: true });
	...
	test('transport flip does not emit auth/required when a proxy handle already exists', async () => {
		...
		const events: Omit<AuthRequiredParams, 'channel'>[] = [];
		disposables.add(agent.onDidRequireAuth(e => events.push(e)));
		configService.updateRootConfig({ claudeUseSingularityProxy: false }); // → native
		configService.updateRootConfig({ claudeUseSingularityProxy: true });  // → proxy; handle persists
	...
	test('transport flip proxy→native does not emit auth/required', () => {
		...
		configService.updateRootConfig({ claudeUseSingularityProxy: false });
	...
	test('proxy-mode authenticate with an unchanged token starts the proxy when no handle exists', async () => {
		// Native mode records the Singularity token without starting the proxy. After
		...
		const { agent, proxy, configService } = createTestContext(disposables, { rootConfig: { claudeUseSingularityProxy: false } });
	...
	test('createSession before authenticate throws ProtocolError(AHP_AUTH_REQUIRED) with protected resources', async () => {
		...
		await assert.rejects(
			() => agent.createSession({ workingDirectories: [URI.file('/workspace')] }),
			(err: Error) =>
	...
	test('authenticate sources configSchema enum from each model\'s reasoning_effort list (Phase 6.1 / Cycle D3 / I5)', async () => {
		...
		// extensions/singularity-chat/src/extension/chatSessions/claude/node/
	...
	test('phase-stub graduation: abortSession + changeModel no longer throw', async () => {
		...
		// calls; matching SingularityAgent's permissive surface keeps the
	...
	test('createSession (non-fork) returns a claude:/<uuid> URI with provisional: true; no DB or SDK contact', async () => {
		...
		await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'tok');

		const result = await agent.createSession({ workingDirectories: [URI.parse('file:///workspace')] });
	...
	test('createSession without a workingDirectory materializes in a shared scratch dir (workspace-less quick chat)', async () => {
		...
		// fallback is now shared with the Singularity agent.
		...
		try {
			...
			const expected = URI.joinPath(userHome, '.singularity', 'chats', sessionId);
	...
	test('resume keeps the existing overlay model (materialize does not clobber on isResume)', async () => {
		...
		const created = await agent.createSession({ workingDirectories: [URI.file('/work-resume')], model: initialModel });
		const sessionId = AgentSession.id(created.session);
		sdk.nextQueryMessages = [makeSystemInitMessage(sessionId), makeResultSuccess(sessionId)];
	...
	test('createSession honors config.session when the workbench pre-mints the URI', async () => {
		...
		// the hint. Mirrors SingularityAgent's `config.session ?
		...
		const result = await agent.createSession({ session: expected, workingDirectories: [URI.file('/work')] });
	...
	test('cold resume recovers the additional directories from the persisted overlay', async () => {
		...
		await ctxA.agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'tok');
		...
		await ctxB.agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'tok');
	...
	test('materialize resolves the SDK agent name from the file frontmatter, not the filename', async () => {
		...
		const created = await agent.createSession({ workingDirectories: [URI.file('/work')], agent: { uri: agentFile.toString() } });
		const sessionId = AgentSession.id(created.session);
		sdk.nextQueryMessages = [makeSystemInitMessage(sessionId), makeResultSuccess(sessionId)];
	...
	test('materialize resolves a built-in (claude-internal) agent selection to its name', async () => {
		...
		const created = await agent.createSession({ workingDirectories: [URI.file('/work')], agent: { uri: 'claude-internal:/agent/Explore' } });
		const sessionId = AgentSession.id(created.session);
		sdk.nextQueryMessages = [makeSystemInitMessage(sessionId), makeResultSuccess(sessionId)];
	...
	test('materialize event payload shape — { session, workingDirectory, project: undefined }', async () => {
		...
		const created = await agent.createSession({ workingDirectories: [cwd] });
		const sessionId = AgentSession.id(created.session);
		sdk.nextQueryMessages = [makeSystemInitMessage(sessionId), makeResultSuccess(sessionId)];
	...
	test('createSession model.config.thinkingLevel flows into Options.effort on first send (M11 / Phase 6.1 C2)', async () => {
		...
		// SingularityAgent's `_getReasoningEffort` pattern at
		// singularityAgent.ts:487. The SDK's `Options.effort` accepts the
	...
	test('proxy credit reports are summed and attached to the turn ChatUsage as singularityUsage', async () => {
		// CAPI bills real Singularity credits per `/v1/messages` request via
		// `singularity_usage.total_nano_aiu`, surfaced by the proxy's
		// `onDidReportCredits` (the SDK strips it from its `result`). The
		// session accumulates every report for the turn and attaches the
		// sum to the turn's ChatUsage as `_meta.singularityUsage.totalNanoAiu`.
		...
		assert.deepStrictEqual(usage.usage._meta?.singularityUsage, { totalNanoAiu: 2_000_000_000 });
	});
	...
	test('sendMessage on a disk-only session (created in another window) resumes from disk', async () => {
		...
		// `_resumeSession`, which mirrors SingularityAgent._resumeSession:
	...
	test('resumed session keeps overlay-derived permissionMode on turn 2 (no silent flip to default)', async () => {
		// Regression for Singularity review feedback on the cross-window
	...
	test('shutdown drains a mix of provisional and materialized sessions', async () => {
		...
		const matCreated = await agent.createSession({ workingDirectories: [URI.file('/work-mat')] });
		sdk.nextQueryMessages = [
		...
		const provCreated = await agent.createSession({ workingDirectories: [URI.file('/work-prov')] });
		const provAborter = (() => {
	...
	test('shutdown clears provisional sessions; concurrent disposeSession is safe', async () => {
		...
		const { agent } = createTestContext(disposables);
		await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'tok');
		const r1 = await agent.createSession({ workingDirectories: [URI.file('/work')] });
		await agent.createSession({ workingDirectories: [URI.file('/work')] });
	...
	test('listSessions returns SDK entries decorated with the per-session DB overlay', async () => {
		...
		const services = new ServiceCollection(
			[ILogService, new NullLogService()],
			...createTestAgentStateServices(disposables),
			[ISingularityApiService, new FakeSingularityApiService()],
	...
	test('listSessions tolerates a corrupt DB without poisoning the rest of the listing', async () => {
		...
		// drop the other entries from the listing. SingularityAgent's
		// `Promise.all`-with-throwing-mapper pattern at singularityAgent.ts:519
	...
	test('listSessions returns an empty list (does not reject) when the SDK fails to load', async () => {
		// Singularity-reviewer comment: `AgentService.listSessions` fans out
		...
		// session list disappears too \u2014 the sibling Singularity provider
		// goes blank. Catching here keeps Claude's row empty while
		// Singularity's row still surfaces.
		const sdk = new FakeClaudeAgentSdkService();
	...
	test('shutdown is idempotent and returns the same memoized promise on concurrent calls', async () => {
		...
		// Mirror of `SingularityAgent.shutdown()` at singularityAgent.ts:1246.
		const { agent } = createTestContext(disposables);
		await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'tok');
		await agent.createSession({ workingDirectories: [URI.file('/work')] });
		await agent.createSession({ workingDirectories: [URI.file('/work')] });
	...
	test('agent.dispose() during a racing first sendMessage aborts the provisional and disposes the WarmQuery', async () => {
		// Singularity reviewer: `dispose()` did not abort provisional
...
suite('ClaudeAgent (Phase 7 §3.4 — _handleCanUseTool)', () => {
	...
	async function materialize(seedConfig?: { permissionMode?: string }): Promise<{
		...
		await ctx.agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'tok');
...
suite('ClaudeAgent (Phase 9 — runtime mutation surface)', () => {
	...
	async function materialize(opts?: { extraMessages?: SDKMessage[]; logService?: ILogService }): Promise<{
		...
		await tick();
		const created = await ctx.agent.createSession({ workingDirectories: [URI.file('/workspace')], model: { id: 'claude-opus-4.6' } });
		const sessionId = AgentSession.id(created.session);
		const advance = new DeferredPromise<void>();
	...
	test('steering_consumed fires when the iterable hands the steering message to the SDK', async () => {
		...
		// fires `steering_consumed` (SDK ack semantics — mirrors Singularity's
...
suite('ClaudeAgent — Phase 11 customizations', () => {
	...
	test('createSession seeds the eager activeClient customizations to the plugin manager', async () => {
		...
		// creation (mirrors the Singularity agent). Without this, built-in skills
	...
	test('reducer-backed customization enablement stays isolated per session', async () => {
		...
		const s1 = await agent.createSession({ session: AgentSession.uri('claude', 'a'), workingDirectories: [URI.file('/work')] });
		const s2 = await agent.createSession({ session: AgentSession.uri('claude', 'b'), workingDirectories: [URI.file('/work')] });
	...
	test('getCustomizations returns [] — provider-level catalogue, not a cross-session aggregator', async () => {
		...
		const s1 = await agent.createSession({ session: AgentSession.uri('claude', 'one'), workingDirectories: [URI.file('/work')] });
		const s2 = await agent.createSession({ session: AgentSession.uri('claude', 'two'), workingDirectories: [URI.file('/work')] });
	...
	test('restart round-trip: a forked peer chat re-materializes from the orchestrator\'s providerData on a fresh agent backed by the same database', async () => {
		...
		await ctxA.agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'tok');
		const created = await ctxA.agent.createSession({ workingDirectories: [URI.file('/work')] });
		const parentId = AgentSession.id(created.session);
...
suite('ClaudeAgent — host OTel session-title spans', () => {
	...
	test('ignores session-title changes belonging to another provider', () => {
		const { stateManager, otelService } = createTestContext(disposables);
		const foreignUri = AgentSession.uri('singularity', 'foreign-title');
		stateManager.createSession({
