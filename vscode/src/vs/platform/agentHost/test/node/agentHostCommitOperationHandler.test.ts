...
import { SingularityApiError, type ISingularityApiService, type ISingularityApiServiceRequestOptions, type ISingularityUtilityChatCompletionRequest } from '../../node/shared/singularityApiService.js';
import { GITHUB_SINGULARITY_PROTECTED_RESOURCE, IAgentService } from '../../common/agentService.js';
...
class TestSingularityApiService implements ISingularityApiService {
	...
	readonly calls: { token: string; request: ISingularityUtilityChatCompletionRequest; options?: ISingularityApiServiceRequestOptions }[] = [];
	response = '```text\nUpdate session changes\n```';
	error: Error | undefined;

	messages(_githubToken: string, request: Anthropic.MessageCreateParamsStreaming, _options?: ISingularityApiServiceRequestOptions): AsyncGenerator<Anthropic.MessageStreamEvent>;
	messages(_githubToken: string, request: Anthropic.MessageCreateParamsNonStreaming, _options?: ISingularityApiServiceRequestOptions): Promise<Anthropic.Message>;
...
function setup(disposables: Pick<DisposableStore, 'add'>, gitService: TestGitService, singularityApiService: TestSingularityApiService, changesets: TestChangesetService, options?: { readonly onCommittedError?: Error }): { handler: AgentHostCommitOperationHandler; session: URI; committedSessions: string[] } {
	...
	return {
		...
		}, createAgentService('gh-repo-token'), createTestGitHubEndpointService(), gitService, singularityApiService, new NullLogService()),
		session,
		committedSessions,
	};
}
...
suite('AgentHostCommitOperationHandler', () => {
	...
	test.skip('generates a commit message, commits all changes, and refreshes changesets', async () => {
		...
		const { handler, session, committedSessions } = setup(disposables, gitService, singularityApiService, changesets);
		...
		assert.deepStrictEqual({
			...
			completion: singularityApiService.calls.map(call => ({ token: call.token, fileIncluded: call.request.messages.some(message => message.content.includes('file.ts')) })),
	...
	test('returns no-op success without generating a message or committing when the working tree is clean', async () => {
		...
		const singularityApiService = new TestSingularityApiService();
		const changesets = new TestChangesetService();
		const { handler, session } = setup(disposables, gitService, singularityApiService, changesets);

		const result = await handler.invoke({ channel: buildUncommittedChangesetUri(session.toString()), operationId: AgentHostCommitOperationHandler.OPERATION_COMMIT }, CancellationToken.None);

		assert.deepStrictEqual({ message: result.message, gitCalls: gitService.calls, completionCalls: singularityApiService.calls.length, changesetCalls: changesets.calls }, {
	...
	test.skip('returns success when post-commit refresh fails', async () => {
		...
		const singularityApiService = new TestSingularityApiService();
		const changesets = new TestChangesetService();
		const { handler, session, committedSessions } = setup(disposables, gitService, singularityApiService, changesets, { onCommittedError: new Error('refresh failed') });
	...
	test('honors cancellation before mutating the repository', async () => {
		...
		assert.deepStrictEqual({ gitCalls: gitService.calls, completionCalls: singularityApiService.calls.length, changesetCalls: changesets.calls }, {
			gitCalls: [],
			completionCalls: 0,
			changesetCalls: [],
		});
	...
	test('maps stale Singularity auth failures to AHP_AUTH_REQUIRED before committing', async () => {
		const gitService = new TestGitService();
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.error = new SingularityApiError(401, {
			type: 'error',
		...
		assert.deepStrictEqual({
			...
			data: err?.data,
			gitCalls: gitService.calls,
			completionCalls: singularityApiService.calls.length,
			changesetCalls: changesets.calls,
			committedSessions,
		}, {
			code: AHP_AUTH_REQUIRED,
			data: [GITHUB_SINGULARITY_PROTECTED_RESOURCE],
	...
	test('maps Singularity token mint auth failures to AHP_AUTH_REQUIRED before committing', async () => {
		const gitService = new TestGitService();
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.error = new Error('Singularity session token mint failed: 403 Forbidden');
		const changesets = new TestChangesetService();
		const { handler, session, committedSessions } = setup(disposables, gitService, singularityApiService, changesets);
