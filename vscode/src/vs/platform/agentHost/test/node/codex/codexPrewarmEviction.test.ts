...
import { ISingularityApiService } from '../../../node/shared/singularityApiService.js';
...
const SINGULARITY_TEST_MODEL = toCodexModelSelectionId('vscode-proxy', 'gpt-test');
...
async function createAgent(disposables: Pick<DisposableStore, 'add'>, options: ICreateAgentOptions = {}): Promise<CodexAgent> {
	...
	instantiationService.stub(ISingularityApiService, { _serviceBrand: undefined, models: async () => models });
...
async function assertPrewarmEvictedOnSend(disposables: Pick<DisposableStore, 'add'>, completePrewarmBeforeSend: boolean): Promise<void> {
	...
	const { session } = await agent.createSession({ workingDirectories: [folder], model: { id: SINGULARITY_TEST_MODEL } });
...
suite('CodexAgent prewarm eviction', () => {
	...
	test('routes provider-qualified models independently and switches one session', async () => {
		...
		agent['_models'].set([
			{ provider: 'singularity', id: SINGULARITY_TEST_MODEL, name: 'GPT Test', supportsVision: false },
			{ provider: 'codex', id: chatGPTModel, name: 'GPT Test', supportsVision: false },
		], undefined);

		const singularity = await agent.createSession({ workingDirectories: [URI.file('/repo/singularity')], model: { id: SINGULARITY_TEST_MODEL } });
		const chatGPT = await agent.createSession({ workingDirectories: [URI.file('/repo/chatgpt')], model: { id: chatGPTModel } });
		const singularityEntry = agent['_sessions'].get(AgentSession.id(singularity.session))!;
		const chatGPTEntry = agent['_sessions'].get(AgentSession.id(chatGPT.session))!;

		const materializeSingularity = agent['_materializeIfNeeded'](singularityEntry, false);
		const singularityStart = await readNextRequest(peer.outbound);
		peer.push({ id: singularityStart.id, result: { thread: { id: 'thread-singularity' } } });
		await materializeSingularity;
		...
		await agent.chats.changeModel(URI.parse(buildDefaultChatUri(singularity.session)), { id: chatGPTModel });
		const persistedAfterSwitch = await agent['_metadataStore'].read(singularity.session);
		const rematerializeSingularity = agent['_materializeIfNeeded'](singularityEntry, false);
		const switchedStart = await readNextRequest(peer.outbound);
		peer.push({ id: switchedStart.id, result: { thread: { id: 'thread-singularity-switched' } } });
		await rematerializeSingularity;

		assert.deepStrictEqual({
			singularityStart: { model: singularityStart.params.model, provider: singularityStart.params.modelProvider },
	...
	test('/compact invokes thread/compact/start instead of starting a prompt turn', async () => {
		...
		const { session } = await agent.createSession({ workingDirectories: [repo], model: { id: SINGULARITY_TEST_MODEL } });
	...
	test('thread start receives custom agents, instructions, skills, and MCP from client plugins', async () => {
		...
		const { session } = await agent.createSession({ session: unsafeSession, workingDirectories: [repo], model: { id: SINGULARITY_TEST_MODEL }, agent: { uri: agentUri.toString() } });
		const entry = agent['_sessions'].get(AgentSession.id(session))!;
	...
	test('fresh multi-root start selects only existing secondary skill directories', async () => {
		...
		try {
			const { session } = await agent.createSession({ workingDirectories: [repoA, repoB, repoC], model: { id: SINGULARITY_TEST_MODEL } });
			const entry = agent['_sessions'].get(AgentSession.id(session))!;
			const start = await readNextRequest(peer.outbound);
	...
	test('unexpected capability-root metadata failures warn without blocking start or exposing paths', async () => {
		...
		try {
			const { session } = await agent.createSession({ workingDirectories: [repoA, repoB], model: { id: SINGULARITY_TEST_MODEL } });
			const entry = agent['_sessions'].get(AgentSession.id(session))!;
			const start = await readNextRequest(peer.outbound);
			peer.push({ id: start.id, result: { thread: { id: 'thread' } } });
			await entry.materializePromise;
	...
	test('multi-root start and turn separate workspace roots from additional writable directories', async () => {
		...
		try {
			...
			const { session } = await agent.createSession({ session: sessionUri, workingDirectories, model: { id: SINGULARITY_TEST_MODEL } });
			const entry = agent['_sessions'].get(AgentSession.id(session))!;
			const start = await readNextRequest(peer.outbound);
	...
	test('consecutive sends replace and remove workspace roots on the existing thread', async () => {
		...
		try {
			const created = await agent.createSession({ workingDirectories: [repoA, repoB], model: { id: SINGULARITY_TEST_MODEL } });
	...
	test('disabled multi-root preserves the existing additional-directory payload', async () => {
		...
		try {
			const { session } = await agent.createSession({ session: sessionUri, workingDirectories: [repoA, repoB], model: { id: SINGULARITY_TEST_MODEL } });
			const entry = agent['_sessions'].get(AgentSession.id(session))!;
			const start = await readNextRequest(peer.outbound);
			peer.push({ id: start.id, result: { thread: { id: 'thread' } } });
			await entry.materializePromise;
	...
	test('enabled multi-root preserves single-folder protocol and sandbox behavior', async () => {
		...
		try {
			const { session } = await agent.createSession({ session: sessionUri, workingDirectories: [repo], model: { id: SINGULARITY_TEST_MODEL } });
			const entry = agent['_sessions'].get(AgentSession.id(session))!;
			const start = await readNextRequest(peer.outbound);
			peer.push({ id: start.id, result: { thread: { id: 'thread' } } });
			await entry.materializePromise;
	...
	test('fork inherits the source workspace roots instead of requested replacements', async () => {
		...
		try {
			const source = await agent.createSession({ workingDirectories: [repoA, repoB], model: { id: SINGULARITY_TEST_MODEL } });
	...
	test('cold resume restores persisted workspace roots', async () => {
		...
		try {
			const created = await agentA.createSession({ workingDirectories: [repoA, repoB], model: { id: SINGULARITY_TEST_MODEL } });
