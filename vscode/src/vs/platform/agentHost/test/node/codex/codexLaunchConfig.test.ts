...
suite('CodexLaunchConfig', () => {
	...
	test('adds the Singularity proxy without selecting it globally', () => {
		...
		assert.deepStrictEqual(config.env, { PATH: '/bin', OPENAI_API_KEY: 'nonce', AI_AGENT: 'github_singularity_vscode_agent' });
		...
		assert.ok(config.args.includes('shell_environment_policy.set.AI_AGENT="github_singularity_vscode_agent"'));
	...
	test('resume explicitly binds each session provider', () => {
		...
		assert.deepStrictEqual(buildCodexResumeParams('vscode-proxy', 'thread-b', { GitHub: { url: 'https://api.githubsingularity.com/mcp/' } }), {
			threadId: 'thread-b',
			modelProvider: 'vscode-proxy',
			config: { mcp_servers: { GitHub: { url: 'https://api.githubsingularity.com/mcp/' } } },
		});
