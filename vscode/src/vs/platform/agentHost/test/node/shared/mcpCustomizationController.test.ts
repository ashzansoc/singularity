...
function harness(store: Pick<DisposableStore, 'add'>, opts: { customizations?: readonly Customization[]; desiredEnabled?: boolean } = {}) {
	...
	const sessionUri = AgentSession.uri('singularity', 'session-1');
	...
	if (opts.desiredEnabled !== undefined) {
		stateManager.dispatchServerAction(session, {
			...
			customizations: [{
				...
				id: 'mcp-top-level:singularity:session-1:search',
				uri: 'mcp-top-level:singularity:session-1:search',
				name: 'search',
...
suite('McpCustomizationController', () => {
		...
		assert.deepStrictEqual(actions, [
			{
				...
				channel: 'mcp://singularity/session-1/fs',
			},
			{
				type: ActionType.SessionMcpServerStateChanged,
				id: 'mcp-child:demo:fs',
	...
	test('bare server (no child match) is surfaced as a full top-level customization', () => {
		...
		const expectedId = 'mcp-top-level:singularity:session-1:search';
		assert.deepStrictEqual(actions, [
			{
				...
				customization: {
					...
					enabled: true,
					state: { kind: McpServerStatus.Ready },
					channel: 'mcp://singularity/session-1/search',
	...
	test('runtimeStates snapshots child and top-level servers by customization id', () => {
		...
		assert.deepStrictEqual(controller.runtimeStates.get(), new Map([
			['mcp-child:demo:fs', { state: { kind: McpServerStatus.Ready }, channel: 'mcp://singularity/session-1/fs' }],
			['mcp-top-level:singularity:session-1:search', { state: { kind: McpServerStatus.Starting }, channel: undefined }],
		]));
		assert.strictEqual(controller.serverNameForCustomizationId('mcp-child:demo:fs'), 'fs');
		assert.strictEqual(controller.serverNameForCustomizationId('mcp-top-level:singularity:session-1:search'), 'search');

		controller.remove('fs');
		assert.deepStrictEqual([...controller.runtimeStates.get().keys()], ['mcp-top-level:singularity:session-1:search']);
	});
	...
	test('parseMcpChannelUri round-trips the controller-minted channel URI', () => {
		const channel = 'mcp://singularity/session-1/fs';
	...
	test('parseMcpChannelUri decodes URL-encoded path segments', () => {
		const channel = 'mcp://singularity/session%2F1/my%20server';
		assert.deepStrictEqual(parseMcpChannelUri(channel), {
			providerId: 'singularity',
			sessionId: 'session/1',
	...
	test('parseMcpChannelUri rejects malformed inputs', () => {
		assert.strictEqual(parseMcpChannelUri('https://singularity/x/y'), undefined);
		assert.strictEqual(parseMcpChannelUri('mcp://'), undefined);
		assert.strictEqual(parseMcpChannelUri('mcp:///session/server'), undefined);
		assert.strictEqual(parseMcpChannelUri('mcp://singularity/session-only'), undefined);
		assert.strictEqual(parseMcpChannelUri('mcp://singularity/session/'), undefined);
		// Bad percent escapes must not throw — caller turns undefined
		// into a clean Method not found, not an internal error.
		assert.strictEqual(parseMcpChannelUri('mcp://singularity/bad%/server'), undefined);
		assert.strictEqual(parseMcpChannelUri('mcp://singularity/session/bad%2'), undefined);
	});
