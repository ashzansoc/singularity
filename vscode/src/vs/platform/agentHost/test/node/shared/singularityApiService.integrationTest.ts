...
/**
 * Integration tests for {@link SingularityApiService.utilityChatCompletion} that
 * hit live GitHub Singularity CAPI. Opt-in only: requires a real GitHub OAuth
 * token with Singularity entitlement in the `SINGULARITY_GITHUB_TOKEN` environment
 * variable. We deliberately do NOT read `GITHUB_TOKEN` — GitHub Actions
 * sets it for every workflow run but it lacks Singularity access, which would
...
import { SingularityApiService } from '../../../node/shared/singularityApiService.js';
import { createTestGitHubEndpointService } from '../testGitHubEndpointService.js';

suite('SingularityApiService.utilityChatCompletion (real CAPI)', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	const githubToken = process.env.SINGULARITY_GITHUB_TOKEN;
	const hasToken = !!githubToken;

	const productService: IProductService = { _serviceBrand: undefined, ...product };

	function createService(): SingularityApiService {
		...
		return new SingularityApiService(boundFetch, new NullLogService(), productService, createTestGitHubEndpointService());
	}
	...
	(hasToken ? test : test.skip)('caches the Singularity session token across calls', async function () {
		this.timeout(60_000);
		const service = createService();
		...
		// Singularity token + resolved model id. Cache-hit assertions live in
		// the unit-test suite (see singularityApiService.test.ts) where we can
		// count `RequestType.SingularityToken` calls against a fake fetch.
