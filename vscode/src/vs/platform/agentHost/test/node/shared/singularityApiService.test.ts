...
import { SINGULARITY_API_ERROR_STATUS_STREAMING, SingularityApiError, SingularityApiService, type FetchFunction } from '../../../node/shared/singularityApiService.js';
...
function createService(fetchImpl: FetchFunction, enterpriseUri?: string): SingularityApiService {
	return new SingularityApiService(fetchImpl, new NullLogService(), testProductService, createTestGitHubEndpointService(enterpriseUri));
}
...
function routingFetch(
	...
	const impl: FetchFunction = async (input, init) => {
		...
		if (url.includes('/token') || url.includes('/singularity_internal')) {
...
function streamService(chunks: Uint8Array[], tokenOverrides?: Record<string, unknown>): SingularityApiService {
...
suite('SingularityApiService', () => {

	ensureNoDisposablesAreLeakedInTestSuite();

	test('combines internal organizations from the Singularity token with login from user discovery', async () => {
		const service = createService(async input => {
			const url = getUrl(input);
			if (url.endsWith('/singularity_internal/user')) {
				return new Response(JSON.stringify({
					login: 'octocat',
					singularityignore_enabled: true,
					endpoints: { api: 'https://api.githubsingularity.com', telemetry: 'https://telemetry.example' },
				}), { status: 200 });
			}
	...
	suite('Endpoint Discovery', () => {
		...
		test('runs endpoint discovery on first request', async () => {
			...
			const service = createService(async (input) => {
				const url = getUrl(input);
				if (url.includes('/singularity_internal')) {
					mintCount++;
					return tokenResponse();
				}
		...
		test('uses endpoints.api from the /singularity_internal/user response as the CAPI base', async () => {
			const { fetch: fetchFn, captured } = routingFetch(
				() => anthropicResponse([{ type: 'text', text: 'ok' }]),
				{ endpoints: { api: 'https://custom.singularity.example.com' } },
			);
			const service = createService(fetchFn);

			await service.messages('gh-tok', baseRequest);
			assert.strictEqual(captured().url, 'https://custom.singularity.example.com/v1/messages');
		});
		...
		test('reuses endpoint discovery when resolving the GitHub login', async () => {
			...
			const service = createService(async input => {
				const url = getUrl(input);
				if (url.includes('/singularity_internal/user')) {
					discoveryCount++;
					return new Response(JSON.stringify({
						login: 'octocat',
						endpoints: { api: 'https://custom.singularity.example.com' },
			...
			assert.deepStrictEqual({ apiEndpoint, login, discoveryCount }, {
				apiEndpoint: 'https://custom.singularity.example.com',
				login: 'octocat',
				discoveryCount: 1,
			});
		});
		...
		test('falls back to default API base when endpoints.api is missing', async () => {
			...
			await service.messages('gh-tok', baseRequest);
			assert.strictEqual(captured().url, 'https://api.githubsingularity.com/v1/messages');
		});
		...
		test('routes endpoint discovery to the GitHub Enterprise host when configured', async () => {
			...
			await service.messages('gh-tok', baseRequest);
			assert.strictEqual(discoveryUrl, 'https://api.acme.ghe.com/singularity_internal/user');
		});
		...
		test('throws on 403 from endpoint discovery', async () => {
			...
			await assert.rejects(
				...
				(err: Error) => err.message.includes('Singularity endpoint discovery failed: 403'),
			);
		});
		...
		test('throws on 500 from endpoint discovery', async () => {
			...
			await assert.rejects(
				...
				(err: Error) => err.message.includes('Singularity endpoint discovery failed: 500'),
			);
		});
		...
		test('error from CAPI does not include the github token', async () => {
			const service = createService(async (input) => {
				const url = getUrl(input);
				if (url.includes('/singularity_internal')) {
					return tokenResponse({ token: 'super-secret-singularity-token-xyz' });
				}
				return new Response('rate limited', { status: 429, statusText: 'Too Many Requests' });
			});
			await assert.rejects(
				() => service.messages('super-secret-gh-token-xyz', baseRequest),
				(err: Error) => !err.message.includes('super-secret-singularity-token-xyz') && !err.message.includes('super-secret-gh-token-xyz'),
			);
		});
	...
	suite('Request Format', () => {
		...
		test('suppressIntegrationId opt-in controls the Singularity-Integration-Id header', async () => {
			const { fetch: fetchFn, captured } = routingFetch(
				() => anthropicResponse([{ type: 'text', text: 'ok' }]),
			);
			const service = createService(fetchFn);
			...
			assert.ok(withHeader['Singularity-Integration-Id'], 'integration id should be present by default');
			assert.strictEqual(suppressed['Singularity-Integration-Id'], undefined, 'integration id should be suppressed when opted in');
		});
	});
	...
	suite('Non-Streaming Responses', () => {
		...
		test('throws on 429 rate limit', async () => {
			...
			await assert.rejects(
				() => service.messages('gh-tok', baseRequest),
				(err: unknown) => err instanceof SingularityApiError
					&& err.status === 429
	...
	suite('Streaming Responses', () => {
		...
		test('throws on error event with message', async () => {
			...
			await assert.rejects(
				...
				(err: unknown) => err instanceof SingularityApiError
					&& err.status === SINGULARITY_API_ERROR_STATUS_STREAMING
	...
	suite('Shared Behavior', () => {
		...
		test('both modes share the same cached singularity token', async () => {
			let mintCount = 0;
			const service = createService(async (input) => {
				const url = getUrl(input);
				if (url.includes('/singularity_internal')) {
	...
	// #region SingularityApiError contract

	suite('SingularityApiError contract', () => {

		async function captureSingularityApiError(promise: Promise<unknown>): Promise<SingularityApiError> {
			try {
				await promise;
			} catch (err) {
				assert.ok(err instanceof SingularityApiError, `expected SingularityApiError, got: ${err instanceof Error ? err.message : String(err)}`);
				return err;
			}
			assert.fail('expected to throw SingularityApiError');
		}
		...
		test('non-2xx with conforming Anthropic envelope: passthrough verbatim', async () => {
			...
			const err = await captureSingularityApiError(service.messages('gh-tok', baseRequest));
		...
		test('SSE error frame with full envelope: passthrough type and message', async () => {
			...
			const err = await captureSingularityApiError(
				collect(service.messages('gh-tok', { ...baseRequest, stream: true as const })),
			);
			assert.deepStrictEqual(
				{ status: err.status, envelope: err.envelope },
				{
					status: SINGULARITY_API_ERROR_STATUS_STREAMING,
					envelope: {
						type: 'error',
		...
		test('SSE error frame with conforming envelope is preserved verbatim (extra fields propagate)', async () => {
			...
			// survive the round-trip through SingularityApiError.envelope.
			const service = streamService([
				sseLines(
					'event: error',
		...
		test('models() non-2xx throws typed error with synthesized envelope', async () => {
			...
			const err = await captureSingularityApiError(service.models('gh-tok'));
			assert.deepStrictEqual(
				{ status: err.status, envelope: err.envelope },
				{
					status: 503,
		...
		test('error message never embeds auth tokens', async () => {
			...
			const err = await captureSingularityApiError(service.messages('super-secret-gh-token-xyz', baseRequest));
			const serialized = JSON.stringify({ message: err.message, envelope: err.envelope });
			assert.ok(!serialized.includes('super-secret-singularity-token-xyz'));
		...
		test('401 still invalidates the cached token (regression)', async () => {
			...
			await captureSingularityApiError(service.messages('gh-tok', baseRequest));
			await service.messages('gh-tok', baseRequest);
			assert.strictEqual(mintCount, 2);
		});
	});
