...
suite('Agent Edit Attribution Service', () => {
	...
	test('emits retained Agent characters from disjoint edits', async () => {
		...
		const marker = await service.recordEdit({
			sessionUri: 'singularitycli:/session-1',
			turnId: 'turn-1',
			toolCallId: 'tool-1',
		...
		await service.flushSession('singularitycli:/session-1');
		...
		assert.deepStrictEqual({
			...
			events: [{
				...
				sourceKey: 'source:Chat.applyEdits-$modelId:model-$harness:singularitycli-$origin:agentHost',
				sourceKeyCleaned: 'source:Chat.applyEdits-$harness:singularitycli-$origin:agentHost',
				conversationId: 'session-1',
				modifiedCount: 2,
				deltaModifiedCount: 2,
				totalModifiedCount: 2,
				origin: 'agentHost',
				harness: 'singularitycli',
	...
	test('normalizes ahp chat harness without coalescing chat resources', async () => {
		const fileService = disposables.add(new FileService(new NullLogService()));
		disposables.add(fileService.registerProvider('file', disposables.add(new InMemoryFileSystemProvider())));
		const singularityResource = URI.file('/workspace/singularity.ts');
		const claudeResource = URI.file('/workspace/claude.ts');
		await fileService.writeFile(singularityResource, VSBuffer.fromString('ab'));
		...
		const singularitySessionUri = 'singularitycli:/session-1';
		const singularityDefaultChatUri = buildDefaultChatUri(singularitySessionUri);
		const singularityPeerChatUri = buildChatUri(singularitySessionUri, 'peer');
		const claudeChatUri = buildChatUri('claude:/session-2', 'peer');

		for (const edit of [
			{ sessionUri: singularityDefaultChatUri, turnId: 'turn-default', filePath: singularityResource.fsPath, modelId: 'singularity-model' },
			{ sessionUri: singularityPeerChatUri, turnId: 'turn-peer', filePath: singularityResource.fsPath, modelId: 'singularity-model' },
		...
		await service.flushSession(singularityDefaultChatUri);
		const afterDefaultChatFlush = events.length;
		await service.flushSession(singularityPeerChatUri);
		...
		assert.deepStrictEqual({
			...
			githubEvents: [
				{ eventName: 'vscode.editTelemetry.editSources.details', harness: 'singularitycli' },
			...
			events: [
				{
					sourceKey: 'source:Chat.applyEdits-$modelId:singularity-model-$harness:singularitycli-$origin:agentHost',
					sourceKeyCleaned: 'source:Chat.applyEdits-$harness:singularitycli-$origin:agentHost',
	...
	test('tracks external drift before a later tool edit and mirrors standalone stats to GitHub', async () => {
		const sessionUri = 'singularitycli:/session-1';
		const fileService = disposables.add(new FileService(new NullLogService()));
		disposables.add(fileService.registerProvider('file', disposables.add(new InMemoryFileSystemProvider())));
		const resource = URI.file('/workspace/file.ts');
	...
	test('attributes an external overwrite without retaining overwritten Agent characters', async () => {
		...
		const service = disposables.add(instantiationService.createInstance(AgentEditAttributionService, async () => undefined, undefined));
		await service.recordEdit({
			sessionUri: 'singularity:/session-1',
			turnId: 'turn-1',
			toolCallId: 'tool-1',
			filePath: resource.fsPath,
			beforeText: 'a',
		...
		await service.flushSession('singularity:/session-1');
	...
	test('paginates standalone coverage acknowledgements without advancing their cutoff', async () => {
		...
		await fileService.writeFile(resource, VSBuffer.fromString('ab'));
		await service.recordEdit({
			sessionUri: 'singularity:/session-live',
			turnId: 'turn-live',
	...
	test('flushes only the closing session when sessions edit the same file', async () => {
		...
		const service = disposables.add(instantiationService.createInstance(AgentEditAttributionService, async () => undefined, undefined));
		await service.recordEdit({
			sessionUri: 'singularity:/session-a',
			turnId: 'turn-a',
			toolCallId: 'tool-a',
			filePath: resource.fsPath,
			beforeText: 'a',
		...
		await service.recordEdit({
			sessionUri: 'singularity:/session-b',
			turnId: 'turn-b',
			toolCallId: 'tool-b',
			filePath: resource.fsPath,
			beforeText: 'ab',
		...
		await service.flushSession('singularity:/session-b');
		const afterFirstFlush = events.map(event => event.conversationId);
		await service.flushSession('singularity:/session-a');
		const acknowledged = await service.prepareFlush({
			resource,
			trigger: 'closed',
			statsUuid: 'stats-1',
	...
	test('bounds same-file reconciliation by one aggregate deadline', async () => {
		...
		for (const [sessionUri, beforeText, afterText] of [
			['singularity:/session-a', 'a', 'ab'],
			['singularity:/session-b', 'ab', 'abc'],
			['singularity:/session-c', 'abc', 'abcd'],
			['singularity:/session-d', 'abcd', 'abcde'],
		] as const) {
			await service.recordEdit({
				sessionUri,
				turnId: 'turn-1',
	...
	test('claims a resource once when flush triggers race', async () => {
		...
		const [_, prepared] = await Promise.all([
			service.flushSession('singularity:/session-1'),
