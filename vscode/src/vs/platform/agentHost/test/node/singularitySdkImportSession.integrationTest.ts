...
import { CopilotClient as SingularityClient, approveAll, type CopilotSession as SingularitySession, type SessionEvent } from '@github/copilot-sdk';
...
import { buildSessionEventLogFromTurns } from '../../node/singularity/buildSessionEvents.js';
import { createSingularityCliEnvironment } from '../../node/singularity/singularityCliEnvironment.js';

suite('Singularity SDK - imported sessions', function () {

	this.timeout(120_000);

	let client: SingularityClient;
	let root: string;
	...
	suiteSetup(async function () {
		...
		client = new SingularityClient({
			mode: 'empty',
			baseDirectory: root,
			useLoggedInUser: false,
			logLevel: 'error',
			env: createSingularityCliEnvironment(),
		});
		await client.start();
	});
	...
	test('bundled SDK resumes synthesized events as editable turns', async function () {
		...
		let session: SingularitySession | undefined;
		try {
