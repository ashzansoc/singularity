...
/**
 * A record/replay HTTP proxy for the CAPI (Singularity API) traffic that the agent
 * host's bundled Singularity SDK/CLI produces.
 *
...
const TEMP_DIR_SUFFIX_RE = /(\$\{workdir\}(?:\/|\\\\)(?:ahp-(?:snapshot|perm-test|plan-test|abort|test|wt-test|subagent-test|subagent-replay|attachment-test|cd-strip-test|coverage-[a-z-]+)-|singularity-(?:cost-report|text-blob)-|read-sdk-simple))[A-Za-z0-9]{6}/g;
 ...
 * (never stored); the one response-side secret is the minted Singularity session
 * token returned by `/singularity_internal/v2/token` (and `session_token` from the
 * auto-model endpoint).
 */
...
const GITHUB_API_PREFIXES = ['/singularity_internal', '/telemetry', '/singularity/mcp_registry'];
...
export interface ICapiReplayProxyOptions {
	...
	/** Real GitHub-API base for `/singularity_internal/*` while recording (e.g. `https://api.github.com`). */
	readonly githubUpstreamUrl?: string;
	/** Real CAPI base for model/`/models` traffic while recording (e.g. `https://api.githubsingularity.com`). */
