...
The prompt snapshots in `providers/singularityPromptsE2E.integrationTest.ts` are also POSIX-only — every model, by construction rather than because of an observed failure.
...
The same test's Singularity branch waits for a `chat/toolCallContentChanged` carrying a terminal resource. On Windows CI that notification never arrives, even though `chat/toolCallStart`, `chat/toolCallReady`, the confirmation round-trip, and `chat/toolCallComplete` all do.
...
- **Steer** (`Use your file tools; do not run a shell command.`) where a file tool exists for the operation and the provider follows the instruction reliably. Reads and missing-file handling use this path for Claude and Singularity.
...
### `create_session` resolves Claude and Codex models to the Singularity provider
...
- Observed: both providers create the child session with provider `singularitycli`. The selected model id is present in more than one provider's global model list, and the session tool does not carry a provider-qualified model selection. Codex also executes `create_session` without surfacing its required pending confirmation.
...
### Singularity prompt snapshots on Windows

- Tests: all models in `singularityPromptsE2E.integrationTest.ts`.
- Scope: Windows.
...
| `worktree session uses the resolved worktree as working directory` | Windows | `os.tmpdir()` yields an 8.3 short path while the shell reports the long path, and Singularity's completed host-terminal call publishes no terminal-content notification. |
| ``strips redundant `cd <workingDirectory> &&` prefix from shell tool calls`` | Singularity on Windows | The turn completes, but `chat/toolCallReady` omits the `toolInput` needed to assert that the prefix was removed. |

Singularity's ordinary provider shell also omits `ToolResultTerminalContent.result.preview` on Windows, while its terminal-shaped resource is not backed by the host terminal manager and cannot be subscribed. These tests are skipped for Singularity on Windows because their direct output oracle would otherwise be empty:
...
- Scope: deterministic replay for every provider; the second test is Singularity-specific.
...
- Observed: plan mode is wired, but the Singularity-oriented prompt does not reliably cause Claude to invoke the tool.
