...
export function defineCustomizationDiscoveryTests(context: IAgentHostE2ETestContext): void {
	...
	const enabled = context.tier === 'parity' && config.provider === 'singularitycli';

	function singularityTest(title: string, run: Mocha.AsyncFunc): void {
	...
	for (const mode of ['scan', 'discover'] as const) {
		singularityTest(`customization discovery: ${mode} finds workspace agents instructions skills and hooks`, async function () {
	...
	singularityTest('customization discovery: discover groups fixed agent instruction files at the workspace root', async function () {
		const workspace = createWorkspace('agent-instructions');
		const files = [
			join(workspace, 'AGENTS.md'),
			join(workspace, 'CLAUDE.md'),
			join(workspace, '.github', 'singularity-instructions.md'),
		];
	...
	singularityTest('customization discovery: configured plugin exposes its agent rule and skill children', async function () {
	...
	singularityTest('customization discovery: filesystem watcher publishes a newly added agent', async function () {
	...
	singularityTest('customization discovery: filesystem watcher removes a deleted agent', async function () {
	...
	singularityTest('customization discovery: filesystem watcher updates an edited agent', async function () {
