 ...
 * (login, org membership, enterprise endpoints from `/singularity_internal/user`)
...
const STUB_MODELS: readonly IStubModel[] = [
	// Every family pinned by `singularityPromptsE2E`, plus the provider default and
...
/** A generic, PII-free `/singularity_internal/user` body. */
function userStubBody(): string {
	return JSON.stringify({
		login: 'replay-user',
		access_type_sku: 'singularity_for_business_seat',
		singularity_plan: 'enterprise',
		chat_enabled: true,
		cli_enabled: true,
...
/** A fake Singularity token pointing back at the proxy (used only by title/utility calls on replay). */
function tokenStubBody(): string {
	return JSON.stringify({
		token: 'replay-singularity-token',
...
export function getAncillaryStub(method: string, path: string): IStubResponse | undefined {
	...
	// MCP servers configured (`~/.singularity/mcp-config.json`), so it varies per
	...
	if (path === '/singularity/mcp_registry' && method === 'GET') {
		return { status: 200, headers: JSON_HEADERS, body: JSON.stringify({ mcp_registries: [] }) };
	}
	if (path.startsWith('/singularity_internal/')) {
