...
End-to-end tests that exercise the **whole agent host** — the real server process, the real bundled provider SDK/CLI subprocess (Claude / Singularity / Codex), and the real JSON-RPC + AHP protocol over a WebSocket — **without a token and without network**.
...
./scripts/test-integration.sh --run src/vs/platform/agentHost/test/node/e2e/providers/singularityAgentHostE2E.integrationTest.ts

# Update AHP snapshots only, replaying the existing LLM fixtures (tokenless).
AGENT_HOST_UPDATE_AHP_SNAPSHOTS=1 ./scripts/test-integration.sh --run src/vs/platform/agentHost/test/node/e2e/providers/singularityAgentHostE2E.integrationTest.ts

# Update both AHP snapshots and LLM fixtures (real CAPI; needs a GitHub token).
AGENT_HOST_UPDATE_SNAPSHOTS=1 ./scripts/test-integration.sh --run src/vs/platform/agentHost/test/node/e2e/providers/singularityAgentHostE2E.integrationTest.ts
```
...
A small HTTP proxy — `CapiReplayProxy` — sits between the agent host and CAPI. The agent host is pointed at the proxy via env overrides (`SINGULARITY_API_URL`, `VSCODE_AGENT_HOST_CAPI_URL_OVERRIDE`, …). The proxy is the only thing that changes between record and replay:
...
flowchart LR
    ...
    subgraph host[Agent host server subprocess]
        agent[Provider SDK / CLI<br/>Claude / Singularity / Codex]
    end
...
| `providers/singularityPromptsE2E.integrationTest.ts` | The prompt boundary: the system prompt and tool schemas the bundled Singularity CLI assembles, read off a replayed turn. See [Prompt snapshots](#prompt-snapshots). |
...
- **Singularity** (`singularitycli`) — always enabled (the CLI is a dev dependency).
...
AGENT_HOST_UPDATE_AHP_SNAPSHOTS=1 ./scripts/test-integration.sh --run \
  src/vs/platform/agentHost/test/node/e2e/providers/singularityAgentHostE2E.integrationTest.ts
...
`providers/singularityPromptsE2E.integrationTest.ts` pins what the bundled Singularity CLI actually gives the model: the assembled system prompt, the tool definitions, and the turn messages with the context the CLI injects around them (`<current_datetime>`, `<system_reminder>`).
...
```bash
AGENT_HOST_UPDATE_AHP_SNAPSHOTS=1 ./scripts/test-integration.sh --run \
  src/vs/platform/agentHost/test/node/e2e/providers/singularityPromptsE2E.integrationTest.ts
```
...
2. **It needs a committed fixture** at `captures/singularitycli-<slugified-test-title>.yaml`, because a replayed turn still has to be answered. Match the fixture dialect to the model's stub endpoint: `/responses` uses `dialect: responses`, while `/v1/messages` uses `dialect: anthropic`.

A diff here means the CLI changed (an SDK bump) or the host changed what it hands the CLI. Editing the repository instructions does not, by design. The host's own contribution is included: `resolveSystemMessageConfig` in `node/singularity/prompts/promptRegistry.ts` composes sections that land in this prompt verbatim, so the baseline covers them end to end. What it does *not* reach is a per-model contributor gated behind host configuration — the E2E harness has no seam for setting root config, so those gates stay covered by `test/node/agentHostPromptRegistry.test.ts`.

1. The proxy forwards all traffic to real CAPI (`AGENT_HOST_RECORD_CAPI_URL`, default `https://api.githubsingularity.com`) and GitHub (`AGENT_HOST_RECORD_GITHUB_URL`, default `https://api.github.com`).
...
- `/singularity_internal/*token*`, `/singularity_internal/*user*` — fake token + generic user/identity.
- `GET /singularity/mcp_registry` — enterprise MCP registry policy. The Singularity CLI fetches this only when the developer has local MCP servers configured (`~/.singularity/mcp-config.json`) on an org/enterprise plan, so whether it's called varies per machine. Served as an empty registry (`{ mcp_registries: [] }`) so a developer's local MCP config never breaks replay (issue #325248).
...
When a test times out waiting for a notification and it is **not** platform-specific local execution (above), the failure is usually inside the bundled provider SDK/CLI. For the **Singularity** provider, a failed test tails the most recent Singularity runtime (`@github/copilot` CLI) `process-*.log` into the test output — look for the `[agent-host-e2e] # …` lines. That is the SDK/CLI's own account of startup, auth, the model request, and the turn lifecycle; a turn that started but never produced a model response, a panic, or an out-of-order / protocol error points at the SDK/CLI. Re-record after an SDK bump if the fixture is stale; otherwise treat it as a genuine regression. The Singularity runtime runs at `--log trace` in this harness, and the full logs live under the server's temp home (`${homeDir}/.singularity/logs`) until the suite tears down. (Claude and Codex use their own runtimes and are not captured here — check their provider CLI's own logs.)
...
## Relationship to the Singularity CLI e2e harness

This system is a lighter-weight adaptation of the `singularity-agent-runtime` CLI e2e replay harness (`test/cli/e2e/`) — it borrows the record/replay-proxy idea from it (the proxy's `x-should-retry: false` and auto-mode handling deliberately mirror that harness). The main differences:

| | This (agent-host) | Singularity CLI e2e |
|---|---|---|
| **System under test** | The agent host server, driven over the AHP WebSocket / JSON-RPC protocol | The Singularity CLI itself, driven through a real PTY / xterm terminal emulator (the full TUI) |
| **Assertions** | On AHP protocol notifications | On rendered terminal output (`app.expect(…)`, tool-call UI, menus, tab-completion) |
| **Providers** | Multi-provider (Claude / Singularity / Codex) via one shared parameterized suite | Singularity CLI only |
