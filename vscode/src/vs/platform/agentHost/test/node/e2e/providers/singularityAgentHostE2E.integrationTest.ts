...
/**
 * Agent host end-to-end tests (Singularity).
 *
 * The cross-provider portion lives in {@link defineAgentHostE2ETests}; this
 * file layers on Singularity-specific assertions (cost metadata, cd-prefix
 * stripping).
 *
 ...
 *
 *   AGENT_HOST_REPLAY_RECORD=1 ./scripts/test-integration.sh --run src/vs/platform/agentHost/test/node/e2e/providers/singularityAgentHostE2E.integrationTest.ts
 *
 * Recording auth: the token is obtained from `gh auth token`, or override with
 * `GITHUB_TOKEN=ghp_xxx`. Replay needs no credential.
 *
 * SAFETY: Recording creates real agent sessions backed by the Singularity SDK.
...
import { SINGULARITY_CONFIG } from './singularityTestConfiguration.js';

const RECORD_ONLY = process.env['AGENT_HOST_REPLAY_RECORD'] === '1';
const isWindows = process.platform === 'win32';

defineAgentHostE2ETests(SINGULARITY_CONFIG);

suite('Agent Host E2E — Singularity (Singularity-specific)', function () {
	...
	suiteSetup(function () {
		lease = new AgentHostE2EServerLease(SINGULARITY_CONFIG);
	});
	...
	teardown(async function () {
		...
		if (errors.length > 0) {
			throw new AggregateError(errors, 'Failed to dispose Singularity-specific E2E test resources');
		}
	});

	test('client tool reaches ready after start and completes', async function () {
		this.timeout(180_000);
		await runAhpSnapshotTest(client, SINGULARITY_CONFIG, this.test!, createdSessions, tempDirs, {
		...
		assert.deepStrictEqual({
			...
			startContributor: { kind: ToolCallContributorKind.Client, clientId: 'singularity-client-tool' },
			readyContributor: { kind: ToolCallContributorKind.Client, clientId: 'singularity-client-tool' },
			deltaCount: 0,
		});
	});
	...
	test('client tool disconnect before permission still completes the turn', async function () {
		this.timeout(180_000);
		const workingDirectory = await mkdtemp(join(tmpdir(), 'singularity-client-tool-disconnect-'));
		tempDirs.push(workingDirectory);
		const clientId = 'singularity-client-tool-disconnect';
		const sessionUri = await createRealSession(client, SINGULARITY_CONFIG, clientId, createdSessions, URI.file(workingDirectory));
	...
	(RECORD_ONLY ? test : test.skip)('accepted steering followed by abort does not block the replacement turn', async function () {
		this.timeout(180_000);
		const workingDirectory = await mkdtemp(join(tmpdir(), 'singularity-steering-abort-'));
		tempDirs.push(workingDirectory);
		const clientId = 'singularity-steering-abort';
		const sessionUri = await createRealSession(client, SINGULARITY_CONFIG, clientId, createdSessions, URI.file(workingDirectory));
		const chatUri = buildDefaultChatUri(sessionUri);
	...
	test('usage reports include Singularity cost metadata', async function () {
		this.timeout(120_000);
		const workingDirectory = await mkdtemp(join(tmpdir(), 'singularity-cost-report-'));
		tempDirs.push(workingDirectory);

		const sessionUri = await createRealSession(client, SINGULARITY_CONFIG, 'real-sdk-usage', createdSessions, URI.file(workingDirectory));
	...
	test('attaches a Python file and reads its function names', async function () {
		...
		const sessionUri = await createRealSession(client, SINGULARITY_CONFIG, 'real-sdk-attachment', createdSessions, URI.file(workingDirectory));
	...
	test('attaches a text blob and reads its function names', async function () {
		...
		const workingDirectory = await mkdtemp(join(tmpdir(), 'singularity-text-blob-'));
		tempDirs.push(workingDirectory);

		const sessionUri = await createRealSession(client, SINGULARITY_CONFIG, 'real-sdk-blob-attachment', createdSessions, URI.file(workingDirectory));
	...
	(isWindows ? test.skip : test)('strips redundant `cd <workingDirectory> &&` prefix from shell tool calls', async function () {
		...
		const sessionUri = await createRealSession(client, SINGULARITY_CONFIG, 'real-sdk-cd-strip', createdSessions, URI.file(workspaceDir));
		...
		const toolStartNotif = await client.waitForNotification(n => {
			...
			const envelope = getActionEnvelope(n);
			const action = envelope.action as ChatToolCallStartAction;
			return envelope.channel === chatUri && action.turnId === turnId && action.toolName === SINGULARITY_CONFIG.shellToolName;
		}, 90_000);
