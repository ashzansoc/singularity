 ...
 * Singularity and Claude (tool names, URI scheme, server startup options).
 */
export interface IAgentHostE2EProviderConfig {
	 ...
	 * by the shell-permission and cd-prefix tests. (`bash` for Singularity,
	 * `Bash` for Claude.)
	 */
	 ...
	 * for Singularity; Claude exposes both `Task` and `Agent` as subagent-kind
	 ...
	 * plan mode. (`exit_plan_mode` for Singularity, `ExitPlanMode` for Claude.)
	 */
	 ...
	 * shared across all agents (Singularity, Codex, Claude) via the host-owned
	 ...
	 * tool (gated by {@link SingularityCliConfigKey.EnableCustomTerminalTool}),
	 * which exposes a terminal resource whose `cwd` / `pwd` output can be
	 * asserted. Currently true only for Singularity — Codex and Claude run shell
	 ...
	 * full coverage; providers that rebuild from the in-process event log (Singularity)
	 ...
	 * Singularity — Claude's plan-mode prompt conventions differ enough that the
...
export class AgentHostE2EServerLease {
	...
	async acquire(testTitle: string, modelTraffic: AgentHostE2EModelTraffic = 'recorded'): Promise<{ server: IServerHandle; client: TestProtocolClient }> {
		...
		if (this._shared && this._server) {
			...
			// Only the Singularity CLI provider writes the `@github/copilot` runtime logs we
			// capture, so only it is run verbosely; Claude/Codex use their own runtimes.
			this._server = await this._target.launch({ ...this._startOptions, capiReplay, logLevel: this._isSingularityProvider ? 'trace' : undefined });
	...
	async restart(): Promise<TestProtocolClient> {
		...
		try {
			this._server = await this._target.launch({
				...
				logLevel: this._isSingularityProvider ? 'trace' : undefined,
			});
		} catch (error) {
			await proxy.close();
			throw error;
	...
	private get _isSingularityProvider(): boolean {
		return this._config.provider === 'singularitycli';
	}

	/**
	 * Tail the most recent Singularity runtime (`@github/copilot` CLI) `process-*.log`
	 ...
	 * The runtime writes these under `${SINGULARITY_HOME}/logs`, and the harness pins
	 * `SINGULARITY_HOME` to `${homeDir}/.singularity` (see `startRealServer`), running it
	 * at `trace`. Only the Singularity CLI provider is captured — Claude/Codex use their
	...
	dumpRuntimeLogsOnFailure(label: string): void {
		if (!this._isSingularityProvider) {
			return;
		}
		try {
			const logsDir = join(this._startOptions.homeDir, '.singularity', 'logs');
			let entries: string[];
			try {
				entries = readdirSync(logsDir);
			} catch {
				// No log dir at all — the CLI never spawned. That itself is a signal.
				process.stdout.write(`[agent-host-e2e] no Singularity runtime logs for failed test "${label}" (CLI never spawned; ${logsDir} absent)\n`);
				return;
			}
			...
			if (!newest) {
				process.stdout.write(`[agent-host-e2e] no Singularity runtime process-*.log for failed test "${label}" under ${logsDir}\n`);
				return;
			}
			const lines = readFileSync(newest.full, 'utf8').split(/\r?\n/);
			const tail = lines.slice(-200);
			process.stdout.write(`[agent-host-e2e] --- Singularity runtime log for failed test "${label}" (${newest.full}; last ${tail.length} of ${lines.length} lines) ---\n`);
			for (const ln of tail) {
				process.stdout.write(`[agent-host-e2e] # ${ln}\n`);
			}
			process.stdout.write('[agent-host-e2e] --- end Singularity runtime log ---\n');
		} catch {
