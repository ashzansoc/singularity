	...
	serialize(options: IAhpSnapshotOptions = {}): string {
		...
		for (let messageIndex = 0; messageIndex < this._messages.length; messageIndex++) {
			...
			if (isMethodMessage(message)) {
				...
				// notifications/tools/list_changed is legitimate behavior (Singularity >= 1.0.72
				...
				// notification appears when the developer's ~/.singularity configures MCP
...
/**
 * Collapses the platform-specific Singularity shell tool names to stable
 * placeholders.
 *
 * The Singularity CLI names its shell tools after the shell it runs: `bash` and
