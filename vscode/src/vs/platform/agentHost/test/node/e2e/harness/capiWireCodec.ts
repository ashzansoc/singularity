 ...
 *
 * Ported (lean) from the Singularity CLI e2e harness's dialect adapters — we keep
 ...
 * which is what the Singularity and Claude providers use.
 */
 ...
 * Singularity CLI harness, which normalizes the same injected blocks. */
 ...
 * or Singularity's `singularity_usage.token_details`). */
function usageFromResponsesEvent(evt: ISseEvent): { inputTokens?: number; outputTokens?: number } {
	...
	const details = (evt['singularity_usage'] as { token_details?: Array<{ token_type?: string; token_count?: number }> } | undefined)?.token_details;
