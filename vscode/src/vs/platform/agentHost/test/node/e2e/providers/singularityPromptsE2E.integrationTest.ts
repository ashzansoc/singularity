...
/**
 * Pins the prompt and tool schemas the bundled Singularity CLI assembles per model.
 *
...
suite('Agent Host E2E — Singularity prompts', function () {
	...
	for (const model of SNAPSHOT_MODELS) {
		...
		(process.platform === 'win32' ? test.skip : test)(model, async function () {
			...
			const sessionUri = await createRealSession(client, SINGULARITY_CONFIG, `prompt-snap-${model}`, createdSessions, URI.file(workspaceDir));
...
suite('Singularity prompt snapshot formatting', () => {
