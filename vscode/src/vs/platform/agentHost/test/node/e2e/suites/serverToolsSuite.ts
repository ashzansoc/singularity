...
export function defineServerToolsTests(context: IAgentHostE2ETestContext): void {
	...
	const supportsCrossSessionDelete = config.provider === 'singularitycli';
	// Claude and Codex start another turn instead of rejecting a message to the current chat.
	const supportsSelfSendRejection = config.provider === 'singularitycli';
	// Model ids are not provider-qualified; Claude and Codex selections currently resolve to Singularity.
	const supportsProviderModelSessionCreation = config.provider === 'singularitycli';
	...
	const supportsServerToolCreateChat = config.provider === 'singularitycli';
	...
	async function addSession(prefix: string, workspace: string, stableResource = false): Promise<IServerToolTestSession> {
		const id = stableResource && config.provider === 'singularitycli' ? `e2e-server-tools-${prefix}` : generateUuid();
