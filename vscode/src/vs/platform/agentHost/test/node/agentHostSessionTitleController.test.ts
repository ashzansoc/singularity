...
suite('AgentHostSessionTitleController', () => {
	...
	function setup(
		singularityApiService = new TestSingularityApiService(),
		title = '',
		getSingularityToken = () => 'gh-token',
		...
		titleActions: string[];
		singularityApiService: TestSingularityApiService;
		octoKitService: TestAgentHostOctoKitService;
	} {
		const stateManager = disposables.add(new AgentHostStateManager(new NullLogService()));
		const db = new TestSessionDatabase();
		const session = URI.parse('agenthost-session://singularity/session-title-test');
		...
		return { controller, stateManager, session, db, titleActions, singularityApiService, octoKitService };
	}
	...
	test('seedTitleFromFirstMessage applies fallback and persists generated title', async () => {
		...
		singularityApiService.response = '"Generated title."';
		const { controller, session, db, titleActions } = setup(singularityApiService);
		...
		assert.deepStrictEqual({
			...
			maxTokens: singularityApiService.utilityCalls[0]?.request.maxTokens,
			promptIncludesUserText: singularityApiService.utilityCalls[0]?.request.messages.some(message => message.content.includes('Please   explain title generation')),
	...
	test('seedTitleFromFirstMessage appends every unique GitHub issue and pull request', async () => {
		...
		const { controller, session, db } = setup(singularityApiService, '', () => 'gh-token', octoKitService);
		...
		const userMessage = singularityApiService.utilityCalls[0].request.messages.find(message => message.role === 'user')?.content;
		assert.deepStrictEqual({
	...
	test('seedTitleFromFirstMessage only fetches links from the configured GitHub host', async () => {
		...
		const { controller, session, db } = setup(singularityApiService, '', () => 'gh-token', octoKitService, () => 'github-token', undefined, () => 'github.enterprise.test');
	...
	test('seedTitleFromFirstMessage times out GitHub context requests', async () => {
		...
		const { controller, session, db } = setup(singularityApiService, '', () => 'gh-token', octoKitService, () => 'github-token', 1);
		const prompt = 'Fix https://github.com/microsoft/vscode/issues/123';
	...
	test('seedTitleFromFirstMessage strips an unexpected trailing Han suffix from a Latin title', async () => {
		...
		for (const testCase of cases) {
			const singularityApiService = new TestSingularityApiService();
			singularityApiService.response = testCase.response;
			const { controller, stateManager, session, db } = setup(singularityApiService);
	...
	test('seedTitleFromFirstMessage does not clobber a changed title', async () => {
		...
		await waitForCondition(() => singularityApiService.utilityCalls.length === 1, 'title generation should start');
	...
	test('seedTitleFromFirstMessage skips sessions with an existing title', async () => {
		const singularityApiService = new TestSingularityApiService();
		const { controller, stateManager, session, db, titleActions } = setup(singularityApiService, 'Forked: Source title');

		controller.seedTitleFromFirstMessage(session.toString(), 'Continue forked session');
		await Promise.resolve();

		assert.deepStrictEqual({
			calls: singularityApiService.utilityCalls.length,
			title: stateManager.getSessionState(session.toString())?.title,
			titles: titleActions,
			persistedTitle: await db.getMetadata('customTitle'),
		}, {
	...
	test('seedProvisionalTitle titles the session from the suggestion without generating', async () => {
		const singularityApiService = new TestSingularityApiService();
		const { controller, stateManager, session, db, titleActions } = setup(singularityApiService);
	...
	test('seedTitleFromFirstMessage replaces a provisional title with a generated title', async () => {
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.response = 'Explain the build';
		const { controller, stateManager, session, db, titleActions } = setup(singularityApiService);
	...
	test('seedTitleFromFirstMessage persists its fallback when replacing a provisional title', async () => {
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.error = new Error('Title generation unavailable');
		const { controller, stateManager, session, db } = setup(singularityApiService);
	...
	async function seedFirstTitle(controller: AgentHostSessionTitleController, singularityApiService: TestSingularityApiService, db: TestSessionDatabase, session: URI, userPrompt: string, title: string): Promise<void> {
		singularityApiService.response = title;
	...
	test('refineTitleFromFirstTurn regenerates the title from the first-turn context', async () => {
		const singularityApiService = new TestSingularityApiService();
		const { controller, stateManager, session, db } = setup(singularityApiService);
		await seedFirstTitle(controller, singularityApiService, db, session, 'Add dark mode toggle', 'First title');

		singularityApiService.response = 'Dark mode setting';
		...
		const lastCall = singularityApiService.utilityCalls[singularityApiService.utilityCalls.length - 1];
	...
	test('refineTitleFromFirstTurn does not clobber a title changed in the meantime', async () => {
		...
		const { controller, stateManager, session, db } = setup(singularityApiService);
		await seedFirstTitle(controller, singularityApiService, db, session, 'Add dark mode toggle', 'First title');
		const callsAfterSeed = singularityApiService.utilityCalls.length;
	...
	test('refineTitleFromFirstTurn ignores tool calls and reasoning, keeping only text parts', async () => {
		...
		singularityApiService.response = 'Refined title';
		...
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(() => singularityApiService.utilityCalls.length >= 2, 'refine should issue a utility call');
	...
	test('generateForkedTitle replaces the inherited title using the whole forked conversation', async () => {
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.response = 'Compaction strategy';
		const { controller, stateManager, session, db, titleActions } = setup(singularityApiService, 'Forked: Source title');
	...
	test('generateForkedTitle does not clobber a title changed during generation', async () => {
		...
		let resolveTitle!: (title: string) => void;
		singularityApiService.responsePromise = new Promise(resolve => { resolveTitle = resolve; });
		const { controller, stateManager, session, db } = setup(singularityApiService, 'Forked: Source title');
		...
		await waitForCondition(() => singularityApiService.utilityCalls.length === 1, 'forked title generation should start');
		stateManager.dispatchServerAction(session.toString(), { type: ActionType.SessionTitleChanged, title: 'Manual title' });
		resolveTitle('Generated title');
		await Promise.resolve();
