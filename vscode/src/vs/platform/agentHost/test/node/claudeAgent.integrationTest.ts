 ...
 *  - Stubbed {@link ISingularityApiService} that yields a canned Anthropic
...
import { type AgentSignal, GITHUB_SINGULARITY_PROTECTED_RESOURCE } from '../../common/agentService.js';
...
class StubSingularityApiService implements ISingularityApiService {
...
suite('ClaudeAgent integration (proxy-backed)', function () {
	...
	test('agent → proxy → CAPI → SSE → agent: end-to-end pipeline with real proxy and stubbed CAPI', async () => {
		...
		// + real ClaudeAgent + stubbed ISingularityApiService + recording SDK
		...
		const capi = new StubSingularityApiService();
		...
		const accepted = await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'gh-int-test-token');
	...
	test('Phase 7 §5.3 — canUseTool / onElicitation closures wired through to Options on materialize', async () => {
		...
		await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'gh-int-test-token');
		const created = await agent.createSession({ workingDirectories: [URI.file('/integration-cwd')] });
		const sessionId = created.session.path.replace(/^\//, '');
		sdk.queryMessages = [
