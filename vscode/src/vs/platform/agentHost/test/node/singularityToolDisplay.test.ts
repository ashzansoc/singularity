...
import { getEditFilePath, getEditFilePaths, getInvocationMessage, getPastTenseMessage, getPermissionDisplay, getShellIntention, getShellLanguage, getStreamingInvocationMessage, getToolDisplayName, getToolInputString, getToolKind, getToolMarkdownContent, isEditTool, isHiddenTool, isMarkdownRenderedTool, synthesizeSkillToolCall } from '../../node/singularity/singularityToolDisplay.js';

type SingularityShellPermissionRequest = Extract<PermissionRequest, { kind: 'shell' }>;
type SingularityCustomToolPermissionRequest = Extract<PermissionRequest, { kind: 'custom-tool' }>;

function shellPermissionRequest(fullCommandText: string, requestSandboxBypass?: boolean): SingularityShellPermissionRequest {
	return {
		kind: 'shell',
...
function customToolPermissionRequest(toolName: string, args: Record<string, unknown>): SingularityCustomToolPermissionRequest {
	return {
		kind: 'custom-tool',
		toolName,
		toolDescription: '',
...
suite('singularityToolDisplay — friendly tool names', () => {

	ensureNoDisposablesAreLeakedInTestSuite();

	test('mirrors internal Singularity CLI friendly labels for representative tools', () => {
		const cases: Array<[toolName: string, displayName: string]> = [
			...
			['show_file', 'Show File'],
			['fetch_singularity_cli_documentation', 'Fetch Documentation'],
...
suite('singularityToolDisplay — edit tool classification', () => {
...
suite('singularityToolDisplay — markdown-rendered tools', () => {
...
suite('getPermissionDisplay — read permission display', () => {
	...
	test('uses the view-tool invocation message for read permissions', () => {
		const display = getPermissionDisplay({
			kind: 'read',
			path: '/Users/connor/Downloads/context7-singularity-debug-main.json',
			intention: 'Read file: /Users/connor/Downloads/context7-singularity-debug-main.json',
		...
		assert.deepStrictEqual({
			...
			invocationMessage: { markdown: 'Reading [context7-singularity-debug-main.json](file:///Users/connor/Downloads/context7-singularity-debug-main.json)' },
			toolInput: undefined,
			permissionKind: 'read',
			permissionPath: '/Users/connor/Downloads/context7-singularity-debug-main.json',
		});
	});
});
...
suite('singularityToolDisplay — built-in tool invocation/past-tense messages', () => {
...
suite('singularityToolDisplay — streaming edit messages', () => {
...
// registered with `skipPermission: true` — see singularityShellTools.ts), but they
// still flow through the tool-execution display pipeline.

suite('singularityToolDisplay — write_/read_ shell tools', () => {
	...
	suite('feedback comment tools (delegated to the shared server-tool group)', () => {
		...
		// These smoke checks only assert that the Singularity display functions
		// delegate to the shared group instead of falling through to the
		// generic `Using/Used "<tool>"` fallback.
		test('Singularity display delegates to the shared group', () => {
