...
import { type ISessionEvent } from './singularityTestEvents.js';
...
import { type ISingularityApiService, type ISingularityApiServiceRequestOptions, type ISingularityUtilityChatCompletionRequest } from '../../node/shared/singularityApiService.js';
...
/**
 * Loads a JSONL fixture of raw Singularity SDK events, runs them through
 ...
 * `events.jsonl` files from `~/.singularity/session-state/`.
 */
...
suite('AgentService (node dispatcher)', () => {
	...
	let singularityAgent: MockAgent;
	...
	setup(async () => {
		...
		singularityAgent = new MockAgent('singularity');
		disposables.add(toDisposable(() => singularityAgent.dispose()));
	});
	...
	suite('registerProvider', () => {
		...
		test('registers a provider successfully', () => {
			service.registerProvider(singularityAgent);
			// No throw - success
		});

		test('throws on duplicate provider registration', () => {
			service.registerProvider(singularityAgent);
			const duplicate = new MockAgent('singularity');
		...
		test('aggregates and deduplicates network diagnostics endpoints', async () => {
			const providerA: IAgent = singularityAgent;
		...
		test('aggregates managed-settings diagnostics from capable providers', async () => {
			const provider: IAgent = singularityAgent;
		...
		test('maps progress events to protocol actions via onDidAction', async () => {
			service.registerProvider(singularityAgent);
			const session = await service.createSession({ provider: 'singularity' });
	...
	test('resolveSessionConfig echoes host-owned worktree values across isolation modes', async () => {
		...
		localService.setWorktreeIsolation(disposables.add(new WorktreeIsolation(
			...
			gitService,
			new TestSingularityApiService(),
			nullSessionDataService,
			new NullLogService(),
		)));
		const agent = new MockAgent('singularity');
	...
	test('provisional materialization preserves and persists multi-root metadata', async () => {
		...
		const db = new TestSessionDatabase();
		const localService = disposables.add(new AgentService(new NullLogService(), fileService, createSessionDataService(db), { _serviceBrand: undefined } as IProductService, createNoopGitService()));
		const agent = new ProvisionalAgent('singularity');
		disposables.add(toDisposable(() => agent.dispose()));
		localService.registerProvider(agent);
		const multiRoot = {
	...
	test('reconciles pending worktree isolation when creating session config changes', async () => {
		...
		const readyAgent = new MockAgent('singularity');
	...
	suite('dispatchAction', () => {
		...
		async function setupTitleGeneration(singularityApiService: TestSingularityApiService): Promise<{ svc: AgentService; agent: MockAgent; session: URI; db: TestSessionDatabase }> {
			const db = new TestSessionDatabase();
			...
			const svc = disposables.add(new AgentService(
				...
				undefined,
				undefined,
				singularityApiService,
			));
			const agent = new MockAgent('singularity');
			disposables.add(toDisposable(() => agent.dispose()));
			svc.registerProvider(agent);
			await svc.authenticate({
				resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource,
				scopes: GITHUB_SINGULARITY_PROTECTED_RESOURCE.scopes_supported,
				token: 'gh-token',
			});
			const session = await svc.createSession({ provider: 'singularity' });
		...
		test('generates and persists an AI title after first-turn fallback title', async () => {
			...
			singularityApiService.response = '"Fix TypeScript compile errors."';
			const { svc, session, db } = await setupTitleGeneration(singularityApiService);
			...
			assert.deepStrictEqual({
				...
				promptIncludesUserText: singularityApiService.utilityCalls[0]?.request.messages.some(message => message.content.includes('Please help me fix the TypeScript compile errors')),
		...
		test('leaves fallback title when AI title generation fails', async () => {
			const singularityApiService = new TestSingularityApiService();
			singularityApiService.error = new Error('title failed');
			const { svc, session, db } = await setupTitleGeneration(singularityApiService);
			...
			await waitForCondition(() => singularityApiService.utilityCalls.length === 1, 'title generation should be attempted');
		...
		test('does not overwrite a manual rename with delayed AI title', async () => {
			...
			singularityApiService.responsePromise = new Promise(resolve => { resolveTitle = resolve; });
			const { svc, session, db } = await setupTitleGeneration(singularityApiService);
			...
			await waitForCondition(() => singularityApiService.utilityCalls.length === 1, 'title generation should be in flight');
		...
		test('aborts pending AI title generation when session is disposed', async () => {
			...
			assert.deepStrictEqual({
				aborted: singularityApiService.utilityCalls[0].options?.signal?.aborted,
		...
		test('generates an AI title for forked sessions from the forked chat', async () => {
			const singularityApiService = new TestSingularityApiService();
			singularityApiService.response = 'Source generated title';
			const { svc, session: sourceSession } = await setupTitleGeneration(singularityApiService);
			...
			singularityApiService.response = 'Forked branch title';
			...
			const forkedCall = singularityApiService.utilityCalls[singularityApiService.utilityCalls.length - 1];
			...
			assert.deepStrictEqual({
				...
				utilityCalls: singularityApiService.utilityCalls.length,
	...
	suite('createSession', () => {
		...
		test('creates session via specified provider', async () => {
			...
			const session = await service.createSession({ provider: 'singularity' });
			assert.strictEqual(AgentSession.provider(session), 'singularity');
		});
		...
		test('honors requested session URI', async () => {
			...
			const requestedSession = AgentSession.uri('singularity', 'requested-session');
			const session = await service.createSession({ provider: 'singularity', session: requestedSession });
	...
	suite('disposeSession', () => {
		...
		test('dispatches to the correct provider and cleans up tracking', async () => {
			...
			assert.strictEqual(singularityAgent.disposeSessionCalls.length, 1);
		});
		...
		test('deletes session data before removing the worktree', async () => {
			...
			svc.registerProvider(singularityAgent);
			const session = await svc.createSession({ provider: 'singularity' });
	...
	suite('aggregation', () => {
		...
		test('listSessions aggregates sessions from all providers', async () => {
			...
			await service.createSession({ provider: 'singularity' });
		...
		test('listSessions overlays custom title from session database', async () => {
			...
			const sessionUri = AgentSession.uri('singularity', sessionId);
		...
		test('listSessions uses SDK title when no custom title exists', async () => {
			service.registerProvider(singularityAgent);
			singularityAgent.sessionMetadataOverrides = { summary: 'Auto-generated Title' };
		...
		test('listSessions never returns subagent sessions', async () => {
			service.registerProvider(singularityAgent);
			const parentSession = await service.createSession({ provider: 'singularity' });
		...
		test('listSessions overlay excludes idle provisional sessions but keeps ones with an active turn (#321269)', async () => {
			...
			const provisionalAgent = new ProvisionalMockAgent('singularity');
			disposables.add(toDisposable(() => provisionalAgent.dispose()));
		...
		test('listSessions overlays live workspace metadata over a stale provider snapshot', async () => {
			...
			const agent = new DelayedListAgent('singularity');
			disposables.add(toDisposable(() => agent.dispose()));
		...
		test('createSession attaches git state into state _meta when working directory is present', async () => {
			...
			const session = await localService.createSession({ provider: 'singularity', workingDirectories: workingDirectory ? [workingDirectory] : undefined });
		...
		test.skip('createSession refreshes branch and uncommitted changesets after git state attaches', async () => {
			...
			const session = await localService.createSession({ provider: 'singularity' });
		...
		test('subscribe to an unknown changeset id fails without restoring the parent session', async () => {
			...
			// changeset URI.
			const sessionUri = URI.from({ scheme: 'singularity', path: '/missing-session' }).toString();
		...
		test('createSession stores live session config', async () => {
			...
			const session = await service.createSession({ provider: 'singularity', config });
		...
		test('seeds activeClient into the initial session state when provided', async () => {
			...
			const session = await service.createSession({ provider: 'singularity', activeClient });
	...
	suite('authenticate', () => {
		...
		test('routes token to provider matching the resource', async () => {
			...
			assert.deepStrictEqual(singularityAgent.authenticateCalls, [{ resource: 'https://api.github.com', token: 'ghp_test123' }]);
		});
		...
		test('returns not authenticated for unknown resource', async () => {
			...
			assert.deepStrictEqual({ result, token: service.getAuthToken({ resource: 'https://unknown.example.com' }), authenticateCalls: singularityAgent.authenticateCalls }, {
		...
		test('stores GitHub Singularity token for operation handlers', async () => {
			service.registerProvider(singularityAgent);

			const result = await service.authenticate({ resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, token: 'singularity-token' });

			assert.deepStrictEqual({ result, token: service.getAuthToken({ resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, scopes: GITHUB_SINGULARITY_PROTECTED_RESOURCE.scopes_supported }), authenticateCalls: singularityAgent.authenticateCalls }, {
				result: { authenticated: true },
				token: 'singularity-token',
				authenticateCalls: [{ resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, token: 'singularity-token' }],
			});
		});
		...
		test('stores tokens for the same resource by scopes', async () => {
			...
			await service.authenticate({ resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, scopes: ['read:user'], token: 'read-token' });
			await service.authenticate({ resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, scopes: ['read:user', 'user:email'], token: 'profile-token' });

			assert.deepStrictEqual({
				readToken: service.getAuthToken({ resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, scopes: ['read:user'] }),
				profileToken: service.getAuthToken({ resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, scopes: ['user:email', 'read:user'] }),
				supersetToken: service.getAuthToken({ resource: GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, scopes: ['user:email'] }),
			}, {
		...
		test('fans out to every provider that owns the resource', async () => {
			...
			// motivating example: both Singularity CLI and Claude consume the
			// GitHub Singularity token). Both must see the token — the
			...
			assert.deepStrictEqual({
				result,
				singularityCalls: singularityAgent.authenticateCalls,
				claudeCalls: claudeAgent.authenticateCalls,
			}, {
				result: { authenticated: true },
				singularityCalls: [{ resource: 'https://api.github.com', token: 'tok' }],
	...
	suite('shutdown', () => {
		...
		test('shuts down all providers', async () => {
			let singularityShutdown = false;
			singularityAgent.shutdown = async () => { singularityShutdown = true; };

			service.registerProvider(singularityAgent);

			await service.shutdown();
			assert.ok(singularityShutdown);
		});
	});
	...
	suite('restoreSession', () => {
		...
		test('restores the AH-owned workspaceless marker onto the summary _meta for any agent', async () => {
			...
			const db = new TestSessionDatabase();
			const localService = disposables.add(new AgentService(new NullLogService(), fileService, createSessionDataService(db), { _serviceBrand: undefined } as IProductService, createNoopGitService()));
			localService.registerProvider(singularityAgent);
			await singularityAgent.createSession();
			const sessionResource = (await singularityAgent.listSessions())[0].session;
			singularityAgent.sessionMessages = [];
			await db.setMetadata('agentHost.workspaceless', 'true');
		...
		test('restores persisted multi-root metadata', async () => {
			...
			const sessionResource = (await singularityAgent.listSessions())[0].session;
			singularityAgent.sessionMessages = [];
			singularityAgent.sessionMetadataOverrides = {
				_meta: { multiRoot: { workspaceFile: 'file:///provider-spoof.code-workspace' } },
			};
			const multiRoot = {
				workspaceFile: 'vscode-remote://ssh-remote+host/work/demo.code-workspace',
		...
		test('restores a session with message history', async () => {
			service.registerProvider(singularityAgent);
			const { session } = await singularityAgent.createSession();
			const sessions = await singularityAgent.listSessions();
		...
		test('re-attaches persisted turn usage on restore', async () => {
			// Providers don't durably record token/credit usage (the Singularity
		...
		test('re-attaches usage over an Auto-model stub, preserving the routing metadata', async () => {
			// A turn that ran on Singularity Auto is restored with a token-less stub
			...
			const agent = disposables.add(new MockAgent('singularity'));
			...
			await db.setTurnUsage('msg-1', JSON.stringify({ inputTokens: 100, outputTokens: 20, model: 'claude-opus-4.8', _meta: { singularityUsage: { totalNanoAiu: 5_000_000_000 } } }));
			...
			assert.deepStrictEqual(
				...
				[{
					...
					_meta: { autoModeResolved, singularityUsage: { totalNanoAiu: 5_000_000_000 } },
				}],
			);
		});
		...
		test('restores chat drafts from session metadata', async () => {
			...
			(singularityAgent as MockAgent & { getChatDraft(chat: URI): Promise<typeof draft | undefined> }).getChatDraft = chat => db.getChatDraft(chat) as Promise<typeof draft | undefined>;
			singularityAgent.sessionMessages = [];
		...
		test('throws when session is not found on backend', async () => {
			service.registerProvider(singularityAgent);
			await assert.rejects(
				() => service.restoreSession(AgentSession.uri('singularity', 'nonexistent')),
		...
		test('restores known session without listing all provider sessions', async () => {
			...
			singularityAgent.listSessions = async () => {
		...
		test('falls back to listing sessions when direct metadata restore fails', async () => {
			...
			singularityAgent.getSessionMetadata = async () => {
				throw new Error('direct metadata unavailable');
			};
			const originalListSessions = singularityAgent.listSessions.bind(singularityAgent);
			let listSessionsCalled = false;
			singularityAgent.listSessions = async () => {
				listSessionsCalled = true;
		...
		test('coalesces concurrent restores for the same session', async () => {
			...
			const agent = disposables.add(new BlockingRestoreAgent('singularity'));
			service.registerProvider(agent);
			const { session } = await agent.createSession();
			service.stateManager.deleteSession(session.toString());
			agent.sessionMessages = [
		...
		test('hydrates session customizations when restoring an existing session', async () => {
			...
			singularityAgent.getSessionCustomizations = async () => {
		...
		test('clears failed restore attempts so sessions can be retried', async () => {
			...
			const agent = disposables.add(new FailingOnceRestoreAgent('singularity'));
			service.registerProvider(agent);
			const { session } = await agent.createSession();
			service.stateManager.deleteSession(session.toString());
			agent.sessionMessages = [
		...
		test('inner assistant messages from subagent do not create extra turns (fixture)', async () => {
			...
			// Load real SDK events from fixture (sanitized from ~/.singularity/session-state/)
			singularityAgent.sessionMessages = await loadFixtureMessages('subagent-session.jsonl', session);
		...
		test('eagerly registers subagent child sessions during parent restore', async () => {
			...
			const agent = new EagerSubagentMockAgent('singularity');
			disposables.add(toDisposable(() => agent.dispose()));
			service.registerProvider(agent);
			const { session } = await agent.createSession();
			const sessions = await agent.listSessions();
		...
		test('inner assistant messages from subagent route via envelope agentId (fixture)', async () => {
			...
			singularityAgent.sessionMessages = await loadFixtureMessages('subagent-session-agentid.jsonl', session);
		...
		test('coalesces concurrent restores for the same subagent session', async () => {
			...
			const agent = disposables.add(new BlockingSubagentAgent('singularity'));
			service.registerProvider(agent);
			const { session } = await agent.createSession();
			const sessions = await agent.listSessions();
			const sessionResource = sessions[0].session;
	...
	suite('createChat', () => {
		...
		test('routes to the provider for a restored session not tracked in the provider map', async () => {
			...
			const agent = disposables.add(new MultiChatAgent('singularity'));
			service.registerProvider(agent);
			const { session } = await agent.createSession();
		...
		test('a peer chat backing session is filtered out of listSessions and stays filtered across a restart', async () => {
			...
			const backingUri = AgentSession.uri('singularity', backingSdkId).toString();
			...
			const agent = disposables.add(new LeakyMultiChatAgent('singularity'));
			...
			const restartAgent = disposables.add(new LeakyMultiChatAgent('singularity'));
			...
			assert.deepStrictEqual({
				...
				markerPersisted: await dbFor(AgentSession.uri('singularity', backingSdkId)).getMetadata('peerChatBacking'),
	...
	suite('createChat side chats', () => {
		...
		test('rejects a side chat whose source turn does not exist', async () => {
			const agent = disposables.add(new SideChatAgent('singularity'));
			service.registerProvider(agent);
			const session = await service.createSession({ provider: 'singularity' });
		...
		test('rejects a side chat whose source chat is in a different session', async () => {
			const agent = disposables.add(new SideChatAgent('singularity'));
			service.registerProvider(agent);
			const sessionA = await service.createSession({ provider: 'singularity' });
			const sessionB = await service.createSession({ provider: 'singularity' });
	...
	suite('chat surface routing', () => {
		...
		test('createSession/createChat/disposeChat/disposeSession prefer the chat surface over legacy methods', async () => {
			const agent = disposables.add(new ChatSurfaceAgent('singularity'));
			service.registerProvider(agent);
	...
	suite('spawn channel routing', () => {
		...
		test('onDidSpawnChat adds the chat to the catalog with a Tool origin from its parent', async () => {
			const agent = disposables.add(new SpawnChannelAgent('singularity'));
			service.registerProvider(agent);
			const session = await service.createSession({ provider: 'singularity' });
	...
	suite('subagent membership sequencing', () => {
		...
		test('membership stays a single entry when the agent also mirrors the subagent onto onDidSpawnChat, regardless of order', async () => {
			// Mirror the real singularity/claude agents, which ALSO bridge their
			...
			const agent = new BridgingSubagentAgent('singularity');
			disposables.add(toDisposable(() => agent.dispose()));
			service.registerProvider(agent);
			const session = await service.createSession({ provider: 'singularity' });
			const parentChat = buildDefaultChatUri(session.toString());
		...
		test('an inner tool call arriving before subagent_started is buffered and drained onto the subagent chat', async () => {
			...
			singularityAgent.fireProgress({ kind: 'action', resource: URI.parse(parentChat), action: { type: ActionType.ChatToolCallStart, turnId: 'turn-1', toolCallId: 'tc-sub', toolName: 'task', displayName: 'Task', contributor: undefined, _meta: { toolKind: undefined, language: undefined } } });
			singularityAgent.fireProgress({ kind: 'action', resource: URI.parse(parentChat), action: { type: ActionType.ChatToolCallReady, turnId: 'turn-1', toolCallId: 'tc-sub', invocationMessage: 'Delegating...', toolInput: undefined, confirmed: ToolCallConfirmationReason.NotNeeded } });

			// Inner tool arrives BEFORE subagent_started (buffered).
			singularityAgent.fireProgress({ kind: 'action', resource: URI.parse(parentChat), parentToolCallId: 'tc-sub', action: { type: ActionType.ChatToolCallStart, turnId: 'turn-1', toolCallId: 'inner-1', toolName: 'read', displayName: 'Read', contributor: undefined, _meta: { toolKind: undefined, language: undefined } } });
			singularityAgent.fireProgress({ kind: 'action', resource: URI.parse(parentChat), parentToolCallId: 'tc-sub', action: { type: ActionType.ChatToolCallReady, turnId: 'turn-1', toolCallId: 'inner-1', invocationMessage: 'Reading...', toolInput: undefined, confirmed: ToolCallConfirmationReason.NotNeeded } });

			// subagent_started arrives and drains the buffer.
			singularityAgent.fireProgress({ kind: 'subagent_started', chat: URI.parse(parentChat), toolCallId: 'tc-sub', agentName: 'explore', agentDisplayName: 'Explore', agentDescription: 'Explores' });
		...
		test('a subagent chat survives subagent_completed (stays live and subscribable, its turn completed)', async () => {
			...
			singularityAgent.fireProgress({ kind: 'subagent_completed', chat: URI.parse(parentChat), toolCallId: 'tc-sub' });
	...
	suite('peer chat catalog persistence', () => {
		...
		test('get_session_context resolves a restored peer chat before reading its transcript', async () => {
			...
			const result = await agent.serverToolHost!.executeTool(
				...
				{ session: `agent-host-session://singularity/${AgentSession.id(session)}?chat=peer-1` },
			);
		...
		test('invalidates a restored peer resolver when its parent session is disposed', async () => {
			...
			const agent = disposables.add(new MultiChatAgent('singularity'));
			localService.registerProvider(agent);
			const session = AgentSession.uri('singularity', 'reused-session');
			await localService.createSession({ provider: 'singularity', session });
			const peerUri = URI.parse(buildChatUri(session, 'peer-1'));
			await db.setMetadata('peerChats', JSON.stringify([{ uri: peerUri.toString(), providerData: 'blob-1' }]));
			localService.stateManager.deleteSession(session.toString());
			await localService.restoreSession(session);
		...
		test('legacy *.chats with no peerChats catalog migrates once into the orchestrator catalog', async () => {
			...
			const db = new TestSessionDatabase();
			const localService = disposables.add(new AgentService(new NullLogService(), fileService, createSessionDataService(db), { _serviceBrand: undefined } as IProductService, createNoopGitService()));
			const agent = disposables.add(new LegacyAgent('singularity'));
			localService.registerProvider(agent);
			const session = await localService.createSession({ provider: 'singularity' });
	...
	suite('subscriber refcount eviction', () => {
		...
		test('an empty session created in this lifetime stays observable until GC fires', async () => {
			service.registerProvider(singularityAgent);
			const sessionResource = await service.createSession({ provider: 'singularity' });
		...
		test('a restored idle session is evicted when its last subscriber drops', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					singularityAgent.releaseSessionCalls.map(u => u.toString()),
					[sessionResource.toString()],
					'provider releaseSession should be invoked for the evicted root',
				);
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 0, 'eviction must not destructively dispose the session');
			});
		});
		...
		test('re-subscribing within the grace cancels the release', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(singularityAgent.releaseSessionCalls.length, 0, 'releaseSession must not fire when the grace was cancelled');
			});
		});
		...
		test('restore waits for provider release to finish', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				const agent = new DelayedReleaseMockAgent('singularity');
				service.registerProvider(agent);
				const { session } = await agent.createSession();
				const sessions = await agent.listSessions();
				const sessionResource = sessions[0].session;
	...
	suite('addSubscriber triggers uncommitted refresh', () => {
		...
		test('addSubscriber for <session>/changeset/uncommitted triggers the first git diff refresh', async () => {
			...
			singularityAgent.resolvedWorkingDirectory = workingDirectory;
			singularityAgent.sessionMetadataOverrides = { workingDirectories: workingDirectory ? [workingDirectory] : undefined };
			...
			const localService = disposables.add(new AgentService(new NullLogService(), fileService, sessionDataService, { _serviceBrand: undefined } as IProductService, gitService));
			localService.registerProvider(singularityAgent);
			const sessionResource = await localService.createSession({ provider: 'singularity' });
	...
	suite('empty-session GC', () => {
		...
		test('an empty unsubscribed session is disposed after the grace period', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 0, 'no GC before grace expires');

				// After the grace period, the session is disposed entirely.
				await new Promise(resolve => setTimeout(resolve, 30_000));
				assert.deepStrictEqual(
					singularityAgent.disposeSessionCalls.map(u => u.toString()),
					[sessionResource.toString()],
		...
		test('a session with at least one turn is not GC-disposed', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 0, 'session with turns must not be GC-disposed');
			});
		});
		...
		test('resubscribe within the grace period cancels GC', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 0, 'GC must be cancelled after resubscribe');
			});
		});
		...
		test('GC is rearmed after a resubscribe-then-unsubscribe cycle', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 0, 'rearmed timer not yet fired');
				await new Promise(resolve => setTimeout(resolve, 2_000));
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 1, 'rearmed timer fires after fresh 30s');
			});
		});
		...
		test('createSession on the same URI cancels a pending GC', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				service.registerProvider(singularityAgent);
				const sessionResource = await service.createSession({ provider: 'singularity', session: AgentSession.uri('singularity', 'recreate-test') });
				service.addSubscriber(sessionResource, 'client-1');
				service.unsubscribe(sessionResource, 'client-1');

				// Re-issue createSession mid-grace.
				await new Promise(resolve => setTimeout(resolve, 5_000));
				await service.createSession({ provider: 'singularity', session: AgentSession.uri('singularity', 'recreate-test') });

				// Wait past the original grace window. If GC wasn't
				// cancelled by createSession, dispose would have fired.
				await new Promise(resolve => setTimeout(resolve, 30_000));
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 0, 'createSession on same URI must cancel pending GC');
			});
		});
		...
		test('a restored session that loaded zero turns is never GC-disposed', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual({
					disposed: singularityAgent.disposeSessionCalls.map(u => u.toString()),
					released: singularityAgent.releaseSessionCalls.map(u => u.toString()),
				}, {
		...
		test('a session restored during the grace window is not GC-disposed', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 0, 'a session restored mid-grace must not be GC-disposed');
			});
		});
		...
		test('a session truncated back to zero turns is not GC-disposed', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(singularityAgent.disposeSessionCalls.length, 0, 'a used session must not be GC-disposed after truncation');
			});
		});
	});
	...
	suite('session config persistence', () => {
		...
		test('createSession persists initial config values to the session DB', async () => {
			...
			const sessionDataService = createSessionDataService(sessionDb);
			const localAgent = new MockAgent('singularity');
			...
			await localService.createSession({ provider: 'singularity', config: { autoApprove: 'autoApprove' } });
		...
		test('createSession does not write configValues when there are no values', async () => {
			...
			await localService.createSession({ provider: 'singularity' });
		...
		test('createSession + restoreSession round-trip restores initial config without any mid-session changes', async () => {
			...
			const session = await localService.createSession({ provider: 'singularity', config: { autoApprove: 'autoApprove' } });
	...
	suite('worktree working directory', () => {
		...
		test('createSession uses agent-resolved working directory in state', async () => {
			...
			singularityAgent.resolvedWorkingDirectory = worktreeDir;
			service.registerProvider(singularityAgent);

			const sourceDir = URI.file('/source/repo');
			const session = await service.createSession({ provider: 'singularity', workingDirectories: [sourceDir] });
		...
		test('createSession falls back to config working directory when agent does not resolve', async () => {
			...
			singularityAgent.resolvedWorkingDirectory = undefined;
			service.registerProvider(singularityAgent);
		...
		test('restoreSession uses agent working directory in state', async () => {
			...
			const worktreeDir = URI.file('/source/repo.worktrees/agents-xyz');
			singularityAgent.sessionMetadataOverrides = { workingDirectories: worktreeDir ? [worktreeDir] : undefined };
			service.registerProvider(singularityAgent);
		...
		test('_resolveWorkingDirectoryBeforeSend returns the full set (index 0 + tail), or undefined when unset', async () => {
			...
			const multi = URI.from({ scheme: 'singularity', path: '/multi' }).toString();
			const single = URI.from({ scheme: 'singularity', path: '/single' }).toString();
			const none = URI.from({ scheme: 'singularity', path: '/none' }).toString();
			inject(multi, [a, b, c]);
			inject(single, [a]);
		...
		test('first-send worktree failure warns and falls back to the original folder', async () => {
			...
			const session = AgentSession.uri('singularity', 'worktree-failure');
			...
			assert.deepStrictEqual({
				...
				persistedFailure: JSON.parse((await database.getMetadata('singularity.worktree.creationFailure'))!),
			}, {
		...
		test('first-send worktree fallback warns when no repository root is resolved', async () => {
			...
			const session = AgentSession.uri('singularity', 'worktree-fallback');
			const sessionResource = session.toString();
			const chat = buildDefaultChatUri(sessionResource);
			localService.stateManager.restoreSession({
				resource: sessionResource,
	...
	suite.skip('item-2: initial changeset seeding at create time', () => {
		...
		test('forked createSession seeds both halves on the forked session', async () => {
			...
			const sourceSession = await service.createSession({ provider: 'singularity' });
		...
		test('provisional session materialization preserves both halves', async () => {
			...
			// flow that Singularity CLI / Claude actually use in production.
			class ProvisionalMockAgent extends MockAgent {
