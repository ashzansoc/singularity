...
import { SINGULARITY_API_ERROR_STATUS_STREAMING, SingularityApiError } from '../../node/shared/singularityApiService.js';
import {
...
function makeApiError(status: number, type: string, message: string, requestId: string | null = 'req-1'): SingularityApiError {
	const envelope: Anthropic.ErrorResponse = {
		type: 'error',
		error: { type: type as Anthropic.ErrorResponse['error']['type'], message },
		request_id: requestId,
	};
	return new SingularityApiError(status, envelope);
}
...
suite('forwardedChatError', () => {
	...
	suite('buildForwardedChatError', () => {
		...
		test('mid-stream streaming sentinel maps to failed', () => {
			const forwarded = buildForwardedChatError(makeApiError(SINGULARITY_API_ERROR_STATUS_STREAMING, 'api_error', 'mid-stream boom'));
	...
	suite('buildForwardedChatErrorFromFields', () => {
		...
		test('maps Singularity SDK error categories to fetch types', () => {
			const actual = [
		...
		test('defaults a quota error without an explicit code to quota_exceeded so the plan message renders', () => {
			// The Singularity CLI SDK reports quota errors only via `errorType: 'quota'`
