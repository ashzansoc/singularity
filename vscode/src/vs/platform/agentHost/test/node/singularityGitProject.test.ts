...
import { projectFromSingularityContext, projectFromRepository, resolveGitProject } from '../../node/singularity/singularityGitProject.js';
...
suite('Singularity Git Project', () => {
	...
	test('falls back to repository context when no git project is available', async () => {
		const project = await projectFromSingularityContext({ repository: 'microsoft/vscode' }, gitService);
