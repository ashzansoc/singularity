...
import type { CopilotClient as SingularityClient, CopilotClientOptions as SingularityClientOptions, CopilotSession as SingularitySession, GitHubTelemetryNotification, PermissionAllowAllMode, PermissionRequest, SessionEvent, SessionEventHandler, SessionEventPayload, SessionEventType, TypedSessionEventHandler } from '@github/copilot-sdk';
...
import { SingularityCliConfigKey } from '../../common/singularityCliConfig.js';
import { AgentHostSingularityMultiRootEnabledConfigKey, AgentHostMigrateLegacySingularityCliEnabledConfigKey, AgentHostPreferLongContextEnabledConfigKey, AgentHostSystemProxyEnabledConfigKey } from '../../common/agentHostSchema.js';
import { IAgentPluginManager, ISyncedCustomization } from '../../common/agentPluginManager.js';
import { getTelemetryChatSessionId } from '../../common/agentTelemetryCorrelation.js';
import { AgentSession, GITHUB_SINGULARITY_PROTECTED_RESOURCE, type AgentSignal, type IAgentCreateChatForkSource, type IAgentSessionMetadata, type IAgentSpawnChatEvent } from '../../common/agentService.js';
...
import { SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE, SingularityAgent, SingularitySessionEntry, rebaseUnder, REFRESH_DEBOUNCE_MS, resolveSingularityOtlpMetricsEndpoint } from '../../node/singularity/singularityAgent.js';
import { SINGULARITY_AGENT_HOST_FILE_LINK_INSTRUCTIONS } from '../../node/singularity/prompts/systemMessage.js';
...
import { SingularityAgentSession } from '../../node/singularity/singularityAgentSession.js';
...
import type { SingularitySessionLaunchPlan, IActiveClientSnapshot } from '../../node/singularity/singularitySessionLauncher.js';
import { ShellManager } from '../../node/singularity/singularityShellTools.js';
import { registerPendingEditContentProvider } from '../../node/singularity/pendingEditContentStore.js';
import { SessionDatabase } from '../../node/sessionDatabase.js';
import { createNullSessionDataService } from '../common/sessionTestHelpers.js';
import { ActiveClientToolSet } from '../../node/activeClientState.js';
import { ByokLmBridgeRegistry, IByokLmBridgeRegistry } from '../../node/byokLmBridgeRegistry.js';
import { ISingularityApiService, type ISingularityApiServiceRequestOptions, type ISingularityUtilityChatCompletionRequest, type IRestrictedTelemetryContext } from '../../node/shared/singularityApiService.js';
 ...
 * live inside the owning session's {@link SingularitySessionEntry}, keyed by chat URI
 ...
 * container internals.
 */
function sessionsMap(agent: SingularityAgent): Map<string, SingularitySessionEntry> {
	return (agent as unknown as { _sessions: Map<string, SingularitySessionEntry> })._sessions;
}

function sdkSessionsMap(agent: SingularityAgent): Map<string, SingularityAgentSession> {
	return (agent as unknown as { _sdkSessionsById: Map<string, SingularityAgentSession> })._sdkSessionsById;
}
...
function setDefaultSessionStub(agent: SingularityAgent, sessionId: string, stub: unknown): void {
	const sessions = sessionsMap(agent);
	const defaultChatKey = buildDefaultChatUri(AgentSession.uri('singularitycli', sessionId).toString());
	let entry = sessions.get(sessionId);
	if (!entry) {
		entry = new SingularitySessionEntry();
		sessions.set(sessionId, entry);
	}
	entry.setDefaultChat(defaultChatKey, new SingularitySessionEntry(stub as SingularityAgentSession));
}

/** Inject a peer-chat stub into its owning session's entry (creating the entry if needed). */
function setPeerChatStub(agent: SingularityAgent, chatUri: URI, stub: unknown): void {
	...
	entry.registerPeerChat(chatUri.toString(), new SingularitySessionEntry(stub as SingularityAgentSession));
}

/** Resolve a peer-chat stub from its owning session's entry. */
function getPeerChatStub(agent: SingularityAgent, chatUri: URI): SingularityAgentSession | undefined {
	const sessionId = AgentSession.id(URI.parse(parseRequiredSessionUriFromChatUri(chatUri)));
...
function hasPeerChatStub(agent: SingularityAgent, chatUri: URI): boolean {
	const sessionId = AgentSession.id(URI.parse(parseRequiredSessionUriFromChatUri(chatUri)));
...
function peerChatCount(agent: SingularityAgent): number {
	let count = 0;
...
class TestSingularityApiService implements ISingularityApiService {
	declare readonly _serviceBrand: undefined;

	readonly utilityCalls: { token: string; request: ISingularityUtilityChatCompletionRequest; options?: ISingularityApiServiceRequestOptions }[] = [];
	...
	messages(_githubToken: string, _request: Anthropic.MessageCreateParamsStreaming, _options?: ISingularityApiServiceRequestOptions): AsyncGenerator<Anthropic.MessageStreamEvent>;
	messages(_githubToken: string, _request: Anthropic.MessageCreateParamsNonStreaming, _options?: ISingularityApiServiceRequestOptions): Promise<Anthropic.Message>;
	...
	async utilityChatCompletion(githubToken: string, request: ISingularityUtilityChatCompletionRequest, options?: ISingularityApiServiceRequestOptions): Promise<string> {
...
type SingularityModelsList = SingularityClient['rpc']['models']['list'];
type SingularityModelInfo = Awaited<ReturnType<SingularityModelsList>>['models'][number];

interface ITestSingularityModelInfo {
	readonly id: string;
	...
	readonly policy?: { readonly state?: NonNullable<SingularityModelInfo['policy']>['state'] };
	readonly billing?: SingularityModelInfo['billing'];
	readonly modelPickerCategory?: SingularityModelInfo['modelPickerCategory'];
	readonly modelPickerPriceCategory?: SingularityModelInfo['modelPickerPriceCategory'];
	readonly supportedReasoningEfforts?: SingularityModelInfo['supportedReasoningEfforts'];
}

interface ITestSingularityClient extends Pick<SingularityClient, 'start' | 'stop' | 'listSessions' | 'createSession' | 'resumeSession' | 'getSessionMetadata' | 'deleteSession'> {
	readonly rpc: {
		readonly sessions: { readonly fork: SingularityClient['rpc']['sessions']['fork'] };
		readonly models: { readonly list: SingularityModelsList };
	};
}

function toSdkModelInfo(model: ITestSingularityModelInfo): SingularityModelInfo {
	return {
		id: model.id,
		name: model.name,
		capabilities: {
...
class TestSingularityClient implements ITestSingularityClient {
	readonly rpc: ITestSingularityClient['rpc'] = {
	...
	readonly modelListRequests: Parameters<SingularityModelsList>[0][] = [];
	...
	readonly modelListResponses: ITestSingularityModelInfo[][] = [];
	readonly getSessionMetadataCalls: string[] = [];
	readonly deletedSessionIds: string[] = [];

	constructor(
		private readonly _sessions: Awaited<ReturnType<ITestSingularityClient['listSessions']>>,
		private readonly _models: readonly ITestSingularityModelInfo[] = [],
	) { }
	...
	async stop(): ReturnType<ITestSingularityClient['stop']> {
		this.stopCallCount++;
		return [];
	}
	async listSessions(): ReturnType<ITestSingularityClient['listSessions']> {
		this.listSessionCallCount++;
		return this._sessions;
	}
	async getSessionMetadata(sessionId: string): ReturnType<ITestSingularityClient['getSessionMetadata']> {
	...
	createSession: ITestSingularityClient['createSession'] = async () => { throw new Error('not implemented'); };
	resumeSession: ITestSingularityClient['resumeSession'] = async () => { throw new Error('not implemented'); };
}
...
class ResumePathSingularityAgent extends SingularityAgent {
	constructor(
		private readonly _singularityClient: ITestSingularityClient,
		...
		@ISingularityApiService singularityApiService: ISingularityApiService,
	) {
		super(logService, instantiationService, sessionDataService, gitService, configurationService, stateManager, createTestGitHubEndpointService(), new MockAgentHostOTelService(), completions, NULL_CHECKPOINT_SERVICE, NULL_REVIEW_SERVICE, environmentService, byokBridgeRegistry, telemetryService, singularityApiService, proxyResolver);
	}

	protected override _createSingularityClient(): SingularityClient {
		return this._singularityClient as SingularityClient;
	}
}

class TestableSingularityAgent extends SingularityAgent {
	private readonly _fakeSessions = new Map<string, IFakeAgentSession>();
	readonly resumeCalls: string[] = [];
	readonly createdClientOptions: SingularityClientOptions[] = [];
	...
	protected override _createSingularityClient(options: SingularityClientOptions): SingularityClient {
	...
	protected override async _resumeSession(sessionId: string, _workingDirectories?: readonly URI[]): Promise<SingularityAgentSession> {
		...
		const sessionUri = AgentSession.uri('singularitycli', sessionId);
		...
		} as unknown as SingularityAgentSession;
		return stub;
	}
}

function getCreatedClientOptions(agent: SingularityAgent): readonly SingularityClientOptions[] {
	assert.ok(agent instanceof TestableSingularityAgent);
	return agent.createdClientOptions;
}

function createTestAgentContext(disposables: Pick<DisposableStore, 'add'>, options?: { sessionDataService?: ISessionDataService; singularityClient?: ITestSingularityClient; useRealResumePath?: boolean; gitService?: TestAgentHostGitService; environmentServiceRegistration?: 'native' | 'none'; pluginManager?: IAgentPluginManager; fileService?: FileService; singularityApiService?: ISingularityApiService; gitHubEndpointService?: IAgentHostGitHubEndpointService; telemetryService?: ITelemetryService; userHome?: URI; logService?: ILogService; proxyResolver?: IAgentHostProxyResolver; byokBridgeRegistry?: IByokLmBridgeRegistry }): { agent: SingularityAgent; instantiationService: IInstantiationService; configurationService: IAgentConfigurationService; fileService: FileService; stateManager: AgentHostStateManager } {
	...
	const singularityApiService = options?.singularityApiService ?? new TestSingularityApiService();
	services.set(ISingularityApiService, singularityApiService);
	...
	const agent = options?.singularityClient
		? instantiationService.createInstance(options.useRealResumePath ? ResumePathSingularityAgent : TestableSingularityAgent, options.singularityClient)
		: instantiationService.createInstance(SingularityAgent);
	return { agent, instantiationService, configurationService: configService, fileService, stateManager };
}

function createTestAgent(disposables: Pick<DisposableStore, 'add'>, options?: { sessionDataService?: ISessionDataService; singularityClient?: ITestSingularityClient; useRealResumePath?: boolean; gitService?: TestAgentHostGitService; environmentServiceRegistration?: 'native' | 'none'; pluginManager?: IAgentPluginManager; fileService?: FileService; singularityApiService?: ISingularityApiService; gitHubEndpointService?: IAgentHostGitHubEndpointService; telemetryService?: ITelemetryService; userHome?: URI; logService?: ILogService; byokBridgeRegistry?: IByokLmBridgeRegistry }): SingularityAgent {
	return createTestAgentContext(disposables, options).agent;
}

type SingularityCreateSessionOptions = Parameters<SingularityClient['createSession']>[0];

function createAgentSessionThroughAgent(agent: SingularityAgent, instantiationService: IInstantiationService, options?: { readonly mockSession?: MockSingularitySession; readonly activeClientToolSet?: ActiveClientToolSet; readonly snapshot?: IActiveClientSnapshot }): { readonly session: SingularityAgentSession; readonly createOptions: () => SingularityCreateSessionOptions | undefined } {
	const sessionUri = AgentSession.uri('singularitycli', 'test-session-1');
	const shellManager = instantiationService.createInstance(ShellManager, sessionUri, undefined);
	let createOptions: SingularityCreateSessionOptions | undefined;
	const mockSession = options?.mockSession ?? new MockSingularitySession();
	const launchPlan: SingularitySessionLaunchPlan = {
		kind: 'create',
		client: {
			createSession: async (options: SingularityCreateSessionOptions) => {
				createOptions = options;
				return mockSession as unknown as SingularitySession;
			},
			resumeSession: async () => mockSession as unknown as SingularitySession,
		},
	...
	const agentInternals = (agent as unknown as {
		...
		_createAgentSession: (launchPlan: SingularitySessionLaunchPlan, customizationDirectory: URI | undefined, activeClient: unknown) => SingularityAgentSession;
	});
...
function sdkSession(sessionId: string, cwd?: string): Awaited<ReturnType<ITestSingularityClient['listSessions']>>[number] {
	return {
		sessionId,
...
async function disposeAgent(agent: SingularityAgent): Promise<void> {
	await agent.shutdown();
	agent.dispose();
	// SingularityAgent.dispose calls super.dispose() from a promise continuation so
...
suite('SingularityAgent', () => {
	const disposables = ensureNoDisposablesAreLeakedInTestSuite();

	test('installs the GitHub telemetry callback in SingularityClientOptions', async () => {
		const client = new TestSingularityClient([]);
		const agent = createTestAgent(disposables, { singularityClient: client }) as TestableSingularityAgent;
		try {
	...
	test('routes exact legacy targets exclusively and falls back to generic forwarding', async () => {
		const client = new TestSingularityClient([]);
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.restrictedTelemetryContexts.set('restricted-token', {
		...
		const agent = createTestAgent(disposables, { singularityClient: client, singularityApiService, telemetryService }) as TestableSingularityAgent;
		try {
			...
			assert.deepStrictEqual({
				...
				generic: ['singularityCli/unknown_restricted', 'singularityCli/tool_call_executed'],
	...
	test('routes exact targets using the current auth token and reflects token changes', async () => {
		const client = new TestSingularityClient([]);
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.restrictedTelemetryContexts.set('token-a', {
			restrictedTelemetryEnabled: true,
		...
		singularityApiService.restrictedTelemetryContexts.set('token-b', {
			restrictedTelemetryEnabled: false,
	...
	test('advertises Singularity as its display name', async () => {
		const agent = createTestAgent(disposables);
		try {
			assert.deepStrictEqual(agent.getDescriptor(), {
				provider: 'singularitycli',
				displayName: 'Singularity',
				description: 'Singularity SDK agent running in the local agent host process',
	...
	test('advertises multipleWorkingDirectories only when the hidden setting is enabled', async () => {
		...
		try {
			const setMultiRoot = (enabled: boolean) => stateManager.dispatchServerAction(ROOT_STATE_URI, {
				...
				config: { [AgentHostSingularityMultiRootEnabledConfigKey]: enabled },
			});
	...
	suite('spawned chat channel', () => {
		function fireSignal(agent: SingularityAgent, signal: AgentSignal): void {
		...
		test('mirrors subagent_started onto onDidSpawnChat; subagent_completed leaves the chat live', async () => {
			...
			try {
				const sessionUri = AgentSession.uri('singularitycli', 'spawn-session');
	...
	test('uses generated Agents-window Singularity CLI branch names', async () => {
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.response = 'add-agent-host-config';
		const generator = new AgentBranchNameGenerator(singularityApiService, new NullLogService());
		...
		assert.deepStrictEqual({
			...
			token: singularityApiService.utilityCalls[0]?.token,
			promptIncludesUserText: singularityApiService.utilityCalls[0]?.request.messages.some(message => message.content.includes('Add agent host config')),
		}, {
	...
	test('keeps generated branch names short', async () => {
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.response = 'a'.repeat(100);
		const generator = new AgentBranchNameGenerator(singularityApiService, new NullLogService());
	...
	test('falls back to first-message slug when generated branch name cannot be used', async () => {
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.response = '!!! ??? ...';
		const generator = new AgentBranchNameGenerator(singularityApiService, new NullLogService());
	...
	test('falls back to first-message slug when branch name generation fails', async () => {
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.error = new Error('failed');
		const generator = new AgentBranchNameGenerator(singularityApiService, new NullLogService());
	...
	test('contributes GHE-aware GitHub and discovered CAPI diagnostics endpoints', async () => {
		...
		const singularityApiService = new TestSingularityApiService();
		singularityApiService.apiEndpoint = 'https://singularity.example.com';
		singularityApiService.userLogin = 'octocat';
		const agent = createTestAgent(disposables, { singularityApiService, gitHubEndpointService: endpointService });
		try {
			await agent.authenticate(endpointService.getSingularityResource().resource, 'token');
			...
			assert.deepStrictEqual({
				...
				endpoints: [
					...
					{ name: 'Singularity API (CAPI)', url: 'https://singularity.example.com/_ping' },
				],
				account: 'octocat',
			});
		} finally {
	...
	test('returns empty models and lists sessions before authentication', async () => {
		...
		const ownedSession = AgentSession.uri('singularitycli', 'owned-before-auth');
		...
		await ownedDb.object.setMetadata('singularity.workingDirectory', URI.file('/workspace').toString());
		ownedDb.dispose();
		const client = new TestSingularityClient([sdkSession('owned-before-auth')]);
		const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client });
		try {
	...
	test('starts the client and creates a provisional session before authentication', async () => {
		const client = new TestSingularityClient([]);
		const agent = createTestAgent(disposables, { singularityClient: client });
		const session = AgentSession.uri('singularitycli', 'unauth-create');
	...
	test('recovers the client and reports telemetry when the SDK connection is closed', async () => {
		...
		const agent = createTestAgent(disposables, { singularityClient: client, telemetryService });
		try {
			await agent.authenticate('https://api.github.com', 'token');
			const models = await waitForState(agent.models, m => m.length > 0);
	...
	test('reports a classified Singularity client startup failure', async () => {
		const client = new TestSingularityClient([]);
	...
	test('coalesces closed connection recovery and preserves an already-cancelled turn', async () => {
		...
		class GatedStopClient extends TestSingularityClient {
			readonly stopGate = new DeferredPromise<void>();

			override async stop(): ReturnType<ITestSingularityClient['stop']> {
		...
		const telemetryService = new RecordingTelemetryService();
		const agent = createTestAgent(disposables, { singularityClient: client, telemetryService });
		const cancelledChat = defaultChatUri(AgentSession.uri('singularitycli', 'cancelled'));
		const failedChat = defaultChatUri(AgentSession.uri('singularitycli', 'failed'));
		const calls = {
		...
		setDefaultSessionStub(agent, 'cancelled', {
			sessionId: 'cancelled',
			sessionUri: AgentSession.uri('singularitycli', 'cancelled'),
		...
		setDefaultSessionStub(agent, 'failed', {
			sessionId: 'failed',
			sessionUri: AgentSession.uri('singularitycli', 'failed'),
			chatUri: failedChat,
		...
		try {
			...
			const failures = telemetryService.errorEvents
				.filter(event => event.eventName === 'agentHost.singularityClientFailure')
				.map(event => event.data as Record<string, unknown>);
			const recoveryTurns = telemetryService.errorEvents
				.filter(event => event.eventName === 'agentHost.singularityClientRecoveryTurnFailed')
				.map(event => event.data as Record<string, unknown>);
			const recovery = telemetryService.events[0].data as Record<string, unknown>;
	...
	test('reports but does not recover or discard for another classified abort failure', async () => {
		const telemetryService = new RecordingTelemetryService();
		const agent = createTestAgent(disposables, { singularityClient: new TestSingularityClient([]), telemetryService });
		const chat = defaultChatUri(AgentSession.uri('singularitycli', 'abort-failure'));
		let discardCount = 0;
	...
	test('createSession infers workspace-less from an omitted workingDirectory and uses a stable scratch dir', async () => {
		...
		try {
			...
			const result = await agent.createSession({
				session: AgentSession.uri('singularitycli', 'temp-fallback'),
			});
			...
			const expected = URI.joinPath(userHome, '.singularity', 'chats', 'temp-fallback');
	...
	suite('quick chat scratch directory', () => {
		test('resume recreates a reaped quick chat scratch dir (ensure-exists on restore)', async () => {
			...
			const session = AgentSession.uri('singularitycli', sessionId);
			const scratchDir = URI.joinPath(userHome, '.singularity', 'chats', sessionId);
			const sessionDataService = disposables.add(new TestSessionDataService());
			const db = sessionDataService.openDatabase(session);
			await db.object.setMetadata('singularity.workingDirectory', scratchDir.toString());
			await db.object.setMetadata('agentHost.workspaceless', 'true');
			db.dispose();
			const client = new TestSingularityClient([sdkSession(sessionId, scratchDir.fsPath)]);
			const agent = createTestAgent(disposables, { singularityClient: client, useRealResumePath: true, sessionDataService, userHome });
			const internals = agent as unknown as { _resumeSession: (id: string) => Promise<unknown> };
			try {
				await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'token');
		...
		test('disposeSession cleans up the quick chat scratch dir', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'qc-dispose');
				const result = await agent.createSession({ session });
				const scratchDir = URI.joinPath(userHome, '.singularity', 'chats', 'qc-dispose');
				await fs.access(scratchDir.fsPath);
	...
	suite('working-directory persistence', () => {
		...
		async function restore(seed: (db: ReturnType<TestSessionDataService['openDatabase']>) => Promise<void>, cwd?: string): Promise<{ list: string[] | undefined; meta: string[] | undefined }> {
			...
			await db.object.setMetadata('singularity.project.resolved', 'true');
			await seed(db);
			db.dispose();
			const client = new TestSingularityClient([sdkSession(sessionId, cwd)]);
			const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client });
			try {
				await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'token');
		...
		test('restores the persisted ordered set from singularity.workingDirectories', async () => {
			const result = await restore(async db => {
				await db.object.setMetadata('singularity.workingDirectories', JSON.stringify([repoA, repoB, repoC].map(d => d.toString())));
				await db.object.setMetadata('singularity.workingDirectory', repoA.toString());
			});
		...
		test('falls back to the legacy single working directory when the set is malformed', async () => {
			const result = await restore(async db => {
				await db.object.setMetadata('singularity.workingDirectories', 'not-json');
				await db.object.setMetadata('singularity.workingDirectory', repoA.toString());
			});
			assert.deepStrictEqual(result, {
				list: [repoA.toString()],
	...
	suite('restart on startup config change', () => {
		...
		class StopCountingClient extends TestSingularityClient {
			stopCount = 0;
		...
		test('passes the configured log level to the Singularity SDK client', async () => {
			const client = new TestSingularityClient([]);
			const { agent, configurationService } = createTestAgentContext(disposables, { singularityClient: client });
			try {
				configurationService.updateRootConfig({ [SingularityCliConfigKey.SingularitySdkLogLevel]: 'trace' });
				await agent.authenticate('https://api.github.com', 'token');
				await agent.listSessions();
		...
		test('uses info when neither the setting nor agent host enables trace', async () => {
			const client = new TestSingularityClient([]);
			const { agent } = createTestAgentContext(disposables, { singularityClient: client });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				await agent.listSessions();
		...
		test('uses trace when the agent host log level is trace', async () => {
			...
			const { agent } = createTestAgentContext(disposables, { singularityClient: client, logService });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				await agent.listSessions();
		...
		test('restarts the client when the Singularity SDK log level changes', async () => {
		...
		test('re-enumerates models after a startup-config restart', async () => {
			...
			try {
				...
				configurationService.updateRootConfig({ [SingularityCliConfigKey.RubberDuck]: true });
				for (let i = 0; i < 500 && client.modelListRequests.length <= requestsBefore; i++) {
					await timeout(1);
				}
		...
		test('does not restart when an unrelated config key changes', async () => {
			const client = new StopCountingClient([]);
			const logService = new MutableLogService();
			const { agent, configurationService } = createTestAgentContext(disposables, { singularityClient: client, logService });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				await agent.listSessions();

				logService.setLevel(LogLevel.Trace);
				configurationService.updateRootConfig({ [SingularityCliConfigKey.EnableCustomTerminalTool]: true });
				await Promise.resolve();
		 ...
		 */
		function reportChatTurnEnded(agent: SingularityAgent): void {
		...
		test('applies a deferred restart when the busy session is disposed instead', async () => {
			...
			try {
				...
				await agent.disposeSession(AgentSession.uri('singularitycli', 'busy'));
	...
	test('models include billing multiplier metadata when SDK provides it', async () => {
		const agent = createTestAgent(disposables, {
			singularityClient: new TestSingularityClient([], [{
				id: 'gpt-4o',
				name: 'GPT-4o',
	...
	suite('contextSize to contextTier mapping', () => {
		const longContextModel: ITestSingularityModelInfo = {
			id: 'claude-sonnet',
			name: 'Claude Sonnet',
			capabilities: { limits: { max_context_window_tokens: 200_000 } },
			billing: {
		...
		async function captureSessionConfig(model: ModelSelection | undefined, models: readonly ITestSingularityModelInfo[], preferLongContext?: boolean): Promise<SingularityCreateSessionOptions | undefined> {
			const sessionDataService = disposables.add(new TestSessionDataService());
			const client = new TestSingularityClient([], models);
			let capturedConfig: SingularityCreateSessionOptions | undefined;
			client.createSession = async config => {
				capturedConfig = config;
				return new MockSingularitySession() as unknown as SingularitySession;
			};
			const { agent, configurationService } = createTestAgentContext(disposables, { sessionDataService, singularityClient: client });
			try {
				...
				await waitForState(agent.models, m => m.length > 0);
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'ctx-session'),
		...
		test('leaves the SDK on its default tier when model has no surcharge and no explicit selection', async () => {
			const freeLongContextModel: ITestSingularityModelInfo = {
				id: 'free-long-ctx',
				name: 'Free Long Ctx',
				capabilities: { limits: { max_context_window_tokens: 200_000 } },
				billing: {
	...
	test('agent-created sessions can resolve session-state paths via INativeEnvironmentService', async () => {
		...
		const previousSingularityHome = process.env['SINGULARITY_HOME'];
		delete process.env['SINGULARITY_HOME'];
		try {
			...
			const result = await onPermissionRequest({
				kind: 'read',
				intention: 'read plan',
				path: URI.file('/mock-home/.singularity/session-state/test-session-1/plan.md').fsPath,
				toolCallId: 'tc-read-plan-agent-composition',
			}, { sessionId: 'test-session-1' });

			assert.strictEqual(result.kind, 'approve-once');
		} finally {
			if (previousSingularityHome === undefined) {
				delete process.env['SINGULARITY_HOME'];
			} else {
				process.env['SINGULARITY_HOME'] = previousSingularityHome;
			}
			await disposeAgent(agent);
		}
	});
	...
	test('client tool call contributor prefers the message sender when it provides the tool', async () => {
		...
		const mockSession = new MockSingularitySession();
		const createdSession = createAgentSessionThroughAgent(agent, instantiationService, {
			mockSession,
			activeClientToolSet,
	...
	test('listSessions only returns sessions with a database', async () => {
		const sessionDataService = disposables.add(new TestSessionDataService());
		const ownedSession = AgentSession.uri('singularitycli', 'owned');
		const ownedDb = sessionDataService.openDatabase(ownedSession);
		// A genuinely owned session persists a working directory at materialize;
		// listing gates on that (an empty DB is a ghost / un-migrated session).
		await ownedDb.object.setMetadata('singularity.workingDirectory', URI.file('/workspace').toString());
		...
		const ghostSession = AgentSession.uri('singularitycli', 'ghost');
		sessionDataService.openDatabase(ghostSession).dispose();

		const client = new TestSingularityClient([sdkSession('owned'), sdkSession('ghost'), sdkSession('external')]);
		const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client });
		try {
			await agent.authenticate('https://api.github.com', 'token');
	...
	test('listSessions reads stored metadata from sessions with a database', async () => {
		const sessionDataService = disposables.add(new TestSessionDataService());
		const legacySession = AgentSession.uri('singularitycli', 'legacy');
		const legacyDb = sessionDataService.openDatabase(legacySession);
		await legacyDb.object.setMetadata('singularity.workingDirectory', URI.file('/workspace').toString());
		legacyDb.dispose();

		const agent = createTestAgent(disposables, { sessionDataService, singularityClient: new TestSingularityClient([sdkSession('legacy')]) });
		try {
			await agent.authenticate('https://api.github.com', 'token');
	...
	test('listSessions does not itself re-emit the workspaceless tag (AgentService overlays it centrally)', async () => {
		const sessionDataService = disposables.add(new TestSessionDataService());
		const session = AgentSession.uri('singularitycli', 'quick');
		const db = sessionDataService.openDatabase(session);
		...
		await db.object.setMetadata('singularity.workingDirectory', URI.file('/scratch/quick').toString());
		await db.object.setMetadata('agentHost.workspaceless', 'true');
		db.dispose();

		const agent = createTestAgent(disposables, { sessionDataService, singularityClient: new TestSingularityClient([sdkSession('quick')]) });
		try {
			await agent.authenticate('https://api.github.com', 'token');
	...
	test('getSessionMetadata reads one SDK session and stored metadata without listing sessions', async () => {
		const sessionDataService = disposables.add(new TestSessionDataService());
		const session = AgentSession.uri('singularitycli', 'target');
		const db = sessionDataService.openDatabase(session);
		await db.object.setMetadata('singularity.workingDirectory', URI.file('/workspace').toString());
		db.dispose();

		const client = new TestSingularityClient([sdkSession('target')]);
		const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client });
		try {
			await agent.authenticate('https://api.github.com', 'token');
	...
	test('getSessionMetadata only returns sessions with a database', async () => {
		const sessionDataService = disposables.add(new TestSessionDataService());
		const session = AgentSession.uri('singularitycli', 'external');
		const client = new TestSingularityClient([sdkSession('external', '/workspace')]);
		const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client });
		try {
			await agent.authenticate('https://api.github.com', 'token');
	...
	test('listSessions does not create databases for unowned SDK sessions', async () => {
		const sessionDataService = disposables.add(new TestSessionDataService());
		const agent = createTestAgent(disposables, { sessionDataService, singularityClient: new TestSingularityClient([sdkSession('external', '/workspace')]) });
		try {
			await agent.authenticate('https://api.github.com', 'token');
	...
	suite('listSessions legacy-CLI surfacing (migration)', () => {
		...
		async function writeExtensionHostMarker(userHome: URI, sessionId: string): Promise<void> {
			const dir = join(getSingularityHomePath(userHome.fsPath, process.env), 'session-state', sessionId);
		...
		test('surfaces an un-adopted extension-host CLI session as adoptable when migrate is ON', async () => {
			...
			const sessionDataService = disposables.add(new TestSessionDataService());
			const client = new TestSingularityClient([sdkSession(sessionId, workingDirectory)]);
			const { agent, configurationService } = createTestAgentContext(disposables, { sessionDataService, singularityClient: client, userHome });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				await writeExtensionHostMarker(userHome, sessionId);
				configurationService.updateRootConfig({ [AgentHostMigrateLegacySingularityCliEnabledConfigKey]: true });

				assert.deepStrictEqual(
					(await agent.listSessions()).map(s => ({ id: AgentSession.id(s.session), adoptable: readSessionEhcliAdoptable(s._meta), cwd: s.workingDirectories?.map(d => d.fsPath) })),
		...
		test('does not surface the legacy CLI session when migrate is OFF', async () => {
			...
			const sessionDataService = disposables.add(new TestSessionDataService());
			const client = new TestSingularityClient([sdkSession(sessionId, workingDirectory)]);
			const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client, userHome });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				await writeExtensionHostMarker(userHome, sessionId);
		...
		test('does not surface an un-owned SDK session without the extension-host marker', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				configurationService.updateRootConfig({ [AgentHostMigrateLegacySingularityCliEnabledConfigKey]: true });
				// No marker written: a standalone Singularity SDK session, not an EH CLI session.
		...
		test('lists an already-adopted (native) session normally, not as adoptable', async () => {
			...
			await db.object.setMetadata('singularity.workingDirectory', URI.file('/workspace').toString());
			db.dispose();
			const client = new TestSingularityClient([sdkSession(sessionId, '/workspace')]);
			const { agent, configurationService } = createTestAgentContext(disposables, { sessionDataService, singularityClient: client, userHome });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				...
				assert.deepStrictEqual(
					(await agent.listSessions()).map(s => ({ id: AgentSession.id(s.session), adoptable: readSessionEhcliAdoptable(s._meta) })),
	...
	suite('createSession activeClient eager-claim', () => {
		...
		test('createSession seeds activeClient tools and syncs customizations', async () => {
			...
			const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client, pluginManager });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				...
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'test-session'),
					workingDirectories: [URI.file('/workspace')],
					activeClient: {
						clientId: 'client-1',
		...
		test('createSession without activeClient does not sync customizations', async () => {
			...
			try {
				...
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'test-session-2'),
					workingDirectories: [URI.file('/workspace')],
				});
		...
		test('provisional session anchors customization discovery to the additional roots (gated)', async () => {
			...
			try {
				...
				const additionalDirsAfterCreate = async (enabled: boolean, workingDirectories: readonly URI[]): Promise<string[]> => {
					...
					const uri = AgentSession.uri('singularitycli', `mrp-${enabled}-${workingDirectories.length}`);
		...
		test('create gates the SDK additionalDirectories on the multi-root flag', async () => {
			const runCreate = async (multiRootEnabled: boolean): Promise<{ workingDirectory: string | undefined; additionalDirectories: string[] | undefined }> => {
				const sessionDataService = disposables.add(new TestSessionDataService());
				const client = new TestSingularityClient([], [{ id: 'claude-sonnet', name: 'Claude Sonnet' }]);
				let capturedConfig: SingularityCreateSessionOptions | undefined;
				client.createSession = async config => {
					capturedConfig = config;
					return new MockSingularitySession() as unknown as SingularitySession;
				...
				const { agent, configurationService } = createTestAgentContext(disposables, { sessionDataService, singularityClient: client });
				try {
					configurationService.updateRootConfig({ [AgentHostSingularityMultiRootEnabledConfigKey]: multiRootEnabled });
					await agent.authenticate('https://api.github.com', 'token');
					await waitForState(agent.models, m => m.length > 0);

					const repoA = URI.file('/repo-a');
					const repoB = URI.file('/repo-b');
					const session = AgentSession.uri('singularitycli', `multi-root-sdk-${multiRootEnabled}`);
		...
		test('session plugin enablement is projected from reducer state per session', async () => {
			...
			try {
				const firstSession = AgentSession.uri('singularitycli', 'first-enable-state');
				const secondSession = AgentSession.uri('singularitycli', 'second-enable-state');
		...
		test('setClientCustomizations publishes parsed agents in SessionCustomizationUpdated', async () => {
			...
			const { agent } = createTestAgentContext(disposables, { sessionDataService, singularityClient: client, pluginManager, fileService });
			...
			try {
				...
				const session = AgentSession.uri('singularitycli', 'sync-customizations-test');
		...
		test('getSessionCustomizations publishes discovered files as Directory customizations', async () => {
			...
			const sessionDataService = disposables.add(new TestSessionDataService());
			const client = new TestSingularityClient([]);
			const { agent } = createTestAgentContext(disposables, { sessionDataService, singularityClient: client, fileService });

			try {
				await agent.authenticate('https://api.github.com', 'token');

				const session = AgentSession.uri('singularitycli', 'session-discovery-directories');
				await agent.createSession({
					session,
				...
				const expectedUris = [
					...
					// user home roots
					URI.from({ scheme: Schemas.inMemory, path: '/mock-home/.singularity/agents' }).toString(),
					URI.from({ scheme: Schemas.inMemory, path: '/mock-home/.agents/skills' }).toString(),
					URI.from({ scheme: Schemas.inMemory, path: '/mock-home/.singularity/skills' }).toString(),
					URI.from({ scheme: Schemas.inMemory, path: '/mock-home/.singularity/instructions' }).toString(),
					URI.from({ scheme: Schemas.inMemory, path: '/mock-home/.singularity/hooks' }).toString(),
				];
		...
		test('getSessionCustomizations starts initial discovery without debounce', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'session-discovery-immediate');
		...
		test('getSessionCustomizations clears discovered files when the root disappears', async () => {
			...
			try {
				...
				const session = AgentSession.uri('singularitycli', 'session-discovery-cleared');
				await agent.createSession({
					session,
					workingDirectories: [workspace],
				});
		...
		test('getSessionCustomizations does not republish discovered directories when watcher changes are discovery-neutral', async () => {
			...
			try {
				...
				const session = AgentSession.uri('singularitycli', 'session-discovery-neutral-watcher-change');
				await agent.createSession({
					session,
					workingDirectories: [workspace],
				});
		...
		test('getSessionCustomizations coalesces burst watcher changes into one discovered refresh publish', async () => {
			...
			try {
				...
				const session = AgentSession.uri('singularitycli', 'session-discovery-burst-watcher-change');
				await agent.createSession({
					session,
					workingDirectories: [workspace],
				});
	...
	suite('provisional sessions', () => {
		...
		test('createSession does not call client.createSession or create worktrees', async () => {
			...
			const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client, gitService });
			try {
				await agent.authenticate('https://api.github.com', 'token');

				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'prov-1'),
					workingDirectories: [URI.file('/workspace')],
		...
		test('sendMessage on the default chat materializes the parent provisional session', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'prov-default-chat'),
					workingDirectories: [URI.file('/workspace')],
				});
		...
		test('disposeSession on provisional session does not touch SDK or worktree', async () => {
			...
			try {
				...
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'prov-2'),
					workingDirectories: [URI.file('/workspace')],
				});
		...
		test('disposeSession removes the session from the SDK on-disk store', async () => {
			...
			try {
				...
				const session = AgentSession.uri('singularitycli', 'persisted-session-1');
		...
		test('disposeSession on provisional session does not call client.deleteSession', async () => {
			...
			try {
				...
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'prov-3'),
					workingDirectories: [URI.file('/workspace')],
				});
		...
		test('disposeSession reopens its lifetime after an SDK delete error', async () => {
			...
			try {
				...
				const session = AgentSession.uri('singularitycli', 'persisted-session-2');
		...
		test('materialization passes VS Code-specific system message to the SDK', async () => {
			const sessionDataService = disposables.add(new TestSessionDataService());
			const client = new TestSingularityClient([]);
			let capturedConfig: Parameters<ITestSingularityClient['createSession']>[0] | undefined;
			client.createSession = async config => {
				capturedConfig = config;
				return new MockSingularitySession() as unknown as SingularitySession;
			};
			...
			try {
				...
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'system-message-session'),
					workingDirectories: [URI.file('/workspace')],
				});
				assert.strictEqual(result.provisional, true);
				...
				assert.deepStrictEqual(systemMessage, {
					...SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE,
					content: SINGULARITY_AGENT_HOST_FILE_LINK_INSTRUCTIONS,
				});
				...
				assert.strictEqual(
					...
					'You are an AI assistant using Singularity CLI runtime in VS Code. You help users with software engineering tasks. When asked about your identity, you must state that you are an AI assistant using Singularity CLI runtime in VS Code.'
				);
			} finally {
				await disposeAgent(agent);
			}
		...
		test('materialization forwards the GitHub token to the SDK at the session level (#318693)', async () => {
			...
			try {
				...
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'session-level-token'),
					workingDirectories: [URI.file('/workspace')],
				});
				assert.strictEqual(result.provisional, true);
		...
		test('failed materialization surfaces the create error', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'gh-token-abc');
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'failed-session-token'),
					workingDirectories: [URI.file('/workspace')],
				});
		...
		test('materialization skips managed shell tools when root config disables the custom terminal tool', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				configurationService.updateRootConfig({ [SingularityCliConfigKey.EnableCustomTerminalTool]: false });

				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'sdk-terminal-defaults'),
					workingDirectories: [URI.file('/workspace')],
				});
				assert.strictEqual(result.provisional, true);
	...
	suite('onClientToolCallComplete', () => {
		 ...
		 * entries without standing up a full Singularity SDK session.
		 */
		function installStubSession(agent: SingularityAgent, sessionId: string): { calls: { toolCallId: string; result: ToolCallResult }[] } {
		...
		test('routes a top-level session URI to its session entry', async () => {
			const agent = createTestAgent(disposables);
			try {
				const sessionUri = AgentSession.uri('singularitycli', 'session-top');
		...
		test('is a no-op when no session entry exists for the resolved id', async () => {
			const agent = createTestAgent(disposables);
			try {
				const sessionUri = AgentSession.uri('singularitycli', 'session-missing');
				const defaultChat = URI.parse(buildDefaultChatUri(sessionUri));
		...
		test('routes a peer chat URI to its chat-session entry', async () => {
			...
			const agent = createTestAgent(disposables);
			try {
				const sessionUri = AgentSession.uri('singularitycli', 'session-with-peer');
		...
		test('routes the default chat URI to the session entry, not a chat-session', async () => {
			...
			const agent = createTestAgent(disposables);
			try {
				const sessionUri = AgentSession.uri('singularitycli', 'session-default');
	...
	suite('peer chat routing and lifecycle', () => {
		...
		function installStubChat(agent: SingularityAgent, chatUri: URI, options?: { permissionOwner?: string; inputOwner?: string }) {
			const events: string[] = [];
			let disposed = false;
			const stub = {
		...
		test('respondToPermissionRequest routes to a peer chat session', async () => {
			const agent = createTestAgent(disposables);
			try {
				const sessionUri = AgentSession.uri('singularitycli', 'session-perm');
		...
		test('respondToUserInputRequest routes to a peer chat session', async () => {
			const agent = createTestAgent(disposables);
			try {
				const sessionUri = AgentSession.uri('singularitycli', 'session-input');
		...
		test('setPendingMessages steers the addressed chat, not the session\'s default chat', async () => {
			...
			const agent = createTestAgent(disposables);
			try {
				const sessionUri = AgentSession.uri('singularitycli', 'session-steer');
		...
		test('disposeSession disposes the session\'s peer chats', async () => {
			...
			try {
				...
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'parent-with-peers'),
					workingDirectories: [URI.file('/workspace')],
				});
		...
		test('disposeChat deletes the SDK chat (via legacy fallback) and drops the live backing without rewriting singularity.chats', async () => {
			const sessionDataService = disposables.add(new TestSessionDataService());
			const client = new TestSingularityClient([]);
			const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'session-dispose-chat');
				const db = sessionDataService.openDatabase(session);
				// A legacy session whose backing still lives in singularity.chats.
				await db.object.setMetadata('singularity.chats', JSON.stringify({
				...
				const remaining = await db.object.getMetadata('singularity.chats');
				assert.deepStrictEqual({
	...
	suite('peer chat create / fork / model+agent / restore round-trip', () => {
		...
		type ChatInternals = {
			...
			_sessions: Map<string, SingularitySessionEntry>;
			_createAgentSession: (launchPlan: SingularitySessionLaunchPlan, customizationDirectory: URI | undefined, activeClient: unknown, identity?: { sessionUri: URI; chatChannelUri: URI }) => SingularityAgentSession;
			_resumeSession: (sessionId: string) => Promise<SingularityAgentSession>;
		...
		/**
		 * Builds a fake {@link SingularityAgentSession} that records the calls
		 ...
		 */
		function makeFakeChatSession(sessionUri: URI, sdkSessionId: string, getMessages?: () => Promise<readonly Turn[]>, owned?: IDisposable): { rec: IFakeChatRecorder; fake: SingularityAgentSession } {
		...
		test('createChat materializes a peer chat, records its backing, and returns providerData (no singularity.chats write)', async () => {
			const sessionDataService = disposables.add(new TestSessionDataService());
			const agent = createTestAgent(disposables, { sessionDataService, singularityClient: new TestSingularityClient([]) });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'create-peer');
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });

				const chatUri = URI.parse(buildChatUri(session, 'peer-a'));
				const internals = agent as unknown as ChatInternals;
				let captured: SingularitySessionLaunchPlan | undefined;
				...
				const db = sessionDataService.openDatabase(session);
				const raw = await db.object.getMetadata('singularity.chats');
				assert.deepStrictEqual({
					...
					// longer writes its private `singularity.chats` metadata.
		...
		test('resumes distinct peer chats in parallel while coalescing duplicate requests', async () => {
			const agent = createTestAgent(disposables, { singularityClient: new TestSingularityClient([]) });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'parallel-peer-resume');
		...
		test('retries a peer resume after initialization fails', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'retry-peer-resume');
				setDefaultSessionStub(agent, AgentSession.id(session), { workingDirectory: URI.file('/workspace'), dispose() { } });
		...
		test('waits for an in-flight peer resume lease before releasing its parent', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'release-during-peer-resume');
		...
		test('waits for peer resume leases in consecutive release cycles', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'consecutive-peer-releases');
				setDefaultSessionStub(agent, AgentSession.id(session), {
					workingDirectory: URI.file('/workspace'),
					hasActiveTurn: false,
					async destroySession() { },
		...
		test('resumes a peer access that starts during parent release', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'peer-access-during-release');
				...
				const chat = URI.parse(buildChatUri(session, 'peer-a'));
				const db = sessionDataService.openDatabase(session);
				await db.object.setMetadata('singularity.chats', JSON.stringify({ 'peer-a': { sdkSessionId: 'sdk-a' } }));
				await agent.materializeChat(chat, JSON.stringify({ sdkSessionId: 'sdk-a' }));
				const internals = agent as unknown as ChatInternals;
				internals._resumeSession = async () => ({
					workingDirectory: URI.file('/workspace'),
		...
		test('does not block a session-sequenced peer resume behind a queued release', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'sequenced-peer-resume-before-release');
				const sessionId = AgentSession.id(session);
		...
		test('wakes peer resume waiters and refuses new leases during shutdown', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'shutdown-wakes-peer-resume');
				const releaseGate = new DeferredPromise<void>();
				let releaseStarted = false;
				setDefaultSessionStub(agent, AgentSession.id(session), {
					workingDirectory: URI.file('/workspace'),
		...
		test('resumes peers after recreating a disposed session ID', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'reused-session-lifetime');
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });
				await agent.disposeSession(session);
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });
				setDefaultSessionStub(agent, AgentSession.id(session), { workingDirectory: URI.file('/workspace'), dispose() { } });
		...
		test('waits for an in-flight peer resume lease before disposing its parent', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'dispose-during-peer-resume');
				setDefaultSessionStub(agent, AgentSession.id(session), {
					workingDirectory: URI.file('/workspace'),
					async destroySession() { },
					dispose() { },
		...
		test('createChat is a no-op for the default chat URI', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'create-default');
				const internals = agent as unknown as ChatInternals;
		...
		test('createChat forks the source chat into a new peer chat and returns the forked chat providerData', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'fork-peer');
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });
		...
		test('createChat side chat forks hidden context and filters inherited turns', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'side-peer');
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });
		...
		test('createChat side chat preserves a local source turn id while forking from the concrete provider anchor', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'side-local-peer');
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });
				const sourceTurn: Turn = {
					id: 't1',
					state: TurnState.Complete,
		...
		test('sendMessage routes a turn to the targeted peer chat only', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'route-msg');
		...
		test('sendMessage throws for a peer chat with no backing chat', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'route-ghost');
		...
		test('changeModel applies to the targeted peer chat only', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'model-route');
				const chatA = URI.parse(buildChatUri(session, 'peer-a'));
				const chatB = URI.parse(buildChatUri(session, 'peer-b'));
				const a = makeFakeChatSession(session, 'sdk-a');
				const b = makeFakeChatSession(session, 'sdk-b');
		...
		test('changeAgent resolves and applies the agent to the targeted peer chat, and clears it with undefined', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'agent-route');
				const chatA = URI.parse(buildChatUri(session, 'peer-a'));
				const a = makeFakeChatSession(session, 'sdk-a');
				const internals = agent as unknown as ChatInternals;
				setPeerChatStub(agent, chatA, a.fake);
		...
		test('round-trips peer chats through providerData + materializeChat and resumes per-chat history after a restart', async () => {
			...
			const sessionDataService = disposables.add(new TestSessionDataService());
			const session = AgentSession.uri('singularitycli', 'restore-rt');
			...
			const agent1 = createTestAgent(disposables, { sessionDataService, singularityClient: new TestSingularityClient([]) });
			try {
			...
			const agent2 = createTestAgent(disposables, { sessionDataService, singularityClient: new TestSingularityClient([]) });
			try {
				...
				let resumed: SingularitySessionLaunchPlan | undefined;
		...
		test('materializeChat falls back to the legacy singularity.chats catalog when providerData is undefined', async () => {
			const sessionDataService = disposables.add(new TestSessionDataService());
			const agent = createTestAgent(disposables, { sessionDataService, singularityClient: new TestSingularityClient([]) });
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'legacy-materialize');
				const db = sessionDataService.openDatabase(session);
				await db.object.setMetadata('singularity.chats', JSON.stringify({
		...
		test('changeModel on a peer chat refreshes its backing and fires onDidChangeChatData', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'model-blob');
				const chatUri = URI.parse(buildChatUri(session, 'peer-a'));
				const internals = agent as unknown as ChatInternals;
	...
	suite('chat surface (IAgentChats)', () => {
		...
		type ConvInternals = {
			...
			_createAgentSession: (launchPlan: SingularitySessionLaunchPlan, dir: URI | undefined, activeClient: unknown, identity?: { sessionUri: URI; chatChannelUri: URI }) => SingularityAgentSession;
		};
		...
		/**
		 * Installs a recording fake {@link SingularityAgentSession} as a peer chat
		 ...
		 */
		function installFake(agent: SingularityAgent, key: string, target: 'chat' | 'session', sessionUri: URI): IFakeConvRecorder {
		 ...
		 */
		function stubBackingSession(agent: SingularityAgent): void {
			(agent as unknown as ConvInternals)._createAgentSession = (launchPlan, _dir, _ac, identity) => {
				return {
					sessionUri: identity?.sessionUri ?? AgentSession.uri('singularitycli', launchPlan.sessionId),
		...
		test('createSession mints a provisional session', async () => {
			const agent = createTestAgent(disposables, { singularityClient: new TestSingularityClient([]) });
			try {
				const session = AgentSession.uri('singularitycli', 'scope-create');
		...
		test('disposeSession tears down a provisional session', async () => {
			const agent = createTestAgent(disposables, { singularityClient: new TestSingularityClient([]) });
			try {
				const session = AgentSession.uri('singularitycli', 'scope-dispose');
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });
				const internals = agent as unknown as ConvInternals;
		...
		test('createChat creates a peer chat and returns its providerData', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'conv-create');
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });
				const chatUri = URI.parse(buildChatUri(session, 'peer-a'));
		...
		test('fork delegates to createChat with the fork source', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'conv-fork');
				await agent.createSession({ session, workingDirectories: [URI.file('/workspace')] });
		...
		test('sendMessage routes a peer chat URI to the peer chat', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'conv-send-peer');
				const chatUri = URI.parse(buildChatUri(session, 'peer-a'));
		...
		test('sendMessage routes a scope (session) URI to the default chat', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'conv-send-default');
		...
		test('abort, changeModel, and changeAgent route a peer URI to the peer chat', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'conv-ops');
				const chatUri = URI.parse(buildChatUri(session, 'peer-a'));
				const rec = installFake(agent, chatUri.toString(), 'chat', session);
		...
		test('getMessages returns the peer chat history', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'conv-history');
				const chatUri = URI.parse(buildChatUri(session, 'peer-a'));
		...
		test('disposeChat disposes the peer chat', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const session = AgentSession.uri('singularitycli', 'conv-dispose');
				const chatUri = URI.parse(buildChatUri(session, 'peer-a'));
				const rec = installFake(agent, chatUri.toString(), 'chat', session);
	...
	suite('client tool refresh on reload (#319516)', () => {
		...
		function getActiveClient(agent: SingularityAgent, session: URI): TestActiveClient {
		...
		test('clientId-only change (reload) does NOT require a restart and updates the live owner', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'reload-session');
		...
		test('a structural tool change still requires a restart', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'tools-change-session');
		...
		test('multiple active clients merge their tools and removal isolates per client', async () => {
			const agent = createTestAgent(disposables);
			try {
				const session = AgentSession.uri('singularitycli', 'multi-client-session');
	...
	suite('config-driven session refresh', () => {
		...
		test('waits for the previous SDK session to disconnect before resuming', async () => {
			...
			const internals = agent as unknown as {
				_resumeSession: (id: string) => Promise<SingularityAgentSession>;
			};
		...
		test('coalesces root and structural divergence into one same-conversation resume before send', async () => {
			const client = new TestSingularityClient([]);
			const { agent, configurationService } = createTestAgentContext(disposables, { singularityClient: client });
			configurationService.updateRootConfig({ [AgentHostSingularityMultiRootEnabledConfigKey]: true });
			const sessionId = 'config-refresh-session';
			const session = AgentSession.uri('singularitycli', sessionId);
			...
			const internals = agent as unknown as {
				_resumeSession: (id: string, workingDirectories?: readonly URI[]) => Promise<SingularityAgentSession>;
			};
			...
			internals._resumeSession = async (id, workingDirectories) => {
				...
				setDefaultSessionStub(agent, sessionId, resumedSession);
				return resumedSession as unknown as SingularityAgentSession;
			};
		...
		test('cold resume applies and persists the complete send snapshot including root removal', async () => {
			...
			try {
				await dbRef.object.setMetadata('singularity.workingDirectory', primary.toString());
				await dbRef.object.setMetadata('singularity.workingDirectories', JSON.stringify([primary, persistedSecondary].map(directory => directory.toString())));
			} finally {
				dbRef.dispose();
			}
			...
			const { agent, configurationService } = createTestAgentContext(disposables, { singularityClient: client, useRealResumePath: true, sessionDataService });
			configurationService.updateRootConfig({ [AgentHostSingularityMultiRootEnabledConfigKey]: true });
			try {
				await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'token');
				...
				try {
					persistedWorkingDirectories = await persistedDbRef.object.getMetadata('singularity.workingDirectories');
				} finally {
	...
	suite('_resumeSession dedup', () => {
		...
		// `_resumeSession(id)` used to construct two `SingularityAgentSession`
		...
		type AgentInternals = {
			_resumeSession: (id: string) => Promise<SingularityAgentSession>;
			_doResumeSession: (id: string) => Promise<SingularityAgentSession>;
		};
		const makeFakeSession = () => ({ dispose: () => { } } as unknown as SingularityAgentSession);

		test('dedupes concurrent calls for the same sessionId', async () => {
			const agent = createTestAgent(disposables);
			const internals = agent as unknown as AgentInternals;
			const deferred = new DeferredPromise<SingularityAgentSession>();
			let doResumeCalls = 0;
		...
		test('post-init shutdown race: disposes the session and throws CancellationError instead of registering on a disposed _sessions map', async () => {
			...
			const agent = createTestAgent(disposables);
			const internals = agent as unknown as {
				_registerInitializedSession: (id: string, s: SingularityAgentSession, client: SingularityClient) => void;
				_shutdownPromise: Promise<void> | undefined;
			};
			let disposed = 0;
			const fakeSession = { dispose: () => { disposed++; } } as unknown as SingularityAgentSession;
			const client = new TestSingularityClient([]) as unknown as SingularityClient;
		...
		test('post-init client replacement race disposes the stale session', async () => {
			const currentClient = new TestSingularityClient([]);
			const agent = createTestAgent(disposables, { singularityClient: currentClient });
			const internals = agent as unknown as {
				_throwIfClientReplaced: (client: SingularityClient, session: SingularityAgentSession) => void;
			};
			let disposed = 0;
			const staleSession = {
			...
			try {
				await agent.listSessions();
				assert.doesNotThrow(() => internals._throwIfClientReplaced(currentClient as unknown as SingularityClient, staleSession));
				assert.throws(
					() => internals._throwIfClientReplaced(new TestSingularityClient([]) as unknown as SingularityClient, staleSession),
		...
		test('shutdown during resume cancels the in-flight resume', async () => {
			...
			const client = new TestSingularityClient([sdkSession('s1', workingDirectory)]);
			const deferredSession = new DeferredPromise<SingularitySession>();
			...
			const internals = agent as unknown as { _resumeSession: (id: string) => Promise<SingularityAgentSession> };
			try {
				await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'token');
				...
				const shutdown = agent.shutdown();
				deferredSession.complete(new MockSingularitySession() as unknown as SingularitySession);
	...
	suite('_resumeSession fallback', () => {
		...
		test('does not restore a persisted custom agent that is absent from the current plugin snapshot', async () => {
			...
			const sessionDataService = disposables.add(new TestSessionDataService());
			const session = AgentSession.uri('singularitycli', 's1');
			const dbRef = sessionDataService.openDatabase(session);
			try {
				await dbRef.object.setMetadata('singularity.workingDirectory', URI.file(workingDirectory).toString());
				await dbRef.object.setMetadata('singularity.agent', JSON.stringify({ uri: 'file:///old-client/data.md' }));
			} finally {
				dbRef.dispose();
			}
			...
			const agent = createTestAgent(disposables, { singularityClient: client, useRealResumePath: true, sessionDataService });
			const internals = agent as unknown as AgentInternals;
			try {
				await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'token');
				await internals._resumeSession('s1');
		...
		test('retries resume without a custom agent when the SDK reports the stored agent is missing', async () => {
			...
			const dbRef = sessionDataService.openDatabase(session);
			try {
				await dbRef.object.setMetadata('singularity.workingDirectory', repo.toString());
				await dbRef.object.setMetadata('singularity.agent', JSON.stringify({ uri: dataAgent.toString() }));
			} finally {
				dbRef.dispose();
			}

			const client = new TestSingularityClient([sdkSession('s1')]);
			const resumeAgents: (string | undefined)[] = [];
			client.resumeSession = async (_sessionId, options) => {
				resumeAgents.push(options?.agent);
			...
			const agent = createTestAgent(disposables, { singularityClient: client, useRealResumePath: true, sessionDataService, fileService });
			const internals = agent as unknown as AgentInternals;
			try {
				await agent.authenticate(GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource, 'token');
				await internals._resumeSession('s1');
		...
		test('resume gates the persisted additional roots on the multi-root flag', async () => {
			const runResume = async (multiRootEnabled: boolean): Promise<string[] | undefined> => {
				...
				try {
					await dbRef.object.setMetadata('singularity.workingDirectory', URI.file(workingDirectory).toString());
					await dbRef.object.setMetadata('singularity.workingDirectories', JSON.stringify([URI.file(workingDirectory).toString(), URI.file(secondary).toString()]));
				} finally {
					dbRef.dispose();
				}
	...
	suite('customization anchoring', () => {
		...
		test('uses signal paths only for Singularity OTLP/HTTP metrics endpoints', () => {
			assert.strictEqual(resolveSingularityOtlpMetricsEndpoint('http://collector:4318', 'http/protobuf'), 'http://collector:4318/v1/metrics');
			assert.strictEqual(resolveSingularityOtlpMetricsEndpoint('http://collector:4318/custom', 'http/json'), 'http://collector:4318/custom');
			assert.strictEqual(resolveSingularityOtlpMetricsEndpoint('https://collector:4317', 'grpc'), 'https://collector:4317');
		});
		...
		test('rebaseUnder rebases paths under the source dir and leaves others untouched', () => {
			...
			assert.strictEqual(
				rebaseUnder(URI.file('/Users/me/.singularity/skills/foo'), original, worktree),
				undefined,
		...
		setup(async () => {
			tmpDir = await fs.mkdtemp(`${os.tmpdir()}/singularity-agent-anchor-test-`);
		});
		...
		async function materializeAndCaptureAnchor(resolvedWorkingDirectory: URI | undefined): Promise<{ anchor: URI | undefined; sdkWorkingDirectory: string | undefined; originalFolder: URI }> {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const result = await agent.createSession({ session: AgentSession.uri('singularitycli', 'anchor-session'), workingDirectories: [originalFolder] });
				assert.strictEqual(result.provisional, true);
		...
		test('worktree skill/instruction directories sent to the SDK resolve inside the worktree', async () => {
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'wt-dirs-session'),
	...
	suite('custom agent worktree translation', () => {
		...
		function provisional(workingDirectory: URI | undefined, agent: AgentSelection | undefined): unknown {
			const sessionUri = AgentSession.uri('singularitycli', 'prov-agent');
		...
		test('_getAlternativeAgentForWorktree returns undefined when there is nothing to translate', async () => {
			...
			try {
				const internals = agent as unknown as AgentInternals;
				const outsideRepoAgent: AgentSelection = { uri: URI.file('/home/me/.singularity/agents/agent.md').toString() };
				assert.deepStrictEqual(
					{
		...
		test('materialization rewrites a repo agent to its worktree copy and persists it (no resolution stubbing)', async () => {
			...
			const client = new TestSingularityClient([]);
			client.createSession = async () => new MockSingularitySession() as unknown as SingularitySession;
			...
			try {
				await agent.authenticate('https://api.github.com', 'token');
				const result = await agent.createSession({
					session: AgentSession.uri('singularitycli', 'agent-translate'),
	...
	suite('ensureSessionAdopted (legacy Singularity CLI migration)', () => {
		...
		test('does not adopt a Singularity SDK session without the extension-host marker', async () => {
			const userHome = URI.file(await fs.mkdtemp(`${os.tmpdir()}/adopt-home-`));
		...
		test('does not re-adopt a session that already has stored working-directory metadata', async () => {
			...
			await seed.object.setMetadata('singularity.workingDirectory', URI.file('/workspace').toString());
			seed.dispose();
			const client = new TestSingularityClient([sdkSession(sessionId, '/workspace')]);
			const agent = createTestAgent(disposables, { sessionDataService, singularityClient: client, userHome });
			try {
