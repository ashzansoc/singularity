...
import {
	...
	SingularityApiError,
	type ISingularityApiService,
	type ISingularityApiServiceRequestOptions,
} from '../../node/shared/singularityApiService.js';
...
interface IFakeCall {
	...
	options: ISingularityApiServiceRequestOptions | undefined;
}
...
type MessagesResult =
	...
	| { kind: 'stream'; events: Anthropic.MessageStreamEvent[]; midStreamError?: SingularityApiError | Error }
...
function createProxyService(fakeApi: FakeSingularityApiService): ClaudeProxyService {
...
suite('ClaudeProxyService', () => {
	...
	suite('Token slot', () => {
		...
		test('start() updates token slot last-writer-wins', async () => {
			const fake = new FakeSingularityApiService();
	...
	suite('Auth', () => {
		...
		async function withProxy<T>(fn: (handle: { baseUrl: string; nonce: string }, fake: FakeSingularityApiService) => Promise<T>): Promise<T> {
			const fake = new FakeSingularityApiService();
	...
	suite('GET /v1/models', () => {
		...
		test('upstream SingularityApiError is re-emitted verbatim with original status', async () => {
			const fake = new FakeSingularityApiService();
			...
			fake.modelsResult = { kind: 'error', error: new SingularityApiError(429, envelope) };
			const service = createProxyService(fake);
			const handle = await service.start(TOKEN);
			try {
				const res = await fetchJson(`${handle.baseUrl}/v1/models`, {
		...
		test('non-SingularityApiError → 502 api_error', async () => {
			const fake = new FakeSingularityApiService();
	...
	suite('Credits reporting', () => {
		...
		test('streaming: singularity_usage.total_nano_aiu fires onDidReportCredits with the session id', async () => {
			const fake = new FakeSingularityApiService();
			...
			// declare `singularity_usage`).
			const delta = events.find(e => e.type === 'message_delta')!;
			(delta as unknown as { singularity_usage: { total_nano_aiu: number } }).singularity_usage = { total_nano_aiu: 750_000_000 };
		...
		test('non-streaming: singularity_usage.total_nano_aiu fires onDidReportCredits', async () => {
			const fake = new FakeSingularityApiService();
			const message = makeMessage('claude-opus-4.6', 'hi');
			(message as unknown as { singularity_usage: { total_nano_aiu: number } }).singularity_usage = { total_nano_aiu: 250_000_000 };
		...
		test('no singularity_usage in the response → no credits report', async () => {
			const fake = new FakeSingularityApiService();
			fake.messagesResult = { kind: 'message', message: makeMessage('claude-opus-4.6', 'hi') };
			const service = createProxyService(fake);
			const reports: { sessionId: string; totalNanoAiu: number }[] = [];
	...
	suite('Streaming', () => {
		...
		test('mid-stream SingularityApiError → SSE error frame, then end, no message_stop after', async () => {
			const fake = new FakeSingularityApiService();
			...
			fake.messagesResult = {
				kind: 'stream',
				events,
				midStreamError: new SingularityApiError(SINGULARITY_API_ERROR_STATUS_STREAMING, upstreamEnvelope),
			};
			const service = createProxyService(fake);
			const handle = await service.start(TOKEN);
			try {
		...
		test('pre-stream SingularityApiError → JSON error response with original status', async () => {
			const fake = new FakeSingularityApiService();
			const envelope: Anthropic.ErrorResponse = {
				type: 'error',
			...
			fake.messagesResult = { kind: 'error', error: new SingularityApiError(401, envelope) };
			const service = createProxyService(fake);
			const handle = await service.start(TOKEN);
			try {
				const res = await fetchJson(`${handle.baseUrl}/v1/messages`, {
		...
		test('pre-stream SingularityApiError with streaming sentinel coerces to 502 but preserves envelope', async () => {
			...
			fake.messagesResult = { kind: 'error', error: new SingularityApiError(SINGULARITY_API_ERROR_STATUS_STREAMING, envelope) };
			const service = createProxyService(fake);
			const handle = await service.start(TOKEN);
			try {
				const res = await fetchJson(`${handle.baseUrl}/v1/messages`, {
		...
		test('mid-stream non-SingularityApiError → synthesized SSE error frame', async () => {
			const fake = new FakeSingularityApiService();
			const events: Anthropic.MessageStreamEvent[] = [
				{ type: 'message_start', message: makeMessage('claude-opus-4.6', '') },
			];
	...
	suite('Abort', () => {
		...
		test('client disconnect mid-stream propagates AbortSignal upstream and writes nothing else', async () => {
			...
			const wrapped: ISingularityApiService = {
				...
				messages: ((_token: string, _body: Anthropic.MessageCreateParams, options?: ISingularityApiServiceRequestOptions) => {
					...
					return gen();
				}) as ISingularityApiService['messages'],
