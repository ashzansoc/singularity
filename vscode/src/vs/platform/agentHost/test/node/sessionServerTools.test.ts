...
suite('SessionServerTools', () => {
	...
	const model: IAgentModelInfo = { provider: 'singularity', id: 'gpt-4o', name: 'GPT-4o', supportsVision: false };
	...
	function sessionMeta(id: string, status: SessionStatus, dir: URI): IAgentSessionMetadata {
		return { session: URI.parse(`singularity:/${id}`), startTime: 0, modifiedTime: 0, status: status | SessionStatus.IsRead, workingDirectories: dir ? [dir] : undefined, summary: `title-${id}` };
	}
	...
	function createAccessor(overrides?: Partial<ISessionServerToolAccessor> & { onCreate?: (config: IAgentCreateSessionConfig) => void; onPrompt?: (session: URI, chat: URI, prompt: string) => void; onCreateChat?: (session: URI, chat: URI, options?: { title?: string; model?: IAgentModelInfo }) => void; onDelete?: (session: URI) => void; depths?: Map<string, number> }): ISessionServerToolAccessor {
		...
		return {
			...
			createSession: overrides?.createSession ?? (async config => { overrides?.onCreate?.(config); return URI.parse('singularity:/new'); }),
	...
	test('serializeSessions includes meaningful metadata when present', () => {
		...
		const rich: IAgentSessionMetadata = {
			session: URI.parse('singularity:/rich'),
			startTime: 0,
	...
	test('serializeSessions reports archived status from the IsArchived status bit', () => {
		...
		const noStatus: IAgentSessionMetadata = { session: URI.parse('singularity:/noStatus'), startTime: 0, modifiedTime: 0, workingDirectories: workspace ? [workspace] : undefined };
		assert.deepStrictEqual(JSON.parse(serializeSessions([archived, notArchived, noStatus])).sessions.map((s: { session: string; status?: string }) => ({ session: s.session, status: s.status })), [
			{ session: 'singularity:/archived', status: 'idle,archived' },
			{ session: 'singularity:/notArchived', status: 'idle' },
			{ session: 'singularity:/noStatus', status: undefined },
		]);
	});
	...
	test('only sessions known to be unread report or filter as unread', () => {
		...
		const unknown: IAgentSessionMetadata = { session: URI.parse('singularity:/unknown'), startTime: 0, modifiedTime: 0, workingDirectories: [workspace] };
		...
		assert.deepStrictEqual({
			...
			serializedUnread: [
				{ session: 'singularity:/unknown', unread: undefined },
				{ session: 'singularity:/unread', unread: true },
				{ session: 'singularity:/read', unread: undefined },
			],
			filteredToUnread: ['singularity:/unread'],
		});
	});
	...
	test('create_session builds config, starts the default chat, and returns an open link', async () => {
		...
		const text = await group.execute(stateManager, 'singularity:/caller', SessionServerToolName.CreateSession, { workspace: workspace.toString(), prompt: 'do it', model: 'gpt-4o' });

		assert.deepStrictEqual(created, { workingDirectories: [workspace], provider: 'singularity', model: { id: 'gpt-4o' } });
		assert.strictEqual(prompted?.prompt, 'do it');
		assert.strictEqual(prompted?.chat.toString(), buildDefaultChatUri(URI.parse('singularity:/new')));
		assert.ok(text.includes('agent-host-session://singularity/new'), 'result carries the open-session link for the pill');
		assert.ok(!text.includes('singularity:/new'), 'result does not echo the raw backend session URI');
		store.dispose();
	});
	...
	test('list_sessions execute returns serialized sessions', async () => {
		...
		const text = await group.execute(stateManager, 'singularity:/caller', SessionServerToolName.ListSessions, {});
		assert.deepStrictEqual(JSON.parse(text).sessions.map((s: { session: string }) => s.session), ['singularity:/s1']);
		store.dispose();
	});
	...
	test('list_sessions filters by status, workspace, changes, archived and creation time', async () => {
		...
		const ids = async (args: object) => JSON.parse(await group.execute(stateManager, 'singularity:/caller', SessionServerToolName.ListSessions, args)).sessions.map((s: { session: string }) => s.session);
		...
		assert.deepStrictEqual({
			...
			byStatus: ['singularity:/needsInput'],
			byArchivedStatus: ['singularity:/archived'],
			byWorkspace: ['singularity:/idle', 'singularity:/needsInput', 'singularity:/withPr'],
			withChanges: ['singularity:/idle'],
			unread: ['singularity:/needsInput'],
			withPullRequest: ['singularity:/withPr'],
			withArchived: ['singularity:/idle', 'singularity:/needsInput', 'singularity:/elsewhere', 'singularity:/archived', 'singularity:/withPr'],
			createdAfter: ['singularity:/needsInput', 'singularity:/elsewhere', 'singularity:/withPr'],
			createdBefore: ['singularity:/idle', 'singularity:/needsInput'],
			combined: ['singularity:/idle'],
			all: ['singularity:/idle', 'singularity:/needsInput', 'singularity:/elsewhere', 'singularity:/withPr'],
		});
		store.dispose();
	});
	...
	test('list_sessions fetches a single session by URI or open link, bypassing other filters', () => {
		...
		assert.deepStrictEqual({
			byUri: ids({ session: 'singularity:/s1' }),
			byLink: ids({ session: 'agent-host-session://singularity/s1' }),
			// A direct lookup returns an archived session even though archived are hidden by default.
			archivedByUri: ids({ session: 'singularity:/archived' }),
			unknown: ids({ session: 'singularity:/nope' }),
		}, {
			byUri: ['singularity:/s1'],
			byLink: ['singularity:/s1'],
			archivedByUri: ['singularity:/archived'],
			unknown: [],
		});
	});
	...
	test('create_session stamps spawn depth and enforces the recursion depth limit', async () => {
		...
		await group.execute(stateManager, 'singularity:/caller', SessionServerToolName.CreateSession, args);
		assert.strictEqual(depths.get('singularity:/new'), 1);

		// A session already at the max spawn depth may not create further sessions.
		depths.set('singularity:/deep', 3);
		await assert.rejects(
			async () => { await group.execute(stateManager, 'singularity:/deep', SessionServerToolName.CreateSession, args); },
			/recursion limit/,
		);
		store.dispose();
	});
	...
	test('create_session enforces a process-wide breadth backstop', async () => {
		...
		let n = 0;
		const group = createSessionServerToolGroup(createAccessor({ createSession: async () => URI.parse(`singularity:/s${n++}`) }));
		const args = { workspace: workspace.toString(), prompt: 'go' };
		for (let i = 0; i < 25; i++) {
			await group.execute(stateManager, 'singularity:/caller', SessionServerToolName.CreateSession, args);
		}
		await assert.rejects(async () => { await group.execute(stateManager, 'singularity:/caller', SessionServerToolName.CreateSession, args); }, /more than 25 sessions/);
		store.dispose();
	});

	test('getCreateChatArgs resolves an explicit session, model, falls back to current, and validates', () => {
		const sessions = [sessionMeta('s1', SessionStatus.Idle, workspace)];
		const explicit = getCreateChatArgs({ session: 'singularity:/s1', prompt: 'hi', title: 'My chat', model: 'gpt-4o' }, sessions, [model]);
		assert.strictEqual(explicit.session.toString(), 'singularity:/s1');
		assert.strictEqual(explicit.title, 'My chat');
		assert.strictEqual(explicit.model?.id, 'gpt-4o');
		const current = getCreateChatArgs({ prompt: 'hi' }, sessions, [model], URI.parse('singularity:/s1'));
		assert.strictEqual(current.session.toString(), 'singularity:/s1');
		assert.throws(() => getCreateChatArgs({ session: 'singularity:/unknown', prompt: 'hi' }, sessions, [model]), /session/);
		assert.throws(() => getCreateChatArgs({ prompt: 'hi' }, sessions, [model]), /session/);
		assert.throws(() => getCreateChatArgs({ prompt: 'hi', model: 'nope' }, sessions, [model], URI.parse('singularity:/s1')), /model/);
	});
	...
	test('create_chat adds a chat to the session, starts the prompt, and returns an open link', async () => {
		...
		const result = await applyCreateChatTool(accessor, { session: 'singularity:/s1', prompt: 'do it', title: 'T', model: 'gpt-4o' });
		assert.strictEqual(result.session, 'singularity:/s1');
		const chatId = URI.parse(result.chat).authority;
		assert.strictEqual(result.openLink, `agent-host-session://singularity/s1?chat=${chatId}`);
		assert.strictEqual(createdChat?.session.toString(), 'singularity:/s1');
	...
	test('send_message targets the default chat / a specific chat, refuses the current chat, and validates', async () => {
		...
		const currentChannel = buildDefaultChatUri('singularity:/s1');

		// Explicit session -> owning session's default chat.
		const toSession = await applySendMessageTool(accessor, { session: 'singularity:/s2', message: 'hi' }, currentChannel);
		assert.strictEqual(prompts.at(-1)?.session.toString(), 'singularity:/s2');
		assert.strictEqual(prompts.at(-1)?.chat.toString(), buildDefaultChatUri('singularity:/s2'));
		assert.strictEqual(prompts.at(-1)?.prompt, 'hi');
		assert.ok(toSession.includes('agent-host-session://singularity/s2'));

		// A create_chat open link -> that specific chat channel.
		await applySendMessageTool(accessor, { session: 'agent-host-session://singularity/s2?chat=c9', message: 'yo' }, currentChannel);
		assert.strictEqual(prompts.at(-1)?.chat.toString(), buildChatUri('singularity:/s2', 'c9'));

		// Refuses messaging the exact current chat channel (self-loop guard).
		await assert.rejects(() => applySendMessageTool(accessor, { session: 'singularity:/s1', message: 'loop' }, currentChannel), /current chat/);
		// Unknown session and missing session/message are rejected.
		await assert.rejects(() => applySendMessageTool(accessor, { session: 'singularity:/nope', message: 'x' }, currentChannel), /known session/);
		assert.throws(() => getSendMessageArgs({ message: 'x' }, []), /session/);
		assert.throws(() => getSendMessageArgs({ session: 'singularity:/s2' }, []), /message/);
	});
	...
	suite('get_session_context', () => {
		...
		test('summary returns per-turn gists (message + reply snippet), no tool calls', () => {
			assert.deepStrictEqual(JSON.parse(serializeSessionContext(URI.parse('singularity:/s1'), undefined, snapshot, 'summary', 10)), {
				session: 'singularity:/s1',
				openLink: 'agent-host-session://singularity/s1',
				detail: 'summary',
				transcript: [
		...
		test('digest adds assistant text and tool-call names', () => {
			const digest = JSON.parse(serializeSessionContext(URI.parse('singularity:/s1'), undefined, snapshot, 'digest', 10));
		...
		test('detail=full targeting a specific chat carries the chat link and tool inputs', () => {
			const full = JSON.parse(serializeSessionContext(URI.parse('singularity:/s1'), 'c9', snapshot, 'full', 10));
			assert.strictEqual(full.openLink, 'agent-host-session://singularity/s1?chat=c9');
		...
		test('transcriptLimit drops older turns and flags truncated', () => {
			const limited = JSON.parse(serializeSessionContext(URI.parse('singularity:/s1'), undefined, snapshot, 'summary', 1));
		...
		test('execute reads from the accessor; cold session returns identity + empty transcript', async () => {
			...
			const live = JSON.parse(await withCtx.execute(stateManager, 'singularity:/caller', SessionServerToolName.GetSessionContext, { session: 'singularity:/s1' }));
			...
			assert.deepStrictEqual(JSON.parse(await cold.execute(stateManager, 'singularity:/caller', SessionServerToolName.GetSessionContext, { session: 'singularity:/s1' })), {
				session: 'singularity:/s1', openLink: 'agent-host-session://singularity/s1', detail: 'summary', transcript: [], hasMoreHistory: false, truncated: false,
			});
			store.dispose();
		});
		...
		test('getSessionContextArgs validates input', () => {
			...
			assert.throws(() => getSessionContextArgs({ session: 'singularity:/nope' }, [sessionMeta('s1', SessionStatus.Idle, workspace)]), /known session/);
			assert.throws(() => getSessionContextArgs({ session: 'singularity:/s1', detail: 'huge' }, [sessionMeta('s1', SessionStatus.Idle, workspace)]), /detail/);
			assert.strictEqual(getSessionContextArgs({ session: 'singularity:/s1', transcriptLimit: 999 }, [sessionMeta('s1', SessionStatus.Idle, workspace)]).transcriptLimit, 50);
		});
	});
	...
	test('get_current_session returns the current session link + metadata', async () => {
		...
		const chatChannel = buildDefaultChatUri('singularity:/s1');
		const text = await group.execute(stateManager, chatChannel, SessionServerToolName.GetCurrentSession, {});
		const parsed = JSON.parse(text);
		assert.strictEqual(parsed.session, 'singularity:/s1');
		assert.strictEqual(parsed.openLink, 'agent-host-session://singularity/s1');
		store.dispose();
	});
	...
	test('getDeleteSessionArgs validates and refuses the current session', () => {
		...
		assert.strictEqual(getDeleteSessionArgs({ session: 'singularity:/s2' }, sessions).toString(), 'singularity:/s2');
		// Accepts the agent-host-session:// open link form (as returned by create_session).
		assert.strictEqual(getDeleteSessionArgs({ session: 'agent-host-session://singularity/s2' }, sessions).toString(), 'singularity:/s2');
		assert.throws(() => getDeleteSessionArgs({ session: 'singularity:/unknown' }, sessions), /session/);
		assert.throws(() => getDeleteSessionArgs({}, sessions), /session/);
		assert.throws(() => getDeleteSessionArgs({ session: 'singularity:/s1' }, sessions, URI.parse('singularity:/s1')), /current session/);
		assert.throws(() => getDeleteSessionArgs({ session: 'agent-host-session://singularity/s1' }, sessions, URI.parse('singularity:/s1')), /current session/);
	});
	...
	test('delete_session deletes the target and returns a confirmation', async () => {
		...
		const text = await applyDeleteSessionTool(accessor, { session: 'singularity:/s2' }, URI.parse('singularity:/s1'));
		assert.strictEqual(deleted?.toString(), 'singularity:/s2');
		assert.ok(text.includes('singularity:/s2'));
	});
});
