...
suite('AgentHostTelemetryReporter', () => {
	...
	const session = 'agent-session://singularity/abc';
	...
	test('toolCallDetails emits standard and restricted aggregates whenever tools were available, and no-ops when none were', async () => {
		...
		await reporter.toolCallDetails({
			provider: 'singularity', session, turnId: 'a1b2c3d4-0000-4000-8000-000000000000', clientType: AgentHostClientType.Unknown, model: 'gpt-x', responseType: 'success',
		...
		await reporter.toolCallDetails({
			provider: 'singularity', session, turnId: 'a1b2c3d4-0000-4000-8000-000000000000', clientType: AgentHostClientType.EditorWindow, model: 'gpt-x', responseType: 'success',
			toolCounts: {}, availableTools: ['grep', 'edit'],
			turnIndex: 2, turnDuration: 1200, messageCharLen: 11,
			numRequests: 1, totalToolCalls: 0, parallelToolCallRounds: 0, parallelToolCallsTotal: 0,
		}); // emitted: tools available, even though no tool calls were made
		await reporter.toolCallDetails({
			provider: 'singularity', session, turnId: 'a1b2c3d4-0000-4000-8000-000000000000', clientType: AgentHostClientType.AgentsWindow, model: 'gpt-x', responseType: 'cancelled',
	...
	test('toolApproval emits chat.toolApproval with AH discriminators and reason mapping', () => {
		...
		reporter.toolApproval({
			provider: 'singularity', session, turnId: 'turn-1',
		...
		reporter.toolApproval({
			provider: 'singularity', session, turnId: 'turn-2',
		...
		reporter.toolApproval({
			provider: 'singularity', session, turnId: 'turn-3',
