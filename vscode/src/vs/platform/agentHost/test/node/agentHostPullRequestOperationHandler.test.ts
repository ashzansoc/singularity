...
import { GITHUB_SINGULARITY_PROTECTED_RESOURCE, GITHUB_REPO_PROTECTED_RESOURCE, type IAgentService } from '../../common/agentService.js';
...
import type { ISingularityApiService, ISingularityApiServiceRequestOptions, ISingularityUtilityChatCompletionRequest } from '../../node/shared/singularityApiService.js';
...
function createAgentService(withSingularityToken = false): IAgentService {
	return {
		getAuthToken: resource => {
			if (resource.resource === GITHUB_REPO_PROTECTED_RESOURCE.resource) {
				return 'gh-token';
			}
			if (withSingularityToken && resource.resource === GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource) {
				return 'singularity-token';
			}
			return undefined;
		},
	} as IAgentService;
...
function setup(disposables: Pick<DisposableStore, 'add'>, gitService: TestGitService, octoKitService: TestOctoKitService, options?: { singularityApiService?: TestSingularityApiService; withSingularityToken?: boolean; turns?: Turn[]; draft?: boolean; autoMergeMethod?: AutoMergeMethod }): { handler: AgentHostPullRequestOperationHandler; session: URI; createdEvents: string[]; singularityApiService: TestSingularityApiService } {
	...
	return {
		handler: new AgentHostPullRequestOperationHandler(
			...
			createAgentService(options?.withSingularityToken), gitService, octoKitService, createTestGitHubEndpointService(), singularityApiService, new NullLogService()),
		session,
		createdEvents,
...
suite('AgentHostPullRequestOperationHandler', () => {
	...
	// Matches the Singularity CLI Agent Window behavior: if the session has
	...
	// When a Singularity token is available, the handler asks the utility model
	...
	test('generates the PR title and description from the conversation via the model', async () => {
		...
		const { handler, session, singularityApiService } = setup(disposables, gitService, octoKitService, { withSingularityToken: true, turns });

		const result = await handler.invoke({ channel: buildSessionChangesetUri(session.toString()), operationId: AgentHostPullRequestOperationHandler.OPERATION_CREATE_PR }, CancellationToken.None);

		const userContent = singularityApiService.calls[0]?.request.messages.find(m => m.role === 'user')?.content ?? '';
		assert.deepStrictEqual({
			message: result.message,
			token: singularityApiService.calls[0]?.token,
	...
	// Without a Singularity token the model is never called and the handler falls
	// back to the branch-name based title/description.
	test('falls back to branch-name title and description without a Singularity token', async () => {
		const gitService = new TestGitService();
		const octoKitService = new TestOctoKitService();
		const { handler, session, singularityApiService } = setup(disposables, gitService, octoKitService);

		await handler.invoke({ channel: buildSessionChangesetUri(session.toString()), operationId: AgentHostPullRequestOperationHandler.OPERATION_CREATE_PR }, CancellationToken.None);

		assert.deepStrictEqual({
			utilityCalls: singularityApiService.calls.length,
			title: octoKitService.lastTitle,
			body: octoKitService.lastBody,
		}, {
			utilityCalls: 0,
	...
	test('falls back to branch-name title and description when generation fails', async () => {
		...
		singularityApiService.error = new Error('utility model unavailable');
		const { handler, session } = setup(disposables, gitService, octoKitService, { withSingularityToken: true, singularityApiService });
