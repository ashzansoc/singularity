...
suite('gitDiffContent', () => {
	...
	test('round-trips paths with spaces, unicode and slashes', () => {
		const sessionUri = 'singularity:/sess/with space?q=1';
	...
	test('uses the absolute path for display and the query for repo path', () => {
		const built = buildGitBlobUri('singularity:/abc', 'cafe1234', 'src/app.ts', '/work/repo/src/app.ts');
		...
		assert.deepStrictEqual({
			...
			parsed: { sessionUri: 'singularity:/abc', sha: 'cafe1234', repoRelativePath: 'src/app.ts' },
		});
	});
	...
	test('returns undefined when the query is missing or incomplete', () => {
		...
		const partialQuery = URI.from({
			scheme: 'git-blob',
			path: '/src/app.ts',
			query: JSON.stringify({ sessionUri: 'singularity:/abc', sha: 'cafe1234' }),
		}).toString();
