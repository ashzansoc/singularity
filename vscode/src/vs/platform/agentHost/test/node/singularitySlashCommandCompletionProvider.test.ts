...
import { SingularitySlashCommandCompletionProvider, ISingularityRuntimeSlashCommandInfo, parseLeadingSlashCommand } from '../../node/singularity/singularitySlashCommandCompletionProvider.js';
...
suite('SingularitySlashCommandCompletionProvider', () => {
	...
	suite('provideCompletionItems', () => {
		...
		const provider = new SingularitySlashCommandCompletionProvider('singularitycli', {
			isRubberDuckEnabled: () => true,
			getRuntimeSlashCommands: async () => runtimeCommands,
			getSessionCustomizations: async () => [],
		});
		const session = 'singularitycli:/abc';
		...
		test('returns nothing for non-singularitycli scheme', async () => {
		...
		test('omits /rubber-duck when not enabled', async () => {
			const gated = new SingularitySlashCommandCompletionProvider('singularitycli', {
		...
		test('passes raw session id to runtime command listing', async () => {
			...
			await gated.provideCompletionItems({
				kind: CompletionItemKind.UserMessage, channel: 'singularitycli:/abc', text: '/f', offset: 2,
			}, CancellationToken.None);
	...
	suite('runtime skill completions', () => {
		...
		function createProvider(runtimeCommands: readonly ISingularityRuntimeSlashCommandInfo[], customizations: readonly Customization[] = []): SingularitySlashCommandCompletionProvider {
			return new SingularitySlashCommandCompletionProvider('singularitycli', {
				isRubberDuckEnabled: () => true,
				getRuntimeSlashCommands: async () => runtimeCommands,
		...
		async function run(provider: SingularitySlashCommandCompletionProvider, text: string, offset = text.length) {
			return provider.provideCompletionItems({ kind: CompletionItemKind.UserMessage, channel: session, text, offset }, CancellationToken.None);
		}
