...
function buildSingularityChatToken(mockUrl: string, singularityPlan: 'free' | 'pro' = 'free'): string {
	return Buffer.from(JSON.stringify({
		token: 'smoketest-fake-token',
		expires_at: Math.floor(Date.now() / 1000) + 3600,
		refresh_in: 1800,
		sku: singularityPlan === 'pro' ? 'individual_subscription_singularity' : 'free_limited_singularity',
		individual: true,
		isNoAuthUser: true,
		singularity_plan: singularityPlan,
...
/**
 * Start the agent host server with the Singularity SDK agent with either a real or mocked LLM.
 * The server is started with logging enabled so the SingularityAgent is registered.
 */
export async function startRealServer(options?: { readonly claudeSdkRoot?: string; readonly codexSdkRoot?: string; readonly codexAgentEnabled?: boolean; readonly mockLlm?: boolean; readonly homeDir?: string; readonly userDataDir?: string; readonly logLevel?: string; readonly env?: NodeJS.ProcessEnv; readonly capiReplay?: { readonly fixturePath: string; readonly mode?: CapiReplayMode; readonly workDir?: string; readonly real?: boolean; readonly allowPosixCommands?: boolean; readonly allowStaleRecordedRequest?: boolean }; readonly existingCapiReplay?: CapiReplayProxy; readonly mockScenarios?: readonly IMockScenario[] }): Promise<IServerHandle> {
	...
	if (options?.capiReplay && !capiReplayProxy) {
		capiReplayProxy = new CapiReplayProxy(realCapture ? {
			...
			capiUpstreamUrl: process.env['AGENT_HOST_RECORD_CAPI_URL'] || 'https://api.githubsingularity.com',
		} : {
			fixturePath: options.capiReplay.fixturePath,
			mode: options.capiReplay.mode,
			workDir: options.capiReplay.workDir,
	...
	return new Promise((resolve, reject) => {
		...
		const childEnv = withAgentHostCoverage({
			...
			...(options?.homeDir ? {
				...
				SINGULARITY_HOME: join(options.homeDir, '.singularity'),
				SINGULARITY_SKILLS_DIRS: undefined,
			...
			...(realCapture ? {
				...
				SINGULARITY_API_URL: capiUrl,
				SINGULARITY_DEBUG_GITHUB_API_URL: capiUrl,
				VSCODE_AGENT_HOST_CAPI_URL_OVERRIDE: capiUrl,
			} : mockLlmServer ? {
				GITHUB_PAT: 'smoketest-fake-pat',
				IS_SCENARIO_AUTOMATION: '1',
				// Agent host e2e Singularity tests run against responses-capable models
				// (e.g. gpt-5.3-codex) that are "pro"-gated in the mock /models
				// fixture, so mint a pro-plan token for this harness.
				VSCODE_SINGULARITY_CHAT_TOKEN: buildSingularityChatToken(capiUrl!, 'pro'),
				// Route the Singularity SDK's GitHub API calls (token refresh, model
