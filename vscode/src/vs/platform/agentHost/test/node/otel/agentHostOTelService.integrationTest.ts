...
function makeOtlpRequest(traceId: string, spanId: string): IOtlpExportTraceServiceRequest {
	...
	return {
		resourceSpans: [{
			...
			scopeSpans: [{
				scope: { name: 'singularity.chat.agent' },
				spans: [{
					traceId,
					spanId,
					name: 'invoke_agent singularitycli',
					kind: OtlpSpanKind.INTERNAL,
					startTimeUnixNano: nowNs,
					endTimeUnixNano: endNs,
					attributes: [
						{ key: 'gen_ai.operation.name', value: { stringValue: 'invoke_agent' } },
						{ key: 'gen_ai.provider.name', value: { stringValue: 'singularity.chat' } },
						{ key: 'gen_ai.agent.name', value: { stringValue: 'singularitycli' } },
...
const OTEL_ENV_KEYS = [
	...
	'SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED',
	'SINGULARITY_OTEL_EXPORTER_TYPE',
	'SINGULARITY_OTEL_ENDPOINT',
	'SINGULARITY_OTEL_FILE_EXPORTER_PATH',
	'SINGULARITY_OTEL_SOURCE_NAME',
	'SINGULARITY_OTEL_PROTOCOL',
...
suite('platform/agentHost - AgentHostOTelService (integration)', () => {
	...
	test('readAgentHostOTelEnv: db mode implies enabled', () => {
		const cfg = readAgentHostOTelEnv({ SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED: 'true' });
	...
	test('readAgentHostOTelEnv: grpc aliases select the gRPC exporter type', () => {
		for (const protocol of ['grpc', 'http/grpc']) {
			const cfg = readAgentHostOTelEnv({
				...
				SINGULARITY_OTEL_EXPORTER_TYPE: 'otlp-http',
	...
	test('getSdkTelemetryConfig: pass-through mode returns user-configured exporter settings', async () => {
		const saved = saveEnv();
		try {
			process.env.SINGULARITY_OTEL_ENABLED = 'true';
			process.env.SINGULARITY_OTEL_EXPORTER_TYPE = 'console';
			process.env.SINGULARITY_OTEL_SOURCE_NAME = 'agent-host';
	...
	test('native SDK config splits DB traces from direct external signals', async () => {
		...
		store.add({ dispose: () => void rm(tmp, { recursive: true, force: true }).catch(() => undefined) });
		try {
			process.env.SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED = 'true';
			process.env.OTEL_EXPORTER_OTLP_ENDPOINT = 'http://collector:4318';
	...
	test('DB mode: starts loopback, persists posted spans to SQLite, and exposes db path', async () => {
		...
		try {
			...
			try {
				...
				deepStrictEqual(names, ['invoke_agent singularitycli', 'invoke_agent singularitycli']);
	...
	test('DB mode: emits session title metadata spans when content capture is enabled', async () => {
		...
		try {
			...
			svc.emitSessionTitleChanged('conv-title', 'singularitycli:/conv-title', `Updated title ${'x'.repeat(300)}`);
			await svc.flush();
			...
			try {
				...
				strictEqual(reader.getSpanAttribute(titleSpan.span_id, AgentHostSessionUriAttribute), 'singularitycli:/conv-title');
	...
	test('DB mode keeps protobuf and gRPC traces local instead of HTTP-posting the wrong wire format', async () => {
		...
		try {
			for (const protocol of ['http/protobuf', 'grpc']) {
				process.env.SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED = 'true';
				process.env.SINGULARITY_OTEL_EXPORTER_TYPE = protocol === 'grpc' ? 'otlp-grpc' : 'otlp-http';
				process.env.OTEL_EXPORTER_OTLP_ENDPOINT = 'http://collector:4318';
				process.env.OTEL_EXPORTER_OTLP_PROTOCOL = protocol;
				let fetchCalls = 0;
				const tmp = await mkdtemp(join(tmpdir(), 'vscode-otel-svc-'));
	...
	test('DB mode + external endpoint: outbound forwarder is configured (best-effort)', async () => {
		...
		try {
			process.env.SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED = 'true';
			process.env.SINGULARITY_OTEL_EXPORTER_TYPE = 'otlp-http';
