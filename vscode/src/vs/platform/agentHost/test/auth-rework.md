...
interface IAuthScheme {
	 ...
	 * per agent provider).
	 *
	 * Example: "github" for GitHub Singularity auth.
	 */
	id: string;
...
## Concrete Example: GitHub Singularity Auth

### Server-side (SingularityAgent)

When the Singularity agent registers, it publishes an auth scheme:

```typescript
// In SingularityAgent.getAuthSchemes():
[{
	scheme: 'bearer',
	id: 'github',
	label: 'GitHub',
...
2. **Per-agent vs. global auth**: The current design has one `resourceMetadata` for the whole server. Should auth schemes be per-agent-provider instead? Per-agent gives finer control (e.g. "Singularity needs GitHub, MockAgent needs nothing") but complicates the protocol. The current proposal uses global metadata with `schemeId` correlation, which the server can internally route to the right agent.
