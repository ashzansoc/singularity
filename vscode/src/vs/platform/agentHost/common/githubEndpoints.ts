...
export interface IGitHubEndpoints {
	 ...
	 * or `undefined` for github.com. Used to point the Singularity CLI at an enterprise
	 * host via `SINGULARITY_GH_HOST`.
	 */
	readonly enterpriseHost: string | undefined;
}

/**
 * The github.com Singularity API host. Distinct from {@link IGitHubEndpoints.apiBaseUri}
 * (`api.github.com`): enterprise installs override this per user via the Singularity token's
 * `endpoints.api`, which is not derivable from the enterprise URI.
 */
export const GITHUB_DOT_COM_SINGULARITY_API_BASE_URI = 'https://api.githubsingularity.com';
...
/**
 * Derives the official GitHub MCP server URL from the per-user Singularity API
 * endpoint returned by `/singularity_internal/user`.
 */
export function gitHubMcpServerUrl(singularityApiBaseUri: string | undefined): string | undefined {
	try {
		const uri = URI.parse(singularityApiBaseUri ?? GITHUB_DOT_COM_SINGULARITY_API_BASE_URI, true);
		if (!uri.authority) {
			return undefined;
		}
...
/**
 * The GitHub Singularity protected resource for the given endpoints. Shared by the
 * endpoint service and tests so the resource identity is defined once.
 */
export function gitHubSingularityResource(endpoints: IGitHubEndpoints): ProtectedResourceMetadata {
	return {
