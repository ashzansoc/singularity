...
export interface UsageInfoMeta {
	...
	cost?: number;
	/** The concrete model selected by Singularity Auto and the routing explanation. */
	autoModeResolved?: IAutoModeResolvedInfo;
	/** Singularity-specific usage breakdown, including nano-AIU totals. */
	singularityUsage?: {
...
export function readUsageInfoMeta(usage: UsageInfo | undefined): UsageInfoMeta {
	...
	const singularityUsage = meta['singularityUsage'];
	if (singularityUsage && typeof singularityUsage === 'object' && !Array.isArray(singularityUsage)) {
		const rawUsage = singularityUsage as Record<string, unknown>;
		const usage: Mutable<NonNullable<UsageInfoMeta['singularityUsage']>> = {};
 ...
 * routing metadata — notably a Singularity Auto turn restored from the event log,
...
export function hasReportedUsage(usage: UsageInfo | undefined): boolean {
	...
	return (typeof meta.singularityUsage?.totalNanoAiu === 'number' && meta.singularityUsage.totalNanoAiu >= 0)
		...
		|| (typeof meta.singularityUsage?.sessionTotalNanoAiu === 'number' && meta.singularityUsage.sessionTotalNanoAiu >= 0)
 ...
 * both the orchestrator (`AgentService` restore/list) and agents (e.g. `SingularityAgent`
...
/**
 * `_meta` key marking a session as an un-adopted legacy Singularity CLI session
...
/** Whether the session is an un-adopted legacy Singularity CLI session surfaced as adoptable. */
