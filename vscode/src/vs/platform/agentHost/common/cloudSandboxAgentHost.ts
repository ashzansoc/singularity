...
// Contracts for connecting to Singularity cloud "sandbox" sessions (agent slug
// `singularity-developer-cli`), which run the Singularity agent runtime (`singularityd`) inside a Mission
...
/**
 * Session URI scheme the Singularity host uses when it creates and lists sessions
...
/** The agent provider advertised by the Singularity host inside the sandbox. */
export const CLOUD_SANDBOX_AGENT_PROVIDER = 'singularity';

/** Prefix of a sealed token envelope (`singularity-sealed.v1.<key_id>.<b64url>`). */
export const CLOUD_SANDBOX_SEALED_TOKEN_PREFIX = 'singularity-sealed.v1.';
...
/**
 * The Singularity cloud agent slug whose tasks run inside a Mission Control sandbox.
 *
 * Sandbox tasks carry this slug today, which is why {@link CloudSandboxEnabledSettingId} exists and
 * defaults to off: the Singularity extension's cloud provider lists tasks by an allowlist of cloud
 * slugs (`singularity-developer`, `singularity-swe-agent`) that excludes this one, so the two providers do
 ...
 */
export const CLOUD_SANDBOX_AGENT_SLUG = 'singularity-developer-cli';

/** A sandbox session discovered from the Singularity task list, enough to seed a session entry. */
...
export interface ICloudSandboxClientToken {
	...
	/** Opaque `singularity-sealed.v1.<key_id>.<b64url>` envelope, when MC sealed a token. */
...
export interface ICloudSandboxEnvironment {
	...
	readonly capabilities?: {
		/** The singularityd **host** version (e.g. `0.6.3`), not the AHP wire protocol version. */
 ...
 * Protocol relay connections to Singularity cloud sandbox environments. Mints
