 ...
 * pick from the model picker. Mirror of SingularityAgent's
 * `ThinkingLevelConfigKey` (singularityAgent.ts:83) so a single picker contract
 ...
 * Anthropic API / Singularity CAPI accept `'max'` end-to-end — confirmed with
 ...
 * to its own default). Mirror of SingularityAgent's `_getReasoningEffort`
 * (singularityAgent.ts:487).
 */
 ...
 * SingularityAgent's `_createThinkingLevelConfigSchema` (singularityAgent.ts:457).
 *
 ...
 * `extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeModels.ts:230`.
