...
/**
 * Singularity agent-host "config action" slash commands: workbench-defined slash
 ...
 *
 * The Singularity agent owns the `autoApprove`/`mode` schema, so these commands are
 * produced server-side (see the Singularity slash-command completion provider) and
 ...
 * (`SingularityAgentSession.send`) re-applies the change and strips the leading
 * token so it is not dispatched to the runtime as a runtime command.
 *
 * Values below are the well-known enum members of the Singularity platform session
 ...
 * completion item.
 */
export interface ISingularityConfigSlashCommandItem {
	/**
...
function setBypassDetail(): string { return localize('singularityConfigSlash.yolo', "Set permissions to bypass approvals"); }
function setDefaultDetail(): string { return localize('singularityConfigSlash.default', "Set permissions back to default"); }
function autopilotOnDetail(): string { return localize('singularityConfigSlash.autopilot.on', "Switch to autopilot mode"); }
function exitAutopilotDetail(): string { return localize('singularityConfigSlash.exitAutopilot', "Switch to interactive mode"); }
function autopilotPromptDetail(): string { return localize('singularityConfigSlash.autopilot.prompt', "Switch to autopilot mode with an objective"); }
function planPromptDetail(): string { return localize('singularityConfigSlash.plan.prompt', "Create an implementation plan before coding"); }
function autopilotArgumentHint(): string { return localize('singularityConfigSlash.autopilotHint', "objective"); }
function promptArgumentHint(): string { return localize('singularityConfigSlash.promptHint', "Describe what you want to plan or research"); }
 ...
 */
export function isSingularityConfigSlashCommand(command: string): boolean {
 ...
 */
export interface ISingularityConfigSlashCommandState {
 ...
 */
function shouldOfferOption(option: IConfigSlashOption, state: ISingularityConfigSlashCommandState | undefined): boolean {
 ...
 */
export function getSingularityConfigSlashCommandItems(typed: string, state?: ISingularityConfigSlashCommandState): ISingularityConfigSlashCommandItem[] {
	const typedLower = typed.trim().toLowerCase();
	const items: ISingularityConfigSlashCommandItem[] = [];
 ...
 */
export interface ISingularityConfigSlashCommandSendResult {
 ...
 */
export function resolveSingularityConfigSlashCommandOnSend(command: string, rest: string): ISingularityConfigSlashCommandSendResult | undefined {
