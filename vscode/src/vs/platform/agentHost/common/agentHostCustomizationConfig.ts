...
export const enum AgentHostConfigKey {
	 ...
	 * `messages` traffic through the local Singularity-CAPI proxy (Singularity-routed
	 * Claude). When false, the Claude Agent SDK talks to Anthropic directly on
	 * the user's own credentials (BYO Anthropic — Phase 19).
	 */
	ClaudeUseSingularityProxy = 'claudeUseSingularityProxy',
	/**
...
export const agentHostCustomizationConfigSchema = createSchema({
	...
	[AgentHostConfigKey.ClaudeUseSingularityProxy]: schemaProperty<boolean>({
		type: 'boolean',
		title: localize('agentHost.config.claudeUseSingularityProxy.title', "Route Claude Through Singularity"),
		description: localize('agentHost.config.claudeUseSingularityProxy.description', "When enabled (the default), the Claude agent routes all requests through GitHub Singularity. When disabled, Claude talks to Anthropic directly using your own credentials (API key or Claude subscription)."),
		default: true,
	}),
	...
	[AgentHostConfigKey.SessionCustomizationDiscoveryMode]: schemaProperty<SessionCustomizationDiscoveryMode>({
		...
		description: localize('agentHost.config.sessionCustomizationDiscoveryMode.description', "Controls whether session-scoped customizations are populated from local file scanning or from Singularity SDK discovery."),
