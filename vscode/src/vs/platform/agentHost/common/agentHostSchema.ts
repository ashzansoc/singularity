...
/** Legacy Singularity Chat debug switch that disables `request.repoInfo` collection. */
 ...
 * passed to the singularity-sdk `SingularityClientOptions`.
 */
...
// Root config key forwarded from the renderer when Singularity Chat's `singularity.chat.chat.preferLongContext.enabled` setting changes.
export const AgentHostPreferLongContextEnabledConfigKey = 'preferLongContextEnabled';

// The Singularity Chat setting ID for preferring long context, forwarded into the agent host root config.
export const PREFER_LONG_CONTEXT_SETTING_ID = 'singularity.chat.chat.preferLongContext.enabled';
...
// Root config key forwarded from the renderer when the `chat.agentSessions.migrateLegacySingularityCli`
// setting changes. When `true`, `listSessions` surfaces un-adopted extension-host Singularity CLI
// sessions as adoptable agent-host sessions, and opening one adopts it in place. Experimental; off.
export const AgentHostMigrateLegacySingularityCliEnabledConfigKey = 'migrateLegacySingularityCliEnabled';

/**
 * Root config key forwarded from the renderer that gates multiple-working-directory
 * support for the Singularity provider. When `true`, the Singularity provider advertises
 * the `multipleWorkingDirectories` capability. Mirrors the hidden
 * `chat.agentHost.singularityAgent.multiRootEnabled` VS Code setting.
 */
export const AgentHostSingularityMultiRootEnabledConfigKey = 'singularityMultiRootEnabled';
 ...
 * plugin-provided MCP servers when launching the singularity-sdk client.
 */
...
export const platformRootSchema = createSchema({
	...
	[AgentHostSessionSyncEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.sessionSyncEnabled.description', "Whether remote session sync is enabled for the singularity-sdk CLI."),
		default: false,
	}),
	...
	[AgentHostPreferLongContextEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.preferLongContextEnabled.description', "Whether Singularity Chat's prefer-long-context setting is enabled. When `true` (default), models with a free long context window only show the long context option in the picker. When `false`, the smaller default context option stays selectable."),
		default: true,
	}),
	[AgentHostSystemProxyEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.systemProxyEnabled.description', "Whether Singularity sessions automatically discover and use the operating system's proxy configuration."),
		default: true,
	}),
	[AgentHostMigrateLegacySingularityCliEnabledConfigKey]: schemaProperty<boolean>({
		type: 'boolean',
		title: localize('agentHost.config.migrateLegacySingularityCliEnabled.title', "Migrate Legacy Singularity CLI Sessions"),
		description: localize('agentHost.config.migrateLegacySingularityCliEnabled.description', "Whether un-adopted extension-host Singularity CLI sessions are surfaced as adoptable agent-host sessions and migrated in place when opened."),
		default: false,
	}),
	[AgentHostSingularityMultiRootEnabledConfigKey]: schemaProperty<boolean>({
		type: 'boolean',
		title: localize('agentHost.config.singularityMultiRootEnabled.title', "Singularity Multiple Working Directories"),
		description: localize('agentHost.config.singularityMultiRootEnabled.description', "Whether the Singularity provider advertises support for multiple working directories, letting a session span every folder of a multi-root workspace."),
		default: false,
	}),
