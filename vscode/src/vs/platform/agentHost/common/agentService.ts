...
/** Configuration key controlling automatic OS system proxy discovery for agent-host Singularity sessions. */
export const AgentHostSystemProxyEnabledSettingId = 'chat.agentHost.systemProxy.enabled';

/**
 * Configuration key gating multiple-working-directory support for the Singularity
 * agent-host provider. When `true`, the Singularity provider advertises the
 ...
 */
export const AgentHostSingularityMultiRootEnabledSettingId = 'chat.agentHost.singularityAgent.multiRootEnabled';
 ...
 * {@link AgentHostSingularityMultiRootEnabledSettingId} because the Claude Agent SDK
 * already supports additional directories while the Singularity SDK does not. Hidden
...
// The Singularity-CLI-specific setting IDs (`customTerminalTool`, `opus48Prompt`,
// `reasoningEffortOverride`, `modelCapabilityOverrides`) live with their
// root-config keys in `singularityCliConfig.ts`.
 ...
 * workbench surfaces the agent host's Claude provider (vs. the GitHub Singularity
...
/**
 * Configuration key that controls the sandbox mode for the Singularity SDK's built-in
 ...
 *  - `false` — Claude is provided by the GitHub Singularity Chat extension.
 *
...
// runtime documents in `extensions/singularity-chat/docs/monitoring/agent_monitoring.md`,
 ...
 * `extensions/singularity-chat/docs/monitoring/agent_monitoring.md` so the same external
 * tooling and `OTEL_EXPORTER_OTLP_*` config recipes work unchanged.
 */
export const AgentHostOTelEnvVars = Object.freeze({
	Enabled: 'SINGULARITY_OTEL_ENABLED',
	ExporterType: 'SINGULARITY_OTEL_EXPORTER_TYPE',
	OtlpEndpoint: 'OTEL_EXPORTER_OTLP_ENDPOINT',
	OtlpEndpointAlt: 'SINGULARITY_OTEL_ENDPOINT',
	...
	FilePath: 'SINGULARITY_OTEL_FILE_EXPORTER_PATH',
	SourceName: 'SINGULARITY_OTEL_SOURCE_NAME',
	ServiceName: 'OTEL_SERVICE_NAME',
	ResourceAttributes: 'OTEL_RESOURCE_ATTRIBUTES',
	DbSpanExporterEnabled: 'SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED',
} as const);
...
export interface IAgentHostManagementService {
	...
	/** Spawn a real execution-owned subagent for a DAG task. */
	executeExecutionTask(request: IAgentHostExecutionTaskRequest): Promise<IAgentHostExecutionTaskResult>;
}

export interface IAgentHostExecutionTaskRequest {
...
export interface IAgentHostExecutionTaskResult {
...
/**
 * Canonical {@link ProtectedResourceMetadata} for the GitHub Singularity
 * resource. Shared between every agent provider that consumes a GitHub
 * Singularity bearer token (e.g. Singularity CLI, Claude) so they advertise an
 ...
 */
export const GITHUB_SINGULARITY_PROTECTED_RESOURCE: ProtectedResourceMetadata = {
 ...
 * {@link GITHUB_SINGULARITY_PROTECTED_RESOURCE} so that the broader `repo`
 ...
 * user to be signed in to GitHub Singularity right now?
 *
 * Returns `true` iff the agent advertises the canonical GitHub Singularity
 * protected resource ({@link GITHUB_SINGULARITY_PROTECTED_RESOURCE}) with
 ...
 */
export function protectedResourcesRequireSingularitySignIn(resources: readonly ProtectedResourceMetadata[]): boolean {
	return resources.some(resource =>
		resource.resource === GITHUB_SINGULARITY_PROTECTED_RESOURCE.resource && resource.required !== false);
}
...
export interface IAgentCreateSessionConfig {
	 ...
	 * Singularity event log seeded on disk and resumes the session so the turns are
...
export namespace AgentSession {
	 ...
	 * The URI scheme is the provider name (e.g., `singularity:/<rawId>`).
	 */
...
/**
 * Implemented by each agent backend (e.g. Singularity SDK).
 * The {@link IAgentService} dispatches to the appropriate agent based on
 * the agent id.
 */
export interface IAgent {
	/** Unique identifier for this provider (e.g. `'singularity'`). */
	 ...
	 * out of band (e.g. a legacy Singularity CLI session was created by the extension
	 ...
	 * extension-host Singularity CLI): if `session` has an on-disk SDK event log but
	 ...
	 * singularity) use this to route the completion to the right chat;
...
export interface IAgentService {
	...
	/** List all available sessions from the Singularity CLI. */
	listSessions(): Promise<IAgentSessionMetadata[]>;
...
export interface IAgentConnection {
	 ...
	 * remote), so the result reflects the environment the Singularity SDK actually
	 * runs in.
	 */
	getNetworkDiagnosticsInfo(): Promise<IAgentHostNetworkDiagnosticsInfo>;
	 ...
	 * environment the Singularity SDK actually runs in.
	 */
	diagnosticsFetch(url: string): Promise<IAgentHostNetworkFetchResult>;
