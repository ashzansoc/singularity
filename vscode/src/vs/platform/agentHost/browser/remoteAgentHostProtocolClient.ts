	...
	async connect(): Promise<void> {
		try {
			...
			const result = await this._dispatchRequest<CommandMap['initialize']['result']>('initialize', {
				...
				// older host (a cloud sandbox running a 0.5.x `singularityd`) can negotiate down
			...
			if (pending) {
				...
				if (hasKey(msg, { error: true })) {
					if (this._shouldLogFailedRequest(pending, msg.error)) {
						this._logService.warn(`[RemoteAgentHostProtocol] Request ${msg.id} failed: ${msg.error.message}`, msg.error);
					}
