<!--
  ...
  See: node/agentService.ts, node/agentHostStateManager.ts,
       node/claude/claudeAgent.ts, node/singularity/singularityAgent.ts,
...
> Singularity all use the unified orchestrator path.
...
A session URI (`ahp-singularity://`, `ahp-claude://`, …) identifies a session. A chat channel URI (`ahp-chat://…`) identifies a chat within a session. The two schemes are structurally distinct; `isAhpChatChannel` / `parseDefaultChatUri` / `buildDefaultChatUri` are the only crossing points. Passing a chat URI where a session URI is expected (or vice versa) is a bug.
...
graph LR
    subgraph SessionsUI["Sessions UI (workbench process)"]
        provider["agentHostSessionsProvider<br/>(singularityChatSessionsProvider)"]
    ...
    subgraph Harnesses["Agent Harnesses"]
        ...
        singularity["SingularityAgent\n_sessions: DisposableMap<id, SingularitySessionEntry>\n_chatBackings: Map<chatUri, IPersistedChat>"]
...
sequenceDiagram
    ...
    else catalog absent (undefined) — one-time legacy migration (Singularity only)
        AS->>A: listLegacyChats(session) [legacy singularity.chats]
...
Each peer chat is backed by a fresh top-level SDK session (`sdkSessionId = generateUuid()`) minted in the same global Claude project store that `listSessions` enumerates. `_createChat` therefore returns `backingSession: AgentSession.uri(this.id, sdkSessionId)` so the orchestrator can suppress that backing from the top-level session list (invariant I7); without it the peer chat would leak as a phantom session. The SDK exposes no delete-chat RPC, so `disposeChat` leaves the backing transcript on disk — the orchestrator-owned catalog simply drops the entry so it is never resumed again. (Claude writes no legacy `claude.chats` blob and has no legacy migration: Claude multi-chat shipped only with the orchestrator-owned catalog, so there is nothing to drain. Singularity keeps its own `singularity.chats` migration because `singularity.chats` predates the catalog.)

### Singularity (`node/singularity/singularityAgent.ts`)

F2 complete (2026-07-01): single `_sessions: DisposableMap<string, SingularitySessionEntry>` keyed by session id; the parallel `_chatSessions` map has been removed.

`SingularitySessionEntry` (`singularityAgent.ts:SingularitySessionEntry`) is an empty subclass of the shared `AgentSessionEntry<SingularityAgentSession>` (`node/agentPeerChats.ts`) — its API matches the base exactly. It is a `Disposable` container holding ALL chats (default + peers) in ONE map keyed by chat URI string:
- `_chats: DisposableMap<string, AgentSessionEntry<SingularityAgentSession>>` — every chat (default + peers) as a leaf entry.
- `defaultChat: SingularityAgentSession | undefined` — the default chat via `_defaultChatKey`; `undefined` while the session is still provisional (not yet materialized).
