...
The **agent host** is a separate utility process (under `src/vs/platform/agentHost/`) that hosts native Singularity, Claude, and Codex runtimes instead of using the extension's in-process harnesses. The agent host has its own OTel pipeline so provider-native traces can be exported to a collector or persisted locally for inspection.

> **Availability:** Insiders / non-stable builds only.

This doc lives next to the code (`IAgentHostOTelService` in [node/otel/agentHostOTelService.ts](node/otel/agentHostOTelService.ts)) because the agent host runs entirely outside the extension host and is independent of the extension-side OTel pipeline (`singularity.chat.chat.otel.*`) documented in `extensions/singularity-chat/docs/monitoring/`.

| Property | Agent Host OTel | Extension OTel |
|---|---|---|
| Process | Separate utility process (`src/vs/platform/agentHost/node/`) | Extension host |
| Settings prefix | `chat.agentHost.otel.*` | `singularity.chat.chat.otel.*` |
| Service | `IAgentHostOTelService` (`node/otel/agentHostOTelService.ts`) | `IOTelService` (`extensions/singularity-chat/src/platform/otel/`) |
| SDK | Singularity `TelemetryConfig`, Claude environment, and Codex `otel.*` launch overrides | `@opentelemetry/sdk-node` directly |
...
```
   ...
   Host.otel.*   ─→  │ singularity-sdk             │ ─────────────── │ user-config  │  │
                 │   │ TelemetryConfig         │                 │ OTLP sink    │  │
   spawn-time    │   └─────────────────────────┘                 └──────────────┘  │
   env binding   │                                                                 │
   in            │   db mode (dbSpanExporter.enabled)                              │
   electron-     │   ┌─────────────────────────┐    127.0.0.1    ┌──────────────┐  │
   AgentHostStar ─→  │ singularity-sdk             │ ─────────────── │ loopback     │  │
...
|---|---|---|
| Singularity | SDK `TelemetryConfig` plus a trace-specific endpoint | Metrics |
...
| Singularity runtime | `github-singularity` |
...
|---|---|---|
| `SINGULARITY_OTEL_ENABLED` | `chat.agentHost.otel.enabled` | Set to `true` only when the setting is on. |
| `SINGULARITY_OTEL_EXPORTER_TYPE` | `chat.agentHost.otel.exporterType` | |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | `chat.agentHost.otel.otlpEndpoint` | Standard OTel env var. `SINGULARITY_OTEL_ENDPOINT` is also accepted as an alternate and takes precedence over `OTEL_EXPORTER_OTLP_ENDPOINT`. |
| `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` | `chat.agentHost.otel.captureContent` | |
| `SINGULARITY_OTEL_FILE_EXPORTER_PATH` | `chat.agentHost.otel.outfile` | |
| `SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED` | `chat.agentHost.otel.dbSpanExporter.enabled` | |
...
| `OTEL_EXPORTER_OTLP_HEADERS` | (inherited) | Auth headers (e.g., `Authorization=Bearer …`). **Not** delivered from managed settings — env delivery would leak the secret to tool subprocesses; managed headers apply to the Singularity Chat extension only. |
...
| Setting | Env var |
|---|---|
| `chat.agentHost.otel.enabled` | `SINGULARITY_OTEL_ENABLED` |
| `chat.agentHost.otel.exporterType` | `SINGULARITY_OTEL_EXPORTER_TYPE` |
| `chat.agentHost.otel.otlpEndpoint` | `OTEL_EXPORTER_OTLP_ENDPOINT` (`SINGULARITY_OTEL_ENDPOINT` also accepted, takes precedence) |
| `chat.agentHost.otel.captureContent` | `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` |
| `chat.agentHost.otel.outfile` | `SINGULARITY_OTEL_FILE_EXPORTER_PATH` |
| `chat.agentHost.otel.dbSpanExporter.enabled` | `SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED` |
