...
import { SingularityApiError, ISingularityApiService } from '../shared/singularityApiService.js';
 ...
 * to CAPI using the **current** GitHub Singularity token — which can rotate
...
export interface ICodexProxyHandle extends ILoopbackProxyHandle {
	...
	/**
	 * Replace the GitHub Singularity token used for outbound CAPI calls. The
...
interface ICodexProxyState {
	 ...
	 * model that is known to be supported by the Singularity CAPI. `undefined`
 ...
 * Singularity CAPI catalog, so forwarding it verbatim yields a 400
 ...
 * in the Singularity Chat extension, which tags Codex requests with the same
 * prefix.
 */
 ...
 * side and forwards to {@link ISingularityApiService.responses} on the
...
export class CodexProxyService extends LoopbackProxyServer<ICodexProxyState, string> implements ICodexProxyService {
	...
	private async _handleResponses(
		...
		// against the Singularity CAPI (which does not expose `codex-auto-review`).
		...
		try {
			...
			const upstream = await this._singularityApiService.responses(dispatchedToken, body, { headers, signal: entry.ac.signal, suppressIntegrationId: true });
 ...
 * transform in `oaiLanguageModelServer.ts` in the Singularity Chat extension,
