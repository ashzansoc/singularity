...
// thread — the same set Singularity passes to its SDK via
 ...
 * or an `http` server with a string `url`. Mirrors Singularity's
...
// the *same* mechanism the Singularity agent uses: report the server as
 ...
 * Singularity agent's normalization: strips the fragment and any trailing slashes
