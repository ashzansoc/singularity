...
/**
 * `supported_endpoints` value (on a Singularity CAPI {@link CCAModel}) that marks
...
const CODEX_SINGULARITY_MODEL_PROVIDER = 'vscode-proxy';
...
export function parseCodexModelSelection(selection: ModelSelection): { readonly modelProvider: string; readonly modelId: string } {
	if (!selection.id.startsWith(CODEX_MODEL_SELECTION_PREFIX)) {
		return { modelProvider: CODEX_SINGULARITY_MODEL_PROVIDER, modelId: selection.id };
	}
 ...
 * read-only peer chat (the "agent team" pattern, mirroring Singularity/Claude) by
...
export class CodexAgent extends Disposable implements IAgent {
	...
	private _singularityModels: readonly IAgentModelInfo[] = [];
	...
	getProtectedResources(): ProtectedResourceMetadata[] {
		return [
			{ ...this._gitHubEndpointService.getSingularityResource(), required: false },
	 ...
	 * codex end of the *same* OAuth mechanism the Singularity agent uses: the
	...
	private _ensureModelProviderAuthenticated(model: ModelSelection | undefined): void {
		const modelProvider = model ? parseCodexModelSelection(model).modelProvider : CODEX_SINGULARITY_MODEL_PROVIDER;
		if (modelProvider !== CODEX_SINGULARITY_MODEL_PROVIDER) {
			return;
		}
	...
	private async _refreshModels(): Promise<void> {
		await Promise.all([this._refreshSingularityModels(), this._refreshCodexModels()]);
		this._models.set([...this._singularityModels, ...this._codexModels], undefined);
	}

	private async _refreshSingularityModels(): Promise<void> {
		const token = this._githubToken;
		if (!token) {
			this._singularityModels = [];
			return;
		}
		try {
			...
			// can only drive models that expose Singularity CAPI's OpenAI-shaped
			...
			const models = all
				...
				.map((m): IAgentModelInfo => ({
					...
					id: toCodexModelSelectionId(CODEX_SINGULARITY_MODEL_PROVIDER, m.id),
					name: m.name ?? m.id,
			...
			this._singularityModels = models;
		} catch (err) {
	...
	respondToPermissionRequest(requestId: string, approved: boolean): void {
		...
		// own registry) and resolve the first match. Mirrors Claude/Singularity.
		const sessions = [
	...
	async getSessionMetadata(session: URI): Promise<IAgentSessionMetadata | undefined> {
		...
		if (!this._sessions.has(sessionId)) {
			...
			const restored = this._createResumedSessionEntry(sessionId, threadId, session, workingDirectory, restoredModel, metadata.workingDirectories, undefined, overlay.agent);
	...
	async listSessions(): Promise<IAgentSessionMetadata[]> {
		...
		try {
			...
			// `Promise.all`, so a rejection here takes the sibling Singularity and
	 ...
	 * so the workbench runs the *same* OAuth sign-in it uses for the Singularity
	 ...
	 * here, mirroring the discovery the Singularity SDK does internally. On
