 ...
 * `extensions/singularity-chat/src/platform/github/common/githubService.ts` so the
 ...
 * `OctoKitService` from the Singularity extension so the API surface is
 ...
 * on session create via the same channel used for `ISingularityApiService`.
 */
export interface IAgentHostOctoKitService {
	 ...
	 *
	 * Mirrors `OctoKitService.createPullRequest` from the Singularity extension.
