...
import { SingularityApiError, SINGULARITY_API_ERROR_STATUS_STREAMING } from './singularityApiService.js';
 ...
 * (the agent host) process and hold the rich {@link SingularityApiError}, but the
 * agent SDKs (Claude, Codex, Singularity CLI) run as child processes that only
 ...
 *
 * Mirrors the Singularity Chat extension's `PROXY_ERROR_PREFIX`
 * (`extensions/singularity-chat/src/extension/chatSessions/claude/common/claudeMessageDispatch.ts`).
 */
...
/**
 * Maps a {@link SingularityApiError} HTTP status (or the mid-stream streaming
 * sentinel) to the extension's `ChatFetchResponseType` string value. Kept in
 * sync with the Singularity Chat extension's error classification so the core
...
/**
 * Builds a {@link IForwardedChatError} from a {@link SingularityApiError}. The
 ...
 */
export function buildForwardedChatError(err: SingularityApiError): IForwardedChatError {
...
/**
 * Fields from a structured agent-SDK error (notably the Singularity CLI SDK's
 * `ErrorData`) used to build a forwarded chat error directly, without a
 * {@link PROXY_ERROR_PREFIX} marker. The Singularity CLI authenticates with CAPI
 ...
 * Singularity CLI SDK's `ErrorData.errorType` values.
 */
...
export function buildForwardedChatErrorFromFields(data: ISdkChatErrorFields): IForwardedChatError | undefined {
	...
	// The Singularity CLI SDK classifies quota errors only via `errorType: 'quota'`
