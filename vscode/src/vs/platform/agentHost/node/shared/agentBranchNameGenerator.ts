...
import { ISingularityApiService, type ISingularityUtilityChatMessage } from './singularityApiService.js';

/**
 * Branch-name prefix for worktree-isolated agent sessions, e.g.
 * `agents/add-feature`. Shared by every agent-host provider (Singularity, Codex,
...
export class AgentBranchNameGenerator implements IAgentBranchNameGenerator {
	...
	private async _generateBranchNameHint(request: IAgentBranchNameGeneratorRequest): Promise<string | undefined> {
		...
		try {
			const rawBranchName = await this._singularityApiService.utilityChatCompletion(request.githubToken, {
	...
	private _buildBranchNamePrompt(userRequest: string): ISingularityUtilityChatMessage[] {
		return [
			{
				role: 'system',
				content: [
