...
import { ISingularityApiService } from './singularityApiService.js';
 ...
 * historical `singularity.worktree.*` prefix so sessions materialized by earlier
 * Singularity builds keep resolving their worktree on archive / unarchive /
 * restore after this logic was unified across agents. All agents (Singularity,
 ...
 */
const WORKTREE_META_BRANCH = 'singularity.worktree.branchName';
const WORKTREE_META_PATH = 'singularity.worktree.path';
export const WORKTREE_META_REPOSITORY_ROOT = 'singularity.worktree.repositoryRoot';
const WORKTREE_META_CREATION_FAILURE = 'singularity.worktree.creationFailure';
 ...
 * full machinery Singularity pioneered so Codex and Claude get identical behavior:
 *
...
export class WorktreeIsolation extends Disposable {
	...
	/** Branch-name generator for worktree sessions; created from {@link ISingularityApiService} unless a test supplies an override. */
	...
	constructor(
		...
		super();
		this._branchNameGenerator = branchNameGenerator ?? new AgentBranchNameGenerator(singularityApiService, this._logService);
	}
	 ...
	 * the provider is torn down, matching Singularity's shutdown drain.
	 */
	 ...
	 * like Singularity. Returns the repository root as the project so agents can merge
 ...
 * the URI string for pathological roots), matching how Singularity names the
