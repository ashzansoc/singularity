 ...
 * Singularity CLI (via @github/copilot-sdk) accept canonical tool schemas
 * whose shapes we can read structurally — Claude uses PascalCase names
 * (`Write`, `Edit`, `MultiEdit`) with `_string` fields, Singularity uses
 ...
 * file-edit tool (`isClaudeFileEditTool`, `isEditTool` in Singularity) has
 ...
 *
 * Supported Singularity CLI tools (one file per call unless noted):
 ...
 * notebooks.
 *
 * @param toolName  Tool identifier (Claude PascalCase or Singularity
...
/**
 * Headers of the V4A patch format the Singularity `apply_patch` tool
 * accepts. Mirrors {@link singularityToolDisplay.APPLY_PATCH_FILE_HEADERS};
 ...
 * order.
 *
 * The Singularity SDK delivers `apply_patch` with `arguments` as a raw
