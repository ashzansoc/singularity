 ...
 * (workspace-less chat) session: `<userHome>/.singularity/chats/<sessionId>`. Shared by the
 * Singularity and Claude agents so both resolve the same cwd for a session that was
 * created with no `workingDirectory`.
 */
export function workspacelessScratchDir(userHome: URI, sessionId: string): URI {
	return joinPath(userHome, '.singularity', 'chats', sessionId);
}
