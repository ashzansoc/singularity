...
import { ISingularityApiService, type ISingularityUtilityChatMessage } from './shared/singularityApiService.js';
 ...
 *
 * The flow mirrors the Singularity CLI extension's `createPullRequest` helper
 * (`extensions/singularity-chat/src/extension/chatSessions/vscode-node/singularityCLIChatSessionsContribution.ts`):
 *
...
export class AgentHostPullRequestOperationHandler implements IChangesetOperationHandler {
	 ...
	 * Singularity token is available or generation fails, so the caller can fall
	...
	private async _generateTitleAndDescription(
		...
		const singularityToken = this._agentService.getAuthToken({
			resource: singularityResource.resource,
			scopes: singularityResource.scopes_supported,
		});
		if (!singularityToken) {
			return undefined;
		}
		...
		try {
			const raw = await this._singularityApiService.utilityChatCompletion(singularityToken, {
	...
	private _buildTitleAndDescriptionPrompt(branchName: string, base: string, conversation: string | undefined, changeSummary: string): ISingularityUtilityChatMessage[] {
