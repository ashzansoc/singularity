...
import { SingularityAgent } from './singularity/singularityAgent.js';
...
import { IByokLmProxyService, NullByokLmProxyService } from './singularity/byokLmProxyService.js';
...
import { registerPendingEditContentProvider } from './singularity/pendingEditContentStore.js';
...
async function main(): Promise<void> {
	...
	if (!options.quiet) {
		...
		diServices.set(IAgentHostGitService, gitService);
		// Register `ISingularityApiService` BEFORE `IClaudeProxyService` —
		// the proxy service constructor requires it.
		const singularityApiService = instantiationService.createInstance(SingularityApiService, fetchFn);
		...
		// owns its branch-name generator, created from ISingularityApiService.
		...
		// to satisfy SingularityAgent / SingularitySessionLauncher DI.
		...
		const singularityAgent = disposables.add(instantiationService.createInstance(SingularityAgent));
		agentService.registerProvider(singularityAgent);
		log('SingularityAgent registered');
