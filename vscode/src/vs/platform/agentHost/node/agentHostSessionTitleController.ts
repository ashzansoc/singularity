...
export class AgentHostSessionTitleController extends Disposable {
	...
	private async _generateTitleFromPrompt(promptContent: string, isConversation: boolean, token: CancellationToken): Promise<string | undefined> {
		...
		const githubToken = this._options.getSingularityToken?.();
		const singularityApiService = this._options.singularityApiService;
		if (!githubToken || !singularityApiService) {
			return undefined;
		}
		...
		try {
			...
			const rawTitle = await singularityApiService.utilityChatCompletion(githubToken, {
	...
	private _buildTitlePrompt(promptContent: string, isConversation: boolean): ISingularityUtilityChatMessage[] {
