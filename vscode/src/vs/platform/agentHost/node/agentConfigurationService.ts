...
export class AgentConfigurationService extends Disposable implements IAgentConfigurationService {
	...
	constructor(
		...
		const singularityCliSchema = singularityCliConfigSchema.toProtocol();
		this._stateManager.rootState.config = {
			schema: {
				type: 'object',
				properties: { ...existing?.schema.properties, ...ownSchema.properties, ...sandboxSchema.properties, ...singularityCliSchema.properties },
			},
	...
	private _loadPersistedRootConfig(): Record<string, unknown> {
		...
		try {
			...
			return {
				...
				...singularityCliConfigSchema.validateOrDefault(parsed, {}),
			};
		} catch (err) {
