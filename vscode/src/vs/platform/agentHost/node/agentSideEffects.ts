...
import { isExecutionToolCallId } from './execution/executionSubagentIds.js';
import { executionOwnedPathsRegistry } from './execution/executionOwnedPathsRegistry.js';
import { executionSubagentCompletionRegistry } from './execution/subagentCompletionRegistry.js';
import { collectExecutionSubagentResult } from './execution/executionSubagentResult.js';
...
import type { ISingularityApiService } from './shared/singularityApiService.js';
...
export interface IAgentSideEffectsOptions {
	...
	/** Get the GitHub token used for Singularity utility title generation. */
	readonly getSingularityToken?: () => string | undefined;
	...
	/** CAPI service used for Singularity utility title generation. */
	readonly singularityApiService?: ISingularityApiService;
	/**
...
export class AgentSideEffects extends Disposable {
	...
	constructor(
		...
		this._titleController = this._register(instantiationService.createInstance(AgentHostSessionTitleController, this._stateManager, {
			...
			getSingularityToken: this._options.getSingularityToken,
			...
			singularityApiService: this._options.singularityApiService,
		}));
	...
	private _dispatchActionForSession(signal: AgentSignal, sessionKey: ProtocolURI, turnId: string, turnIdRouting: AgentSignalTurnIdRouting, agent?: IAgent): void {
		...
		if (action.type === ActionType.ChatTurnComplete) {
			...
			this._maybeResolveExecutionSubagent(sessionKey, true);
		}
		...
		if (action.type === ActionType.ChatError) {
			...
			this._toolCallTracker.clearSession(sessionKey);
			this._markSessionUnread(sessionUri);
			this._maybeResolveExecutionSubagent(sessionKey, false, action.error?.message);
		}
	}
	...
	/**
	 * Starts an execution-engine-owned subagent turn (no SDK task tool).
	 * Returns the turn id that {@link runExecutionSubagentTurn} should forward.
	 */
	startExecutionSubagentTurn(params: {
		parentChatUri: string;
		toolCallId: string;
		taskPrompt: string;
		title: string;
	}): string {
		const parentChatURI = params.parentChatUri;
		const toolCallId = params.toolCallId;
		const parentSessionUri = parseRequiredSessionUriFromChatUri(parentChatURI);
		const subagentChatUri = buildSubagentChatUri(parentSessionUri, toolCallId);
		const turnId = generateUuid();

		this._subagentChats.set({
			parentChatUri: parentChatURI,
			toolCallId,
			sessionUri: parentSessionUri,
			chatUri: subagentChatUri,
			turnStopWatch: StopWatch.create(false),
		}, parentChatURI, toolCallId);

		this._logService.info(`[AgentSideEffects] Starting execution subagent turn: ${subagentChatUri} (parent=${parentChatURI}, toolCallId=${toolCallId})`);
		return turnId;
	}

	/** Forwards an execution subagent turn to the agent backend. */
	runExecutionSubagentTurn(params: {
		parentChatUri: string;
		subagentChatUri: string;
		turnId: string;
		taskPrompt: string;
	}): void {
		this.handleAction(params.subagentChatUri, {
			type: ActionType.ChatTurnStarted,
			turnId: params.turnId,
			startedAt: new Date().toISOString(),
			message: { text: params.taskPrompt, origin: { kind: MessageKind.User } },
		});
	}

	private _maybeResolveExecutionSubagent(subagentChatUri: ProtocolURI, ok: boolean, error?: string): void {
		if (!isSubagentChatUri(subagentChatUri)) {
			return;
		}
		for (const subagent of this._subagentChats.values()) {
			if (subagent.chatUri !== subagentChatUri || !isExecutionToolCallId(subagent.toolCallId)) {
				continue;
			}
			if (!executionSubagentCompletionRegistry.has(subagent.toolCallId)) {
				return;
			}
			const state = this._stateManager.getSessionState(subagentChatUri);
			const pending = executionSubagentCompletionRegistry.get(subagent.toolCallId);
			const subagentResult = ok
				? collectExecutionSubagentResult(state, pending?.taskId ?? subagent.toolCallId, [])
				: undefined;
			executionSubagentCompletionRegistry.resolve(subagent.toolCallId, {
				ok,
				error,
				subagentResult,
			});
	...
	private _isParentEditingOwnedPath(parentChatUri: ProtocolURI, toolInput: unknown): boolean {
		if (!executionOwnedPathsRegistry.hasActiveExecution(parentChatUri)) {
			return false;
		}
		const paths = extractToolFilePaths(toolInput);
		return paths.some(path => executionOwnedPathsRegistry.isPathOwnedByChild(parentChatUri, path));
	}
	...
	completeSubagentSession(parentChatURI: ProtocolURI, toolCallId: string): void {
		...
		if (isExecutionToolCallId(toolCallId)) {
			const subagent = this._subagentChats.get(parentChatURI, toolCallId);
			if (subagent) {
				this._maybeResolveExecutionSubagent(subagent.chatUri, true);
			} else {
				executionSubagentCompletionRegistry.resolve(toolCallId, { ok: true });
			}
		}
	...
	private async _handleToolReady(e: IAgentToolPendingConfirmationSignal, sessionKey: ProtocolURI, turnId: string, agent: IAgent): Promise<void> {
		if (!isSubagentChatUri(sessionKey) && this._isParentEditingOwnedPath(sessionKey, getInlineToolInput(e.state.toolInput))) {
			this._logService.warn(`[AgentSideEffects] Denying parent tool on child-owned path during active execution: chat=${sessionKey}, toolCallId=${e.state.toolCallId}`);
	...
	handleAction(channel: ProtocolURI, action: StateAction, clientId?: string, clientType = AgentHostClientType.Unknown): void {
		...
		switch (action.type) {
			case ActionType.ChatTurnStarted: {
				...
				// (e.g. SingularityAgentSession) and reset on its `send()` call.
	 ...
	 * Singularity SDK's `assistant.usage` event is explicitly ephemeral, and the
	...
	private _trackTurnUsage(channel: ProtocolURI, action: ChatAction): void {
		...
		// `SingularityAgentSession._turnId`). No turn can ever match it, and no
...
function extractToolFilePaths(toolInput: unknown): string[] {
	if (!toolInput || typeof toolInput !== 'object') {
		return [];
	}
	const record = toolInput as Record<string, unknown>;
	const paths: string[] = [];
	for (const key of ['path', 'filePath', 'targetFile', 'file_path', 'relativeWorkspacePath']) {
