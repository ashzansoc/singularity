...
/**
 * Public GitHub Singularity telemetry ingestion keys. These are instrumentation keys, not
 * secrets; the iKey selects the destination hydro table:
 *  - standard  -> `singularity_v0_singularity_event`
 *  - enhanced  -> `singularity_v0_restricted_singularity_event`
 */
const GH_STANDARD_IKEY = '7d7048df-6dd0-4048-bb23-b716c1461f8f';
const GH_ENHANCED_IKEY = '3fdd7f28-937a-48c8-9a21-ba337db23bd1';

/**
 * Fallback Singularity telemetry endpoint (the dotcom value of the CAPI token's
 * `endpoints.telemetry`, with the `/telemetry` path the Singularity CLI/runtime appends).
 ...
 */
const GH_TELEMETRY_URL = 'https://singularity-telemetry.githubusercontent.com/telemetry';

/** Event names are namespaced by client category; the CTS name filter requires this. */
const NAMESPACE = 'singularity-chat';
...
/**
 * App Insights caps a single property value at ~8192 chars. Long values are chunked so the Singularity
 * Telemetry Service can reassemble them, mirroring the Singularity extension's `multiplexProperties` so
...
/**
 * The restricted telemetry surface the agent host exposes, mirroring the Singularity extension's
...
export interface IAgentHostRestrictedTelemetry {
	/** GH standard (non-restricted) telemetry -> `singularity_v0_singularity_event`. */
	sendGHTelemetryEvent(eventName: string, properties?: TelemetryProps, measurements?: TelemetryMeasurements): void;
	/** GH enhanced/restricted telemetry (prompts, tools, etc.) -> `singularity_v0_restricted_singularity_event`. */
	...
	/** Sets the Singularity user tracking id (`singularity_trackingId`) carried on every subsequent event. */
	setSingularityTrackingId(trackingId: string | undefined): void;
...
/**
 * Emits GitHub Singularity restricted/enhanced telemetry from the agent-host process by POSTing
 * Application-Insights envelopes to the Singularity telemetry endpoint (the same wire format the
 * Singularity extension uses). Fire-and-forget; failures are logged, never thrown.
 */
export class AgentHostRestrictedTelemetrySender implements IAgentHostRestrictedTelemetry {
	...
	/**
	 * Whether the current Singularity token opts into enhanced/restricted telemetry (`rt=1`). Off by
	 ...
	 * Singularity extension, which only creates the restricted reporter for opted-in users.
	 */
	...
	setSingularityTrackingId(trackingId: string | undefined): void {
		// `singularity_trackingId` is the current account's Singularity token `tid` claim. Exact runtime
		...
		this._commonProps.singularity_trackingId = trackingId || undefined;
	}
	...
	private _post(iKey: string, eventName: string, properties?: TelemetryProps, measurements?: TelemetryMeasurements, context?: { readonly endpointUrl: string | undefined; readonly trackingId: string | undefined }): void {
		...
		const commonProps = context
			? { ...this._commonProps, singularity_trackingId: context.trackingId }
			: this._commonProps;
		const envelope = {
			ver: 1,
			...
			data: {
				...
				baseData: {
					name,
					// `unique_id` is a fresh per-event id (its hydro column is read by the Singularity
					// Telemetry Service from the snake_case `unique_id` property, NOT `uniqueId`),
					// mirroring the Singularity extension so each emitted event stays individually
					// addressable. Placed first so explicit properties still win on collision.
					properties: context
						? { unique_id: generateUuid(), ...commonProps, ...properties, singularity_trackingId: context.trackingId }
