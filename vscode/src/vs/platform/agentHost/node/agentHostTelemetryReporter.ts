...
export interface IAgentHostSkillContentReadReport {
	...
	content: string;
	/** Where the skill was discovered (project, personal-singularity, plugin, builtin, …). */
...
export class AgentHostTelemetryReporter {
	...
	/**
	 * Mirrors the Singularity extension's enhanced GH `request.options.tools` event for the agent-host
	...
	/**
	 * Mirrors the Singularity extension's restricted `conversation.messageText` event (the panel-chat
	 ...
	 * (`singularity_v0_restricted_singularity_event`) and internal MSFT pipelines; the agent host observes
	...
	/**
	 * Mirrors the Singularity extension's restricted `skillContentRead` event (`skillTelemetry.ts` ->
	...
	toolInvoked(report: IAgentHostToolInvokedReport): void {
		...
		// previously emitted by `SingularityAgentSession`). Action signals are keyed
