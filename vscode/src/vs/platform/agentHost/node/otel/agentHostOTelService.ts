...
export function readAgentHostOTelEnv(env: NodeJS.ProcessEnv): ResolvedConfig {
	const dbSpanExporter = isTruthy(env.SINGULARITY_OTEL_DB_SPAN_EXPORTER_ENABLED);
	const otlpEndpoint = env.OTEL_EXPORTER_OTLP_ENDPOINT ?? env.SINGULARITY_OTEL_ENDPOINT;
	const filePath = env.SINGULARITY_OTEL_FILE_EXPORTER_PATH;
	const explicitlyEnabled = isTruthy(env.SINGULARITY_OTEL_ENABLED);
	...
	const rawType = (env.SINGULARITY_OTEL_EXPORTER_TYPE ?? '').trim().toLowerCase();
	const protocol = (env.OTEL_EXPORTER_OTLP_PROTOCOL ?? env.SINGULARITY_OTEL_PROTOCOL ?? '').trim().toLowerCase();
	...
	return {
		...
		otlpEndpoint,
		filePath,
		sourceName: env.SINGULARITY_OTEL_SOURCE_NAME,
