...
export class AgentHostTelemetryService extends Disposable implements IAgentHostTelemetryService {
	...
	/**
	 * Whether the current Singularity token opts into enhanced/restricted telemetry (`rt=1`). Defaults
	 * to `false` so nothing restricted is sent until an authenticated token confirms the opt-in,
	 * keeping public users off the enhanced pipeline the way the Singularity extension does.
	 */
	...
	constructor(
		...
		singularitySdkVersion?: string,
		singularityRuntimeVersion?: string,
	) {
		super();
		if (isDisposable(_delegate)) {
			this._register(_delegate);
		}
		if (singularitySdkVersion) {
			// __GDPR__COMMON__ "common.singularitySdkVersion" : { "classification": "SystemMetaData", "purpose": "PerformanceAndHealth", "comment": "The version of the Singularity SDK used by the Agent Host process." }
			this._delegate.setCommonProperty('common.singularitySdkVersion', singularitySdkVersion);
		}
		if (singularityRuntimeVersion) {
			// __GDPR__COMMON__ "common.singularityRuntimeVersion" : { "classification": "SystemMetaData", "purpose": "PerformanceAndHealth", "comment": "The version of the Singularity runtime used by the Agent Host process." }
			this._delegate.setCommonProperty('common.singularityRuntimeVersion', singularityRuntimeVersion);
		}
	}
	...
	setSingularityTrackingId(trackingId: string | undefined): void {
		this._restricted?.setSingularityTrackingId(trackingId);
	}
...
async function resolveSingularityExtensionVersion(environmentService: INativeEnvironmentService, fileService: IFileService, logService: ILogService): Promise<string | undefined> {
	if (!environmentService.builtinExtensionsPath) {
		return undefined;
	}
	try {
		const manifest = JSON.parse((await fileService.readFile(joinPath(URI.file(environmentService.builtinExtensionsPath), 'singularity', 'package.json'))).value.toString()) as { version?: unknown };
		return typeof manifest.version === 'string' ? manifest.version : undefined;
	} catch (error) {
		logService.debug(`[agentHostTelemetry] Failed to resolve Singularity extension version: ${error instanceof Error ? error.message : String(error)}`);
		return undefined;
	}
}
...
export async function createAgentHostTelemetryService(options: IAgentHostTelemetryServiceOptions): Promise<IAgentHostTelemetryService> {
	...
	const extensionVersion = loggingOnly ? undefined : await resolveSingularityExtensionVersion(environmentService, fileService, logService);
	...
	return disposables.add(new AgentHostTelemetryService(telemetryService, restricted, productService.singularityVersions?.sdk, productService.singularityVersions?.runtime));
}
