...
import { AgentHostTaskExecutor } from './execution/agentHostTaskExecutor.js';
import { collectExecutionSubagentResult } from './execution/executionSubagentResult.js';
import type { ExecutionTaskRequest } from './execution/executionTypes.js';
import type { IAgentHostExecutionTaskRequest, IAgentHostExecutionTaskResult } from '../common/agentService.js';
...
import { SingularityApiService, ISingularityApiService } from './shared/singularityApiService.js';
...
import { AgentHostEditTelemetryEnabledConfigKey, AgentHostMigrateLegacySingularityCliEnabledConfigKey, platformRootSchema } from '../common/agentHostSchema.js';
...
export class AgentService extends Disposable implements IAgentService {
	...
	private _executionTaskExecutor: AgentHostTaskExecutor | undefined;
	...
	constructor(
		...
		singularityApiService?: ISingularityApiService,
		...
		const effectiveSingularityApiService = singularityApiService ?? instantiationService.createInstance(SingularityApiService, fetchFn);
		services.set(ISingularityApiService, effectiveSingularityApiService);
		...
		this._sideEffects = this._register(instantiationService.createInstance(AgentSideEffects, this._stateManager, {
			...
			agents: this._agents,
			singularityApiService: effectiveSingularityApiService,
			getSingularityToken: () => {
				return this.getAuthToken({
					resource: this._gitHubEndpointService.getSingularityResource().resource,
					scopes: this._gitHubEndpointService.getSingularityResource().scopes_supported,
				});
			},
			getGitHubToken: () => {
				return this.getAuthToken({
	...
	async listSessions(): Promise<IAgentSessionMetadata[]> {
		...
		// transiently drop a session (e.g. `SingularityAgent.listSessions` returns
	 ...
	 * Singularity CLI session created by the extension host). Re-list and announce any
	 ...
	 *
	 * Agent backends don't durably record token/credit usage — the Singularity
	 ...
	 * turn is keyed by. Today only Singularity does, via `setTurnEventId`. Claude
	...
	private async _doRestoreSession(session: URI, sessionStr: string): Promise<void> {
		...
		// Adopt-on-open for a surfaced un-adopted legacy Singularity CLI session: seed its
		...
		const migrateLegacyEnabled = this._configurationService.getRootValue(platformRootSchema, AgentHostMigrateLegacySingularityCliEnabledConfigKey) === true;
	...
	private _trackPendingSubagentChatFromEnvelope(envelope: ActionEnvelope): void {
		...
		if (action.type === ActionType.ChatToolCallStart || action.type === ActionType.ChatToolCallDelta || action.type === ActionType.ChatToolCallReady) {
			...
			// first reveals it (Singularity at Start, Claude at Ready) — later
	...
	async executeExecutionTask(request: IAgentHostExecutionTaskRequest): Promise<IAgentHostExecutionTaskResult> {
		const executor = this._getExecutionTaskExecutor();
		return executor.executeTask(request as ExecutionTaskRequest);
	}

	private _getExecutionTaskExecutor(): AgentHostTaskExecutor {
		if (!this._executionTaskExecutor) {
			this._executionTaskExecutor = new AgentHostTaskExecutor({
				logService: this._logService,
				spawnHooks: {
					onChatSpawned: event => this._onChatSpawned(event),
					startExecutionSubagentTurn: params => this._sideEffects.startExecutionSubagentTurn(params),
					runSubagentTurn: params => {
						this._sideEffects.runExecutionSubagentTurn(params);
						return Promise.resolve();
					},
					collectSubagentResult: (subagentChatUri, taskId, ownedPaths) => {
						const state = this._stateManager.getSessionState(subagentChatUri);
						return collectExecutionSubagentResult(state, taskId, ownedPaths);
					},
				},
			});
		}
		return this._executionTaskExecutor;
	}
