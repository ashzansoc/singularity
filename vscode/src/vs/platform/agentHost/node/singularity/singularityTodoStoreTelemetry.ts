...
type TodoStoreOperationClassification = {
	...
	owner: 'amunger';
	comment: 'Tracks successful Singularity CLI SQL operations that access todo item or dependency storage.';
};
...
export function reportSingularityTodoStoreOperation(telemetryService: ITelemetryService, session: URI, toolCallId: string, toolName: string, toolInput: Readonly<Record<string, unknown>> | undefined): void {
	const operation = getSingularityTodoStoreOperationData(toolName, toolInput);
	if (!operation) {
		return;
	}
...
export function getSingularityTodoStoreOperationData(toolName: string, toolInput: Readonly<Record<string, unknown>> | undefined): ITodoStoreOperationData | undefined {
	if (toolName !== 'sql') {
		return undefined;
	}
