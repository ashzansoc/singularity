 ...
 *
 * The Singularity SDK owns the authoritative event-format schema version; this
 ...
 * genuine agent run.
 */
const MIGRATION_PRODUCER = 'vscode-singularity-migration';

/**
 * Options controlling how {@link buildSessionEventsFromTurns} synthesizes a
 * Singularity SDK event log from VS Code turns.
 */
export interface IBuildSessionEventsOptions {
	...
	/** Singularity application version string for `session.start`. Defaults to `0.0.0`. */
	readonly singularityVersion?: string;
...
/**
 * Translates a sequence of VS Code {@link Turn}s into a Singularity SDK
 ...
 *
 * Every event envelope id must be a UUID (the Singularity runtime rejects non-UUID
...
export function buildSessionEventsFromTurns(turns: readonly Turn[], options: IBuildSessionEventsOptions): SessionEvent[] {
	...
	push({
		...
		data: {
			...
			copilotVersion: options.singularityVersion ?? '0.0.0',
