...
type InstructionSource = Awaited<ReturnType<SingularityClient['rpc']['instructions']['discover']>>['sources'][number];
...
const CUSTOMIZATION_DISCOVERY_DEBUG_LOG_PATH = undefined; //'/tmp/singularity-customization-discovery-debug.log';
const AGENT_INSTRUCTION_FILENAMES = new Set(['agents.md', 'claude.md', 'gemini.md', 'singularity-instructions.md']);
...
const searchRoots: { workspace: ISearchRoot[]; user: ISearchRoot[] } = {
	...
	user: [
		{ path: ['.singularity', 'agents'], type: DiscoveredType.Agent, name: '~/.singularity' },
		{ path: ['.agents', 'skills'], recursive: true, type: DiscoveredType.Skill, name: '~/.agents' },
		{ path: ['.singularity', 'skills'], recursive: true, type: DiscoveredType.Skill, name: '~/.singularity' },
		{ path: ['.singularity', 'instructions'], recursive: true, type: DiscoveredType.Instruction, name: '~/.singularity' },
		{ path: ['.singularity', 'hooks'], recursive: true, type: DiscoveredType.Hook, name: '~/.singularity' },
	],
};


/**
 * Builds the list of instruction file candidates used by the Singularity CLI.
 *
...
const fixedDiscoveryFiles: { workspace: IFixedDiscoveryFile[]; user: IFixedDiscoveryFile[] } = {
	workspace: [
		{ path: ['.github'], filenames: ['singularity-instructions.md'], type: DiscoveredType.AgentInstruction },
		...
		{ path: ['.github', 'singularity'], filenames: ['settings.json', 'settings.local.json'], type: DiscoveredType.Hook },
		{ path: ['.claude'], filenames: ['settings.json', 'settings.local.json'], type: DiscoveredType.Hook },
	],
	user: [
		{ path: ['.singularity'], filenames: ['singularity-instructions.md'], type: DiscoveredType.AgentInstruction },
	],
};
...
export class SessionCustomizationDiscovery extends Disposable {
	 ...
	 * from non-primary roots are intentionally NOT discovered because the Singularity
	...
	private async getDiscoveredDirectories(client: SingularityClient, token: CancellationToken): Promise<readonly IDiscoveredDirectory[]> {
	...
	private getHooksDiscoveryPaths(): IDiscoveredDirectory[] {
		...
		for (const root of searchRoots.workspace) {
			if (root.type === DiscoveredType.Hook) {
				// Hooks: primary working directory only (Singularity limitation).
	...
	public async discover(client: SingularityClient, token: CancellationToken): Promise<readonly DirectoryCustomization[]> {
	...
	private async discoverAgents(discoveryRequest: AgentsDiscoverRequest, client: SingularityClient, token: CancellationToken): Promise<AgentCustomization[]> {
	...
	private async discoverRules(discoveryRequest: AgentsDiscoverRequest, client: SingularityClient, token: CancellationToken): Promise<RuleCustomization[]> {
	...
	private async discoverSkills(discoveryRequest: AgentsDiscoverRequest, client: SingularityClient, token: CancellationToken): Promise<SkillCustomization[]> {
	...
	private async discoverHooks(token: CancellationToken): Promise<HookCustomization[]> {
		...
		await Promise.all([
			// Hooks: primary working directory only (Singularity limitation — see _hookWorkingDirectories).
	...
	public async scan(token: CancellationToken): Promise<readonly IDiscoveredDirectory[]> {
		...
		// PRIMARY working directory only (Singularity limitation — see _hookWorkingDirectories);
	 ...
	 * directory (e.g. `.github`, `.github/agents`, `.singularity`) is created and
	...
	/**
	 * For fixed discovery files (e.g. AGENTS.md, singularity-instructions.md,
