...
import type { ContextTier, CopilotClient as SingularityClient, ElicitationContext, ElicitationResult, ExitPlanModeRequest, ExitPlanModeResult, NamedProviderConfig, PermissionRequest, PermissionRequestResult, ProviderModelConfig, ResumeSessionConfig, SessionConfig, SessionHooks, Tool, Verbosity } from '@github/copilot-sdk';
import { coalesce } from '../../../../base/common/arrays.js';
import { Schemas } from '../../../../base/common/network.js';
import { URI } from '../../../../base/common/uri.js';
import { IFileService } from '../../../files/common/files.js';
import { ILogService, LogLevel } from '../../../log/common/log.js';
import { SingularityCliConfigKey, applyModelFamilyAlias, singularityCliConfigSchema, normalizeToolSearchDeferThreshold } from '../../common/singularityCliConfig.js';
...
import { ShellManager, createShellTools, type IUnsandboxedCommandConfirmationRequest } from './singularityShellTools.js';
import { toSdkHooks, toSdkInstructionDirectories, toSdkMcpServers, toSdkMcpServersFromConfigMap, toSdkSessionCustomAgents, toSdkSkillDirectories } from './singularityPluginConverters.js';
import { buildSandboxConfigForSdk, type SingularitySandboxConfig } from './sandboxConfigForSdk.js';
import type { ISingularityPluginInfo } from './singularityAgent.js';
 ...
 * SDK's two-valued {@link SessionConfig.contextTier} by {@link getSingularityContextTier}.
 */
...
export interface IActiveClientSnapshot {
	...
	readonly plugins: readonly ISingularityPluginInfo[];
...
export interface ISingularitySessionRuntime {
...
export interface ISingularitySessionLauncher {
	/**
	 * Creates an unowned SDK session wrapper. The caller is responsible for
	 * registering or disposing the returned wrapper.
	 */
	launch(plan: SingularitySessionLaunchPlan, runtime: ISingularitySessionRuntime): Promise<SingularitySessionWrapper>;
}

type SingularitySessionClient = Pick<SingularityClient, 'createSession' | 'resumeSession'>;

interface ISingularitySessionLaunchBase {
	readonly client: SingularitySessionClient;
...
export interface ISingularityCreateSessionLaunchPlan extends ISingularitySessionLaunchBase {
...
export interface ISingularityResumeSessionLaunchPlan extends ISingularitySessionLaunchBase {
...
export type SingularitySessionLaunchPlan = ISingularityCreateSessionLaunchPlan | ISingularityResumeSessionLaunchPlan;

export function isSingularityReasoningEffort(value: unknown): value is AgentHostReasoningEffort {
...
function getSingularitySdkErrorCode(err: unknown): number | undefined {
...
/**
 * Messages from a failed Singularity SDK `session.resume` that positively indicate
...
/**
 * Decide whether a Singularity SDK `resumeSession` failure should fall back to
...
function shouldCreateEmptySessionAfterResumeError(err: unknown): boolean {
	if (getSingularitySdkErrorCode(err) !== -32603) {
		return false;
	}
...
function isCustomAgentNotFoundError(err: unknown): boolean {
	return getSingularitySdkErrorCode(err) === -32603 && /\bCustom agent '.+' not found\b/i.test(getErrorMessage(err));
}
 ...
 */
export function getSingularityReasoningEffort(model: ModelSelection | undefined, effortOverride?: string): SessionConfig['reasoningEffort'] {
	if (isSingularityReasoningEffort(effortOverride)) {
		return toSdkReasoningEffort(effortOverride);
	}
	const thinkingLevel = model?.config?.[ThinkingLevelConfigKey];
	return isSingularityReasoningEffort(thinkingLevel) ? toSdkReasoningEffort(thinkingLevel) : undefined;
}
 ...
 * `SingularityAgent._changeModel` (mid-session model change) for consistency.
 */
export function resolveSingularityReasoningEffort(model: ModelSelection | undefined, configurationService: IAgentConfigurationService, logService: ILogService, sessionId: string): SessionConfig['reasoningEffort'] {
	const rawOverride = configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.ReasoningEffortOverride);
	// '' is the schema's unset marker, so an unset override reads as `undefined`.
	const override = rawOverride ? rawOverride : undefined;
	if (override !== undefined) {
		if (isSingularityReasoningEffort(override)) {
			logService.info(`[Singularity:${sessionId}] Applying reasoning-effort override '${override}'`);
		} else {
			logService.warn(`[Singularity:${sessionId}] Ignoring invalid reasoning-effort override '${override}'; expected one of [${ReasoningEfforts.join(', ')}]`);
		}
	}
	return getSingularityReasoningEffort(model, override);
}

export function getSingularityContextTier(model: ModelSelection | undefined, longContextWindow?: number, freeLongContext?: boolean): SessionConfig['contextTier'] {
 ...
 *
 * Extracted from {@link SingularitySessionLauncher} so the synthesis and gating are
...
export async function resolveByokSessionConfig(
	...
	try {
		...
		logService.warn(`[Singularity:${sessionId}] Failed to enumerate BYOK models from renderer bridges`, err);
		return {};
	}
	...
	try {
		...
		logService.warn(`[Singularity:${sessionId}] Failed to start BYOK loopback proxy`, err);
		return {};
	}
	...
	logService.info(`[Singularity:${sessionId}] Wired ${models.length} BYOK model(s) across ${providers.length} provider(s) via loopback proxy ${handle.baseUrl}`);
	return { providers, models };
}

export class SingularitySessionLauncher implements ISingularitySessionLauncher {
	 ...
	 * {@link disposeByokProxyHandle} when the owning Singularity client/runtime is
	...
	async launch(plan: SingularitySessionLaunchPlan, runtime: ISingularitySessionRuntime): Promise<SingularitySessionWrapper> {
		...
		try {
			...
			this._logService.trace(`[Singularity:${plan.sessionId}] Calling SDK resumeSession...`);
			const raw = await this._withTraceContext(plan.sessionId, () => plan.client.resumeSession(plan.sessionId, config));
			this._logService.trace(`[Singularity:${plan.sessionId}] SDK resumeSession succeeded after ${stopWatch.elapsed()}ms`);
			await this._applySandboxConfig(raw, sandboxConfig, plan.sessionId);
			return new SingularitySessionWrapper(raw);
		} catch (err) {
			let resumeError = err;
			const errCode = getSingularitySdkErrorCode(resumeError);
			const errMsg = getErrorMessage(resumeError);
			this._logService.warn(`[Singularity:${plan.sessionId}] SDK resumeSession failed: code=${errCode}, message=${errMsg}`);
			if (plan.resolvedAgentName && isCustomAgentNotFoundError(resumeError)) {
				...
				this._logService.warn(`[Singularity:${plan.sessionId}] Stored custom agent '${plan.resolvedAgentName}' was not found; retrying resume without a custom agent`);
				try {
					...
					this._logService.warn(`[Singularity:${plan.sessionId}] SDK resumeSession without custom agent failed: code=${getSingularitySdkErrorCode(retryErr)}, message=${getErrorMessage(retryErr)}`);
				}
			}
			...
			if (!shouldCreateEmptySessionAfterResumeError(resumeError)) {
				this._logService.warn(`[Singularity:${plan.sessionId}] Resume failure does not indicate an empty session; surfacing it instead of replacing the session with an empty one`);
				throw resumeError;
			}

			this._logService.warn(`[Singularity:${plan.sessionId}] Resume reported no session history; falling back to createSession with same ID`);
			...
			this._logService.info(`[Singularity:${plan.sessionId}] Fallback createSession succeeded`);
			return wrapper;
		}
	}

	private _withTraceContext<T>(sessionId: string, fn: () => T): T {
		const sessionUri = AgentSession.uri('singularitycli', sessionId).toString();
		return this._otelService.withTraceContext(this._otelService.getSessionTraceContext(sessionId, sessionUri), fn);
	}

	private async _createSession(plan: ISingularityCreateSessionLaunchPlan, config: ResumeSessionConfig, sandboxConfig: SingularitySandboxConfig | undefined): Promise<SingularitySessionWrapper> {
		const raw = await this._withTraceContext(plan.sessionId, () => plan.client.createSession({
			...config,
			sessionId: plan.sessionId,
			streaming: true,
			model: plan.model?.id,
			reasoningEffort: resolveSingularityReasoningEffort(plan.model, this._configurationService, this._logService, plan.sessionId),
			contextTier: getSingularityContextTier(plan.model, plan.longContextWindow, plan.freeLongContext),
	...
	private async _applyVerbosity(session: SingularitySessionWrapper['session'], verbosity: Verbosity, sessionId: string): Promise<void> {
		try {
			await session.rpc.options.update({ verbosity });
			this._logService.info(`[Singularity:${sessionId}] Applied '${verbosity}' verbosity`);
		} catch (err) {
			this._logService.warn(`[Singularity:${sessionId}] Failed to apply '${verbosity}' verbosity`, err);
		}
	}
	 ...
	 *
	 * Returns `undefined` when {@link SingularityCliConfigKey.EnableCustomTerminalTool}
	 ...
	 * `buildSandboxConfigForCLI` does for the Singularity extension's CLI path.
	 */
	private _computeSandboxConfig(): SingularitySandboxConfig | undefined {
		const enableCustomTerminalTool = this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.EnableCustomTerminalTool) === true;
	 ...
	 */
	private async _applySandboxConfig(session: SingularitySessionWrapper['session'], sandboxConfig: SingularitySandboxConfig | undefined, sessionId: string): Promise<void> {
		if (!sandboxConfig) {
			return;
		}
		try {
			await session.rpc.options.update({ sandboxConfig });
			this._logService.info(`[Singularity:${sessionId}] Applied SDK sandboxConfig via session.options.update`);
		} catch (err) {
			this._logService.warn(`[Singularity:${sessionId}] Failed to apply SDK sandboxConfig`, err);
		}
	}
	 ...
	 *
	 * **Ownership invariant.** The caller MUST stop the Singularity client/runtime
	 ...
	 * Invoked from `SingularityAgent._stopClient` / `SingularityAgent.shutdown` after the
	 * client has stopped.
	 */
	...
	private async _buildSessionConfig(plan: SingularitySessionLaunchPlan, runtime: ISingularitySessionRuntime): Promise<ResumeSessionConfig> {
		...
		if (enableCustomTerminalTool) {
			if (!plan.shellManager) {
				throw new Error(`ShellManager is required to launch Singularity session '${plan.sessionId}'`);
			}
		...
		if (model && effectiveModel !== model) {
			this._logService.info(`[Singularity:${plan.sessionId}] Model capability override: routing prompt for '${model.id}' as family '${effectiveModel?.id}'`);
		}
		const toolSearchActive = this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.ToolSearchEnabled) === true
			&& agentHostModelSupportsToolSearch(effectiveModel?.id)
			&& clientToolNames.has(CLIENT_TOOL_SEARCH_REFERENCE_NAME);
		const toolSearchDeferThreshold = normalizeToolSearchDeferThreshold(this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.ToolSearchDeferThreshold));
		const promptContext: IAgentHostPromptContext = {
			getSetting: key => this._configurationService.getRootValue(singularityCliConfigSchema, key),
		...
		this._logService.info(`[Singularity:${plan.sessionId}] Resolved system message: ${describeSystemMessageConfig(systemMessage)}`);
		if (this._logService.getLevel() <= LogLevel.Trace) {
			...
			this._logService.trace(`[Singularity:${plan.sessionId}] System message config: ${JSON.stringify(systemMessage, (_key, value) => typeof value === 'function' ? '[transform fn]' : value)}`);
		}
		return {
			...byok,
			clientName: AGENT_HOST_SINGULARITY_CLIENT_NAME,
			enableMcpApps: true,
			githubMcpToolConfig: { disableFormDeferral: true },
			enableFileHooks: true,
			enableConfigDiscovery: true,
			requestExtensions: false, // force-disable singularity extension management tools (otherwise enabled in experimental mode)
			...
			// GitHub identity (login, Singularity plan, endpoints) to drive
