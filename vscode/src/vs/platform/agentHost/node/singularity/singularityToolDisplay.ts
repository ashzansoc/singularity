...
// =============================================================================
// Singularity CLI built-in tool interfaces
//
// The Singularity CLI (via @github/copilot-sdk) exposes these built-in tools. Tool names
...
// tool events and the Singularity Chat extension's CLI display table.
//
...
/**
 * Known Singularity CLI tool names. These are the `toolName` values that appear
 * in `tool.execution_start` events from the SDK.
 */
const enum SingularityToolName {
	...
	ShowFile = 'show_file',
	FetchSingularityCliDocumentation = 'fetch_singularity_cli_documentation',
...
interface ISingularityShellToolArgs {
	command: string;
	timeout?: number;
}

/** Parameters for file tools (`view`, `edit`, `create`). */
interface ISingularityFileToolArgs {
	path: string;
}

interface ISingularityEditToolArgs extends ISingularityFileToolArgs {
	old_str?: string;
	new_str?: string;
}

interface ISingularityCreateToolArgs extends ISingularityFileToolArgs {
	file_text?: string;
}

interface ISingularityInsertToolArgs extends ISingularityFileToolArgs {
	insert_line?: number;
	new_str?: string;
}

interface ISingularityStrReplaceEditorToolArgs extends ISingularityEditToolArgs, ISingularityCreateToolArgs, ISingularityInsertToolArgs {
	command?: string;
}

/**
 * Parameters for the `view` tool. The Singularity CLI accepts an optional
 * `view_range: [startLine, endLine]` (1-based, inclusive). `endLine` may be
 * `-1` to mean "to end of file".
 */
interface ISingularityViewToolArgs extends ISingularityFileToolArgs {
	view_range?: number[];
}
...
/**
 * Parameters for the `grep` tool. The Singularity CLI's `grep` accepts the same
 * rich rg-flag schema as `rg`; the older narrower shape (e.g. `include`) is
 * no longer used.
 */
interface ISingularityGrepToolArgs {
	pattern: string;
	path?: string;
...
interface ISingularityGlobToolArgs {
	pattern: string;
	path?: string;
}

/** Parameters for the `sql` tool. */
interface ISingularitySqlToolArgs {
	description?: string;
	query?: string;
}

/** Parameters for the `web_fetch` tool. */
interface ISingularityWebFetchToolArgs {
	url: string;
}

/**
 * Parameters shared by the agent-coordination tools (`read_agent`,
 * `write_agent`). The Singularity CLI identifies the target agent by its
 * human-readable `agent_id` (e.g. `math-helper`).
 */
interface ISingularityAgentToolArgs {
	agent_id?: string;
}
...
function getAgentId(parameters: Record<string, unknown> | undefined): string | undefined {
	const agentId = (parameters as ISingularityAgentToolArgs | undefined)?.agent_id;
 ...
 */
interface ISingularityApplyPatchToolArgs {
	input?: string;
 ...
 * duplicates removed.
 *
 * Accepts either a structured args object ({@link ISingularityApplyPatchToolArgs})
 * or a bare patch string. The Singularity SDK delivers `apply_patch` with
 ...
 */
function getApplyPatchFiles(args: string | ISingularityApplyPatchToolArgs | undefined): string[] {
...
const EDIT_TOOL_NAMES: ReadonlySet<string> = new Set([
	SingularityToolName.Edit,
	SingularityToolName.StrReplace,
	SingularityToolName.Insert,
	SingularityToolName.Create,
	SingularityToolName.ApplyPatch,
	SingularityToolName.GitApplyPatch,
]);
...
export function isEditTool(toolName: string, command?: string): boolean {
	...
	if (toolName === SingularityToolName.StrReplaceEditor) {
...
export function getEditFilePaths(parameters: unknown): string[] {
	if (typeof parameters === 'string') {
		...
		// string. Singularity SDK delivers `apply_patch` arguments as a bare
	...
	const patchArgs = parameters as ISingularityApplyPatchToolArgs;
	if (typeof patchArgs.input === 'string' || typeof patchArgs.patch === 'string') {
		return getApplyPatchFiles(patchArgs);
	}

	const args = parameters as ISingularityFileToolArgs;
...
const SHELL_TOOL_NAMES: ReadonlySet<string> = new Set([
	SingularityToolName.Bash,
	SingularityToolName.PowerShell,
]);
...
const WRITE_SHELL_TOOL_NAMES: ReadonlySet<string> = new Set([
	SingularityToolName.WriteBash,
	SingularityToolName.WritePowerShell,
]);
...
const READ_SHELL_TOOL_NAMES: ReadonlySet<string> = new Set([
	SingularityToolName.ReadBash,
	SingularityToolName.ReadPowerShell,
]);
...
const SEARCH_TOOL_NAMES: ReadonlySet<string> = new Set([
	SingularityToolName.Grep,
	SingularityToolName.Rg,
	SingularityToolName.Glob,
]);
...
const HIDDEN_TOOL_NAMES: ReadonlySet<string> = new Set([
	SingularityToolName.ReportIntent,
	SingularityToolName.Skill,
]);
...
export function isAgentCoordinationTool(toolName: string): boolean {
	return toolName === SingularityToolName.ListAgents
		|| toolName === SingularityToolName.ReadAgent
		|| toolName === SingularityToolName.WriteAgent;
}

/**
 * Returns true when the tool is Singularity's internal Autopilot completion signal.
 */
export function isTaskCompleteTool(toolName: string): boolean {
	return toolName === SingularityToolName.TaskComplete;
}
 ...
 * argument. The Singularity shell tools (`bash`/`powershell`) carry a short
...
// Display helpers
//
// These functions translate Singularity CLI tool names and arguments into
...
export function parseSingularityStreamingToolInput(raw: string): unknown {
...
export function getToolDisplayName(toolName: string): string {
	...
	switch (toolName) {
		case SingularityToolName.StrReplaceEditor:
		case SingularityToolName.Edit:
		case SingularityToolName.StrReplace:
		case SingularityToolName.Insert: return localize('toolName.edit', "Edit File");
		case SingularityToolName.Create: return localize('toolName.create', "Create File");
		case SingularityToolName.View: return localize('toolName.read', "Read");
		case SingularityToolName.Bash:
		case SingularityToolName.PowerShell: return localize('toolName.shell', "Run Shell Command");
		case SingularityToolName.ReadBash:
		case SingularityToolName.ReadPowerShell: return localize('toolName.readTerminal', "Read Terminal");
		case SingularityToolName.WriteBash: return localize('toolName.writeBash', "Write to Bash");
		case SingularityToolName.WritePowerShell: return localize('toolName.writePowerShell', "Write to PowerShell");
		case SingularityToolName.StopBash:
		case SingularityToolName.StopPowerShell:
		case SingularityToolName.BashShutdown:
		case SingularityToolName.PowerShellShutdown: return localize('toolName.stopShell', "Stop Terminal Session");
		case SingularityToolName.ListBash:
		case SingularityToolName.ListPowerShell: return localize('toolName.listShellSessions', "List Shell Sessions");
		case SingularityToolName.Grep:
		case SingularityToolName.Rg:
		case SingularityToolName.Glob: return localize('toolName.search', "Search");
		case SingularityToolName.SearchCodeSubagent: return localize('toolName.searchCode', "Search Code");
		case SingularityToolName.ApplyPatch: return localize('toolName.applyPatch', "Apply Patch");
		case SingularityToolName.GitApplyPatch: return localize('toolName.patch', "Patch");
		case SingularityToolName.CodeqlChecker: return localize('toolName.codeqlChecker', "CodeQL Security Scan");
		case SingularityToolName.CodeReview: return localize('toolName.codeReview', "Code Review");
		case SingularityToolName.ReplyToComment: return localize('toolName.replyToComment', "Reply to Comment");
		case SingularityToolName.Think: return localize('toolName.think', "Thinking");
		case SingularityToolName.ReportIntent: return localize('toolName.reportIntent', "Report Intent");
		case SingularityToolName.ReportProgress: return localize('toolName.reportProgress', "Progress update");
		case SingularityToolName.WebSearch: return localize('toolName.webSearch', "Web Search");
		case SingularityToolName.WebFetch: return localize('toolName.fetchWebContent', "Fetch Web Content");
		case SingularityToolName.UpdateTodo: return localize('toolName.updateTodo', "Update Todo");
		case SingularityToolName.ShowFile: return localize('toolName.showFile', "Show File");
		case SingularityToolName.FetchSingularityCliDocumentation: return localize('toolName.fetchSingularityCliDocumentation', "Fetch Documentation");
		case SingularityToolName.ProposeWork: return localize('toolName.proposeWork', "Propose Work");
		case SingularityToolName.TaskComplete: return localize('toolName.taskComplete', "Task Complete");
		case SingularityToolName.AskUser: return localize('toolName.askUser', "Ask User");
		case SingularityToolName.Skill: return localize('toolName.invokeSkill', "Invoke Skill");
		case SingularityToolName.Task: return localize('toolName.task', "Delegate Task");
		case SingularityToolName.ListAgents: return localize('toolName.listAgents', "List Agents");
		case SingularityToolName.ReadAgent: return localize('toolName.readAgent', "Read Agent");
		case SingularityToolName.ExitPlanMode: return localize('toolName.exitPlanModeFull', "Exit Plan Mode");
		case SingularityToolName.Sql: return localize('toolName.sql', "Execute SQL");
		case SingularityToolName.Lsp: return localize('toolName.lsp', "Language Server");
		case SingularityToolName.CreatePullRequest: return localize('toolName.createPullRequest', "Create Pull Request");
		case SingularityToolName.GhAdvisoryDatabase: return localize('toolName.ghAdvisoryDatabase', "Check Dependencies");
		case SingularityToolName.StoreMemory: return localize('toolName.storeMemory', "Store Memory");
		case SingularityToolName.ParallelValidation: return localize('toolName.parallelValidation', "Validate Changes");
		case SingularityToolName.WriteAgent: return localize('toolName.writeAgent', "Write to Agent");
		case SingularityToolName.McpReload: return localize('toolName.mcpReload', "Reload MCP Config");
		case SingularityToolName.McpValidate: return localize('toolName.mcpValidate', "Validate MCP Config");
		case SingularityToolName.ToolSearchToolRegex: return localize('toolName.toolSearchToolRegex', "Search Tools");
...
export function getInvocationMessage(toolName: string, displayName: string, parameters: Record<string, unknown> | undefined, resolvePath: ToolPathResolver = identityPathResolver): StringOrMarkdown {
	...
	if (SHELL_TOOL_NAMES.has(toolName)) {
		const args = parameters as ISingularityShellToolArgs | undefined;
		if (args?.command) {
	...
	switch (toolName) {
		case SingularityToolName.View: {
			const args = parameters as ISingularityViewToolArgs | undefined;
		...
		case SingularityToolName.Edit:
		case SingularityToolName.StrReplace: {
			const args = parameters as ISingularityFileToolArgs | undefined;
			if (typeof args?.path === 'string' && args.path) {
		...
		case SingularityToolName.Insert: {
			const args = parameters as ISingularityFileToolArgs | undefined;
			if (typeof args?.path === 'string' && args.path) {
		...
		case SingularityToolName.Create: {
			const args = parameters as ISingularityFileToolArgs | undefined;
			if (typeof args?.path === 'string' && args.path) {
		...
		case SingularityToolName.StrReplaceEditor: {
			const command = (parameters as ISingularityStrReplaceEditorToolArgs | undefined)?.command;
			switch (command) {
				case 'view':
					return getInvocationMessage(SingularityToolName.View, displayName, parameters, resolvePath);
				case 'create':
					return getInvocationMessage(SingularityToolName.Create, displayName, parameters, resolvePath);
				case 'insert':
					return getInvocationMessage(SingularityToolName.Insert, displayName, parameters, resolvePath);
				case 'edit':
				case 'str_replace':
				default:
					return getInvocationMessage(SingularityToolName.Edit, displayName, parameters, resolvePath);
			}
		}
		case SingularityToolName.Grep: {
			const args = parameters as ISingularityGrepToolArgs | undefined;
			if (args?.pattern) {
		...
		case SingularityToolName.Rg: {
			const args = parameters as ISingularityRgToolArgs | undefined;
			if (args?.pattern) {
				return md(localize('toolInvoke.grepPattern', "Searching for {0}", appendEscapedMarkdownInlineCode(truncate(args.pattern, 80))));
			}
			return localize('toolInvoke.grep', "Searching files");
		}
		case SingularityToolName.Glob: {
			const args = parameters as ISingularityGlobToolArgs | undefined;
			if (args?.pattern) {
		...
		case SingularityToolName.ApplyPatch:
		case SingularityToolName.GitApplyPatch: {
		...
		case SingularityToolName.Sql: {
			const args = parameters as ISingularitySqlToolArgs | undefined;
			return args?.description || localize('toolInvoke.sql', "Executing SQL query");
		}
		case SingularityToolName.WebFetch: {
			const args = parameters as ISingularityWebFetchToolArgs | undefined;
			if (args?.url) {
		...
		case SingularityToolName.ExitPlanMode:
			return localize('toolInvoke.exitPlanMode', "Presenting plan");
		case SingularityToolName.Task:
		...
		case SingularityToolName.ListAgents:
		case SingularityToolName.ReadAgent:
		case SingularityToolName.WriteAgent:
...
/**
 * Returns the progressively refined message shown while Singularity generates tool input.
 */
export function getStreamingInvocationMessage(toolName: string, displayName: string, parameters: unknown, resolvePath: ToolPathResolver = identityPathResolver): StringOrMarkdown {
	...
	switch (toolName) {
		case SingularityToolName.Edit:
		case SingularityToolName.StrReplace: {
			const args = objectParameters as ISingularityEditToolArgs | undefined;
		...
		case SingularityToolName.Create: {
			const args = objectParameters as ISingularityCreateToolArgs | undefined;
			return getStreamingCreateMessage(args?.path, streamingToolTextLineCount(args?.file_text), resolvePath);
		}
		case SingularityToolName.Insert: {
			const args = objectParameters as ISingularityInsertToolArgs | undefined;
			return getStreamingInsertMessage(args?.path, streamingToolTextLineCount(args?.new_str), resolvePath);
		}
		case SingularityToolName.StrReplaceEditor: {
			const args = objectParameters as ISingularityStrReplaceEditorToolArgs | undefined;
			const command = args?.command;
			switch (command) {
				case 'view':
					return getInvocationMessage(SingularityToolName.View, displayName, objectParameters, resolvePath);
				case 'create':
					return getStreamingCreateMessage(args?.path, streamingToolTextLineCount(args?.file_text), resolvePath);
				case 'insert':
					return getStreamingInsertMessage(args?.path, streamingToolTextLineCount(args?.new_str), resolvePath);
		...
		case SingularityToolName.ApplyPatch:
		case SingularityToolName.GitApplyPatch: {
			const args = objectParameters as ISingularityApplyPatchToolArgs | undefined;
...
export function getPastTenseMessage(toolName: string, displayName: string, parameters: Record<string, unknown> | undefined, success: boolean, resultText?: string, resolvePath: ToolPathResolver = identityPathResolver): StringOrMarkdown {
	...
	switch (toolName) {
		...
		case SingularityToolName.StrReplaceEditor: {
			...
			switch (command) {
				case 'view':
					return getPastTenseMessage(SingularityToolName.View, displayName, parameters, success, resultText, resolvePath);
				case 'create':
					return getPastTenseMessage(SingularityToolName.Create, displayName, parameters, success, resultText, resolvePath);
				case 'insert':
					return getPastTenseMessage(SingularityToolName.Insert, displayName, parameters, success, resultText, resolvePath);
				case 'edit':
				case 'str_replace':
				default:
					return getPastTenseMessage(SingularityToolName.Edit, displayName, parameters, success, resultText, resolvePath);
			}
		}
		case SingularityToolName.Grep: {
			const args = parameters as ISingularityGrepToolArgs | undefined;
...
//
// The Singularity SDK emits a `skill` tool call (which we hide) and, separately, a
...
export function synthesizeSkillToolCall(
	...
	return {
		toolCallId,
		toolName: SingularityToolName.Skill,
		displayName,
		invocationMessage,
		pastTenseMessage,
	};
...
export function getToolInputString(toolName: string, parameters: Record<string, unknown> | undefined, rawArguments: string | undefined): string | undefined {
	...
	if (SHELL_TOOL_NAMES.has(toolName) || WRITE_SHELL_TOOL_NAMES.has(toolName)) {
		...
		if (typeof command === 'object' && command !== null && hasKey(command, { command: true })) {
			return (command as ISingularityShellToolArgs).command;
		}
		return rawArguments;
	}
...
export function getToolKind(toolName: string, parameters?: Record<string, unknown>): ToolKind | undefined {
	...
	if (toolName === SingularityToolName.View
		|| (toolName === SingularityToolName.StrReplaceEditor && parameters?.['command'] === 'view')) {
		return 'read';
	}
	return undefined;
}
 ...
 * arguments of a Singularity SDK subagent tool call. The Singularity `task` tool
...
export function getShellLanguage(toolName: string): string {
	switch (toolName) {
		case SingularityToolName.PowerShell:
		case SingularityToolName.WritePowerShell:
		case SingularityToolName.ReadPowerShell: return 'powershell';
...
export function getPermissionDisplay(request: PermissionRequest, workingDirectory?: URI, isNewFile?: boolean): {
	...
	const shellConfirmationTitle = requestSandboxBypass
		? localize('singularity.permission.shell.bypass.title', "Run in terminal outside the sandbox?")
		: localize('singularity.permission.shell.title', "Run in terminal?");
	...
	switch (request.kind) {
		case 'shell': {
			...
			stripRedundantCdPrefix(SingularityToolName.Bash, shellParams, workingDirectory);
			...
			return {
				...
				invocationMessage: intention ?? getInvocationMessage(SingularityToolName.Bash, getToolDisplayName(SingularityToolName.Bash), cleanedCommand ? { command: cleanedCommand } : undefined),
		...
		case 'custom-tool': {
			...
			return {
				confirmationTitle: localize('singularity.permission.default.title', "Allow tool call?"),
				invocationMessage: md(localize('singularity.permission.default.message', "Allow the model to call {0}?", appendEscapedMarkdownInlineCode(toolName ?? request.kind))),
		...
		case 'write': {
			const toolName = isNewFile ? SingularityToolName.Create : SingularityToolName.Edit;
			return {
				confirmationTitle: isNewFile
					? localize('singularity.permission.create.title', "Create file?")
					: localize('singularity.permission.write.title', "Write file?"),
		...
		case 'mcp': {
			const title = toolName ?? localize('singularity.permission.mcp.defaultTool', "MCP Tool");
			return {
				confirmationTitle: serverName
					? localize('singularity.permission.mcp.title', "Allow tool from {0}?", serverName)
					: localize('singularity.permission.default.title', "Allow tool call?"),
		...
		case 'read':
			return {
				confirmationTitle: localize('singularity.permission.read.title', "Allow reading file outside of workspace?"),
				invocationMessage: getInvocationMessage(SingularityToolName.View, getToolDisplayName(SingularityToolName.View), path ? { path } : undefined),
				permissionKind: 'read',
				permissionPath: path,
			};
		case 'url': {
			...
			return {
				confirmationTitle: localize('singularity.permission.url.title', "Fetch URL?"),
				invocationMessage: md(localize('singularity.permission.url.message', "Allow fetching web content?")),
