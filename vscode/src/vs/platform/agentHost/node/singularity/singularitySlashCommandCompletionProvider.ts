...
import { getSingularityConfigSlashCommandItems, ISingularityConfigSlashCommandState, isSingularityConfigSlashCommand } from '../../common/singularityConfigSlashCommands.js';
...
import type { CopilotSession as SingularitySession } from '@github/copilot-sdk';
...
/**
 * Lookup hooks used by {@link SingularitySlashCommandCompletionProvider} to
 * retrieve runtime slash command metadata and apply feature gating.
 */
export interface ISingularitySlashCommandSessionInfo {
	/**
	...
	getRuntimeSlashCommands?(sessionId: string, options?: ISingularityRuntimeSlashCommandQueryOptions): Promise<readonly ISingularityRuntimeSlashCommandInfo[]>;
	 ...
	 */
	getSessionConfigState?(sessionId: string): ISingularityConfigSlashCommandState | undefined;
}

export interface ISingularityRuntimeSlashCommandQueryOptions {
	readonly maxWaitMs?: number;
}

/**
 * Completion provider for Singularity CLI slash commands. Only fires for
 * sessions whose URI scheme is `singularitycli` and only when the input begins
 * with `/`.
 *
 ...
 * attachments. Runtime command dispatch is text-side in `SingularityAgentSession.send`;
 * client-side config commands also share the same leading slash parser.
 */
export class SingularitySlashCommandCompletionProvider implements IAgentHostCompletionItemProvider {
	...
	constructor(
		private readonly singularitycliId: string,
		private readonly _sessionInfo: ISingularitySlashCommandSessionInfo,
	...
	async provideCompletionItems(params: CompletionsParams, _token: CancellationToken): Promise<readonly CompletionItem[]> {
		if (AgentSession.provider(params.channel) !== this.singularitycliId) {
			return [];
		}
	...
	private async _getRuntimeSlashCommandCompletionInfo(sessionId: string, typed: string, { rangeStart, rangeEnd }: { rangeStart: number; rangeEnd: number }, returnJustSkills: boolean): Promise<CompletionItem[]> {
		...
		for (const command of runtimeCommands) {
			...
			if (isSingularityConfigSlashCommand(command.name) || command.aliases?.some(alias => isSingularityConfigSlashCommand(alias))) {
				continue;
			}
			...
			const options: (NonNullable<NonNullable<ISingularityRuntimeSlashCommandInfo['input']>['choices']>[number] & { argumentHint?: string })[] = [];
		...
		if (!returnJustSkills) {
			...
			for (const item of getSingularityConfigSlashCommandItems(typed, configState)) {
				completionItems.push({
...
export type ISingularityRuntimeSlashCommandInfo = Awaited<ReturnType<SingularitySession['rpc']['commands']['list']>>['commands'][number];
