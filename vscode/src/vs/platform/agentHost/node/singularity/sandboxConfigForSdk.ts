...
type SdkSandboxConfig = NonNullable<Parameters<SingularitySession['rpc']['options']['update']>[0]['sandboxConfig']>;

export type SingularitySandboxConfig = SdkSandboxConfig & {
	readonly allowBypass?: boolean;
};

/**
 * Translate the AgentHost's host-side sandbox configuration into the
 * opaque `sandboxConfig` shape the Singularity SDK forwards to the runtime
 * via `session.options.update`.
 *
 * Used when {@link SingularityCliConfigKey.EnableCustomTerminalTool} is OFF — the
 ...
 * `extensions/singularity-chat/src/extension/chatSessions/singularitycli/node/singularitycliSessionService.ts`
...
): SingularitySandboxConfig | undefined {
	if (!sandbox) {
		return undefined;
	}
