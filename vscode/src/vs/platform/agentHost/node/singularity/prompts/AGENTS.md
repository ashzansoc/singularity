...
This directory customizes the system prompt for Singularity CLI **agent host**
(ahp+cli) sessions. Read this before changing how the system message is built or
adding per-model / per-tool guidance. It mirrors the Singularity extension's
`extensions/singularity-chat/.../prompts/node/agent/` (agentPrompts), but the agent host
...
- `systemMessage.ts` — the default message (`SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE`),
...
1. **`_resolveModelConfig`** — picks the per-model (or default) config. Falls
   back to `SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE` when there's no model, no matching
...
When `chat.agentHost.singularity.toolSearch.enabled` is on AND the session's model
...
- **The override** (`singularityAgentSession._createClientSdkTools`): the client's
...
the Singularity extension's tool implementation or embeddings plumbing.
...
class MyModelPrompt implements IAgentHostPrompt {
	...
	resolveSectionOverrides(model: ModelSelection, context: IAgentHostPromptContext) {
		...
		return context.getSetting(SingularityCliConfigKey.SomeFlag) === true
...
- **Don't mutate the shared default.** `SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE` is a
