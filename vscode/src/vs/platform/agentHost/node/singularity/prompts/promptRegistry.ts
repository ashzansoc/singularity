...
import { singularityCliConfigSchema } from '../../../common/singularityCliConfig.js';
import type { SchemaValue } from '../../../common/agentHostSchema.js';
import type { ModelSelection } from '../../../common/state/protocol/state.js';
import { appendSystemMessageContent, SINGULARITY_AGENT_HOST_FILE_LINK_INSTRUCTIONS, SINGULARITY_AGENT_HOST_WORKSPACELESS_INSTRUCTIONS, SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE, fullSystemPrompt, sectionOverrides } from './systemMessage.js';
import { resolveToolInstructionsOverride, toolSearchInstructionLines } from './toolInstructions.js';

type SingularityCliConfigDefinition = typeof singularityCliConfigSchema.definition;

/**
 * Read-time context handed to prompt contributors so they can gate behavior on
 * host configuration — the agent-host equivalent of the Singularity extension
 * injecting `IConfigurationService` into a resolver.
 *
 * Scoped to the Singularity CLI config schema so contributors (and tests) read
 * settings in a fully-typed way without depending on the whole configuration
 * service.
 */
export interface IAgentHostPromptContext {
	/**
	 * Returns the host-level value for a Singularity CLI setting, or `undefined`
	 * when unset. Mirrors `IAgentConfigurationService.getRootValue` bound to
	 * {@link singularityCliConfigSchema}.
	 */
	getSetting<K extends keyof SingularityCliConfigDefinition & string>(key: K): SchemaValue<SingularityCliConfigDefinition[K]> | undefined;
	 ...
	 * being present, the agent-host equivalent of the Singularity extension
	 ...
	 * {@link SINGULARITY_AGENT_HOST_WORKSPACELESS_INSTRUCTIONS}) telling the agent its
...
/**
 * Per-model system-prompt contributor for Singularity CLI agent-host sessions.
 *
 * Mirrors the Singularity extension's `IAgentPrompt`, but — because the agent host
 ...
 * Singularity extension's `IAgentPromptCtor`: a contributor matches a model either
...
/**
 * Registry of per-model system-prompt contributors for Singularity CLI agent-host
 * sessions. Mirrors the Singularity extension's `PromptRegistry`: contributors
...
export class AgentHostPromptRegistry {
	...
	resolveSystemMessageConfig(model: ModelSelection | undefined, context: IAgentHostPromptContext): SystemMessageConfig {
		...
		return appendSystemMessageContent(withWorkspacelessScratch, SINGULARITY_AGENT_HOST_FILE_LINK_INSTRUCTIONS);
	}

	/**
	 * Resolves the per-model config, before universal sections are layered on.
	 *
	 * Falls back to {@link SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE} when the model is
	...
	private _resolveModelConfig(model: ModelSelection | undefined, context: IAgentHostPromptContext): SystemMessageConfig {
		if (!model) {
			return SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE;
		}
	 ...
	 * {@link SINGULARITY_AGENT_HOST_WORKSPACELESS_INSTRUCTIONS}) as customize-mode
	...
	private _withWorkspacelessScratch(config: SystemMessageConfig, context: IAgentHostPromptContext): SystemMessageConfig {
		...
		const content = config.content ? `${config.content}\n\n${SINGULARITY_AGENT_HOST_WORKSPACELESS_INSTRUCTIONS}` : SINGULARITY_AGENT_HOST_WORKSPACELESS_INSTRUCTIONS;
