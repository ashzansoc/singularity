...
import { SINGULARITY_AGENT_HOST_IDENTITY } from './systemMessage.js';
 ...
 * is opt-in via {@link SingularityCliConfigKey.Opus48Prompt}. Off → falls back to the
...
class Claude48OpusPromptResolver implements IAgentHostPrompt {
	...
	resolveSectionOverrides(_model: ModelSelection, context: IAgentHostPromptContext): Partial<Record<SystemMessageSection, SectionOverride>> | undefined {
		return context.getSetting(SingularityCliConfigKey.Opus48Prompt) === true ? opus48SectionOverrides() : undefined;
	}
}
