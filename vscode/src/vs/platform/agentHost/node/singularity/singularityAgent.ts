...
import { CopilotClient as SingularityClient, RuntimeConnection, type CopilotClientOptions as SingularityClientOptions, type GitHubTelemetryNotification, type ManagedSettingsResolvedData, type SessionMode as SingularitySdkMode } from '@github/copilot-sdk';
...
import { SingularityCliConfigKey, singularityCliConfigSchema, type SingularitySdkLogLevelSetting } from '../../common/singularityCliConfig.js';
import { AgentHostMcpServersConfigKey, AgentHostSingularityMultiRootEnabledConfigKey, AgentHostPreferLongContextEnabledConfigKey, AgentHostSessionSyncEnabledConfigKey, AgentHostSystemProxyEnabledConfigKey, AgentHostMigrateLegacySingularityCliEnabledConfigKey, AutoApproveLevel, SessionMode, migrateLegacyAutopilotConfig, platformRootSchema, platformSessionSchema, type AgentHostMcpServers } from '../../common/agentHostSchema.js';
...
import { ISingularityConfigSlashCommandState } from '../../common/singularityConfigSlashCommands.js';
import { getSingularityHomePath } from '../../common/singularityHome.js';
...
interface ISingularityRuntimeManagedSettingsSdk {
	getManagedSettings(input?: { token?: string; host?: string }): Promise<{ account?: string; resolved: ManagedSettingsResolvedData }>;
}

function isSingularityRuntimeManagedSettingsSdk(value: unknown): value is ISingularityRuntimeManagedSettingsSdk {
...
import { SingularityAgentSession } from './singularityAgentSession.js';
import { createSingularityCliEnvironment } from './singularityCliEnvironment.js';
import { resolveSingularityCliSpawnPath } from './singularityCliLauncher.js';
import { ISingularitySessionContext, projectFromSingularityContext } from './singularityGitProject.js';
import { parsedPluginsEqual, toChildCustomizations } from './singularityPluginConverters.js';
import { SingularityGitHubTelemetryForwarder } from './singularityGitHubTelemetryForwarder.js';
import { SingularitySessionLauncher, ContextSizeConfigKey, ThinkingLevelConfigKey, getSingularityContextTier, isSingularityReasoningEffort, resolveSingularityReasoningEffort, type SingularitySessionLaunchPlan, type IActiveClientSnapshot } from './singularitySessionLauncher.js';
import { ShellManager } from './singularityShellTools.js';
import { isAgentHostTelemetryService } from '../agentHostTelemetryService.js';
import { ISingularityApiService, type IRestrictedTelemetryContext } from '../shared/singularityApiService.js';
import { AgentHostGitHubTelemetryRouter } from '../agentHostGitHubTelemetryRouter.js';
import { AgentHostClientType } from '../../common/agentHostClientInfo.js';
import { SingularitySlashCommandCompletionProvider, ISingularityRuntimeSlashCommandQueryOptions } from './singularitySlashCommandCompletionProvider.js';
...
import { SINGULARITY_INTEGRATION_ID } from '../../../endpoint/common/licenseAgreement.js';
import { getAppNodeModulesPath } from '../appNodeModules.js';
import { SingularitySlashCommandProvider } from './singularitySlashCommandProvider.js';
import { classifySingularityClientFailure, createSingularityFailureCorrelation, reportSingularityClientFailure, reportSingularityClientRecovery, reportSingularityClientRecoveryTurn, type SingularityClientFailureKind, type SingularityClientFailureOperation, type ISingularityFailureCorrelation } from './singularityFailureTelemetry.js';

const RUNTIME_SLASH_COMMAND_COMPLETION_WAIT_MS = 300;
const SINGULARITY_CAPI_URL = 'https://api.githubsingularity.com';

interface ISingularityClosedConnectionRecoveryResult {
	readonly failedTurnIds: ReadonlySet<string>;
	readonly stopSucceeded: boolean;
}

function isSingularityConnectionClosedError(error: unknown): boolean {
	return classifySingularityClientFailure(error) === 'connectionClosed';
}

/**
 * Proxy env vars that indicate the environment already configures a proxy.
 */
const SINGULARITY_PROXY_ENV_KEYS = ['HTTPS_PROXY', 'https_proxy', 'HTTP_PROXY', 'http_proxy', 'ALL_PROXY', 'all_proxy'] as const;
/**
 * Proxy env vars we set when injecting the resolved CAPI proxy.
 */
const SINGULARITY_PROXY_SET_ENV_KEYS = ['HTTP_PROXY', 'HTTPS_PROXY'] as const;
...
async function resolveSingularityCliPath(nodeModulesUri: URI): Promise<string> {
	const tried: string[] = [];
	const packagePrefixes = ['singularity', 'copilot'];
	for (const prefix of packagePrefixes) {
		for (const platformPackage of getSingularityPlatformPackageCandidates()) {
			const cliPath = URI.joinPath(nodeModulesUri, '@github', `${prefix}-${platformPackage}`, 'index.js').fsPath;
			tried.push(cliPath);
		...
		const topLevelPath = URI.joinPath(nodeModulesUri, '@github', prefix, 'index.js').fsPath;
		tried.push(topLevelPath);
		if (await fileExists(topLevelPath)) {
			return topLevelPath;
		}
	}
...
async function resolveSingularityCliPathForSdk(nodeModulesUri: URI): Promise<string> {
	const indexJsPath = await resolveSingularityCliPath(nodeModulesUri);
	return resolveSingularityCliSpawnPath(indexJsPath);
}

export type ISingularityPluginInfo = IParsedPlugin & { readonly pluginDir?: URI };

/**
 * A session that has been requested by a client but has not yet been
 * materialized into a real Singularity SDK session, worktree, or persisted
 * metadata. Created by {@link SingularityAgent.createSession} when no fork is
 * requested, and consumed by {@link SingularityAgent._materializeProvisional}
 * on the first {@link SingularityAgent.sendMessage}.
 *
 ...
 *
 * `model` absorbs {@link SingularityAgent.changeModel} updates that arrive
...
class SingularitySessionLifetime {
	...
	private _defaultResume: Promise<SingularityAgentSession> | undefined;
	private readonly _peerResumes = new Map<string, Promise<SingularityAgentSession | undefined>>();
	...
	resumeDefault(factory: () => Promise<SingularityAgentSession>): Promise<SingularityAgentSession> {
	...
	resumePeer(chatKey: string, factory: () => Promise<SingularityAgentSession | undefined>): Promise<SingularityAgentSession | undefined> {
...
export { SINGULARITY_AGENT_HOST_SYSTEM_MESSAGE } from './prompts/systemMessage.js';

type SingularityModelInfo = Awaited<ReturnType<SingularityClient['rpc']['models']['list']>>['models'][number];
 ...
 * {@link SingularityAgent._sessions} map (no parallel maps). The default chat is
 * optional because a Singularity session can exist as a provisional record (in
 * {@link SingularityAgent._provisionalSessions}) whose SDK-backed default chat has
 ...
 */
export class SingularitySessionEntry extends AgentSessionEntry<SingularityAgentSession> { }

export function resolveSingularityOtlpMetricsEndpoint(endpoint: string, protocol: 'http/json' | 'http/protobuf' | 'grpc'): string {
	if (protocol === 'grpc') {
		return endpoint;
	}
	try {
...
/**
 * Agent provider backed by the Singularity SDK {@link SingularityClient}.
 */
export class SingularityAgent extends Disposable implements IAgent {
	readonly id = 'singularitycli' as const;
	 ...
	 * {@link SingularityAgentSession}. Each session contributes a single
	 ...
	 * a `SINGULARITY_GH_HOST` change keeps the same token while changing the CAPI
	...
	private _client: SingularityClient | undefined;
	private _clientStarting: Promise<SingularityClient> | undefined;
	...
	private _closedConnectionRecovery: { readonly clientFailureId: string; readonly promise: Promise<ISingularityClosedConnectionRecoveryResult> } | undefined;
	...
	/** Reflects the `rt=1` field on the GitHub Singularity bearer token; gates enhanced GH telemetry. */
	...
	private readonly _sessions = this._register(new DisposableMap<string, SingularitySessionEntry>());
	/** SDK session id -> individual chat session, used to route connection-global SDK telemetry in O(1). */
	private readonly _sdkSessionsById = new Map<string, SingularityAgentSession>();
	/**
	 ...
	 * agent no longer writes `singularity.chats`).
	 */
	...
	private readonly _sessionLifetimes = new Map<string, SingularitySessionLifetime>();
	/**
	 * Sessions created by a client but not yet materialized into a Singularity
	...
	private readonly _sessionLauncher: SingularitySessionLauncher;
	private readonly _gitHubTelemetryForwarder: SingularityGitHubTelemetryForwarder;
	...
	private readonly _slashCommandProvider: SingularitySlashCommandProvider;
	...
	constructor(
		...
		@ISingularityApiService private readonly _singularityApiService: ISingularityApiService,
		...
		this._sessionLauncher = this._instantiationService.createInstance(SingularitySessionLauncher);
		this._gitHubTelemetryForwarder = this._instantiationService.createInstance(SingularityGitHubTelemetryForwarder, () => this._restrictedTelemetryEnabled);
		this._slashCommandProvider = new SingularitySlashCommandProvider(() => this._ensureClient().then(c => c.rpc.commands.list().then(c => c.commands)), this._logService);
		...
		this._register(completions.registerProvider(new SingularitySlashCommandCompletionProvider(this.id,
			{
		...
		this._register(this._configurationService.onDidRootConfigChange(() => {
			this._restartClientIfStartupConfigChanged().catch(err =>
				this._logService.error('[Singularity] Failed to apply root config change', err)
			);
		}));
		...
		this._register(this._configurationService.onDidRootConfigChange(() => {
			const enabled = this._isMigrateLegacySingularityCliEnabled();
		...
		this._register(this._byokBridgeRegistry.onDidChangeModels(() => {
			this._logService.info('[Singularity] BYOK bridge changed; refreshing models');
			this._refreshByokModels();
		}));

		// `SINGULARITY_GH_HOST` is a subprocess env var (applied in `_ensureClient`) the
		...
		this._register(this._gitHubEndpointService.onDidChange(() => {
			this._restartClientIfStartupConfigChanged().catch(err =>
				this._logService.error('[Singularity] Failed to restart client after endpoint change', err)
			);
		}));
	}
	...
	private _lastSingularitySdkLogLevelSetting: SingularitySdkLogLevelSetting = this._getSingularitySdkLogLevelSetting();
	...
	private _lastMigrateLegacyEnabled: boolean = this._isMigrateLegacySingularityCliEnabled();
	...
	private _isRubberDuckEnabled(): boolean {
		return this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.RubberDuck) === true;
	}

	private _getSingularitySdkLogLevelSetting(): SingularitySdkLogLevelSetting {
		return this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.SingularitySdkLogLevel) ?? 'info';
	}

	private _resolveSingularitySdkLogLevel(configured: SingularitySdkLogLevelSetting): NonNullable<SingularityClientOptions['logLevel']> {
	...
	private _isMigrateLegacySingularityCliEnabled(): boolean {
		return this._configurationService.getRootValue(platformRootSchema, AgentHostMigrateLegacySingularityCliEnabledConfigKey) === true;
	}
	...
	private async _restartClientIfStartupConfigChanged(): Promise<void> {
		...
		const singularitySdkLogLevelSetting = this._getSingularitySdkLogLevelSetting();
		const enterpriseHost = this._getEnterpriseHost();
		const systemProxyEnabled = this._isSystemProxyEnabled();
		if (this._lastSessionSyncEnabled === sessionSync && this._lastRubberDuckEnabled === rubberDuck && this._lastSingularitySdkLogLevelSetting === singularitySdkLogLevelSetting && this._lastEnterpriseHost === enterpriseHost && this._lastSystemProxyEnabled === systemProxyEnabled) {
			return;
		}
		const changed = [
			...
			this._lastSingularitySdkLogLevelSetting !== singularitySdkLogLevelSetting ? `singularitySdkLogLevel=${singularitySdkLogLevelSetting}` : undefined,
		...
		this._lastSingularitySdkLogLevelSetting = singularitySdkLogLevelSetting;
	...
	private async _requestClientRestart(reason: string): Promise<void> {
		...
		if (busyChats > 0) {
			this._logService.info(`[Singularity] Deferring SingularityClient restart (${reason}) until ${busyChats} in-flight turn(s) finish`);
			return;
		}
	...
	private async _applyPendingClientRestart(): Promise<void> {
		...
		this._logService.info(`[Singularity] Restarting SingularityClient (${reason})`);
		...
		// (`SINGULARITY_GH_HOST` routes through this same helper). Re-enumerate
	...
	/**
	 * Called by a {@link SingularityAgentSession} when its turn ends. Scheduled off
	...
	private _onChatTurnEnded(): void {
		...
		queueMicrotask(() => {
			this._applyPendingClientRestart().catch(err =>
				this._logService.error('[Singularity] Failed to apply deferred client restart', err)
			);
		});
	}

	private async _recoverFromClosedConnection(error: unknown, operation: SingularityClientFailureOperation, correlation?: ISingularityFailureCorrelation): Promise<ISingularityClosedConnectionRecoveryResult | undefined> {
		const failureKind = classifySingularityClientFailure(error);
		if (!failureKind) {
			return undefined;
		}
		...
		reportSingularityClientFailure(this._telemetryService, clientFailureId, failureKind, operation, this._chatsWithActiveTurn(), recoveryStarted, error, correlation);
	...
	private async _runClosedConnectionRecovery(clientFailureId: string, failureKind: SingularityClientFailureKind): Promise<ISingularityClosedConnectionRecoveryResult> {
		const stopWatch = StopWatch.create();
		const result = await this._doRecoverFromClosedConnection(clientFailureId);
		reportSingularityClientRecovery(this._telemetryService, {
			clientFailureId,
			failureKind,
	...
	private async _doRecoverFromClosedConnection(clientFailureId: string): Promise<ISingularityClosedConnectionRecoveryResult> {
		this._logService.error('[Singularity] Recovering from closed SDK connection');
		const failedTurnIds = new Set<string>();
		const error: ErrorInfo = {
			errorType: 'providerConnectionClosed',
			message: localize('singularityAgent.connectionClosed', "The Singularity CLI stopped unexpectedly. Retry your request."),
		};
		for (const entry of this._sessions.values()) {
			for (const chat of entry.allChatSessions()) {
				...
				if (failedTurnId) {
					...
					reportSingularityClientRecoveryTurn(
						this._telemetryService,
						clientFailureId,
						createSingularityFailureCorrelation(chat.sessionUri, chat.chatUri, failedTurnId, chat.sessionId),
					);
				}
			}
		}
		...
		try {
			...
			stopSucceeded = false;
			this._logService.error(error, '[Singularity] Failed to stop closed SDK client');
		}
		this._capiModels = [];
		this._publishModels();
	...
	private async _retryAfterClosedConnection<T>(operation: SingularityClientFailureOperation, task: () => Promise<T>, correlation?: ISingularityFailureCorrelation): Promise<T> {
		try {
			return await task();
		} catch (error) {
	...
	private _clientFailureCorrelation(chat: URI, turnId?: string): ISingularityFailureCorrelation {
		const context = this._getChatContext(chat);
		const chatUri = URI.parse(context.chatKey);
		return createSingularityFailureCorrelation(context.session, chatUri, turnId, context.target?.sessionId ?? context.sessionId);
	}
	...
	protected _createSingularityClient(options: SingularityClientOptions): SingularityClient {
		return new SingularityClient(options);
	}
	...
	getDescriptor(): IAgentDescriptor {
		return {
			provider: 'singularitycli',
			displayName: 'Singularity',
			description: localize('singularityAgent.description', "Singularity agent running in the local agent host process"),
			capabilities: {
	...
	private _isMultiRootEnabled(): boolean {
		return this._configurationService.getRootValue(platformRootSchema, AgentHostSingularityMultiRootEnabledConfigKey) === true;
	}

	getProtectedResources(): ProtectedResourceMetadata[] {
		return [
			this._gitHubEndpointService.getSingularityResource(),
	...
	async getNetworkDiagnosticsEndpoints(): Promise<readonly IAgentHostNetworkEndpoint[]> {
		let capiUrl = process.env['VSCODE_AGENT_HOST_CAPI_URL_OVERRIDE'] || SINGULARITY_CAPI_URL;
		if (this._githubToken) {
			try {
				capiUrl = await this._singularityApiService.resolveApiEndpoint(this._githubToken) || capiUrl;
			} catch (error) {
				this._logService.debug(`[Singularity] CAPI endpoint discovery for network diagnostics failed; using ${capiUrl}: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
		...
		return [
			...
			{ name: 'Singularity API (CAPI)', url: capiPingUrl.toString() },
		];
	}

	async getNetworkDiagnosticsAccount(): Promise<string | undefined> {
		return this._githubToken ? this._singularityApiService.resolveUserLogin?.(this._githubToken) : undefined;
	}
	...
	async getManagedSettingsDiagnostics(): Promise<IAgentHostManagedSettingsSnapshot> {
		...
		const cliPath = await resolveSingularityCliPathForSdk(nodeModulesUri);
		const runtimeSdkPath = join(dirname(cliPath), 'sdk', 'index.js');
		if (!await fileExists(runtimeSdkPath)) {
			throw new Error(`Singularity runtime SDK not found at ${runtimeSdkPath}`);
		}
		const runtimeSdk: unknown = await import(pathToFileURL(runtimeSdkPath).href);
		if (!isSingularityRuntimeManagedSettingsSdk(runtimeSdk)) {
			throw new Error('Singularity runtime SDK does not expose getManagedSettings()');
		}
	...
	async authenticate(resource: string, token: string): Promise<boolean> {
		...
		if (resource !== this._gitHubEndpointService.getSingularityResource().resource) {
			return false;
		}
		const tokenChanged = this._githubToken !== token;
		this._githubToken = token;
		this._updateRestrictedTelemetry(token);
		this._logService.info(`[Singularity] Auth token ${tokenChanged ? 'updated' : 'unchanged'}`);
		if (tokenChanged) {
	...
	private _updateRestrictedTelemetry(githubToken: string | undefined): void {
		...
		// CAPI Singularity session token confirms the `rt=1` opt-in. The GitHub token here carries no
		// `rt`/`tid` claims — those live in the Singularity session token, which the API service mints —
		// so the real values are resolved asynchronously below. Mirrors how the Singularity extension
		// reads `rt`/`tid` off its `SingularityToken` rather than the GitHub token.
	...
	private async _resolveRestrictedTelemetry(githubToken: string): Promise<void> {
		try {
			const ctx = await this._singularityApiService.resolveRestrictedTelemetryContext(githubToken);
			...
			this._logService.debug(`[Singularity] Restricted telemetry resolution failed: ${err instanceof Error ? err.message : String(err)}`);
		}
	}
	...
	private _applyRestrictedTelemetry(context: IRestrictedTelemetryContext | undefined): void {
		...
		if (rtEnabled !== this._restrictedTelemetryEnabled) {
			...
			this._logService.info(`[Singularity] Enhanced (restricted) telemetry ${rtEnabled ? 'enabled for this account' : 'disabled'}`);
		...
		// enhanced GH telemetry (kept off for public users), `tid` becomes `singularity_trackingId`, and
		...
		if (isAgentHostTelemetryService(this._telemetryService)) {
			...
			this._telemetryService.setSingularityTrackingId(context?.trackingId);
	...
	private async _routeGitHubTelemetry(notification: GitHubTelemetryNotification): Promise<void> {
		...
		try {
			const context = await this._singularityApiService.resolveRestrictedTelemetryContext(githubToken);
			if (this._githubToken !== githubToken) {
				return;
			}
			...
			this._logService.debug(`[Singularity:${sessionId}] Restricted telemetry context resolution failed; dropping ${notification.event.kind}: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
		...
		this._modelRefreshSchedule.value = disposableTimeout(() => {
			void (async () => {
				try {
					...
					this._logService.error(err, '[Singularity] Failed to schedule model refresh');
				} finally {
					if (this._scheduledModelRefresh === scheduled) {
						this._scheduledModelRefresh = undefined;
						this._modelRefreshSchedule.clear();
	...
	private async _refreshModels(attempt = 0, generation = this._modelCatalogGeneration): Promise<void> {
		...
		try {
			...
			if (attempt + 1 < this._modelRefreshMaxAttempts) {
				...
				this._logService.warn(`[Singularity] Failed to refresh models (attempt ${attempt + 1}), retrying in ${delay}ms`, err);
			...
			// shows through.
			this._logService.error(err, '[Singularity] Failed to refresh models');
			this._publishModels();
		}
	}
	...
	private _publishModels(): void {
		// Prefer BYOK (OpenRouter via TokenRouter) over CAPI catalog models.
		// CAPI `auto` routes to claude-haiku and never hits the user's OpenRouter key.
		let models = this._byokModels.length > 0 ? this._byokModels : this._capiModels;
		if (this._byokModels.length === 0) {
			// Never surface CAPI Auto when BYOK is unavailable — it always routes to claude-haiku.
	...
	private _refreshByokModels(): void {
		...
		this._logService.trace(`[Singularity] Found ${this._byokModels.length} BYOK models${this._byokModels.length ? ': ' + this._byokModels.map(m => m.name).join(', ') : ''}`);
		this._publishModels();
	}
	...
	private _stopClient(): Promise<void> {
		...
		const stopping = (async () => {
			...
			// ownership invariant on `SingularitySessionLauncher.disposeByokProxyHandle`.
	...
	private async _ensureClient(): Promise<SingularityClient> {
		if (this._shutdownPromise) {
		...
		const singularitySdkLogLevelSettingAtStartup = this._getSingularitySdkLogLevelSetting();
		...
		const clientStarting = (async () => {
			this._logService.info('[Singularity] Starting SingularityClient...');
			...
			const env = createSingularityCliEnvironment();
			...
			if (process.platform === 'linux') {
				const enabledFlags = env['SINGULARITY_CLI_ENABLED_FEATURE_FLAGS'];
				const flags = new Set((enabledFlags ?? '').split(',').map(f => f.trim()).filter(Boolean));
				flags.add('SHELL_SPAWN_BACKEND');
				env['SINGULARITY_CLI_ENABLED_FEATURE_FLAGS'] = [...flags].join(',');
			}

			// Identify VS Code's agent host traffic in CAPI
			env['GITHUB_SINGULARITY_INTEGRATION_ID'] = SINGULARITY_INTEGRATION_ID;
			this._logService.info(`[Singularity] Set CLI env: GITHUB_SINGULARITY_INTEGRATION_ID=${SINGULARITY_INTEGRATION_ID}`);

			// Point the Singularity CLI at a configured GitHub Enterprise host for its
			// authentication and CAPI endpoint discovery. `SINGULARITY_GH_HOST` is
			// Singularity-CLI-specific (it does not affect the `gh` CLI). Unset for
			// github.com so the CLI uses its default host.
			const enterpriseHost = this._getEnterpriseHost();
			if (enterpriseHost) {
				env['SINGULARITY_GH_HOST'] = enterpriseHost;
				this._logService.info(`[Singularity] Set CLI env: SINGULARITY_GH_HOST=${enterpriseHost}`);
			}
			...
			// (see build/.moduleignore), mirroring `SingularityCLISDK.getPackage` in the extension.
			...
			this._logService.info(`[Singularity] Resolved CLI path: ${cliPath}`);
			...
			if (nativeTelemetry) {
				env['OTEL_SERVICE_NAME'] = 'github-singularity';
			...
			if (nativeTelemetry?.external) {
				env['OTEL_EXPORTER_OTLP_METRICS_ENDPOINT'] = resolveSingularityOtlpMetricsEndpoint(nativeTelemetry.external.endpoint, nativeTelemetry.external.protocol);
				env['OTEL_EXPORTER_OTLP_METRICS_PROTOCOL'] = nativeTelemetry.external.protocol;
			} else if (nativeTelemetry) {
				env['OTEL_METRICS_EXPORTER'] = 'none';
			}
			const singularitySdkLogLevelAtStartup = this._resolveSingularitySdkLogLevel(singularitySdkLogLevelSettingAtStartup);

			const clientOptions: SingularityClientOptions = {
				useLoggedInUser: false,
				connection: RuntimeConnection.forStdio({ path: cliPath }),
				env,
				telemetry,
				logLevel: singularitySdkLogLevelAtStartup,
				enableRemoteSessions: sessionSyncAtStartup,
				onGetTraceContext: () => this._otelService.getCurrentTraceContext() ?? {},
				onGitHubTelemetry: notification => { void this._routeGitHubTelemetry(notification).catch(err => this._logService.trace(`[Singularity] GitHub telemetry routing failed: ${err instanceof Error ? err.message : String(err)}`)); },
			};
			const client = this._createSingularityClient(clientOptions);
			try {
				await client.start();
			} catch (error) {
				const failureKind = classifySingularityClientFailure(error);
				if (failureKind && error instanceof Error) {
					reportSingularityClientFailure(this._telemetryService, generateUuid(), failureKind, 'startClient', this._chatsWithActiveTurn(), false, error);
			...
			if (this._isSessionSyncEnabled() !== sessionSyncAtStartup || this._isRubberDuckEnabled() !== rubberDuckAtStartup || this._getSingularitySdkLogLevelSetting() !== singularitySdkLogLevelSettingAtStartup || this._getEnterpriseHost() !== enterpriseHostAtStartup || this._isSystemProxyEnabled() !== systemProxyEnabledAtStartup) {
				await client.stop();
				throw new Error('Singularity startup config changed while the client was starting');
			}
			this._logService.info('[Singularity] SingularityClient started successfully');
			this._client = client;
			this._clientStarting = undefined;
			return client;
		})();
	...
	private _createThinkingLevelConfigSchemaProperty(reasoningEfforts: readonly string[] | undefined, defaultReasoningEffort: string | undefined, modelId: string | undefined): ConfigPropertySchema | undefined {
		// Only advertise efforts the Singularity launcher actually accepts, otherwise the picker would
		// surface a level that is silently dropped when the session is launched.
		const supportedReasoningEfforts = reasoningEfforts?.filter(isSingularityReasoningEffort);
		...
		return {
			type: 'string',
			title: localize('singularity.modelThinkingLevel.title', "Thinking Level"),
			description: localize('singularity.modelThinkingLevel.description', "Controls how much reasoning effort the model uses."),
	 ...
	 * `extensions/singularity-chat/src/extension/chat/vscode-node/languageModelAccess.ts`.
	 *
	 ...
	 * two-valued `contextTier` at the SDK boundary by {@link getSingularityContextTier}, using the model's long-context
	...
	private _createContextSizeConfigSchemaProperty(billing: ICAPIModelBilling | undefined): ConfigPropertySchema | undefined {
		...
		if (this._isPreferLongContextEnabled() && !hasLongContextSurcharge(billing)) {
			return {
				type: 'number',
				title: localize('singularity.modelContextSize.title', "Context Size"),
				description: localize('singularity.modelContextSize.description', "Selects the context window size for this model."),
				default: longContextMax,
				enum: [longContextMax],
				enumLabels: [formatTokenCount(longContextMax)],
				enumDescriptions: [
					localize('singularity.modelContextSize.longerSessions', "Longer sessions"),
				],
			};
		}
		...
		return {
			...
			enumDescriptions: [
				localize('singularity.modelContextSize.default', "Default"),
				localize('singularity.modelContextSize.longerSessions', "Longer sessions"),
			],
		};
	}
	 ...
	 * {@link getSingularityContextTier} to decide whether a numeric selection opts into `long_context`.
	 ...
	 */
	private _createModelPickerMeta(modelInfo: SingularityModelInfo, billing: ICAPIModelBilling | undefined): Record<string, unknown> | undefined {
		return createPricingMetaFromBilling(billing, modelInfo.modelPickerPriceCategory, modelInfo.modelPickerCategory);
	}

	private _createModelConfigSchema(m: SingularityModelInfo, billing: ICAPIModelBilling | undefined): ConfigSchema | undefined {
	...
	async listSessions(): Promise<IAgentSessionMetadata[]> {
		this._logService.info('[Singularity] Listing sessions...');
		const sessions = await this._retryAfterClosedConnection('listSessions', async () => {
			const client = await this._ensureClient();
			return client.listSessions();
		});
		const migrateLegacy = this._isMigrateLegacySingularityCliEnabled();
		...
		const mapped = await Promise.all(sessions.map(async s => {
			...
			if (!metadata?.workingDirectory) {
				...
				// genuinely un-adopted extension-host Singularity CLI session as adoptable
		...
		this._logService.info(`[Singularity] Found ${result.length} sessions`);
		return result;
	}
	...
	async getSessionMetadata(session: URI): Promise<IAgentSessionMetadata | undefined> {
		...
		}, createSingularityFailureCorrelation(session, URI.parse(buildDefaultChatUri(session)), undefined, sessionId));
		if (!sessionMetadata) {
			return undefined;
		}
	...
	private async _listModels(gitHubToken: string): Promise<IAgentModelInfo[]> {
		this._logService.info('[Singularity] Listing models...');
		const client = await this._ensureClient();
		...
		this._logService.info(`[Singularity] Found ${result.length} models: ${result.map(m => m.name).join(', ')}`);
		return result;
	}
	...
	private async _resolveCreateWorkingDirectory(sessionConfig: IAgentCreateSessionConfig, sessionId: string, isWorkspaceless: boolean): Promise<URI> {
		...
		// dir (mirroring the GitHub app's `<singularityHome>/chats/<id>`) rather than
		...
		this._logService.trace(`[Singularity] No workingDirectory provided, defaulting to temp directory: ${workingDirectory.fsPath}`);
		return workingDirectory;
	}

	/**
	 * Stable per-session scratch directory for a workspace-less chat:
	 * `<userHome>/.singularity/chats/<sessionId>`. Deterministic, persistent, and
	...
	private async _ensureWorkspacelessScratchDir(scratchDir: URI, sessionId: string): Promise<void> {
		try {
			await fs.mkdir(scratchDir.fsPath, { recursive: true });
			this._logService.trace(`[Singularity:${sessionId}] Workspace-less scratch directory ready: ${scratchDir.fsPath}`);
		} catch (error) {
			this._logService.warn(`[Singularity:${sessionId}] Failed to ensure workspace-less scratch directory '${scratchDir.fsPath}': ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	private async _cleanupWorkspacelessScratchDir(scratchDir: URI, sessionId: string): Promise<void> {
		try {
			...
			this._logService.trace(`[Singularity:${sessionId}] Removed workspace-less scratch directory: ${scratchDir.fsPath}`);
		} catch (error) {
			this._logService.warn(`[Singularity:${sessionId}] Failed to remove workspace-less scratch directory '${scratchDir.fsPath}': ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	private _resolveChatTarget(chat: URI): { session: URI; chat: URI } {
		...
		if (!parsed) {
			throw new Error(`Singularity chat operation requires an AHP chat URI: ${chat.toString()}`);
		}
		return { session: URI.parse(parsed.session), chat: chat };
	}

	private _getChatContext(chatOrSession: URI): { session: URI; sessionId: string; chatKey: string; target: SingularityAgentSession | undefined; isPeerChat: boolean } {
	 ...
	 * default chat is the primary {@link SingularityAgentSession} of the owning
	 * {@link SingularitySessionEntry}.
	 */
	private _findAnySession(sessionId: string): SingularityAgentSession | undefined {
		return this._sessions.get(sessionId)?.defaultChat;
	}

	private _getRuntimeSlashCommands(sessionId: string, options?: ISingularityRuntimeSlashCommandQueryOptions) {
	 ...
	 */
	private _findPeerChat(session: URI, chat: URI): SingularityAgentSession | undefined {
	 ...
	 */
	private _ensureEntry(sessionId: string): SingularitySessionEntry {
		let entry = this._sessions.get(sessionId);
		if (!entry) {
			entry = new SingularitySessionEntry();
	...
	async createSession(config?: IAgentCreateSessionConfig): Promise<IAgentCreateSessionResult> {
		...
		this._logService.info(`[Singularity] Creating session... ${sessionConfig.model ? `model=${sessionConfig.model.id}` : ''}`);
		...
		if (sessionConfig.fork) {
			...
			return this._queueSession(sourceSessionId, async () => {
				this._logService.info(`[Singularity] Forking session ${sourceSessionId} at turnId=${sessionConfig.fork!.turnId}`);
				...
				try {
					...
					this._logService.warn(`[Singularity] Failed to copy session database for fork: ${err instanceof Error ? err.message : String(err)}`);
				}
				...
				this._logService.info(`[Singularity] Forked session created: ${session.toString()}`);
				...
				try {
					...
					this._logService.warn(`[Singularity] Failed to copy reviewed ref for fork: ${err instanceof Error ? err.message : String(err)}`);
				}

				const project = await projectFromSingularityContext({ cwd: workingDirectory.fsPath }, this._gitService);
		...
		// Non-fork path: create a *provisional* session. The Singularity SDK
		...
		if (this._findAnySession(sessionId)) {
			this._logService.info(`[Singularity] createSession is a no-op: session already materialized: ${sessionUri.toString()}`);
			const project = await projectFromSingularityContext({ cwd: workingDirectory.fsPath }, this._gitService);
		...
		this._logService.info(`[Singularity] Session created (provisional): ${sessionUri.toString()}`);
		return { session: sessionUri, resolvedWorkingDirectory: workingDirectory, provisional: true, ...(project ? { project } : {}) };
	}

	/**
	 * Materializes an imported conversation into a real, editable Singularity
	 * session. Translates the supplied turns into a Singularity event log, seeds it
	...
	private async _importConversation(sessionConfig: IAgentCreateSessionConfig, sessionId: string, workingDirectory: URI): Promise<IAgentCreateSessionResult> {
		...
		return this._queueSession(sessionId, async () => {
			this._logService.info(`[Singularity] Importing conversation into session ${sessionId} (${importConfig.turns.length} turns)`);
			...
			const projectPromise = projectFromSingularityContext({ cwd: workingDirectory.fsPath }, this._gitService);
			const eventsPath = join(getSingularityHomePath(this._environmentService.userHome.fsPath, process.env), 'session-state', sessionId, 'events.jsonl');
			...
			this._logService.info(`[Singularity] Imported session created: ${sessionUri.toString()}`);
			return { session: sessionUri, resolvedWorkingDirectory: workingDirectory, ...(project ? { project } : {}) };
		});
	}

	/**
	 * Whether an on-disk Singularity session was created by the VS Code extension-host
	 * Singularity CLI feature — identified by its `vscode.metadata.json` marker under
	 * `~/.singularity/session-state/<id>/`. Distinguishes EH CLI sessions (the only
	 * ones we migrate) from other Singularity SDK sessions that share the same store
	 * (standalone `singularity` CLI runs, Local agent sessions, …).
	 */
	/** Absolute path of the extension-host Singularity CLI `vscode.metadata.json` marker for `sessionId`. */
	private _extensionHostCliMarkerPath(sessionId: string): string {
		return join(getSingularityHomePath(this._environmentService.userHome.fsPath, process.env), 'session-state', sessionId, 'vscode.metadata.json');
	}
	...
	/**
	 * Reads the VS Code-layer custom title the extension-host Singularity CLI feature
	...
	/**
	 * Adopt-on-open for legacy extension-host Singularity CLI sessions. If `session`
	 * has an on-disk SDK event log (`~/.singularity/session-state/<id>/`) but no
	...
	async ensureSessionAdopted(session: URI): Promise<boolean> {
		...
		return this._queueSession(sessionId, async () => {
			...
			// Only migrate legacy EH Singularity CLI sessions — never other Singularity SDK
			// sessions (standalone CLI, Local agent, …) that share `~/.singularity`.
			...
			this._logService.info(`[Singularity] Adopting legacy session ${sessionId} in place (reusing on-disk events.jsonl)`);
			...
			// is also what makes the legacy extension-host Singularity CLI list stop
	...
	/**
	 * Promotes a {@link IProvisionalSession} into a real Singularity SDK session
	 ...
	 * `isolation === 'worktree'`), instantiates the {@link SingularityAgentSession},
	 ...
	 */
	private async _materializeProvisional(sessionId: string, resolvedWorkingDirectories?: readonly URI[]): Promise<SingularityAgentSession> {
		const provisional = this._provisionalSessions.get(sessionId);
		if (!provisional) {
		...
		let agentSession: SingularityAgentSession | undefined;
		...
		try {
			...
			const launchPlan: SingularitySessionLaunchPlan = {
				kind: 'create',
				client,
				sessionId,
				workingDirectory,
		...
		this._checkpointService.captureBaselineCheckpoint(sessionUri, materializedWorkingDirectories).catch(err => {
			this._logService.warn(`[Singularity:${sessionId}] Baseline checkpoint capture failed: ${err instanceof Error ? err.message : String(err)}`);
		});

		this._logService.info(`[Singularity] Session materialized: ${sessionUri.toString()}`);
	...
	private async _resolveAgentWhenMaterializing(provisional: IProvisionalSession, snapshot: IActiveClientSnapshot, workingDirectory: URI | undefined): Promise<{ agent: AgentSelection; name: string } | undefined> {
		...
		if (alternativeAgentName && alternativeAgent) {
			this._logService.info(`[Singularity] Agent file ${agent.uri} is in the original repo; using worktree agent ${alternativeAgent?.uri}`);
	...
	async sessionConfigCompletions(_params: IAgentSessionConfigCompletionsParams): Promise<SessionConfigCompletionsResult> {
		// Branch completions (the only dynamic Singularity property) are owned by the
	...
	removeActiveClient(session: URI, clientId: string): void {
		const sessionId = AgentSession.id(session);
		this._logService.info(`[Singularity:${sessionId}] removeActiveClient: clientId=${clientId}`);
	...
	onClientToolCallComplete(session: URI, chat: URI, toolCallId: string, result: ToolCallResult): void {
		...
		if (!isDefaultChatUri(chat)) {
			...
			if (!peerChat) {
				this._logService.warn(`[Singularity:${sessionId}] Dropping client tool completion for missing peer chat: chat=${chat.toString()}, toolCallId=${toolCallId}, success=${result.success}`);
				return;
			}
			this._logService.info(`[Singularity:${sessionId}] Routing client tool completion to peer chat: chat=${chat.toString()}, toolCallId=${toolCallId}, success=${result.success}`);
			peerChat.handleClientToolCallComplete(toolCallId, result);
		} else {
			const entry = this._findAnySession(sessionId);
			if (!entry) {
				this._logService.warn(`[Singularity:${sessionId}] Dropping client tool completion for missing default chat: chat=${chat.toString()}, toolCallId=${toolCallId}, success=${result.success}`);
				return;
			}
			this._logService.info(`[Singularity:${sessionId}] Routing client tool completion to default chat: chat=${chat.toString()}, toolCallId=${toolCallId}, success=${result.success}`);
	...
	private async _sendMessageOnce(chat: URI, prompt: string, attachments?: readonly MessageAttachment[], turnId?: string, senderClientId?: string, clientType = AgentHostClientType.Unknown, workingDirectories?: readonly URI[]): Promise<void> {
		...
		if (context.isPeerChat) {
			...
			if (!entry) {
				throw new Error(`[Singularity] sendMessage for unknown chat: ${chat.toString()}`);
			}
			if (turnId) {
		...
		await this._queueSession(context.sessionId, async () => {
			...
			let entry: SingularityAgentSession | undefined;
			...
			this._logService.info(`[Singularity:${context.sessionId}] sendMessage: cachedEntry=${hadCachedEntry}, hasActiveClient=${!!activeClient}, activeClientId=${activeClient ? '(set)' : '(none)'}`);
			...
			if (entry && (rootsChanged || structuralConfigChanged)) {
				this._logService.info(`[Singularity:${context.sessionId}] Session configuration changed, refreshing session. clients=[${activeClient ? [...activeClient.toolSet.clientIds()].join(', ') || '(none)' : '(none)'}]`);
			...
			if (!entry) {
				this._logService.info(`[Singularity:${context.sessionId}] No cached entry${hadCachedEntry ? ' (was evicted by requiresRestart)' : ''}, calling _resumeSession`);
			}
			...
			try {
				...
				this._logService.error(`[Singularity:${context.sessionId}] entry.send() failed: code=${errCode}, message=${errMsg}, hadCachedEntry=${hadCachedEntry}, errorType=${err?.constructor?.name}`);
				throw err;
			}
		});
	}
	...
	/**
	 * Translates the AHP-side `mode` to the Singularity SDK's three-mode space
	 ...
	 */
	private _resolveSdkMode(session: URI): SingularitySdkMode | undefined {
	 ...
	 */
	private _getSessionConfigState(sessionId: string): ISingularityConfigSlashCommandState {
	...
	setPendingMessages(chat: URI, steeringMessage: PendingMessage | undefined, _queuedMessages: readonly PendingMessage[]): void {
		const context = this._getChatContext(chat);
		if (!context.target) {
			this._logService.warn(`[Singularity:${context.sessionId}] setPendingMessages: chat not found for ${chat.toString()}`);
			return;
		}
	...
	async getSessionMessages(session: URI): Promise<readonly Turn[]> {
		...
		if (subagentInfo) {
			...
			const parentEntry = this._findAnySession(rootSessionId) ?? await this._resumeSession(rootSessionId).catch(err => {
				this._logService.warn(`[Singularity:${rootSessionId}] Failed to resume root for subagent restore`, err);
				return undefined;
			});
			if (!parentEntry) {
				return [];
		...
		const entry = context.target ?? await this._resumeSession(sessionId).catch(err => {
			...
			this._logService.warn(`[Singularity:${sessionId}] Failed to resume session for message lookup`, err);
			return undefined;
		});
		if (!entry) {
			return [];
	...
	async getSubagentSessions(session: URI): Promise<readonly IRestoredSubagentSession[]> {
		...
		const entry = this._findAnySession(sessionId) ?? await this._resumeSession(sessionId).catch(err => {
			this._logService.warn(`[Singularity:${sessionId}] Failed to resume session for subagent lookup`, err);
			return undefined;
		});
	...
	async releaseSession(session: URI): Promise<void> {
		...
		await lifetime.queueSession(() => lifetime.release(async () => {
			...
			this._logService.info(`[Singularity:${sessionId}] Releasing idle session from memory (durable state preserved)`);
	...
	private async _abortSession(chat: URI): Promise<void> {
		try {
			...
			if (!isSingularityConnectionClosedError(error)) {
	...
	private async _createChat(chat: URI, options?: IAgentCreateChatOptions): Promise<IAgentCreateChatResult | void> {
		...
		const parsed = parseChatUri(chat);
		if (!parsed) {
			throw new Error(`[Singularity] createChat: malformed chat URI ${chat.toString()}`);
		}
		...
		await queue(async () => {
			...
			let launchPlan: SingularitySessionLaunchPlan;
			let sdkSessionId: string;
			let sideChat: IPersistedChat['sideChat'];
			if (options?.fork) {
				if (!workingDirectory) {
					throw new Error(`[Singularity] createChat fork: missing working directory for session ${session.toString()}`);
				}
				const sourceEntry = await this._resolveChatEntry(session, options.fork.source);
				if (!sourceEntry) {
					throw new Error(`[Singularity] createChat fork: source chat ${options.fork.source.toString()} not found`);
				}
				...
				if (!workingDirectory) {
					throw new Error(`[Singularity] createChat side chat: missing working directory for session ${session.toString()}`);
				}
				const sourceEntry = await this._resolveChatEntry(session, options.sideChat.source);
				if (!sourceEntry) {
					throw new Error(`[Singularity] createChat side chat: source chat ${options.sideChat.source.toString()} not found`);
				}
			...
			try {
				...
				this._ensureEntry(sessionId).registerPeerChat(chatKey, new SingularitySessionEntry(agentSession));
				...
				// peer-chat catalog (`singularity.chats` is no longer written).
				...
				this._logService.info(`[Singularity] Created additional chat ${chatKey} in session ${session.toString()}${options?.fork ? ' (forked)' : ''}`);
			} catch (error) {
				agentSession?.dispose();
				throw error;
			}
	...
	/**
	 * Resolves the {@link SingularityAgentSession} backing a chat URI — the
	 ...
	 */
	private async _resolveChatEntry(session: URI, chatUri: URI): Promise<SingularityAgentSession | undefined> {
		const sessionId = AgentSession.id(session);
	 ...
	 */
	private async _forkSdkChat(client: SingularityClient, sourceEntry: SingularityAgentSession, turnId: string, targetDbDir: URI): Promise<{ sessionId: string; inheritedTurnCount: number }> {
		...
		try {
			...
			this._logService.warn(`[Singularity] Failed to copy session database for chat fork: ${err instanceof Error ? err.message : String(err)}`);
		}
	...
	private async _disposeChat(session: URI, chat: URI): Promise<void> {
		...
		// `singularity.chats`; it only drops the live backing and SDK chat.
		...
		if (sdkSessionId) {
			try {
				...
				this._logService.warn(`[Singularity] Failed to delete SDK session for chat ${chatKey}: ${err instanceof Error ? err.message : String(err)}`);
			}
		}
	}
	 ...
	 * falls back to a one-time read of its own `singularity.chats` blob. Best-effort
	...
	async materializeChat(chat: URI, providerData: string | undefined): Promise<void> {
		...
		if (providerData !== undefined) {
			...
			if (!backing) {
				this._logService.warn(`[Singularity] materializeChat: dropping corrupt providerData for ${chatKey}`);
				return;
			}
		} else {
	 ...
	 * legacy `singularity.chats` catalog, mapping each entry to its channel URI and
	 ...
	 * `singularity.chats` catalog (seeding the live map) for sessions that have not
	...
	private _getOrCreateSessionLifetime(sessionId: string): SingularitySessionLifetime | undefined {
		if (this._isShuttingDown) {
			return undefined;
		}
		let lifetime = this._sessionLifetimes.get(sessionId);
		if (!lifetime) {
			lifetime = new SingularitySessionLifetime();
	...
	private _resetSessionLifetime(sessionId: string): void {
		if (!this._isShuttingDown && this._sessionLifetimes.get(sessionId)?.isPermanentlyClosed) {
			this._sessionLifetimes.set(sessionId, new SingularitySessionLifetime());
		}
	}
	...
	/**
	 * Returns the SDK-backed {@link SingularityAgentSession} for an additional peer
	 ...
	 */
	private async _ensureChatSession(session: URI, chat: URI): Promise<SingularityAgentSession | undefined> {
		const chatKey = chat.toString();
	...
	private async _doEnsureChatSession(session: URI, chat: URI, lifetime: SingularitySessionLifetime): Promise<SingularityAgentSession | undefined> {
		const chatKey = chat.toString();
		const sessionId = AgentSession.id(session);
		...
		try {
			...
			const workingDirectory = parentEntry?.workingDirectory
				?? this._provisionalSessions.get(sessionId)?.workingDirectory;
			if (!workingDirectory) {
				this._logService.warn(`[Singularity] Cannot resume chat ${chatKey}: missing working directory`);
				return undefined;
			}
			const client = await this._ensureClient();
			const activeClient = this._getOrCreateActiveClient(session, workingDirectory);
			...
			this._ensureEntry(sessionId).registerPeerChat(chatKey, new SingularitySessionEntry(agentSession));
			this._sdkSessionsById.set(agentSession.sessionId, agentSession);
			this._logService.info(`[Singularity] Resumed additional chat ${chatKey} in session ${session.toString()}`);
			return agentSession;
		} catch (error) {
			agentSession?.dispose();
			this._logService.warn(`[Singularity] Failed to resume additional chat ${chatKey}: ${error instanceof Error ? error.message : String(error)}`);
			throw error;
		} finally {
			lease.dispose();
		}
	...
	async truncateSession(session: URI, turnId: string | undefined, chat: URI): Promise<void> {
		...
		await this._queueSession(sessionId, async () => {
			this._logService.info(`[Singularity:${sessionId}] Truncating ${isPeerChat ? `peer chat ${chat.toString()}` : 'session'}${turnId !== undefined ? ` at turnId=${turnId}` : ' (all turns)'}`);
			...
			if (!entry) {
				this._logService.info(`[Singularity:${sessionId}] No chat entry resolved for truncation; nothing to truncate`);
				return;
			}
			...
			if (eventId) {
				...
				this._logService.info(`[Singularity:${sessionId}] No event ID found for truncation, nothing to truncate`);
			}

			this._logService.info(`[Singularity:${sessionId}] Session truncated`);
		});
	}
	...
	private async _changeModelOnce(chat: URI, model: ModelSelection): Promise<void> {
		...
		// resolveSingularityReasoningEffort); computed at the point of use so the
		// provisional-session path doesn't resolve or log it prematurely.
		if (context.isPeerChat) {
			await context.target?.setModel(model.id, resolveSingularityReasoningEffort(model, this._configurationService, this._logService, context.sessionId), getSingularityContextTier(model, longContextWindow, freeLongContext));
		...
		if (entry) {
			await entry.setModel(model.id, resolveSingularityReasoningEffort(model, this._configurationService, this._logService, context.sessionId), getSingularityContextTier(model, longContextWindow, freeLongContext));
		}
	...
	async shutdown(): Promise<void> {
		if (!this._shutdownPromise) {
			...
			this._shutdownPromise = (async () => {
				...
				this._modelRefreshRetry.clear();
				this._logService.info('[Singularity] Shutting down...');
	...
	private async _configureProxyEnv(env: Record<string, string | undefined>): Promise<void> {
		...
		if (proxy) {
			for (const key of SINGULARITY_PROXY_SET_ENV_KEYS) {
				env[key] = proxy;
			}
			this._logService.info('[Singularity] Resolved CAPI proxy and forwarded HTTP_PROXY/HTTPS_PROXY to Singularity SDK');
		}
	}
	...
	private async _resolveProxyForSdk(env: Record<string, string | undefined> = process.env): Promise<string | undefined> {
		...
		if (SINGULARITY_PROXY_ENV_KEYS.some(key => env[key])) {
			this._logService.debug('[Singularity] Proxy env var already set; leaving Singularity SDK proxy configuration to the environment');
			return undefined;
		}

		let capiUrl = env['VSCODE_AGENT_HOST_CAPI_URL_OVERRIDE'] || SINGULARITY_CAPI_URL;
		if (this._githubToken) {
			try {
				const discovered = await this._singularityApiService.resolveApiEndpoint(this._githubToken);
				if (discovered) {
					capiUrl = discovered;
				}
			} catch (error) {
				this._logService.debug(`[Singularity] CAPI endpoint discovery for proxy resolution failed; using ${capiUrl}: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
		...
		try {
			...
			this._logService.warn(`[Singularity] Failed to resolve CAPI proxy for ${capiUrl}: ${error instanceof Error ? error.message : String(error)}`);
			return undefined;
		}
	}
	...
	/**
	 * Instantiates a {@link SingularityAgentSession} for the given session id.
	 * The caller is responsible for awaiting {@link SingularityAgentSession.initializeSession}
	 ...
	 */
	private _createAgentSession(launchPlan: SingularitySessionLaunchPlan, customizationDirectory: URI | undefined, activeClient: ActiveClient, identity?: ISingularityAgentSessionIdentity): SingularityAgentSession {
	...
	private _throwIfClientReplaced(client: SingularitySessionLaunchPlan['client'], agentSession: SingularityAgentSession): void {
	...
	private _registerInitializedSession(sessionId: string, agentSession: SingularityAgentSession, client: SingularitySessionLaunchPlan['client']): void {
		this._throwIfClientReplaced(client, agentSession);
		...
		entry.setDefaultChat(defaultChatKey, new SingularitySessionEntry(agentSession));
		this._sdkSessionsById.set(agentSession.sessionId, agentSession);
	}
	...
	private async _releaseSessionResources(sessionId: string): Promise<void> {
		...
		if (entry) {
			try {
				...
				this._logService.warn(`[Singularity:${sessionId}] Failed to destroy session before cleanup: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
		this._sessions.deleteAndDispose(sessionId);
	...
	protected _resumeSession(sessionId: string, workingDirectories?: readonly URI[]): Promise<SingularityAgentSession> {
		const lifetime = this._getOrCreateSessionLifetime(sessionId);
		if (!lifetime) {
	...
	private async _doResumeSession(sessionId: string, workingDirectories?: readonly URI[]): Promise<SingularityAgentSession> {
		this._logService.info(`[Singularity:${sessionId}] _resumeSession called — session not in memory, resuming...`);
		const client = await this._ensureClient();
		...
		const sessionMetadata = await client.getSessionMetadata(sessionId).catch(err => {
			this._logService.warn(`[Singularity:${sessionId}] getSessionMetadata failed`, err);
			return undefined;
		});
		...
		if (!workingDirectory) {
			throw new Error(`workingDirectory is required to resume Singularity session '${sessionId}'`);
		}
		...
		if (storedMetadata.agent && !resolvedAgentName) {
			this._logService.info(`[Singularity:${sessionId}] Stored custom agent is not available in the current plugin snapshot; resuming without a custom agent`);
		}
		const launchPlan: SingularitySessionLaunchPlan = {
			kind: 'resume',
			client,
	...
	private static readonly _META_MODEL = 'singularity.model';
	private static readonly _META_AGENT = 'singularity.agent';
	private static readonly _META_CWD = 'singularity.workingDirectory';
	/** Persisted ordered working-directory set (JSON array of URI strings; index 0 = primary). */
	private static readonly _META_CWDS = 'singularity.workingDirectories';
	private static readonly _META_CUSTOMIZATION_DIRECTORY = 'singularity.customizationDirectory';
	private static readonly _META_PROJECT_RESOLVED = 'singularity.project.resolved';
	private static readonly _META_PROJECT_URI = 'singularity.project.uri';
	private static readonly _META_PROJECT_DISPLAY_NAME = 'singularity.project.displayName';
	/** Persisted catalog of additional (non-default) peer chats, keyed by chatId. */
	private static readonly _META_CHATS = 'singularity.chats';

	/**
	 * Reads the agent's legacy peer-chat catalog (`singularity.chats`) for a
	...
	private async _readPersistedChats(session: URI): Promise<Map<string, IPersistedChat>> {
		...
		try {
			const raw = await ref.object.getMetadata(SingularityAgent._META_CHATS);
			if (!raw) {
				return new Map();
			}
			...
			this._logService.warn(`[Singularity] Failed to read persisted chats for ${session.toString()}: ${err instanceof Error ? err.message : String(err)}`);
			return new Map();
		} finally {
			ref.dispose();
		}
	...
	private async _storeSessionMetadata(session: URI, model: ModelSelection | undefined, workingDirectory: URI | undefined, workingDirectories: readonly URI[] | undefined, customizationDirectory: URI | undefined, project: IAgentSessionProjectInfo | undefined, projectResolved = project !== undefined, configValues?: Record<string, unknown>, customTitle?: string): Promise<void> {
		...
		try {
			...
			if (model) {
				work.push(db.setMetadata(SingularityAgent._META_MODEL, this._serializeModelSelection(model)));
			}
			if (workingDirectory) {
				work.push(db.setMetadata(SingularityAgent._META_CWD, workingDirectory.toString()));
			}
			...
			if (workingDirectories) {
				work.push(db.setMetadata(SingularityAgent._META_CWDS, JSON.stringify(workingDirectories.map(d => d.toString()))));
			}
			if (customizationDirectory) {
				work.push(db.setMetadata(SingularityAgent._META_CUSTOMIZATION_DIRECTORY, customizationDirectory.toString()));
			}
			if (projectResolved) {
				work.push(db.setMetadata(SingularityAgent._META_PROJECT_RESOLVED, 'true'));
			}
			if (project) {
				work.push(db.setMetadata(SingularityAgent._META_PROJECT_URI, project.uri.toString()));
				work.push(db.setMetadata(SingularityAgent._META_PROJECT_DISPLAY_NAME, project.displayName));
			}
	...
	private async _readSessionMetadata(session: URI): Promise<{ model?: ModelSelection; agent?: AgentSelection; workingDirectory?: URI; workingDirectories?: readonly URI[]; customizationDirectory?: URI; workspaceless?: boolean }> {
		...
		try {
			const [model, agent, cwd, cwds, customizationDirectory, workspaceless] = await Promise.all([
				ref.object.getMetadata(SingularityAgent._META_MODEL),
				ref.object.getMetadata(SingularityAgent._META_AGENT),
				ref.object.getMetadata(SingularityAgent._META_CWD),
				ref.object.getMetadata(SingularityAgent._META_CWDS),
				ref.object.getMetadata(SingularityAgent._META_CUSTOMIZATION_DIRECTORY),
	...
	private async _readStoredSessionMetadata(session: URI): Promise<{ model?: ModelSelection; agent?: AgentSelection; workingDirectory?: URI; workingDirectories?: readonly URI[]; customizationDirectory?: URI; project?: IAgentSessionProjectInfo; resolved: boolean; workspaceless?: boolean } | undefined> {
		...
		try {
			const [model, agent, cwd, cwds, customizationDirectory, resolved, uri, displayName, workspaceless] = await Promise.all([
				...
				ref.object.getMetadata(SingularityAgent._META_CWDS),
				ref.object.getMetadata(SingularityAgent._META_CUSTOMIZATION_DIRECTORY),
				ref.object.getMetadata(SingularityAgent._META_PROJECT_RESOLVED),
				ref.object.getMetadata(SingularityAgent._META_PROJECT_URI),
				ref.object.getMetadata(SingularityAgent._META_PROJECT_DISPLAY_NAME),
				ref.object.getMetadata(AH_META_WORKSPACELESS_DB_KEY),
			]);
			const workingDirectory = cwd ? URI.parse(cwd) : undefined;
	...
	private async _storeSessionAgentMetadata(session: URI, agent: AgentSelection | undefined): Promise<void> {
		...
		try {
			...
			await dbRef.object.setMetadata(SingularityAgent._META_AGENT, agent ? this._serializeAgentSelection(agent) : '');
		} finally {
			dbRef.dispose();
		}
	}
	...
	private _resolveSessionProject(context: ISingularitySessionContext | undefined, limiter: Limiter<IAgentSessionProjectInfo | undefined>, projectByContext: Map<string, Promise<IAgentSessionProjectInfo | undefined>>): Promise<IAgentSessionProjectInfo | undefined> {
		...
		if (!project) {
			project = limiter.queue(() => projectFromSingularityContext(context, this._gitService));
			projectByContext.set(key, project);
		}
		return project;
	}

	private _projectContextKey(context: ISingularitySessionContext | undefined): string | undefined {
		if (context?.cwd) {
	...
	override dispose(): void {
		...
		this.shutdown().catch(err => {
			this._logService.warn('[Singularity] Shutdown failed during dispose', err);
...
class SessionDiscoveredEntry extends Disposable {
	...
	constructor(
		...
		userHome: URI,
		private readonly _getClient: () => Promise<SingularityClient>,
	...
	private async _refresh(token: CancellationToken): Promise<boolean> {
		try {
			...
			this._logService.warn(`[Singularity:SessionDiscoveredEntry] Discovery/bundle failed: ${err instanceof Error ? err.message : String(err)}`);
...
export function mapToParsedPlugin(customizations: readonly DirectoryCustomization[]): IParsedPlugin | undefined {
	...
	return {
		format: PluginFormat.Singularity,
		hooks: [],
		mcpServers: [],
		skills: skills,
		agents: agents,
 ...
 * one per {@link SingularityAgentSession}. Each session controller holds
...
class PluginController extends Disposable {
	...
	public async getClient(): Promise<SingularityClient> {
	...
	public async resolveConfiguredCustomization(customization: PluginCustomization): Promise<IResolvedCustomization> {
		...
		if (!parsed) {
			return {
				customization: {
					...customization,
					load: { kind: CustomizationLoadStatus.Error, message: localize('singularityAgent.pluginParseError', "Error parsing plugin.") },
				},
			};
		}
	...
	public async tryParsePlugin(pluginDir: URI): Promise<IParsedPlugin | undefined> {
		try {
			...
			this._logService.warn(`[Singularity:PluginController] Error parsing plugin '${pluginDir.toString()}': ${error instanceof Error ? error.message : String(error)}`);
			return undefined;
		}
	}
}
...
class SessionPluginController extends Disposable {
	...
	public async getCustomizationsSettled(): Promise<readonly Customization[]> {
		const entry = this._discoveredEntry();
		await Promise.all([
			this._parent.hostSync().catch(err => this._logService.warn('[Singularity:SessionPluginController] Host customization update failed', err)),
			...[...this._clients.values()].map(client => client.sync.catch(err => this._logService.warn('[Singularity:SessionPluginController] Client customization sync failed', err))),
			entry?.whenSettled(),
		]);
	...
	public async getAppliedPlugins(): Promise<readonly ISingularityPluginInfo[]> {
		const entry = this._discoveredEntry();
		const [host] = await Promise.all([
			this._parent.hostSync().catch(err => {
				this._logService.warn('[Singularity:SessionPluginController] Host customization update failed', err);
				return this._parent.hostCustomizations();
			}),
			...[...this._clients.values()].map(client => client.sync.catch(err => {
				this._logService.warn('[Singularity:SessionPluginController] Client customization sync failed', err);
	...
	public sync(clientId: string, customizations: ClientPluginCustomization[], options?: { quiet?: boolean }) {
		...
		const promise = client.sync = prev.catch(err => {
			this._logService.warn('[Singularity:SessionPluginController] Previous customization sync failed', err);
		}).then(async () => {
	...
	public async retryFailedClientSyncIfNeeded(): Promise<void> {
		...
		for (const [clientId, client] of [...this._clients]) {
			...
			this._logService.info(`[Singularity:SessionPluginController] Retrying ${inputs.length} previously-failed client customization(s) for ${clientId}`);
			await this.sync(clientId, inputs).catch(err => {
				this._logService.warn('[Singularity:SessionPluginController] Retried client customization sync failed', err);
			});
		}
	}
 ...
 * {@link SingularityAgent.getOrCreateActiveClient}. Reads/writes flow straight
 ...
 * client's slice.
 */
class SingularityActiveClientHandle implements IActiveClient {
	constructor(
...
class ActiveClient extends Disposable {
	 ...
	 * with the session's {@link SingularityAgentSession} so a window reload (new
	...
	private readonly _handles = new Map<string, SingularityActiveClientHandle>();
	...
	getOrCreateHandle(clientId: string, displayName: string | undefined): SingularityActiveClientHandle {
		let handle = this._handles.get(clientId);
		if (!handle) {
			handle = new SingularityActiveClientHandle(this, clientId, displayName);
