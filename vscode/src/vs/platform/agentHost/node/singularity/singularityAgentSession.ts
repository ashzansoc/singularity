...
import type { CopilotSession as SingularitySession, CurrentToolMetadata, ElicitationContext, ElicitationFieldValue, ElicitationResult, ElicitationSchema, ElicitationSchemaField, ExitPlanModeCompletedData, ExitPlanModeRequest, ExitPlanModeResult, McpServersLoadedServer, MessageOptions, PermissionAllowAllMode, PermissionAutoApproval, PermissionRequest, PermissionRequestResult, PermissionResult, SessionConfig, SessionHooks, SessionMode as SingularitySdkMode, Tool, ToolResultObject, McpServerStatus as SdkMcpServerStatus } from '@github/copilot-sdk';
...
import { SingularityCliConfigKey, applyModelFamilyAlias, coercePinnedFlashModelId, singularityCliConfigSchema } from '../../common/singularityCliConfig.js';
...
import { resolveSingularityConfigSlashCommandOnSend } from '../../common/singularityConfigSlashCommands.js';
...
import { SingularitySessionWrapper } from './singularitySessionWrapper.js';
import { clientToolNamesFromSnapshot, type SingularitySessionLaunchPlan, type IActiveClientSnapshot, type ISingularitySessionLauncher, type ISingularitySessionRuntime } from './singularitySessionLauncher.js';
...
import { buildSingularitySystemNotification } from './singularitySystemNotification.js';
import { parseLeadingSlashCommand } from '../../common/agentHostSlashCommand.js';
import type { IUnsandboxedCommandConfirmationRequest, ShellManager } from './singularityShellTools.js';
import { NonPtyShellTerminalStreams } from './singularityNonPtyShellTerminals.js';
import { buildSandboxConfigForSdk, type SingularitySandboxConfig } from './sandboxConfigForSdk.js';
import type { IAgentServerToolHost } from '../../common/agentServerTools.js';
import { getEditFilePaths, getInvocationMessage, getPastTenseMessage, getPermissionDisplay, getShellIntention, getShellLanguage, getStreamingInvocationMessage, getSubagentMetadata, getTaskCompleteMarkdown, getToolDisplayName, getToolInputString, getToolKind, isAgentCoordinationTool, isEditTool, isHiddenTool, isShellTool, isTaskCompleteTool, parseSingularityStreamingToolInput, synthesizeSkillToolCall, tryStringify } from './singularityToolDisplay.js';
...
import { addSimpleAttachmentDisplayKindToMimeType } from './singularityAttachmentUtils.js';
...
import { createSingularityFailureCorrelation, reportSingularityModelCallFailure, reportSingularitySdkSessionError } from './singularityFailureTelemetry.js';
import { reportSingularityTodoStoreOperation } from './singularityTodoStoreTelemetry.js';

type SingularitySdkAttachment = Required<MessageOptions>['attachments'][number];
type SingularityCommandInvocationResult = Awaited<ReturnType<SingularitySession['rpc']['commands']['invoke']>>;
type RuntimeSlashCommandInfo = Awaited<ReturnType<SingularitySession['rpc']['commands']['list']>>['commands'][number];
...
interface SingularityExitPlanModeResponse extends ExitPlanModeResult {
...
interface ISingularityActiveToolCall {
...
interface ISingularityStreamingToolCall {
	input: string;
...
function toSingularitySdkMode(mode: string | undefined): SingularitySdkMode | undefined {
...
function getSingularityCLISessionStateDir(userHome: string): string {
	return join(getSingularityHomePath(userHome, process.env), SESSION_STATE_DIRECTORY);
}

/**
 * Matches the temp file names the Singularity SDK uses when spilling large tool
 ...
 *  - `<timestamp>-singularity-tool-output-<6-char-id>.txt` (large tool result)
 *  - `singularity-tool-output-<timestamp>-<6-char-id>.txt` (streaming output buffer)
 *
 ...
 */
const SINGULARITY_SDK_TOOL_OUTPUT_BASENAME_RE = /^(?:\d{10,}-singularity-tool-output-[a-z0-9]{6}|singularity-tool-output-\d{10,}-[a-z0-9]{6})\.txt$/i;

function isSingularitySdkToolOutputTempFile(filePath: string, tmpDir: string): boolean {
	...
	return SINGULARITY_SDK_TOOL_OUTPUT_BASENAME_RE.test(basename);
}

/**
 * Options for constructing a {@link SingularityAgentSession}.
 */
export interface ISingularityAgentSessionOptions {
	readonly sessionUri: URI;
	readonly chatChannelUri: URI;
	readonly rawSessionId: string;
	readonly onDidSessionProgress: Emitter<AgentSignal>;
	readonly sessionLauncher: ISingularitySessionLauncher;
	readonly launchPlan: SingularitySessionLaunchPlan;
	 ...
	 * bare top-level customizations via {@link SingularityAgentSession.topLevelMcpCustomizations}.
	 */
...
/**
 * Lifecycle state of a {@link SingularityTurn}.
 *
 ...
 */
type SingularityTurnState = 'pending' | 'running' | 'completed' | 'aborted';

/**
 * Encapsulates all per-turn bookkeeping for a single protocol turn, plus an
 * explicit lifecycle {@link SingularityTurn.state}. Holding this state on one
...
class SingularityTurn {

	private _state: SingularityTurnState = 'pending';
	private readonly _stopWatch = StopWatch.create(false);

	/**
	 * This turn's own Singularity cost in nano-AIU, summed from the `singularityUsage`
	...
	get state(): SingularityTurnState { return this._state; }
...
/**
 * Encapsulates a single Singularity SDK session and all its associated bookkeeping.
 *
 * Created by {@link SingularityAgent}, one instance per active session. Disposing
 * this class tears down all per-session resources (SDK wrapper, edit tracker,
 * database reference, pending permissions).
 */
export class SingularityAgentSession extends Disposable {
	...
	private readonly _activeToolCalls = new Map<string, ISingularityActiveToolCall>();
	private readonly _streamingToolCalls = new Map<string, ISingularityStreamingToolCall>();
	 ...
	 * permission requests, keyed by tool call id. The Singularity CLI runtime emits
	 ...
	 * RPC with a structured {@link SingularityExitPlanModeResponse} (which the CLI
	...
	private readonly _pendingPlanReviews = new PendingRequestRegistry<
		SingularityExitPlanModeResponse,
		{
	 ...
	 * {@link SingularityTurn.state} lifecycle. Created (`pending`) by
	 * {@link resetTurnState}, finalized by {@link _completeActiveTurn}.
	 */
	private _currentTurn: SingularityTurn | undefined;
	 ...
	 * per-event `singularityUsage` instead (see {@link SingularityTurn.singularityNanoAiu}).
	 */
	...
	private _wrapper!: SingularitySessionWrapper;
	...
	private _lastAppliedMode: SingularitySdkMode | undefined;
	...
	private readonly _sessionLauncher: ISingularitySessionLauncher;
	private readonly _launchPlan: SingularitySessionLaunchPlan;
	private readonly _isLaunchTokenStillCurrent: () => boolean;
	/** Notifies the agent that this chat's turn ended. See {@link ISingularityAgentSessionOptions.onTurnEnded}. */
	...
	constructor(
		options: ISingularityAgentSessionOptions,
		...
		this._slashCommandProvider = new SingularitySlashCommandProvider(() => this._wrapper.session.rpc.commands.list({ includeBuiltins: true, includeSkills: true, includeClientCommands: true }).then(c => c.commands), this._logService);
		...
		const effectiveModel = applyModelFamilyAlias(model, this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.ModelCapabilityOverrides));
		this._toolSearchActive = this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.ToolSearchEnabled) === true
	...
	private _shouldDropUnmappedSubagentEvent(e: { readonly agentId?: string }, eventName: string): boolean {
		...
		if (!parentToolCallId && e.agentId) {
			this._logService.warn(`[Singularity:${this.sessionId}] Dropping ${eventName} for unknown subagent agentId=${e.agentId}`);
			return true;
		}
		return false;
	}
	...
	private _getStreamingToolCallDisplay(toolName: string, input: string) {
		const partialInput = parseSingularityStreamingToolInput(input);
	...
	private _emitStreamingToolCallDisplay(toolCallId: string, streaming: ISingularityStreamingToolCall): void {
	...
	resetTurnState(turnId: string, senderClientId?: string, clientType = AgentHostClientType.Unknown): void {
		...
		this._currentTurn = new SingularityTurn(turnId, this._nextTurnOrdinal++, senderClientId, clientType);
	}
	...
	private async _refreshSessionUsageMetrics(): Promise<boolean> {
		try {
			...
			this._logService.trace(`[Singularity:${this.sessionId}] usage.getMetrics RPC failed: ${getErrorMessage(err)}`);
			return false;
		}
	}

	/**
	 * The parent-scope Singularity billing metadata for the active turn: the turn's
	 * own accumulated cost plus the SDK's session-wide total. Absent until
	 * something has actually been billed.
	 */
	private _parentSingularityUsageMeta(): UsageInfoMeta['singularityUsage'] | undefined {
		const turnNanoAiu = this._currentTurn?.singularityNanoAiu ?? 0;
	...
	private async _readContextAttribution(): Promise<IContextAttributionData | undefined> {
		...
		try {
			...
			this._logService.trace(`[Singularity:${this.sessionId}] contextAttribution RPC failed: ${getErrorMessage(err)}`);
			return undefined;
		}
		if (!attribution) {
			this._logService.trace(`[Singularity:${this.sessionId}] contextAttribution: null/empty`);
			return undefined;
		}
		if (this._logService.getLevel() <= LogLevel.Trace) {
			this._logService.trace(`[Singularity:${this.sessionId}] contextAttribution: totalTokens=${attribution.totalTokens}, entries=${JSON.stringify(attribution.entries.map(e => ({ kind: e.kind, id: e.id, label: e.label, tokens: e.tokens, parentId: e.parentId })))}`);
		}
		return attribution;
	}
	...
	private _clearActiveTurn(): void {
		...
		try {
			...
			this._logService.error(err, `[Singularity:${this.sessionId}] onTurnEnded callback failed`);
		}
	}

	private _reportToolCallDetails(turn: SingularityTurn, responseType: 'success' | 'cancelled' | 'failed'): void {
		...
		}).catch(err => this._logService.trace(`[Singularity:${this.sessionId}] Telemetry emission failed: ${getErrorMessage(err)}`));
	}
	...
	private _emitMarkdownDelta(content: string, parentToolCallId?: string): void {
		...
		if (!turn) {
			...
			this._logService.error(`[Singularity:${this.sessionId}] Markdown delta emitted with no active turn; dropping`);
			return;
		}
	...
	private _emitReasoningDelta(content: string, parentToolCallId?: string): void {
		const turn = this._currentTurn;
		if (!turn) {
			this._logService.error(`[Singularity:${this.sessionId}] Reasoning delta emitted with no active turn; dropping`);
			return;
		}
				...
				return {
					...
					handler: this._guarded(async (_args: Record<string, unknown>, invocation) => {
						try {
							...
							this._logService.error(error, `[Singularity:${this.sessionId}] Failed in tool-search handler: toolCallId=${invocation.toolCallId}`);
			...
			return {
				...
				handler: this._guarded(async (_args: Record<string, unknown>, { toolCallId }) => {
					try {
						...
						this._logService.error(error, `[Singularity:${this.sessionId}] Failed in client tool handler: tool=${def.name}, toolCallId=${toolCallId}`);
						throw error;
					}
	...
	private _guarded<A extends unknown[], R>(handler: (...args: A) => Promise<R>, cancelled: R, label: string): (...args: A) => Promise<R> {
		return async (...args) => {
			...
			if (token.isCancellationRequested) {
				this._logService.info(`[Singularity:${this.sessionId}] Discarding ${label} callback received while aborting`);
				return cancelled;
			}
			const result = await raceCancellation(handler(...args), token, cancelled);
			if (token.isCancellationRequested) {
				this._logService.info(`[Singularity:${this.sessionId}] Discarding ${label} callback result after abort`);
				return cancelled;
			}
			return result;
		};
	...
	private _toToolSearchResult(clientResult: ToolResultObject, availableTools: readonly CurrentToolMetadata[] | undefined): ToolResultObject {
		...
		this._logService.info(`[Singularity:${this.sessionId}] tool_search override: availableTools=${availableTools?.length ?? 0}, deferred=${deferred.size}, clientMatched=[${clientNames.join(', ')}] -> toolReferences=[${toolReferences.join(', ')}]`);
		...
		return host.definitions.map(def => ({
			...
			handler: async (args: Record<string, unknown>): Promise<ToolResultObject> => {
				try {
					...
					this._logService.error(error, `[Singularity:${this.sessionId}] Failed in server tool handler: tool=${def.name}`);
	...
	private _createRuntimeAdapter(): ISingularitySessionRuntime {
		return {
			...
			handleExitPlanModeRequest: this._guarded((request, invocation) => this._handleExitPlanModeRequest(request, invocation), { approved: false } satisfies SingularityExitPlanModeResponse, 'exit-plan-mode'),
	...
	private async _handleMcpAuthRequest(request: McpAuthRequest): Promise<McpAuthResult | null | undefined> {
		const githubToken = request.reason === 'initial' && this._scopesFromChallenge(request.wwwAuthenticateParams?.scope).length === 0
			...
			: undefined;
		if (githubToken) {
			this._logService.info(`[Singularity:${this.sessionId}] Reusing the existing GitHub token for initial GitHub MCP authentication`);
		...
		this._logService.info(`[Singularity:${this.sessionId}] MCP server '${request.serverName}' requires authentication for ${resource.resource}`);
		return result;
	}
	...
	private async _initialGitHubMcpToken(request: McpAuthRequest): Promise<string | undefined> {
		...
		try {
			const resolvedUrl = gitHubMcpServerUrl(await this._singularityApiService.resolveApiEndpoint(githubToken));
			if (resolvedUrl) {
				configuredUrls.push(resolvedUrl);
			}
		} catch (error) {
			this._logService.warn(`[Singularity:${this.sessionId}] Failed to resolve the GitHub MCP server URL: ${getErrorMessage(error)}`);
			return undefined;
		}
	...
	private _protectedResourceFromMcpAuthRequest(request: McpAuthRequest): ProtectedResourceMetadata {
		if (request.resourceMetadata) {
			try {
				...
				this._logService.warn(`[Singularity:${this.sessionId}] Ignoring invalid MCP protected-resource metadata for '${request.serverName}'`);
			} catch (err) {
				this._logService.warn(`[Singularity:${this.sessionId}] Failed to parse MCP protected-resource metadata for '${request.serverName}'`, err);
			}
		}
	...
	async send(prompt: string, attachments?: readonly MessageAttachment[], turnId?: string, mode?: SingularitySdkMode, senderClientId?: string, clientType = AgentHostClientType.Unknown): Promise<void> {
	...
	private async _send(prompt: string, attachments: readonly MessageAttachment[] | undefined, mode: SingularitySdkMode | undefined): Promise<void> {
		this._logService.info(`[Singularity:${this.sessionId}] sendMessage called: "${prompt.substring(0, 100)}${prompt.length > 100 ? '...' : ''}" (${attachments?.length ?? 0} attachments)`);
		...
		if (slashCommand?.command === 'compact') {
			try {
				...
				if (typeof usedTokens === 'number') {
					...
					await this._refreshSessionUsageMetrics();
					const singularityUsage = this._parentSingularityUsageMeta();
					...
					const meta: UsageInfoMeta = {
						...(singularityUsage ? { singularityUsage } : {}),
				...
				this.emitInitialMarkdown(localize('singularityAgent.compactionCompleted', "Compaction completed"));
			} catch (err) {
				...
				this._logService.error(err, `[Singularity:${this.sessionId}] rpc.history.compact failed`);
				throw err;
			}
		...
		const configAction = slashCommand ? resolveSingularityConfigSlashCommandOnSend(slashCommand.command, slashCommand.rawRest) : undefined;
		if (configAction) {
			...
			const sdkMode = toSingularitySdkMode(configAction.applyConfig[SessionConfigKey.Mode]);
			if (sdkMode) {
				mode = sdkMode;
			}
			prompt = configAction.strippedPrompt;
		} else if (slashCommand?.command === 'rubber-duck') {
			if (this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.RubberDuck) !== true) {
			...
			if (runtimeSlashCommand && runtimeSlashCommand.kind !== 'skill') {
				let result: SingularityCommandInvocationResult;
				try {
					...
					this._logService.error(err, `[Singularity:${this.sessionId}] rpc.commands.invoke(${slashCommand.command}) failed`);
					throw err;
				}
				switch (result.kind) {
					case 'text':
					...
					case 'completed':
						...
						break;
					case 'agent-prompt': {
						const runtimeMode = toSingularitySdkMode(result.mode);
						if (runtimeMode) {
							mode = runtimeMode;
						}
					...
					case 'select-subcommand':
						this._emitMarkdownDelta(localize(
							'singularitySlashCommand.selectSubcommandResult',
					...
					default:
						...
						this._logService.warn(`[Singularity:${this.sessionId}] Unhandled slash command result kind: ${(result as { kind: string }).kind}`);
						break;
				}
		...
		this._logService.info(`[Singularity:${this.sessionId}] session.send() returned`);
	}

	private async _toSdkAttachments(attachments: readonly MessageAttachment[] | undefined): Promise<SingularitySdkAttachment[] | undefined> {
		...
		if (sdkAttachments?.length) {
			this._logService.trace(`[Singularity:${this.sessionId}] Attachments: ${JSON.stringify(sdkAttachments.map(attachment => ({ type: attachment.type })))}`);
		}
	...
	async hasRuntimeSlashCommand(command: string): Promise<boolean> {
		try {
			...
			this._logService.warn(`[Singularity:${this.sessionId}] rpc.commands.list failed`, err);
			return false;
		}
	}
	...
	/**
	 * Translate a protocol {@link MessageAttachment} into the Singularity CLI SDK's `attachments` payload shape. Resource
	 ...
	 */
	private async _toSdkAttachment(attachment: MessageAttachment): Promise<SingularitySdkAttachment | undefined> {
		...
		if (attachment.selection) {
			try {
				...
				this._logService.warn(`[Singularity:${this.sessionId}] Failed to read selected text for ${uri.toString()}: ${err}`);
	 ...
	 */
	async applyMode(mode: SingularitySdkMode | undefined): Promise<void> {
		...
		try {
			...
			this._logService.info(`[Singularity:${this.sessionId}] rpc.mode.set succeeded: mode=${mode}`);
		} catch (err) {
			this._logService.error(err, `[Singularity:${this.sessionId}] rpc.mode.set failed: mode=${mode}`);
		}
	}
	...
	async sendSteering(steeringMessage: PendingMessage): Promise<void> {
		...
		this._logService.info(`[Singularity:${this.sessionId}] Sending steering message: "${steeringMessage.message.text.substring(0, 100)}"`);
		try {
			await this._reconcileMcpServerEnablement();
			...
			this._logService.error(`[Singularity:${this.sessionId}] Steering message failed`, err);
		} finally {
	...
	async abort(): Promise<void> {
		this._logService.info(`[Singularity:${this.sessionId}] Aborting session...`);
		this._beginAbort();
	...
	override dispose(): void {
		void this._editTracker.flushAttribution().catch(error => {
			this._logService.warn(`[Singularity:${this.sessionId}] Failed to flush edit attribution: ${error}`);
		});
		this._beginAbort();
		super.dispose();
	}
	...
	async setModel(model: string, reasoningEffort?: SessionConfig['reasoningEffort'], contextTier?: SessionConfig['contextTier']): Promise<void> {
		const pinned = coercePinnedFlashModelId(model) ?? model;
		this._logService.info(`[Singularity:${this.sessionId}] Changing model to: ${pinned}${pinned !== model ? ` (pinned from ${model})` : ''}`);
		this._lastSeenModelId = pinned;
		await this._wrapper.session.setModel(pinned, { reasoningEffort, contextTier });
	}

	/**
	 * Dispatches an MCP JSON-RPC method received on the `mcp://` side
	 * channel to the Singularity SDK's `session.rpc.mcp.*` surface.
	 *
	 * Mapping:
	...
	async startMcpServer(id: string): Promise<void> {
		...
		if (!serverName) {
			this._logService.warn(`[Singularity:${this.sessionId}] Cannot start unknown MCP server customization ${id}`);
			return;
		}
		try {
	...
	private async _reconcileMcpServerEnablement(): Promise<void> {
		...
		for (const server of this._mcpCustomizations.serverEnablement()) {
			...
			try {
				...
				this._logService.error(e, `[Singularity:${this.sessionId}] Failed to ${desired ? 'enable' : 'disable'} MCP server ${server.serverName}`);
			}
		}
		if (changed) {
			await this._refreshMcpServersFromRpc();
	...
	async stopMcpServer(id: string): Promise<void> {
		const serverName = this._mcpCustomizations.serverNameForCustomizationId(id);
		if (!serverName) {
			this._logService.warn(`[Singularity:${this.sessionId}] Cannot stop unknown MCP server customization ${id}`);
			return;
		}
	 ...
	 * Singularity runtime owns the MCP→chat-completion conversion and the
	...
	async setAgent(agentName?: string): Promise<void> {
		if (agentName) {
			...
			this._logService.info(`[Singularity:${this.sessionId}] Selecting custom agent: ${name}`);
			try {
				await this._wrapper.session.rpc.agent.select({ name });
			} catch (err) {
				this._logService.error(err, `[Singularity:${this.sessionId}] rpc.agent.select failed: name=${name}`);
				throw err;
			}
		} else {
			this._logService.info(`[Singularity:${this.sessionId}] Clearing custom agent selection`);
			try {
				await this._wrapper.session.rpc.agent.deselect();
			} catch (err) {
				this._logService.error(err, `[Singularity:${this.sessionId}] rpc.agent.deselect failed`);
				throw err;
			}
		}
	}
	...
	private async _handlePermissionRequest(
		...
		try {
			...
			if (!toolCallId) {
				...
				this._logService.warn(`[Singularity:${this.sessionId}] Permission request without toolCallId, auto-denying: kind=${request.kind}`);
				return { kind: 'reject' };
			}
			if (this._unroutableSubagentToolCallIds.delete(toolCallId)) {
				this._logService.error(`[Singularity:${this.sessionId}] Rejecting permission request for unroutable subagent tool call: toolCallId=${toolCallId}, kind=${request.kind}`);
				return { kind: 'reject' };
			}
			...
			if (approvedSignature !== undefined) {
				...
				if (!managedApprovalRequired && (request.kind === 'write' || request.kind === 'read') && safeStringify(request) === approvedSignature) {
					this._logService.info(`[Singularity:${this.sessionId}] Auto-approving duplicate ${request.kind} permission request for tool call ${toolCallId}`);
					return { kind: 'approve-once' };
				}
			}
			...
			if (!managedApprovalRequired && sessionResourcePath) {
				this._logService.info(`[Singularity:${this.sessionId}] Auto-approving internal session resource ${sessionResourcePath}`);
				return { kind: 'approve-once' };
			}
			...
			if (!managedApprovalRequired && request.kind === 'read' && typeof request.path === 'string'
				...
				this._logService.info(`[Singularity:${this.sessionId}] Auto-approving session attachment ${request.path}`);
				return { kind: 'approve-once' };
			}

			// Auto-approve reads of large-tool-output temp files written by the
			// Singularity SDK itself. The SDK spills oversized tool results to
			// `os.tmpdir()/singularity-tool-output-…txt` and then asks the model
			// to read them back in a follow-up turn — no need to confirm.
			if (!managedApprovalRequired && request.kind === 'read' && typeof request.path === 'string') {
				if (isSingularitySdkToolOutputTempFile(request.path, this._environmentService.tmpDir.fsPath)) {
					this._logService.info(`[Singularity:${this.sessionId}] Auto-approving Singularity SDK tool-output temp file ${request.path}`);
					return { kind: 'approve-once' };
				}
			}
			...
			if (!managedApprovalRequired && request.kind === 'custom-tool' && typeof request.toolName === 'string'
				...
				this._logService.info(`[Singularity:${this.sessionId}] Auto-approving server tool ${request.toolName}`);
				return { kind: 'approve-once' };
			}
			...
			if (isShellRequest && shellLanguage === undefined) {
				this._logService.warn(`[Singularity:${this.sessionId}] Shell permission request has no recognized shell tool name; requiring confirmation: toolCallId=${toolCallId}, toolName=${shellToolName ?? '(missing)'}`);
			}
			...
			if (!managedApprovalRequired && request.kind === 'custom-tool'
				...
				this._logService.info(`[Singularity:${this.sessionId}] Auto-approving client tool ${request.toolName} because its result arrived before the permission request`);
				return { kind: 'approve-once' };
			}

			this._logService.info(`[Singularity:${this.sessionId}] Requesting confirmation for tool call: ${toolCallId}`);
			...
			if (!managedApprovalRequired && isShellRequest && !requestSandboxBypass && await this._isShellSandboxedByDefault()) {
				...
				if (this._pendingPermissions.has(toolCallId)) {
					...
					this._logService.info(`[Singularity:${this.sessionId}] Auto-approving sandboxed shell command for tool call ${toolCallId}`);
					return { kind: 'approve-once' };
				}
				return { kind: 'reject' };
			}
			...
			this._logService.info(`[Singularity:${this.sessionId}] Permission response: toolCallId=${toolCallId}, result=${result.kind}`);
			...
			this._logService.error(error, `[Singularity:${this.sessionId}] Failed to handle permission request: kind=${request.kind}, toolCallId=${request.toolCallId ?? 'missing'}`);
			throw error;
		}
	}
	...
	private _getInternalSessionResourcePath(request: PermissionRequest): string | undefined {
		...
		const sessionStateDir = normalizePath(URI.file(getSingularityCLISessionStateDir(this._environmentService.userHome.fsPath)));
	...
	private async _isShellSandboxedByDefault(): Promise<boolean> {
		...
		// `SingularitySessionLauncher` reads when forwarding `sandboxConfig` to
	...
	private _isCustomTerminalToolEnabled(): boolean {
		return this._configurationService.getRootValue(singularityCliConfigSchema, SingularityCliConfigKey.EnableCustomTerminalTool) === true;
	}

	/**
	 * The SDK-shaped sandbox policy for this session, mirroring
	 * {@link SingularitySessionLauncher}'s computation: `undefined` when the custom
	 ...
	 */
	private _computeSdkSandboxConfig(): SingularitySandboxConfig | undefined {
		if (this._isCustomTerminalToolEnabled()) {
			return undefined;
		}
	...
	private async _syncPermissionModeAfterConfigChange(): Promise<void> {
		try {
			...
			this._logService.error(error, `[Singularity:${this.sessionId}] Failed to apply permission config change; aborting active turn`);
			try {
				await this.abort();
			} catch (abortError) {
				this._logService.error(abortError, `[Singularity:${this.sessionId}] Failed to abort after permission config sync failure`);
			}
		}
	}
		...
		return this._permissionModeSequencer.queue(async () => {
			...
			if (this._autoApprovalExperimentalModeEnabled !== experimentalModeEnabled) {
				...
				if (!experimentalResult.success) {
					throw new Error(`Singularity SDK rejected experimental mode update required by permission mode '${mode}'`);
				}
				this._autoApprovalExperimentalModeEnabled = experimentalModeEnabled;
				this._logService.info(`[Singularity:${this.sessionId}] ${experimentalModeEnabled ? 'Enabled' : 'Disabled'} SDK experimental mode for permission mode '${mode}'`);
			}
			...
			if (!result.success || (result.mode !== undefined && result.mode !== mode)) {
				throw new Error(`Singularity SDK rejected permission mode '${mode}'`);
			}
	...
	private async _applyEffectiveSandboxConfig(failOnError = false): Promise<void> {
		...
		const sandboxConfig: SingularitySandboxConfig | { enabled: false } = (base && !this._isBypassApprovals()) ? base : { enabled: false };
		try {
			const result = await this._wrapper.session.rpc.options.update({ sandboxConfig });
			if (!result.success) {
				throw new Error('Singularity SDK rejected sandbox config update');
			}
		} catch (err) {
			if (failOnError) {
				throw err;
			}
			this._logService.warn(`[Singularity:${this.sessionId}] Failed to update sandbox config for request`, err);
		}
	}
	...
	private async _buildEditsForPermission(request: PermissionRequest, toolCallId: string): Promise<{ items: FileEdit[] } | undefined> {
		...
		try {
			...
			this._logService.warn(`[Singularity:${this.sessionId}] Failed to check file for edit preview: ${filePath}`, err);
		}
		...
		try {
			...
			this._logService.warn(`[Singularity:${this.sessionId}] Failed to write pending edit content for ${filePath}`, err);
			return undefined;
		}
		...
		if (!this._pendingPermissions.has(toolCallId)) {
			this._fileService.del(afterUri).catch(err => {
				this._logService.warn(`[Singularity:${this.sessionId}] Failed to delete orphaned pending edit content: ${afterUri.toString()}`, err);
			});
			return undefined;
		}
	...
	private async _handleUserInputRequest(
		...
		if (!this.hasActiveTurn) {
			this._logService.warn(`[Singularity:${this.sessionId}] Rejecting user input request without an active turn`);
			return { answer: 'No active turn', wasFreeform: true };
		}

		const questionPreview = request.question.substring(0, 100);
		try {
			this._logService.info(`[Singularity:${this.sessionId}] User input request: requestId=${requestId}, question="${questionPreview}"`);
			...
			this._logService.info(`[Singularity:${this.sessionId}] User input response: requestId=${requestId}, response=${result.response}`);
			...
			this._logService.error(error, `[Singularity:${this.sessionId}] Failed to handle user input request: question="${questionPreview}"`);
			throw error;
		}
	}
	...
	private async _handleElicitationRequest(context: ElicitationContext): Promise<ElicitationResult> {
		...
		if (!this.hasActiveTurn) {
			this._logService.warn(`[Singularity:${this.sessionId}] Rejecting elicitation request without an active turn`);
		...
		try {
			const requestId = generateUuid();
			this._logService.info(`[Singularity:${this.sessionId}] Elicitation request: requestId=${requestId}, mode=${context.mode ?? 'form'}, source=${context.elicitationSource ?? '<unknown>'}, message="${messagePreview}"`);
			...
			this._logService.info(`[Singularity:${this.sessionId}] Elicitation response: requestId=${requestId}, response=${result.response}`);
			...
			this._logService.error(error, `[Singularity:${this.sessionId}] Failed to handle elicitation request: message="${messagePreview}"`);
			throw error;
		}
	}
	 ...
	 * {@link SingularityExitPlanModeResponse} that the CLI can feed into
	...
	private async _handlePreToolUse(input: PreToolUseHookInput): Promise<void> {
		try {
			...
			this._logService.error(error, `[Singularity:${this.sessionId}] Failed in onPreToolUse: tool=${input.toolName}`);
			throw error;
		}
	}
	...
	private async _handlePostToolUse(input: PostToolUseHookInput): Promise<void> {
		try {
			...
			this._logService.error(error, `[Singularity:${this.sessionId}] Failed in onPostToolUse: tool=${input.toolName}`);
			throw error;
		}
	}
	...
	private async _beginRepoInfoTelemetry(telemetryMessageId: string, clientType: AgentHostClientType, isCurrent: () => boolean): Promise<{ readonly context: IAgentHostRestrictedTelemetryContext; readonly baseBranch: string | undefined } | undefined> {
		...
		try {
			...
			this._logService.warn(`[Singularity:${this.sessionId}] Failed to resolve repository info telemetry context: ${getErrorMessage(error)}`);
			return undefined;
		}
	...
	private async _resolveRepoInfoTelemetryContext(): Promise<{ readonly context: IAgentHostRestrictedTelemetryContext; readonly baseBranch: string | undefined } | undefined> {
		...
		const [rawContext, baseBranch] = await Promise.all([
			this._singularityApiService.resolveRestrictedTelemetryContext(githubToken),
	...
	private _toRepoInfoTelemetryContext(context: IRestrictedTelemetryContext): IAgentHostRestrictedTelemetryContext {
		return {
			...
			singularityIgnoreEnabled: context.singularityIgnoreEnabled,
		};
	}
	...
	private _subscribeToEvents(): void {
		...
		this._register(wrapper.onSystemNotification(e => {
			const notification = buildSingularitySystemNotification(e);
			if (!notification) {
				this._logService.trace(`[Singularity:${sessionId}] Ignoring system.notification kind=${e.data.kind.type}`);
				return;
			}

			this._logService.info(`[Singularity:${sessionId}] System notification received: kind=${e.data.kind.type}`);
			if (this._turnId) {
				this._emitAction({
					type: ActionType.ChatResponsePart,
					turnId: this._turnId,
			...
			if (!notification.startsTurn) {
				this._logService.trace(`[Singularity:${sessionId}] Ignoring passive system.notification kind=${e.data.kind.type} without an active turn`);
				return;
			}
		...
		this._register(wrapper.onMessageDelta(e => {
			this._logService.trace(`[Singularity:${sessionId}] delta: ${e.data.deltaContent}`);
		...
		this._register(wrapper.onMessage(e => {
			this._logService.info(`[Singularity:${sessionId}] Full message received: ${e.data.content.length} chars`);
			this._resumeSubagentForEvent(e);
			// Report the enhanced GH `request.options.tools` event for this model call — parity with
			// the Singularity extension, which emits it per LLM request. `assistant.message` is the
			...
			if (!e.agentId) {
				...
				void this._telemetryReporter.assistantMessageReceived(this.sessionUri.toString(), clientType, e.data.clientRequestId, this._appliedSnapshot.tools).catch(err => this._logService.trace(`[Singularity:${this.sessionId}] Telemetry emission failed: ${getErrorMessage(err)}`));
				// Restricted `conversation.messageText` (source=model): the model's raw response text.
				void this._telemetryReporter.modelMessageText(this.sessionUri.toString(), clientType, e.data.content, this._turnOrdinal, e.data.clientRequestId).catch(err => this._logService.trace(`[Singularity:${this.sessionId}] Telemetry emission failed: ${getErrorMessage(err)}`));
		...
		this._register(wrapper.onToolCallDelta(e => {
			this._logService.trace(`[Singularity:${sessionId}] Tool call delta: ${e.data.toolName ?? '<pending>'} (${e.data.toolCallId})`);
			this._resumeSubagentForEvent(e);
			...
			if (e.data.toolName) {
				if (streaming.toolName && streaming.toolName !== e.data.toolName) {
					this._logService.warn(`[Singularity:${sessionId}] Tool call ${e.data.toolCallId} changed name while streaming from ${streaming.toolName} to ${e.data.toolName}`);
				} else {
		...
		this._register(wrapper.onToolStart(e => {
			if (isHiddenTool(e.data.toolName)) {
				...
				this._logService.trace(`[Singularity:${sessionId}] Tool started (hidden): ${e.data.toolName}`);
				return;
			}
			this._logService.info(`[Singularity:${sessionId}] Tool started: ${e.data.toolName}`);
			...
			if (streamed?.toolName && streamed.toolName !== e.data.toolName) {
				this._logService.warn(`[Singularity:${sessionId}] Tool call ${e.data.toolCallId} started as ${e.data.toolName} after streaming as ${streamed.toolName}`);
			}
			this._resumeSubagentForEvent(e);
			...
			// eslint-disable-next-line local/code-no-untyped-meta-access -- Singularity SDK's own typed `_meta`, not the AHP protocol bag.
			...
			if (isClientTool && !contributor) {
				this._logService.warn(`[Singularity:${sessionId}] Client tool '${e.data.toolName}' started with no connected client; failing it immediately.`);
		...
		this._register(wrapper.onToolComplete(async e => {
			...
			if (!parentToolCallId && e.agentId) {
				this._logService.warn(`[Singularity:${this.sessionId}] Dropping tool.execution_complete for unknown subagent agentId=${e.agentId}`);
				return;
			}
			if (e.data.success && tracked.contributor === undefined) {
				const telemetrySession = parentToolCallId
					...
					: this.sessionUri;
				reportSingularityTodoStoreOperation(this._telemetryService, telemetrySession, e.data.toolCallId, tracked.toolName, tracked.parameters);
			}
			this._logService.info(`[Singularity:${sessionId}] Tool completed: ${e.data.toolCallId}`);
			this._reportToolApprovalIfNoPermission(e.data.toolCallId);
			this._activeToolCalls.delete(e.data.toolCallId);
			...
			for (const filePath of filePaths) {
				try {
					...
					this._logService.warn(`[Singularity:${sessionId}] Failed to take completed edit`, err);
				}
			}
		...
		this._register(wrapper.onIdle(e => {
			this._logService.info(`[Singularity:${sessionId}] Session idle`);
			if (e.data.aborted) {
				this._resetAbortToken();
			}
			...
			if (e.data.aborted) {
				...
				if (turn.isRunning) {
					this._logService.trace(`[Singularity:${sessionId}] Idle from abort; tearing down running turn ${turn.id}`);
					this._reportToolCallDetails(turn, 'cancelled');
					turn.markAborted();
					this._clearActiveTurn();
				} else {
					this._logService.trace(`[Singularity:${sessionId}] Idle from abort; leaving ${turn.state} turn ${turn.id} open`);
				}
				return;
			}
		...
		this._register(wrapper.onSkillInvoked(e => {
			this._logService.info(`[Singularity:${sessionId}] Skill invoked: ${e.data.name} (${e.data.path})`);
			this._resumeSubagentForEvent(e);
		...
		this._register(wrapper.onSubagentStarted(e => {
			...
			this._logService.info(`[Singularity:${sessionId}] Subagent started: toolCallId=${e.data.toolCallId}, agent=${e.data.agentName}`);
			const tracked = this._activeToolCalls.get(e.data.toolCallId);
			this._onDidSessionProgress.fire({
		...
		this._register(wrapper.onSessionError(e => {
			this._logService.error(`[Singularity:${sessionId}] Session error: ${e.data.errorType} - ${e.data.message}`);
			reportSingularitySdkSessionError(this._telemetryService, e, createSingularityFailureCorrelation(this.sessionUri, this._chatChannelUri, this._turnId, this.sessionId));
			if (this._currentTurn) {
				this._reportToolCallDetails(this._currentTurn, 'failed');
			}
			// Prefer the structured SDK fields (the Singularity CLI classifies its own
		...
		this._register(wrapper.onModelCallFailure(e => {
			reportSingularityModelCallFailure(this._telemetryService, e, createSingularityFailureCorrelation(this.sessionUri, this._chatChannelUri, this._turnId, this.sessionId));
		}));
		...
		this._register(wrapper.onAutoModeResolved(e => {
			...
			this._logService.info(`[Singularity:${sessionId}] Auto mode resolved to ${e.data.chosenModel}${e.data.reasoningBucket ? ` (${e.data.reasoningBucket})` : ''}`);
			if (!turnId) {
				return;
			}
			if (!e.agentId) {
		...
		this._register(wrapper.onUsage(e => {
			...
			// `singularityUsage` is marked `asInternal` in the SDK schema so it is not exposed on the generated
			// `AssistantUsageData` type, but it is present at runtime. Read it dynamically.
			const singularityUsage = readSingularityUsage(e.data);
			...
			// the credit total for the given scope. `singularityUsage` is the scope's
			// Singularity billing metadata, or `undefined` when nothing is billed yet.
			const buildUsage = (context: UsageContext, scopedSingularityUsage: UsageInfoMeta['singularityUsage'], isParentScope: boolean): UsageInfo => {
				...
				if (scopedSingularityUsage) {
					metadata.singularityUsage = scopedSingularityUsage;
				}
				if (quotaSnapshots) {
			...
			if (turn && singularityUsage) {
				turn.singularityNanoAiu += singularityUsage.totalNanoAiu;
				if (parentToolCallId) {
					const scopedTotal = (turn.subagentNanoAiuByToolCallId.get(parentToolCallId) ?? 0) + singularityUsage.totalNanoAiu;
			...
			const parentUsage = buildUsage(parentContext, this._parentSingularityUsageMeta(), true);
			...
			if (parentToolCallId) {
				...
				const subagentSingularityUsage = singularityUsage && scopedTotal !== undefined
					? { ...singularityUsage, totalNanoAiu: scopedTotal }
					: undefined;
				this._emitAction({
					type: ActionType.ChatUsage,
					turnId: this._turnId,
					usage: buildUsage(eventContext, subagentSingularityUsage, false),
				}, parentToolCallId);
			}
		}));
		...
		this._register(wrapper.onUsage(async e => {
			...
			const singularityUsage = this._parentSingularityUsageMeta();
			if (!attribution && !singularityUsage) {
				return;
			}
		...
		this._register(wrapper.onSessionCompactionComplete(async e => {
			...
			const singularityUsage = readSingularityUsage(e.data.compactionTokensUsed);
			const turn = this._currentTurn;
			...
			const emitParentUsage = (): string | undefined => {
				const turnId = this._turnId;
				const parentSingularityUsage = this._parentSingularityUsageMeta();
				const turnTokenTotals = this._currentTurn?.tokenTotals;
				if (!turnId || (!parentSingularityUsage && !turnTokenTotals)) {
					return undefined;
				}
				...
				const usage: UsageInfo = {
					...
					_meta: {
						...(base?._meta ?? {}),
						...(parentSingularityUsage ? { singularityUsage: parentSingularityUsage } : {}),
						...(turnTokenTotals ? { turnTokenTotals } : {}),
					},
				};
				lastParentUsage = usage;
		...
		this._register(wrapper.onReasoningDelta(e => {
			this._logService.trace(`[Singularity:${sessionId}] Reasoning delta: ${e.data.deltaContent.length} chars`);
			this._resumeSubagentForEvent(e);
		...
		this._register(wrapper.onSessionModeChanged(e => {
			...
			// `agentId`.
			if (e.agentId) {
				this._logService.trace(`[Singularity:${sessionId}] Ignoring subagent session.mode_changed: agentId=${e.agentId}, ${e.data.previousMode} -> ${e.data.newMode}`);
				return;
			}
			this._logService.info(`[Singularity:${sessionId}] session.mode_changed: ${e.data.previousMode} -> ${e.data.newMode}`);
	...
	private _seedMcpServersFromRpc(): void {
		this._refreshMcpServersFromRpc().catch(err => {
			this._logService.warn(`[Singularity:${this.sessionId}] Failed to seed MCP server inventory`, err);
		});
	}
	...
	private _logMcpServerLifecycle(server: IMcpLifecycleLogInfo & { readonly origin: McpLifecycleOrigin }): void {
		...
		const message = `[Singularity:${this.sessionId}] MCP server '${server.name}' ${server.status} (${state.kind})${detail}`;
	 ...
	 */
	private _syncAhpConfigFromSdkMode(sdkMode: SingularitySdkMode): void {
	 ...
	 * resolved {@link SingularityExitPlanModeResponse} flows back to the CLI, which
	 ...
	 */
	private async _handleExitPlanModeRequest(data: ExitPlanModeRequest, _invocation: { sessionId: string }): Promise<SingularityExitPlanModeResponse> {
		const turnId = this._currentTurn?.id;
		if (!turnId) {
			this._logService.warn(`[Singularity:${this.sessionId}] Rejecting plan review request without an active turn`);
			return { approved: false };
		}
		const requestId = generateUuid();
		const questionId = generateUuid();
		this._logService.info(`[Singularity:${this.sessionId}] exitPlanMode.request: rpcId=${requestId}, actions=[${data.actions.join(',')}], recommended=${data.recommendedAction}`);
		...
		try {
			...
			this._logService.warn(`[Singularity:${this.sessionId}] rpc.plan.read failed for exit_plan_mode: ${err instanceof Error ? err.message : String(err)}`);
		}
		if (this._currentTurn?.id !== turnId) {
			this._logService.warn(`[Singularity:${this.sessionId}] Rejecting plan review request after its turn ended`);
			return { approved: false };
		}
		...
		try {
			...
			this._logService.error(err, `[Singularity:${this.sessionId}] exitPlanMode.request handler failed: rpcId=${requestId}`);
			return { approved: false };
		}
	}
		...
		this._register(wrapper.onUserMessage(e => {
			...
			void (async () => {
				...
				try {
					...
					this._logService.trace(`[Singularity:${sessionId}] Failed to fetch instruction sources for telemetry: ${getErrorMessage(err)}`);
					return;
				}
				...
				for (const s of sources) {
					// The SDK marks singularity-instructions.md (home/repo) and root-level
				...
				type AgentHostInstructionsCollectedClassification = {
					provider: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'The Agent Host provider that emitted this event (e.g. singularitycli). Absent on local rows; use presence to distinguish AH from local.' };
					...
					agentInstructionsCount: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; isMeasurement: true; comment: 'Number of top-level agent instruction files (singularity-instructions.md, AGENTS.md, CLAUDE.md, GEMINI.md) among the loaded sources.' };
				...
				this._logService.trace(`[Singularity:${sessionId}] instructionsCollected telemetry failed: ${getErrorMessage(err)}`);
			});
		}));
	}
	...
	private _subscribeForLogging(): void {
		...
		this._register(wrapper.onUnhandledEvent(e => {
			this._logService.trace(`[Singularity:${sessionId}] Unhandled SDK event: ${safeStringify(e)}`);
		}));

		this._register(wrapper.onSessionStart(e => {
			this._logService.trace(`[Singularity:${sessionId}] Session started: model=${e.data.selectedModel ?? 'default'}, producer=${e.data.producer}`);
		}));

		this._register(wrapper.onSessionResume(e => {
			this._logService.trace(`[Singularity:${sessionId}] Session resumed: eventCount=${e.data.eventCount}`);
		}));
		...
		this._register(wrapper.onSessionInfo(e => {
			...
			const message = `[Singularity:${sessionId}] [${e.data.infoType}]: ${e.data.message}`;
		...
		this._register(wrapper.onSessionWarning(e => {
			this._logService.warn(`[Singularity:${sessionId}] ${e.data.message}`, new OtelData({ warningType: e.data.warningType }));
		}));

		this._register(wrapper.onSessionModelChange(e => {
			this._logService.trace(`[Singularity:${sessionId}] Model changed: ${e.data.previousModel ?? '(none)'} -> ${e.data.newModel}`);
			if (!e.agentId) {
				this._promptCacheRefreshGeneration++;
		...
		this._register(wrapper.onManagedSettingsResolved(e => {
			this._logService.info(`[Singularity:${sessionId}] Managed settings resolved: source=${e.data.source}, managedKeys=${e.data.managedKeys.join(',') || '(none)'}, bypassPermissionsDisabled=${e.data.bypassPermissionsDisabled}, failClosed=${e.data.failClosed}`);
		}));

		this._register(wrapper.onManagedSettingsEnforced(e => {
			this._logService.warn(`[Singularity:${sessionId}] Managed settings enforced: action=${e.data.action}, setting=${e.data.setting}, escalation=${e.data.escalation ?? '(none)'}, failClosed=${e.data.failClosed}, message=${e.data.message}`);
		}));

		this._register(wrapper.onSessionHandoff(e => {
			this._logService.trace(`[Singularity:${sessionId}] Session handoff: sourceType=${e.data.sourceType}, remoteSessionId=${e.data.remoteSessionId ?? '(none)'}`);
		}));

		this._register(wrapper.onSessionTruncation(e => {
			this._logService.trace(`[Singularity:${sessionId}] Session truncation: removed ${e.data.tokensRemovedDuringTruncation} tokens, ${e.data.messagesRemovedDuringTruncation} messages`);
		}));

		this._register(wrapper.onSessionSnapshotRewind(e => {
			this._logService.trace(`[Singularity:${sessionId}] Snapshot rewind: upTo=${e.data.upToEventId}, eventsRemoved=${e.data.eventsRemoved}`);
		}));

		this._register(wrapper.onSessionShutdown(e => {
			this._logService.trace(`[Singularity:${sessionId}] Session shutdown: type=${e.data.shutdownType}, apiDuration=${e.data.totalApiDurationMs}ms`);
		}));

		this._register(wrapper.onSessionUsageInfo(e => {
			this._logService.trace(`[Singularity:${sessionId}] Usage info: ${e.data.currentTokens}/${e.data.tokenLimit} tokens, ${e.data.messagesLength} messages`);
		}));

		this._register(wrapper.onSessionCompactionStart(() => {
			this._logService.trace(`[Singularity:${sessionId}] Compaction started`);
		}));

		this._register(wrapper.onSessionCompactionComplete(e => {
			this._logService.trace(`[Singularity:${sessionId}] Compaction complete: success=${e.data.success}, tokensRemoved=${e.data.tokensRemoved ?? '?'}`);
		}));

		this._register(wrapper.onUserMessage(e => {
			this._logService.trace(`[Singularity:${sessionId}] User message: ${e.data.content.length} chars, ${e.data.attachments?.length ?? 0} attachments`);
			...
			if (!e.agentId && (!e.data.source || e.data.source.toLowerCase() === 'user')) {
				void this._telemetryReporter.userMessageText(this.sessionUri.toString(), this._currentTurn?.clientType ?? AgentHostClientType.Unknown, e.data.content, this._turnOrdinal).catch(err => this._logService.trace(`[Singularity:${this.sessionId}] Telemetry emission failed: ${getErrorMessage(err)}`));
			}
		}));

		this._register(wrapper.onPendingMessagesModified(() => {
			this._logService.trace(`[Singularity:${sessionId}] Pending messages modified`);
		}));

		this._register(wrapper.onTurnStart(e => {
			this._currentTurn?.markRunning();
			this._logService.trace(`[Singularity:${sessionId}] Turn started: ${e.data.turnId}`);
			if (!e.agentId) {
				...
				this._cancelActiveRepoInfoTelemetry();
				const turn: NonNullable<SingularityAgentSession['_activeRepoInfoTurn']> = {
					telemetryMessageId,
					cancelled: false,
		...
		this._register(wrapper.onIntent(e => {
			this._logService.trace(`[Singularity:${sessionId}] Intent: ${e.data.intent}`);
		...
		this._register(wrapper.onReasoning(e => {
			this._logService.trace(`[Singularity:${sessionId}] Reasoning: ${e.data.content.length} chars`);
		}));

		this._register(wrapper.onTurnEnd(e => {
			this._logService.trace(`[Singularity:${sessionId}] Turn ended: ${e.data.turnId}`);
		}));

		this._register(wrapper.onAbort(e => {
			this._logService.trace(`[Singularity:${sessionId}] Aborted: ${e.data.reason}`);
			this._cancelActiveRepoInfoTelemetry();
		...
		this._register(wrapper.onToolUserRequested(e => {
			this._logService.trace(`[Singularity:${sessionId}] Tool user-requested: ${e.data.toolName} (${e.data.toolCallId})`);
		}));

		this._register(wrapper.onToolPartialResult(e => {
			this._logService.trace(`[Singularity:${sessionId}] Tool partial result: ${e.data.toolCallId} (${e.data.partialOutput.length} chars)`);
			const tracked = this._activeToolCalls.get(e.data.toolCallId);
		...
		this._register(wrapper.onToolProgress(e => {
			this._logService.trace(`[Singularity:${sessionId}] Tool progress: ${e.data.toolCallId} - ${e.data.progressMessage}`);
		}));

		this._register(wrapper.onSkillInvoked(e => {
			this._logService.trace(`[Singularity:${sessionId}] Skill invoked: ${e.data.name} (${e.data.path})`);
		}));

		this._register(wrapper.onSubagentStarted(e => {
			this._logService.trace(`[Singularity:${sessionId}] Subagent started: ${e.data.agentName} (${e.data.agentDisplayName})`);
		}));

		this._register(wrapper.onSubagentCompleted(e => {
			this._completeSubagentTurn(e.agentId, e.data.toolCallId);
			this._logService.trace(`[Singularity:${sessionId}] Subagent completed: ${e.data.agentName}`);
		}));

		this._register(wrapper.onSubagentFailed(e => {
			this._completeSubagentTurn(e.agentId, e.data.toolCallId);
			this._logService.error(`[Singularity:${sessionId}] Subagent failed: ${e.data.agentName} - ${e.data.error}`);
		}));

		this._register(wrapper.onSubagentSelected(e => {
			this._logService.trace(`[Singularity:${sessionId}] Subagent selected: ${e.data.agentName}`);
		}));

		this._register(wrapper.onHookStart(e => {
			this._logService.trace(`[Singularity:${sessionId}] Hook started: ${e.data.hookType} (${e.data.hookInvocationId})`);
		}));

		this._register(wrapper.onHookEnd(e => {
			this._logService.trace(`[Singularity:${sessionId}] Hook ended: ${e.data.hookType} (${e.data.hookInvocationId}), success=${e.data.success}`);
		...
		this._register(wrapper.onSystemMessage(e => {
			this._logService.trace(`[Singularity:${sessionId}] System message [${e.data.role}]: ${e.data.content.length} chars`);
		}));
	}
	...
	async truncateAtEventId(eventId: string, keepTurnId?: string): Promise<void> {
		this._logService.info(`[Singularity:${this.sessionId}] Truncating via SDK RPC at eventId=${eventId}`);
		const result = await this._wrapper.session.rpc.history.truncate({ eventId });
		this._logService.info(`[Singularity:${this.sessionId}] SDK truncation removed ${result.eventsRemoved} events`);
	...
	private _deletePendingEditContent(toolCallId: string): void {
		...
		this._fileService.del(uri).catch(err => {
			this._logService.warn(`[Singularity:${this.sessionId}] Failed to delete pending edit content: ${uri.toString()}`, err);
		});
	}
...
/**
 * Reads the SDK's internal `singularityUsage` billing payload, carried on both the `assistant.usage`
 ...
 */
function readSingularityUsage(raw: unknown): { totalNanoAiu: number } & Record<string, unknown> | undefined {
	if (!raw || typeof raw !== 'object') {
		return undefined;
	}
	const usage = (raw as Record<string, unknown>).singularityUsage;
