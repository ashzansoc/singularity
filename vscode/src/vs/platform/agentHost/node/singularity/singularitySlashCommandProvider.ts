...
type RuntimeSlashCommandInfo = Awaited<ReturnType<SingularityClient['rpc']['commands']['list']>>['commands'][number];

export class SingularitySlashCommandProvider {
	...
	async getSlashCommands(options?: { readonly maxWaitMs?: number }): Promise<readonly RuntimeSlashCommandInfo[]> {
		try {
			...
			this._logService.warn(`[Singularity] rpc.commands.list failed`, err);
			return [];
		}
	}
