...
import { deriveGitHubEndpoints, gitHubSingularityResource, gitHubRepoResource, IGitHubEndpoints } from '../common/githubEndpoints.js';
...
export interface IAgentHostGitHubEndpointService {
	...
	/** The GitHub Singularity protected resource, computed against the configured endpoints. */
	getSingularityResource(): ProtectedResourceMetadata;
	 ...
	 * or `undefined` for github.com. Used to set `SINGULARITY_GH_HOST` for the Singularity CLI.
	 */
	 ...
	 * GitHub API host (`api.<host>`) used for `singularity_internal` endpoints (token
...
export class AgentHostGitHubEndpointService extends Disposable implements IAgentHostGitHubEndpointService {
	...
	getSingularityResource(): ProtectedResourceMetadata {
		return gitHubSingularityResource(this._endpoints);
	}
