...
Add a `ClaudeAgent` provider to the agent host that registers with `IAgentService`, advertises Anthropic models from `ISingularityApiService.models()`, and authenticates against the same GitHub resource as `SingularityAgent`. **No SDK / subprocess / sendMessage** in this phase — those are Phase 6+. Most `IAgent` methods throw `Error('TODO: Phase N')`.
...
**Exit criteria:** With `chat.agentHost.claudeAgent.enabled: true`, a workbench client connecting to the agent host sees a `claude` provider in root state, can pick a Claude model, and `sendMessage()` throws `TODO: Phase 6`. With the setting off (the default), only `singularitycli` is registered.
...
| **Modify** | `src/vs/platform/agentHost/node/agentHostMain.ts` | Conditionally register `ClaudeAgent` next to `SingularityAgent` based on the env var. |
| **Modify** | `src/vs/platform/agentHost/node/agentHostServerMain.ts` | Register `ISingularityApiService`, `IClaudeProxyService`, and `ClaudeAgent` (gated on env var or `--enable-claude-agent`). Currently has none of these. |
...
|---|---|
| Class shell, `Disposable`, emitter, observable | `src/vs/platform/agentHost/node/singularity/singularityAgent.ts` (in-tree `IAgent` reference) |
| `getDescriptor` / `getProtectedResources` / `authenticate` | `SingularityAgent` ONLY — extension has no `IAgent` analog |
| Model filter | `extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeModels.ts:165–179` |
...
Mirror `SingularityAgent` lines 249–266. Use `localize()` for user-visible strings:
...
Mirror `SingularityAgent.authenticate()` (lines 277–309):
...
Copy SingularityAgent's pattern (lines 299–313):
...
private async _refreshModels(): Promise<void> {
    ...
    try {
        const all = await this._singularityApiService.models(tokenAtStart);
...
Field references (`src/typings/singularity-api.d.ts`):
- `vendor` → line 113
...
`ISingularityApiService` and `IClaudeProxyService` are already registered at lines 110–112. Add one more `registerProvider` call after `SingularityAgent`, **gated on the env var**:

```ts
agentService.registerProvider(instantiationService.createInstance(SingularityAgent));
...
Currently this file does NOT register `ISingularityApiService` or `IClaudeProxyService`. Inside the `if (!options.quiet)` block (around line 188), before `instantiationService.createInstance(SingularityAgent)`:

```ts
const singularityApiService = instantiationService.createInstance(SingularityApiService, undefined);
diServices.set(ISingularityApiService, singularityApiService);
...
// Inside the !options.quiet block, after SingularityAgent registration:
...
- `SingularityApiService`, `ISingularityApiService` from `./shared/singularityApiService.js`
...
**`undefined` second arg to `SingularityApiService` is intentional** — that's the `fetchFn` slot, and `agentHostMain.ts:110` does the same.

## 5. Test file spec

New file: `src/vs/platform/agentHost/test/node/claudeAgent.test.ts`. Mirror the harness style of `src/vs/platform/agentHost/test/node/singularityAgent.test.ts` (read lines 233–262 for the `createTestAgentContext` pattern).
...
- `ISingularityApiService` mock: `models(token)` returns a canned `CCAModel[]` mixing Anthropic + non-Anthropic + non-tool-calls + non-picker.
...
13. **Stale-write guard:** if a slow `models(tokA)` call resolves *after* `authenticate(tokB)` has already published `[B]`, the late `[A]` result must be discarded.
    ...
    ```
    Codifies the invariant that the in-tree `SingularityAgent` already relies on but doesn't currently test.
...
| `agentHostServerMain.ts` lacks DI for `ISingularityApiService`/`IClaudeProxyService` — server mode would crash. | Add both registrations as in §4.4. |
| `CCAModel.supported_endpoints` is `string[] \| undefined` — direct `.includes()` will throw. | Use `?.includes()` everywhere. |
| `_refreshModels` writes after `await` — token may have rotated. | Snapshot `_githubToken` at start, gate the write on equality. (Mirrors SingularityAgent.) |
...
| `IClaudeProxyService` constructor requires `ISingularityApiService` (verified Phase 2). | Register `ISingularityApiService` first in `agentHostServerMain.ts`. |
...
| Forgetting to gate — default-on registration would expose `TODO: Phase N` errors to every Insiders user. | Both `agentHostMain.ts` and `agentHostServerMain.ts` MUST gate on `process.env[AgentHostEnableClaudeEnvVar] === '1'`. Smoke without setting the env var: only `singularitycli` should appear in root state. |
...
- [ ] `id = 'claude'` (not `'claude-code'`, not `'singularitycli'`).
- [ ] Constructor takes exactly `@ILogService`, `@ISingularityApiService`, `@IClaudeProxyService` (no other deps).
...
- [ ] `agentHostServerMain.ts` registers `ISingularityApiService`, `IClaudeProxyService`, AND `ClaudeAgent` (was missing all three). `ClaudeAgent` registration is gated on `--enable-claude-agent` OR the env var; the file-header usage line documents the new flag.
...
- [ ] **Default-off:** with no setting and no env var, a workbench client connecting to the agent host sees only `'singularitycli'` in root state — no `'claude'` provider.
- [ ] **Opt-in:** with `chat.agentHost.claudeAgent.enabled: true` (or `VSCODE_AGENT_HOST_ENABLE_CLAUDE=1` set on the parent process) and a fresh agent host, the client sees `'claude'` listed alongside `'singularitycli'`.
...
| `agentHostServerMain.ts` server crashes at startup | DI registration order | Register `ISingularityApiService` BEFORE `IClaudeProxyService`. |
...
- [x] **Gate verified enabled:** re-launched via `launch-smoke.sh` (which sets `VSCODE_AGENT_HOST_ENABLE_CLAUDE=1`); both providers register. See `registration.log` (`Registering agent provider: singularitycli` + `…: claude`).
...
No. `dispose()` clears `this._githubToken` *before* `super.dispose()`, so any hung `_refreshModels` will, on resume, see `this._githubToken !== tokenAtStart` and drop its write — making the in-flight refresh a no-op. Test 13 (stale-write guard) codifies this invariant. Fire-and-forget; matches `SingularityAgent`.
