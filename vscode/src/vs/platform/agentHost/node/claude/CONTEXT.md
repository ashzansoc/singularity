...
Anthropic requests into GitHub Singularity CAPI calls.

## Language

**Agent Host**:
The utility process that owns all `IAgent` providers (Singularity, Claude). Hosts
this code.
...
the inbound side and `ISingularityApiService` on the outbound side. Registered
...
**CAPI**:
GitHub Singularity's chat completions API, accessed through `ISingularityApiService`.
...
- The **Claude Proxy** translates inbound **Anthropic Messages API** requests
  into outbound `ISingularityApiService` calls, then to **CAPI**.
...
`ISingularityApiService`. One instance per agent host process.
...
- Port `extensions/singularity-chat/src/extension/chatSessions/claude/{common,node}/claudeModelId.ts`
...
- Applied at `POST /v1/messages` after auth, before model translation.
  ...
  `ISingularityApiServiceRequestOptions.headers['anthropic-beta']`. If
...
The reference (`extensions/singularity-chat/.../claudeLanguageModelServer.ts`) is a
**byte-passthrough** — it pipes raw upstream SSE chunks straight to the
client. Our proxy can't do that: `ISingularityApiService.messages()` returns
...
- Single body: `JSON.stringify(message)` where `message` is the
  ...
  `ISingularityApiService.messages()` overload, with the `model` field
...
`ISingularityApiServiceRequestOptions.signal`.
...
try {
    const stream = singularityApi.messages(body, { signal: ac.signal, headers });
...
- **No polling**. Pass `signal` to `ISingularityApiService.messages()`; the
...
- **Non-streaming branch**: same pattern. The awaited
  `ISingularityApiService.messages()` rejects with `AbortError`; we `return`
...
(`FakeSingularityApiService`); everything else (real `http.Server`, real
`AbortController`, real parser, real filter) is real. Per the codebase
guideline: minimal mocking, mock only true I/O boundaries.

```ts
class FakeSingularityApiService implements ISingularityApiService {
    nextMessagesResult: Anthropic.Message
        | AsyncGenerator<Anthropic.MessageStreamEvent>
        | Anthropic.APIError;
    lastCall: { body: Anthropic.MessageCreateParams; options: ISingularityApiServiceRequestOptions };
...
1. Temporarily modify the existing `SingularityAgent` (or wherever the
...
7. Revert the temporary `SingularityAgent` change.
...
- **HTTP proxy support** (`HTTP_PROXY` / `HTTPS_PROXY` env vars, VS Code's
  ...
  to CAPI through `ISingularityApiService`, so any outbound proxying is
  ...
  agent / dispatcher injection point on `ISingularityApiService` and a way
...
`extensions/singularity-chat/.../claudeCodeAgent.ts` reference implementation.
...
- "SDK" = the `@anthropic-ai/claude-agent-sdk` API + its bundled CLI
  subprocess.
- "Reference extension" = the production Singularity extension's Claude
  support under `extensions/singularity-chat/src/extension/chatSessions/claude/`.
...
[`claudeCodeAgent.ts`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts):
...
UI value is clamped to `'xhigh'` at the seam (see [`claudeCodeAgent.ts:196`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts#L196)).
...
| Reference | [claudeCodeAgent.ts](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts) `_createPromptIterable` |
...
|---|---|---|---|
| `Options.canUseTool(toolName, input, { suggestions })` for arbitrary tool names | Before any tool that the SDK has NOT auto-approved via `permissionMode` is executed (see [PermissionMode docstring, sdk.d.ts:1558](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1558)) | **Permission** (`SessionToolCallReady` → `respondToPermissionRequest`) | Standard tool-permission gate |
...
**`canUseTool` return type** (locked invariant, [sdk.d.ts:1582](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1582)):
...
**`onElicitation` return type** ([sdk.d.ts:966](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L966), [sdk.d.ts:1163](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1163)):
...
observation at
[`claudeMessageDispatch.ts:200`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/common/claudeMessageDispatch.ts#L200).
...
#### Sibling: how `SingularityAgent` implements the same surface

The Singularity CLI agent ([`singularityAgentSession.ts`](../singularity/singularityAgentSession.ts))
implements the identical IAgent contract against a different SDK.
The shape is the same; the SDK callbacks differ.

| Concern | `ClaudeAgent` (this folder) | `SingularityAgent` ([../singularity/](../singularity/)) |
|---|---|---|
...
| Inbound resolution | `respondToPermissionRequest`/`respondToUserInputRequest` walk `_sessions.values()`, return `boolean` on first match | Identical pattern ([`singularityAgent.ts:1239-1254`](../singularity/singularityAgent.ts#L1239-L1254)) |
| Auto-approve hook | None on the host — `_handleCanUseTool` is a pure UI bridge. The SDK owns all auto-approval / auto-denial via `permissionMode` ([sdk.d.ts:1558](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1558)) before `canUseTool` is invoked. | Host-side: internal session-resource paths, `singularity-tool-output-*.txt` SDK temp files, `autopilot` config |
...
| Status (Phase 6) | Stub — both methods throw `TODO: Phase 7` ([`claudeAgent.ts:790, 794`](claudeAgent.ts#L790-L794)). Re-implementation explicitly mirrors `singularityAgent.ts:1239-1254` (see [phase7-plan.md](phase7-plan.md)) | Fully implemented |

The Singularity CLI SDK pre-resolves the Claude-side fan-in: its single
...
| Reference | [`claudeCodeAgent.ts:289, 733-735`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts#L289) |
...
| Reference | [sdkSessionAdapter.ts](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/sessionParser/sdkSessionAdapter.ts), [claudeMessageDispatch.ts](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/common/claudeMessageDispatch.ts) (live dispatcher) |
...
| Reference | [claudeCodeAgent.ts](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts) `_processMessages`, [claudeMessageDispatch.ts](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/common/claudeMessageDispatch.ts) `dispatchMessage` |
...
- **Internal materialization on first `sendMessage`.** When the
  ...
  it:
  1. Resolves the effective `workingDirectory`. Singularity's
...
disk
([claudeCodeAgent.ts:457-459](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts#L457-L459)):
...
([sdk.d.ts](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts) lines 552–571):
...
both independently include worktree removal on the Singularity path.
...
| `dispose()` | provider | sync surface; may chain async | Singularity: yes; Claude: no | Singularity: `shutdown().finally(super.dispose)` — cooperative. Claude: synchronous wrapper-then-proxy teardown, no graceful drain. The provider choice is intentional: Claude's wrapper is the stronger ownership and must dispose before the IPC handle. |
...
#### Asymmetries between Singularity and Claude

| | SingularityAgent | ClaudeAgent |
|---|---|---|
...
These asymmetries are deliberate. Singularity's worktree story requires
...
| IAgent surface | SDK primitive(s) | What it does |
|---|---|---|
| `setPendingMessages?(chat, steeringMessage, queuedMessages)` (optional) | Yield an `SDKUserMessage` with `priority: 'now'` into the prompt iterable that was passed to `query()` ([sdk.d.ts:3067-3086](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L3067-L3086)) | Notifies the agent that the chat's pending-message state changed. The agent reacts by yielding the steering content as an `SDKUserMessage` whose `priority` is `'now'`, which the SDK treats as "preempt the current turn and run me first." |
...
([cli.js:1030](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/cli.js#L1030):
...
([cli.js:8661](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/cli.js#L8661))
...
|---|---|---|
| Prompt iterable (passed to `query({ prompt })`) | **Yes** — [claudeCodeAgent.ts:504-506, 518-587](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts#L504-L506) | Long-lived `async function*` that yields *every* `SDKUserMessage` for the session's lifetime. |
| `Query.streamInput(stream)` ([sdk.d.ts:1910](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1910)) | **No** — zero callers under `extensions/singularity-chat/src/extension/chatSessions/claude/**` | An alternate transport for pushing additional messages into a live `Query`. The reference impl never invokes it; the prompt iterable absorbs that role. |
| `Query.interrupt()` ([sdk.d.ts:1745](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1745)) | **No** for steering — the abort is implicit, driven by the SDK's own `'now'`-priority watcher. The host *does* use the `AbortController` for explicit aborts (M4). | Stops generation outright; would orphan the steering message. |
...
[claudeCodeAgent.ts:580](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts#L580)
...
##### Singularity: in-place via SDK RPC

SingularityAgent implements `truncateSession`
([singularityAgent.ts:1179-1212](../../node/singularity/singularityAgent.ts#L1179-L1212)).
...
The Singularity SDK exposes a session-mutation RPC
...
| `setPendingMessages` (steering) | Implemented; injects via Singularity SDK's `send({ mode: 'immediate' })` | Implemented (planned Phase 9); yields `SDKUserMessage` with `priority: 'now'` into the existing prompt iterable |
...
| Singularity CLI SDK | `session.send({ ... })` (no `mode`) | `session.send({ ..., mode: 'immediate' })` |
...
|---|---|---|---|
| Active model | `model?: string` | `setModel(model?)` | `SDKControlSetModelRequest` ([sdk.d.ts:2425](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L2425)) |
| Permission mode | `permissionMode?: PermissionMode` | `setPermissionMode(mode)` | `SDKControlSetPermissionModeRequest` ([sdk.d.ts:2433](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L2433)) |
| Effort level | `effort?: EffortLevel` (5 values incl. `'max'`) ([sdk.d.ts:1213](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1213)) | `applyFlagSettings({ effortLevel })` ([sdk.d.ts:1789](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1789)) — note: 4-value subset, no `'max'` ([sdk.d.ts:4292](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L4292)) | `SDKControlApplyFlagSettingsRequest` ([sdk.d.ts:2080](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L2080)) |
...
type ([sdk.d.ts:977-1532](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L977-L1532)).
...
([claudeCodeAgent.ts:195-196](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts#L195-L196)):
...
(see [claudeCodeModels.ts:190-214](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeModels.ts#L190-L214)).
...
- `Query` runtime methods are *control-channel writes* over an
  ...
  ([SDKControlRequestInner](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L2373)
...
provider does.* SingularityAgent and the Claude provider both build
...
| SingularityAgent | Hardcoded literal `{ provider: 'singularitycli', displayName: 'Singularity', description: '…' }` ([singularityAgent.ts:256-262](../singularity/singularityAgent.ts#L256-L262)) |
...
`IAgent.id` and `IAgentDescriptor.provider` MUST be equal; SingularityAgent
hardcodes both to `'singularitycli'` ([singularityAgent.ts:206, 258](../singularity/singularityAgent.ts#L206)).
...
| Claude provider source | **CAPI** via `ISingularityApiService.models(githubToken)` ([singularityApiService.ts:228-281](../shared/singularityApiService.ts#L228-L281)) — *not* `Query.supportedModels()` |
...
reach the IAgent provider through the same Singularity Chat API the
reference extension uses, not through Anthropic's API. The
Singularity subscription is the auth principal; CAPI proxies the
...
The shared service `ISingularityApiService` already exposes the right
primitive ([singularityApiService.ts:281](../shared/singularityApiService.ts#L281)):

```ts
models(githubToken: string, options?: ISingularityApiServiceRequestOptions): Promise<CCAModel[]>;
```
...
4. `capabilities.supports.tool_calls === true` — matches the
   ...
   [claudeCodeModels.ts:154-164](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeModels.ts#L154-L164).
...
- `configSchema` ← synthesized from `CCAModel.capabilities.supports.thinking`
  ...
  `_createThinkingLevelConfigSchema` pattern SingularityAgent uses
  ([singularityAgent.ts:457-484](../singularity/singularityAgent.ts#L457-L484)).
...
- `_meta` ← billing multiplier and any other CAPI-specific
  side-channel data (matches SingularityAgent's `multiplierNumeric`
  pattern).

The refresh trigger is identical to SingularityAgent: re-call
`ISingularityApiService.models()` whenever the GitHub token changes
(see SingularityAgent's `_refreshModels()` at [singularityAgent.ts:300-317](../singularity/singularityAgent.ts#L300-L317)
and its `authenticate()`-driven invocation at [singularityAgent.ts:295](../singularity/singularityAgent.ts#L295)).
...
doesn't need to. `ISingularityApiService.models()` is the agent-host
...
is a per-provider side-channel; SingularityAgent uses it for billing
...
| Claude SDK source | **Top-level** `listSessions(options?): Promise<SDKSessionInfo[]>` ([sdk.d.ts:729](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L729)) — *not* a `Query` method |
| `SDKSessionInfo` shape | `{ sessionId, summary, lastModified, customTitle?, firstPrompt?, gitBranch?, cwd?, tag?, createdAt }` ([sdk.d.ts:2782-2825](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L2782-L2825)) |
...
in `ClaudeCodeSdkService` ([claudeCodeSdkService.ts:78-117](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeSdkService.ts#L78-L117)),
and `getAllSessions(token)` ([claudeCodeSessionService.ts:75-110](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/sessionParser/claudeCodeSessionService.ts#L75-L110))
...
[sessionParser/README.md](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/sessionParser/README.md).)

##### SingularityAgent's `listSessions` shape

SingularityAgent ([singularityAgent.ts:525-558](../singularity/singularityAgent.ts#L525-L558))
joins three sources:
...
- **No-sidecar filter (SingularityAgent only).** Sessions without sidecar
  metadata are *dropped* — so Singularity's `listSessions` returns only
...
- **Provisional sessions are not included** ([singularityAgent.ts:736-742](../singularity/singularityAgent.ts#L736-L742)).
  They have no SDK session yet, so `client.listSessions()` doesn't
  know about them and there's no sidecar until materialization
  (M9).

##### Claude's `listSessions` differences from SingularityAgent

The Claude provider should mirror SingularityAgent's join pattern, but
the two SDKs disagree on which fields they carry:

| Field | SingularityAgent source | Claude source |
|---|---|---|
...
| Claude SDK source | **Top-level** `getSessionInfo(sessionId, options?): Promise<SDKSessionInfo \| undefined>` ([sdk.d.ts:581](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L581)) |
...
SingularityAgent implements it ([singularityAgent.ts:560-591](../singularity/singularityAgent.ts#L560-L591))
...
matching primitive) for parity with SingularityAgent. Callers must
...
locally from host knowledge. SingularityAgent's resolver
([singularityAgent.ts:819-877](../singularity/singularityAgent.ts#L819-L877))
...
- **`permissionMode`** — six values
  ...
  type at [sdk.d.ts:1560](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L1560).
...
- **`isolation` / `branch`** — share with SingularityAgent's pattern;
...
SingularityAgent ([singularityAgent.ts:880-887](../singularity/singularityAgent.ts#L880-L887))
uses it for one property — `branch` — backed by an
`IAgentHostGitService` shell-out and capped at
`_BRANCH_COMPLETION_LIMIT = 25` ([singularityAgent.ts:217](../singularity/singularityAgent.ts#L217)).
...
(matching SingularityAgent's `if (property === 'branch')` shape) —
...
1. **Cross-property resolution.** `values` can encode resolutions
   the schema can't. SingularityAgent's branch resolves to
   `currentBranch` for `folder` isolation but `defaultBranch` for
   `worktree` ([singularityAgent.ts:833](../singularity/singularityAgent.ts#L833))
...
3. **Intentional gaps.** A property may be in `schema` but not in
   `values` — e.g. SingularityAgent omits a `permissions` slot from
   `values` so auto-approval falls through ([singularityAgent.ts:867-870](../singularity/singularityAgent.ts#L867-L870)).
...
| Surface | SingularityAgent | Claude provider |
|---|---|---|
| `getDescriptor` | Synthetic literal | Synthetic literal |
| `models` | CAPI via `client.listModels()` (SingularityClient wraps CAPI) | **CAPI** via `ISingularityApiService.models(githubToken)` filtered to `vendor === 'Anthropic'` ∩ `supported_endpoints ∋ '/v1/messages'` ∩ `model_picker_enabled === true` — *not* `Query.supportedModels()` |
...
([claudeCodeSdkService.ts:78-117](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeSdkService.ts#L78-L117))
...
shared `ISingularityApiService` rather than a vendor-specific SDK
client.
...
- **`models` is sourced from CAPI, not from the Claude SDK.**
  ...
  `ISingularityApiService.models()` and apply the three-predicate
...
- **`getSessionMetadata?` is a parity surface.** Where the SDK
  ...
  `getSessionMetadata` on Singularity), the provider should implement
...
`SDKControlRequestInner` union ([sdk.d.ts:2373](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts#L2373)),
...
401/403 handling, model listing — is `ISingularityApiService`'s job
(M12), not the agent's.
...
|---|---|
| `getProtectedResources()` ([agentService.ts:480](../../common/agentService.ts#L480)) | Return `[GITHUB_SINGULARITY_PROTECTED_RESOURCE]` ([agentService.ts:200-206](../../common/agentService.ts#L200-L206)). Synchronous, hardcoded, identical to SingularityAgent. |
...
| `AHP_AUTH_REQUIRED` throw | Any session lifecycle method that runs before `authenticate()` has landed must throw `ProtocolError(AHP_AUTH_REQUIRED, msg, this.getProtectedResources())`. SingularityAgent does this at [singularityAgent.ts:382-385](../singularity/singularityAgent.ts#L382-L385); ClaudeAgent currently throws a plain `Error` at [claudeAgent.ts:413-416](claudeAgent.ts#L413-L416) — to be corrected in Phase 6.1 Cycle B (see [phase6.1-plan.md](phase6.1-plan.md)). |

#### Why ClaudeAgent's `authenticate` ordering differs from SingularityAgent

SingularityAgent commits `_githubToken` first, then runs `_stopClient()`
