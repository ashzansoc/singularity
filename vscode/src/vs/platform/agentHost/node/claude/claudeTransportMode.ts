 ...
 * local Singularity-CAPI proxy (requires GitHub auth); `native` talks to Anthropic
...
export interface IClaudeTransportModeInputs {
	/**
	 * User/workspace-set value of `claudeUseSingularityProxy`, or `undefined` when
	...
	/** Whether a GitHub Singularity token has been captured (i.e. signed in). */
 ...
 *
 *  1. An explicit `claudeUseSingularityProxy` setting is a HARD override.
 *  2. Feature flag off means today's default behavior (always proxy).
 *  3. Signed in to GitHub prefers Singularity (proxy).
 ...
 *     window sign-in gate.
 *
 * Native mode drops the GitHub Singularity protected resource, so getting this
 ...
 * advertises the GitHub Singularity resource, so the server never asks the client
