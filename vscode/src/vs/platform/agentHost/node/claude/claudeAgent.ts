...
import { projectFromSingularityContext } from '../singularity/singularityGitProject.js';
import { ISingularityApiService } from '../shared/singularityApiService.js';
 ...
 * `extensions/singularity-chat/src/platform/endpoint/common/endpointProvider.ts`
 ...
 * those are Singularity/CAPI concepts. Reuses the shared effort-schema helpers so
 ...
 *   state advertises Claude alongside Singularity CLI.
 ...
 * - {@link models} observable derived from {@link ISingularityApiService.models}
...
export class ClaudeAgent extends Disposable implements IAgent {
	 ...
	 * the Singularity-CAPI proxy; `native` talks to Anthropic directly on the user's
	 * own credentials. Resolved from the precedence in {@link resolveClaudeTransportMode}
	 * (explicit `claudeUseSingularityProxy` override; else the experimentation flag,
	 ...
	 * returned by every subsequent call. Mirrors `SingularityAgent.shutdown`
	 * at singularityAgent.ts:1246. Phase 5 has no async work so the race
	...
	constructor(
		...
		// strips `singularity_usage` from its `result`). Route every report to
		...
		// provider, so gate on our own provider id. Mirrors `SingularityAgent`.
		...
		//
		// Resolution now depends on the `claudeUseSingularityProxy` setting, the
	...
	private _resolveTransportMode(hasGitHubToken: boolean = this._githubToken !== undefined): ClaudeTransportMode {
		// An absent `claudeUseSingularityProxy` stays `undefined` so the pure function
		// can tell an explicit override from "fall through to the flag/sign-in rules".
		const explicitProxy = this._configurationService.getRootValue(agentHostCustomizationConfigSchema, AgentHostConfigKey.ClaudeUseSingularityProxy);
	...
	private _applyTransportModeChange(next: ClaudeTransportMode): void {
		...
		if (next === 'proxy' && !this._proxyHandle) {
			this._onDidRequireAuth.fire({
				resource: this._gitHubEndpointService.getSingularityResource().resource,
	...
	getProtectedResources(): ProtectedResourceMetadata[] {
		// Native (BYO-Anthropic) mode does not *require* GitHub Singularity auth — the
		// SDK owns the Anthropic credential. Rather than DROP the Singularity resource,
		...
		//      `protectedResourcesRequireSingularitySignIn`, which checks
		//      `required !== false`), so no sign-in is forced.
		// The optional repo resource is kept for git operations either way.
		const singularityResource = this._gitHubEndpointService.getSingularityResource();
		return [
			this._transportMode === 'proxy' ? singularityResource : { ...singularityResource, required: false },
	...
	async authenticate(resource: string, token: string): Promise<boolean> {
		...
		// A GitHub Singularity token is arriving (sign-in). Re-resolve the transport
		// with the token now available: absent an explicit `claudeUseSingularityProxy`,
		...
		// `claudeUseSingularityProxy=false`, so `_applyTransportModeChange` here is a
	...
	/**
	 * Whether the Claude provider routes through the Singularity-CAPI proxy.
	...
	private async _refreshModels(): Promise<void> {
		...
		// `claudeUseSingularityProxy: false`; the flag-driven path only picks native
	...
	/**
	 * Proxied (Singularity-CAPI) model source: fetch via {@link ISingularityApiService},
	...
	private async _fetchProxyModels(token: string): Promise<readonly IAgentModelInfo[]> {
		...
		const all = await this._singularityApiService.models(token, { headers: { 'User-Agent': userAgent }, suppressIntegrationId: true });
		return all
	...
	async createSession(config: IAgentCreateSessionConfig = {}): Promise<IAgentCreateSessionResult> {
		...
		if (existing) {
			...
			// tools/customizations must still reach Claude (mirrors Singularity).
		...
		// fork) runs in a stable per-session scratch dir shared with the Singularity
		...
		const project = requestedWorkingDirectory
			? await projectFromSingularityContext({ cwd: requestedWorkingDirectory.fsPath }, this._gitService)
			: undefined;
	 ...
	 * SDK at session creation, mirroring the Singularity agent. Runs for fresh AND
	...
	private async _forkSession(config: IAgentCreateSessionConfig, fork: NonNullable<IAgentCreateSessionConfig['fork']>): Promise<IAgentCreateSessionResult> {
		...
		return this._sessionSequencer.queue(sourceSessionId, async () => {
			...
			try {
				project = await projectFromSingularityContext({ cwd: workingDirectory.fsPath }, this._gitService);
			} catch (err) {
	 ...
	 * `SingularityAgent._resumeSession`. Reads `workingDirectory` from the
	...
	async listSessions(): Promise<IAgentSessionMetadata[]> {
		...
		// sibling Singularity provider gets nuked too. Catch and log instead.
	 ...
	 * {@link SingularityAgent.getSessionMetadata} but accepts the
	 ...
	 * sidecar (the way Singularity's variant does). The SDK is the source
	 ...
	 * per-chat config), so — matching Singularity's session-scoped approvals — we
	...
	getCustomizations(): readonly Customization[] {
		...
		// (the equivalent of Singularity's `agentHost.customizations` setting).
		...
		// agent host, lift `PluginController` out of `singularity/singularityAgent.ts`
	...
	override dispose(): void {
		...
		// subprocess. (Singularity reviewer: dispose lifecycle.)
		//
