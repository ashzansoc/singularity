 ...
 * reused at `tool_result` time. Mirrors Singularity's `IToolStartInfo`
 * shape: Singularity stashes it at `tool.execution_start` (where the
 * Singularity SDK hands over complete args); Claude stashes it at the
 ...
 *
 * Mirror of Singularity's `pendingTools: Map<toolCallId, IToolStartInfo>`
 * pattern in
 * [`mapSessionEvents.ts`](../singularity/mapSessionEvents.ts) — only the
 * seam differs (Singularity's SDK hands over complete args at
