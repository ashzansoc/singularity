...
- A real GitHub Singularity login. Models only populate after authenticate, and
  the Anthropic catalog is only visible to authenticated Singularity accounts.
...
1. Both `singularitycli` AND `claude` providers registered.
...
# → e.g. "Pick Session Type, Singularity CLI" [ref=eXXX]
...
npx @playwright/cli snapshot
SNAP=$(ls -t .playwright-cli/page-*.yml | head -1)
grep -nE 'option "(Singularity CLI|Claude)"' "$SNAP"
```

Expected: two `option "Singularity CLI" [ref=…]` and `option "Claude" [ref=…]`
lines.
...
|---------|--------------|
| `verify-claude-logs.sh` fails at check 1 (`singularitycli` missing) | A registration was deleted from `agentHostMain.ts` or `agentHostServerMain.ts`. |
...
| `verify-claude-logs.sh` fails at check 5 (zero models) | The §3.5 filter rejected everything. Inspect the upstream `[Singularity] Found N models` log line and check vendor / `supported_endpoints` / `model_picker_enabled` / `tool_calls`. |
...
| Picker shows only "Singularity CLI" but registration log is fine | Root state never propagated. Check the `autorun` in `agentSideEffects.ts` — `_publishAgentInfos` should fire on every `agents` observable change. |
