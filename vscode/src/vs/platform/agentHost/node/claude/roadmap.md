...
`SingularityAgent`, that uses the **Claude Agent SDK** to drive Anthropic-format
tool-using sessions — but with all `messages` API traffic **proxied through a
local server** that translates Anthropic requests into calls to GitHub Singularity's
CAPI via `ISingularityApiService`. Users authenticate once with their GitHub
Singularity credentials and get Claude models without ever talking to Anthropic
directly.
...
└────────────────────────┘        │
                  ...
                  └──────────────────────────────────┘
                                  ...
                                  ▼
                  ┌──────────────────────────────────┐
                  │  ISingularityApiService              │  ← Phase 1 ✅
                  │   token mint • CAPI • SSE        │
...
`IAgent` surface as `SingularityAgent`, so the workbench / sessions UI doesn't
need to know whether it's talking to Singularity or Claude.
...
- `extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeLanguageModelServer.ts`
...
- `extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts`
  ...
  customizations. **Caveats:**
    ...
    - Uses `mcpServers` config for HTTP-based MCP (lines 391–416) — does
      **not** use `createSdkMcpServer` + `tool()`. The in-process tool path
      lives in `extensions/singularity-chat/src/extension/chatSessions/claude/common/mcpServers/ideMcpServer.ts`.
    ...
    - Pins `settingSources: ['user', 'project', 'local']` (line 482). Does
      **not** include `managed`.
- `extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeSdkService.ts`
  ...
  60–74).
- `src/vs/platform/agentHost/node/singularity/singularityAgent.ts` — `IAgent` reference
...
### Phase 1 — `ISingularityApiService` ✅ **DONE**

Foundational gateway to the Singularity CAPI: token mint + cache + invalidation,
`messages` (streaming + non-streaming), `models`, abort propagation, SSE body
cancellation. Lives at `node/shared/singularityApiService.ts`. Fully unit-tested
(70 tests). Council-reviewed and hardened (C1/C2/S1/S2 fixes applied).

### Phase 1.5 — Widen `ISingularityApiService` to raw Anthropic events ✅ **DONE**
...
```typescript
interface ISingularityApiServiceRequestOptions {
    headers?: Readonly<Record<string, string>>;
    signal?: AbortSignal;
}

interface ISingularityApiService {
    messages(token: string, req: Anthropic.MessageCreateParamsStreaming,    options?: ISingularityApiServiceRequestOptions): AsyncGenerator<Anthropic.MessageStreamEvent>;
    messages(token: string, req: Anthropic.MessageCreateParamsNonStreaming, options?: ISingularityApiServiceRequestOptions): Promise<Anthropic.Message>;
    countTokens(token: string, req: Anthropic.MessageCountTokensParams, options?: ISingularityApiServiceRequestOptions): Promise<Anthropic.MessageTokensCount>;
    models(token: string, options?: ISingularityApiServiceRequestOptions): Promise<CCAModel[]>;
}
```
...
2. **Signal-sharing bug in deduped async operations:** the original
   `_getSingularityToken` forwarded the caller's `AbortSignal` into the shared
...
4. **`@vscode/copilot-api` typings:** the package's `.d.ts` files use
   ...
   "nodenext"`. Ambient declarations in `src/typings/singularity-api.d.ts` are
...
`ISingularityApiService` on the outbound side. **No `IAgent` integration in this
...
- **Model ID resolution:** rewrite `requestBody.model` against
  `ISingularityApiService.models()` (CCAModel list), mirroring
...
- Routes:
  - `POST /v1/messages` — streaming and non-streaming, request shape passed
    through to `ISingularityApiService.messages()` after model resolution. Stream
    re-emits `MessageStreamEvent`s as SSE.
  - `GET /v1/models` — pass-through to `ISingularityApiService.models()`, mapped
    to Anthropic's `/v1/models` response shape. **Net new** vs. the
    reference.
  - `POST /v1/messages/count_tokens` — pass-through to
    `ISingularityApiService.countTokens()` (or 501 if CAPI doesn't support it).
...
- Abort propagation: client disconnect → cancel the upstream
  `ISingularityApiService` call (already supports abort).
- **Lifetime:** **one proxy per `ClaudeAgent`**, shared across all sessions.

Tests:

- `/v1/messages` non-streaming → mocked `ISingularityApiService` → correct
...
- `/v1/models` → forwards to `ISingularityApiService.models()`, returns Anthropic
  shape.
- `/v1/messages/count_tokens` → pass-through (or 501).
- Bearer enforcement: missing nonce, wrong nonce, missing `.sessionId`
  segment, `x-api-key` alone → 401 without ever touching `ISingularityApiService`.
...
same payload shape Anthropic would, and `ISingularityApiService` sees the right
calls.

### Phase 3 — Ground the SDK contract in the production reference ✅ **DONE**

The Singularity extension already ships a working integration of
...
- [`extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts)
  — `ClaudeCodeSession` builds the SDK `Options` and runs the message loop.
- [`extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeLanguageModelServer.ts`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeLanguageModelServer.ts)
...
- [`extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeSdkService.ts`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeSdkService.ts)
...
same version `extensions/singularity-chat/package.json` ships. Versions > 0.2.112 add
...
- **`/v1/models`:** the extension's proxy 404s it; ours forwards to
  `ISingularityApiService.models()`. Either is fine in practice; the SDK does
...
the pattern in `node/singularity/singularityAgent.ts`.
...
- `getDescriptor(): IAgentDescriptor` — only `provider`, `displayName`,
  ...
  agent supports.
- `getProtectedResources()` — same github.com resource the Singularity agent
  declares.
...
- `models` observable — derived from `ISingularityApiService.models()`, filtered
...
- **`sendMessage` attachments and `turnId`:** `IAgent.sendMessage` takes
  ...
  association. Mirror how `singularityAgent.ts:730` handles this.
- Map SDK events → `AgentSignal`s on `onDidSessionProgress`, mirroring
  `mapSessionEvents.ts` in the Singularity folder.
...
Tests: integration test with a stubbed `ISingularityApiService` returning a
...
**Why deferred — structural mismatch.** Singularity's fork path
([`singularityAgent.ts:660-714`](../singularity/singularityAgent.ts)) calls
`sourceEntry.getNextTurnEventId(turnId) → toEventId`, an O(1) primitive the
Singularity SDK provides natively. The Claude SDK has no equivalent: `forkSession`
...
extension's
[`claudeChatSessionContentProvider.ts:341`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/vscode-node/claudeChatSessionContentProvider.ts)
...
[`claudeCodeAgent.ts:569`](../../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeCodeAgent.ts))
...
- Persist the customization-directory metadata via `setMetadata` on the
  ...
  [`singularityAgent.ts`](../singularity/singularityAgent.ts)).
...
**Architectural model:** Singularity's `getNextTurnEventId(turnId)`. Phase 6.5
...
`Query` is still not started until the first `sendMessage`" and Singularity,
...
parallel to Singularity's fork tests (create → N turns → fork at N-1 →
...
- **Server side (provider-specific, the gap).** After a restore the handler
  ...
  provider's optional `IAgent.truncateSession(session, turnId)`. Singularity
  implements it in place via the SDK RPC `history.truncate({ eventId })`
  ([`singularityAgent.ts:~1987`](../singularity/singularityAgent.ts) →
  `singularityAgentSession.truncateAtEventId`), keeping the **same** session id
...
[`sdk.d.ts`](../../../../../../extensions/singularity-chat/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts):
...
**Approach — mirror Singularity's shape, no alias indirection.**
...
1. **Resolve the anchor.** Reuse Phase 6.5's resolver exactly:
   ...
   (`[0..turnId]` INCLUSIVE), matching the workbench and Singularity.
...
3. **Reconcile local state.** Prune our session-data DB turns past the kept
   ...
   `singularityAgentSession.truncateAtEventId`) so future `getNextTurnEventId` /
   anchor lookups don't reference dropped turns. No metadata id rebinding is
   needed — the id is unchanged.
4. **Serialize** the whole operation through `_sessionSequencer` (as Singularity
...
- **Write timing — truncation finalizes on the next turn (by design — DECIDED).**
  ...
  `appendEntry`. (Contrast Singularity's `truncateAtEventId`, which rewrites the
...
`Query.interrupt()`, compaction), the Singularity-vs-Claude asymmetry row, and the
...
**Architectural model:** Singularity's `truncateSession` →
`truncateAtEventId` (CONTEXT M10 §"Singularity: in-place via SDK RPC"). Phase 6.7
is the Claude-side equivalent: where Singularity rewrites the transcript via its
...
test parallel to Singularity's truncate path (create → N turns → truncate at turn K
...
- **ExitPlanMode richer response shape.** Phase 7 ships a simple 2-button
  ...
  [`exitPlanModeHandler.ts`](../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/common/toolPermissionHandlers/exitPlanModeHandler.ts):
  ...
  why?' }` (production extension's exact wording). SingularityAgent already has
  a richer `IExitPlanModeResponse { approved, selectedAction?,
  autoApproveEdits?, feedback? }` contract
  ([singularityAgent.ts:106-123](./singularity/singularityAgent.ts#L106-L123),
  [singularityAgentSession.ts:1439-1518](./singularity/singularityAgentSession.ts#L1439-L1518))
...
Build the Claude analog of `fileEditTracker.ts` from `node/singularity/`.
...
### Phase 8.5 — Rich tool-call rendering parity with Singularity ✅ **DONE**
...
`"Find files"`, ...). Singularity's [`singularityToolDisplay.ts`](../singularity/singularityToolDisplay.ts)
...
- **Port the Singularity helper shape** into
...
Singularity's rendering quality — permission cards show the actual
...
Exit criteria: parity with Singularity agent on stop / steer / switch model.
...
- Reference for the MCP-tool path: `extensions/singularity-chat/src/extension/chatSessions/claude/common/mcpServers/ideMcpServer.ts`
...
split is gone. `SingularityAgent` uses the same pattern but stays as
...
3. **Reference implementation** — singularity's `_handleElicitationRequest`
   ([singularityAgentSession.ts:2239](../singularity/singularityAgentSession.ts)) does the
...
- **Subagent URI scheme** — helpers are in `state/sessionState.ts`:
  ...
  210). See `singularityAgent.ts:34` for the import and `singularityAgent.ts:761`
  for usage.
...
- **`listSessions()` filter** — decide whether subagents appear in the
  top-level session list (Singularity's behavior is the reference).
...
- Telemetry events for proxy request/response counts, model usage, token
  ...
  instructions in `singularity-instructions.md`).
...
`SingularityAgent`, we ship the real `uri` and the client reads content on
...
(the `SingularityAgent` model), not by trusting the SDK query payloads.
...
`SingularityAgent` already scans disk for customizations
...
- New Claude customization **disk-scan resolver** (mirrors
  `SingularityAgent`'s discovery) covering, scoped by the session's `cwd`
...
([`claudeSkills.ts`](../../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeSkills.ts)
...
> keeps using `ISingularityApiService.models()` exactly as today.
...
- **One branch point** in `ClaudeAgent`: `_isProxyEnabled()` returns
  ...
  ClaudeUseSingularityProxy`. Phase 19 flips this to the real
  `IAgentConfigurationService.getRootValue` read.
- **Proxy branch → `ISingularityApiService.models()`** — today's
...
- `ClaudeAgent` proxied model path unchanged: `models` observable
  populated from a stubbed `ISingularityApiService.models()`, filtered to
...
> provider talk to Anthropic **without** the Singularity proxy in the path —
...
Today the Claude provider is *Singularity-routed Claude*: every `/v1/messages`
call is minted against a GitHub Singularity token, translated by
`ClaudeProxyService`, and billed through CAPI. That is the right default
for Singularity subscribers, but it excludes two populations:
...
1. Users who have a **direct Anthropic relationship** (API key or Claude
   ...
   their Singularity entitlement.
2. Users on networks / orgs where the Singularity CAPI path is unavailable but
...
2. **The model picker — build the native branch + flip the seam.**
   - Phase 18 laid only a **seam**: `ClaudeAgent._isProxyEnabled()` hardcoded
     `true`, proxy branch on `ISingularityApiService.models()`, native branch a
   ...
   - **Flip the flag to the real read:** `_isProxyEnabled()` →
     `IAgentConfigurationService.getRootValue(..., ClaudeUseSingularityProxy)`
   ...
   - **No commercial-metadata overlay in native mode** — multiplier /
     policy / default ordering are Singularity/CAPI concepts; native
...
3. **Per-turn cost / "tracking between requests".**
   - *Proxied (today):* the proxy is the **only** place that sees CAPI's
     real billed credits (`singularity_usage.total_nano_aiu`), because the SDK
     ...
     injected into the `ChatUsage` signal as `_meta.singularityUsage` by
     `_enrichSignalWithCredits`.
   - *Native (this phase):* there is no proxy and no `singularity_usage` —
...
4. **Auth gating + protected resources.**
   - *Proxied (today):* `authenticate()` requires the
     `GITHUB_SINGULARITY_PROTECTED_RESOURCE` token; without it
     `_ensureAuthenticated()` throws `AHP_AUTH_REQUIRED` and the proxy is
     never started. `getProtectedResources()` advertises GitHub Singularity +
     GitHub repo.
   - *Native (this phase):* the GitHub Singularity token is **not** required to
     reach Anthropic. The provider must not block session creation behind a
     Singularity sign-in it will never use. Decisions:
     - `getProtectedResources()` in native mode drops
       `GITHUB_SINGULARITY_PROTECTED_RESOURCE` (keep
       ...
       from this list, so dropping it removes the spurious Singularity
       sign-in.
...
- **New `AgentHostConfigKey`** (e.g. `ClaudeUseSingularityProxy =
  'claudeUseSingularityProxy'`) declared in
  [`agentHostCustomizationConfig.ts`](../../common/agentHostCustomizationConfig.ts)
  with a `schemaProperty<boolean>({ ..., default: true })`, **default
  `true`** so existing Singularity users are unaffected. It joins the schema
  ...
  - *Naming open question:* the key is Claude-specific today, but the
    "use the Singularity proxy vs. bring-your-own credentials" axis may
    generalise to Codex later. Decide whether to namespace it
    (`claudeUseSingularityProxy`) or model a more general transport key now.
...
- **`ClaudeAgent` reads it** via
  ...
  AgentHostConfigKey.ClaudeUseSingularityProxy)` (defaulting to `true` when
...
- **No env var, no setting, no starter change.** This deletes the
  `VSCODE_AGENT_HOST_CLAUDE_USE_SINGULARITY_PROXY` /
  `chat.agentHost.claudeAgent.useSingularityProxy` plumbing the earlier draft
  proposed.
- **Scope: host-level for v1.** All sessions on the host share one
  ...
  user could run one chat on Singularity quota and another on their own key)
...
- `ClaudeAgent` resolves the `ClaudeUseSingularityProxy` root value into a
...
- Proxied mode keeps today's behaviour: acquire `IClaudeProxyHandle` in
  `authenticate()`, wire `onDidReportCredits`, advertise the Singularity
...
- Native mode: skip the proxy handle entirely (do **not** call
  ...
  Singularity protected resource, populate models via Phase 18's
...
- `AgentHostConfigKey.ClaudeUseSingularityProxy` RootConfigState property
...
- Transport resolution: `ClaudeUseSingularityProxy` root value `false` →
...
- `ClaudeAgent` native mode: `authenticate` does not call
  `IClaudeProxyService.start`; `getProtectedResources` omits Singularity;
...
- Native `ChatUsage` carries SDK `result` usage and **no**
  `_meta.singularityUsage`; proxied `ChatUsage` still carries it.
...
- With the `claudeUseSingularityProxy` RootConfigState value set to `false`
  ...
  `lsof`).
- Flip the root value back to `true` → Singularity-routed behaviour returns
...
their own Anthropic credentials with no Singularity token, CAPI call, or proxy
...
(credits, Bearer correlation, `ANTHROPIC_BASE_URL` injection, Singularity
...
- **`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` in native mode** — in
  proxied mode it's a Singularity leak-tightness guarantee. In native mode
...
- **Anthropic-direct authentication via the proxy** (`x-api-key` against
  ...
  Singularity-routed only and ignores `x-api-key` by design. Direct Anthropic
...
- **Cross-agent feature parity at every step.** Phases 4–14 catch up to
  `SingularityAgent` incrementally; gaps are acceptable mid-flight.
