...
import {
	SINGULARITY_API_ERROR_STATUS_STREAMING,
	SingularityApiError,
	ISingularityApiService,
	type ISingularityApiServiceRequestOptions,
} from '../shared/singularityApiService.js';
 ...
 *
 * - `proxy`: Singularity-routed Claude (the default). All `messages` traffic goes
 *   through the local {@link IClaudeProxyHandle} → Singularity CAPI.
 ...
 * for a `/v1/messages` request as `singularity_usage.total_nano_aiu` on the
...
export interface IClaudeProxyService {
	 ...
	 * `singularity_usage.total_nano_aiu`. Consumers accumulate per turn to
	 * surface real per-turn Singularity credits (the SDK-computed
 ...
 * billed credits under `singularity_usage.total_nano_aiu`. The published
 * Anthropic SDK types don't declare it, so narrow through this shape
 * (mirrors `messagesApi.ts` in the Singularity extension).
 */
interface ISingularityUsageEnvelope {
	readonly singularity_usage?: { readonly total_nano_aiu?: number };
}

/**
 * Read `singularity_usage.total_nano_aiu` off an Anthropic stream event or
 * message, returning `undefined` unless it is a finite, non-negative
 * number.
 */
function readSingularityUsageNanoAiu(event: unknown): number | undefined {
	const value = (event as ISingularityUsageEnvelope | undefined)?.singularity_usage?.total_nano_aiu;
	return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : undefined;
}

/**
 * Local HTTP proxy that speaks the Anthropic Messages API on the inbound
 * side and {@link ISingularityApiService} on the outbound side. The Claude
...
export class ClaudeProxyService extends LoopbackProxyServer<IClaudeProxyState, string> implements IClaudeProxyService {
	 ...
	 * the request carried no credits (`singularity_usage` absent) or the
	...
	private async _handleModels(req: http.IncomingMessage, res: http.ServerResponse, runtime: IClaudeProxyRuntime): Promise<void> {
		...
		let models: CCAModel[];
		try {
			models = await this._singularityApiService.models(runtime.state.githubToken, { headers, suppressIntegrationId: true });
		} catch (err) {
	...
	private async _sendNonStreamingMessage(
		...
		const options: ISingularityApiServiceRequestOptions = { headers, signal: entry.ac.signal, suppressIntegrationId: true };
		let message: Anthropic.Message;
		try {
			message = await this._singularityApiService.messages(runtime.state.githubToken, body, options);
		} catch (err) {
		...
		this._reportCredits(sessionId, readSingularityUsageNanoAiu(message));
	...
	private async _streamMessages(
		...
		try {
			stream = this._singularityApiService.messages(runtime.state.githubToken, body, options);
		} catch (err) {
		...
		// Tracks the latest `singularity_usage.total_nano_aiu` seen on the
		...
		try {
			if (!first.done) {
				reportedNanoAiu = readSingularityUsageNanoAiu(first.value) ?? reportedNanoAiu;
			...
			while (true) {
				...
				reportedNanoAiu = readSingularityUsageNanoAiu(next.value) ?? reportedNanoAiu;
			...
			// `singularity_usage.total_nano_aiu` seen on the stream
			// (assign-last-wins, matching the Singularity messages client).
	...
	private _writeUpstreamErrorResponse(res: http.ServerResponse, err: unknown, embedChatError = false): void {
		...
		if (err instanceof SingularityApiError) {
			...
			const status = err.status === SINGULARITY_API_ERROR_STATUS_STREAMING ? 502 : err.status;
...
/**
 * Build the headers we forward to {@link ISingularityApiService.messages}
...
/**
 * Returns a copy of a {@link SingularityApiError}'s Anthropic envelope with a
 ...
 */
function embedForwardedChatError(err: SingularityApiError): Anthropic.ErrorResponse {
