...
**Phase 1 (done):** `ISingularityApiService` at
`src/vs/platform/agentHost/node/shared/singularityApiService.ts`. A
gateway to GitHub Singularity's chat completions API ("**CAPI**") that
exposes:

```ts
interface ISingularityApiService {
    readonly _serviceBrand: undefined;
    messages(githubToken: string, body: Anthropic.MessageCreateParamsNonStreaming, options?: ISingularityApiServiceRequestOptions): Promise<Anthropic.Message>;
    messages(githubToken: string, body: Anthropic.MessageCreateParamsStreaming, options?: ISingularityApiServiceRequestOptions): AsyncGenerator<Anthropic.MessageStreamEvent>;
...
`ISingularityApiService` is hand-rolled HTTP (not the `@anthropic-ai/sdk`
...
**Phase 1.5 (done):** widened `ISingularityApiService` to use raw
`Anthropic.MessageStreamEvent` types and accept
`ISingularityApiServiceRequestOptions { headers?, signal? }`.

### What this phase delivers

A `ClaudeProxyService` — a local HTTP server that **speaks the
Anthropic Messages API on the inbound side and `ISingularityApiService`
...
- We don't ship Anthropic API keys; we ship GitHub Singularity tokens.
...
A working version exists in the Singularity **extension** at
`extensions/singularity-chat/src/extension/chatSessions/claude/`. Our proxy
...
1. The reference is a **byte-passthrough** — it pipes upstream SSE
   chunks verbatim. Our `ISingularityApiService.messages()` returns
...
- **Streaming `event: error` SSE frames** — `status` is set to the
  sentinel `SINGULARITY_API_ERROR_STATUS_STREAMING = 520` (the upstream
...
Phase 2's Q10 does an `instanceof SingularityApiError` check and
re-serializes `err.envelope` with `err.status`; non-`SingularityApiError`
...
reference
([claudeLanguageModelServer.ts L348-361](../../../../extensions/singularity-chat/src/extension/chatSessions/claude/node/claudeLanguageModelServer.ts#L348-L361))
...
`ISingularityApiService.messages()` returns
...
`ISingularityApiServiceRequestOptions.signal`. Two distinct abort
...
try {
    const stream = singularityApi.messages(githubToken, body, { signal: entry.ac.signal, headers });
...
- **No polling**. Pass `signal` to `ISingularityApiService.messages()`;
...
- **Non-streaming branch**: same pattern. The awaited
  `ISingularityApiService.messages()` rejects with `AbortError`; we
...
`ISingularityApiService` throws `SingularityApiError` carrying `err.status`
+ `err.envelope` (the parsed envelope, verbatim). We just
re-serialize:

```ts
catch (err) {
    if (err instanceof SingularityApiError) {
...
```ts
const envelope = err instanceof SingularityApiError
    ? err.envelope
...
- [ ] Mid-stream client disconnect → upstream
  `ISingularityApiService.messages()` receives `signal.aborted`;
...
- [ ] CAPI `SingularityApiError` (per §1.5) → re-emitted with original
...
- [ ] Network error (non-`SingularityApiError`) → 502 `api_error`
...
- `anthropicBetas.test.ts` — port the 7 reference fixtures from
  `extensions/singularity-chat/.../test/extractSessionId.spec.ts`
...
`ISingularityApiService`; uses real `http.Server` to hit
`127.0.0.1:<port>`.

`FakeSingularityApiService` is the **only** mock — everything else (real
...
`SingularityApiError` becomes SSE error frame with `err.envelope`;
pre-stream `SingularityApiError` becomes JSON error with `err.status`;
...
0. **Pre-Phase 2 dependency (§1.5)**
   - Widen `ISingularityApiService` to throw `SingularityApiError` (or
...
3. **Routes**
   ...
   - tests at each step
4. **Real CAPI smoke** with the temporary `SingularityAgent` hook
...
- **HTTP proxy support** (`HTTP_PROXY` / `HTTPS_PROXY` env vars,
  ...
  proxy talks to CAPI through `ISingularityApiService`, so any outbound
  ...
  `ISingularityApiService` and a way to source the config from the
...
- **Phase 1 throws plain `Error`, not `Anthropic.APIError`.**
  Resolved by §1.5 (widen Phase 1 to `SingularityApiError`).
- **`ISingularityApiService` methods take `githubToken` as first
  parameter.** Resolved — interface sketch (§1), Q9 pseudocode, and
  `FakeSingularityApiService` (§4) all updated.
