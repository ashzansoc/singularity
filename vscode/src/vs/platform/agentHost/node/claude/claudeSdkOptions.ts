...
export async function buildOptions(
	...
	const settingsEnv: Record<string, string> = {
		...telemetryEnv,
		// Proxied (Singularity-routed) mode points the SDK at the local proxy on a
 ...
 *
 * - **Proxied (Singularity-routed), `true` (default):** a *sparse* env. Credentials
 ...
 *   must not leak to the Singularity proxy (stripped). `PATH` for ripgrep is
 ...
 * strip pattern in `SingularityAgent._ensureClient()`.
 *
...
export function buildSubprocessEnv(proxied: boolean = true): Record<string, string | undefined> {
	...
	// personal ANTHROPIC_API_KEY must not leak to the Singularity proxy.
