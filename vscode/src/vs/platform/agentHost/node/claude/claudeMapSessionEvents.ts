...
function mapResult(
	...
	if (message.subtype === 'success') {
		...
		// estimate, not what CAPI actually bills. Real Singularity credits come
		// from CAPI's `singularity_usage.total_nano_aiu`, which the proxy
		// captures and `ClaudeAgentSession` attaches to this action as
		// `_meta.singularityUsage.totalNanoAiu` (the key the workbench reads).
		signals.push({
			kind: 'action',
			resource: session,
			action: {
	...
	// completing empty. Mirrors the Singularity Chat extension's
 ...
 * the proxy can relay. Mirrors the Singularity Chat extension's
 * `getResultErrorText`.
 */
