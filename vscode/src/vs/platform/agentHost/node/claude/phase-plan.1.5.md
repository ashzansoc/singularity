# Phase 1.5 Implementation Plan — Widen `ISingularityApiService` to Raw Anthropic Events
...
| `src/vs/platform/agentHost/node/shared/singularityApiService.ts` | Drop custom types; rewrite interface + implementation |
| `src/vs/platform/agentHost/test/node/shared/singularityApiService.test.ts` | Migrate 80 existing tests + add ~15 new |
...
2. Drop `ISingularityMessagesRequest`, `ISingularityChatMessage`, `ISingularityMessagesResponse` entirely.
...
4. New `ISingularityApiServiceRequestOptions { headers?, signal? }` replaces the positional `signal` everywhere.
...
**File:** `src/vs/platform/agentHost/node/shared/singularityApiService.ts`
...
- Delete `ISingularityChatMessage` (lines ~18–21)
- Delete `ISingularityMessagesRequest` (lines ~22–38, including doc comment)
- Delete `ISingularityMessagesResponse` (lines ~40–51, including doc comment)

### 1-C. Add `ISingularityApiServiceRequestOptions` in their place
...
Surviving types (`ISingularityTokenEnvelope`, `ICachedToken`, `ICapiInit`) stay untouched.

---

## Step 2 — Rewrite `ISingularityApiService` interface (lines ~142–180)
...
## Step 3 — Rewrite `SingularityApiService` public methods (lines ~199–237)
...
Replace all internal `signal` references with `options?.signal`. Pass `options?.signal` to `_getSingularityToken`.
...
**File:** `src/vs/platform/agentHost/test/node/shared/singularityApiService.test.ts`

### 5-A. Imports — replace `ISingularityChatMessage` with Anthropic SDK type

```typescript
// Remove:
import type { ISingularityChatMessage, ... } from '../../...singularityApiService.js';
// Add:
import Anthropic from '@anthropic-ai/sdk';
import type { ISingularityApiServiceRequestOptions, ISingularityApiService } from '../../...singularityApiService.js';
```
...
### 5-D. `ISingularityChatMessage[]` usage in tests (lines ~99, ~518)

Replace with `Anthropic.MessageParam[]`:

```typescript
// was: const messages: ISingularityChatMessage[] = [...]
...
scripts/test.sh --grep singularityApiService
```
...
|------|-------|-----------|
| `_sendRequest` still cherry-picks fields | `singularityApiService.ts:337–343` | Must spread `...rest` from SDK request; delete old remapping |
