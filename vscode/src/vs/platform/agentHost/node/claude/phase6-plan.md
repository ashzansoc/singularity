...
7. The proxy-backed integration test (real `ClaudeProxyService` + real `@anthropic-ai/claude-agent-sdk` + stubbed `ISingularityApiService`) passes end-to-end against a canned Anthropic stream.
...
Mirrors SingularityAgent's `IProvisionalSession` at [`singularityAgent.ts:67-82`](../singularity/singularityAgent.ts#L67-L82) plus an `AbortController` for the Q8 shutdown-during-materialize race.
 ...
 * `_sessionSequencer` linearizes turn-driving. Mirrors SingularityAgent
 * (`singularityAgent.ts:265`).
 */
...
**New constructor dependency.** `ClaudeAgent`'s Phase-5 constructor at [`claudeAgent.ts:136-141`](claudeAgent.ts#L136-L141) does NOT inject git context. Phase 6 adds `@IAgentHostGitService` (resolved as `private readonly _gitService: IAgentHostGitService`, imported from `'../agentHostGitService.js'`) so `createSession` can call `projectFromSingularityContext(...)` (imported from `'../singularity/singularityGitProject.js'`). Mirrors SingularityAgent at [`singularityAgent.ts:843`](../singularity/singularityAgent.ts#L843). Test fakes use `createNoopGitService()` from `'../../test/common/sessionTestHelpers.js'`.
...
async createSession(config: IAgentCreateSessionConfig = {}): Promise<IAgentCreateSessionResult> {
    ...
    // SingularityAgent (`singularityAgent.ts:732-746`). We deliberately do NOT
    ...
    const project = await projectFromSingularityContext(
...
private async _materializeProvisional(sessionId: string): Promise<ClaudeAgentSession> {
    ...
    // Mirrors SingularityAgent (`singularityAgent.ts:7, 17, 434-450`).
    const settingsEnv = {
    ...
    // race those notifications past durable state. SingularityAgent at
    // `singularityAgent.ts:843-848` awaits `_storeSessionMetadata` before
...
**`Options.env` contract** (sdk.d.ts:1075-1078): "Merged on top of `process.env` — entries here override... Set a key to `undefined` to remove an inherited variable." Mirrors SingularityAgent's strip pattern at [`singularityAgent.ts:434-450`](../singularity/singularityAgent.ts#L434-L450).
...
Mirrors Singularity's `mapSessionEvents.ts`. Pure function that takes one `SDKMessage` plus the `sessionUri`, the active `turnId`, and mutable mapper state, and returns zero or more `AgentSignal`s. Pure-function testability is the reason it's its own module instead of a private method on the session class.
...
Real `ClaudeProxyService` + real `@anthropic-ai/claude-agent-sdk` + stubbed `ISingularityApiService` returning a canned Anthropic stream `[message_start, content_block_start(text), content_block_delta('hello'), content_block_stop, message_delta(usage), message_stop]` followed by terminal SDK messages so `result` arrives.
...
| `process.execPath` in a utility process needs `ELECTRON_RUN_AS_NODE=1`. | `_buildSubprocessEnv()` sets it. Mirror of `singularityAgent.ts:434-450`. Integration test asserts the env value. |
...
**Critical SDK divergence from SingularityAgent**: Claude SDK's `forkSession(sessionId, { upToMessageId, title })` at [sdk.d.ts:540-565](../../../../../../node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts) takes a **message UUID**, not an event id. This is structurally different from SingularityAgent's `getNextTurnEventId(turnId) → toEventId` pattern. Mirroring SingularityAgent's pattern would have been wrong.
...
**Q2: Fork → Phase 6.5.** Claude SDK's `forkSession` API is structurally different from Singularity's (message UUID vs event id). Doing it right requires `sdk.getSessionMessages` lookup. Stacked PR keeps Phase 6 focused.
...
**Q6: Signal emission via shared `Emitter<AgentSignal>`.** Session emits via the agent's emitter (passed in constructor) — not its own. Mirrors SingularityAgent's pattern.
