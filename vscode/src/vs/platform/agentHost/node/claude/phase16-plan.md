...
- **SingularityAgent** behavior — unchanged. If a shared module is factored out, Singularity keeps identical behavior behind its own config.
- **Shipping file content bytes** through the protocol — like SingularityAgent, we ship the editable `uri` only; the client reads content on open.
...
3. ✓ **Discovered → protocol mapper.** Convert discovered entries into `Customization` objects: `DirectoryCustomization` containers (agents/skills/commands) with child `uri` = real source file, plus top-level `McpServerCustomization`. Mirror SingularityAgent's `toDiscoveredChildCustomization` shape ([singularityAgent.ts:2258-2310](../singularity/singularityAgent.ts#L2258)).
...
- **D1 — Claude-specific resolver reusing `pluginParsers.ts` (grill-confirmed).** Reuse `pluginParsers.ts` (the genuinely provider-neutral layer); write Claude's own root-walking + watcher under `node/claude/customizations/`. Do **not** reuse or factor `node/singularity/sessionCustomizationDiscovery.ts` — it's Singularity-convention-coded and couples the providers. Least coupling, no layering risk.
...
- **Layering (`node/claude` → shared/singularity).** If any factoring reaches into `node/singularity`, `valid-layers-check` may complain. *Mitigation:* keep the resolver Claude-local (D1); run `npm run valid-layers-check`.
...
- **Watcher leaks / event storms.** Unbounded re-scan on rapid disk changes. *Mitigation:* debounce + cancelable scan (the Singularity `SessionDiscoveredEntry` pattern); register watcher as a session disposable.
...
- SingularityAgent disk-scan reference: [sessionCustomizationDiscovery.ts](../singularity/sessionCustomizationDiscovery.ts), [singularityAgent.ts](../singularity/singularityAgent.ts) (`toDiscoveredChildCustomization` ~L2258)
