...
export class ClaudeAgentSession extends Disposable {
	...
	/**
	 * Real Singularity credits (in nano-AIU) billed by CAPI for the current
	 ...
	 * Singularity credits overlay (the proxy is the only credit source).
	 */
	...
	/**
	 * Inject the turn's accumulated Singularity credits into its `ChatUsage`
	 * signal as `_meta.singularityUsage.totalNanoAiu` — the well-known key the
		...
		return {
			...
			action: {
				...
				usage: {
					...
					_meta: {
						...usage._meta,
						singularityUsage: { totalNanoAiu: this._currentTurnNanoAiu },
					},
				},
			},
		};
