...
export class AgentHostCommitOperationHandler implements IChangesetOperationHandler {
	...
	private async _invoke(params: InvokeChangesetOperationParams, token: CancellationToken, signal: AbortSignal): Promise<InvokeChangesetOperationResult> {
		...
		if (!authToken) {
			throw new ProtocolError(
				AHP_AUTH_REQUIRED,
				localize('agentHost.changeset.commit.authRequired', "Sign in to GitHub Singularity to generate a commit message."),
				[singularityResource],
			);
		}
		...
		let message: string;
		try {
			message = this._cleanCommitMessage(await this._singularityApiService.utilityChatCompletion(authToken, {
			...
			if (this._isAuthFailure(err)) {
				throw new ProtocolError(
					AHP_AUTH_REQUIRED,
					localize('agentHost.changeset.commit.authExpired', "Authentication is required to generate a commit message. Please sign in to GitHub Singularity and try again."),
					[singularityResource],
				);
			}
			throw err;
	...
	private _isAuthFailure(err: unknown): boolean {
		...
		return /\b(401|403)\b/.test(message)
			&& /\b(auth|authorization|unauthorized|forbidden|token|singularity endpoint discovery|singularity session token mint)\b/i.test(message);
	}
