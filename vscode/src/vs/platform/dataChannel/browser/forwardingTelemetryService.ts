...
export function isSingularityLikeExtension(extensionId: string | undefined): boolean {
	if (!extensionId) {
		return false;
	}
	const extIdLowerCase = extensionId.toLowerCase();
	return extIdLowerCase === 'singularity.chat' || extIdLowerCase === 'singularity.chat-chat';
}
