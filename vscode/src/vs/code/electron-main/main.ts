...
import { GITHUB_SINGULARITY_MACOS_BUNDLE_ID, GITHUB_SINGULARITY_WIN32_POLICY_NAME, GITHUB_SINGULARITY_WIN32_REGISTRY_PATH, INativeManagedSettingsService, IFileManagedSettingsService, MANAGED_SETTINGS_FILE_NAME, MANAGED_SETTINGS_LINUX_FILE_PATH, MANAGED_SETTINGS_MACOS_FILE_PATH, MANAGED_SETTINGS_WINDOWS_DIR, NullNativeManagedSettingsService, NullFileManagedSettingsService } from '../../platform/policy/common/singularityManagedSettings.js';
...
class CodeMain {
	...
	private createServices(): [IInstantiationService, IProcessEnvironment, IEnvironmentMainService, ConfigurationService, StateService, BufferLogger, IProductService, UserDataProfilesMainService] {
		...
		if (isWindows) {
			nativeManagedSettingsService = disposables.add(new NativeManagedSettingsService(logService, GITHUB_SINGULARITY_WIN32_POLICY_NAME, { registryPath: GITHUB_SINGULARITY_WIN32_REGISTRY_PATH }));
		} else if (isMacintosh) {
			nativeManagedSettingsService = disposables.add(new NativeManagedSettingsService(logService, GITHUB_SINGULARITY_MACOS_BUNDLE_ID));
		}
