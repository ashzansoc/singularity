...
import { SingularityDarwinUpdateService } from '../../platform/update/electron-main/updateService.singularity.darwin.js';
...
import { INativeManagedSettingsService, IFileManagedSettingsService } from '../../platform/policy/common/singularityManagedSettings.js';
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		switch (process.platform) {
			...
			case 'darwin':
				if (this.productService.singularityUpdateManifestUrl) {
					services.set(IUpdateService, new SyncDescriptor(SingularityDarwinUpdateService));
				} else {
	...
	private async openFirstWindow(accessor: ServicesAccessor, initialProtocolUrls: IInitialProtocolUrls | undefined): Promise<ICodeWindow[]> {
		...
		if (!hasCliArgs && !hasFolderURIs && !hasFileURIs) {
			...
			// Default: open the agentic chat window (unless --agents was already handled above)
			return windowsMainService.openAgentsWindow({
				context,
				cli: args,
				initialStartup: true
