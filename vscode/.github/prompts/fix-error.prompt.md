...
5. **Monitor the PR — BLOCKING**: You MUST NOT complete the task until the monitoring loop below is done.
   - Wait 2 minutes after each push, then check for Singularity review comments using `gh pr view <number> --json reviews,comments` and `gh api repos/{owner}/{repo}/pulls/{number}/comments`.
