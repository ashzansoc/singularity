...
- `singularity-runtime-logs/process-*.log` — the Singularity runtime (`@github/copilot`
  CLI) process logs, captured by `dumpFailureDiagnostics` when a Singularity-runtime
  ...
  AgentHost) write a full log run at `trace`; Chat Sessions editor (Singularity CLI /
  Claude) and Local sessions write only a minimal startup log here (whether the
  runtime came up), with their detailed diagnostics in `GitHub Singularity Chat.log`.
