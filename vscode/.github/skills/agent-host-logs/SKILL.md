...
description: 'Analyze Agent Host debug log exports. Use when given an ah-logs or ahp-logs zip/folder, an Export Agent Host Debug Logs bundle, events.jsonl, AHP JSONL transport logs, Agent Host.log, remote-agenthost.log, or singularity-logs.'
---
...
```text
Window/client <-> AHP <-> Agent Host process <-> Singularity SDK
```

| Path | What it shows |
|---|---|
| `events.jsonl` | Persisted Singularity SDK events for the selected session: turns, messages, tools, permissions, hooks, skills, and subagents. It can cover a much longer period than the other logs. |
...
| `singularity-logs/*.log` | Singularity SDK process logs that mention the selected session ID. A process log may contain other sessions too. |
...
3. Start with the file closest to the symptom:
   ...
   - SDK behavior: `singularity-logs/*.log`
...
- `events.jsonl`, Singularity SDK logs, and AHP timestamps are normally UTC. The plain `.log` files may use local machine time; remote logs may use another timezone.
- An AHP log is connection-scoped and can contain multiple sessions. A Singularity SDK process log can also contain multiple sessions.
