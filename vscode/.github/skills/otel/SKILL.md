---
name: otel
description: OpenTelemetry instrumentation for the Singularity Chat extension — covers the four agent execution paths, the IOTelService abstraction, span/metric/event conventions, and the relationship between code and the user/developer monitoring docs. Use when adding/changing OTel spans, metrics, or events; instrumenting a new agent surface; touching the Singularity CLI bridge or Claude span emission; or updating `extensions/singularity-chat/docs/monitoring/agent_monitoring*.md`.
---

# OpenTelemetry Instrumentation Skill

When adding, changing, or reviewing OTel telemetry in the Singularity Chat extension, **always read the two source-of-truth docs first** and **always keep them in sync with the code you change**.

## 1. Authoritative Documents

The `extensions/singularity-chat/docs/monitoring/` directory contains the two specs that define the OTel contract for the extension. Treat them like the layout / layer specs in `vs/sessions`.

| Document | Path | Audience | Covers |
|---|---|---|---|
| User-facing | `extensions/singularity-chat/docs/monitoring/agent_monitoring.md` | Extension users | Quick start, settings, env vars, exported spans/metrics/events, backend setup guides |
| Architecture | `extensions/singularity-chat/docs/monitoring/agent_monitoring_arch.md` | Developers | Multi-agent strategies, span hierarchies, file structure, instrumentation points, `IOTelService`, configuration channels |
| Visual flow | `extensions/singularity-chat/docs/monitoring/otel-data-flow.html` | Developers | Renders the bridge data flow for the in-process Singularity CLI agent |
...
| **Singularity CLI in-process** | Extension host (same process) | **Bridge SpanProcessor** — SDK creates spans natively; bridge forwards to debug panel | SDK native spans via bridge |
| **Singularity CLI terminal** | Separate terminal process | Forward OTel env vars | N/A (separate process) |
...
```
extensions/singularity-chat/src/platform/otel/
├── common/
...
│   ├── agentOTelEnv.ts         # deriveSingularityCliOTelEnv / deriveClaudeOTelEnv
...
extensions/singularity-chat/src/extension/
├── chatSessions/
│   ├── singularitycli/node/
│   │   ├── singularityCliBridgeSpanProcessor.ts  # Bridge: SDK spans → IOTelService (+ hook span enrichment)
│   │   ├── singularitycliSession.ts              # Root invoke_agent singularitycli span + traceparent + hook stash
│   │   └── singularitycliSessionService.ts       # Bridge installation + env var setup
│   └── claude/
...
| `singularity.chat.*` | Singularity-specific vendor namespace. | **Preferred — new attributes go here.** |
| `singularity_chat.*` | Original VS Code-only namespace. Several keys remain for backwards compatibility. | **Legacy — keep emitting; do not add new keys here.** |

### Dual-emit rules

- When adding a new attribute that belongs to Singularity's vendor namespace, emit it under `singularity.chat.*` only — do **not** introduce a `singularity_chat.*` twin.
- When **renaming** an existing `singularity_chat.*` attribute to its `singularity.chat.*` equivalent (e.g., `singularity_chat.repo.*` → `singularity.chat.git.*`, `gen_ai.usage.reasoning_tokens` → `gen_ai.usage.reasoning.output_tokens`), **dual-emit both keys indefinitely**. Downstream readers (Agent Debug Log, Chronicle, SQLite span store, OTLP collectors) may depend on the legacy key.
- Mark the legacy row in [agent_monitoring.md](../../../extensions/singularity-chat/docs/monitoring/agent_monitoring.md) with **Legacy** in the "Requirement" column and a pointer to the preferred key. No sunset date — legacy keys live on indefinitely.
- Hash sensitive identifiers (e.g., MCP server names) with `hashTelemetryValue` from [`util/node/crypto.ts`](../../../extensions/singularity-chat/src/util/node/crypto.ts). Emit hashes unconditionally; raw values only when `captureContent` is enabled.

## 4. Service Layer & Selection

`IOTelService` ([otelService.ts](../../../extensions/singularity-chat/src/platform/otel/common/otelService.ts)) is the only abstraction consumers should depend on — never import the OTel SDK directly outside `node/otelServiceImpl.ts`. Three implementations:
...
Selection happens in [`src/extension/extension/vscode-node/services.ts`](../../../extensions/singularity-chat/src/extension/extension/vscode-node/services.ts): exactly one of `NodeOTelService` or `InMemoryOTelService` is bound to `IOTelService` per extension host based on `resolveOTelConfig().enabled`.

## 5. Span / Metric / Event Conventions

Follow the [OTel GenAI semantic conventions](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/gen-ai/). **Always use the constants from [`genAiAttributes.ts`](../../../extensions/singularity-chat/src/platform/otel/common/genAiAttributes.ts) — never raw string literals.**
...
| `singularity_chat.*` | `SingularityChatAttr` | `singularity_chat.session_id`, `singularity_chat.chat_session_id`, `singularity_chat.hook_*` |
| `singularity.chat.*` | `SingularityCliSdkAttr` | SDK-emitted hook attributes (read-only — bridge & debug panel) |
...
- `deriveSingularityCliOTelEnv` / `deriveClaudeOTelEnv` in [`agentOTelEnv.ts`](../../../extensions/singularity-chat/src/platform/otel/common/agentOTelEnv.ts).
...
1. Add the attribute key as a constant to `genAiAttributes.ts` (under `GenAiAttr`, `SingularityChatAttr`, or a new domain group). Never inline a raw `'singularity_chat.foo'` literal.
2. Add it to the public barrel in [`index.ts`](../../../extensions/singularity-chat/src/platform/otel/common/index.ts) if it lives in a new group.
...
### When changing the Singularity CLI bridge

The bridge (`singularityCliBridgeSpanProcessor.ts`) reaches into `_delegate._activeSpanProcessor._spanProcessors` — internal OTel SDK v2 state. This is documented as a known risk. If you touch it:
...
- Add a unit test in `singularityCliBridgeSpanProcessor.spec.ts`.
...
```bash
# From extensions/singularity-chat/
...
- The Agent Debug Log panel in VS Code still shows the full span tree for foreground, Singularity CLI, and Claude sessions.
...
- ❌ Hard-coded attribute keys: `'singularity_chat.hook_type'` instead of `SingularityChatAttr.HOOK_TYPE`.
