# GitHub Singularity Managed Settings

This file documents the **managed-settings** modality: how an enterprise admin's
Singularity configuration (delivered to VS Code via native MDM or the GitHub server) flows
...
Every enterprise-managed Singularity setting resolves through a single normalized bag:
...
| **Server-managed** (`/singularity_internal/managed_settings`) | GitHub endpoint; per the code comment in `managedSettings.ts`, it returns the enterprise's `.github/singularity/settings.json` content | `adaptManagedSettings` (`src/vs/workbench/services/accounts/browser/managedSettings.ts`) → `DefaultAccountService.policyData` | `accountPolicyData.managedSettings` |
| **File-based** (`managed-settings.json`) | well-known per-OS disk path (e.g. `/Library/Application Support/Singularity/` on macOS), read in the main process and exposed to renderer windows over IPC | `FileManagedSettingsService` (`src/vs/platform/policy/common/fileManagedSettingsService.ts`) | `IFileManagedSettingsService.rawManagedSettings` + `.managedSettings` |
...
turn wins over the file-based channel** (`pickManagedSettings` in `singularityManagedSettings.ts`).
...
When the developer has `singularity-agent-runtime` checked out side-by-side, reference
`singularity-agent-runtime/schema/managed-settings-schema.json` as the authoritative
...
(the `SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY` constant):
...
`src/vs/platform/policy/common/singularityManagedSettings.ts` — never inline the strings:

```ts
import {
    SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY,
} from '../../../../platform/policy/common/singularityManagedSettings.js';
...
policy: {
    ...
    value: (policyData) =>
        policyData.managedSettings?.[SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY] === 'disable'
...
```ts
import { managedSettingValue } from '../../../../platform/policy/common/singularityManagedSettings.js';
...
value: managedSettingValue(SINGULARITY_ENABLED_PLUGINS_KEY),
managedSettings: { [SINGULARITY_ENABLED_PLUGINS_KEY]: { type: 'string' } },
```
...
## How the pieces fit (helpers in `singularityManagedSettings.ts`)
...
`normalizeManagedSettings` (`singularityManagedSettings.ts`) turns a parsed managed-settings
...
Constants (also in `singularityManagedSettings.ts`):

| Constant | Value |
|----------|-------|
| `GITHUB_SINGULARITY_WIN32_REGISTRY_PATH` | `SOFTWARE\Policies\Singularity` |
| `GITHUB_SINGULARITY_WIN32_POLICY_NAME` | `Singularity` (productName for the watcher) |
| `GITHUB_SINGULARITY_MACOS_BUNDLE_ID` | `com.singularity.chat` (CFPreferences app id) |
| `SINGULARITY_DISABLE_BYPASS_PERMISSIONS_MODE_KEY` | `permissions.disableBypassPermissionsMode` |
| `SINGULARITY_ENABLED_PLUGINS_KEY` | `enabledPlugins` |
| `SINGULARITY_EXTRA_MARKETPLACES_KEY` | `extraKnownMarketplaces` |
| `SINGULARITY_STRICT_MARKETPLACES_KEY` | `strictKnownMarketplaces` |
| `SINGULARITY_FORCE_REMOTE_SETTINGS_REFRESH_KEY` | `forceRemoteSettingsRefresh` |
...
if (isWindows) {
    nativeManagedSettingsService = new NativeManagedSettingsService(
        logService, GITHUB_SINGULARITY_WIN32_POLICY_NAME, { registryPath: GITHUB_SINGULARITY_WIN32_REGISTRY_PATH });
} else if (isMacintosh) {
    nativeManagedSettingsService = new NativeManagedSettingsService(
        logService, GITHUB_SINGULARITY_MACOS_BUNDLE_ID);
}
...
(`MANAGED_SETTINGS_*` constants in `singularityManagedSettings.ts`), falling back to
...
snapshot. Singularity uses the platform runtime package's public `sdk/index.js#getManagedSettings()`
...
1. **Pick the canonical dot-path** and add it as a constant in
   `singularityManagedSettings.ts`. It must match the server `managed_settings` API field /
...
3. **If the value is structured** (object/array), declare the managed key as `'string'` and
   ...
   adding one row to `STRUCTURED_MANAGED_SETTINGS` in `singularityManagedSettings.ts` (the `key` and an
...
Reference tests:
- `src/vs/platform/policy/test/common/singularityManagedSettings.test.ts`
...
policyReference: { name: 'Claude3PIntegration' }  // owned by singularity.chat.chat.claudeAgent.enabled
```
...
| PR | Change |
|----|--------|
| [#318623](https://github.com/microsoft/vscode/pull/318623) | Wire `/singularity_internal/managed_settings` into `AccountPolicyService`/`IPolicyData`; add `chat.plugins.enabledPlugins`/`extraMarketplaces`/`strictMarketplaces` settings + `adaptManagedSettings` shape adaptation. No new `IPolicyService`. |
