...
Policies allow enterprise administrators to lock configuration settings via OS-level mechanisms (Windows Group Policy, macOS managed preferences, Linux config files) or via Singularity account-level policy data. This skill covers the complete procedure.
...
| **Singularity managed settings (native MDM)** | `NativeManagedSettingsService` via `@vscode/policy-watcher` | Watches `SOFTWARE\Policies\Singularity` (Windows) / `com.singularity.chat` prefs (macOS); feeds the canonical `managedSettings` bag — see [github-managed-settings.md](./github-managed-settings.md) |
| **Singularity managed settings (file)** | `FileManagedSettingsService` | Reads + watches `managed-settings.json` from a well-known per-OS path in the main process, exposed to renderers over IPC; lowest-precedence managed-settings channel — see [github-managed-settings.md](./github-managed-settings.md) |
...
| `src/vs/platform/policy/common/singularityManagedSettings.ts` | Managed-settings key constants + well-known file paths, `collectManagedSettingsDefinitions`, `projectManagedSettings`, the shared `normalizeManagedSettings` (single normalizer for all channels), `pickManagedSettings` (per-key channel precedence), `INativeManagedSettingsService` / `IFileManagedSettingsService` |
| `src/vs/platform/policy/node/nativeManagedSettingsService.ts` | Native MDM watcher (`@vscode/policy-watcher`) for Singularity managed settings |
...
If this policy should also be controllable via Singularity account policy data (from `IPolicyData`), add a `value` function:
...
- `Extensions`
- `IntegratedTerminal`
- `InteractiveSession` (used for all chat/Singularity policies)
- `Telemetry`
- `Update`
...
If your setting is a **GitHub Preview Feature** — meaning it's a Singularity/chat feature that organizations can disable via their GitHub account-level policy — you **must** add a `value` function that checks `policyData.chat_preview_features_enabled`.

### When to add this flag

Add the `chat_preview_features_enabled` check when **all** of these apply:

- The setting controls a Singularity or chat feature (e.g., agent tools, hooks, MCP, auto-approve)
...
The `chat_preview_features_enabled` field on `IPolicyData` (defined in `src/vs/base/common/defaultAccount.ts`) is populated from the user's GitHub Singularity token entitlements. When an organization admin disables preview features, `chat_preview_features_enabled` is set to `false`.
...
GitHub Singularity enterprise admins can lock settings through a **managed-settings** bag.
VS Code feeds the bag from **two** channels: native MDM (Windows registry / macOS plist)
and the GitHub `/singularity_internal/managed_settings` endpoint. (The external
...
For managed-settings-specific examples that combine `policyReference` with Singularity
