...
(vscode.env.uiKind === vscode.UIKind.Web ? suite.skip : suite)('chat - run_in_terminal', () => {
	...
	setup(async () => {
		...
		// The test extension contributes both `test-lm-vendor` and `singularity`; focused
		...
		disposables.push(
			...
			vscode.lm.registerLanguageModelChatProvider('singularity', testLanguageModelProvider),
		);
