...
// TODO: this now became flaky with built-in singularity
suite('chat', () => {
	...
	setup(() => {
		...
		disposables.push(lm.registerLanguageModelChatProvider('singularity', {
	...
	// Agent Host Singularity when the agent host is enabled. `newLocalChat` opens the
	...
	test('can access node-pty module', async function () {
		// Required for singularity cli in chat extension.
