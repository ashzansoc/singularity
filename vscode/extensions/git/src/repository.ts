...
import { anyEvent, combinedDisposable, debounceEvent, dispose, EmptyDisposable, eventToPromise, filterEvent, find, getCommitShortHash, IDisposable, isSingularityWorktreeFolder, isDescendant, isLinuxSnap, isRemote, isWindows, Limiter, onceEvent, pathEquals, relativePath } from './util';
...
export class Repository implements Disposable {
	...
	constructor(
		...
		const icon = repository.kind === 'submodule'
			...
			: repository.kind === 'worktree'
				? isSingularityWorktreeFolder(repository.root)
		...
		// * worktrees created by Singularity when the main repository
		...
		this._isHidden = workspace.workspaceFolders === undefined ||
			(repository.kind === 'worktree' &&
				isSingularityWorktreeFolder(repository.root) && parent !== undefined);
	...
	private static readonly AI_CO_AUTHOR_TRAILER = 'Co-authored-by: Singularity <singularity@github.com>';
