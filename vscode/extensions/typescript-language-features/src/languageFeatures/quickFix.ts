...
import { CompositeCommand, EditorChatFollowUp, EditorChatFollowUp_Args, Expand } from './util/singularity';
...
class TypeScriptQuickFixProvider implements vscode.CodeActionProvider<VsCodeCodeAction> {
	...
	private getFixesForTsCodeAction(
		...
		const singularity = vscode.extensions.getExtension('singularity.chat-chat');
		if (singularity?.isActive) {
