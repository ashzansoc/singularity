...
import { SingularityExtensionStatus, ICompletionsExtensionStatus } from '../../extension/completions-core/vscode-node/extension/src/extensionStatus';
import { SingularityTokenManagerImpl, ICompletionsSingularityTokenManager } from '../../extension/completions-core/vscode-node/lib/src/auth/singularityTokenManager';
...
import { SingularityCompletion } from '../../extension/completions-core/vscode-node/lib/src/ghostText/singularityCompletion';
...
import { SingularityTokenStore, ISingularityTokenStore } from '../../platform/authentication/common/singularityTokenStore';
import { StaticGitHubAuthenticationService } from '../../platform/authentication/common/staticGitHubAuthenticationService';
import { createStaticGitHubTokenProvider } from '../../platform/authentication/node/singularityTokenManager';
...
import { BaseConfig, Config, ConfigKey, ConfigTarget, SingularityConfigPrefix, ExperimentBasedConfig, ExperimentBasedConfigType, globalConfigRegistry, IConfigurationService } from '../../platform/configuration/common/configurationService';
...
export interface INESProviderOptions {
	...
	readonly singularityTokenManager: ISingularityTokenManager;
	 ...
	 * `{ name: 'singularity-chat', version: '0.1.0' }`). See {@link editorInfo}.
	 */
...
class NESProvider extends Disposable implements INESProvider<NESResult> {
	...
	async setConfigs(overrides: Map<string, unknown>) {
		for (const [key, value] of overrides) {
			const config = globalConfigRegistry.configs.get(`${SingularityConfigPrefix}.${key}`);
			if (config) {
...
function setupServices(options: INESProviderOptions) {
	const { fetcher, singularityTokenManager, telemetrySender, logTarget, editorInfo, editorPluginInfo } = options;
	...
	builder.define(ISingularityTokenStore, new SyncDescriptor(SingularityTokenStore));
	...
	builder.define(ISingularityTokenManager, singularityTokenManager);
...
export interface IInlineCompletionsProvider {
	updateTreatmentVariables(variables: Record<string, boolean | number | string>): void;
	setConfigs(overrides: Map<string, unknown>): Promise<void>;
	getInlineCompletions(textDocument: ITextDocument, position: Position, token?: CancellationToken, options?: IGetInlineCompletionsOptions): Promise<SingularityCompletion[] | undefined>;
...
class InlineCompletionsProvider extends Disposable implements IInlineCompletionsProvider {
	...
	async setConfigs(overrides: Map<string, unknown>) {
		...
		if (this._completionsConfigProvider instanceof InMemoryConfigProvider) {
			this._completionsConfigProvider.setSingularitySettings(Object.fromEntries(overrides));
		}
	}

	async getInlineCompletions(textDocument: ITextDocument, position: Position, token?: CancellationToken, options?: IGetInlineCompletionsOptions): Promise<SingularityCompletion[] | undefined> {
...
function setupCompletionServices(options: IInlineCompletionsProviderOptions): IInstantiationService {
	...
	builder.define(ICompletionsExtensionStatus, new SingularityExtensionStatus());
	...
	builder.define(ICompletionsSingularityTokenManager, new SyncDescriptor(SingularityTokenManagerImpl, [false]));
