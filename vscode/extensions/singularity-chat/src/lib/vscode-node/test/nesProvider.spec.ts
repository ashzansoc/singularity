...
class TestSingularityTokenManager implements ISingularityTokenManager {
	_serviceBrand: undefined;

	onDidSingularityTokenRefresh = new Emitter<void>().event;

	async getSingularityToken(force?: boolean): Promise<SingularityToken> {
		return new SingularityToken(createTestExtendedTokenInfo({ token: 'fixedToken' }));
	}
...
describe('NESProvider Facade', () => {
	it('should handle getNextEdit call with a document URI', async () => {
		...
		const nextEditProvider = createNESProvider({
			workspace,
			fetcher,
			singularityTokenManager: new TestSingularityTokenManager(),
			telemetrySender,
			terminalService,
			logTarget,
		...
		nextEditProvider.updateTreatmentVariables({
			'config.singularity.chat.chat.advanced.inlineEdits.xtabProvider.defaultModelConfigurationString': '{ "modelName": "xtab-test", "promptingStrategy": "singularityNesXtab", "includeTagsInCurrentFile": false }',
		});
		...
		const event = telemetrySender.events.find(e => e.eventName === 'singularity-nes/provideInlineEdit');
	...
	describe('languageDiagnosticsService injection', () => {
		...
		async function runNESRequest(diagnosticsService: ILanguageDiagnosticsService | undefined): Promise<{ payload: string }> {
			...
			nextEditProvider.updateTreatmentVariables({
				'config.singularity.chat.chat.advanced.inlineEdits.xtabProvider.defaultModelConfigurationString': JSON.stringify({
					modelName: 'xtab-test',
					promptingStrategy: 'singularityNesXtab',
