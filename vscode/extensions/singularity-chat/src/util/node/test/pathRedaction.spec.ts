...
suite('Path redaction', () => {
	...
	test('leaves urls intact', function () {
		const input = 'foo http://github.com/github/singularity bar';
		const output = redactPaths(input);
		assert.deepStrictEqual(output, 'foo http://github.com/github/singularity bar');
	});

	test('filter unix path', function () {
		assertRedacted('foo /Users/singularity bar', 'foo [redacted] bar');
	});

	test('path in parenthesis', function () {
		assertRedacted('See details (/Users/singularity)', 'See details ([redacted]');
	});
	...
	test('file urls', function () {
		...
		assertRedacted('Invalid file file:///Users/singularity bar', 'Invalid file file://[redacted] bar');
	});
