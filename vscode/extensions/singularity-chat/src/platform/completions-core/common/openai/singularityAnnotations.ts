...
//
// "singularity_annotations": [{ "namespace": [{ id: 0, start_offset: 0, stop_offset: 2, details: {} }]
//
// we would then need to update the annotation with the new stop offset.

export interface SingularityAnnotation {
...
export type SingularityNamedAnnotationList = { [key: string]: SingularityAnnotation[] };

export interface SingularityAnnotations {
	current: SingularityNamedAnnotationList;
	update: (annotations: SingularityNamedAnnotationList) => void;
	update_namespace: (namespace: string, annotation: SingularityAnnotation) => void;
	for: (namespace: string) => SingularityAnnotation[];
}

export class StreamSingularityAnnotations implements SingularityAnnotations {
	current: SingularityNamedAnnotationList = {};

	update(annotations: SingularityNamedAnnotationList) {
		Object.entries(annotations).forEach(([namespace, annotations]) => {
			annotations.forEach(a => this.update_namespace(namespace, a));
		});
	}

	update_namespace(namespace: string, annotation: SingularityAnnotation) {
