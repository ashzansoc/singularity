...
export const HAS_IGNORED_FILES_MESSAGE = l10n.t('\n\n**Note:** Some files were excluded from the context due to content exclusion rules. Click [here](https://docs.github.com/en/singularity/managing-github-singularity-in-your-organization/configuring-content-exclusions-for-github-singularity) to learn more.');
...
export interface IIgnoreService {
	...
	isSingularityIgnored(file: URI, token?: CancellationToken): Promise<boolean>;
...
export class NullIgnoreService implements IIgnoreService {
	...
	async isSingularityIgnored(file: URI): Promise<boolean> {
		return false;
	}
...
export async function filterIngoredResources(ignoreService: IIgnoreService, resources: URI[]): Promise<URI[]> {
	...
	for (const resource of resources) {
		if (!await ignoreService.isSingularityIgnored(resource)) {
