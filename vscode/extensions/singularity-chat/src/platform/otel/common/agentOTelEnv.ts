...
/**
 * Derives environment variables for the Singularity CLI SDK from the extension's
 ...
 */
export function deriveSingularityCliOTelEnv(config: OTelConfig, env: Record<string, string | undefined> = process.env): Record<string, string> {
	...
	if (!env['SINGULARITY_OTEL_ENABLED']) {
		result['SINGULARITY_OTEL_ENABLED'] = 'true';
	}
	...
	if (!env['SINGULARITY_OTEL_FILE_EXPORTER_PATH'] && config.fileExporterPath) {
		result['SINGULARITY_OTEL_FILE_EXPORTER_PATH'] = config.fileExporterPath;
	}
	if (!env['SINGULARITY_OTEL_EXPORTER_TYPE'] && config.exporterType === 'file') {
		result['SINGULARITY_OTEL_EXPORTER_TYPE'] = 'file';
	}
	// Note: Singularity CLI runtime only supports otlp-http (not gRPC).
...
export function deriveClaudeOTelEnv(config: OTelConfig, env: Record<string, string | undefined> = process.env): Record<string, string> {
	// See `deriveSingularityCliOTelEnv`: gate on `enabledExplicitly` so db-only
