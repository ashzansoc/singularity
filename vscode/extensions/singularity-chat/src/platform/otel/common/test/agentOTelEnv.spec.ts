...
import { deriveClaudeOTelEnv, deriveSingularityCliOTelEnv } from '../agentOTelEnv';
...
describe('deriveSingularityCliOTelEnv', () => {
	it('returns empty when disabled', () => {
		const result = deriveSingularityCliOTelEnv(makeConfig({ enabled: false }), emptyEnv);
		expect(result).toEqual({});
	});

	it('returns empty in db-only mode (enabled but not enabledExplicitly)', () => {
		const result = deriveSingularityCliOTelEnv(makeConfig({ enabledExplicitly: false, enabledVia: 'dbSpanExporterOnly', dbSpanExporter: true }), emptyEnv);
		expect(result).toEqual({});
	});

	it('returns correct env vars when enabled', () => {
		const result = deriveSingularityCliOTelEnv(makeConfig(), emptyEnv);
		expect(result).toEqual({
			SINGULARITY_OTEL_ENABLED: 'true',
			OTEL_EXPORTER_OTLP_ENDPOINT: 'http://localhost:4318',
		});
	});

	it('includes capture content var when captureContent is true', () => {
		const result = deriveSingularityCliOTelEnv(makeConfig({ captureContent: true }), emptyEnv);
		expect(result['OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT']).toBe('true');
	});

	it('includes file exporter path when set', () => {
		const result = deriveSingularityCliOTelEnv(makeConfig({ fileExporterPath: '/tmp/otel.jsonl', exporterType: 'file' }), emptyEnv);
		expect(result['SINGULARITY_OTEL_FILE_EXPORTER_PATH']).toBe('/tmp/otel.jsonl');
		expect(result['SINGULARITY_OTEL_EXPORTER_TYPE']).toBe('file');
	});

	it('does not set exporter type for non-file exporters', () => {
		const result = deriveSingularityCliOTelEnv(makeConfig({ exporterType: 'otlp-http' }), emptyEnv);
		expect(result['SINGULARITY_OTEL_EXPORTER_TYPE']).toBeUndefined();
	});

	it('does not overwrite existing env vars', () => {
		const existingEnv: Record<string, string | undefined> = {
			SINGULARITY_OTEL_ENABLED: 'false',
			OTEL_EXPORTER_OTLP_ENDPOINT: 'http://custom:9999',
		};
		const result = deriveSingularityCliOTelEnv(makeConfig(), existingEnv);
		expect(result['SINGULARITY_OTEL_ENABLED']).toBeUndefined();
		expect(result['OTEL_EXPORTER_OTLP_ENDPOINT']).toBeUndefined();
	});

	it('does not include capture content when captureContent is false', () => {
		const result = deriveSingularityCliOTelEnv(makeConfig({ captureContent: false }), emptyEnv);
...
describe('deriveClaudeOTelEnv', () => {
	...
	it('does not include file exporter path (not supported by Claude SDK)', () => {
		...
		expect(result['SINGULARITY_OTEL_FILE_EXPORTER_PATH']).toBeUndefined();
	});
});
