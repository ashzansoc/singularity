...
import { SingularityChatAttr, SingularityAttr } from '../genAiAttributes';
...
describe('workspaceMetadataToOTelAttributes', () => {
	it('maps all fields to correct OTel attribute keys', () => {
		...
		expect(attrs[SingularityChatAttr.FILE_RELATIVE_PATH]).toBe('src/index.ts');
	});

	it('dual-emits singularity.chat.git.* alongside singularity_chat.repo.* and derives github.org', () => {
		const attrs = workspaceMetadataToOTelAttributes({
			headBranchName: 'feature/x',
			headCommitHash: 'cafef00d',
			remoteUrl: 'https://github.com/microsoft/vscode.git',
		});
		expect(attrs[SingularityAttr.GIT_BRANCH]).toBe('feature/x');
		expect(attrs[SingularityAttr.GIT_COMMIT_SHA]).toBe('cafef00d');
		expect(attrs[SingularityAttr.GIT_REPOSITORY]).toBe('https://github.com/microsoft/vscode.git');
		expect(attrs[SingularityAttr.GITHUB_ORG]).toBe('microsoft');
	...
	it('omits github.org for non-github remotes', () => {
		...
		expect(attrs[SingularityAttr.GIT_REPOSITORY]).toBe('https://gitlab.com/org/repo.git');
		expect(attrs[SingularityAttr.GITHUB_ORG]).toBeUndefined();
	});
});
