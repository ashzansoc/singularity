...
import { SingularityChatAttr, GenAiAttr, GenAiOperationName, GenAiProviderName } from '../genAiAttributes';
...
describe('Agent Trace Hierarchy', () => {
	it('produces invoke_agent, chat, and execute_tool spans with correct attributes', async () => {
		...
		expect(agentSpan.name).toBe('invoke_agent singularity');
		expect(agentSpan.kind).toBe(SpanKind.INTERNAL);
		expect(agentSpan.attributes[GenAiAttr.OPERATION_NAME]).toBe('invoke_agent');
		expect(agentSpan.attributes[GenAiAttr.AGENT_NAME]).toBe('singularity');
	...
	it('emits session start event and agent metrics', async () => {
		...
		emitSessionStartEvent(otel, 'sess-abc', 'gpt-4o', 'singularity');
		GenAiMetrics.incrementSessionCount(otel);
		GenAiMetrics.recordAgentDuration(otel, 'singularity', 15.2);
		GenAiMetrics.recordAgentTurnCount(otel, 'singularity', 4);
		emitAgentTurnEvent(otel, 0, 1500, 250, 2);

		// Session event
		expect(otel.logRecords).toHaveLength(2); // session.start + agent.turn
		expect(otel.logRecords[0].attributes?.['event.name']).toBe('singularity_chat.session.start');

		// Agent turn event
		expect(otel.logRecords[1].attributes?.['event.name']).toBe('singularity_chat.agent.turn');
		expect(otel.logRecords[1].attributes?.['turn.index']).toBe(0);

		// Metrics
		expect(otel.counters).toHaveLength(1);
		expect(otel.counters[0].name).toBe('singularity_chat.session.count');
		expect(otel.metrics).toHaveLength(2);
		expect(otel.metrics[0].name).toBe('singularity_chat.agent.invocation.duration');
		expect(otel.metrics[1].name).toBe('singularity_chat.agent.turn.count');
	});
	...
	it('records edit acceptance and survival metrics', () => {
		...
		expect(otel.counters[0].name).toBe('singularity_chat.edit.acceptance.count');
		expect(otel.counters[0].attributes?.[SingularityChatAttr.EDIT_SOURCE]).toBe('inline_chat');
		expect(otel.counters[0].attributes?.[SingularityChatAttr.EDIT_OUTCOME]).toBe('accepted');
		expect(otel.counters[0].attributes?.[SingularityChatAttr.LANGUAGE_ID]).toBe('typescript');

		expect(otel.counters[1].name).toBe('singularity_chat.edit.acceptance.count');
		expect(otel.counters[1].attributes?.[SingularityChatAttr.EDIT_OUTCOME]).toBe('rejected');

		// Chat edit outcome counter
		expect(otel.counters[2].name).toBe('singularity_chat.chat_edit.outcome.count');
		expect(otel.counters[2].attributes?.[SingularityChatAttr.EDIT_SOURCE]).toBe('chat_editing');
		expect(otel.counters[2].attributes?.[SingularityChatAttr.EDIT_OUTCOME]).toBe('accepted');
		expect(otel.counters[2].attributes?.[SingularityChatAttr.HAS_REMAINING_EDITS]).toBe(false);

		// Survival histograms
		expect(otel.metrics).toHaveLength(2);
		expect(otel.metrics[0].name).toBe('singularity_chat.edit.survival.four_gram');
		expect(otel.metrics[0].value).toBe(0.85);
		expect(otel.metrics[0].attributes?.[SingularityChatAttr.EDIT_SOURCE]).toBe('inline_chat');
		expect(otel.metrics[0].attributes?.[SingularityChatAttr.TIME_DELAY_MS]).toBe(30000);

		expect(otel.metrics[1].name).toBe('singularity_chat.edit.survival.no_revert');
	...
	it('omits optional attributes when undefined', () => {
		...
		expect(otel.counters[0].attributes?.[SingularityChatAttr.LANGUAGE_ID]).toBeUndefined();
		expect(otel.counters[1].attributes?.[SingularityChatAttr.LANGUAGE_ID]).toBeUndefined();
		expect(otel.counters[1].attributes?.[SingularityChatAttr.HAS_REMAINING_EDITS]).toBeUndefined();
	});
});
