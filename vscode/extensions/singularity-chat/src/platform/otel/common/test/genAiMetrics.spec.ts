...
import { GenAiAttr, GenAiOperationName, GenAiProviderName, GenAiTokenType, SingularityAttr, StdAttr } from '../genAiAttributes';
...
describe('GenAiMetrics', () => {
	it('recordOperationDuration calls recordMetric with correct attributes', () => {
		...
		GenAiMetrics.recordOperationDuration(otel, 1.5, {
			...
			serverAddress: 'api.singularity.com',
			errorType: 'timeout',
		});
		...
		expect(otel.recordMetric).toHaveBeenCalledWith('gen_ai.client.operation.duration', 1.5, {
			...
			[StdAttr.SERVER_ADDRESS]: 'api.singularity.com',
	...
	it('recordToolCallCount increments counter', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('singularity_chat.tool.call.count', 1, {
	...
	it('recordToolCallDuration records histogram', () => {
		...
		expect(otel.recordMetric).toHaveBeenCalledWith('singularity_chat.tool.call.duration', 500, {
	...
	it('recordAgentDuration records histogram', () => {
		...
		expect(otel.recordMetric).toHaveBeenCalledWith('singularity_chat.agent.invocation.duration', 15.2, {
	...
	it('incrementSessionCount increments counter', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('singularity_chat.session.count');
	});
	...
	it('recordCloudOperation records count and duration tagged with backend version', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('singularity_chat.cloud.operation.count', 1, {
			operation: 'createSession',
			[SingularityAttr.CLOUD_BACKEND_VERSION]: 'v2',
			success: true,
		});
		expect(otel.recordMetric).toHaveBeenCalledWith('singularity_chat.cloud.operation.duration', 1200, {
			operation: 'createSession',
			[SingularityAttr.CLOUD_BACKEND_VERSION]: 'v2',
		});
	});
	...
	it('recordCloudOperation omits the duration histogram when no duration is provided', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('singularity_chat.cloud.operation.count', 1, {
			operation: 'followUp',
			[SingularityAttr.CLOUD_BACKEND_VERSION]: 'v1',
			success: false,
		});
	...
	it('incrementCloudError increments the guardrail counter with operation, backend version and error type', () => {
		...
		expect(otel.incrementCounter).toHaveBeenCalledWith('singularity_chat.cloud.error.count', 1, {
	...
	it('cloud session and pr_ready counters tag the backend version', () => {
		...
		GenAiMetrics.incrementCloudSessionCount(otel, 'singularity', 'v2');
		GenAiMetrics.incrementCloudPrReadyCount(otel, 'v1');

		expect(otel.incrementCounter).toHaveBeenCalledWith('singularity_chat.cloud.session.count', 1, {
			partner_agent: 'singularity',
			[SingularityAttr.CLOUD_BACKEND_VERSION]: 'v2',
		});
		expect(otel.incrementCounter).toHaveBeenCalledWith('singularity_chat.cloud.pr_ready.count', 1, {
			[SingularityAttr.CLOUD_BACKEND_VERSION]: 'v1',
		});
	});
});
