 ...
 * Emits both the legacy `singularity_chat.repo.*` namespace and the canonical
 * `singularity.chat.git.*` namespace.
 */
export function workspaceMetadataToOTelAttributes(
	...
	if (metadata.headBranchName) {
		attrs[SingularityChatAttr.REPO_HEAD_BRANCH_NAME] = metadata.headBranchName;
		attrs[SingularityAttr.GIT_BRANCH] = metadata.headBranchName;
	}
	if (metadata.headCommitHash) {
		attrs[SingularityChatAttr.REPO_HEAD_COMMIT_HASH] = metadata.headCommitHash;
		attrs[SingularityAttr.GIT_COMMIT_SHA] = metadata.headCommitHash;
	}
	if (metadata.remoteUrl) {
		attrs[SingularityChatAttr.REPO_REMOTE_URL] = metadata.remoteUrl;
		attrs[SingularityAttr.GIT_REPOSITORY] = metadata.remoteUrl;
		const org = extractGitHubOrg(metadata.remoteUrl);
		if (org) {
			attrs[SingularityAttr.GITHUB_ORG] = org;
		}
	}
	if (metadata.fileRelativePath) {
		attrs[SingularityChatAttr.FILE_RELATIVE_PATH] = metadata.fileRelativePath;
	}
	return attrs;
}
