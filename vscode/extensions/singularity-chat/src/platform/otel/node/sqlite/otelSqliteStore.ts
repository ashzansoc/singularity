...
import { SingularityChatAttr, GenAiAttr } from '../../common/genAiAttributes';
...
const DENORMALIZED_ATTRS: Record<string, string> = {
	...
	chat_session_id: SingularityChatAttr.CHAT_SESSION_ID,
	turn_index: SingularityChatAttr.TURN_INDEX,
	ttft_ms: SingularityChatAttr.TIME_TO_FIRST_TOKEN,
};
 ...
 *
 * Patterns from singularity-agent-runtime SessionStore:
...
export class OTelSqliteStore {
	...
	/**
	 * Coalesce TTFT from foreground extension (`singularity_chat.time_to_first_token`, ms)
	 * and CLI runtime. The CLI runtime historically emitted `singularity.chat.time_to_first_chunk`
	...
	private _ttftMs(span: ICompletedSpanData): number | null {
		const foreground = this._attr(span, SingularityChatAttr.TIME_TO_FIRST_TOKEN);
		if (foreground !== null) { return foreground as number; }
		const cli = span.attributes['gen_ai.response.time_to_first_chunk']
			?? span.attributes['singularity.chat.time_to_first_chunk'];
