...
/**
 * Produces structured `singularity.chat.tool.parameters.*` attributes for shell,
...
export function extractToolParameters(toolName: string, input: unknown): ToolParameterAttributes {
	...
	if (SHELL_TOOL_NAMES.has(toolName)) {
		...
		if (command) {
			gatedAttrs[SingularityAttr.TOOL_PARAM_COMMAND] =
	...
	if (FILE_TOOL_NAMES.has(toolName)) {
		...
		if (filePath) {
			gatedAttrs[SingularityAttr.TOOL_PARAM_FILE_PATH] = filePath;
		}
		const editType = classifyEditType(toolName, obj);
		if (editType) {
			attrs[SingularityAttr.TOOL_PARAM_EDIT_TYPE] = editType;
		}
	}

	const skillName = pickFirstString(obj, ['skill_name', 'skillName', 'skill']);
	if (skillName) {
		attrs[SingularityAttr.TOOL_PARAM_SKILL_NAME] = skillName;
	}
	...
	if (toolName.startsWith('mcp__')) {
		...
		if (sep > 0) {
			...
			attrs[SingularityAttr.TOOL_PARAM_MCP_SERVER_NAME_HASH] = hashTelemetryValue(serverName);
			attrs[SingularityAttr.TOOL_PARAM_MCP_TOOL_NAME] = mcpToolName;
			gatedAttrs[SingularityAttr.TOOL_PARAM_MCP_SERVER_NAME] = serverName;
		}
