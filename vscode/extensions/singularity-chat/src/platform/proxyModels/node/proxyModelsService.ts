...
export class ProxyModelsService extends Disposable implements IProxyModelsService {
	...
	constructor(
		...
		const singularityTokenObs = observableFromEvent(this, this._tokenStore.onDidStoreUpdate, () => this._tokenStore.singularityToken);

		this._register(autorun(reader => {
			const singularityToken = singularityTokenObs.read(reader);
			const cts = new CancellationTokenSource();
			this._fetchLatestModels(singularityToken, cts.token).then(models => {
				if (models === undefined) {
					return;
				}
	...
	private async _fetchLatestModels(singularityToken: SingularityToken | undefined, token: CancellationToken): Promise<WireTypes.ModelList.t | undefined> {
		if (!singularityToken) {
			return undefined;
		}
