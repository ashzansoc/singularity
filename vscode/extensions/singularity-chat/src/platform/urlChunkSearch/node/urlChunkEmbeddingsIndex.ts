...
export class UrlChunkEmbeddingsIndex extends Disposable {
	...
	async findInUrls(
		...
		try {
			...
			// Never fail fetch_webpage because ranking/auth (Singularity token backoff, CAPI 403, etc.) failed.
			this._logService.info(`urlChunkEmbeddingsIndex: Ranking unavailable (${e instanceof Error ? e.message : String(e)}); returning unranked content.`);
			return undefined;
		}
	}
