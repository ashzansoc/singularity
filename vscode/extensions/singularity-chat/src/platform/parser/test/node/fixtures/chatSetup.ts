...
import { ChatViewId, IChatWidgetService, showSingularityView } from './chat.js';
...
class SetupChatAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatMode | undefined, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { disposable: IDisposable; agent: SetupChatAgent } {
		return instantiationService.invokeFunction(accessor => {
			...
			let id: string;
			let description = localize('chatDescription', "Ask Singularity");
			switch (location) {
			...
			return SetupChatAgent.registerAgents(instantiationService, chatAgentService, id, `${defaultChat.providerName} Singularity`, true, description, location, mode, context, controller);
		});
	}
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpSingularityNeeded', "You need to set up Singularity to use Chat."));
	...
	private async doInvokeWithoutSetup(request: IChatAgentRequest, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService, languageModelToolsService: ILanguageModelToolsService): Promise<IChatAgentResult> {
		...
		progress({
			...
			content: new MarkdownString(localize('waitingSingularity', "Getting Singularity ready.")),
		});

		await this.forwardRequestToSingularity(requestModel, progress, chatService, languageModelsService, chatAgentService, chatWidgetService, languageModelToolsService);

		return {};
	}

	private async forwardRequestToSingularity(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		try {
			await this.doForwardRequestToSingularity(requestModel, progress, chatService, languageModelsService, chatAgentService, chatWidgetService, languageModelToolsService);
		} catch (error) {
			progress({
				kind: 'warning',
				content: new MarkdownString(localize('singularityUnavailableWarning', "Singularity failed to get a response. Please try again."))
			});
		}
	}

	private async doForwardRequestToSingularity(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		if (this.pendingForwardedRequests.has(requestModel.session.sessionId)) {
			throw new Error('Request already in progress');
		}

		const forwardRequest = this.doForwardRequestToSingularityWhenReady(requestModel, progress, chatService, languageModelsService, chatAgentService, chatWidgetService, languageModelToolsService);
	...
	private async doForwardRequestToSingularityWhenReady(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		...
		// Singularity. Waiting for the registration of the agent is not
		...
		if (whenLanguageModelReady instanceof Promise || whenAgentReady instanceof Promise || whenToolsModelReady instanceof Promise) {
			const timeoutHandle = setTimeout(() => {
				progress({
					kind: 'progressMessage',
					content: new MarkdownString(localize('waitingSingularity2', "Singularity is almost ready.")),
				});
			}, 10000);
				...
				if (ready === 'error' || ready === 'timedout') {
					progress({
						...
						content: new MarkdownString(ready === 'timedout' ?
							localize('singularityTookLongWarning', "Singularity took too long to get ready. Please review the guidance in the Chat view.") :
							localize('singularityFailedWarning', "Singularity failed to get ready. Please review the guidance in the Chat view.")
						)
					});

					// This means Singularity is unhealthy and we cannot retry the
		...
		const setupListener = Event.runAndSubscribe(this.controller.value.onDidChange, (() => {
			switch (this.controller.value.step) {
				...
				case ChatSetupStep.Installing:
					progress({
						kind: 'progressMessage',
						content: new MarkdownString(localize('installingSingularity', "Getting Singularity ready.")),
					});
					break;
			}
		}));
		...
		if (typeof result?.success === 'boolean') {
			if (result.success) {
				if (result.dialogSkipped) {
					progress({
						kind: 'progressMessage',
						content: new MarkdownString(localize('singularitySetupSuccess', "Singularity setup finished successfully."))
					});
				} else if (requestModel) {
					// Replace agent part with the actual Singularity agent
					let newRequest = this.replaceAgentInRequestModel(requestModel, chatAgentService);
					// Then replace any tool parts with the actual Singularity tools
					newRequest = this.replaceToolInRequestModel(newRequest);
					await this.forwardRequestToSingularity(newRequest, progress, chatService, languageModelsService, chatAgentService, chatWidgetService, languageModelToolsService);
				}
			} else {
				progress({
					kind: 'warning',
					content: new MarkdownString(localize('singularitySetupError', "Singularity setup failed."))
				});
			}
		}
	...
	private replaceToolInRequestModel(requestModel: IChatRequestModel): IChatRequestModel {
		...
		const toolId = toolPart.toolId.replace(/setup.tools\./, `singularity_`.toLowerCase());
...
class ChatSetup {
	...
	private getPrimaryButton(): string {
		...
		return localize('useSingularityButton', "Use Singularity");
	}

	private getDialogTitle(): string {
		if (this.context.state.entitlement === ChatEntitlement.Unknown) {
			return this.context.state.registered ? localize('signUp', "Sign in to use Singularity") : localize('signUpFree', "Sign in to use Singularity for free");
		}

		if (this.context.state.entitlement === ChatEntitlement.Pro) {
			return localize('singularityProTitle', "Start using Singularity Pro");
		}

		return this.context.state.registered ? localize('singularityTitle', "Start using Singularity") : localize('singularityFreeTitle', "Start using Singularity for free");
	}
	...
	private createDialog(disposables: DisposableStore): HTMLElement {
		...
		// Header
		const header = localize({ key: 'headerDialog', comment: ['{Locked="[Singularity]({0})"}'] }, "[Singularity]({0}) is your AI pair programmer. Write code faster with completions, fix bugs and build new features across multiple files, and learn about your codebase through chat.", defaultChat.documentationUrl);
		...
		if (this.telemetryService.telemetryLevel !== TelemetryLevel.NONE) {
			const settings = localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({0})"}', '{Locked="]({1})"}'] }, "Singularity Free and Pro may show [public code]({0}) suggestions and we may use your data for product improvement. You can change these [settings]({1}) at any time.", defaultChat.publicCodeMatchesUrl, defaultChat.manageSettingsUrl);
...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
				...
				if (!defaultAgentDisposables.value) {
					...
					for (const mode of [ChatMode.Ask, ChatMode.Edit, ChatMode.Agent]) {
						...
						panelAgentDisposables.add(agent.onUnresolvableError(() => {
							...
							// Singularity is unhealthy for some reason. We clear our panel
							// registration to give Singularity a chance to show a custom message
							// to the user from the views and stop pretending as if there was
							// a functional agent.
							this.logService.error('[chat setup] Unresolvable error from Singularity agent registration, clearing registration.');
	...
	private registerActions(context: ChatEntitlementContext, requests: ChatEntitlementRequests, controller: Lazy<ChatSetupController>): void {
		...
		const CHAT_SETUP_ACTION_LABEL = localize2('triggerChatSetup', "Use AI Features with Singularity for free...");
		...
		class ChatSetupTriggerAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor, mode: ChatMode): Promise<void> {
				...
				const chatWidgetPromise = showSingularityView(viewsService, layoutService);
				if (mode) {
				...
				if (success === false && !lifecycleService.willShutdown) {
					const { confirmed } = await dialogService.confirm({
						type: Severity.Error,
						message: localize('setupErrorDialog', "Singularity setup failed. Would you like to try again?"),
		...
		class ChatSetupTriggerWithoutDialogAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const chatWidget = await showSingularityView(viewsService, layoutService);
				ChatSetup.getInstance(instantiationService, context, controller).skipDialog();
				chatWidget?.acceptInput(localize('setupSingularity', "Set up Singularity."));
			}
		}
		...
		class ChatSetupFromAccountsAction extends Action2 {
			...
			constructor() {
				super({
					...
					title: localize2('triggerChatSetupFromAccounts', "Sign in to use Singularity..."),
					menu: {
		...
		class ChatSetupHideAction extends Action2 {
			...
			static readonly TITLE = localize2('hideChatSetup', "Hide Singularity");
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const { confirmed } = await dialogService.confirm({
					message: localize('hideChatSetupConfirm', "Are you sure you want to hide Singularity?"),
					detail: localize('hideChatSetupDetail', "You can restore Singularity by running the '{0}' command.", CHAT_SETUP_ACTION_LABEL.value),
					primaryButton: localize('hideChatSetupButton', "Hide Singularity")
				});
		...
		class UpgradePlanAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('managePlan', "Upgrade to Singularity Pro"),
...
class ChatSetupController extends Disposable {
	...
	async setup(options?: { forceSignIn?: boolean }): Promise<boolean> {
		...
		const title = localize('setupChatProgress', "Getting Singularity ready...");
	...
	private async doSetup(options: { forceSignIn?: boolean }, watch: StopWatch): Promise<boolean> {
		...
		try {
			...
			const trusted = await this.workspaceTrustRequestService.requestWorkspaceTrust({
				message: localize('singularityWorkspaceTrust', "Singularity is currently only supported in trusted workspaces.")
			});
			if (!trusted) {
	...
	private async signIn(providerId: string): Promise<{ session: AuthenticationSession | undefined; entitlement: ChatEntitlement | undefined }> {
		...
		if (!session && !this.lifecycleService.willShutdown) {
			const { confirmed } = await this.dialogService.confirm({
				...
				detail: localize('unknownSignInErrorDetail', "You must be signed in to use Singularity."),
				primaryButton: localize('retry', "Retry")
			});
	...
	private async install(session: AuthenticationSession | undefined, entitlement: ChatEntitlement, providerId: string, watch: StopWatch): Promise<boolean> {
		...
		try {
			...
			if (
				entitlement !== ChatEntitlement.Limited &&	// User is not signed up to Singularity Free
				entitlement !== ChatEntitlement.Pro &&		// User is not signed up to Singularity Pro
				entitlement !== ChatEntitlement.Unavailable	// User is eligible for Singularity Free
			) {
				if (!session) {
					try {
	...
	private async doInstall(): Promise<void> {
		...
		if (error) {
			if (!this.lifecycleService.willShutdown) {
				const { confirmed } = await this.dialogService.confirm({
					type: Severity.Error,
					message: localize('unknownSetupError', "An error occurred while setting up Singularity. Would you like to try again?"),
