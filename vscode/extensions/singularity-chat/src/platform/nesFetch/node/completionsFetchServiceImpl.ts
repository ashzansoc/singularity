...
export class CompletionsFetchService implements ICompletionsFetchService {
	...
	protected async _fetchFromUrl(url: string, options: Completions.Internal.FetchOptions, ct: CancellationToken): Promise<Result<FetchResponse, Completions.CompletionsFetchFailure>> {
		...
		try {
			...
			if (response.status === 200 && this.authService.singularityToken?.isFreeUser && this.authService.singularityToken?.isChatQuotaExceeded) {
				this.authService.resetSingularityToken();
			}
			...
			if (response.status !== 200) {
				if (response.status === 402) {
					...
					if (!this.authService.singularityToken?.isCompletionsQuotaExceeded) {
						this.authService.resetSingularityToken(response.status);
						await this.authService.getSingularityToken();
					}
				}
