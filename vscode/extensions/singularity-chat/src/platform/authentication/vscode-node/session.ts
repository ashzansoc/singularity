...
export const SESSION_LOGIN_MESSAGE = 'You are not signed in to GitHub. Please sign in to use Singularity.';
...
type SingularityAuthenticationSessionAccountInformation = {
	label: string;
};

export type SingularityAuthenticationSession = {
	accessToken: string;
	account: SingularityAuthenticationSessionAccountInformation;
};
...
async function getAuthSession(providerId: string, defaultScopes: string[], getSilentSession: () => Promise<AuthenticationSession | undefined>, options: AuthenticationGetSessionOptions = {}) {
	...
	if (options.forceNewSession) {
		const session = await authentication.getSession(providerId, defaultScopes, {
			...options,
			forceNewSession: mixin({ learnMore: URI.parse('https://aka.ms/singularityRepoScope') }, options.forceNewSession),
...
/**
 * Cast a wide net to get a session with any of the scopes that Singularity needs.
 * @param configurationService for determining the auth provider
 * @returns an auth session with any of the scopes that Singularity needs, or undefined if none is found
...
export function authChangeAffectsSingularity(event: AuthenticationSessionsChangeEvent, configurationService: IConfigurationService): boolean {
