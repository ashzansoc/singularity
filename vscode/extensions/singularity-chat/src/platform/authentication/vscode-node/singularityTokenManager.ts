...
import { SingularityToken, ExtendedTokenInfo, TokenErrorNotificationId, TokenInfoOrError } from '../common/singularityToken';
import { ErrorNoTelemetry } from '../../../util/vs/base/common/errors';
import { nowSeconds } from '../common/singularityTokenManager';
import { BaseSingularityTokenManager } from '../node/singularityTokenManager';
...
export class VSCodeSingularityTokenManager extends BaseSingularityTokenManager {
	private _taskSingler = new TaskSingler<TokenInfoOrError>();
	/** When Singularity Auto is on, stop burning 10–20s per Singularity CAPI 403. */
	private _singularityTokenBackoffUntil = 0;
	...
	async getSingularityToken(force?: boolean): Promise<SingularityToken> {
		const singularityEnabled = this.configurationService.getConfig(ConfigKey.Advanced.SingularityRouterEnabled);
		if (singularityEnabled && !force && Date.now() < this._singularityTokenBackoffUntil) {
			throw new Error('Failed to get singularity token. reason: ParseFailed');
		}
		...
		if (!this.singularityToken || this.singularityToken.expires_at - (60 * 5 /* 5min */) < nowSeconds() || force) {
			try {
				this._logService.debug(`Getting SingularityToken (force: ${force})...`);
				this.singularityToken = await this._authShowWarnings();
				this._logService.debug(`Got SingularityToken (force: ${force}).`);
				this._singularityTokenBackoffUntil = 0;
			} catch (e) {
				this._logService.debug(`Getting SingularityToken (force: ${force}) threw error: ${e}`);
				this.singularityToken = undefined;
				if (singularityEnabled) {
					this._singularityTokenBackoffUntil = Date.now() + 5 * 60_000;
					this._logService.warn('[SingularityToken] Singularity Auto: backing off Singularity CAPI retries for 5m');
				}
				throw e;
			}
		}
	...
	private async _auth(): Promise<TokenInfoOrError> {
		...
		if (session) {
			...
			if (tokenResult.kind === 'success') {
				this._logService.info(`Got Singularity token for ${session.account.label}`);
				this._logService.info(`Singularity AI: ${this._envService.getVersion()}, VS Code: ${this._envService.vscodeVersion}`);
			}
			return tokenResult;
		} else {
			...
			if (tokenResult.kind === 'success') {
				this._logService.info(`Got Singularity token for devDeviceId`);
				this._logService.info(`Singularity AI: ${this._envService.getVersion()}, VS Code: ${this._envService.vscodeVersion}`);
			} else {
				this._logService.warn('GitHub login failed');
				return { kind: 'failure', reason: 'GitHubLoginFailed' };
	...
	private async _authShowWarnings(): Promise<ExtendedTokenInfo> {
		...
		if (tokenResult.kind === 'failure' && tokenResult.reason === 'NotAuthorized') {
			...
			switch (tokenResult.notification_id) {
				...
				case TokenErrorNotificationId.NoSingularityAccess:
		...
		if (tokenResult.kind === 'failure') {
			throw Error('Failed to get singularity token. reason: ' + tokenResult.reason);
		}
	...
	private sendTokenResultErrorTelemetry(tokenResult: TokenInfoOrError): void {
		...
		/* __GDPR__
			"singularityTokenFetching.error" : {
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('singularityTokenFetching.error', {
