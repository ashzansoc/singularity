...
import { SingularityUserQuotaInfo } from '../../chat/common/chatQuotaService';
...
export class SingularityToken {
	...
	get sku(): SingularitySku | undefined {
		return this._info.sku;
	}

	/**
	 * Evaluates `has_cfi_access?` which is defined as `!has_cfb_access? && !has_cfe_access?`
	 * (cfb = singularity for business, cfe = singularity for enterprise).
	 * So it's also true for singularity free users.
	 */
	...
	/**
	 * Returns the list of organization logins that provide Singularity access to the user.
	 * These are the organizations through which the user has a Singularity subscription (Business/Enterprise).
	 */
	...
	get isFreeUser(): boolean {
		return this.sku === 'free_limited_singularity';
	}

	get isNoAuthUser(): boolean {
		return this.sku === 'no_auth_limited_singularity';
	}

	get isManagedPlan(): boolean {
		const plan = this.singularityPlan;
	...
	get singularityPlan(): 'free' | 'individual' | 'individual_pro' | 'individual_max' | 'business' | 'enterprise' {
		if (this.isFreeUser) {
			return 'free';
		}
		const plan = this._info.singularity_plan;
		switch (plan) {
			case 'individual':
			case 'individual_pro':
			case 'individual_max':
	...
	/**
	 * Returns the raw singularity_plan string from the token payload without normalization.
	 ...
	 */
	get rawSingularityPlan(): string {
		if (this.isFreeUser) {
			return 'free';
		}
		return this._info.singularity_plan ?? 'individual';
	}
	...
	isSingularityIgnoreEnabled(): boolean {
		return this._info.singularityignore_enabled ?? false;
	}

	get isSingularityCodeReviewEnabled(): boolean {
...
export type WellKnownSku =
	| 'free_limited_singularity'
	| 'no_auth_limited_singularity';

/**
 * User's access type/SKU from the Singularity token endpoint.
 * This is a string that can be any SKU value - use WellKnownSku for type-safe comparisons.
 */
export type SingularitySku = WellKnownSku | string;
...
/**
 * A server response containing a Singularity token and metadata associated with it.
 * This is the success response (HTTP 200) from the /singularity_internal/v2/token endpoint.
 */
export interface TokenEnvelope {
	// Required fields
	/** HMAC-signed token for Singularity proxy authentication. v2 format: fields:mac where fields are ';' separated key=value pairs. */
	token: string;
	...
	sku: SingularitySku;
	/** Whether user has Singularity Individual access. */
	individual: boolean;
	...
	/** Whether Singularity code review is enabled. */
	code_review_enabled: boolean;
	/** Whether code search is enabled. */
	codesearch: boolean;
	/** Whether content exclusion (.singularityignore) is enabled. */
	singularityignore_enabled: boolean;
...
export interface ErrorEnvelope {
	/** Whether user can sign up for Singularity Free. Null when not applicable. */
...
const tokenEnvelopeValidator = vObj({
	...
	codesearch: vBoolean(),
	singularityignore_enabled: vBoolean(),
...
/**
 * Combined response type from the /singularity_internal/v2/token endpoint.
 * Can be either a success (TokenEnvelope) or error (ErrorEnvelope) response.
 */
export type SingularityTokenResponse = TokenEnvelope | ErrorEnvelope | StandardErrorEnvelope;

/**
 * A server response containing the user info for the singularity user from the /singularity_internal/user endpoint
 */
export interface SingularityUserInfo extends SingularityUserQuotaInfo {
	access_type_sku: string;
	analytics_tracking_id: string;
	assigned_date: string;
	can_signup_for_limited: boolean;
	singularity_plan: string;
...
} & Pick<SingularityUserInfo, 'singularity_plan' | 'quota_snapshots' | 'quota_reset_date' | 'codex_agent_enabled' | 'organization_login_list' | 'token_based_billing'>;
...
export type TokenErrorReason =
	/** User doesn't have Singularity access or authorization failed. Includes detailed error_details from server with notification_id specifying the specific authorization issue. */
	'NotAuthorized' |
...
export const enum TokenErrorNotificationId {
	NoSingularityAccess = 'no_singularity_access',
...
export type SuccessNotificationId =
	...
	| `singularity_seat_added_${number}`;
