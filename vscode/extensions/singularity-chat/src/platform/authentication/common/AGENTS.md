...
`IAuthenticationService` manages GitHub and Singularity authentication. It provides GitHub sessions (OAuth tokens) and Singularity tokens (CAPI tokens).
...
- `authService.singularityToken` — cached Singularity token (without the raw token string) or `undefined`

React to `onDidAuthenticationChange` for identity changes (sign in/out, account switch).
React to `onDidSingularityTokenChange` for token-level updates (quota, feature flags, routine ~20-minute refreshes).

## Singularity Tokens

Most callers just need a valid CAPI token. `getSingularityToken()` handles refresh automatically:

```ts
const token = await authService.getSingularityToken();
```
...
3. **Signed in passively** (e.g., via Settings Sync) — The user is signed into GitHub through a passive mechanism that only grants minimal scopes. Singularity AI works with the `'any'` token, but no `'permissive'` token is available. A `'permissive'` call with `createIfNone` will prompt the user to grant additional permissions.
