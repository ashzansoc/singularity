...
import { SingularityToken, createTestExtendedTokenInfo, isErrorEnvelope, isStandardErrorEnvelope, isTokenEnvelope, validateTokenEnvelope } from '../../common/singularityToken';
import { BaseSingularityTokenManager, SingularityTokenManagerFromGitHubToken } from '../../node/singularityTokenManager';

// This is a fake version of SingularityTokenManagerFromGitHubToken.
class RefreshFakeSingularityTokenManager extends BaseSingularityTokenManager {
	calls = 0;
	constructor(
	...
	async getSingularityToken(force?: boolean): Promise<SingularityToken> {
		...
		if (!force && this.singularityToken) {
			return new SingularityToken(this.singularityToken);
		}
		this.singularityToken = createTestExtendedTokenInfo({ token: 'done', username: 'fake', singularity_plan: 'unknown' });
		return new SingularityToken(this.singularityToken);
	}
}

describe('Singularity token unit tests', function () {
	...
	it('includes editor information in token request', async function () {
		...
		const tokenManager = disposables.add(accessor.get(IInstantiationService).createInstance(RefreshFakeSingularityTokenManager, 1));
	...
	it(`notifies about token on token retrieval`, async function () {
		const tokenManager = disposables.add(accessor.get(IInstantiationService).createInstance(RefreshFakeSingularityTokenManager, 3));
		const deferredTokenPromise = new DeferredPromise<SingularityToken>();
		tokenManager.onDidSingularityTokenRefresh(async () => {
			const notifiedValue = await tokenManager.getSingularityToken();
			deferredTokenPromise.complete(notifiedValue);
		});
		await tokenManager.getSingularityToken(true);
	...
	it('invalid GitHub token', async function () {
		const fetcher = new StaticFetcherService({
			...
			message: 'You do not have access to Singularity',
			error_details: {
		...
		const tokenManager = accessor.get(IInstantiationService).createInstance(SingularityTokenManagerFromGitHubToken, 'invalid', 'invalid-user');
		const result = await tokenManager.checkSingularityToken();
	...
	it('network request failed', async function () {
		...
		const tokenManager = accessor.get(IInstantiationService).createInstance(SingularityTokenManagerFromGitHubToken, 'valid', 'valid-user');
		const result = await tokenManager.checkSingularityToken();
		expect(result).toEqual({
			kind: 'failure',
	...
	it('properly propagates errors', async function () {
		...
		const tokenManager = accessor.get(IInstantiationService).createInstance(SingularityTokenManagerFromGitHubToken, 'invalid', 'invalid-user');
		try {
			await tokenManager.checkSingularityToken();
		} catch (err: any) {
	...
	it('ignore v1 token', async function () {
		...
		const singularityToken = new SingularityToken(createTestExtendedTokenInfo({ token, username: 'fake', singularity_plan: 'unknown' }));
		expect(singularityToken.getTokenValue('tid')).toBeUndefined();
	});
	...
	it('parsing v2 token', async function () {
		...
		const singularityToken = new SingularityToken(createTestExtendedTokenInfo({ token, username: 'fake', singularity_plan: 'unknown' }));
		expect(singularityToken.getTokenValue('tid')).toBe('0123456789abcdef0123456789abcdef');
	});
	...
	it('parsing v2 token, multiple values', async function () {
		...
		const singularityToken = new SingularityToken(createTestExtendedTokenInfo({ token, username: 'fake', singularity_plan: 'unknown' }));
		expect(singularityToken.getTokenValue('rt')).toBe('1');
		expect(singularityToken.getTokenValue('ssc')).toBe('0');
		expect(singularityToken.getTokenValue('foo')).toBeUndefined();
	});
	...
	it('HTTP 401 unauthorized', async function () {
		...
		const tokenManager = accessor.get(IInstantiationService).createInstance(SingularityTokenManagerFromGitHubToken, 'bad-token', 'bad-user');
		const result = await tokenManager.checkSingularityToken();
		expect(result).toEqual({
			kind: 'failure',
			reason: 'HTTP401',
...
describe('Token envelope validators', function () {
	it('isTokenEnvelope returns true for valid token', function () {
		const validToken = {
			...
			refresh_in: 300,
			sku: 'free_limited_singularity',
			individual: true,
			blackbird_clientside_indexing: false,
			code_quote_enabled: false,
			code_review_enabled: false,
			codesearch: false,
			singularityignore_enabled: false,
	...
	it('isErrorEnvelope returns true for valid error envelope', function () {
		const validError = {
			...
			error_details: {
				...
				notification_id: 'no_singularity_access',
				title: 'No Access',
				url: 'https://github.com/settings/singularity',
			},
		};
	...
	describe('validateTokenEnvelope', function () {
		it('returns strict strategy for fully valid token envelope', function () {
			...
			if (result.strategy === 'strict') {
				...
				expect(result.envelope.sku).toBe('free_limited_singularity');
			}
		});
...
describe('SingularityToken class', function () {
	it('isFreeUser returns true for free_limited_singularity sku', function () {
		const token = new SingularityToken(createTestExtendedTokenInfo({ sku: 'free_limited_singularity' }));
		expect(token.isFreeUser).toBe(true);
		expect(token.isNoAuthUser).toBe(false);
	});

	it('isNoAuthUser returns true for no_auth_limited_singularity sku', function () {
		const token = new SingularityToken(createTestExtendedTokenInfo({ sku: 'no_auth_limited_singularity' }));
		expect(token.isFreeUser).toBe(false);
		expect(token.isNoAuthUser).toBe(true);
	});

	it('isTelemetryEnabled reflects token state', function () {
		const enabledToken = new SingularityToken(createTestExtendedTokenInfo({ telemetry: 'enabled' }));
		const disabledToken = new SingularityToken(createTestExtendedTokenInfo({ telemetry: 'disabled' }));
	...
	it('isPublicSuggestionsEnabled reflects token state', function () {
		const enabledToken = new SingularityToken(createTestExtendedTokenInfo({ public_suggestions: 'enabled' }));
		const disabledToken = new SingularityToken(createTestExtendedTokenInfo({ public_suggestions: 'disabled' }));
		const unconfiguredToken = new SingularityToken(createTestExtendedTokenInfo({ public_suggestions: 'unconfigured' }));
	...
	it('singularityPlan returns correct plan type', function () {
		const freeToken = new SingularityToken(createTestExtendedTokenInfo({ sku: 'free_limited_singularity', singularity_plan: 'free' }));
		const individualToken = new SingularityToken(createTestExtendedTokenInfo({ sku: 'singularity_individual', singularity_plan: 'individual' }));
		const businessToken = new SingularityToken(createTestExtendedTokenInfo({ sku: 'singularity_business', singularity_plan: 'business' }));
		const enterpriseToken = new SingularityToken(createTestExtendedTokenInfo({ sku: 'singularity_enterprise', singularity_plan: 'enterprise' }));

		expect(freeToken.singularityPlan).toBe('free');
		expect(individualToken.singularityPlan).toBe('individual');
		expect(businessToken.singularityPlan).toBe('business');
		expect(enterpriseToken.singularityPlan).toBe('enterprise');
	});

	it('isChatQuotaExceeded for free users with zero quota', function () {
		const exceededToken = new SingularityToken(createTestExtendedTokenInfo({
			sku: 'free_limited_singularity',
			limited_user_quotas: { chat: 0, completions: 10 }
		}));
		const notExceededToken = new SingularityToken(createTestExtendedTokenInfo({
			sku: 'free_limited_singularity',
			limited_user_quotas: { chat: 5, completions: 10 }
		}));
		const nonFreeToken = new SingularityToken(createTestExtendedTokenInfo({
			sku: 'singularity_individual',
	...
	it('isInternal detects GitHub and Microsoft organizations', function () {
		const githubOrgToken = new SingularityToken(createTestExtendedTokenInfo({
			organization_list: ['4535c7beffc844b46bb1ed4aa04d759a']
		}));
		const microsoftOrgToken = new SingularityToken(createTestExtendedTokenInfo({
			organization_list: ['a5db0bcaae94032fe715fb34a5e4bce2']
		}));
		const externalToken = new SingularityToken(createTestExtendedTokenInfo({
			organization_list: ['some-other-org']
		}));
		const noOrgToken = new SingularityToken(createTestExtendedTokenInfo({
			organization_list: []
		}));
	...
	it('codeQuoteEnabled reflects token state', function () {
		const enabledToken = new SingularityToken(createTestExtendedTokenInfo({ code_quote_enabled: true }));
		const disabledToken = new SingularityToken(createTestExtendedTokenInfo({ code_quote_enabled: false }));
		expect(enabledToken.codeQuoteEnabled).toBe(true);
		expect(disabledToken.codeQuoteEnabled).toBe(false);
	});

	it('isSingularityCodeReviewEnabled reflects token state', function () {
		const enabledToken = new SingularityToken(createTestExtendedTokenInfo({ code_review_enabled: true }));
		const disabledToken = new SingularityToken(createTestExtendedTokenInfo({ code_review_enabled: false }));
		expect(enabledToken.isSingularityCodeReviewEnabled).toBe(true);
		expect(disabledToken.isSingularityCodeReviewEnabled).toBe(false);
	});

	it('isExpandedClientSideIndexingEnabled reflects token state', function () {
		const enabledToken = new SingularityToken(createTestExtendedTokenInfo({ blackbird_clientside_indexing: true }));
		const disabledToken = new SingularityToken(createTestExtendedTokenInfo({ blackbird_clientside_indexing: false }));
	...
	it('isBlackbirdExternalIndexingEnabled returns false when flag is absent', function () {
		const token = new SingularityToken(createTestExtendedTokenInfo({ token: 'tid=test' }));
	...
	it('isBlackbirdExternalIndexingEnabled returns false when flag is 0', function () {
		const token = new SingularityToken(createTestExtendedTokenInfo({ token: 'blackbird_external_indexing=0;tid=test' }));
		expect(token.isBlackbirdExternalIndexingEnabled()).toBe(false);
	});

	it('isBlackbirdExternalIndexingEnabled returns true when flag is 1', function () {
		const token = new SingularityToken(createTestExtendedTokenInfo({ token: 'blackbird_external_indexing=1;tid=test' }));
...
class StaticFetcherService implements IFetcherService {
	...
	async fetch(url: string, options: FetchOptions): Promise<Response> {
		...
		if (url.endsWith('singularity_internal/v2/token')) {
		...
		} else if (url.endsWith('singularity_internal/notification')) {
