...
export async function makeSearchGraphQLRequest(
	...
	const query = `
		query FetchSingularityAgentPullRequests($searchQuery: String!, $first: Int!, $after: String) {
	...
	logService.debug(`[GitHubAPI] FetchSingularityAgentPullRequests: host=${host}, searchQuery=${searchQuery}, resultCount=${nodes.length}, errors=${JSON.stringify(result?.errors)}`);
	return nodes;
}
