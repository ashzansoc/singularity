...
/**
 * Result of checking if Singularity cloud agent is enabled for a repository.
 */
...
export interface CustomAgentListOptions {
	target?: 'github-singularity' | 'vscode';
...
export interface IOctoKitService {
	...
	/**
	 * Returns the list of Singularity pull requests for a given user on a specific repo.
	...
	/**
	 * Returns the list of Singularity sessions for a given pull request.
	 ...
	 */
	getSingularitySessionsForPR(prId: string, authOptions: AuthOptions): Promise<SessionInfo[]>;

	/**
	 * Returns the logs for a specific Singularity session.
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 */
	getSessionLogs(sessionId: string, authOptions: AuthOptions): Promise<string>;

	/**
	 * Returns the information for a specific Singularity session.
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 */
	getSessionInfo(sessionId: string, authOptions: AuthOptions): Promise<SessionInfo | undefined>;

	/**
	 * Posts a new Singularity agent job.
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 */
	postSingularityAgentJob(
		owner: string,
		name: string,
		apiVersion: string,
	...
	/**
	 * Gets all open Singularity sessions.
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 */
	...
	/**
	 * Gets the list of available models for the Singularity coding agent.
	 * Returns an empty array if the user doesn't have access to the model picker
	 * (e.g., Singularity Business or Enterprise users before rollout).
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
	 * @returns An array of available models. The first model is always 'Auto' and should be the default.
	 */
	getSingularityAgentModels(authOptions: AuthOptions): Promise<CCAModel[]>;

	/**
	 * Gets the list of assignable actors (users/bots) for a repository.
	 * This is used to check if partner agents like Singularity are available for assignment.
	 * @param owner The repository owner
	 * @param repo The repository name
	 * @param authOptions - Authentication options. By default, uses silent auth and throws {@link PermissiveAuthRequiredError} if not authenticated.
