...
export class OctoKitService extends BaseOctoKitService implements IOctoKitService {
	...
	async getSingularitySessionsForPR(prId: string, authOptions: AuthOptions): Promise<SessionInfo[]> {
		try {
			const authToken = (await this._getPermissiveSession(authOptions))?.accessToken;
			if (!authToken) {
				this._logService.trace('No authentication token available for getSingularitySessionsForPR');
			...
			}, { type: RequestType.SingularitySessions, prId });
			if (!response.ok) {
				throw new Error(`Failed to fetch singularity sessions for PR ${prId}: ${response.statusText}`);
			}
	...
	async getSessionLogs(sessionId: string, authOptions: AuthOptions): Promise<string> {
		try {
			...
			}, { type: RequestType.SingularitySessionLogs, sessionId });
			if (!response.ok) {
	...
	async getSessionInfo(sessionId: string, authOptions: AuthOptions): Promise<SessionInfo | undefined> {
		try {
			...
			}, { type: RequestType.SingularitySessionDetails, sessionId });
			if (!response.ok) {
	...
	async postSingularityAgentJob(owner: string, name: string, apiVersion: string, payload: RemoteAgentJobPayload, authOptions: AuthOptions): Promise<RemoteAgentJobResponse | ErrorResponseWithStatusCode | undefined> {
		try {
			const authToken = (await this._getPermissiveSession(authOptions))?.accessToken;
			if (!authToken) {
				this._logService.trace('No authentication token available for postSingularityAgentJob');
				throw new PermissiveAuthRequiredError();
			}
			const response = await this._capiClientService.makeRequest<Response>({
				method: 'POST',
			...
			}, { type: RequestType.SingularityAgentJob, owner, repo: name, apiVersion, payload });
			if (!response.ok) {
				return {
	...
	async getJobByJobId(owner: string, repo: string, jobId: string, userAgent: string, authOptions: AuthOptions): Promise<JobInfo | undefined> {
		try {
			...
			}, { type: RequestType.SingularityAgentJob, owner, repo, jobId });
			if (!response.ok) {
	...
	async getJobBySessionId(owner: string, repo: string, sessionId: string, userAgent: string, authOptions: AuthOptions): Promise<JobInfo | undefined> {
		try {
			...
			}, { type: RequestType.SingularityAgentJob, owner, repo, sessionId });
			if (!response.ok) {
	...
	async getAllSessions(nwo: string | undefined, open: boolean, authOptions: AuthOptions): Promise<SessionInfo[]> {
		try {
			...
			}, { type: RequestType.SingularitySessions, nwo, resourceState: open ? 'draft,open' : undefined });
	...
	async getCustomAgents(owner: string, repo: string, options: CustomAgentListOptions, authOptions: AuthOptions): Promise<CustomAgentListItem[]> {
		try {
			...
			const response = await this._capiClientService.makeRequest<Response>({
				...
				type: RequestType.SingularityCustomAgents,
				owner,
				repo,
	...
	async getCustomAgentDetails(owner: string, repo: string, agentName: string, version: string, authOptions: AuthOptions): Promise<CustomAgentDetails | undefined> {
		try {
			...
			}, { type: RequestType.SingularityCustomAgentsDetail, owner, repo, version, customAgentName: agentName });
	...
	async getSingularityAgentModels(authOptions: AuthOptions): Promise<CCAModel[]> {
		try {
			const authToken = (await this._getPermissiveSession(authOptions))?.accessToken;
			if (!authToken) {
				this._logService.trace('No authentication token available for getSingularityAgentModels');
				throw new PermissiveAuthRequiredError();
			}
			const response = await this._capiClientService.makeRequest<Response>({
				method: 'GET',
			...
			if (!response.ok) {
				this._logService.trace(`Failed to fetch Singularity agent models: ${response.statusText}`);
				return [];
			}
			...
			if (e instanceof PermissiveAuthRequiredError) {
				this._logService.trace('[getSingularityAgentModels] No permissive GitHub session');
			} else {
				this._logService.error(e);
			}
			return [];
	...
	async isCCAEnabled(owner: string, repo: string, authOptions: AuthOptions): Promise<CCAEnabledResult> {
		try {
			...
			}, { type: RequestType.SingularityAgentJobEnabled, owner, repo });
