...
import { SingularityChatAttr, GenAiAttr, GenAiOperationName, IOTelService } from '../../otel/common';
...
function isPromptOrInstructionsFile(uri: URI): boolean {
	...
	if (filename === 'singularity-instructions.md') {
		return true;
	}
...
export class AutomaticInstructionsCollector implements IAutomaticInstructionsCollector {
	...
	async collect({ sessionResource, references, tools, modeInstructions2 }: IAutomaticInstructionsCollectorContext, token: CancellationToken): Promise<readonly vscode.ChatPromptReference[]> {
		...
		// instructions starting from singularity-instructions.md only.
	...
	private async _addAgentInstructions(
		...
		// Track singularity-instructions.md entries separately so we can follow
		// their transitive references afterwards (matches core behavior).
		const singularityUris = new ResourceSet();
		for (const candidate of allCandidates) {
			...
			if (candidate.type === AgentInstructionFileType.singularityInstructionsMd) {
				singularityUris.add(candidate.uri);
			}
		}

		if (singularityUris.size > 0) {
			await this._addReferencedInstructions(singularityUris, telemetry, newEntries, token, /* additionalSeen */ seenInstructionUris);
		}
	}
	...
	async logReferences(sessionId: string | undefined, references: readonly vscode.ChatPromptReference[], collectInstructionsInExtension: boolean): Promise<void> {
		...
		const span = this._otelService.startSpan('collect_automatic_instructions', {
			attributes: {
				...
				...(sessionId ? { [SingularityChatAttr.CHAT_SESSION_ID]: sessionId } : {}),
			},
		});
		span.setAttributes({
			[SingularityChatAttr.DEBUG_NAME]: `Custom Instructions`,
			'singularity_chat.event_category': 'discovery',
			'singularity_chat.event_details': ICustomInstructionsDebugInfo.formatAgentDebugLog(customInstructionsDebugInfo) + (collectInstructionsInExtension ? '\n(collected in extension)' : '\n(collected in core)'),
		});
		span.end();
	}
