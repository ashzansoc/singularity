...
const SINGULARITY_CONFIG_FOLDER = '.singularity';
const SINGULARITY_CUSTOM_INSTRUCTIONS_FILENAME = 'singularity-instructions.md';
...
export class AgentInstructionsLocator extends Disposable {
	 ...
	 * `singularity-instructions.md` files that apply to the current workspace.
	 */
	public async listAgentInstructions(token: CancellationToken, logger?: AgentInstructionsLogger): Promise<IAgentInstructionFile[]> {
		...
		// `useSingularityInstructionsFiles` gates both workspace and personal
		// `singularity-instructions.md` discovery.
		// Reuses the existing extension config (default true) instead of hard-coding the qualified key.
		const useSingularityInstructionsFiles = this.configurationService.getConfig(ConfigKey.UseInstructionFiles) !== false;
		if (!useSingularityInstructionsFiles) {
			logger?.logInfo('Singularity instructions files are disabled via configuration.');
		} else {
			const singularityInstructionsFile: IWorkspaceInstructionFile = { fileName: SINGULARITY_CUSTOM_INSTRUCTIONS_FILENAME, type: AgentInstructionFileType.singularityInstructionsMd };
			promises.push(this.findFilesInRoots(rootFolders, GITHUB_CONFIG_FOLDER, [singularityInstructionsFile], token, resolvedAgentFiles)); // singularity-instructions.md in .github folder under workspace root
			promises.push(this.findFilesInRoots([this.envService.userHome], SINGULARITY_CONFIG_FOLDER, [singularityInstructionsFile], token, resolvedAgentFiles)); // singularity-instructions.md in ~/.singularity folder
		}
