...
suite('AgentInstructionsLocator', () => {
	...
	test('should collect parent folder singularity-instructions.md and AGENTS.md when includeWorkspaceFolderParents is enabled', async () => {
		await mockFiles(fileSystem, [
			...
			{ path: `${parentFolder}/.github/singularity-instructions.md`, contents: ['Parent singularity instructions'] },
		...
		expect(paths).not.toContain(`${parentFolder}/.github/singularity-instructions.md`);
		...
		expect(paths).toContain(`${parentFolder}/.github/singularity-instructions.md`);
	...
	test('singularity-instructions and AGENTS.md', async () => {
		...
		expect(paths).toEqual([
			`${rootFolder}/.github/singularity-instructions.md`,
	...
	test('should collect ~/.singularity/singularity-instructions.md when enabled', async () => {
		const userHome = '/home/testuser';
		await mockFiles(fileSystem, [
			{ path: `${userHome}/.singularity/singularity-instructions.md`, contents: ['Singularity guidelines from home'] },
			{ path: `${rootFolder}/src/file.ts`, contents: ['console.log("test");'] },
		]);
		...
		let result = await locator.listAgentInstructions(CancellationToken.None);
		let paths = result.map(f => f.uri.path);
		expect(paths).toContain(`${userHome}/.singularity/singularity-instructions.md`);

		await configService.setConfig(ConfigKey.UseInstructionFiles, false);
		result = await locator.listAgentInstructions(CancellationToken.None);
		paths = result.map(f => f.uri.path);
		expect(paths).not.toContain(`${userHome}/.singularity/singularity-instructions.md`);
	});
	...
	suite('multi-root workspace', () => {
		...
		test('should collect AGENTS.md and singularity-instructions.md from multi-root workspace', async () => {
			await mockFiles(fileSystem, [
				{ path: `${rootFolder1}/AGENTS.md`, contents: ['Root 1 agents'] },
				{ path: `${rootFolder1}/.github/singularity-instructions.md`, contents: ['Root 1 singularity'] },
				{ path: `${rootFolder2}/AGENTS.md`, contents: ['Root 2 agents'] },
				{ path: `${rootFolder2}/.github/singularity-instructions.md`, contents: ['Root 2 singularity'] },
			]);
			...
			expect(paths).toContain(`${rootFolder1}/.github/singularity-instructions.md`);
			expect(paths).toContain(`${rootFolder2}/AGENTS.md`);
			expect(paths).toContain(`${rootFolder2}/.github/singularity-instructions.md`);
		});
	});
});
