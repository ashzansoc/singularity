...
suite('AutomaticInstructionsCollector', () => {
	...
	suite('agent instructions', () => {
		test('adds singularity-instructions.md, AGENTS.md and CLAUDE.md returned by the locator', async () => {
			const singularity: IAgentInstructionFile = { uri: URI.joinPath(rootFolderUri, '.github/singularity-instructions.md'), type: AgentInstructionFileType.singularityInstructionsMd };
			...
			promptsService.setAgentInstructions([singularity, agentsMd, claudeMd]);

			const result = await callCollect();

			const paths = result.filter(e => isInstructionFile(e)).map(e => e.value.path);
			expect(paths).toContain(singularity.uri.path);
		...
		test('does not duplicate when the file is already attached', async () => {
			const singularity: IAgentInstructionFile = { uri: URI.joinPath(rootFolderUri, '.github/singularity-instructions.md'), type: AgentInstructionFileType.singularityInstructionsMd };
			promptsService.setAgentInstructions([singularity]);

			// Pre-attach the same URI as an instruction-file reference.
			const preAttached: ChatPromptReference = {
				id: `${InstructionFileIdPrefix}.root__${singularity.uri.toString()}`,
				name: 'pre-attached',
				value: singularity.uri,
			};
	...
	suite('referenced instructions', () => {
		test('follows #file: references in singularity-instructions.md', async () => {
			...
			promptsService.setAgentInstructions([
				{ uri: singularityUri, type: AgentInstructionFileType.singularityInstructionsMd },
			]);
			promptsService.setFileContent(singularityUri, 'Be helpful #file:./referenced.instructions.md');
			...
			const paths = result.filter(e => isInstructionFile(e)).map(e => e.value.path);
			expect(paths).toContain(singularityUri.path);
		...
		test('handles transitive (nested) references', async () => {
			...
			promptsService.setFileContent(singularityUri, 'Level 1 #file:./level2.instructions.md');
		...
		test('skips references to files outside the workspace that are not prompt files', async () => {
			...
			promptsService.setFileContent(singularityUri, 'See #file:/tmp/external.md');
		...
		test('skips reference following when setting is disabled (Agent mode)', async () => {
			...
			promptsService.setFileContent(singularityUri, '#file:./referenced.instructions.md');
			await mockFiles(fileSystem, [
	...
	suite('telemetry', () => {
		test('aggregates instruction counts in the emitted event', async () => {
			...
			promptsService.setAgentInstructions([singularity, agentsMd]);
