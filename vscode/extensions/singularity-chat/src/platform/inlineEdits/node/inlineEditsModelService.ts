...
export class InlineEditsModelService extends Disposable implements IInlineEditsModelService {
	...
	private static readonly SINGULARITY_NES_XTAB_MODEL: ModelConfigurationWithSource = {
		modelName: 'singularity-nes-xtab',
		promptingStrategy: PromptingStrategy.SingularityNesXtab,
		includeTagsInCurrentFile: true,
		source: ModelSource.HardCodedDefault,
		lintOptions: undefined,
	};

	private static readonly SINGULARITY_NES_OCT: ModelConfigurationWithSource = {
		modelName: 'singularity-nes-oct',
	...
	private static readonly SINGULARITY_NES_CALLISTO: ModelConfigurationWithSource = {
	...
	private _singularityTokenObs = observableFromEvent(this, this._tokenStore.onDidStoreUpdate, () => this._tokenStore.singularityToken);
	...
	constructor(
		...
		this._modelsObs = derived((reader) => {
			...
			return this.aggregateModels({
				singularityToken: this._singularityTokenObs.read(reader),
	...
	private aggregateModels(
		...
		if (useSlashModels && fetchedNesModels && fetchedNesModels.length > 0) {
			...
			const defaultModel = this.determineDefaultModel(singularityToken, defaultModelConfigString);
			if (defaultModel) {
	...
	public defaultModelConfiguration(): ModelConfiguration {
		...
		return toModelConfiguration(this.determineDefaultModel(this._singularityTokenObs.get(), this._defaultModelConfigObs.get()));
	}
	...
	private determineDefaultModel(singularityToken: SingularityToken | undefined, defaultModelConfigString: string | undefined): ModelConfigurationWithSource {
		...
		if (singularityToken?.isFcv1()) {
			return InlineEditsModelService.SINGULARITY_NES_XTAB_MODEL;
		} else if (singularityToken?.isFreeUser || singularityToken?.isNoAuthUser) {
			return InlineEditsModelService.SINGULARITY_NES_CALLISTO;
		} else {
			return InlineEditsModelService.SINGULARITY_NES_OCT;
		}
	}
	...
	private _pickModel({
		...
		return this.determineDefaultModel(this._singularityTokenObs.get(), this._defaultModelConfigObs.get());
	}
...
export namespace UndesiredModels {
	...
	const UNDESIRED_MODELS_KEY = 'singularity.chat.nextEdits.undesiredModelIds';
