...
function getRateLimitMessage(fetchResult: ChatFetchError, singularityPlan: string | undefined): string {
	...
	if (fetchResult.capiError?.code?.startsWith('agent_mode_limit_exceeded')) { // Rate limited in agent mode
		return l10n.t({
			...
			args: [retryAfterString, 'https://aka.ms/github-singularity-rate-limit-error'],
			comment: [`{Locked=']({'}`]
		});
	}
	...
	if (fetchResult.capiError?.code?.startsWith('user_global_rate_limited')) {
		if (singularityPlan === 'free' || singularityPlan === 'individual' || singularityPlan === 'individual_pro') {
			return l10n.t({
		...
		if (fetchResult.retryAfter) {
			...
			if (fetchResult.isAuto) {
				return l10n.t({
					...
					args: [resetDateString, 'https://aka.ms/github-singularity-rate-limit-error'],
					comment: [`{Locked=']({'}`]
				});
			}
	...
	if (fetchResult.capiError?.code?.startsWith('integration_rate_limited')) {
		return l10n.t({
			message: 'Sorry, Singularity AI is currently experiencing high demand. Please try again in {0}. [Learn More]({1})',
			args: [retryAfterString, 'https://aka.ms/github-singularity-rate-limit-error'],
			comment: [`{Locked=']({'}`]
		});
	}

	const tokenRouterReason = fetchResult.reason ?? '';
	if (/maximum\s+\d+\s+requests\s+within\s+\d+\s+minutes|TokenRouter rate limit/i.test(tokenRouterReason)) {
		const betaOnePerMin = /maximum\s+1\s+requests/i.test(tokenRouterReason);
		if (betaOnePerMin) {
			return l10n.t({
				message: 'Beta access allows 1 AI request per minute. Please wait {0}, then send your message again. Each agent step may need its own minute. [Learn More]({1})',
				args: [retryAfterString, 'https://aka.ms/github-singularity-rate-limit-error'],
				comment: [`{Locked=']({'}`]
			});
		}
		return l10n.t({
			message: 'TokenRouter shared rate limit reached. Please wait {0} before trying again. [Learn More]({1})',
			args: [retryAfterString, 'https://aka.ms/github-singularity-rate-limit-error'],
			comment: [`{Locked=']({'}`]
		});
	}
	...
	if (fetchResult?.capiError?.code && fetchResult?.capiError?.message) {
		if (fetchResult.isAuto) {
			return l10n.t({
				...
				args: [retryAfterString, 'https://aka.ms/github-singularity-rate-limit-error', fetchResult.capiError.message, fetchResult.capiError.code],
				comment: [`{Locked=']({'}`]
			});
		}
		return l10n.t({
		...
		switch (singularityPlan) {
			case 'free':
				return resetDateString
					? l10n.t(`You've reached your monthly credit limit. Upgrade to Singularity Pro or wait until your credits reset on {0}.`, resetDateString)
					: l10n.t(`You've reached your monthly credit limit. Upgrade to Singularity Pro or wait for your credits to reset.`);
			case 'individual':
				return resetDateString
					? l10n.t(`You've reached your monthly credit limit. Please enable additional paid credits, upgrade to Singularity Pro+, or wait until your credits reset on {0}.`, resetDateString)
					: l10n.t(`You've reached your monthly credit limit. Please enable additional paid credits, upgrade to Singularity Pro+, or wait for your credits to reset.`);
			case 'individual_pro':
			case 'individual_max':
				return resetDateString
			...
			case 'enterprise':
				return resetDateString
					? l10n.t(`You've reached your credit limit. To continue working, please contact your organization's Singularity admin or wait until your credits reset on {0}.`, resetDateString)
					: l10n.t(`You've reached your credit limit. To continue working, please contact your organization's Singularity admin or wait for your credits to reset.`);
			default:
				return resetDateString
					? l10n.t(`You've reached your credit limit. For additional paid credits, please reach out to your organization's Singularity admin or wait until your credits reset on {0}.`, resetDateString)
					: l10n.t(`You've reached your credit limit. For additional paid credits, please reach out to your organization's Singularity admin or wait for your credits to reset.`);
		}
	}

	switch (singularityPlan) {
		case 'free':
			return l10n.t(`You've reached your monthly chat messages quota. Upgrade to Singularity Pro or wait for your allowance to renew.`);
		case 'individual':
			return l10n.t(`You've exhausted your premium model quota. Please enable additional paid premium requests, upgrade to Singularity Pro+, or wait for your allowance to renew.`);
		case 'individual_pro':
		case 'individual_max':
		...
		case 'business':
		case 'enterprise':
			return l10n.t(`You've exhausted your credits. To continue working, please contact your organization's Singularity admin or wait for your allowance to renew.`);
		default:
			return l10n.t(`You've exhausted your premium model quota. For additional paid premium requests, please reach out to your organization's Singularity admin or wait for your allowance to renew.`);
	}
}

function getQuotaHitMessage(fetchResult: ChatFetchError, singularityPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): string {
	...
	if (fetchResult.capiError?.code === 'quota_exceeded') {
		...
		return l10n.t({
			message: 'You cannot accrue additional premium requests at this time. Please contact [GitHub Support]({0}) to continue using Singularity.',
			args: ['https://support.github.com/contact'],
			comment: [`{Locked=']({'}`]
		});
	} else if (fetchResult.capiError?.code === 'additional_spend_limit_reached') {
		if (singularityPlan === 'business' || singularityPlan === 'enterprise') {
		...
		return l10n.t({
			...
			args: ['https://github.com/settings/singularity/features'],
			comment: [`{Locked=']({'}`]
		});
...
export function getErrorDetailsFromChatFetchError(fetchResult: ChatFetchError, singularityPlan: string | undefined, gitHubOutageStatus: GitHubOutageStatus, isUsageBasedBilling?: boolean, quotaResetDate?: string): ChatErrorDetails {
	return { code: fetchResult.type, ...getErrorDetailsFromChatFetchErrorInner(fetchResult, singularityPlan, gitHubOutageStatus, isUsageBasedBilling, quotaResetDate) };
}

function getErrorDetailsFromChatFetchErrorInner(fetchResult: ChatFetchError, singularityPlan: string | undefined, gitHubOutageStatus: GitHubOutageStatus, isUsageBasedBilling?: boolean, quotaResetDate?: string): ChatErrorDetails {
	...
	switch (fetchResult.type) {
		...
		case ChatFetchResponseType.RateLimited:
			details = {
				message: getRateLimitMessage(fetchResult, singularityPlan),
		...
		case ChatFetchResponseType.QuotaExceeded:
			details = {
				message: getQuotaHitMessage(fetchResult, singularityPlan, isUsageBasedBilling, quotaResetDate),
	...
	if (gitHubOutageStatus !== GitHubOutageStatus.None) {
		const outageMsg = l10n.t({
			message: 'Note: GitHub is currently experiencing a service disruption. This may be affecting Singularity. Check [GitHub Status]({0}) for details.',
	...
	switch (category) {
		case FilterReason.Copyright:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, the response matched public code so it was blocked. Please rephrase your prompt. [Learn more](https://aka.ms/singularity-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/singularity-chat-filtered-docs)'}`]
				});
			} else {
		...
		case FilterReason.Prompt:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, your prompt was filtered by the Responsible AI Service. Please rephrase your prompt and try again. [Learn more](https://aka.ms/singularity-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/singularity-chat-filtered-docs)'}`]
				});
			} else {
		...
		default:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, the response was filtered by the Responsible AI Service. Please rephrase your prompt and try again. [Learn more](https://aka.ms/singularity-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/singularity-chat-filtered-docs)'}`]
				});
			} else {
