...
/**
 * This is the quota info we get from the `singularity_internal/user` endpoint.
 * It is accessed via the singularity token object
 */
export interface SingularityUserQuotaInfo {
...
export interface IChatQuotaService {
	...
	additionalUsageEnabled: boolean;
	/** AIC credits accumulated for the given turn, from singularity_usage.total_nano_aiu. */
	...
	/** Accumulate per-request cost from singularity_usage.total_nano_aiu (in nano-AIUs), scoped to a turn. */
	...
	clearQuota(): void;
	/**
	 * Fetches up-to-date quota data from the `singularity_internal/user` endpoint.
