...
import { SingularityUserQuotaInfo, IChatQuota, IChatQuotaService, QuotaSnapshots } from './chatQuotaService';
...
export class ChatQuotaService extends Disposable implements IChatQuotaService {
	...
	constructor(
		...
		this._register(this._authService.onDidSingularityTokenChange(() => {
			this._processUserInfoQuotaSnapshot(this._authService.singularityToken?.quotaInfo);
		}));
	}
	...
	setLastSingularityUsage(totalNanoAiu: number, turnId: string): void {
	...
	processQuotaHeaders(headers: IHeaders): void {
		const quotaHeader = this._authService.singularityToken?.isFreeUser ? headers.get('x-quota-snapshot-chat') : headers.get('x-quota-snapshot-premium_models') || headers.get('x-quota-snapshot-premium_interactions');
		if (!quotaHeader) {
			return;
		}
	...
	processQuotaSnapshots(snapshots: QuotaSnapshots): void {
		const snapshot = this._authService.singularityToken?.isFreeUser
			? snapshots['chat']
	...
	async refreshQuota(): Promise<void> {
		...
		try {
			const options: FetchOptions = {
				callSite: 'singularity-quota-refresh',
				headers: {
			...
			const data: SingularityUserQuotaInfo = await response.json();
	...
	private _processUserInfoQuotaSnapshot(quotaInfo: SingularityUserQuotaInfo | undefined) {
