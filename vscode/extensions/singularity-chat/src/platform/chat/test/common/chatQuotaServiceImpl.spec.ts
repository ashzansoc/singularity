...
function createMockAuthService(): IAuthenticationService {
	return {
		_serviceBrand: undefined,
		singularityToken: undefined,
		onDidSingularityTokenChange: new Emitter().event,
...
function createMockAuthServiceWithEmitter(opts?: { isFreeUser?: boolean }) {
	...
	const authService = {
		_serviceBrand: undefined,
		singularityToken: undefined as { isFreeUser: boolean; quotaInfo: ReturnType<typeof makeQuotaInfo> } | undefined,
		onDidSingularityTokenChange: emitter.event,
	} as unknown as IAuthenticationService;
	return {
		authService, emitter, setToken: (quotaInfo: ReturnType<typeof makeQuotaInfo>) => {
			(authService as any).singularityToken = { isFreeUser: opts?.isFreeUser ?? false, quotaInfo };
		}
	};
}
...
describe('ChatQuotaService', () => {
	...
	describe('setLastSingularityUsage', () => {
		test('converts nano-AIUs to AIC credits', () => {
			const svc = create();
			svc.setLastSingularityUsage(1_000_000_000, TURN_A); // 1 AIC
			expect(svc.getCreditsForTurn(TURN_A)).toBe(1);
		});

		test('handles fractional AIC values', () => {
			const svc = create();
			svc.setLastSingularityUsage(500_000_000, TURN_A); // 0.5 AIC
			expect(svc.getCreditsForTurn(TURN_A)).toBe(0.5);
		});

		test('accumulates across multiple calls', () => {
			const svc = create();
			svc.setLastSingularityUsage(300_000_000, TURN_A); // 0.3 AIC
			svc.setLastSingularityUsage(200_000_000, TURN_A); // 0.2 AIC
	...
	describe('resetTurnCredits', () => {
		...
		test('allows fresh accumulation after reset', () => {
			...
			svc.setLastSingularityUsage(1_000_000_000, TURN_A);
			svc.resetTurnCredits(TURN_A);
			svc.setLastSingularityUsage(500_000_000, TURN_A);
			expect(svc.getCreditsForTurn(TURN_A)).toBe(0.5);
		});
	...
	describe('concurrent turn isolation', () => {
		test('credits for different turns are independent', () => {
			const svc = create();
			svc.setLastSingularityUsage(1_000_000_000, TURN_A);
			svc.setLastSingularityUsage(2_000_000_000, TURN_B);
			expect(svc.getCreditsForTurn(TURN_A)).toBe(1);
	...
	describe('realistic turn lifecycle simulation', () => {
		...
		async function simulateApiCall(
			...
			svc.setLastSingularityUsage(nanoAiu, turnId);
		}
		...
		test('agentic turn with subagents and helper calls', async () => {
			...
			svc.setLastSingularityUsage(8_640_000, turnId);
			// prompt categorization (gpt-4o-mini)
			svc.setLastSingularityUsage(56_160_000, turnId);
			// progress messages x2 (gpt-4o-mini)
			svc.setLastSingularityUsage(16_470_000, turnId);
			svc.setLastSingularityUsage(16_350_000, turnId);
			// main agent call (Claude Opus)
			svc.setLastSingularityUsage(24_782_500_000, turnId);
			// subagent call (Claude Opus)
			svc.setLastSingularityUsage(25_580_500_000, turnId);
			// final agent call (Claude Opus)
			svc.setLastSingularityUsage(61_741_750_000, turnId);
			// language model wrapper (gpt-4o-mini)
			svc.setLastSingularityUsage(63_180_000, turnId);
		...
		test('interleaved API responses from parallel turns', async () => {
			...
			svc.setLastSingularityUsage(1_000_000_000, turnX);
			// Response from turn Y arrives (interleaved)
			svc.setLastSingularityUsage(2_000_000_000, turnY);
			// Another response from turn X
			svc.setLastSingularityUsage(3_000_000_000, turnX);
			// Another response from turn Y
			svc.setLastSingularityUsage(4_000_000_000, turnY);
			// Final response from turn X
			svc.setLastSingularityUsage(500_000_000, turnX);
		...
		test('turn reset during a concurrent turn does not affect the other', async () => {
			...
			svc.setLastSingularityUsage(5_000_000_000, turnA);
			svc.setLastSingularityUsage(3_000_000_000, turnB);

			// User sends a new message in window A → new turn resets A
			svc.resetTurnCredits(turnA);

			// Meanwhile, turn B is still accumulating
			svc.setLastSingularityUsage(2_000_000_000, turnB);

			// New turn A starts accumulating fresh
			svc.setLastSingularityUsage(1_000_000_000, turnA);
		...
		test('many concurrent turns (stress test)', () => {
			...
			for (let t = 0; t < turnCount; t++) {
				...
				for (let c = 0; c < callsPerTurn; c++) {
					svc.setLastSingularityUsage(100_000_000, turnId); // 0.1 AIC each
				}
			}
		...
		test('completed turns are cleaned up on reset', () => {
			const svc = create();
			svc.setLastSingularityUsage(1_000_000_000, 'old-turn');
	...
	describe('runSubagent dual-key aggregation', () => {
		...
		test('parent-only turn (no subagents) uses request.id key', () => {
			...
			svc.setLastSingularityUsage(24_000_000_000, requestId);
			svc.setLastSingularityUsage(2_000_000_000, requestId);
		...
		test('subagent credits accumulate under parent turn.id', () => {
			...
			svc.setLastSingularityUsage(24_000_000_000, parentRequestId);

			// Subagent calls (creditKey = parentTurnId, routed via chatRequestId)
			svc.setLastSingularityUsage(33_000_000_000, parentTurnId);
			svc.setLastSingularityUsage(33_000_000_000, parentTurnId);
		...
		test('two subagent turns do not interfere', () => {
			...
			svc.setLastSingularityUsage(24_000_000_000, reqA);
			svc.setLastSingularityUsage(33_000_000_000, turnA);
			svc.setLastSingularityUsage(33_000_000_000, turnA);

			// Turn B: parent + 5 subagents
			svc.setLastSingularityUsage(25_000_000_000, reqB);
			for (let i = 0; i < 5; i++) {
				svc.setLastSingularityUsage(33_000_000_000, turnB);
			}
		...
		test('subagent handler does not corrupt parent credits', () => {
			...
			svc.resetTurnCredits(parentTurnId);
			svc.setLastSingularityUsage(24_000_000_000, parentReqId);
		...
		test('overhead calls (title, categorization) are separate from turn credits', () => {
			...
			svc.setLastSingularityUsage(8_000_000, titleTurnId);
			svc.setLastSingularityUsage(56_000_000, categorizationTurnId);
	...
	describe('processUserInfoQuotaSnapshot via auth change', () => {
		...
		test('no-ops when singularityToken has no quotaInfo', () => {
			const { authService, emitter } = createMockAuthServiceWithEmitter({ isFreeUser: true });
			const svc = new ChatQuotaService(authService, new TestLogService(), createMockCapiClientService());

			(authService as any).singularityToken = { isFreeUser: true, quotaInfo: undefined };
			emitter.fire();
