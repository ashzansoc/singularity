...
import { SingularityToken, FetchOptions, IDomainChangeResponse, RequestMetadata } from '@vscode/copilot-api';
...
export async function collectCapturedTelemetry(capiClientService: ICAPIClientService, fetcherService: IFetcherService): Promise<CapturedTelemetry<EventData | ExceptionData>[]> {
	const url = capiClientService.singularityTelemetryURL;
...
async function _withTelemetryCapture<T>(
	...
	const extensionId = 'singularity-test';
	...
	testingServiceCollection.define(ICAPIClientService, {
		singularityTelemetryURL: telemetryUrl,
		_serviceBrand: undefined,
		_domainService: undefined,
		_fetcherService: undefined,
		updateDomains: function (singularityToken: SingularityToken | undefined, enterpriseUrlConfig: string | undefined): IDomainChangeResponse {
	...
	await setupGHTelemetry(ghTelemetry, accessor.get(ICAPIClientService), accessor.get(IEnvService), accessor.get(ISingularityTokenStore), extensionId, forceTelemetry);
