...
export class UserInfoStore extends Disposable {
	...
	static INTERNAL_ORG_STORAGE_KEY = 'exp.singularity.chat.internalOrg';
	static SKU_STORAGE_KEY = 'exp.singularity.chat.sku';
	static IS_FCV1_STORAGE_KEY = 'exp.singularity.chat.isFcv1';
	static IS_SN_STORAGE_KEY = 'exp.singularity.chat.isSn';
	static IS_VSCODE_TEAM_MEMBER_STORAGE_KEY = 'exp.singularity.chat.isVscodeTeamMember';
	static ORGANIZATION_LIST_STORAGE_KEY = 'exp.singularity.chat.organizationList';
	constructor(private readonly context: IVSCodeExtensionContext, singularityTokenStore: ISingularityTokenStore) {
		super();

		if (singularityTokenStore) {
			const getInternalOrg = (): string | undefined => {
				if (singularityTokenStore.singularityToken?.isVscodeTeamMember) {
					return 'vscode';
				} else if (singularityTokenStore.singularityToken?.isGitHubInternal) {
					return 'github';
				} else if (singularityTokenStore.singularityToken?.isMicrosoftInternal) {
					return 'microsoft';
				}
				return undefined;
			};

			singularityTokenStore.onDidStoreUpdate(() => {
				this.updateUserInfo(getInternalOrg(), singularityTokenStore.singularityToken?.sku, singularityTokenStore.singularityToken?.isFcv1(), singularityTokenStore.singularityToken?.isSn(), singularityTokenStore.singularityToken?.isVscodeTeamMember, singularityTokenStore.singularityToken?.organizationList);
			});

			if (singularityTokenStore.singularityToken) {
				this.updateUserInfo(getInternalOrg(), singularityTokenStore.singularityToken.sku, singularityTokenStore.singularityToken.isFcv1(), singularityTokenStore.singularityToken.isSn(), singularityTokenStore.singularityToken.isVscodeTeamMember, singularityTokenStore.singularityToken.organizationList);
			} else {
	 ...
	 * This can be used in ExP to target specific organizations via the X-GitHub-Singularity-OrganizationList filter.
	 */
...
export class BaseExperimentationService extends Disposable implements IExperimentationService {
	...
	constructor(
		...
		this._userInfoStore = new UserInfoStore(context, singularityTokenStore);
