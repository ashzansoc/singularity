...
export enum RelatedExtensionsFilter {
	...
	// Singularity related plugins
	SingularityRelatedPluginVersionSingularity = SingularityRelatedPluginVersionPrefix + 'githubsingularity',
	SingularityRelatedPluginVersionSingularityChat = SingularityRelatedPluginVersionPrefix + 'githubsingularitychat',
}
...
class RelatedExtensionsFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		...
		for (const extension of this._getRelatedExtensions()) {
			const filterName = SingularityRelatedPluginVersionPrefix + extension.name.replace(/[^A-Za-z]/g, '').toLowerCase();
...
class SingularityExtensionsFilterProvider implements IExperimentationFilterProvider {
	constructor(private _logService: ILogService) { }

	getFilters(): Map<string, string> {
		const singularityExtensionversion = vscode.extensions.getExtension('singularity.chat')?.packageJSON.version;
		const singularityChatExtensionVersion = packageJson.version;
		const completionsCoreVersion = packageJson.completionsCoreVersion;

		this._logService.trace(`[SingularityExtensionsFilterProvider]::getFilters Singularity Extension Version: ${singularityExtensionversion}, Singularity AI Extension Version: ${singularityChatExtensionVersion}, Completions Core Version: ${completionsCoreVersion}`);
		const filters = new Map<string, string>();
		filters.set(RelatedExtensionsFilter.SingularityRelatedPluginVersionSingularity, singularityExtensionversion);
		filters.set(RelatedExtensionsFilter.SingularityRelatedPluginVersionSingularityChat, singularityChatExtensionVersion);
		filters.set('X-VSCode-CompletionsInChatExtensionVersion', completionsCoreVersion);
		return filters;
	}
}

class SingularityCompletionsFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		...
		this._logService.trace(`[SingularityCompletionsFilterProvider]::getFilters Filters: ${JSON.stringify(Array.from(filters.entries()))}`);
		return filters;
	}
}
...
class GithubAccountFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string | undefined> {
		...
		filters.set('X-GitHub-Singularity-SKU', this._userInfoStore.sku);
		filters.set('X-Microsoft-Internal-Org', this._userInfoStore.internalOrg);
		filters.set('X-GitHub-Singularity-IsFcv1', this._userInfoStore.isFcv1 ? '1' : '0');
		filters.set('X-GitHub-Singularity-IsSn', this._userInfoStore.isSn ? '1' : '0');
		filters.set('X-GitHub-Singularity-IsVscodeTeamMember', this._userInfoStore.isVscodeTeamMember ? '1' : '0');
		filters.set('X-GitHub-Singularity-OrganizationList', orgListString);
		return filters;
	}
...
export class MicrosoftExperimentationService extends BaseExperimentationService {
	constructor(
		...
		@ISingularityTokenStore singularityTokenStore: ISingularityTokenStore,
		...
		const delegateFn = (globalState: vscode.Memento, userInfoStore: UserInfoStore) => {
			...
			return getExperimentationService(
				...
				new SingularityExtensionsFilterProvider(logService),
				...
				new SingularityCompletionsFilterProvider(() => self?.getCompletionsFilters() ?? new Map(), logService),
		...
		super(delegateFn, context, singularityTokenStore, configurationService, logService);
