...
suite('Microsoft Telemetry Sender', function () {
	...
	let mockTokenStore: ISingularityTokenStore;
	let mockToken: SingularityToken;
	...
	beforeEach(() => {
		...
		mockToken = new SingularityToken(createTestExtendedTokenInfo({
			token: 'tid=testTid',
			sku: 'testSku',
...
suite('GitHub Telemetry Sender', function () {
	...
	const commonTelemetryData = {
		properties: {
			singularity_build: new TelemetryTrustedValue('1'),
			singularity_buildType: new TelemetryTrustedValue(!!process.env.BUILD_SOURCEVERSION ? 'prod' : 'dev'),
			singularity_trackingId: new TelemetryTrustedValue('testId'),
		...
		expect(mockLogger.logUsage).toHaveBeenCalledWith(
			'testEvent',
			{
				properties: {
					...
					singularity_version: new TelemetryTrustedValue(lastCall![1].properties.singularity_version.value),
