...
import { SingularityToken, createTestExtendedTokenInfo } from '../../../authentication/common/singularityToken';
...
describe('ExP Service Tests', () => {
	...
	let singularityTokenService: ISingularityTokenStore;
	let extensionContext: IVSCodeExtensionContext;

	const GitHubProToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'token-gh-pro', username: 'fake', sku: 'pro', singularity_plan: 'unknown', organization_list: ['4535c7beffc844b46bb1ed4aa04d759a'] }));
	const GitHubAndMicrosoftEnterpriseToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'token-gh-msft-enterprise', username: 'fake', sku: 'enterprise', singularity_plan: 'unknown', organization_list: ['4535c7beffc844b46bb1ed4aa04d759a', 'a5db0bcaae94032fe715fb34a5e4bce2'] }));
	const MicrosoftEnterpriseToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'token-msft-enterprise', username: 'fake', sku: 'enterprise', singularity_plan: 'unknown', organization_list: ['a5db0bcaae94032fe715fb34a5e4bce2'] }));
	const NoOrgFreeToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'token-no-org-free', username: 'fake', sku: 'free', singularity_plan: 'unknown' }));
	const VscodeTeamMemberToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'token-vscode-team', username: 'fake', sku: 'enterprise', singularity_plan: 'unknown', isVscodeTeamMember: true }));
	const NonVscodeTeamMemberToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'token-non-vscode-team', username: 'fake', sku: 'enterprise', singularity_plan: 'unknown', isVscodeTeamMember: false }));
	const SnEnabledToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'sn=1;tid=test', username: 'fake', sku: 'pro', singularity_plan: 'unknown', organization_list: ['4535c7beffc844b46bb1ed4aa04d759a'] }));
	const SnDisabledToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'sn=0;tid=test', username: 'fake', sku: 'pro', singularity_plan: 'unknown', organization_list: ['4535c7beffc844b46bb1ed4aa04d759a'] }));

	beforeAll(() => {
		const testingServiceCollection = createPlatformServices();
		accessor = testingServiceCollection.createTestingAccessor();
		extensionContext = accessor.get(IVSCodeExtensionContext);
		singularityTokenService = accessor.get(ISingularityTokenStore);
	...
	beforeEach(() => {
		...
		singularityTokenService.singularityToken = undefined;
	});
	...
	it('should return treatments based on singularity token', async () => {
		...
		singularityTokenService.singularityToken = GitHubProToken;
		...
		singularityTokenService.singularityToken = GitHubAndMicrosoftEnterpriseToken;
		await treatmentsChangePromise;
		...
		singularityTokenService.singularityToken = MicrosoftEnterpriseToken;
		await treatmentsChangePromise;
		...
		singularityTokenService.singularityToken = NoOrgFreeToken;
		await treatmentsChangePromise;
	...
	it('should not fire events when relevant user info does not change', async () => {
		...
		// We need a separate token just to make sure we get passed the singularity token change guard
		const newGitHubProToken = new SingularityToken(createTestExtendedTokenInfo({
			token: 'github-test', username: 'fake',
			sku: 'pro', singularity_plan: 'unknown',
			organization_list: ['4535c7beffc844b46bb1ed4aa04d759a']
		}));
		singularityTokenService.singularityToken = newGitHubProToken; // Same token
	...
	it('should detect VS Code team member correctly', async () => {
		...
		const treatmentsChangePromise = GetNewTreatmentsChangedPromise();
		singularityTokenService.singularityToken = VscodeTeamMemberToken;
		await treatmentsChangePromise;

		// Verify isVscodeTeamMember is set in UserInfoStore
		const userInfoStore = new UserInfoStore(extensionContext, singularityTokenService);
	...
	it('should detect non-VS Code team member correctly', async () => {
		...
		const treatmentsChangePromise = GetNewTreatmentsChangedPromise();
		singularityTokenService.singularityToken = NonVscodeTeamMemberToken;
		await treatmentsChangePromise;
	...
	it('should use cached VS Code team member status on initialization', async () => {
		...
		const newUserInfoStore = new UserInfoStore(extensionContext, singularityTokenService);
	...
	it('should detect sn token flag correctly', async () => {
		...
		const treatmentsChangePromise = GetNewTreatmentsChangedPromise();
		singularityTokenService.singularityToken = SnEnabledToken;
		await treatmentsChangePromise;
	...
	it('should detect sn token flag as false when explicitly disabled', async () => {
		...
		const treatmentsChangePromise = GetNewTreatmentsChangedPromise();
		singularityTokenService.singularityToken = SnDisabledToken;
		await treatmentsChangePromise;
