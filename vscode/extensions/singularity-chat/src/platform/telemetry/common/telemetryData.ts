...
export class TelemetryData {
	...
	extendWithEditorAgnosticFields(envService: IEnvService): void {
		...
		this.properties['singularity_version'] = `singularity/${envService.getVersion()}`;
	...
	extendWithConfigProperties(configService: IConfigurationService, envService: IEnvService, telemetryConfig: ITelemetryUserConfig): void {
		...
		configProperties['singularity.build'] = envService.getBuild();
		configProperties['singularity.buildType'] = envService.getBuildType();

		if (telemetryConfig.trackingId) {
			configProperties['singularity.trackingId'] = telemetryConfig.trackingId;
		}
...
function addRequiredProperties(envService: IEnvService, properties: { [key: string]: string }) {
	properties['unique_id'] = generateUuid(); // add a unique id to the telemetry event so singularity-foundations can correlate with duplicate events
