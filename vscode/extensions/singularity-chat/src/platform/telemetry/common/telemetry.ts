...
import type { SingularityToken } from '../../authentication/common/singularityToken';
...
export class TelemetryUserConfigImpl implements ITelemetryUserConfig {
	...
	constructor(
		...
		this.updateFromToken(this._tokenStore.singularityToken);
		this._tokenStore.onDidStoreUpdate(() => {
			this.updateFromToken(this._tokenStore.singularityToken);
		});
	}

	private updateFromToken(token: SingularityToken | undefined) {
		if (!token) {
			return;
		}
 ...
 */
export function createTrackingIdGetter(tokenStore: ISingularityTokenStore): () => string | undefined {
	let cachedTrackingId = tokenStore.singularityToken?.getTokenValue('tid');
	tokenStore.onDidStoreUpdate(() => {
		const trackingId = tokenStore.singularityToken?.getTokenValue('tid');
		if (trackingId) {
...
export interface IGHTelemetryService {
	...
	/**
	 * Enhanced telemetry events contain additional data such as user prompts and suggestions. Like standard telemetry events, it can disabled with VS Code's telemetry settings or the Singularity settings page.
	 *
	 * You can manage this setting on the Singularity settings page https://github.com/settings/singularity/features
	 * Learn about configuring this telemetry at https://docs.github.com/en/singularity/managing-singularity/managing-singularity-as-an-individual-subscriber/managing-your-singularity-plan/managing-singularity-policies-as-an-individual-subscriber#enabling-or-disabling-prompt-and-suggestion-collection
	 * Learn more about the data collected at https://github.com/features/singularity/#faq
	 */
