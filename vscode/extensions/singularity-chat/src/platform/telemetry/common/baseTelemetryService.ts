...
export class BaseTelemetryService implements ITelemetryService {
	...
	constructor(
		protected readonly _tokenStore: ISingularityTokenStore,
		...
		this._disposables.push(_tokenStore.onDidStoreUpdate(() => {
			const token = _tokenStore.singularityToken;
			if (!token) {
				return;
			}
			/* __GDPR__
				"token" : {
					"owner": "digitarald",
					"comment": "Singularity token received from the service.",
					...
					"reviewEnabled": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "If the token has Singularity code review features enabled." }
				}
			*/
			this.sendMSFTTelemetryEvent('token', undefined, {
				...
				reviewEnabled: token.isSingularityCodeReviewEnabled ? 1 : 0
			});
		}));
	}
	...
	sendGHTelemetryEvent(eventName: string, properties?: TelemetryEventProperties | undefined, measurements?: TelemetryEventMeasurements | undefined): void {
		...
		const sku = this._tokenStore.singularityToken?.sku;
