...
const reviewDiffContextKey = 'singularity.chat.chat.reviewDiff.enabled';
const reviewDiffReposContextKey = 'singularity.chat.chat.reviewDiff.enabledRootUris';
const numberOfReviewCommentsKey = 'singularity.chat.chat.review.numberOfComments';
...
export class ReviewServiceImpl implements IReviewService {
	...
	private _commentController = this._disposables.add(vscode.comments.createCommentController('github-singularity-review', 'Code Review'));
	...
	constructor(
		...
		this._disposables.add(vscode.workspace.onDidChangeConfiguration(e => {
			...
			if (e.affectsConfiguration('singularity.chat.advanced') || e.affectsConfiguration('singularity.chat.advanced.review.intent')) {
	...
	isReviewDiffEnabled() {
		return this._configurationService.getConfig(ConfigKey.ReviewAgent) && this._authenticationService.singularityToken?.isSingularityCodeReviewEnabled || false;
	}
	...
	getDiagnosticCollection(): ReviewDiagnosticCollection {
		return this._diagnosticCollection || this._disposables.add(this._diagnosticCollection = vscode.languages.createDiagnosticCollection('singularity.chat.chat.review'));
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			...
			if (this._comments.length === 1) {
				vscode.commands.executeCommand('singularity.chat.chat.review.next');
				this._monitorActiveThread = setInterval(() => {
					...
					if (active !== this._activeThread) {
						...
						if (active) {
							vscode.commands.executeCommand('singularity.chat.chat.review.current', active);
						}
					}
				}, 500);
			}
		...
		const comments = [
			{
				...
				author: {
					...
					iconPath: URI.joinPath(this._contextService.extensionUri, 'assets', 'singularity.png'),
				},
			}
		];
		return comments;
