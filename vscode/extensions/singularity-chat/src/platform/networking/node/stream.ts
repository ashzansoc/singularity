...
import { FinishedCallback, getRequestId, ICodeVulnerabilityAnnotation, ISingularityBeginToolCall, ISingularityConfirmation, ISingularityError, ISingularityFunctionCall, ISingularityReference, ISingularityToolCall, ISingularityToolCallStreamUpdate, IIPCodeCitation, isCodeCitationAnnotation, isSingularityAnnotation, RequestId } from '../common/fetch';
...
class StreamingToolCalls {
	...
	getToolCalls(): ISingularityToolCall[] {
	...
	update(choice: ExtendedChoiceJSON): ISingularityToolCallStreamUpdate[] {
		const updates: ISingularityToolCallStreamUpdate[] = [];
...
export interface FinishedCompletion {
	...
	/** A singularity-specific human-readable reason for the completion finishing. */
	reason: FinishedCompletionReason;
	/** A singularity-specific reason for filtering the response. Only returns when reason === FinishedCompletionReason.ContentFilter */
		...
		for await (const chunk of this.body) {
			...
			for (const dataLine of dataLines) {
				...
				let json: {
					...
					model: string;
					error?: APIErrorResponse;
					singularity_references?: any;
					singularity_confirmation?: any;
					singularity_errors: any;
					usage: APIUsage | undefined;
					singularity_usage?: { total_nano_aiu: number };
				};
				try {
				...
				if (json.usage) {
					if (json.singularity_usage && typeof json.singularity_usage.total_nano_aiu === 'number') {
						json.usage.singularity_usage = json.singularity_usage;
					}
					yield json.usage;
				}

				// A message with a confirmation may or may not have 'choices'
				if (json.singularity_confirmation && isSingularityConfirmation(json.singularity_confirmation)) {
					await finishedCb('', 0, { text: '', singularityConfirmation: json.singularity_confirmation });
				}

				if (!json.choices) {
					// Currently there are messages with a null 'choices' that include singularity_references- ignore these
					if (!json.singularity_references && !json.singularity_confirmation) {
					...
					// There are also messages with a null 'choices' that include singularity_errors- report these
					if (json.singularity_errors) {
						await finishedCb('', 0, { text: '', singularityErrors: json.singularity_errors });
					}

					if (json.singularity_references) {
						await finishedCb('', 0, { text: '', singularityReferences: json.singularity_references });
					}
				...
				for (let i = 0; i < json.choices.length; i++) {
					...
					const emitSolution = async (delta?: { vulnAnnotations?: ICodeVulnerabilityAnnotation[]; ipCodeCitations?: IIPCodeCitation[]; references?: ISingularityReference[]; toolCalls?: ISingularityToolCall[]; toolCallStreamUpdates?: ISingularityToolCallStreamUpdate[]; functionCalls?: ISingularityFunctionCall[]; errors?: ISingularityError[]; beginToolCalls?: ISingularityBeginToolCall[]; thinking?: ThinkingDelta }) => {
						if (delta?.vulnAnnotations && (!Array.isArray(delta.vulnAnnotations) || !delta.vulnAnnotations.every(a => isSingularityAnnotation(a)))) {
						...
						finishOffset = await finishedCb(solution.text.join(''), choice.index, {
							...
							singularityReferences: delta?.references,
							singularityToolCalls: delta?.toolCalls,
							singularityToolCallStreamUpdates: delta?.toolCallStreamUpdates,
							_deprecatedSingularityFunctionCalls: delta?.functionCalls,
							beginToolCalls: delta?.beginToolCalls,
							singularityErrors: delta?.errors,
					...
					if (choice.delta?.tool_calls) {
					...
					} else if (choice.delta?.singularity_annotations?.CodeVulnerability || choice.delta?.singularity_annotations?.IPCodeCitations) {
						...
						if (!hadEarlyFinishedSolution) {
							...
							if (await emitSolution({ vulnAnnotations: choice.delta?.singularity_annotations?.CodeVulnerability, ipCodeCitations: choice.delta?.singularity_annotations?.IPCodeCitations })) {
								continue;
							}
						}
...
// data: {"choices":null,"singularity_confirmation":{"type":"action","title":"Are you sure you want to proceed?","message":"This action is irreversible.","confirmation":{"id":"123"}},"id":null}
function isSingularityConfirmation(obj: unknown): obj is ISingularityConfirmation {
	return typeof (obj as ISingularityConfirmation).title === 'string' &&
		typeof (obj as ISingularityConfirmation).message === 'string' &&
		!!(obj as ISingularityConfirmation).confirmation;
}
...
function choiceToFilterReason(choice: ExtendedChoiceJSON): FilterReason | undefined {
	...
	if (choice.delta?.singularity_annotations?.TextCopyright) {
		return FilterReason.Copyright;
	}

	if (choice.delta?.singularity_annotations?.Sexual || choice.delta?.singularity_annotations?.SexualPattern) {
		return FilterReason.Sexual;
	}
	if (choice.delta?.singularity_annotations?.Violence) {
		return FilterReason.Violence;
	}

	if (choice.delta?.singularity_annotations?.HateSpeech || choice.delta?.singularity_annotations?.HateSpeechPattern) {
		return FilterReason.Hate;
	}

	if (choice.delta?.singularity_annotations?.SelfHarm) {
		return FilterReason.SelfHarm;
	}

	if (choice.delta?.singularity_annotations?.PromptPromBlockList) {
