...
suite('getRequestId', () => {
	...
	test('only X-Singularity-Experiment header', () => {
		const headers = new HeadersImpl({ 'X-Singularity-Experiment': 'exp1' });
		const result = getRequestId(headers);
		assert.strictEqual(result.serverExperiments, 'exp1');
	});

	test('only x-singularity-api-exp-assignment-context header', () => {
		const headers = new HeadersImpl({ 'x-singularity-api-exp-assignment-context': 'ctx1' });
		const result = getRequestId(headers);
	...
	test('both headers combined with semicolon', () => {
		const headers = new HeadersImpl({
			'X-Singularity-Experiment': 'exp1',
			'x-singularity-api-exp-assignment-context': 'ctx1',
		});
		const result = getRequestId(headers);
