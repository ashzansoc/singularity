...
export interface IEndpointFetchOptions {
	...
	/** Force a specific fetcher (prefer node-fetch over electron-fetch when needed). */
	useFetcher?: FetcherId;
	/**
	 * Send only OpenAI-compatible headers (Authorization, Content-Type, X-Request-Id).
	 * Omits OpenAI-Intent / X-GitHub-Api-Version / Singularity interaction headers that some
	 * third-party gateways reject with an opaque HTML 403.
	 */
	plainOpenAIHeaders?: boolean;
}
...
/**
 * Whether requests for this endpoint are routed through CAPI (the Singularity proxy)
...
export interface IMakeChatRequestOptions {
	...
	/** Top-level turn ID for credit accumulation. When set, singularity_usage costs
...
export interface IChatEndpoint extends IEndpoint {
	...
	/** The model ID- this may change and will be `singularity-utility` for the utility (fallback) model. Use `family` to switch behavior based on model type. */
	 ...
	 * the chat fetcher must not fall back to the CAPI Singularity token for the `Authorization`
...
function networkRequest(
	...
	const extra = endpoint.getExtraHeaders ? endpoint.getExtraHeaders(location, options.interactionTypeOverride) : {};
	// Some third-party OpenAI gateways reject Singularity/CAPI headers with opaque HTML 403.
	const safeAdditional = endpointFetchOptions?.plainOpenAIHeaders
		? Object.fromEntries(
			Object.entries(additionalHeaders ?? {}).filter(([key]) =>
				!/^(OpenAI-|X-GitHub-|X-Interaction|X-Agent-|X-Initiator|Singularity-)/i.test(key)
			),
		)
		: (additionalHeaders ?? {});

	const headers: ReqHeaders = endpointFetchOptions?.plainOpenAIHeaders
		? {
	...
	if (!endpointFetchOptions?.plainOpenAIHeaders) {
	...
	const request: FetchOptions = {
		...
		useFetcher: endpointFetchOptions?.useFetcher ?? useFetcher,
