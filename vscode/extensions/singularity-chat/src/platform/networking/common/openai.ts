...
import { ISingularityReference, RequestId } from './fetch';
...
export interface APIUsage {
	 ...
	 *
	 * @remark it's an optional field because Singularity Proxy returns this information but not CAPI as of 18 Jun 2025
	 */
	...
	/**
	 * Singularity billing usage for this request, returned by CAPI.
	 */
	singularity_usage?: {
...
/**
 * Converts a nano-AIU value from singularity_usage to a credit number.
...
export type CAPIChatMessage = OpenAI.ChatMessage & {
	 ...
	 */
	singularity_references?: ISingularityReference[];
	/**
	 * CAPI confirmations used in this message.
	 */
	singularity_confirmations?: { state: string; confirmation: any }[];

	singularity_cache_control?: {
		'type': 'ephemeral';
	};
 ...
 *
 * **Extra:** the raw message can have `singularity_references` and
 * `singularity_confirmations` properties, which are copied to the CAPI message.
 */
...
export function rawMessageToCAPI(message: Raw.ChatMessage[] | Raw.ChatMessage, callback?: RawMessageConversionCallback): CAPIChatMessage | CAPIChatMessage[] {
	...
	if ('singularity_references' in message) {
		out.singularity_references = (message as any).singularity_references;
	}
	if ('singularity_confirmations' in message) {
		out.singularity_confirmations = (message as any).singularity_confirmations;
	}
	...
	if (message.content.find(part => part.type === ChatCompletionContentPartKind.CacheBreakpoint)) {
		out.singularity_cache_control = { type: 'ephemeral' };
	}
