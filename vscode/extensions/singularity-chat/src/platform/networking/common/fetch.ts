...
export function getRequestId(headers: IHeaders, json?: any): RequestId {
	const serverExperiments = headers.get('X-Singularity-Experiment') || '';
	const capiExpAssignmentContext = headers.get('x-singularity-api-exp-assignment-context') || '';
	return {
...
export function isSingularityAnnotation(thing: unknown): thing is ICodeVulnerabilityAnnotation {
...
export interface ISingularityReference {
	type: string;
	id: string;
...
export interface ISingularityToolCall {
	name: string;
	arguments: string;
	id: string;
}

export interface ISingularityToolCallStreamUpdate {
	name: string;
	arguments: string;
	id?: string;
}

export interface ISingularityBeginToolCall {
	name: string;
	id?: string;
}
 ...
 * @deprecated
 */
export interface ISingularityFunctionCall {
	name: string;
	arguments: string;
}

export interface ISingularityError {
	type: string;
	code: string;
	message: string;
	agent: string;
...
export function isSingularityWebReference(reference: unknown) {
	return typeof reference === 'object' && !!reference && 'title' in reference && 'excerpt' in reference && 'url' in reference;
}

export interface ISingularityWebReference {
	title: string;
	excerpt: string;
	url: string;
}

export interface ISingularityConfirmation {
	title: string;
	message: string;
	confirmation: any;
}
...
export interface IResponseDelta {
	...
	singularityReferences?: ISingularityReference[];
	singularityErrors?: ISingularityError[];
	singularityToolCalls?: ISingularityToolCall[];
	singularityToolCallStreamUpdates?: ISingularityToolCallStreamUpdate[];
	beginToolCalls?: ISingularityBeginToolCall[];
	_deprecatedSingularityFunctionCalls?: ISingularityFunctionCall[];
	singularityConfirmation?: ISingularityConfirmation;
...
export interface IToolCallResponsePart extends ISingularityToolCall {
...
export interface IConfirmationResponsePart extends ISingularityConfirmation {
...
export interface OptionalChatRequestParams {
	...
	// TODO@ulugbekna: not sure params below are supported by Singularity proxy
	...
	singularity_thread_id?: string;
	singularity_skills?: string[];
