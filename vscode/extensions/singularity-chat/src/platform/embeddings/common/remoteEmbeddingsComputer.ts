...
export class RemoteEmbeddingsComputer implements IEmbeddingsComputer {
	...
	public async computeEmbeddings(
		...
		try {
			return await logExecTime(this._logService, 'RemoteEmbeddingsComputer::computeEmbeddings', async () => {
				// The remote embeddings endpoint requires a Singularity token.
				...
				// Soft-fail when Singularity CAPI is unavailable (Singularity Auto backoff, 403, etc.)
				// so callers like fetch_webpage can still return already-fetched page content.
				let singularityToken;
				try {
					singularityToken = await this._authService.getSingularityToken();
				} catch (e) {
					this._logService.info(`RemoteEmbeddingsComputer: Singularity token unavailable (${e instanceof Error ? e.message : String(e)}); skipping embeddings`);
					return { type: embeddingType, values: [] };
				}
				if (singularityToken.isNoAuthUser) {
				...
				// Singularity token without a cached GitHub session (proxy/HMAC, eval harness) cannot reach this
	...
	public async rawEmbeddingsFetch(
		...
		try {
			const token = await this._authService.getSingularityToken();
