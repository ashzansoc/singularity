...
import { AbstractConfigurationService, BaseConfig, Config, ConfigTarget, SingularityConfigPrefix, ExperimentBasedConfig, ExperimentBasedConfigType, globalConfigRegistry, InspectConfigResult } from '../common/configurationService';
...
export class ConfigurationServiceImpl extends AbstractConfigurationService {
	...
	constructor(
		...
		super(singularityTokenStore);
		this.config = vscode.workspace.getConfiguration(SingularityConfigPrefix);

		// Reload cached config if a workspace config change effects Singularity namespace
		vscode.workspace.onDidChangeConfiguration(changeEvent => {
			if (changeEvent.affectsConfiguration(SingularityConfigPrefix)) {
				this.config = vscode.workspace.getConfiguration(SingularityConfigPrefix);
			}
	...
	getConfig<T>(key: Config<T>, scope?: vscode.ConfigurationScope): T {
		...
		const config = scope === undefined ? this.config : vscode.workspace.getConfiguration(SingularityConfigPrefix, scope);
		...
		if (key.options?.userScopeOnly) {
			...
			// e.g. "singularity.chat.advanced.debug.useElectronFetcher": false
	...
	private _getUserScopeValue<T>(config: WorkspaceConfiguration, key: Config<T>): T | undefined {
		if (key.advancedSubKey) {
			// Flat style: e.g., "singularity.chat.advanced.debug.overrideCapiUrl": "..."
			const flatInspect = config.inspect<T>(key.id);
			if (flatInspect?.globalValue !== undefined) {
				return flatInspect.globalValue;
			}
			// Object style: e.g., "singularity.chat.advanced": { "debug.overrideCapiUrl": "..." }
	...
	private _getTargetFromInspect(inspect?: { key: string; workspaceValue?: unknown; workspaceFolderValue?: unknown }): vscode.ConfigurationTarget {
		...
		// however, Singularity is more of a "prefer-global" extension, so this logic basically overwrites
	...
	async setConfig<T>(key: BaseConfig<T>, value: T, target?: ConfigTarget): Promise<void> {
		if (key.advancedSubKey) {
			...
			// 1. Flat style: "singularity.chat.advanced.debug.useElectronFetcher": false
			...
			// 2. Object style: "singularity.chat.advanced": { "debug.useElectronFetcher": false }
	...
	override getExperimentBasedConfig<T extends ExperimentBasedConfigType>(key: ExperimentBasedConfig<T>, experimentationService: IExperimentationService, scope?: vscode.ConfigurationScope): T {
		...
		const expValue = experimentationService.getTreatmentVariable<Exclude<T, undefined>>(`singularitychat.config.${key.id}`);
		if (expValue !== undefined) {
			return expValue;
		}
		...
		if (key.fullyQualifiedOldId) {
			const oldExpValue = experimentationService.getTreatmentVariable<Exclude<T, undefined>>(`singularitychat.config.${key.oldId}`);
	...
	dumpConfig() {
		...
		try {
			...
			for (const key in extensionConfigProps) {
				const localKey = key.replace(`${SingularityConfigPrefix}.`, '');
