...
export const SingularityConfigPrefix = 'singularity.chat';
...
export interface IConfigurationService {
	...
	/**
	 * Proxies vscode.workspace.getConfiguration to allow getting a configuration value that is not in the Singularity namespace.
	 * @param configKey The config key to look up
	 */
	 ...
	 * This emits for all changes, not just changes to the Singularity settings.
	 */
...
export abstract class AbstractConfigurationService extends Disposable implements IConfigurationService {
	...
	constructor(singularityTokenStore?: ISingularityTokenStore) {
		super();
		if (singularityTokenStore) {
			this._register(singularityTokenStore.onDidStoreUpdate(() => {
				this._setUserInfo({
					isInternal: !!singularityTokenStore.singularityToken?.isInternal
				});
			}));
		}
	}
...
export interface BaseConfig<T> {
	/**
	 * Key as it appears in settings.json minus the "singularity.chat." prefix.
	 * e.g. "advanced.debug.overrideProxyUrl"
	 */
	readonly id: string;

	/**
	 * The old key as it appears in settings.json minus the "singularity.chat." prefix.
	 */
	...
	/**
	 * The fully qualified id, e.g. "singularity.chat.advanced.debug.overrideProxyUrl".
	 * Use this with `affectsConfiguration` from the ConfigurationChangeEvent
	 */
	readonly fullyQualifiedId: string;

	/**
	 * The fully qualified old id, e.g. "singularity.chat.advanced.debug.overrideProxyUrl".
	 */
	readonly fullyQualifiedOldId?: string | undefined;

	/**
	 * The `X` in `singularity.chat.advanced.X` settings.
	 */
...
function toBaseConfig<T>(key: string, defaultValue: T, options: ConfigOptions | undefined): BaseConfig<T> {
	const fullyQualifiedId = `${SingularityConfigPrefix}.${key}`;
	const fullyQualifiedOldId = options?.oldKey ? `${SingularityConfigPrefix}.${options.oldKey}` : undefined;
	...
	const advancedSubKey = fullyQualifiedId.startsWith('singularity.chat.advanced.') ? fullyQualifiedId.substring('singularity.chat.advanced.'.length) : undefined;
	return { id: key, oldId: options?.oldKey, isPublic, fullyQualifiedId, fullyQualifiedOldId, advancedSubKey, defaultValue, options };
}

class ConfigRegistry {
	/**
	 * A map of all registered configs, keyed by their full id, eg `singularity.chat.advanced.debug.overrideProxyUrl`.
	 */
...
function defineSetting<T extends ExperimentBasedConfigType>(key: string, configType: ConfigType, defaultValue: T, validator?: IValidator<T>, options?: ConfigOptions, expOptions?: { experimentName?: string }): Config<T> | ExperimentBasedConfig<T> {
	if (configType === ConfigType.ExperimentBased) {
		...
		if (value.advancedSubKey) {
			// This is a `singularity.chat.advanced.*` setting
...
function migrateSetting(newKey: string, oldKey: string): void {
	ConfigurationMigrationRegistry.registerConfigurationMigrations([{
		key: `${SingularityConfigPrefix}.${oldKey}`,
		migrateFn: async (migrationValue: any) => {
			return [
				[`${SingularityConfigPrefix}.${newKey}`, { value: migrationValue }],
				[`${SingularityConfigPrefix}.${oldKey}`, { value: undefined }]
			];
		}
	}]);
}
...
export const enum CHAT_MODEL {
	...
	NES_XTAB = 'singularity-nes-xtab', // xtab model hosted in prod in proxy
...
export namespace ConfigKey {
	...
	*/
	...
	export namespace Advanced {
		...
		*/
		 ...
		 */
		export const AutoModeV2Enabled = defineSetting<boolean>('chat.autoMode.v2.enabled', ConfigType.ExperimentBased, true, undefined, undefined, { experimentName: 'singularitychat.autoModeV2Enabled' });
		/**
		 * Prefer the local Singularity cost-aware router for Auto model selection
		 * instead of CAPI Auto v2 / hydra. Falls back to legacy Auto when routing fails.
		 */
		export const SingularityRouterEnabled = defineSetting<boolean>('chat.singularityRouter.enabled', ConfigType.Simple, true);
		/** Enrich chat prompts via Singularity Prompt Engine v2 (graph → IR). */
		export const SingularityPromptEngineEnabled = defineSetting<boolean>('chat.singularityPromptEngine.enabled', ConfigType.Simple, true);
		/**
		 * Visual Critic stop-hook loop for frontend Agent sessions.
		 * Off = design-spec once + single implement pass; On = iterate until pass/max.
		 */
		export const VisualCriticEnabled = defineSetting<boolean>('chat.visualCritic.enabled', ConfigType.Simple, false);
		...
		export const WorkspaceEnableCodeSearchExternalIngest = defineSetting<boolean>('chat.workspace.codeSearchExternalIngest.enabled', ConfigType.ExperimentBased, true, undefined, undefined, { experimentName: 'singularitychat.config.chat.advanced.workspace.codeSearchExternalIngest.enabled' });
		...
		/** When enabled, the main agent's manage_todo_list tool is disabled and a background singularity-utility-small model maintains the todo list instead. */
	...
	*/
	export namespace TeamInternal {
		...
		export const InlineEditsYieldToSingularity = defineTeamInternalSetting<boolean>('chat.advanced.inlineEdits.yieldToSingularity', ConfigType.ExperimentBased, false);
	...
	/** Whether the Singularity TypeScript context provider is enabled and if how */
	...
	export const TerminalToDebuggerEnabled = defineSetting('chat.singularityDebugCommand.enabled', ConfigType.Simple, true);
