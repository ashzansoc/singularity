...
export abstract class BaseCAPIClientService extends CAPIClient implements ICAPIClientService {
	...
	override makeRequest<T>(request: MakeRequestOptions, requestMetadata: RequestMetadata): Promise<T> {
		// Inject AB Exp Context headers (legacy VScode-ABExpContext and new standardized X-Singularity-Client-Exp-Assignment-Context) if available
		if (this.abExpContext) {
			if (!request.headers) {
				request.headers = {};
			}
			request.headers['VScode-ABExpContext'] = this.abExpContext;
			request.headers['X-Singularity-Client-Exp-Assignment-Context'] = this.abExpContext;
		}
	...
	override createResponsesWebSocket(options: { headers?: Record<string, string> }): Promise<WebSocketConnection> {
		if (this.abExpContext) {
			...
			options.headers['X-Singularity-Client-Exp-Assignment-Context'] = this.abExpContext;
		}
