...
export class SingularityChatEndpoint extends ChatEndpoint {
	constructor(
...
/**
 * Resolves the built-in Singularity model used for the `singularity-utility-small`
 * internal family (formerly `singularity-fast`). This is the small/fast model
 ...
 */
export class SingularityUtilitySmallChatEndpoint {
	...
	static async resolve(modelFetcher: IModelMetadataFetcher, instantiationService: IInstantiationService): Promise<IChatEndpoint> {
		...
		try {
			modelMetadata = await modelFetcher.getChatModelFromCapiFamily(SingularityUtilitySmallChatEndpoint.capiFamily);
		} catch {
			...
			// `singularity-utility-small` caller.
			modelMetadata = await modelFetcher.getSingularityUtilityModel();
		}
		return instantiationService.createInstance(SingularityChatEndpoint, modelMetadata);
	}
}

/**
 * Resolves the built-in Singularity model used for the `singularity-utility`
 * internal family (formerly `singularity-base`). This is the API-marked
 * default base model — whichever model the CAPI `/models` response
 * flags with `is_chat_fallback === true`.
 */
export class SingularityUtilityChatEndpoint {
	static async resolve(modelFetcher: IModelMetadataFetcher, instantiationService: IInstantiationService): Promise<IChatEndpoint> {
		const modelMetadata = await modelFetcher.getSingularityUtilityModel();
		return instantiationService.createInstance(SingularityChatEndpoint, modelMetadata);
	}
}
