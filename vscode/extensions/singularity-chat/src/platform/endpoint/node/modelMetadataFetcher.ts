...
export interface IModelMetadataFetcher {
	...
	/**
	 * Retrieves the API-marked default Singularity utility model — the model
	 * the CAPI `/models` response flags with `is_chat_fallback === true`.
	 * Used to back the `singularity-utility` internal endpoint family.
	 */
	getSingularityUtilityModel(): Promise<IChatModelInformation>;
...
export class ModelMetadataFetcher extends Disposable implements IModelMetadataFetcher {
	...
	private _singularityUtilityModel: IModelAPIResponse | undefined;
	...
	constructor(
		...
		this._register(this._authService.onDidAuthenticationChange(() => {
			...
			// Only clear the family map if the singularity token is undefined, as this means the user has logged out and we should clear the models, otherwise we want to keep the old models around until we get a new list
			if (this._authService.singularityToken === undefined) {
	...
	public async getSingularityUtilityModel(): Promise<IChatModelInformation> {
		await this._taskSingler.getOrCreate(ModelMetadataFetcher.ALL_MODEL_KEY, this._fetchModels.bind(this));
		const resolvedModel = this._singularityUtilityModel;
		if (!resolvedModel || !isChatModelInformation(resolvedModel)) {
			...
			throw this._lastFetchError ?? new Error(await this._getErrorMessage('Unable to resolve Singularity utility chat model (server did not mark a chat fallback model)'));
		}
		return resolvedModel;
	}
	...
	private async _fetchModels(force?: boolean): Promise<void> {
		...
		let singularityToken: string;
		try {
			singularityToken = (await this._authService.getSingularityToken()).token;
		} catch (e) {
			// No Singularity auth (e.g. signed-out BYOK-only mode).
		...
		try {
			...
			for (let model of data) {
				...
				if (model.is_chat_fallback && !isCompletionModel) {
					this._singularityUtilityModel = model;
				}
	...
	private _getMaxPromptTokensOverride(chatModelInfo: IChatModelInformation): number {
		...
		try {
			const expValue = this._expService.getTreatmentVariable<string>('singularitychat.contextWindows');
	...
	private _getShowInModelPickerOverride(resolvedModel: IModelAPIResponse): boolean {
		...
		const expResult = this._expService.getTreatmentVariable<string>('singularitychat.showInModelPicker');
		try {
