...
describe('OpenAIResponsesProcessor tool search events', () => {
	...
	it('handles client tool_search_call as singularityToolCall', () => {
		...
		// Second delta: completed singularityToolCall
		expect(deltas[1].singularityToolCalls).toBeDefined();
		expect(deltas[1].singularityToolCalls![0]).toMatchObject({
			id: 'call_abc',
			name: 'tool_search',
			arguments: '{"query":"Find shipping tools"}',
		});
