...
import { FinishedCallback, getRequestId, ISingularityToolCall, OptionalChatRequestParams } from '../../networking/common/fetch';
...
export async function defaultNonStreamChatResponseProcessor(response: Response, finishCallback: FinishedCallback, telemetryData: TelemetryData) {
	...
	for (let i = 0; i < (jsonResponse?.choices?.length || 0); i++) {
		...
		// Thinking models often put the answer in reasoning_content
		// and leave content empty — promote so Agent/Auto don't show an empty-response error.
		const rawContent = rawMessage.content;
		const reasoningFallback =
			(typeof rawMessage.reasoning_content === 'string' && rawMessage.reasoning_content) ||
			(typeof rawMessage.reasoning === 'string' && rawMessage.reasoning) ||
			'';
		const resolvedContent =
			(typeof rawContent === 'string' && rawContent.length > 0 ? rawContent : undefined)
			?? (Array.isArray(rawContent) && rawContent.length ? rawContent : undefined)
			?? (reasoningFallback || undefined)
			?? rawContent
			?? '';
		const message: Raw.AssistantChatMessage = {
			...
			toolCalls: rawMessage.toolCalls ?? rawMessage.tool_calls,
		};
		...
		const functionCall: ISingularityToolCall[] = [];
		...
		await finishCallback(messageText, i, {
			text: messageText,
			singularityToolCalls: functionCall,
