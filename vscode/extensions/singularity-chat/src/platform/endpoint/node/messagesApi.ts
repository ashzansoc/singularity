...
/** IP Code Citation annotation from Messages API singularity_annotations */
...
interface AnthropicCompletionState {
	...
	readonly singularityUsage?: { total_nano_aiu: number };
}
...
function buildAnthropicCompletion(state: AnthropicCompletionState, logService: ILogService): ChatCompletion {
	...
	return {
		...
		usage: {
			...
			singularity_usage: state.singularityUsage,
		},
...
export async function processNonStreamingResponseFromMessagesEndpoint(
	...
	return new AsyncIterableObject<ChatCompletion>(async feed => {
		...
		const delta: IResponseDelta = {
			text: textContent,
			...(toolCalls.length > 0 ? {
				singularityToolCalls: toolCalls.map(tc => ({
					id: tc.id,
					name: tc.name,
					arguments: tc.arguments,
				})),
...
export class AnthropicMessagesProcessor {
	...
	private singularityUsage?: { total_nano_aiu: number };
	...
	/**
	 * Extract IP code citations from singularity_annotations and convert to IIPCodeCitation format
	 */
	...
	public push(chunk: AnthropicStreamEvent, _onProgress: FinishedCallback): ChatCompletion | undefined {
		...
		switch (chunk.type) {
			...
			case 'content_block_delta':
				if (chunk.delta) {
					if (chunk.delta.type === 'text_delta' && chunk.delta.text) {
						const ipCitations = this.extractIPCodeCitations(chunk.singularity_annotations);
			...
			case 'message_delta':
				...
				if (chunk.singularity_usage && typeof chunk.singularity_usage.total_nano_aiu === 'number') {
					this.singularityUsage = chunk.singularity_usage;
				}
			...
			case 'message_stop': {
				...
				return buildAnthropicCompletion({
					...
					singularityUsage: this.singularityUsage,
				}, this.logService);
			}
			case 'error': {
