	...
	public push(chunk: OpenAI.Responses.ResponseStreamEvent, _onProgress: FinishedCallback): ChatCompletion | undefined {
		...
		switch (chunk.type) {
			case 'error':
				...
				onProgress({ text: '', singularityErrors: [{ agent: 'openai', code: chunk.code || 'unknown', message: chunk.message, type: 'error', identifier: chunk.param || undefined }] });
			...
			case 'response.output_item.done':
				if (chunk.item.type === 'function_call') {
					...
					if (tsCall.execution === 'client' && tsCall.call_id) {
						// Client-executed tool search completed: emit as a completed singularityToolCall
						this.toolCallInfo.delete(chunk.output_index);
						onProgress({
							text: '',
							singularityToolCalls: [{
			...
			case 'response.completed': {
				...
				return {
					...
					usage: {
						...
						singularity_usage: capiChunk.singularity_usage?.total_nano_aiu !== undefined ? capiChunk.singularity_usage : undefined,
					},
