...
import { SingularityUtilityChatEndpoint, SingularityUtilitySmallChatEndpoint } from '../../node/singularityChatEndpoint';
...
class FakeModelMetadataFetcher implements IModelMetadataFetcher {
	...
	async getSingularityUtilityModel(): Promise<IChatModelInformation> {
		if (!this._utilityModel) {
			throw new Error('Unable to resolve Singularity utility chat model (server did not mark a chat fallback model)');
		}
 ...
 * ultimately selected without building the full `SingularityChatEndpoint`.
 */
...
describe('SingularityUtilitySmallChatEndpoint.resolve (issue #321184)', () => {
	test('falls back to the base utility model when the gpt-4o-mini family is absent from /models', async () => {
		...
		// than letting the lookup throw through every singularity-utility-small caller.
		await SingularityUtilitySmallChatEndpoint.resolve(fetcher, service);
	...
	test('sibling SingularityUtilityChatEndpoint.resolve already degrades gracefully', async () => {
		// The sibling resolver is unaffected because it uses getSingularityUtilityModel().
		const fetcher = new FakeModelMetadataFetcher([
			chatModel('gpt-4.1', 'gpt-4.1', /* isChatFallback */ true),
		]);
		const { service, lastModel } = fakeInstantiationService();

		await SingularityUtilityChatEndpoint.resolve(fetcher, service);
