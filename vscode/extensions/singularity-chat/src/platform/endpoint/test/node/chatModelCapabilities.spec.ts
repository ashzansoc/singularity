...
import { getModelCapabilityOverride, isGpt56, isKimiFamily, modelCanUseApplyPatchExclusively, modelCanUseReplaceStringExclusively, modelSupportsApplyPatch, modelSupportsContextEditing, modelSupportsMultiReplaceString, modelSupportsPDFDocuments, modelSupportsReplaceString, modelSupportsToolSearch } from '../../common/chatModelCapabilities';
...
describe('isGpt56', () => {
	test('matches bare and provider-prefixed luna/sol/terra ids', () => {
		expect(isGpt56('gpt-5.6-luna')).toBe(true);
		expect(isGpt56('openai/gpt-5.6-luna')).toBe(true);
		expect(isGpt56(fakeModel('openai/gpt-5.6-luna'))).toBe(true);
		expect(isGpt56(fakeModel('gpt-5.6-sol'))).toBe(true);
		expect(isGpt56(fakeModel('unknown', 'openai/gpt-5.6-terra'))).toBe(true);
		expect(isGpt56('gpt-5.5')).toBe(false);
		expect(isGpt56(fakeModel('gpt-5.4'))).toBe(false);
	});
});
