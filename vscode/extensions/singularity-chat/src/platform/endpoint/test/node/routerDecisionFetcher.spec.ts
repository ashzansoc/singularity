...
describe('RouterDecisionFetcher', () => {
	test('includes singularity_plan in request body from rawSingularityPlan', async () => {
		let capturedBody: string | undefined;

		const mockAuthService = {
			getSingularityToken: vi.fn().mockResolvedValue({
				token: 'test-token',
				rawSingularityPlan: 'individual_edu',
				singularityPlan: 'individual', // normalized — should NOT be used
			}),
			onDidChangeSingularityToken: vi.fn(),
		};
		...
		expect(parsed.singularity_plan).toBe('individual_edu');
