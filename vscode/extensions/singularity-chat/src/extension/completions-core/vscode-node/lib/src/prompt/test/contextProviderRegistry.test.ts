...
suite('ContextProviderRegistry', function () {
		...
		for (const provider of ['enabledProvider', '*']) {
			test(`enable ${provider} provider(s) via ${method}`, async function () {
				if (method === 'feature_flag') {
					telemetryData.filtersAndExp.exp.variables.singularitycontextproviders = provider;
				} else {
					telemetryData.filtersAndExp.exp.variables.singularitycontextproviders = '';
	...
	test('all providers are enabled in debug mode', async function () {
		...
		telemetryData.filtersAndExp.exp.variables.singularitycontextproviders = '';
		serviceCollectionClone.define(ICompletionsRuntimeModeService, RuntimeMode.fromEnvironment(false, [], { GITHUB_SINGULARITY_DEBUG: 'true' }));
