...
import { SingularityCompletion } from './singularityCompletion';
...
export interface ICompletionsLastGhostText {
	...
	shownCompletions: SingularityCompletion[];
...
export function handleGhostTextShown(accessor: ServicesAccessor, cmp: SingularityCompletion) {
 ...
 */
function handleLineAcceptance(accessor: ServicesAccessor, cmp: SingularityCompletion, acceptedLength: number) {
	const last = accessor.get(ICompletionsLastGhostText);
