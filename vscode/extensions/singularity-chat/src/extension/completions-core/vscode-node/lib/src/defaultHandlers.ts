...
export function handleException(accessor: ServicesAccessor, err: unknown, origin: string, _logger: Logger = logger): void {
	...
	if (err instanceof Error) {
		...
		} else if (error.code === 'SingularityPromptLoadFailure') {
			statusReporter.setWarning('Corrupted Singularity installation');
		} else if (`${error.code}`.startsWith('SingularityPromptWorkerExit')) {
