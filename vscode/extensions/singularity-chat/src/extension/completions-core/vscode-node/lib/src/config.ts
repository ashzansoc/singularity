...
import { SingularityConfigPrefix } from './constants';
...
export const ConfigKey = {
	 ...
	 * https://github.com/github/singularity/issues/19286
	 */
...
export interface ICompletionsConfigProvider {
	...
	onDidChangeSingularitySettings: Event<ConfigProvider>;
}
...
export abstract class ConfigProvider implements ICompletionsConfigProvider {
	...
	abstract onDidChangeSingularitySettings: Event<ConfigProvider>;
...
export class DefaultsOnlyConfigProvider extends ConfigProvider {
	...
	override onDidChangeSingularitySettings = () => {
...
export class InMemoryConfigProvider extends ConfigProvider {
	protected readonly singularityEmitter = new Emitter<this>();
	readonly onDidChangeSingularitySettings = this.singularityEmitter.event;
	...
	setConfig(key: ConfigKeyType, value: unknown): void {
		this.setSingularitySettings({ [key]: value });
	}

	setSingularitySettings(settings: Record<ConfigKeyType, unknown>): void {
		...
		this.singularityEmitter.fire(this);
	}
...
export function getConfigDefaultForKey<T>(key: string): T {
	...
	throw new Error(`Missing config default value: ${SingularityConfigPrefix}.${key}`);
}
...
export function editorVersionHeaders(accessor: ServicesAccessor): { [key: string]: string } {
	...
	return {
		...
		'Singularity-Language-Server-Version': BuildInfo.getVersion(),
	};
}
