...
export class StreamedCompletionSplitter {
	...
	private cacheCompletion(delta: RequestDelta, completion: StreamingCompletion, offset?: number) {
		...
		apiChoice.singularityAnnotations = this.adjustedAnnotations(apiChoice, completion, trimmed);
	...
	private adjustedAnnotations(
	...
	): SingularityNamedAnnotationList | undefined {
		if (choice.singularityAnnotations === undefined) { return undefined; }
		...
		const adjusted: SingularityNamedAnnotationList = {};
		for (const [name, annotationGroup] of Object.entries(choice.singularityAnnotations)) {
