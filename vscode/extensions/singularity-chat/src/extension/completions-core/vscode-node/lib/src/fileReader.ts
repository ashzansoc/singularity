...
import { SingularityTextDocument, ITextDocument, TextDocumentIdentifier, TextDocumentResult } from './textDocument';
	...
	protected async readFile(uri: string): Promise<TextDocumentResult<ITextDocument>> {
		...
		try {
			...
			if (rcmResult.status === 'valid') {
				const doc = SingularityTextDocument.create(uri, 'UNKNOWN', -1, text);
