...
export type GetGhostTextOptions = ExtractPromptOptions & {
	 ...
	 * Overridable via config or exp variable: `singularityvscodedebouncethreshold`.
	 */
	debounceMs?: number;
};
...
export class GhostTextComputer {
	...
	private async getGhostTextWithoutAbortHandling(
		...
		if (prompt.type === 'singularityContentExclusion') {
			logger.debug('Singularity not available, due to content exclusion');
			return {
				type: 'abortedBeforeIssued',
				reason: 'Singularity not available due to content exclusion',
				telemetryData: mkBasicResultTelemetry(preIssuedTelemetryDataWithExp),
			};
		}
		...
		return this.statusReporter.withProgress(async () => {
			...
			requestContext.headers = {
				...requestContext.headers,
				'X-Singularity-Async': 'true',
				'X-Singularity-Speculative': ghostTextOptions.isSpeculative ? 'true' : 'false',
			};
			...
			for (const choice of postProcessedChoicesArray) {
				...
				const res: CompletionResult = {
					...
					suffixCoverage,
					singularityAnnotations: choice.singularityAnnotations,
