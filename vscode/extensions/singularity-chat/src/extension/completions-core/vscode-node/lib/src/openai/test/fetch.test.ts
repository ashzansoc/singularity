...
suite('"Fetch" unit tests', function () {
	...
	let resetSpy: Sinon.SinonSpy<Parameters<ICompletionsSingularityTokenManager['resetToken']>>;
	...
	setup(function () {
		...
		resetSpy = Sinon.spy(accessor.get(ICompletionsSingularityTokenManager), 'resetToken');
	});

	test('Empty/whitespace completions are stripped', async function () {
		const fetcher = new SyntheticCompletions(['', ' ', '\n'], accessor.get(ICompletionsSingularityTokenManager));
		const params: CompletionParams = {
			...
			engineModelId: '',
			count: 1,
			uiKind: SingularityUiKind.GhostText,
	...
	test('If in the split context experiment, send the context field as part of the request', async function () {
		...
		const params: CompletionParams = {
			...
			languageId: 'python',
			repoInfo: undefined,
			engineModelId: 'singularity-codex',
			count: 1,
			uiKind: SingularityUiKind.GhostText,
			postOptions: {},
			ourRequestId: generateUuid(),
		...
		telemetryWithExp.filtersAndExp.exp.variables.singularityenablepromptcontextproxyfield = true;
	...
	test('calls out possible proxy for 4xx requests without x-github-request-id', async function () {
		...
		assert.deepStrictEqual(
			...
			'Last response was a 418 error and does not appear to originate from GitHub. Is a proxy or firewall intercepting this request? https://gh.io/singularity-firewall'
		);
	});
	...
	test.skip('properly handles 402 (free plan exhausted) responses from proxy', async function () {
		const fetcherService = accessor.get(ICompletionsOpenAIFetcherService);
		const tokenManager = accessor.get(ICompletionsSingularityTokenManager);
...
async function assertResponseWithStatus(
	...
	serviceCollection.define(ICompletionsOpenAIFetcherService, new SyncDescriptor(LiveOpenAIFetcher));
	const accessor = serviceCollection.createTestingAccessor();
	const singularityTokenManager = accessor.get(ICompletionsSingularityTokenManager);
	await singularityTokenManager.primeToken(); // Trigger initial status
