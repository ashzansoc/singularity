...
import { SingularityAnnotations, StreamSingularityAnnotations } from '../../../../../../platform/completions-core/common/openai/singularityAnnotations';
...
import { SingularityToken, ICompletionsSingularityTokenManager } from '../auth/singularityTokenManager';
...
type CompletionFetchRequestFields = {
	...
	/** Singularity-only: NWO of repository, if any */
	nwo?: string;
	/**
...
/** OAI API completion request, along with additional fields specific to Singularity. */
export type CompletionRequest = BaseFetchRequest &
	CompletionFetchRequestFields & {
		/** Singularity-only: extra arguments for completion processing. */
		extra: Partial<CompletionRequestExtra>;
	};

/**
 * Completion request arguments that are Singularity-specific and don't exist in
 * the OAI API.
 */
...
function uiKindToIntent(uiKind: SingularityUiKind): string | undefined {
	switch (uiKind) {
		case SingularityUiKind.GhostText:
			return 'singularity-ghost';
		case SingularityUiKind.Panel:
			return 'singularity-panel';
	}
}

// Request methods

export interface SingularityError {
	type: string;
	code: string;
	message: string;
	identifier: string;
...
export interface SingularityConfirmation {
	type: string;
	title: string;
	message: string;
...
export interface SingularityReference {
	type: string;
	id: string;
...
export interface RequestDelta {
	...
	index?: number;
	requestId?: RequestId;
	annotations?: SingularityAnnotations;
	singularityErrors?: SingularityError[];
	singularityConfirmation?: SingularityConfirmation;
	singularityReferences?: SingularityReference[];
...
export type CompletionHeaders = {
	...
	Host?: string;
	Connection?: string;
	'X-Singularity-Async'?: string;
	'X-Singularity-Speculative'?: string;
};

function getProxyEngineUrl(accessor: ServicesAccessor, token: SingularityToken, modelId: string, endpoint: string): string {
...
export const CMDQuotaExceeded = 'singularity.chat.completions.quotaExceeded';
...
export class LiveOpenAIFetcher extends OpenAIFetcher {
	...
	constructor(
		...
		@ICompletionsSingularityTokenManager private readonly singularityTokenManager: ICompletionsSingularityTokenManager,
	...
	async fetchAndStreamCompletions(
		...
		const singularityToken = this.singularityTokenManager.token ?? await this.singularityTokenManager.getToken();
...

		{
			...
			const uri = this.instantiationService.invokeFunction(getProxyEngineUrl, singularityToken, engineModelId, endpoint);
...

			{
				...
				fullHeaders['Openai-Organization'] = 'github-singularity';
	...
	public static async *convertStreamToApiChoices(resp: ResponseStream, finishedCb: FinishedCallback, baseTelemetryData: TelemetryWithExp, cancel?: CancellationToken): AsyncIterable<APIChoice> {
		...
		const createAPIChoice = (
			...
			singularityAnnotations: accumulator.annotations.current,
		});
					...
					if (hasFinishReason || hasNewLine) {
						...
						solutionDecision = await finishedCb(completion.accumulator.responseSoFar, {
							...
							getAPIJsonData: () => ({
								...
								singularity_annotations: completion.accumulator.annotations.current,
	...
	async handleError(
		...
		singularityToken: SingularityToken
		...
		if (response.status === 402) {
			...
			const event = onSingularityToken(this.authenticationService, t => {
		...
		if (isClientError(response) && !response.headers.get('x-github-request-id')) {
			const message = `Last response was a ${response.status} error and does not appear to originate from GitHub. Is a proxy or firewall intercepting this request? https://gh.io/singularity-firewall`;
		...
		if (response.status === 401 || response.status === 403) {
			...
			this.singularityTokenManager.resetToken(response.status);
...
class CompletionAccumulator {
	...
	public readonly annotations: SingularityAnnotations = new StreamSingularityAnnotations();
	...
	public append(choice: Completion.Choice): void {
		...
		if (choice.singularity_annotations) {
			this.annotations.update(choice.singularity_annotations);
		}
	}
}
