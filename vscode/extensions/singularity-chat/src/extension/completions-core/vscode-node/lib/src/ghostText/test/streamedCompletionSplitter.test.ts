...
import { CompletionResults, SingularityUiKind, ICompletionsOpenAIFetcherService, LiveOpenAIFetcher } from '../../openai/fetch';
...
class FakeCompletionsFetchService implements ICompletionsFetchService {
	...
	constructor(private readonly completionTexts: string[], private readonly annotations?: SingularityNamedAnnotationList) { }
	async fetch(
		_url: string,
		_secretKey: string,
		...
		async function* makeStream(): AsyncIterable<Completion> {
			const choices: Completion.Choice[] = completionTexts.map((text, i) => ({
				...
				text,
				singularity_annotations: annotations,
			}));
			yield {
				choices,
				system_fingerprint: '',
...
suite('StreamedCompletionSplitter', function () {
	...
	test('adjusts start_offset in any annotations present in cached split choices', async function () {
		...
		Sinon.assert.calledWith(
			...
			Sinon.match.string,
			Sinon.match({
				singularityAnnotations: Sinon.match({
	...
	test('omits any annotation from split choices where start_offset does not intersect the choice', async function () {
		...
		await readChoices(result);
		Sinon.assert.calledTwice(cacheFunction);
		Sinon.assert.calledWith(cacheFunction, Sinon.match.string, Sinon.match({ singularityAnnotations: undefined }));
		Sinon.assert.calledWith(
			cacheFunction,
			Sinon.match.string,
			Sinon.match({
