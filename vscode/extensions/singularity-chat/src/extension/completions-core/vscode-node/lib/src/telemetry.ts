...
export class TelemetryData {
	...
	extendWithEditorAgnosticFields(accessor: ServicesAccessor): void {
		...
		this.properties['singularity_version'] = `singularity/${BuildInfo.getVersion()}`;
	 ...
	 * e.g. { 'singularity.autocompletion.count': 3 }
	 */
	extendWithConfigProperties(accessor: ServicesAccessor): void {
		...
		configProperties['singularity.build'] = BuildInfo.getBuild();
		configProperties['singularity.buildType'] = BuildInfo.getBuildType();
...
/**
 * Send a telemetry message as-is, without the usual Singularity-specific processing from
...
function createRequiredProperties(accessor: ServicesAccessor) {
	...
	const properties: TelemetryProperties = {
		unique_id: generateUuid(), // add a unique id to the telemetry event so singularity-foundations can correlate with duplicate events
...
export abstract class SingularityTelemetryReporter {
...
export interface ICompletionsTelemetryReporters {
	...
	getReporter(accessor: ServicesAccessor, store?: TelemetryStore): SingularityTelemetryReporter | undefined;
	getEnhancedReporter(accessor: ServicesAccessor): SingularityTelemetryReporter | undefined;
	getFTReporter(accessor: ServicesAccessor): SingularityTelemetryReporter | undefined;
	setReporter(reporter: SingularityTelemetryReporter): void;
	setEnhancedReporter(reporter: SingularityTelemetryReporter): void;
	setFTReporter(reporter: SingularityTelemetryReporter): void;
...
export class TelemetryReporters implements ICompletionsTelemetryReporters {
	...
	private reporter: SingularityTelemetryReporter | undefined;
	private reporterEnhanced: SingularityTelemetryReporter | undefined;
	private reporterFT: SingularityTelemetryReporter | undefined;

	getReporter(accessor: ServicesAccessor, store = TelemetryStore.Standard): SingularityTelemetryReporter | undefined {
