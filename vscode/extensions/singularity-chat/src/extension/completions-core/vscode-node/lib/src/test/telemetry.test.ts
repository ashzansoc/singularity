...
import { NoopSingularityTelemetryReporter } from './noopTelemetry';
...
suite('Telemetry unit tests', function () {
	...
	test('Adds additional fields', async function () {
		...
		assert.ok(telemetry.properties.singularity_build);
		assert.ok(telemetry.properties.singularity_buildType);
		// assert.ok(telemetry.properties.singularity_trackingId);
		...
		assert.ok(telemetry.properties.singularity_version);
...
suite('TelemetryReporters unit tests', function () {
	test('deactivate is safe to call synchronously', async function () {
		const accessor = createLibTestingContext().createTestingAccessor();
		const oldRepoter = new NoopSingularityTelemetryReporter();
		const oldRestrictedReporter = new NoopSingularityTelemetryReporter();
