...
export class Features implements ICompletionsFeaturesService {
	...
	async updateExPValuesAndAssignments(
		...
		const token = this.singularityTokenManager.token ?? await this.singularityTokenManager.getToken();
	...
	/**
	 * Request a Singularity token and use that token to call updateExPValuesAndAssignments. Do NOT call this at startup.
	 * Instead, register a onSingularityToken handler and use that token with updateExPValuesAndAssignments directly.
	 */
	...
	private createExpConfigAndFilters(token: SingularityToken) {
	...
	/** @returns the string for singularitycustomengine, or "" if none is set. */
	...
	/** @returns the string for singularitycustomenginetargetengine, or undefined if none is set. */
	...
	getContextProviderExpSettings(languageId: string): ContextProviderExpSettings | undefined {
		const value = this.experimentationService.getTreatmentVariable<string>(`config.singularity.chat.chat.contextprovider.${languageId}`);
