...
async function citationCheck(
	...
	if (!singularityAnnotations || (singularityAnnotations.ip_code_citations?.length ?? 0) < 1) {
		// Do not request citations if in blocking mode
		if (singularityTokenManager.getLastToken()?.getTokenValue('sn') === '1') { return; }
	...
	for (const citation of singularityAnnotations.ip_code_citations) {
