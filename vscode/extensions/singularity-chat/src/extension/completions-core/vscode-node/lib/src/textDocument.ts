...
export class SingularityTextDocument implements ITextDocument {
	private constructor(
		readonly uri: string,
	...
	static withChanges(textDocument: ITextDocument, changes: TextDocumentContentChangeEvent[], version: number) {
		...
		return new SingularityTextDocument(textDocument.uri, lspDoc, textDocument.detectedLanguageId);
	}
	...
	applyEdits(edits: TextEdit[]) {
		...
		return new SingularityTextDocument(this.uri, lspDoc, this.detectedLanguageId);
	}
	...
	static create(
		...
		return new SingularityTextDocument(
			normalizeUri(uri),
