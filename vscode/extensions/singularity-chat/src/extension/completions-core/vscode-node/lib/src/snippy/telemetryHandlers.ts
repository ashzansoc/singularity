...
class SingularityOutputLogTelemetry extends CodeQuoteTelemetry {
	constructor() {
		super('github_singularity_log');
	}
...
export const singularityOutputLogTelemetry = new SingularityOutputLogTelemetry();
...
class SnippyTelemetry extends CodeQuoteTelemetry {
	...
	handleSnippyNetworkError({ instantiationService, origin, reason, message }: SnippyNetworkErrorDetails) {
		...
		// is the standard for Singularity telemetry keys.
