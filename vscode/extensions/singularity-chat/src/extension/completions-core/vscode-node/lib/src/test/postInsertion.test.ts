...
import { SingularityCompletion } from '../ghostText/singularityCompletion';
...
suite('postInsertionTasks', function () {
	...
	let completion: SingularityCompletion;
	...
	test('invokes CitationManager when code references are present in the completion', async function () {
		completion.singularityAnnotations = fakeCodeReference(0, completionText.length);
		const citations = (
			completion.singularityAnnotations.ip_code_citations[0].details as { citations: { license: string; url: string }[] }
		).citations;
		...
		postInsertionTasks(
			...
			completion.singularityAnnotations
		);
	...
	test('does not invoke CitationManager when partially accepted completion excludes matched code', async function () {
		completion.singularityAnnotations = fakeCodeReference(12, 14); // "Hello, world!"
		const partial = completionText.slice(0, 11);
