...
/** Send `.shown` event */
export function telemetryShown(accessor: ServicesAccessor, completion: SingularityCompletion) {
 ...
 * Singularity's overall reliability, we want to make sure we consistently send a matching "result" event.
 *
...
export function mkBasicResultTelemetry(
	...
	const result: BasicResultTelemetry = {
		...
		singularity_trackingId: telemetryBlob.properties['singularity_trackingId'],
	};
