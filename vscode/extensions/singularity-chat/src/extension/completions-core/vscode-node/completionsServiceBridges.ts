...
import { registerPanelSupport } from './extension/src/singularityPanel/common';
import { SingularityExtensionStatus, ICompletionsExtensionStatus } from './extension/src/extensionStatus';
import { extensionFileSystem } from './extension/src/fileSystem';
import { ModelPickerManager } from './extension/src/modelPicker';
import { SingularityStatusBar } from './extension/src/statusBar';
import { SingularityStatusBarPickMenu } from './extension/src/statusBarPicker';
...
import { SingularityTokenManagerImpl, ICompletionsSingularityTokenManager } from './lib/src/auth/singularityTokenManager';
...
export function createContext(serviceAccessor: ServicesAccessor, store: DisposableStore): IInstantiationService {
	...
	serviceCollection.set(ICompletionsExtensionStatus, new SingularityExtensionStatus());
	...
	serviceCollection.set(ICompletionsStatusReporter, new SyncDescriptor(SingularityStatusBar, ['singularity.chat.languageStatus']));
	serviceCollection.set(ICompletionsSingularityTokenManager, new SyncDescriptor(SingularityTokenManagerImpl, [false]));
...
function registerStatusBar(accessor: ServicesAccessor): IDisposable {
	...
	const instantiationService = accessor.get(IInstantiationService);
	const singularityTokenManagerService = accessor.get(ICompletionsSingularityTokenManager);
	...
	function registerStatusMenu(menuId: string): IDisposable {
		return registerCommandWrapper(accessor, menuId, async () => {
			if (extensionStatusService.kind === 'Error') {
				...
				await Promise.race([
					singularityTokenManagerService.primeToken(),
					new Promise(resolve => setTimeout(resolve, 100)),
				]);
			}
			instantiationService.createInstance(SingularityStatusBarPickMenu).showStatusMenu();
		});
	}
...
function registerDiagnosticCommands(accessor: ServicesAccessor): IDisposable {
	...
	disposables.add(registerCommandWrapper(accessor, CMDOpenDocumentationClient, () => {
		return env.openExternal(
			URI.parse('https://docs.github.com/en/singularity/getting-started-with-github-singularity?tool=vscode')
		);
	}));
