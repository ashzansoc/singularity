...
import { SingularityCompletion } from '../../../lib/src/ghostText/singularityCompletion';
...
export class GhostTextProvider {
	...
	async provideInlineCompletionItems(
		...
		const items: GhostTextCompletionItem[] = rawCompletions.map(completion => {
			...
			return {
				...
				range: newRange,
				singularityCompletion: completion,
	...
	handleDidShowCompletionItem(item: GhostTextCompletionItem) {
		this.instantiationService.invokeFunction(handleGhostTextShown, item.singularityCompletion);
	}
	...
	handleDidPartiallyAcceptCompletionItem(item: GhostTextCompletionItem, info: number | PartialAcceptInfo) {
		...
		this.instantiationService.invokeFunction(handlePartialGhostTextPostInsert, item.singularityCompletion, info.acceptedLength);
	}

	async handleEndOfLifetime(completionItem: GhostTextCompletionItem, reason: InlineCompletionEndOfLifeReason) {
		const singularityCompletion = completionItem.singularityCompletion;
		switch (reason.kind) {
			case InlineCompletionEndOfLifeReasonKind.Accepted: {
				completionItem.telemetryBuilder.setAcceptance('accepted');
				this.instantiationService.invokeFunction(handleGhostTextPostInsert, singularityCompletion);
			...
			case InlineCompletionEndOfLifeReasonKind.Rejected: {
				...
				this.instantiationService.invokeFunction(telemetry, 'ghostText.dismissed', singularityCompletion.telemetry);
				return;
			}
