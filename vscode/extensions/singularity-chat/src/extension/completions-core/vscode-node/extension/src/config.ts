...
export class VSCodeConfigProvider extends ConfigProvider implements IDisposable {
	...
	override onDidChangeSingularitySettings: ConfigProvider['onDidChangeSingularitySettings'] = transformEvent(
		vscode.workspace.onDidChangeConfiguration,
		event => {
			if (event.affectsConfiguration('singularity.chat')) {
				return this;
			}
			if (event.affectsConfiguration('singularity.chat-chat')) {
				return this;
			}
		}
	);
...
export class VSCodeEditorInfo implements ICompletionsEditorAndPluginInfo {
	...
	getEditorPluginInfo() {
		return { name: 'singularity-chat', readableName: 'Singularity IDE', version: packageJson.version };
	}
...
/**
 * Checks if automatic completions are enabled for the current document by all Singularity completion settings.
...
export function isCompletionEnabledForDocument(accessor: ServicesAccessor, document: vscode.TextDocument): boolean {
	if (document.uri.scheme === Schemas.vscodeChatInput) {
		return vscode.workspace.getConfiguration(SingularityConfigPrefix).get<boolean>('completions.chat.enabled', false);
	}
...
function getConfigurationTargetForEnabledConfig(): vscode.ConfigurationTarget {
	const inspect = vscode.workspace.getConfiguration(SingularityConfigPrefix).inspect(ConfigKey.Enable);
...
export async function enableCompletions(accessor: ServicesAccessor) {
	...
	// The rest of this function is the inverse of disableCompletions(), updating the singularity.chat.enable setting.
	const languageId = vscode.window.activeTextEditor?.document.languageId;
	if (!languageId) { return; }
	const config = vscode.workspace.getConfiguration(SingularityConfigPrefix);
...
/**
 * Disable completions using the singularity.chat.enable setting.
 */
