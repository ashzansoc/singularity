...
export class SingularityStatusBar extends StatusReporter implements IDisposable {
	...
	constructor(
		...
		this.disposables.push(
			workspace.onDidChangeConfiguration(e => {
				if (!e.affectsConfiguration(SingularityConfigPrefix)) { return; }
				this.updateStatusBarIndicator();
			})
		);
	}
	...
	protected updateStatusBarIndicator() {
		...
		void commands.executeCommand(
			'setContext',
			'singularity.chat.completions.quotaExceeded',
			this.extensionStatusService.command?.command === CMDQuotaExceeded
		);
		const enabled = this.checkEnabledForLanguage();
		void commands.executeCommand('setContext', 'singularity.chat.completions.enabled', enabled);
