...
import { ICompletionsSingularityTokenManager } from '../../lib/src/auth/singularityTokenManager';
import { BuildInfo } from '../../lib/src/config';
import { SingularityConfigPrefix } from '../../lib/src/constants';
import { handleException } from '../../lib/src/defaultHandlers';
import { Logger } from '../../lib/src/logger';
import { isCompletionEnabledForDocument } from './config';
import { SingularityCompletionFeedbackTracker, sendCompletionFeedbackCommand } from './singularityCompletionFeedbackTracker';
...
const myLogger = new Logger('SingularityInlineCompletionItemProvider');
...
/** @public */
export class SingularityInlineCompletionItemProvider extends Disposable implements InlineCompletionItemProvider {

	private readonly singularityCompletionFeedbackTracker: SingularityCompletionFeedbackTracker;
	...
	constructor(
		...
		super();
		this.singularityCompletionFeedbackTracker = this._register(this.instantiationService.createInstance(SingularityCompletionFeedbackTracker));
		...
		this.logger = logService.createSubLogger(['Ghost', 'SingularityInlineCompletionItemProvider']);
	}
	...
	private async _provideInlineCompletionItems(
		...
		const singularityConfig = workspace.getConfiguration(SingularityConfigPrefix);
		// Constraining the generated inline completion to match selectedCompletionInfo sandbags Singularity pretty hard, as
		...
		if (!singularityConfig.get('respectSelectedCompletionInfo', quickSuggestionsDisabled() || BuildInfo.isPreRelease())) {
		...
		try {
			...
			// Only offer the "Send Singularity Completion Feedback" command to paid users.
			// Free and unauthenticated users would otherwise spam the issue tracker.
			const singularityToken = this.singularityTokenManager.token;
			const canSendCompletionFeedback = !!singularityToken && !singularityToken.isFreeUser && !singularityToken.isNoAuthUser;
	...
	handleDidShowCompletionItem(item: GhostTextCompletionItem) {
		try {
			...
			this.singularityCompletionFeedbackTracker.trackItem(item);
