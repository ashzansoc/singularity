...
import { SingularityUiKind, ICompletionsOpenAIFetcherService } from '../../../../lib/src/openai/fetch';
...
/**
 * Given an `ISolutionManager` with the context of a specific "Open Singularity" request,
...
export async function launchSolutions(accessor: ServicesAccessor, solutionManager: SolutionManager): Promise<SolutionsStream> {
	...
	const promptSetup = await setupPromptAndTelemetry(accessor, solutionManager, 'open singularity', solutionsLogger);
	...
	const completionParams = {
		...
		uiKind: SingularityUiKind.Panel,
		postOptions,
	...
	const solutions = asyncIterableMapFilter(choices, async (apiChoice: APIChoice) => {
		...
		logger.info(`Open Singularity completion: [${apiChoice.completionText}]`);

		// For completions that can happen in any location in the middle of the code we try to find the existing code
		// that should be displayed in the OpenSingularity panel so the code is nicely formatted/highlighted.
		...
		const solution: UnformattedSolution = {
			...
			singularityAnnotations: apiChoice.singularityAnnotations,
		};
		return solution;
	});
