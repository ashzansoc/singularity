...
suite('CodeReference', function () {
	...
	suite('subscriptions', function () {
		...
		test('should be updated correctly when token change events received', function () {
			...
			const enabledToken = new SingularityToken(createTestExtendedTokenInfo({ token: `test token ${generateUuid()}`, username: 'fixedTokenManager', singularity_plan: 'unknown', code_quote_enabled: true }));
			const disabledToken = new SingularityToken(createTestExtendedTokenInfo({ token: `test token ${generateUuid()}`, username: 'fixedTokenManager', singularity_plan: 'unknown', code_quote_enabled: false }));

			codeQuote.onSingularityToken(enabledToken);
			...
			codeQuote.onSingularityToken(disabledToken);
