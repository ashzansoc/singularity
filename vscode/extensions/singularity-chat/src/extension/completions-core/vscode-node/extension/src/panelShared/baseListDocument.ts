...
// BaseListDocument to be shared with both the singularity and comparison completion panels.
	...
	onSolution(unformatted: UnformattedSolution) {
		...
		const postInsertionCallback = () => {
			...
			return this.instantiationService.invokeFunction(postInsertionTasks,
				...
				unformatted.singularityAnnotations
			);
		};
		...
		const baseCompletion: BasePanelCompletion = {
			...
			singularityAnnotations: unformatted.singularityAnnotations,
