...
import { SingularityLogger } from './outputChannel';
...
export class LoggingCitationManager extends Disposable implements ICompletionsCitationManager {
	...
	private logger?: SingularityLogger;
	...
	constructor(
		...
		const disposable = onSingularityToken(authenticationService, _ => {
			if (this.logger) {
				return;
			}
			this.logger = instantiationService.createInstance(SingularityLogger);
	...
	async handleIPCodeCitation(citation: IPDocumentCitation): Promise<void> {
		...
		singularityOutputLogTelemetry.handleWrite({ instantiationService: this.instantiationService });
