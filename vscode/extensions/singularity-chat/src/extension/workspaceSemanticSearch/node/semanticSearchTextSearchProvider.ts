...
export class SemanticSearchTextSearchProvider extends Disposable implements vscode.AITextSearchProvider {
	...
	public readonly name: string = 'Singularity';
	public static feedBackSentKey = 'singularity.chat.search.feedback.sent';
	...
	private async getEndpoint() {
		this._endpoint = this._endpoint ?? await this._endpointProvider.getChatEndpoint('singularity-utility-small');
	...
	provideAITextSearchResults(query: string, options: vscode.TextSearchProviderOptions, progress: vscode.Progress<vscode.TextSearchResult2>, token: vscode.CancellationToken): vscode.ProviderResult<vscode.TextSearchComplete2> {
		...
		const getResults = async () => {
			...
			const result = await this.workspaceChunkSearch.searchFileChunks(
				...
				new TelemetryCorrelationId('singularitySearchPanel'),
				chatProgress,
				token,
			);
	...
	reportTelemetry() {
		/* __GDPR__
		"singularity.search.request" : {
			"owner": "osortega",
			"comment": "Singularity search request.",
			"chunkCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of singularity search code chunks." },
			"rankResult": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Result of the singularity search ranking." },
			"rankResultsCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of the results from singularity search ranking." },
			"combinedResultsCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of combined results from singularity search." },
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('singularity.search.request', {
	...
	async treeSitterAIKeywords(query: string, progress: vscode.Progress<vscode.AISearchResult>, chunks: FileChunk[], token: vscode.CancellationToken): Promise<void> {
		...
		if (searchKeywordsIntent) {
			...
			/* __GDPR__
		"singularity.search.keywords" : {
			"owner": "osortega",
			"comment": "Singularity keywords request.",
			"keywordResult": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Result of the singularity keywords request." },
			"keywordsCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of keywords found by singularity search." },
		...
		*/
			this._telemetryService.sendMSFTTelemetryEvent('singularity.search.keywords', {
