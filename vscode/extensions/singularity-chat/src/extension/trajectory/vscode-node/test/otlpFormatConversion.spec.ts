...
describe('OTLP Format Conversion', () => {
	...
	describe('wrapInResourceSpans + parseResourceSpans', () => {
		it('round-trips spans through OTLP envelope', () => {
			const spans = [
				makeSpan({ name: 'invoke_agent singularity', attributes: { [SingularityChatAttr.SESSION_ID]: 'sess-1' } }),
				makeSpan({ name: 'chat gpt-4o', spanId: 'span2id234567890', parentSpanId: 'abcdef0123456789' }),
			];

			const exported = wrapInResourceSpans(spans, { 'service.name': 'singularity-chat', 'service.version': '1.0.0' });
			const json = JSON.stringify(exported);
			const imported = parseResourceSpans(json);

			expect(imported.length).toBe(2);
			expect(imported[0].name).toBe('invoke_agent singularity');
