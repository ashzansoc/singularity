...
import { SingularityChatAttr, SingularityCliSdkAttr, GenAiAttr, GenAiOperationName } from '../../../platform/otel/common/index';
...
export function extractSessionId(span: ICompletedSpanData): string | undefined {
	return asString(span.attributes[SingularityChatAttr.SESSION_ID])
...
export function completedSpanToDebugEvent(span: ICompletedSpanData): vscode.ChatDebugEvent | undefined {
	...
	switch (opName) {
		...
		default:
			// SDK native hook spans use 'singularity.chat.hook.type' instead of gen_ai.operation.name
			if (span.name.startsWith('hook ') && asString(span.attributes[SingularityCliSdkAttr.HOOK_TYPE])) {
...
export function resolveUserMessageFromSpan(span: ICompletedSpanData): vscode.ChatDebugUserMessageEvent {
	...
	const promptContext = asString(span.attributes[SingularityChatAttr.PROMPT_CONTEXT]);
	if (promptContext) {
		sections.push(new vscode.ChatDebugMessageSection('Context', promptContext));
	}
	const promptInstructions = asString(span.attributes[SingularityChatAttr.PROMPT_INSTRUCTIONS]);
	if (promptInstructions) {
		sections.push(new vscode.ChatDebugMessageSection('Instructions', promptInstructions));
	}
	const userRequest = asString(span.attributes[SingularityChatAttr.USER_REQUEST]);
	if (userRequest) {
...
export function resolveAgentResponseFromSpan(span: ICompletedSpanData): vscode.ChatDebugAgentResponseEvent {
	...
	// Reasoning content
	const reasoning = asString(span.attributes[SingularityChatAttr.REASONING_CONTENT]);
	if (reasoning) {
...
function spanToModelTurnEvent(span: ICompletedSpanData): vscode.ChatDebugModelTurnEvent {
	...
	evt.durationInMillis = span.endTime - span.startTime;
	evt.timeToFirstTokenInMillis = asNumber(span.attributes[SingularityChatAttr.TIME_TO_FIRST_TOKEN]);
	evt.maxInputTokens = asNumber(span.attributes[SingularityChatAttr.MAX_PROMPT_TOKENS]);
	evt.maxOutputTokens = asNumber(span.attributes[GenAiAttr.REQUEST_MAX_TOKENS]);
	evt.requestName = asString(span.attributes[SingularityChatAttr.DEBUG_NAME])
		?? asString(span.attributes[GenAiAttr.AGENT_NAME]);
	evt.status = spanStatusToString(span.status.code as SpanStatusCode);
	evt.singularityUsageNanoAiu = asNumber(span.attributes[SingularityChatAttr.SINGULARITY_USAGE_NANO_AIU]);
	return evt;
}
...
function spanToSubagentEvent(span: ICompletedSpanData): vscode.ChatDebugSubagentInvocationEvent {
	...
	const turnCount = asNumber(span.attributes[SingularityChatAttr.TURN_COUNT]);
...
function resolveHookExecutionContent(span: ICompletedSpanData): vscode.ChatDebugEventHookContent {
	const hookType = asString(span.attributes[SingularityChatAttr.HOOK_TYPE]) ?? 'unknown';
	const content = new vscode.ChatDebugEventHookContent(hookType);
	content.command = asString(span.attributes['singularity_chat.hook_command']);
	const resultKind = asString(span.attributes[SingularityChatAttr.HOOK_RESULT_KIND]);
	...
	content.input = asString(span.attributes[SingularityChatAttr.HOOK_INPUT]);
	content.output = asString(span.attributes[SingularityChatAttr.HOOK_OUTPUT]);
	if (span.status.code === SpanStatusCode.ERROR && span.status.message) {
		content.errorMessage = span.status.message;
	}
	content.exitCode = asNumber(span.attributes['singularity_chat.hook_exit_code']);
	return content;
}

function spanToHookExecutionEvent(span: ICompletedSpanData): vscode.ChatDebugGenericEvent {
	const hookType = asString(span.attributes[SingularityChatAttr.HOOK_TYPE]) ?? 'unknown';
	const hookCommand = asString(span.attributes['singularity_chat.hook_command']);
	const resultKind = asString(span.attributes[SingularityChatAttr.HOOK_RESULT_KIND]);
...
/**
 * Convert an SDK native hook span (singularity.chat.hook.*) to a debug panel event.
 * SDK uses span name "hook {type}" and attributes in the singularity.chat.hook.* namespace.
 */
function spanToSdkHookEvent(span: ICompletedSpanData): vscode.ChatDebugGenericEvent {
	const hookType = asString(span.attributes[SingularityCliSdkAttr.HOOK_TYPE]) ?? 'unknown';
...
function spanToGenericEvent(span: ICompletedSpanData): vscode.ChatDebugGenericEvent {
	const name = asString(span.attributes[SingularityChatAttr.DEBUG_NAME]) ?? span.name;
	const evt = new vscode.ChatDebugGenericEvent(name, vscode.ChatDebugLogLevel.Info, new Date(span.startTime));
	evt.id = span.spanId;
	evt.parentEventId = span.parentSpanId;
	evt.details = asString(span.attributes[SingularityChatAttr.MARKDOWN_CONTENT])
		?? asString(span.attributes['singularity_chat.event_details']);
	evt.category = asString(span.attributes['singularity_chat.event_category']);
	return evt;
}
...
function resolveModelTurnContent(span: ICompletedSpanData): vscode.ChatDebugEventModelTurnContent {
	const requestName = asString(span.attributes[SingularityChatAttr.DEBUG_NAME])
		?? asString(span.attributes[GenAiAttr.AGENT_NAME])
		?? span.name;
	...
	content.durationInMillis = span.endTime - span.startTime;
	content.timeToFirstTokenInMillis = asNumber(span.attributes[SingularityChatAttr.TIME_TO_FIRST_TOKEN]);
	content.requestId = asString(span.attributes[GenAiAttr.RESPONSE_ID]);
	content.requestOptions = asString(span.attributes[SingularityChatAttr.REQUEST_OPTIONS]);
	content.maxInputTokens = asNumber(span.attributes[SingularityChatAttr.MAX_PROMPT_TOKENS]);
	...
	const requestShape = asString(span.attributes[SingularityChatAttr.REQUEST_SHAPE]);
	if (requestShape) {
...
function entryToModelTurnEvent(entry: IDebugLogEntry): vscode.ChatDebugModelTurnEvent {
	...
	evt.singularityUsageNanoAiu = entry.attrs.singularityUsageNanoAiu as number | undefined;
	return evt;
}
