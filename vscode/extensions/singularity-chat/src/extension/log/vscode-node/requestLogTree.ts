...
const showHtmlCommand = 'vscode.singularity.chat.showRequestHtmlItem';
const exportLogItemCommand = 'singularity.chat.chat.debug.exportLogItem';
const exportPromptArchiveCommand = 'singularity.chat.chat.debug.exportPromptArchive';
...
const exportPromptLogsAsJsonCommand = 'singularity.chat.chat.debug.exportPromptLogsAsJson';
const exportAllPromptLogsAsJsonCommand = 'singularity.chat.chat.debug.exportAllPromptLogsAsJson';
const saveCurrentMarkdownCommand = 'singularity.chat.chat.debug.saveCurrentMarkdown';
const showRawRequestBodyCommand = 'singularity.chat.chat.debug.showRawRequestBody';
...
export class RequestLogTree extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(vscode.window.registerTreeDataProvider('singularity-chat', this.chatRequestProvider));
		...
		this._register(vscode.commands.registerCommand(exportLogItemCommand, async (treeItem: TreeItem) => {
			...
			const saveUri = await vscode.window.showSaveDialog({
				...
				filters: {
					'Singularity Markdown': ['singularitymd'],
					'Markdown': ['md'],
					'All Files': ['*']
				},
		...
		// Save the currently opened chat log (ccreq:*.singularitymd) to a file
		this._register(vscode.commands.registerCommand(saveCurrentMarkdownCommand, async (...args: any[]) => {
			...
			if (resource.scheme !== ChatRequestScheme.chatRequestScheme) {
				vscode.window.showWarningMessage('This command only works for Singularity request documents.');
				return;
			}
		...
		this._register(vscode.commands.registerCommand(exportPromptArchiveCommand, async (treeItem: ChatPromptItem) => {
			...
			try {
				...
				const tempDir = path.join(os.tmpdir(), `vscode-singularity-export-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`);
				...
				for (const logEntry of logEntries) {
					...
					switch (logEntry.kind) {
						case LoggedInfoKind.Request: {
							const requestEntry = logEntry as ILoggedRequestInfo;
							const debugName = requestEntry.entry.debugName.replace(/\W/g, '_');
							filename = `${debugName}_${logEntry.id}.singularitymd`;
							break;
						}
						case LoggedInfoKind.ToolCall: {
							const toolEntry = logEntry as ILoggedToolCall;
							const toolName = toolEntry.name.replace(/\W/g, '_');
							filename = `tool_${toolName}_${logEntry.id}.singularitymd`;
							break;
						}
						default:
							continue;
		...
		this._register(vscode.commands.registerCommand(exportAllPromptLogsAsJsonCommand, async (savePath?: string) => {
			...
			if (savePath && typeof savePath === 'string') {
				...
				const defaultFilename = `singularity_all_prompts_${timestamp}.json`;
		...
		this._register(vscode.commands.registerCommand('singularity.chat.debug.showOutputChannel', async () => {
			outputChannel.show();
		}));
	}
}
...
class ChatRequestItem extends vscode.TreeItem {
	...
	${startTimeStr}`;
		...
		this.iconPath ??= new vscode.ThemeIcon('singularity');
	}
}
...
class LogTreeFilters extends Disposable {
	...
	private getStorageKey(name: string): string {
		return `singularity.chat.chat.debug.${name}Hidden`;
	}
	...
	private setShown(name: string, value: boolean): void {
		vscode.commands.executeCommand('setContext', `singularity.chat.chat.debug.${name}Hidden`, !value);
...
class LogTreeFilterCommands extends Disposable {
	constructor(filters: LogTreeFilters) {
		...
		this._register(vscode.commands.registerCommand('singularity.chat.chat.debug.showElements', () => filters.setElementsShown(true)));
		this._register(vscode.commands.registerCommand('singularity.chat.chat.debug.hideElements', () => filters.setElementsShown(false)));
		this._register(vscode.commands.registerCommand('singularity.chat.chat.debug.showTools', () => filters.setToolsShown(true)));
		this._register(vscode.commands.registerCommand('singularity.chat.chat.debug.hideTools', () => filters.setToolsShown(false)));
		this._register(vscode.commands.registerCommand('singularity.chat.chat.debug.showNesRequests', () => filters.setNesRequestsShown(true)));
		this._register(vscode.commands.registerCommand('singularity.chat.chat.debug.hideNesRequests', () => filters.setNesRequestsShown(false)));
		this._register(vscode.commands.registerCommand('singularity.chat.chat.debug.showGhostRequests', () => filters.setGhostRequestsShown(true)));
		this._register(vscode.commands.registerCommand('singularity.chat.chat.debug.hideGhostRequests', () => filters.setGhostRequestsShown(false)));
	}
}
