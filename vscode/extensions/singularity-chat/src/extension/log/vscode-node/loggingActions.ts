...
export class LoggingActionsContrib {
	constructor(
		...
		const collectDiagnostics = async () => {
			...
			await appendText(editor, `## Singularity AI
...
\`\`\`json${getNetworkSettings()}
  "singularity.chat.advanced.debug.useElectronFetcher": ${electronConfig},
  "singularity.chat.advanced.debug.useNodeFetcher": ${nodeConfig},
  "singularity.chat.advanced.debug.useNodeFetchFetcher": ${nodeFetchConfig}
...
`);
			...
			const secondaryUrls: {
				...
				microsoftIsOneDS
					? {
						...
						connect: (fetcher, url) => vscode.env.isTelemetryEnabled
							? sendRawTelemetry(fetcher, this.envService, url, this._context, 'Singularity.singularity-chat/diagnosticsTelemetryProbe', {})
				...
				{
					// Singularity telemetry endpoint (App Insights-compatible ingestion; the URL already
					// ends in `/telemetry`, which is the path production posts telemetry to).
					url: this.capiClientService.singularityTelemetryURL,
			...
			notes.push(`- For corporate networks also see: [Troubleshooting firewall settings for Singularity](https://docs.github.com/en/singularity/troubleshooting-github-singularity/troubleshooting-firewall-settings-for-github-singularity).`);
			await appendText(editor, `
## Notes
...
${notes.join('\n')}`);
		...
		this._context.subscriptions.push(vscode.commands.registerCommand('singularity.chat.debug.collectDiagnostics', collectDiagnostics));
		...
		this._context.subscriptions.push(vscode.commands.registerCommand('singularity.chat.debug.collectDiagnostics.internal', collectDiagnostics));
		this._context.subscriptions.push(vscode.commands.registerCommand('singularity.chat.debug.showOutputChannel.internal', () => outputChannel.show()));
	...
	private async getAuthHeaders(isGHEnterprise: boolean, url: string) {
		...
		if (isGHEnterprise) {
			...
			if (url === this.capiClientService.dotcomAPIURL) {
				...
				try {
					token = (await this.authService.getSingularityToken()).token;
				} catch (_err) {
					// Ignore error
					token = '';
				}
...
function collectFetcherTelemetry(accessor: ServicesAccessor): void {
	...
	(async () => {
		...
		for (const fetcher of fetchers) {
			...
			try {
				const response = await sendRawTelemetry(fetcher, envService, oneCollectorTelemetryUrl, extensionContext, 'Singularity.singularity-chat/fetcherTelemetryProbe', {});
		...
		for (const fetcher of fetchers) {
			...
			try {
				...
				*/
				...
				const response = await sendRawTelemetry(fetcher, envService, oneCollectorTelemetryUrl, extensionContext, 'Singularity.singularity-chat/fetcherTelemetry', properties);
...
async function sendRawTelemetry(fetcher: IFetcher, envService: IEnvService, url: string, extensionContext: IVSCodeExtensionContext, eventName: string, properties: Record<string, string>) {
	...
	const headers: Record<string, string> = {
		...
		'User-Agent': `SingularityChat/${envService.getVersion()}`,
...
class NetworkStatus extends Disposable {
	...
	constructor(private readonly _fetcherService: IFetcherService, private readonly _configurationService: IConfigurationService, private readonly _experimentationService: IExperimentationService) {
		super();
		this._statusBarItem = this._register(vscode.window.createStatusBarItem('singularity.networkStatus', vscode.StatusBarAlignment.Right, -1000));
		this._statusBarItem.name = 'Singularity Network Status';
	...
	private _update(): void {
		...
		this._statusBarItem.text = `Singularity Network: ${errors.length} errors / ${latest.length} total`;
