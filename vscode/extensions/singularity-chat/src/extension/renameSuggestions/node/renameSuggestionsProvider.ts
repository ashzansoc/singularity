...
export class RenameSuggestionsProvider implements vscode.NewSymbolNamesProvider {
	...
	protected isEnabled(triggerKind: NewSymbolNameTriggerKind) {
		if (triggerKind === NewSymbolNameTriggerKind.Invoke) {
			return true;
		} else if (this._authService.singularityToken?.isFreeUser || this._authService.singularityToken?.isNoAuthUser) {
			return false;
		} else {
	...
	/**
	 * @throws {Error} with `message = 'SingularityFeatureUnavailableOrDisabled' if the feature is not available
	 * @throws {Error} with `message = 'SingularityIgnoredDocument' if the document is Singularity-ignored
	 */
	async provideNewSymbolNames(_document: vscode.TextDocument, range: vscode.Range, triggerKind: NewSymbolNameTriggerKind, token: vscode.CancellationToken): Promise<NewSymbolName[] | null> {
		...
		if (!this.isEnabled(triggerKind)) {
			throw new Error('SingularityFeatureUnavailableOrDisabled');
		}

		if (await this._ignoreService.isSingularityIgnored(document.uri)) {
			throw new Error('SingularityIgnoredDocument');
		}
			...
			if (token.isCancellationRequested) {
				...
				if (token.isCancellationRequested) {
					...
					if (token.isCancellationRequested) {
						...
						if (RenameSuggestionsProvider.shouldShowQuotaExceededDialog(triggerKind, fetchResult.type, this._authService.singularityToken?.isNoAuthUser ?? false)) {
							await this._notificationService.showQuotaExceededDialog({ isNoAuthUser: this._authService.singularityToken?.isNoAuthUser ?? false });
						}
	...
	// - idea: include peer symbols (e.g., other methods in the same class) in the prompt for singularity to see conventions in the code
	...
	private _sendPublicTelemetry({
		...
		/* __GDPR__
			"provideRenameSuggestions" : {
				...
				"replyFormat": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Singularity reply format: 'jsonStringArray' | 'multiJsonStringArray' | 'list' | 'unknown'" },
