...
import { SingularityChatAttr, GenAiAttr, GenAiOperationName } from '../../../platform/otel/common/index';
...
export class ChatDebugFileLoggerService extends Disposable implements IChatDebugFileLoggerService {
	...
	private _getDebugLogsDir(): URI | undefined {
		...
		// both local and Singularity CLI sessions). Only when no workspace is open is
	...
	private _ensureSession(sessionId: string, hasOwnSpans = false): void {
		...
		if (childInfo) {
			...
			this._bufferEntry(sessionId, {
				...
				status: 'ok',
				attrs: {
					singularityVersion: this._envService.getVersion(),
	...
	private _onSpanCompleted(span: ICompletedSpanData): void {
		...
		const parentChatSessionId = asString(span.attributes[SingularityChatAttr.PARENT_CHAT_SESSION_ID]);
		const debugLogLabel = asString(span.attributes[SingularityChatAttr.DEBUG_LOG_LABEL]);
		...
		if (opName === GenAiOperationName.CHAT) {
			...
			if (outputMessages) {
				const reasoningContent = asString(span.attributes[SingularityChatAttr.REASONING_CONTENT]);
				this._bufferEntry(sessionId, {
					ts: span.endTime,
					dur: 0,
					sid: sessionId,
	...
	private _onSpanEvent(event: ISpanEventData): void {
		...
		const eventSessionId = event.attributes[SingularityChatAttr.CHAT_SESSION_ID];
	...
	private _onTurnBoundaryEvent(event: ISpanEventData): void {
		...
		const sessionId = typeof event.attributes[SingularityChatAttr.CHAT_SESSION_ID] === 'string'
			? event.attributes[SingularityChatAttr.CHAT_SESSION_ID] as string
		...
		switch (opName) {
			...
			case GenAiOperationName.CHAT: {
				const model = asString(span.attributes[GenAiAttr.REQUEST_MODEL])
					?? asString(span.attributes[GenAiAttr.RESPONSE_MODEL])
					?? 'unknown';
				const debugName = asString(span.attributes[SingularityChatAttr.DEBUG_NAME])
				...
				return {
					...
					attrs: {
						...
						...(span.attributes[GenAiAttr.USAGE_CACHE_READ_INPUT_TOKENS] !== undefined
							...
							: {}),
						...(span.attributes[SingularityChatAttr.TIME_TO_FIRST_TOKEN] !== undefined
							? { ttft: asNumber(span.attributes[SingularityChatAttr.TIME_TO_FIRST_TOKEN]) }
							: {}),
						...(span.attributes[GenAiAttr.RESPONSE_ID] !== undefined
							? { responseId: asString(span.attributes[GenAiAttr.RESPONSE_ID]) }
							: {}),
						...(span.attributes[SingularityChatAttr.USER_REQUEST] !== undefined
							? { userRequest: String(span.attributes[SingularityChatAttr.USER_REQUEST]) }
							: {}),
						...
						...(span.attributes[GenAiAttr.REQUEST_TOP_P] !== undefined
							...
							: {}),
						...(span.attributes[SingularityChatAttr.REQUEST_OPTIONS] !== undefined
							? { requestOptions: String(span.attributes[SingularityChatAttr.REQUEST_OPTIONS]) }
							: {}),
						...(span.attributes[SingularityChatAttr.REQUEST_SHAPE] !== undefined
							? { requestShape: String(span.attributes[SingularityChatAttr.REQUEST_SHAPE]) }
							: {}),
						...(span.attributes[SingularityChatAttr.SINGULARITY_USAGE_NANO_AIU] !== undefined
							? { singularityUsageNanoAiu: asNumber(span.attributes[SingularityChatAttr.SINGULARITY_USAGE_NANO_AIU]) }
							: {}),
						...(isError && span.status.message ? { error: span.status.message } : {}),
					},
				};
			...
			case 'core_event': {
				...
				return {
					...
					status: isError ? 'error' : 'ok',
					attrs: {
						...(span.attributes['singularity_chat.event_details'] !== undefined
							? { details: truncate(String(span.attributes['singularity_chat.event_details']), MAX_ATTR_VALUE_LENGTH) }
							: {}),
						...(span.attributes['singularity_chat.event_category'] !== undefined
							? { category: String(span.attributes['singularity_chat.event_category']) }
							: {}),
					},
				};
			}
			...
			case GenAiOperationName.EXECUTE_HOOK: {
				const hookType = asString(span.attributes[SingularityChatAttr.HOOK_TYPE]) ?? span.name;
				return {
					ts: span.startTime,
					dur: duration,
					sid: sessionId,
					...
					status: isError ? 'error' : 'ok',
					attrs: {
						...(span.attributes['singularity_chat.hook_command'] !== undefined
							? { command: truncate(String(span.attributes['singularity_chat.hook_command']), MAX_ATTR_VALUE_LENGTH) }
							: {}),
						...(span.attributes[SingularityChatAttr.HOOK_INPUT] !== undefined
							? { input: truncate(String(span.attributes[SingularityChatAttr.HOOK_INPUT]), MAX_ATTR_VALUE_LENGTH) }
							: {}),
						...(span.attributes[SingularityChatAttr.HOOK_OUTPUT] !== undefined
							? { output: truncate(String(span.attributes[SingularityChatAttr.HOOK_OUTPUT]), MAX_ATTR_VALUE_LENGTH) }
							: {}),
						...(span.attributes[SingularityChatAttr.HOOK_RESULT_KIND] !== undefined
							? { resultKind: String(span.attributes[SingularityChatAttr.HOOK_RESULT_KIND]) }
							: {}),
						...(isError && span.status.message ? { error: span.status.message } : {}),
					},
				};
	...
	private _extractSessionId(span: ICompletedSpanData): string | undefined {
		const directSessionId = asString(span.attributes[SingularityChatAttr.CHAT_SESSION_ID])
