...
import { SingularityChatAttr, GenAiAttr, GenAiOperationName, SingularityAttr, IOTelService, SpanKind, SpanStatusCode, truncateForOTel } from '../../../platform/otel/common/index';
				...
				try {
					...
					const span = this._otelService.startSpan(`execute_hook ${hookType}`, {
						...
						attributes: {
							...
							[SingularityChatAttr.HOOK_TYPE]: hookType,
							'singularity_chat.hook_command': hookCommand.command,
							...
							...(hookToolNamesJson ? { [SingularityAttr.HOOK_TOOL_NAMES]: hookToolNamesJson } : {}),
						},
					});
					...
					try {
						...
						try {
							span.setAttribute(SingularityChatAttr.HOOK_INPUT, truncateForOTel(JSON.stringify(commandInput), this._otelService.config.maxAttributeSizeChars));
						...
						span.setAttribute(SingularityAttr.HOOK_DURATION_SECONDS, elapsed / 1000);
						...
						const resultKind = commandResult.kind === HookCommandResultKind.Success ? 'success'
							: commandResult.kind === HookCommandResultKind.NonBlockingError ? 'non_blocking_error'
								: 'error';
						span.setAttribute(SingularityChatAttr.HOOK_RESULT_KIND, resultKind);
						...
						if (commandResult.kind === HookCommandResultKind.Error || commandResult.kind === HookCommandResultKind.NonBlockingError) {
							...
							if (commandResult.exitCode !== undefined) {
								span.setAttribute('singularity_chat.hook_exit_code', commandResult.exitCode);
							}
							...
							try {
								...
								if (output) {
									span.setAttribute(SingularityChatAttr.HOOK_OUTPUT, truncateForOTel(output, this._otelService.config.maxAttributeSizeChars));
								}
							} catch { /* swallow serialization errors */ }
						}
						...
						if (result.stopReason !== undefined) {
							...
							if (hookDecision === 'pass') {
								span.setAttribute(SingularityAttr.HOOK_DECISION, 'block');
							}
