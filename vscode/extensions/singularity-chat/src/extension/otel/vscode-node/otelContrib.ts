...
const OPEN_OTEL_SETTINGS_COMMAND = 'singularity.chat.chat.otel.openSettings';
const STATUS_ACTIVE_COMMAND = 'singularity.chat.chat.otel.statusActive';
const OTEL_ENABLED_EXPLICITLY_CONTEXT_KEY = 'singularity.chat.otel.enabledExplicitly';
const CHAT_STATUS_ITEM_ID = 'singularity.otelStatus';
...
export class OTelContrib extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('singularity.chat.chat.otel.flush', async () => {
			if (!this._otelService.config.enabled) {
				return;
			}
		...
		this._register(vscode.commands.registerCommand('singularity.chat.chat.otel.exportAgentTracesDB', async (savePath?: vscode.Uri | string) => {
	...
	private _watchForReloadRequiredChanges(): void {
		...
		this._register(vscode.workspace.onDidChangeConfiguration(async e => {
			...
			const message = endpointChanged
				? vscode.l10n.t("Singularity OTel endpoint will change to {0} after reload.", String(currentConfig.get(endpointSetting.fullyQualifiedId)))
				: vscode.l10n.t("Singularity OTel settings changed - a reload is required for the change to take effect.");
	...
	/**
	 * Surfaces the active OTel configuration as a row in the Singularity AI
	...
	private _configureTerminalEnv(): void {
		...
		collection.replace('SINGULARITY_OTEL_ENABLED', 'true');
	...
	private _registerOpenSettingsCommand(): void {
		this._register(vscode.commands.registerCommand(OPEN_OTEL_SETTINGS_COMMAND, async () => {
			...
			await vscode.commands.executeCommand('workbench.action.openSettings', 'singularity.chat.chat.otel');
		}));
	...
	private _fireActivatedTelemetry(): void {
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('otel.activated', {
			...
			hasCustomServiceName: String(config.serviceName !== 'singularity-chat'),
