...
suite('InlineEditTriggerer', () => {
	...
	function createOutputDocument(): { document: TextDocument; textEditor: TextEditor; selection: Selection } {
		const uri = Uri.parse('output:extension-output-Singularity.singularity-chat-#1-GitHub Singularity Chat');
	...
	suite('Document filtering', () => {
		...
		test('Ignores singularity-ignored documents (not in workspace)', () => {
			const { document, textEditor } = createTextDocument();
			// Remove from workspace to simulate singularity-ignored
	...
	suite('Document switch edge cases', () => {
		test('Does not trigger on document switch to singularity-ignored destination', () => {
			const doc1 = createTextDocument(undefined, Uri.file('file1.py'));
			// doc2 is NOT added to vscWorkspace (singularity-ignored)
			...
			const switchEvents = firedEvents.filter(e => e.data.reason === NesTriggerReason.ActiveDocumentSwitch);
			assert.strictEqual(switchEvents.length, 0, 'Should not trigger switch for singularity-ignored destination');
			assert.strictEqual(firedEvents.length, initialCount, 'No new events should fire');
		});
	...
	suite('Text change listener edge cases', () => {
		test('Text change on singularity-ignored doc does not track but updates lastEditTimestamp', () => {
			// Create a doc that is NOT in vscWorkspace (singularity-ignored)
			const uri = Uri.file('ignored.py');
			const doc = createTextDocumentData(uri, 'content', 'python');
			// Do NOT call vscWorkspace.addDoc — simulates singularity-ignored
	...
	suite('Interaction edge cases', () => {
		...
		test('Singularity-ignored doc in selection listener returns early before rejection check', () => {
			...
			assert.strictEqual(firedEvents.length, 0,
				'Singularity-ignored doc should return early in selection listener');
		});
