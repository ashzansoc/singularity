...
describe('DocumentFilter', () => {
	...
	it('can react to singularity.enable config changes for off-by-default language id', async () => {
		const defaultsConfigService = new DefaultsOnlyConfigurationService();
	...
	it('can react to singularity.enable config changes for javascript', async () => {
		const defaultsConfigService = new DefaultsOnlyConfigurationService();
		const defaultConfig = defaultsConfigService.getConfig(ConfigKey.Enable);
