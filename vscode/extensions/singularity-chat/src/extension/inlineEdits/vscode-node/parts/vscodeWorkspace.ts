...
export class VSCodeWorkspace extends ObservableWorkspace implements IDisposable {
	...
	private readonly _textDocsWithShouldTrackFlag = mapObservableArrayCached(this, this._vscodeTextDocuments, (doc, store) => {
		...
		const updateShouldTrack = () => {
			// @ulugbekna: not sure if invoking `isSingularityIgnored` on every textDocument-edit event is a good idea
			// 	also not sure if we should be enforcing local singularity-ignore rules (vs only remote-exclusion rules)
	...
	*/
	 ...
	 * per-language `singularity.chat.enable` setting. See {@link DocumentFilter.isLanguageEnabled}.
	 */
