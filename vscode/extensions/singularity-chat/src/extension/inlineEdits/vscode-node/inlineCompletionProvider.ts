...
export class InlineCompletionProviderImpl extends Disposable implements InlineCompletionItemProvider {
	...
	private async _provideInlineCompletionItems(
		...
		// Respect the per-language `singularity.chat.enable` setting for NES, just like ghost text does.
		...
		try {
			...
			// per-language `singularity.chat.enable` setting for that target too. The active document was
	...
	private async _trackSurvivalRate(item: LlmCompletionInfo) {
		...
		this._instantiationService.createInstance(
			...
			res => {
				...
				*/
				this._telemetryService.sendTelemetryEvent('reportInlineEditSurvivalRate', { microsoft: true, github: { eventNamePrefix: 'singularity-nes/' } },
					{
