...
import { SingularityInlineCompletionItemProvider } from '../../completions-core/vscode-node/extension/src/vscodeInlineCompletionItemProvider';
import { ISingularityInlineCompletionItemProviderService } from '../../completions/common/singularityInlineCompletionItemProviderService';
...
export class JointCompletionsProviderContribution extends Disposable implements IExtensionContribution {
	...
	// private readonly _yieldToSingularity = this._configurationService.getExperimentBasedConfigObservable(ConfigKey.TeamInternal.InlineEditsYieldToSingularity, this._expService);
	...
	private readonly _singularityToken = observableFromEvent(this, this._authenticationService.onDidSingularityTokenChange, () => this._authenticationService.singularityToken);

	public readonly inlineEditsEnabled = derived(this, (reader) => {
		const singularityToken = this._singularityToken.read(reader);
		if (singularityToken === undefined) {
			return false;
		}
		if (singularityToken.isCompletionsQuotaExceeded) {
			return false;
		}
		return true;
	});
	...
	private readonly _internalActionsEnabled = derived(this, (reader) => {
		return !!this._singularityToken.read(reader)?.isInternal && !this._hideInternalInterface.read(reader);
	});
	...
	constructor(
		...
		@ISingularityInlineCompletionItemProviderService private readonly _singularityInlineCompletionItemProviderService: ISingularityInlineCompletionItemProviderService,
		...
		this._register(autorun((reader) => { // FX
			...
			reader.store.add(autorun((reader) => {
				...
				let completionsProvider: SingularityInlineCompletionItemProvider | undefined;
				{
					...
					if (
						(!modelUnification || unificationStateValue?.codeUnification || extensionUnification || configEnabled || this._singularityToken.read(reader)?.isNoAuthUser) &&
						!isExcluded
					) {
						completionsProvider = this._singularityInlineCompletionItemProviderService.getOrCreateProvider() as SingularityInlineCompletionItemProvider;
					}

					void vscode.commands.executeCommand('setContext', 'singularity.chat.extensionUnification.activated', extensionUnification);

					if (extensionUnification && completionsProvider) {
						const completionsInstaService = this._singularityInlineCompletionItemProviderService.getOrCreateInstantiationService();
				...
				if (unificationStateValue?.modelUnification) {
					if (!excludes.includes('singularity.chat')) {
						excludes.push('singularity.chat');
					}
				}
...
class JointCompletionsProvider extends Disposable implements vscode.InlineCompletionItemProvider {
	...
	constructor(
		private readonly _completionsProvider: SingularityInlineCompletionItemProvider | undefined,
