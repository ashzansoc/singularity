...
export const reportFeedbackCommandId = 'singularity.chat.debug.inlineEdit.reportFeedback';
const pickProviderId = 'singularity.chat.debug.inlineEdit.pickProvider';
...
export class InlineEditDebugComponent extends Disposable {
	...
	constructor(
		...
		this._register(commands.registerCommand(pickProviderId, async (args: unknown) => {
			...
			const pick = await window.showWarningMessage(`Inline edits provider set to ${selectedProvider}. Reloading will undo this change. Set "singularity.chat.${ConfigKey.TeamInternal.InlineEditsProviderId.id}": "${selectedProvider}" in your settings file to make the change persistent.`, 'Open settings (JSON)');
			if (!pick) { return; }

			await commands.executeCommand('workbench.action.openSettingsJson', { revealSetting: { key: `singularity.chat.${ConfigKey.TeamInternal.InlineEditsProviderId.id}`, edit: true } });
		}));
	}
