...
import {
	buildChatHistoryFromEvents, createSingularityCLIToolInvocation, enrichToolInvocationWithSubagentMetadata, extractCdPrefix, FakeToolsService, getAffectedUrisForEditTool, isSingularityCliEditToolCall, isSingularityCLIToolThatCouldRequirePermissions, isTodoRelatedSqlQuery, processToolExecutionComplete, processToolExecutionStart, RequestIdDetails, stripReminders, ToolCall, updateTodoListFromSqlItems
} from '../singularityCLITools';
...
describe('SingularityCLITools', () => {
	const logger = new TestLogService();
	describe('isSingularityCliEditToolCall', () => {
		it('detects StrReplaceEditor edit commands (non-view)', () => {
			expect(isSingularityCliEditToolCall({ toolName: 'str_replace_editor', arguments: { command: 'str_replace', path: '/tmp/a' } })).toBe(true);
			expect(isSingularityCliEditToolCall({ toolName: 'str_replace_editor', arguments: { command: 'insert', path: '/tmp/a', new_str: '' } })).toBe(true);
			expect(isSingularityCliEditToolCall({ toolName: 'str_replace_editor', arguments: { command: 'create', path: '/tmp/a' } })).toBe(true);
		});
		it('excludes StrReplaceEditor view command', () => {
			expect(isSingularityCliEditToolCall({ toolName: 'str_replace_editor', arguments: { command: 'view', path: '/tmp/a' } })).toBe(false);
		});
		it('always true for Edit & Create tools', () => {
			expect(isSingularityCliEditToolCall({ toolName: 'edit', arguments: { path: '' } })).toBe(true);
			expect(isSingularityCliEditToolCall({ toolName: 'create', arguments: { path: '' } })).toBe(true);
		});
	});
	...
	describe('formatModelDetails', () => {
		it('prefers credits over multiplier when credits are available', () => {
			expect(formatModelDetails('GPT 5.4', 2, 5)).toBe('5 credits');
		});

		it('falls back to multiplier when credits are undefined', () => {
			expect(formatModelDetails('GPT 5.4', 2, undefined)).toBe('2x');
		});

		it('returns empty when both credits and multiplier are absent', () => {
			expect(formatModelDetails('GPT 5.4', undefined, undefined)).toBe('');
		});
	});

	describe('formatModelDetailsWithCredits', () => {
		it('formats integer credits as plural', () => {
			expect(formatModelDetailsWithCredits('GPT 5.4', 5)).toBe('5 credits');
		});

		it('formats exactly 1 credit as singular', () => {
			expect(formatModelDetailsWithCredits('GPT 5.4', 1)).toBe('1 credit');
		});

		it('formats fractional credits with one decimal place', () => {
			expect(formatModelDetailsWithCredits('GPT 5.4', 16.31565)).toBe('16.3 credits');
		});
	});

	describe('formatModelDetailsWithMultiplier', () => {
		it('formats with multiplier suffix', () => {
			expect(formatModelDetailsWithMultiplier('GPT 5.4', 2)).toBe('2x');
		});

		it('formats 0x multiplier for included models', () => {
			expect(formatModelDetailsWithMultiplier('GPT 5.4', 0)).toBe('0x');
		});

		it('returns empty when multiplier is undefined', () => {
			expect(formatModelDetailsWithMultiplier('GPT 5.4', undefined)).toBe('');
		});
	});
	...
	describe('buildChatHistoryFromEvents', () => {
		it('builds turns with user and assistant messages including PR metadata', () => {
			...
			if (prPart) {
				...
				expect((prPart as any).command.command).toBe('singularity.chat.chat.openPullRequestReroute');
			}
			if (markdownPart) {
		...
		it('createSingularityCLIToolInvocation formats str_replace_editor view with range', () => {
			const invocation = createSingularityCLIToolInvocation({ toolName: 'str_replace_editor', toolCallId: 'id3', arguments: { command: 'view', path: '/tmp/file.ts', view_range: [1, 5] } }) as ChatToolInvocationPart;
		...
		it('filters out instruction file attachments', () => {
			const events: any[] = [
				{
					type: 'user.message', data: {
						content: 'Hello',
						attachments: [
							{ type: 'file', path: '/workspace/.github/singularity-instructions.md', displayName: 'singularity-instructions.md' },
			...
			expect(paths).not.toContain('/workspace/.github/singularity-instructions.md');
	...
	describe('createSingularityCLIToolInvocation', () => {
		it('returns undefined for report_intent', () => {
			expect(createSingularityCLIToolInvocation({ toolName: 'report_intent', toolCallId: 'id', arguments: { intent: '' } })).toBeUndefined();
		});
		it('creates thinking progress part for think tool', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'think', toolCallId: 'tid', arguments: { thought: 'Analyzing' } });
			expect(part).toBeInstanceOf(ChatResponseThinkingProgressPart);
		});
		it('formats bash tool invocation with description', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'bash', toolCallId: 'b1', arguments: { command: 'ls', description: 'List files' } });
		...
		it('formats str_replace_editor create', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'str_replace_editor', toolCallId: 'e1', arguments: { command: 'create', path: '/tmp/x.ts' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it.skip('formats show_file invocation with path', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'show_file', toolCallId: 'sf1', arguments: { path: '/tmp/file.ts' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it.skip('formats show_file invocation with diff mode', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'show_file', toolCallId: 'sf2', arguments: { path: '/tmp/file.ts', diff: true } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it.skip('formats show_file invocation with view_range', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'show_file', toolCallId: 'sf3', arguments: { path: '/tmp/file.ts', view_range: [10, 20] } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			const msg = getInvocationMessageText(part as ChatToolInvocationPart);
		...
		it('formats propose_work invocation with title', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'propose_work', toolCallId: 'pw1', arguments: { workType: 'code_change', workTitle: 'Refactor auth', workDescription: 'desc' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('returns markdown part for task_complete invocation with summary', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'task_complete', toolCallId: 'tc1', arguments: { summary: 'Fixed the bug' } });
		...
		it('returns undefined for task_complete invocation without summary', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'task_complete', toolCallId: 'tc2', arguments: {} });
			expect(part).toBeUndefined();
		});
		it('formats ask_user invocation with question', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'ask_user', toolCallId: 'au1', arguments: { question: 'Which DB?' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats ask_user invocation with structured message', () => {
			const part = createSingularityCLIToolInvocation({
				toolName: 'ask_user',
				toolCallId: 'au2',
				arguments: {
		...
		it('formats skill invocation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'skill', toolCallId: 'sk1', arguments: { skill: 'pdf' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('pdf');
		});
		it('formats task invocation with description', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'task', toolCallId: 't1', arguments: { description: 'Run tests', prompt: 'Run all unit tests', agent_type: 'task' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('Run tests');
		});
		it('formats read_agent invocation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'read_agent', toolCallId: 'ra1', arguments: { agent_id: 'agent-123' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('agent-123');
		});
		it('formats exit_plan_mode invocation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'exit_plan_mode', toolCallId: 'ep1', arguments: { summary: 'Plan summary' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toMatch(/plan/i);
		});
		it('formats sql invocation with description', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'sql', toolCallId: 'sq1', arguments: { description: 'Query todos', query: 'SELECT * FROM todos' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('Query todos');
		});
		it('formats lsp invocation with file', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'lsp', toolCallId: 'lsp1', arguments: { operation: 'goToDefinition', file: '/tmp/app.ts', line: 10, character: 5 } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			const msg = getInvocationMessageText(part as ChatToolInvocationPart);
		...
		it('formats lsp invocation without file', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'lsp', toolCallId: 'lsp2', arguments: { operation: 'workspaceSymbol', query: 'MyClass' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats create_pull_request invocation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'create_pull_request', toolCallId: 'pr1', arguments: { title: 'Fix auth flow', description: 'Summary of changes', draft: false } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats search_code_subagent invocation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'search_code_subagent', toolCallId: 'sc1', arguments: { query: 'find auth middleware' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats store_memory invocation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'store_memory', toolCallId: 'sm1', arguments: { subject: 'naming', fact: 'Use camelCase', citations: 'src/foo.ts:1', reason: 'consistency', category: 'general' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('naming');
		});
		it('creates invocation for fetch_singularity_cli_documentation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'fetch_singularity_cli_documentation', toolCallId: 'fd1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for list_agents', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'list_agents', toolCallId: 'la1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for list_bash', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'list_bash', toolCallId: 'lb1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for list_powershell', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'list_powershell', toolCallId: 'lp1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for gh-advisory-database', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'gh-advisory-database', toolCallId: 'gh1', arguments: { dependencies: [{ name: 'lodash', version: '4.17.0', ecosystem: 'npm' }] } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('creates invocation for parallel_validation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'parallel_validation', toolCallId: 'pv1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('formats apply_patch invocation', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'apply_patch', toolCallId: 'ap1', arguments: { input: '*** Begin Patch\n*** End Patch' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats write_agent invocation with agent_id', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'write_agent', toolCallId: 'wa1', arguments: { agent_id: 'agent-42', message: 'Hello agent' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
			expect(getInvocationMessageText(part as ChatToolInvocationPart)).toContain('agent-42');
		});
		it('creates invocation for mcp_reload', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'mcp_reload', toolCallId: 'mr1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		it('formats mcp_validate invocation with path', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'mcp_validate', toolCallId: 'mv1', arguments: { path: '/home/user/.singularity/config/mcp-config.json' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('formats tool_search_tool_regex invocation with pattern', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'tool_search_tool_regex', toolCallId: 'ts1', arguments: { pattern: 'search.*file' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		...
		it('creates invocation for codeql_checker', () => {
			const part = createSingularityCLIToolInvocation({ toolName: 'codeql_checker', toolCallId: 'cq1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
	});
	...
	describe('isSingularityCLIToolThatCouldRequirePermissions', () => {
		...
		it('returns true for edit tool calls (create, edit)', () => {
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'create', toolCallId: '1', arguments: { path: '/tmp/a' } }))).toBe(true);
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'edit', toolCallId: '2', arguments: { path: '/tmp/b' } }))).toBe(true);
		});

		it('returns true for str_replace_editor non-view commands', () => {
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'str_replace_editor', toolCallId: '3', arguments: { command: 'str_replace', path: '/tmp/a' } }))).toBe(true);
		});

		it('returns true for bash and powershell', () => {
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'bash', toolCallId: '4', arguments: { command: 'echo hi' } }))).toBe(true);
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'powershell', toolCallId: '5', arguments: { command: 'echo hi' } }))).toBe(true);
		});

		it('returns true for view tool', () => {
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'view', toolCallId: '6', arguments: { path: '/tmp/a' } }))).toBe(true);
		});

		it('returns false for MCP tools even if tool name matches', () => {
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'bash', toolCallId: '7', mcpServerName: 'my-server', arguments: { command: 'echo' } }))).toBe(false);
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'view', toolCallId: '8', mcpServerName: 'my-server', arguments: { path: '/tmp' } }))).toBe(false);
		});

		it('returns false for non-permission tools like think, report_intent, glob', () => {
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'think', toolCallId: '9', arguments: { thought: 'hmm' } }))).toBe(false);
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'report_intent', toolCallId: '10', arguments: {} }))).toBe(false);
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'glob', toolCallId: '11', arguments: { pattern: '*.ts' } }))).toBe(false);
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'grep', toolCallId: '12', arguments: { pattern: 'foo' } }))).toBe(false);
		});

		it('returns false for str_replace_editor view command (not an edit)', () => {
			expect(isSingularityCLIToolThatCouldRequirePermissions(makeEvent({ toolName: 'str_replace_editor', toolCallId: '13', arguments: { command: 'view', path: '/tmp/a' } }))).toBe(false);
		});
	});
