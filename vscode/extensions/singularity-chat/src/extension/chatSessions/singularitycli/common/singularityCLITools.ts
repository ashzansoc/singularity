...
import { ISingularityTool } from '../../../tools/common/toolsRegistry';
...
import { extractChatPromptReferences, getFolderAttachmentPath } from './singularityCLIPrompt';
...
type FetchSingularityCliDocumentationTool = {
	toolName: 'fetch_singularity_cli_documentation';
	arguments: Record<string, never>;
};
...
export type ToolInfo = StringReplaceEditorTool | EditTool | CreateTool | ViewTool | InsertTool |
	...
	ShowFileTool | FetchSingularityCliDocumentationTool | ProposeWorkTool | TaskCompleteTool |
...
function isInstructionAttachmentPath(path: string): boolean {
	...
	return normalizedPath.endsWith('/.github/singularity-instructions.md')
		|| (normalizedPath.includes('/.github/instructions/') && normalizedPath.endsWith('.md'));
}

export function isSingularityCliEditToolCall(data: { toolName: string; arguments?: unknown }): boolean {
...
export function isSingularityCLIToolThatCouldRequirePermissions(event: ToolExecutionStartEvent): boolean {
	const toolCall = event.data as unknown as ToolCall;
	if (isSingularityCliEditToolCall(toolCall)) {
		return true;
	}
...
export function processToolExecutionStart(event: ToolExecutionStartEvent, pendingToolInvocations: Map<string, [ChatToolInvocationPart | ChatResponseMarkdownPart | ChatResponseThinkingProgressPart, toolData: ToolCall, parentToolCallId: string | undefined]>, workingDirectory?: URI): ChatToolInvocationPart | ChatResponseMarkdownPart | ChatResponseThinkingProgressPart | undefined {
	const toolInvocation = createSingularityCLIToolInvocation(event.data as ToolCall, undefined, workingDirectory);
	if (toolInvocation) {
...
export const CLI_TOOL_EVENT_HANDLERS: ToolEventHandlers<ToolCall> = {
	...
	isEditToolCall: isSingularityCliEditToolCall,
	getEditedUris: getAffectedUrisForEditTool,
};

/**
 * Creates a formatted tool invocation part for SingularityCLI tools
 */
export function createSingularityCLIToolInvocation(data: {
...
const ToolFriendlyNameAndHandlers: { [K in ToolCall['toolName']]: [title: string, pre: (invocation: ChatToolInvocationPart, toolCall: ToolCallFor<K>, editId?: string, workingDirectory?: URI) => void, post: (invocation: ChatToolInvocationPart, toolCall: ToolCallFor<K>, result: ToolCallResult, workingDirectory?: URI) => void] } = {
	...
	'fetch_singularity_cli_documentation': [l10n.t('Fetch Documentation'), emptyInvocation, genericToolInvocationCompleted],
...
export class FakeToolsService implements IToolsService {
	...
	readonly singularityTools = new Map<ToolName, ISingularityTool<unknown>>();
	...
	modelSpecificTools: IObservable<{ definition: LanguageModelToolDefinition; tool: ISingularityTool<unknown> }[]> = constObservable([]);
	...
	getSingularityTool(): ISingularityTool<unknown> | undefined {
		return undefined;
	}
