...
export interface ISingularityCLIMCPHandler {
	readonly _serviceBrand: undefined;
	loadMcpConfig(sessionUri: URI): Promise<{ mcpConfig: Record<string, MCPServerConfig> | undefined; disposable: IDisposable }>;
}

export const ISingularityCLIMCPHandler = createServiceIdentifier<ISingularityCLIMCPHandler>('ISingularityCLIMCPHandler');

export class SingularityCLIMCPHandler implements ISingularityCLIMCPHandler {
	...
	private async loadMcpConfigWithGateway(sessionUri: URI): Promise<{ mcpConfig: Record<string, MCPServerConfig> | undefined; disposable: IDisposable }> {
		...
		try {
			...
			if (gateway) {
				...
				this.logService.warn('[SingularityCLIMCPHandler]   gateway failed to start');
				disposable.dispose();
			}
		} catch (error) {
			this.logService.warn(`[SingularityCLIMCPHandler]   gateway error: ${error}`);
		}
	...
	private normalizeServerName(originalName: string): string | undefined {
		...
		if (!normalized) {
			this.logService.error(`[SingularityCLIMCPHandler] Failed to normalize server name '${originalName}' - result is empty`);
			return undefined;
		}

		if (normalized !== originalName) {
			this.logService.trace(`[SingularityCLIMCPHandler] Normalized server '${originalName}' to '${normalized}'`);
		}
	...
	private async addBuiltInGitHubServer(config: Record<string, MCPServerConfig>): Promise<void> {
		try {
			...
			if (!definitions || definitions.length === 0) {
				this.logService.trace('[SingularityCLIMCPHandler] No GitHub MCP server definitions available.');
				return;
			}
			...
			this.logService.trace('[SingularityCLIMCPHandler] Added built-in GitHub MCP server.');
		} catch (error) {
			this.logService.warn(`[SingularityCLIMCPHandler] Failed to add built-in GitHub MCP server: ${error}`);
		}
	}
}
