...
import { PINNED_BYOK_FLASH_MODEL_ID } from '../../../../platform/env/node/singularityBundledEnv';
import { IGitService } from '../../../../platform/git/common/gitService';
import { PermissiveAuthRequiredError } from '../../../../platform/github/common/githubService';
import { ILogService } from '../../../../platform/log/common/logService';
import { GenAiMetrics } from '../../../../platform/otel/common/genAiMetrics';
import { SingularityChatAttr, GenAiAttr, GenAiOperationName, GenAiProviderName, IOTelService, ISpanHandle, resolveWorkspaceOTelMetadata, SpanKind, SpanStatusCode, TraceContext, truncateForOTel, workspaceMetadataToOTelAttributes } from '../../../../platform/otel/common/index';
...
import {
	...
	shouldAwaitDesignDirector,
	promptNeedsDesignIntelligence,
	type DesignIntelligenceProgressUpdate,
} from '../../../../platform/endpoint/node/designIntelligence';
...
import { clearTodoList, enrichToolInvocationWithSubagentMetadata, isSingularityCliEditToolCall, isSingularityCLIToolThatCouldRequirePermissions, isTodoRelatedSqlQuery, processToolExecutionComplete, processToolExecutionStart, stripReminders, ToolCall, updateTodoListFromSqlItems } from '../common/singularityCLITools';
import { clearPendingSingularityCLIRequestContext, setPendingSingularityCLIRequestContext } from '../common/pendingRequestContext';
import { LocalSession, Session, SessionIdForCLI } from '../common/utils';
import { getSingularityCLISessionDir } from './cliHelpers';
import type { SingularityCliBridgeSpanProcessor } from './singularityCliBridgeSpanProcessor';
import { ISingularityCLIImageSupport } from './singularityCLIImageSupport';
...
/**
 * Known commands that can be sent to a SingularityCLI session instead of a free-form prompt.
 */
export type SingularityCLICommand = 'compact' | 'plan' | 'fleet' | 'remote';

/**
 * The set of all known SingularityCLI commands.  Used by callers that need to
 * distinguish a slash-command from a regular prompt at runtime.
 */
export const singularityCLICommands: readonly SingularityCLICommand[] = ['compact', 'plan', 'fleet', 'remote'] as const;

export class SingularityCLIQuotaExceededError extends Error {
	constructor(message: string) {
		super(message);
		this.name = 'SingularityCLIQuotaExceededError';
	}
}

/**
 * Shared Mission Control state keyed by SDK session ID.
 * SingularityCLISession instances are recreated per request, so MC state
...
class SingularityCLIResponseStreamRouter {
	...
	private static _isClosedStreamError(error: unknown): boolean {
		...
		return message.toLowerCase().includes(SingularityCLIResponseStreamRouter._closedStreamErrorFragment);
	}
	...
	private _handleCallError(error: unknown, method: string, args: unknown[], stream: vscode.ChatResponseStream): unknown {
		if (SingularityCLIResponseStreamRouter._isClosedStreamError(error)) {
			if (this._stream === stream) {
				this._stream = undefined;
			}
			this._logService.trace(`[SingularityCLISession] Dropping ${method} for closed response stream in session ${this._sessionId}`);
			return this._fallback(method, args);
		}
		throw error;
	}
 ...
 */
export type SingularityCLISessionInput =
	| { readonly prompt: string; readonly source?: SendOptions['source'] }
	| { readonly prompt?: string; readonly command: SingularityCLICommand; readonly source?: SendOptions['source'] };

function getPromptLabel(input: SingularityCLISessionInput): string {
	if ('command' in input) {
...
function getRemoteControlArgs(input: SingularityCLISessionInput): string {
...
export interface ISingularityCLISession extends IDisposable {
	...
	handleRequest(
		...
		input: SingularityCLISessionInput,
...
export class SingularityCLISession extends DisposableStore implements ISingularityCLISession {
	...
	private readonly _streamRouter: SingularityCLIResponseStreamRouter;
	...
	private _bridgeProcessor: SingularityCliBridgeSpanProcessor | undefined;
	...
	setBridgeProcessor(bridge: SingularityCliBridgeSpanProcessor | undefined): void {
	...
	constructor(
		...
		@ISingularityCLIImageSupport private readonly _imageSupport: ISingularityCLIImageSupport,
		...
		this._streamRouter = new SingularityCLIResponseStreamRouter(this.logService, this.sessionId);
	...
	private _applyEffectiveSandboxConfig(bypassApprovals: boolean): void {
		...
		try {
			...
			this.logService.error(error, '[SingularityCLISession] Failed to update sandbox config for request');
		}
	}
	...
	public async handleRequest(
		...
		// Permanent model pin: the Singularity CLI agent session always runs on
		// DeepSeek V4 Flash 0731 through the configured gateway (OpenRouter with
		// your key). Auto/other model selections (e.g. claude-haiku) are forced
		// back to DeepSeek Flash so the chat can never resolve elsewhere. The
		// user's chosen reasoning effort / context tier are preserved.
		const PINNED_MODEL_ID = PINNED_BYOK_FLASH_MODEL_ID;
		const requestedModel = model?.model;
		if (!requestedModel || !/deepseek-v4-flash/i.test(requestedModel)) {
			model = {
		...
		const capturingToken = new CapturingToken(`Singularity CLI | ${promptLabel}`, 'worktree', undefined, undefined, this.sessionId);
	...
	private async _handleRequestImpl(
		...
		return this._otelService.startActiveSpan(
			'invoke_agent singularitycli',
			{
				kind: SpanKind.INTERNAL,
				attributes: {
					[GenAiAttr.OPERATION_NAME]: GenAiOperationName.INVOKE_AGENT,
					[GenAiAttr.AGENT_NAME]: 'singularitycli',
					[GenAiAttr.PROVIDER_NAME]: GenAiProviderName.GITHUB,
					[GenAiAttr.CONVERSATION_ID]: this.sessionId,
					[SingularityChatAttr.SESSION_ID]: this.sessionId,
					[SingularityChatAttr.CHAT_SESSION_ID]: this.sessionId,
					...(modelId ? { [GenAiAttr.REQUEST_MODEL]: modelId } : {}),
					[SingularityChatAttr.USER_REQUEST]: truncateForOTel(promptLabel, this._otelService.config.maxAttributeSizeChars),
	...
	private async _handleRequestImplInner(
		...
		this.logService.info(`[SingularityCLISession] Invoking session ${this.sessionId}`);
		const disposables = new DisposableStore();
		const logStartTime = Date.now();
			...
			const abortHandle = setTimeout(() => {
				...
				if (!wroteResponseContent) {
					try {
						...
						this.logService.trace(`[SingularityCLISession] Unable to mark interrupted response: ${error instanceof Error ? error.message : String(error)}`);
					}
				}
				this._sdkSession.abort();
			}, 250);
		...
		// chat debug logs view for the in-process Singularity CLI experience.
		...
		// metrics, and the agent response for the in-process Singularity CLI experience (the SDK performs
		...
		clearTodoList(this._toolsService, request.toolInvocationToken, token).catch(err => {
			this.logService.error(err, '[SingularityCLISession] Failed to clear todo list at start of session');
		});
		/**
		...
		try {
			...
			disposables.add(toDisposable(this._sdkSession.on('permission.requested', async (event) => {
				...
				if (!isSandboxBypassShell && (effectivePermissionLevel === 'autoApprove' || effectivePermissionLevel === 'autopilot')) {
					this.logService.trace(`[SingularityCLISession] Auto Approving ${permissionRequest.kind} request (permission level: ${effectivePermissionLevel})`);
				...
				if (!isSandboxBypassShell && permissionRequest.kind === 'shell' && this._sandboxEnabled) {
					this.logService.trace(`[SingularityCLISession] Auto Approving shell request (sandbox is enabled)`);
					this._sdkSession.respondToPermission(requestId, { kind: 'approve-once' });
					return;
				}
				...
				try {
					...
					this.logService.error(error, `[SingularityCLISession] Error handling permission request of kind ${permissionRequest.kind}`);
					flushPendingInvocationMessageForToolCallId(permissionRequest.toolCallId);
			...
			if (shouldHandleExitPlanModeRequests) {
				disposables.add(toDisposable(this._sdkSession.on('exit_plan_mode.requested', async (event) => {
					...
					try {
						...
						this.logService.error(error, '[SingularityCLISession] Error handling exit plan mode');
			...
			disposables.add(toDisposable(this._sdkSession.on('assistant.usage', (event) => {
				...
				// Accumulate per-turn credits from SDK singularityUsage data
				const singularityUsage = (event.data as unknown as Record<string, unknown>).singularityUsage;
				let singularityUsageNanoAiu: number | undefined;
				if (singularityUsage && typeof singularityUsage === 'object') {
					const { totalNanoAiu } = singularityUsage as { totalNanoAiu?: number };
					if (typeof totalNanoAiu === 'number') {
						singularityUsageNanoAiu = totalNanoAiu;
						this._chatQuotaService.setLastSingularityUsage(totalNanoAiu, request.id);
					}
				}
				// Sync the live per-category quota state the SDK reports (internal-only field) so the
				// quota UI stays current without a separate `singularity_internal/user` fetch. This mirrors
				// the extension-host chat path, which processes `singularity_quota_snapshots` from CAPI.
			...
			disposables.add(toDisposable(this._sdkSession.on('tool.execution_start', (event) => {
				...
				if (isSingularityCliEditToolCall(event.data)) {
					flushPendingInvocationMessages();
					...
					if (responsePart instanceof ChatResponseThinkingProgressPart) {
						...
						if (isSingularityCLIToolThatCouldRequirePermissions(event)) {
			...
			disposables.add(toDisposable(this._sdkSession.on('tool.execution_complete', (event) => {
				...
				// for the Singularity CLI agent. CLI tools execute inside the SDK and never reach
				...
				if (toolName === 'sql' && event.data.success) {
					try {
						...
						if (isTodoRelatedSqlQuery(query)) {
							const sessionDir = getSingularityCLISessionDir(this.sessionId);
							this._todoSqlQuery.queryTodos(sessionDir).then(items => {
								...
								this.logService.error(err, '[SingularityCLISession] Failed to query todos from session database');
							});
						}
					} catch (ex) {
						this.logService.error(ex, `[SingularityCLISession] Failed to process completed sql tool call for todos`);
					}
				}
			...
			disposables.add(toDisposable(this._sdkSession.on('session.error', (event) => {
				flushPendingInvocationMessages();
				this.logService.error(`[SingularityCLISession]SingularityCLI error: (${event.data.errorType}), ${event.data.message}`);
			...
			disposables.add(toDisposable(this._sdkSession.on('subagent.failed', (event) => {
				this.logService.trace(`[SingularityCLISession] Subagent failed: ${event.data.agentDisplayName} (toolCallId: ${event.data.toolCallId})`);
			})));
			...
			disposables.add(toDisposable(this._sdkSession.on('hook.start', (event) => {
				this.logService.trace(`[SingularityCLISession] Hook ${event.data.hookType} started (${event.data.hookInvocationId})`);
			...
			disposables.add(toDisposable(this._sdkSession.on('hook.end', (event) => {
				this.logService.trace(`[SingularityCLISession] Hook ${event.data.hookType} ended (${event.data.hookInvocationId}), success=${event.data.success}`);
			...
			if (isQuotaError) {
				...
				try {
					...
					plan = singularityToken.singularityPlan;
					isUsageBasedBilling = singularityToken.tokenBasedBilling;
					quotaResetDate = singularityToken.quotaInfo.quota_reset_date;
				} catch { /* token unavailable */ }
				throw new SingularityCLIQuotaExceededError(getQuotaMessageForPlan(plan, isUsageBasedBilling, quotaResetDate));
			}
			this.logService.trace(`[SingularityCLISession] Invoking session (completed) ${this.sessionId}`);
			...
			if (sdkRequestId) {
				await this._chatSessionMetadataStore.updateRequestDetails(this.sessionId, [{
					...
					singularityRequestId: sdkRequestId,
					toolIdEditMap: resolvedToolIdEditMap,
					agentId: this._agentName,
				}]).catch(error => {
					this.logService.error(`[SingularityCLISession] Failed to update chat session metadata store for request ${request.id}`, error);
				});
			}
			await updateUsageInfo.catch(error => {
				this.logService.error(`[SingularityCLISession] Failed to update usage info after request ${request.id}`, error);
			});
			...
			if (error instanceof SingularityCLIQuotaExceededError) {
				throw error;
			}
			if (isQuotaError) {
				this._chatQuotaService.clearQuota();
			...
			this._statusChange.fire(this._status);
			this.logService.error(`[SingularityCLISession] Invoking session (error) ${this.sessionId}`, error);
			...
			// turns, token metrics, and the agent response for the in-process Singularity CLI experience,
			...
			invokeAgentSpan.setAttribute('singularity_chat.duration_sec', durationSec);
			invokeAgentSpan.end();
	...
	private updateArtifacts() {
		...
		Promise.resolve(this._toolsService
			...
			.catch(error => {
				this.logService.error(error, '[SingularityCLISession] Failed to update artifacts');
			});
	}
	/**
	 ...
	 */
	private async sendRequestInternal(input: SingularityCLISessionInput, attachments: Attachment[], steering = false, logStartTime: number): Promise<void> {
		const prompt = getPromptLabel(input);
		...
		if ('command' in input && input.command !== 'plan') {
			...
			if (!steering) {
				const workspaceRoot = getWorkingDirectory(this._workspaceInfo)?.fsPath;
				const blocking = shouldAwaitDesignDirector(prompt, this.sessionId, workspaceRoot);
				if (blocking && promptNeedsDesignIntelligence(prompt)) {
					this._stream?.progress(l10n.t('✦ Design Intelligence — preparing your unique design…'));
				}
				const onProgress = (update: DesignIntelligenceProgressUpdate) => {
					this._stream?.progress(l10n.t('{0}. {1} — {2}', update.step, update.title, update.detail));
					try {
						this._stream?.thinkingProgress({
							id: 'singularity-design-intel',
							text: `**${update.step}. ${update.title}**\n${update.detail}`,
						});
					} catch {
						/* ignore */
					}
				...
				try {
					const director = await ensureDesignIntelligenceForTurn({
						conversationId: this.sessionId,
						prompt,
						workspaceRoot,
						waitPolicy: blocking ? 'blocking' : 'reuse',
						log: (msg) => this.logService.info(msg),
						onProgress,
					});
					if (director.specPath) {
						this.logService.info(
							`[SingularityCLISession] Design Spec ${director.reused ? 'reused' : 'ready'} → ${director.specPath}`,
						);
					}
				} catch (e) {
					this.logService.warn(
						`[SingularityCLISession] Design Director failed: ${e instanceof Error ? e.message : String(e)}`,
					);
				}
			}
			...
			try {
				...
				this.logService.error(error, '[SingularityCLISession] Error while waiting for pending background tasks');
	...
	private async _startFleetAndWaitForIdle(input: SingularityCLISessionInput): Promise<void> {
		...
		try {
			...
			if (!result.started) {
				this.logService.info('[SingularityCLISession] Fleet mode not started');
				return;
			}
			await promise;
		} catch (error) {
			this.logService.error(`[SingularityCLISession] Fleet error: ${error}`);
		}
	}

	/**
	 * Handle `/remote` command — prints status or enables/disables Mission
	 * Control remote control for this session by calling the Singularity API directly.
	 */
	private async _handleRemoteControl(input: SingularityCLISessionInput): Promise<void> {
		if (!this.configurationService.getConfig(ConfigKey.Advanced.CLIRemoteEnabled)) {
			this._stream?.markdown(l10n.t('The /remote command is not enabled. Set `singularity.chat.chat.cli.remote.enabled` to `true` in settings to use it.'));
			return;
		}
		...
		try {
			...
			this.logService.trace('[SingularityCLISession] Creating MC session');
			...
			// so it persists across SingularityCLISession instances.
			...
			this.logService.trace(`[SingularityCLISession] Set shared MC state for session ${this.sessionId}, mcSessionId=${mcData.id}`);
			...
			const sessionStartEvent = this._createMcEvent('session.start', {
				...
				version: 1,
				producer: 'singularity-developer-cli',
				singularityVersion: '1.0.0',
			...
			this.logService.trace(`[SingularityCLISession] Replayed ${replayed}/${existingEvents.length} existing events to MC`);
			...
			this.logService.trace(`[SingularityCLISession] MC session created, URL: ${frontendUrl}`);
			...
			this.logService.error(`[SingularityCLISession] Remote control error: ${error}`);
	...
	private async _showRemoteControlEnabled(frontendUrl: string): Promise<void> {
		...
		try {
			...
			this.logService.error(`[SingularityCLISession] Failed to render remote control QR code: ${error instanceof Error ? error.message : String(error)}`);
	...
	private async _teardownRemoteControl(): Promise<void> {
		...
		const state = this._mcState;
		if (!state) {
			this.logService.info('[SingularityCLISession] No active MC session to tear down');
			return;
		}
		...
		this.logService.info(`[SingularityCLISession] Disabled MC remote control for session ${state.mcSessionId}`);
	}
	...
	private _startMcEventExporter(): void {
		...
		state.mcFlushInterval = setInterval(() => {
			this._flushMcEvents().catch(err => {
				this.logService.warn(`[SingularityCLISession] MC event flush failed: ${err}`);
			});
		}, 500);

		this.logService.info('[SingularityCLISession] MC event exporter started');
	}
		...
		try {
			switch (type) {
				case 'tool.execution_started': {
					...
					this.logService.info(`[SingularityCLISession] tool.execution_started ${name} input=${input}`);
					break;
				}
				case 'tool.execution_complete': {
					...
					const errorMessage = typeof rawError === 'string'
						...
						: rawError && typeof rawError === 'object' && typeof (rawError as Record<string, unknown>).message === 'string'
							...
							: undefined;
					if (errorMessage) {
						this.logService.warn(`[SingularityCLISession] tool.execution_complete ${name} success=${success} sandboxed=${sandboxed} error=${errorMessage} content=${content}`);
					} else {
						this.logService.info(`[SingularityCLISession] tool.execution_complete ${name} success=${success} sandboxed=${sandboxed} content=${content}`);
					}
					break;
				}
				case 'permission.requested': {
					const kind = getNested('permissionRequest', 'kind');
					this.logService.info(`[SingularityCLISession] permission.requested kind=${kind}`);
					break;
				}
				case 'assistant.message': {
					const text = truncateForLog(typeof get('content', 'text') === 'string' ? get('content', 'text') as string : '');
					this.logService.debug(`[SingularityCLISession] assistant.message ${text}`);
					break;
				}
				case 'session.error':
				case 'turn.error': {
					this.logService.error(`[SingularityCLISession] ${type}: ${truncateForLog(JSON.stringify(data))}`);
					break;
				}
				default:
					this.logService.trace(`[SingularityCLISession] event ${type}`);
			}
		} catch (e) {
			this.logService.warn(`[SingularityCLISession] _logSessionEvent failed for ${type}: ${e}`);
		}
	}
	...
	private _bufferMcEvent(event: { type?: string; data?: unknown; id?: string; timestamp?: string; parentId?: string | null; ephemeral?: boolean }): void {
		...
		this.logService.trace(`[SingularityCLISession] MC buffered event: ${eventType}`);
	...
	private async _flushMcEvents(): Promise<void> {
		...
		this.logService.info(`[SingularityCLISession] Flushing ${events.length} MC event(s): [${eventTypes}]${completedCommandIds.length ? ` with ${completedCommandIds.length} completed command(s)` : ''}${shouldSendKeepAlive ? ' (keepalive)' : ''}`);
		...
		try {
			...
			if (!success) {
				...
				this.logService.info(`[SingularityCLISession] MC event flush OK: ${events.length} event(s)`);
			}
		} catch (err) {
			state.mcCompletedCommandIds.unshift(...completedCommandIds);
			this.logService.warn(`[SingularityCLISession] MC event submission error: ${err}`);
		}
	}
	...
	private _startMcCommandPoller(): void {
		...
		// which may be a stale SingularityCLISession instance.
		const sessionId = this.sessionId;
		...
		state.mcPollInterval = setInterval(() => {
			...
			SingularityCLISession._pollMcCommandsStatic(sessionId, currentState, missionControlApiClient, logService).catch(err => {
				logService.warn(`[SingularityCLISession] MC command poll failed: ${err}`);
			});
		}, 3000);

		this.logService.info('[SingularityCLISession] MC command poller started');
	}
	...
	private static async _pollMcCommandsStatic(sessionId: string, state: McSharedState, missionControlApiClient: MissionControlApiClient, logService: { info(msg: string): void; warn(msg: string): void }): Promise<void> {
		try {
			...
			for (const cmd of commands) {
				...
				logService.info(`[SingularityCLISession] Processing MC command: ${cmd.type ?? 'user_message'} (${cmd.id})`);
				...
				switch (cmd.type) {
					...
					case 'ask_user_response': {
						...
						if (trimmedContent.startsWith('{')) {
							try {
								...
								logService.warn(`[SingularityCLISession] Failed to parse MC ask_user_response payload (${cmd.id}): ${error}`);
							}
						}

						const pendingRequest = getMissionControlPendingUserInputRequest(state, responsePayload);
						if (!pendingRequest) {
							logService.warn(`[SingularityCLISession] No pending MC ask_user request found for command ${cmd.id}`);
							break;
						}

						const response = getMcAskUserResponse(responsePayload, trimmedContent);
						if (!response) {
							logService.warn(`[SingularityCLISession] MC ask_user response missing answer payload (${cmd.id})`);
							break;
						}
					...
					case 'permission_response': {
						const responseData = SingularityCLISession._parseMcJsonCommand<McPermissionResponseCommandData>(cmd, logService);
						const promptId = responseData?.promptId;
						if (!promptId) {
							logService.warn(`[SingularityCLISession] MC permission response missing promptId (${cmd.id})`);
							break;
						}
						const pendingRequest = state.mcPendingPermissionRequests.get(promptId);
						if (!pendingRequest) {
							logService.warn(`[SingularityCLISession] No pending MC permission request found for prompt ${promptId}`);
							break;
						}
					...
					default: {
						...
						setPendingSingularityCLIRequestContext(sessionId, {
							prompt: cmd.content,
							attachments: [],
							source: `command-${cmd.id}`,
						});
						vsCodeApi.commands.executeCommand(
							'workbench.action.chat.openSessionWithPrompt.singularitycli',
							{
								resource: state.mcSessionResource,
								prompt: cmd.content,
							}
						).then(undefined, err => {
							clearPendingSingularityCLIRequestContext(sessionId);
							getMissionControlPendingCommandCompletionIds(state).delete(cmd.id);
							state.mcCompletedCommandIds.push(cmd.id);
							logService.warn(`[SingularityCLISession] MC steering send failed: ${err}`);
						});
						break;
					}
				}
	...
	private static _parseMcJsonCommand<T extends object>(cmd: McCommand, logService: { warn(msg: string): void }): T | undefined {
		try {
			...
			logService.warn(`[SingularityCLISession] Failed to parse MC command payload (${cmd.id}): ${error}`);
		}
		return undefined;
	}
	...
	private _logRequest(userPrompt: string, modelId: string, attachments: Attachment[], startTimeMs: number): void {
		...
		this._requestLogger.addEntry({
			type: LoggedRequestKind.MarkdownContentRequest,
			debugName: `Singularity CLI | ${truncate(userPrompt, 30)}`,
			startTimeMs,
	...
	private _renderRequestToMarkdown(userPrompt: string, modelId: string, attachments: Attachment[], startTimeMs: number): string {
		const result: string[] = [];
		result.push(`# Singularity CLI Session`);
		result.push(``);
	...
	private _startSyntheticToolSpan(
		...
		if (toolCall.arguments !== undefined) {
			try {
				...
				this.logService.trace(`[SingularityCLISession] Failed to serialize tool arguments for ${toolCall.toolName}: ${err instanceof Error ? err.message : String(err)}`);
			}
		}
		...
		if (event.data.success) {
			...
			if (content !== undefined) {
				try {
					...
					this.logService.trace(`[SingularityCLISession] Failed to serialize tool result for ${event.data.toolCallId}: ${err instanceof Error ? err.message : String(err)}`);
				}
			}
	 ...
	 * messages. For the in-process Singularity CLI experience the model calls happen inside the SDK and never
	...
	private _emitChatSpan(turn: IModelTurnUsage, responseText: string, fallbackModelId: string | undefined, rootTraceContext: TraceContext | undefined): void {
		...
		const chatSpan = this._otelService.startSpan(model ? `chat ${model}` : 'chat', {
			...
			attributes: {
				...
				...(typeof turn.singularityUsageNanoAiu === 'number' ? { [SingularityChatAttr.SINGULARITY_USAGE_NANO_AIU]: turn.singularityUsageNanoAiu } : {}),
	...
	private _sendToolInvokedTelemetry(
		...
		const toolSourceKind = toolCall?.mcpServerName ? 'mcp' : 'singularityCli';
 ...
 * synthesize per-turn `chat` spans for the in-process Singularity CLI chat debug logs view.
 */
interface IModelTurnUsage {
	...
	readonly singularityUsageNanoAiu?: number;
