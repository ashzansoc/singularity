...
import { SingularityChatAttr, SingularityCliSdkAttr, GenAiAttr, GenAiOperationName } from '../../../../platform/otel/common/genAiAttributes';
...
/**
 * Hook event data stashed by singularitycliSession for bridge enrichment.
 */
...
/**
 * Bridge SpanProcessor that forwards completed spans from the Singularity CLI SDK's
 ...
 *
 * The processor injects `singularity_chat.chat_session_id` on each forwarded span
 * using a traceId → sessionId mapping maintained by the extension.
 */
export class SingularityCliBridgeSpanProcessor implements SpanProcessor {
	/**
	 * Maps OTel traceId → VS Code chat session ID.
	 * Populated when singularitycliSession.ts creates its root `invoke_agent singularitycli` span.
	 */
	private readonly _traceIdToSessionId = new Map<string, string>();
	private _disposed = false;

	/**
	 * Hook event data stashed by singularitycliSession for enriching SDK hook spans.
	 ...
	 * Called by singularitycliSession before the SDK span ends.
	 */
	...
	onEnd(span: ReadableSpan): void {
		...
		const invocationId = span.attributes[SingularityCliSdkAttr.HOOK_INVOCATION_ID];
		if (span.name.startsWith('hook ') && span.attributes[SingularityCliSdkAttr.HOOK_TYPE] && typeof invocationId === 'string') {
	...
	private _injectEnrichedHookSpan(span: ICompletedSpanData, hookInvocationId: string): void {
		...
		attrs[SingularityChatAttr.HOOK_TYPE] = data.hookType;
		if (data.input) {
			attrs[SingularityChatAttr.HOOK_INPUT] = data.input;
		}
		if (data.output) {
			attrs[SingularityChatAttr.HOOK_OUTPUT] = data.output;
		}
		if (data.resultKind) {
			attrs[SingularityChatAttr.HOOK_RESULT_KIND] = data.resultKind;
		}
	...
	private _toCompletedSpan(span: ReadableSpan, sessionId: string): ICompletedSpanData {
		...
		if (sessionId && !baseAttributes[SingularityChatAttr.CHAT_SESSION_ID]) {
			baseAttributes[SingularityChatAttr.CHAT_SESSION_ID] = sessionId;
		}
