...
import { FakeToolsService, ToolCall } from '../../common/singularityCLITools';
import { Session } from '../../common/utils';
import { SingularityCLISession } from '../singularitycliSession';
import { PermissionRequest } from '../permissionHelpers';
import { IQuestion, IQuestionAnswer, IUserQuestionHandler, UserInputResponse } from '../userInputHelpers';
import { NullISingularityCLIImageSupport } from './testHelpers';
...
// Minimal shapes for types coming from the Singularity SDK we interact with
...
describe('SingularityCLISession', () => {
	...
	async function createSession(opts?: { sandboxEnabled?: boolean }): Promise<SingularityCLISession> {
	...
	async function createSessionWith(otelService: IOTelService, sandboxEnabled = false): Promise<SingularityCLISession> {
		...
		const sandboxConfig = sandboxEnabled
			...
			: undefined;
		return disposables.add(new SingularityCLISession(
			...
			instaService,
			requestLogger,
			new NullISingularityCLIImageSupport(),
			toolsService,
			...
			{ _serviceBrand: undefined, resetTurnCredits() { }, getCreditsForTurn() { return undefined; }, setLastSingularityUsage() { }, processQuotaSnapshots(snapshots: QuotaSnapshots) { processedQuotaSnapshots.push(snapshots); } } as any,
			telemetryService
		));
	}
	...
	it('emits languageModelToolInvoked telemetry for each completed tool invocation', async () => {
		...
		const session = await createSession();
		session.attachStream(new MockChatResponseStream());
		const sessionResource = Uri.parse('singularitycli:/test-session');
		...
		expect(invokedSnapshot).toEqual([
			{ result: 'success', chatSessionId: 'singularitycli:/test-session', toolId: 'bash', toolExtensionId: undefined, toolSourceKind: 'singularityCli', hasInvocationTimeMs: true },
			{ result: 'error', chatSessionId: 'singularitycli:/test-session', toolId: 'mcp_tool', toolExtensionId: undefined, toolSourceKind: 'mcp', hasInvocationTimeMs: true },
			{ result: 'userCancelled', chatSessionId: 'singularitycli:/test-session', toolId: 'bash', toolExtensionId: undefined, toolSourceKind: 'singularityCli', hasInvocationTimeMs: true },
			{ result: 'success', chatSessionId: 'singularitycli:/test-session', toolId: 'completion_tool', toolExtensionId: undefined, toolSourceKind: 'singularityCli', hasInvocationTimeMs: false },
		]);
	});
	...
	it('uses remote permission responses when Mission Control is active', async () => {
		...
		await (SingularityCLISession as any)._pollMcCommandsStatic(
			session.sessionId,
			remoteState,
			{
