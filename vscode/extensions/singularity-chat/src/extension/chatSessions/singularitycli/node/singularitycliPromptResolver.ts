...
import { generateUserPrompt } from '../../../prompts/node/agent/singularityCLIPrompt';
import { getWorkingDirectory, isIsolationEnabled, IWorkspaceInfo } from '../../common/workspaceInfo';
import { ISingularityCLIImageSupport, isImageMimeType } from './singularityCLIImageSupport';
import { ISingularityCLISkills } from './singularityCLISkills';
import { CancellationToken } from '../../../../util/vs/base/common/cancellation';
import { IVSCodeExtensionContext } from '../../../../platform/extContext/common/extensionContext';

export class SingularityCLIPromptResolver {
	constructor(
		@ISingularityCLIImageSupport private readonly imageSupport: ISingularityCLIImageSupport,
		@ILogService private readonly logService: ILogService,
		@IFileSystemService private readonly fileSystemService: IFileSystemService,
		@IWorkspaceService private readonly workspaceService: IWorkspaceService,
		@IInstantiationService private readonly instantiationService: IInstantiationService,
		@IIgnoreService private readonly ignoreService: IIgnoreService,
		@ISingularityCLISkills private readonly skillsService: ISingularityCLISkills,
		@IVSCodeExtensionContext private readonly extensionContext: IVSCodeExtensionContext,
	) { }

	/**
	 * Generates the final prompt for the Singularity CLI agent, resolving variables and preparing attachments.
	...
	private async constructChatVariablesAndAttachments(variables: ChatVariablesCollection, workspaceInfo: IWorkspaceInfo, additionalWorkspaces: IWorkspaceInfo[], token: vscode.CancellationToken): Promise<[variables: ChatVariablesCollection, Attachment[]]> {
		...
		await Promise.all(Array.from(variables).map(async variable => {
			...
			// Images will be attached using regular attachments via Singularity CLI SDK.
			...
			if (isLocation(variableRef.value)) {
				if (await this.ignoreService.isSingularityIgnored(variableRef.value.uri)) {
					return;
				}
				fileFolderReferences.push(variableRef);
				validReferences.push(variableRef);
			...
			if (URI.isUri(variableRef.value)) {
				if (await this.ignoreService.isSingularityIgnored(variableRef.value)) {
					return;
				}
				...
				// Files and directories will be attached using regular attachments via Singularity CLI SDK.
				validReferences.push(variableRef);
				fileFolderReferences.push(variableRef);
				return;
			}
		...
		await Promise.all(fileOrFolderReferences.map(async ref => {
			if (ref.value instanceof ChatReferenceBinaryData) {
				...
				try {
					...
					this.logService.error(`[SingularityCLISession] Failed to store image: ${error}`);
				}
				return;
			}
			...
			if (isLocation(ref.value)) {
				try {
					...
					this.logService.error(`[SingularityCLISession] Failed to attach location ${ref.value.uri.fsPath}: ${ex}`);
				}
				return;
			}
			...
			try {
				...
				if (!type) {
					this.logService.error(`[SingularityCLISession] Ignoring attachment as it's not a file/directory (${uri.fsPath})`);
					return;
				}
				attachments.push({
					type,
				...
				this.logService.error(`[SingularityCLISession] Failed to attach ${uri.fsPath}: ${error}`);
			}
		}));
	...
	private async translateWorkspaceRefToWorkingDirectoryRef(ref: vscode.ChatPromptReference, workspaceInfo: IWorkspaceInfo, additionalWorkspaces: IWorkspaceInfo[], folderToWorktreeMap: ResourceMap<vscode.Uri>, token: vscode.CancellationToken): Promise<vscode.ChatPromptReference> {
		try {
			...
			this.logService.error(error, `[SingularityCLISession] Failed to translate workspace reference`);
			return ref;
		}
	}
...
/**
 * Never include this variable in Singularity CLI prompts when using git worktrees (isolation).
