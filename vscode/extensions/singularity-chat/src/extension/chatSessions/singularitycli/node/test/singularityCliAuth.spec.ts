...
type SingularitySDKPackage = typeof import('@github/copilot/sdk');

describe('SingularityCLISDK Authentication', () => {
	const disposables = new DisposableStore();
	let instantiationService: IInstantiationService;
	let logService: ILogService;

	class TestSingularityCLISDK extends SingularityCLISDK {
		public override async getPackage(): Promise<SingularitySDKPackage> {
			return {
				logger: {
					setLogWriter: () => { },
				},
			...
			} as unknown as SingularitySDKPackage;
		}
	...
	it('should skip token validation when proxy URL is configured', async () => {
		...
		const sdk = new TestSingularityCLISDK(
		...
		expect(authInfo.singularityUser?.endpoints?.api).toBe('https://proxy.example.com');
	});
