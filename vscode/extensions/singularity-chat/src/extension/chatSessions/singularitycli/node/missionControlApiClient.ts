 ...
 * requires a permissive GitHub token (same scope as the Singularity Coding Agent
 ...
 *
 * Wraps the four endpoints used by the Singularity CLI `/remote` command:
 ...
 * by `IOctoKitService.postSingularityAgentJob`.
 */
export class MissionControlApiClient {
	...
	async createSession(
		...
		const res = await this._fetcherService.fetch(url, {
			callSite: 'singularitycli.mc.createSession',
			method: 'POST',
			headers,
			json: {
				owner_id: ownerId,
	...
	async submitEvents(
		...
		try {
			...
			const res = await this._fetcherService.fetch(url, {
				callSite: 'singularitycli.mc.submitEvents',
				method: 'POST',
				headers,
				json: {
					events,
	...
	async getPendingCommands(sessionId: string): Promise<McCommand[]> {
		try {
			...
			const res = await this._fetcherService.fetch(url, {
				callSite: 'singularitycli.mc.getPendingCommands',
				method: 'GET',
				headers,
				timeout: REQUEST_TIMEOUT_MS,
			});
	...
	async deleteSession(sessionId: string): Promise<void> {
		try {
			...
			await this._fetcherService.fetch(url, {
				callSite: 'singularitycli.mc.deleteSession',
	 ...
	 *
	 * Auth strategy follows `IOctoKitService.postSingularityAgentJob`: request a
	 ...
	 *
	 * The base URL is resolved via the Singularity token's `endpoints.api` which is
	 * GHES-aware.
	 */
	private async _buildRequest(
		...
		const singularityToken = await this._authService.getSingularityToken();
		const baseUrl = singularityToken.endpoints?.api;
		if (!baseUrl) {
			throw new Error('Singularity API endpoint is not available');
		}

		const url = `${baseUrl.replace(/\/+$/, '')}${path}`;
		const headers: Record<string, string> = {
			'Authorization': `Bearer ${session.accessToken}`,
			'Singularity-Integration-Id': INTEGRATION_ID,
		};
		return { url, headers };
	}
}
