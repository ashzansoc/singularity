...
import { SingularityCLIPromptResolver } from '../singularitycliPromptResolver';
import { MockSkillLocations, NullISingularityCLIImageSupport } from './testHelpers';

// Mock generateUserPrompt to avoid TSX rendering complexity in unit tests
vi.mock('../../../../prompts/node/agent/singularityCLIPrompt', () => ({
...
describe('SingularityCLIPromptResolver', () => {
	...
	function createResolver(overrideSkillLocations?: MockSkillLocations, overrideExtensionContext?: IVSCodeExtensionContext) {
		const imageSupport = new NullISingularityCLIImageSupport();
		...
		return new SingularityCLIPromptResolver(
			imageSupport as unknown as ISingularityCLIImageSupport,
			logService,
			fileSystemService,
			workspaceService,
