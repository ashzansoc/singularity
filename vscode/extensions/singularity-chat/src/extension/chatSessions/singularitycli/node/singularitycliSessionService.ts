...
import { deriveSingularityCliOTelEnv } from '../../../../platform/otel/common/agentOTelEnv';
...
import { buildChatHistoryFromEvents, RequestIdDetails, stripReminders } from '../common/singularityCLITools';
...
import { getAgentFileNameFromFilePath, ISingularityCLIAgents, ISingularityCLIModels, ISingularityCLISDK, isEnabledForSingularityCLI } from './singularityCli';
import { SingularityCliBridgeSpanProcessor } from './singularityCliBridgeSpanProcessor';
import { SingularityCLISession, ISingularityCLISession } from './singularitycliSession';
import { ISingularityCLISkills } from './singularityCLISkills';
import { ISingularityCLIMCPHandler, McpServerMappings, remapCustomAgentTools } from './mcpHandler';


const SINGULARITY_CLI_WORKSPACE_JSON_FILE_KEY = 'singularity.chat.cli.workspaceSessionFile';
const AGENT_HOST_DEFAULT_SESSIONS_PROVIDER_SETTING_ID = 'chat.agentHost.defaultSessionsProvider';
const SINGULARITY_CLI_HIDE_EXTENSION_HOST_EDITOR_SETTING_ID = 'chat.editor.singularityCli.hideExtensionHost';
const AGENT_HOST_SINGULARITY_CLIENT_NAME = 'vscode-agent-host';
export const SINGULARITY_CLI_CHAT_PANEL_SYSTEM_MESSAGE = 'You are Singularity, an AI coding assistant created by Zansoc Labs. You help users with software engineering tasks. When asked about your identity, name, model, or who made you: say you are Singularity, created by Zansoc Labs. Never mention Microsoft, GitHub, Singularity, VS Code, Visual Studio Code, or any underlying model name or provider.';

type SDKPackage = Awaited<ReturnType<ISingularityCLISDK['getPackage']>>;

export interface ISingularityCLISessionItem {
	readonly id: string;
...
export interface ISingularityCLISessionService {
	...
	onDidChangeSession: Event<ISingularityCLISessionItem>;
	onDidCreateSession: Event<ISingularityCLISessionItem>;

	getSessionWorkingDirectory(sessionId: string): Uri | undefined;

	// Session metadata querying
	getSessionItem(sessionId: string, token: CancellationToken): Promise<ISingularityCLISessionItem | undefined>;
	getSessionTitle(sessionId: string, token: CancellationToken): Promise<string>;
	getAllSessions(token: CancellationToken): Promise<readonly ISingularityCLISessionItem[]>;
	...
	getSession(options: IGetSessionOptions, token: CancellationToken): Promise<IReference<ISingularityCLISession> | undefined>;
	createSession(options: ICreateSessionOptions, token: CancellationToken): Promise<IReference<ISingularityCLISession>>;
...
export const ISingularityCLISessionService = createServiceIdentifier<ISingularityCLISessionService>('ISingularityCLISessionService');

export class SingularityCLISessionService extends Disposable implements ISingularityCLISessionService {
	...
	private readonly _onDidChangeSession = this._register(new Emitter<ISingularityCLISessionItem>());
	public readonly onDidChangeSession = this._onDidChangeSession.event;
	private readonly _onDidCreateSession = this._register(new Emitter<ISingularityCLISessionItem>());
	...
	private readonly _sessionTracker: SingularityCLISessionWorkspaceTracker;
	...
	private readonly _cachedSessionItems = new Map<string, ISingularityCLISessionItem>();
	...
	constructor(
		...
		@ISingularityCLIMCPHandler private readonly mcpHandler: ISingularityCLIMCPHandler,
		@ISingularityCLIAgents private readonly agents: ISingularityCLIAgents,
		...
		@ISingularityCLISkills private readonly singularityCLISkills: ISingularityCLISkills,
		...
		@ISingularityCLIModels private readonly _singularityCLIModels: ISingularityCLIModels,
		...
		this._sessionManager = new Lazy<Promise<internal.LocalSessionManager>>(async () => {
			try {
				...
				if (this._otelService.config.enabledExplicitly) {
					const otelEnv = deriveSingularityCliOTelEnv(this._otelService.config);
					...
					process.env['SINGULARITY_OTEL_FILE_EXPORTER_PATH'] = devNull;
					delete process.env['OTEL_EXPORTER_OTLP_ENDPOINT'];
					delete process.env['SINGULARITY_OTEL_ENDPOINT'];
				}
				...
				this.logService.error(`Failed to initialize Singularity CLI Session Manager: ${error}`);
				throw error;
			}
		});
		this._sessionTracker = this.instantiationService.createInstance(SingularityCLISessionWorkspaceTracker);
	}
	...
	private get sessionFileMonitoringDisabledSettingId(): string {
		return this._agentSessionsWorkspace.isAgentSessionsWorkspace
			...
			: SINGULARITY_CLI_HIDE_EXTENSION_HOST_EDITOR_SETTING_ID;
	}
	...
	private async getSDKPackage(): Promise<SDKPackage> {
		return this.singularityCLISDK.getPackage();
	}
	...
	private _registerOTelEnvSnapshot(): void {
		...
		const keys = [
			'SINGULARITY_OTEL_ENABLED',
			'OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT',
			'OTEL_EXPORTER_OTLP_ENDPOINT',
			'SINGULARITY_OTEL_ENDPOINT',
			'SINGULARITY_OTEL_EXPORTER_TYPE',
			'SINGULARITY_OTEL_FILE_EXPORTER_PATH',
		];
	...
	protected monitorSessionFiles(): void {
		...
		try {
			const sessionDir = joinPath(this.nativeEnv.userHome, '.singularity', 'session-state');
			...
			disposables.dispose();
			this.logService.error('Failed to monitor Singularity CLI session files:', error);
			return;
		}
	 ...
	 */
	public async getSessionItem(sessionId: string, token: CancellationToken): Promise<ISingularityCLISessionItem | undefined> {
		return this.getSessionItemImpl(sessionId, 'inMemorySession', token);
	}

	public async getSessionItemImpl(sessionId: string, source: 'inMemorySession' | 'disk', token: CancellationToken): Promise<ISingularityCLISessionItem | undefined> {
		...
		// // We can get the item from cache, as the ISingularityCLISessionItem doesn't store anything that changes.
		// // Except the title
		...
		if (metadata.clientName === AGENT_HOST_SINGULARITY_CLIENT_NAME) {
			return;
		}
	...
	private _getAllSessionsProgress: Promise<readonly ISingularityCLISessionItem[]> | undefined;
	private _isGettingSessions: number = 0;
	/** Singularity CLI native addon is missing in this checkout — do not respawn it on every hello. */
	private _cliNativeUnavailable = false;
	async getAllSessions(token: CancellationToken): Promise<readonly ISingularityCLISessionItem[]> {
		if (this._cliNativeUnavailable) {
			return [];
		}
	...
	async _getAllSessions(token: CancellationToken): Promise<readonly ISingularityCLISessionItem[]> {
		...
		try {
			...
			// Skip sessions the agent host already lists (both surfaces share `~/.singularity/session-state/`).
			const agentHostDataDir = this._getAgentHostSessionDataDir();
			const sessionsWithoutAgentHostClientName = sessionMetadataList.filter(metadata => metadata.clientName !== AGENT_HOST_SINGULARITY_CLIENT_NAME);
			...
			// Convert SessionMetadata to ISingularityCLISession
			const diskSessions: ISingularityCLISessionItem[] = coalesce(await Promise.all(
				sessionMetadataList.map(async (metadata): Promise<ISingularityCLISessionItem | undefined> => {
					const sanitizedSessionId = metadata.sessionId.replace(/[^a-zA-Z0-9_.-]/g, '-');
					if (metadata.clientName === AGENT_HOST_SINGULARITY_CLIENT_NAME || agentHostOwnedSessionIds.has(sanitizedSessionId)) {
						return;
					}
			...
			const newSessions = coalesce(await Promise.all(Array.from(this._sessionWrappers.values())
				...
				.map(async (session): Promise<ISingularityCLISessionItem | undefined> => {
			...
			const allSessions = diskSessions
				.map((session): ISingularityCLISessionItem => {
					return {
						...session,
			...
			if (/Native addon/.test(msg)) {
				this._cliNativeUnavailable = true;
				this.logService.warn('Singularity CLI native addon missing; skipping session list');
				return [];
			}
	...
	private async constructSessionItem(metadata: LocalSessionMetadata, token: CancellationToken): Promise<ISingularityCLISessionItem | undefined> {
	...
	private async constructSessionItemFromWrappedSession(session: RefCountedSession, token: CancellationToken): Promise<ISingularityCLISessionItem | undefined> {
	...
	private async constructSessionItemImpl(metadata: LocalSessionMetadata, token: CancellationToken): Promise<ISingularityCLISessionItem | undefined> {
		const workingDirectory = metadata.context?.cwd ? URI.file(metadata.context.cwd) : undefined;
		this._sessionWorkingDirectories.set(metadata.sessionId, workingDirectory);
	...
	public async createSession(options: ICreateSessionOptions, token: CancellationToken): Promise<RefCountedSession> {
		const resource = options.sessionId ? SessionIdForCLI.getResource(options.sessionId) : URI.from({ scheme: 'singularity-cli', path: `mcp-gateway-${generateUuid()}` });
		...
		try {
			...
			if (sessionOptions.singularityUrl) {
				...
				const singularityUser = { endpoints: { api: sessionOptions.singularityUrl } };
				const host = 'https://github.com' as const;
				const authInfo = authType === 'token'
					? { type: 'token' as const, token: 'mock-token', host, singularityUser }
					: { type: 'hmac' as const, hmac: 'empty', host, singularityUser };
				sdkSession.updateOptions({ authInfo });
			}
			this.logService.trace(`[SingularityCLISession] Created new SingularityCLI session ${sdkSession.sessionId}.`);

			const session = this.createSingularitySession(sdkSession, options.workspace, options.agent?.name, sessionManager, sessionOptions.sandboxConfig);
	...
	private _installBridgeIfNeeded(): void {
		...
		try {
			...
			// The in-process Singularity CLI SDK does not register its own JS OTel
			...
			// synthesized directly from the SDK event stream in `singularitycliSession.ts`.
			if ((delegate as Record<string, unknown>).__singularityChatOwnProvider) {
				this.logService.info('[SingularityCLISession] Global OTel provider is the extension provider; skipping bridge install (debug entries are synthesized from SDK events)');
				return;
			}
			...
			if (Array.isArray(processorArray)) {
				this._bridgeProcessor = new SingularityCliBridgeSpanProcessor(this._otelService);
				processorArray.push(this._bridgeProcessor);
				this.logService.info('[SingularityCLISession] Bridge SpanProcessor installed on SDK TracerProvider');
			} else {
				this.logService.warn('[SingularityCLISession] Could not access SDK TracerProvider internals — debug panel will not show SDK spans');
			}
		} catch (err) {
			this.logService.warn(`[SingularityCLISession] Failed to install bridge SpanProcessor: ${err}`);
		}
	}
	...
	protected async createSessionsOptions(options: ICreateSessionOptions & { mcpServers?: SessionOptions['mcpServers'] }): Promise<Readonly<SessionOptions>> {
		const [agentInfos, skillLocations] = await Promise.all([
			...
			this.singularityCLISkills.getSkillsLocations(CancellationToken.None),
		]);
		...
		const systemMessage: NonNullable<SessionOptions['systemMessage']> = {
			...
			sections: {
				identity: {
					action: 'replace',
					content: SINGULARITY_CLI_CHAT_PANEL_SYSTEM_MESSAGE,
				},
			},
		};
		if (variablesContext) {
		...
		const singularityUrl = this.configurationService.getConfig(ConfigKey.Shared.DebugOverrideProxyUrl) || undefined;
		if (singularityUrl) {
			allOptions.singularityUrl = singularityUrl;
		}
		...
		try {
			{
				...
				if (session) {
					this.logService.trace(`[SingularityCLISession] Reusing SingularityCLI session ${options.sessionId}.`);
			...
			try {
				...
				if (!sdkSession) {
					this.logService.error(`[SingularityCLISession] SingularityCLI failed to get session ${options.sessionId}.`);
					return undefined;
				}
	...
	private async getChatHistoryImpl({ sessionId, workspace }: { sessionId: string; workspace: IWorkspaceInfo }, token: CancellationToken): Promise<{ history: (ChatRequestTurn2 | ChatResponseTurn2)[]; events: readonly SessionEvent[] }> {
		...
		if (existingSession) {
			...
			try {
				...
				shutdown = sessionManager.closeSession(sessionId).catch(error => {
					this.logService.error(`[SingularityCLISession] Failed to close session ${sessionId} after fetching chat history: ${error}`);
				});
			} catch (error) {
				this.logService.error(`[SingularityCLISession] Failed to read session ${sessionId}, it may be corrupted: ${error}`);
				return { history: [], events: [] };
			} finally {
				await shutdown;
			}
		...
		// Build lookup from singularityRequestId → RequestDetails for the callback
		const legacyMappings: RequestDetails[] = [];
		const detailsBySingularityId = new Map<string, RequestIdDetails>();
		...
		for (const d of storedDetails) {
			if (d.singularityRequestId) {
				...
				detailsBySingularityId.set(d.singularityRequestId, { requestId: d.vscodeRequestId, toolIdEditMap: d.toolIdEditMap, modeInstructions, responseModelId: d.responseModelId, creditsUsed: d.creditsUsed, isUsingAutoModel: d.isUsingAutoModel });
			}
		}
		const getVSCodeRequestId = (sdkRequestId: string) => {
			const stored = detailsBySingularityId.get(sdkRequestId);
			if (stored) {
				return stored;
			}
			const mapping = this.singularityCLISDK.getRequestId(sdkRequestId);
			if (mapping) {
				detailsBySingularityId.set(sdkRequestId, mapping);
				legacyMappings.push({
		...
		if (legacyMappings.length > 0) {
			void this._chatSessionMetadataStore.updateRequestDetails(sessionId, legacyMappings).catch(error => {
				this.logService.error(`[SingularityCLISession] Failed to update chat session metadata store with legacy mappings for session ${sessionId}`, error);
			});
		}
		return { history, events };
	}
		...
		this._customAgentLookupRebuild = (async () => {
			...
			try {
				...
				for (const agent of agents) {
					if (!agent.enabled || !isEnabledForSingularityCLI(agent)) {
						continue;
					}
					const keys = coalesce([
						agent.name?.trim(),
	...
	private async getModelDetailsById(): Promise<ReadonlyMap<string, ModelDetailsInfo>> {
		const models = await this._singularityCLIModels.getModels().catch(ex => {
	...
	public async forkSession({ sessionId, requestId, workspace }: { sessionId: string; requestId: string | undefined; workspace: IWorkspaceInfo }, token: CancellationToken): Promise<string> {
		...
		if (requestId) {
			...
			if (requestToTruncateTo) {
				...
				const translatedSDKEvent = storedDetails.find(d => d.vscodeRequestId === requestToTruncateTo.id || d.singularityRequestId === requestToTruncateTo.id)?.singularityRequestId;
				...
				if (!toEventId) {
					this.logService.warn(`[SingularityCLISession] Cannot find SDK event id for request id ${requestId} in session ${sessionId}. Will fork without truncation.`);
				}
			} else {
				this.logService.warn(`[SingularityCLISession] Failed to find request ${requestId} in session ${sessionId} history. Will fork without truncation.`);
			}
		}
	...
	public async tryGetPartialSessionHistory(sessionId: string): Promise<readonly (ChatRequestTurn2 | ChatResponseTurn2)[] | undefined> {
		...
		try {
			...
			this.logService.warn(`[SingularityCLISession] Failed to reconstruct partial session ${sessionId}: ${error}`);
			return undefined;
		}
	}
	...
	private async getFirstUserMessageFromSession(sessionId: string, token: CancellationToken): Promise<string | undefined> {
		...
		try {
			...
			this.logService.warn(`[SingularityCLISession] Failed to get session title for session ${sessionId}: ${error}`);
		}
		...
		if (!firstUserMessage) {
			try {
				...
				this.logService.warn(`[SingularityCLISession] Failed to load session for first user message ${sessionId}: ${error}`);
			}
		}

		this._chatSessionMetadataStore.setSessionFirstUserMessage(sessionId, firstUserMessage ?? '').catch(err => {
			this.logService.warn(`[SingularityCLISession] Failed to store first user message for session ${sessionId}: ${err}`);
		});

		return firstUserMessage;
	}

	private createSingularitySession(sdkSession: Session, workspaceInfo: IWorkspaceInfo, agentName: string | undefined, sessionManager: internal.LocalSessionManager, sandboxConfig: SessionOptions['sandboxConfig']): RefCountedSession {
		sdkSession.permissions.setRequired({ required: true });
		const session = this.instantiationService.createInstance(SingularityCLISession, workspaceInfo, agentName, sdkSession, [], sandboxConfig);
		this._debugFileLogger.startSession(session.sessionId).catch(err => {
			this.logService.error('[SingularityCLISession] Failed to start debug log session', err);
		});
		session.add(toDisposable(() => {
			this._debugFileLogger.endSession(session.sessionId).catch(err => {
				this.logService.error('[SingularityCLISession] Failed to end debug log session', err);
			});
		}));
		...
		try {
			...
			if (shouldCloseSession && sessionManager) {
				await sessionManager.closeSession(sessionId).catch(error => {
					this.logService.error(`[SingularityCLISession] Failed to close session ${sessionId} after updating title metadata: ${error}`);
				});
			}
		}
	}
...
async function readSessionEventsFile(sessionId: string, findFirstEventType?: string): Promise<SessionEvent[]> {
	const sessionDirPath = getSingularityCLISessionDir(sessionId);
...
export class SingularityCLISessionWorkspaceTracker {
	...
	constructor(
		...
		this._initializeSessionStorageFiles = new Lazy<Promise<{ global: Uri; workspace: Uri }>>(async () => {
			const globalFile = joinPath(this.context.globalStorageUri, 'singularity.cli.oldGlobalSessions.json');
			let workspaceFile = joinPath(this.context.globalStorageUri, 'singularity.cli.workspaceSessions.json');
			...
			if (this.workspaceService.getWorkspaceFolders().length) {
				let workspaceFileName = this.context.workspaceState.get<string | undefined>(SINGULARITY_CLI_WORKSPACE_JSON_FILE_KEY);
				if (!workspaceFileName) {
					workspaceFileName = `singularity.cli.workspaceSessions.${generateUuid()}.json`;
					await this.context.workspaceState.update(SINGULARITY_CLI_WORKSPACE_JSON_FILE_KEY, workspaceFileName);
				}
...
export class RefCountedSession extends RefCountedDisposable implements IReference<SingularityCLISession> {
	constructor(public readonly object: SingularityCLISession) {
		super(object);
	}
	dispose(): void {
		this.release();
