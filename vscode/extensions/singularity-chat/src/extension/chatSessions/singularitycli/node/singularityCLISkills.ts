...
import { isEnabledForSingularityCLI } from './singularityCli';
import { resolveSkillConfigLocations } from '../../common/skillConfigLocations';

export interface ISingularityCLISkills {
	readonly _serviceBrand: undefined;
	getSkillsLocations(token: CancellationToken): Promise<Uri[]>;
}

export const ISingularityCLISkills = createServiceIdentifier<ISingularityCLISkills>('ISingularityCLISkills');

export class SingularityCLISkills extends Disposable implements ISingularityCLISkills {
	...
	public async getSkillsLocations(token: CancellationToken): Promise<Uri[]> {
		...
		(await this.promptsService.getSkills(token))
			.filter(isEnabledForSingularityCLI)
