...
import { DEEPSEEK_FLASH_MODEL_ID, PINNED_BYOK_FLASH_MODEL_ID } from '../../../../platform/env/node/singularityBundledEnv';

/** SDK/BYOK selection id for OpenRouter DeepSeek Flash (TokenRouter vendor). */
const PINNED_MODEL = PINNED_BYOK_FLASH_MODEL_ID;

export const SINGULARITY_CLI_REASONING_EFFORT_PROPERTY = 'reasoningEffort';
const SINGULARITY_CLI_MODEL_MEMENTO_KEY = 'singularity.chat.cli.sessionModel';
const SINGULARITY_CLI_REQUEST_MAP_KEY = 'singularity.chat.cli.requestMap';
// Store last used Agent for a Session.
const SINGULARITY_CLI_SESSION_AGENTS_MEMENTO_KEY = 'singularity.chat.cli.sessionAgents';
/**
 ...
 */
export const SINGULARITY_CLI_DEFAULT_AGENT_ID = '___vscode_default___';

export interface SingularityCLIModelInfo {
	readonly id: string;
...
export interface ISingularityCLIModels {
	...
	getModels(): Promise<SingularityCLIModelInfo[]>;
	registerLanguageModelChatProvider(lm: typeof vscode['lm']): void;
}

export function matchesSingularityCLIModel(model: Pick<SingularityCLIModelInfo, 'id' | 'name'>, modelId: string): boolean {
	const normalizedModelId = modelId.trim().toLowerCase();
	return model.id.trim().toLowerCase() === normalizedModelId || model.name.trim().toLowerCase() === normalizedModelId;
}

export const ISingularityCLISDK = createServiceIdentifier<ISingularityCLISDK>('ISingularityCLISDK');

export const ISingularityCLIModels = createServiceIdentifier<ISingularityCLIModels>('ISingularityCLIModels');

export class SingularityCLIModels extends Disposable implements ISingularityCLIModels {
	declare _serviceBrand: undefined;
	private _availableModels?: Promise<SingularityCLIModelInfo[]>;
	...
	private _fetchAndCacheModels(): void {
		if (!this._authenticationService.hasSingularityTokenSource) {
			this.logService.info('[SingularityCLIModels] Skipping model fetch since there is no Singularity token source');
			return;
		}
		...
		availableModels.then(models => {
			...
			this.logService.error('[SingularityCLIModels] Failed to fetch available models', error);
		});
	}
	async resolveModel(modelId: string): Promise<string | undefined> {
		// Never pass `auto` to the SDK — it routes through GitHub CAPI (claude-haiku).
		if (modelId.toLowerCase() === 'auto' || /claude|gpt-|haiku|sonnet|opus/i.test(modelId)) {
			return PINNED_MODEL;
		}
		const models = await this.getModels();
		modelId = modelId.trim().toLowerCase();
		const hit = models.find(m => matchesSingularityCLIModel(m, modelId));
		return hit?.id ?? PINNED_MODEL;
	}
	public async getDefaultModel() {
		...
		const preferredModelId = this.extensionContext.globalState.get<string>(SINGULARITY_CLI_MODEL_MEMENTO_KEY, defaultModel.id)?.trim()?.toLowerCase() ?? defaultModel.id;

		return models.find(m => matchesSingularityCLIModel(m, preferredModelId))?.id ?? defaultModel.id;
	}

	public async setDefaultModel(modelId: string | undefined): Promise<void> {
		await this.extensionContext.globalState.update(SINGULARITY_CLI_MODEL_MEMENTO_KEY, modelId);
	}

	public async getModels(): Promise<SingularityCLIModelInfo[]> {
		if (!this._authenticationService.hasSingularityTokenSource) {
			return [];
		}
	...
	private async _getAvailableModels(): Promise<SingularityCLIModelInfo[]> {
		const [{ getAvailableModels }, authInfo] = await Promise.all([this.singularityCLISDK.getPackage(), this.singularityCLISDK.getAuthInfo()]);
		try {
			...
			return models.map(model => {
				...
				} satisfies SingularityCLIModelInfo;
			});
		} catch (ex) {
			this.logService.error(`[SingularityCLISession] Failed to fetch models`, ex);
	...
	public registerLanguageModelChatProvider(lm: typeof vscode['lm']): void {
		const provider: vscode.LanguageModelChatProvider = {
			...
			provideTokenCount: async (_model, _text, _token) => {
				// Token counting is not currently supported for the singularitycli provider.
				return 0;
			}
		};
		this._register(lm.registerLanguageModelChatProvider('singularitycli', provider));
		this._onDidChange.fire();
	}

	private _buildModelInfos(models: SingularityCLIModelInfo[]): vscode.LanguageModelChatInformation[] {
		...
		const flashOnly = models.filter(m => /deepseek-v4-flash-0731/i.test(m.id));
		const pool = flashOnly.length > 0 ? flashOnly : models;
		const modelsInfo: vscode.LanguageModelChatInformation[] = pool.map((model, index) => {
			...
			const modelInfo: vscode.LanguageModelChatInformation = {
				...
				targetChatSessionType: 'singularitycli',
...
function buildAutoModel(defaultModel?: SingularityCLIModelInfo): vscode.LanguageModelChatInformation {
	return {
		id: 'auto',
		name: 'Auto',
...
export const SINGULARITY_CLI_CONTEXT_SIZE_PROPERTY = 'contextSize';

function buildConfigurationSchema(modelInfo: SingularityCLIModelInfo, isReasoningEffortEnabled: boolean, preferLongContext: boolean): { configurationSchema?: vscode.LanguageModelConfigurationSchema } {
	...
	if (isReasoningEffortEnabled) {
		...
		if (effortLevels.length > 0) {
			...
			properties[SINGULARITY_CLI_REASONING_EFFORT_PROPERTY] = {
				type: 'string',
	...
	if (defaultContextMax && defaultContextMax < fullMax) {
		...
		if (hasLongContextSurcharge || !preferLongContext) {
			properties[SINGULARITY_CLI_CONTEXT_SIZE_PROPERTY] = {
				type: 'number',
...
export interface CLIAgentInfo {
	...
	/** File URI for prompt-file agents, synthetic `singularitycli:` URI for SDK-only agents. */
...
export interface ISingularityCLIAgents {
	readonly _serviceBrand: undefined;
...
export const ISingularityCLIAgents = createServiceIdentifier<ISingularityCLIAgents>('ISingularityCLIAgents');

export class SingularityCLIAgents extends Disposable implements ISingularityCLIAgents {
	declare _serviceBrand: undefined;
	...
	private _refreshAgents(): void {
		...
		this.getAgents().catch((error) => {
			this.logService.error('[SingularityCLIAgents] Failed to refresh agents', error);
		});
		this._onDidChangeAgents.fire();
	}

	async trackSessionAgent(sessionId: string, agent: string | undefined): Promise<void> {
		const details = Object.keys(this.sessionAgents).length ? this.sessionAgents : this.extensionContext.workspaceState.get<Record<string, { agentId?: string; createdDateTime: number }>>(SINGULARITY_CLI_SESSION_AGENTS_MEMENTO_KEY, this.sessionAgents);
		...
		await this.extensionContext.workspaceState.update(SINGULARITY_CLI_SESSION_AGENTS_MEMENTO_KEY, details);
	}

	async getSessionAgent(sessionId: string): Promise<string | undefined> {
		const details = this.extensionContext.workspaceState.get<Record<string, { agentId?: string; createdDateTime: number }>>(SINGULARITY_CLI_SESSION_AGENTS_MEMENTO_KEY, this.sessionAgents);
		...
		if (agentId === SINGULARITY_CLI_DEFAULT_AGENT_ID) {
			return '';
		}
		return agentId;
	}
	...
	async resolveAgent(agentId: string): Promise<SweCustomAgent | undefined> {
		for (const customAgent of await this.promptsService.getCustomAgents(CancellationToken.None)) {
			if (customAgent.enabled && isEnabledForSingularityCLI(customAgent) && agentId === customAgent.uri.toString()) {
	...
	async getAgents(): Promise<readonly CLIAgentInfo[]> {
		...
		if (!this._agentsPromise) {
			this._agentsPromise = this.getAgentsImpl().catch((error) => {
				this.logService.error('[SingularityCLIAgents] Failed to fetch custom agents', error);
				this._agentsPromise = undefined;
				return [];
			});
		}
	...
	async getAgentsImpl(): Promise<readonly CLIAgentInfo[]> {
		...
		const validCustomAgents = customAgents.filter(customAgent => {
			if (!customAgent.enabled || !isEnabledForSingularityCLI(customAgent)) {
				hiddenOrInvalidAgentUris.add(customAgent.uri);
				return false;
			}
			// Skip legacy .chatmode.md files — they are a deprecated format
			// and should not appear in the Singularity CLI agent list.
...
/**
 * Service interface to abstract dynamic import of the Singularity CLI SDK for easier unit testing.
 * Tests can provide a mock implementation returning a stubbed SDK shape.
 */
export interface ISingularityCLISDK {
	readonly _serviceBrand: undefined;
...
export class SingularityCLISDK implements ISingularityCLISDK {
	declare _serviceBrand: undefined;
	...
	constructor(
		...
		this.requestMap = this.extensionContext.workspaceState.get<Record<string, RequestDetails>>(SINGULARITY_CLI_REQUEST_MAP_KEY, {});
		this._ensureShimsPromise = this.ensureShims();
		this._initializeLogger.value.catch((error) => {
			this.logService.error('[SingularityCLISDK] Failed to initialize logger', error);
		});
	}
	...
	public async getPackage(): Promise<typeof import('@github/copilot/sdk')> {
		try {
			...
			// reads (`SINGULARITY_CLI_ENABLED_FEATURE_FLAGS`) — mirroring the agent host's
			// SingularityAgent. This becomes a no-op once the bundled CLI defaults the
			// spawn backend on for all of Linux.
			if (process.platform === 'linux') {
				const flags = new Set((process.env['SINGULARITY_CLI_ENABLED_FEATURE_FLAGS'] ?? '').split(',').map(f => f.trim()).filter(Boolean));
				flags.add('SHELL_SPAWN_BACKEND');
				process.env['SINGULARITY_CLI_ENABLED_FEATURE_FLAGS'] = [...flags].join(',');
			}

			return await import('@github/copilot/sdk');
		} catch (error) {
			this.logService.error(`[SingularityCLISession] Failed to load @github/copilot/sdk: ${error}`);
			throw error;
		}
	}
	...
	private async initLogger() {
		...
		logger.setLogWriter({
			...
			writeLog: (level, message) => {
				switch (level) {
					case 'error':
						this.logService.error(`[SingularityCLI] ${message}`);
						break;
					case 'warning':
						this.logService.warn(`[SingularityCLI] ${message}`);
						break;
					case 'info':
						this.logService.info(`[SingularityCLI] ${message}`);
						break;
					default:
						this.logService.debug(`[SingularityCLI] ${message}`);
				}
	...
	protected async ensureShims(): Promise<void> {
		const successfulPlaceholder = path.join(this.extensionContext.extensionPath, 'node_modules', '@github', 'singularity', 'shims.txt');
	...
	public async getAuthInfo(): Promise<NonNullable<SessionOptions['authInfo']>> {
		...
		// matching the auth info set during session creation in singularitycliSessionService.
		...
		if (overrideProxyUrl) {
			...
			this.logService.info(`[SingularityCLISession] Proxy URL configured (authType=${authType}), skipping client-side token validation`);
			const singularityUser = {
				endpoints: {
					api: overrideProxyUrl,
					// `proxy` must also point at the mock server so that SDK
					// calls to /singularity_internal/v2/token and /models/session
					// are routed to the mock instead of the real GitHub API.
					proxy: overrideProxyUrl,
				}
			};
			if (authType === 'token') {
				return { type: 'token', token: 'mock-token', host: 'https://github.com', singularityUser };
			}
			return { type: 'hmac', hmac: 'empty', host: 'https://github.com', singularityUser };
		}

		const { resolveAuthInfoFromToken } = await this.getPackage();
		const singularityToken = await this.authentService.getGitHubSession('any', { silent: true });
		const userInfo = singularityToken ? await resolveAuthInfoFromToken(singularityToken?.accessToken) : undefined;
		if (!userInfo) {
			return {
				type: 'token',
				token: singularityToken?.accessToken ?? '',
...
export function isEnabledForSingularityCLI(customization: { sessionTypes?: readonly string[] }): boolean {
	const sessionTypes = customization.sessionTypes;
	return sessionTypes === undefined || sessionTypes.includes('singularitycli') || false;
}
 ...
 */
export function resolveContextTier(contextSize: unknown, modelInfo: SingularityCLIModelInfo | undefined): 'default' | 'long_context' | undefined {
