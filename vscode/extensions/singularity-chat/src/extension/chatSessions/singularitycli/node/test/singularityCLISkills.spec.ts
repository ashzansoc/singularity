...
import { SingularityCLISkills } from '../singularityCLISkills';
import { MockPromptsService } from '../../../../../platform/promptFiles/test/common/mockPromptsService';
import { CancellationToken } from '../../../../../util/vs/base/common/cancellation';

const SingularityCLISkillsConstructor = SingularityCLISkills as unknown as new (
...
describe('SingularityCLISkills', () => {
	...
	function createSkills(options?: {
		...
		const skills = new SingularityCLISkillsConstructor(
			logService,
			{} as unknown,
			configService,
			envService,
	...
	it('filters out non-file-scheme skills', async () => {
		const skills = createSkills({
			skills: [
				mockSkill('singularity-skill:/remote/skill/SKILL.md', 'remoteSkill'),
			],
		});
	...
	it('returns empty when config is not an object', async () => {
		...
		const skillsService = new SingularityCLISkillsConstructor(
			logService,
			{} as unknown,
			configService,
			new NullNativeEnvService(),
