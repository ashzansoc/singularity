...
describe('MissionControlApiClient', () => {
	it('uses the shared integration id for all mission control requests', async () => {
		...
		const authenticationService = {
			...
			getSingularityToken: vi.fn(async () => ({ token: 'singularity-token', endpoints: { api: 'https://api.github.test/' } })),
		...
		expect(requests.map(({ options }) => options.headers?.['Singularity-Integration-Id'])).toEqual([
			INTEGRATION_ID,
			INTEGRATION_ID,
			INTEGRATION_ID,
			INTEGRATION_ID,
