...
import type { TodoItem, TodoSqlWorkerApi } from './singularityCLITodoWorker';
...
export class TodoSqlQuery {
	...
	private ensureWorker(): RpcProxy<TodoSqlWorkerApi> {
		if (!this._proxy) {
			this._worker = new WorkerWithRpcProxy<TodoSqlWorkerApi>(
				join(__dirname, 'singularityCLITodoWorker.js')
			);
	 ...
	 * @param sessionDir - The session state directory (e.g. ~/.singularity/session-state/{session-id})
