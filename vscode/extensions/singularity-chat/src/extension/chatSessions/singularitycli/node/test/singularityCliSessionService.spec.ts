...
import { FakeToolsService } from '../../common/singularityCLITools';
...
import { ISingularityCLISDK } from '../singularityCli';
import { SingularityCLISession, ISingularityCLISession } from '../singularitycliSession';
import { SINGULARITY_CLI_CHAT_PANEL_SYSTEM_MESSAGE, SingularityCLISessionService, SingularityCLISessionWorkspaceTracker, ISingularityCLISessionItem } from '../singularitycliSessionService';
import { SingularityCLIMCPHandler } from '../mcpHandler';
...
import { MockCliSdkSession, MockCliSdkSessionManager, MockSkillLocations, NullSingularityCLIAgents, NullSingularityCLIModels, NullISingularityCLIImageSupport } from './testHelpers';

// Re-export for backward compatibility with other spec files
export { MockCliSdkSession, MockCliSdkSessionManager, MockSkillLocations, NullSingularityCLIAgents, NullISingularityCLIImageSupport } from './testHelpers';
...
describe('SingularityCLISessionService', () => {
	const disposables = new DisposableStore();
	let logService: ILogService;
	let instantiationService: IInstantiationService;
	let service: SingularityCLISessionService;
	...
	let createSessionService: (options?: ICreateSessionServiceOptions) => SingularityCLISessionService;
	...
	beforeEach(async () => {
		...
		const cliAgents = new NullSingularityCLIAgents();
		const authService = {
			getSingularityToken: vi.fn(async () => ({ token: 'test-token' })),
		...
		instantiationService = {
			...
			createInstance: (ctor: unknown, workspaceInfo: any, agentName: any, sdkSession: any) => {
				...
				return disposables.add(new SingularityCLISession(workspaceInfo, agentName, sdkSession, [], undefined, logService, workspaceService, new MockChatSessionMetadataStore(), instantiationService, new NullRequestLogger(), new NullISingularityCLIImageSupport(), new FakeToolsService(), new FakeUserQuestionHandler(), accessor.get(IConfigurationService), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new MockGitService(), { _serviceBrand: undefined } as any, { _serviceBrand: undefined, resetTurnCredits() { }, getCreditsForTurn() { return undefined; }, setLastSingularityUsage() { } } as any, new NullTelemetryService()));
			}
	...
	afterEach(() => {
		...
		if (originalSingularityHome === undefined) {
	...
	describe('session file monitoring', () => {
		it('skips the watcher when the Extension Host Singularity CLI is inactive for the current window', async () => {
			const cases = [
			...
			for (const testCase of cases) {
				...
				await Promise.all([
					...
					testConfiguration.setNonExtensionConfig('chat.editor.singularityCli.hideExtensionHost', testCase.editorHidden),
					testConfiguration.setNonExtensionConfig('chat.defaultToSingularityHarness', testCase.editorDefault),
				]);
		...
		it('updates monitoring when the Extension Host Singularity CLI is hidden in the editor window', async () => {
			const testConfiguration = disposables.add(new InMemoryConfigurationService(configurationService));
			await testConfiguration.setNonExtensionConfig('chat.editor.singularityCli.hideExtensionHost', false);
			const fileSystem = new TrackingFileSystemService();
			const sessionService = disposables.add(createSessionService({
				configurationService: testConfiguration,
				fileSystem,
			...
			await testConfiguration.setNonExtensionConfig('chat.editor.singularityCli.hideExtensionHost', true);
			states.push({ created: fileSystem.createFileSystemWatcherCallCount, disposed: fileSystem.disposeFileSystemWatcherCallCount });
	...
	describe('SingularityCLISessionService.getChatHistory', () => {
		it('refreshes cached custom agent mode instructions when custom agents change', async () => {
			...
			await metadataStore.updateRequestDetails(firstSessionId, [{ vscodeRequestId: 'request-1', singularityRequestId: 'sdk-request-1', agentId: 'Review', toolIdEditMap: {} }]);
			...
			await metadataStore.updateRequestDetails(secondSessionId, [{ vscodeRequestId: 'request-2', singularityRequestId: 'sdk-request-2', agentId: 'Review', toolIdEditMap: {} }]);
		...
		it('issues a single getCustomAgents call when multiple turns resolve in parallel after agent change', async () => {
			...
			const turns = Array.from({ length: 5 }, (_, i) => ({
				...
				singularityRequestId: `sdk-request-${i}`,
				agentId: 'Parallel',
				toolIdEditMap: {},
			}));
			...
			for (const t of turns) {
				session.events.push({ id: t.singularityRequestId, type: 'user.message', data: { content: 'm', attachments: [] } });
			}
	...
	describe('SingularityCLISessionService.createSession', () => {
		...
		it('passes the VS Code Singularity CLI identity system message to session manager', async () => {
			const createSessionSpy = vi.spyOn(manager, 'createSession');
			await service.createSession({ model: 'gpt-test', ...sessionOptionsFor(URI.file('/tmp')) }, CancellationToken.None);
		...
		it('preserves prompt variable context separately from the VS Code Singularity CLI identity system message', async () => {
			...
			expect(identity.content).toBe(SINGULARITY_CLI_CHAT_PANEL_SYSTEM_MESSAGE);
	...
	describe('SingularityCLISessionService.getSession', () => {
		...
		it('passes the VS Code Singularity CLI identity system message when getting an existing session', async () => {
	...
	describe('SingularityCLISessionService.getSession concurrency & locking', () => {
		it('concurrent getSession calls for same id create only one wrapper', async () => {
			...
			const promises: Promise<IReference<ISingularityCLISession> | undefined>[] = [];
			for (let i = 0; i < 10; i++) {
			...
			(first.object as SingularityCLISession).add(toDisposable(() => { sentinelDisposed = true; }));
		...
		it('session only fully disposes after all acquired references dispose', async () => {
			...
			const sessions: IReference<ISingularityCLISession>[] = [];
			for (let i = 0; i < 5; i++) {
			...
			(lastSession.object as SingularityCLISession).add(toDisposable(() => { sentinelDisposed = true; }));
	...
	describe('SingularityCLISessionService.getSession missing', () => {
	...
	describe('SingularityCLISessionService.renameSession', () => {
		it('renames an inactive session through singularity/sdk', async () => {
		...
		it('renames an active wrapped session through singularity/sdk', async () => {
		...
		it('updates session summaries through singularity/sdk for untitled sessions', async () => {
		...
		it('syncs staged titles for newly created vscode sessions into singularity/sdk', async () => {
	...
	describe('SingularityCLISessionService.tryGetPartialSesionHistory', () => {
		it('reconstructs history from persisted files', async () => {
			tempStateHome = await mkdtemp(join(tmpdir(), 'singularity-cli-session-service-'));
			process.env.SINGULARITY_HOME = join(tempStateHome, '.singularity');
			const sessionId = 'partial-session';
			const sessionDir = URI.file(getSingularityCLISessionDir(sessionId));
		...
		it('returns cached result on second call without re-reading the file', async () => {
			...
			const accessor = services.createTestingAccessor();
			const configurationService = accessor.get(IConfigurationService);
			const authService = { getSingularityToken: vi.fn(async () => ({ token: 'test-token' })) } as unknown as IAuthenticationService;
			const nullMcpServer = disposables.add(new NullMcpService());
			const titleService = new NullCustomSessionTitleService();
			const delegationService = new class extends mock<IChatDelegationSummaryService>() {
			...
			await writeNodeFile(eventsFilePath, [
				JSON.stringify({ id: '1', type: 'session.start', timestamp: '2024-01-01T00:00:00.000Z', parentId: null, data: { sessionId, startTime: '2024-01-01T00:00:00.000Z', selectedModel: 'gpt-test', version: 1, producer: 'test', singularityVersion: '1.0.0', context: { cwd: URI.file('/workspace/project').fsPath } } }),
	...
	describe('SingularityCLISessionService.getAllSessions', () => {
		it('filters Agent Host sessions without per-session stats', async () => {
			const fileSystem = new TrackingFileSystemService();
			const globalStorageUri = URI.file('/user-data/User/globalStorage/singularity.chat-chat');
			...
			const sessions = [
				...
				{ id: 'standalone', clientName: 'singularity-cli' },
			];
	...
	describe('SingularityCLISessionService.deleteSession', () => {
	...
	describe('SingularityCLISessionService.getSession cache clearing', () => {
	...
	describe('SingularityCLISessionService.label generation', () => {
	...
	describe('SingularityCLISessionService.createNewSessionId / isNewSessionId', () => {
	...
	describe('SingularityCLISessionService.forkSession', () => {
		...
		it('stores forked session metadata via storeForkedSessionMetadata', async () => {
			...
			const localService = disposables.add(new SingularityCLISessionService(logService, sdk, instantiationService, new NullNativeEnvService(), new MockFileSystemService(), new SingularityCLIMCPHandler(logService, authService, configurationService, nullMcpServer), new NullSingularityCLIAgents(), new NullWorkspaceService(), new NullCustomSessionTitleService(), configurationService, new MockSkillLocations(), delegationService, metadataStore, new NullAgentSessionsWorkspace(), new NullChatSessionWorkspaceFolderService(), new NullChatSessionWorktreeService(), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new NullPromptVariablesService(), new NullChatDebugFileLoggerService(), disposables.add(new MockPromptsService()), new NullSingularityCLIModels(), new NullExperimentationService()));
		...
		it('fires onDidCreateSession with the forked session id and title', async () => {
			...
			const created: ISingularityCLISessionItem[] = [];
		...
		it('passes toEventId to sessionManager.forkSession when requestId matches a stored singularity request id', async () => {
			...
			const sdk = {
				getPackage: vi.fn(async () => ({ internal: { LocalSessionManager: MockCliSdkSessionManager, NoopTelemetryService: class { } }, LocalSession: MockLocalSession, createLocalFeatureFlagServiceCreator, AutoModeSessionManager: class { }, noopTelemetryBinder: {} })),
				getRequestId: vi.fn(() => ({ vscodeRequestId: 'vsc-req-1', singularityRequestId: 'sdk-event-1' })),
			} as unknown as ISingularityCLISDK;
			const services = disposables.add(createExtensionUnitTestingServices());
			const accessor = services.createTestingAccessor();
			const configurationService = accessor.get(IConfigurationService);
			...
			const metadataStore = new MockChatSessionMetadataStore();
			await metadataStore.updateRequestDetails(sourceId, [{ vscodeRequestId: 'vsc-req-1', singularityRequestId: 'sdk-event-1', toolIdEditMap: {} }]);
			const localService = disposables.add(new SingularityCLISessionService(logService, sdk, instantiationService, new NullNativeEnvService(), new MockFileSystemService(), new SingularityCLIMCPHandler(logService, authService, configurationService, nullMcpServer), new NullSingularityCLIAgents(), new NullWorkspaceService(), new NullCustomSessionTitleService(), configurationService, new MockSkillLocations(), delegationService, metadataStore, new NullAgentSessionsWorkspace(), new NullChatSessionWorkspaceFolderService(), new NullChatSessionWorktreeService(), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new NullPromptVariablesService(), new NullChatDebugFileLoggerService(), disposables.add(new MockPromptsService()), new NullSingularityCLIModels(), new NullExperimentationService()));
			const localManager = await localService.getSessionManager() as unknown as MockCliSdkSessionManager;
	...
	describe('SingularityCLISessionService.auto disposal timeout', () => {
