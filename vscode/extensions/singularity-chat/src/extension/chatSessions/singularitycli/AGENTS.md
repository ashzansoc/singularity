# Singularity CLI Integration

This folder contains the Singularity CLI integration for VS Code Chat. It enables users to open a new Chat window and interact with a Singularity CLI agent instance directly within VS Code. **VS Code provides the UI, Singularity CLI SDK provides the smarts.**

> **Important:** The Singularity CLI agent functionality is powered by the `@github/copilot/sdk` package. See the SDK package for full type definitions.
...
└─────────────────────────┬───────────────────────────────────────┘
                          ...
                          ▼
┌─────────────────────────────────────────────────────────────────┐
│                   SingularityCLISessionService                       │
│  (node/singularitycliSessionService.ts)                              │
│  - Manages SDK LocalSessionManager lifecycle                     │
│  - Creates, retrieves, and caches SingularityCLISession instances    │
...
└─────────────────────────┬───────────────────────────────────────┘
                          ...
                          ▼
┌─────────────────────────────────────────────────────────────────┐
│                      SingularityCLISession                           │
│  (node/singularitycliSession.ts)                                     │
...
└─────────────────────────┬───────────────────────────────────────┘
                          ...
                          ▼
┌─────────────────────────────────────────────────────────────────┐
│                 Singularity CLI SDK (@github/copilot/sdk)            │
...
│  - Discoverable by CLI via lock files in ~/.singularity/ide/          │
└─────────────────────────────────────────────────────────────────┘
```
...
│   ├── singularityCLITools.ts      # Tool type definitions and processing helpers
│   ├── singularityCLIPrompt.ts     # Prompt reference extraction and parsing
│   ├── customSessionTitleService.ts
│   ├── delegationSummaryService.ts
│   ├── utils.ts                # SessionIdForCLI namespace (URI scheme: 'singularitycli')
│   └── test/
│
├── node/                       # Node.js-specific (SDK integration, filesystem, permissions)
│   ├── singularityCli.ts           # ISingularityCLISDK, SingularityCLIModels, SingularityCLIAgents
│   ├── singularitycliSession.ts    # SingularityCLISession — main session wrapper
│   ├── singularitycliSessionService.ts  # Session lifecycle management
│   ├── permissionHelpers.ts    # Permission request handlers
│   ├── singularitycliPromptResolver.ts  # Resolves prompts with variables and attachments
│   ├── singularityCLISkills.ts     # Skills location resolution
│   ├── singularityCLIImageSupport.ts    # Image attachment handling
...
└── vscode-node/                # VS Code API-dependent (commands, MCP tools, UI)
    ├── singularityCLIFolderMru.ts  # Folder MRU (most-recently-used) service
    └── test/
```
...
## Key Components
### `node/singularityCli.ts`

**ISingularityCLISDK / SingularityCLISDK**
- Service interface wrapping the dynamic `import('@github/copilot/sdk')` for dependency injection and testability

**ISingularityCLIModels / SingularityCLIModels**
- Fetches and caches available AI models from the SDK via `getAvailableModels()`
- Registers a `LanguageModelChatProvider` with `targetChatSessionType: 'singularitycli'` so VS Code's model picker shows CLI models
...
**ISingularityCLIAgents / SingularityCLIAgents**
...
**ISingularityCLISessionService / SingularityCLISessionService**
- Central service managing the lifecycle of all Singularity CLI sessions

### `common/singularityCLITools.ts`

Defines all tool type interfaces used by the Singularity CLI agent:
...
**ISingularityCLIMCPHandler / SingularityCLIMCPHandler**
...
### `node/singularityCLIImageSupport.ts`

**ISingularityCLIImageSupport / SingularityCLIImageSupport**
- Stores image data as files in extension global storage (`singularity-cli-images/`)
...
Path helpers for Singularity CLI directories.

## Message Flow

1. **User sends message** in VS Code Chat
2. **SingularityCLISessionService** creates or retrieves an existing session wrapper
3. **SingularityCLISession.handleRequest()** is called:
...
Singularity CLI sessions are persisted to `~/.singularity/session-state/<sessionId>/` directories containing:
...
2. Add a `case '<kind>':` in the permission switch in `node/singularitycliSession.ts` (~line 468)
...
1. Add a listener in `node/singularitycliSession.ts` using `this._sdkSession.on(eventName, handler)`
...
**Singularity CLI  commands** (user-facing, sent programmatically):
...
**VS Code Session commands** (registered via `registerCLIChatCommands` in `vscode-node/singularityCLIChatSessions.ts`):

## Configuration

The integration respects these VS Code settings (all under `singularity.chat.chat.cli.*`):
...
- `@github/copilot/sdk`: Official Singularity CLI SDK (session management, tools, permissions, events)

## Deprecated Code

V1 registration in `../vscode-node/singularityCLIChatSessionsContribution.ts` and `registerSingularityCLIServicesV1` are deprecated. All new development should use `SingularityCLISessionService` and the controller-based V2 API.
