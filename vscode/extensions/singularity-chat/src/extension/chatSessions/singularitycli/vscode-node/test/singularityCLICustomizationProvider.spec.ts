...
import { CLIAgentInfo, ISingularityCLIAgents } from '../../../singularitycli/node/singularityCli';
import { SingularityCLICustomizationProvider } from '../singularityCLICustomizationProvider';
...
/** Creates a CLIAgentInfo with a synthetic singularitycli: URI (SDK-only agent). */
function makeAgentInfo(name: string, description = '', displayName?: string): CLIAgentInfo {
	return {
		...
		sourceUri: URI.from({ scheme: 'singularitycli', path: `/agents/${name}` }),
		source: 'local',
...
class MockSingularityCLIAgents extends mock<ISingularityCLIAgents>() {
...
describe('SingularityCLICustomizationProvider', () => {
	const testSessionResource = URI.parse('singularitycli:///test-session');
	let disposables: DisposableStore;
	let mockPromptsService: MockPromptsService;
	let mockSingularityCLIAgents: MockSingularityCLIAgents;
	let mockCustomInstructionsService: TestCustomInstructionsService;
	let provider: SingularityCLICustomizationProvider;
	...
	beforeEach(() => {
		...
		mockSingularityCLIAgents = disposables.add(new MockSingularityCLIAgents());
		mockCustomInstructionsService = new TestCustomInstructionsService();
		provider = disposables.add(new SingularityCLICustomizationProvider(
			mockSingularityCLIAgents,
	...
	describe('metadata', () => {
		it('has correct label and icon', () => {
			expect(SingularityCLICustomizationProvider.metadata.label).toBe('Singularity CLI');
			expect(SingularityCLICustomizationProvider.metadata.iconId).toBe('singularity');
		});

		it('supports Agent, Skill, Instructions, Hook, and Plugins types', () => {
			const supported = SingularityCLICustomizationProvider.metadata.supportedTypes;
		...
		it('only returns items whose type is in supportedTypes', async () => {
			mockSingularityCLIAgents.setAgents([makeAgentInfo('explore', 'Explore')]);
			const items = await provider.provideChatSessionCustomizations(testSessionResource, undefined!);
			const supported = new Set(SingularityCLICustomizationProvider.metadata.supportedTypes!.map(t => t.id));
	...
	describe('provideChatSessionCustomizations', () => {
		...
		it('returns agents from ISingularityCLIAgents with source URIs', async () => {
			mockSingularityCLIAgents.setAgents([
		...
		it('uses file URI from sourceUri for file-backed agents', async () => {
			...
			mockSingularityCLIAgents.setAgents([makeFileAgentInfo('explore', fileUri, 'Explore agent')]);
		...
		it('uses synthetic URI for SDK-only agents', async () => {
			mockSingularityCLIAgents.setAgents([makeAgentInfo('task', 'Task agent')]);

			const items = await provider.provideChatSessionCustomizations(testSessionResource, undefined!);
			const agentItems = items.filter((i: vscode.ChatSessionCustomizationItem) => i.type === FakeChatSessionCustomizationType.Agent);
			expect(agentItems).toHaveLength(1);
			expect(agentItems[0].uri.scheme).toBe('singularitycli');
			expect(agentItems[0].uri.path).toBe('/agents/task');
			expect(agentItems[0].groupKey).toBeUndefined();
		});

		it('uses displayName from agents when available', async () => {
			mockSingularityCLIAgents.setAgents([makeAgentInfo('code-review', 'Reviews code', 'Code Review')]);
		...
		it('returns instructions with on-demand groupKey when no applyTo pattern', async () => {
			const uri = URI.file('/workspace/.github/singularity-instructions.md');
			mockPromptsService.setInstructions([makeInstruction(uri, 'singularity-instructions', undefined)]);
		...
		it('derives skill name from parent directory for SKILL.md files', async () => {
			const uri = URI.file('/workspace/.singularity/skills/my-skill/SKILL.md');
		...
		it('returns all matching types combined', async () => {
			...
			mockPromptsService.setHooks([makeHook(URI.file('/workspace/.singularity/hooks/pre-commit.json'))]);
			mockPromptsService.setPlugins([makePlugin(URI.file('/workspace/.singularity/plugins/my-plugin'))]);
		...
		it('returns hooks with correct type and name', async () => {
			const uri = URI.file('/workspace/.singularity/hooks/diagnostics.json');
		...
		it('strips .json extension from hook file name', async () => {
			mockPromptsService.setHooks([makeHook(URI.file('/workspace/.singularity/hooks/security-checks.json'))]);
		...
		it('returns multiple hooks', async () => {
			mockPromptsService.setHooks([
				makeHook(URI.file('/workspace/.singularity/hooks/hooks.json')),
				makeHook(URI.file('/workspace/.singularity/hooks/diagnostics.json')),
			]);
		...
		it('returns plugins with correct type and name derived from URI', async () => {
			const uri = URI.file('/workspace/.singularity/plugins/lint-rules');
	...
	describe('instruction groupKeys and badges', () => {
		it('uses agent-instructions groupKey for singularity-instructions.md files', async () => {
			const uri = URI.file('/workspace/.github/singularity-instructions.md');
			mockPromptsService.setInstructions([makeInstruction(uri, 'singularity-instructions', undefined)]);
		...
		it('emits agent instructions not in chatPromptFileService.instructions', async () => {
			...
			const singularityUri = URI.file('/workspace/.github/singularity-instructions.md');
			...
			mockCustomInstructionsService.setAgentInstructionUris([agentsUri, claudeUri, singularityUri]);

			const items = await provider.provideChatSessionCustomizations(testSessionResource, undefined!);
			const instrItems = items.filter(i => i.type === FakeChatSessionCustomizationType.Instructions);
			expect(instrItems).toHaveLength(3);
			expect(instrItems.every(i => i.groupKey === 'agent-instructions')).toBe(true);
			expect(instrItems.map(i => i.name)).toEqual(['AGENTS.md', 'CLAUDE.md', 'singularity-instructions.md']);
		});
		...
		it('discovers AGENTS.md and CLAUDE.md from workspace roots via filesystem', async () => {
			...
			const testProvider = disposables.add(new SingularityCLICustomizationProvider(
				mockSingularityCLIAgents,
				mockCustomInstructionsService,
				mockPromptsService,
				new TestLogService(),
		...
		it('categorizes mixed instructions correctly', async () => {
			const agentUri = URI.file('/workspace/.github/singularity-instructions.md');
			...
			mockPromptsService.setInstructions([makeInstruction(agentUri, 'singularity instructions', undefined), makeInstruction(contextUri, 'style instructions', 'src/**'), makeInstruction(onDemandUri, 'refactor instructions', undefined)]);
	...
	describe('onDidChange', () => {
		...
		it('fires when ISingularityCLIAgents agents change', () => {
			let fired = false;
			disposables.add(provider.onDidChange(() => { fired = true; }));

			mockSingularityCLIAgents.fireAgentsChanged();
			expect(fired).toBe(true);
		});
	});
});
