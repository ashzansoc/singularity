...
import { copyRipgrepShim } from '../../../singularitycli/node/ripgrepShim';

describe('SingularityCLI SDK Upgrade', function () {
	const extensionPath = path.join(__dirname, '..', '..', '..', '..', '..', '..');
	const singularitySDKPath = path.join(extensionPath, 'node_modules', '@github', 'singularity');
	...
	it('should not contain new native binaries nor removed native binaries', async function () {
		// This is a very basic check to ensure that when the Singularity CLI SDK is upgraded,
		...
		const existingBinaries = new Set(await findAllBinaries(singularitySDKPath));
		...
		].map(p => path.join(singularitySDKPath, p)));

		// Exclude ripgrep files that we copy over in src/extension/chatSessions/singularitycli/node/ripgrepShim.ts (until we get better API/solution from SDK)
		const ripgrepFilesWeCopy = path.join(singularitySDKPath, 'sdk', 'ripgrep', 'bin');
		...
		for (const binary of existingBinaries) {
			...
			if (!knownBinaries.has(binary)) {
				errors.push(`Unexpected native binary found in Singularity CLI SDK: ${path.relative(singularitySDKPath, binary)}`);
			}
		}
		...
		for (const binary of knownBinaries) {
			...
			if (!existingBinaries.has(binary)) {
				if (isNonCurrentSingularityPlatformBinary(singularitySDKPath, binary) || isNonCurrentPvRecorderBinary(singularitySDKPath, binary)) {
					continue;
				}
				errors.push(`Expected native binary missing from Singularity CLI SDK: ${path.relative(singularitySDKPath, binary)}`);
			}
		}
...
const singularityPlatformArchs = new Set([
	'darwin-arm64',
	'darwin-x64',
	'linux-arm64',
	'linux-x64',
...
function currentSingularityPlatformArch(): string {
...
function isNonCurrentSingularityPlatformBinary(singularitySDKPath: string, binary: string): boolean {
	const relativeSegments = path.relative(singularitySDKPath, binary).split(path.sep);
	const platformArch = relativeSegments.find(segment => singularityPlatformArchs.has(segment));
	return platformArch !== undefined && platformArch !== currentSingularityPlatformArch();
}

function isNonCurrentPvRecorderBinary(singularitySDKPath: string, binary: string): boolean {
	const relative = path.relative(singularitySDKPath, binary).split(path.sep).join(path.posix.sep);
	const pvRecorderPrefix = 'pvrecorder/node_modules/@picovoice/pvrecorder-node/lib/';
	if (!relative.startsWith(pvRecorderPrefix)) {
		return false;
	}

	const currentPlatformArch = currentSingularityPlatformArch();
...
async function copyBinaries(extensionPath: string) {
	const singularitySDKPath = path.join(extensionPath, 'node_modules', '@github', 'singularity');
	const vscodeRipgrepPath = path.join(singularitySDKPath, 'ripgrep', 'bin', process.platform + '-' + process.arch);
