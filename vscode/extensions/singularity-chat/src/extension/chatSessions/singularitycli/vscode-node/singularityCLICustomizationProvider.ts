...
import { ISingularityCLIAgents, isEnabledForSingularityCLI } from '../../singularitycli/node/singularityCli';
import { INativeEnvService } from '../../../../platform/env/common/envService';

export class SingularityCLICustomizationProvider extends Disposable implements vscode.ChatSessionCustomizationProvider {
	...
	constructor(
		...
		this._register(this.singularityCLIAgents.onDidChangeAgents(() => this._onDidChange.fire()));
	}
	...
	async provideChatSessionCustomizations(_sessionResource: vscode.Uri, token: vscode.CancellationToken): Promise<vscode.ChatSessionCustomizationItem[]> {
		const [agents, instructions, skills, hooks, plugins] = await Promise.all([
			...
			this.logService.error(`[SingularityCLICustomizationProvider] failed to get customizations: ${err}`);
			return [];
		})));

		this.logService.debug(`[SingularityCLICustomizationProvider] agents (${agents.length}): ${agents.map(a => a.name).join(', ') || '(none)'}`);
		this.logService.debug(`[SingularityCLICustomizationProvider] instructions (${instructions.length}): ${instructions.map(i => i.name).join(', ') || '(none)'}`);
		this.logService.debug(`[SingularityCLICustomizationProvider] skills (${skills.length}): ${skills.map(s => s.name).join(', ') || '(none)'}`);
		this.logService.debug(`[SingularityCLICustomizationProvider] hooks (${hooks.length}): ${hooks.map(h => h.name).join(', ') || '(none)'}`);

		this.logService.debug(`[SingularityCLICustomizationProvider] plugins (${plugins.length}): ${plugins.map(p => p.name).join(', ') || '(none)'}`);

		const items = [...agents, ...instructions, ...skills, ...hooks, ...plugins];
		this.logService.debug(`[SingularityCLICustomizationProvider] total: ${items.length} items`);
		return items;
	}
	...
	/**
	 * Builds agent items from ISingularityCLIAgents, which already merges SDK
	...
	private async getAgentItems(_token: vscode.CancellationToken): Promise<vscode.ChatSessionCustomizationItem[]> {
		const agentInfos = await this.singularityCLIAgents.getAgents();
	 ...
	 * implementation:
	 * - agent-instructions: AGENTS.md, CLAUDE.md, singularity-instructions.md
	...
	private async getInstructionItems(token: CancellationToken): Promise<vscode.ChatSessionCustomizationItem[]> {
		...
		// (singularity-instructions.md) plus workspace-root AGENTS.md and CLAUDE.md
		...
		// Emit agent instruction files (AGENTS.md, CLAUDE.md, singularity-instructions.md)
		...
		for (const instruction of await this.promptsService.getInstructions(token)) {
			...
			if (!isEnabledForSingularityCLI(instruction)) {
				continue; // only include instructions that are relevant for singularitycli
			}
	...
	private async getSkillItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionCustomizationItem[]> {
		return (await this.promptsService.getSkills(token)).filter(isEnabledForSingularityCLI).map(s => ({
			uri: s.uri,
	...
	private async getHookItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionCustomizationItem[]> {
		return (await this.promptsService.getHooks(token)).filter(isEnabledForSingularityCLI).map(h => ({
			uri: h.uri,
	...
	private async getPluginItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionCustomizationItem[]> {
		return (await this.promptsService.getPlugins(token)).filter(isEnabledForSingularityCLI).map(p => ({
			uri: p.uri,
...
function getSearchRoots() {
	return {
		...
		user: [
			{ path: ['.singularity', 'agents'], type: vscode.ChatSessionCustomizationType.Agent },
			{ path: ['.agents', 'skills'], recursive: true, type: vscode.ChatSessionCustomizationType.Skill },
			{ path: ['.singularity', 'instructions'], recursive: true, type: vscode.ChatSessionCustomizationType.Instructions },
			{ path: ['.singularity', 'hooks'], recursive: true, type: vscode.ChatSessionCustomizationType.Hook },
		],
	};
}
