...
export class InProcHttpServer extends Disposable {
	...
	constructor(
		...
		private readonly _sessionTracker: ISingularityCLISessionTracker,
	) {
		super();
	}
	...
	private async _handlePost(mcpOptions: McpProviderOptions, req: express.Request, res: express.Response): Promise<void> {
		const sessionId = req.headers['mcp-session-id'] ?? req.headers['x-singularity-session-id'];
		...
		if (sessionId && existingTransport) {
			...
			const clientPid = parseInt(req.headers['x-singularity-pid'] as string, 10);
			const clientPpid = parseInt(req.headers['x-singularity-parent-pid'] as string, 10);
