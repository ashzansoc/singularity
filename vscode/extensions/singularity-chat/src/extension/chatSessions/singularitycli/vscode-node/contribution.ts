...
import { SingularityCLISessionTracker, ISingularityCLISessionTracker } from './singularityCLISessionTracker';
...
export function getServices(): ConstructorParameters<typeof ServiceCollection> {
	return [
		[ISingularityCLISessionTracker, new SingularityCLISessionTracker()]
	];
}
export class SingularityCLIContrib extends Disposable {
	...
	constructor(
		...
		const logger = this.logService.createSubLogger('SingularityCLI');
	...
	private async _startMcpServer(logger: ILogger, httpServer: InProcHttpServer, diffState: DiffStateManager, selectionState: SelectionState, contentProvider: ReadonlyContentProvider): Promise<void> {
		try {
			const { serverUri, headers } = await httpServer.start({
				id: 'vscode-singularity-cli',
				serverLabel: 'VS Code Singularity CLI',
				serverVersion: '0.0.1',
