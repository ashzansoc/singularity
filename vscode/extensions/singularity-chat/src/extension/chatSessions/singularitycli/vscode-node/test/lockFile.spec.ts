...
describe('createLockFile', () => {
	...
	beforeEach(() => {
		originalEnv = process.env.SINGULARITY_HOME;
		process.env.SINGULARITY_HOME = path.join(testDir, '.singularity');
	});
	...
	afterEach(async () => {
		...
		if (originalEnv !== undefined) {
			process.env.SINGULARITY_HOME = originalEnv;
		} else {
	...
	it('should create lock file in .singularity directory', async () => {
		...
		expect(handle.path).toMatch(/\.singularity[/\\]ide.*\.lock$/);
	...
	it('should create .singularity directory if it does not exist', async () => {
		const mockServerUri = { path: '/tmp/server.sock', scheme: 'http' } as any;
		const handle = await createLockFile(mockServerUri, {}, logger);
		createdLockFile = handle.path;

		const singularityDir = path.dirname(handle.path);
		const stat = await fs.stat(singularityDir);
...
describe('cleanupStaleLockFiles', () => {
	...
	const singularityDir = path.join(testDir, '.singularity', 'ide');
	let originalEnv: string | undefined;

	beforeEach(async () => {
		originalEnv = process.env.SINGULARITY_HOME;
		process.env.SINGULARITY_HOME = path.join(testDir, '.singularity');
		await fs.mkdir(singularityDir, { recursive: true });
	});
	...
	it('should remove lockfiles for non-running processes', async () => {
		const staleLockFile = path.join(singularityDir, 'stale.lock');
		const staleLockInfo = {
	...
	it('should keep lockfiles for running processes', async () => {
		const activeLockFile = path.join(singularityDir, 'active.lock');
	...
	it('should return 0 when singularity directory does not exist', async () => {
		await fs.rm(singularityDir, { recursive: true, force: true });
