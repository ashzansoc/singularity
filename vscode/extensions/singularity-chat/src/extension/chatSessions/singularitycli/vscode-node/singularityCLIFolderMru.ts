...
export class SingularityCLIFolderMruService implements IChatFolderMruService {
	...
	async getRecentlyUsedFoldersImpl(token: CancellationToken): Promise<FolderRepositoryMRUEntry[]> {
		...
		for (const session of (sessions ?? [])) {
			...
			if (session.workingDirectory.path.includes('.worktrees/singularity-')) {
				continue;
			}
		...
		for (const repo of this.gitService.getRecentRepositories()) {
			if (repo.rootUri.path.includes('.worktrees/singularity-')) {
				continue;
			}
