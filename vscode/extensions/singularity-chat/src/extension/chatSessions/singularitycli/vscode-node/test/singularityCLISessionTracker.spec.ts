...
import { SingularityCLISessionTracker, getParentPid } from '../singularityCLISessionTracker';
...
describe('SingularityCLISessionTracker', () => {
	let tracker: SingularityCLISessionTracker;

	beforeEach(() => {
		tracker?.dispose();
		tracker = new SingularityCLISessionTracker();
		mockTerminals.value = [];
		mockIsWindows.value = false;
	...
	describe('setSessionName and getSessionDisplayName', () => {
		it('should return sessionId when no name is set', () => {
			tracker.registerSession('session-1', { pid: 1234, ppid: 5678 });
			expect(tracker.getSessionDisplayName('session-1')).toBe('Singularity CLI Session');
		});
