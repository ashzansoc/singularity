...
import { SingularityCLIModelInfo, ISingularityCLIAgents, ISingularityCLIModels } from '../../../singularitycli/node/singularityCli';
...
import { SingularityCLIChatSessionInitializer } from '../singularityCLIChatSessionInitializer';
...
class TestSessionService extends mock<ISingularityCLISessionService>() {
	...
	override createSession = vi.fn(async (): Promise<IReference<ISingularityCLISession>> => ({
		object: makeSessionObject(),
		dispose: vi.fn(),
	}));
	override getSession = vi.fn(async (): Promise<IReference<ISingularityCLISession> | undefined> => ({
		object: makeSessionObject(),
		dispose: vi.fn(),
	}));
}
...
class TestModels extends mock<ISingularityCLIModels>() {
	declare readonly _serviceBrand: undefined;
	...
	override getModels = vi.fn(async (): Promise<SingularityCLIModelInfo[]> => []);
}

class TestAgents extends mock<ISingularityCLIAgents>() {
	declare readonly _serviceBrand: undefined;
...
function makeSessionObject(overrides?: Partial<ISingularityCLISession>): ISingularityCLISession {
	return {
...
function makeChatResource(sessionId: string = 'untitled:new-session'): vscode.Uri {
	return URI.from({ scheme: 'singularitycli', path: `/${sessionId}` }) as unknown as vscode.Uri;
}
...
function createInitializer(overrides?: {
	...
	const initializer = new SingularityCLIChatSessionInitializer(
		sessionService,
		folderRepoManager,
		workspaceService,
		models,
