...
import { getSingularityCliStateDir } from '../node/cliHelpers';
...
export async function createLockFile(serverUri: vscode.Uri, headers: Record<string, string>, logger: ILogger): Promise<LockFileHandle> {
	const singularityDir = getSingularityCliStateDir();
	logger.trace(`Creating lock file in: ${singularityDir}`);

	await fs.mkdir(singularityDir, { recursive: true, mode: 0o700 });

	const uuid = generateUuid();
	const lockFilePath = path.join(singularityDir, `${uuid}.lock`);
...
export async function cleanupStaleLockFiles(logger: ILogger): Promise<number> {
	...
	let files: string[];
	try {
		files = await fs.readdir(singularityDir);
	} catch {
		return 0;
	}
	...
	const results = await Promise.all(lockFiles.map(async (file) => {
		const filePath = path.join(singularityDir, file);
		try {
