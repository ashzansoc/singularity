...
import { getSingularityBulkMetadataFile, getSingularityCLISessionDir } from '../../singularitycli/node/cliHelpers';
import { ISingularityCLIAgents } from '../../singularitycli/node/singularityCli';

// const WORKSPACE_FOLDER_MEMENTO_KEY = 'singularity.chat.cli.sessionWorkspaceFolders';
// const WORKTREE_MEMENTO_KEY = 'singularity.chat.cli.sessionWorktrees';
const LEGACY_BULK_METADATA_FILENAME = 'singularitycli.session.metadata.json';
const LEGACY_BULK_MIGRATED_KEY = 'singularity.chat.cli.legacyBulkMigrated';
 ...
 * (`~/.singularity/vscode.session.metadata.cache.json`). Older entries (by `modified`)
 * are evicted from the file but remain available via the per-session metadata files
 * (`~/.singularity/session-state/{id}/vscode.metadata.json`) and the JSONL worktree index.
 */
...
export class ChatSessionMetadataStore extends Disposable implements IChatSessionMetadataStore {
	...
	/** Path of the shared bulk metadata cache file in `~/.singularity/`. */
	private readonly _cacheFile = Uri.file(getSingularityBulkMetadataFile());
	...
	private async initializeStorage(): Promise<void> {
		...
		// globalStorageUri to the shared `~/.singularity/` location.
	...
	public getMetadataFileUri(sessionId: string): vscode.Uri {
		return Uri.joinPath(Uri.file(getSingularityCLISessionDir(sessionId)), 'vscode.metadata.json');
	}

	private getRequestMappingFileUri(sessionId: string): vscode.Uri {
		return Uri.joinPath(Uri.file(getSingularityCLISessionDir(sessionId)), REQUEST_MAPPING_FILENAME);
	}
	...
	async getSessionAgent(sessionId: string): Promise<string | undefined> {
		...
		return findLast(details, d => !!d.modeInstructions)?.modeInstructions?.uri ?? findLast(details, d => !!d.agentId)?.agentId ?? this.singularityCLIAgents.getSessionAgent(sessionId);
	}
	...
	private async writeToGlobalStorage(allMetadata: Record<string, ChatSessionMetadataFile>): Promise<void> {
		...
		// Per-session files in `~/.singularity/session-state/{id}/vscode.metadata.json` remain
	...
	/**
	 * Merges the per-install legacy bulk file (`globalStorageUri/singularitycli/…`) into
	 * the shared `~/.singularity/` bulk file using last-modified-wins. This handles:
	...
	private async migrateLegacyBulkFile(): Promise<void> {
		...
		const legacyCacheFile = Uri.joinPath(this.extensionContext.globalStorageUri, 'singularitycli', LEGACY_BULK_METADATA_FILENAME);
		...
		try {
			...
			await this.extensionContext.globalState.update(LEGACY_BULK_MIGRATED_KEY, true);
			this.logService.info('[ChatSessionMetadataStore] Migrated legacy bulk metadata file to ~/.singularity/');
		} catch (err) {
