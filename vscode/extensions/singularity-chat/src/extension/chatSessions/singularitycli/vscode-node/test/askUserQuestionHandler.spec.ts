...
import { IQuestion } from '../../../singularitycli/node/userInputHelpers';
import { IAnswerResult, UserQuestionHandler } from '../askUserQuestionHandler';

function makeAskQuestionsTool(invokeResult: LanguageModelToolResult | undefined, resolveInput?: unknown): ISingularityTool<unknown> {
	return {
		invoke: vi.fn(async () => invokeResult),
		resolveInput,
	} as unknown as ISingularityTool<unknown>;
}

function makeToolsService(tool: ISingularityTool<unknown> | undefined): IToolsService {
...
function makeHandler(tool: ISingularityTool<unknown> | undefined) {
