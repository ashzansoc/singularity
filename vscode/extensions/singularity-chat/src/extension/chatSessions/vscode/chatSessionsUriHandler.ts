...
import { getRepoId } from './singularityCodingAgentUtils';

export const GHPR_EXTENSION_ID = 'GitHub.vscode-pull-request-github';
const PENDING_CHAT_SESSION_STORAGE_KEY = 'singularity.chat.pendingChatSession';
...
export class ChatSessionsUriHandler extends Disposable implements CustomUriHandler {
			...
			case UriHandlerPaths.OpenSession:
				{
					...
					/* __GDPR__
						"singularity.codingAgent.deeplink" : {
							"owner": "rebornix",
					...
					*/
					this._telemetryService.sendTelemetryEvent('singularity.codingAgent.deeplink', { microsoft: true, github: false }, {
						sessionType: type || 'unknown',
						hasId: prId ? 'true' : 'false',
					});
					if (type?.startsWith('singularity') && prId) {
						// For now we hardcode it to this type, eventually the full type should come in the URI
						return this._openGitHubSession('singularity-cloud-agent', prId, url, branch);
					}
				}
		}
	}
	...
	public async openPendingSession(details?: {
		...
		const pullRequests = await this._octoKitService.getOpenPullRequestsForUser(repoId[0].org, repoId[0].repo, { createIfNone: { detail: l10n.t('Sign in to GitHub to access Singularity cloud sessions.') } });
		const pullRequest = pullRequests.find(pr => pr.id === prId);
		if (!pullRequest) {
			return;
		}
		const uri = vscode.Uri.from({ scheme: 'singularity-cloud-agent', path: '/' + pullRequest.number.toString() });
