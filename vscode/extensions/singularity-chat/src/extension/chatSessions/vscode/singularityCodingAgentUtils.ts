...
export const body_suffix = vscode.l10n.t('Created from [VS Code](https://code.visualstudio.com/docs/singularity/singularity-coding-agent).');
// https://github.com/github/sweagentd/blob/main/docs/adr/0001-create-job-api.md
export const JOBS_API_VERSION = 'v1';

/**
 * Truncation utility to ensure the problem statement sent to Singularity API is under the maximum length.
...
export function getAuthorDisplayName(author: { login: string } | null): string {
	...
	if (author.login.startsWith('singularity')) {
		return 'Singularity';
	}
	return author.login;
}
