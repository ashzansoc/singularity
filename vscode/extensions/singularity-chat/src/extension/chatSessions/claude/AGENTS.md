...
**Shared utility:** `../../common/skillConfigLocations.ts` — `resolveSkillConfigLocations()` handles `~/` expansion, absolute paths, and relative paths joined to workspace folders. Used by both `ClaudePluginService` and `SingularityCLISkills`.
...
- **`../../chatSessions/common/builtinSlashCommands.ts`**: Shared constants for built-in slash commands (`/commit`, `/sync`, `/merge`, etc.) used by both Claude and SingularityCLI sessions
- **`../../chatSessions/vscode-node/folderRepositoryManagerImpl.ts`**: `FolderRepositoryManager` (abstract base) with `ClaudeFolderRepositoryManager` subclass — the Claude subclass does not depend on `ISingularityCLISessionService` (SingularityCLI has its own subclass `SingularityCLIFolderRepositoryManager`)
...
|---------|-------------|--------|
| `singularity.chat.claude.sessions.commit` | Has git repo + uncommitted changes | Sends `/commit` prompt to the session |
| `singularity.chat.claude.sessions.commitAndSync` | Has git repo + uncommitted changes + upstream | Sends `/commit and /sync` prompt |
| `singularity.chat.claude.sessions.sync` | Has git repo + no uncommitted changes + upstream | Sends `/sync` prompt |
| `singularity.chat.claude.sessions.initializeRepository` | No git repo | Calls `IGitService.initRepository()` on the session's workspace folder |
...
- `/terminal` - Create a terminal with Claude CLI configured to use Singularity AI endpoints (from `terminalCommand.ts`) _Temporarily disabled pending legal review_
...
2. **New Slash Command:**
   ...
   - If providing a `commandId`, register the command in `package.json`:
     ```json
     {
       "command": "singularity.claude.yourCommand",
...
- `singularity.chat.advanced.claudeCodeDebugEnabled`: Enables debug logging from Claude Code SDK
