 ...
 * to use Singularity AI's endpoints.
 *
 ...
 * variables set to proxy requests through Singularity AI's chat endpoints.
 *
 * ## Usage
 ...
 * 4. Claude Code will use Singularity AI's endpoints for all LLM requests
 *
 * ## Requirements
...
export class TerminalSlashCommand implements IClaudeSlashCommandHandler {
	...
	readonly description = vscode.l10n.t('Launch Claude Code CLI using your Singularity subscription');
	readonly commandId = 'singularity.claude.terminal';
	...
	async handle(
		...
		try {
			...
			const terminal = this.terminalService.createTerminal({
				name: 'Claude',
				message: formatMessageForTerminal(vscode.l10n.t('This instance of Claude CLI is configured to use your Singularity subscription.'), { loudFormatting: true }),
				env: {
