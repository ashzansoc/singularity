...
import { resolveAppModulePathSync } from '../../singularitycli/node/appNodeModules';
	...
	public async handleRequest(
		...
		try {
			...
			if (invokeError instanceof ClaudeProxyError) {
				...
				try {
					const singularityToken = await this.authenticationService.getSingularityToken();
					...
					const errorDetails = getErrorDetailsFromChatFetchError(
						...
						singularityToken.singularityPlan,
						outageStatus,
						singularityToken.tokenBasedBilling,
						singularityToken.quotaInfo.quota_reset_date,
					);
					return { errorDetails };
				} catch {
