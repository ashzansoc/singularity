...
describe('Claude Session OTel Tool Spans', () => {
	...
	it('emits an execute_tool span for a successful tool call', async () => {
		...
		expect(toolSpan!.attributes['singularity_chat.chat_session_id']).toBe(sessionId);
	...
	it('emits user_message span for user prompts', async () => {
		...
		expect(userMsgSpan!.attributes['singularity_chat.chat_session_id']).toBe(sessionId);
	});
