	...
	private async handleAuthedMessagesRequest(bodyString: string, headers: http.IncomingHttpHeaders, res: http.ServerResponse, sessionId: string | undefined): Promise<void> {
		...
		try {
			...
			const doRequest = () => streamingEndpoint.makeChatRequest2({
				debugName: 'Claude Singularity Proxy',
...
export function processNonStreamingPassThroughResponse(
	...
	onUsage: ((usage: { promptTokens: number; completionTokens: number; singularityCredits?: number }) => void) | undefined,
			...
			for await (const completion of inner) {
				...
				if (completion.usage && onUsage) {
					onUsage({
						...
						singularityCredits: nanoAiuToCredits(completion.usage.singularity_usage?.total_nano_aiu),
					});
				}
			}
		} catch (e) {
...
class ClaudeStreamingPassThroughEndpoint implements IChatEndpoint {
	...
	private _processNonStreamingResponse(
		...
		const onUsage = sessionId
			? (usage: { promptTokens: number; completionTokens: number; singularityCredits?: number }) => {
