...
import { getRepoId } from '../vscode/singularityCodingAgentUtils';
...
export class SingularityCloudGitOperationsManager {
	constructor(
	...
	async commitAndPushChanges(): Promise<string> {
		const { repository, remoteName, baseRef } = await this.repoInfo();
		const asyncBranch = await this.generateRandomBranchName(repository, 'singularity');
