...
import { PythonTerminalService } from './singularityCLIPythonTerminalService';
import { SingularityCLITerminalLinkProvider, SessionDirResolver } from './singularityCLITerminalLinkProvider';

//@ts-ignore
import powershellScript from './singularityCLIShim.ps1';

const SINGULARITY_CLI_SHIM_JS = 'singularityCLIShim.js';
const SINGULARITY_CLI_COMMAND = 'singularity';
const SINGULARITY_ICON = new ThemeIcon('singularity');

export type TerminalOpenLocation = 'panel' | 'editor' | 'editorBeside';

export interface ISingularityCLITerminalIntegration extends Disposable {
...
export const ISingularityCLITerminalIntegration = createServiceIdentifier<ISingularityCLITerminalIntegration>('ISingularityCLITerminalIntegration');

export class SingularityCLITerminalIntegration extends Disposable implements ISingularityCLITerminalIntegration {
	...
	private readonly _linkProvider: SingularityCLITerminalLinkProvider | undefined;
	constructor(
		...
		if (configurationService.getConfig(ConfigKey.Advanced.CLITerminalLinks)) {
			this._linkProvider = new SingularityCLITerminalLinkProvider(logService, workspaceService);
	...
	private async initialize(): Promise<void> {
		...
		const storageLocation = path.join(globalStorageUri.fsPath, 'singularityCli');
		this.terminalService.contributePath('singularity-cli', storageLocation, { command: SINGULARITY_CLI_COMMAND }, true);

		await fs.mkdir(storageLocation, { recursive: true });

		if (process.platform === 'win32') {
			this.powershellScriptPath = path.join(storageLocation, `${SINGULARITY_CLI_COMMAND}.ps1`);
			await fs.writeFile(this.powershellScriptPath, powershellScript);
			const singularityPowershellScript = `@echo off
powershell -ExecutionPolicy Bypass -File "${this.powershellScriptPath}" %*
`;
			this.shellScriptPath = path.join(storageLocation, `${SINGULARITY_CLI_COMMAND}.bat`);
			await fs.writeFile(this.shellScriptPath, singularityPowershellScript);
			...
			// support console stdin (see singularityCLIShim.ts header). Instead, delegate to
			...
			const singularityBashScript = `#!/bin/sh\nexec "${posixBatPath}" "$@"\n`;
			this.posixShellScriptPath = path.join(storageLocation, SINGULARITY_CLI_COMMAND);
			await fs.writeFile(this.posixShellScriptPath, singularityBashScript);
		} else {
			const singularityShellScript = `#!/bin/sh
unset NODE_OPTIONS
ELECTRON_RUN_AS_NODE=1 "${process.execPath}" "${path.join(storageLocation, SINGULARITY_CLI_SHIM_JS)}" "$@"`;
			await fs.copyFile(path.join(__dirname, SINGULARITY_CLI_SHIM_JS), path.join(storageLocation, SINGULARITY_CLI_SHIM_JS));
			this.shellScriptPath = path.join(storageLocation, SINGULARITY_CLI_COMMAND);
			this.powershellScriptPath = path.join(storageLocation, `singularityCLIShim.ps1`);
			await fs.writeFile(this.shellScriptPath, singularityShellScript);
			await fs.writeFile(this.powershellScriptPath, powershellScript);
		...
		const provideTerminalProfile = async () => {
			...
			const options = await getCommonTerminalOptions('Singularity CLI', this._authenticationService, this._otelService, 'panel');
		...
		this._register(window.registerTerminalProfileProvider('singularity-cli', { provideTerminalProfile }));
	...
	public async openTerminal(name: string, cliArgs: string[] = [], cwd?: string, location: TerminalOpenLocation = 'editor'): Promise<Terminal | undefined> {
		...
		if (shellPathAndArgs && (shellPathAndArgs.shell !== 'powershell' && shellPathAndArgs.shell !== 'pwsh')) {
			...
			if (terminal) {
				...
				const command = this.buildCommandForPythonTerminal(shellPathAndArgs?.singularityCommand, cliArgs, shellPathAndArgs);
		...
		if (!shellPathAndArgs) {
			...
			const command = this.buildCommandForTerminal(terminal, SINGULARITY_CLI_COMMAND, cliArgs);
	...
	private sendTerminalOpenTelemetry(sessionType: string, shell: string, terminalCreationMethod: string, location: TerminalOpenLocation): void {
		/* __GDPR__
			"singularitycli.terminal.open" : {
				"owner": "DonJayamanne",
				"comment": "Event sent when a Singularity CLI terminal is opened.",
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent('singularitycli.terminal.open', {
			sessionType,
			shell,
	...
	private buildCommandForPythonTerminal(singularityCommand: string, cliArgs: string[], shellInfo: IShellInfo) {
		let commandPrefix = '';
		...
		return `${commandPrefix}${quoteArgsForShell(singularityCommand, [])} ${cliArgs.join(' ')} ${exitCommand}`;
	}

	private buildCommandForTerminal(terminal: Terminal, singularityCommand: string, cliArgs: string[]) {
		return `${quoteArgsForShell(singularityCommand, [])} ${cliArgs.join(' ')}`;
	}
	...
	private async sendCommandToTerminal(terminal: Terminal, command: string, waitForPythonActivation: boolean, shellInfo: IShellInfo | undefined = undefined): Promise<void> {
		...
		try {
			...
			if (waitForPythonActivation) {
				...
				// Else if we send too early, the singularity command might not get executed properly.
	...
	private async getShellInfo(cliArgs: string[]): Promise<IShellInfo | undefined> {
		...
		const iconPath = SINGULARITY_ICON;
		...
		if (shellBasename === 'zsh' && this.shellScriptPath) {
			return {
				...
				iconPath,
				singularityCommand: this.shellScriptPath,
				exitCommand: `&& exit`
			};
			...
			return {
				...
				iconPath,
				singularityCommand: bashScriptPath,
				exitCommand: `&& exit`
			};
			...
			return {
				...
				iconPath,
				singularityCommand: this.powershellScriptPath,
				exitCommand: `&& exit`
			};
...
async function getCommonTerminalOptions(name: string, authenticationService: IAuthenticationService, otelService: IOTelService, location: TerminalOpenLocation = 'editor'): Promise<TerminalOptions> {
	...
	if (session) {
		options.env = {
			...
			// New Token name for Singularity
			SINGULARITY_GITHUB_TOKEN: session.accessToken,
			...
			// `deriveSingularityCliOTelEnv` returns `{}` when not `enabledExplicitly`.
			...deriveSingularityCliOTelEnv(otelService.config, {}),
		};
	}
	return options;
}
