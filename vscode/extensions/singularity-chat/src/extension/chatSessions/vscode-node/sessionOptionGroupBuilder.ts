 ...
 * 4. `singularity-worktree-*` branches excluded
...
import { isWelcomeView } from '../singularitycli/node/singularityCli';
...
export const OPEN_REPOSITORY_COMMAND_ID = 'singularity.chat.cli.sessions.openRepository';
...
const SINGULARITY_WORKTREE_PATTERN = 'singularity-worktree-';
...
export class SessionOptionGroupBuilder implements ISessionOptionGroupBuilder {
	...
	constructor(
		...
		@IChatFolderMruService private readonly singularityCLIFolderMruService: IChatFolderMruService,
	...
	async provideChatSessionProviderOptionGroups(previousInputState: vscode.ChatSessionInputState | undefined, selectedFolderUri?: vscode.Uri): Promise<vscode.ChatSessionProviderOptionGroup[]> {
		...
		if (isWelcomeView(this.workspaceService)) {
			...
			const repositories = await this.singularityCLIFolderMruService.getRecentlyUsedFolders(CancellationToken.None);
	...
	async buildExistingSessionInputStateGroups(resource: vscode.Uri, token: vscode.CancellationToken): Promise<vscode.ChatSessionProviderOptionGroup[]> {
		...
		const optionGroups: vscode.ChatSessionProviderOptionGroup[] = [];
		const folderInfo = await this.folderRepositoryManager.getFolderRepository(singularitycliSessionId, undefined, token);
		const repositories = isWelcomeView(this.workspaceService) ? folderMRUToChatProviderOptions(await this.singularityCLIFolderMruService.getRecentlyUsedFolders(token)) : this.getRepositoryOptionItems();
		...
		const worktreeProperties = await this.chatSessionWorktreeService.getWorktreeProperties(singularitycliSessionId);
	...
	async getBranchOptionItemsForRepository(repoUri: Uri, headBranchName: string | undefined): Promise<vscode.ChatSessionProviderOptionItem[]> {
		...
		return this._getBranchOptionItemsForRepositorySequencer.queue(key, async () => {
			...
			for (const ref of localBranches) {
				...
				if (ref.name.includes(SINGULARITY_WORKTREE_PATTERN)) {
					continue;
				}
