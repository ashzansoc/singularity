 ...
 * {@link IOTelService} (OTel metrics). Constructed once in `SingularityCloudSessionsProvider` with the
...
export class CloudBackendInstrumentation implements ICloudBackendInstrumentation {
	...
	sessionCreated(outcome: CloudBackendOutcome, durationMs: number, error?: unknown): void {
		if (outcome === 'failure') {
			/* __GDPR__
				"singularitycloud.chat.sessionCreate" : {
					"owner": "osortega",
			...
			*/
			this._telemetryService.sendMSFTTelemetryErrorEvent('singularitycloud.chat.sessionCreate', {
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent('singularitycloud.chat.sessionCreate', {
				backendVersion: this.backendVersion,
				outcome,
			}, { durationMs });
		}
	...
	sessionActivated(durationMs: number): void {
		/* __GDPR__
			"singularitycloud.chat.sessionActivated" : {
				"owner": "osortega",
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('singularitycloud.chat.sessionActivated', {
			backendVersion: this.backendVersion,
		}, { durationMs });
	...
	followUp(outcome: CloudBackendOutcome, error?: unknown): void {
		if (outcome === 'failure') {
			/* __GDPR__
				"singularitycloud.chat.followup" : {
					"owner": "osortega",
			...
			*/
			this._telemetryService.sendMSFTTelemetryErrorEvent('singularitycloud.chat.followup', {
				backendVersion: this.backendVersion,
				outcome,
				errorType: cloudErrorType(error),
				errorMessage: errorMessageOf(error),
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent('singularitycloud.chat.followup', {
				backendVersion: this.backendVersion,
				outcome,
			});
		}
	...
	operationFailed(operation: CloudBackendOperation, error: unknown, status?: number): void {
		...
		/* __GDPR__
			"singularitycloud.chat.operationError" : {
				"owner": "osortega",
		...
		*/
		this._telemetryService.sendMSFTTelemetryErrorEvent('singularitycloud.chat.operationError', {
			backendVersion: this.backendVersion,
			operation,
			errorType,
			errorMessage: errorMessageOf(error),
	...
	legacyJobInvoke(hasHeadRef: boolean): void {
		/* __GDPR__
			"singularitycloud.chat.remoteAgentJobInvoke" : {
				"owner": "joshspicer",
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('singularitycloud.chat.remoteAgentJobInvoke', {
	...
	legacyJobPullRequestReady(): void {
		/* __GDPR__
			"singularitycloud.chat.remoteAgentJobPullRequestReady" : {
				"owner": "joshspicer",
				"comment": "Event sent when a remote agent job first returns pull request information."
			}
		*/
		this._telemetryService.sendMSFTTelemetryEvent('singularitycloud.chat.remoteAgentJobPullRequestReady');
	}
}
