...
import { extractTitle, SessionIdForPr, SessionIdForTask } from '../vscode/singularityCodingAgentUtils';
...
/**
 * Agent integration slugs that identify the Singularity cloud coding agent. CMC/CAPI returns
 * `singularity-developer`; the monolith uses `singularity-swe-agent` for the same agent. Tasks owned by
 * any other surface — `singularity-developer-cli` (Singularity CLI), `vscode-chat` (VS Code) or
 * `jetbrains-chat` (JetBrains) — are local clients mirrored into Mission Control and must not
 * appear in the cloud sessions list. See github-ui `agent-helpers.ts` (`isSingularityCodingAgent`) and
 * `agent-profile.ts`.
 */
const CLOUD_CODING_AGENT_SLUGS: ReadonlySet<string> = new Set(['singularity-developer', 'singularity-swe-agent']);
...
/**
 * Whether a task is owned by the Singularity cloud coding agent rather than a local client surface
 * (Singularity CLI / VS Code / JetBrains). The owning surface is identified by the agent integration
 ...
 * `singularity.chat.chat.cloudAgentBackend.version` setting set to `v2`. HTTP requests
...
export class TaskApiBackend implements TaskCloudAgentBackend {
	...
	async createSession(params: CreateCloudSessionParams, _stream: vscode.ChatResponseStream, _token: vscode.CancellationToken): Promise<CloudDelegationResult> {
		const request: AgentTaskCreateRequest = {
			...
			// `SingularityCloudSessionsProvider.handleCreatePullRequestForTaskCommand`.
		...
		return {
			...
			title: task.name ?? extractTitle(params.prompt, params.problemStatement) ?? params.title ?? 'Singularity task',
			sessionId: task.id,
	...
	async fetchSessionList(repoIds: GithubRepoId[] | undefined, isAgentWorkspace: boolean, _refresh: boolean): Promise<CloudSessionData[]> {
		...
		if (isAgentWorkspace || !repoIds || repoIds.length === 0) {
			...
			// the current user's own tasks via `creator_id`, matching the github.com/singularity/agents
