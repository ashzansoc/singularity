...
import { getSingularityHome } from '../singularitycli/node/cliHelpers';

const UNTRUSTED_SINGULARITY_HOME_MESSAGE = l10n.t('The Singularity home directory is not trusted. Please trust the directory to open this file.');
...
interface SingularityCLITerminalLink extends TerminalLink {
	uri?: Uri;
	terminal: Terminal;
	pathText: string;
	line?: number;
...
/**
 * Resolves relative file links in Singularity CLI terminal output.
 *
 * Singularity CLI paths are relative to the session-state directory, not the
 * workspace root. VS Code's built-in detector cannot resolve that context.
 */
export class SingularityCLITerminalLinkProvider implements TerminalLinkProvider<SingularityCLITerminalLink> {

	private readonly _singularityTerminals = new WeakSet<Terminal>();
	...
	/**
	 * Marks a terminal as a Singularity CLI terminal.
	 */
	registerTerminal(terminal: Terminal): void {
		this._singularityTerminals.add(terminal);
	}
	...
	async provideTerminalLinks(context: TerminalLinkContext, token: CancellationToken): Promise<SingularityCLITerminalLink[]> {
		...
		if (!this._singularityTerminals.has(context.terminal) && sessionDirs.length === 0) {
			return [];
		}
		const links: SingularityCLITerminalLink[] = [];
	...
	async handleTerminalLink(link: SingularityCLITerminalLink): Promise<void> {
		try {
			...
			if (this.workspaceService && this._isInSingularityHome(uriToOpen)) {
				const trusted = await this.workspaceService.requestResourceTrust({
					uri: Uri.file(getSingularityHome()),
					message: UNTRUSTED_SINGULARITY_HOME_MESSAGE,
				});
				if (!trusted) {
					return;
				}
	 ...
	 */
	private _isInSingularityHome(uri: Uri): boolean {
		return extUriBiasedIgnorePathCase.isEqualOrParent(uri, Uri.file(getSingularityHome()));
	}
	 ...
	 * 1. Each session-state directory (e.g., `~/.singularity/session-state/{uuid}/`)
	 * 2. Workspace folders
