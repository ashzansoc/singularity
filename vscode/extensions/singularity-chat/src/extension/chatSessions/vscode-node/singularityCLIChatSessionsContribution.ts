...
import { getSingularityCLISessionDir } from '../singularitycli/node/cliHelpers';
import { SINGULARITY_CLI_CONTEXT_SIZE_PROPERTY, SINGULARITY_CLI_REASONING_EFFORT_PROPERTY, ISingularityCLIAgents, ISingularityCLIModels, ISingularityCLISDK, isWelcomeView, resolveContextTier } from '../singularitycli/node/singularityCli';
import { SingularityCLIPromptResolver } from '../singularitycli/node/singularitycliPromptResolver';
import { builtinSlashSCommands, SingularityCLICommand, singularityCLICommands, SingularityCLIQuotaExceededError, ISingularityCLISession } from '../singularitycli/node/singularitycliSession';
import { ISingularityCLISessionItem, ISingularityCLISessionService } from '../singularitycli/node/singularitycliSessionService';
import { buildMcpServerMappings } from '../singularitycli/node/mcpHandler';
import { ISingularityCLISessionTracker } from '../singularitycli/vscode-node/singularityCLISessionTracker';
import { ISingularityCLIChatSessionItemProvider } from './singularityCLIChatSessions';
import { getSingularityCLIModelDetails, persistSingularityCLIResponseModelId } from './singularityCLIModelDetails';
import { ISingularityCLITerminalIntegration, TerminalOpenLocation } from './singularityCLITerminalIntegration';
...
import { convertReferenceToVariable } from '../singularitycli/vscode-node/singularityCLIPromptReferences';
...
const LAST_USED_ISOLATION_OPTION_KEY = 'singularity.chat.cli.lastUsedIsolationOption';
const OPEN_REPOSITORY_COMMAND_ID = 'singularity.chat.cli.sessions.openRepository';
const OPEN_IN_SINGULARITY_CLI_COMMAND_ID = 'singularity.chat.cli.openInSingularityCLI';
...
const _invalidSingularityCLISessionIdsWithErrorMessage = new Map<string, string>();

namespace SessionIdForCLI {
	export function getResource(sessionId: string): vscode.Uri {
		return vscode.Uri.from({
			scheme: 'singularitycli', path: `/${sessionId}`,
		});
	}
	...
	export function isCLIResource(resource: vscode.Uri): boolean {
		return resource.scheme === 'singularitycli';
	}
}
...
function getSessionLoadFailureIssueInfo(invalidSessionMessage: string): { readonly issueBody: string; readonly issueUrl: string } {
	...
	const issueTitle = '[Singularity CLI] Failed to load chat session';
	const issueBody = `## Description\n\nFailed to load a Singularity CLI chat session.\n\n## Environment\n\n- Platform: ${runtimeInfo.platform}\n- VS Code: ${runtimeInfo.vscodeInfo}\n- Chat Extension Version: ${runtimeInfo.extensionVersion}\n\n## Error\n\n\`\`\`\n${invalidSessionMessage}\n\`\`\``;
...
export async function resolveSessionDirsForTerminal(
	sessionTracker: ISingularityCLISessionTracker,
	...
	for (const id of activeIds) {
		...
		const dir = Uri.file(getSingularityCLISessionDir(id));
...
export class SingularityCLIChatSessionItemProvider extends Disposable implements vscode.ChatSessionItemProvider, ISingularityCLIChatSessionItemProvider {
	...
	constructor(
		@ISingularityCLISessionService private readonly singularitycliSessionService: ISingularityCLISessionService,
		@ISingularityCLISessionTracker private readonly sessionTracker: ISingularityCLISessionTracker,
		@ISingularityCLITerminalIntegration private readonly terminalIntegration: ISingularityCLITerminalIntegration,
		...
		if (configurationService.getConfig(ConfigKey.Advanced.CLIChatLazyLoadSessionItem)) {
			this.resolveChatSessionItem = async (item: vscode.ChatSessionItem, token: vscode.CancellationToken): Promise<vscode.ChatSessionItem | undefined> => {
				...
				const session = await this.singularitycliSessionService.getSessionItem(sessionId, token);
		...
		this._register(this.singularitycliSessionService.onDidChangeSessions(() => {
	...
	/**
	 * We should remove this or move this to SingularityCLISessionService
	 */
	...
	public async provideChatSessionItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionItem[]> {
		...
		const sessions = await this.singularitycliSessionService.getAllSessions(token);
		...
		void this.commandExecutionService.executeCommand('setContext', 'singularity.chat.chat.cliSessionsEmpty', count === 0);
		this.logService.info(`[SingularityCLIChatSessionContentProvider] listSessions took ${stopwatch.elapsed()}ms`);
		return diskSessions;
	}
	...
	public async toChatSessionItem(session: ISingularityCLISessionItem, options?: { readonly includeChanges?: boolean }, token: vscode.CancellationToken = CancellationToken.None): Promise<vscode.ChatSessionItem> {
	...
	public async detectPullRequestOnSessionOpen(sessionId: string): Promise<void> {
		try {
			...
			if (worktreeProperties?.version !== 2
				...
				this.logService.debug(`[SingularityCLIChatSessionItemProvider] Skipping PR detection on session open for ${sessionId}: version=${worktreeProperties?.version}, prState=${worktreeProperties?.version === 2 ? worktreeProperties.pullRequestState : 'n/a'}, branch=${!!worktreeProperties?.branchName}, repoPath=${!!worktreeProperties?.repositoryPath}`);
				return;
			}

			this.logService.debug(`[SingularityCLIChatSessionItemProvider] Detecting PR on session open for ${sessionId}, branch=${worktreeProperties.branchName}, existingPrUrl=${worktreeProperties.pullRequestUrl ?? 'none'}`);
			...
			if (prResult) {
				...
				if (currentProperties?.version === 2
					...
					this.logService.debug(`[SingularityCLIChatSessionItemProvider] Updating PR metadata for ${sessionId}: url=${prResult.url}, state=${prResult.state} (was url=${currentProperties.pullRequestUrl ?? 'none'}, state=${currentProperties.pullRequestState ?? 'none'})`);
					...
					this.logService.debug(`[SingularityCLIChatSessionItemProvider] PR metadata unchanged for ${sessionId}, skipping update`);
				}
			} else {
				this.logService.debug(`[SingularityCLIChatSessionItemProvider] No PR found via GitHub API for ${sessionId}`);
			}
		} catch (error) {
			this.logService.trace(`[SingularityCLIChatSessionItemProvider] Failed to detect pull request on session open for ${sessionId}: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	public async createSingularityCLITerminal(location: TerminalOpenLocation = 'editor', name?: string, cwd?: string): Promise<void> {
		// TODO@rebornix should be set by CLI
		const terminalName = name || process.env.SINGULARITYCLI_TERMINAL_TITLE || l10n.t('Singularity CLI');
		await this.terminalIntegration.openTerminal(terminalName, [], cwd, location);
	}

	public async resumeSingularityCLISessionInTerminal(sessionItem: vscode.ChatSessionItem): Promise<void> {
		...
		try {
			...
			if (terminal) {
				...
				this.terminalIntegration.setTerminalSessionDir(terminal, Uri.file(getSingularityCLISessionDir(id)));
			}
		} finally {
			token.dispose();
		}
...
export class SingularityCLIChatSessionContentProvider extends Disposable implements vscode.ChatSessionContentProvider {
	...
	private readonly _activeSessionsById = new Map<string, ISingularityCLISession>();
	/**
	...
	constructor(
		private readonly sessionItemProvider: SingularityCLIChatSessionItemProvider,
		...
		@IChatSessionWorktreeService private readonly singularityCLIWorktreeManagerService: IChatSessionWorktreeService,
		...
		this._register(this.singularityCLIAgents.onDidChangeAgents(() => {
			this._onDidChangeChatSessionProviderOptions.fire();
		}));
	}
	...
	private async getDefaultUntitledSessionRepositoryOption(singularitycliSessionId: string | undefined, token: vscode.CancellationToken) {
		...
		const folderInfo = singularitycliSessionId ? await this.folderRepositoryManager.getFolderRepository(singularitycliSessionId, undefined, token) : undefined;
	...
	async provideChatSessionContent(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		const stopwatch = new StopWatch();
		try {
			const singularitycliSessionId = SessionIdForCLI.parse(resource);
			const isUntitled = this.sessionItemProvider.isNewSession(singularitycliSessionId);
			if (isUntitled) {
			...
			this.logService.info(`[SingularityCLIChatSessionContentProvider] provideChatSessionContent for ${resource.toString()} took ${stopwatch.elapsed()}ms`);
		}
	}
	...
	async provideChatSessionContentForUntitledSession(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		const singularitycliSessionId = SessionIdForCLI.parse(resource);
		this._currentSessionId = singularitycliSessionId;
		const folderRepo = await this.folderRepositoryManager.getFolderRepository(singularitycliSessionId, undefined, token);
		const isUntitled = this.sessionItemProvider.isNewSession(singularitycliSessionId);
		const [history, title] = await Promise.all([
			isUntitled ? Promise.resolve([]) : this.getSessionHistory(singularitycliSessionId, folderRepo, token),
			this.customSessionTitleService.getCustomSessionTitle(singularitycliSessionId),
		]);

		const options: Record<string, string | vscode.ChatSessionProviderOptionItem> = {};

		// Use FolderRepositoryManager to get folder/repository info (no trust check needed for UI population)
		const defaultRepo = await this.getDefaultUntitledSessionRepositoryOption(singularitycliSessionId, token);
		if (defaultRepo) {
			...
			if (defaultRepoIsTrusted) {
				...
				this.folderRepositoryManager.setNewSessionFolder(singularitycliSessionId, defaultRepo);
				...
				if (repoInfo.repository && isIsolationOptionFeatureEnabled(this.configurationService)) {
					if (!_sessionIsolation.has(singularitycliSessionId)) {
						const lastUsed = this.context.globalState.get<IsolationMode>(LAST_USED_ISOLATION_OPTION_KEY, IsolationMode.Workspace);
						_sessionIsolation.set(singularitycliSessionId, lastUsed);
					}
					const isolationMode = _sessionIsolation.get(singularitycliSessionId)!;
				...
				const shouldShowBranch = !isIsolationOptionFeatureEnabled(this.configurationService) || _sessionIsolation.get(singularitycliSessionId) === IsolationMode.Worktree;
				const branchItems = await this.getBranchOptionItems();
				if (branchItems.length > 0 && shouldShowBranch) {
					_sessionBranch.set(singularitycliSessionId, branchItems[0].id);
	...
	async provideChatSessionContentForExistingSession(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		...
		void this.sessionItemProvider.detectPullRequestOnSessionOpen(singularitycliSessionId);

		const folderRepo = await this.folderRepositoryManager.getFolderRepository(singularitycliSessionId, undefined, token);
		const [history, title, folderInfo, worktreeProperties] = await Promise.all([
			this.getSessionHistory(singularitycliSessionId, folderRepo, token),
			this.customSessionTitleService.getCustomSessionTitle(singularitycliSessionId),
			this.folderRepositoryManager.getFolderRepository(singularitycliSessionId, undefined, token),
			this.singularityCLIWorktreeManagerService.getWorktreeProperties(singularitycliSessionId)
		]);
		...
		_sessionIsolation.set(singularitycliSessionId, IsolationMode.Workspace);
		if (worktreeProperties?.repositoryPath) {
			_sessionIsolation.set(singularitycliSessionId, IsolationMode.Worktree);
		...
		_sessionTurnCount.set(singularitycliSessionId, history.length);
	...
	private async getSessionHistory(sessionId: string, workspaceInfo: IWorkspaceInfo, token: vscode.CancellationToken) {
		try {
			_invalidSingularityCLISessionIdsWithErrorMessage.delete(sessionId);
			...
			if (partialHistory) {
				_invalidSingularityCLISessionIdsWithErrorMessage.set(sessionId, error.message || String(error));
	...
	private _getActiveSessionForResourceId(sessionId: string): ISingularityCLISession | undefined {
	...
	trackActiveSession(resourceSessionId: string, session: ISingularityCLISession): void {
		this._activeSessionsById.set(resourceSessionId, session);
		this._activeSessionsById.set(session.sessionId, session);
	}

	untrackActiveSession(resourceSessionId: string | undefined, session: ISingularityCLISession | undefined, hasPendingRequests: boolean): void {
...
export class SingularityCLIChatSessionParticipant extends Disposable {

	constructor(
		private readonly contentProvider: SingularityCLIChatSessionContentProvider,
		private readonly promptResolver: SingularityCLIPromptResolver,
		private readonly sessionItemProvider: SingularityCLIChatSessionItemProvider,
		private readonly cloudSessionProvider: SingularityCloudSessionsProvider | undefined,
		...
		@ISingularityCLISessionService private readonly sessionService: ISingularityCLISessionService,
		@IChatSessionWorktreeService private readonly singularityCLIWorktreeManagerService: IChatSessionWorktreeService,
		@IChatSessionWorktreeCheckpointService private readonly singularityCLIWorktreeCheckpointService: IChatSessionWorktreeCheckpointService,
		@IChatSessionWorkspaceFolderService private readonly workspaceFolderService: IChatSessionWorkspaceFolderService,
		...
		@IFolderRepositoryManager private readonly folderRepositoryManager: IFolderRepositoryManager,
		@IConfigurationService private readonly configurationService: IConfigurationService,
		@ISingularityCLISDK private readonly singularityCLISDK: ISingularityCLISDK,
		@IChatSessionMetadataStore private readonly chatSessionMetadataStore: IChatSessionMetadataStore,
		@ICustomSessionTitleService private readonly customSessionTitleService: ICustomSessionTitleService,
		@IOctoKitService private readonly octoKitService: IOctoKitService,
	 ...
	 *    {@link SingularityCLISession} detects the session is still busy and routes
	...
	private sendTelemetryForHandleRequest(request: vscode.ChatRequest, chatSessionContext: vscode.ChatSessionContext | undefined): void {
		...
		/* __GDPR__
		"singularitycli.chat.invoke" : {
			"owner": "joshspicer",
			"comment": "Event sent when a SingularityCLI chat request is made.",
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent('singularitycli.chat.invoke', {
	...
	private async handleRequestImpl(request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<vscode.ChatResult | void> {
		...
		let activeSession: ISingularityCLISession | undefined;
		...
		try {
			...
			const [authInfo,] = await Promise.all([this.singularityCLISDK.getAuthInfo().catch((ex) => this.logService.error(ex, 'Authorization failed')), this.lockRepoOptionForSession(context, token)]);
			if (!authInfo) {
			...
			const invalidSessionMessage = _invalidSingularityCLISessionIdsWithErrorMessage.get(id);
			...
			if (context.history.length === 0) {
				...
				await this.singularityCLIWorktreeCheckpointService.handleRequest(session.object.sessionId);
			}
			...
			const isSingularityCLICommand = !!request.command && (singularityCLICommands as readonly string[]).includes(request.command);
			if (request.command === 'delegate') {
			...
			} else if (isSingularityCLICommand && (!isUntitled || request.command === 'remote')) {
				const { prompt, attachments } = request.prompt
					...
					: { prompt: '', attachments: [] };
				await session.object.handleRequest(request, { command: request.command as SingularityCLICommand, prompt }, attachments, model, authInfo, token);
				await this.commitWorktreeChangesIfNeeded(request, session.object, token);
			...
			const { result, responseModelId } = await getSingularityCLIModelDetails(session.object, model, this.singularityCLIModels, this.logService, modelDetailsEnabled, creditsUsed);
			await persistSingularityCLIResponseModelId(session.object.sessionId, request.id, responseModelId, model?.model === 'auto', this.chatSessionMetadataStore, this.logService, creditsUsed);
			...
			if (ex instanceof SingularityCLIQuotaExceededError) {
	...
	private async commitWorktreeChangesIfNeeded(request: vscode.ChatRequest, session: ISingularityCLISession, token: vscode.CancellationToken): Promise<void> {
		...
		try {
			if (session.status === vscode.ChatSessionStatus.Completed) {
				...
				if (isIsolationEnabled(session.workspace)) {
					...
					await this.singularityCLIWorktreeManagerService.handleRequestCompleted(session.sessionId);
				} else if (workingDirectory) {
				...
				await this.singularityCLIWorktreeCheckpointService.handleRequestCompleted(session.sessionId, request.id);
				if (workingDirectory) {
					void clearChangesCacheForAffectedSessions(workingDirectory, [session.sessionId], this.logService, this.chatSessionMetadataStore, this.workspaceFolderService, this.singularityCLIWorktreeManagerService, this.sessionItemProvider).catch(ex => this.logService.error(ex, 'Failed to clear changes cache after request completion'));
				}
			}
	...
	private async handlePullRequestCreated(session: ISingularityCLISession): Promise<void> {
		const sessionId = session.sessionId;
		let prUrl = session.createdPullRequestUrl;
		let prState = '';

		this.logService.debug(`[SingularityCLIChatSessionParticipant] handlePullRequestCreated for ${sessionId}: createdPullRequestUrl=${prUrl ?? 'none'}`);

		const worktreeProperties = await this.singularityCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		...
		if (!prUrl) {
			...
			if (worktreeProperties.branchName && worktreeProperties.repositoryPath) {
				this.logService.debug(`[SingularityCLIChatSessionParticipant] No PR URL from session, attempting retry detection for ${sessionId}, branch=${worktreeProperties.branchName}`);
				const prResult = await this.detectPullRequestWithRetry(sessionId);
				prUrl = prResult?.url;
				prState = prResult?.state ?? prUrl ? 'open' : '';
			} else {
				this.logService.debug(`[SingularityCLIChatSessionParticipant] Skipping retry detection for ${sessionId}: branch=${worktreeProperties.branchName ?? 'none'}, repoPath=${!!worktreeProperties.repositoryPath}`);
			}
		}

		if (!prUrl) {
			this.logService.debug(`[SingularityCLIChatSessionParticipant] No PR detected for ${sessionId} after all attempts`);
			return;
		}

		try {
			await this.singularityCLIWorktreeManagerService.setWorktreeProperties(sessionId, {
				...worktreeProperties,
	...
	private async detectPullRequestWithRetry(sessionId: string): Promise<{ url: string; state: string } | undefined> {
		const maxRetries = SingularityCLIChatSessionParticipant._PR_DETECTION_RETRY_COUNT;
		const initialDelay = SingularityCLIChatSessionParticipant._PR_DETECTION_INITIAL_DELAY_MS;

		for (let attempt = 0; attempt < maxRetries; attempt++) {
			const delay = initialDelay * Math.pow(2, attempt);
			this.logService.debug(`[SingularityCLIChatSessionParticipant] PR detection retry for ${sessionId}: attempt ${attempt + 1}/${maxRetries}, waiting ${delay}ms`);
			await new Promise<void>(resolve => setTimeout(resolve, delay));

			const prResult = await this.detectPullRequestForSession(sessionId);
			if (prResult) {
				this.logService.debug(`[SingularityCLIChatSessionParticipant] PR detected on attempt ${attempt + 1} for ${sessionId}: url=${prResult.url}, state=${prResult.state}`);
				return prResult;
			}
		}

		this.logService.debug(`[SingularityCLIChatSessionParticipant] PR detection exhausted all ${maxRetries} retries for ${sessionId}`);
		return undefined;
	}
	...
	private async detectPullRequestForSession(sessionId: string): Promise<{ url: string; state: string } | undefined> {
		try {
			const worktreeProperties = await this.singularityCLIWorktreeManagerService.getWorktreeProperties(sessionId);
			if (!worktreeProperties?.branchName || !worktreeProperties.repositoryPath) {
				this.logService.debug(`[SingularityCLIChatSessionParticipant] detectPullRequestForSession: missing worktree info for ${sessionId}, branch=${worktreeProperties?.branchName ?? 'none'}, repoPath=${!!worktreeProperties?.repositoryPath}`);
				return undefined;
			}
			...
			this.logService.debug(`[SingularityCLIChatSessionParticipant] Failed to detect pull request via GitHub API: ${error instanceof Error ? error.message : String(error)}`);
			return undefined;
		}
	}
	...
	private async getOrCreateSession(request: vscode.ChatRequest, chatSessionContext: vscode.ChatSessionContext, stream: vscode.ChatResponseStream, options: { model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined; agent: SweCustomAgent | undefined; newBranch?: Promise<string | undefined>; sessionParentId?: string; permissionLevel?: string }, disposables: DisposableStore, token: vscode.CancellationToken): Promise<{ session: IReference<ISingularityCLISession> | undefined; trusted: boolean }> {
		...
		if (isNewSession) {
			...
			if (worktreeProperties) {
				void this.singularityCLIWorktreeManagerService.setWorktreeProperties(session.object.sessionId, worktreeProperties);
			}
		}
	...
	private async getModelId(request: vscode.ChatRequest | undefined, token: vscode.CancellationToken): Promise<{ model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined> {
		...
		const model = promptFile?.header?.model ? await getModelFromPromptFile(promptFile.header.model, this.singularityCLIModels) : undefined;
		if (token.isCancellationRequested) {
			return undefined;
		}
		if (model) {
	...
	private async handleDelegationToCloud(session: ISingularityCLISession, request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) {
		...
		const worktreeProperties = await this.singularityCLIWorktreeManagerService.getWorktreeProperties(session.sessionId);
	...
	private async handleDelegationFromAnotherChat(
		...
		if (cancelled || token.isCancellationRequested) {
			stream.markdown(l10n.t('Singularity CLI delegation cancelled.'));
			return {};
		}
		const workingDirectory = getWorkingDirectory(workspaceInfo);
		const worktreeProperties = workspaceInfo.worktreeProperties;
		...
		void vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.singularitycli', {
		...
		stream.markdown(l10n.t('A Singularity CLI session has begun working on your request. Follow its progress in the sessions list.'));
	...
	private async recordPushToSession(
		session: ISingularityCLISession,
		userPrompt: string,
...
export function registerCLIChatCommands(
	singularitycliSessionItemProvider: SingularityCLIChatSessionItemProvider,
	singularityCLISessionService: ISingularityCLISessionService,
	singularityCLIWorktreeManagerService: IChatSessionWorktreeService,
	singularityCLIWorktreeCheckpointService: IChatSessionWorktreeCheckpointService,
	gitService: IGitService,
	gitCommitMessageService: IGitCommitMessageService,
	gitExtensionService: IGitExtensionService,
	toolsService: IToolsService,
	singularityCliWorkspaceSession: IChatSessionWorkspaceFolderService,
	contentProvider: SingularityCLIChatSessionContentProvider,
	...
	async function shouldKeepWorktreeForOtherSessions(sessionId: string, worktreePath: vscode.Uri): Promise<boolean> {
		const siblings = await getBlockingSiblingSessionsForFolder(
			...
			sessionId,
			metadataStore,
			singularityCliWorkspaceSession,
		);
		return siblings.length > 0;
	}
	async function deleteSessionById(sessionId: string, options?: { keepWorktree?: boolean; sessionLabel?: string }): Promise<void> {
		const worktree = await singularityCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		const worktreePath = await singularityCLIWorktreeManagerService.getWorktreePath(sessionId);

		await singularityCLISessionService.deleteSession(sessionId);
		await singularityCliWorkspaceSession.deleteTrackedWorkspaceFolder(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.delete', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			const id = SessionIdForCLI.parse(sessionItem.resource);
			const sessionId = singularitycliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
			const worktreePath = await singularityCLIWorktreeManagerService.getWorktreePath(sessionId);
			...
			if (result === deleteLabel) {
				...
				singularitycliSessionItemProvider.notifySessionsChange();
			}
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('agents.singularity.chat.cli.deleteSessions', async (sessionItems?: vscode.ChatSessionItem[], options?: { skipConfirmation?: boolean }) => {
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.resumeInTerminal', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			await singularitycliSessionItemProvider.resumeSingularityCLISessionInTerminal(sessionItem);
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		if (newTitle) {
			...
			if (trimmedTitle) {
				const id = SessionIdForCLI.parse(sessionItem.resource);
				const sessionId = singularitycliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
				await singularityCLISessionService.renameSession(sessionId, trimmedTitle);
				singularitycliSessionItemProvider.notifySessionsChange();
			}
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.setTitle', async (sessionItem?: vscode.ChatSessionItem, title?: string) => {
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.newSession', async () => {
		await singularitycliSessionItemProvider.createSingularityCLITerminal('editor', l10n.t('Singularity CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.newSessionToSide', async () => {
		await singularitycliSessionItemProvider.createSingularityCLITerminal('editorBeside', l10n.t('Singularity CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand(OPEN_IN_SINGULARITY_CLI_COMMAND_ID, async (sourceControlContext?: unknown) => {
		const rootUri = getSourceControlRootUri(sourceControlContext);
		await singularitycliSessionItemProvider.createSingularityCLITerminal('editor', l10n.t('Singularity CLI'), rootUri?.fsPath);
	}));
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.openWorktreeInNewWindow', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.openWorktreeInTerminal', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.copyWorktreeBranchName', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		const id = SessionIdForCLI.parse(sessionItem.resource);
		const sessionId = singularitycliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
		const worktreeProperties = await singularityCLIWorktreeManagerService.getWorktreeProperties(sessionId);
	...
	const applyChanges = async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		...
		try {
			// Apply changes
			const sessionId = SessionIdForCLI.parse(resource);
			await singularityCLIWorktreeManagerService.applyWorktreeChanges(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.chat.applySingularityCLIAgentSessionChanges', applyChanges));
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.chat.applySingularityCLIAgentSessionChanges.apply', applyChanges));
	...
	const mergeChanges = async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri, syncWithRemote: boolean = false) => {
		...
		await vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.singularitycli', {
			resource,
			prompt,
			attachedContext: [{
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.chat.mergeSingularityCLIAgentSessionChanges.merge', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await mergeChanges(sessionItemOrResource);
	}));

	disposableStore.add(vscode.commands.registerCommand('singularity.chat.chat.mergeSingularityCLIAgentSessionChanges.mergeAndSync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await mergeChanges(sessionItemOrResource, true);
	}));

	disposableStore.add(vscode.commands.registerCommand('singularity.chat.sessions.refreshChanges', async (resource?: vscode.Uri) => {
		if (!resource) {
			return;
		}
		...
		const sessionId = SessionIdForCLI.parse(resource);
		const worktreeProperties = await singularityCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		const workspaceFolder = await singularityCliWorkspaceSession.getSessionWorkspaceFolder(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.sessions.initializeRepository', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		await singularityCliWorkspaceSession.trackSessionWorkspaceFolder(sessionId, workspaceFolder.fsPath, repositoryProperties);
		singularityCliWorkspaceSession.clearWorkspaceChanges(sessionId);
	...
	const setHasGitOperationInProgress = async (sessionId: string, inProgress: boolean, commandId = '') => {
		...
		if (worktreeProperties) {
			...
			if (!inProgress) {
				...
				await singularityCLIWorktreeManagerService.refreshWorktreeChanges(sessionId);
			}

			await singularityCLIWorktreeManagerService.updateWorktreeProperties(sessionId, {
				hasGitOperationInProgress: inProgress
			});

			await singularitycliSessionItemProvider.refreshSession({ reason: 'update', sessionId });
		...
		// Workspace
		const workspaceFolder = await singularityCliWorkspaceSession.getSessionWorkspaceFolder(sessionId);
		const repositoryProperties = await singularityCliWorkspaceSession.getRepositoryProperties(sessionId);
		...
		if (!inProgress) {
			// Refresh the changes after the git operation is complete
			await singularityCliWorkspaceSession.refreshWorkspaceChanges(sessionId);
		}

		await singularityCliWorkspaceSession.setRepositoryProperties(sessionId, {
	...
	const commit = async (sessionId: string, sync: boolean) => {
		...
		try {
			...
			logService.error(`[SingularityCLI] Failed to generate commit message for session ${sessionId}: ${error instanceof Error ? error.message : String(error)}`);
		}

		if (!message) {
			message = `Singularity CLI session ${sessionId} changes`;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.sessions.commit', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'singularity.chat.sessions.commit');
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.sessions.commitAndSync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'singularity.chat.sessions.commitAndSync');
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.sessions.sync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'singularity.chat.sessions.sync');
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.sessions.discardChanges', async (sessionResource: vscode.Uri, ref: string, ...resources: vscode.Uri[]) => {
		...
		await singularityCLIWorktreeCheckpointService.updateLastCheckpoint(sessionId);
	...
	const createPullRequest = async (sessionItemOrResource: vscode.ChatSessionItem | vscode.Uri | undefined, isDraft: boolean) => {
		...
		const sessionId = SessionIdForCLI.parse(resource);
		let worktreeProperties = await singularityCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		if (!worktreeProperties || worktreeProperties.version !== 2) {
		...
		try {
			const commandId = isDraft
				? 'singularity.chat.chat.createDraftPullRequestSingularityCLIAgentSession.createDraftPR'
				: 'singularity.chat.chat.createPullRequestSingularityCLIAgentSession.createPR';
			...
			worktreeProperties = await singularityCLIWorktreeManagerService.getWorktreeProperties(sessionId);
			if (worktreeProperties && worktreeProperties.version === 2) {
				await singularityCLIWorktreeManagerService.setWorktreeProperties(sessionId, {
					...worktreeProperties,
					changes: undefined,
					pullRequestUrl
				});
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.chat.createPullRequestSingularityCLIAgentSession.createPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await createPullRequest(sessionItemOrResource, false);
	}));

	disposableStore.add(vscode.commands.registerCommand('singularity.chat.chat.createDraftPullRequestSingularityCLIAgentSession.createDraftPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await createPullRequest(sessionItemOrResource, true);
	}));

	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.commitToWorktree', async (args?: { worktreeUri?: vscode.Uri; fileUri?: vscode.Uri }) => {
		...
		try {
			...
			const sessionIds = await singularitycliSessionItemProvider.getAssociatedSessions(worktreeUri);
			await Promise.all(sessionIds.map(async sessionId => {
				const props = await singularityCLIWorktreeManagerService.getWorktreeProperties(sessionId);
				if (props) {
					await singularityCLIWorktreeManagerService.setWorktreeProperties(sessionId, { ...props, changes: undefined });
				} else {
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.commitToRepository', async (args?: { repositoryUri?: vscode.Uri; fileUri?: vscode.Uri }) => {
...
async function getModelFromPromptFile(models: readonly string[], singularityCLIModels: ISingularityCLIModels): Promise<string | undefined> {
	for (const model of models) {
		let modelId = await singularityCLIModels.resolveModel(model);
		if (modelId) {
			return modelId;
		}
		...
		modelId = await singularityCLIModels.resolveModel(model.substring(0, model.indexOf('(')).trim());
		if (modelId) {
			return modelId;
		}
	}
