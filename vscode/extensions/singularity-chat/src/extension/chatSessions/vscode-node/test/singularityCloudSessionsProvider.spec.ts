...
import { ChatSessionContentBuilder, extractTaskErrorDetail, formatTaskStoppedMessage } from '../singularityCloudSessionContentBuilder';
import { getCloudSessionItemMetadata, normalizeInitialSessionOptions, parseSessionLogChunksSafely, taskStateToChatSessionStatus } from '../singularityCloudSessionsProvider';
import { TaskApiBackend, parseRepoFromTaskUrl, isCloudCodingAgentTask } from '../taskApiBackend';
import { isActiveTaskState, isFailedTaskState } from '../../vscode/singularityCodingAgentUtils';
...
describe('singularityCloudSessionsProvider helpers', () => {
	it('coerces object-shaped initialSessionOptions into option entries', () => {
		const logService = new RecordingLogService();
		const sessionResource = vscode.Uri.parse('singularity-cloud-agent:/1');
	...
	it('includes the task branch in PR-less cloud session metadata', () => {
		expect(getCloudSessionItemMetadata(
			...
			{ owner: 'microsoft', repo: 'vscode', baseRef: 'main', headRef: 'singularity/task-branch' },
		)).toEqual({
			owner: 'microsoft',
			name: 'vscode',
			host: 'github.com',
			branch: 'singularity/task-branch',
		});
	});
});
...
describe('ChatSessionContentBuilder', () => {
	it('ignores malformed tool_calls payloads instead of throwing', async () => {
		const builder = new ChatSessionContentBuilder('singularity-cloud-agent', new TestGitService(), new TestLogService());
		const logs = [
...
describe('ChatSessionContentBuilder Task API history', () => {
	const newBuilder = () =>
		new ChatSessionContentBuilder('singularity-cloud-agent', new TestGitService(), new TestLogService());
	...
	it('renders a stopped notice (not a progress spinner) when the task terminally failed but its latest session state is still active', async () => {
		...
		expect(summarise(history)).toEqual([
			...
			{ kind: 'response', parts: [{ type: 'markdown', value: 'Singularity stopped: an error occurred' }] },
		]);
	});
	...
	it('renders a cancellation reason (not an error) when the task was cancelled with no error event', async () => {
		...
		expect(summarise(history)).toEqual([
			...
			{ kind: 'response', parts: [{ type: 'markdown', value: 'Singularity stopped: cancelled' }] },
		]);
	});
	...
	it('includes the concrete error detail from a bootstrap session.error in the stopped notice', async () => {
		...
		expect(summarise(history)).toEqual([
			{ kind: 'request', prompt: 'Do the thing' },
			{ kind: 'response', parts: [{ type: 'markdown', value: 'Singularity stopped: (launch_failed) Failed to launch agent' }] },
		]);
	});
...
describe('extractTaskErrorDetail / formatTaskStoppedMessage', () => {
	...
	it('uses the detail when present, else a state-derived reason', () => {
		expect(formatTaskStoppedMessage('failed', '(launch_failed) Failed to launch agent'))
			.toBe('Singularity stopped: (launch_failed) Failed to launch agent');
		expect(formatTaskStoppedMessage('cancelled', undefined)).toBe('Singularity stopped: cancelled');
		expect(formatTaskStoppedMessage('timed_out', undefined)).toBe('Singularity stopped: timed out');
		expect(formatTaskStoppedMessage('failed', undefined)).toBe('Singularity stopped: an error occurred');
	});
});
...
describe('TaskApiBackend', () => {
	...
	it('fetchSessionList resolves a global-list task repo by numeric id when it has no html_url', async () => {
		...
		const client = new FakeTaskApiClient({
			globalTasks: [
				{ id: 'g1', state: 'completed', created_at: '2026-03-27T00:00:00Z', agent_collaborators: [{ slug: 'singularity-developer' }], repository: { id: 123 } } as unknown as AgentTask,
				{ id: 'g2', state: 'completed', created_at: '2026-03-27T00:00:00Z', agent_collaborators: [{ slug: 'singularity-developer' }], repository: { id: 123 } } as unknown as AgentTask,
			],
		});
		const octoKitService = new MockOctoKitService();
	...
	it('fetchSessionList carries the raw task lifecycle state so settled tasks are not shown as in_progress', async () => {
		...
		const client = new FakeTaskApiClient({ repoTasks: [{ id: 't-idle', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'singularity-developer' }] } as unknown as AgentTask] });
		const octoKitService = new MockOctoKitService();
		octoKitService.getCurrentAuthedUser = async () => ({ id: 4242, login: 'octocat', name: 'The Octocat', avatar_url: '' });
		const backend = new TaskApiBackend(client, new TestLogService(), octoKitService, NullCloudBackendInstrumentation);
	...
	it('fetchSessionList shows only cloud coding agent tasks and excludes local-client tasks (CLI / VS Code / JetBrains)', async () => {
		const repoTasks = [
			{ id: 'cloud-dev', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'singularity-developer' }] },
			{ id: 'cloud-swe', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'singularity-swe-agent' }] },
			{ id: 'cli', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'singularity-developer-cli' }] },
...
describe('isCloudCodingAgentTask', () => {
	it('keeps cloud coding agent slugs and rejects local-client / missing / malformed slugs', () => {
		...
		expect({
			'singularity-developer': classify([{ slug: 'singularity-developer' }]),
			'singularity-swe-agent': classify([{ slug: 'singularity-swe-agent' }]),
			'singularity-developer-cli': classify([{ slug: 'singularity-developer-cli' }]),
			...
			'singularity-developer': true,
			'singularity-swe-agent': true,
			'singularity-developer-cli': false,
			'vscode-chat': false,
