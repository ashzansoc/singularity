...
vi.mock('../singularityCLIPythonTerminalService', () => ({
...
vi.mock('../singularityCLITerminalLinkProvider', () => ({
	SingularityCLITerminalLinkProvider: class {
...
import { SingularityCLITerminalIntegration } from '../singularityCLITerminalIntegration';
...
class TestExtensionContext {
	...
	extension = { id: 'Singularity.singularity-chat' };
	extensionUri = { fsPath: '/tmp/extensions/singularity-chat' };
...
describe('SingularityCLITerminalIntegration', () => {
	...
	beforeEach(async () => {
		...
		integration = new SingularityCLITerminalIntegration(
	...
	describe('openTerminal', () => {
		...
		it('should set sessionType to "new" when no cliArgs provided', async () => {
			...
			const event = telemetryService.events.find(e => e.name === 'singularitycli.terminal.open');
		...
		it('should fall back to terminalService when getShellInfo returns undefined', async () => {
			...
			const freshIntegration = new SingularityCLITerminalIntegration(
				new TestExtensionContext() as unknown as IVSCodeExtensionContext,
				authService as unknown as IAuthenticationService,
				terminalService as unknown as ITerminalService,
				envService as unknown as IEnvService,
	...
	describe('initialize', () => {
		it('should contribute path to terminal service', async () => {
			expect(terminalService.contributePathSpy).toHaveBeenCalledWith(
				'singularity-cli',
				expect.stringContaining('singularityCli'),
				expect.objectContaining({ command: 'singularity' }),
				true,
			);
		});
	...
	describe('getCommonTerminalOptions (via openTerminal)', () => {
		...
		it('should include auth env vars when session is available', async () => {
			...
			expect(callArgs.env).toEqual({
				...
				SINGULARITY_GITHUB_TOKEN: 'test-token-123',
			});
		});
	});
});
