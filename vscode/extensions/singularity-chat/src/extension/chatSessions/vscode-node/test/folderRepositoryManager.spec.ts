...
/**
 * Fake implementation of ISingularityCLISessionService for testing.
 */
...
describe('SingularityCLIFolderRepositoryManager', () => {
	const disposables = new DisposableStore();
	let manager: SingularityCLIFolderRepositoryManager;
	let worktreeService: FakeChatSessionWorktreeService;
	let workspaceFolderService: FakeChatSessionWorkspaceFolderService;
	let sessionService: FakeSingularityCLISessionService;
	...
	beforeEach(() => {
		...
		sessionService = new FakeSingularityCLISessionService();
		...
		manager = new SingularityCLIFolderRepositoryManager(
			worktreeService,
	...
	describe('getFolderRepository', () => {
		...
		it('returns worktree info for sessions with worktrees', async () => {
			...
			worktreeService.setTestWorktreeProperties(sessionId, {
				autoCommit: true,
				baseCommit: 'abc123',
				branchName: 'singularity-worktree',
				repositoryPath: '/repo',
	...
	describe('uncommitted changes prompting in initializeFolderRepository', () => {
		...
		it('uses delegation title when no session ID', async () => {
			...
			expect(toolsService.invokeTool).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					input: expect.objectContaining({
						title: 'Delegate to Singularity CLI',
						modifiedFiles: [
							expect.objectContaining({
