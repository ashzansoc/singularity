...
import { type SingularityCLIModelInfo, type ISingularityCLIModels, type ISingularityCLISDK } from '../../singularitycli/node/singularityCli';
import { SingularityCLIPromptResolver } from '../../singularitycli/node/singularitycliPromptResolver';
import { SingularityCLISession, SingularityCLISessionInput, ISingularityCLISession } from '../../singularitycli/node/singularitycliSession';
import { SingularityCLISessionService, SingularityCLISessionWorkspaceTracker, ISingularityCLISessionService } from '../../singularitycli/node/singularitycliSessionService';
import { ISingularityCLIMCPHandler } from '../../singularitycli/node/mcpHandler';
import { MockCliSdkSession, MockCliSdkSessionManager, MockSkillLocations, NullSingularityCLIAgents, NullISingularityCLIImageSupport } from '../../singularitycli/node/test/testHelpers';
import { IQuestion, IQuestionAnswer, IUserQuestionHandler } from '../../singularitycli/node/userInputHelpers';
import { CustomSessionTitleService } from '../../singularitycli/vscode-node/customSessionTitleServiceImpl';
import { SingularityCLIChatSessionContentProvider, SingularityCLIChatSessionItemProvider, SingularityCLIChatSessionParticipant } from '../singularityCLIChatSessionsContribution';
import { SingularityCloudSessionsProvider } from '../singularityCloudSessionsProvider';
import { SingularityCLIFolderRepositoryManager } from '../folderRepositoryManagerImpl';
...
vi.mock('../singularityCLITerminalIntegration', () => {
	...
	class SingularityCLITerminalIntegration {
		dispose() { }
		openTerminal = vi.fn(async () => { });
	}
	return {
		ISingularityCLITerminalIntegration: createServiceIdentifier('ISingularityCLITerminalIntegration'),
		SingularityCLITerminalIntegration
	};
});
...
class FakeModels {
	...
	getModels = vi.fn(async () => [{ id: 'base', name: 'Base', maxContextWindowTokens: 128000, supportsVision: false }] as SingularityCLIModelInfo[]);
...
class FakeCloudProvider extends mock<SingularityCloudSessionsProvider>() {
	override delegate = vi.fn(async () => ({
		...
		author: 'Test Author',
		linkTag: '#1'
	})) as unknown as SingularityCloudSessionsProvider['delegate'];
}
...
function createChatContext(sessionId: string, isUntitled: boolean, ...requests: TestChatRequest[]): vscode.ChatContext {
	...
	const normalizedSessionId = isUntitled && !sessionId.startsWith('untitled-')
		...
		: sessionId;
	const resource = vscode.Uri.from({ scheme: 'singularitycli', path: `/${normalizedSessionId}` });
...
class TestSingularityCLISession extends SingularityCLISession {
	public requests: Array<{ input: SingularityCLISessionInput; attachments: Attachment[]; model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined; authInfo: NonNullable<SessionOptions['authInfo']>; token: vscode.CancellationToken }> = [];
	...
	public static handleRequestHook: ((request: { id: string; toolInvocationToken: vscode.ChatParticipantToolToken; sessionResource?: vscode.Uri }, input: SingularityCLISessionInput) => Promise<void>) | undefined;
	...
	override get status(): vscode.ChatSessionStatus | undefined {
		return TestSingularityCLISession.statusOverride;
	}
	override handleRequest(request: { id: string; toolInvocationToken: vscode.ChatParticipantToolToken; sessionResource?: vscode.Uri }, input: SingularityCLISessionInput, attachments: Attachment[], model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined, authInfo: NonNullable<SessionOptions['authInfo']>, token: vscode.CancellationToken): Promise<void> {
		this.requests.push({ input, attachments, model, authInfo, token });
		if (TestSingularityCLISession.handleRequestHook) {
			return TestSingularityCLISession.handleRequestHook(request, input);
		}
		return TestSingularityCLISession.nextHandleRequestResult ?? Promise.resolve();
	}
	override attachStream(stream: vscode.ChatResponseStream): ReturnType<SingularityCLISession['attachStream']> {
	...
	override getLastResponseModelId(): string | undefined {
		return TestSingularityCLISession.lastResponseModelId;
	}
}


class FakeSingularityCLISessionService extends mock<ISingularityCLISessionService>() {
	private _sessionWorkingDirs = new Map<string, vscode.Uri>();
	override tryGetPartialSessionHistory: ISingularityCLISessionService['tryGetPartialSessionHistory'] = vi.fn(async () => undefined);
...
describe('SingularityCLIChatSessionParticipant.handleRequest', () => {
	const disposables = new DisposableStore();
	let promptResolver: SingularityCLIPromptResolver;
	let itemProvider: SingularityCLIChatSessionItemProvider;
	...
	let sessionService: SingularityCLISessionService;
	let telemetry: ITelemetryService;
	let tools: FakeToolsService;
	let participant: SingularityCLIChatSessionParticipant;
	let workspaceService: IWorkspaceService;
	let instantiationService: IInstantiationService;
	let logService: ILogService;
	let configurationService: InMemoryConfigurationService;
	let manager: MockCliSdkSessionManager;
	let mcpHandler: ISingularityCLIMCPHandler;
	let folderRepositoryManager: SingularityCLIFolderRepositoryManager;
	let cliSessionServiceForFolderManager: FakeSingularityCLISessionService;
	let contentProvider: SingularityCLIChatSessionContentProvider;
	let sdk: ISingularityCLISDK;
	let customSessionTitleService: CustomSessionTitleService;
	const cliSessions: TestSingularityCLISession[] = [];

	beforeEach(async () => {
		cliSessions.length = 0;
		TestSingularityCLISession.nextHandleRequestResult = undefined;
		TestSingularityCLISession.handleRequestHook = undefined;
		TestSingularityCLISession.statusOverride = undefined;
		TestSingularityCLISession.lastResponseModelId = undefined;
		// By default, simulate VS Code core opening the delegated session and
		// re-invoking handleRequest with the singularitycli:// resource. This matches
		...
		mockExecuteCommand.mockImplementation(async (command: string, args: any) => {
			if (command === 'workbench.action.chat.openSessionWithPrompt.singularitycli') {
		...
		} as unknown as ISingularityCLISDK;
		const services = disposables.add(createExtensionUnitTestingServices());
		const accessor = services.createTestingAccessor();
		disposables.add(accessor);
		promptResolver = new class extends mock<SingularityCLIPromptResolver>() {
		...
		itemProvider = new class extends mock<SingularityCLIChatSessionItemProvider>() {
			override swap = vi.fn();
		...
		cliSessionServiceForFolderManager = new FakeSingularityCLISessionService();
		...
		mcpHandler = new class extends mock<ISingularityCLIMCPHandler>() {
		...
		instantiationService = {
			...
			createInstance: (ctor: unknown, workspaceInfo: any, agentName: any, sdkSession: any) => {
				if (ctor === SingularityCLISessionWorkspaceTracker) {
					return new class extends mock<SingularityCLISessionWorkspaceTracker>() {
				...
				const session = new TestSingularityCLISession(workspaceInfo, agentName, sdkSession, [], undefined, logService, workspaceService, new MockChatSessionMetadataStore(), instantiationService, new NullRequestLogger(), new NullISingularityCLIImageSupport(), new FakeToolsService(), new FakeUserQuestionHandler(), accessor.get(IConfigurationService), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new FakeGitService(), { _serviceBrand: undefined } as any, { _serviceBrand: undefined, resetTurnCredits() { }, getCreditsForTurn() { return undefined; }, setLastSingularityUsage() { } } as any, new NullTelemetryService());
		...
		folderRepositoryManager = new SingularityCLIFolderRepositoryManager(
			worktree,
			workspaceFolderService,
			cliSessionServiceForFolderManager as unknown as ISingularityCLISessionService,
			git,
			workspaceService,
			logService,
			tools,
		...
		participant = new SingularityCLIChatSessionParticipant(
			contentProvider,
			promptResolver,
			itemProvider,
			cloudProvider,
			undefined,
			git,
			models as unknown as ISingularityCLIModels,
			new NullSingularityCLIAgents(),
			sessionService,
			worktree,
	...
	it('applies live permissionLevel option changes to an active session', async () => {
		const provider = Object.create(SingularityCLIChatSessionContentProvider.prototype) as SingularityCLIChatSessionContentProvider;
		(provider as unknown as { sessionItemProvider: SingularityCLIChatSessionItemProvider }).sessionItemProvider = itemProvider;
		(provider as unknown as { _activeSessionsById: Map<string, ISingularityCLISession> })._activeSessionsById = new Map<string, ISingularityCLISession>();
		const activeSession = {
		...
		await provider.provideHandleOptionsChange(Uri.parse('singularitycli:/untitled-session'), [
	...
	it('scopes live permissionLevel changes to the targeted session', async () => {
		...
		(provider as unknown as { sessionItemProvider: SingularityCLIChatSessionItemProvider }).sessionItemProvider = itemProvider;
		(provider as unknown as { _activeSessionsById: Map<string, ISingularityCLISession> })._activeSessionsById = new Map<string, ISingularityCLISession>();
		const sessionA = { sessionId: 'sdk-a', setPermissionLevel: vi.fn() } as unknown as ISingularityCLISession;
		const sessionB = { sessionId: 'sdk-b', setPermissionLevel: vi.fn() } as unknown as ISingularityCLISession;
		...
		await provider.provideHandleOptionsChange(Uri.parse('singularitycli:/resource-b'), [
			{ optionId: 'permissionLevel', value: 'autopilot' }
		], disposables.add(new CancellationTokenSource()).token);
	...
	it('clears permissionLevel on an active session when option value is undefined', async () => {
		...
		(provider as unknown as { sessionItemProvider: SingularityCLIChatSessionItemProvider }).sessionItemProvider = itemProvider;
		(provider as unknown as { _activeSessionsById: Map<string, ISingularityCLISession> })._activeSessionsById = new Map<string, ISingularityCLISession>();
		const activeSession = { sessionId: 'sdk-session', setPermissionLevel: vi.fn() } as unknown as ISingularityCLISession;
		itemProvider.untitledSessionIdMapping.set('untitled-session', activeSession.sessionId);
		provider.trackActiveSession('untitled-session', activeSession);
	...
	it('returns early when yield is requested and attaches steering stream while the session is still running', async () => {
		...
		TestSingularityCLISession.handleRequestHook = vi.fn((_request, input) => {
	...
	it('defers worktree handleRequestCompleted until all steering requests complete', async () => {
		...
		TestSingularityCLISession.statusOverride = vscode.ChatSessionStatus.Completed;
		...
		const sessionResource = vscode.Uri.from({ scheme: 'singularitycli', path: `/${sessionId}` });
		const stream = new MockChatResponseStream();
	...
	it('defers untitled session swap while a steering request is still pending', async () => {
		...
		const steeringSessionResource = vscode.Uri.from({ scheme: 'singularitycli', path: '/untitled-temp-steering' });
		const stream = new MockChatResponseStream();
		...
		otherRequest.sessionResource = vscode.Uri.from({ scheme: 'singularitycli', path: `/${otherSessionId}` });
	...
	it('hydrates invalid sessions from partial history and blocks follow-up requests', async () => {
		...
		const invalidSessionService = new class extends FakeSingularityCLISessionService {
			...
			override getChatHistory = vi.fn(async () => {
				throw new Error('Failed to load session. Unknown event type: custom.unknown.');
			}) as unknown as ISingularityCLISessionService['getChatHistory'];
			override createSession = vi.fn(async () => {
				throw new Error('createSession should not be called for invalid sessions');
			});
			override tryGetPartialSessionHistory: ISingularityCLISessionService['tryGetPartialSessionHistory'] = vi.fn(async () => ([{} as unknown as vscode.ChatRequestTurn, {} as unknown as vscode.ChatResponseTurn]));
		}();
		invalidSessionService.setTestSessionWorkingDirectory(sessionId, Uri.file(`${sep}workspace`));
		const invalidContentProvider = new SingularityCLIChatSessionContentProvider(
			itemProvider,
			new NullSingularityCLIAgents(),
		...
		const invalidParticipant = new SingularityCLIChatSessionParticipant(
	...
	it('handles /delegate command from another chat (has uncommitted changes and user copies changes)', async () => {
		...
		expect(delegateCallArgs[1].input.title).toBe('Delegate to Singularity CLI');
	...
	it('returns live response details from the model reported by assistant usage', async () => {
		...
		] as SingularityCLIModelInfo[]);
		TestSingularityCLISession.lastResponseModelId = 'claude-opus-4.7';
		const request = new TestChatRequest('my prompt');
		const context = createChatContext(sessionId, false, request);
		const stream = new MockChatResponseStream();
		const token = disposables.add(new CancellationTokenSource()).token;
	...
	describe('chatSessionContext lost workaround (core bug)', () => {
		...
		//    then calls vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.singularitycli', ...)
		// 3. VS Code core opens the new session and calls handleRequest again with the singularitycli:// resource,
		//    but due to a core bug chatSessionContext may be undefined
		// 4. The workaround detects the singularitycli:// scheme + stored contextForRequest data and
		...
		beforeEach(() => {
			...
			// calling handleRequest again with the singularitycli:// resource but with chatSessionContext lost.
			mockExecuteCommand.mockImplementation(async (command: string, args: any) => {
				if (command === 'workbench.action.chat.openSessionWithPrompt.singularitycli') {
		...
		it('full delegation round-trip: executeCommand triggers callback that uses workaround to reconstruct context and reuse session', async () => {
			...
			// the singularitycli:// resource but chatSessionContext=undefined (the core bug).
			...
			expect(mockExecuteCommand).toHaveBeenCalledWith(
				'workbench.action.chat.openSessionWithPrompt.singularitycli',
				expect.objectContaining({
					resource: expect.objectContaining({ scheme: 'singularitycli' }),
		...
		it('does not attempt workaround for non-singularitycli resource and proceeds with normal delegation', async () => {
			const request = new TestChatRequest('do some work');
			// Default sessionResource is test://session/... (not singularitycli scheme),
	...
	describe('agent tool references via modeInstructions2', () => {
		class MockSingularityCLIAgentsWithCustomAgent extends NullSingularityCLIAgents {
		...
		function makeParticipantWithAgents(agents: MockSingularityCLIAgentsWithCustomAgent): SingularityCLIChatSessionParticipant {
			const nullDelegationService = new class extends mock<IChatDelegationSummaryService>() {
				override async summarize(_context: vscode.ChatContext, _token: vscode.CancellationToken): Promise<string | undefined> {
					return undefined;
				}
			}();
			return new SingularityCLIChatSessionParticipant(
				contentProvider,
				promptResolver,
				itemProvider,
				cloudProvider,
		...
		it('preserves agent tools when modeInstructions2 has no tool references', async () => {
			const agentParticipant = makeParticipantWithAgents(new MockSingularityCLIAgentsWithCustomAgent(['original-tool']));
		...
		it('preserves null tools when modeInstructions2 has no tool references', async () => {
			const agentParticipant = makeParticipantWithAgents(new MockSingularityCLIAgentsWithCustomAgent(null));
			const createSessionSpy = vi.spyOn(sessionService, 'createSession');
		...
		it('does not use session agent when no modeInstructions2 is provided', async () => {
			const agentParticipant = makeParticipantWithAgents(new MockSingularityCLIAgentsWithCustomAgent(['tool-a']));
			const createSessionSpy = vi.spyOn(sessionService, 'createSession');
	...
	describe('PR detection with retry', () => {
		...
		it('skips retry when session already has createdPullRequestUrl', async () => {
			...
			TestSingularityCLISession.handleRequestHook = vi.fn(async () => {
	...
	describe('sdkToUntitledUriMapping lifecycle', () => {
		...
		it('maps SDK session ID to the original untitled URI', async () => {
			...
			expect(capturedUri!.scheme).toBe('singularitycli');
