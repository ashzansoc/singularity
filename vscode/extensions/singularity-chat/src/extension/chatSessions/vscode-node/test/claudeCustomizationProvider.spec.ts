...
describe('ClaudeCustomizationProvider', () => {
	const testSessionResource = URI.parse('claude:///test-session');
	...
	describe('skills from .claude/ paths', () => {
		...
		it('filters out skills not under .claude/', async () => {
			mockPromptsService.setSkills([
				mockSkill(URI.file('/workspace/.github/skills/singularity-skill/SKILL.md'), 'singularity-skill'),
				mockSkill(URI.file('/workspace/.singularity/skills/other/SKILL.md'), 'other-skill'),
			]);
