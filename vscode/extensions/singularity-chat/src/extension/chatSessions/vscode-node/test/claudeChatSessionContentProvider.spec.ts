...
describe('ClaudeChatSessionItemController', () => {
	...
	describe('command handlers', () => {
		it('commit command sends /commit prompt to the session', async () => {
			...
			await findCommandHandler('singularity.chat.claude.sessions.commit')(resource);
		...
		it('commitAndSync command sends combined /commit and /sync prompt', async () => {
			...
			await findCommandHandler('singularity.chat.claude.sessions.commitAndSync')(resource);
		...
		it('sync command sends /sync prompt to the session', async () => {
			...
			await findCommandHandler('singularity.chat.claude.sessions.sync')(resource);
		...
		it('commit command extracts resource from ChatSessionItem', async () => {
			...
			await findCommandHandler('singularity.chat.claude.sessions.commit')(sessionItem);
		...
		it('commands do not execute when resource is undefined', async () => {
			...
			await findCommandHandler('singularity.chat.claude.sessions.commit')(undefined);
			await findCommandHandler('singularity.chat.claude.sessions.commitAndSync')(undefined);
			await findCommandHandler('singularity.chat.claude.sessions.sync')(undefined);
		...
		it('initializeRepository calls gitService.initRepository with workspace folder', async () => {
			...
			const resource = ClaudeSessionUri.forSessionId(sessionId);
			await findCommandHandler('singularity.chat.claude.sessions.initializeRepository')(resource);
