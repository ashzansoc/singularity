...
export class ClaudeChatSessionItemController extends Disposable {
	...
	constructor(
		...
		this._autoPermissionsEnabled = observableFromEvent(
			...
			Event.any(
				...
				_authenticationService.onDidSingularityTokenChange,
			),
			() => _configurationService.getExperimentBasedConfig(ConfigKey.ClaudeAgentAllowAutoPermissions, experimentationService)
				...
				&& (_authenticationService.singularityToken?.isEditorPreviewFeaturesEnabled() ?? true),
		);
	...
	private _registerCommands(): void {
		this._register(vscode.commands.registerCommand('singularity.chat.claude.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		this._registerPromptCommand('singularity.chat.claude.sessions.commit', builtinSlashCommands.commit);
		this._registerPromptCommand('singularity.chat.claude.sessions.commitAndSync', `${builtinSlashCommands.commit} and ${builtinSlashCommands.sync}`);
		this._registerPromptCommand('singularity.chat.claude.sessions.sync', builtinSlashCommands.sync);

		this._register(vscode.commands.registerCommand('singularity.chat.claude.sessions.initializeRepository', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
			const resource = getSessionResource(sessionItemOrResource);
			if (!resource) {
				return;
			}
