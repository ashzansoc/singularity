...
import { CLI_TOOL_EVENT_HANDLERS } from '../singularitycli/common/singularityCLITools';
import { getAuthorDisplayName, isFailedTaskState } from '../vscode/singularityCodingAgentUtils';
...
/**
 * The notice shown for a terminally-stopped task (v2): `Singularity stopped: <reason>`. The reason is
...
export function formatTaskStoppedMessage(state: AgentTaskState, detail: string | undefined): string {
	...
	return vscode.l10n.t('Singularity stopped: {0}', reason);
}
		...
		const sessionResults = await Promise.all(
			sessions.map(async (session, sessionIndex) => {
				...
				if (sessionIndex === 0 && pullRequest.author && vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
					...
					const card = new vscode.ChatResponsePullRequestPart({ command: 'singularity.chat.chat.openPullRequestReroute', title: vscode.l10n.t('View Pull Request {0}', `#${pullRequest.number}`), arguments: [pullRequest.number] }, pullRequest.title, plaintextBody, getAuthorDisplayName(pullRequest.author), `#${pullRequest.number}`);
			...
			turns.forEach((turn, index) => {
				...
				if (index === 0 && pullRequest && pullRequest.author && vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
					const card = new vscode.ChatResponsePullRequestPart(
						{ command: 'singularity.chat.chat.openPullRequestReroute', title: vscode.l10n.t('View Pull Request {0}', `#${pullRequest.number}`), arguments: [pullRequest.number] },
						pullRequest.title,
						pullRequest.body,
				...
				// surface a "Singularity stopped: <reason>" notice instead of a stuck progress spinner —
