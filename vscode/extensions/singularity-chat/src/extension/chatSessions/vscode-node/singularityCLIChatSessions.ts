...
import { clearPendingSingularityCLIRequestContext, setPendingSingularityCLIRequestContext, takePendingSingularityCLIRequestContext } from '../singularitycli/common/pendingRequestContext';
...
import { ISingularityCLIChatSessionInitializer, SessionInitOptions } from '../singularitycli/vscode-node/singularityCLIChatSessionInitializer';
...
export interface ISingularityCLIChatSessionItemProvider extends IDisposable {
...
export class SingularityCLIChatSessionContentProvider extends Disposable implements vscode.ChatSessionContentProvider, ISingularityCLIChatSessionItemProvider {
		...
		if (controller.onDidChangeChatSessionItemState) {
			this._register(controller.onDidChangeChatSessionItemState(async (item) => {
				...
				try {
					...
					this.logService.error(`[SingularityCLI] Failed to persist archived state for session ${sessionId}:`, error);
				}
				if (item.archived) {
					// Skip worktree cleanup if other live sessions still depend on this worktree.
					const worktreePath = await this.singularityCLIWorktreeManagerService.getWorktreePath(sessionId);
					if (worktreePath) {
						...
						if (siblings.length > 0) {
							this.logService.trace(`[SingularityCLI] Skipping worktree cleanup for archived session ${sessionId}: ${siblings.length} other session(s) still use the worktree`);
							return;
						}
					}
					try {
						const result = await this.singularityCLIWorktreeManagerService.cleanupWorktreeOnArchive(sessionId);
						this.logService.trace(`[SingularityCLI] Worktree cleanup for session ${sessionId}: ${result.cleaned ? 'cleaned' : result.reason}`);
					} catch (error) {
						this.logService.error(`[SingularityCLI] Failed to cleanup worktree for archived session ${sessionId}:`, error);
					}
				} else {
					try {
						const result = await this.singularityCLIWorktreeManagerService.recreateWorktreeOnUnarchive(sessionId);
						this.logService.trace(`[SingularityCLI] Worktree recreation for session ${sessionId}: ${result.recreated ? 'recreated' : result.reason}`);
						if (result.recreated) {
							await this.refreshSession({ reason: 'update', sessionId });
						}
					} catch (error) {
						this.logService.error(`[SingularityCLI] Failed to recreate worktree for unarchived session ${sessionId}:`, error);
					}
				}
			}));
		}
	...
	public async toChatSessionItem(session: ISingularityCLISessionItem, options?: { readonly includeChanges?: boolean }, token?: vscode.CancellationToken): Promise<vscode.ChatSessionItem> {
		...
		let worktreeProperties = await raceCancellation(this.singularityCLIWorktreeManagerService.getWorktreeProperties(session.id), token);
		...
		if (options?.includeChanges || ((await this.hasCachedChanges(session.id, worktreeProperties)))) {
			...
			worktreeProperties = await raceCancellation(this.singularityCLIWorktreeManagerService.getWorktreeProperties(session.id), token);
			if (token.isCancellationRequested) {
				return item;
			}
	...
	private async hasCachedChanges(sessionId: string, worktreeProperties: Awaited<ReturnType<IChatSessionWorktreeService['getWorktreeProperties']>>): Promise<boolean> {
		...
		const [hasCachedWorktreeChanges, hasCachedWorkspaceChanges] = await Promise.all([
			this.singularityCLIWorktreeManagerService.hasCachedChanges(sessionId),
	...
	private async buildChanges(
		...
		if (worktreeProperties?.repositoryPath && await vscode.workspace.isResourceTrusted(vscode.Uri.file(worktreeProperties.repositoryPath))) {
			...
			changes.push(...(await raceCancellation(this.singularityCLIWorktreeManagerService.getWorktreeChanges(sessionId), token) ?? []));
	...
	async provideChatSessionContent(resource: Uri, token: vscode.CancellationToken, context?: { readonly inputState: vscode.ChatSessionInputState }): Promise<vscode.ChatSession> {
		...
		try {
			...
			if (singularitycliSessionId.startsWith('untitled:') || singularitycliSessionId.startsWith('untitled-')) {
				return {
					history: [],
					requestHandler: undefined,
				};
			}
			if (this.sessionService.isNewSessionId(singularitycliSessionId)) {
				...
				this._prDetectionService.detectPullRequest(singularitycliSessionId);
				...
				const [history, title, optionGroups] = await Promise.all([
					...
					this.sessionService.getSessionTitle(singularitycliSessionId, token),
...
export class SingularityCLIChatSessionParticipant extends Disposable {
	...
	constructor(
		private readonly sessionItemProvider: ISingularityCLIChatSessionItemProvider,
		...
		@ISingularityCLIChatSessionInitializer private readonly sessionInitializer: ISingularityCLIChatSessionInitializer,
	...
	private async authenticate(): Promise<NonNullable<SessionOptions['authInfo']>> {
		const authInfo = await this.singularityCLISDK.getAuthInfo().catch((ex) => this.logService.error(ex, 'Authorization failed'));
		if (!authInfo) {
	...
	private async resolveInput(
	...
	): Promise<{ input: { prompt: string; command?: SingularityCLICommand; source?: SendOptions['source'] }; attachments: Attachment[] }> {
		const contextForRequest = takePendingSingularityCLIRequestContext(session.sessionId);
		...
		if (request.command && !request.prompt && !isNewSession) {
			const input = (singularityCLICommands as readonly string[]).includes(request.command)
				? { command: request.command as SingularityCLICommand, prompt: '' }
				: { prompt: `/${request.command}` };
			return { input, attachments: [] };
		}

		const { prompt, attachments } = await this.promptResolver.resolvePrompt(request, undefined, [], session.workspace, [], token);
		const input = (request.command && (singularityCLICommands as readonly string[]).includes(request.command))
			? { command: request.command as SingularityCLICommand, prompt }
			: { prompt };
		return { input, attachments };
	}
	...
	private async handleRequestImpl(request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<vscode.ChatResult | void> {
		...
		let session: IReference<ISingularityCLISession> | undefined = undefined;
		...
		try {
			...
			const invalidSessionMessage = _invalidSingularityCLISessionIdsWithErrorMessage.get(sessionId);
			...
			await persistSingularityCLIResponseModelId(sdkSessionId, request.id, responseModelId, model?.model === 'auto', this.chatSessionMetadataStore, this.logService, creditsUsed);
	...
	private async getOrCreateSession(request: vscode.ChatRequest, chatResource: vscode.Uri, options: SessionInitOptions, disposables: DisposableStore, token: vscode.CancellationToken): Promise<{ session: IReference<ISingularityCLISession> | undefined; isNewSession: boolean; model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined; agent: SweCustomAgent | undefined; trusted: boolean }> {
	...
	private async handleDelegationFromAnotherChat(
		...
		setPendingSingularityCLIRequestContext(session.object.sessionId, { prompt, attachments });
		void vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.singularitycli', {
			...
			clearPendingSingularityCLIRequestContext(session.object.sessionId);
			this.logService.error(error, '[SingularityCLIChatSessionContentProvider] Failed to open Singularity CLI session');
		});
...
export function registerCLIChatCommands(
	...
	singularityCLIFolderMruService: IChatFolderMruService,
	envService: INativeEnvService,
	fileSystemService: IFileSystemService,
	sessionTracker: ISingularityCLISessionTracker,
	terminalIntegration: ISingularityCLITerminalIntegration,
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.delete', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			...
			const worktreePath = await singularityCLIWorktreeManagerService.getWorktreePath(id);
		...
		if (sessionItem?.resource) {
			...
			try {
				...
				if (terminal) {
					...
					terminalIntegration.setTerminalSessionDir(terminal, Uri.file(getSingularityCLISessionDir(id)));
				}
			} finally {
				token.dispose();
			}
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		if (newTitle) {
			...
			if (trimmedTitle) {
				await singularityCLISessionService.renameSession(id, trimmedTitle);
	...
	const createSingularityCLITerminal = async (location: TerminalOpenLocation = 'editor', name?: string, cwd?: string): Promise<void> => {
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.newSession', async () => {
		await createSingularityCLITerminal('editor', l10n.t('Singularity CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.newSessionToSide', async () => {
		await createSingularityCLITerminal('editorBeside', l10n.t('Singularity CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand(OPEN_IN_SINGULARITY_CLI_COMMAND_ID, async (sourceControlContext?: unknown) => {
		...
		await createSingularityCLITerminal('editor', l10n.t('Singularity CLI'), rootUri?.fsPath);
	}));
	...
	disposableStore.add(vscode.commands.registerCommand('singularity.chat.cli.sessions.copyWorktreeBranchName', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		const id = SessionIdForCLI.parse(sessionItem.resource);
		const worktreeProperties = await singularityCLIWorktreeManagerService.getWorktreeProperties(id);
	...
	disposableStore.add(vscode.commands.registerCommand(OPEN_REPOSITORY_COMMAND_ID, async ({ inputState }: { inputState: vscode.ChatSessionInputState; sessionResource: vscode.Uri | undefined }) => {
		...
		const mruItems = await singularityCLIFolderMruService.getRecentlyUsedFolders(CancellationToken.None);
		...
		if (!(await checkPathExists(selectedFolderUri, fileSystemService))) {
			await singularityCLIFolderMruService.deleteRecentlyUsedFolder(selectedFolderUri);
