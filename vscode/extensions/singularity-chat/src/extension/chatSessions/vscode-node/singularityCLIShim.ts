...
// in singularityCLITerminalIntegration.ps1 instead. This is because Electron on Windows
// is not built with support for console stdin.
// ⚠️⚠️⚠️

/*
 * Universal Singularity CLI bootstrapper
 *
...
/**
 * Returns the version of Singularity CLI installed.
 ...
 */
function getSingularityInfo(): { installed: true; version?: string } | undefined {
	const result = spawnSync('singularity --version', { env, shell: true, encoding: 'utf8' });
...
function runBrew(label: string) {
	const result = spawnSync('brew', ['install', 'singularity-cli'], { stdio: 'inherit', env });
	if (result.error) {
...
function runCurl(label: string) {
	const result = spawnSync('bash', ['-c', 'curl -fsSL https://gh.io/singularity-install | bash'], { stdio: 'inherit', env });
	if (result.error) {
...
function runWget(label: string) {
	const result = spawnSync('bash', ['-c', 'wget -qO- https://gh.io/singularity-install | bash'], { stdio: 'inherit', env });
	if (result.error) {
...
function installSingularityCLI(label: string, update = false): boolean {
	// Try npm first
...
async function ensureInstalled() {
	const version = getSingularityInfo();
	if (!version) {
		warn('Cannot find Singularity CLI (https://docs.github.com/en/singularity/how-tos/set-up/install-singularity-cli)');
		if (await promptYes('Install Singularity CLI?')) {
			if (installSingularityCLI('Installing')) {
...
async function validateVersion(version: string) {
	if (!versionGte(version, REQUIRED_VERSION)) {
		warn(`Singularity CLI version ${version} is not compatible.`);
		log(`Version ${REQUIRED_VERSION} or later is required.`);
		if (await promptYes('Update Singularity CLI?')) {
			if (installSingularityCLI('Update', true)) {
				return true;
			}
			await pressKeyToExit();
		} else {
...
(async function main() {
	...
	if (!info) {
		warn('Error: Could not locate Singularity CLI after update.');
		await pressKeyToExit('Try manually reinstalling (https://docs.github.com/en/singularity/how-tos/set-up/install-singularity-cli)');
	}
	...
	spawnSync('singularity', args, { stdio: 'inherit', env });
	process.exit(0);
})();
