 ...
 */
export const UNTRUSTED_FOLDER_MESSAGE = l10n.t('The selected folder is not trusted. Please trust the folder to continue with the {0}.', 'Singularity CLI');
...
export abstract class FolderRepositoryManager extends Disposable implements IFolderRepositoryManager {
	...
	private async promptForUncommittedChangesAction(
		...
		const title = isDelegation
			? l10n.t('Delegate to Singularity CLI')
			: l10n.t('Uncommitted Changes');
		const message = isDelegation
			? l10n.t('Singularity CLI will work in an isolated worktree to implement your requested changes.')
			+ '\n\n'
...
// #region SingularityCLIFolderRepositoryManager

/**
 * SingularityCLI-specific implementation that resolves folder information for
 * existing sessions using the CLI session service as a fallback.
 */
export class SingularityCLIFolderRepositoryManager extends FolderRepositoryManager {
	constructor(
