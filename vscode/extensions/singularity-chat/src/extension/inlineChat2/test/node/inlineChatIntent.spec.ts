...
import { SingularityInteractiveEditorResponse } from '../../../inlineChat/node/promptCraftingTypes';
...
import { SingularityChatAttr, GenAiAttr, GenAiOperationName, SingularityAttr } from '../../../../platform/otel/common/genAiAttributes';
...
suite('InlineChatIntent', () => {
	...
	test('Metadata is set on the latest turn', async () => {
		...
		const mockIgnoreService = {
			isSingularityIgnored: vi.fn().mockResolvedValue(false)
		...
		expect(metadata).toBeInstanceOf(SingularityInteractiveEditorResponse);
		...
		expect(harness.otelService.spans).toEqual([
			expect.objectContaining({
				...
				attributes: expect.objectContaining({
					...
					[SingularityChatAttr.SESSION_ID]: 'someId',
					[SingularityChatAttr.USER_REQUEST]: 'test prompt',
					[SingularityChatAttr.TURN_COUNT]: 1,
	...
	test('Inline tool loop aggregates tokens, tool rounds, and metrics across recovered tool failures', async () => {
		...
		expect({
			...
			rootSpan: {
				statusCode: SpanStatusCode.OK,
				attributes: expect.objectContaining({
					[SingularityChatAttr.TURN_COUNT]: 2,
		...
		expect(harness.otelService.metrics).toEqual(expect.arrayContaining([
			{
				name: 'singularity_chat.agent.invocation.duration',
				value: expect.closeTo(0.5, 2),
				attributes: { [GenAiAttr.AGENT_NAME]: 'Inline Chat' },
			},
			{
				name: 'singularity_chat.agent.turn.count',
				value: 2,
				attributes: { [GenAiAttr.AGENT_NAME]: 'Inline Chat' },
			},
		]));
...
function createInlineChatHarness(options: InlineChatHarnessOptions = {}) {
		...
		const mockEndpoint = {
			...
			makeChatRequest2: vi.fn().mockImplementation(async (requestOptions: { finishedCb?: (text: string, index: number, delta: { singularityToolCalls?: readonly TestToolCall[] }) => Promise<undefined> }) => {
				...
				if (response.toolCalls) {
					await requestOptions.finishedCb?.('', 0, { singularityToolCalls: response.toolCalls });
				}
				return {
