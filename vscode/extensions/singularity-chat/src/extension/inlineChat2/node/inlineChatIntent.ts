...
import { SingularityChatAttr, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiProviderName, SingularityAttr, normalizeResponseModel, StdAttr, stringifyToolDefinitionsForOTel, truncateForOTel } from '../../../platform/otel/common/index';
...
import { SingularityToolMode } from '../../tools/common/toolsRegistry';
...
import { SingularityInteractiveEditorResponse, InteractionOutcome } from '../../inlineChat/node/promptCraftingTypes';
...
export class InlineChatIntent implements IIntent {
	...
	async handleRequest(conversation: Conversation, request: vscode.ChatRequest, stream: vscode.ChatResponseStream, token: CancellationToken, documentContext: IDocumentContext | undefined, _agentName: string, _location: ChatLocation, chatTelemetry: ChatTelemetryBuilder): Promise<vscode.ChatResult> {
		...
		if (await this._ignoreService.isSingularityIgnored(request.location2.document.uri, token)) {
			return {
				errorDetails: {
					message: l10n.t('inlineChat.ignored', 'Singularity is disabled for this file.'),
				}
			};
		}
		...
		turn.setMetadata(new SingularityInteractiveEditorResponse(
			undefined,
		...
		if (result.lastResponse.type !== ChatFetchResponseType.Success) {
			...
			const details = getErrorDetailsFromChatFetchError(result.lastResponse, this._authenticationService.singularityToken?.singularityPlan, outageStatus, this._authenticationService.singularityToken?.tokenBasedBilling, this._authenticationService.singularityToken?.quotaInfo.quota_reset_date);
			return {
				errorDetails: {
		...
		return this._otelService.startActiveSpan(
			'invoke_agent Inline Chat',
			{
				...
				attributes: {
					...
					[SingularityChatAttr.SESSION_ID]: conversation.sessionId,
					[SingularityChatAttr.INTENT]: this._intent.id,
					[SingularityChatAttr.LOCATION]: ChatLocation.toStringShorter(ChatLocation.Editor),
					[SingularityChatAttr.USER_REQUEST]: truncateForOTel(request.prompt, this._otelService.config.maxAttributeSizeChars),
					[SingularityAttr.AGENT_TYPE]: 'builtin',
				},
			},
			async span => {
	...
	private async _makeRequestAndRunTools(endpoint: IChatEndpoint, request: vscode.ChatRequest, stream: vscode.ChatResponseStream, messages: Raw.ChatMessage[], inlineChatTools: vscode.LanguageModelToolInformation[], telemetry: InlineChatTelemetry, token: CancellationToken) {
		...
		const fetchResult = await endpoint.makeChatRequest2({
			...
			finishedCb: async (_text, _index, delta) => {
				...
				if (!isNonEmptyArray(delta.singularityToolCalls)) {
					return undefined;
				}

				const exitToolCall = delta.singularityToolCalls.find(candidate => candidate.name === INLINE_CHAT_EXIT_TOOL_NAME);
				const singularityToolCalls = exitToolCall ? [exitToolCall] : delta.singularityToolCalls;

				for (const toolCall of singularityToolCalls) {
					...
					toolExecutions.push((async () => {
						try {
							...
							const singularityTool = this._toolsService.getSingularityTool(toolCall.name as ToolName);
							if (singularityTool?.resolveInput) {
								input = await singularityTool.resolveInput(input, {
									request,
									stream,
								...
								}, SingularityToolMode.FullContext);
							}
