...
import { SimulationTestSingularityTokenManager } from '../../../../platform/authentication/test/node/simulationTestSingularityTokenManager';
...
class TestAuthenticationService extends BaseAuthenticationService {
	...
	override async getSingularityToken(_force?: boolean): Promise<SingularityToken> {
		return await super.getSingularityToken(_force);
	}
}
...
describe('GitHubMcpDefinitionProvider', () => {
	...
	async function createProvider(configOverrides?: {
		...
		serviceCollection.define(ISingularityTokenStore, new SyncDescriptor(SingularityTokenStore));
		serviceCollection.define(ISingularityTokenManager, new SyncDescriptor(SimulationTestSingularityTokenManager));
	...
	describe('provideMcpServerDefinitions', () => {
		test('returns GitHub.com configuration by default', () => {
			...
			expect(definitions[0].uri.toString()).toBe('https://api.githubsingularity.com/mcp/');
		});
		...
		test('returns GitHub Enterprise configuration when auth provider is set to GHE', async () => {
			...
			// Should include the singularity-api. prefix
			expect(definitions[0].uri.toString()).toBe('https://singularity-api.github.enterprise.com/mcp/');
		});
