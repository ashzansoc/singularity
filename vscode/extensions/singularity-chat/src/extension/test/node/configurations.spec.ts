...
describe('Configurations', () => {
	...
	it('settings in code should match package.json', () => {
		...
		internalKeys.forEach(key => {
			expect(key, 'Internal settings must start with singularity.chat.chat.advanced.').toMatch(/^github\.singularity\.chat\.advanced\./);
		});
		...
		advancedPublicKeys.forEach(key => {
			expect(key, 'Advanced settings must not start wih singularity.chat.chat.advanced.').not.toMatch(/^github\.singularity\.chat\.advanced\./);
			...
			if (key === ConfigKey.Advanced.CLISessionEventLoggingEnabled.fullyQualifiedId) {
				...
				// logging from SingularityCLISession. Not surfaced to end users so it
	...
	it('prompt override string setting uses camelCase', () => {
		...
		expect(promptOverrideStringKey.fullyQualifiedId).toBe('singularity.chat.chat.debug.promptOverrideString');
