...
suite('Singularity Chat Sanity Test', function () {
	...
	suiteSetup(async function () {
		...
		const extension = vscode.extensions.getExtension('Github.singularity-chat');
		...
		// Warm the Singularity token cache so the first chat request does not pay that cost.
		await realInstaAccessor.invokeFunction(async (accessor) => {
			const token = await accessor.get(IAuthenticationService).getSingularityToken();
			assert.ok(token, 'Singularity token must be available before tests run');
		});
	});
