...
class FakeModelMetadataFetcher implements IModelMetadataFetcher {
	...
	async getSingularityUtilityModel(): Promise<IChatModelInformation> {
		return this._fakeChatModel('singularity-utility');
	}
...
class SingularityMatchableModelMetadataFetcher implements IModelMetadataFetcher {
	public onDidModelsRefresh = Event.None;
	...
	async getSingularityUtilityModel(): Promise<IChatModelInformation> {
		return makeChatModel('singularity-utility');
	}
	async getChatModelFromCapiFamily(family: string): Promise<IChatModelInformation> {
...
suite('ProductionEndpointProvider — utility model overrides', () => {
	...
	function setFetcher(models: IChatModelInformation[]): void {
		...
		endpointProvider._modelFetcher = new SingularityMatchableModelMetadataFetcher(models);
	}
	...
	test('no override configured — falls through to default singularity-utility resolution', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
		const endpoint = await endpointProvider.getChatEndpoint('singularity-utility');
		assert.strictEqual(endpoint.model, 'singularity-utility');
	});

	test('no override configured — uses the Singularity utility model when the selected main agent model is BYOK and a Singularity token is available', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
	...
	test('no override configured — rejects when the selected main agent model is BYOK and no Singularity token is available (air-gapped)', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
		// Simulate a signed-out / air-gapped BYOK session with no Singularity token source.
		// @ts-expect-error — access the protected auth service to stub its token-source signal.
		sandbox.stub(endpointProvider._authService, 'hasSingularityTokenSource').get(() => false);
		await endpointProvider.getChatEndpoint(makeFakeLanguageModelChat({ vendor: 'anthropic' }));

		await assert.rejects(
			() => endpointProvider.getChatEndpoint('singularity-utility'),
			/No utility model is configured/
		);
	});

	test('Singularity default applies when the selected main agent model is BYOK', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
		await endpointProvider.getChatEndpoint(makeFakeLanguageModelChat({ vendor: 'anthropic' }));
		await configService.setNonExtensionConfig('chat.byokUtilityModelDefault', 'singularity');
	...
	test('main agent default uses the selected BYOK model for utility families', async () => {
		...
		const utilityEndpoint = await endpointProvider.getChatEndpoint('singularity-utility');
		const smallUtilityEndpoint = await endpointProvider.getChatEndpoint('singularity-utility-small');
	...
	test('BYOK utility default does not replace utility models for a Singularity main agent model', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
		await endpointProvider.getChatEndpoint(makeFakeLanguageModelChat({ vendor: 'singularity', id: 'gpt-5' }));
		await configService.setNonExtensionConfig('chat.byokUtilityModelDefault', 'mainAgent');

		const endpoint = await endpointProvider.getChatEndpoint('singularity-utility');

		assert.ok(endpoint instanceof SingularityChatEndpoint);
		assert.strictEqual(endpoint.model, 'singularity-utility');
	});
	...
	test('main agent default follows changes to the selected BYOK model', async () => {
		...
		const firstEndpoint = await endpointProvider.getChatEndpoint('singularity-utility');

		await endpointProvider.getChatEndpoint(makeFakeLanguageModelChat({ vendor: 'customendpoint', id: 'gemma4' }));
		const secondEndpoint = await endpointProvider.getChatEndpoint('singularity-utility');
	...
	test('singularity-vendor override resolves to the matching model from the model fetcher', async () => {
		setFetcher([makeChatModel('singularity-utility'), makeChatModel('gpt-4o-mini')]);
		await configService.setNonExtensionConfig('chat.utilityModel', 'singularity/gpt-4o-mini');
	...
	test('singularity-vendor override falls back to default when no singularity model matches', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
		await configService.setNonExtensionConfig('chat.utilityModel', 'singularity/gpt-4o-mini');
	...
	test('singularity-vendor override falls back to default when more than one singularity model matches (ambiguous)', async () => {
		setFetcher([
			makeChatModel('singularity-utility'),
	...
	test('configuration change fires onDidModelsRefresh so cached endpoints get invalidated', async () => {
		...
		try {
			await configService.setNonExtensionConfig('chat.utilityModel', 'singularity/gpt-4o-mini');
			await configService.setNonExtensionConfig('chat.utilitySmallModel', 'singularity/gpt-4o-mini');
			await configService.setNonExtensionConfig('chat.byokUtilityModelDefault', 'mainAgent');
	...
	test('non-singularity vendor override resolves to an extension-contributed endpoint', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
		const fakeModel = makeFakeLanguageModelChat({ vendor: 'anthropic', id: 'claude-haiku-4.5' });
		sandbox.stub(lm, 'selectChatModels').resolves([fakeModel]);
		await configService.setNonExtensionConfig('chat.utilityModel', 'anthropic/claude-haiku-4.5');
	...
	test('non-singularity vendor override falls back when lm.selectChatModels returns no matches', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
	...
	test('non-singularity vendor override falls back when lm.selectChatModels returns multiple matches (ambiguous)', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
	...
	test('non-singularity vendor override falls back when lm.selectChatModels throws', async () => {
		setFetcher([makeChatModel('singularity-utility')]);
	...
	test('arbitrary CAPI model family resolves to that model (execution/search subagent override)', async () => {
		...
		setFetcher([makeChatModel('singularity-utility'), makeChatModel(subagentModel)]);
