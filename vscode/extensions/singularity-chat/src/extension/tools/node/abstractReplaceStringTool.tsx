...
import { ISingularityTool } from '../common/toolsRegistry';
...
export abstract class AbstractReplaceStringTool<T extends { explanation: string }> implements ISingularityTool<T> {
		...
		try {
			...
			try {
				healed = await healReplaceStringParams(
					...
					eol,
					await this.endpointProvider.getChatEndpoint('singularity-utility-small'),
					token
				);
