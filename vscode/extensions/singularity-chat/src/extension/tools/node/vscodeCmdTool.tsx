...
const noConfirmationCommandsWithoutArgs = new Set([
	'singularity.chat.debug.collectDiagnostics',
	'breadcrumbs.toggle',
	...
	'git.stashStaged',
	'git.stashView',
	'singularity.chat.chat.completions.disable',
	'singularity.chat.chat.completions.enable',
	'singularity.chat.chat.completions.toggle',
	'singularity.chat.chat.tools.memory.showMemories',
	'singularity.chat.open.walkthrough',
	'singularity.chat.report',
...
class VSCodeCmdTool implements vscode.LanguageModelTool<IVSCodeCmdToolToolInput> {
	...
	async prepareInvocation(options: vscode.LanguageModelToolInvocationPrepareOptions<IVSCodeCmdToolToolInput>, token: vscode.CancellationToken): Promise<vscode.PreparedToolInvocation> {
		...
		// Populate the Quick Open box with command ID rather than command name to avoid issues where Singularity didn't use the precise name,
		// or when the Singularity response language (Spanish, French, etc.) might be different here than the UI one.
		const commandStr = commandUri(quickOpenCommand, ['>' + commandId]);
		const hasArguments = !!options.input.args?.length;
		const markdownString = new MarkdownString();
		markdownString.appendMarkdown(l10n.t(`Singularity will execute the [{0}]({1}) (\`{2}\`) command.`, options.input.name, commandStr, options.input.commandId));
		if (hasArguments) {
