...
import { SingularityToolMode, ISingularityTool, ISingularityToolCtor, ToolRegistry } from '../common/toolsRegistry';
...
export interface ISearchSubagentParams {
	 ...
	 * Only active when config.singularity.chat.chat.searchSubagent.thoroughnessEnabled is true.
	 */
...
class SearchSubagentTool implements ISingularityTool<ISearchSubagentParams> {
	...
	async resolveInput(input: ISearchSubagentParams, promptContext: IBuildPromptContext, _mode: SingularityToolMode): Promise<ISearchSubagentParams> {
...
ToolRegistry.registerTool(ExploreSubagentTool as unknown as ISingularityToolCtor);
