...
import { assertPathNotOwnedByExecutionSubagent } from './executionPathGuard';
...
export class CreateFileTool implements ISingularityTool<ICreateFileParams> {
	...
	async invoke(options: vscode.LanguageModelToolInvocationOptions<ICreateFileParams>, token: vscode.CancellationToken) {
		await assertPathNotOwnedByExecutionSubagent(options.input.filePath, this._promptContext);
