...
import { ISingularityTool, ISingularityToolCtor, ToolRegistry } from '../../common/toolsRegistry';
...
export class TestToolsService extends BaseToolsService implements IToolsService {
	...
	get tools(): LanguageModelToolInformation[] {
		return Array.from(this._tools.values()).map(tool => {
			const owned = this._singularityTools.get(getToolName(tool.name) as ToolName);
			return owned?.value.alternativeDefinition?.(tool) ?? tool;
		});
	}

	private readonly _singularityTools: Map<ToolName, Lazy<ISingularityTool<unknown>>>;
	get singularityTools() {
		return new Map(Iterable.map(this._singularityTools.entries(),
	...
	constructor(
		...
		this._singularityTools = new Map(filteredTools
	...
	protected getFilteredTools(disabledTools: Set<string>): readonly ISingularityToolCtor[] {
	...
	async invokeTool(contributedName: string, options: vscode.LanguageModelToolInvocationOptions<unknown>, token: CancellationToken): Promise<LanguageModelToolResult2> {
		...
		const tool = this._singularityTools.get(name as ToolName)?.value || this.getModelSpecificTools().get(contributedName)?.tool;
	...
	override getSingularityTool(name: string): ISingularityTool<unknown> | undefined {
		const tool = this._singularityTools.get(name as ToolName)?.value;
		return tool;
	}
	...
	addTestToolOverride(info: LanguageModelToolInformation, tool: vscode.LanguageModelTool<unknown>): void {
		this._tools.set(info.name, info);
		this._singularityTools.set(info.name as ToolName, new Lazy(() => tool));
	}
}
...
export class NoopTestToolsService extends TestToolsService {
	...
	protected override getFilteredTools(_disabledTools: Set<string>): readonly ISingularityToolCtor[] {
