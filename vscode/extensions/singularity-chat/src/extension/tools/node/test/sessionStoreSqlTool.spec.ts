...
import type { ISingularityTokenManager } from '../../../../platform/authentication/common/singularityTokenManager';
...
function createMockServices() {
	...
	const tokenManager: ISingularityTokenManager = {
		_serviceBrand: undefined as any,
		getSingularityToken: vi.fn(async () => ({ token: 'test-token', endpoints: { api: 'https://api.test.com' } })),
	} as any;
...
describe('SessionStoreSqlTool', () => {
	...
	describe('schema-error regression anchors', () => {
		...
		it('SQLite modelDescription in package.json carries required anchors', () => {
			const pkg = JSON.parse(fs.readFileSync(path.join(singularityRoot, 'package.json'), 'utf-8'));
			const entry = (pkg.contributes.languageModelTools as { name: string; modelDescription?: string }[])
				.find(t => t.name === 'singularity_sessionStoreSql');
			expect(entry?.modelDescription, 'singularity_sessionStoreSql entry missing modelDescription').toBeDefined();
		...
		it('chronicle slash prompts reference the chronicle skill', () => {
			const promptDir = path.join(singularityRoot, 'assets', 'prompts');
		...
		it('chronicle SKILL.md Cost Tips section carries required anchors', () => {
			const skill = fs.readFileSync(
				path.join(singularityRoot, 'assets', 'prompts', 'skills', 'chronicle', 'SKILL.md'),
				'utf-8',
			);
			const required = [
				'### Cost Tips',
