...
import { SingularityToolMode } from '../../common/toolsRegistry';
...
suite('FindFiles', () => {
	...
	suite('resolveInput', () => {
		...
		async function testIt(input: IFindFilesToolParams, context: SingularityToolMode) {
			const tool = accessor.get(IInstantiationService).createInstance(FindFilesTool);
		...
		test('resolveInput with FullContext and no maxResults', async () => {
			await testIt({ query: 'hello' }, SingularityToolMode.FullContext);
		});

		test('resolveInput with FullContext and maxResults < 200', async () => {
			await testIt({ query: 'hello', maxResults: 50 }, SingularityToolMode.FullContext);
		});

		test('resolveInput with FullContext and maxResults > 200', async () => {
			await testIt({ query: 'hello', maxResults: 300 }, SingularityToolMode.FullContext);
		});

		test('resolveInput with PartialContext and no maxResults', async () => {
			await testIt({ query: 'hello' }, SingularityToolMode.PartialContext);
		});

		test('resolveInput with PartialContext and maxResults defined', async () => {
			await testIt({ query: 'hello', maxResults: 123 }, SingularityToolMode.PartialContext);
		});
	});
});
...
suite('FindFiles - absolute workspace folder path', () => {
	...
	test('absolute path survives resolveInput without corruption', async () => {
		...
		const resolved = await tool.resolveInput({ query: folder1 }, null!, SingularityToolMode.PartialContext);
	...
	test('absolute path with glob survives resolveInput without corruption', async () => {
		...
		const resolved = await tool.resolveInput({ query: `${folder1}/**/*.ts` }, null!, SingularityToolMode.PartialContext);
		await tool.invoke({ input: resolved, toolInvocationToken: null! }, CancellationToken.None);
