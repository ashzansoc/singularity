...
export class NewNotebookTool implements ISingularityTool<IBuildPromptContext> {
	...
	async invoke(options: vscode.LanguageModelToolInvocationOptions<IBuildPromptContext>, token: CancellationToken): Promise<LanguageModelToolResult> {
		...
		try {
			// Get the endpoint
			const planningEndpoint = await this.endpointProvider.getChatEndpoint(options.model || 'singularity-utility');
	...
	async resolveInput(input: IBuildPromptContext, promptContext: IBuildPromptContext, mode: SingularityToolMode): Promise<IBuildPromptContext> {
