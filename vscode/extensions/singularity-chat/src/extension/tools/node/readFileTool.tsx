...
import { ISingularityTool, ToolRegistry } from '../common/toolsRegistry';
...
export class ReadFileTool implements ISingularityTool<ReadFileParams> {
	...
	async prepareInvocation(options: vscode.LanguageModelToolInvocationPrepareOptions<ReadFileParams>, token: vscode.CancellationToken): Promise<vscode.PreparedToolInvocation | undefined> {
		...
		try {
			...
			if (isExternal) {
				// Still check content exclusion (singularity ignore)
				...
				return {
					invocationMessage: new MarkdownString(l10n.t`Exploring ${formatUriForFileWidget(uri)}`),
					pastTenseMessage: new MarkdownString(l10n.t`Explored ${formatUriForFileWidget(uri)}`),
			...
			try {
				...
				if (String(e).includes('seems to be binary')) {
					return {
						invocationMessage: new MarkdownString(l10n.t`Exploring binary file ${formatUriForFileWidget(uri)}`),
						pastTenseMessage: new MarkdownString(l10n.t`Explored binary file ${formatUriForFileWidget(uri)}`),
					};
				}
		...
		if (start === 1 && end === documentSnapshot.lineCount) {
			if (skillInfo) {
				...
				if (this.customInstructionsService.isSkillMdFile(uri)) {
					return {
						invocationMessage: new MarkdownString(l10n.t`Exploring skill ${formatUriForFileWidget(uri, { vscodeLinkType: 'skill', linkText: skillName })}`),
						pastTenseMessage: new MarkdownString(l10n.t`Explored skill ${formatUriForFileWidget(uri, { vscodeLinkType: 'skill', linkText: skillName })}`),
					};
				} else {
					return {
						invocationMessage: new MarkdownString(l10n.t`Exploring skill \`${skillName}\`: ${formatUriForFileWidget(uri)}`),
						pastTenseMessage: new MarkdownString(l10n.t`Explored skill \`${skillName}\`: ${formatUriForFileWidget(uri)}`),
					};
				}
			}
			return {
		...
		if (this.customInstructionsService.isSkillFile(uri)) {
			if (skillInfo) {
				...
				if (this.customInstructionsService.isSkillMdFile(uri)) {
					return {
						invocationMessage: new MarkdownString(l10n.t`Exploring skill ${formatUriForFileWidget(readLocation, { vscodeLinkType: 'skill', linkText: skillName })}, lines ${start} to ${end}`),
						pastTenseMessage: new MarkdownString(l10n.t`Explored skill ${formatUriForFileWidget(readLocation, { vscodeLinkType: 'skill', linkText: skillName })}, lines ${start} to ${end}`),
					};
				} else {
					return {
						invocationMessage: new MarkdownString(l10n.t`Exploring skill \`${skillName}\`: ${formatUriForFileWidget(readLocation)}, lines ${start} to ${end}`),
						pastTenseMessage: new MarkdownString(l10n.t`Explored skill \`${skillName}\`: ${formatUriForFileWidget(readLocation)}, lines ${start} to ${end}`),
					};
				}
			}
		}
		return {
			invocationMessage: new MarkdownString(l10n.t`Exploring ${formatUriForFileWidget(readLocation)}, lines ${start} to ${end}`),
			pastTenseMessage: new MarkdownString(l10n.t`Explored ${formatUriForFileWidget(readLocation)}, lines ${start} to ${end}`),
		};
	}
...
class ReadFileResult extends PromptElement<ReadFileResultProps> {
	...
	override async render() {
		// Only check content exclusion (singularity ignore) - external file confirmation was already handled in prepareInvocation
