...
import { SingularityChatAttr, emitToolCallEvent, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiToolType, StdAttr, truncateForOTel } from '../../../platform/otel/common/index';
...
import { ISingularityTool, ISingularityToolExtension, modelSpecificToolApplies, ToolRegistry } from '../common/toolsRegistry';
import { BaseToolsService } from '../common/toolsService';

export class ToolsService extends BaseToolsService {
	declare _serviceBrand: undefined;

	private readonly _singularityTools: Lazy<Map<ToolName, ISingularityTool<unknown>>>;

	// Extensions to override definitions for existing tools.
	private readonly _toolExtensions: Lazy<Map<ToolName, ISingularityToolExtension<unknown>>>;
	...
	get tools(): ReadonlyArray<vscode.LanguageModelToolInformation> {
		...
		const contributedTools = [...input]
			.sort((a, b) => {
				...
				const aIsBuiltin = a.name.startsWith('vscode_') || a.name.startsWith('singularity_');
				const bIsBuiltin = b.name.startsWith('vscode_') || b.name.startsWith('singularity_');
			...
			.map(tool => {
				const owned = this.getSingularityTool(getToolName(tool.name));
	...
	public get singularityTools() {
		return this._singularityTools.value;
	}
	...
	constructor(
		...
		super(logService);
		this._singularityTools = new Lazy(() => new Map(ToolRegistry.getTools().map(t => [t.toolName, _instantiationService.createInstance(t)] as const)));
	...
	invokeTool(name: string | ToolName, options: vscode.LanguageModelToolInvocationOptions<Object>, token: vscode.CancellationToken): Thenable<vscode.LanguageModelToolResult | vscode.LanguageModelToolResult2> {
		...
		const span = this._otelService.startSpan(`execute_tool ${name}`, {
			...
			attributes: {
				...
				...(chatSessionId ? { [SingularityChatAttr.SESSION_ID]: chatSessionId } : {}),
		...
		// Structured `singularity.chat.tool.parameters.*`. Hashes and edit_type emit
	...
	override getSingularityTool(name: string): ISingularityTool<unknown> | undefined {
		return this._singularityTools.value.get(name as ToolName) || this.getModelSpecificTools().get(name)?.tool;
	}
	...
	getEnabledTools(request: vscode.ChatRequest, endpoint: IChatEndpoint, filter?: (tool: vscode.LanguageModelToolInformation) => boolean | undefined): vscode.LanguageModelToolInformation[] {
		...
		return tools
			...
			.map(tool => {
				...
				const owned = override?.tool || this.getSingularityTool(toolName);
