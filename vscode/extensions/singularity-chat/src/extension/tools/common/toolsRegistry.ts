...
export enum SingularityToolMode {
	/**
...
export interface ISingularityToolExtension<T> {
	/**
	 ...
	 * provided, Singularity will ask the model to generate the necessary input.
	 */
	 ...
	 */
	resolveInput?(input: T, promptContext: IBuildPromptContext, mode: SingularityToolMode): Promise<T>;
	 ...
	 * the Singularity extension, not other extensions' usages via `vscode.lm.tools`.
	 *
...
export interface ISingularityTool<T> extends ISingularityToolExtension<T> {
	invoke?: vscode.LanguageModelTool<T>['invoke'];
	prepareInvocation?: vscode.LanguageModelTool<T>['prepareInvocation'];
}

export interface ISingularityModelSpecificTool<T> extends ISingularityTool<T> {
	/**
...
export function isVscodeLanguageModelTool(tool: ISingularityTool<unknown>): tool is vscode.LanguageModelTool<unknown> {
	return typeof tool.invoke === 'function';
}

export interface ISingularityToolCtor {
	...
	new(...args: never[]): ISingularityTool<unknown>;
}

export interface IModelSpecificToolCtor {
	new(...args: never[]): ISingularityModelSpecificTool<unknown>;
}

export interface ISingularityToolExtensionCtor extends IModelSpecificToolCtor {
	readonly toolName: ToolName;
	new(...args: never[]): ISingularityToolExtension<unknown>;
}

export const ToolRegistry = new class {
	private _tools: Array<ISingularityToolCtor> = [];
	private _toolExtensions: Array<ISingularityToolExtensionCtor> = [];
	...
	public registerTool(tool: ISingularityToolCtor) {
	...
	public getTools(): readonly ISingularityToolCtor[] {
		return this._tools;
	}
	...
	public registerToolExtension(tool: ISingularityToolExtensionCtor) {
	...
	public getToolExtensions(): readonly ISingularityToolExtensionCtor[] {
