...
import { ISingularityModelSpecificTool, ISingularityTool } from './toolsRegistry';
...
export interface IToolsService {
	 ...
	 */
	singularityTools: ReadonlyMap<ToolName, ISingularityTool<unknown>>;

	/**
	 * Model-specific tool instances. These are NOT included in the
	 * {@link singularityTools} map, and may update at runtime.
	 */
	modelSpecificTools: IObservable<{ definition: vscode.LanguageModelToolDefinition; tool: ISingularityTool<unknown> }[]>;

	getSingularityTool(name: string): ISingularityTool<unknown> | undefined;
...
export abstract class BaseToolsService extends Disposable implements IToolsService {
	...
	abstract singularityTools: ReadonlyMap<ToolName, ISingularityTool<unknown>>;
	...
	protected readonly _modelSpecificTools = new ObservableMap</* tool name */string, { definition: vscode.LanguageModelToolDefinition; tool: ISingularityModelSpecificTool<unknown> }>();
	public get modelSpecificTools() {
		return this._modelSpecificTools.observable.map(v => [...v.values()]);
	}

	abstract getSingularityTool(name: string): ISingularityTool<unknown> | undefined;
