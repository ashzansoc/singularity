...
export class PromptFileContextContribution extends Disposable {
	...
	private async register(): Promise<IDisposable> {
		...
		try {
			...
			const resolver: Singularity.ContextResolver<Singularity.SupportedContextItem> = {
				async resolve(request: Singularity.ResolveRequest, token: vscode.CancellationToken): Promise<Singularity.SupportedContextItem[]> {
			...
			if (singularityAPI) {
				disposables.add(singularityAPI.registerContextProvider(provider));
			}
	...
	private getContext(languageId: string): Singularity.SupportedContextItem[] {
			...
			case PromptFileLangageId.agent: {
				...
				return [
					...
					},
					{
						...
						value: `vscode, github-singularity`,
					},
					{
						name: '`handoffs` is optional and is a sequence of mappings with `label`, `agent`, `prompt`, `send`, and `model` properties. The `model` property uses the format `Model Name (vendor)` (e.g., `GPT-4.1 (singularity)`)',
						value: [
							`handoffs:`,
	...
	private async getSingularityApi(): Promise<Singularity.ContextProviderApiV1 | undefined> {
		...
		this.logService.info('Singularity extension found');
		try {
