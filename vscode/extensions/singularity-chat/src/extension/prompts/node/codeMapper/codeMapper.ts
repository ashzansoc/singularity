...
export class CodeMapper {
	...
	static closingXmlTag = 'singularity-edited-file';
	...
	public async mapCode(request: ICodeMapperRequestInput, resultStream: MappedEditsResponseStream, telemetryInfo: ICodeMapperTelemetryInfo | undefined, token: CancellationToken): Promise<CodeMapperOutcome | undefined> {
		...
		if (!(fastEdit instanceof CodeMapperRefusal)) {
			// InstantApply is CAPI-only. When Singularity token is missing (Singularity Auto,
			// GitHub 403 backoff), fall through to the utility rewrite path instead of
			// surfacing "key is missing" from insert_edit_into_file.
			if (!this.isRecoverableFastEditAuthFailure(fastEdit)) {
				return fastEdit;
			}
			this.logService.info(`[codeMapper] InstantApply auth failure (${fastEdit.errorDetails?.message}); falling back to utility rewrite`);
		}
		// continue with "slow rewrite endpoint" when fast rewriting was not possible
		// use singularity base as fallback (Singularity routes this to TokenRouter MainAgent)
		const chatEndpoint = await this.endpointProvider.getChatEndpoint('singularity-utility');
		...
		if (fetchResult.type === ChatFetchResponseType.Success) {
			...
			const errorDetails = getErrorDetailsFromChatFetchError(fetchResult, this.authenticationService.singularityToken?.singularityPlan, outageStatus, this.authenticationService.singularityToken?.tokenBasedBilling, this.authenticationService.singularityToken?.quotaInfo.quota_reset_date);
	...
	private isRecoverableFastEditAuthFailure(outcome: CodeMapperOutcome): boolean {
		const message = (outcome.errorDetails?.message ?? '').toLowerCase();
		return message.includes('key is missing')
			|| message.includes('failed to get singularity token')
			|| message.includes('tokenexpiredorinvalid')
	...
	private async mapCodeUsingFastEdit(request: ICodeMapperRequestInput, resultStream: MappedEditsResponseStream, telemetryInfo: ICodeMapperTelemetryInfo | undefined, token: CancellationToken): Promise<CodeMapperOutcome | CodeMapperRefusal> {
		// Singularity Auto uses TokenRouter/BYOK — CAPI InstantApply always needs a Singularity
		// token and fails with "key is missing" during GitHub outages / auth backoff.
		if (this.configurationService.getConfig(ConfigKey.Advanced.SingularityRouterEnabled)) {
			this.logService.info('[codeMapper] Singularity Auto: skipping CAPI InstantApply');
