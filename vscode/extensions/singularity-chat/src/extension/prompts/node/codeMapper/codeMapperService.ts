...
class DocumentCodeMapper extends Disposable implements ICodeMapperService {
	...
	async mapCode(request: IMapCodeRequest, responseStream: vscode.MappedEditsResponseStream, telemetryInfo: ICodeMapperTelemetryInfo | undefined, token: vscode.CancellationToken): Promise<IMapCodeResult | undefined> {
		...
		const hasExistingMarker = codeBlock.code.includes(EXISTING_CODE_MARKER);
		if ((!documentContext || (documentContext.getText().length === 0)) && !hasExistingMarker) {
		...
		// Singularity Auto: insert_edit_into_file already supplies the full file body from the agent
		// turn. Applying it locally avoids a second TokenRouter call (shared 5 RPM account limit).
		if (
			this._configurationService.getConfig(ConfigKey.Advanced.SingularityRouterEnabled)
			&& request.location === 'tool'
			&& documentContext
			&& !hasExistingMarker
		) {
			const lastLine = Math.max(0, documentContext.lineCount - 1);
			const lastCol = documentContext.lineCount > 0 ? documentContext.lineAt(lastLine).text.length : 0;
			responseStream.textEdit(
				codeBlock.resource,
				TextEdit.replace(new Range(0, 0, lastLine, lastCol), codeBlock.code),
			);
			this._logService.info('[codeMapper] Singularity Auto: direct full-file apply (skipped TokenRouter InstantApply/utility)');
			this._telemetryService.sendMSFTTelemetryEvent('codemapper.completeCodeBlock');
			return {
				telemetry: {
					requestId: String(telemetryInfo?.chatRequestId ?? ''),
					requestSource: String(telemetryInfo?.chatRequestSource ?? 'tool'),
					chatRequestModel: String(telemetryInfo?.chatRequestModel ?? ''),
					speculationRequestId: 'singularity-direct-apply',
					mapper: 'singularity-direct',
				},
			};
		}
