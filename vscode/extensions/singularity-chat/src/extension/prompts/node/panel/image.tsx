...
export class HistoricalImage extends PromptElement<HistoricalImageProps, unknown> {
	...
	override async render(_state: unknown, sizing: PromptSizing) {
		...
		// Editor preview features are gated only when a Singularity token explicitly
		// disables them (org policy). Signed-out / BYOK users have no Singularity token
		// and must not be blocked, so default to enabled when the token is absent.
		if (!this.promptEndpoint.supportsVision || !(this.authService.singularityToken?.isEditorPreviewFeaturesEnabled() ?? true)) {
			return undefined;
		}
...
export class Image extends PromptElement<ImageProps, unknown> {
	...
	override async render(_state: unknown, sizing: PromptSizing) {
		// See HistoricalImage: only an explicit org-policy Singularity token disables preview
		// features; a missing token (signed-out / BYOK) must still send the image.
		const previewFeaturesEnabled = this.authService.singularityToken?.isEditorPreviewFeaturesEnabled() ?? true;
