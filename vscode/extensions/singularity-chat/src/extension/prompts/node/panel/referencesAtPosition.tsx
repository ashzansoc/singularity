...
/**
 * @remark Excludes references that are in singularity-ignored files.
 */
export class ReferencesAtPosition extends PromptElement<Props> {
	...
	async render(state: void, sizing: PromptSizing) {
		if (await this.ignoreService.isSingularityIgnored(this.props.document.uri)) {
	...
	private async findReferences(timeoutMs: number): Promise<[CodeExcerpt[], CodeExcerpt[]]> {
		...
		for (const ref of foundRefs) {
			if (await this.ignoreService.isSingularityIgnored(ref.uri)) {
				continue;
			}
