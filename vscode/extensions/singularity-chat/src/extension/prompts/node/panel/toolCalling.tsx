...
export interface ChatToolCallsProps extends BasePromptElementProps {
	...
	readonly toolCallMode?: SingularityToolMode;
			...
			children.push(
				<KeepWith priority={index} flexGrow={index + 1} flexReserve={`/${1 / reserve1N}`}>
					{hydratedInstantiationService.invokeFunction(buildToolResultElement, {
						...
						toolCallMode: this.props.toolCallMode ?? SingularityToolMode.PartialContext,
...
function buildToolResultElement(accessor: ServicesAccessor, props: ToolResultOpts) {
	...
	async function getToolResult(sizing: PromptSizing) {
		...
		const singularityTool = toolsService.getSingularityTool(props.toolCall.name as ToolName);
			...
			if (toolResult === undefined) {
				try {
					...
					if (singularityTool?.resolveInput) {
						inputObj = await singularityTool.resolveInput(inputObj, promptContext, props.toolCallMode);
					}
					...
					// #region agent log
					fetch('http://127.0.0.1:7317/ingest/c078ac08-8779-46ff-b139-9beac8ffe002',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f71a1a'},body:JSON.stringify({sessionId:'f71a1a',runId:'post-fix',hypothesisId:'H6',location:'toolCalling.tsx:invoke',message:'tool invoke duration',data:{name:props.toolCall.name,ms:Date.now()-toolT0,parallel:Boolean(tool?.name && toolsCalledInParallel.has(tool.name as ToolName))},timestamp:Date.now()})}).catch(()=>{});
					// #endregion
...
class McpLinkedResourceToolResult extends PromptElement<{ resourceUri: URI; mimeType: string | undefined; count: number } & BasePromptElementProps> {
	...
	async render() {
		if (await this.ignoreService.isSingularityIgnored(this.props.resourceUri)) {
			return null;
		}
