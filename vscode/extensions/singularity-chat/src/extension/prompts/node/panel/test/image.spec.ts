...
describe('Image', () => {
	test('sends image to a vision-capable model when signed out (no Singularity token)', async () => {
		// Signed-out repro: the default test token store has no Singularity token.
	...
	test('sends image when signed in and editor preview features are enabled', async () => {
		const testingServiceCollection = createExtensionUnitTestingServices();
		const accessor = testingServiceCollection.createTestingAccessor();
		setSingularityToken(accessor.get(IAuthenticationService), new SingularityToken(createTestExtendedTokenInfo({ token: 'tid=abc' })));
	...
	test('omits image when organization policy disables editor preview features', async () => {
		...
		setSingularityToken(accessor.get(IAuthenticationService), new SingularityToken(createTestExtendedTokenInfo({ token: 'editor_preview_features=0' })));
