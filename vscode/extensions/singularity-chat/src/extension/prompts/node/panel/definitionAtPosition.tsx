...
export type State =
	...
	| {
		/** Document should be ignored by Singularity */
		k: 'ignored';
	}
	;
...
/**
 * @remark Excludes definitions that are in singularity-ignored files.
 */
export class DefinitionAtPosition extends PromptElement<Props, State> {
	...
	override async prepare(): Promise<State> {
		if (await this._ignoreService.isSingularityIgnored(this.props.document.uri)) {
			return { k: 'ignored' };
		}
		...
		const nonIgnoredDefs = arrays.coalesce(
			await Promise.all(
				foundDefs.map(async def => {
					...
					return await this._ignoreService.isSingularityIgnored(uri) ? undefined : def;
				})
			)
		);
