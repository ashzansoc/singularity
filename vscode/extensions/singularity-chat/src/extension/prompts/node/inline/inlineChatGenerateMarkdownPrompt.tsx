...
export class InlineChatGenerateMarkdownPrompt extends PromptElement<InlineChatGenerateMarkdownPromptProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const isIgnored = await this._ignoreService.isSingularityIgnored(context.document.uri);
		if (isIgnored) {
...
export class MarkdownBlock extends SafePromptElement<MarkdownBlockProps> {
	...
	async render(state: void) {
		const isIgnored = this.props.uri ? await this._ignoreService.isSingularityIgnored(this.props.uri) : false;
		if (isIgnored) {
