...
export class Diagnostics extends PromptElement<DiagnosticsProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const isIgnored = await this.ignoreService.isSingularityIgnored(documentContext.document.uri);
		if (isIgnored) {
		...
		if (diagnostic.relatedInformation) {
			for (const relatedInformation of diagnostic.relatedInformation) {
				try {
					...
					if (await this.ignoreService.isSingularityIgnored(location.uri)) {
