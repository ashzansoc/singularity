...
function createHarness(): IHarness {
	...
	const scriptedEndpoint = instantiationService.createInstance(MockEndpoint, 'singularity-utility-small');
	...
	scriptedEndpoint.makeChatRequest2 = async (options: IMakeChatRequestOptions): Promise<ChatResponse> => {
		...
		await options.finishedCb?.('', 0, { text: '', singularityToolCalls: toolCalls });
		return {
			...
			resolvedModel: 'singularity-utility-small',
		};
	};
