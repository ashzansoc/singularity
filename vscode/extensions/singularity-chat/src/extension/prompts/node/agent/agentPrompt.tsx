...
import { MemoryContextPrompt, MemoryInstructionsPrompt, GlobalMemoryPolicyPrompt, BrainGlobalMemoryPrompt } from '../../../tools/node/memoryContextPrompt';
...
export class AgentPrompt extends PromptElement<AgentPromptProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const SingularityIdentityRules = customizations.SingularityIdentityRulesClass;
		...
		const baseAgentInstructions = <>
			<SystemMessage>
				...
				<SingularityIdentityRules />
				<SafetyRules />
			</SystemMessage>
			<SystemMessage>
				<GlobalMemoryPolicyPrompt />
			</SystemMessage>
			{instructions}
		...
		const baseInstructions = <>
			...
			{isVoiceModeInput && <SystemMessage priority={80}>
				Voice Mode is active, and you are Singularity speaking directly to the user. Keep the final response concise and easy to understand aloud. Do not expose internal reasoning.<br />
	...
	render() {
		return <UserMessage>
			...
			{this.props.isNewChat && <>
				<MemoryContextPrompt sessionResource={this.props.sessionResource} />
				<BrainGlobalMemoryPrompt />
			</>}
...
export class AgentUserMessage extends PromptElement<AgentUserMessageProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const hasLlmWikiTool = !!this.props.availableTools?.find(tool => tool.name === ToolName.LlmWiki);
...
export class AgentTasksInstructions extends PromptElement<AgentTasksInstructionsProps> {
	...
	async render() {
		...
		const taskGroups = (await Promise.all(taskGroupsRaw.map(async ([folder, tasks]) => {
			...
			if (await this._ignoreService.isSingularityIgnored(tasksFile)) {
				return undefined;
			}
