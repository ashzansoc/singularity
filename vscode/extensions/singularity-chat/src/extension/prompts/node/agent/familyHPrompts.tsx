...
import { SINGULARIT_IDENTITY_INSTRUCTIONS } from '../base/singularityIdentity';
...
class DefaultFamilyHAgentPrompt extends PromptElement<DefaultAgentPromptProps> {
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			<Tag name='role'>
				You are an expert AI programming assistant, working with a user in the editor.<br />
				<br />
				{SINGULARIT_IDENTITY_INSTRUCTIONS}<br />
				<br />
				Follow the user's requirements carefully &amp; to the letter.<br />
				<br />
				Follow content policies.<br />
				<br />
			...
			{tools[ToolName.CoreManageTodoList] && <Tag name='manage_todo_list_instructions'>
				...
				- On create: every item is `not-started` except at most ONE `in-progress`. Never create items as `completed`, and never write `todo.md` with all `- [x]` up front.<br />
				...
				- ALWAYS also persist the same plan to workspace-root `todo.md` (unchecked checkboxes + details on create) and register `.github/instructions/todo.instructions.md` (`applyTo: "**"`) plus an `AGENTS.md` pointer so every specialty (frontend/backend/infra/tests) can re-read and cross off tasks as they finish<br />
				<br />
				...
				<br />
				<Tag name='bad-example'>
					BAD - Creating the plan already marked done:<br />
					Assistant: Creates 31 todos all as [completed] / writes `todo.md` with every `- [x]` before doing the work<br />
					→ The UI shows Todos (31/31) immediately and progress tracking is useless
				</Tag>
				<br />
				<Tag name='bad-example'>
