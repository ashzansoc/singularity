...
import { getActiveDesignBrief } from '../../../../platform/endpoint/node/designSourcePlanner';
import { getActiveDesignSpecPrompt, getActiveSkillPrompt } from '../../../../platform/endpoint/node/designIntelligence';
...
export interface ReminderInstructionsProps extends BasePromptElementProps {
	...
	readonly hasLlmWikiTool: boolean;
}
...
export class DefaultReminderInstructions extends PromptElement<ReminderInstructionsProps> {
	async render(state: void, sizing: PromptSizing) {
		return <>
			...
			{this.props.hasTodoTool && <PersistentTodoMdReminder />}
			{this.props.hasLlmWikiTool && <LlmWikiReminder />}
		</>;
	}
}

/**
 * Workspace todo.md is a read-only projection when the execution engine is active.
 * Do not create standalone todo.md plans — the engine writes todo.md from ExecutionStore.
 */
export class PersistentTodoMdReminder extends PromptElement {
	async render() {
		return <>
			<Tag name='persistentTodoMd'>
				For multi-step work, the execution engine maintains the canonical task graph in `.singularity/execution/` and projects status to workspace-root `todo.md`.<br />
				1. Use `{ToolName.CoreManageTodoList}` as a view of execution tasks — do not invent free-form parallel todo lists.<br />
				2. Do NOT manually create or rewrite `todo.md` with a full upfront plan; the engine projects live batch status.<br />
				3. Mark tasks completed only after the work is actually done. One in-progress task at a time.<br />
				4. If execution engine is off, keep session todos via manage_todo_list only (no standalone todo.md authoring).<br />
			</Tag>
		</>;
	}
}
...
/**
 * Karpathy LLM Wiki — persistent compounding knowledge base in markdown.
 */
export class LlmWikiReminder extends PromptElement {
	async render() {
		return <>
			<Tag name='llmWiki'>
				This workspace may have a Singularity LLM Wiki at `.singularity/wiki` (see SCHEMA.md).<br />
				For research, reading notes, or accumulating knowledge over time: use {ToolName.LlmWiki} to ingest sources, query with citations, lint health, and file grounded answers back.<br />
				Never modify `raw/`. Read `wiki/index.md` first on queries. Do not fabricate answers when the wiki has no confident coverage.<br />
			</Tag>
		</>;
	}
}
...
/**
 * Anti-slop taste rules for ALL models (including Qwen / DeepSeek frontend specialty).
 * GPT-5 prompts have richer frontend_guidance; this is the baseline so non-GPT lanes
 * cannot collapse into zinc + blue→purple + Lucide card + MeshDistort templates.
 */
export class FrontendTasteRules extends PromptElement {
	async render() {
		return <>
			<Tag name='frontend_tasks'>
				When doing frontend design tasks, follow Design Intelligence: Design Spec → implement → visual QA.<br />
				The Design Specification owns visual identity. You IMPLEMENT it — do not reinvent art direction when a Spec exists.<br />
				If `.singularity/design-spec.json` already exists, USE IT and do not create or overwrite a new Spec. Only write a Spec when none exists yet.<br />
				Also use `.singularity/skill.json` when present — it is the selected agency specialist lens for this project.<br />
				Do ONLY what the user asked. Do not expand into multi-step rebuild plans, new manage_todo_list roadmaps, or scaffolding a new Vite/Next app unless the user explicitly requested that.<br />
				On Visual Critic refine: make the minimum file edits to fix the listed findings inside the existing project — then stop.<br />
				DEFAULT STACK (always for HTML/CSS/JS/React frontend work): React Bits + GodUI.<br />
				- React Bits: https://github.com/DavidHDev/react-bits — install with `npx shadcn@latest add @react-bits/&lt;Component&gt;-TS-TW` (text animations, backgrounds, interactive UI).<br />
				- GodUI: https://godui.design — prefer GodUI MCP (`npx -y @godui/mcp@latest`); else `npx shadcn@latest add "https://godui.design/r/&lt;component&gt;.json"`. Use components, animated icons, overlays, AI surfaces, glass.<br />
				- Actually install these into the project and restyle to the Design Spec — do not imitate from memory. shadcn/Radix stay the primitive layer when needed.<br />
				- REFERENCE / “SOMETHING LIKE” + URL: Before coding, load the `clone-website` skill. Recon the URL with screenshot_page / read_page / fetch_webpage, write `docs/research/reference-digest.md`, then build the user’s product matching that craft (not phishing). Template: https://github.com/JCodesMore/ai-website-cloner-template<br />
				- Brand-first hero: product name is a hero-level signal, not only tiny nav text. Remove the nav and the page must still belong to this brand.<br />
				- One composition above the fold: brand, one headline, one supporting sentence, one CTA group, one dominant product visual. No card spam in the hero.<br />
				- Typography: implement the FULL Design Spec typography system — pairing, weights, letter-spacing, line-height, heading scale, measure, and mono usage — not faces alone. Default = Experimental → Syne + Manrope + IBM Plex Mono with its metrics. Inter/Geist only when Spec personality lists them.<br />
				- Color: define CSS variables; ban dark zinc/slate + blue→purple / indigo / violet as the page identity unless the Spec justifies it.<br />
				- Hero visual must communicate the product (custom SVG/diagram/screenshot). Ban MeshDistortMaterial / floating orb / bokeh wallpaper as the hero.<br />
				- Do not use a Lucide-icon + generic sentence card grid as the Features section identity unless Spec justifies — prefer GodUI animated icons when motion helps.<br />
				- If nav links to #pricing / #docs / #product, those sections must exist with product-specific copy.<br />
				- Libraries are implementation tools, not art directors — no stack theater.<br />
				- Ship 2–3 intentional motions; acceptance is visual, not "Hero.tsx exists".<br />
...
/**
 * Injects Design Spec + Automode design-source plan when frontend specialty is active.
 */
export class ActiveDesignSourceBrief extends PromptElement {
	async render() {
		const spec = getActiveDesignSpecPrompt()?.trim();
		const skill = getActiveSkillPrompt()?.trim();
		const brief = getActiveDesignBrief()?.trim();
		if (!spec && !skill && !brief) {
			return undefined;
		}
		return <>
			{skill && <Tag name='agencySkill'>
				{skill}
			</Tag>}
			{spec && <Tag name='designSpecification'>
				{spec}
			</Tag>}
			{brief && <Tag name='singularityDesignSourcePlan'>
				{brief}
			</Tag>}
		</>;
	}
		...
		return <InstructionMessage>
			...
			<Tag name='structuredWorkflow'>
				...
				**CRITICAL - Before ending your turn:**<br />
				- Review and update the todo list: mark finished work completed, leave unfinished items not-started/in-progress, and note skipped/blocked items with explanations.<br />
				- Do not mark remaining work completed just to clear the list. Unchecked unfinished items are correct.<br />
				</>}
				<br />
