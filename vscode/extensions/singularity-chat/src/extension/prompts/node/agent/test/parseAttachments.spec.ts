...
import { extractChatPromptReferences } from '../../../../chatSessions/singularitycli/common/singularityCLIPrompt';
import { SingularityCLIImageSupport } from '../../../../chatSessions/singularitycli/node/singularityCLIImageSupport';
import { SingularityCLIPromptResolver } from '../../../../chatSessions/singularitycli/node/singularitycliPromptResolver';
import { MockSkillLocations } from '../../../../chatSessions/singularitycli/node/test/testHelpers';
...
suite('SingularityCLI Generate & parse prompts', () => {
	(['emptyWorkspace', 'workspace', 'worktree'] as const).forEach(workspaceType => {
		suite(workspaceType, () => {
			...
			let resolver: SingularityCLIPromptResolver;
			...
			beforeEach(() => {
				...
				const imageSupport = new class extends mock<SingularityCLIImageSupport>() {
				...
				resolver = new SingularityCLIPromptResolver(imageSupport, logService, fileSystem, workspaceService, services.seal(), accessor.get(IIgnoreService), new MockSkillLocations(), new MockExtensionContext() as unknown as IVSCodeExtensionContext);
			});
			afterEach(() => {
				disposables.clear();
				vi.resetAllMocks();
	...
	console.log("This is an example");
			...
			test('excludes customizations index from references and attachments', async () => {
				const customizationsIndex = {
					...
					name: 'customizations',
					value: URI.file('/workspace/.github/singularity-instructions.md')
				};
				const regularFileRef = {
					id: 'regular-file',
					name: 'regular.ts',
				...
				const customizationsRef = resolved.references.find(r => URI.isUri(r.value) && (r.value as URI).fsPath.includes('singularity-instructions.md'));
				expect(customizationsRef).toBeUndefined();
				// Regular file reference should still be included
				const regularRef = resolved.references.find(r => URI.isUri(r.value) && (r.value as URI).fsPath.includes('regular.ts'));
				expect(regularRef).toBeDefined();
				// Attachment for customizations index should not be present
				const customizationsAttachment = resolved.attachments.find(a => a.type === 'file' && a.path.includes('singularity-instructions.md'));
