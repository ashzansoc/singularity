 ...
 */
class SingularityCLIAgentUserMessage extends PromptElement<AgentUserMessageProps> {
	constructor(
	...
	async render(state: void, sizing: PromptSizing) {
		...
		// We leave that for Singularity CLI SDK to handle.
...
export async function generateUserPrompt(request: ChatRequest, prompt: string | undefined, chatVariables: ChatVariablesCollection, instantiationService: IInstantiationService): Promise<string> {
	...
	const { messages } = await renderPromptElement(instantiationService, endpoint, SingularityCLIAgentUserMessage, {
		chatVariables,
		endpoint,
	...
	// Singularity CLI only accepts a plain text prompt. Any non-text content (e.g. images) is delivered
