...
import { SINGULARITY_DEBUG_COMMAND } from './singularityDebugCommandContribution';

const PROVIDER_ID = 'singularity-chat.terminalToDebugging';
const PROVIDER_ID2 = 'singularity-chat.terminalToDebuggingSuccess';
...
export class OnboardTerminalTestsContribution extends Disposable implements vscode.TerminalQuickFixProvider {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('singularity.chat.chat.rerunWithSingularityDebug', () => {
			...
			terminal.sendText(`${SINGULARITY_DEBUG_COMMAND} ${execution.execution.commandLine.value}`, true);
		}));
	}
	...
	async provideTerminalQuickFixes(
		...
		return {
			terminalCommand: `${SINGULARITY_DEBUG_COMMAND} ${commandMatchResult.commandLine}`,
			shouldExecute: false,
		};
	}
}
