...
export class SingularityDebugCommandHandle {
	public static SINGULARITY_LABEL = 'Singularity';
	...
	question(message: string, defaultValue?: string, singleKey = false): Promise<string | undefined> {
		return this.rpc.callMethod('question', { message: withLabel('blue', SingularityDebugCommandHandle.SINGULARITY_LABEL, message), defaultValue, singleKey });
	}

	confirm(message: string, defaultValue?: boolean): Promise<boolean | undefined> {
		return this.rpc.callMethod('confirm', { message: withLabel('blue', SingularityDebugCommandHandle.SINGULARITY_LABEL, message), defaultValue });
	}

	printLabel(color: KnownColors, message: string): Promise<void> {
		return this.output('stdout', withLabel(color, SingularityDebugCommandHandle.SINGULARITY_LABEL, message) + '\r\n');
	}
