...
import { SingularityDebugCommandSessionFactory } from '../node/singularityDebugCommandSessionFactory';
import { SimpleRPC } from '../node/singularityDebugWorker/rpc';
import { IStartOptions, StartResultKind } from '../node/singularityDebugWorker/shared';
import { SingularityDebugCommandHandle } from './singularityDebugCommandHandle';
import { handleDebugSession } from './singularityDebugCommandSession';

//@ts-ignore
import powershellScript from '../node/singularityDebugWorker/singularityDebugWorker.ps1';

// When enabled, holds the storage location of binaries for the PATH:
const WAS_REGISTERED_STORAGE_KEY = 'singularity-chat.terminalToDebugging.registered';
// Nonce exposed to terminals so the legitimate `singularity-debug` worker can
// authenticate itself against the URI handler (see VSCODE-410).
const SINGULARITY_DEBUG_NONCE_STORAGE_KEY = 'singularity-chat.terminalToDebugging.nonce';
export const SINGULARITY_DEBUG_NONCE_ENV_VAR = 'SINGULARITY_DEBUG_NONCE';
export const SINGULARITY_DEBUG_COMMAND = `singularity-debug`;
const DEBUG_COMMAND_JS = 'singularityDebugCommand.js';

export class SingularityDebugCommandContribution extends Disposable implements vscode.UriHandler {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('singularity.chat.chat.startSingularityDebugCommand', async () => {
			const term = vscode.window.createTerminal();
			term.show(false);
			term.sendText('singularity-debug <your command here>', false);
		}));
	...
	private async ensureTask(workspaceFolder: URI | undefined, def: vscode.TaskDefinition, handle: SingularityDebugCommandHandle): Promise<boolean> {
		if (!workspaceFolder) {
	...
	handleUri(uri: vscode.Uri): vscode.ProviderResult<void> {
		...
		/* __GDPR__
			"uriHandler" : {
				"owner": "lramos15",
				"comment": "Reports when the uri handler is called in the singularity extension",
		...
		*/
		...
		const socket = connect(pipePath, () => {
			...
			const handle = new SingularityDebugCommandHandle(rpc);
			const { launchConfigService, authService, nonce } = this;
			const logService = this.logService;
			const exit = (code: number, error?: string) => handle.exit(code, error);
			const factory = this.instantiationService.createInstance(SingularityDebugCommandSessionFactory, {
			...
			rpc.registerMethod('start', async function start(opts: IStartOptions): Promise<void> {
				...
				if (!authService.singularityToken) {
					await authService.getGitHubSession('any', { createIfNone: { detail: l10n.t('Sign in to GitHub to use Singularity debug.') } });
				}
	...
	private ensureNonce(): string {
		let nonce = this.context.workspaceState.get<string>(SINGULARITY_DEBUG_NONCE_STORAGE_KEY);
		if (!nonce || typeof nonce !== 'string' || nonce.length < 32) {
			nonce = randomBytes(16).toString('hex');
			this.context.workspaceState.update(SINGULARITY_DEBUG_NONCE_STORAGE_KEY, nonce);
		}

		if (this.context.environmentVariableCollection.get(SINGULARITY_DEBUG_NONCE_ENV_VAR)?.value !== nonce) {
			this.context.environmentVariableCollection.replace(SINGULARITY_DEBUG_NONCE_ENV_VAR, nonce);
		}
	...
	private async registerEnvironment() {
		...
		if (!enabled) {
			if (previouslyStoredAt) {
				...
				this.terminalService.removePathContribution('singularity-debug');
				await fs.rm(previouslyStoredAt.location, { recursive: true, force: true });
			}
		} else if (!previouslyStoredAt) {
			// 2. enabling a disabled state
			this.terminalService.contributePath('singularity-debug', storageLocation, { command: SINGULARITY_DEBUG_COMMAND });
	...
	private async fillStoragePath(storagePath: string) {
		...
		if (process.platform === 'win32') {
			const ps1Path = path.join(storagePath, `${SINGULARITY_DEBUG_COMMAND}.ps1`);
			...
			await fs.writeFile(path.join(storagePath, `${SINGULARITY_DEBUG_COMMAND}.bat`), makeBatScript(ps1Path));
		} else {
			const shPath = path.join(storagePath, SINGULARITY_DEBUG_COMMAND);
