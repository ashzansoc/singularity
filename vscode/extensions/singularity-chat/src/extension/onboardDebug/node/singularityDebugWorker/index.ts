...
// in singularityDebugWorker.ps1 instead. This is because Electron on Windows
...
if (!args.length || flagConfig[Flags.Help]) {
	console.log(`Usage: singularity-debug [${Object.keys(flagConfig).join('] [')}] <command> <args...>`);
	console.log('');
	console.log('Options:');
...
const server = createServer(socket => {
	...
	const opts: IStartOptions = {
		...
		nonce: process.env.SINGULARITY_DEBUG_NONCE || '',
	};
...
const pipeName = `singularity-dbg.${process.pid}-${randomBytes(4).toString('hex')}.sock`;
