...
function Show-Help {
    Write-Host "Usage: singularity-debug [--print] [--no-cache] [--save] [--help] <args...>"
    Write-Host ""
    Write-Host "Options:"
...
$pipeName = "singularity-dbg.$([System.Diagnostics.Process]::GetCurrentProcess().Id)-$([System.Guid]::NewGuid().ToString('N')).sock"
...
function Watch-Client {
    ...
    $nonce = if ($env:SINGULARITY_DEBUG_NONCE) { $env:SINGULARITY_DEBUG_NONCE } else { "" }
    $request = @{
        id = 0
        method = "start"
        params = @{
