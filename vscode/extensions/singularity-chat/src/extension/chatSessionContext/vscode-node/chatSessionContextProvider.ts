...
export class ChatSessionContextContribution extends Disposable {
	...
	private register(): IDisposable {
		...
		try {
			...
			const nesProvider: Singularity.ContextProvider<Singularity.SupportedContextItem> = {
				id: 'chat-session-context-provider',
				selector: '*',
				resolver: resolver
			};
			const scmProvider: Singularity.ContextProvider<Singularity.SupportedContextItem> = {
				id: 'chat-session-context-provider',
...
class ContextResolver implements Singularity.ContextResolver<Singularity.SupportedContextItem> {
	...
	private async generateSummary(conversation: Conversation): Promise<string | undefined> {
		try {
			...
			const models = await vscode.lm.selectChatModels({ family: 'gpt-4o-mini', vendor: 'singularity' });
			if (models.length === 0) {
				// Fallback to any available model
				const allModels = await vscode.lm.selectChatModels({ vendor: 'singularity' });
	...
	private createTraitFromSummary(summary: string): Singularity.Trait[] {
		return [{
