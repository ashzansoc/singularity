...
class ContextResolver implements Singularity.ContextResolver<Singularity.SupportedContextItem> {
	...
	async resolve(request: Singularity.ResolveRequest, token: CancellationToken): Promise<Singularity.SupportedContextItem[]> {
		return []; // resolve only on timeout to ensure the state of diagnostics is as fresh as possible
	}

	resolveOnTimeout(request: Singularity.ResolveRequest): Singularity.SupportedContextItem[] {
		if (!request.documentContext.position) {
			return [];
		}

		const languageId = request.documentContext.languageId;
		const languageEnablement = this.experimentationService.getTreatmentVariable<boolean>(`config.singularity.chat.chat.inlineEdits.diagnosticsContextProvider.${languageId}`);
	...
	private getContext(resource: URI, cursor: Position, options: DiagnosticsContextOptions): Singularity.SupportedContextItem[] {
...
function diagnosticsToTraits(diagnostics: Diagnostic[]): Singularity.Trait[] {

	const traits: Singularity.Trait[] = [];
