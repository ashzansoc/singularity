...
- Add the following to your VS Code `settings.json` (File -> Preferences -> Settings):
    ```
    "singularity.chat.chat.languageContext.typescript.enabled": true,
	"singularity.chat.advanced.contextProviders": ["typescript-ai-context-provider"]
    ```
- Run the `watch` task with `cmd+shift+B`
- Start the "Launch Singularity Extension - TS Server in Debug Mode" launch config
