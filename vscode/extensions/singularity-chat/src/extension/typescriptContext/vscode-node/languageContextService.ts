...
enum ErrorPart {
	...
	SingularityExtension = 'singularity-extension'
}
...
class TelemetrySender {
	...
	public sendSpeculativeRequestTelemetry(context: RequestContext, originalRequestId: string, numberOfItems: number): void {
		...
		if (shouldSendTelemetry) {
			/* __GDPR__
				"typescript-context-plugin.completion-context.speculative" : {
					"owner": "dirkb",
					"comment": "Telemetry for singularity inline completion context",
		...
		this.logService.debug(`TypeScript Singularity context speculative request: [${context.requestId} - ${originalRequestId}, numberOfItems: ${numberOfItems}]`);
	}
			...
			for (const error of errorData) {
				/* __GDPR__
					"typescript-context-plugin.completion-context.error" : {
						"owner": "dirkb",
						"comment": "Telemetry for singularity inline completion context errors",
						"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
						"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" },
	...
	public sendRequestOnTimeoutTelemetry(context: RequestContext, data: ContextItemSummary, cacheState: CacheState): void {
		...
		/* __GDPR__
			"typescript-context-plugin.completion-context.on-timeout" : {
				"owner": "dirkb",
				"comment": "Telemetry for singularity inline completion context on timeout",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
				"opportunityId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The opportunity id" },
				"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" },
				"total": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Total number of context items", "isMeasurement": true },
		...
		*/
		...
		this.logService.debug(`TypeScript Singularity context on timeout: [${context.requestId}, ${JSON.stringify(stats, undefined, 0)}]`);
	}
	...
	public sendRequestFailureTelemetry(context: RequestContext, data: { error: protocol.ErrorCode; message: string; stack?: string }): void {
		/* __GDPR__
			"typescript-context-plugin.completion-context.failed" : {
				"owner": "dirkb",
				"comment": "Telemetry for singularity inline completion context in failure case",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
				"opportunityId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The opportunity id" },
				"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" },
	...
	public sendRequestCancelledTelemetry(context: RequestContext, timeTaken: number): void {
		/* __GDPR__
			"typescript-context-plugin.completion-context.cancelled" : {
				"owner": "dirkb",
				"comment": "Telemetry for singularity inline completion context in cancellation case",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
				"opportunityId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The opportunity id" },
				"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" },
				"timeTaken": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Time taken to provide the completion", "isMeasurement": true }
		...
		*/
		...
		this.logService.debug(`TypeScript Singularity context request ${context.requestId} got cancelled.`);
	}
	...
	public sendIntegrationTelemetry(requestId: string, document: string, versionMismatch?: string): void {
		/* __GDPR__
			"typescript-context-plugin.integration.failed" : {
				"owner": "dirkb",
				"comment": "Telemetry for Singularity inline chat integration.",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
		...
		if (registered) {
			/* __GDPR__
				"typescript-context-plugin.inline-completion-provider.registered" : {
					"owner": "dirkb",
					"comment": "Telemetry for Singularity inline completions",
					"source": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The source of the request" }
				}
			*/
			this.telemetryService.sendMSFTTelemetryEvent(
...
export class LanguageContextServiceImpl implements ILanguageContextService, vscode.Disposable {
	...
	private async doIsTypeScriptActivated(languageId: string): Promise<boolean> {
		...
		try {
			...
			const response: protocol.PingResponse | undefined = await vscode.commands.executeCommand('typescript.tsserverRequest', '_.singularity.ping', LanguageContextServiceImpl.ExecConfig, CancellationToken.None);
	...
	async populateCache(document: vscode.TextDocument, position: vscode.Position, context: RequestContext): Promise<void> {
		...
		try {
			...
			try {
				const promise: Thenable<protocol.ComputeContextResponse> = vscode.commands.executeCommand('typescript.tsserverRequest', '_.singularity.context', args, LanguageContextServiceImpl.ExecConfig, token);
...
const showContextInspectorViewContextKey = `singularity.chat.chat.showContextInspectorView`;
export class InlineCompletionContribution implements vscode.Disposable, TokenBudgetProvider {
	...
	constructor(
		...
		if (languageContextService instanceof LanguageContextServiceImpl) {
			this.disposables.add(vscode.commands.registerCommand('singularity.chat.debug.showContextInspectorView', async () => {
	...
	private async register(): Promise<void> {
		...
		try {
			...
			const self = this;
			const resolver: Singularity.ContextResolver<Singularity.SupportedContextItem> = {
				resolve(request: Singularity.ResolveRequest, token: vscode.CancellationToken): Promise<Singularity.SupportedContextItem[]> | AsyncIterable<Singularity.SupportedContextItem> {
					...
					if (Array.isArray(items)) {
						const convertedItems: Singularity.SupportedContextItem[] = [];
			...
			if (typeof languageContextService.getContextOnTimeout === 'function') {
				resolver.resolveOnTimeout = (request) => {
					...
					const result: Singularity.SupportedContextItem[] = [];
					for (const item of items) {
						const converted = self.convertItem(item);
						if (converted === undefined) {
							continue;
			...
			const provider: Singularity.ContextProvider<Singularity.SupportedContextItem> = {
				id: 'typescript-ai-context-provider',
				selector: { scheme: 'file', language: 'typescript' },
				resolver: resolver
			};

			// For legacy register with the singularity API
			const singularityAPI = await this.getSingularityApi();
			if (singularityAPI !== undefined) {
				this.registrations.add(singularityAPI.registerContextProvider(provider));
			}
			...
			logService.info('Registered TypeScript context provider with Singularity inline completions.');
		} catch (error) {
	...
	private getDocumentAndPosition(request: Singularity.ResolveRequest, token?: vscode.CancellationToken): [vscode.TextDocument | undefined, vscode.Position | undefined] {
	...
	private convertItem(item: ContextItem): Singularity.SupportedContextItem | undefined {
		if (item.kind === ContextKind.Snippet) {
			const converted: Singularity.CodeSnippet = {
			...
			const converted: Singularity.Trait = {
				importance: item.priority * 100,
				id: item.id,
				name: item.name,
				value: item.value
			...
			const converted: Singularity.DiagnosticBag = {
				importance: item.priority * 100,
				id: item.id,
				uri: item.uri,
				values: item.values
	...
	private async getSingularityApi(): Promise<Singularity.ContextProviderApiV1 | undefined> {
		const singularityExtension = vscode.extensions.getExtension('Singularity.singularity');
		if (singularityExtension === undefined) {
			// this.telemetrySender.sendActivationFailedTelemetry(ErrorLocation.Client, ErrorPart.SingularityExtension, 'Singularity extension not found', undefined);
			// this.logService.error('Singularity extension not found');
			return undefined;
		}
		try {
			const api = await singularityExtension.activate();
			return api.getContextProviderAPI('v1');
		} catch (error) {
			if (error instanceof Error) {
				this.telemetrySender.sendActivationFailedTelemetry(ErrorLocation.Client, ErrorPart.SingularityExtension, error.message, error.stack);
				this.logService.error('Error activating Singularity extension:', error.message);
			} else {
				this.telemetrySender.sendActivationFailedTelemetry(ErrorLocation.Client, ErrorPart.SingularityExtension, 'Unknown error', undefined);
				this.logService.error('Error activating Singularity extension: Unknown error.');
			}
			return undefined;
		}
	}
