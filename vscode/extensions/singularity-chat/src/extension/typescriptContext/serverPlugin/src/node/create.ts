...
export function create(info: tt.server.PluginCreateInfo): tt.LanguageService {
	...
	if (info.session !== undefined) {
		try {
			...
			info.session.addProtocolHandler('_.singularity.ping', () => {
			...
			try {
				...
				if (versionSupported) {
					...
					info.session.addProtocolHandler('_.singularity.context', computeContextHandler);
					info.session.addProtocolHandler('_.singularity.prepareNesRename', prepareNesRenameHandler);
					info.session.addProtocolHandler('_.singularity.postNesRename', nesRenameHandler);
				}

			} catch (e) {
				if (e instanceof Error) {
					pingResult = { kind: 'error', message: e.message, stack: e.stack };
					info.session.logError(e, '_.singularity.installHandler');
				} else {
					pingResult = { kind: 'error', message: 'Unknown error' };
					info.session.logError(new Error('Unknown error'), '_.singularity.installHandler');
				}
			}
		} catch (error) {
			if (error instanceof Error) {
				info.session.logError(error, '_.singularity.installPingHandler');
			} else {
				info.session.logError(new Error('Unknown error'), '_.singularity.installPingHandler');
			}
		} finally {
