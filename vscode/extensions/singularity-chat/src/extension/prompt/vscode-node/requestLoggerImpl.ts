...
function processDeltasToMessage(deltas: IResponseDelta[]): string {
	return deltas.map((d, i) => {
		...
		if (d.singularityToolCalls) {
			if (i > 0) {
				text += '\n';
			}

			text += d.singularityToolCalls.map(c => {
...
class LoggedRequestInfo implements ILoggedRequestInfo {
	...
	toJSON(): object {
		...
		const metadata = {
			...
			singularityUsageAic: this.entry.type === LoggedRequestKind.ChatMLSuccess && typeof this.entry.usage?.singularity_usage?.total_nano_aiu === 'number'
				? this.entry.usage.singularity_usage.total_nano_aiu / 1_000_000_000 : undefined,
...
`;
	...
	private _renderRequestToMarkdown(id: string, entry: LoggedRequest): string {
		...
		if (entry.type === LoggedRequestKind.ChatMLSuccess) {
			...
			if (typeof entry.usage?.singularity_usage?.total_nano_aiu === 'number') {
				const aic = entry.usage.singularity_usage.total_nano_aiu / 1_000_000_000;
				result.push(`singularityUsage    : ${aic.toFixed(2)} AIC (${entry.usage.singularity_usage.total_nano_aiu} nano-AIU)`);
			}
