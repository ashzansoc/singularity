...
import { isTrivialChatPrompt } from '../../../platform/endpoint/node/singularityPromptEngineBridge';
...
export class ChatTitleProvider implements vscode.ChatTitleProvider {
	...
	async provideChatTitle(
		...
		const prompt = firstRequest.prompt?.trim() ?? '';
		if (isTrivialChatPrompt(prompt) || shouldSkipAuxiliaryTokenRouterLlm()) {
			const words = prompt.split(/\s+/).slice(0, 6).join(' ');
			return words.length > 40 ? `${words.slice(0, 37)}…` : (words || 'Chat');
		}
