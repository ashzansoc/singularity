...
export class PseudoStopStartResponseProcessor implements IResponseProcessor {
	...
	/**
	 * Cache of `stagedDeltasToApply.map(d => d.text).join('')`. Invalidated
	 * explicitly on every mutation of the staged array; recomputed lazily inside
	 * checkForKeyWords. This removes the O(staged²) full re-join per delta on
	 * the streaming hot path during partial stop-word accumulation while
	 * keeping byte-identical stop-word detection.
	 */
	private _stagedText: string | undefined = undefined;
	...
	protected applyDeltaToProgress(delta: IResponseDelta, progress: ChatResponseStream) {
		...
		if (delta.singularityToolCallStreamUpdates?.length) {
			const now = Date.now();
			for (const update of delta.singularityToolCallStreamUpdates) {
	...
	private updateStagedDeltasUpToIndex(stopWordIdx: number, length: number): IResponseDelta[] {
		this._stagedText = undefined; // staged array will be mutated
	...
	protected checkForKeyWords(pseudoStopWords: string[], delta: IResponseDelta, applyDeltaToProgress: (delta: IResponseDelta) => void): string | undefined {
		// Recompute the staged text lazily: only when the staged array has been
		// mutated since last use (cache is invalidated at every mutation site),
		// instead of re-joining the whole staged list on every single delta.
		if (this._stagedText === undefined) {
			this._stagedText = this.stagedDeltasToApply.map(d => d.text).join('');
		}
		const textDelta = this._stagedText + delta.text;
		...
		for (const pseudoStopWord of pseudoStopWords) {
			...
			this._stagedText = undefined; // staged array mutated below
		...
		this._stagedText = undefined; // staged array reset below
