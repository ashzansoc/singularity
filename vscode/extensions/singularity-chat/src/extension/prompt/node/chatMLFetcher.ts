...
import { collectSystemTextsFromRequestBody, SingularityChatAttr, emitInferenceDetailsEvent, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiProviderName, normalizeProviderMessages, normalizeResponseModel, StdAttr, stringifyToolDefinitionsForOTel, stringifyToolsRawForTelemetry, toSystemInstructions, truncateForOTel } from '../../../platform/otel/common/index';
...
import { reportSingularityProjectTokenUsage } from '../../../platform/chat/node/singularityTokenUsage';
import { tracePerfChatRequest } from '../../../platform/chat/node/singularityPerfTrace';
...
export class ChatMLFetcherImpl extends AbstractChatMLFetcher {
	...
	public async fetchMany(opts: IFetchMLOptions, token: CancellationToken): Promise<ChatResponses> {
		...
		try {
			...
			if (!payloadValidationResult.isValid) {
				...
				let singularityToken: SingularityToken | undefined;
				try {
					// TokenRouter/BYOK chat must not join Singularity CAPI token minting.
					// A 403 + fetcher fallback (electron-fetch → node-fetch → node-http) is ~20s
					// and is coalesced with AuthUpgradeAsk, so Hello waits on GitHub even when
					// the completion never uses that token.
					if (!this._configurationService.getConfig(ConfigKey.Advanced.SingularityRouterEnabled)) {
						singularityToken = await this._authenticationService.getSingularityToken();
					}
				} catch {
					// BYOK / air-gapped: no Singularity token available. Continue without one.
				}
				usernameToScrub = singularityToken?.username ?? this._authenticationService.singularityToken?.username;
				...
				if (otelInferenceSpan) {
					...
					if (lastUserMsg?.content) {
						...
						otelInferenceSpan.setAttribute(SingularityChatAttr.USER_REQUEST, truncateForOTel(userContent, this._otelService.config.maxAttributeSizeChars));
					}
				...
				if (otelInferenceSpan) {
					...
					if (requestOptions) {
						otelInferenceSpan.setAttribute(SingularityChatAttr.REQUEST_OPTIONS, truncateForOTel(JSON.stringify(requestOptions), this._otelService.config.maxAttributeSizeChars));
					}
					const requestShape = pickRequestShapeMetadata(requestBody);
					if (requestShape) {
						otelInferenceSpan.setAttribute(SingularityChatAttr.REQUEST_SHAPE, truncateForOTel(JSON.stringify(requestShape), this._otelService.config.maxAttributeSizeChars));
					}
				}
			...
			switch (response.type) {
				case FetchResponseKind.Success: {
					...
					if (result.type === ChatFetchResponseType.Success) {
						tracePerfChatRequest({
							modelId: normalizedResponseModel ?? chatEndpoint.model,
					...
					if (result.type === ChatFetchResponseType.Success && result.usage) {
						// Store singularity_usage for per-request credits display, scoped to the turn.
						// Skip background requests — they are not part of an active user turn.
						if (typeof result.usage.singularity_usage?.total_nano_aiu === 'number' && turnId && interactionType !== 'conversation-background') {
							this._chatQuotaService.setLastSingularityUsage(result.usage.singularity_usage.total_nano_aiu, topLevelTurnId ?? turnId);
						}
						...
						reportSingularityProjectTokenUsage(
							result.usage,
							normalizedResponseModel ?? chatEndpoint.model,
						);
						...
						otelInferenceSpan?.setAttributes({
							...
							...(result.usage.prompt_tokens_details?.cache_creation_input_tokens !== undefined
								...
								: {}),
							[SingularityChatAttr.TIME_TO_FIRST_TOKEN]: timeToFirstToken,
							...
							...(result.serverRequestId ? { [SingularityChatAttr.SERVER_REQUEST_ID]: result.serverRequestId } : {}),
							...(result.usage.completion_tokens_details?.reasoning_tokens
								...
								: {}),
							...(typeof result.usage.singularity_usage?.total_nano_aiu === 'number'
								? { [SingularityChatAttr.SINGULARITY_USAGE_NANO_AIU]: result.usage.singularity_usage.total_nano_aiu }
								: {}),
						});
					}
					...
					if (otelInferenceSpan && result.type === ChatFetchResponseType.Success) {
						...
						const toolCalls = streamRecorder.deltas
							.filter(d => d.singularityToolCalls?.length)
							.flatMap(d => d.singularityToolCalls!.map(tc => ({
						...
						if (hasThinking) {
							...
							otelInferenceSpan.setAttribute(SingularityChatAttr.REASONING_CONTENT, truncateForOTel(reasoningText || '[encrypted]', this._otelService.config.maxAttributeSizeChars));
						}
					}
				...
				case FetchResponseKind.Canceled:
					...
					otelInferenceSpan?.setAttributes({
						...
						[SingularityChatAttr.CANCELED]: true,
					});
					otelInferenceSpan?.end();
					otelInferenceSpan = undefined;
	...
	private async _getAuthHeaders(isGHEnterprise: boolean, url: string) {
		...
		if (isGHEnterprise) {
			...
			if (url === this._capiClientService.dotcomAPIURL) {
				...
				try {
					token = (await this._authenticationService.getSingularityToken()).token;
				} catch (_err) {
					// Ignore error
					token = '';
				}
	...
	private async _doFetchAndStreamChat(
		...
		const otelSpan = this._otelService.startSpan(`chat ${chatEndpointInfo.model}`, {
			...
			attributes: {
				...
				[SingularityChatAttr.MAX_PROMPT_TOKENS]: chatEndpointInfo.modelMaxPromptTokens,
				...(serverAddress ? { [StdAttr.SERVER_ADDRESS]: serverAddress } : {}),
				...((conversationId ?? chatSessionId) ? { [SingularityChatAttr.SESSION_ID]: conversationId ?? chatSessionId } : {}),
		...
		try {
			...
			// opt out of the CAPI Singularity-token fallback to avoid leaking the user's bearer to a
			...
			if (!chatEndpointInfo.ownsAuthorization) {
				secretKey ??= singularityToken?.token;
			}
	...
	private async _doFetchViaWebSocket(
		...
		if (request.messages?.some((m: CAPIChatMessage) => Array.isArray(m.content) ? m.content.some(c => 'image_url' in c) : false) && chatEndpointInfo.supportsVision) {
			additionalHeaders['Singularity-Vision-Request'] = 'true';
		}
				...
				await new Promise<void>((resolve, reject) => {
					handle.onEvent(event => {
						...
						if (event.type === 'response.completed') {
							const snapshots = (event as any).singularity_quota_snapshots;
		...
		if (this._authenticationService.singularityToken?.isFreeUser && this._authenticationService.singularityToken?.isChatQuotaExceeded) {
			this._authenticationService.resetSingularityToken();
		}
	...
	private async _doFetchViaHttp(
		...
		if (response.status === 200 && this._authenticationService.singularityToken?.isFreeUser && this._authenticationService.singularityToken?.isChatQuotaExceeded) {
			this._authenticationService.resetSingularityToken();
		}
		...
		if (response.status !== 200) {
			...
			return {
				result: await this._handleError(telemetryData, response, ourRequestId, !!chatEndpointInfo.ownsAuthorization),
				fetcher: response.fetcher,
				bytesReceived: response.bytesReceived,
		...
		// CAPI will return us a Singularity Edits Session Header which is our token to using the speculative decoding endpoint
		// We should store this in the auth service for easy use later
		if (response.headers.get('Singularity-Edits-Session')) {
			this._authenticationService.speculativeDecodingEndpointToken = response.headers.get('Singularity-Edits-Session') ?? undefined;
		}
	...
	private async _handleError(
		...
		if (400 <= response.status && response.status < 500) {
			...
			if (response.status === 401 || response.status === 403) {
				// BYOK / gateway endpoints own their auth — a 403 here is usually
				// "model not on free tier" / upstream capacity HTML, not an expired Singularity token.
				if (ownsAuthorization) {
					const detail = typeof jsonData?.detail === 'string' ? jsonData.detail
						: typeof jsonData?.message === 'string' ? jsonData.message
							: undefined;
					const htmlForbidden = /<!doctype html>/i.test(text) || /403 Forbidden/i.test(text);
					const reason = detail
						|| (htmlForbidden
							? `Upstream gateway returned HTTP ${response.status} (often capacity). Retry in a few seconds.`
				...
				this._authenticationService.resetSingularityToken(response.status);
				return {
					type: FetchResponseKind.Failed,
					modelRequestId: modelRequestIdObj,
					failKind: ChatFailKind.TokenExpiredOrInvalid,
			...
			if (response.status === 402) {
				...
				if (!this._authenticationService.singularityToken?.isChatQuotaExceeded) {
					this._authenticationService.resetSingularityToken(response.status);
			...
			if (response.status === 429) {
				...
				rateLimitReason = jsonData?.message ?? jsonData?.detail ?? jsonData?.code ?? rateLimitReason;
				if (typeof rateLimitReason === 'object') {
	...
	private async processSuccessfulResponse(
		...
		const successfulCompletions = completions.filter(c => {
			if (successFinishReasons.has(c.finishReason)) {
				return true;
			}
			if (c.finishReason === FinishedCompletionReason.ClientIterationDone || c.finishReason === FinishedCompletionReason.ClientDone) {
				return getTextPart(c.message.content).trim().length > 0;
			}
			return false;
		});
	...
	private async _handleWebSocketCAPIError(event: CAPIWebSocketErrorEvent, modelRequestId: RequestId): Promise<ChatRequestFailed> {
		...
		if (codePrefix === 'quota_exceeded' || codePrefix === 'free_quota_exceeded' || codePrefix === 'overage_limit_reached' || codePrefix === 'billing_not_configured' || codePrefix === 'additional_spend_limit_reached') {
			// Refresh the singularity token so isChatQuotaExceeded reflects the new state,
			// matching the HTTP 402 handler behavior.
			if (!this._authenticationService.singularityToken?.isChatQuotaExceeded) {
				this._authenticationService.resetSingularityToken(402);
				await this._authenticationService.getSingularityToken();
			}
			return {
				type: FetchResponseKind.Failed,
 ...
 * in Singularity Edits, Singularity AI, Agent Mode, and Inline Chat.
