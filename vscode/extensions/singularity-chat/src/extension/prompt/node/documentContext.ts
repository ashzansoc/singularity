...
export namespace IDocumentContext {
	...
	export function inferDocumentContext(request: vscode.ChatRequest, activeEditor: vscode.TextEditor | undefined, previousTurns: Turn[]): IDocumentContext | undefined {
		...
		if (result) {
			const lastTurnWithInlineResponse = findLast(previousTurns, turn => Boolean(turn.getMetadata(SingularityInteractiveEditorResponse)));
			const data = lastTurnWithInlineResponse?.getMetadata(SingularityInteractiveEditorResponse);
