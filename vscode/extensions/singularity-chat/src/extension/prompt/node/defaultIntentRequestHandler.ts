...
import { IChatHookService, UserPromptSubmitHookInput, UserPromptSubmitHookOutput, StopHookInput } from '../../../platform/chat/common/chatHookService';
...
import { CanceledResult, ChatFetchError, ChatFetchResponseType, ChatLocation, ChatResponse, getErrorDetailsFromChatFetchError, RESPONSE_CONTAINED_NO_CHOICES } from '../../../platform/chat/common/commonTypes';
import { IConversationOptions } from '../../../platform/chat/common/conversationOptions';
import { ISessionTranscriptService } from '../../../platform/chat/common/sessionTranscriptService';
import { ConfigKey, IConfigurationService } from '../../../platform/configuration/common/configurationService';
import { maybeEnrichMessagesWithPromptEngine } from '../../../platform/endpoint/node/singularityPromptEngineBridge';
...
import { SingularityInteractiveEditorResponse, InteractionOutcomeComputer } from '../../inlineChat/node/promptCraftingTypes';
...
export class DefaultIntentRequestHandler {
	...
	constructor(
		...
		private readonly handlerOptions: IDefaultIntentRequestHandlerOptions = { maxToolCallIterations: 15, confirmOnMaxToolIterations: false },
	...
	private makeResponseStreamParticipants(intentInvocation: IIntentInvocation): ResponseStreamParticipant[] {
		...
		if (this.documentContext && this.location === ChatLocation.Editor) {
			participants.push(stream => {
				const firstTurnWithAIEditCollector = this.conversation.turns.find(turn => turn.getMetadata(SingularityInteractiveEditorResponse)?.editSurvivalTracker);
				this._editSurvivalTracker = firstTurnWithAIEditCollector?.getMetadata(SingularityInteractiveEditorResponse)?.editSurvivalTracker ?? this._editSurvivalTrackerService.initialize(this.documentContext!.document.document);
	...
	private async _onDidReceiveResponse({ response, toolCalls, interactionOutcome }: IToolCallingResponseEvent) {
		...
		if (this.documentContext) {
			this.turn.setMetadata(new SingularityInteractiveEditorResponse(
	...
	private async getErrorDetails(error: ChatFetchError) {
		...
		return getErrorDetailsFromChatFetchError(error, this._authenticationService.singularityToken?.singularityPlan, status, this._authenticationService.singularityToken?.tokenBasedBilling, this._authenticationService.singularityToken?.quotaInfo.quota_reset_date);
	}
	...
	private async processResult(fetchResult: ChatResponse, responseMessage: string, chatResult: ChatResult | void, metadataFragment: Partial<IResultMetadata>, baseModelTelemetry: ConversationalBaseTelemetryData, rounds: IToolCallRound[]): Promise<ChatResult> {
		switch (fetchResult.type) {
				...
				if (fetchResult.type === ChatFetchResponseType.RateLimited
					...
					if (this._configurationService.getConfig(ConfigKey.RateLimitAutoSwitchToAuto)) {
						...
						errorDetails.confirmationButtons = [
							{ data: { singularitySwitchToAutoOnRateLimit: true, alwaysSwitchToAuto: true } satisfies ISwitchToAutoOnRateLimitConfirmation, label: l10n.t('Switch to Auto (always)') },
							{ data: { singularitySwitchToAutoOnRateLimit: true, alwaysSwitchToAuto: false } satisfies ISwitchToAutoOnRateLimitConfirmation, label: l10n.t('Switch to Auto') },
						];
					}
				}
				const chatResult = { errorDetails, metadata: metadataFragment };
			...
			case ChatFetchResponseType.Length: {
				// Prefer keeping the partial response as a successful turn so the user
				// is not asked to manually Continue / rephrase after auto-continuations
				// have already been exhausted (or if truncation slipped through).
				const truncated =
					'truncatedValue' in fetchResult && typeof fetchResult.truncatedValue === 'string'
						? fetchResult.truncatedValue
						: responseMessage;
				if (truncated.trim().length > 0 || rounds.some(r => r.response.trim().length > 0 || r.toolCalls.length)) {
					this._logService.info('[DefaultIntentRequestHandler] Length limit reached — accepting partial response without error');
					return this.processSuccessfulFetchResult(
						truncated || responseMessage || rounds.map(r => r.response).filter(Boolean).join('\n'),
						fetchResult.requestId,
						{ ...(chatResult || {}), metadata: metadataFragment },
						baseModelTelemetry,
						rounds,
					);
				}
				const errorDetails = await this.getErrorDetails(fetchResult);
				const chatResultWithErr = { errorDetails, metadata: metadataFragment };
				this.turn.setResponse(TurnStatus.Error, undefined, baseModelTelemetry.properties.messageId, chatResultWithErr);
				return chatResultWithErr;
			}
			...
			case ChatFetchResponseType.Unknown: {
				if (
					fetchResult.reason === RESPONSE_CONTAINED_NO_CHOICES
					&& (
						responseMessage.trim().length > 0
						|| rounds.some(r => r.response.trim().length > 0 || r.toolCalls.length > 0)
					)
				) {
					this._logService.info('[DefaultIntentRequestHandler] Empty final chunk — accepting tool-loop output without error');
					return this.processSuccessfulFetchResult(
						responseMessage || rounds.map(r => r.response).filter(Boolean).join('\n') || 'Done.',
						fetchResult.requestId,
						{ ...(chatResult || {}), metadata: metadataFragment },
						baseModelTelemetry,
						rounds,
				...
				const errorDetails = fetchResult.reason === RESPONSE_CONTAINED_NO_CHOICES && shouldSkipAuxiliaryTokenRouterLlm()
					? {
						code: fetchResult.type,
						message: l10n.t({
							message: 'Sorry, the model returned an empty response after waiting for the beta rate limit. Please wait about a minute and try again. [Learn More]({0})',
							args: ['https://aka.ms/github-singularity-rate-limit-error'],
							comment: [`{Locked=']({'}`],
						}),
					}
					: await this.getErrorDetails(fetchResult);
				const chatResult = { errorDetails, metadata: metadataFragment };
				this.turn.setResponse(TurnStatus.Error, undefined, baseModelTelemetry.properties.messageId, chatResult);
				return chatResult;
			}
...
class DefaultToolCallingLoop extends ToolCallingLoop<IDefaultToolLoopOptions> {
	...
	/**
	 * Design Intelligence Visual Critic — when a frontend session is active and
	 * the model tries to stop, score the UI and force refine on FAIL (≤ 3 iters).
	 */
	protected override async executeStopHook(
		input: StopHookInput,
		sessionId: string,
		outputStream: ChatResponseStream | undefined,
		token: CancellationToken,
	): Promise<{ shouldContinue: boolean; reasons?: string[] }> {
		const base = await super.executeStopHook(input, sessionId, outputStream, token);
		if (base.shouldContinue) {
			return base;
		}

		// Critic toggle off → design-spec + single implement pass only (no refine loop)
		const criticEnabled = this._configurationService.getConfig(ConfigKey.Advanced.VisualCriticEnabled);
		if (!criticEnabled) {
			this._logService.info('[DefaultToolCallingLoop] Visual Critic disabled — allowing stop after single pass');
			return base;
		}

		// Only run Visual Critic for Agent frontend specialty sessions
		if (this.options.intent?.id !== Intent.Agent || !isFrontendSessionActive(sessionId)) {
			return base;
		}

		// Avoid critic on stop_hook_active loops that already have other blockers
		if (input.stop_hook_active && !isFrontendSessionActive(sessionId)) {
			return base;
		}

		try {
			outputStream?.progress(l10n.t('Design Intelligence: Visual Critic (capture + score)…'));
			const critic = await runVisualCriticForAgentStop({
				conversationId: sessionId,
				log: (msg) => this._logService.info(msg),
			});
			if (critic.shouldContinue && critic.reasons.length) {
				this._logService.info('[DefaultToolCallingLoop] Visual Critic FAIL — forcing refine');
				outputStream?.progress(l10n.t('Visual Critic FAIL — refining (≤3)…'));
				return { shouldContinue: true, reasons: critic.reasons };
			}
			this._logService.info('[DefaultToolCallingLoop] Visual Critic PASS — allowing stop');
			outputStream?.progress(l10n.t('Visual Critic PASS'));
		} catch (e) {
			this._logService.warn(
				`[DefaultToolCallingLoop] Visual Critic error: ${e instanceof Error ? e.message : String(e)}`,
			);
		}
	...
	protected override async buildPrompt(buildPromptContext: IBuildPromptContext, progress: Progress<ChatResponseReferencePart | ChatResponseProgressPart>, token: CancellationToken): Promise<IBuildPromptResult> {
		...
		const engineEnabled = this._configurationService.getConfig(ConfigKey.Advanced.SingularityPromptEngineEnabled);
		if (buildPromptResult.messages.length) {
			// Always run bridge: Context Engine ingest+inject for Agent/Ask/Edit;
			// Prompt Engine compile when enabled.
			buildPromptResult.messages = await maybeEnrichMessagesWithPromptEngine(
				buildPromptResult.messages,
				{
					prompt: buildPromptContext.query || this.options.request.prompt || '',
					intent: this.options.intent?.id,
					skipContextInject: (buildPromptContext.toolCallRounds?.length ?? 0) > 0,
				},
				engineEnabled,
				progress,
			);
	...
	protected override async fetch(opts: ToolCallingLoopFetchOptions, token: CancellationToken): Promise<ChatResponse> {
		...
		const modelId = this.options.invocation.endpoint.model ?? '';
		const isFlash = /deepseek-v4-flash/i.test(modelId);
		return this.options.invocation.endpoint.makeChatRequest2({
			...opts,
			modeChanged,
			modelCapabilities: {
				...opts.modelCapabilities,
				enableThinking: !isFlash && isThinkingLocation && opts.modelCapabilities?.enableThinking,
				reasoningEffort,
	...
	protected override async getAvailableTools(outputStream: ChatResponseStream | undefined, token: CancellationToken): Promise<LanguageModelToolInformation[]> {
		...
		for (const name of [
			...
			this.toolGrouping.ensureExpanded(name);
		}
