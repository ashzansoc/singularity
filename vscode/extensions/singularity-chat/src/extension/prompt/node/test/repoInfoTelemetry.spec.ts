...
suite('RepoInfoTelemetry', () => {
	...
	let singularityTokenStore: ISingularityTokenStore;
	...
	beforeEach(() => {
		...
		singularityTokenStore = accessor.get(ISingularityTokenStore);
	...
	test('should not send any telemetry for non-internal users', async () => {
		...
		const nonInternalToken = new SingularityToken(createTestExtendedTokenInfo({
			token: 'test-token',
		...
		singularityTokenStore.singularityToken = nonInternalToken;
	...
	function setupInternalUser() {
		const internalToken = new SingularityToken(createTestExtendedTokenInfo({
			token: 'tid=test;rt=1',
			sku: 'free_limited_singularity',
			expires_at: 9999999999,
			refresh_in: 180000,
		...
		singularityTokenStore.singularityToken = internalToken;
	}
