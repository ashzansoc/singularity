...
suite('PromptCategorizerService', () => {
	...
	beforeEach(() => {
		...
		const singularityTokenStore = {
			singularityToken: { isInternal: true },
		};
		...
		service = new PromptCategorizerService(
			...
			singularityTokenStore as unknown as ISingularityTokenStore,
	...
	test('runs prompt categorization when main endpoint is non-BYOK', async () => {
		...
		expect(endpointProvider.getChatEndpoint).toHaveBeenNthCalledWith(2, 'singularity-utility-small');
