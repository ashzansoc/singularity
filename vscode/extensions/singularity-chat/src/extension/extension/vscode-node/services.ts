...
import { createStaticGitHubTokenProvider, getOrCreateTestingSingularityTokenManager } from '../../../platform/authentication/node/singularityTokenManager';
import { AuthenticationService } from '../../../platform/authentication/vscode-node/authenticationService';
import { VSCodeSingularityTokenManager } from '../../../platform/authentication/vscode-node/singularityTokenManager';
...
import { SingularityInlineCompletionItemProviderService } from '../../completions/vscode-node/singularityInlineCompletionItemProviderService';
...
export function registerServices(builder: IInstantiationServiceBuilder, extensionContext: ExtensionContext): void {
	...
	if (isTestMode || isScenarioAutomation) {
		...
		// method parameters.
		builder.define(ISingularityTokenManager, getOrCreateTestingSingularityTokenManager(env.devDeviceId));
	} else {
		setupTelemetry(builder, extensionContext, internalAIKey, ariaKey);
		builder.define(ISingularityTokenManager, new SyncDescriptor(VSCodeSingularityTokenManager));
	}
	...
	builder.define(ISingularityInlineCompletionItemProviderService, new SyncDescriptor(SingularityInlineCompletionItemProviderService));
	...
	const sessionStoreDbPath = extensionContext.globalStorageUri
		...
		: path.join(os.tmpdir(), 'singularity-session-store.db');
	...
	const otelDbPath = extensionContext.globalStorageUri
		...
		: path.join(os.tmpdir(), 'singularity-agent-traces.db');
	...
	const otelSettings = workspace.getConfiguration('singularity.chat.chat.otel');
...
function setupTelemetry(builder: IInstantiationServiceBuilder, extensionContext: ExtensionContext, _internalAIKey: string, _externalAIKey: string) {
