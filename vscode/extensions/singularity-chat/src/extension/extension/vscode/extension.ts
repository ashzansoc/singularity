...
import { SingularityExtensionApi } from '../../api/vscode/extensionApi';
...
export async function baseActivate(configuration: IExtensionActivationConfiguration) {
	...
	const showSwitchToReleaseViewCtxKey = 'singularity.chat.interactiveSession.switchToReleaseChannel';
	...
	const result = {
		getAPI(version: number) {
			if (version > SingularityExtensionApi.version) {
				throw new Error('Invalid Singularity AI extension API version. Please upgrade Singularity AI.');
			}

			return instantiationService.createInstance(SingularityExtensionApi);
		}
	};
