...
function hydrateBYOKErrorMessages(response: ChatResponse): ChatResponse {
	if (response.type === ChatFetchResponseType.Failed && response.streamError) {
		const streamError = response.streamError as { message?: string; type?: string; param?: unknown };
		const message = typeof streamError.message === 'string' && streamError.message.length
			? streamError.message
			: JSON.stringify(streamError);
		const isRateLimit = /rate.?limit|rate_limit_exceeded|free tier requests/i.test(message)
			|| streamError.type === 'rate_limit_exceeded';
		if (isRateLimit) {
			return {
				...
				isAuto: false,
				capiError: { code: streamError.type, message },
			};
		}
		return {
			type: response.type,
...
export class OpenAIEndpoint extends ChatEndpoint {
	 ...
	 * CAPI Singularity bearer token nor raise a missing-key error for these requests.
	 */
