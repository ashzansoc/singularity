...
/** True when the user supplied their own gateway key (OpenRouter / TokenRouter / DeepSeek). */
function hasOwnLlmCredentials(): boolean {
	return !!(
...
/**
 * Signed-out users are allowed; signed-in users without a Singularity token (e.g. enterprise-managed errors) are denied to avoid bypassing policy.
 * Users with their own API keys are always allowed so OpenRouter BYOK works regardless of GitHub entitlement flags.
 */
export function isClientBYOKAllowed(hasGitHubSession: boolean, singularityToken: Omit<SingularityToken, 'token'> | undefined): boolean {
	if (hasOwnLlmCredentials()) {
		return true;
	}
	if (!hasGitHubSession) {
		return true;
	...
	return singularityToken.isInternal || singularityToken.isIndividual || singularityToken.isClientBYOKEnabled();
}
