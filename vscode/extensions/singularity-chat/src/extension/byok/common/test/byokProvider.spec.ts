...
describe('isClientBYOKAllowed', () => {
	function mockToken(props: { isInternal?: boolean; isIndividual?: boolean; isClientBYOKEnabled?: boolean }): Omit<SingularityToken, 'token'> {
		return {
		...
		} as unknown as Omit<SingularityToken, 'token'>;
	}
	...
	it('denies BYOK when signed-in but the Singularity token is unavailable (e.g. EnterpriseManagedError)', () => {
	...
	it('allows BYOK when the user supplied their own OpenRouter key', () => {
