...
import { SingularityChatAttr, GenAiAttr, NoopOTelService, resolveOTelConfig } from '../../../../platform/otel/common/index';
...
describe('AnthropicLMProvider', () => {
	...
	it('stamps session ids on the chat span from the CapturingToken', async () => {
		...
		expect(chatSpan?.attributes[SingularityChatAttr.SESSION_ID]).toBe('session-xyz');
		expect(chatSpan?.attributes[SingularityChatAttr.CHAT_SESSION_ID]).toBe('session-xyz');
	}, 30_000);
	...
	it('omits session ids on the chat span when no CapturingToken is available', async () => {
		...
		expect(chatSpan?.attributes[SingularityChatAttr.SESSION_ID]).toBeUndefined();
		expect(chatSpan?.attributes[SingularityChatAttr.CHAT_SESSION_ID]).toBeUndefined();
	}, 30_000);
});
