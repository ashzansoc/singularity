...
import { SingularityLanguageModelWrapper } from '../../conversation/vscode-node/languageModelAccess';
...
/** Avoid hammering gateway /models (20–80s) on every vscode.lm.selectChatModels call. */
const OPENAI_COMPAT_MODELS_TTL_MS = 10 * 60_000;
const openAiCompatibleModelsCache = new Map<string, { at: number; models: BYOKKnownModels }>();
const openAiCompatibleModelsInflight = new Map<string, Promise<BYOKKnownModels>>();
...
export abstract class AbstractOpenAICompatibleLMProvider<T extends LanguageModelChatConfiguration = LanguageModelChatConfiguration> extends AbstractLanguageModelChatProvider<T, OpenAICompatibleLanguageModelChatInformation<T>> {
	protected readonly _lmWrapper: SingularityLanguageModelWrapper;
	...
	protected async getAllModels(silent: boolean, apiKey: string | undefined, configuration: T | undefined): Promise<OpenAICompatibleLanguageModelChatInformation<T>[]> {
		...
		if (modelsUrl) {
			const cacheKey = `${modelsUrl}::${apiKey?.slice(0, 16) ?? ''}`;
			const cached = openAiCompatibleModelsCache.get(cacheKey);
			if (cached && Date.now() - cached.at < OPENAI_COMPAT_MODELS_TTL_MS) {
				return byokKnownModelsToAPIInfoWithEffort(this._name, cached.models).map(model => ({
					...model,
					url: modelsUrl,
				}));
			}
			let inflight = openAiCompatibleModelsInflight.get(cacheKey);
			if (!inflight) {
				inflight = this.getModelsFromEndpoint(modelsUrl, silent, apiKey)
					.then((models) => {
						openAiCompatibleModelsCache.set(cacheKey, { at: Date.now(), models });
						return models;
					})
					.finally(() => {
						openAiCompatibleModelsInflight.delete(cacheKey);
					});
				openAiCompatibleModelsInflight.set(cacheKey, inflight);
			}
	...
	protected getDiscoveryExtraHeaders(_apiKey: string | undefined): IStringDictionary<string> {
		return {};
	}
	...
	private async getModelsFromEndpoint(endpoint: string, silent: boolean, apiKey: string | undefined): Promise<BYOKKnownModels> {
		...
		try {
			const headers: IStringDictionary<string> = {
				...
				...this.getDiscoveryExtraHeaders(apiKey),
			};
