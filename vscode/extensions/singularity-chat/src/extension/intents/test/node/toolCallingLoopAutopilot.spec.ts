...
import { IToolCallingLoopOptions, IToolCallSingleResult, isRepeatedToolCallLoop, ToolCallingLoop } from '../../node/toolCallingLoop';
...
describe('isRepeatedToolCallLoop', () => {
	function round(names: string[]): IToolCallRound {
		return {
			id: 'r',
			response: '',
			toolInputRetry: 0,
			toolCalls: names.map((name, i) => ({ name, arguments: '{}', id: `c${i}` })),
		};
	}

	it('detects six identical read_file rounds', () => {
		expect(isRepeatedToolCallLoop(Array.from({ length: 6 }, () => round(['read_file'])))).toBe(true);
	});

	it('does not flag mixed tools or short history', () => {
		expect(isRepeatedToolCallLoop(Array.from({ length: 5 }, () => round(['read_file'])))).toBe(false);
		expect(isRepeatedToolCallLoop([
			round(['read_file']),
			round(['read_file']),
			round(['create_file']),
			round(['read_file']),
