...
class UsageCapturingStream extends ChatResponseStreamImpl {
	public readonly usages: Array<{ promptTokens: number; completionTokens: number; singularityCredits: number | undefined }>;

	constructor() {
		const usages: Array<{ promptTokens: number; completionTokens: number; singularityCredits: number | undefined }> = [];
		super(
			() => { },
			() => { },
			undefined,
			...
			(usage) => {
				usages.push({
					...
					singularityCredits: usage.singularityCredits
				});
			}
		);
		this.usages = usages;
	...
	protected override async fetch(): Promise<ChatResponse> {
		return {
			...
			usage: {
				...
				completion_tokens: 20,
				total_tokens: 120,
				singularity_usage: { total_nano_aiu: 5_000_000_000 }
			},
			resolvedModel: 'gpt-4.1'
		};
	}
...
describe('ToolCallingLoop usage reporting', () => {
	...
	it('reports usage for regular requests', async () => {
		...
		expect(stream.usages).toEqual([{ promptTokens: 100, completionTokens: 20, singularityCredits: undefined }]);
	});
	...
	it('reports credits-only usage for subagent requests', async () => {
		...
		expect(stream.usages).toEqual([{ promptTokens: 0, completionTokens: 0, singularityCredits: 5 }]);
	});

	it('accumulates singularity credits across iterations within a turn', async () => {
		const request = createMockChatRequest();
		const loop = instantiationService.createInstance(
			CreditsTestToolCallingLoop,
			{
		...
		expect(stream.usages.map(u => u.singularityCredits)).toEqual([5, 10]);
	});
});
