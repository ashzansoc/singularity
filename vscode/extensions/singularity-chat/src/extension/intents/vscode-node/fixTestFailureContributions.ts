...
function registerTestMessageSparkles(store: DisposableStore, telemetryService: ITelemetryService, testProvider: ITestProvider) {
	...
	store.add(vscode.commands.registerCommand('singularity.chat.tests.fixTestFailure.fromInline', (item: vscode.TestItem) => {
	...
	store.add(vscode.commands.registerCommand('singularity.chat.tests.fixTestFailure', openFixChat));
...
function registerTestFailureCodeAction(testProvider: ITestProvider, configurationService: IConfigurationService, store: DisposableStore) {
	store.add(vscode.languages.registerCodeActionsProvider('*', {
		provideCodeActions(document, range, context, token) {
			const singularityCodeActionsEnabled = configurationService.getConfig(ConfigKey.EnableCodeActions);
			...
			ca.command = {
				...
				command: 'singularity.chat.tests.fixTestFailure',
				arguments: [{
