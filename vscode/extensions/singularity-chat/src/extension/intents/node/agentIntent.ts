...
import { getActiveDesignBrief, setActiveDesignBrief } from '../../../platform/endpoint/node/designSourcePlanner';
import { formatRoutedModelLabel } from '../../../platform/endpoint/node/routedModelLabel';
import { chatRequestNeedsBlockingTools, isTrivialChatPrompt } from '../../../platform/endpoint/node/singularityPromptEngineBridge';
...
import { capAgentToolCallIterations, getAgentMaxRequests } from '../common/agentConfig';
...
export function isBackgroundTodoAgentEnabled(
	...
	const token = authenticationService.singularityToken;
...
export function resolveSummarizeThresholdTokens(value: number | undefined, effectiveMaxTokens: number, settingId: string): number | undefined {
	...
	if (value < 100) {
		throw new Error(`Setting singularity.chat.${settingId} is too low; use a ratio in the range (0, 1] (e.g. 0.8 for 80%) or an absolute token count of at least 100.`);
	}
...
export const getAgentTools = async (accessor: ServicesAccessor, request: vscode.ChatRequest, model?: IChatEndpoint) => {
	...
	// Singularity token source, so on BYOK / custom endpoints it can abort the chat
	...
	const tools = toolsService.getEnabledTools(request, model, tool => {
		...
		if (tool.tags?.includes('mcp') || tool.name === 'singularity_list_connectors') {
			return true;
		}
...
export class AgentIntent extends EditCodeIntent {
	...
	constructor(
		...
		super(instantiationService, endpointProvider, configurationService, expService, codeMapperService, _workspaceService, { intentInvocation: AgentIntentInvocation, processCodeblocks: false });
	...
	override async handleRequest(
		...
		// Surface routed model in the chat stream + response footer (Singularity Auto).
		// Permanent model pin: Auto routing is forced to DeepSeek V4 Flash via
		// OpenRouter, so the hosted router's suggestion (e.g. claude-haiku /
		// gpt-5-mini) is never the actual model. Suppress the chip entirely —
		// it only ever showed a model we override, which misleads.
		const routingDecision = this._automodeService.consumeLastRoutingDecision();
		const pinnedFlash = routingDecision && /deepseek-v4-flash/i.test(routingDecision.resolvedModel);
		if (routingDecision && pinnedFlash) {
			const label = formatRoutedModelLabel(routingDecision.resolvedModel, routingDecision.resolvedModelName);
			stream.progress(l10n.t('Routed to {0} (`{1}`)', label, routingDecision.resolvedModel));
		}

		// Design Intelligence: await Director on greenfield UI; background refresh on tweaks.
		const prompt = request.prompt ?? '';
		const sessionId = conversation.sessionId;
		const root = this._workspaceService.getWorkspaceFolders()[0]?.fsPath;
		const awaitDesignDirector = shouldAwaitDesignDirector(prompt, sessionId, root);
		if (awaitDesignDirector && promptNeedsDesignIntelligence(prompt)) {
			stream.progress(l10n.t('✦ Design Intelligence — preparing your unique design…'));
		}
		if (awaitDesignDirector) {
			await this._ensureDesignIntelligence(conversation, request, stream);
		} else {
			void this._ensureDesignIntelligence(conversation, request, stream);
		}

		try {
			const result = await super.handleRequest(conversation, request, stream, token, documentContext, agentName, location, chatTelemetry, yieldRequested);
			if (routingDecision) {
				result.details = formatRoutedModelLabel(routingDecision.resolvedModel, routingDecision.resolvedModelName);
			}
			return result;
		} finally {
	...
	/**
	 * Agent-mode Design Director — writes `.singularity/design-spec.json` and
	 * injects Spec into the system prompt before Flash/agent implements UI.
	 */
	private async _ensureDesignIntelligence(
		conversation: Conversation,
		request: vscode.ChatRequest,
		stream: vscode.ChatResponseStream,
	): Promise<void> {
		...
		const onProgress = (update: DesignIntelligenceProgressUpdate) => {
			stream.progress(l10n.t('{0}. {1} — {2}', update.step, update.title, update.detail));
			try {
				...
				/* thinkingProgress may be unavailable */
			}
		};

		try {
			const result = await ensureDesignIntelligenceForTurn({
			...
			const existing = getActiveDesignBrief();
			const merged = mergeBriefWithDesignSpec(
				existing || 'Singularity Design Intelligence active — follow Design Specification.',
				sessionId,
			);
			setActiveDesignBrief(merged);
			if (result.ok) {
				this._logService.info(`[AgentIntent] Design Spec ${result.reused ? 'reused' : 'ready'} ${result.specPath ?? ''}`);
			} else {
				this._logService.info(`[AgentIntent] Design Spec background skipped: ${result.error || 'unknown'}`);
			}

		} catch (e) {
			this._logService.warn(
				`[AgentIntent] Design Director failed: ${e instanceof Error ? e.message : String(e)}`,
			);
		}
	}
	...
	private async handleSummarizeCommand(
		...
		try {
			...
			stream.progress(l10n.t('Summarizing...'));
			...
			if (!summaryMetadata) {
				stream.markdown(l10n.t('Unable to summarize conversation.'));
				return {};
			}
			...
			if (summaryMetadata.usage) {
				stream.usage({
					...
					singularityCredits: nanoAiuToCredits(summaryMetadata.usage.singularity_usage?.total_nano_aiu),
					promptTokenDetails: summaryMetadata.promptTokenDetails,
				});
			}

			stream.markdown(l10n.t('Summarized.'));
...
export class AgentIntentInvocation extends EditCodeIntentInvocation implements IIntentInvocation {
	...
	/** Collect workspace instruction files once per request — not on every tool round. */
	private _collectedInstructionRefs: vscode.ChatPromptReference[] | undefined;
	...
	public override getAvailableTools(): Promise<vscode.LanguageModelToolInformation[]> {
		return this.instantiationService.invokeFunction(async (accessor) => {
			// Agent mode must keep file + terminal tools on every turn, including
			// follow-ups like "continue". Heuristic stripping left DeepSeek asking
			// the user to enable run_in_terminal / create_file.
			return getAgentTools(accessor, this.request);
		});
	}
	...
	override async buildPrompt(
		...
		if (collectInstructionsInExtension && chatRequestNeedsBlockingTools(this.request)) {
			if (this._collectedInstructionRefs === undefined) {
				this._collectedInstructionRefs = [...await this._automaticInstructionsCollector.collect(this.request, token)];
			}
			if (this._collectedInstructionRefs.length > 0) {
				promptContext = { ...promptContext, chatVariables: ChatVariablesCollection.merge(promptContext.chatVariables, new ChatVariablesCollection(this._collectedInstructionRefs)) };
			}
		}
		// Parity scan (`compare`) re-walks the workspace on every tool round and blocked
		// first-token delivery (~19s in this repo). Keep logging off the coding path.


		// Add any references from the codebase invocation to the request
		const codebase = chatRequestNeedsBlockingTools(this.request)
			? await this._getCodebaseReferences(promptContext, token)
			: undefined;
		...
		if (summarizationEnabled && backgroundSummarizer?.state === BackgroundSummarizationState.Completed) {
			if (contextRatio >= BackgroundSummarizationThresholds.applyMinRatio) {
				...
				if (bgResult) {
					...
					progress.report(new ChatResponseProgressPart2(l10n.t('Summarized'), async () => l10n.t('Summarized')));
			...
			if (e instanceof BudgetExceededError && summarizationEnabled) {
				...
				if (backgroundSummarizer && (backgroundSummarizer.state === BackgroundSummarizationState.InProgress || backgroundSummarizer.state === BackgroundSummarizationState.Completed)) {
					...
					if (backgroundSummarizer.state === BackgroundSummarizationState.InProgress) {
						...
						progress.report(new ChatResponseProgressPart2(l10n.t('Summarizing...'), async () => {
							try { await summaryPromise; } catch { }
							return l10n.t('Summarized');
						}));
	...
	modifyErrorDetails(errorDetails: vscode.ChatErrorDetails, response: ChatResponse): vscode.ChatErrorDetails {
		if (!errorDetails.responseIsFiltered) {
			errorDetails.confirmationButtons = [
				...(errorDetails.confirmationButtons ?? []),
				{ data: { singularityContinueOnError: true } satisfies IContinueOnErrorConfirmation, label: l10n.t('Try Again') },
			];
		}
		return errorDetails;
	}
