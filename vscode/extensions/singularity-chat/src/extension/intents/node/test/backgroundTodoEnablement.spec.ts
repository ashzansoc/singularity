...
import { setSingularityToken } from '../../../../platform/authentication/common/staticGitHubAuthenticationService';
...
describe('isBackgroundTodoAgentEnabled', () => {
	...
	const capiEndpoint = { urlOrRequestMetadata: { type: RequestType.ChatCompletions }, modelProvider: 'singularity' } as unknown as IChatEndpoint;
	const byokEndpoint = { urlOrRequestMetadata: 'https://api.example.com/v1/chat', modelProvider: 'custom' } as unknown as IChatEndpoint;

	const paidToken = new SingularityToken(createTestExtendedTokenInfo({ sku: 'singularity_individual', singularity_plan: 'individual' }));
	const freeToken = new SingularityToken(createTestExtendedTokenInfo({ sku: 'free_limited_singularity' }));
	const noAuthToken = new SingularityToken(createTestExtendedTokenInfo({ sku: 'no_auth_limited_singularity' }));

	function auth(singularityToken: SingularityToken | undefined): IAuthenticationService {
		return { singularityToken } as unknown as IAuthenticationService;
	}
	...
	function isEnabled(endpoint: IChatEndpoint, singularityToken: SingularityToken | undefined, experimentEnabled: boolean, request: TestChatRequest = new TestChatRequest('fix the bug')): boolean {
		return isBackgroundTodoAgentEnabled(endpoint, config(experimentEnabled), expService, auth(singularityToken), request);
	}
	...
	test('disabled when there is no Singularity token (signed out)', () => {
...
describe('getAgentTools background todo enablement', () => {
	...
	beforeAll(() => {
		...
		const paidToken = new SingularityToken(createTestExtendedTokenInfo({ sku: 'singularity_individual', singularity_plan: 'individual' }));
		setSingularityToken(accessor.get(IAuthenticationService), paidToken);
...
describe('AgentIntentInvocation._maybeStartBackgroundTodoAgentPass subagent guard', () => {
	...
	function makeStub(request: TestChatRequest, processorLookup: () => unknown) {
		return {
			...
			toolsService: {},
			telemetryService: {},
			authenticationService: { singularityToken: paidToken },
			logService: { debug: () => { } },
		};
	}
...
describe('AgentIntent._runFinalBackgroundTodoPass model-eligibility guard', () => {
	...
	function makeStub(endpoint: IChatEndpoint | undefined, processor: unknown) {
		return {
			...
			configurationService: { getExperimentBasedConfig: () => true },
			expService: {},
			_authenticationService: { singularityToken: paidToken },
		};
	}
