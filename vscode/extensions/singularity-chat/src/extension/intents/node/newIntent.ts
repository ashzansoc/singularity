...
export const CreateProjectCommand = 'singularity.chat.createProject';
export const CreateFileCommand = 'singularity.chat.createFile';
export const OpenFileCommand = 'singularity.chat.openFile';

export class NewWorkspacePreviewContentManagerImpl implements INewWorkspacePreviewContentManager {
	declare readonly _serviceBrand: undefined;
	private readonly singularityContentManager: NewWorkspaceSingularityContentManager;
	...
	constructor(
		...
		this.singularityContentManager = instantiationService.createInstance(NewWorkspaceSingularityContentManager);
	...
	set(responseId: string, projectName: string, fileTree: ChatResponseFileTreePart, serviceArgs: any) {
		...
		if (isGithubWorkspaceUri(fileTree.baseUri)) {
			...
			this.singularityContentManager.set(responseId, projectName, fileTree, serviceArgs);
		}
	}
	...
	get(uri: Uri): FileTreeDataWithContent | undefined {
		...
		if (isGithubWorkspaceUri(uri)) {
			...
			fileContents = this.singularityContentManager.get(uri.authority, uri.path, this.prevFileContents);
		}
...
export class NewWorkspaceSingularityContentManager {
	...
	private _getProjectMetadata(fullPath: string) {
		// Format: vscode-singularity-workspace://<sessionId>/<projectName>/<filePath>
...
class NewWorkspaceGitHubContentManager {
	...
	private _getProjectMetadata(fullPath: string) {
		// Format: vscode-singularity-github-workspace://<sessionId>/<projectName>/<filePath>
		const [, projectName, ...path] = fullPath.split('/');
		return { projectName, path };
	}
...
class NewWorkspaceFileContentManager {
	...
	private _getFileMetadata(fullPath: string) {
		// Format: vscode-singularity-file://<sessionId>/<projectName>/<filePath>
		const [, projectName, ...path] = fullPath.split('/');
		return { projectName, path };
	}
...
export const SingularityWorkspaceScheme = 'vscode-singularity-workspace';
export const GithubWorkspaceScheme = 'vscode-singularity-github-workspace';
export const SingularityFileScheme = 'vscode-singularity-file';

function getNewPreviewUri(requestId: string | undefined, filePath?: string, isGithubRepo: boolean = false,) {
	return Uri.from({
		scheme: isGithubRepo ? GithubWorkspaceScheme : SingularityWorkspaceScheme,
		authority: requestId ?? '',
...
class NewWorkspaceResponseProcessor {
	...
	async processResponse(context: IResponseProcessorContext, inputStream: AsyncIterable<IResponsePart>, outputStream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<void> {
		...
		if (turn.id &&
			...
			const markdownString = new MarkdownString(l10n.t(`Hint: You can [regenerate this project without using this sample](command:workbench.action.chat.open?{0}) or use this [setting](command:workbench.action.openSettings?%5B%22singularity.chat.chat.useProjectTemplates%22%5D) to configure the behavior.`, query));
...
function isCopiltoFileWorkspaceUri(uri: Uri): boolean {
	return uri.scheme === SingularityFileScheme;
}
