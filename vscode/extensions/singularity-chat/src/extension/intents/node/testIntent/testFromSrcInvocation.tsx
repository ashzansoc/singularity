...
import { SingularityIdentityRules } from '../../../prompts/node/base/singularityIdentity';
...
export class TestFromSourceInvocation implements IIntentInvocation {
	 ...
	 *
	 * @remark respects singularity-ignored
	 */
	private async findTestFileForSourceFile(token: CancellationToken): Promise<TestExampleFile | null> {
		...
		if (testExampleFile === undefined || (await this.ignoreService.isSingularityIgnored(testExampleFile))) {
			return null;
		}
...
type Props = PromptElementProps<{
	/**
	 * @remark Assumes the document has already been checked for singularity-ignore, ie, don't pass singularity-ignored files.
	 */
