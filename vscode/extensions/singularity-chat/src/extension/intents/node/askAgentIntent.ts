...
import { chatRequestNeedsBlockingTools } from '../../../platform/endpoint/node/singularityPromptEngineBridge';
...
export class AskAgentIntentInvocation extends AgentIntentInvocation {
	...
	public override async getAvailableTools(): Promise<vscode.LanguageModelToolInformation[]> {
		const tools = await getTools(this.instantiationService, this.request);
		if (chatRequestNeedsBlockingTools(this.request)) {
			return tools;
		}
		return tools.filter(tool =>
			tool.tags?.includes('mcp')
			|| tool.name === 'singularity_list_connectors'
