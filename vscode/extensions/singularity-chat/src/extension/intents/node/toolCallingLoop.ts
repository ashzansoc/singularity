...
import { CanceledResult, ChatFetchResponseType, ChatLocation, ChatResponse, RESPONSE_CONTAINED_NO_CHOICES } from '../../../platform/chat/common/commonTypes';
...
import { SingularityChatAttr, emitAgentTurnEvent, emitSessionStartEvent, GenAiAttr, GenAiMetrics, GenAiOperationName, GenAiProviderName, SingularityAttr, normalizeResponseModel, resolveWorkspaceOTelMetadata, StdAttr, stringifyToolDefinitionsForOTel, truncateForOTel, workspaceMetadataToOTelAttributes } from '../../../platform/otel/common/index';
...
import { ChatResponsePullRequestPart, LanguageModelDataPart2, LanguageModelPartAudience, LanguageModelTextPart, LanguageModelToolResult2 } from '../../../vscodeTypes';
import { TOKENROUTER_BETA_MAX_LOOP_MS, TOKENROUTER_BETA_TOOL_CALL_HARD_CAP } from '../common/agentConfig';
...
/** Same tool set 6 rounds in a row = stuck (read_file / create_file retry loops). */
export function isRepeatedToolCallLoop(rounds: readonly IToolCallRound[], window = 6): boolean {
	if (rounds.length < window) {
		return false;
	}
	const signatures = rounds.slice(-window).map(r =>
		r.toolCalls.map(c => c.name).sort().join(',')
	);
	const first = signatures[0];
	return !!first && signatures.every(s => s === first);
}
...
export abstract class ToolCallingLoop<TOptions extends IToolCallingLoopOptions = IToolCallingLoopOptions> extends Disposable {
	...
	/** Auto-continuations after finish_reason=length (output truncated). */
	private lengthContinuationCount = 0;

	/**
	 * Running total of Singularity credits across every model call in the current
	 ...
	 */
	private _accumulatedSingularityCredits: number | undefined;
	...
	/** Max automatic "continue from where you left off" passes after output length truncation. */
	...
	private planningProgressDeferred: DeferredPromise<void> | undefined;
	/**
	...
	private async _runAutopilotGoalClassifier(result: IToolCallSingleResult, token: CancellationToken): Promise<{ done: boolean; impossible?: boolean; reason: string } | undefined> {
		try {
			const endpoint = await this._endpointProvider.getChatEndpoint('singularity-utility-small');
	...
	/** Cursor-style "Planning next moves" between tool results and the next model turn. */
	private showPlanningNextMoves(outputStream: ChatResponseStream | undefined): void {
		if (!outputStream || this.options.request.subAgentInvocationId) {
			return;
		}
		this.resolvePlanningProgress();
		const deferred = new DeferredPromise<void>();
		this.planningProgressDeferred = deferred;
		outputStream.progress(l10n.t('Planning next moves'), async () => {
			await deferred.p;
			return l10n.t('Planning next moves');
		});
	}

	private resolvePlanningProgress(): void {
		if (this.planningProgressDeferred) {
			this.planningProgressDeferred.complete(undefined);
			this.planningProgressDeferred = undefined;
		}
	}

	private static basenameFromToolPath(filePath: string): string {
		const normalized = filePath.replace(/\\/g, '/');
		const parts = normalized.split('/');
		return parts[parts.length - 1] || filePath;
	}

	/** Roll up completed investigate tools into "Explored a.ts, b.ts, (N searches)". */
	private reportExplorationSummary(outputStream: ChatResponseStream | undefined, round: IToolCallRound): void {
		if (!outputStream || this.options.request.subAgentInvocationId) {
			return;
		}
		...
		for (const call of round.toolCalls) {
			if (!isInvestigatingTool(call.name)) {
				continue;
			}
			const toolName = getToolName(call.name);
			if (
				toolName === ToolName.FindTextInFiles
				|| toolName === ToolName.FindFiles
				|| toolName === ToolName.SearchWorkspaceSymbols
				|| toolName === ToolName.Codebase
				|| toolName === ToolName.GithubTextSearch
				|| toolName === ToolName.GithubSemanticRepoSearch
				|| toolName === ToolName.ToolSearch
			) {
				searchCount++;
				continue;
			}
			if (toolName !== ToolName.ReadFile && toolName !== ToolName.ListDirectory && toolName !== ToolName.ReadProjectStructure) {
				continue;
			}
			try {
				const args = JSON.parse(call.arguments) as Record<string, unknown>;
				const raw = args.filePath ?? args.path ?? args.directoryPath ?? args.uri;
				if (typeof raw !== 'string' || !raw.trim()) {
					continue;
				}
				const name = ToolCallingLoop.basenameFromToolPath(raw.trim());
		...
		if (!fileNames.length && searchCount === 0) {
			return;
		}
		const filePart = fileNames.length ? fileNames.join(', ') : undefined;
		const searchPart = searchCount === 0
			? undefined
			: searchCount === 1
				? l10n.t('1 search')
				: l10n.t('{0} searches', searchCount);
		const summary = filePart && searchPart
			? l10n.t('Explored {0}, ({1})', filePart, searchPart)
			: filePart
				? l10n.t('Explored {0}', filePart)
				: l10n.t('Explored ({0})', searchPart!);
		void outputStream.progress(summary, async () => summary);
	}
	...
	/**
	 * Whether the loop should auto-retry after a failed fetch.
	 * Always retries transient gateway/worker failures (e.g. HTTP 546 WORKER_RESOURCE_LIMIT).
	 * TokenRouter/BYOK rate limits rotate to the next DeepSeek model (Flash → Pro).
	 * Broader retries only in auto-approve/autopilot mode.
	 * Does not retry quota-exceeded or cancellation errors.
	 */
	protected shouldAutoRetry(response: ChatResponse): boolean {
		...
		switch (response.type) {
			case ChatFetchResponseType.RateLimited:
				// Account RPM is shared across models — do not burn more requests here.
				// TokenRouterOpenAIEndpoint waits out the window for "5 requests / 1 minute".
				return false;
			...
			case ChatFetchResponseType.NetworkError: {
				const reason = `${response.reason} ${'streamError' in response && response.streamError ? JSON.stringify(response.streamError) : ''}`;
				if (/WORKER_RESOURCE_LIMIT|\b546\b|\b502\b|\b503\b|\b504\b|not having enough compute|temporarily unavailable|ECONNRESET|ETIMEDOUT|socket hang up/i.test(reason)) {
					return true;
				}
				break;
			}
	...
	public async run(outputStream: ChatResponseStream | undefined, token: CancellationToken): Promise<IToolCallLoopResult> {
		const agentName = this.agentName ?? 'Singularity';
		...
		return this._otelService.startActiveSpan(
			`invoke_agent ${agentName}`,
			{
				...
				attributes: {
					...
					[SingularityChatAttr.SESSION_ID]: this.options.conversation.sessionId,
					...(chatSessionId ? { [SingularityChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}),
					...(parentChatSessionId ? { [SingularityChatAttr.PARENT_CHAT_SESSION_ID]: parentChatSessionId } : {}),
					...(debugLogLabel ? { [SingularityChatAttr.DEBUG_LOG_LABEL]: debugLogLabel } : {}),
					...(customModeName ? { [SingularityChatAttr.MODE_NAME]: customModeName } : {}),
					[SingularityAttr.AGENT_TYPE]: agentType,
			...
			async (span) => {
				...
				// Always capture user input message for the debug panel
				{
					...
					if (userMessage) {
						span.setAttribute(SingularityChatAttr.USER_REQUEST, truncateForOTel(userMessage, maxLen));
					}
					// Emit user_message span event for real-time debug panel streaming
					if (userMessage) {
						span.addEvent('user_message', { content: truncateForOTel(userMessage, maxLen), ...(chatSessionId ? { [SingularityChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}) });
					}
				}
				...
				try {
					...
					span.setAttributes({
						[SingularityChatAttr.TURN_COUNT]: result.toolCallRounds.length,
	...
	private async _runLoop(outputStream: ChatResponseStream | undefined, token: CancellationToken, agentSpan?: ISpanHandle, chatSessionId?: string): Promise<IToolCallLoopResult> {
		...
		const maxLoopMs = shouldSkipAuxiliaryTokenRouterLlm() ? TOKENROUTER_BETA_MAX_LOOP_MS : undefined;

		while (true) {
			if (lastResult && maxLoopMs && Date.now() - loopStartedAt > maxLoopMs) {
				this._logService.warn(
					`[ToolCallingLoop] Stopping after ${Math.round(maxLoopMs / 60_000)}m wall clock (TokenRouter RPM) request=${this.options.request.id} rounds=${this.toolCallRounds.length}`,
				);
				outputStream?.warning(l10n.t('Stopped after {0} minutes to avoid a TokenRouter rate-limit loop. Send a narrower follow-up if you want more work.', Math.round(maxLoopMs / 60_000)));
				lastResult = this.hitToolCallLimit(outputStream, lastResult);
				break;
			}
			if (lastResult && isRepeatedToolCallLoop(this.toolCallRounds)) {
				// Log only — do not stop the agent or show a blocking warning. The tool
				// call limit (with silent auto-extension) is the safety net instead.
				this._logService.trace(
					`[ToolCallingLoop] Repeated tool pattern detected request=${this.options.request.id} last=${this.toolCallRounds.at(-1)?.toolCalls.map(c => c.name).join(',')}`,
				);
			}
			if (lastResult && i++ >= this.options.toolCallLimit) {
				// Silently increase the limit and continue without a confirmation dialog.
				const rpmCapped = shouldSkipAuxiliaryTokenRouterLlm();
				const hardCap = rpmCapped ? TOKENROUTER_BETA_TOOL_CALL_HARD_CAP : 200;
				if (this.options.toolCallLimit < hardCap) {
					this.options.toolCallLimit = Math.min(Math.round(this.options.toolCallLimit * 3 / 2), hardCap);
					if (this.options.request.permissionLevel === 'autopilot') {
			...
			try {
				...
				agentSpan?.addEvent('turn_start', { turnId, ...(chatSessionId ? { [SingularityChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}) });
				this.resolveAutopilotProgress();
				this.resolvePlanningProgress();
				...
				agentSpan?.addEvent('turn_end', { turnId, ...(chatSessionId ? { [SingularityChatAttr.CHAT_SESSION_ID]: chatSessionId } : {}) });
				...
				if (!result.round.toolCalls.length || result.response.type !== ChatFetchResponseType.Success) {
					...
					// Output hit max_tokens / finish_reason=length — automatically continue
					// instead of stopping and asking the user to click Continue.
					if (result.response.type === ChatFetchResponseType.Length) {
						if (this.lengthContinuationCount < ToolCallingLoop.MAX_LENGTH_CONTINUATIONS) {
							this.lengthContinuationCount++;
							const reason =
							...
							this._logService.info(
								`[ToolCallingLoop] Auto-continuing after length truncation ` +
								`(${this.lengthContinuationCount}/${ToolCallingLoop.MAX_LENGTH_CONTINUATIONS})`,
							);
							outputStream?.progress(
								l10n.t(
									'Response hit length limit — continuing automatically ({0}/{1})…',
									this.lengthContinuationCount,
									ToolCallingLoop.MAX_LENGTH_CONTINUATIONS,
								),
							);
							this.stopHookReason = reason;
							result.round.hookContext = formatHookContext([reason]);
							this.stopHookUserInitiated = true;
							continue;
						}
						this._logService.warn(
							`[ToolCallingLoop] Hit max length continuations (${ToolCallingLoop.MAX_LENGTH_CONTINUATIONS}); stopping with partial response`,
						);
					}
					...
					if (result.response.type !== ChatFetchResponseType.Success && this.shouldAutoRetry(result.response)) {
						if (result.response.type === ChatFetchResponseType.RateLimited) {
							try {
								const ep = await this._endpointProvider.getChatEndpoint(this.options.request);
								if (ep.isExtensionContributed || ep.ownsAuthorization) {
									...
									await automode.resolveRateLimitFailover(ep.model, this.options.request);
									outputStream?.progress(l10n.t('Model rate-limited — switching to another DeepSeek model…'));
								} else {
									const { IAutomodeService } = await import('../../../platform/endpoint/node/automodeService');
									const automode = this._instantiationService.invokeFunction(a => a.get(IAutomodeService));
								...
								this._logService.warn(
									`[ToolCallingLoop] Rate-limit failover setup failed: ${e instanceof Error ? e.message : String(e)}`,
								);
							}
						}
			...
			if (result instanceof LanguageModelToolResult2) {
				for (const part of result.content) {
					if (part instanceof LanguageModelDataPart2 && part.mimeType === 'application/pull-request+json' && part.audience?.includes(LanguageModelPartAudience.User)) {
						...
						outputStream?.push(new ChatResponsePullRequestPart({ command: 'singularity.chat.chat.openPullRequestReroute', title: l10n.t('View Pull Request {0}', data.linkTag), arguments: [Number(data.linkTag.substring(1))] }, data.title, data.description, data.author, data.linkTag));
					}
				}
			}
		}
	...
	private hitToolCallLimit(stream: ChatResponseStream | undefined, lastResult: IToolCallSingleResult) {
		// Never prompt the user to continue — callers disable ToolCallLimitBehavior.Confirm.
	...
	public async runOne(outputStream: ChatResponseStream | undefined, iterationNumber: number, token: CancellationToken): Promise<IToolCallSingleResult> {
		...
		if (iterationNumber === 0) {
			this._accumulatedSingularityCredits = undefined;
		}
		...
		if (!this.toolsAvailableEmitted && this.agentSpan && availableTools.length > 0) {
			...
			if (toolDefsJson) {
				this.agentSpan.addEvent('tools_available', {
					...
					...(this.chatSessionIdForTools ? { [SingularityChatAttr.CHAT_SESSION_ID]: this.chatSessionIdForTools } : {}),
				});
			}
		}
		...
		// After prior-round tools finish (during buildPrompt), surface Cursor-style status rows.
		if (iterationNumber > 0) {
			const previousRound = this.toolCallRounds.at(-1);
			if (previousRound?.toolCalls.some(call => isInvestigatingTool(call.name))) {
				this.reportExplorationSummary(outputStream, previousRound);
			}
			if (previousRound?.toolCalls.length) {
				this.showPlanningNextMoves(outputStream);
			}
		}
		...
		const fetchOptions: ToolCallingLoopFetchOptions = {
			...
			finishedCb: async (text, index, delta) => {
				...
				if (delta.singularityToolCalls) {
					toolCalls.push(...delta.singularityToolCalls.map((call): IToolCall => ({
						...call,
		...
		if (fetchResult.type === ChatFetchResponseType.Success && fetchResult.usage && stream) {
			...
			const callCredits = nanoAiuToCredits(fetchResult.usage.singularity_usage?.total_nano_aiu);
			if (callCredits !== undefined) {
				this._accumulatedSingularityCredits = (this._accumulatedSingularityCredits ?? 0) + callCredits;
			}
			if (this.shouldReportUsageToContextWidget()) {
				stream.usage({
					...
					singularityCredits: this._accumulatedSingularityCredits,
					promptTokenDetails,
				});
			} else if (this._accumulatedSingularityCredits !== undefined) {
		...
		// Preserve truncated text so auto-continue can pick up mid-response.
		const truncatedText =
			fetchResult.type === ChatFetchResponseType.Length
				? (fetchResult.truncatedValue ?? '')
				: '';
