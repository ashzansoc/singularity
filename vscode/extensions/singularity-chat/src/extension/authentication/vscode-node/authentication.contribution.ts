...
class AuthUpgradeAsk extends Disposable {
	private static readonly AUTH_UPGRADE_ASK_KEY = 'singularity.shownPermissiveTokenModal';
	...
	constructor(
		...
		super();
		this._register(commands.registerCommand('singularity.chat.chat.triggerPermissiveSignIn', async () => {
	...
	private async waitForChatEnabled() {
		if (!this._authenticationService.hasSingularityTokenSource) {
			// BYOK / air-gapped: do not wait for a Singularity token that may never arrive.
			return;
		}
		if (this._configurationService.getConfig(ConfigKey.Advanced.SingularityRouterEnabled)) {
			// Chat uses TokenRouter. Do not mint a Singularity CAPI token (403 + 3 fetchers ~20s)
			// and do not hang forever waiting for one after that failure.
			return;
		}
		...
		try {
			...
			this._logService.error(error, 'Failed to get singularity token');
			return;
		}

		if (this._authenticationService.singularityToken !== undefined) {
			return;
		}
		...
		await Event.toPromise(
			Event.filter(
				...
				() => this._authenticationService.singularityToken !== undefined
			)
		);
	}
