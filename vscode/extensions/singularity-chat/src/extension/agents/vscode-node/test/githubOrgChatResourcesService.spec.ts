...
suite('GitHubOrgChatResourcesService', () => {
	...
	suite('getPreferredOrganizationName', () => {
		...
		test('prefers Singularity sign-in org over arbitrary first org when no workspace repo', async () => {
			mockWorkspaceService.setWorkspaceFolders([]);
			mockOctoKitService.setUserOrganizations(['firstorg', 'singularityorg', 'thirdorg']);
			// Set Singularity token with organization_login_list indicating Singularity access through 'singularityorg'
			mockAuthService.singularityToken = {
				organizationLoginList: ['singularityorg'],
			} as any;

			const service = createService();
			const orgName = await service.getPreferredOrganizationName();

			assert.equal(orgName, 'singularityorg');
		});

		test('prefers workspace repo org over Singularity sign-in org', async () => {
			mockWorkspaceService.setWorkspaceFolders([URI.file('/workspace')]);
			mockGitService.setRepositoryFetchUrls({
				rootUri: URI.file('/workspace'),
				remoteFetchUrls: ['https://github.com/workspaceorg/repo.git']
			});
			mockOctoKitService.setUserOrganizations(['workspaceorg', 'singularityorg']);
			mockAuthService.singularityToken = {
				organizationLoginList: ['singularityorg'],
			} as any;
		...
		test('uses Singularity org even when not in paginated user org list', async () => {
			mockWorkspaceService.setWorkspaceFolders([]);
			mockOctoKitService.setUserOrganizations(['firstorg', 'secondorg']);
			// Singularity org may not appear in paginated user org list but is still valid
			mockAuthService.singularityToken = {
				organizationLoginList: ['singularityorg'],
			} as any;
			...
			// Singularity token orgs are trusted since they represent validated membership
			assert.equal(orgName, 'singularityorg');
		});

		test('falls back to first org when no Singularity token available', async () => {
			mockWorkspaceService.setWorkspaceFolders([]);
			mockOctoKitService.setUserOrganizations(['firstorg', 'secondorg']);
			mockAuthService.singularityToken = undefined;
		...
		test('uses first matching Singularity org when multiple are available', async () => {
			mockWorkspaceService.setWorkspaceFolders([]);
			mockOctoKitService.setUserOrganizations(['thirdorg', 'secondsingularityorg', 'firstsingularityorg']);
			mockAuthService.singularityToken = {
				organizationLoginList: ['firstsingularityorg', 'secondsingularityorg'],
			} as any;

			const service = createService();
			const orgName = await service.getPreferredOrganizationName();

			// Should match 'firstsingularityorg' first in the singularity org list iteration
			assert.equal(orgName, 'firstsingularityorg');
		});
	});
