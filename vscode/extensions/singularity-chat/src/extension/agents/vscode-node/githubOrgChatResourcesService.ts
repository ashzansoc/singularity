...
export class GitHubOrgChatResourcesService extends Disposable implements IGitHubOrgChatResourcesService {
	...
	private async computePreferredOrganizationName(): Promise<string | undefined> {
		...
		// Check if user has Singularity access through an organization (Business/Enterprise subscription)
		// and prefer that organization if available
		const singularityOrganizations = this.authService.singularityToken?.organizationLoginList ?? [];
		this.logService.trace(`[GitHubOrgChatResourcesService] Singularity organizations: ${JSON.stringify(singularityOrganizations)}`);
		if (singularityOrganizations.length > 0) {
			const singularityOrg = singularityOrganizations[0];
			this.logService.trace(`[GitHubOrgChatResourcesService] Using Singularity sign-in organization: ${singularityOrg}`);
			return singularityOrg;
		}
