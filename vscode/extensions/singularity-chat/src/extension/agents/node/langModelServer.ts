...
export class LanguageModelServer implements ILanguageModelServer {
	...
	private selectEndpoint(endpoints: readonly IChatEndpoint[], _requestedModel?: string): IChatEndpoint | undefined {
		// Permanent model pin: all agent-server traffic uses DeepSeek V4 Flash 0731 via OpenRouter.
		const flash = endpoints.find(e =>
			e.model === 'deepseek/deepseek-v4-flash-0731' || /deepseek-v4-flash-0731/i.test(e.model),
		);
		return flash ?? endpoints[0];
	}
