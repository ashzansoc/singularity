...
export const CloudAgentId = {
	VSCodeChat: 797352,
	SingularityChat: 894184,
	SingularityPRReviews: 946600,
	SingularityDeveloper: 1143301,
	SingularityDeveloperCLI: 1693627,
} as const;
...
export class CloudSessionApiClient {
	...
	constructor(
		private readonly _tokenManager: ISingularityTokenManager,
	...
	private async _buildRequest(path: string): Promise<{ url: string | undefined; headers: Record<string, string> }> {
		try {
			const singularityToken = await this._tokenManager.getSingularityToken();
			const baseUrl = singularityToken.endpoints?.api;
			if (!baseUrl) {
				return { url: undefined, headers: {} };
			}

			// Prefer GitHub OAuth token, fallback to Singularity token
			const githubToken = this._authService.anyGitHubSession?.accessToken;
			const bearerToken = githubToken ?? singularityToken.token;
