...
describe('extractAgentName', () => {
	it('returns trimmed attribute value when present', () => {
		expect(extractAgentName({ name: 'invoke_agent', attributes: { 'gen_ai.agent.name': '  singularity  ' } }))
			.toBe('singularity');
	});

	it('parses agent name from span name when attribute is absent', () => {
		expect(extractAgentName({ name: 'invoke_agent singularity', attributes: {} }))
			.toBe('singularity');
	});

	it('parses multi-word agent name from span name', () => {
		expect(extractAgentName({ name: 'invoke_agent GitHub Singularity Chat', attributes: {} }))
			.toBe('GitHub Singularity Chat');
	});
	...
	it('falls back to span name when attribute is empty string', () => {
		expect(extractAgentName({ name: 'invoke_agent singularity', attributes: { 'gen_ai.agent.name': '' } }))
			.toBe('singularity');
	});
