...
export function translateSpan(
	...
	if (operationName === GenAiOperationName.INVOKE_AGENT) {
		...
		if (!state.started && !subagentId) {
			...
			pushEvent(events, state, 'session.start', {
				...
				version: 1,
				producer: 'vscode-singularity-chat',
	...
	if (operationName === GenAiOperationName.CHAT) {
		...
		if (typeof inputTokens === 'number' && typeof outputTokens === 'number') {
			...
			const timeToFirstToken = span.attributes[SingularityChatAttr.TIME_TO_FIRST_TOKEN] as number | undefined;
		...
		case 'session_start': {
			if (!state.started) {
				...
				pushEventAt(events, state, ts, 'session.start', {
					...
					version: 1,
					producer: 'vscode-singularity-chat',
					singularityVersion: typeof entry.attrs.singularityVersion === 'string' ? entry.attrs.singularityVersion : '1.0.0',
					startTime: ts,
					context: {
