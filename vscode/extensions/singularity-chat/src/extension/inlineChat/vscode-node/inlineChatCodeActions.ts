...
export class QuickFixesProvider implements vscode.CodeActionProvider {
	...
	private static readonly fixKind = vscode.CodeActionKind.QuickFix.append('singularity');
	private static readonly explainKind = vscode.CodeActionKind.QuickFix.append('explain').append('singularity');
	private static readonly reviewKind = vscode.CodeActionKind.RefactorRewrite.append('review').append('singularity');
	...
	async provideCodeActions(doc: vscode.TextDocument, range: vscode.Range, context: vscode.CodeActionContext, cancellationToken: vscode.CancellationToken): Promise<vscode.CodeAction[] | undefined> {
		...
		const singularityCodeActionsEnabled = this.configurationService.getConfig(ConfigKey.EnableCodeActions);
		if (!singularityCodeActionsEnabled) {
			return;
		}

		if (await this.ignoreService.isSingularityIgnored(doc.uri)) {
			return;
		}
		...
		if (altTextQuickFixes) {
			altTextQuickFixes.command = {
				...
				command: 'singularity.chat.chat.generateAltText',
				arguments: [
					{
		...
		if (this.reviewService.isCodeFeedbackEnabled() && !activeTextEditor.selection.isEmpty) {
			...
			reviewAction.command = {
				...
				command: 'singularity.chat.chat.review',
			};
		...
		explainAction.command = {
			...
			command: 'singularity.chat.chat.explain',
			arguments: [query],
		};
...
export class RefactorsProvider implements vscode.CodeActionProvider {
	...
	private static readonly generateOrModifyKind = vscode.CodeActionKind.RefactorRewrite.append('singularity');
	...
	async provideCodeActions(
		...
		return this.provideGenerateUsingSingularityCodeAction(doc, range);
	}

	/**
	 * Provides code action `Generate using Singularity` or `Modify using Singularity`.
	 * - `Generate using Singularity` is shown when the selection is empty and the line of the selection contains only white-space characters or tabs.
	 * - `Modify using Singularity` is shown when the selection is not empty and the selection does not contain only white-space characters or tabs.
	 */
	private provideGenerateUsingSingularityCodeAction(doc: vscode.TextDocument, range: vscode.Range): vscode.CodeAction[] | undefined {
