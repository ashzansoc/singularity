...
suite('githubReviewAgent', () => {
	...
	describe('parseLine', () => {
		...
		test('returns empty array when no singularity_references', () => {
			const result = parseLine('data: {"choices":[]}');
			assert.deepStrictEqual(result, []);
		});

		test('returns empty array when singularity_references is empty', () => {
			const result = parseLine('data: {"singularity_references":[]}');
			assert.deepStrictEqual(result, []);
		});
	...
	describe('githubReview', () => {
		...
		test('processes patches and returns review comments from API response', async () => {
			...
			class TestAuthenticationService extends MockAuthenticationService {
				override getSingularityToken(_force?: boolean): Promise<SingularityToken> {
					return Promise.resolve(new SingularityToken(createTestExtendedTokenInfo({ token: 'test-token', code_review_enabled: true })));
				}
			}
		...
		test('returns info error when all files are ignored', async () => {
			...
			const ignoreService = {
				isSingularityIgnored: () => Promise.resolve(true),
			};
