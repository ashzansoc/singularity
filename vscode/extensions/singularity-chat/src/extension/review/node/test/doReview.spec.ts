...
suite('doReview', () => {
	...
	describe('ReviewSession', () => {
		...
		class MockAuthService {
			_serviceBrand: undefined;
			singularityToken: SingularityToken | null = null;
			tokenToReturn: SingularityToken | null = null;
			...
			getSingularityToken(): Promise<SingularityToken> {
				...
				return Promise.resolve(new SingularityToken(createTestExtendedTokenInfo({ token: 'test-token' })));
			}
		}
		...
		test('returns undefined when user is not authenticated (isNoAuthUser)', async () => {
			...
			mockAuth.singularityToken = new SingularityToken(createTestExtendedTokenInfo({
				token: 'test',
				// This makes isNoAuthUser return true
			}));
			// Simulate no-auth user by setting the token's isNoAuthUser property
			Object.defineProperty(mockAuth.singularityToken, 'isNoAuthUser', { value: true });
		...
		test('returns undefined when selection group but no editor', async () => {
			const mockAuth = new MockAuthService();
			mockAuth.singularityToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'test' }));
		...
		test('proceeds with empty selection when scopeSelector throws non-cancellation error (fall-through behavior)', async () => {
			...
			const mockAuth = new MockAuthService();
			mockAuth.singularityToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'test', code_review_enabled: true }));
			mockAuth.tokenToReturn = mockAuth.singularityToken;
		...
		test('returns error result when getSingularityToken throws', async () => {
			const mockAuth = new MockAuthService();
			mockAuth.singularityToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'test' }));
		...
		test('uses legacy review path when code_review_enabled is false', async () => {
			...
			mockAuth.tokenToReturn = new SingularityToken(createTestExtendedTokenInfo({
				token: 'test',
