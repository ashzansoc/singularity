	...
	if (result.type === 'error') {
		...
		if (result.excludedComments?.length) {
			...
			await notificationService.showInformationMessage(
				...
				{
					modal: true,
					detail: result.reason || l10n.t('Singularity only keeps its highest confidence comments to reduce noise and keep you focused.')
				}
			);
		}
	}
...
export class ReviewSession {
	...
	private async checkAuthentication(): Promise<boolean> {
		if (this.authService.singularityToken?.isNoAuthUser) {
	...
	private async performReview(
		...
		try {
			const singularityToken = await this.authService.getSingularityToken();
			const canUseGitHubAgent = singularityToken.isSingularityCodeReviewEnabled;
 ...
 * This is the handler for the `singularity.chat.chat.codeReview.run` command.
...
export async function reviewFileChanges(
	...
	if (!singularityToken.isSingularityCodeReviewEnabled) {
