...
export async function githubReview(
	...
	const ignored = await Promise.all(changes.map(i => ignoreService.isSingularityIgnored(i.document.uri)));
	...
	if (filteredChanges.length === 0) {
		...
		return {
			type: 'error',
			severity: 'info',
			reason: l10n.t('All input documents are ignored by configuration. Check your .singularityignore file.')
		};
	}
 ...
 * This is the entry point for the `singularity.chat.chat.codeReview.run` command,
...
export async function githubReviewFileUris(
	...
	const ignored = await Promise.all(changes.map(c => ignoreService.isSingularityIgnored(c.uri)));
	const filteredChanges = changes.filter((_, i) => !ignored[i]);
	if (filteredChanges.length === 0) {
		logService.info('All input documents are ignored. Skipping feedback generation.');
		return {
...
interface ParsedResponse {
	singularity_references?: RawReference[];
}
...
export function parseLine(line: string): ResponseReference[] {
	...
	if (Array.isArray(parsedLine.singularity_references) && parsedLine.singularity_references.length > 0) {
		return parsedLine.singularity_references.filter(hasType) as ResponseReference[];
	} else {
		return [];
	}
}
...
async function fetchComments(logService: ILogService, authService: IAuthenticationService, capiClientService: ICAPIClientService, fetcherService: IFetcherService, envService: IEnvService, customInstructionsService: ICustomInstructionsService, workspaceService: IWorkspaceService, kind: 'selection' | 'diff', repository: Repository | undefined, baseFileContents: FileState[], headFileContents: FileState[], cancellationToken: CancellationToken) {
	...
	try {
		const singularityToken = await authService.getSingularityToken();
		response = await capiClientService.makeRequest({
			method: 'POST',
			headers: {
				Authorization: 'Bearer ' + singularityToken.token,
				'X-Singularity-Code-Review-Mode': 'ide',
			},
