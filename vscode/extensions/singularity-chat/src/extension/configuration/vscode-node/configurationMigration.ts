...
ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'singularity.chat.chat.experimental.setupTests.enabled',
	migrateFn: async (value: any) => {
		return [
			['singularity.chat.chat.setupTests.enabled', { value }],
			['singularity.chat.chat.experimental.setupTests.enabled', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'singularity.chat.chat.experimental.codeGeneration.instructions',
	migrateFn: async (value: any) => {
		return [
			['singularity.chat.chat.codeGeneration.instructions', { value }],
			['singularity.chat.chat.experimental.codeGeneration.instructions', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'singularity.chat.chat.experimental.codeGeneration.useInstructionFiles',
	migrateFn: async (value: any) => {
		return [
			['singularity.chat.chat.codeGeneration.useInstructionFiles', { value }],
			['singularity.chat.chat.experimental.codeGeneration.useInstructionFiles', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'singularity.chat.chat.experimental.testGeneration.instructions',
	migrateFn: async (value: any) => {
		return [
			['singularity.chat.chat.testGeneration.instructions', { value }],
			['singularity.chat.chat.experimental.testGeneration.instructions', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'singularity.chat.chat.planAgent.model',
	migrateFn: async (value: any) => {
		return [
			['chat.planAgent.defaultModel', { value }],
			['singularity.chat.chat.planAgent.model', { value: undefined }]
		];
	}
}]);

const oldCursorJumpKey = 'singularity.chat.chat.advanced.inlineEdits.nextCursorPrediction.enabled';
const newCursorJumpKey = 'singularity.chat.nextEditSuggestions.extendedRange';
ConfigurationMigrationRegistry.registerConfigurationMigrations([{
