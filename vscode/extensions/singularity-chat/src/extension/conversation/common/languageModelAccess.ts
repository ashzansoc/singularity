 ...
 * Singularity and BYOK code paths.
 */
...
export function getModelCapabilitiesDescription(endpoint: IChatEndpoint | LanguageModelChatInformation | { name: string; family: string }): string | undefined {
	...
	if (family.includes('gpt') || name.includes('gpt') || family.includes('codex') || name.includes('codex')) {
		...
		if (name.includes('singularity')) {
			return l10n.t('GPT model fine-tuned for Singularity code completions.');
		}
		if (name.includes('4o')) {
 ...
 */
const AUTO_MODEL_DOCS_URL = 'https://docs.github.com/en/singularity/concepts/models/auto-model-selection';
