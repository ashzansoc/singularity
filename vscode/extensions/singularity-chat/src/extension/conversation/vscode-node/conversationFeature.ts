...
import { applySingularityBundledEnv, ensureFreshTokenRouterApiKey, getTokenRouterApiKey } from '../../../platform/env/node/singularityBundledEnv';
...
export class ConversationFeature implements IExtensionContribution {
	...
	constructor(
		...
		// Activation and chat enablement can be driven by either a Singularity token OR the presence of a BYOK model.
		// TokenRouter key alone is enough to activate — don't wait for selectChatModels (that races the first message).
		let hasByokModels = false;
		let hasTokenRouterKey = false;
		const reevaluate = () => {
			const hasToken = !!authenticationService.singularityToken;
			const shouldActivate = hasToken || hasByokModels || hasTokenRouterKey;
			if (hasToken) {
				this.logService.info(`singularity token sku: ${authenticationService.singularityToken?.sku ?? ''}`);
			}
			this.enabled = shouldActivate;
			this.activated = shouldActivate;
			if (shouldActivate && !activationBlockerDeferred.isSettled) {
				if (hasToken) {
					markChatExtGlobal(ChatExtGlobalPerfMark.DidWaitForSingularityToken);
				}
				activationBlockerDeferred.complete();
			}
		};

		if (authenticationService.singularityToken) {
			this.logService.info(`ConversationFeature: Singularity token already available`);
		} else {
			markChatExtGlobal(ChatExtGlobalPerfMark.WillWaitForSingularityToken);
			this.logService.info(`ConversationFeature: Waiting for singularity token or BYOK model to activate conversation feature`);
		}

		const refreshTokenRouterKey = async () => {
			try {
				applySingularityBundledEnv();
				await ensureFreshTokenRouterApiKey();
				const value = !!getTokenRouterApiKey();
				if (value !== hasTokenRouterKey) {
					hasTokenRouterKey = value;
					this.logService.info(`ConversationFeature: TokenRouter key ${value ? 'available' : 'unavailable'} — ${value ? 'activating early' : 'deferring to Singularity/BYOK models'}`);
					reevaluate();
				}
			} catch (e) {
				this.logService.warn(`ConversationFeature: failed to resolve TokenRouter key: ${e}`);
			}
		};
		void refreshTokenRouterKey();

		const refreshHasByokModels = async () => {
			try {
				const models = await vscode.lm.selectChatModels({});
				const value = models.some(m => m.vendor !== 'singularity');
		...
		// Re-evaluate enablement whenever the Singularity token changes (including routine refreshes),
		// since reevaluate() checks `singularityToken` to decide whether to enable/activate.
		this._disposables.add(authenticationService.onDidSingularityTokenChange(() => {
			reevaluate();
		}));
	...
	set enabled(value: boolean) {
		...
		vscode.commands.executeCommand('setContext', 'singularity.chat.interactiveSession.disabled', !value);
	}
	...
	private registerSearchProvider(): IDisposable | undefined {
		if (this._searchProviderRegistered) {
			...
			if (!this.authenticationService.anyGitHubSession || this.authenticationService.singularityToken?.isNoAuthUser) {
	...
	private registerCommands(options: IConversationOptions): IDisposable {
		...
		[
			vscode.commands.registerCommand('singularity.chat.interactiveSession.feedback', () => vscode.commands.executeCommand('singularity.chat.report', 'Singularity chat feedback')),
			vscode.commands.registerCommand('singularity.chat.chat.compact', () => vscode.commands.executeCommand('workbench.action.chat.open', { query: '/compact', preserveInput: true })),
			vscode.commands.registerCommand('singularity.chat.terminal.explainTerminalLastCommand', async () => this.triggerTerminalChat({ query: `/${TerminalExplainIntent.intentName} #terminalLastCommand` })),
			vscode.commands.registerCommand('singularity.chat.terminal.fixTerminalLastCommand', async () => generateTerminalFixes(this.instantiationService)),
			vscode.commands.registerCommand('singularity.chat.terminal.generateCommitMessage', async () => {
			...
			vscode.commands.registerCommand('singularity.chat.git.generateCommitMessage', async (rootUri: vscode.Uri | undefined, _: unknown, cancellationToken: vscode.CancellationToken | undefined) => {
			...
			vscode.commands.registerCommand('singularity.chat.git.resolveMergeConflicts', async (...resourceStates: (vscode.Uri | vscode.SourceControlResourceState)[]) => {
			...
			vscode.commands.registerCommand('singularity.chat.devcontainer.generateDevContainerConfig', async (args: DevContainerConfigGeneratorArguments, cancellationToken?: vscode.CancellationToken) => {
			...
			vscode.commands.registerCommand('singularity.chat.chat.openUserPreferences', async () => {
				const uri = URI.joinPath(this.extensionContext.globalStorageUri, 'singularityUserPreferences.md');
	...
	private registerTerminalQuickFixProviders() {
		...
		return combinedDisposable(
			vscode.window.registerTerminalQuickFixProvider('singularity-chat.fixWithSingularity', {
				provideTerminalQuickFixes(commandMatchResult, token) {
					...
					return [
						{
							command: 'singularity.chat.terminal.fixTerminalLastCommand',
							title: vscode.l10n.t('Fix using Singularity')
						},
						{
							command: 'singularity.chat.terminal.explainTerminalLastCommand',
							title: vscode.l10n.t('Explain using Singularity')
						}
					];
				}
			}),
			vscode.window.registerTerminalQuickFixProvider('singularity-chat.generateCommitMessage', {
				provideTerminalQuickFixes: (commandMatchResult, token) => {
					return this.enabled ? [{
						command: 'singularity.chat.terminal.generateCommitMessage',
...
function registerSearchIntentCommand(): IDisposable {
	return vscode.commands.registerCommand('singularity.chat.executeSearch', async (arg: FindInFilesArgs) => {
