...
import { SingularityFileScheme, SingularityWorkspaceScheme, CreateFileCommand, CreateProjectCommand, GithubWorkspaceScheme, INewWorkspacePreviewContentManager, OpenFileCommand } from '../../intents/node/newIntent';
...
export function registerNewWorkspaceIntentCommand(previewContentManager: INewWorkspacePreviewContentManager, logService: ILogService, options: IConversationOptions) {
	const singularityWorkspaceProvider = new NewWorkspacePreviewFileSystemProvider(previewContentManager);
	const githubWorkspaceProvider = new NewWorkspacePreviewFileSystemProvider(previewContentManager);
	const singularityTextDocumentProvider = new NewWorkspaceTextDocumentProvider(previewContentManager);

	return Disposable.from(
		workspace.registerFileSystemProvider(SingularityWorkspaceScheme,
			singularityWorkspaceProvider,
			{ isReadonly: new MarkdownString(l10n.t('This file preview was generated by Singularity and may contain surprises or mistakes.\n\nAsk followup questions to refine it, then press Create Workspace.')) }),
		workspace.registerFileSystemProvider(GithubWorkspaceScheme,
			githubWorkspaceProvider,
			{ isReadonly: true }),
		workspace.registerTextDocumentContentProvider(SingularityFileScheme, singularityTextDocumentProvider),
			...
			if (uri) {
				...
				window.tabGroups.all.forEach(group => {
					group.tabs.forEach((tab) => {
						if (tab.input instanceof TabInputText && tab.input.uri.scheme === SingularityFileScheme) {
		...
		singularityWorkspaceProvider,
		githubWorkspaceProvider,
		singularityTextDocumentProvider,
	);
}
...
async function createWorkspace(logService: ILogService, workspaceRoot: Uri | undefined, parentFolder: Uri, fileTreePart: ChatResponseFileTreePart) {
	...
	window.tabGroups.all.forEach(group => {
		group.tabs.forEach((tab) => {
			if (tab.input instanceof TabInputText && tab.input.uri.scheme === SingularityWorkspaceScheme) {
				tabsToClose.push(tab);
			}
		});
	});
 ...
 *  - Singularity new-workspace flow: `<projectName>/<inside>` (no leading slash).
