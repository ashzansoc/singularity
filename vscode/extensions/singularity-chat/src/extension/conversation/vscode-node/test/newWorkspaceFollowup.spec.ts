 ...
 * `/<repo>/...`, the singularity flow emits `<projectName>/...`). The platform-
...
suite('resolveProjectFileUri', () => {
	...
	test('handles a non-prefixed file path (singularity new-workspace flow)', () => {
	...
	test('source preview URI built from the unmodified `file` resolves to the expected preview path', () => {
		...
		const baseUri = Uri.parse('vscode-singularity-github-workspace://req/myrepo');
		...
		// Singularity new-workspace flow: emitted file = `myproject/src/index.ts`.
		const singularityBase = Uri.parse('vscode-singularity-workspace://req/myproject');
		expect(Uri.joinPath(singularityBase, 'myproject/src/index.ts').path).toBe('/myproject/myproject/src/index.ts');
	});
});
