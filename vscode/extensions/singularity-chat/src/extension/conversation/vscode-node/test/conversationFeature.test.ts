...
import { setSingularityToken, StaticGitHubAuthenticationService } from '../../../../platform/authentication/common/staticGitHubAuthenticationService';
...
suite('Conversation feature test suite', function () {
	...
	test('The feature is enabled and activated in test mode', function () {
		...
		try {
			...
			setSingularityToken(accessor.get(IAuthenticationService), singularityToken);
	...
	test('The feature activates without a Singularity token when a non-singularity (BYOK) language model is available', async function () {
		...
		const conversationFeature = instaService.createInstance(ConversationFeature);
		try {
			// No Singularity token is set; activation and enablement should be driven by BYOK availability.
