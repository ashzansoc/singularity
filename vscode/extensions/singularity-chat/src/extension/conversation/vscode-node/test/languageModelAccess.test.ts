...
import { SingularityToken, createTestExtendedTokenInfo } from '../../../../platform/authentication/common/singularityToken';
import { ISingularityTokenManager } from '../../../../platform/authentication/common/singularityTokenManager';
...
import { buildUtilityAliasModelInfo, SingularityLanguageModelWrapper, LanguageModelAccess } from '../languageModelAccess';
...
suite('SingularityLanguageModelWrapper', () => {
	...
	suite('validateRequest - invalid', () => {
		let wrapper: SingularityLanguageModelWrapper;
		let endpoint: IChatEndpoint;
		setup(async () => {
			createAccessor();
			endpoint = await accessor.get(IEndpointProvider).getChatEndpoint('singularity-utility');
			wrapper = instaService.createInstance(SingularityLanguageModelWrapper);
		});
...
suite('LanguageModelAccess model info', () => {
	test('does not wait for utility alias endpoint resolution', async () => {
		...
		const singularityToken = new SingularityToken(createTestExtendedTokenInfo({ token: 'token', username: 'fake', singularity_plan: 'unknown' }));
		const testingServiceCollection = createExtensionTestingServices();
		testingServiceCollection.define(ISingularityTokenManager, {
			_serviceBrand: undefined,
			onDidSingularityTokenRefresh: Event.None,
			getSingularityToken: async () => singularityToken,
			resetSingularityToken: () => { },
		} as unknown as ISingularityTokenManager);
	...
	test('refreshes utility aliases when an override uses the same model id from another provider', async () => {
		...
		internals._utilityAliasEndpoints.set('singularity-utility-small', publishedEndpoint);
		internals._promptBaseCountCache = { getBaseCount: async () => 0 };
		try {
			await internals._refreshUtilityOverrides();
			assert.strictEqual(internals._resolvedUtilityEndpoints.get('singularity-utility-small')?.endpoint, resolvedEndpoint);
		} finally {
			languageModelAccess.dispose();
		}
	});
...
suite('buildUtilityAliasModelInfo', () => {
	...
	function makeSingularityEndpoint(overrides: Partial<IChatEndpoint>): IChatEndpoint {
		const endpoint = makeEndpoint(overrides);
		Object.setPrototypeOf(endpoint, SingularityChatEndpoint.prototype);
		return endpoint;
	}
	...
	test('clones an existing singularity-provider entry, overriding id/family/selectable/default', () => {
		const endpoint = makeSingularityEndpoint({ model: 'gpt-4o-mini' });
		const base = makeBaseModelInfo({ id: 'gpt-4o-mini', requiresAuthorization: { label: 'octocat' } });
		const result = buildUtilityAliasModelInfo('singularity-utility-small', endpoint, [base], /* baseCount */ 50, undefined);
	...
	test('synthesizes for non-singularity endpoints when a singularity model shares the same id', () => {
		...
		const base = makeBaseModelInfo({ id: 'gpt-4o-mini', requiresAuthorization: { label: 'octocat' } });
		const result = buildUtilityAliasModelInfo('singularity-utility-small', endpoint, [base], /* baseCount */ 50, { label: 'octocat' });
	...
	test('synthesizes when no matching base entry exists, subtracting baseCount and completion reserve', () => {
		...
		const result = buildUtilityAliasModelInfo('singularity-utility', endpoint, [], /* baseCount */ 100, { label: 'octocat' });

		assert.strictEqual(result.synthesized, true);
		assert.strictEqual(result.info.id, 'singularity-utility');
		assert.strictEqual(result.info.name, 'Unknown Model');
		assert.strictEqual(result.info.family, 'singularity-utility');
		...
		// `vscode.lm.selectChatModels({ vendor: 'singularity', id: 'singularity-utility' })`
