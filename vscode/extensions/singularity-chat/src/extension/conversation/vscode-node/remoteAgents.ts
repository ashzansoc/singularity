...
import { ISingularityReference } from '../../../platform/networking/common/fetch';
...
import { ISingularityChatResult, ISingularityChatResultIn } from '../../prompt/common/conversation';
...
export class RemoteAgentContribution implements IDisposable {
	...
	private async _doRefreshRemoteAgents(): Promise<void> {
		...
		try {
			...
			this.logService.error(e, 'Failed to load remote singularity agents');
		}
	...
	private checkAuthorized(agent: string) {
		...
		const key = `singularity.agent.${agent}.authorized`;
	...
	private async setAuthorized(agent: string, isGlobal = false) {
		...
		await memento.update(`singularity.agent.${agent}.authorized`, true);
	}

	private registerAgent(agentData: IAgent | null): Disposable {
		const store = new DisposableStore();
		const participantId = `singularity.chat-dynamic.${agentData?.slug ?? GITHUB_PLATFORM_AGENT_ID}`;
		...
		const agent = store.add(chat.createDynamicChatParticipant(participantId, dynamicProps, async (request, context, responseStream, token): Promise<ISingularityChatResult> => {
			...
			const metadata: ISingularityChatResult['metadata'] & Record<string, unknown> = {
				sessionId,
				modelMessageId,
				responseId,
			...
			if (request.acceptedConfirmationData) {
				for (const data of request.acceptedConfirmationData) {
					if (data?.url) {
						...
						return { metadata } satisfies ISingularityChatResult;
			...
			if (!agentData?.slug) {
				...
				if (!accessToken) {
					if (this.authenticationService.isMinimalMode) {
						responseStream.markdown(l10n.t('Minimal mode is enabled. You will need to change `singularity.chat.advanced.authPermissions` to `default` to use this feature.'));
						responseStream.button({
							title: l10n.t('Open Settings (JSON)'),
							command: 'workbench.action.openSettingsJson',
							arguments: [{ revealSetting: { key: 'singularity.chat.advanced.authPermissions' } }]
						});
					} else {
			...
			try {
				...
				// Collect singularity skills and references to be sent in the request
				const singularityReferences = [];
				const { singularity_skills } = await this.resolveSingularitySkills(slug, request);
				...
				try {
					...
					if (result.clientReferences) {
						singularityReferences.push(...result.clientReferences);
					}
				...
				const reportedReferences = new Map<string, ISingularityReference>();
				const agentReferences: ISingularityReference[] = [];
				...
				let hadSingularityErrorsOrConfirmations = false;
				...
				const response = await endpoint.makeChatRequest(
					...
					[
						...prepareRemoteAgentHistory(participantId, context),
						{
							...
							...(singularityReferences.length ? { singularity_references: singularityReferences } : undefined),
							...(confirmations?.length ? { singularity_confirmations: confirmations } : undefined),
						}
					],
					async (result, _, delta) => {
						if (delta.singularityReferences) {

							const processReference = (reference: ISingularityReference, parentReference?: ISingularityReference) => {
							...
							for (const reference of delta.singularityReferences) {
						...
						if (delta._deprecatedSingularityFunctionCalls) {
							for (const call of delta._deprecatedSingularityFunctionCalls) {
								switch (call.name) {
									case 'bing-search': {
										try {
						...
						if (delta.singularityErrors && typeof responseStream.warning === 'function') {
							hadSingularityErrorsOrConfirmations = true;
							for (const error of delta.singularityErrors) {
								if (reportedProgress) {
						...
						if (delta.singularityConfirmation) {
							hadSingularityErrorsOrConfirmations = true;
							const confirm = delta.singularityConfirmation;
					...
					undefined,
					{
						...
						singularity_thread_id: sessionId,
						...(singularity_skills ? { singularity_skills } : undefined)
					},
					true,
					{
				...
				metadata['singularity_references'] = [...new Set(reportedReferences.values()).values(), ...agentReferences];
				...
				if (response.type !== ChatFetchResponseType.Success) {
					...
					if (response.reason.includes('400 no docs found')) {
						...
						return { metadata, nextQuestion: { prompt: request.prompt, participant: participantId, command: request.command } } satisfies ISingularityChatResult;
					...
					} else if (response.type !== ChatFetchResponseType.Unknown || !hadSingularityErrorsOrConfirmations) {
						return {
				...
				provider: {
					provideCompletionItems: async (query, token) => {
						...
						return items.map<ChatCompletionItem>(i => {
							const item = new ChatCompletionItem(`singularity.${i.name}`, '#' + i.name, [{ value: i.insertText, level: ChatVariableLevel.Full, description: i.description }]);
	...
	private async prepareClientPlatformReferences(variables: ChatPromptReference[], slug: string) {
		...
		const redactFileContents = async (document: TextDocument, range?: Range) => {
			...
			if (await this.ignoreService.isSingularityIgnored(document.uri)) {
	...
	private async resolveSingularitySkills(agent: string, request: ChatRequest): Promise<{ singularity_skills: string[] }> {
		if (agent === GITHUB_PLATFORM_AGENT_NAME) {
			...
			return { singularity_skills: [...skills] };
		}

		return { singularity_skills: [] };
	}
...
function prepareRemoteAgentHistory(agentId: string, context: ChatContext): Raw.ChatMessage[] {
	...
	for (const h of context.history) {
		...
		if (h instanceof ChatResponseTurn) {
			const singularity_references = h.result.metadata?.['singularity_references'];
			...
			result.push({
				...
				...(singularity_references ? { singularity_references } : undefined)
			});
		}
	}
...
function getOrCreateSessionId(context: ChatContext): string {
	...
	for (const h of context.history) {
		if (h instanceof ChatResponseTurn) {
			const maybeSessionId = (h.result as ISingularityChatResultIn).metadata?.sessionId;
