...
import { ISingularityChatResultIn } from '../../prompt/common/conversation';
...
class ChatAgents implements IDisposable {
	...
	/** White mark on dark themes; black mark on light themes. */
	private getSingularityChatIconPath(): { light: vscode.Uri; dark: vscode.Uri } {
		const assets = vscode.Uri.joinPath(this.extensionContext.extensionUri, 'assets');
		return {
			light: vscode.Uri.joinPath(assets, 'singularity-logo-dark.png'),
			dark: vscode.Uri.joinPath(assets, 'singularity-logo-light.png'),
		};
	}
	...
	private registerTerminalPanelAgent(): IDisposable {
		const terminalPanelAgent = this.createAgent(terminalAgentName, Intent.Terminal, { id: 'singularity.chat.terminalPanel' });
	...
	private registerEditingAgent(): IDisposable {
		...
		editingAgent.iconPath = this.getSingularityChatIconPath();
	...
	private registerDefaultAgent(): IDisposable {
		...
		defaultAgent.iconPath = this.getSingularityChatIconPath();
...
Learn more about [Singularity](https://docs.github.com/singularity/using-github-singularity/getting-started-with-github-singularity?tool=vscode&utm_source=editor&utm_medium=chat-panel&utm_campaign=2024q3-em-MSFT-getstarted) in [Visual Studio Code](https://code.visualstudio.com/docs/singularity/overview). Or explore the [Singularity walkthrough](command:singularity.chat.open.walkthrough).`,
			comment: `{Locked='](command:inlineChat.start)'}`
		});
		const markdownString = new vscode.MarkdownString(helpPostfix);
		markdownString.isTrusted = { enabledCommands: ['inlineChat.start', 'singularity.chat.open.walkthrough'] };
	...
	private getChatParticipantHandler(id: string, name: string, defaultIntentIdOrGetter: IntentOrGetter): vscode.ChatExtendedRequestHandler {
		return async (request, context, stream, token): Promise<vscode.ChatResult> => {
			...
			try {
				...
				if ((result as ISingularityChatResultIn).metadata?.shouldAutoSwitchToAuto) {
	...
	private async switchToBaseModel(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<ChatRequest> {
		...
		if (endpoint.multiplier === 0 || request.model.vendor !== 'singularity' || endpoint.multiplier === undefined) {
			return request;
		}
		if (this._chatQuotaService.additionalUsageEnabled || !this._chatQuotaService.quotaExhausted) {
			return request;
		}
		const baseEndpoint = await this.endpointProvider.getChatEndpoint('singularity-utility');
		const baseLmModel = (await vscode.lm.selectChatModels({ id: baseEndpoint.model, family: baseEndpoint.family, vendor: 'singularity' }))[0];
		if (!baseLmModel) {
			return request;
		}
		...
		if (this.authenticationService.singularityToken?.isIndividual) {
	...
	private async switchToAutoModel(request: vscode.ChatRequest, stream: vscode.ChatResponseStream, alwaysSwitchToAuto: boolean): Promise<ChatRequest> {
		const autoModel = (await vscode.lm.selectChatModels({ id: 'auto', vendor: 'singularity' }))[0];
		if (!autoModel) {
			return request;
		}
		...
		if (alwaysSwitchToAuto) {
			await vscode.workspace.getConfiguration('singularity.chat').update('chat.rateLimitAutoSwitchToAuto', true, vscode.ConfigurationTarget.Global);
		}
