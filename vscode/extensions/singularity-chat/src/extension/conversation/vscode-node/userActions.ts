...
import { Conversation, ISingularityChatResultIn } from '../../prompt/common/conversation';
...
export class UserFeedbackService implements IUserFeedbackService {
	...
	handleUserAction(e: vscode.ChatUserActionEvent, agentId: string): void {
		...
		const result = e.result as ISingularityChatResultIn;
		const conversation = result.metadata?.responseId && this.conversationStore.getConversation(result.metadata.responseId);

		if (typeof conversation === 'object' && conversation.getLatestTurn().getMetadata(SingularityInteractiveEditorResponse)) {
	...
	private handleApplyAction(e: vscode.ChatApplyAction, agentId: string, result: ISingularityChatResultIn): void {
		sendUserActionTelemetry(
			this.telemetryService,
			vscode.window.activeTextEditor?.document,
			{
	...
	private _handleChatUserAction(sessionId: string | undefined, conversation: Conversation, event: vscode.ChatUserActionEvent) {
		...
		const response = conversation.getLatestTurn().getMetadata(SingularityInteractiveEditorResponse);
		if (!response) {
			return;
		}
