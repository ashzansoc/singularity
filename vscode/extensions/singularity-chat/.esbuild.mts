...
const webviewBuildOptions = {
	...
	entryPoints: [
		{ in: 'src/extension/completions-core/vscode-node/extension/src/singularityPanel/webView/suggestionsPanelWebview.ts', out: 'suggestionsPanelWebview' },
	],
} satisfies esbuild.BuildOptions;
		...
		build.onLoad({ filter: /^vscode-dynamic$/, namespace: 'vscode-fallback' }, () => {
			return {
				contents: `
					...
					if (typeof SINGULARITY_SIMULATION_VSCODE !== 'undefined') {
						vscode = SINGULARITY_SIMULATION_VSCODE;
					} else {
						try {
...
const nodeExtHostBuildOptions = {
	...
	entryPoints: [
		...
		{ in: './src/extension/chatSessions/singularitycli/node/singularityCLITodoWorker.ts', out: 'singularityCLITodoWorker' },
		{ in: './src/extension/onboardDebug/node/singularityDebugWorker/index.ts', out: 'singularityDebugCommand' },
		{ in: './src/extension/chatSessions/vscode-node/singularityCLIShim.ts', out: 'singularityCLIShim' },
...
async function typeScriptServerPluginPackageJsonInstall(): Promise<void> {
	await mkdir('./node_modules/@vscode/singularity-typescript-server-plugin', { recursive: true });
	const source = path.join(import.meta.dirname, './src/extension/typescriptContext/serverPlugin/package.json');
	const destination = path.join(import.meta.dirname, './node_modules/@vscode/singularity-typescript-server-plugin/package.json');
	try {
...
const typeScriptServerPluginBuildOptions = {
	...
	logLevel: 'info',
	minify: !isDev,
	outdir: './node_modules/@vscode/singularity-typescript-server-plugin/dist',
	platform: 'node',
...
async function moveSourceMapsToSeparateDir(): Promise<void> {
	...
	const outputDirs = [
		'./dist',
		'./node_modules/@vscode/singularity-typescript-server-plugin/dist',
	];
