...
export default new class NoUnlayeredFiles implements eslint.Rule.RuleModule {
	...
	create(context: eslint.Rule.RuleContext): eslint.Rule.RuleListener {
		...
		// Use only the path relative to extensions/singularity-chat/ to avoid false positives
		...
		const singularityPrefix = `extensions${path.sep}singularity${path.sep}`;
		const idx = context.filename.indexOf(singularityPrefix);
		const relativePath = idx >= 0
			? context.filename.slice(idx + singularityPrefix.length)
			: context.filename;
