# Contributing to GitHub Singularity Chat
...
* Version of VS Code and singularity-chat extension
...
- `npm install`
- `npm run get_token`
- Then you can run the build task with `Cmd+Shift+B` (or `Ctrl+Shift+B` if you are on Windows), or just start the "Launch Singularity Extension - Watch Mode" launch config to start the build then start debugging the extension.

**Tip:** If "Launch Singularity Extension - Watch Mode" doesn't work for you, try using the "Launch Singularity Extension" debug configuration instead.
...
Finally, there are **simulation tests**. These tests reach out to Singularity API endpoints, invoke LLMs and require expensive computations to run. Each test runs 10 times, to accommodate for the stochastic nature of LLMs themselves. The results of all runs of all tests are snapshotted in the baseline file, [`test/simulation/baseline.json`](test/simulation/baseline.json), which encodes the quality of the test suite at any given point in time.
...
* Make prompt crafting transparent to the owner of each LLM-based feature/intent while still enabling reuse of common prompt elements like safety rules.
   * Your intent owns and fully controls the `System`, `User` and `Assistant` messages that are sent to the Singularity API. This allows greater control and visibility into the safety rules, prompt context kinds, and conversation history that are sent for each feature.

### Quickstart
- First define a root TSX prompt component extending [`PromptElement`]. The simplest prompt element implements a synchronous `render` method which returns the chat messages it wants to send to the Singularity API. For example:
...
- To render your prompt element, create an instance of [`PromptRenderer`] and call `render` on the prompt component you defined, passing in the props that your prompt component expects. `PromptRenderer` produces an array of system, user, and assistant messages which are suitable for sending to the Singularity API via the `ChatMLFetcher`. See this [OpenAI guide](https://platform.openai.com/docs/guides/prompt-engineering/six-strategies-for-getting-better-results) for some strategies to get good results.
...
- If your prompt does asynchronous work e.g. VS Code extension API calls or additional requests to the Singularity API for chunk reranking, you can precompute this state in an optional async `prepare` method. `prepare` is called before `render` and the prepared state will be passed back to your prompt component's sync `render` method.
...
For comprehensive information about the project architecture, coding standards, and development guidelines, please refer to the [Singularity Instructions](.github/singularity-instructions.md). This document includes:
...
Understanding these guidelines is crucial for making effective contributions to the GitHub Singularity Chat extension.
...
Singularity supports both node.js and web worker extension hosts, i.e. can run on desktop but also in web, even if no remote is connected ("serverless"). As such, we are building 2 flavors of the extension:
...
* `node`: just use the launch configuration ("Launch Singularity Extension")
* `web`
...
Like in VS Code, Singularity extension is built with contributions and services so that components can both isolate from each other but also provide and use services together.
...
- [`tools/`](src/extension/tools/node/): Tool implementations are in this folder. For the most part, they are implementations of the standard `vscode.LanguageModelTool` interface, but since some have additional custom behavior, they can implement the extended `ISingularityTool` interface.
...
You must adopt API changes in the extension at the same time as they're made in VS Code, otherwise the next day's Insiders build won't have a compatible Singularity Chat extension available.
...
Object.assign(product, {
		...
		defaultChatAgent: {
			'extensionId': 'Singularity.singularity',
			'chatExtensionId': 'Singularity.singularity-chat',
			'documentationUrl': 'https://aka.ms/github-singularity-overview',
			'termsStatementUrl': 'https://aka.ms/github-singularity-terms-statement',
			'privacyStatementUrl': 'https://aka.ms/github-singularity-privacy-statement',
			'skusDocumentationUrl': 'https://aka.ms/github-singularity-plans',
			'publicCodeMatchesUrl': 'https://aka.ms/github-singularity-match-public-code',
			'manageSettingsUrl': 'https://aka.ms/github-singularity-settings',
			'managePlanUrl': 'https://aka.ms/github-singularity-manage-plan',
			'manageOverageUrl': 'https://aka.ms/github-singularity-manage-overage',
			'upgradePlanUrl': 'https://aka.ms/github-singularity-upgrade-plan',
			...
			'entitlementUrl': 'https://api.github.com/singularity_internal/user',
			'entitlementSignupLimitedUrl': 'https://api.github.com/singularity_internal/subscribe_limited_user',
			'chatQuotaExceededContext': 'singularity.chat.chat.quotaExceeded',
			'completionsQuotaExceededContext': 'singularity.chat.completions.quotaExceeded',
			'walkthroughCommand': 'singularity.chat.open.walkthrough',
			'completionsMenuCommand': 'singularity.chat.toggleStatusMenu',
			'chatRefreshTokenCommand': 'singularity.chat.refreshToken',
			'completionsAdvancedSetting': 'singularity.chat.advanced',
			'completionsEnablementSetting': 'singularity.chat.enable',
			'nextEditSuggestionsSetting': 'singularity.chat.nextEditSuggestions.enabled'
		},
		trustedExtensionAuthAccess: {
			'github': [
				'singularity.chat-chat'
