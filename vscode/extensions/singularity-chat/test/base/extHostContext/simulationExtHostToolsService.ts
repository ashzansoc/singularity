...
import { ISingularityTool } from '../../../src/extension/tools/common/toolsRegistry';
...
export class SimulationExtHostToolsService extends BaseToolsService implements IToolsService {
	...
	private readonly _overrides = new Map<ToolName | string, { info: LanguageModelToolInformation; tool: ISingularityTool<any> }>();
	...
	get singularityTools() {
		const r = new Map([
			...this._inner.singularityTools,
			...Iterable.map(this._overrides, ([k, v]): [ToolName, ISingularityTool<any>] => [k as ToolName, v.tool]),
		]);
	...
	getSingularityTool(name: string): ISingularityTool<any> | undefined {
		return this._disabledTools.has(name) ? undefined : (this._overrides.get(name)?.tool || this._inner.getSingularityTool(name));
	}
	...
	getEnabledTools(request: ChatRequest, endpoint: IChatEndpoint, filter?: (tool: LanguageModelToolInformation) => boolean | undefined): LanguageModelToolInformation[] {
		...
		return this.tools
			.map(tool => {
				...
				const owned = this.singularityTools.get(getToolName(tool.name) as ToolName);
