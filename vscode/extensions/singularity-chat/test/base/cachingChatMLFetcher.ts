...
import { FinishedCallback, ISingularityToolCall, IResponseDelta, OptionalChatRequestParams } from '../../src/platform/networking/common/fetch';
...
export type CachedExtraData = { cacheMetadata: CachedResponseMetadata | undefined; singularityFunctionCalls?: ISingularityToolCall[]; logprobs?: ChoiceLogProbs };
			...
			if (this.cacheMode !== CacheMode.Disable) {
				...
				if (cacheValue) {
					if (cacheValue.type === ChatFetchResponseType.Success) {
						await opts.finishedCb?.(cacheValue.value[0], 0, { text: cacheValue.value[0], singularityToolCalls: cacheValue.singularityFunctionCalls, logprobs: cacheValue.logprobs });
					} else if (cacheValue.type === ChatFetchResponseType.Length) {
						await opts.finishedCb?.(cacheValue.truncatedValue, 0, { text: cacheValue.truncatedValue, singularityToolCalls: cacheValue.singularityFunctionCalls, logprobs: cacheValue.logprobs });
					}
...
`;
			...
			if (
				...
				const cachedResponse: CachedResponse = {
					...result,
					cacheMetadata,
					singularityFunctionCalls: callbackWrapper.singularityFunctionCalls,
...
class FinishedCallbackWrapper {
	public readonly singularityFunctionCalls: ISingularityToolCall[] = [];
	...
	public getCb(): FinishedCallback {
		return async (text: string, index: number, delta: IResponseDelta): Promise<number | undefined> => {
			if (delta.singularityToolCalls) {
				this.singularityFunctionCalls.push(...delta.singularityToolCalls);
			}
			if (delta.logprobs) {
				if (!this.logprobs) {
