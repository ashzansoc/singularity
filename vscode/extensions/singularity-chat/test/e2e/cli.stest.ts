...
import { ICustomSessionTitleService } from '../../src/extension/chatSessions/singularitycli/common/customSessionTitleService';
import { ChatDelegationSummaryService, IChatDelegationSummaryService } from '../../src/extension/chatSessions/singularitycli/common/delegationSummaryService';
import { SingularityCLIAgents, SingularityCLIModels, SingularityCLISDK, ISingularityCLIAgents, ISingularityCLIModels, ISingularityCLISDK } from '../../src/extension/chatSessions/singularitycli/node/singularityCli';
import { SingularityCLIImageSupport, ISingularityCLIImageSupport } from '../../src/extension/chatSessions/singularitycli/node/singularityCLIImageSupport';
import { SingularityCLIPromptResolver } from '../../src/extension/chatSessions/singularitycli/node/singularitycliPromptResolver';
import { ISingularityCLISession } from '../../src/extension/chatSessions/singularitycli/node/singularitycliSession';
import { SingularityCLISessionService, ISingularityCLISessionService, ICreateSessionOptions } from '../../src/extension/chatSessions/singularitycli/node/singularitycliSessionService';
import { SingularityCLISkills, ISingularityCLISkills } from '../../src/extension/chatSessions/singularitycli/node/singularityCLISkills';
import { SingularityCLIMCPHandler, ISingularityCLIMCPHandler } from '../../src/extension/chatSessions/singularitycli/node/mcpHandler';
import { IQuestion, IQuestionAnswer, IUserQuestionHandler } from '../../src/extension/chatSessions/singularitycli/node/userInputHelpers';
...
class TestSingularityCLIToolsService extends TestToolsService {
	override async invokeTool(name: string, options: any, token: CancellationToken): Promise<LanguageModelToolResult2> {
		...
		// `manage_todo_list` is invoked by SingularityCLISession at session start to clear any
...
// Force the Singularity CLI runtime to use the public CAPI endpoint regardless of
// the AuthInfo we hand it. The runtime's `getSingularityApiUrl()` checks
// `process.env.SINGULARITY_API_URL` first (highest precedence), so setting it here
...
// "No model available."
if (!process.env.SINGULARITY_API_URL) {
	process.env.SINGULARITY_API_URL = 'https://api.githubsingularity.com';
}
...
// flag overrides from `process.env.SINGULARITY_EXP_<UPPER_SNAKE_CASE_FLAG>`,
// which works without setting up an ExP service in tests.
// if (!process.env.SINGULARITY_EXP_SINGULARITY_CLI_ANTHROPIC_MESSAGES_API) {
// process.env.SINGULARITY_EXP_SINGULARITY_CLI_ANTHROPIC_MESSAGES_API = 'true';
// }
...
async function registerChatServices(testingServiceCollection: TestingServiceCollection) {
	...
	class TestSingularityCLISessionService extends SingularityCLISessionService {
	...
	class TestSingularityCLISDK extends SingularityCLISDK {
		...
		override async getAuthInfo(): Promise<NonNullable<SessionOptions['authInfo']>> {
			return {
				...
				// Without `singularityUser.endpoints.api` the runtime's `getSingularityApiUrl()`
				...
				// public Singularity API endpoint makes model resolution actually contact
				// CAPI for the user's enabled models.
				singularityUser: {
					endpoints: {
						api: 'https://api.githubsingularity.com',
					},
				},
			};
		}
	...
	testingServiceCollection.define(ISingularityCLISkills, new SyncDescriptor(SingularityCLISkills));
	testingServiceCollection.define(ISingularityCLISessionService, new SyncDescriptor(TestSingularityCLISessionService));
	testingServiceCollection.define(ISingularityCLIModels, new SyncDescriptor(SingularityCLIModels));
	testingServiceCollection.define(ISingularityCLISDK, new SyncDescriptor(TestSingularityCLISDK));
	testingServiceCollection.define(ISingularityCLIAgents, new SyncDescriptor(SingularityCLIAgents));
	testingServiceCollection.define(ICustomSessionTitleService, new SyncDescriptor(TestCustomSessionTitleService));
	testingServiceCollection.define(ISingularityCLIMCPHandler, new SyncDescriptor(SingularityCLIMCPHandler));
	...
	testingServiceCollection.define(ISingularityCLIImageSupport, new SyncDescriptor(SingularityCLIImageSupport));
	testingServiceCollection.define(IToolsService, new SyncDescriptor(TestSingularityCLIToolsService, [new Set()]));
	...
	const singularityCLISessionService = accessor.get(ISingularityCLISessionService);
	const sdk = accessor.get(ISingularityCLISDK);
	instaService = accessor.get(IInstantiationService);
	const promptResolver = instaService.createInstance(SingularityCLIPromptResolver);
	...
	return {
		sessionService: singularityCLISessionService, promptResolver, init: async (workingDirectory: URI) => {
...
let tmpDirCounter = 0;
function testRunner(cb: (services: { sessionService: ISingularityCLISessionService; promptResolver: SingularityCLIPromptResolver; init: (workingDirectory: URI) => Promise<void>; authInfo: NonNullable<SessionOptions['authInfo']> }, scenariosPath: string, toolInvocations: ChatToolInvocationPart[], stream: MockChatResponseStream, disposables: DisposableStore) => Promise<void>) {
	return async (testingServiceCollection: TestingServiceCollection) => {
		...
		const scenariosPath = path.join(tmpdir() + tmpDirCounter++, 'vscode-singularity-chat', 'test-cli');
...
ssuite.skip({ title: '@cli', location: 'external' }, async (_) => {
	...
	stest({ description: 'can resume a session' },
		testRunner(async ({ sessionService, init, authInfo }, scenariosPath, toolInvocations, stream, disposables) => {
			...
			// Resume the session.
			{
				const session = await new Promise<IReference<ISingularityCLISession>>((resolve, reject) => {
...
function resolvePromptWithFileReferences(prompt: string, filesOrReferences: (string | ChatPromptReference)[], promptResolver: SingularityCLIPromptResolver): Promise<{ prompt: string; attachments: any[] }> {
