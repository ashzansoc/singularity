...
function detectedParticipantToIntentId(detected: ChatParticipantDetectionResult) {
	switch (detected.participant) {
		case 'singularity.chat.default':
			return 'unknown';
		case `singularity.chat.${editingSessionAgentEditorName}`:
			if (detected.command) {
				return detected.command;
			}
			return 'unknown';
		case 'singularity.chat.terminalPanel':
			return 'terminalExplain';
		case `singularity.chat.${editsAgentName}`:
		...
		case 'singularity.chat.vscode':
			return 'vscode';
		case 'singularity.chat-dynamic.platform':
			return 'singularity.chat-dynamic.platform';
	}
