...
import { SingularityToolMode } from '../../../src/extension/tools/common/toolsRegistry';
...
ssuite({ title: 'tooltest', subtitle: 'toolcall', location: 'panel' }, (inputPath) => {
	...
	async function invokeTool(
		...
		const tool = toolsService.getSingularityTool(toolName);
		...
		if (tool.resolveInput) {
			...
			processedArgs = await tool.resolveInput(processedArgs, context, SingularityToolMode.FullContext);
		}
