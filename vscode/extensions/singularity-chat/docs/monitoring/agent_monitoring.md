...
Singularity Chat can export **traces**, **metrics**, and **events** via [OpenTelemetry](https://opentelemetry.io/) (OTel) — giving you real-time visibility into agent interactions, LLM calls, tool executions, and token usage.
...
The fastest way to see Singularity Chat traces locally — no cloud account required. This guide uses the [Aspire Dashboard](https://aspire.dev/dashboard/standalone/), a lightweight container image from Microsoft that provides a trace viewer with a built-in OTLP endpoint. It can be used standalone, without the rest of .NET Aspire.

### Prerequisites

- **Docker** installed
- **VS Code** with the GitHub Singularity Chat extension
...
```json
{
  "singularity.chat.chat.otel.enabled": true,
  "singularity.chat.chat.otel.captureContent": true
}
```
...
Open Singularity Chat and send any message — for example, ask a question in Agent mode.
...
Open **Settings** (`Ctrl+,`) and search for `singularity otel`:

| Setting | Type | Default | Description |
|---|---|---|---|
| `singularity.chat.chat.otel.enabled` | boolean | `false` | Enable OTel emission |
| `singularity.chat.chat.otel.exporterType` | string | `"otlp-http"` | `otlp-http`, `otlp-grpc`, `console`, or `file` |
| `singularity.chat.chat.otel.otlpEndpoint` | string | `"http://localhost:4318"` | OTLP collector endpoint |
| `singularity.chat.chat.otel.captureContent` | boolean | `false` | Capture full prompt/response content |
| `singularity.chat.chat.otel.protocol` | string | `""` | OTLP wire protocol: `http/json` (default), `http/protobuf`, or `grpc` |
| `singularity.chat.chat.otel.serviceName` | string | `""` | Override the `service.name` resource attribute |
| `singularity.chat.chat.otel.resourceAttributes` | object | `{}` | Extra resource attributes (`{ "key": "value" }`) |
| `singularity.chat.chat.otel.headers` | object | `{}` | Extra OTLP exporter headers, applied directly to the exporter (`{ "key": "value" }`) |
| `singularity.chat.chat.otel.maxAttributeSizeChars` | integer | `0` | Max characters per OTel content attribute (prompts, tool args/results, hook input/output). `0` (the default) disables truncation so backends with no per-attribute limit get full payloads. Set to a positive value to match your backend's per-attribute size limit — consult your backend's documentation. The value counts JavaScript string characters (UTF-16 code units); for non-ASCII content one character can be multiple UTF-8 bytes on the wire. |
| `singularity.chat.chat.otel.outfile` | string | `""` | File path for JSON-lines output |
| `singularity.chat.chat.otel.dbSpanExporter.enabled` | boolean | `false` | Persist OTel spans to a local SQLite database for the **Chat: Export Agent Traces DB** command. Implicitly enables OTel. |

### Environment Variables

Environment variables take precedence over VS Code settings, and **enterprise managed settings (policy) take precedence over both** — admins can centrally mandate any `singularity.chat.chat.otel.*` value.

| Variable | Default | Description |
|---|---|---|
| `SINGULARITY_OTEL_ENABLED` | `false` | Enable OTel. Also enabled when `OTEL_EXPORTER_OTLP_ENDPOINT` is set. |
| `SINGULARITY_OTEL_ENDPOINT` | — | OTLP endpoint URL (takes precedence over `OTEL_EXPORTER_OTLP_ENDPOINT`) |
...
| `SINGULARITY_OTEL_PROTOCOL` | — | Override OTLP protocol (`grpc` or `http`). Falls back to `OTEL_EXPORTER_OTLP_PROTOCOL`. |
| `OTEL_SERVICE_NAME` | `singularity-chat` | Service name in resource attributes |
| `OTEL_RESOURCE_ATTRIBUTES` | — | Extra resource attributes (`key1=val1,key2=val2`) |
| `SINGULARITY_OTEL_CAPTURE_CONTENT` | `false` | Capture full prompt/response content |
| `SINGULARITY_OTEL_MAX_ATTRIBUTE_SIZE_CHARS` | `0` | Override the max character size for OTel content attributes. `0` (default) disables truncation; set to a positive value when your backend has a per-attribute limit. Takes precedence over the `maxAttributeSizeChars` setting. |
| `SINGULARITY_OTEL_LOG_LEVEL` | `info` | Min log level: `trace`, `debug`, `info`, `warn`, `error` |
| `SINGULARITY_OTEL_FILE_EXPORTER_PATH` | — | Write all signals to this file (JSON-lines) |
| `SINGULARITY_OTEL_HTTP_INSTRUMENTATION` | `false` | Enable HTTP-level OTel instrumentation |
...
- `SINGULARITY_OTEL_ENABLED=true`, or
- `OTEL_EXPORTER_OTLP_ENDPOINT` is set, or
- `singularity.chat.chat.otel.enabled` is `true`, or
- `singularity.chat.chat.otel.dbSpanExporter.enabled` is `true` (the SDK pipeline must be active to feed the SQLite store).

### Commands

| Command | Description |
|---|---|
| **Chat: Export Agent Traces DB** (`singularity.chat.chat.otel.exportAgentTracesDB`) | Export the local SQLite span database to a `.db` file. Only available when `singularity.chat.chat.otel.dbSpanExporter.enabled` is `true`. |
...
>
> Singularity Chat emits OTel attributes under three namespaces:
>
...
> - **`singularity.chat.*`** — Canonical Singularity-specific namespace. Prefer this for new dashboards and alerts.
> - **`singularity_chat.*`** — Original VS Code extension namespace. Several keys (notably `singularity_chat.repo.*` and `gen_ai.usage.reasoning_tokens`) are now **dual-emitted alongside the `singularity.chat.*` equivalents**. Tables below mark these rows as **Legacy** with a pointer to the preferred key.
>
...
Singularity Chat emits a hierarchical span tree for each agent interaction:

```
invoke_agent singularity                           [~15s]
...
| `gen_ai.agent.name` | Required | `singularity` |
...
| `singularity.chat.agent.type` | Always | `builtin` \| `custom` \| `plugin` |
| `singularity.chat.git.repository` | When in a repo | `https://github.com/microsoft/vscode.git` |
| `singularity.chat.git.branch` | When in a repo | `main` |
| `singularity.chat.git.commit_sha` | When in a repo | `deadbeef...` |
| `singularity.chat.github.org` | GitHub remotes only | `microsoft` |
| `singularity_chat.repo.remote_url` | **Legacy** — prefer `singularity.chat.git.repository` | `https://github.com/...` |
| `singularity_chat.repo.head_branch_name` | **Legacy** — prefer `singularity.chat.git.branch` | `main` |
| `singularity_chat.repo.head_commit_hash` | **Legacy** — prefer `singularity.chat.git.commit_sha` | `deadbeef...` |
| `singularity_chat.turn_count` | Always | `4` |
...
| `singularity_chat.session_id` | Session correlation | `a1b2c3d4-...` |
| `singularity_chat.chat_session_id` | Session correlation | VS Code chat session ID |
| `gen_ai.request.max_tokens` | Always | `2048` |
| `gen_ai.request.temperature` | When set | `0.1` |
| `gen_ai.request.top_p` | When set | `0.95` |
| `singularity_chat.request.max_prompt_tokens` | Always | `128000` |
...
| `singularity_chat.time_to_first_token` | On response | `450` |
...
| `singularity_chat.debug_name` | When available | `agentMode` |
...
| `singularity.chat.tool.parameters.edit_type` | Edit tools | `create` \| `update` \| `str_replace` \| `insert` |
| `singularity.chat.tool.parameters.skill_name` | When invoking a skill | `auto-perf-optimize` |
| `singularity.chat.tool.parameters.mcp_server_name_hash` | MCP tools | SHA-256 hex of server name |
| `singularity.chat.tool.parameters.mcp_tool_name` | MCP tools | `search_issues` |
| `singularity.chat.tool.parameters.command` | Shell tools, opt-in (captureContent) | `npm test` (truncated to 256 chars) |
| `singularity.chat.tool.parameters.file_path` | File tools, opt-in (captureContent) | `/src/app.ts` |
| `singularity.chat.tool.parameters.mcp_server_name` | MCP tools, opt-in (captureContent) | `github` |
...
| Metric | Type | Unit | Description |
|---|---|---|---|
| `singularity_chat.tool.call.count` | Counter | calls | Tool invocations by name and success |
| `singularity_chat.tool.call.duration` | Histogram | ms | Tool execution latency |
| `singularity_chat.agent.invocation.duration` | Histogram | s | Agent mode end-to-end duration |
| `singularity_chat.agent.turn.count` | Histogram | turns | LLM round-trips per agent invocation |
| `singularity_chat.session.count` | Counter | sessions | Chat sessions started |
| `singularity_chat.time_to_first_token` | Histogram | s | Time to first SSE token |

**`singularity_chat.tool.call.count` attributes:** `gen_ai.tool.name`, `success` (boolean)

**`singularity_chat.tool.call.duration` attributes:** `gen_ai.tool.name`

**`singularity_chat.agent.invocation.duration` attributes:** `gen_ai.agent.name`

**`singularity_chat.agent.turn.count` attributes:** `gen_ai.agent.name`

**`singularity_chat.time_to_first_token` attributes:** `gen_ai.request.model`
...
| Metric | Type | Unit | Description |
|---|---|---|---|
| `singularity_chat.edit.acceptance.count` | Counter | edits | Edit accept/reject decisions (inline chat, chat editing, hunk-level) |
| `singularity_chat.chat_edit.outcome.count` | Counter | edits | File-level chat editing session outcomes (accepted/rejected/saved) |
| `singularity_chat.lines_of_code.count` | Counter | lines | Lines of code added/removed by accepted agent edits |
| `singularity_chat.edit.survival.four_gram` | Histogram | ratio (0-1) | 4-gram text similarity survival score |
| `singularity_chat.edit.survival.no_revert` | Histogram | ratio (0-1) | No-revert survival score |
| `singularity_chat.user.action.count` | Counter | actions | User engagement: copy, insert, apply, followup |
| `singularity_chat.user.feedback.count` | Counter | votes | Thumbs up/down on chat responses |
| `singularity_chat.agent.edit_response.count` | Counter | responses | Agent edit responses by success/error |
| `singularity_chat.agent.summarization.count` | Counter | events | Context summarization outcomes (applied/failed) |
| `singularity_chat.pull_request.count` | Counter | PRs | Pull requests created via CLI agent |
| `singularity_chat.cloud.session.count` | Counter | sessions | Cloud/remote agent sessions by partner |
| `singularity_chat.cloud.pr_ready.count` | Counter | events | Remote agent job PR ready notifications (v1/Jobs API only) |
| `singularity_chat.cloud.operation.count` | Counter | operations | Cloud backend operation outcomes (create, fetch, follow-up, PR) by backend version |
| `singularity_chat.cloud.operation.duration` | Histogram | ms | Cloud backend operation latency by backend version |
| `singularity_chat.cloud.error.count` | Counter | errors | Cloud backend operation failures by backend version and error type |

**`singularity_chat.edit.acceptance.count` attributes:** `singularity_chat.edit.source` (`inline_chat`/`chat_editing`/`chat_editing_hunk`/`apply_patch`/`replace_string`/`code_mapper`), `singularity_chat.edit.outcome` (`accepted`/`rejected`), `singularity_chat.language_id` (optional)

**`singularity_chat.chat_edit.outcome.count` attributes:** `singularity_chat.edit.source`, `singularity_chat.edit.outcome` (`accepted`/`rejected`/`saved`), `singularity_chat.language_id` (optional), `singularity_chat.has_remaining_edits` (optional)

**`singularity_chat.lines_of_code.count` attributes:** `type` (`added`/`removed`), `singularity_chat.language_id` (optional)

**`singularity_chat.edit.survival.four_gram` attributes:** `singularity_chat.edit.source`, `singularity_chat.time_delay_ms`

**`singularity_chat.edit.survival.no_revert` attributes:** `singularity_chat.edit.source`, `singularity_chat.time_delay_ms`

**`singularity_chat.user.action.count` attributes:** `action` (`copy`/`insert`/`apply`/`followup`)

**`singularity_chat.user.feedback.count` attributes:** `rating` (`positive`/`negative`)

**`singularity_chat.agent.edit_response.count` attributes:** `outcome` (`success`/`error`)

**`singularity_chat.agent.summarization.count` attributes:** `outcome` (`applied`/`failed`)

**`singularity_chat.cloud.session.count` attributes:** `partner_agent` (`singularity`/`claude`/`codex`), `singularity.chat.cloud.backend_version` (`v1`/`v2`, optional)

**`singularity_chat.cloud.pr_ready.count` attributes:** `singularity.chat.cloud.backend_version` (`v1`, optional)

**`singularity_chat.cloud.operation.count` attributes:** `operation` (`createSession`/`fetchSessionList`/`fetchContent`/`fetchEvents`/`pollUpdate`/`followUp`/`findTaskForPullRequest`/`createPullRequest`/`sessionActivated`), `singularity.chat.cloud.backend_version` (`v1`/`v2`), `success`

**`singularity_chat.cloud.operation.duration` attributes:** `operation`, `singularity.chat.cloud.backend_version` (`v1`/`v2`)

**`singularity_chat.cloud.error.count` attributes:** `operation`, `singularity.chat.cloud.backend_version` (`v1`/`v2`), `error.type` (low-cardinality classifier, e.g. `http_500`)
...
#### `singularity_chat.session.start`
...
##### `singularity_chat.edit.feedback`
...
##### `singularity_chat.edit.hunk.action`
...
##### `singularity_chat.edit.survival`
...
##### `singularity_chat.user.feedback`
...
##### `singularity_chat.cloud.session.invoke`

Emitted when a cloud/remote agent session is started.

| Attribute | Description |
|---|---|
| `partner_agent` | `singularity`, `claude`, or `codex` |
...
| Attribute | Value |
|---|---|
| `service.name` | `singularity-chat` (override via the `singularity.chat.chat.otel.serviceName` setting, `OTEL_SERVICE_NAME`, or enterprise policy) |
...
{
  "singularity.chat.chat.otel.enabled": true,
  "singularity.chat.chat.otel.exporterType": "otlp-grpc",
  "singularity.chat.chat.otel.otlpEndpoint": "http://localhost:4317"
}
```
...
{
  "singularity.chat.chat.otel.enabled": true,
  "singularity.chat.chat.otel.otlpEndpoint": "https://collector.example.com:4318"
}
```

> **Note:** Authentication headers can be set via the `singularity.chat.chat.otel.headers` setting (a `{ "key": "value" }` map applied directly to the exporter) or the `OTEL_EXPORTER_OTLP_HEADERS` environment variable, and can be mandated by enterprise policy. See [Environment Variables](#environment-variables).
...
{
  "singularity.chat.chat.otel.enabled": true,
  "singularity.chat.chat.otel.exporterType": "file",
  "singularity.chat.chat.otel.outfile": "/tmp/singularity-otel.jsonl"
}
```
...
{
  "singularity.chat.chat.otel.enabled": true,
  "singularity.chat.chat.otel.exporterType": "console"
}
```
...
When an agent invokes a subagent (e.g., via the `runSubagent` tool), Singularity Chat automatically propagates the trace context so the subagent's `invoke_agent` span is parented to the calling agent's `execute_tool` span. This produces a connected trace tree:

```
invoke_agent singularity                           [~30s]
  ├── chat gpt-4o                              [~3s]
...
## Background Agents (Singularity CLI)

When OTel is enabled, **all agent types** are automatically instrumented — no additional configuration needed. The same settings that enable foreground agent traces also enable Singularity CLI traces.

### Singularity CLI (Background Agent)

The Singularity CLI SDK runs in the same VS Code process and produces a rich trace hierarchy including subagents, permissions, hooks, and tool calls:

```
singularity-chat invoke_agent singularitycli           [~45s]  ← extension wrapper
  └── github-singularity invoke_agent              [~42s]  ← SDK native spans
...
The extension wrapper span (`invoke_agent singularitycli`, service `singularity-chat`) parents the SDK's native spans (service `github-singularity`). Both appear in the same trace in your collector.
...
> **Content in the debug panel**: When OTel export is disabled (the default), the debug panel automatically captures full prompt/response content. When OTel export is enabled, content capture is controlled by the `captureContent` setting — the same flag applies to both the debug panel and OTLP export. To see content in the debug panel while OTel is enabled, set `singularity.chat.chat.otel.captureContent` to `true`.

### Singularity CLI (Terminal Session)

Terminal CLI sessions ("New Singularity CLI Session") run as a separate process. When OTel is enabled, the extension forwards `SINGULARITY_OTEL_ENABLED` and `OTEL_EXPORTER_OTLP_ENDPOINT` to the terminal process. Terminal traces appear as **independent root traces** (service `github-singularity`) — they are not linked to extension traces.
...
| `service.name` | Source |
|---|---|
| `singularity-chat` | Foreground agent, CLI wrapper, and Claude agent spans (extension-emitted) |
| `github-singularity` | CLI SDK native spans + CLI terminal |
| `claude-code` | Claude Code subprocess SDK telemetry (when `CLAUDE_CODE_ENABLE_TELEMETRY` is forwarded) |

Within the `singularity-chat` service, distinguish agent types by `gen_ai.agent.name`:

| `gen_ai.agent.name` | Agent Type |
|---|---|
| `GitHub Singularity Chat` | Foreground agent (agent mode) |
| `singularitycli` | CLI wrapper span |
| `claude` | Claude agent |
...
When OTel is enabled, Claude agent sessions produce extension-level spans (service `singularity-chat`) following GenAI semantic conventions.
...
```
singularity-chat invoke_agent claude               [~33s]
...
| `singularity.chat.agent.type` | `builtin` |
| `singularity.chat.git.repository` | `https://github.com/microsoft/vscode.git` |
| `singularity.chat.git.branch` | `main` |
| `singularity.chat.git.commit_sha` | `deadbeef...` |
| `singularity.chat.github.org` | `microsoft` |
| `singularity_chat.turn_count` | `8` |
| `singularity_chat.total_cost_usd` | `0.067` (session-wide, includes subagents) |
| `singularity_chat.chat_session_id` | VS Code session ID |
...
| `singularity_chat.session_id` | Session correlation | `a1b2c3d4-...` |
| `singularity_chat.chat_session_id` | Session correlation | VS Code chat session ID |
| `singularity.chat.tool.parameters.edit_type` | Edit tools (`Write`, `Edit`, `MultiEdit`, `NotebookEdit`) | `create` \| `str_replace` \| `update` |
| `singularity.chat.tool.parameters.mcp_server_name_hash` | MCP tools | SHA-256 hex of server name |
| `singularity.chat.tool.parameters.mcp_tool_name` | MCP tools | `search_issues` |
| `singularity.chat.tool.parameters.command` | Shell tools (`Bash`), opt-in (captureContent) | `npm test` (truncated to 256 chars) |
| `singularity.chat.tool.parameters.file_path` | File tools (`Read`, `Edit`, `MultiEdit`, `Write`, `NotebookEdit`), opt-in (captureContent) | `/src/app.ts` |
| `singularity.chat.tool.parameters.mcp_server_name` | MCP tools, opt-in (captureContent) | `github` |
...
| `singularity_chat.session_id` | Session correlation | `a1b2c3d4-...` |
| `singularity_chat.chat_session_id` | Session correlation | VS Code chat session ID |
| `singularity_chat.hook_type` | Required | `PreToolUse` |
| `singularity_chat.hook_result_kind` | Always | `success` \| `error` \| `non_blocking_error` |
| `singularity.chat.hook.decision` | Always | `pass` \| `block` \| `non_blocking_error` |
| `singularity.chat.hook.duration` | Always | `0.142` (seconds) |
| `singularity.chat.hook.tool_names` | When tool-scoped | `["bash"]` (JSON array) |
| `singularity_chat.hook_input` | Always | hook input payload (truncated) |
| `singularity_chat.hook_output` | On success | hook stdout (truncated) |
| `error.type` | On error | `Error` |
...
**Metrics** — Track token usage trends by model and provider, monitor tool success rates via `singularity_chat.tool.call.count`, and watch perceived latency with `singularity_chat.time_to_first_token`. Agent activity metrics (`singularity_chat.edit.acceptance.count`, `singularity_chat.edit.survival.four_gram`, `singularity_chat.lines_of_code.count`) power accept rate and edit survival dashboards. All metrics carry the same resource attributes (`service.name`, `service.version`, `session.id`) for consistent filtering.

**Events** — `singularity_chat.session.start` tracks session creation. `singularity_chat.tool.call` events provide per-invocation timing and error details. `singularity_chat.edit.feedback` and `singularity_chat.edit.survival` events enable drill-down into which edits were accepted/rejected and how code survival varies by edit source. `singularity_chat.user.feedback` links thumbs-up/down votes to specific conversations for quality investigation. `gen_ai.client.inference.operation.details` gives the full LLM call record including token usage and, when content capture is enabled, the complete prompt/response messages. Use `gen_ai.conversation.id` to correlate all signals belonging to the same session.
...
Singularity Chat's OTel data works with any OTLP-compatible backend. This section provides step-by-step setup guides for recommended backends.
...
{
  "singularity.chat.chat.otel.enabled": true,
  "singularity.chat.chat.otel.exporterType": "otlp-http",
  "singularity.chat.chat.otel.otlpEndpoint": "http://localhost:4328"
}
```
...
**4. Generate telemetry** — Open Singularity Chat and send any message (e.g., use Agent mode).

**5. Verify data:**

- **Jaeger (local):** Open http://localhost:16687, select service `singularity-chat`, click **Find Traces**.
...
| where message contains "GenAI" or message contains "singularity_chat"
...
customMetrics
| where timestamp > ago(1h)
| where name startswith "gen_ai" or name startswith "singularity_chat"
...
{
  "singularity.chat.chat.otel.enabled": true,
  "singularity.chat.chat.otel.otlpEndpoint": "http://localhost:4318"
}
```

**3. Verify:** Open http://localhost:16686, select service `singularity-chat`, and click **Find Traces**.
...
{
  "singularity.chat.chat.otel.enabled": true,
  "singularity.chat.chat.otel.otlpEndpoint": "http://localhost:3000/api/public/otel",
  "singularity.chat.chat.otel.captureContent": true
}
```
...
Any OTLP-compatible backend works with Singularity Chat's OTel output. Some options:
