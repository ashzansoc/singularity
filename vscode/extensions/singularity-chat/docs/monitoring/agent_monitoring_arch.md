...
This document describes the architecture, code structure, and conventions for the OpenTelemetry instrumentation in the Singularity Chat extension. It covers all four agent execution paths.
...
### Singularity CLI Bridge SpanProcessor

The extension injects a `SingularityCliBridgeSpanProcessor` into the SDK's `BasicTracerProvider` to forward completed spans to the debug panel. See [otel-data-flow.html](otel-data-flow.html) for the full visual diagram.

```
Extension Root (tracer A):
  invoke_agent singularitycli → traceparent → SDK
...
```
invoke_agent singularity (INTERNAL)          ← toolCallingLoop.ts
...
#### Singularity CLI in-process (Bridge)

```
invoke_agent singularitycli (INTERNAL)       ← singularitycliSession.ts (tracer A)
└── [traceparent linked]
...
#### Singularity CLI terminal (independent)

```
invoke_agent (CLIENT)                    ← standalone singularity binary
│   service.name = github-singularity
├── chat gpt-4o (CLIENT)
...
src/extension/chatSessions/singularitycli/node/
├── singularityCliBridgeSpanProcessor.ts  # Bridge: SDK spans → IOTelService (+ hook span enrichment)
├── singularitycliSession.ts              # Root invoke_agent span + traceparent + hook event stash
...
| `singularitycliSession.ts` | `invoke_agent singularitycli` wrapper span + traceparent propagation + hook event stash |
| `singularityCliBridgeSpanProcessor.ts` | Bridge: SDK `ReadableSpan` → `ICompletedSpanData` (with hook-span enrichment) |
| `singularitycliSessionService.ts` | Bridge installation + OTel env vars for SDK |
| `singularityCLITerminalIntegration.ts` | OTel env vars forwarded to terminal process |
...
2. `SINGULARITY_OTEL_*` env vars
3. `OTEL_EXPORTER_OTLP_*` standard env vars
4. VS Code settings (`singularity.chat.chat.otel.*`)
5. Defaults (lowest)
...
| `envVar` | `SINGULARITY_OTEL_ENABLED=true` |
| `setting` | `singularity.chat.chat.otel.enabled` is `true` |
| `otlpEndpointEnvVar` | `OTEL_EXPORTER_OTLP_ENDPOINT` is set without an explicit enable |
| `dbSpanExporterOnly` | Only `singularity.chat.chat.otel.dbSpanExporter.enabled` is on — implicitly turns OTel on so the SDK pipeline can feed the SQLite store |
...
| Extension Config | Singularity CLI (`deriveSingularityCliOTelEnv`) | Claude Code (`deriveClaudeOTelEnv`) |
|---|---|---|
| `enabled` | `SINGULARITY_OTEL_ENABLED=true` | `CLAUDE_CODE_ENABLE_TELEMETRY=1`, `OTEL_METRICS_EXPORTER=otlp`, `OTEL_LOGS_EXPORTER=otlp` |
...
| `fileExporterPath` | `SINGULARITY_OTEL_FILE_EXPORTER_PATH` (+ `SINGULARITY_OTEL_EXPORTER_TYPE=file` when `exporterType === 'file'`) | N/A (Claude SDK has no file exporter) |
...
The CLI SDK's OTel (`OtelLifecycle`) is **always initialized** regardless of user OTel settings. This ensures the debug panel always receives SDK native spans via the bridge. The `SINGULARITY_OTEL_ENABLED` env var is set before `LocalSessionManager` construction so the SDK creates its `OtelSessionTracker`.
...
| Source | `service.name` |
|---|---|
| Extension (Provider A) | `singularity-chat` |
| Singularity CLI SDK / terminal | `github-singularity` |
...
All free-form content attributes (prompts, tool arguments/results, hook input/output, reasoning text) **must** be passed through `truncateForOTel(value, otel.config.maxAttributeSizeChars)` before being set on a span. The default `maxAttributeSizeChars` is `0` (no truncation), matching the [OTel spec default of `Infinity`](https://opentelemetry.io/docs/specs/otel/common/#attribute-limits) for `AttributeValueLengthLimit`. Users whose OTel backend caps per-attribute size should set `singularity.chat.chat.otel.maxAttributeSizeChars` (or the `SINGULARITY_OTEL_MAX_ATTRIBUTE_SIZE_CHARS` env var) to a positive value so OTLP batches stay under the backend cap — consult the backend's documentation for the appropriate value.
...
| `singularity_chat.*` | Extension-specific (legacy; several keys dual-emit alongside `singularity.chat.*`) | `singularity_chat.session_id`, `singularity_chat.chat_session_id` |
| `singularity.chat.*` | Canonical Singularity namespace — extension-emitted enrichment (foreground agent, Claude agent, CLI bridge) + CLI SDK internal metrics | `singularity.chat.agent.type`, `singularity.chat.git.repository`, `singularity.chat.tool.parameters.edit_type`, `singularity.chat.hook.decision`, `singularity.chat.cost`, `singularity.chat.aiu` |
...
For **extension-emitted** spans (`invoke_agent`, `chat`, `execute_tool`, `execute_hook` produced by the foreground agent, Claude agent, BYOK providers, and `chatMLFetcher`), `gen_ai.conversation.id` carries the **VS Code chat session id** on every exported span, so a vendor-agnostic OTLP backend can group all spans of a session on the standard GenAI key without a traceId → root join. The vendor keys `singularity_chat.session_id` / `singularity_chat.chat_session_id` are dual-emitted for the Agent Debug Log and SQLite span store (`COALESCE(conversation_id, chat_session_id)`).
...
- BYOK `chat` spans (`anthropicProvider.ts`, `geminiNativeProvider.ts`) read the session id from the in-scope `CapturingToken.chatSessionId` and emit `gen_ai.conversation.id` + `singularity_chat.session_id` + `singularity_chat.chat_session_id`.
- `embeddings` spans are intentionally not session-scoped and carry no session id.

### Singularity CLI in-process (SDK-native spans)

These spans are produced by the Singularity runtime SDK, not the extension, so there are **two independent export paths** with different correlation coverage:

- **Debug panel copy** (via the bridge → `injectCompletedSpan` → `onDidCompleteSpan`): `singularityCliBridgeSpanProcessor.ts` injects `singularity_chat.chat_session_id` **and** `gen_ai.conversation.id` (session id) onto the copied span for any span that lacks them, keyed by a traceId → session map. This affects only the debug-panel / file-logger view.
- **SDK's own OTLP export** (via the runtime's `BatchSpanProcessor`): the bridge **cannot** mutate these spans. Their `gen_ai.conversation.id` is whatever the runtime stamps — currently `invoke_agent` and `chat` (and `execute_tool` once [github/singularity-agent-runtime#12383](https://github.com/github/singularity-agent-runtime/pull/12383) lands). Closing this gap for other CLI child spans must happen in the runtime.
...
src/extension/chatSessions/singularitycli/node/test/
└── singularityCliBridgeSpanProcessor.spec.ts   # Bridge processor tests
