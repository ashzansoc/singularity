...
export class GitHubAuthenticationProvider implements vscode.AuthenticationProvider, vscode.Disposable {
	...
	private readonly _loginInFlight = new Map<string, Promise<vscode.AuthenticationSession>>();
	...
	public async createSession(scopes: string[], options?: GitHubAuthenticationProviderOptions): Promise<vscode.AuthenticationSession> {
		...
		const existing = sessions.find(s =>
			...
			&& (!loginWith || s.account.label === loginWith)
		);
		if (existing) {
			this._logger.info(`Reusing existing GitHub session for scopes: ${scopeKey}`);
			return existing;
		}
		const inflight = this._loginInFlight.get(scopeKey);
		if (inflight) {
			this._logger.info(`Joining in-flight GitHub login for scopes: ${scopeKey}`);
			return inflight;
		}
		const login = this._createSessionInner(scopes, sortedScopes, options).finally(() => {
			this._loginInFlight.delete(scopeKey);
		});
		this._loginInFlight.set(scopeKey, login);
		return login;
	}

	private async _createSessionInner(scopes: string[], sortedScopes: string[], options?: GitHubAuthenticationProviderOptions): Promise<vscode.AuthenticationSession> {
		try {
